...
| `TestFieldDeclarations`, `TestIndex`, `OrphanLabelLinter`, `TestBundleTokenDefs` | Floors are zero and pass at the narrow scope over the little installed there; point_of_sale's dead lock-date constraint read 0 narrow and 4 with point_of_sale installed. |

The fuller-install grading is `./gates.sh --lint-full` (`odoo/addons/test_lint/lint_full_scope.sh`): installs a named module set into a scratch database, repeats `-i` until nothing more goes missing, runs `/test_lint`, drops the database.
