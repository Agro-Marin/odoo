...
version_info = ...
nt_service_name = ...
exec((ROOTDIR / "odoo" / "release.py").open("rb").read())
...
def run_cmd(
    cmd: list[str], chdir: str | None = None, timeout: float | None = None
...
def _rpc_count_modules(
    addr: str = "http://127.0.0.1", port: int = 8069, dbname: str = "mycompany"
) -> None:
    ...
    if len(modules) > 1:
        toinstallmodules = xmlrpclib.ServerProxy(
            "%s:%s/xmlrpc/2/object" % (addr, port)
        ).execute(
            dbname,
            uid,
            "admin",
        ...
        if toinstallmodules:
            ...
            msg = "Installation of package failed"
            raise OdooTestError(msg)
        _logger.info("Package test: successfuly installed %s modules", len(modules))
    else:
        _logger.error("Package test: FAILED. Not able to install base.")
        msg = "Package test: FAILED. Not able to install base."
        raise OdooTestError(msg)


def publish(
    args: argparse.Namespace, pub_type: str, extensions: list[str]
) -> list[str]:
    ...
    def _publish(release: str) -> str:
        build_path = Path(args.build_dir) / release
        filename = release.rsplit(os.path.sep, maxsplit=1)[-1]
        release_dir = Path(args.pub) / pub_type
...
# ---------------------------------------------------------
def gen_deb_package(args: argparse.Namespace, published_files: list[str]) -> None:
    # Executes command to produce file_name in path, and moves it to args.pub/deb
    def _gen_file(
        args: argparse.Namespace, command: list[str], file_name: str, path: str
    ) -> None:
        cur_tmp_file_path = Path(path) / file_name
        with cur_tmp_file_path.open("w") as out:
            subprocess.call(command, stdout=out, cwd=path)
        shutil.copy(cur_tmp_file_path, Path(args.pub) / "deb" / file_name)
...
# ---------------------------------------------------------
def rpm_sign(args: argparse.Namespace, file_name: str) -> None:
    """Genereate a rpm repo in publish directory"""
    # Sign the RPM
    rpmsign = pexpect.spawn(
        "/bin/bash",
        ["-c", "rpm --resign %s" % file_name],
        cwd=Path(args.pub) / "rpm",
    )
...
def _prepare_build_dir(
    args: argparse.Namespace, win32: bool = False, move_addons: bool = True
) -> None:
    """Copy files to the build directory"""
    _logger.info('Preparing build dir "%s"', args.build_dir)
    cmd = [
        "rsync",
        "-a",
        "--delete",
    ...
    if not move_addons:
        return
    for addon_path in glob(str(Path(args.build_dir) / "addons" / "*")):
        if args.blacklist is None or Path(addon_path).name not in args.blacklist:
            try:
                shutil.move(addon_path, Path(args.build_dir) / "odoo" / "addons")
            except shutil.Error as e:
                _logger.warning("Warning '%s' while moving addon '%s", e, addon_path)
                if addon_path.startswith(args.build_dir) and Path(addon_path).is_dir():
                    _logger.info("Removing '%s'", addon_path)
                    try:
                        shutil.rmtree(addon_path)
                    except shutil.Error as rm_error:
                        _logger.warning("Cannot remove '%s': %s", addon_path, rm_error)
...
class Docker:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        docker_templates = {
            "tgz": Path(args.build_dir) / "setup/package.dfsrc",
            "deb": Path(args.build_dir) / "setup/package.dfdebian",
            "rpm": Path(args.build_dir) / "setup/package.dffedora",
            "win": Path(args.build_dir) / "setup/package.dfwine",
        }
        self.docker_template = (
            Path(docker_templates[self.arch])
    ...
    def build_image(self) -> None:
        ...
        shutil.copy(Path(self.args.build_dir) / "requirements.txt", self.docker_dir)
        run_cmd(
    ...
    def test_odoo(self) -> None:
        _logger.info("Starting to test Odoo install test")
        ...
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            ...
            if (Path(args.build_dir) / "odoo.pid").exists():
                try:
        ...
        if self.is_running():
            self.stop()
            raise OdooTestTimeoutError(
                "Odoo pid file never appeared after %s sec" % INSTALL_TIMEOUT
            )
        raise OdooTestError(
...
class DockerTgz(Docker):
    ...
    def build(self) -> None:
        _logger.info("Start building python tgz package")
        self.run(
            "python3 setup.py sdist --quiet --formats=gztar,zip",
            self.args.build_dir,
            "odoo-src-build-%s" % TSTAMP,
        )
        Path(glob("%s/dist/odoo-*.tar.gz" % self.args.build_dir)[0]).rename(
            "%s/odoo_%s.%s.tar.gz" % (self.args.build_dir, VERSION, TSTAMP)
        )
        Path(glob("%s/dist/odoo-*.zip" % self.args.build_dir)[0]).rename(
            "%s/odoo_%s.%s.zip" % (self.args.build_dir, VERSION, TSTAMP)
        )
        _logger.info("Finished building python tgz package")

    def start_test(self) -> None:
        if not self.args.test:
            return
        _logger.info("Start testing python tgz package")
        cmds = [
        ...
        self.test_odoo()
        _logger.info("Finished testing tgz package")
...
class DockerDeb(Docker):
    ...
    def build(self) -> None:
        _logger.info("Start building debian package")
        # Append timestamp to version for the .dsc to refer the right .tar.gz
        cmds = [
            "sed -i '1s/^.*$/odoo (%s.%s) stable; urgency=low/' debian/changelog"
            % (VERSION, TSTAMP)
        ]
        ...
        _logger.info("Finished building debian package")

    def start_test(self) -> None:
        if not self.args.test:
            return
        _logger.info("Start testing debian package")
        cmds = [
            "service postgresql start",
        ...
        self.test_odoo()
        _logger.info("Finished testing debian package")
...
class DockerRpm(Docker):
    ...
    def build(self) -> None:
        _logger.info("Start building fedora rpm package")
        ...
        Path(glob("%s/dist/odoo-*.noarch.rpm" % self.args.build_dir)[0]).rename(
            "%s/odoo_%s.%s.rpm" % (self.args.build_dir, VERSION, TSTAMP)
        )
        _logger.info("Finished building fedora rpm package")

    def start_test(self) -> None:
        if not self.args.test:
            return
        _logger.info("Start testing rpm package")
        cmds = [
        ...
        self.test_odoo()
        _logger.info("Finished testing rpm package")

    def gen_rpm_repo(self, args: argparse.Namespace, rpm_filepath: str) -> None:
        pub_repodata_path = Path(args.pub) / "rpm" / "repodata"
        # Removes the old repodata
        if pub_repodata_path.is_dir():
        ...
        _logger.info("Start creating rpm repo")
        self.run("createrepo /data/src/", temp_path, "odoo-rpm-createrepo-%s" % TSTAMP)
        shutil.copytree(Path(temp_path) / "repodata", pub_repodata_path)
...
class DockerWine(Docker):
    ...
    def build(self) -> None:
        _logger.info("Start building %s package", self.package_name)
        ...
        _logger.info("Finished building %s package", self.package_name)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "-t", "--test", action="store_true", default=False, help="Test built packages"
    )
    ap.add_argument(
        "-s",
        "--sign",
    ...
    try:
        if args.build_tgz:
            ...
            try:
                ...
                _logger.error("Won't publish the tgz release.\n Exception: %s", e)
        if args.build_rpm:
            _prepare_build_dir(args)
            ...
            try:
                ...
                if args.sign:
                    _logger.info("Signing rpm package")
                    rpm_sign(args, published_files[0])
                    _logger.info("Generate rpm repo")
                    docker_rpm.gen_rpm_repo(args, published_files[0])
            except Exception as e:
                _logger.error("Won't publish the rpm release.\n Exception: %s", e)
        if args.build_deb:
            ...
            try:
                ...
                _logger.error("Won't publish the deb release.\n Exception: %s", e)
        if args.build_win:
            ...
            try:
                ...
                _logger.error("Won't publish the exe release.\n Exception: %s", e)
        if args.build_iot:
            _prepare_build_dir(args, win32=True)
            ...
            try:
                ...
                _logger.error("Won't publish the iot release.\n Exception: %s", e)
    except Exception as e:
        _logger.error("Something bad happened ! : %s", e)
        traceback.print_exc()
    finally:
        if args.no_remove:
            _logger.info('Build dir "%s" not removed', args.build_dir)
        elif Path(args.build_dir).exists():
            shutil.rmtree(args.build_dir)
            _logger.info("Build dir %s removed", args.build_dir)


if __name__ == "__main__":
    args = parse_args()
    if Path(args.build_dir).exists():
        _logger.error('Build dir "%s" already exists.', args.build_dir)
        sys.exit(1)
    main(args)
