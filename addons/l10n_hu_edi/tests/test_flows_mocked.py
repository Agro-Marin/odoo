...
class L10nHuEdiTestFlowsMocked(L10nHuEdiTestCommon, TestAccountMoveSendCommon):
    ...
    def test_send_invoice_and_credit_note(self):
        with self.patch_post(), freeze_time("2024-01-25T15:28:53Z"):
            invoice = self.create_invoice_simple()
            invoice.action_post()
            send_and_print = self.create_send_and_print(invoice, sending_methods=[])
            self.assertTrue(
                send_and_print.extra_edi_checkboxes
                and send_and_print.extra_edi_checkboxes.get("hu_nav_30", {}).get(
                    "checked"
                )
            )
            ...
            self.assertRecordValues(
                invoice,
                [{"l10n_hu_edi_state": "confirmed", "l10n_hu_invoice_chain_index": -1}],
            )
    ...
    def test_send_invoice_warning(self):
        with tools.file_open(
            "l10n_hu_edi/tests/mocked_requests/queryTransactionStatus_response_warning.xml",
            "r",
        ) as response_file:
    ...
    def test_send_invoice_error(self):
        with tools.file_open(
            "l10n_hu_edi/tests/mocked_requests/queryTransactionStatus_response_error.xml",
            "r",
        ) as response_file:
            response_data = response_file.read()
        with (
            ...
            self.assertRecordValues(
                invoice,
                [{"l10n_hu_edi_state": "rejected", "l10n_hu_invoice_chain_index": 0}],
            )
    ...
    def test_timeout_recovery_fail(self):
        ...
        with tools.file_open(
            "l10n_hu_edi/tests/mocked_requests/queryTransactionStatus_response_original.xml",
            "r",
        ) as response_file:
            response_data = response_file.read()
        # Advance 10 minutes so the timeout recovery mechanism triggers.
        with (
            freeze_time("2024-01-25T15:38:53Z"),
            self.patch_post({"queryTransactionStatus": response_data}),
        ):
            with contextlib.suppress(UserError):
    ...
    def test_cancel_invoice_pending(self):
        with freeze_time("2024-01-25T15:28:53Z"):
            with self.patch_post():
                ...
                self.assertRecordValues(invoice, [{"l10n_hu_edi_state": "confirmed"}])
            with tools.file_open(
                "l10n_hu_edi/tests/mocked_requests/queryTransactionStatus_response_annulment_pending.xml",
                "r",
            ) as response_file:
                response_data = response_file.read()
            with self.patch_post({"queryTransactionStatus": response_data}):
    ...
    def test_cancel_invoice_done(self):
        with freeze_time("2024-01-25T15:28:53Z"):
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/mocked_requests/queryTransactionStatus_response_annulment_done.xml",
                "r",
            ) as response_file:
                response_data = response_file.read()
            with self.patch_post({"queryTransactionStatus": response_data}):
    ...
    def test_cancel_and_resend(self):
        ...
        with freeze_time("2024-01-25T15:28:53Z"):
            with self.patch_post():
                ...
                send_and_print = self.create_send_and_print(
                    credit_note, sending_methods=[]
                )
                self.assertTrue(
                    send_and_print.extra_edi_checkboxes
                    and send_and_print.extra_edi_checkboxes.get("hu_nav_30", {}).get(
    ...
    def patch_post(self, responses=None):
        ...
        class MockedSession:
            def post(self, url, data, headers, timeout=None):
                ...
                if (
                    base_url not in (prod_url, demo_url)
                    or service not in mocked_requests
                ):
                    test_case.fail(f"Invalid POST url: {url}")

                with tools.file_open(
                    f"l10n_hu_edi/tests/mocked_requests/{service}_request.xml", "rb"
                ) as expected_request_file:
                    test_case.assertXmlTreeEqual(
                        test_case.get_xml_tree_from_string(data),
                        test_case.get_xml_tree_from_string(
                            expected_request_file.read()
                        ),
                    )
                ...
                if responses and service in responses:
                    ...
                    with tools.file_open(
                        f"l10n_hu_edi/tests/mocked_requests/{service}_response.xml", "r"
                    ) as response_file:
