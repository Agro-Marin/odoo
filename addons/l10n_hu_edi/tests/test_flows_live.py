...
class L10nHuEdiTestFlowsLive(L10nHuEdiTestCommon, TestAccountMoveSendCommon):
    ...
    def test_send_invoice_and_credit_note(self):
        ...
        with self.set_invoice_name(credit_note, "RINV/2024/"):
            ...
            cancel_wizard = (
                self.env["l10n_hu_edi.cancellation"]
                .with_context({"default_invoice_id": credit_note.id})
                .create(
                    {
                        "code": "ERRATIC_DATA",
                        "reason": "Some reason...",
    ...
    def test_timeout_recovery_fail(self):
        ...
        self.assertTrue(
            send_and_print.extra_edi_checkboxes
            and send_and_print.extra_edi_checkboxes.get("hu_nav_30", {}).get("checked")
        )
        self.assertFalse(invoice._l10n_hu_edi_check_invoices())
        with (
            self.patch_call_nav_endpoint("manageInvoice", make_request=False),
        ...
        self.assertRecordValues(
            invoice,
            [{"l10n_hu_edi_state": "send_timeout", "l10n_hu_invoice_chain_index": -1}],
        )
    ...
    def test_timeout_recovery_success(self):
        ...
        with self.set_invoice_name(invoice, "INV/2024/"):
            ...
            self.assertFalse(invoice._l10n_hu_edi_check_invoices())
            with (
                self.patch_call_nav_endpoint("manageInvoice"),
                contextlib.suppress(UserError),
            ):
                send_and_print.action_send_and_print()
    ...
    def set_invoice_name(self, invoice, prefix):
        try:
            ...
            if invoice.l10n_hu_edi_state not in [
                "confirmed",
                "confirmed_warning",
                "cancel_sent",
                "cancel_pending",
                ...
                with tools.file_open(
                    "l10n_hu_edi/tests/credentials.py", "a"
                ) as credentials_file:
        ...
        def mock_call_nav_endpoint(self, mode, service, data, timeout=20):
            if service == endpoint:
                ...
                raise L10nHuEdiConnectionError(
                    "Freeze! This is a timeout!", code="timeout"
                )
            return real_call_nav_endpoint(self, mode, service, data, timeout=timeout)

        with mock.patch.object(
            L10nHuEdiConnection, "_call_nav_endpoint", new=mock_call_nav_endpoint
        ):
            yield
