...
class L10nHuEdiTestInvoiceXml(L10nHuEdiTestCommon):
    ...
    def test_invoice_and_credit_note(self):
        with freeze_time("2024-02-01"):
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/invoice_xmls/invoice_simple.xml", "rb"
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/invoice_xmls/credit_note.xml", "rb"
            ) as expected_xml_file:
                self.assertXmlTreeEqual(
    ...
    def test_invoice_complex_huf(self):
        with freeze_time("2024-02-01"):
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/invoice_xmls/invoice_complex_huf.xml", "rb"
            ) as expected_xml_file:
                self.assertXmlTreeEqual(
                    self.get_xml_tree_from_string(invoice_xml),
                    self.get_xml_tree_from_string(expected_xml_file.read()),
    ...
    def test_invoice_complex_eur(self):
        with freeze_time("2024-02-01"):
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/invoice_xmls/invoice_complex_eur.xml", "rb"
            ) as expected_xml_file:
                self.assertXmlTreeEqual(
                    self.get_xml_tree_from_string(invoice_xml),
                    self.get_xml_tree_from_string(expected_xml_file.read()),
    ...
    def test_advance_invoice(self):
        ...
        with freeze_time("2024-01-01"):
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/invoice_xmls/invoice_advance.xml", "rb"
            ) as expected_xml_file:
                self.assertXmlTreeEqual(
        ...
        with freeze_time("2024-01-15"):
            self.env["account.payment.register"].with_context(
                active_ids=advance_invoice.ids, active_model="account.move"
        ...
        with freeze_time("2024-02-01"):
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/invoice_xmls/invoice_final.xml", "rb"
            ) as expected_xml_file:
                self.assertXmlTreeEqual(
    ...
    def test_multi_currency_tax_sign(self):
        ...
        expected_value = tools.float_round(
            10000 * self.tax_vat.amount / (100 * currency_eur.rate),
            self.currency.decimal_places,
        )

        self.assertEqual(
            out_invoice._l10n_hu_get_invoice_totals_for_report()[
                "total_vat_amount_in_huf"
            ],
            expected_value,
        )
        self.assertEqual(
            in_invoice._l10n_hu_get_invoice_totals_for_report()[
                "total_vat_amount_in_huf"
            ],
            expected_value,
        )
        self.assertEqual(
            out_refund._l10n_hu_get_invoice_totals_for_report()[
                "total_vat_amount_in_huf"
            ],
            -expected_value,
        )
        self.assertEqual(
            in_refund._l10n_hu_get_invoice_totals_for_report()[
                "total_vat_amount_in_huf"
            ],
            -expected_value,
        )
    ...
    def test_invoice_simple_deduction(self):
        with freeze_time("2024-02-01"):
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/invoice_xmls/invoice_simple_discount.xml", "rb"
            ) as expected_xml_file:
                self.assertXmlTreeEqual(
                    self.get_xml_tree_from_string(invoice_xml),
                    self.get_xml_tree_from_string(expected_xml_file.read()),
    ...
    def test_invoice_tax_price_include(self):
        with freeze_time("2024-02-01"):
            ...
            with tools.file_open(
                "l10n_hu_edi/tests/invoice_xmls/invoice_tax_price_include.xml", "rb"
            ) as expected_xml_file:
                self.assertXmlTreeEqual(
                    self.get_xml_tree_from_string(invoice_xml),
                    self.get_xml_tree_from_string(expected_xml_file.read()),
