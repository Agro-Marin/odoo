...
class L10n_Hu_EdiCancellation(models.TransientModel):
    ...
    def button_request_cancel(self):
        with L10nHuEdiConnection(self.env) as connection:
            ...
            self.invoice_id._l10n_hu_edi_request_cancel(
                connection, self.code, self.reason
            )
        ...
        formatted_message = self.env["mixin.account.move.send"]._format_error_html(
            self.invoice_id.l10n_hu_edi_messages
        )
        ...
        if self.invoice_id.l10n_hu_edi_messages.get("blocking_level") == "error":
            raise UserError(
                self.env["mixin.account.move.send"]._format_error_text(
                    self.invoice_id.l10n_hu_edi_messages
                )
            )
