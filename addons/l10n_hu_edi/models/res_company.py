...
class ResCompany(models.Model):
    ...
    def _compute_l10n_hu_edi_config_id(self):
        self._compute_config_link("l10n_hu_edi_config_id")
    ...
    def _l10n_hu_edi_configure_company(self):
        ...
        for company in self:
            ...
            profit_account = (
                ...
                .ref("l10n_hu_969", raise_if_not_found=False)
            )
            loss_account = (
                self.env["account.chart.template"]
                .with_company(company)
                .ref("l10n_hu_869", raise_if_not_found=False)
            )
            rounding_method = self.env.ref(
                "l10n_hu_edi.cash_rounding_1_huf", raise_if_not_found=False
            )
    ...
    def _l10n_hu_edi_test_credentials(self):
        with L10nHuEdiConnection(self.env) as connection:
            for company in self:
                ...
                try:
                    connection.do_token_exchange(
                        company._l10n_hu_edi_get_credentials_dict()
                    )
        ...
        for company in self:
            ...
            for transaction in islice(transactions_to_query, 10):
                ...
                for processing_result in results["processing_results"]:
                    invoice_name = processing_result["original_xml"].findtext(
                        "data:invoiceNumber", namespaces=XML_NAMESPACES
                    )
                    canonicalized_attachment = etree.canonicalize(
                        processing_result["original_file"]
                    )
                    annulment_invoice_name = processing_result["original_xml"].findtext(
                    ...
                    if matched_invoice:
                        ...
                        matched_invoice.l10n_hu_edi_transaction_code = transaction[
                            "transaction_code"
                        ]
                        matched_invoice._l10n_hu_edi_process_query_transaction_result(
