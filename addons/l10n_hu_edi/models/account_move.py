...
class AccountMove(models.Model):
    ...
    def _check_posted_if_active(self):
        ...
        for move in self:
            if move.state in ["draft", "cancel"] and move.l10n_hu_edi_state not in [
                False,
                "rejected",
                "cancelled",
            ]:
    ...
    def _compute_message_html(self):
        for move in self:
            if move.l10n_hu_edi_messages:
                move.l10n_hu_edi_message_html = self.env[
                    "mixin.account.move.send"
                ]._format_error_html(move.l10n_hu_edi_messages)
            else:
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        self.filtered(
            lambda m: m.l10n_hu_edi_state not in [False, "rejected", "cancelled"]
    ...
    def _compute_l10n_hu_edi_attachment_filename(self):
        for move in self:
            move.l10n_hu_edi_attachment_filename = (
                f"{move.name.replace('/', '_')}.xml" if move.name else "nav30.xml"
            )
    ...
    def _inverse_delivery_date(self):
        ...
        self._conditional_add_to_compute(
            "invoice_currency_rate", lambda m: m.country_code == "HU"
        )
    ...
    def l10n_hu_edi_button_update_status(self, from_cron=False):
        ...
        invoices_to_query = self.filtered(
            lambda m: "query_status" in m._l10n_hu_edi_get_valid_actions()
        )
        ...
        with L10nHuEdiConnection(self.env) as connection:
            ...
            if any(
                ...
                recover_transactions_error = (
                    self.company_id._l10n_hu_edi_recover_transactions(connection)
                )

        # Error handling.
        for invoice in invoices_to_query:
            # Log invoice status in chatter.
            formatted_message = self.env["mixin.account.move.send"]._format_error_html(
                invoice.l10n_hu_edi_messages
            )
        ...
        for invoice in invoices_to_query:
            if (
                invoice.l10n_hu_edi_messages.get("blocking_level") == "error"
                or recover_transactions_error
            ):
                if invoice.l10n_hu_edi_messages.get("blocking_level") == "error":
                    error_text = self.env["mixin.account.move.send"]._format_error_text(
                        invoice.l10n_hu_edi_messages
                    )
                else:
                    error_text = self.env["mixin.account.move.send"]._format_error_text(
    ...
    def _l10n_hu_get_chain_base(self):
        ...
        while modification_invoices:
            base_invoices |= modification_invoices.filtered(
                lambda m: not m.reversed_entry_id and not m.debit_origin_id
            )
            modification_invoices = (
                modification_invoices.reversed_entry_id
                | modification_invoices.debit_origin_id
            )
    ...
    def _l10n_hu_get_chain_invoices(self):
        ...
        next_invoices = self
        while (
            next_invoices := next_invoices.reversal_move_ids
            | next_invoices.debit_note_ids
        ):
    ...
    def _l10n_hu_edi_set_chain_index(self):
        ...
        if base_invoice == self:
            self.l10n_hu_invoice_chain_index = (
                -1
            )  # -1 indicates a base invoice (0 indicates the chain index was not set).
        else:
            # Lock base invoice to prevent concurrent updates, ensuring sequence integrity.
            base_invoice._l10n_hu_edi_acquire_lock()

            chain_indexes_already_sent = (
                base_invoice._l10n_hu_get_chain_invoices()
                .filtered(
    ...
    def _l10n_hu_edi_check_invoices(self):
        ...
        hu_bank_account_regex = re.compile(
            r"\d{8}-\d{8}-\d{8}|\d{8}-\d{8}|[A-Z]{2}\d{2}[0-9A-Za-z]{11,30}"
        )

        # This contains all the advance invoices that correspond to final invoices in `self`.
        advance_invoices = (
            self.filtered(lambda m: not m._is_downpayment())
            .invoice_line_ids._get_downpayment_lines()
            .mapped("move_id")
        )
        ...
        checks = {
            ...
            "company_vat_invalid": {
                "records": self.company_id.filtered(
                    lambda c: (
                        ...
                        or (
                            c.l10n_hu_group_vat
                            and not hu_vat_regex.fullmatch(c.l10n_hu_group_vat)
                        )
                    )
                ),
                "message": _(
            ...
            "partner_vat_invalid": {
                "records": self.partner_id.commercial_partner_id.filtered(
                    lambda p: (
                        p.is_company
                        and p.country_code == "HU"
                        and (
                            (p.vat and not hu_vat_regex.fullmatch(p.vat))
                            or (
                                p.l10n_hu_group_vat
                                and not hu_vat_regex.fullmatch(p.l10n_hu_group_vat)
                            )
                        )
                    )
                ),
            ...
            "partner_address_missing": {
                "records": self.partner_id.commercial_partner_id.filtered(
                    lambda p: (
                        p.is_company
                        and (
                            not p.country_id or not p.zip or not p.city or not p.street
                        )
                    ),
                ),
            ...
            "invoice_advance_not_paid": {
                "records": advance_invoices.filtered(
                    lambda m: (
                        ...
                        or (
                            m.l10n_hu_edi_state in [False, "rejected", "cancelled"]
                            and m not in self
        ...
        errors = {
            f"l10n_hu_edi_{check}": {
                ...
                "action": values["records"]._get_records_action(
                    name=values["action_text"]
                ),
            }
    ...
    def _l10n_hu_edi_upload(self, connection):
        ...
        for invoice in invoices_sorted:
            ...
            # Generate XML
            invoice.l10n_hu_edi_attachment = base64.b64encode(
                invoice._l10n_hu_edi_generate_xml()
            )
    ...
    def _l10n_hu_edi_get_operation_type(self):
        ...
        all_invoices_residual_zero = all(
            invoice.amount_residual == 0 for invoice in modification_invoices
        )
    ...
    def _l10n_hu_edi_upload_single_batch(self, connection):
        try:
            token_result = connection.do_token_exchange(
    ...
    def _l10n_hu_edi_query_status(self, connection):
        ...
        invoices = self | self.search(
            [
                ...
                (
                    ...
                    "in",
                    self.mapped("l10n_hu_edi_transaction_code"),
                ),
        ...
        for __, invoices_grouped in groupby(
            invoices, lambda m: (m.company_id, m.l10n_hu_edi_transaction_code)
        ):
            self.env["account.move"].union(
                *invoices_grouped
            )._l10n_hu_edi_query_status_single_batch(connection)
    ...
    def _l10n_hu_edi_process_query_transaction_result(
        ...
        def get_errors_from_processing_result(processing_result):
            return [
                ...
                for message in processing_result.get("business_validation_messages", [])
                + processing_result.get("technical_validation_messages", [])
            ]
        ...
        if processing_result["invoice_status"] in ["RECEIVED", "PROCESSING", "SAVED"]:
            ...
            if self.l10n_hu_edi_state in ["sent", "send_timeout"]:
                if (
                    not processing_result["business_validation_messages"]
                    and not processing_result["technical_validation_messages"]
                ):
                    self.write(
                        {
            ...
            elif self.l10n_hu_edi_state in [
                "cancel_sent",
                "cancel_timeout",
                "cancel_pending",
            ]:
                if annulment_status == "NOT_VERIFIABLE":
                    ...
                    to_cancel = (
                        self
                        if self.reversed_entry_id or self.debit_origin_id
                        else self._l10n_hu_get_chain_invoices().filtered(
    ...
    def _l10n_hu_edi_generate_xml(self):
        ...
        return etree.tostring(
            cleanup_xml_node(invoice_data, remove_blank_nodes=False),
    ...
    def _l10n_hu_edi_get_invoice_values(self):
        ...
        def get_vat_data(partner, force_vat=None):
            if partner.country_code == "HU" or force_vat:
                return {
                    "tax_number": partner.l10n_hu_group_vat
                    or (force_vat or partner.vat),
                    "group_member_tax_number": partner.l10n_hu_group_vat
                    and (force_vat or partner.vat),
                }
        ...
        invoice_values = {
            ...
            "supplier": supplier,
            "supplier_vat_data": get_vat_data(
            ...
            or "OTHER",
            "customer_vat_data": get_vat_data(customer)
        ...
        prev_chain_invoices = base_invoice._l10n_hu_get_chain_invoices().filtered(
            lambda m: (
                m.l10n_hu_invoice_chain_index
                and m.l10n_hu_invoice_chain_index < self.l10n_hu_invoice_chain_index
            )
        )
        first_line_number = (
            sum(
                len(
                    move.line_ids.filtered(
                        lambda l: l.display_type in ["product", "rounding"]
                    )
                )
        ...
        for line_number, line in enumerate(
            self.line_ids.filtered(
                lambda l: l.display_type in ["product", "rounding"]
            ).sorted(lambda l: l.display_type),
            ...
            line_values = {
                ...
                "lineNatureIndicator": {False: "OTHER", "service": "SERVICE"}.get(
            ...
            if "is_downpayment" in line and line.is_downpayment:
                ...
                if not self._is_downpayment():
                    ...
                    last_reconciled_payment = reconciled_moves.filtered(
                        lambda m: m.origin_payment_id or m.statement_line_id
            ...
            if line.display_type == "product":
                ...
                discount_value_signed = self.currency_id.round(
                    price_unit_signed * line.quantity - price_net_signed
                )
                price_total_signed = sign * line.price_total
                vat_amount_signed = self.currency_id.round(
                    price_total_signed - price_net_signed
                )
                ...
                line_values.update(
                    {
                        ...
                        "unitPriceHUF": currency_huf.round(
                            price_unit_signed * currency_rate
                        ),
                        "discountValue": discount_value_signed,
                        "discountRate": line.discount / 100.0,
                        "lineNetAmount": price_net_signed,
                        "lineNetAmountHUF": currency_huf.round(
                            price_net_signed * currency_rate
                        ),
                        "lineVatData": not self.currency_id.is_zero(vat_amount_signed),
                        "lineVatAmount": vat_amount_signed,
                        "lineVatAmountHUF": currency_huf.round(
                            vat_amount_signed * currency_rate
                        ),
                        "lineGrossAmountNormal": price_total_signed,
                        "lineGrossAmountNormalHUF": currency_huf.round(
                            price_total_signed * currency_rate
                        ),
                    }
                )
                ...
                amount_huf = (
                    ...
                    else currency_huf.round(line.amount_currency * currency_rate)
                )
                line_values.update(
                    {
                        "vat_tax": atk_tax,
            ...
            line_values["lineDescription"] = (
                line_values["lineDescription"] or line.product_id.display_name
            )
        ...
        tax_amounts_by_tax = {
            line.tax_line_id: {
                ...
                "vatRateVatAmountHUF": -line.balance
                if is_company_huf
                else currency_huf.round(-line.amount_currency * currency_rate),
            }
        ...
        invoice_values["tax_summary"] = [
            {
                "vat_tax": vat_tax,
                "vatPercentage": float_round(vat_tax.amount / 100.0, 4),
                "vatRateNetAmount": self.currency_id.round(
                    sum(l["lineNetAmount"] for l in lines_values_by_tax)
                ),
                "vatRateNetAmountHUF": currency_huf.round(
                    sum(l["lineNetAmountHUF"] for l in lines_values_by_tax)
                ),
                "vatRateVatAmount": tax_amounts_by_tax.get(vat_tax, {}).get(
                    "vatRateVatAmount", 0.0
                ),
                "vatRateVatAmountHUF": tax_amounts_by_tax.get(vat_tax, {}).get(
                    "vatRateVatAmountHUF", 0.0
                ),
            }
            for vat_tax, lines_values_by_tax in groupby(
                invoice_values["lines_values"], lambda l: l["vat_tax"]
            )
        ]

        total_vat = self.currency_id.round(
            sum(
                tax_vals["vatRateVatAmount"]
                for tax_vals in invoice_values["tax_summary"]
            )
        )
        total_vat_huf = currency_huf.round(
            sum(
                tax_vals["vatRateVatAmountHUF"]
                for tax_vals in invoice_values["tax_summary"]
            )
        )
    ...
    def _get_name_invoice_report(self):
        self.check_singleton()
        return (
            "l10n_hu_edi.report_invoice_document"
            if self.country_code == "HU"
            else super()._get_name_invoice_report()
        )
