...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        enabled_moves = moves.filtered(
            lambda m: "upload" in m._l10n_hu_edi_get_valid_actions()
        )._origin
    ...
    def _l10n_hu_edi_cron_update_status(self):
        ...
        invoices_pending = self.env["account.move"].search(
            [("l10n_hu_edi_state", "not in", final_states)]
        )
        ...
        if any(m.state not in final_states for m in invoices_pending):
            ...
            self.env.ref("l10n_hu_edi.ir_cron_update_status")._trigger(
    ...
    def _call_web_service_before_invoice_pdf_render(self, invoices_data):
        ...
        invoices_to_upload = invoices_hu.filtered(
            lambda m: "upload" in m._l10n_hu_edi_get_valid_actions()
        )
        ...
        invoices_to_upload.invalidate_recordset(
        ...
        with L10nHuEdiConnection(self.env) as connection:
            ...
            )._l10n_hu_edi_query_status(connection)

        # STEP 3: Schedule update status of pending invoices in 10 minutes.
        if any(
            m.l10n_hu_edi_state
            not in [False, "confirmed", "confirmed_warning", "rejected"]
            for m in invoices_hu
        ):
            self.env.ref("l10n_hu_edi.ir_cron_update_status")._trigger(
                at=fields.Datetime.now() + timedelta(minutes=10)
            )
