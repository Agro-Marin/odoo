...
class L10nHuEdiConnection:
    ...
    def do_token_exchange(self, credentials):
        ...
        request_data = self.env["ir.qweb"]._render(
            "l10n_hu_edi.token_exchange_request", template_values
        )
        request_data = etree.tostring(
            cleanup_xml_node(request_data, remove_blank_nodes=False),
            xml_declaration=True,
            encoding="UTF-8",
        )

        response_xml = self._call_nav_endpoint(
            credentials["mode"], "tokenExchange", request_data
        )
        self._parse_error_response(response_xml)

        encrypted_token = response_xml.findtext(
            "api:encodedExchangeToken", namespaces=XML_NAMESPACES
        )
        token_validity_to = response_xml.findtext(
            "api:tokenValidityTo", namespaces=XML_NAMESPACES
        )
        try:
            # Convert into a naive UTC datetime, since Odoo can't store timezone-aware datetimes
            token_validity_to = (
                dateutil.parser.isoparse(token_validity_to)
                .astimezone(UTC)
                .replace(tzinfo=None)
            )
        except ValueError:
        ...
        try:
            token = decrypt_aes128(
                credentials["replacement_key"].encode(),
                b64decode(encrypted_token.encode()),
            ).decode()
    ...
    def do_manage_invoice(self, credentials, token, invoice_operations):
        ...
        for invoice_operation in invoice_operations:
            invoice_data_b64 = b64encode(invoice_operation["invoice_data"]).decode(
                "utf-8"
            )
        ...
        template_values.update(
            self._get_header_values(credentials, invoice_hashs=invoice_hashes)
        )

        request_data = self.env["ir.qweb"]._render(
            "l10n_hu_edi.manage_invoice_request", template_values
        )
        request_data = etree.tostring(
            cleanup_xml_node(request_data, remove_blank_nodes=False),
            xml_declaration=True,
        ...
        response_xml = self._call_nav_endpoint(
            credentials["mode"], "manageInvoice", request_data, timeout=60
        )
        self._parse_error_response(response_xml)

        transaction_code = response_xml.findtext(
            "api:transactionId", namespaces=XML_NAMESPACES
        )
    ...
    def do_query_transaction_status(
        self, credentials, transaction_code, return_original_request=False
    ):
        ...
        request_data = self.env["ir.qweb"]._render(
            "l10n_hu_edi.query_transaction_status_request", template_values
        )
        request_data = etree.tostring(
            cleanup_xml_node(request_data, remove_blank_nodes=False),
            xml_declaration=True,
        ...
        response_xml = self._call_nav_endpoint(
            credentials["mode"], "queryTransactionStatus", request_data
        )
        self._parse_error_response(response_xml)
        ...
        results = {
            ...
            "annulment_status": response_xml.findtext(
                "api:processingResults/api:annulmentData/api:annulmentVerificationStatus",
                namespaces=XML_NAMESPACES,
            ),
        }
        for processing_result_xml in response_xml.iterfind(
            "api:processingResults/api:processingResult", namespaces=XML_NAMESPACES
        ):
            processing_result = {
                "index": processing_result_xml.findtext(
                    "api:index", namespaces=XML_NAMESPACES
                ),
                "invoice_status": processing_result_xml.findtext(
                    "api:invoiceStatus", namespaces=XML_NAMESPACES
                ),
                "business_validation_messages": [],
                "technical_validation_messages": [],
            }
            for message_xml in processing_result_xml.iterfind(
                "api:businessValidationMessages", namespaces=XML_NAMESPACES
            ):
                processing_result["business_validation_messages"].append(
                    {
                        "validation_result_code": message_xml.findtext(
                            "api:validationResultCode", namespaces=XML_NAMESPACES
                        ),
                        "validation_error_code": message_xml.findtext(
                            "api:validationErrorCode", namespaces=XML_NAMESPACES
                        ),
                        "message": message_xml.findtext(
                            "api:message", namespaces=XML_NAMESPACES
                        ),
                    }
                )
            for message_xml in processing_result_xml.iterfind(
                "api:technicalValidationMessages", namespaces=XML_NAMESPACES
            ):
                processing_result["technical_validation_messages"].append(
                    {
                        "validation_result_code": message_xml.findtext(
                            "common:validationResultCode", namespaces=XML_NAMESPACES
                        ),
                        "validation_error_code": message_xml.findtext(
                            "common:validationErrorCode", namespaces=XML_NAMESPACES
                        ),
                        "message": message_xml.findtext(
                            "common:message", namespaces=XML_NAMESPACES
                        ),
                    }
                )
            if return_original_request:
                try:
                    original_file = b64decode(
                        processing_result_xml.findtext(
                            "api:originalRequest", namespaces=XML_NAMESPACES
                        )
                    )
    ...
    def do_query_transaction_list(
        self, credentials, datetime_from, datetime_to, page=1
    ):
        ...
        request_data = self.env["ir.qweb"]._render(
            "l10n_hu_edi.query_transaction_list_request", template_values
        )
        request_data = etree.tostring(
            cleanup_xml_node(request_data, remove_blank_nodes=False),
            xml_declaration=True,
        ...
        response_xml = self._call_nav_endpoint(
            credentials["mode"], "queryTransactionList", request_data
        )
        self._parse_error_response(response_xml)

        available_pages = response_xml.findtext(
            "api:transactionListResult/api:availablePage", namespaces=XML_NAMESPACES
        )
        try:
        ...
        transactions = []
        for transaction_xml in response_xml.iterfind(
            "api:transactionListResult/api:transaction", namespaces=XML_NAMESPACES
        ):
            try:
                send_time = datetime.fromisoformat(
                    transaction_xml.findtext(
                        "api:insDate", namespaces=XML_NAMESPACES
                    ).replace("Z", "")
                )
            except ValueError as e:
                raise L10nHuEdiConnectionError(
            ...
            transactions.append(
                {
                    "transaction_code": transaction_xml.findtext(
                        "api:transactionId", namespaces=XML_NAMESPACES
                    ),
                    "annulment": transaction_xml.findtext(
                        "api:technicalAnnulment", namespaces=XML_NAMESPACES
                    )
                    == "true",
                    "username": transaction_xml.findtext(
                        "api:insCusUser", namespaces=XML_NAMESPACES
                    ),
                    "source": transaction_xml.findtext(
                        "api:source", namespaces=XML_NAMESPACES
                    ),
                    "send_time": send_time,
                }
            )
    ...
    def do_manage_annulment(self, credentials, token, annulment_operations):
        ...
        template_values = {"exchangeToken": token, "annulments": []}
        ...
        template_values.update(
            self._get_header_values(credentials, invoice_hashs=annulment_hashes)
        )

        request_data = self.env["ir.qweb"]._render(
            "l10n_hu_edi.manage_annulment_request", template_values
        )
        request_data = etree.tostring(
            cleanup_xml_node(request_data, remove_blank_nodes=False),
            xml_declaration=True,
        ...
        response_xml = self._call_nav_endpoint(
            credentials["mode"], "manageAnnulment", request_data, timeout=60
        )
        self._parse_error_response(response_xml)
    ...
    def _call_nav_endpoint(self, mode, service, data, timeout=20):
        ...
        try:
            response_object = self.session.post(
                f"{url}{service}", data=data, headers=headers, timeout=timeout
            )
    ...
    def _parse_error_response(self, response_xml):
        error_code = response_xml.findtext(
            "common:result/common:errorCode", namespaces=XML_NAMESPACES
        )
        message = response_xml.findtext(
            "common:result/common:message", namespaces=XML_NAMESPACES
        )
        if error_code:
            errors = [f"{error_code}: {message}"]
            for message_xml in response_xml.iterfind(
                "api:technicalValidationMessages", namespaces=XML_NAMESPACES
            ):
                message = message_xml.findtext("api:message", namespaces=XML_NAMESPACES)
                error_code = message_xml.findtext(
                    "api:validationErrorCode", namespaces=XML_NAMESPACES
                )
