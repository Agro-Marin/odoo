...
class AccountMoveLine(models.Model):
    ...
    l10n_gr_edi_available_cls_category = fields.Char(
        compute="_compute_l10n_gr_edi_available_cls_category"
    )
    l10n_gr_edi_available_cls_type = fields.Char(
        compute="_compute_l10n_gr_edi_available_cls_type"
    )
    l10n_gr_edi_available_cls_vat = fields.Char(
        compute="_compute_l10n_gr_edi_available_cls_type"
    )
    ...
    def _auto_init(self):
        ...
        for column_name, column_type in (
            ...
            if not column_exists(self.env.cr, "account_move_line", column_name):
                create_column(
                    self.env.cr, "account_move_line", column_name, column_type
                )
    ...
    def _compute_l10n_gr_edi_available_cls_category(self):
        for line in self:
            ...
            is_income = (
                ...
                and (
                    not line.l10n_gr_edi_detail_type
                    or line.l10n_gr_edi_detail_type == "2"
                )
            )

            line.l10n_gr_edi_available_cls_category = self.env[
                "l10n_gr_edi.preferred_classification"
            ]._get_l10n_gr_edi_available_cls_category(
    ...
    def _compute_l10n_gr_edi_available_cls_type(self):
        for line in self:
            ...
            if (
                inv_type
                and CLASSIFICATION_MAP[inv_type] == "associate"
                and line.move_id.l10n_gr_edi_correlation_id
            ):
                inv_type = line.move_id.l10n_gr_edi_correlation_id.l10n_gr_edi_inv_type
            ...
            if cls_category:
                line.l10n_gr_edi_available_cls_type = self.env[
                    "l10n_gr_edi.preferred_classification"
                ]._get_l10n_gr_edi_available_cls_type(inv_type, cls_category)
                line.l10n_gr_edi_available_cls_vat = self.env[
                    "l10n_gr_edi.preferred_classification"
                ]._get_l10n_gr_edi_available_cls_vat(inv_type, cls_category)
            else:
    ...
    def _l10n_gr_edi_get_preferred_classification_id(self, with_category=False):
        ...
        if with_category:  # for _compute_l10n_gr_edi_cls_type
            domain = [
                ...
                (
                    ...
                    "in",
                    self.l10n_gr_edi_available_cls_type.split(","),
                ),
            ]
            ...
            domain = [
                ...
                (
                    ...
                    "in",
                    self.l10n_gr_edi_available_cls_category.split(","),
                ),
            ]
        ...
        if not preferred_id:
            preferred_id = self.env.ref(
                "l10n_gr_edi.default_preferred_classification", raise_if_not_found=False
            )
        if not preferred_id:
    ...
    def _compute_l10n_gr_edi_cls_category(self):
        for line in self:
            if not line.l10n_gr_edi_available_cls_category:
                ...
                line.l10n_gr_edi_cls_category
                and line.l10n_gr_edi_cls_category
                in line.l10n_gr_edi_available_cls_category
            ):
    ...
    def _compute_l10n_gr_edi_cls_type(self):
        for line in self:
            if not line.l10n_gr_edi_available_cls_type:
                ...
                line.l10n_gr_edi_cls_type
                and line.l10n_gr_edi_cls_type in line.l10n_gr_edi_available_cls_type
            ):
    ...
    def _compute_l10n_gr_edi_need_exemption_category(self):
        for line in self:
            ...
            line.l10n_gr_edi_need_exemption_category = (
                len(taxes) == 1 and taxes.amount == 0
            )
        ...
        for line in self:
            ...
            if (
                ...
                if line.l10n_gr_edi_tax_exemption_category:
                    ...
                    line.l10n_gr_edi_tax_exemption_category = (
                        taxes.l10n_gr_edi_default_tax_exemption_category or "1"
                    )
            else:
