...
class AccountMove(models.Model):
    _name = "account.move"
    _inherit = ["account.move", "mixin.exchange.subject"]
    ...
    l10n_gr_edi_url = fields.Char(
        compute="_compute_from_l10n_gr_edi_document_ids",
        store=True,
    )
    l10n_gr_edi_available_inv_type = fields.Char(
    ...
    l10n_gr_edi_need_correlated = fields.Boolean(
        compute="_compute_l10n_gr_edi_need_fields"
    )
    l10n_gr_edi_need_payment_method = fields.Boolean(
        compute="_compute_l10n_gr_edi_need_fields"
    )
    l10n_gr_edi_enable_view_mydata = fields.Boolean(
        compute="_compute_l10n_gr_edi_enable_fields"
    )
    l10n_gr_edi_enable_send_invoices = fields.Boolean(
        compute="_compute_l10n_gr_edi_enable_fields"
    )
    l10n_gr_edi_enable_send_expense_classification = fields.Boolean(
        compute="_compute_l10n_gr_edi_enable_fields"
    )
    ...
    def _compute_from_l10n_gr_edi_document_ids(self):
        for move in self:
            invoice = move._l10n_gr_edi_get_transmission("invoice")
            classification = move._l10n_gr_edi_get_transmission("classification")

            move.l10n_gr_edi_mark = (
                invoice.reference or classification.l10n_gr_edi_mark or False
            )
            move.l10n_gr_edi_cls_mark = (
                classification.reference
                if classification.state == "accepted"
                else False
            )
            move.l10n_gr_edi_url = invoice.l10n_gr_edi_url or False

            if invoice.state == "accepted":
                move.l10n_gr_edi_state = "invoice_sent"
            elif classification.state == "accepted":
                move.l10n_gr_edi_state = "bill_sent"
            elif classification.l10n_gr_edi_mark:
                move.l10n_gr_edi_state = "bill_fetched"
            else:
                move.l10n_gr_edi_state = False

            settled = invoice if invoice.state == "accepted" else classification
            move.l10n_gr_edi_attachment_id = settled.attachment_id or False
    ...
    def _compute_l10n_gr_edi_available_inv_type(self):
        for move in self:
            if move.is_sale_document(include_receipts=True):
                move.l10n_gr_edi_available_inv_type = ",".join(
                    INVOICE_TYPES_HAVE_INCOME
                )
    ...
    def _compute_l10n_gr_edi_need_fields(self):
        for move in self:
            ...
            move.l10n_gr_edi_need_payment_method = (
                move.l10n_gr_edi_inv_type in TYPES_WITH_MANDATORY_PAYMENT
            )
            ...
            barcode_params = urlencode(
                {
                    "barcode_type": "QR",
                    "quiet": 0,
                    "value": document.l10n_gr_edi_url,
                    "width": 180,
                    "height": 180,
                }
            )
    ...
    def _l10n_gr_edi_add_address_vals(self, values):
        ...
        inv_type_require_counterpart = (
            self.l10n_gr_edi_inv_type in TYPES_WITH_MANDATORY_COUNTERPART
        )

        conditional_address_keys = (
            "issuer_name",
            "issuer_postal_code",
            "issuer_city",
            "counterpart_vat",
        ...
        values.update(
            {
                "issuer_vat_number": self.company_id.vat.replace("EL", "").replace(
                    "GR", ""
                ),
            ...
            values.update(
                {
                    ...
                    "issuer_city": (self.company_id.city or "").encode("ISO-8859-7")
                    or None,
                }
            )
        ...
        if inv_type_allows_counterpart:
            values.update(
                {
                    "counterpart_vat": self.commercial_partner_id.vat.replace(
                        "EL", ""
                    ).replace("GR", ""),
                    "counterpart_country": self.commercial_partner_id.country_code,
                    "counterpart_branch": (
                        self.commercial_partner_id.l10n_gr_edi_branch_number or 0
                    ),
                }
            )
            if partner_not_from_greece:
                values["counterpart_name"] = self.commercial_partner_id.name.encode(
                    "ISO-8859-7"
                )

        if inv_type_require_counterpart or (
            inv_type_allows_counterpart and partner_not_from_greece
        ):
            values.update(
                {
                    "counterpart_postal_code": self.commercial_partner_id.zip,
                    "counterpart_city": (self.commercial_partner_id.city or "").encode(
                        "ISO-8859-7"
                    )
                    or None,
                }
    ...
    def _l10n_gr_edi_add_payment_method_vals(self, values):
        ...
        if (
            not values["payment_details"]
            and self.l10n_gr_edi_inv_type in TYPES_WITH_MANDATORY_PAYMENT
        ):
    ...
    def _l10n_gr_edi_common_base_line_details_values(self, base_line):
        ...
        if line.l10n_gr_edi_cls_category:
            cls_vals_list = (
                cls_vals["ecls"]
                if line.l10n_gr_edi_cls_category in CLASSIFICATION_CATEGORY_EXPENSE
                else cls_vals["icls"]
            )
            cls_type = line.l10n_gr_edi_cls_type or ""
            if (
                len(cls_type) > 0 and cls_type[0] == "X"
            ):  # handle duplicate E3 type on inv type 17.5
                cls_type = cls_type[1:]
    ...
    def _l10n_gr_edi_get_invoices_xml_vals(self):
        ...
        for move in self.sorted(key="id"):
            ...
            for line_no, base_line in enumerate(base_lines, start=1):
                ...
                if (
                    line.tax_ids
                    and move.l10n_gr_edi_inv_type not in TYPES_WITH_VAT_EXEMPT
                ):
                    tax = base_line["tax_details"]["taxes_data"][0][
                        "tax"
                    ]  # here, `tax` is guaranteed to be a single `account.tax` record
                    vat_category = VALID_TAX_CATEGORY_MAP[int(tax.amount)]
                if (
                    vat_category == 7
                    and move.l10n_gr_edi_inv_type in TYPES_WITH_VAT_CATEGORY_8
                ):
                    vat_category = 8
                ...
                details.append(
                    {
                        ...
                        if move.l10n_gr_edi_inv_type
                        not in TYPES_WITH_FORBIDDEN_QUANTITY
                        else "",
            ...
            invoice_values = {
                ...
                "header_correlate": move.l10n_gr_edi_correlation_id.l10n_gr_edi_mark
                or "",
                "details": details,
    ...
    def _l10n_gr_edi_get_pre_error_dict(self):
        ...
        if self.company_id.country_code != "GR" and (
        ...
        if self.commercial_partner_id:
            ...
            if (
                self.commercial_partner_id.country_code != "GR"
                or self.l10n_gr_edi_inv_type in TYPES_WITH_MANDATORY_COUNTERPART
            ) and (
                not self.commercial_partner_id.zip
        ...
        move_disallow_classification = (
            self.is_purchase_document(include_receipts=True)
            and self.l10n_gr_edi_inv_type in TYPES_WITH_FORBIDDEN_CLASSIFICATION
        )
        ...
        for line_no, line in enumerate(self.invoice_line_ids, start=1):
            ...
            if (
                not line.l10n_gr_edi_cls_category
                and line.l10n_gr_edi_available_cls_category
    ...
    @api.model
    def _l10n_gr_edi_send(self, kind: str) -> None:
        transmissions = self.env["exchange.transmission"]
        for move in self:
            transmissions |= move._l10n_gr_edi_get_open_transmission(kind)
        transmissions._send_many()

    def l10n_gr_edi_try_send_invoices(self):
        self.env["res.company"]._with_locked_records(self)
        self._l10n_gr_edi_send("invoice")

    def l10n_gr_edi_try_send_expense_classification(self):
        self._l10n_gr_edi_send("classification")
        if len(self) == 1:
            rejected = self._l10n_gr_edi_get_transmission("classification")
            if rejected.state == "rejected" and rejected.message:
                raise UserError(rejected.message)
    ...
    def _get_exchange_channel(self):
        return self._get_exchange_channel_of("mydata")
    ...
    def _l10n_gr_edi_get_open_transmission(self, kind: str):
        """The draft a fetched bill already carries, or a new one.
        ...
        if latest and not latest.is_settled:
            return latest
        return self._add_transmission("issue", kind=kind)

    def _on_transmission_changed(self) -> None:
        super()._on_transmission_changed()
        self._compute_from_l10n_gr_edi_document_ids()
