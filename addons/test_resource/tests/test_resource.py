...
class TestResource(TestResourceCommon):
    def test_calendars_validity_within_period(self):
        ...
        self.assertEqual(
            1,
            len(calendars[self.jean.resource_id.id]),
            "Jean should only have one calendar",
        )
        jean_entry = calendars[self.jean.resource_id.id]
        jean_calendar = next(iter(jean_entry))
        self.assertEqual(
            self.jean.resource_calendar_id,
            jean_calendar,
            "It should be Jean's Calendar",
        )
        self.assertFalse(
            jean_entry[jean_calendar] - interval,
            "Interval should cover all calendar's validity",
        )
        self.assertFalse(
            interval - jean_entry[jean_calendar],
        ...
        self.assertEqual(
            1, len(calendars[False]), "False (default) should only have one calendar"
        )
        ...
        self.assertFalse(
            false_entry[false_calendar] - interval,
            "Interval should cover all calendar's validity",
        )
        self.assertFalse(
            interval - false_entry[false_calendar],
            "Calendar validity should cover all interval",
        )
    ...
    def test_resource_without_calendar(self):
        ...
        unavailabilities = resource._get_unavailable_intervals(
