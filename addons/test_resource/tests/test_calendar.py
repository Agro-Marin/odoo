...
class TestCalendar(TestResourceCommon):
        ...
        self.env["resource.calendar.leaves"].create(
            {
                ...
                "date_from": self.datetime_str(
                    2018, 4, 3, 0, 0, 0, tzinfo=self.jean.tz
                ),
                "date_to": self.datetime_str(
                    2018, 4, 3, 23, 59, 59, tzinfo=self.jean.tz
                ),
            }
        )
        ...
        self.env["resource.calendar.leaves"].create(
            {
                ...
                "date_from": self.datetime_str(
                    2018, 4, 5, 0, 0, 0, tzinfo=self.jean.tz
                ),
                "date_to": self.datetime_str(
                    2018, 4, 5, 23, 59, 59, tzinfo=self.jean.tz
                ),
            }
        )
        ...
        self.env["resource.calendar.leaves"].create(
            {
                ...
                "resource_id": False,
                "date_from": self.datetime_str(
                    2018, 4, 3, 0, 0, 0, tzinfo=self.patel.tz
                ),
        ...
        leave = self.env["resource.calendar.leaves"].create(
            {
                ...
                "resource_id": False,
                "date_from": self.datetime_str(
                    2018, 4, 3, 9, 0, 0, tzinfo=self.patel.tz
                ),
                "date_to": self.datetime_str(
                    2018, 4, 3, 12, 0, 0, tzinfo=self.patel.tz
                ),
            }
        )
        ...
        leave = self.env["resource.calendar.leaves"].create(
            {
                ...
                "date_to": self.datetime_str(
                    2018, 4, 3, 0, 0, 10, tzinfo=self.patel.tz
                ),
            }
        )
        ...
        leave = self.env["resource.calendar.leaves"].create(
            {
                ...
                "resource_id": False,
                "date_from": self.datetime_str(
                    2018, 4, 11, 4, 0, 0, tzinfo=self.jules.tz
                ),
                "date_to": self.datetime_str(
                    2018, 4, 13, 4, 0, 0, tzinfo=self.jules.tz
                ),
            }
        )
        ...
        leave = self.env["resource.calendar.leaves"].create(
            {
                ...
                "resource_id": False,
                "date_from": self.datetime_str(
                    2018, 4, 9, 0, 0, 0, tzinfo=self.jules.tz
                ),
                "date_to": self.datetime_str(
                    2018, 4, 9, 23, 59, 0, tzinfo=self.jules.tz
                ),
            }
        )
    ...
    def test_calendar_working_hours_24(self):
        self.calendar_jean.attendance_ids.filtered(
            lambda a: a.dayofweek == "2"
        ).unlink()
    ...
    def test_plan_hours(self):
        self.env["resource.calendar.leaves"].create(
            {
                ...
                "resource_id": False,
                "date_from": self.datetime_str(
                    2018, 4, 11, 0, 0, 0, tzinfo=self.jean.tz
                ),
                "date_to": self.datetime_str(
                    2018, 4, 11, 23, 59, 59, tzinfo=self.jean.tz
                ),
            }
        )

        time = self.calendar_jean.plan_hours(
            2,
            self.datetime_tz(2018, 4, 10, 0, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=False,
        )
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 10, 10, 0, 0, tzinfo=self.jean.tz)
        )
        ...
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 12, 12, 0, 0, tzinfo=self.jean.tz)
        )

        time = self.calendar_jean.plan_hours(
            5,
            self.datetime_tz(2018, 4, 10, 15, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=True,
        )
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 12, 12, 0, 0, tzinfo=self.jean.tz)
        ...
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 6, 14, 0, 0, tzinfo=self.jean.tz)
        )

        # zero planning with holidays
        time = self.calendar_jean.plan_hours(
            0,
            self.datetime_tz(2018, 4, 11, 0, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=True,
        )
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 12, 8, 0, 0, tzinfo=self.jean.tz)
        )
        time = self.calendar_jean.plan_hours(
            0,
            self.datetime_tz(2018, 4, 10, 0, 0, 0, tzinfo=self.jean.tz),
        ...
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 10, 8, 0, 0, tzinfo=self.jean.tz)
        )
        ...
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 10, 8, 0, 0, 720000, tzinfo=self.jean.tz)
        )
        ...
        self.assertEqual(
            time, self.datetime_tz(2019, 9, 16, 16, 0, 0, tzinfo=self.jean.tz)
        )
    ...
    def test_plan_days(self):
        ...
        time = self.calendar_jean.plan_days(
            1,
            self.datetime_tz(2018, 4, 10, 0, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=False,
        )
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 10, 16, 0, 0, tzinfo=self.jean.tz)
        )
        ...
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 12, 16, 0, 0, tzinfo=self.jean.tz)
        )

        time = self.calendar_jean.plan_days(
            4,
            self.datetime_tz(2018, 4, 10, 16, 0, 0, tzinfo=self.jean.tz),
            compute_leaves=True,
        )
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 17, 16, 0, 0, tzinfo=self.jean.tz)
        )
        ...
        self.assertEqual(
            time, self.datetime_tz(2018, 3, 27, 8, 0, 0, tzinfo=self.jean.tz)
        )
        ...
        self.assertEqual(
            time, self.datetime_tz(2018, 4, 10, 0, 0, 0, tzinfo=self.jean.tz)
        )
    ...
    def test_closest_time(self):
        ...
        self.assertFalse(
            calendar_dt, "It should not return any value for unattended days"
        )
        ...
        calendar_dt = self.calendar_john._get_closest_work_time(
            dt, search_range=(range_start, range_end)
        )
        self.assertFalse(
            calendar_dt, "It should not return any value if dt outside of range"
        )
        ...
        calendar_dt = self.calendar_john._get_closest_work_time(dt)
        self.assertEqual(
            calendar_dt, start, "It should return the start of the closest attendance"
        )
        ...
        self.assertEqual(
            calendar_dt, end, "It should return the end of the closest attendance"
        )
    ...
    def test_attendance_interval_edge_tz(self):
        ...
        attendances = self.calendar_jean._attendance_intervals_batch(
            datetime.combine(
                date(2023, 1, 1), datetime.min.time(), tzinfo=timezone("UTC")
            ),
            datetime.combine(
                date(2023, 1, 31), datetime.max.time(), tzinfo=timezone("UTC")
            ),
        )
        last_attendance = list(attendances[False])[-1]
        self.assertEqual(
            last_attendance[0].replace(tzinfo=None), datetime(2023, 1, 31, 8)
        )
        self.assertEqual(
            last_attendance[1].replace(tzinfo=None),
            datetime(2023, 1, 31, 15, 59, 59, 999999),
        )
        ...
        self.assertEqual(
            last_attendance[1].replace(tzinfo=None), datetime(2023, 1, 31, 16)
        )
    ...
    def test_resource_calendar_update(self):
        ...
        holiday = self.env["resource.calendar.leaves"].create(
            {
                ...
                "calendar_id": self.calendar_jean.id,
                "date_from": self.datetime_str(
                    2024, 5, 1, 0, 0, 0, tzinfo=self.jean.tz
                ),
                "date_to": self.datetime_str(
                    2024, 5, 1, 23, 59, 59, tzinfo=self.jean.tz
                ),
            }
        )
        ...
        leave = self.env["resource.calendar.leaves"].create(
            {
                ...
                "resource_id": self.jean.resource_id.id,
                "date_from": self.datetime_str(
                    2024, 5, 10, 8, 0, 0, tzinfo=self.jean.tz
                ),
                "date_to": self.datetime_str(
                    2024, 5, 10, 16, 0, 0, tzinfo=self.jean.tz
                ),
            }
        )
        ...
        self.assertEqual(
            leave.calendar_id, self.calendar_jules, "Leave calendar should update"
        )
        self.assertEqual(
            holiday.calendar_id, self.calendar_jean, "Global leave shouldn't change"
        )
    ...
    def test_compute_work_time_with_two_weeks_calendar(self):
        ...
        def create_attendance_ids(attendance_list):
            return [
                (0, 0, {"week_type": str(i), **attendance})
                for i in range(2)
