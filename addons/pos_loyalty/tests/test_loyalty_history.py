...
class TestPOSLoyaltyHistory(TestPointOfSaleHttpCommon):
    ...
    def test_duplicate_coupon_confirm(self):
        ...
        def check_coupon(points, history_count):
            created_card = self.env["loyalty.card"].search(
                [
                    ("program_id", "=", ewallet_program.id),
                    ("partner_id", "=", test_partner.id),
                ]
            ...
            self.assertEqual(
                created_card.points,
                points,
                "The coupon should have 50 points after the first confirmation.",
            )
            self.assertEqual(
                len(created_card.history_ids),
                history_count,
                "The history should have one entry after the first confirmation.",
            )
    ...
    def test_programs_loaded(self):
        ...
        eur_loyalty = self.env["loyalty.program"].create(
            {"name": "EUR program", "currency_id": eur_currency.id}
        )
