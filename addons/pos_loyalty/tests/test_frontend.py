...
class TestUi(TestPointOfSaleHttpCommon):
    ...
    def setUpClass(cls):
                ...
                "reward_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "discount_product_ids": cls.whiteboard_pen.product_variant_id
                            | cls.magnetic_board.product_variant_id
                            | cls.desk_organizer.product_variant_id,
                        },
                    )
                ],
            }
        ...
        cls.coupon1, cls.coupon2, cls.coupon3, cls.coupon4 = (
            cls.coupon_program.coupon_ids
        )
    ...
    def test_pos_loyalty_tour_basic(self):
        ...
        # check coupon usage
        self.assertEqual(
            self.coupon1.points, 0, "The coupon should have consumed its points."
        )
        self.assertEqual(
            self.coupon2.points, 4.5, "The coupon was used but never validated."
        )
        ...
        free_multi_product = self.env["loyalty.program"].create(
            {
                ...
                "rule_ids": [
                    (
                        ...
                        {
                            "product_ids": (
                                self.wall_shelf.product_variant_id
                                | self.small_shelf.product_variant_id
                            ).ids,
                            "reward_point_mode": "unit",
                            "minimum_qty": 0,
                        },
                ...
                "reward_ids": [
                    (
                        ...
                        {
                            "reward_type": "product",
                            "reward_product_tag_id": self.env["product.tag"]
                            .create(
                                {
                                    "name": "reward_product_tag",
                                    "product_product_ids": (
                                        self.desk_pad.product_variant_id
                                        | self.monitor_stand.product_variant_id
                                    ).ids,
                                }
                            )
                            .id,
    ...
    def test_loyalty_free_product_loyalty_program(self):
        ...
        aaa_loyalty_card = loyalty_program.coupon_ids.filtered(
            lambda coupon: coupon.partner_id.id == partner_aaa.id
        )
        ...
        self.assertEqual(
            loyalty_program.pos_order_count,
            2,
            msg="Only 2 orders should have reward lines.",
        )
        self.assertAlmostEqual(aaa_loyalty_card.points, 1)

        bbb_loyalty_card = loyalty_program.coupon_ids.filtered(
            lambda coupon: coupon.partner_id.id == partner_bbb.id
        )
        ccc_loyalty_card = loyalty_program.coupon_ids.filtered(
            lambda coupon: coupon.partner_id.id == partner_ccc.id
        )
        ...
        reward_orderline = self.main_pos_config.current_session_id.order_ids[
            -1
        ].lines.filtered(lambda line: line.is_reward_line)
        self.assertEqual(
            len(reward_orderline.ids),
            0,
            msg="Reference: Order4_no_reward. Last order should have no reward line.",
        )
    ...
    def test_gift_card_program(self):
        ...
        gift_card_program = self.create_programs([("arbitrary_name", "gift_card")])[
            "arbitrary_name"
        ]
        ...
        gift_card_creation_history = self.env["loyalty.history"].search(
            [("card_id", "=", gift_card_program.coupon_ids.id)]
        )
        self.assertEqual(
            gift_card_creation_history.issued,
            50.0,
            "The gift card should have 50 points issued.",
        )
        ...
        loyalty_history = self.env["loyalty.history"].search(
            [
                ("card_id", "=", gift_card_program.coupon_ids.id),
                ("id", "!=", gift_card_creation_history.id),
            ]
        )
    ...
    def test_ewallet_program(self):
        ...
        self.env.ref("loyalty.ewallet_product_50").product_tmpl_id.write(
            {"active": True}
        )
        # Create ewallet program
        ewallet_program = self.create_programs([("arbitrary_name", "ewallet")])[
            "arbitrary_name"
        ]
        ...
        ewallet_aaa = self.env["loyalty.card"].search(
            [
        ...
        ewallet_bbb = self.env["loyalty.card"].search(
            [
    ...
    def test_multiple_gift_wallet_programs(self):
        ...
        ewallet_1_aaa = self.env["loyalty.card"].search(
            [
                ("partner_id", "=", partner_aaa.id),
                ("program_id", "=", programs["ewallet_1"].id),
            ]
        )
        self.assertEqual(len(ewallet_1_aaa), 1)
        self.assertAlmostEqual(ewallet_1_aaa.points, 18, places=2)
        ewallet_2_aaa = self.env["loyalty.card"].search(
            [
                ("partner_id", "=", partner_aaa.id),
                ("program_id", "=", programs["ewallet_2"].id),
            ]
        )
        self.assertEqual(len(ewallet_2_aaa), 1)
        self.assertAlmostEqual(ewallet_2_aaa.points, 40, places=2)
        ewallet_1_bbb = self.env["loyalty.card"].search(
            [
                ("partner_id", "=", partner_bbb.id),
                ("program_id", "=", programs["ewallet_1"].id),
            ]
        ...
        ewallet_2_bbb = self.env["loyalty.card"].search(
            [
                ("partner_id", "=", partner_bbb.id),
                ("program_id", "=", programs["ewallet_2"].id),
            ]
        ...
        self.main_pos_config2.write(
            {
                ...
                "available_pricelist_ids": [
                    (4, pricelist.id),
                    (4, self.main_pos_config.pricelist_id.id),
                ],
    ...
    def test_promotion_program_with_global_discount(self):
        ...
        if not self.env["ir.module.module"].search(
            [("name", "=", "pos_discount"), ("state", "=", "installed")]
        ):
                ...
                "rule_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "minimum_qty": 2,
                            "product_ids": [
                                (6, 0, [self.product_a.id, self.product_b.id])
                            ],
                        },
                    )
                ],
    ...
    def test_specific_product_discount_with_global_discount(self):
        ...
        self.env["loyalty.program"].create(
            {
                ...
                "trigger": "auto",
                "applies_on": "current",
                "rule_ids": [(0, 0, {"reward_point_mode": "order", "minimum_qty": 0})],
                "reward_ids": [
                    (
                        0,
                        0,
        ...
        self.main_pos_config2.with_user(self.pos_user).open_ui()
        self.start_pos_tour(
            "PosLoyaltySpecificProductDiscountWithGlobalDiscount",
            pos_config=self.main_pos_config2,
        )
    ...
    def test_discount_with_reward_product_domain(self):
        ...
        product_category_office = self.env["product.category"].create(
            {"name": "Office furnitures", "parent_id": product_category_base.id}
        )
        ...
        self.promo_program = self.env["loyalty.program"].create(
            {
                ...
                "program_type": "promotion",
                "pos_ok": True,
                "rule_ids": [(0, 0, {"minimum_amount": 0, "minimum_qty": 0})],
                "reward_ids": [
                    (
                        0,
                        0,
    ...
    def test_gift_card_price_no_tax(self):
        ...
        gift_card_program.payment_program_discount_product_id.taxes_id = self.env[
            "account.tax"
        ].create(
            {
                "name": "Test Tax",
    ...
    def test_physical_gift_card_sale(self):
        ...
        for expected_code in expected_coupons:
            self.assertIn(
                expected_code,
                coupon_codes,
                f"Expected coupon code '{expected_code}' not found",
            )

        # Check if the expected number of coupons are generated
        self.assertEqual(
            len(gift_card_program.coupon_ids), 3, "Three coupons should be generated"
        )
        ...
        for coupon in gift_card_program.coupon_ids:
            if coupon.code in expected_coupons:
                self.assertEqual(
                    coupon.points,
                    expected_coupons[coupon.code],
                    f"Coupon points for '{coupon.code}' should be {expected_coupons[coupon.code]}",
                )
            else:
                # This is the auto-generated coupon with 50 points
                self.assertEqual(
                    coupon.points, 100, "Auto-generated coupon should have 100 points"
                )
    ...
    def test_gift_card_code_links_to_correct_program(self):
        ...
        self.assertTrue(
            len(program1.coupon_ids)
            == len(program2.coupon_ids)
            == len(program3.coupon_ids)
            == 1
        )
    ...
    def test_dont_grant_points_reward_order_lines(self):
        ...
        loyalty_card = loyalty_program.coupon_ids.filtered(
            lambda coupon: coupon.partner_id.id == partner.id
        )
        ...
        discount_product = self.env["product.product"].search(
            [
                (
                    "id",
                    "=",
                    self.code_promo_program.reward_ids.discount_line_product_id.id,
                )
            ]
        )
    ...
    def test_change_reward_value_with_language(self):
        ...
        loyalty_program.reward_ids.update_field_translations(
            "description", {"en_US": "A en_US name which should not be displayed"}
        )
    ...
    def test_customer_loyalty_points_displayed(self):
        ...
        loyalty_program = self.create_programs([("Loyalty P", "loyalty")])["Loyalty P"]
        self.env["loyalty.card"].create(
            {"partner_id": john_doe.id, "program_id": loyalty_program.id, "points": 0}
        )
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "CustomerLoyaltyPointsDisplayed",
            login="pos_user",
        )
    ...
    def test_specific_product_reward_pos_combo(self):
        ...
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}",
            "PosComboSpecificProductProgram",
            login="pos_user",
        )
    ...
    def test_next_order_coupon_program_expiration_date(self):
        ...
        loyalty_program = self.env["loyalty.program"].create(
            {
                ...
                "portal_visible": True,
                "date_to": date.today() + timedelta(days=2),
                "rule_ids": [(0, 0, {"minimum_amount": 10, "minimum_qty": 0})],
                "reward_ids": [
                    (
                        0,
                        0,
        ...
        self.assertEqual(
            ...
            "Coupon not generated with correct expiration date",
        )
    ...
    def test_loyalty_on_order_with_fixed_tax(self):
        ...
        self.env["loyalty.program"].search(
            [("id", "!=", self.auto_promo_program_next.id)]
        ).write({"active": False})
        self.auto_promo_program_next.coupon_ids = [
            Command.create({"code": "563412", "points": 10})
        ]
    ...
    def test_physical_gift_card_invoiced(self):
        ...
        self.assertEqual(
            len(gift_card_program.coupon_ids), 1, "Gift card not generated"
        )
        self.assertEqual(
            gift_card_program.coupon_ids[0].code,
            "test-card-1234",
            "Gift card code not correct",
        )
        self.assertEqual(
            gift_card_program.coupon_ids[0].partner_id,
            partner,
            "Gift card partner id not correct",
        )
    ...
    def test_two_variant_same_discount(self):
        ...
        self.start_tour(
            ...
            "test_two_variant_same_discount",
            login="pos_user",
        )
    ...
    def test_settle_dont_give_points_again(self):
        ...
        """
        if not self.env["ir.module.module"].search(
            [("name", "=", "pos_settle_due"), ("state", "=", "installed")]
        ):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_settle_dont_give_points_again",
            login="accountman",
        )
    ...
    def test_refund_does_not_decrease_points(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_refund_does_not_decrease_points",
            login="pos_user",
        )
    ...
    def test_multiple_loyalty_products(self):
        ...
        self.start_tour(
            "/pos/web?config_id=%d" % self.main_pos_config.id,
            "test_multiple_loyalty_products",
            login="pos_user",
        )
    ...
    def test_physical_gift_card(self):
        ...
        created_gift_card = self.env["loyalty.card"].search(
            [("points", "=", 999)], order="id desc", limit=1
        )
    ...
    def test_scan_loyalty_card_select_customer(self):
        self.env["loyalty.program"].search([]).write({"active": False})
        self.test_partner = self.env["res.partner"].create({"name": "AAA Test Partner"})
    ...
    def test_confirm_coupon_programs_one_by_one(self):
        ...
        with patch.object(
            pos_order, "confirm_coupon_programs", confirm_coupon_programs_patch
        ):
            self.start_pos_tour(
                "test_confirm_coupon_programs_one_by_one", login="pos_user"
            )
    ...
    def test_specific_reward_product_tax_included_excluded(self):
        ...
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}",
            "test_specific_reward_product_tax_included_included",
            login="pos_user",
        )
        ...
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}",
            "test_specific_reward_product_tax_included_excluded",
            login="pos_user",
        )
    ...
    def test_order_reward_product_tax_included_excluded(self):
        ...
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}",
            "test_order_reward_product_tax_included_included",
            login="pos_user",
        )
        tax_01.price_include_override = "tax_excluded"
        # Discount should be the same even if tax mode is changed
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}",
            "test_order_reward_product_tax_included_excluded",
            login="pos_user",
        )
    ...
    def test_loyalty_is_not_processed_for_draft_order(self):
        ...
        self.start_pos_tour(
            "test_loyalty_is_not_processed_for_draft_order", login="pos_user"
        )
    ...
    def test_race_conditions_update_program(self):
        ...
        self.start_tour(
            ...
            "test_race_conditions_update_program",
            login="pos_user",
        )
