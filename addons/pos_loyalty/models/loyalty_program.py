...
class LoyaltyProgram(models.Model):
    ...
    def _compute_pos_report_print_id(self):
        for program in self:
            program.pos_report_print_id = (
                program.communication_plan_ids.pos_report_print_id[:1]
            )
        ...
        for program in self:
            ...
            if program.pos_report_print_id:
                if not program.mail_template_id:
                    mail_template_label = program._fields.get(
                        "mail_template_id"
                    ).get_description(self.env)["string"]
                    pos_report_print_label = program._fields.get(
                        "pos_report_print_id"
                    ).get_description(self.env)["string"]
                    raise UserError(
                        _(
