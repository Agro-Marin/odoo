...
class PosConfig(models.Model):
        ...
        return (
            ...
            .search(
                [
                    ...
                    "|",
                    ("pricelist_ids", "=", False),
                    ("pricelist_ids", "in", self.available_pricelist_ids.ids),
    ...
    def _check_before_creating_new_session(self):
        ...
        gift_card_programs = self._get_program_ids().filtered(
            lambda p: p.program_type == "gift_card"
        )
        ...
        if gift_card_programs:
            for gc_program in gift_card_programs:
                ...
                if len(gc_program.reward_ids) > 1:
                    raise UserError(
                        _("Invalid gift card program. More than one reward.")
                    )
                ...
                if rule.reward_point_amount != 1 or rule.reward_point_mode != "money":
                    raise UserError(
                        _(
                            "Invalid gift card program rule. Use 1 point per currency spent."
                        )
                    )
                reward = gc_program.reward_ids
                if (
                    reward.reward_type != "discount"
                    or reward.discount_mode != "per_point"
                    or reward.discount != 1
                ):
                    raise UserError(
                        _(
                            "Invalid gift card program reward. Use 1 currency per point discount."
                        )
                    )
                if not gc_program.mail_template_id:
                    raise UserError(
                        _(
                            "There is no email template on the gift card program and your pos is set to print them."
                        )
                    )
                if not gc_program.pos_report_print_id:
                    raise UserError(
                        _(
                            "There is no print report on the gift card program and your pos is set to print them."
                        )
                    )
    ...
    def use_coupon_code(self, code, creation_date, partner_id, pricelist_id):
        ...
        if (
            ...
            or (
                program.limit_usage
                and program.sudo().total_order_count >= program.max_usage
            )
        ):
        ...
        elif not program.reward_ids or not any(
            r.required_points <= coupon.points for r in program.reward_ids
        ):
            ...
            error_message = _(
                "This coupon is not available with the current pricelist."
            )
