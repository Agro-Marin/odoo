...
beforeEach(async () => {
    ...
    env["account.analytic.line"].create([
        {
            x_plan1_id: accounts[0],
            analytic_distribution: { [accounts[0]]: 100 },
            amount: 1,
        },
        {
            x_plan1_id: accounts[1],
            analytic_distribution: { [accounts[1]]: 100 },
            amount: 10,
        },
        {
            x_plan1_id: accounts[2],
            analytic_distribution: { [accounts[2]]: 100 },
            amount: 100,
        },
        {
            x_plan1_id: accounts[3],
            analytic_distribution: { [accounts[3]]: 100 },
            amount: 1000,
        },
        {
            x_plan1_id: accounts[4],
            analytic_distribution: { [accounts[4]]: 100 },
            amount: 10000,
        },
        {
            x_plan1_id: accounts[5],
            analytic_distribution: { [accounts[5]]: 100 },
            amount: 100000,
        },
    ]);
});
...
test("Analytic single-edit no dynamic", async () => {
    ...
    await contains(
        ".o_list_table tbody tr:nth-child(1) .o_list_record_selector input",
    ).click();
    await contains(
        ".o_list_table tbody tr:nth-child(2) .o_list_record_selector input",
    ).click();

    await contains(
        ".o_list_table tbody tr:first .o_field_analytic_distribution",
    ).click();
    await animationFrame();
    expect(".analytic_distribution_popup").toHaveCount(1);
    // all the fields should be displayed
    expect(".analytic_distribution_popup tbody tr:first .o_field_many2one").toHaveCount(
        1,
    );
    // we shouldn't display the button-links to hide/display the fields
    expect(".analytic_distribution_popup .o_list_table thead th:first a").toHaveCount(
        0,
    );
...
test("Analytic dynamic multi-edit", async () => {
    ...
    expect(
        ".o_list_table tbody tr:nth-child(1) .o_field_analytic_distribution .o_tag_badge_text",
    ).toHaveText("Los Angeles");
    expect(
        ".o_list_table tbody tr:nth-child(2) .o_field_analytic_distribution .o_tag_badge_text",
    ).toHaveText("New York");
    ...
    await animationFrame();
    await contains(
        ".analytic_distribution_popup .o_list_table thead th:first a",
    ).click();
    expect(".analytic_distribution_popup tbody tr:first .o_field_many2one").toHaveCount(
        1,
    );
    await contains(
        ".analytic_distribution_popup tbody tr:first .o_field_many2one",
    ).click();
    await contains(
        ".analytic_distribution_popup tbody tr:first .o_field_many2one input",
    ).edit("Brussels", { confirm: false });
    await runAllTimers();
    await contains(
        ".analytic_distribution_popup tbody tr:first .o_field_many2one .o_input_dropdown a",
    ).click();
    await contains(".o_list_renderer").click(); // close the widget
