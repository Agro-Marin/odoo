...
class MixinAnalyticPlanFields(models.AbstractModel):
    """Add one field per analytic plan to the model"""

    _name = "mixin.analytic.plan.fields"
    ...
    def _get_analytic_accounts(self):
        return self.env["account.analytic.account"].browse(
            [self[fname].id for fname in self._get_plan_fnames() if self[fname]]
        )

    def _get_distribution_key(self):
        return ",".join(
            str(account_id) for account_id in self._get_analytic_accounts().ids
        )
