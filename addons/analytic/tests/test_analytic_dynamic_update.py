...
class TestAnalyticDynamicUpdate(AnalyticCommon):
    def test_configurations(self):
        ...
        def capture_create():
            ...
            with patch.object(
                self.env.registry["account.analytic.line"], "create", patch_create
            ):
                yield container
        ...
        for comment, init, update, expect in [
            (
                ...
                [
                    {
                        plan1: self.analytic_account_1.id,
                        plan2: self.analytic_account_3.id,
                        "amount": 10.0,
                    },
                    {
                        plan1: self.analytic_account_2.id,
                        plan2: self.analytic_account_3.id,
                        "amount": 15.0,
                    },
                    {
                        plan1: self.analytic_account_1.id,
                        plan2: self.analytic_account_4.id,
                        "amount": 30.0,
                    },
                    {
                        plan1: self.analytic_account_2.id,
            ...
            with self.subTest(comment=comment):
                lines = self.env["account.analytic.line"].create(
                    [{"name": "test"} | vals for vals in init]
                )
