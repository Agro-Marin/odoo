...
class TestAnalyticAccount(AnalyticCommon):
    ...
    def setUpClass(cls):
        ...
        cls.distribution_1, cls.distribution_2 = cls.env[
        ...
        cls.company_b_branch = cls.env["res.company"].create(
            {"name": "B Branch", "parent_id": cls.company.id}
        )
    ...
    def test_aggregates(self):
        ...
        self.assertEqual(
            ...
            {key: {"aggregator": "sum"} for key in fields_to_agg},
    ...
    def test_get_plans_without_options(self):
        ...
        self.assertEqual(
            1,
            len(plans_json) - self.analytic_plan_offset,
        ...
        self.assertEqual(
            2,
            len(plans_json) - self.analytic_plan_offset,
            "All root plans should be available",
        )
    ...
    def test_get_plans_with_option(self):
        ...
        self.assertEqual(
            2,
            len(plans_json) - self.analytic_plan_offset,
            "Both plans should be available",
        )
    ...
    def test_all_account_count_with_subplans(self):
        ...
        self.env["account.analytic.account"].create(
            [
                ...
                {"name": "Child Account", "plan_id": self.analytic_sub_plan.id},
                {
                    "name": "Grand Child Account",
                    "plan_id": self.analytic_sub_sub_plan.id,
                },
            ]
        )

        expected_values = {
            self.analytic_plan: 3,
            self.analytic_sub_plan: 2,
            self.analytic_sub_sub_plan: 1,
        }
        ...
        analyst = self.env["res.users"].create(
            {
                "login": "analyst",
                "group_ids": [
                    Command.set(self.env.ref("analytic.group_analytic_accounting").ids)
                ],
    ...
    def test_analytic_account_branches(self):
        ...
        account_user = self.env.ref(
            "account.analytic.model_account_analytic_line", raise_if_not_found=False
        )
        if timesheet_user and not account_user:
            self.skipTest(
                "`hr_timesheet` overrides analytic rights. Without `account` the test would crash"
            )
    ...
    def test_change_parent_plan_conflict(self):
        ...
        with self.assertRaisesRegex(
            RedirectWarning, "Making this change would wipe out"
        ):
            self.analytic_plan_1.parent_id = self.analytic_plan_2
    ...
    def test_change_sys_param(self):
        ...
        current_project_plan, _other_plans = self.env[
