...
class TestLeadConvertMass(crm_common.TestLeadConvertMassCommon):
    ...
    def test_assignment_salesmen(self):
        ...
        with self.assertQueryCount(user_sales_manager=232):
    ...
    def test_assignment_salesmen_wteam(self):
        ...
        with self.assertQueryCount(user_sales_manager=205):
    ...
    def test_mass_convert_internals(self):
        (self.lead_w_partner | self.lead_w_email_lost).write({"user_id": False})
    ...
    def test_mass_convert_deduplicate(self):
        ...
        lead_1_final = self.lead_1

        _customer2, lead_w_partner_dups = self._create_duplicates(
            self.lead_w_partner, create_opp=False
        )
        lead_w_partner_final = lead_w_partner_dups[0]
        lead_w_partner_dups_partner = lead_w_partner_dups[1]
        ...
        self.assertEqual(
            mass_convert.duplicated_lead_ids, self.lead_1 | self.lead_w_partner
        )
    ...
    def test_mass_convert_find_existing(self):
        ...
        self.assertNotEqual(
            lead.partner_id,
            wrong_partner,
            "Partner Id should not match the wrong contact",
        )
    ...
    def test_mass_convert_performances(self):
        ...
        with self.assertQueryCount(user_sales_manager=1096):
            mass_convert = (
                self.env["crm.lead2opportunity.partner.mass"]
                .with_context(
                    {
