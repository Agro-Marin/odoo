...
All use `user._send_gamification_notification(type, data)` which calls
...
|--------|-------|---------|
| `get_activity_feed(limit)` | `gamification.activity` | Unified feed with privacy filtering |
| `get_analytics_summary()` | `gamification.engagement.snapshot` | Latest metrics + 7-day trends |
| `_get_karma_leaderboard(limit)` | `res.users` | Top karma users (privacy-filtered) |
| `send_kudos_from_dashboard(...)` | `res.users` | Create kudos from dashboard inline form |
| `get_season_leaderboard(limit)` | `gamification.season` | Karma earned during season window |
| `get_suggested_mentors(limit)` | `gamification.mentorship` | Higher-karma users available for mentoring |
