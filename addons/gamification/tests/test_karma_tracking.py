...
class TestKarmaTrackingCommon(common.TransactionCase):
    ...
    def test_user_as_erp_manager(self):
        ...
        self.assertEqual(
            trackings[0].reason, "User Creation (Test User #%i)" % self.test_user.id
        )
    ...
    def test_user_tracking(self):
        ...
        self.assertEqual(
            tracking_1.reason, "Add Manually (Test User #%i)" % self.test_user.id
        )
        ...
        self.assertEqual(
            tracking_2.reason, "Add Manually (Test User #%i)" % self.test_user.id
        )
