...
ORDER_CUSTOMIZATION_ID = "urn:fdc:peppol.eu:poacc:trns:order:3"


class MixinOrder(models.AbstractModel):
    _inherit = "mixin.order"

    _edi_ubl_builder = ""

    def _get_edi_builders(self):
        builders = super()._get_edi_builders()
        if self._edi_ubl_builder:
            builders = [*builders, self.env[self._edi_ubl_builder]]
        return builders


class MixinOrderDocumentImport(models.AbstractModel):
    _inherit = "mixin.order.document.import"

    _edi_ubl_builder = ""

    def _import_file_type_rules(self):
        rules = super()._import_file_type_rules()
        if not self._edi_ubl_builder:
            return rules
        return [
            (
                self._edi_ubl_builder,
                findtext_equals(CUSTOMIZATION_ID, ORDER_CUSTOMIZATION_ID),
            ),
            *rules,
        ]
    ...
    def _get_edi_decoder(self, file_data, new=False):
        if (
            self._edi_ubl_builder
            and file_data["import_file_type"] == self._edi_ubl_builder
        ):
            _debug.logic("edi_decoder", orders=self, format="ubl_bis3")
            return {
                "priority": 20,
                "decoder": self.env[self._edi_ubl_builder]._import_order_ubl,
            }
