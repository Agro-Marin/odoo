...
class TestSMSPost(SMSCommon, MockLinkTracker):
    ...
    def test_sms_send_batch_size(self):
        ...
        def _send(
            sms_self, unlink_failed=False, unlink_sent=True, raise_exception=False
        ):
            self.count += 1
            return DEFAULT
        ...
        with patch.object(
            SmsModel, "_send", autospec=True, side_effect=_send
        ) as _send_mock:
    ...
    def test_sms_send_batch_size_out_of_range(self):
        ...
        for value in ("-3", "0", "not a number", ""):
            with self.subTest(value=value):
                ...
                self.assertGreater(
                    self.env["sms.sms"]._get_send_batch_size(),
                    0,
        ...
        with patch.object(SmsModel, "_send", autospec=True, side_effect=_send):
            self.env["sms.sms"].browse(self.sms_all.ids).send()
        self.assertEqual(
            self.count, 1, "and the whole batch still goes out, in one piece"
        )
    ...
    def test_sms_send_crash_employee(self):
        with self.assertRaises(exceptions.AccessError):
            self.env["sms.sms"].with_user(self.user_employee).browse(
                self.sms_all.ids
            ).send()
    ...
    def test_sms_send_delete_all(self):
        with self.mockSMSGateway(sms_allow_unlink=True, sim_error="jsonrpc_exception"):
            self.env["sms.sms"].browse(self.sms_all.ids).send(
                unlink_failed=True, unlink_sent=True, raise_exception=False
            )
    ...
    def test_sms_send_delete_failed(self):
        with self.mockSMSGateway(
            ...
            self.env["sms.sms"].browse(self.sms_all.ids).send(
                unlink_failed=True, unlink_sent=False, raise_exception=False
            )
        remaining = self.sms_all.exists().filtered(lambda s: not s.to_delete)
    ...
    def test_sms_send_delete_none(self):
        with self.mockSMSGateway(
            ...
            self.env["sms.sms"].browse(self.sms_all.ids).send(
                unlink_failed=False, unlink_sent=False, raise_exception=False
            )
    ...
    def test_sms_send_delete_sent(self):
        with self.mockSMSGateway(
            ...
            self.env["sms.sms"].browse(self.sms_all.ids).send(
                unlink_failed=False, unlink_sent=True, raise_exception=False
            )
        remaining = self.sms_all.exists().filtered(lambda s: not s.to_delete)
