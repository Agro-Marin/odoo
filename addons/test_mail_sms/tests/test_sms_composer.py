...
class TestSMSComposerComment(SMSCommon, TestSMSRecipients):
    ...
    * add test for sanitized / wrong numbers;
    ...
    def test_composer_comment_not_mail_thread(self):
        with self.with_user("employee"):
            ...
            composer = (
                ...
                .with_context(active_model="test_orm.category", active_id=record.id)
                .create(
                    {
            ...
            [
                {
                    ...
                    "number": self.test_numbers_san[1],
                }
            ],
            self._test_body,
            messages,
    ...
    def test_composer_comment_field(self):
        ...
        record_numbers_all = [
            {
                "mobile_nbr_ids": self.test_numbers[0],
                "phone_nbr_ids": self.test_numbers[1],
            },
            {
                "mobile_nbr_ids": self.test_numbers[0],
                "phone_nbr_ids": self.test_numbers[1],
            ...
            {
                "mobile_nbr_ids": "invalid_phone_nbr",
                "phone_nbr_ids": self.test_numbers[1],
            },
        ]
        phone_fields = ["mobile_nbr_ids", "phone_nbr_ids", "mobile_nbr_ids"]
        ...
        for (
            record_numbers,
            phone_field,
            expected_form_number,
            expected_sent_number,
        ) in zip(
            record_numbers_all,
            phone_fields,
            expected_form_numbers,
            expected_sent_numbers,
            strict=True,
        ):
            with self.subTest(
                phone_field=phone_field, record_number=record_numbers[phone_field]
            ):
                self.test_record.write(
                    {
                        fname: [Command.clear(), Command.create({"number": number})]
                        for fname, number in record_numbers.items()
                    }
                )
                composer_form = Form(
                    self.env["sms.composer"].with_context(
                        ...
                        default_body=self._test_body,
                    )
                )
                self.assertEqual(
                    composer_form.recipient_single_number_itf, expected_form_number
                )
    ...
    def test_composer_comment_field_w_numbers(self):
        with self.with_user("employee"):
            composer = (
                ...
                .with_context(
                    active_model="mail.test.sms",
                    active_id=self.test_record.id,
                    default_number_field_name="mobile_nbr_ids",
                )
                .create(
                    {
                        "body": self._test_body,
        ...
        self.assertSMSNotification(
            [
                ...
                {"number": self.random_numbers_san[0]},
                {"number": self.random_numbers_san[1]},
            ],
            self._test_body,
            messages,
        )
    ...
    def test_composer_comment_invalid_field(self):
        ...
        sms_composer = self.env["sms.composer"].create(
            {
                "body": self._test_body,
                "number_field_name": "phone_nbr_ids",
        ...
        self.assertNotIn(",".join(test_record._fields), "phone_nbr_ids")
        with self.mockSMSGateway():
            sms_composer._action_send_sms()
        self.assertSMSNotification(
            [{"number": self.random_numbers_san[0]}], self._test_body
        )
    ...
    def test_composer_default_recipient(self):
        ...
        self.test_record.write(
            {
                "phone_nbr_ids": [
                    Command.clear(),
                    Command.create({"number": "0123456789"}),
                ],
            }
        )
        with self.with_user("employee"):
        ...
        self.assertEqual(
            ...
            self.test_record.customer_id.display_name,
        )
    ...
    def test_composer_internals(self):
        ...
        self.assertEqual(composer.number_field_name, "phone_nbr_ids")
        ...
        self.assertEqual(self.test_record.phone_nbr_ids.number, self.random_numbers[0])
    ...
    def test_composer_comment_wo_partner_wo_value_update(self):
        ...
        default_field_name = self.env["mail.test.sms"]._get_phone_number_fields()[0]
        ...
        with self.with_user("employee"):
            ...
            self.assertEqual(
                composer.recipient_single_number_itf, self.random_numbers_san[0]
            )
            self.assertFalse(composer.recipient_single_number)
        ...
        self.assertEqual(
            self.test_record[default_field_name].number, self.random_numbers_san[0]
        )
        self.assertSMSNotification(
            [
                {
    ...
    def test_composer_numbers_no_model(self):
        with self.with_user("employee"):
            composer = (
                self.env["sms.composer"]
                .with_context(default_composition_mode="numbers")
                .create(
                    {
                        "body": self._test_body,
                        "numbers": ",".join(self.random_numbers),
...
class TestSMSComposerBatch(SMSCommon):
    ...
    def test_composer_batch_active_ids(self):
        ...
        for record, message in zip(self.records, messages, strict=True):
            self.assertSMSNotification(
...
class TestSMSComposerMass(SMSCommon):
    ...
    def test_composer_mass_active_ids(self):
        ...
        for partner, record in zip(self.partners, self.records, strict=True):
            self.assertSMSOutgoing(
                partner, None, content="Hello %s zizisse an SMS." % record.name
            )
    ...
    def test_composer_mass_active_ids_w_blacklist(self):
        ...
        for partner, record in zip(self.partners[5:], self.records[5:], strict=True):
            self.assertSMSOutgoing(
                partner,
    ...
    def test_composer_mass_active_ids_w_blacklist_and_done(self):
        ...
        for p in self.partners[5:8]:
            p.phone_ids = self.partners[5].phone_ids
        ...
        self.assertSMSOutgoing(
            self.partners[5],
            self.partners[5].phone_sanitized,
            content="Hello %s zizisse an SMS." % self.records[5].name,
        )
        for partner, record in zip(self.partners[8:], self.records[8:], strict=True):
            self.assertSMSOutgoing(
                partner,
                partner.phone_sanitized,
                content="Hello %s zizisse an SMS." % record.name,
        ...
        # duplicates
        for partner, record in zip(self.partners[6:8], self.records[6:8], strict=True):
            self.assertSMSCanceled(
                partner,
                partner.phone_sanitized,
    ...
    def test_composer_mass_active_ids_w_template_and_lang(self):
        ...
        self.sms_template.with_context(
            lang="fr_FR"
        ).body = "Cher·e· {{ object.display_name }} ceci est un SMS."
    ...
    def test_composer_template_context_action(self):
        ...
        self.sms_template.with_context(
            lang="fr_FR"
        ).body = "Hello {{ object.display_name }} ceci est en français."
        # set template to try to use customer lang
        self.sms_template.write(
            {
                "lang": "{{ object.customer_id.lang }}",
        ...
        with self.with_user("employee"):
            ...
            self.assertEqual(
                composer.body,
                "Hello %s ceci est en français." % test_record_2.display_name,
            )
        ...
        with self.with_user("employee"):
            ...
            self.assertEqual(
                composer.body, "Dear {{ object.display_name }} this is an SMS."
            )
