...
class TestPhoneBlacklist(SMSCommon, TestSMSRecipients):
    ...
    def test_phone_blacklist_create_unblacklisted(self):
        ...
        unblacklisted_record = (
            self.env["phone.blacklist"]
            .sudo()
            .create([{"number": phone_number, "active": False}])
        )
        self.assertFalse(
            unblacklisted_record.active,
            "Creating an unblacklisted record resulted in a blacklisted one",
        )

        # Make sure that an attempt to re-create unblacklisted number will leave the number as it was (unblacklisted)
        still_unblacklisted_record = (
            self.env["phone.blacklist"]
            .sudo()
            .create([{"number": phone_number, "active": False}])
        )
        self.assertFalse(
            still_unblacklisted_record.active,
    ...
    def test_phone_blacklist_internals(self):
        with self.with_user("employee"):
            ...
            bl_record = (
                self.env["phone.blacklist"]
                .sudo()
                .create([{"number": self.test_numbers_san[1]}])
            )
            ...
            rec = self.env["mail.test.sms.bl"].search(
    ...
    def test_phone_blacklist_unblacklisted(self):
        ...
        bl_record = (
            self.env["phone.blacklist"]
            .sudo()
            .create([{"number": phone_number, "active": True}])
        )
        num_of_records = (
            self.env["phone.blacklist"].with_context(active_test=False).search_count([])
        )
