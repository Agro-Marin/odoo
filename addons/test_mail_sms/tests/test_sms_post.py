    ...
    * add tests for new mail.message and mixin.mail.thread fields;
    ...
    def test_sms_channel_survives_notify_skip_followers(self):
        ...
        for skip_followers in (False, True):
            with self.subTest(skip_followers=skip_followers):
                ...
                self.assertEqual(
                    message.sudo().notification_ids.notification_type,
                    "sms",
    ...
    def test_message_sms_marks_only_the_addressed_partners(self):
        ...
        addressed = Followers._get_recipient_data(
            test_record, "sms", subtype_id, self.partner_1.ids
        )[test_record.id]
        self.assertEqual(addressed[self.partner_1.id]["notif"], "sms")
        self.assertNotEqual(
            addressed[self.partner_2.id]["notif"],
            "sms",
        ...
        for pids in ([], None):
            with self.subTest(pids=pids):
                data = Followers._get_recipient_data(
                    test_record, "sms", subtype_id, pids
                )[test_record.id]
                self.assertFalse(
    ...
    def test_message_sms_internals_body(self):
        with self.with_user("employee"), self.mockSMSGateway():
            ...
            messages = test_record._message_sms(
                "<p>Mega SMS<br/>Top moumoutte</p>", partner_ids=self.partner_1.ids
            )

        self.assertEqual(
            messages.body, "<p>&lt;p&gt;Mega SMS&lt;br/&gt;Top moumoutte&lt;/p&gt;</p>"
        )  # html should not be interpreted
        self.assertEqual(messages.subtype_id, self.env.ref("mail.mt_note"))
        self.assertSMSNotification(
            [{"partner": self.partner_1}], "<p>Mega SMS<br/>Top moumoutte</p>", messages
        )
    ...
    def test_message_sms_model_partner(self):
        ...
        self.assertSMSNotification(
            [{"partner": self.partner_1}, {"partner": self.partner_2}],
            self._test_body,
            messages,
        )
    ...
    def test_message_sms_model_w_partner_only(self):
        with self.with_user("employee"):
            record = self.env["mail.test.sms.partner"].create(
                {"customer_id": self.partner_1.id}
            )
    ...
    def test_message_sms_on_field_wo_partner_default_field(self):
        ...
        self.assertSMSNotification(
            [{"number": self.test_numbers_san[1]}], self._test_body, messages
        )
    ...
    def test_message_sms_on_field_wo_partner_default_field_2(self):
        ...
        self.assertSMSNotification(
            [{"number": self.test_numbers_san[0]}], self._test_body, messages
        )
    ...
    def test_message_sms_on_numbers(self):
        with self.with_user("employee"), self.mockSMSGateway():
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, sms_numbers=self.random_numbers_san
            )
        self.assertSMSNotification(
            [
                {"number": self.random_numbers_san[0]},
    ...
    def test_message_sms_on_numbers_sanitization(self):
        with self.with_user("employee"), self.mockSMSGateway():
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, sms_numbers=self.random_numbers
            )
        self.assertSMSNotification(
            [
                {"number": self.random_numbers_san[0]},
    ...
    def test_message_sms_on_partner_ids(self):
        with self.with_user("employee"), self.mockSMSGateway():
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, partner_ids=(self.partner_1 | self.partner_2).ids
            )
    ...
    def test_message_sms_schedule(self):
        ...
        schedules = (
            ...
            .sudo()
            .search([("mail_message_id", "=", messages.id)])
        )
        ...
        self.assertSMSNotification(
            [{"partner": self.partner_1}], "Testing Scheduled Notifications", messages
        )
    ...
    def test_message_sms_with_template_fallback(self):
        ...
        self.assertSMSNotification(
            [{"partner": self.partner_1, "number": self.test_numbers_san[1]}],
            "Fallback for %s" % self.test_record.id,
            messages,
        )
    ...
    def test_message_sms_with_template_xmlid(self):
        ...
        with self.with_user("employee"):
            with self.mockSMSGateway():
                test_record = self.env["mail.test.sms"].browse(self.test_record.id)
                messages = test_record._message_sms_with_template(
                    template_xmlid="test_mail_full.this_should_exists"
                )
...
class TestSMSPostException(SMSCommon, TestSMSRecipients):
    ...
    def test_message_sms_w_numbers_invalid(self):
        ...
        with self.with_user("employee"), self.mockSMSGateway():
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, sms_numbers=random_numbers
            )
    ...
    def test_message_sms_w_partners_nocountry(self):
        ...
        self.assertSMSNotification(
            [{"partner": self.test_record.customer_id}], self._test_body, messages
        )
    ...
    def test_message_sms_crash_credit_single(self):
        with (
            self.with_user("employee"),
            self.mockSMSGateway(nbr_t_error={self.partner_2._phone_format(): "credit"}),
        ):
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body,
    ...
    def test_message_sms_crash_server_crash(self):
        with (
            self.with_user("employee"),
            self.mockSMSGateway(sim_error="jsonrpc_exception"),
        ):
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body,
    ...
    def test_message_sms_crash_wrong_number(self):
        with (
            self.with_user("employee"),
            self.mockSMSGateway(sim_error="wrong_number_format"),
        ):
            test_record = self.env["mail.test.sms"].browse(self.test_record.id)
            messages = test_record._message_sms(
                self._test_body, partner_ids=(self.partner_1 | self.partner_2).ids
...
class TestSMSApi(SMSCommon):
    ...
    def test_message_schedule_sms(self):
        with self.with_user("employee"):
            with self.mockSMSGateway():
                self.env["mail.test.sms"].browse(
                    self.records.ids
                )._message_sms_schedule_mass(body=self._test_body, mass_keep_log=False)
    ...
    def test_message_schedule_sms_w_log(self):
        with self.with_user("employee"):
            with self.mockSMSGateway():
                self.env["mail.test.sms"].browse(
                    self.records.ids
                )._message_sms_schedule_mass(body=self._test_body, mass_keep_log=True)
