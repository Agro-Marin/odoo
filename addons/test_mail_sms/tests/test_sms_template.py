...
class TestSmsTemplate(SMSCommon, TestSMSRecipients):
    ...
    def test_sms_template_render(self):
        rendered_body = self.sms_template._render_template(
            self.sms_template.body, self.sms_template.model, self.test_record.ids
        )
        self.assertEqual(
            rendered_body[self.test_record.id],
    ...
    def test_sms_template_lang(self):
        ...
        self.sms_template.update_field_translations("body", {"fr_FR": self.body_fr})
        ...
        self.assertEqual(
            self.sms_template.with_context(lang="fr_FR").body, self.body_fr
        )

        rid_to_lang = self.sms_template._render_lang(
            (self.test_record | test_record_2).ids
        )
        self.assertEqual(
            set(rid_to_lang.keys()), set((self.test_record | test_record_2).ids)
        )
        ...
        tpl_to_rids = self.sms_template._classify_per_lang(
            (self.test_record | test_record_2).ids
        )
