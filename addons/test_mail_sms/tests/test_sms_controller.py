...
class TestSmsController(TestSMSActionsCommon):
    ...
    def setUpClass(cls):
        ...
        cls.notification_processing, cls.notification_pending = cls.env[
            "mail.notification"
        ].create(
            [
                {
                    ...
                    "sms_tracker_ids": [
                        Command.create({"sms_uuid": cls.sms_processing.uuid})
                    ],
                },
                {
                    "mail_message_id": cls.msg.id,
                    ...
                    "sms_tracker_ids": [
                        Command.create({"sms_uuid": cls.sms_sent.uuid})
                    ],
                },
            ]
        )
        ...
        statuses = [
            {
                ...
                "uuids": [self.notification_pending.sms_tracker_ids.sms_uuid],
            }
        ]
        self.assertEqual(self._make_webhook_jsonrpc_request(statuses), "OK")
    ...
    def test_webhook_update_notification_multiple_statuses(self):
        statuses = [
            {
                "sms_status": "sent",
                "uuids": [self.notification_processing.sms_tracker_ids.sms_uuid],
            },
            {
                "sms_status": "delivered",
                "uuids": [self.notification_pending.sms_tracker_ids.sms_uuid],
    ...
    @mute_logger(
        ...
        "odoo.addons.sms.controllers.main",
        "odoo.http",
    )
    ...
    def test_webhook_update_succeeds_with_non_existent_uuids(self):
        statuses = [
            {"sms_status": "delivered", "uuids": ["00000000000000000000000000000000"]}
        ]
        self.assertEqual(self._make_webhook_jsonrpc_request(statuses), "OK")
