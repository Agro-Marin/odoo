...
class TestPhoneFormat(TransactionCase):
    def test_phone_format_country_guess(self):
        ...
        test_names = [
            "customer country",
            "first guest country",
            "second guest country",
            "record country",
            "existing prefix",
        ]

        for partner_vals, record_vals, input_number, expected_number, test_name in zip(
            partner_vals_all,
            record_vals_all,
            input_numbers,
            expected_numbers,
            ...
            for partner, base_vals, vals in zip(
                partners, base_partner_vals, partner_vals, strict=True
            ):
    ...
    def test_phone_format_perf(self):
        ...
        test_records = self.env["mail.test.sms"].create(
            [
                ...
                for nocountry_p, country_p in zip(
                    nocountry_partners, country_partners, strict=True
                )
            ]
        )
