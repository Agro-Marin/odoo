...
class TestSaleStockMargin(TestStockValuationCommon):
    ...
    def _setup_multicurrency(self):
        ...
        self.other_currency = (
            self.env.ref("base.EUR") if self.company_currency == usd else usd
        )
    ...
    def test_sale_stock_margin_6(self):
        ...
        self.assertEqual(
            order_line_1.purchase_price, 50, "Sales order line cost should be 50.00"
        )
        self.assertEqual(
            order_line_2.purchase_price, 40, "Sales order line cost should be 40.00"
        )

        self.assertEqual(
            order_line_1.margin, 50, "Sales order line profit should be 50.00"
        )
        self.assertEqual(
            order_line_2.margin, 40, "Sales order line profit should be 40.00"
        )
    ...
    def test_so_and_multicompany(self):
        ...
        new_company_currency = (
            ...
            if main_company_currency == self.env.ref("base.USD")
        ...
        self.env["res.currency.rate"].create(
            [
                {
                    "currency_id": main_company_currency.id,
                    "rate": 1,
                    "name": date,
                    "company_id": False,
                },
    ...
    def test_add_product_on_delivery_price_unit_on_sale(self):
        ...
        self._create_sale_order_line(
            sale_order, products[0], 10, products[0].list_price
        )
        sale_order.action_confirm()
    ...
    def test_add_standard_product_on_delivery_cost_on_sale_order(self):
        ...
        self.assertEqual(
            sale_order.line_ids.filtered(
                lambda sol: sol.product_id == product2
            ).purchase_price,
            10,
        )
    ...
    def test_add_avco_product_on_delivery_cost_on_sale_order(self):
        ...
        sale_order = self._create_sale_order()
        self._create_sale_order_line(
            sale_order, self.product, 10, self.product.list_price
        )
        sale_order.action_confirm()
        delivery = sale_order.picking_ids[0]
        with Form(delivery) as delivery_form:
    ...
    def test_avco_does_not_mix_products_on_compute_avg_price(self):
        ...
        self.assertEqual(
            second_delivery.move_ids.sale_line_id, sale_order.line_ids - sale_order_line
        )
    ...
    def test_avco_calc(self):
        ...
        with freeze_time() as freeze:
            ...
            self.assertEqual(
                sol.purchase_price,
                30,
                "purchase_price should match product's standard_price",
            )
            self.assertEqual(
                sol.margin,
                140,
            ...
            self.assertEqual(
                sol.purchase_price,
                30,
                "purchase_price = (qty_delivered * purchase_unit_from_delivery + qty_from_std_price * standard_price)/(qty_from_delivery + qty_from_std_price) = (1 * 30 + 1 * 30)/ (1 + 1) = 30",
            )
            self.assertEqual(
                sol.margin,
                140,
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                75,
                "standard_price for avco = (3 * 30 + 2 * 142.5) / (3 + 2) = 75: 3 remaining + 2 added to stock",
            )
            self.assertEqual(
                sol.purchase_price, 30, "purchase_price shouldn't have changed"
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ...
            self.assertEqual(
                sol.purchase_price,
                60,
                "purchase_price = (qty_delivered * purchase_unit_from_delivery + qty_from_std_price * standard_price)/(qty_from_delivery + qty_from_std_price) = (3 * 60 + 0 * 75)/ (3 + 0) = 60",
            )
            self.assertEqual(
                sol.margin,
                80,
                "margin = (sale price - purchase_price) * SOL quantity = (100 - 60) * 2 = 80",
            )
    ...
    def test_avco_zero_quantity(self):
        ...
        with freeze_time() as freeze:
            ...
            self.assertEqual(
                sol.margin,
                70,
                "margin = (sale price - purchase_price) * SOL quantity = (100 - 30) * 1 = 70",
            )
            so.action_confirm()
            ...
            self.assertEqual(
                sol2.purchase_price,
                self.product_avco_auto.standard_price,
                "0 Quantity and 0 Delivered => default to standard price",
            )
            self.assertEqual(
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                40,
                "standard_price for avco = (4 * 30 + 2 * 60) / (4 + 2) = 40: 4 existing + 2 added to stock",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            move = sol.move_ids
            ...
            self.assertEqual(
                sol.margin,
                60,
                "margin = (sale price - purchase_price) * SOL quantity = (100 - 40) * 1 = 60",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                30,
                "standard_price for avco = (5 * 40 + 2 * 5) / (5 + 2) = 30: 1 delivered, 5 remaining + 2 added to stock",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                33,
                "standard_price for avco = (7 * 30 + 3 * 40) / (7 + 3)) = 33: 7 remaining + 3 returned",
            )
            self.assertEqual(
                sol.purchase_price,
                self.product_avco_auto.standard_price,
                "< 0 Delivered => default to standard price",
            )
            self.assertEqual(
                sol.margin,
                67,
                "margin = (sale price - purchase_price) * SOL quantity = (100 - 33) * 1 = 67",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                32.5,
                "standard_price for avco = (10 * 33 + 2 * 30) / 12 = 32.5: 10 remaining + 2 added to stock",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            sol.product_qty = 0
            self.assertEqual(
            ...
            self.assertEqual(
                sol3.margin,
                -65,
                "margin = SOL qty * sale price - purchase_price * qty_delivered = (0 - 32.5) * 2 = -65",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ...
            self.assertEqual(
                self.product_avco_auto.standard_price,
                30,
                "standard_price for avco = (10 * 32.5 + 2 * 17.5) / (10 + 2) = 30: 2 delivered, 10 remaining + 2 added to stock",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            stock_return_picking_form = Form(
                self.env["stock.return.picking"].with_context(
            ...
            self.assertEqual(
                sol3.purchase_price,
                32.5,
                "purchase_price = 2 * 32.5 + 1 * 32.5) / (2 + 1) = 32.5",
            )
            self.assertEqual(
                sol3.margin,
                -32.5,
                "margin = SOL qty * sale price - purchase_price * qty_delivered = (0 - 32.5) * 1 = -32.5",
            )
