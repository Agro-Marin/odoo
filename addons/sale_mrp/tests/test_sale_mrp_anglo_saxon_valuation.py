...
class TestSaleMRPAngloSaxonValuation(TestSaleCommon, ValuationReconciliationTestCommon):
    ...
    def test_sale_mrp_kit_bom_cogs(self):
        ...
        self.component_b = self._create_product(
            name="Component B", is_storable=True, standard_price=4.00
        )
        self.component_bb = self._create_product(
            name="Component BB", is_storable=False, standard_price=5.00
        )
        self.kit_a = self._create_product(
            name="Kit A", is_storable=True, standard_price=0.00
        )
        self.kit_b = self._create_product(
            name="Kit B", is_storable=False, standard_price=0.00
        )
        pack_2 = self.env["uom.uom"].create(
            {
                "name": "Pack of 2",
                "relative_factor": 2,
                "relative_uom_id": self.kit_a.uom_id.id,
            }
        )
    ...
    def test_sale_mrp_anglo_saxon_variant(self):
        ...
        self.pt_attr1_v2 = self.product_template.attribute_line_ids[
            0
        ].product_template_value_ids[1]
        self.variant_1 = self.product_template._get_variant_for_combination(
            self.pt_attr1_v1
        )
        self.variant_2 = self.product_template._get_variant_for_combination(
            self.pt_attr1_v2
        )
        ...
        def check_cogs_entry_values(invoice, expected_value):
            ...
            aml_expense = aml.filtered(
                lambda l: l.display_type == "cogs" and l.debit > 0
            )
            aml_output = aml.filtered(
                lambda l: l.display_type == "cogs" and l.credit > 0
            )
            self.assertEqual(
                aml_expense.debit,
                expected_value,
    ...
    def test_anglo_saxo_return_and_credit_note(self):
        ...
        kit = self._create_product(
            name="Simple Kit", is_storable=True, standard_price=0
        )
        component = self._create_product(
            name="Compo A", is_storable=True, standard_price=0
        )
        ...
        self.assertEqual(
            cogs_aml.credit, 20, "Should be to the value of the returned component"
        )
    ...
    def test_kit_avco_fully_owned_and_delivered_invoice_post_delivery(self):
        ...
        compo01 = self._create_product(
            name="Compo 01", is_storable=True, standard_price=10
        )
        compo02 = self._create_product(
            name="Compo 02", is_storable=True, standard_price=20
        )
    ...
    def test_kit_avco_partially_owned_and_delivered_invoice_post_delivery(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            compo01, self.company_data["default_warehouse"].lot_stock_id, 1
        )
        self.env["stock.quant"]._update_available_quantity(
            compo02,
            self.company_data["default_warehouse"].lot_stock_id,
        ...
        self.env["stock.quant"]._update_available_quantity(
            compo02, self.company_data["default_warehouse"].lot_stock_id, 1
        )
    ...
    def test_anglo_saxo_kit_subkits(self):
        ...
        component_b = self._create_product(
            name="Component B", is_storable=True, standard_price=6.00
        )
        subkit_a = self._create_product(
            name="Subkit A", is_storable=True, standard_price=0.00
        )
        subkit_b = self._create_product(
            name="Subkit B", is_storable=True, standard_price=0.00
        )
        main_kit = self._create_product(
            name="Main kit", is_storable=True, standard_price=0.00
        )
    ...
    def test_sell_kit_invoice_before_delivery(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            compo01, warehouse.lot_stock_id, 1.0
        )
        self.env["stock.quant"]._update_available_quantity(
            compo02, warehouse.lot_stock_id, 2.0
        )
        self.env["mrp.bom"].create(
            {
                "type": "phantom",
