...
class TestSaleMrpProcurement(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env.ref("base.group_user").write(
            {"implied_ids": [(4, cls.env.ref("product.group_product_variant").id)]}
        )
    ...
    def test_sale_mrp_pickings(self):
        ...
        self.env.ref("stock.route_warehouse0_mto").active = True
        # Create warehouse
        self.customer_location = self.env["ir.model.data"]._xmlid_to_res_id(
        ...
        self.assertEqual(
            sale_order_so0.mrp_production_count, 2, "2 Mos for the 2 sale order line"
        )
        self.assertEqual(sale_order_so0.mrp_production_ids[0].product_qty, 1)
        self.assertEqual(
            sale_order_so0.mrp_production_ids[0].mrp_production_child_count, 1
        )
        self.assertRecordValues(
            sale_order_so0.mrp_production_ids[0]._get_children(),
            [
                {
