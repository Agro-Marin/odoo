...
class TestMultistepManufacturing(TestMrpCommon):
    ...
    def test_00_manufacturing_step_one(self):
        ...
        self.assertEqual(
            mo_procurement.location_dest_id.id,
            self.warehouse.lot_stock_id.id,
        ...
        so = self.env["sale.order"].create(
            {
                ...
                "line_ids": [
                    (0, 0, {"product_id": prod1.id, "product_qty": 1}),
    ...
    def test_mto_cancel_3_steps_mo(self):
        ...
        self.assertEqual(
            self.sale_order.picking_ids.move_ids.procure_method, "make_to_order"
        )
        mo = self.sale_order.mrp_production_ids
        self.assertTrue(mo)
        self.assertEqual(
            self.sale_order.picking_ids.move_ids.move_orig_ids, mo.move_finished_ids
        )
        mo.action_cancel()
