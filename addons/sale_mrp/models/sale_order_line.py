...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_transferred(self):
        ...
        for line in lines_by_stock_move:
            ...
            if relevant_bom:
                        ...
                        any(
                            ...
                            or (
                                ...
                                and float_compare(
                                    m.quantity,
                                    sum(
                                        sub_m.product_uom_id._compute_quantity_reconcile(
                                            sub_m.quantity, m.product_uom_id
                                        )
                                        for sub_m in m.returned_move_ids
                                        if sub_m.state == "done"
                                    ),
                                    precision_rounding=m.product_uom_id.rounding,
                                )
                                > 0
                            )
                            for m in moves
                ...
                order_qty = line.product_uom_id._compute_quantity_reconcile(
                    line.product_qty, relevant_bom.product_uom_id
                )
                qty_transferred = moves._get_kit_quantity(
                    line.product_id, order_qty, relevant_bom, filters
                )
                line.qty_transferred = (
                    relevant_bom.product_uom_id._compute_quantity_reconcile(
                        qty_transferred, line.product_uom_id
                    )
                )
    ...
    def _prepare_qty_transferred(self):
        ...
        for order_line in self:
            if order_line.qty_transferred_method == "stock_move":
                ...
                if relevant_bom:
                    ...
                    moves = order_line.move_ids.filtered(
                        lambda m: (
                            m.state == "done" and m.location_dest_usage != "inventory"
                        )
                    )
                    filters = {
                        # in/out perspective w/ respect to moves is flipped for sale order document
                    ...
                    order_qty = order_line.product_uom_id._compute_quantity_reconcile(
                        order_line.product_qty, relevant_bom.product_uom_id
                    )
                    qty_transferred = moves._get_kit_quantity(
                        order_line.product_id, order_qty, relevant_bom, filters
                    )
                    delivered_qties[order_line] += (
                        relevant_bom.product_uom_id._compute_quantity_reconcile(
                            qty_transferred, order_line.product_uom_id
                        )
                    )
