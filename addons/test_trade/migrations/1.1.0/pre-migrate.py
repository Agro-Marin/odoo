...
The XML now declares `test_trade.order`, but the record was loaded under
...
def migrate(cr, version):
    ...
    cr.execute(
        ...
        UPDATE ir_sequence s
           SET code = 'test_trade.order'
          FROM ir_model_data d
         WHERE d.model = 'ir.sequence'
           AND d.module = 'test_trade'
           AND d.name = 'seq_test_trade_order'
           AND d.res_id = s.id
    ...
    cr.execute(
        ...
        UPDATE ir_model_data
         ...
         WHERE model = 'ir.sequence'
           AND module = 'test_trade'
           AND name = 'seq_test_trade_order'
        """,
    )
