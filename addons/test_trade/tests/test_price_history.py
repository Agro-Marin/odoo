...
class TestPriceHistory(TestTradeOrderCase):
    ...
    def _confirmed_line(self, price, qty=1.0, partner=None, product=None):
        order = self.env["test_trade.order"].create(
            {"partner_id": (partner or self.partner).id}
        )
        line = self.env["test_trade.order.line"].create(
            {
                "order_id": order.id,
    ...
    def _wizard(self, target, partner=None):
        return self.env["test_trade.order.line.price.history"].create(
            {
                "line_id": target.id,
    ...
    def test_a_draft_line_is_not_in_the_statistics(self):
        order = self.env["test_trade.order"].create({"partner_id": self.partner.id})
    ...
    def test_opening_the_full_history_scopes_it_to_the_product(self):
        ...
        self.assertEqual(action["res_model"], "test_trade.order.line")
