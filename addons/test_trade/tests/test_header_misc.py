...
class TestHeaderMisc(TestTradeOrderCase):
    ...
    def test_action_view_business_doc(self):
        ...
        self.assertEqual(action["res_model"], "test_trade.order")
    ...
    def test_display_name_suffix_with_context(self):
        ...
        named = order.with_context(test_trade_order_show_partner_name=True)

        self.assertIn(self.partner.name, named.display_name)

    def test_rec_names_search_toggles_partner(self):
        Model = self.env["test_trade.order"]

        self.assertEqual(Model._rec_names_search, ["name"])
        self.assertIn(
            "partner_id.name",
            Model.with_context(
                test_trade_order_show_partner_name=True
            )._rec_names_search,
        )

    def test_import_templates_shape(self):
        templates = self.env["test_trade.order"].get_import_templates()
    ...
    def test_is_late_search_rejects_bad_operator(self):
        with self.assertRaises(ValueError):
            self.env["test_trade.order"].search([("is_late", ">", True)])
    ...
    def test_is_late_field_agrees_with_its_own_filter(self):
        ...
        found = self.env["test_trade.order"].search(
    ...
    def test_is_late_is_false_on_an_unsaved_order(self):
        self.assertFalse(self.env["test_trade.order"].new({}).is_late)
    ...
    def _confirmed_order_for(self, company, product):
        ...
        self.env["test_trade.order.line"].create(
            {
                "order_id": order.id,
