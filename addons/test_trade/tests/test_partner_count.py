...
class TestPartnerOrderCount(TransactionCase):
    """Cover ``trade``'s res.partner helpers.
    ...
    def _make_orders(self, partner, count):
        return self.env["test_trade.order"].create(
    ...
    def test_count_rolls_up_through_the_partner_tree(self):
        ...
        self.assertEqual(self.child.test_trade_order_count, 3)
        self.assertEqual(self.parent.test_trade_order_count, 5, "3 child + 2 own")
        self.assertEqual(
            self.grandparent.test_trade_order_count,
            6,
    ...
    def test_archived_children_still_count(self):
        ...
        self.assertEqual(self.archived_child.test_trade_order_count, 4)
        self.assertEqual(self.parent.test_trade_order_count, 4)
        self.assertEqual(self.grandparent.test_trade_order_count, 4)
    ...
    def test_count_is_zero_without_the_group(self):
        ...
        count = self.child.with_user(portal).sudo().test_trade_order_count
