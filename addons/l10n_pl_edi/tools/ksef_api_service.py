...
class KsefApiService:
    ...
    def _send_request(self, method, endpoint, is_auth_retry=False, **kwargs):
        """
        ...
        try:
            ...
            if response.status_code == 401 and not is_auth_retry:
                ...
                return self._send_request(
                    method, endpoint, is_auth_retry=True, **kwargs
                )
    ...
    def refresh_access_token(self):
        ...
        try:
            ...
            if not new_access_token:
                raise UserError(
                    self.env._(
                        "Failed to retrieve a new access token from KSeF response."
                    )
                )
    ...
    def query_invoice_metadata(self, query_criteria, page_size=100, page_offset=0):
        ...
        try:
            response = self._send_request(
                "POST", endpoint, json=query_criteria, params=params
            )
            return response.json()
