...
class XadesSigner:
    ...
    def __init__(self, key_pem, cert_pem, private_key_password=None):
        self.private_key = serialization.load_pem_private_key(
            key_pem, password=private_key_password
        )
    ...
    def _prepare_qualifying_properties(self, signature_node, sig_id, props_id):
        NS_DS = "http://www.w3.org/2000/09/xmldsig#"
        NS_XADES = "http://uri.etsi.org/01903/v1.3.2#"

        object_node = etree.SubElement(signature_node, etree.QName(NS_DS, "Object"))
        qualifying_props_node = etree.SubElement(
            object_node,
            etree.QName(NS_XADES, "QualifyingProperties"),
            Target=f"#{sig_id}",
        )
        signed_props_node = etree.SubElement(
            qualifying_props_node,
            etree.QName(NS_XADES, "SignedProperties"),
            Id=props_id,
        )
        signed_sig_props_node = etree.SubElement(
            signed_props_node, etree.QName(NS_XADES, "SignedSignatureProperties")
        )

        now = datetime.now(UTC)
        etree.SubElement(
            signed_sig_props_node, etree.QName(NS_XADES, "SigningTime")
        ).text = now.strftime("%Y-%m-%dT%H:%M:%SZ")

        signing_cert_node = etree.SubElement(
            signed_sig_props_node, etree.QName(NS_XADES, "SigningCertificate")
        )
        cert_node = etree.SubElement(signing_cert_node, etree.QName(NS_XADES, "Cert"))
        cert_digest_node = etree.SubElement(
            cert_node, etree.QName(NS_XADES, "CertDigest")
        )
        etree.SubElement(
            cert_digest_node,
            etree.QName(NS_DS, "DigestMethod"),
        ...
        etree.SubElement(
            cert_digest_node, etree.QName(NS_DS, "DigestValue")
        ).text = base64.b64encode(cert_digest.finalize()).decode("utf-8")

        issuer_serial_node = etree.SubElement(
            cert_node, etree.QName(NS_XADES, "IssuerSerial")
        )
        etree.SubElement(
            issuer_serial_node, etree.QName(NS_DS, "X509IssuerName")
        ).text = self.cert.issuer.rfc4514_string()
        etree.SubElement(
    ...
    def sign_authentication_challenge(self, challenge_code, nip):
        ...
        is_subject = (
            clean_nip in subject_str or "VATPL" in subject_str or "PNOPL" in subject_str
        )
        identifier_type = (
            "certificateSubject" if is_subject else "certificateFingerprint"
        )
        ...
        signed_info_node = etree.SubElement(
            signature_node, etree.QName(NS_DS, "SignedInfo")
        )
        etree.SubElement(
            signed_info_node,
            etree.QName(NS_DS, "CanonicalizationMethod"),
            Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#",
        )
        etree.SubElement(
            signed_info_node,
            etree.QName(NS_DS, "SignatureMethod"),
            Algorithm=sig_alg_uri,
        )

        ref1 = etree.SubElement(
            signed_info_node,
            etree.QName(NS_DS, "Reference"),
            Id=f"reference-{uuid.uuid4()}",
            URI="",
        )
        etree.SubElement(
            etree.SubElement(ref1, etree.QName(NS_DS, "Transforms")),
            etree.QName(NS_DS, "Transform"),
        ...
        etree.SubElement(
            etree.SubElement(ref2, etree.QName(NS_DS, "Transforms")),
            etree.QName(NS_DS, "Transform"),
            Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#",
        )
        etree.SubElement(
        ...
        signed_info_c14n = etree.tostring(
            signed_info_node, method="c14n", exclusive=True, with_comments=False
        )

        if is_rsa:
            signature_val = self.private_key.sign(
        ...
        etree.SubElement(
            signature_node, etree.QName(NS_DS, "SignatureValue")
        ).text = base64.b64encode(signature_val).decode("utf-8")
        ...
        etree.SubElement(
            x509_data, etree.QName(NS_DS, "X509Certificate")
        ).text = "".join(cert_pem.decode("utf-8").splitlines()[1:-1])

        object_node = signature_node.xpath("./ds:Object", namespaces=nsmap)[0]
        signature_node.append(object_node)

        return etree.tostring(
            root, xml_declaration=True, encoding="utf-8", standalone="no"
        ).decode("utf-8")
