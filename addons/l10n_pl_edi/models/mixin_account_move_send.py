...
class AccountMoveSend(models.AbstractModel):
    ...
    def _call_web_service_before_invoice_pdf_render(self, invoices_data):
            ...
            for move in moves:
                invoices_data[move]["error"] = {
                    "error_title": self.env._(
                        "Error when sending invoices to the KSeF"
                    ),
        ...
        def is_ksef(move):
            return (
                "pl_ksef" in invoices_data[move].get("extra_edis", [])
                and not move.l10n_pl_edi_status
            )
        ...
        for company, moves in moves_by_company.items():
            ...
            for move in moves:
                if move.invoice_date > fields.Date.context_today(self):
                    set_error(
                        move,
                        self.env._("The move was skipped because it is future-dated"),
                    )
                    continue
    ...
    def _l10n_pl_edi_try_status_fetch_from_ksef(self, invoices_data):
        moves_to_update = (
            self.env["account.move"]
            .union(*invoices_data)
            .filtered(lambda m: m.l10n_pl_edi_ref and not m.l10n_pl_edi_number)
        )
        attempts = 5
        ...
        if moves_to_update:
            ...
            self.env.ref(
                "l10n_pl_edi.cron_auto_checks_the_polish_invoice_status"
            )._trigger()
