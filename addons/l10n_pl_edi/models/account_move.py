...
class AccountMove(models.Model):
    ...
    def _l10n_pl_edi_check_mandatory_fields(self):
        ...
        if not company.vat:
            errors["vat_missing"] = {
                "message": self.env._(
                    "You must have a VAT number to be able to issue e-invoices\n"
                )
            }
        if not company.country_id or not company.street or not company.city:
            errors["address_problem"] = {
                "message": self.env._("Please complete the address of your company\n")
            }
        return errors
    ...
    def _l10n_pl_edi_get_ksef_invoice_type(self):
        ...
        # 2. Handle Sales Invoices
            ...
            has_dp_lines = any(
                line._get_downpayment_lines() for line in self.invoice_line_ids
            )

            if has_dp_lines:
                # Check for Settlement (ROZ): Contains Regular lines AND Negative Downpayment lines
                has_regular_lines = any(
                    not line._get_downpayment_lines() for line in self.invoice_line_ids
                )
                has_deducted_dp = any(
    ...
    def _l10n_pl_edi_get_xml_values(self):
        ...
        def get_address(partner):
            return re.sub(
                r"\n+",
                r" ",
                partner.with_context(without_country_name=True)._display_address(True),
            )

        def get_tags(code):
            return self.env["account.account.tag"]._get_tax_tags(
                code, self.env.ref("base.pl").id
            )

        def get_tag_names(line):
            return line.tax_tag_ids.with_context(lang="en_US").mapped(
                lambda x: re.sub(r"^[+-]", r"", x.name or "")
            )

        def get_amounts_from_tag(tax_tag_string):
            lines = self.line_ids.filtered(
                lambda line: line.tax_tag_ids & get_tags(tax_tag_string)
            )
        ...
        def get_amounts_from_tag_in_PLN_currency(tax_group_id):
            conversion_line = (
                self.invoice_line_ids.sorted(
                    lambda line: abs(line.balance), reverse=True
                )[0]
                if self.invoice_line_ids
                else None
            )
            conversion_rate = (
                abs(conversion_line.balance / conversion_line.amount_currency)
                if self.currency_id != self.env.ref("base.PLN") and conversion_line
                else 1
            )
        ...
        for group in self.tax_totals.get("groups_by_subtotal", {}).values():
            tax_rate_str = str(
                int(
                    (
                        group["tax_group_amount_type"] == "percent"
                        and group["tax_group_amount"]
                    )
                    or 0
                )
            )
        ...
        if "KOR" in ksef_type:
            ...
            correction_info = {
                ...
                "DataWystFaKorygowanej": origin.invoice_date
                if origin
        ...
        return {
            ...
            "special_transactions": {"OSS_Base", "OSS_Tax", "Triangular Sale"}
            & invoice_tag_names,
            "triangular_transaction": "1"
            if "Triangular Sale" in invoice_tag_names
            else "2",
            "prefiks_podatnika": bool(
                {"K_21", "K_12", "Triangular Sale"} & invoice_tag_names
            ),
            "reverse_charge": any(
                invoice_line_vals["P_12"] == "oo"
    ...
    def _l10n_pl_edi_generate_qr_link(self):
        ...
        if self.l10n_pl_edi_attachment_file:
            mode = (
                ...
                .sudo()
                .get_param("l10n_pl_edi_ksef.mode", "prod")
            )
            base_link = (
                "https://qr.ksef.mf.gov.pl/invoice/"
                if mode == "prod"
                else "https://qr-test.ksef.mf.gov.pl/invoice/"
            )
            return (
                f"{base_link}"
    ...
    def _l10n_pl_edi_get_status_mapping(self):
        ...
        return {
            100: (
                "sent",
                self.env._(
                    "KSeF Status: Invoice accepted for further processing (Code: 100)."
                ),
            ),
            150: (
                "sent",
                self.env._("KSeF Status: Processing in progress (Code: 150)."),
            ),
            200: (
                "accepted",
                self.env._("KSeF Status: Success (Code: 200). Invoice accepted."),
            ),
            405: (
                "rejected",
                self.env._("KSeF Status: Rejected (Code: 405). Processing canceled."),
            ),
            410: (
                "rejected",
                self.env._(
                    "KSeF Status: Rejected (Code: 410). Incorrect scope of permissions."
                ),
            ),
            415: (
                "rejected",
                self.env._(
                    "KSeF Status: Rejected (Code: 415). It is not possible to send an invoice with an attachment."
                ),
            ),
            430: (
                "rejected",
                self.env._(
                    "KSeF Status: Rejected (Code: 430). Invoice file verification error."
                ),
            ),
            435: (
                "rejected",
                self.env._("KSeF Status: Rejected (Code: 435). File decryption error."),
            ),
            440: (
                "rejected",
                self.env._("KSeF Status: Rejected (Code: 440). Duplicate invoice."),
            ),
            450: (
                "rejected",
                self.env._(
                    "KSeF Status: Rejected (Code: 450). Invoice document semantics verification error."
                ),
            ),
            500: (
                "rejected",
                self.env._("KSeF Status: Error (Code: 500). Unknown error."),
            ),
        }
    ...
    def action_l10n_pl_edi_update_invoice_status(self):
        ...
        def ask_status_api():
            ...
            try:
                ...
                return {
                    "l10n_pl_edi_header": self.env._(
                        "Failed to check KSeF status: %s", e
                    ),
                }
            ...
            status_description = status_info.get(
                "description", self.env._("No description provided.")
            )
    ...
    def action_l10n_pl_edi_get_invoice_UPO(self):
            ...
            self.l10n_pl_edi_upo_id = self.env["ir.attachment"].create(
                {
                    ...
                    "description": self.env._(
                        "KSeF Official Confirmation of Receipt (UPO)"
                    ),
                    "type": "binary",
        ...
        self.sudo().with_context(no_new_invoice=True).message_post(
            body=self.env._(
                "The KSeF Official Confirmation of Receipt (UPO) has been downloaded and attached."
            ),
    ...
    def _get_fields_to_detach(self):
        # EXTENDS account
        return super()._get_fields_to_detach() + [
    ...
    def l10n_pl_edi_get_ksef_bill_vals_from_xml(self, xml_content):
        ...
        def get_tax(tax_name):
            if xml_id := p12_to_tax_xml_id_map.get(tax_name):
                if tax := self.env["account.chart.template"].ref(
                    xml_id, raise_if_not_found=False
                ):
                    return tax
                raise UserError(
                    self.env._(
                        "Purchase tax corresponding to '%s' required for the KSeF import was not found in the system.",
                        tax_name,
                    )
                )
        ...
        def parse_fa3_bill_xml(xml_content):
            ...
            invoice_date_due = get_value(
                invoice_node, "{*}Platnosc/{*}TerminPlatnosci/{*}Termin"
            )
            ...
            for line_node in move_line_nodes:
                ...
                if P_9A := get_value(line_node, "{*}P_9A"):
                    ...
                    if tax_amount is None:
                        raise UserError(
                            self.env._(
                                "Tax corresponding to '%s' required to derive the net unit price from gross price during KSeF import could not be interpreted.",
                                tax_name,
                            )
                        )
        ...
        def get_ksef_bill_vals(data):
            ...
            if not currency:
                raise UserError(
                    self.env._(
                        "Currency '%s' from the KSeF bill was not found.",
                        data["currency_code"],
                    )
                )
            ...
            for line in data["lines"]:
                ...
                mapped_tax = get_tax(tax_name) or get_tax_fallback(
                    tax_name, fiscal_position
                )
                if not mapped_tax:
                    raise UserError(
                        self.env._(
    ...
    def _cron_l10n_pl_edi_download_bills(self):
        for company in self.env["res.company"].search(
            ...
            blocking_error = self.with_company(
                company
            )._l10n_pl_edi_download_bills_from_ksef()
            if blocking_error:
                break
    ...
    def _l10n_pl_edi_download_bills_from_ksef(self):
        ...
        blocking_error = self._get_bills_metadata(service)
    ...
    def _get_bills_metadata(self, service):
        ...
        while has_more:
            ...
            if response.get("error"):
                blocking_error = self._handle_download_bills_from_ksef_error(
                    response["error"]
                )
                break
            ...
            invoice_numbers.extend(
                invoice["ksefNumber"] for invoice in response["invoices"]
            )
        ...
        if to_process := [
            invoice_nr
            for invoice_nr in invoice_numbers
            if invoice_nr not in already_processed
        ]:
    ...
    def _get_bills_data(self, service, bills_to_fetch):
        for bill in bills_to_fetch:
            invoice_nr = bill.l10n_pl_edi_number
            response = service.get_invoice_by_ksef_number(invoice_nr)
            try:
                if response.get("error"):
                    return self._handle_download_bills_from_ksef_error(
                        response["error"]
                    )

                bill_data = self.l10n_pl_edi_get_ksef_bill_vals_from_xml(
                ...
                with mute_logger("odoo.db.cursor"), self.env.cr.savepoint():
                    ...
                    bill.message_post(
                        body=self.env._("Bill Fetched Successfully from KSeF")
                    )
            except UserError as e:
