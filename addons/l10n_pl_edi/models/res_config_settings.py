    ...
    def _l10n_pl_edi_ksef_authenticate(self):
        ...
        if not vat or not vat.startswith("PL"):
            raise ValidationError(
                self.env._("A polish VAT number must be set on your company.")
            )
        if not self.l10n_pl_edi_certificate:
            raise ValidationError(
                self.env._(
                    "Please set up a valid KSeF Certificate, with its Private Key set"
                )
            )
        ...
        key_bytes = base64.b64decode(
            self.l10n_pl_edi_certificate.private_key_id.pem_key
        )
        cert_bytes = base64.b64decode(self.l10n_pl_edi_certificate.pem_certificate)
        private_key_password = (
            self.l10n_pl_edi_certificate.private_key_id.password or ""
        ...
        if not access_token or not refresh_token:
            raise ValidationError(
                self.env._("Failed to retrieve access or refresh tokens.")
            )
