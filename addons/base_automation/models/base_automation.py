...
class BaseAutomation(models.Model):
    ...
    def _check_trigger(self):
        for automation in self:
            if (
                ...
                raise exceptions.ValidationError(
                    _(
                        "Mail event can not be configured on model %s. Only models with discussion feature can be used.",
    ...
    def _check_trigger_state(self):
        for automation in self:
            ...
            if warning_actions:
                raise exceptions.ValidationError(
                    _(
                        ...
                        children=", ".join(warning_actions.mapped("name")),
                    ),
                )
            ...
            if automation.trigger == "on_change" and no_code_actions:
                raise exceptions.ValidationError(
                    _(
                        '"On live update" automation rules can only be used with "Execute Python Code" action type.',
                    ),
                )
            ...
            if automation.trigger == "on_unlink" and mail_actions:
                raise exceptions.ValidationError(
                    _(
                        "Email, follower or activity action types cannot be used when deleting records, "
    ...
    def action_manual_trigger(self):
        ...
        if self.trigger != "on_hand":
            raise exceptions.ValidationError(
                _(
                    "This automation is not configured for manual triggering. "
                    "Please set trigger to 'Manual trigger'.",
                ),
            )
        ...
        if active_model != self.model_name:
            raise exceptions.ValidationError(
                _(
                    "This automation is configured for model '%(expected)s' but you are trying to use it on '%(actual)s'.",
