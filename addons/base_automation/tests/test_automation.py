        ...
        automation = self.env["base.automation"].create(
            {
                ...
                "model_id": model.id,
                "trigger_field_ids": [
                    (6, 0, [self.env.ref("base.field_ir_filters__name").id])
                ],
            }
        )
        action = self.env["ir.actions.server"].create(
        ...
        automation = self.env["base.automation"].create(
            {
                ...
                "trigger": "on_write",
                "model_id": model.id,
                "trigger_field_ids": [(6, 0, [email_field.id])],
            }
        )
        action = self.env["ir.actions.server"].create(
            {
        ...
        automation = self.env["base.automation"].create(
            {
                ...
                "trigger": "on_write",
                "model_id": model.id,
                "trigger_field_ids": [(6, 0, [name_field.id, email_field.id])],
            }
        )
        action = self.env["ir.actions.server"].create(
            {
        ...
        automation = self.env["base.automation"].create(
            {
                ...
                "model_id": model.id,
                "trigger_field_ids": [
                    (6, 0, [self.env.ref("base.field_res_partner__email").id])
                ],
            }
        )
        action = self.env["ir.actions.server"].create(
