...
class TestPosFlow(TestInPosBase):
    ...
    def test_invoice_order_after_session_closed(self):
            ...
            pos_order_going_to_invoice = self._create_order(
                {
                    ...
                    "payments": [
                        (self.bank_pm1, 630.0)
                    ],  # Payment of 630 through bank payment method
                }
            )
            ...
            pos_order_with_invoice = self._create_order(
                {
                    ...
                    "payments": [
                        (self.bank_pm1, 945.0)
                    ],  # Payment of 945 through bank payment method
        ...
        self.assertFalse(
            pos_order_going_to_invoice.reversed_move_ids,
            "Order with 'to_invoice' = False should have no reversal moves after session is closed.",
        )
        self.assertFalse(
            pos_order_with_invoice.reversed_move_ids,
        ...
        reversal_moves = self.env["account.move"].search(
            [("reversed_pos_order_id", "=", pos_order_going_to_invoice.id)]
        )
        self.assertEqual(
            pos_order_going_to_invoice.reversed_move_ids,
            reversal_moves,
