...
class TestEfakturCoretax(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.company_data["company"].phone_ids = [
            Command.create({"number": "12345", "type": "landline"})
        ]
    ...
    def _verify_error_message(self, ex, err_count, messages=[]):
        ...
        )  # -1 because the first line of error message ("Unable to Download ...") is always there
    ...
    def test_download_efaktur_invalid_invoice(self):
                ...
                "invoice_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "price_unit": 100000,
                            "quantity": 1,
                            "tax_ids": [self.luxury_tax.id, self.non_luxury_tax.id],
                        },
                    )
                ],
            }
                ...
                "invoice_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "price_unit": 100000,
                            "quantity": 1,
                            "tax_ids": [self.non_luxury_tax.id, self.stlg_tax.id],
                        },
                    )
                ],
            }
        ...
        with self.assertRaisesRegex(
            ValidationError, r".*does not allow 0% \(Zero-rated or Exempt\) taxes\."
        ):
        ...
        with self.assertRaisesRegex(
            ValidationError, r".*must always have tax amount 0"
        ):
    ...
    def test_efaktur_xml_partner_edit(self):
        ...
        result_tree = etree.fromstring(
            out_invoice.l10n_id_coretax_document._generate_efaktur_invoice()
        )
    ...
    def test_efaktur_xml_multiple_invoices(self):
        ...
        result_tree = etree.fromstring(
            invoices.l10n_id_coretax_document._generate_efaktur_invoice()
        )
        expected_tree = self.with_applied_xpath(
            etree.fromstring(self.sample_xml),
            """
    ...
    def test_efaktur_tax_include(self):
        ...
        result_tree = etree.fromstring(
            move.l10n_id_coretax_document._generate_efaktur_invoice()
        )
        expected_tree = self.with_applied_xpath(
            etree.fromstring(self.sample_xml),
            """
    ...
    def test_efaktur_download_separate(self):
        ...
        self.assertNotEqual(
            invoices[0].l10n_id_coretax_document, invoices[1].l10n_id_coretax_document
        )
    ...
    def test_efaktur_download_mismatch_flow(self):
        ...
        document_to_edit = self.env["l10n_id_efaktur_coretax.document"].browse(
            document_ids
        )
