...
class EfakturDocument(models.Model):
    ...
    def _create_efaktur_xml_attachment(self, regenerate=False):
        ...
        no_trx_code_entries = self.invoice_ids.filtered(
            lambda x: not x.l10n_id_kode_transaksi
        )
        non_invoice_entries = self.invoice_ids.filtered(
            ...
            attachment = self.env["ir.attachment"].create(
                {
                    ...
                    "name": "efaktur_%s.xml"
                    % (
                        fields.Datetime.to_string(fields.Datetime.now()).replace(
                            " ", "_"
                        )
                    ),
                    "type": "binary",
    ...
    def _generate_efaktur_invoice(self):
        ...
        xml_content = self.env["ir.qweb"]._render(
            ...
            {"data": invoice_data, "TIN": self.company_id.vat},
        )
        return etree.tostring(
            cleanup_xml_node(
                xml_content, remove_blank_text=False, remove_blank_nodes=False
            ),
