...
class AccountMoveLine(models.Model):
    ...
    def _l10n_id_coretax_build_invoice_line_vals(self, vals):
        ...
        luxury_tax_group = ChartTemplate.ref(
            "l10n_id_tax_group_luxury_goods", raise_if_not_found=False
        )
        stlg_tax_group = ChartTemplate.ref(
            "l10n_id_tax_group_stlg", raise_if_not_found=False
        )
        zero_tax_group_0 = ChartTemplate.ref(
            "l10n_id_tax_group_0", raise_if_not_found=False
        )
        zero_tax_group_exempt = ChartTemplate.ref(
            "l10n_id_tax_group_exempt", raise_if_not_found=False
        )
        zero_tax_groups = {zero_tax_group_0, zero_tax_group_exempt} - {False}

        zero_tax = self.tax_ids.filtered(
            lambda tax: tax.tax_group_id in zero_tax_groups
        )
        luxury_tax = self.tax_ids.filtered(
            lambda tax: tax.tax_group_id == luxury_tax_group
        )
        ...
        line_val = {
            ...
            "Code": product.l10n_id_product_code.code
            or self.env.ref("l10n_id_efaktur_coretax.product_code_000000_goods").code,
            "Name": product.name,
            "Unit": self.product_uom_id.l10n_id_uom_code.code,
            "Price": idr.round(tax_res["total_excluded"]),
            "Qty": self.quantity,
            "TotalDiscount": idr.round(
                self.discount * tax_res["total_excluded"] * self.quantity / 100
            ),
            "TaxBase": idr.round(self.price_subtotal),  # DPP
            "VATRate": 12,
            "STLGRate": stlg_tax.amount if stlg_tax else 0.0,
        }
        if self.move_id.l10n_id_kode_transaksi == "01" or (
            not regular_tax and not zero_tax
        ):
            line_val["OtherTaxBase"] = line_val["TaxBase"]
        else:
            line_val["OtherTaxBase"] = idr.round(self.price_subtotal * 11 / 12)

        line_val["VAT"] = idr.round(
            line_val["OtherTaxBase"] * line_val["VATRate"] / 100
        )
        line_val["STLG"] = idr.round(
            line_val["STLGRate"] * line_val["OtherTaxBase"] / 100
        )
        ...
        for field in numerical_fields:
            line_val[field] = float_repr(
                line_val[field], precision_digits=self.currency_id.decimal_places
            )
