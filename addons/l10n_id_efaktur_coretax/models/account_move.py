...
class AccountMove(models.Model):
    ...
    l10n_id_coretax_facility_info_07 = fields.Selection(
        [
            ...
            (
                "TD.01106",
                "6 - untuk Kontraktor Perjanjian Karya Pengusahaan Pertambangan Batubara Generasi I",
            ),
            (
                "TD.01107",
                "7 - untuk Penyerahan bahan bakar minyak untuk Kapal Angkutan Laut Luar Negeri",
            ),
            (
                "TD.01108",
            ...
            (
                "TD.01110",
                "10 - untuk BKP tertentu yang bersifat strategis berupa anode slime",
            ),
            (
                "TD.01111",
            ...
            (
                "TD.01115",
                "15 - Penyerahan Barang dan Jasa Dalam Rangka Penanganan Pandemi COVID-19 (PMK 239/PMK. 03/2020)",
            ),
            (
                "TD.01116",
            ...
            (
                "TD.01122",
                "22 - BKP strategis tertentu dalam bentuk anode slime dan emas butiran",
            ),
    ...
    l10n_id_coretax_custom_doc_month_year = fields.Date(
        string="Custom Document Month and Year"
    )
    ...
    def _compute_kode_transaksi(self):
        for move in self:
            move.l10n_id_kode_transaksi = (
                move.commercial_partner_id.l10n_id_kode_transaksi
            )
    ...
    def _check_tax_groups(self):
        ...
        for move in self:
            ...
            luxury_group = (
                self.env["account.chart.template"]
                .with_company(move.company_id.id)
                .ref("l10n_id_tax_group_luxury_goods", raise_if_not_found=False)
            )
            zero_group = (
                self.env["account.chart.template"]
                .with_company(move.company_id.id)
                .ref("l10n_id_tax_group_0", raise_if_not_found=False)
            )
            exempt_group = (
                self.env["account.chart.template"]
                .with_company(move.company_id.id)
                .ref("l10n_id_tax_group_exempt", raise_if_not_found=False)
            )
            stlg_group = (
                self.env["account.chart.template"]
                .with_company(move.company_id.id)
                .ref("l10n_id_tax_group_stlg", raise_if_not_found=False)
            )
    ...
    def download_efaktur(self):
        ...
        for record in self:
            ...
            if record.l10n_id_kode_transaksi == "07":
                if not (
                    record.l10n_id_coretax_add_info_07
            ...
            if record.l10n_id_kode_transaksi == "08":
                if not (
                    record.l10n_id_coretax_add_info_08
        ...
        if not self.l10n_id_coretax_document:
            self.l10n_id_coretax_document = self.env[
                "l10n_id_efaktur_coretax.document"
            ].create(
                {
        ...
        elif len(self.l10n_id_coretax_document) > 1 or set(
        ...
        vals.update(
            {
                ...
                "CustomDocMonthYear": (
                    self.l10n_id_coretax_custom_doc_month_year
                    and self.l10n_id_coretax_custom_doc_month_year.strftime("%m%Y")
                )
                or "",
                "FacilityStamp": "",
                "RefDesc": self.name,
                "SellerIDTKU": self.company_id.vat
                + (self.company_id.partner_id.l10n_id_tku or "000000"),
                "BuyerDocument": l10n_id_buyer_document_type_mapping_to_xml.get(
                    partner.l10n_id_buyer_document_type,
                    partner.l10n_id_buyer_document_type,
                ),
                "BuyerTin": partner.vat
                if partner.l10n_id_buyer_document_type == "TIN"
                else "0000000000000000",
                "BuyerCountry": COUNTRY_CODE_MAP.get(partner.country_id.code),
                "BuyerDocumentNumber": partner.l10n_id_buyer_document_number
                if partner.l10n_id_buyer_document_type != "TIN"
                else "",
