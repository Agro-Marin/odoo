...
class SaleOrderLine(models.Model):
    ...
    def _get_action_per_item(self):
        ...
        if self.env.user.has_group("stock.group_stock_user"):
            stock_move_read_group = self.env["stock.move"]._read_group(
                [("sale_line_id", "in", self.ids)], ["sale_line_id"], ["id:array_agg"]
            )
            stock_move_ids_per_sol = {
                sale_line.id: ids for sale_line, ids in stock_move_read_group
            }
        for sol in self:
            ...
            if not sol.is_service and stock_move_ids:
                action_per_sol[sol.id] = (
                    stock_move_action,
                    stock_move_ids[0] if len(stock_move_ids) == 1 else False,
                )
