...
class ResourceCalendarLeaves(models.Model):
            ...
            for calendar_id in resource_calendars.ids:
                ...
                if calendar_data is None:
                    ...
                    calendar_data.update(
                        date_from=min(
                            date_from_min.replace(tzinfo=UTC),
                            calendar_data["date_from"],
                        ),
                        date_to=max(
                            date_to_max.replace(tzinfo=UTC), calendar_data["date_to"]
                        ),
        ...
        for (
            ...
            for leave in cal_attendance_intervals_params_entry["leaves"]:
                ...
                for date_from, date_to, _dummy in work_hours_data:
                    if date_to > leave.date_from.replace(
                        tzinfo=UTC
                    ) and date_from < leave.date_to.replace(tzinfo=UTC):
                        tmp_start = max(date_from, leave.date_from.replace(tzinfo=UTC))
                        tmp_end = min(date_to, leave.date_to.replace(tzinfo=UTC))
                        results[calendar_id][leave.id][tmp_start.date()] += (
                            tmp_end - tmp_start
                        ).total_seconds() / 3600
    ...
    def _timesheet_create_lines(self):
        ...
        employee_ids_all = [
            _id for __, employees in employees_groups for _id in employees._ids
        ]
        ...
        holidays_by_employee = {
            employee.id: [
                ...
                for date_from, date_to in zip(date_from_list, date_to_list, strict=True)
            ]
            ...
            for employee in employees:
                ...
                for index, (day_date, work_hours_count) in enumerate(work_hours_list):
                    if not holidays or all(
                        not (date_from <= day_date and date_to >= day_date)
                        for date_from, date_to in holidays
                    ):
                        vals_list.append(
        ...
        for leave in self:
            if not leave.calendar_id:
                for calendar_id, calendar_employees in mapped_employee.items():
                    ...
                    vals_list = get_timesheets_data(
                        calendar_employees, work_hours_list, vals_list
                    )
            else:
                employees = mapped_employee.get(
                    leave.calendar_id.id, self.env["hr.employee"]
                )
    ...
    def _generate_public_time_off_timesheets(self, employees):
        ...
        timesheet_dates_per_employee_id = {
            employee.id: date for employee, date in timesheet_read_group
        }
        for leave in self:
            for employee in employees:
                if (
    ...
    def _get_overlapping_hr_leaves(self, domain=None):
        ...
        if self.calendar_id:
            leave_domain += [
                ("resource_calendar_id", "in", [False, self.calendar_id.id])
            ]
    ...
    def write(self, vals):
        ...
        if date_from or date_to or "calendar_id" in vals:
            global_time_off_updated = self.filtered(
                lambda r: (
                    (date_from is not None and r.date_from != date_from)
                    or (date_to is not None and r.date_to != date_to)
                    or (calendar_id is None or r.calendar_id.id != calendar_id)
                )
            )
            ...
            if calendar_id:
                for gto in global_time_off_updated:
                    domain = (
                        []
                        if gto.calendar_id
                        else [("resource_calendar_id", "!=", calendar_id)]
                    )
