...
class TestTimesheetHolidays(TestCommonTimesheet):
    def setUp(self):
        ...
        self.Allocations = self.env["hr.leave.allocation"].with_context(
            mail_create_nolog=True, mail_notrack=True
        )
    ...
    def test_validate_worked_leave(self):
        ...
        self.assertEqual(
            len(holiday.timesheet_ids),
            0,
            "No timesheet should be created for a leave of worked time type",
        )
    ...
    def test_timesheet_time_off_including_public_holiday(self):
        ...
        self.env["resource.calendar.leaves"].create(
            {
                ...
                "date_from": datetime(2022, 1, 26, 7, 0, 0, 0),
        ...
        self.assertEqual(
            len(holiday.timesheet_ids),
            4,
            "4 timesheets should be generated for this time off.",
        )
        ...
        self.assertEqual(
            len(timesheets.filtered("global_leave_id")),
            1,
            "1 timesheet should be linked to global leave",
        )

    def test_delete_timesheet_after_new_holiday_covers_whole_timeoff(self):

        leave_start_datetime = datetime(2022, 1, 31, 7, 0, 0, 0)
        ...
        self.assertFalse(
            time_off.timesheet_ids, "0 timesheet should remain for this time off."
        )
    ...
    def test_timeoff_task_creation_with_holiday_leave(self):
        ...
        new_timesheet_count = self.env["account.analytic.line"].search_count(
            [("holiday_id", "!=", False)]
        )
    ...
    def test_timesheet_timeoff_flexible_employee(self):
        ...
        self.assertEqual(
            len(timesheet), 3, "Three timesheets should be created for each leave day"
        )
        self.assertEqual(
            sum(timesheet.mapped("unit_amount")),
            24,
    ...
    def test_multi_create_timesheets_from_calendar(self):
        ...
        timesheets = (
            ...
            .with_context(timesheet_calendar=True)
            .create(
                [
                    {
        ...
        self.assertEqual(
            len(timesheets),
            2,
            "Two leaves should have been created: one for each employee",
        )
        ...
        time_off = self.Requests.with_user(self.user_employee).create(
            {
                ...
                "employee_id": self.empl_employee.id,
                "holiday_status_id": self.hr_leave_type_with_ts.id,
                "request_date_from": datetime(2025, 7, 12),
    ...
    def test_timeoff_validation_fully_flexible_employee(self):
        ...
        self.assertEqual(
            time_off.state,
            "validate",
            "The time off for a fully flexible employee should be validated",
        )
