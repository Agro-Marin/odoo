...
class CrmLead(models.Model):
    ...
    iap_enrich_done = fields.Boolean(
    ...
    def _compute_show_enrich_button(self):
        for lead in self:
            if (
                not lead.active
                or not lead.email_from
                or lead.email_state == "incorrect"
                or lead.iap_enrich_done
                or lead.reveal_id
    ...
    def iap_enrich(self, *, batch_size=50):
        ...
        def process_leads(leads):
            ...
            try:
                ...
                _logger.info(
                    "Batch of %s leads successfully enriched", len(lead_emails)
                )
    ...
    def _merge_get_fields_specific(self):
        return {
            **super()._merge_get_fields_specific(),
            "iap_enrich_done": lambda fname, leads: any(
                lead.iap_enrich_done for lead in leads
            ),
        }
