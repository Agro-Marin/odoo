...
class ProductProduct(models.Model):
    ...
    def _read_grouping_sets(self, domain, grouping_sets, aggregates=(), order=None):
        ...
        base_result = super()._read_grouping_sets(
            domain, grouping_sets, base_aggregates, order
        )
        ...
        for grouping_spec, grouping in zip(grouping_sets, base_result, strict=False):
            ...
            for *other, records in grouping:
                for index, spec in enumerate(
                    itertools.chain(grouping_spec, aggregates)
                ):
                    if spec in self._SPECIAL_SUM_AGGREGATES:
                        field_name = spec.split(":")[0]
                        other.insert(index, sum(records.mapped(field_name)))
    ...
    def _compute_product_margin_fields_values(self):
        ...
        res = {
            product_id: {
                ...
                "turnover": 0.0,
                "sale_avg_price": 0.0,
                "purchase_avg_price": 0.0,
                "sale_num_invoiced": 0.0,
        ...
        for product_id, avg, qty, total, _dummy in self.env.cr.fetchall():
            ...
            res[product_id]["total_margin_rate"] = (
                ...
                and res[product_id]["total_margin"]
                * 100
                / res[product_id].get("turnover", 0.0)
            ) or 0.0
        for product in self:
            res[product.id]["normal_cost"] = (
                product.standard_price * res[product.id]["purchase_num_invoiced"]
            )
            res[product.id]["purchase_gap"] = (
                res[product.id]["normal_cost"] - res[product.id]["total_cost"]
            )
            res[product.id]["expected_margin"] = (
                res[product.id].get("sale_expected", 0.0)
                - res[product.id]["normal_cost"]
            )
            res[product.id]["expected_margin_rate"] = (
                res[product.id].get("sale_expected", 0.0)
                and res[product.id]["expected_margin"]
                * 100
                / res[product.id].get("sale_expected", 0.0)
            ) or 0.0
