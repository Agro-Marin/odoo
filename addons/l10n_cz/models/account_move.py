...
class AccountMove(models.Model):
    ...
    def _compute_taxable_supply_date(self):
        ...
        for move in self.filtered(
            lambda m: m.country_code == "CZ" and not m.taxable_supply_date
        ):
    ...
    def _compute_show_taxable_supply_date(self):
        ...
        for move in self.filtered(
            lambda m: m.country_code == "CZ" and m.move_type != "entry"
        ):
    ...
    def _compute_date(self):
        ...
        for move in self:
            if (
                move.country_code == "CZ"
                and move.taxable_supply_date
                and move.state == "draft"
                and not move.statement_line_id
            ):
