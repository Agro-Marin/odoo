...
class L10nInEwaybill(models.Model):
        ...
        if picking_id := self.picking_id:
            if self._is_incoming():
                ...
                seller_buyer_details = {
                    ...
                    "buyer_details": picking_id._l10n_in_get_invoice_partner()
                    or picking_id.partner_id,
                    "ship_to_details": picking_id.partner_id,
                }
            if picking_id.picking_type_id.code == "dropship" and (
            ...
            if ewaybill.picking_id and ewaybill.state == "pending":
                ewaybill.fiscal_position_id = (
                    self.env["account.fiscal.position"]._get_fiscal_position(
                        (ewaybill._is_incoming() and ewaybill.partner_bill_from_id)
    ...
    def action_set_to_challan(self):
        ...
        if self.state != "pending":
            raise UserError(
                _("The challan can only be generated in the Pending state.")
            )
        self.write(
            {
                "state": "challan",
            ...
            for line in self.move_ids:
                ...
                if not hsn_code:
                    error_message.append(
                        _("HSN code is not set in product %s", line.product_id.name)
                    )
                elif not re.match(r"^[0-9]+$", hsn_code):
                    error_message.append(
                        _(
    ...
    def _check_state(self):
        ...
        if (not self._is_incoming() and picking_state != "done") or (
            self._is_incoming() and picking_state not in ("done", "assigned")
        ):
            error_message.append(
                _(
    ...
    def _l10n_in_tax_details_by_stock_move(self, move):
        taxes = move.ewaybill_tax_ids.compute_all(
            price_unit=move.ewaybill_price_unit, quantity=move.quantity
        )
        for tax in taxes["taxes"]:
            ...
            for gst_tax_name in ["igst", "sgst", "cgst"]:
                if (
                    self.env.ref("l10n_in.tax_tag_%s" % (gst_tax_name)).id
                    in tax["tag_ids"]
                ):
                    tax_name = gst_tax_name
            if self.env.ref("l10n_in.tax_tag_cess").id in tax["tag_ids"]:
                tax_name = (
                    tax_id.amount_type != "percent" and "cess_non_advol"
                ) or "cess"
    ...
    def _get_l10n_in_ewaybill_line_details(self, line, tax_details):
        if self.picking_id:
            ...
            line_details = {
                ...
                "hsnCode": AccountMove._l10n_in_extract_digits(
                    product.l10n_in_hsn_code
                ),
                ...
                "taxableAmount": AccountMove._l10n_in_round_value(
                ...
                for gst_type in gst_types:
                    if tax_rate := tax.get(f"{gst_type}_rate"):
                        gst_tax_rates.update(
                            {
                                f"{gst_type}Rate": AccountMove._l10n_in_round_value(
                                    tax_rate
                                )
                            }
                        )
                if cess_rate := tax.get("cess_rate"):
                    line_details["cessRate"] = AccountMove._l10n_in_round_value(
                        cess_rate
                    )
            ...
            line_details.update(
                gst_tax_rates
                or dict.fromkeys([f"{gst_type}Rate" for gst_type in gst_types], 0)
            )
            return line_details
    ...
    def _prepare_ewaybill_tax_details_json_payload(self):
        if self.picking_id:
            ...
            return {
                ...
                "totalValue": round_value(
                    tax_details["tax_details"].get("total_excluded", 0.00)
                ),
                **{
