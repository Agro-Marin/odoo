...
class StockMove(models.Model):
    ...
    l10n_in_ewaybill_ids = fields.One2many(related='picking_id.l10n_in_ewaybill_ids')
    ...
    @api.depends('l10n_in_ewaybill_ids')
    def _compute_l10n_in_ewaybill_price_unit(self):
        for line in self:
            ewaybill = line.l10n_in_ewaybill_ids and line.l10n_in_ewaybill_ids[0]
            if ewaybill.state == 'pending' and line.picking_id.country_code == 'IN':
                line.ewaybill_price_unit = line._l10n_in_get_product_price_unit()

    @api.depends('l10n_in_ewaybill_ids.fiscal_position_id')
        ...
        for line in self:
            ...
            if ewaybill.state == 'pending' and line.picking_id.country_code == 'IN':
                ...
                if taxes_details['is_from_order']:
                    ...
                    if fiscal_position := ewaybill.fiscal_position_id:
