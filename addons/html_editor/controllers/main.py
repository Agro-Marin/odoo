"""HTML editor controllers for attachment management, media library, and SVG shapes."""
...
def get_existing_attachment(IrAttachment: object, vals: dict) -> object | None:
    """Check if an attachment already exists for the same vals.

    Return it if so, None otherwise.
    """
    fields = dict(vals)
    ...
    if fields.get("type") == "url":
        ...
        if not (raw or datas):
            return None
        domain.append(
            ("checksum", "=", IrAttachment._compute_checksum(raw or b64decode(datas)))
        )
    return IrAttachment.search(domain, limit=1) or None


class HTML_Editor(http.Controller):
    """Controller for HTML editor operations including attachments and shapes."""

    def _get_shape_svg(self, module: str, *segments: str) -> str:
        """Read and return an SVG shape file from a module's static directory."""
        shape_path = str(Path(module, "static", *segments))
        try:
    ...
    def _update_svg_colors(self, options: dict, svg: str) -> tuple[str, dict]:
        """Replace default palette colors in SVG with user-specified colors."""
        user_colors = []
        svg_options = {}
        default_palette = {
            "1": "#3AADAA",
        ...
        def subber(match: re.Match) -> str:
            """Replace matched color with the user-specified mapping."""
            key = match.group().upper()
            return color_mapping.get(key, key)

        return re.sub(regex, subber, svg), svg_options

    def replace_animation_duration(self, shape_animation_speed: float, svg: str) -> str:
        """Replace animation durations in SVG and CSS with speed-modified values.

        Take a speed value and an SVG string containing animations. Use regular
        expressions to find and replace the duration values in both CSS animation
        rules and SVG duration attributes based on the provided speed.

        Args:
            shape_animation_speed: The speed used to calculate new animation durations.
            svg: The SVG string containing animations.

        Returns:
            The modified SVG string with updated animation durations.
        ...
        def callback_css_animation_rule(match: re.Match) -> str:
            """Modify CSS animation duration based on the speed ratio."""
        ...
        def callback_svg_dur_timecount_val(match: re.Match) -> str:
            """Modify SVG dur attribute based on the speed ratio."""
        ...
        def callback_css_animation_ratio(match: re.Match) -> str:
            """Replace CSS animation ratio variable with current ratio value."""
    ...
    def remove(self, ids: list[int], **kwargs: object) -> dict:
        """Remove web-based image attachments not used by any view.

        Return a dict mapping attachments which would not be removed (if any)
        to the views preventing their removal.
        """
        self._clean_context()
    ...
    def _clean_context(self) -> None:
        """Remove allowed_company_ids from context to avoid website restrictions."""
    ...
    def get_image_info(self, src: str = "") -> dict:
        """Determine attachment information for use as a base for modification.

        Return image info so it can be used to modify it again
        (crop/optimization/filters).
        """
        self._clean_context()
        attachment = None
    ...
    def video_url_data(
        ...
        hide_dm_logo: bool = False,
        hide_dm_share: bool = False,
        start_from: int | bool = False,
    ) -> dict:
        """Return embed URL data for the given video URL."""
    ...
    def modify_image(
        ...
        """Create a modified copy of an attachment and return its image_src.

        The returned URL is intended to be inserted into the DOM.
        """
        self._clean_context()
    ...
    def save_library_media(self, media: dict) -> list[dict]:
        """Save images from the media library as new attachments.

        Create dynamic SVGs if needed. The ``media`` dict maps media IDs to
        dicts with keys ``query``, ``is_dynamic_svg``, and ``dynamic_colors``.
        """
        attachments = []
        ...
        response = requests.post(
            f"{library_endpoint}/media-library/1/download_urls",
            data=params,
            timeout=10,
        )
    ...
    def shape(self, module: str, filename: str, **kwargs: object) -> http.Response:
        """Return a color-customized SVG (background shape or illustration)."""
        svg = None
    ...
    def image_shape(
        self, module: str, filename: str, img_key: str, **kwargs: object
    ) -> http.Response:
        """Return an SVG image shape with the specified image embedded."""
        # Used for compatibility
        if module == "web_editor":
            module = "html_builder"
    ...
    def generate_text(self, prompt: str, conversation_history: list) -> str:
        """Generate text content using the OLG API."""
        try:
    ...
    def get_ice_servers(self) -> list:
        """Return the list of configured ICE servers for WebRTC."""
    ...
    def bus_broadcast(
        ...
        """Broadcast editor collaboration data over the bus."""
    ...
    def link_preview_metadata(self, preview_url: str) -> dict | None:
        """Return Open Graph metadata for an external URL."""
    ...
    def link_preview_metadata_internal(self, preview_url: str) -> dict:
        """Return link preview metadata for an internal Odoo URL."""
        try:
    ...
    def media_library_search(self, **params: object) -> dict:
        """Search the media library for images matching the given parameters."""
        ICP = request.env["ir.config_parameter"].sudo()
