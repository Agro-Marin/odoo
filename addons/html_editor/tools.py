"""Video embedding utilities and HTML editor history divergence handling."""
...
def get_video_source_data(video_url: str) -> tuple[str, str, re.Match[str]] | None:
    """Return the video source, document ID, and regex match from the given URL.
...
def get_video_url_data(
    ...
    start_from: str | bool = False,
) -> dict[str, object]:
    """Return platform name, embed URL, video ID, and params for the given URL."""
...
def get_video_embed_code(video_url: str) -> Markup | None:
    """Return an embeddable iframe for the given video URL, or ``None`` if invalid."""
...
def get_video_thumbnail(video_url: str) -> bytes | None:
    """Return the thumbnail image for the given video URL, or ``None`` if unavailable."""
    source = get_video_source_data(video_url)
    if source is None:
        return None
...
def handle_history_divergence(
    record: object, html_field_name: str, vals: dict[str, object]
) -> None:
    """Handle editor collaboration history divergence on HTML field write.

    Must be called in a context with write access to the record, as it
    may write to the bus.
    """
    ...
    def _notify_bus(last_step_id: str | None) -> None:
