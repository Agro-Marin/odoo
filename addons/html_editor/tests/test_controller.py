"""Tests for html_editor controllers."""
...
class TestController(HttpCase):
    """Test html_editor HTTP controller endpoints."""

    @classmethod
    def setUpClass(cls) -> None:
        """Set up test users, headers, and sample data."""
        super().setUpClass()
    ...
    def test_01_upload_document(self) -> None:
        """Test uploading and downloading a document attachment."""
    ...
    def test_02_illustration_shape(self) -> None:
        """Test SVG illustration shape color replacement."""
        self.authenticate("admin", "admin")
...
<svg viewBox="0 0 400 400">
  ...
  <rect x="80" y="80" width="300" height="300" style="fill:#383E45;" />
    ...
    def test_03_get_image_info(self) -> None:
        """Test retrieving image info for an uploaded GIF."""
    ...
    def test_04_admin_attachment(self) -> None:
        """Test admin user can upload and search attachments."""
    ...
    def test_05_internal_link_preview(self) -> None:
        """Test internal link preview for various URL patterns."""
        self.authenticate(self.admin, self.admin)

        def _get_full_url(pathname: str) -> str:
            return f"{self.base_url()}{pathname}"

        def _patched_get_link_preview_from_url(url: str) -> dict | bool:
            if url == _get_full_url("/page-with-description"):
                return {
                    "og_description": "Mocked page description",
                }
            elif url == _get_full_url(
                "/page-without-description"
            ) or url == _get_full_url("/shop/category/1"):
                return {
                    "og_description": None,
                }
            else:
        ...
        response_without_preview_name = self.url_open(
            ...
            data=json_safe.dumps(
                {
                    "params": {
                        "preview_url": _get_full_url(
                            f"/odoo/users/{self.portal_user.id}"
                        ),
                    }
                }
            ),
            ...
            data=json_safe.dumps(
                {
                    "params": {
                        "preview_url": _get_full_url(
                            f"/odoo/res.users/{self.portal_user.id}"
                        ),
                    }
                }
            ),
