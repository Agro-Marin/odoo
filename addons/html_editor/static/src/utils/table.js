/** @odoo-module native */
import { isTableCell } from "./dom_info.js";
...
export function getSelectedCellsMergeInfo(editableDocument, tableGrid, targetCell) {
    ...
    if (selectedTds.length <= 1) {
        ...
        const td =
            selectedTds[0] ?? (anchorNode && closestElement(anchorNode, isTableCell));
        return {
            canMerge: false,
    ...
    const colIndexes = selectedTds.map((td, i) =>
        getGridColumnIndex(td, rowIndexes[i]),
    );
    ...
    const containsMergedCell = selectedTds.some(
        (td) => td.rowSpan > 1 || td.colSpan > 1,
    );
