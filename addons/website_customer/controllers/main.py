...
class WebsiteCustomer(GoogleMap):
    ...
    def customers(self, country=None, industry=None, page=0, **post):
        ...
        if industry:
            ...
            if (
                ...
                industry_groups = sorted(
                    industry_groups, key=lambda group: group[0].name or ""
                )
            ...
            industries.append(
                {
                    ...
                    "industry_id": g_industry
                    and (g_industry.id, g_industry.display_name),
                }
            )
        ...
        partners = Partner.sudo().search(
            domain, offset=pager["offset"], limit=self._references_per_page
        )
        google_maps_api_key = request.website.google_maps_api_key

        tags = Tag.search(
            [("website_published", "=", True), ("partner_ids", "in", partners.ids)],
            ...
            if partner.exists() and partner.website_published:
                if request.env["ir.http"]._slug(partner) != current_slug:
                    return request.redirect(
                        "/customers/%s" % request.env["ir.http"]._slug(partner)
                    )
                values = {}
