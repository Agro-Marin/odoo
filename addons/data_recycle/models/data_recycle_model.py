...
class Data_RecycleModel(models.Model):
    ...
    notify_user_ids = fields.Many2many(
        ...
        string="Notify Users",
        domain=lambda self: [
            ("all_group_ids", "in", self.env.ref("base.group_system").id)
        ],
    ...
    def _check_recycle_action(self):
        for model in self:
            if (
                model.recycle_action == "archive"
                and "active" not in self.env[model.res_model_name]
            ):
                raise UserError(
                    _(
    ...
    def _recycle_records(self, batch_commits=False):
        ...
        existing_recycle_records = (
            self.env["data_recycle.record"]
            .with_context(active_test=False)
            .search([("recycle_model_id", "in", self.ids)])
        )
        ...
        for recycle_model in self:
            rule_domain = (
                Domain(ast.literal_eval(recycle_model.domain))
                if recycle_model.domain and recycle_model.domain != "[]"
                else Domain.TRUE
            )
            if (
                recycle_model.time_field_id
                and recycle_model.time_field_delta
                and recycle_model.time_field_delta_unit
            ):
                ...
                delta = relativedelta(
                    **{
                        recycle_model.time_field_delta_unit: recycle_model.time_field_delta
                    }
                )
                rule_domain &= Domain(
            ...
            if recycle_model.recycle_mode == "automatic":
                for records_to_create_batch in batched(
                    records_to_create, DR_CREATE_STEP_AUTO, strict=False
                ):
                    self.env["data_recycle.record"].create(
                        records_to_create_batch
                    ).action_validate()
                ...
                records_to_clean += records_to_create
        for records_to_clean_batch in batched(
            records_to_clean, DR_CREATE_STEP_MANUAL, strict=False
        ):
    ...
    def _notify_records_to_recycle(self):
        for recycle in self.search([("recycle_mode", "=", "manual")]):
            ...
            if (
                not recycle.last_notification
                or (recycle.last_notification + delta) < fields.Datetime.now()
            ):
    ...
    def _send_notification(self, delta):
        ...
        records_count = self.env["data_recycle.record"].search_count(
            [("recycle_model_id", "=", self.id), ("create_date", ">=", last_date)]
        )
