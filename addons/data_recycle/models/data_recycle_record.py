...
class Data_RecycleRecord(models.Model):
    ...
    res_model_id = fields.Many2one(
        related="recycle_model_id.res_model_id", store=True, readonly=True
    )
    res_model_name = fields.Char(
        related="recycle_model_id.res_model_name", store=True, readonly=True
    )
    ...
    def action_validate(self):
        ...
        original_records = {
            "%s_%s" % (r._name, r.id): r for r in self._original_records()
        }
        for record in self:
            original_record = original_records.get(
