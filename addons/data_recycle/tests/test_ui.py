...
class TestDataRecycleUi(HttpCase):
    def test_validate_a_grouped_selection(self):
        ...
        banks = self.env["res.bank"].create(
            [{"name": "Tour bank %s" % i} for i in range(3)]
        )
        ...
        self.start_tour(
            "/odoo/field-recycle", "data_recycle_validate_grouped", login="admin"
        )
    ...
    def test_the_unfiltered_warning_tracks_the_filter(self):
        ...
        self.assertEqual(
            unfiltered.domain, "[]", "this is the value the alert compares against"
        )
        ...
        self.start_tour(
            form % unfiltered.id, "data_recycle_unfiltered_warning", login="admin"
        )
        self.start_tour(
            form % filtered.id, "data_recycle_filtered_no_warning", login="admin"
        )
        self.start_tour(
            form % by_age.id, "data_recycle_filtered_no_warning", login="admin"
        )
