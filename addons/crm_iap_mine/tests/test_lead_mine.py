...
class TestLeadMine(TestCrmCommon, MockIAPReveal):
    ...
    def test_mine_error_credit(self):
        mine_request = self.env["crm.iap.lead.mining.request"].browse(
    ...
    def test_mine_wpeople(self):
        ...
        with self.mock_IAP_mine(
            mine_request, name_list=["Heinrich", "Rivil", "LidGen"]
        ):
            mine_request.action_submit()
        ...
        self.assertEqual(
            len(self._new_leads), 3, "Number of leads should match mine request"
        )

        for base_name in ["Heinrich", "Rivil", "LidGen"]:
            lead = self._new_leads.filtered(
                lambda lead: lead.name == "%s GmbH" % base_name
            )
            ...
            self.assertEqual(
                lead.reveal_id, "123456789", "Ensure reveal_id is set to Duns"
            )
            ...
            self.assertEqual(lead._phone_get_number().number, "4930499193937")
