...
patch(PaymentForm.prototype, {
    ...
    updateAmount(ev) {
        if (ev.target.value >= 0) {
            ...
            if (
                ev.target.id === "other_amount" ||
                ev.target.id === "other_amount_value"
            ) {
                this.el
                    .querySelectorAll('input[name="o_donation_amount"][type="radio"]')
                    .forEach((radioEl) => (radioEl.checked = false));
    ...
    async submitForm(ev) {
        ...
        const considerAmountInput = otherAmountRadioEl
            ? otherAmountRadioEl.checked
            : true;
        if (
    ...
    async _initiatePaymentFlow() {
        if (document.querySelector(".o_donation_payment_form")) {
            ...
            for (const id in mandatoryFields) {
                ...
                const isInvalid =
                    !fieldEl.value.trim() ||
                    (id === "email" && !fieldEl.checkValidity());
        ...
        return document.querySelector(".o_donation_payment_form")
            ? {
                  ...
                  partner_details: {
                      ...
                      country_id: this.el.querySelector('select[name="country_id"]')
                          .value,
                  },
