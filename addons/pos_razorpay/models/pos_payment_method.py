...
class PosPaymentMethod(models.Model):
    ...
    def razorpay_make_payment_request(self, data):
        ...
        body.update(
            {"amount": data.get("amount"), "externalRefNumber": data.get("referenceId")}
        )
        response = razorpay._call_razorpay(endpoint="pay", payload=body)
        if response.get("success") and not response.get("errorCode"):
            return {"success": True, "p2pRequestId": str(response.get("p2pRequestId"))}
        default_error_msg = _(
    ...
    def razorpay_fetch_payment_status(self, data):
        ...
        if response.get("success") and not response.get("errorCode"):
            ...
            if (
                ...
                and payment_messageCode == "P2P_DEVICE_TXN_DONE"
            ):
                return {
                    "status": response.get("status"),
                    "authCode": response.get("authCode"),
                ...
                payment_status in ["VOIDED", "AUTHORIZED_REFUNDED"]
                and payment_messageCode == "P2P_DEVICE_TXN_DONE"
            ):
                return {
                ...
                or payment_messageCode == "P2P_DEVICE_CANCELED"
            ):
                return {
                    "error": str(
                        response.get("message", _("Razorpay POS transaction failed"))
                    ),
                    "payment_messageCode": payment_messageCode,
                }
            elif payment_messageCode in [
                "P2P_DEVICE_RECEIVED",
                "P2P_DEVICE_SENT",
                "P2P_STATUS_QUEUED",
            ]:
        ...
        default_error_msg = _(
            "Razorpay POS payment status request expected errorCode not found in the response"
        )
        error = response.get("errorMessage") or default_error_msg
        return {"error": str(error)}
    ...
    def razorpay_cancel_payment_request(self, data):
        ...
        default_error_msg = _(
            "Razorpay POS payment cancel request expected errorCode not found in the response"
        )
    ...
    def _check_razorpay_terminal(self):
        if any(
            record.use_payment_terminal == "razorpay"
