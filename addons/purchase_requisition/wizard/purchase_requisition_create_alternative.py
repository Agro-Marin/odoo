    ...
    def _get_alternative_values(self):
        ...
        if self.copy_products and origin_po:
            supplierinfo = self.env['product.supplierinfo'].search([
                ('product_tmpl_id', 'in', origin_po.line_ids.product_id.product_tmpl_id.ids),
        ...
        for partner in self.partner_ids:
            ...
            if self.copy_products and origin_po:
                val['line_ids'] = [Command.create(self._get_alternative_line_value(line, product_tmpl_ids_with_description)) for line in origin_po.line_ids]
            vals.append(val)
