...
class PurchaseRequisitionCreateAlternative(models.TransientModel):
    ...
    def action_create_alternative(self):
        ...
        alt_purchase_orders.line_ids._compute_tax_ids()
        action = {
    ...
    def _get_alternative_values(self):
            ...
            supplierinfo = self.env["product.supplierinfo"].search(
                [
                    (
                        "product_tmpl_id",
                        "in",
                        origin_po.line_ids.product_id.product_tmpl_id.ids,
                    ),
        ...
        for partner in self.partner_ids:
            product_tmpl_ids_with_description = partner_product_tmpl_dict.get(
                partner.id, set()
            )
            val = {
            ...
            if self.copy_products and origin_po:
                val["line_ids"] = [
                    Command.create(
                        self._get_alternative_line_value(
                            line, product_tmpl_ids_with_description
                        )
                    )
                    for line in origin_po.line_ids
                ]
            vals.append(val)
    ...
    @api.model
    def _get_alternative_line_value(
        self, order_line, product_tmpl_ids_with_description
    ):
        has_product_description = (
            order_line.product_id.product_tmpl_id.id
            in product_tmpl_ids_with_description
        )
        return {
