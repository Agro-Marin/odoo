...
class ProductSupplierinfo(models.Model):
    ...
    purchase_requisition_id = fields.Many2one(
        ...
        related="purchase_requisition_line_id.agreement_id",
        string="Agreement",
    )
...
class ProductProduct(models.Model):
    ...
    def _prepare_sellers(self, params=False):
        ...
        if (
            ...
            and params["order_id"]._fields.get("agreement_id")
        ):
            return sellers.filtered(
                lambda s: (
                    not s.purchase_requisition_id
                    or s.purchase_requisition_id == params["order_id"].agreement_id
                )
            )
        else:
            return sellers
