...
class GamificationBadgeUser(models.Model):
    ...
    def _check_employee_related_user(self):
        for badge_user in self:
            if (
                badge_user.employee_id
                and badge_user.employee_id
                not in badge_user.user_id.with_context(
                    allowed_company_ids=badge_user.user_id.company_ids.ids
                ).employee_ids
            ):
    ...
    def _compute_has_edit_delete_access(self):
        ...
        for badge_user in self:
            badge_user.has_edit_delete_access = (
