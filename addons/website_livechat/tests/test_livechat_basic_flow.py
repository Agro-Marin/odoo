...
class TestLivechatBasicFlowHttpCase(HttpCaseWithUserDemo, TestLivechatCommon):
    def test_channel_created_on_user_interaction(self):
        ...
        self.assertFalse(
            channel, "Channel should not be created until user sends a message"
        )
        ...
        self.assertTrue(
            channel, "Channel should be created after sending the first message"
        )
    ...
    def test_livechat_username(self):
        ...
        self.assertEqual(
            ...
            "%s %s"
            % (
                f"Visitor #{channel_1.livechat_visitor_id.id}",
                self.operator.livechat_username,
            ),
        )
        channel_1.unlink()
        ...
        self.assertEqual(
            channel_2.name,
            "%s %s"
            % (f"Visitor #{channel_2.livechat_visitor_id.id}", self.operator.name),
        )
    ...
    def test_basic_flow_with_rating(self):
        ...
        self._send_rating(
            channel, self.visitor, 5, "This deboulonnage was fine but not topitop."
        )
        ...
        self.assertEqual(
            channel.message_ids[0].author_id,
            self.env.ref("base.partner_root"),
            "Odoobot must be the sender of the 'left the conversation' message.",
        )
        self.assertIn(
            f"Visitor #{channel.livechat_visitor_id.id}", channel.message_ids[0].body
        )
        self.assertTrue(
    ...
    def test_basic_flow_without_rating(self):
        ...
        self.assertEqual(
            channel.message_ids[0].author_id,
            self.env.ref("base.partner_root"),
            "Odoobot must be the author the message.",
        )
        self.assertIn(
            f"Visitor #{channel.livechat_visitor_id.id}", channel.message_ids[0].body
        )
        self.assertTrue(
            channel.livechat_end_dt,
            "The livechat session must be inactive since visitor left the conversation.",
        )
    ...
    def test_visitor_info_access_rights(self):
        ...
        livechat_info = next(
            c for c in init_messaging["discuss.channel"] if c["id"] == channel.id
        )
    ...
    def _common_basic_flow(self):
        ...
        self.assertEqual(
            ...
            "%s %s"
            % (
                f"Visitor #{channel.livechat_visitor_id.id}",
                self.operator.livechat_username,
            ),
        )
        ...
        self.assertEqual(
            channel.message_ids[0].author_id.id,
            False,
            "The author of the message is not a partner.",
        )
        self.assertEqual(
            channel.message_ids[0].email_from,
            self.visitor.display_name,
            "The sender's email should be the visitor's email.",
        )
        ...
        self.assertEqual(
            ...
            "The author of the message should be the operator.",
        )
        self.assertEqual(
            channel.message_ids[0].email_from,
            self.operator.email,
            "The sender's email should be the operator's email.",
        )
    ...
    def test_channel_visitor_data(self):
        ...
        operator_member = channel.channel_member_ids.filtered(
        ...
        self.assertEqual(
            Store().add(channel).get_result(),
            {
                ...
                "res.country": [
                    {
                        "code": "BE",
                        "id": self.env["ir.model.data"]._xmlid_to_res_id("base.be"),
                    }
                ],
                "res.lang": [
                    {"id": self.env.ref("base.lang_en").id, "name": "English (US)"}
                ],
                ...
                "website.visitor": [
                    {
                        "country_id": self.env["ir.model.data"]._xmlid_to_res_id(
                            "base.be"
                        ),
