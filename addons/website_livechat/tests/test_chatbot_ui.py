...
class TestLivechatChatbotUICommon(
    TestGetOperatorCommon, TestWebsiteLivechatCommon, ChatbotCase
):
    def setUp(self):
        super().setUp()
...
class TestLivechatChatbotUI(TestLivechatChatbotUICommon):
    def _check_complete_chatbot_flow_result(self):
        ...
        livechat_discuss_channel = self.env["discuss.channel"].search(
            [
                ...
                (
                    "chatbot_current_step_id.chatbot_script_id",
                    "=",
                    self.chatbot_script.id,
                ),
                ("message_ids", "!=", False),
            ]
        )
        ...
        expected_messages = [
            ...
            (
                "I think it's outrageous that you ask for all my personal information!",
                False,
                False,
            ),
        ...
        for conversation_message, expected_message in zip(
            ...
            if user_script_answer_id:
                self.assertEqual(
                    ...
                    .search(
                        [("mail_message_id", "=", conversation_message.id)], limit=1
                    )
                    .user_script_answer_id,
                )
    ...
    def test_chatbot_test_page_tour(self):
        bob_manager = tests.new_test_user(
            self.env,
            login="bob_user",
            groups="im_livechat.im_livechat_group_manager,base.group_user",
        )
        ...
        self.start_tour(
            test_page_url, "website_livechat_chatbot_test_page_tour", login="bob_user"
        )
    ...
    def test_chatbot_fw_operator_matching_lang(self):
        ...
        self.env.ref("website.default_website").language_ids = self.env[
            "res.lang"
        ].search([("code", "in", ("fr_FR", "en_US"))])
    ...
    def test_chatbot_user_input_saved_on_last_step(self):
        ...
        user_answer_message = self.env["chatbot.message"].search(
            [
                ...
                (
                    "discuss_channel_id.livechat_channel_id",
                    "=",
        ...
        self.assertIn(
            ...
            "Email was saved on last step",
        )
        self.assertTrue(
            user_answer_message.discuss_channel_id.livechat_end_dt,
            "Livechat ended after last step",
        )
