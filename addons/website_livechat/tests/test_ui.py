...
class TestLivechatUI(HttpCaseWithUserDemo, TestLivechatCommon):
    ...
    def test_complete_rating_flow_ui_logged_in(self):
        self.start_tour(
            "/",
            "website_livechat_complete_flow_tour_logged_in",
    ...
    def test_no_rating_flow_ui(self):
        ...
        self.assertEqual(
            len(channel), 1, "There can only be one channel created for 'Visitor Tour'."
        )
        self.assertTrue(
            channel.livechat_end_dt,
            "Livechat must be inactive after closing the chat window.",
        )
    ...
    def _check_end_of_rating_tours(self):
        ...
        self.assertTrue(
            channel.livechat_end_dt, "Livechat must be inactive after rating."
        )
