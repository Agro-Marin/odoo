...
class Im_LivechatChannel(models.Model):
        ...
        **kwargs,
        ...
        if visitor_sudo:
            ...
            for discuss_channel in (
                self.env["discuss.channel"].sudo().search(pending_chats_domain)
            ):
                ...
                discuss_channel._close_livechat_session(
                    cancel=True, operator=operator_name
                )
    ...
    def create(self, vals_list):
        ...
        if self.env.context.get("create_from_website"):
            ...
            bot = self.env.ref(
                "im_livechat.chatbot_script_welcome_bot", raise_if_not_found=False
            )
            ...
            for channel in channels:
                ...
                self.env.user._bus_send(
                    "simple_notification",
                    {
                        "type": "success",
                        "message": self.env._(
                            "Channel created: %(name)s", name=channel.name
                        ),
                    },
                )
