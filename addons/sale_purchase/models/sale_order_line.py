...
class SaleOrderLine(models.Model):
    ...
    purchase_line_count = fields.Integer(
        "Number of generated purchase items", compute="_compute_purchase_count"
    )
    ...
    def _onchange_service_product_uom_qty(self):
        if (
            ...
            and self.product_id.with_company(
    ...
    def write(self, vals):
        ...
        if "product_uom_qty" in vals:
            ...
            increased_lines = self.sudo().filtered(
                lambda r: (
                    r.product_id.with_company(
                        r._purchase_service_get_company()
                    ).service_to_purchase
        ...
        if increased_lines:
            increased_lines._purchase_increase_ordered_qty(
                vals["product_uom_qty"], increased_values
            )
        if decreased_lines:
            decreased_lines._purchase_decrease_ordered_qty(
                vals["product_uom_qty"], decreased_values
            )
        return result
        ...
        if supplierinfo:
            price_unit = (
                ...
                ._fix_tax_included_price_company(
                    supplierinfo.price, supplier_taxes, taxes, purchase_order.company_id
                )
            )
            if (
                purchase_order.currency_id
                and supplierinfo.currency_id != purchase_order.currency_id
            ):
    ...
    def _purchase_service_get_product_name(
        self, supplierinfo, purchase_order, quantity
    ):
        product_ctx = {
    ...
    def _purchase_service_prepare_line_values(self, purchase_order, quantity=False):
        ...
        supplierinfo = self.product_id._select_seller(
            ...
            and purchase_order.date_order.date(),  # and purchase_order.date_order[:10],
        ...
        name = self._purchase_service_get_product_name(
            supplierinfo, purchase_order, quantity
        )
        ...
        purchase_line_vals = {
            ...
            "product_uom_id": supplierinfo.product_uom_id.id
