...
class PaymentTransaction(models.Model):
    ...
    def _send_refund_request(self):
        ...
        if tx_status in const.TRANSACTION_STATUS_MAPPING["voided"]:
            ...
            if tx_status in const.TRANSACTION_STATUS_MAPPING["authorized"]:
                ...
                res_content = authorize_api.void(
                    self.source_transaction_id.provider_reference
                )
            else:
    ...
    def _extract_amount_data(self, payment_data):
        ...
        if (
            "err_code" in tx_details
        ):  # Transaction details are missing when an API error occurs.
        ...
        currency = (
            self.provider_id.available_currency_ids
        )  # The currency has not been removed from the provider.
        return {
            "amount": float(amount),
    ...
    def _apply_updates(self, payment_data):
        ...
        if status_code == "1":  # Approved
            ...
            if status_type in ("auth_capture", "prior_auth_capture"):
                ...
                ):  # Validation txs are authorized and then voided
    ...
    def _extract_token_values(self, payment_data):
        ...
        if (
            not cust_profile or "payment_profile_id" not in cust_profile
