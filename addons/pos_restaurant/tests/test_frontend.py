...
class TestFrontendCommon(TestPointOfSaleHttpCommon):
    ...
    def setUpClass(cls):
        ...
        drinks_category = cls.env["pos.category"].create(
        ...
        cls.desk_organizer = cls.env["product.product"].create(
            {
                ...
                "list_price": 5.10,
                "pos_categ_ids": [
                    (4, drinks_category.id)
                ],  # will put it as a drink for convenience
            }
        )
        ...
        second_pricelist = cls.env["product.pricelist"].create(
            {"name": "Second Pricelist"}
        )
        cls.pos_config.write({"pricelist_id": pricelist.id})
        cls.pos_config.write(
            {"available_pricelist_ids": [(6, 0, [pricelist.id, second_pricelist.id])]}
        )
...
class TestFrontend(TestFrontendCommon):
    ...
    def test_05_tip_screen(self):
        ...
        order4 = self.env["pos.order"].search(
            [("pos_reference", "ilike", "%-000004")], limit=1, order="id desc"
        )
    ...
    def test_06_split_bill_screen(self):
        ...
        orders = self.env["pos.order"].search(
            [("pos_reference", "!=", "")], limit=2, order="id desc"
        )
    ...
    def test_10_save_last_preparation_changes(self):
        ...
        self.assertTrue(
            self.pos_config.current_session_id.order_ids.last_order_preparation_change,
            "There should be a last order preparation change",
        )
        self.assertTrue(
            "Coca"
            in self.pos_config.current_session_id.order_ids.last_order_preparation_change,
    ...
    def test_12_order_tracking(self):
        ...
        order1 = self.env["pos.order"].search(
            [("pos_reference", "ilike", "%-000001")], limit=1, order="id desc"
        )
    ...
    def test_14_pos_payment_sync(self):
        ...
        def assert_payment(lines_count, amount):
            ...
            self.assertEqual(
                round(sum(payment.amount for payment in order.payment_ids), 2), amount
            )
    ...
    def test_pos_restaurant_course(self):
        ...
        self.assertNotEqual(
            order.course_ids[0].fired_date, order.course_ids[1].fired_date
        )
        ...
        self.product_test = self.env["product.product"].create(
            {
                ...
                "list_price": 10,
                "pos_categ_ids": [
                    (6, 0, [self.env["pos.category"].search([], limit=1).id])
                ],
                "taxes_id": False,
            }
        )
    ...
    def test_user_on_residual_order(self):
        ...
        orders = self.env["pos.order"].search([])
        self.assertEqual(
            orders[0].user_id.id, self.pos_user.id, "Pos user not registered on order"
        )
        self.assertEqual(
            orders[1].user_id.id, self.pos_admin.id, "Pos admin not registered on order"
        )
    ...
    def test_tax_in_merge_table_order_line(self):
        ...
        self.pos_config.with_user(self.pos_user).open_ui()
        self.start_pos_tour(
            "test_tax_in_merge_table_order_line_tour", login="pos_admin"
        )
        line_1 = self.env["pos.order.line"].search(
            [("full_product_name", "=", "product_1")]
        )
        line_2 = self.env["pos.order.line"].search(
            [("full_product_name", "=", "product_2")]
        )
    ...
    def test_multiple_preparation_printer_different_categories(self):
        ...
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}",
            "test_multiple_preparation_printer_different_categories",
            login="pos_user",
        )
    ...
    def test_preset_timing_restaurant(self):
        ...
        cancelled_orders = self.env["pos.order"].search(
            [("state", "=", "cancel")], limit=3
        )
    ...
    def test_restaurant_preset_eatin_tour(self):
        self.pos_config.write(
            {
                "use_presets": True,
                "default_preset_id": self.env.ref(
                    "pos_restaurant.pos_takein_preset", False
                ).id,
            }
        )
    ...
    def test_combo_preparation_receipt_layout(self):
        ...
        self.start_tour(
            ...
            "test_combo_preparation_receipt_layout",
            login="pos_user",
        )
    ...
    def test_fast_payment_validation_from_restaurant_product_screen_with_automatic_receipt_printing(
        self,
    ):
        self.env["pos.printer"].create(
            {
        ...
        self.assertEqual(
            ...
            1,
            "There should be one payment method used for the fast payment",
        )
        self.assertEqual(
    ...
    def test_fast_payment_validation_from_restaurant_product_screen_without_automatic_receipt_printing(
        self,
    ):
        self.env["pos.printer"].create(
            {
