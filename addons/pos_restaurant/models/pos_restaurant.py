...
class RestaurantFloor(models.Model):
    ...
    def _unlink_except_active_pos_session(self):
        confs = self.mapped("pos_config_ids").filtered(
            lambda c: c.module_pos_restaurant
        )
    ...
    def write(self, vals):
        for floor in self:
            for config in floor.pos_config_ids:
                if config.has_active_session and (
                    vals.get("pos_config_ids") or vals.get("active")
                ):
                    raise UserError(
                        self.env._(
...
class RestaurantTable(models.Model):
    ...
    floor_id = fields.Many2one(
        "restaurant.floor", string="Floor", index="btree_not_null"
    )
    ...
    seats = fields.Integer(
        "Seats", default=1, help="The default number of customer served at this table."
    )
    color = fields.Char(
        "Color",
    ...
    @api.ondelete(at_uninstall=False)
    def _unlink_except_active_pos_session(self):
        confs = self.mapped("floor_id.pos_config_ids").filtered(
            lambda c: c.module_pos_restaurant
        )
        opened_session = self.env["pos.session"].search(
            [("config_id", "in", confs.ids), ("state", "!=", "closed")]
