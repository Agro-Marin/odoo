...
class PosConfig(models.Model):
    ...
    def _get_fields_forbidden_change(self):
        forbidden_keys = super()._get_fields_forbidden_change()
    ...
    def create(self, vals_list):
        for vals in vals_list:
            is_restaurant = (
                "module_pos_restaurant" in vals and vals["module_pos_restaurant"]
            )
            if is_restaurant:
    ...
    def load_onboarding_bar_scenario(self, with_demo_data=True):
        journal, payment_methods_ids = self._create_journal_and_payment_methods(
            cash_journal_vals={"name": "Cash Bar", "show_on_dashboard": False}
        )
    ...
    def load_onboarding_restaurant_scenario(self, with_demo_data=True):
        ...
        if bool(presets):
            ...
            ).implied_by_ids |= self.env.ref("base.group_user")
        if not self.env.ref("pos_restaurant.floor_main", raise_if_not_found=False):
            convert.convert_file(
                self._get_env_with_clean_context(),
                "pos_restaurant",
        ...
        existing_session = self.env.ref(
            "pos_restaurant.pos_closed_session_3", raise_if_not_found=False
        )
        if (
            with_demo_data
