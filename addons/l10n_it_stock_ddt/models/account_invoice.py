    ...
    def _get_ddt_values(self):
        ...
        for line in self.invoice_line_ids.filtered(
            ...
            if len(done_moves_related) <= 1:
                ...
                total_invoices = (
                    done_moves_related.mapped("sale_line_id.invoice_line_ids")
                    .filtered(
                        lambda l: (
                    ...
                    .sorted(lambda l: (l.move_id.invoice_date, l.move_id.id))
                )
                total_invs = [
                    (
                ...
                for move in done_moves_related.sorted(lambda m: (m.date, m.id)):
                    ...
                    while float_compare(move_qty, 0, precision_rounding=rounding) > 0:
                        if (
                            float_compare(inv[0], move_qty, precision_rounding=rounding)
                            > 0
                        ):
                        ...
                        if (
                            invoice_line == line
                            and line_count
                            not in invoice_line_pickings.get(move.picking_id, [])
                        ):
                            invoice_line_pickings.setdefault(
                                move.picking_id, []
                            ).append(line_count)
    ...
    def _compute_ddt_ids(self):
        it_out_invoices = self.filtered(
            lambda i: (
                ...
                and i.company_id.account_fiscal_country_id.code == "IT"
            )
        )
