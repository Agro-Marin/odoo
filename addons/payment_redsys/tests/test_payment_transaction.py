...
class TestPaymentTransaction(RedsysCommon):
    ...
    def test_no_item_missing_from_merchant_parameters(self):
        ...
        self.assertEqual(
            merchant_parameters["DS_MERCHANT_AMOUNT"], str(converted_amount)
        )
        self.assertEqual(
            merchant_parameters["DS_MERCHANT_CURRENCY"], tx.currency_id.iso_numeric
        )
        self.assertEqual(merchant_parameters["DS_MERCHANT_ORDER"], tx.reference)
        self.assertEqual(
            merchant_parameters["DS_MERCHANT_PAYMETHODS"], "C"
        )  # credit card
    ...
    def test_apply_updates_sets_payment_method(self):
        ...
        self.assertEqual(
            tx.payment_method_id, self.env.ref("payment.payment_method_visa")
        )
