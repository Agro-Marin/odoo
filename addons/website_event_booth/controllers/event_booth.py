...
class WebsiteEventBoothController(WebsiteEventController):
    ...
    def event_booth_main(self, event, booth_category_id=False, booth_ids=False):
        ...
        return request.render(
            ...
            self._prepare_booth_main_values(
                event, booth_category_id=booth_category_id, booth_ids=booth_ids
            )
            | {"seo_object": event.booth_menu_ids},
        )
    ...
    def event_booth_register(self, event, booth_category_id, event_booth_ids):
        ...
        return request.redirect(
            ("/event/%s/booth/register_form?" % event.id)
            + urlencode(
                {
    ...
    def event_booth_contact_form(self, event, booth_ids=None, booth_category_id=None):
        ...
        return request.render(
            ...
            self._prepare_booth_contact_form_values(
                event, booth_ids, booth_category_id
            ),
        )
    ...
    def _prepare_booth_contact_form_values(self, event, booth_ids, booth_category_id):
        ...
        event_booths = (
            request.env["event.booth"]
            .sudo()
            .browse([int(booth_id) for booth_id in booth_ids.split(",")])
        )
        default_contact = {}
    ...
    def _check_booth_registration_values(
        self, booths, contact_email, booth_category=False
    ):
        if not booths:
            return "boothError"
        ...
        if request.env.user._is_public() and email_normalized:
            partner = (
                ...
                .sudo()
                .search([("email_normalized", "=", email_normalized)], limit=1)
            )
            if partner:
    ...
    def _prepare_booth_main_values(
        self, event, booth_category_id=False, booth_ids=False
    ):
        event_sudo = event.sudo()
        available_booth_categories = event_sudo.event_booth_category_available_ids
        chosen_booth_category = available_booth_categories.filtered(
            lambda cat: cat.id == booth_category_id
        )
        default_booth_category = (
            available_booth_categories[0]
            if available_booth_categories
        ...
        return {
            ...
            "selected_booth_category_id": (
                chosen_booth_category or default_booth_category
            ).id,
            "selected_booth_ids": booth_ids
            if booth_category_id == chosen_booth_category.id and booth_ids
            else False,
            "slots": event.event_slot_ids._filter_open_slots().grouped("date"),
        }
    ...
    def check_booths_availability(self, event_booth_ids=None):
        ...
        return {
            "unavailable_booths": booths.filtered(
                lambda booth: not booth.is_available
            ).ids
        }

    @http.route(
        ["/event/booth_category/get_available_booths"], type="jsonrpc", auth="public"
    )
    def get_booth_category_available_booths(self, event_id, booth_category_id):
        ...
        return [{"id": booth.id, "name": booth.name} for booth in booth_ids]
