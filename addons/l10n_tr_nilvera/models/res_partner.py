...
class ResPartner(models.Model):
    ...
    def _compute_nilvera_customer_alias_id(self):
        for record in self:
            record.l10n_tr_nilvera_customer_alias_id = (
                record.l10n_tr_nilvera_customer_alias_ids[:1]
            )
    ...
    def _check_nilvera_customer(self):
        ...
        with _get_nilvera_client(self.env.company) as client:
            response = client.request(
                "GET",
                "/general/GlobalCompany/Check/TaxNumber/"
                + urllib.parse.quote(self.vat),
            ...
            if response.status_code == 200:
                ...
                if not query_result:
                    ...
                    to_keep = persisted_aliases.filtered(
                        lambda a: a.name not in aliases_to_remove
                    )
