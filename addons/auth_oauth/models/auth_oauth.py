...
class AuthOauthProvider(models.Model):
    ...
    )  # Name of the OAuth2 entity, Google, etc
    client_id = fields.Char(string="Client ID")  # Our identifier
    auth_endpoint = fields.Char(
        string="Authorization URL", required=True
    )  # OAuth provider URL to authenticate users
    scope = fields.Char(
        default="openid profile email"
    )  # OAUth user data desired to access
    validation_endpoint = fields.Char(
        string="UserInfo URL", required=True
    )  # OAuth provider URL to get user information
    data_endpoint = fields.Char()
    enabled = fields.Boolean(string="Allowed")
    css_class = fields.Char(
        string="CSS class", default="fa-solid fa-right-to-bracket text-primary"
    )
    body = fields.Char(
        required=True,
