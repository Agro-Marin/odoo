...
class TestAccountUblCii(AccountTestInvoicingCommon):
    ...
    def test_peppol_eas_endpoint(self):
        ...
        with self.check_peppol_vals(
            partner, expected=("0184", partner.company_registry)
        ):
            pass
        ...
        with self.check_peppol_vals(
            partner_1, expected=("0184", "12345674"), reset=False
        ):
            pass
        ...
        with self.check_peppol_vals(
            partner_2, expected=("0184", "12345674"), reset=False
        ):
        ...
        self.assertEqual(
            (partner_2.peppol_eas, partner_2.peppol_endpoint), ("0208", "12345674")
        )
    ...
    def test_partner_ubl_cii_formats(self):
        ...
        with patch.object(
            ...
            self.assertEqual(
                Partner._get_ubl_cii_formats(), ["ubl_no_country", "peppol", "cii"]
            )
            self.assertEqual(
                Partner._get_ubl_cii_formats_by_country()["NZ"], ["peppol"]
            )
            self.assertEqual(
                Partner._get_ubl_cii_formats_by_country()["AU"], ["peppol", "cii"]
            )
            self.assertEqual(
                Partner._get_peppol_formats(), ["ubl_no_country", "peppol"]
            )
            self.assertEqual(
                partner_au._get_suggested_ubl_cii_edi_format(), "cii"
            )  # AU matches 2 formats but 'cii' has a lower sequence
