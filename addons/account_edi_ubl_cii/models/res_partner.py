...
from odoo.addons.account.models.res_company import PEPPOL_DEFAULT_COUNTRIES
...
class ResPartner(models.Model):
    ...
    def _check_peppol_fields(self):
        for partner in self:
            if partner.peppol_endpoint and partner.peppol_eas:
                error = self._prepare_error_peppol_endpoint(
    ...
    def _get_ubl_cii_formats_info(self):
        return {
            ...
            "ubl_a_nz": {
                "countries": ["NZ", "AU"],
                "on_peppol": False,
            },  # Not yet available through Odoo's Access Point, although it's a Peppol valid format
    ...
    def _get_suggested_ubl_cii_edi_format(self):
        ...
        if country_code in format_mapping:
            ...
            if len(formats_by_country) == 1:
                ...
                return min(
                    formats_by_country,
                    key=lambda e: formats_info[e].get("sequence", 100),
                )  # we use a sequence of 100 by default
        return False

    def _get_suggested_peppol_edi_format(self):
        self.check_singleton()
        suggested_format = (
            self.commercial_partner_id._get_suggested_ubl_cii_edi_format()
        )
        return (
            suggested_format
            if suggested_format in self.env["res.partner"]._get_peppol_formats()
            else "ubl_bis3"
        )
    ...
    def _compute_is_ubl_format(self):
        for partner in self:
            partner.is_ubl_format = (
                partner.invoice_edi_format in self._get_ubl_cii_formats()
            )
    ...
    def _compute_is_peppol_edi_format(self):
        for partner in self:
            partner.is_peppol_edi_format = (
                partner.invoice_edi_format in self._get_peppol_formats()
            )
    ...
    def _compute_peppol_endpoint(self):
        ...
        for partner in self:
            ...
            if country_code in EAS_MAPPING:
                ...
                value = partner._get_peppol_endpoint_value(
                    country_code, field, partner.peppol_eas
                )
                if (
                    field
                    and value
                    and not partner._prepare_error_peppol_endpoint(
                        partner.peppol_eas, value
                    )
                ):
                ...
                if partner.peppol_eas not in eas_to_field.keys():
                    ...
                    for eas, field in eas_to_field.items():
                        if field and field in partner._fields:
                            value = partner._get_peppol_endpoint_value(
                                country_code, field, eas
                            )
                            if value and not partner._prepare_error_peppol_endpoint(
                                eas, value
                            ):
                                new_eas = eas
                                break
    ...
    def _prepare_error_peppol_endpoint(self, eas, endpoint):
        ...
        if eas == "0208" and not re.match(r"^\d{10}$", endpoint):
            return _(
                "The Peppol endpoint is not valid. The expected format is: 0239843188"
            )
        ...
        if eas == "0007" and not re.match(r"^\d{10}$", endpoint):
            return _(
                "The Peppol endpoint is not valid. "
                "It should contain exactly 10 digits (Company Registry number)."
                "The expected format is: 1234567890"
            )
        if (
            PEPPOL_ENDPOINT_INVALIDCHARS_RE.search(endpoint)
            or not 1 <= len(endpoint) <= 50
        ):
            return _(
