...
class AccountEdiUBL(models.AbstractModel):
    ...
    def _ubl_default_tax_subtotal_tax_category_grouping_key(
        self, tax_grouping_key, vals
    ):
    ...
    def _ubl_default_allowance_charge_early_payment_grouping_key(
        self, base_line, tax_data, vals, currency
    ):
        ...
        tax_grouping_key = self._ubl_default_tax_category_grouping_key(
    ...
    def _ubl_default_payable_amount_tax_withholding_grouping_key(
        self, base_line, tax_data, vals, currency
    ):
    ...
    def _ubl_default_base_line_item_classified_tax_category_grouping_key(
        self, base_line, tax_data, vals, currency
    ):
    ...
    def _ubl_turn_emptying_taxes_as_new_base_lines(self, base_lines, company, vals):
        ...
        new_base_lines = AccountTax._dispatch_taxes_into_new_base_lines(
            base_lines, company, exclude_function
        )
    ...
    def _ubl_add_base_line_ubl_values_allowance_charges_recycling_contribution(
        self, vals
    ):
        ...
        for base_line in base_lines:
            ...
            allowance_charges_recycling_contribution = ubl_values[
                "allowance_charges_recycling_contribution"
            ] = []
            allowance_charges_recycling_contribution_currency = ubl_values[
                "allowance_charges_recycling_contribution_currency"
            ] = []
    ...
    def _ubl_add_base_line_ubl_values_allowance_charges_excise(self, vals):
        ...
        for base_line in base_lines:
            ...
            allowance_charges_excise_currency = ubl_values[
                "allowance_charges_excise_currency"
            ] = []
            for tax_data in taxes_data:
    ...
    def _ubl_add_base_line_ubl_values_allowance_charges_discount(self, vals):
        ...
        for base_line in base_lines:
            ...
            if base_line["currency_id"].is_zero(
                raw_discount_amount_currency
    ...
    def _ubl_add_base_line_ubl_values_line_extension_amount(
        self, vals, use_company_currency=False
    ):
        ...
        for base_line in base_lines:
            ...
            amount = (
                ...
                + sum(
                    (1 if allowance_charge_values["is_charge"] else -1)
                    * allowance_charge_values["amount"]
                    for allowance_charge_values in ubl_values[
                        f"allowance_charges_recycling_contribution{suffix}"
                    ]
                )
                + sum(
                    (1 if allowance_charge_values["is_charge"] else -1)
                    * allowance_charge_values["amount"]
                    for allowance_charge_values in ubl_values[
                        f"allowance_charges_excise{suffix}"
                    ]
                )
            )
    ...
    def _ubl_add_base_line_ubl_values_price(self, vals):
        ...
        for base_line in base_lines:
            ...
            for currency_suffix in ("_currency", ""):
                ubl_values[f"price_amount{currency_suffix}"] = tax_details[
                    f"raw_gross_price_unit{currency_suffix}"
                ]

    def _ubl_add_values_tax_currency_code_company_currency_if_foreign_currency(
        self, vals
    ):
        ...
        company = vals["company"]
        currency = vals["currency_id"]
        vals["tax_currency_code"] = (
            None if currency == company.currency_id else company.currency_id.name
        )
    ...
    def _ubl_add_values_tax_currency_code(self, vals):
        ...
        :param vals:    Some custom data.
        """
        self._ubl_add_values_tax_currency_code_company_currency_if_foreign_currency(
            vals
        )
    ...
    def _ubl_add_values_tax_totals(self, vals):
        ...
        def tax_category_grouping_function(base_line, tax_data, sub_currency):
            tax_grouping_key = self._ubl_default_tax_category_grouping_key(
                base_line, tax_data, vals, sub_currency
            )
            if not tax_grouping_key:
                return None
            return self._ubl_default_tax_subtotal_tax_category_grouping_key(
                tax_grouping_key, vals
            )

        def tax_subtotal_grouping_function(base_line, tax_data, sub_currency):
            tax_category_grouping_key = tax_category_grouping_function(
                base_line, tax_data, sub_currency
            )
            if not tax_category_grouping_key:
                return None
            return self._ubl_default_tax_subtotal_grouping_key(
                tax_category_grouping_key, vals
            )

        def tax_totals_grouping_function(base_line, tax_data, sub_currency):
            tax_subtotal_grouping_key = tax_subtotal_grouping_function(
                base_line, tax_data, sub_currency
            )
            if not tax_subtotal_grouping_key:
                return None
            return self._ubl_default_tax_total_grouping_key(
                tax_subtotal_grouping_key, vals
            )
        ...
        for sub_currency, suffix in ((currency, "_currency"), (company_currency, "")):
            ...
            for grouping_key, values in values_per_grouping_key.items():
                ...
                tax_total_grouping_key = self._ubl_default_tax_total_grouping_key(
                    grouping_key, vals
                )
                if not tax_total_grouping_key:
                    continue

                tax_total_values = ubl_values[target_key][
                    frozendict(tax_total_grouping_key)
                ]
            ...
            for grouping_key, values in values_per_grouping_key.items():
                ...
                tax_subtotal_grouping_key = self._ubl_default_tax_subtotal_grouping_key(
                    grouping_key, vals
                )
                if not tax_subtotal_grouping_key:
                    continue
                ...
                tax_total_values["subtotals"][frozendict(tax_subtotal_grouping_key)][
                    "tax_categories"
                ][frozendict(grouping_key)] = {
                    **grouping_key,
                    "base_amount": values[f"base_amount{suffix}"],
                    "tax_amount": sign * values[f"tax_amount{suffix}"],
                }
    ...
    def _ubl_add_values_payable_amount_tax_withholding(self, vals):
        ...
        for sub_currency, suffix in ((currency, "_currency"), (company_currency, "")):
            ...
            for grouping_key, values in values_per_grouping_key.items():
                ...
                ubl_values[f"payable_amount_tax_withholding{suffix}"] -= values[
                    f"tax_amount{suffix}"
                ]
    ...
    def _ubl_add_values_payable_rounding_amount(self, vals):
        ...
        for grouping_key, values in values_per_grouping_key.items():
            ...
            ubl_values["payable_rounding_amount_currency"] += values[
    ...
    def _ubl_add_values_allowance_charge_early_payment(self, vals):
        ...
        for sub_currency, suffix in ((currency, "_currency"), (company_currency, "")):
            ...
            allowance_charges_early_payment = ubl_values[
                f"allowance_charges_early_payment{suffix}"
            ] = []
            for grouping_key, values in values_per_grouping_key.items():
                if not grouping_key:
                    continue
    ...
    def _ubl_get_line_item_commodity_classification_node_from_intrastat_code(
        self, vals, intrastat_code
    ):
        return {
    ...
    def _ubl_get_line_item_commodity_classification_node_from_unspsc_code(
        self, vals, unspsc_code
    ):
        return {
            "cbc:ItemClassificationCode": {
    ...
    def _ubl_get_line_item_commodity_classification_node_from_cpv_code(
        self, vals, cpv_code
    ):
        return {
            "cbc:ItemClassificationCode": {
    ...
    def _ubl_add_line_item_commodity_classification_nodes(self, vals):
        ...
        if self.module_installed("account_intrastat"):
            ...
            if intrastat_code.code:
                nodes.append(
                    self._ubl_get_line_item_commodity_classification_node_from_intrastat_code(
                        vals, intrastat_code
                    )
                )
        ...
        if self.module_installed("product_unspsc"):
            ...
            if unspsc_code.code:
                nodes.append(
                    self._ubl_get_line_item_commodity_classification_node_from_unspsc_code(
                        vals, unspsc_code
                    )
                )
        ...
        if self.module_installed("l10n_ro_cpv_code"):
            ...
            if cpv_code.code:
                nodes.append(
                    self._ubl_get_line_item_commodity_classification_node_from_cpv_code(
                        vals, cpv_code
                    )
                )
    ...
    def _ubl_add_line_item_classified_tax_category_nodes(
        self, vals, in_foreign_currency=True
    ):
        AccountTax = self.env["account.tax"]
        item_node = vals["item_node"]
        base_line = vals["line_vals"]["base_line"]
        ...
        for grouping_key, values in aggregated_values.items():
            ...
            classified_tax_category_nodes.append(
                self._ubl_get_line_item_node_classified_tax_category_node(
                    vals,
                    {
                        **grouping_key,
                        "base_amount": values[f"base_amount{suffix}"],
    ...
    def _ubl_get_line_allowance_charge_recycling_contribution_node(
        self, vals, recycling_contribution_values
    ):
    ...
    def _ubl_add_line_allowance_charge_nodes_for_discount(
        self, vals, in_foreign_currency=True
    ):
        ...
        allowance_charges_nodes.append(
            self._ubl_get_line_allowance_charge_discount_node(
                vals,
                {
                    "currency": currency,
                    "percent": base_line["discount"],
    ...
    def _ubl_add_line_allowance_charge_nodes_for_recycling_contribution_taxes(
        self, vals, in_foreign_currency=True
    ):
        line_node = vals["line_node"]
        base_line = vals["line_vals"]["base_line"]
        ...
        for tax_data in base_line["tax_details"]["taxes_data"]:
            ...
            allowance_charges_nodes.append(
                self._ubl_get_line_allowance_charge_recycling_contribution_node(
                    vals,
                    {
                        "tax": tax_data["tax"],
                        "is_charge": tax_data["tax_amount"] > 0.0,
    ...
    def _ubl_add_line_allowance_charge_nodes_for_excise_taxes(
        self, vals, in_foreign_currency=True
    ):
        line_node = vals["line_node"]
        base_line = vals["line_vals"]["base_line"]
        ...
        for tax_data in base_line["tax_details"]["taxes_data"]:
            ...
            allowance_charges_nodes.append(
                self._ubl_get_line_allowance_charge_excise_node(
                    vals,
                    {
                        "tax": tax_data["tax"],
                        "is_charge": tax_data["tax_amount"] > 0.0,
    ...
    def _ubl_get_allowance_charge_early_payment(self, vals, early_payment_values):
        ...
        return {
            ...
            "cac:TaxCategory": [
                self._ubl_get_allowance_charge_early_payment_tax_category_node(
                    vals, tax_category
                )
    ...
    def _ubl_get_allowance_charge_early_payment_tax_category_node(
        self, vals, tax_category
    ):
        return {
            "_currency": tax_category["currency"],
            "cbc:ID": {"_text": tax_category["tax_category_code"]},
    ...
    def _ubl_get_tax_subtotal_node(self, vals, tax_subtotal):
        ...
        return {
            ...
            "cbc:TaxableAmount": {
                "_text": FloatFmt(
                    tax_subtotal["base_amount"], min_dp=currency.decimal_places
                ),
                "currencyID": currency.name,
            },
            "cbc:TaxAmount": {
                "_text": FloatFmt(
                    tax_subtotal["tax_amount"], min_dp=currency.decimal_places
                ),
                "currencyID": currency.name,
            },
            "cac:TaxCategory": [
    ...
    def _ubl_tax_totals_node_grouping_key(self, base_line, tax_data, vals, currency):
        tax_category_key = self._ubl_default_tax_category_grouping_key(
            base_line, tax_data, vals, currency
        )
    ...
    def _ubl_add_tax_totals_nodes(self, vals):
        ...
        new_base_lines = [
            {
                **base_line,
                "_tax_totals_keys": {
                    (
                        tax_data["tax"] if tax_data else None,
                        currency,
                    ): self._ubl_tax_totals_node_grouping_key(
                        base_line, tax_data, vals, currency
                    )
        ...
        for sub_currency, suffix in iter_currency:
            ...
            for grouping_key, values in values_per_grouping_key.items():
                ...
                tax_total_values = collected_tax_totals_values[target_key][
                    frozendict(tax_total_key)
                ]
            ...
            for grouping_key, values in values_per_grouping_key.items():
                ...
                tax_subtotal_values = tax_total_values["subtotals"][
                    frozendict(tax_subtotal_key)
                ]
