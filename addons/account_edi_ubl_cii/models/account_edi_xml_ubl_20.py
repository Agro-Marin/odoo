...
class AccountEdiXmlUBL20(models.AbstractModel):
    ...
    def _export_invoice(self, invoice):
        ...
        errors = [
            constraint
            for constraint in self._export_invoice_constraints(invoice, vals).values()
            if constraint
        ]
    ...
    def _dispatch_base_lines_recycling_contribution_taxes(
        self, base_lines, company, vals
    ):
        ...
        for base_line in base_lines:
            ...
            recycling_contribution_taxes_data = base_line["_ubl_values"][
            ...
            for tax_data in taxes_data:
                if is_recycling_contribution(tax_data):
                    ...
                    tax_details["raw_total_excluded_currency"] += tax_data[
                        "raw_tax_amount_currency"
                    ]
                    tax_details["raw_total_excluded"] += tax_data["raw_tax_amount"]
                    tax_details["total_excluded_currency"] += tax_data[
                        "tax_amount_currency"
                    ]
    ...
    def _turn_emptying_taxes_as_new_base_lines(self, base_lines, company, vals):
        if not vals["fixed_taxes_as_allowance_charges"]:
            return base_lines
        return self._ubl_turn_emptying_taxes_as_new_base_lines(
            base_lines, company, vals
        )
    ...
    def _setup_base_lines(self, vals):
        ...
        self._dispatch_base_lines_recycling_contribution_taxes(
            base_lines, company, vals
        )

        # Manage taxes for emptying.
        base_lines = self._turn_emptying_taxes_as_new_base_lines(
            base_lines, company, vals
        )
        ...
        document_node.update(
            {
                ...
                "cbc:InvoiceTypeCode": {
                    "_text": 389 if vals["process_type"] == "selfbilling" else 380
                }
                if vals["document_type"] == "invoice"
                else None,
                "cbc:Note": {
                    "_text": html2plaintext(invoice.narration)
                ...
                "cac:OrderReference": {
                    ...
                    "cbc:SalesOrderID": {
                        "_text": ",".join(
                            invoice.invoice_line_ids.sale_line_ids.order_id.mapped(
                                "name"
                            )
                        )
                    }
    ...
    def _add_invoice_delivery_nodes(self, document_node, vals):
        ...
        if (
            gln := "global_location_number" in partner_shipping._fields
            and partner_shipping.global_location_number
        ):
        ...
        document_node[monetary_total_tag].update(
            {
                ...
                "cbc:PayableRoundingAmount": {
                    "_text": self.format_float(
                        vals["cash_rounding_base_amount_currency"], vals["currency_dp"]
                    ),
                    "currencyID": vals["currency_name"],
                }
                if vals["cash_rounding_base_amount_currency"]
                else None,
                "cbc:PayableAmount": {
                    "_text": self.format_float(
                        invoice.amount_residual, vals["currency_dp"]
                    ),
                    "currencyID": vals["currency_name"],
                },
            }
    ...
    def _add_document_currency_vals(self, vals):
        ...
        currency = (
            vals["company_currency_id"]
            if vals["use_company_currency"]
            else vals["currency_id"]
        )
    ...
    def _add_document_monetary_total_vals(self, vals):
        ...
        def fixed_total_grouping_function(base_line, tax_data):
            if (
                vals["fixed_taxes_as_allowance_charges"]
                and tax_data
        ...
        for base_line in vals["base_lines"]:
            aggregated_tax_details = self.env[
                "account.tax"
            ]._aggregate_base_line_tax_details(base_line, fixed_total_grouping_function)
            ...
            for currency_suffix in ["", "_currency"]:
                ...
                if self._is_document_allowance_charge(base_line):
                    if base_line_total_excluded < 0.0:
                        vals[
                            f"total_allowance{currency_suffix}"
                        ] += -base_line_total_excluded
                    else:
        ...
        for currency_suffix in ["", "_currency"]:
            vals[f"tax_exclusive_amount{currency_suffix}"] = (
                vals[f"total_lines{currency_suffix}"]
                + vals[f"total_charge{currency_suffix}"]
        ...
        base_lines_aggregated_tax_details = self.env[
            ...
            vals["base_lines"], non_fixed_total_grouping_function
        )
        aggregated_tax_details = self.env[
            "account.tax"
        ]._aggregate_base_lines_aggregated_values(base_lines_aggregated_tax_details)
        for currency_suffix in ["", "_currency"]:
            vals[f"tax_inclusive_amount{currency_suffix}"] = vals[
                f"tax_exclusive_amount{currency_suffix}"
            ] + sum(
                tax_details[f"tax_amount{currency_suffix}"]
                for grouping_key, tax_details in aggregated_tax_details.items()
                if grouping_key
        ...
        for currency_suffix in ["", "_currency"]:
            ...
            for base_line in vals.setdefault("cash_rounding_base_lines", []):
                tax_details = base_line["tax_details"]
                vals[f"cash_rounding_base_amount{currency_suffix}"] += tax_details[
                    f"total_excluded{currency_suffix}"
                ]
        ...
        party_node = {
            ...
            "cac:PartyName": {
                "cbc:Name": {
                    ...
                    if partner.name
                    else commercial_partner.display_name
                },
            },
    ...
    def _get_financial_account_node(self, vals):
        ...
        if bank:
            financial_institution_branch = {
                "cbc:ID": {"_text": bank.bic, "schemeID": "BIC"},
    ...
    def _add_document_tax_total_nodes(self, document_node, vals):
        ...
        base_lines_aggregated_tax_details = self.env[
            ...
            vals["base_lines"], vals["tax_grouping_function"]
        )
        aggregated_tax_details = self.env[
            "account.tax"
        ]._aggregate_base_lines_aggregated_values(base_lines_aggregated_tax_details)
        document_node["cac:TaxTotal"] = self._get_tax_total_node(
            {
                **vals,
    ...
    def _get_tax_total_node(self, vals):
        ...
        return {
            "cbc:TaxAmount": {
                "_text": self.format_float(
                    sign * total_tax_amount, vals["currency_dp"]
                ),
                "currencyID": vals["currency_name"],
            },
            "cac:TaxSubtotal": [
    ...
    def _get_tax_subtotal_node(self, vals):
        ...
        return {
            "cbc:TaxableAmount": {
                "_text": self.format_float(
                    tax_details[f"base_amount{currency_suffix}"], vals["currency_dp"]
                ),
                "currencyID": vals["currency_name"],
            },
            "cbc:TaxAmount": {
                "_text": self.format_float(
                    sign * tax_details[f"tax_amount{currency_suffix}"],
                    vals["currency_dp"],
                ),
                "currencyID": vals["currency_name"],
            },
            "cbc:Percent": {"_text": grouping_key["amount"]}
            if grouping_key["amount_type"] == "percent"
            else None,
    ...
    def _get_tax_category_node(self, vals):
        ...
        return {
            ...
            "cbc:TaxExemptionReasonCode": {
                "_text": grouping_key.get("tax_exemption_reason_code")
            },
            "cbc:TaxExemptionReason": {
                "_text": grouping_key.get("tax_exemption_reason")
            },
            "cac:TaxScheme": {
                "cbc:ID": {"_text": "VAT"},
            },
    ...
    def _add_document_monetary_total_nodes(self, document_node, vals):
        ...
        document_node[monetary_total_tag] = {
            "cbc:LineExtensionAmount": {
                "_text": self.format_float(
                    vals[f"total_lines{currency_suffix}"], vals["currency_dp"]
                ),
                "currencyID": vals["currency_name"],
            },
            "cbc:TaxExclusiveAmount": {
                "_text": self.format_float(
                    vals[f"tax_exclusive_amount{currency_suffix}"], vals["currency_dp"]
                ),
                "currencyID": vals["currency_name"],
            },
            ...
            "cbc:AllowanceTotalAmount": {
                "_text": self.format_float(
                    vals[f"total_allowance{currency_suffix}"], vals["currency_dp"]
                ),
                "currencyID": vals["currency_name"],
            }
            if vals[f"total_allowance{currency_suffix}"]
            else None,
            "cbc:ChargeTotalAmount": {
                "_text": self.format_float(
                    vals[f"total_charge{currency_suffix}"], vals["currency_dp"]
                ),
                "currencyID": vals["currency_name"],
            }
            if vals[f"total_charge{currency_suffix}"]
            else None,
            "cbc:PrepaidAmount": {
    ...
    def _get_document_allowance_charge_node(self, vals):
        ...
        aggregated_tax_details = self.env[
            "account.tax"
        ]._aggregate_base_line_tax_details(base_line, vals["tax_grouping_function"])
    ...
    def _add_document_line_gross_subtotal_and_discount_vals(self, vals):
        ...
        raw_total_excluded_currency = base_line["tax_details"][
        ...
        for recycling_contribution_tax_data in base_line.get("_ubl_values", {}).get(
            "recycling_contribution_taxes_data", []
        ):
        ...
        if discount_factor != 0.0:
            ...
            gross_subtotal = company_currency.round(
                raw_total_excluded / discount_factor
            )
        else:
            gross_subtotal_currency = base_line["currency_id"].round(
        ...
        line_node.update(
            {
                ...
                "cbc:LineExtensionAmount": {
                    "_text": self.format_float(
                        vals[f"total_excluded{currency_suffix}"], vals["currency_dp"]
                    ),
                    "currencyID": vals["currency_name"],
                },
            }
    ...
    def _add_document_line_allowance_charge_nodes(self, line_node, vals):
        if vals["document_type"] not in {"credit_note", "debit_note"}:
            ...
            line_node["cac:AllowanceCharge"].extend(
                self._get_line_fixed_tax_allowance_charge_nodes(vals)
            )

    def _get_line_discount_allowance_charge_node(self, vals):
        currency_suffix = vals["currency_suffix"]
        if float_is_zero(
            vals[f"discount_amount{currency_suffix}"],
            precision_digits=vals["currency_dp"],
        ):
            return None
        ...
        return {
            "cbc:ChargeIndicator": {
                "_text": "false"
                if vals[f"discount_amount{currency_suffix}"] > 0
                else "true"
            },
            ...
            allowance_charge_nodes.append(
                {
                    "cbc:ChargeIndicator": {
                        "_text": "true"
                        if tax_data[f"tax_amount{currency_suffix}"] > 0
                        else "false"
                    },
    ...
    def _add_document_line_tax_total_nodes(self, line_node, vals):
        ...
        line_node["cac:TaxTotal"] = self._get_tax_total_node(
            {**vals, "aggregated_tax_details": aggregated_tax_details, "role": "line"}
        )
    ...
    def _export_invoice_constraints(self, invoice, vals):
        ...
        constraints.update(
            {
                "ubl20_supplier_name_required": self._check_required_fields(
                    vals["supplier"], "name"
                ),
                "ubl20_customer_name_required": self._check_required_fields(
                    vals["customer"].commercial_partner_id, "name"
                ),
                "ubl20_invoice_name_required": self._check_required_fields(
                    invoice, "name"
                ),
                "ubl20_invoice_date_required": self._check_required_fields(
    ...
    def _import_retrieve_partner_vals(self, tree, role):
        ...
        return {
            "vat": self._find_value(
                f".//cac:{role}Party//cbc:CompanyID[string-length(text()) > 5]", tree
            ),
            "phone": self._find_value(
                f".//cac:{role}Party//cac:Contact//cbc:Telephone", tree
            ),
            "email": self._find_value(
                f".//cac:{role}Party//cac:Contact//cbc:ElectronicMail", tree
            ),
            "name": self._find_value(f".//cac:{role}Party//cbc:RegistrationName", tree)
            or self._find_value(f".//cac:{role}Party//cac:Contact//cbc:Name", tree),
    ...
    def _get_postal_address(self, tree, role):
        return {
            "country_code": self._find_value(
                f".//cac:{role}Party//cac:PostalAddress/cac:Country/cbc:IdentificationCode",
                tree,
            ),
            "street": self._find_value(
                f".//cac:{role}Party//cac:PostalAddress/cbc:StreetName", tree
            ),
            "additional_street": self._find_value(
                f".//cac:{role}Party//cac:PostalAddress/cbc:AdditionalStreetName", tree
            ),
            "city": self._find_value(
                f".//cac:{role}Party//cac:PostalAddress/cbc:CityName", tree
            ),
            "zip": self._find_value(
                f".//cac:{role}Party//cac:PostalAddress/cbc:PostalZone", tree
            ),
            "state_code": self._find_value(
                f".//cac:{role}Party//cac:PostalAddress/cbc:CountrySubentityCode", tree
            ),
        }
    ...
    def _import_fill_invoice(self, invoice, tree, qty_factor):
        ...
        invoice_values["currency_id"], currency_logs = self._import_currency(
            tree, ".//{*}DocumentCurrencyCode"
        )
        ...
        invoice_values["narration"] = self._import_description(
            tree, xpaths=["./{*}Note", "./{*}PaymentTerms/{*}Note"]
        )
        ...
        # ==== invoice_incoterm_id ====
        incoterm_code = tree.findtext(
            "./{*}TransportExecutionTerms/{*}DeliveryTerms/{*}ID"
        )
        if incoterm_code:
        ...
        allowance_charges_line_vals, allowance_charges_logs = (
            self._import_document_allowance_charges(
                tree, invoice, invoice.journal_id.type, qty_factor
            )
        )
        logs += self._import_prepaid_amount(
            invoice, tree, "./{*}LegalMonetaryTotal/{*}PrepaidAmount", qty_factor
        )
        line_tag = (
            "InvoiceLine"
        ...
        invoice_line_vals, line_logs = self._import_lines(
            invoice,
            tree,
            "./{*}" + line_tag,
        ...
        rounding_line_vals, rounding_logs = self._import_rounding_amount(
            invoice,
            tree,
        ...
        invoice_values = {
            **invoice_values,
            "invoice_line_ids": [
                Command.create(line_value) for line_value in line_vals
            ],
        }
    ...
    def _get_line_xpaths(self, document_type=False, qty_factor=1):
        ...
        if document_type == "order":
            ...
            document_type and document_type in ("in_invoice", "out_invoice")
        ) or qty_factor == -1:
    ...
    def _correct_invoice_tax_amount(self, tree, invoice):
        ...
        for elem in tree.findall("./{*}TaxTotal/{*}TaxSubtotal"):
            ...
            if (percentage is not None and percentage.text is not None) and (
                amount is not None and amount.text is not None
            ):
                ...
                if taxes:
                    ...
                    if tax_lines:
                        ...
                        tax_lines_total = currency.round(
                            sign * sum(tax_lines.mapped("amount_currency"))
                        )
