...
class L10nPlAccountPaymentRegister(models.TransientModel):
    ...
    l10n_pl_bank_verification_ids = fields.Many2many(
    ...
    def _compute_l10n_pl_bank_verification(self):
        ...
        for wizard in self:
            ...
            for batch in wizard.batches:
                if self._batch_need_check(batch):
                    partner_to_partner_banks[batch["payment_values"]["partner_id"]] |= (
                        self._get_partner_bank_from_batch(batch)
                    )

            partner_bank_data = [
                (partner_id, partner_banks)
                for partner_id, partner_banks in partner_to_partner_banks.items()
            ]
            verifications = (
            ...
            wizard.sudo().l10n_pl_bank_verification_ids = verifications
