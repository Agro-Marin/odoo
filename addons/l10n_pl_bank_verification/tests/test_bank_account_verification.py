...
class TestL10nPlBankAccountVerification(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.pl_supplier, cls.pl_supplier_bank_account, cls.pl_supplier_move = (
            cls._create_partner_bank_and_move(
                cls, "1111111111", "PL61109010140000071219812874"
            )
        )  # valid bank account number
        cls.invoicing_user = new_test_user(
            cls.env, login="invoicing_user", groups="account.group_account_invoice"
        )
    ...
    def _create_payments_for_moves(self, moves):
        ...
        if action_create_payment.get("res_id"):
            return self.env[action_create_payment["res_model"]].browse(
                action_create_payment["res_id"]
            )
        return self.env[action_create_payment["res_model"]].search(
            action_create_payment["domain"]
        )
    ...
    def _create_partner_bank_and_move(
        self, vat, account_number=False, amount=15000, country="pl", post_move=True
    ):
    ...
    def _check_form_fields(
        self,
        moves,
        invalid_bank_accounts=False,
        not_found_partners=False,
        incomplete_partners=False,
    ):
        """
        ...
        with Form.from_action(self.env, moves.action_register_payment()) as wiz_form:
            ...
            self.assertEqual(
                ...
                (not_found_partners or self.env["res.partner"]).ids,
            )
            self.assertEqual(
                wiz_form.l10n_pl_incomplete_data_partner_ids.ids,
                (incomplete_partners or self.env["res.partner"]).ids,
            )
    ...
    def test_register_single_payment_with_valid_bank_account_check(self):
        ...
        self.assertRecordValues(
            payment.l10n_pl_verification_id,
            [
                {
    ...
    def test_register_single_payment_with_valid_bank_account_check_as_invoicing_user(
        ...
        with self.assertRaises(AccessError):
            self.env["l10n_pl.bank.account.verification"].with_user(
                self.invoicing_user
            ).search([])
    ...
    def test_register_single_payment_with_null_subject_result(self):
        supplier, supplier_bank, move = self._create_partner_bank_and_move(
            "PL3333333333", "61109010140000071219812999"
    ...
    def test_register_multiple_payments_with_an_invalid_vat(self):
        supplier, supplier_bank, move = self._create_partner_bank_and_move(
            "0000000000", "61109010140000071219812870"
        )  # not found vat number
        ...
        self.assertRecordValues(
            payments.l10n_pl_verification_id,
            [
                {
                    "verification_status": "not_found_partner",
                    "verification_timestamp": self.date,
    ...
    def test_register_multiple_payments_with_valid_bank_account_check(self):
        supplier, supplier_bank, move = self._create_partner_bank_and_move(
            "2222222222", "61109010140000071219812875"
        )  # valid bank account number
        moves = move + self.pl_supplier_move
    ...
    def test_register_multiple_payments_with_one_bank_account_not_found(self):
        supplier, bank_account, move = self._create_partner_bank_and_move(
            "PL2222222222", "61109010140000071219812800"
        )  # invalid bank account number
        moves = move + self.pl_supplier_move
    ...
    def test_register_payment_under_verification_limit(self):
        ...
        _supplier, _bank_account, move_other_partner = (
            self._create_partner_bank_and_move(
                "PL2222222222", "61109010140000071219812800", amount=500
            )
        )
    ...
    def test_create_single_payment_invalid_bank_account(self):
        self.pl_supplier_bank_account.acc_number = (
            "PL61109010140000071219812999"  # invalid
        )
        payment = self._create_payment()
        self.assertRecordValues(
            payment.l10n_pl_verification_id,
    ...
    def test_partner_no_vat_dont_create_multiple_verifications(self):
        ...
        self._check_form_fields(
            self.pl_supplier_move, incomplete_partners=self.pl_supplier
        )
        verification = self.env["l10n_pl.bank.account.verification"].search([])
