...
class TestStockValuationCommon(BaseCommon):
    ...
    def _create_invoice(
        self, product=None, quantity=1.0, price_unit=None, post=True, **kwargs
    ):
        return self._create_account_move(
            "out_invoice", product, quantity, price_unit, post, **kwargs
        )
    ...
    def _create_bill(
        ...
        return self._create_account_move(
            "in_invoice", product, quantity, price_unit, post, **kwargs
        )

    def _create_credit_note(
        self, product, quantity=1.0, price_unit=1.0, post=True, **kwargs
    ):
        move_type = kwargs.pop("move_type", "out_refund")
        return self._create_account_move(
            move_type, product, quantity, price_unit, post, **kwargs
        )

    def _refund(self, move_to_refund, quantity=None, post=True):
        reversal = (
            self.env["account.move.reversal"]
            .with_context(active_ids=move_to_refund.ids, active_model="account.move")
            .create(
                {
    ...
    def _close(self, auto_post=True, at_date=None):
        action = self.company.action_close_stock_valuation(
            at_date=at_date, auto_post=auto_post
        )
    ...
    def _use_price_diff(self):
        ...
        self.category_standard.property_price_difference_account_id = (
            self.account_price_diff.id
        )
        self.category_standard_auto.property_price_difference_account_id = (
            self.account_price_diff.id
        )
    ...
    def _use_inventory_location_accounting(self):
        ...
        inventory_locations = self.env["stock.location"].search(
            [("usage", "=", "inventory"), ("company_id", "=", self.company.id)]
        )
        ...
        out_move = env["stock.move"].create(
            {
                ...
                "location_dest_id": kwargs.get(
                    "location_dest_id", self.customer_location.id
                ),
                "product_uom_id": kwargs.get("uom_id", self.uom.id),
                "product_uom_qty": quantity,
                "picking_type_id": kwargs.get(
    ...
    def setUpClass(cls):
        ...
        cls.inventory_location = cls.env["stock.location"].search(
            [("usage", "=", "inventory"), ("company_id", "=", cls.company.id)], limit=1
        )
        ...
        cls.branch = cls._create_company(
        ...
        with freeze_time(fields.Datetime.now() - timedelta(seconds=10)):
            ...
            cls.product = (
                ...
                .create({**product_common_vals, "name": "Storable Product"})
                .with_context(clean_context(cls.env.context))
            )
            cls.product_standard = (
                cls.env["product.product"]
                .create(
                    {
                        **product_common_vals,
