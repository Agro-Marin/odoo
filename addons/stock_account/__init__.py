...
def _create_product_value(env):
    ...
    for company in env["res.company"].search([]):
        ...
        product_vals_list += [
            ...
            if not product.company_id or product.company_id == company
        ]
...
def _configure_journals(env):
    ...
    for company in env["res.company"].search(
        ...
        data["res.company"] = {
            company.id: {
                "account_stock_journal_id": full_data["res.company"][company.id].get(
                    "account_stock_journal_id"
                ),
                "account_stock_valuation_id": full_data["res.company"][company.id].get(
        ...
        if journal:
            ...
            data["account.journal"] = ChartTemplate._get_stock_account_journal(
                template_code
            )
