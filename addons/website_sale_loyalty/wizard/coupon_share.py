...
class CouponShare(models.TransientModel):
    ...
    def _check_program(self):
        if self.filtered(
            lambda record: (
                not record.coupon_id and record.program_id.program_type == "coupons"
            )
        ):
    ...
    def _compute_promo_code(self):
        for record in self:
            record.promo_code = (
                record.coupon_id.code
                or record.program_id.rule_ids.filtered("code")[:1].code
            )
