...
class WebsiteSale(main.WebsiteSale):
    ...
    def pricelist(self, promo, reward_id=None, **post):
        ...
        if coupon_status.get("not_found"):
            ...
            if len(coupon_status) == 1:
                ...
                if reward and (
                    not reward.multi_product or request.env.context.get("product_id")
                ):
                    reward_successfully_applied = self._apply_reward(
                        order_sudo, reward, coupon
                    )
    ...
    def activate_coupon(self, code, r="/shop", **kw):
        url_parts = urlsplit(r)
        url_query = dict(parse_qsl(url_parts.query))
        url_query.pop("coupon_error", False)
        ...
        redirect = urlunsplit(url_parts._replace(query=urlencode(url_query)))
    ...
    def _apply_reward(self, order, reward, coupon):
        ...
        product = product_id and request.env["product.product"].sudo().browse(
            product_id
        )
        try:
