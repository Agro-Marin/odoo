...
class TestHealthCheckInterval(TransactionCase):
    ...
    def test_the_cron_only_checks_endpoints_whose_interval_has_passed(self):
        Endpoint = self.env["api.endpoint.outbound"]
...
class TestPerEndpointLogRetention(TransactionCase):
    ...
    def _old_log(self, endpoint, days):
        log = self.env["api.event.log"].create(
            {
                "direction": "outbound",
                "channel_id": f"api.endpoint.outbound,{endpoint.id}",
                "request_method": "GET",
        ...
        self.env.cr.execute(
            "UPDATE api_event_log SET date_completed = %s WHERE id = %s",
    ...
    def test_an_endpoint_retention_overrides_the_global_one(self):
        self.env["ir.config_parameter"].sudo().set_param(
            "api_transport.log_retention_days", "90"
        )
