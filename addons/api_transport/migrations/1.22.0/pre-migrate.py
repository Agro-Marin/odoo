...
def _drop_stale_crons(cr):
    """Remove the three crons api_transport replaced, companion included.
...
def _group_id(cr, name):
    ...
    searched `api_transport` alone, found nothing and merged nothing. Accepting
...
def _carry_grants(cr, old_id, new_id):
    ...
    whatever the old one did. For `api.endpoint.outbound` that holds; for
...
def migrate(cr, version):
    ...
    under `api_transport`, where 1.6.0 was supposed to have put them, found none
    ...
    api_transport does not declare those menus, views and actions, so adopting
    ...
    if remaining:
        _logger.warning(
            "19.0.1.22.0: %s still owns %s. api_transport does not declare them, "
