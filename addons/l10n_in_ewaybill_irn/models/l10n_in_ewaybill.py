...
class L10nInEwaybill(models.Model):
    ...
    def _compute_is_process_through_irn(self):
        for ewaybill in self:
            ...
            ewaybill.is_process_through_irn = (
                ...
                and not (
                    account_move_id.move_type == "out_refund"
                    or account_move_id.debit_origin_id
                )
            ...
            return dict(
                filter(
                    lambda kv: kv[1],
                    {
                        ...
                        "TransDocDt": self.transportation_doc_date
    ...
    def _ewaybill_generate_irn_json(self):
        ...
        if self.account_move_id.l10n_in_gst_treatment == "overseas":
            json_payload["ExpShipDtls"] = self.env[
                "account.move"
            ]._get_l10n_in_edi_partner_details(
                self.partner_ship_to_id, set_phone_and_email=False, set_vat=False
            )
            json_payload["ExpShipDtls"].pop(
                "Nm"
            )  # 'Nm'(Name) is not included in ExpShipDtls as per the JSON schema
        return json_payload
    ...
    def _generate_ewaybill_by_irn(self):
        ...
        self._lock_ewaybill()
        try:
            response = self._ewaybill_generate_by_irn(
                self._ewaybill_generate_irn_json()
            )
    ...
    def _get_edi_irn_number(self):
        self.check_singleton()
        l10n_in_edi_response_json = (
            self.account_move_id._get_l10n_in_edi_response_json()
        )
                ...
                response.update(
                    {
                        "odoo_warning": [
                            {
                                "message": self._get_default_help_message(
