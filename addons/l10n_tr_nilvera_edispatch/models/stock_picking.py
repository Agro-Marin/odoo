...
class StockPicking(models.Model):
        ...
        for picking in self:
            ...
            if picking.partner_id:
                ...
                picking.message_post(
                    body=self.env._(
                        "e-Dispatch will not be generated as the Delivery Address is not set."
                    )
                )
        return res
    ...
    def _l10n_tr_get_edispatch_errors_on_done(self):
        ...
        if self.l10n_tr_nilvera_dispatch_type == "MATBUDAN":
            ...
            if (
                ...
                error_messages["invalid_matbudan_number"] = {
                    "message": self.env._(
                        "Printed Delivery Note Number of 16 characters is required."
                    ),
                }
        ...
        invalid_tckn_drivers = (
            self.l10n_tr_nilvera_driver_ids - invalid_country_drivers
        ).filtered(
        ...
        if drivers := len(invalid_country_drivers):
            error_messages["invalid_driver_country"] = {
                ...
                "action_text": self.env._(
                    "View %s",
                    (drivers == 1 and invalid_country_drivers.name)
                    or self.env._("Drivers"),
                ),
        ...
        if drivers := len(invalid_tckn_drivers):
            driver_placeholder = (drivers > 1 and self.env._("Drivers")) or self.env._(
                "%s's", invalid_tckn_drivers.name
            )
            error_messages["invalid_driver_tckn"] = {
                "message": self.env._("%s TCKN is required.", driver_placeholder),
                "action_text": self.env._(
                    "View %s",
                    (drivers == 1 and invalid_tckn_drivers.name)
                    or self.env._("Drivers"),
                ),
                "action": invalid_tckn_drivers._get_records_action(
                    name=self.env._("Drivers")
                ),
            }
        ...
        if (
            ...
            error_messages["required_carrier_details"] = {
                "message": self.env._(
                    "Carrier is required (optional when both the Driver and Vehicle Plate are filled)."
                ),
            }

        elif (
            not self.l10n_tr_nilvera_carrier_id and not self.l10n_tr_nilvera_driver_ids
        ):
        ...
        if self.state not in {"assigned", "done"}:
            return {
                "invalid_transfer_state": {
                    "message": self.env._(
                        "Please validate the transfer first to generate the XML"
                    ),
                }
            }
    ...
    def _l10n_tr_generate_edispatch_xml(self):
        ...
        values = {
            ...
            "printed_date": self.l10n_tr_nilvera_delivery_date
            and self.l10n_tr_nilvera_delivery_date.strftime("%Y-%m-%d"),
            "drivers": drivers,
        ...
        xml_content = self.env["ir.qweb"]._render(
            "l10n_tr_nilvera_edispatch.l10n_tr_edispatch_format", values
        )
    ...
    def _get_partner_vals_from_xml(self, tree, xpath):
        ...
        if party is None:
            return None
        return {
            "name": self._get_tag_text("./cac:PartyName/cbc:Name", party)
            or f"{self._get_tag_text('./cac:Person/cbc:FirstName', party)} {self._get_tag_text('./cac:Person/cbc:FamilyName', party)}",
            "vat": self._get_tag_text(
                './cac:PartyIdentification/cbc:ID[@schemeID="VKN" or @schemeID="TCKN"]',
                party,
            ),
            "street": self._get_tag_text("./cac:PostalAddress/cbc:StreetName", party),
            "city": self._get_tag_text(
                "./cac:PostalAddress/cbc:CitySubdivisionName", party
            ),
            ...
            "country": self._get_tag_text(
                "./cac:PostalAddress/cac:Country/cbc:Name", party
            ),
    ...
    def _create_partner_from_xml(self, partner_vals):
        if (state := partner_vals.pop("state", None)) and (
            ...
            .with_context(lang="tr_TR")
            .search([("name", "=", country)], limit=1)
        ):
            partner_vals.update({"country_id": country_id.id, "code": country_id.code})

        if (code := partner_vals.pop("code", None)) and code != "TR":
            partner_vals["l10n_tr_nilvera_edispatch_customs_zip"] = partner_vals.pop(
                "zip", ""
            )
    ...
    def _get_or_create_products_from_xml(self, receipt_lines):
        product_names = [
            self._get_tag_text("./cac:Item/cbc:Name", receipt)
        ...
        for receipt in receipt_lines:
            ...
            if name not in existing_products:
                unece_code = receipt.find(
                    "./cbc:DeliveredQuantity", namespaces=UBL_NAMESPACES
                ).get("unitCode", "")
                products_to_create.append(
                    {
                        "name": name,
                        "default_code": self._get_tag_text(
                            "./cac:Item/cac:SellersItemIdentification/cbc:ID", receipt
                        ),
                        "uom_id": self.env["uom.uom"]
                        ._get_uom_from_unece_code(unece_code)
                        .id,
                    }
                )
        ...
        if products_to_create:
            ...
            existing_products.update(
                {product.name: product.id for product in created_products}
            )
            ...
            values.append(
                {
                    ...
                    "product_uom_qty": self._get_tag_text(
                        "./cbc:DeliveredQuantity", receipt
                    ),
                    "picking_id": self.id,
    ...
    def _import_vehicle_plate(self, tree):
        vehicle_plate = self._get_tag_text(
            ".//cac:RoadTransport/cbc:LicensePlateID", tree
        )
        if not vehicle_plate:
            return None
        vehicle_plate_id = self.env["l10n_tr.nilvera.trailer.plate"].search_fetch(
            [("name", "=", vehicle_plate), ("plate_number_type", "=", "vehicle")],
            ["id"],
            limit=1,
        )
    ...
    def _import_trailer_plate_ids(self, tree):
        plate_ids = []
        trailer_plates = tree.findall(
            ".//cac:TransportHandlingUnit/cac:TransportEquipment",
            namespaces=UBL_NAMESPACES,
        )
        existing_plates = dict(
            self.env["l10n_tr.nilvera.trailer.plate"]._read_group(
    ...
    def _import_drivers(self, tree):
        ...
        existing_partners = dict(
            ResPartner.with_context(active_test=False)._read_group(
                [("country_id.code", "=", "TR"), ("is_company", "=", False)],
                ["name"],
                ["id:min"],
            )
        )
        ...
        if partners_to_create:
            partner_id = ResPartner.with_context(no_vat_validation=True).create(
                partners_to_create
            )
    ...
    def _import_matbudan_data(self, tree):
        additional_doc_infos = tree.findall(
            ".//cac:AdditionalDocumentReference", namespaces=UBL_NAMESPACES
        )
        for doc in additional_doc_infos:
            if self._get_tag_text("./cbc:DocumentType", doc) == "MATBU":
                return {
                    "l10n_tr_nilvera_delivery_date": self._get_tag_text(
                        "./cbc:IssueDate", doc
                    ),
                    "l10n_tr_nilvera_delivery_printed_number": self._get_tag_text(
                        "./cbc:ID", doc
                    ),
                }
        return None
    ...
    def _import_partners(self, tree):
        ...
        existing_dict = {
            partner["vat"] or partner["name"]: partner["id"]
        ...
        for xpath, vals in partner_data.items():
            ...
            partners_vals[xpath_to_field[xpath]] = existing_dict.get(
                key
            ) or self._create_partner_from_xml(vals)
    ...
    def _import_edispatch_fields(self, tree):
        vals = {
            ...
            "l10n_tr_nilvera_dispatch_type": self._get_tag_text(
                "./cbc:DespatchAdviceTypeCode", tree
            ),
        }

        if vals["l10n_tr_nilvera_dispatch_type"] == "MATBUDAN" and (
            matbu_info := self._import_matbudan_data(tree)
        ):
    ...
    def _update_data_from_xml(self, file_data):
        ...
        vals_to_update = {
            ...
            "origin": self._get_tag_text(
                "./cbc:ID", tree
            ),  # sequence of the e-Receipt obtained from XML.
            "move_ids": [
                Command.create(value) for value in self._import_receipt_lines(tree)
            ],
        }
        ...
        self.message_post(
            body=self.env._("e-Receipt uploaded successfully."),
            attachment_ids=[file_data["attachment"].id],
        )
    ...
    def l10n_tr_import_ereceipts(self, attachment_ids):
        ...
        picking_ids, files_with_errors = self._l10n_tr_create_receipts_from_attachment(
