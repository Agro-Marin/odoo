...
class OrderMixin(models.AbstractModel):
    ...
    # Control fields
    # Note: Using actual field names from sale/purchase modules for compatibility
    ...
    # Note: Using actual field names from sale/purchase (sent, printed_before, count_sent, count_print)
    sent = fields.Boolean(
        string="Sent",
        default=False,
        copy=False,
    ...
    partner_ref = fields.Char(
        ...
        copy=False,
        help="Partner's reference for this order (vendor reference, customer PO, etc.)",
    )
    ...
    # Invoicing journal (common to sale and purchase)
    journal_id = fields.Many2one(
        ...
        readonly=False,
        check_company=True,
        help="Journal for invoicing this order",
    )
    ...
    is_expired = fields.Boolean(
        ...
        help="True if order is draft and past validity date",
    )

    # Display type name (Quotation vs Order)
    type_name = fields.Char(
        string="Type Name",
        compute="_compute_type_name",
        help="Human readable order type name",
    )
    ...
    def _compute_journal_id(self):
        """Compute default journal for invoicing.

        Default implementation sets to False. Child models should override
        to select appropriate journal based on order type (sale/purchase).
        """
        self.journal_id = False

    @api.depends("state", "date_validity")
    def _compute_is_expired(self):
        """Check if order is expired based on validity date."""
    ...
    def _compute_type_name(self):
        """Compute human-readable type name.

        Child models should override to provide order-type specific names:
        - Sale: "Quotation" / "Sales Order"
        ...
        if locked_orders:
            raise UserError(
                _(
                    ...
                    ", ".join(locked_orders.mapped("name")),
                )
            )
