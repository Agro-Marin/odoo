...
class MixinOrderLineFields(models.AbstractModel):
    ...
    abstract models (``mixin.order``, and this mixin itself) so every other
    ...
    #
    # It must point at ``mixin.order`` rather than be left undeclared: a
    ...
    # up on ``mixin.order`` at registry setup. Leaving ``order_id`` out — or
    ...
    def _get_merge_date_field(self):
        ...
        ``mixin.order.merge`` treats every pair as compatible. purchase.order.line
