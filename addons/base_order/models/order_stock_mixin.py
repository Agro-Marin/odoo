...
class OrderStockMixin(models.AbstractModel):
        ...
        for order in self:
            ...
            # Some transfers done - check if partial or just started
                ...
                if order._has_partial_transfer():
                    ...
                    order.transfer_status = order._get_started_transfer_status()
    ...
    def _get_transfer_status_selection(self):
        ...
        Examples:
            - Sale: [('pending', 'Not Delivered'), ('partial', 'Partially Delivered'), ...]
            - Purchase: [('pending', 'Not Received'), ('partial', 'Partially Received'), ...]
    ...
    def _get_no_transfer_status(self):
        ...
        Returns:
            str: Status value (e.g., False, 'pending', 'no')
        """
        return False
    ...
    def _get_started_transfer_status(self):
        """Return status value when transfer started but no qty transferred.

        Examples:
            - Sale: 'started'
            - Purchase: 'partial' (no started state)
...
class OrderLineStockMixin(models.AbstractModel):
    ...
    move_ids = fields.One2many(
        ...
        inverse_name="sale_line_id",  # Override in child models
        string="Stock Moves",
        help="Stock moves related to this order line",
    )
