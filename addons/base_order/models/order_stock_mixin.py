...
class OrderStockMixin(models.AbstractModel):
    ...
    # Previously duplicated in sale_stock and purchase_stock
    incoterm_id = fields.Many2one(
        comodel_name="account.incoterms",
        string="Incoterm",
        help="International Commercial Terms - define responsibilities for delivery",
    )
    incoterm_location = fields.Char(
        string="Incoterm Location",
        help="Named place for the selected Incoterm",
    )

    # Stock references for tracking
    # Previously: stock_reference_ids (sale) / reference_ids (purchase)
    reference_ids = fields.Many2many(
        comodel_name="stock.reference",
        string="Stock References",
        copy=False,
        help="References linking this order to stock operations",
    )

    # Picking policy (how to handle partial availability)
    # Previously only in sale_stock, now available for all order types
    picking_policy = fields.Selection(
        ...
        default="direct",
        required=True,
        help="If 'As soon as possible', pickings will be created immediately. "
             "If 'When all products are ready', pickings will wait until all "
    ...
    def _add_reference(self, references):
        """Link stock references to this order.

        Args:
            references: stock.reference recordset to link
        """
        self.ensure_one()
        self.reference_ids = [Command.link(ref.id) for ref in references]

    def _remove_reference(self, references):
        """Unlink stock references from this order.

        Args:
            references: stock.reference recordset to unlink
        """
        self.ensure_one()
        self.reference_ids = [Command.unlink(ref.id) for ref in references]
...
class OrderLineStockMixin(models.AbstractModel):
    ...
    # Cancellation propagation (previously only in purchase_stock)
    propagate_cancel = fields.Boolean(
        string="Propagate Cancellation",
        default=True,
        help="If checked, cancelling this line will also cancel related stock moves. "
             "If unchecked, stock moves will be converted to make-to-stock.",
    )

    # Final destination location (previously only in purchase_stock)
    location_final_id = fields.Many2one(
        comodel_name="stock.location",
        string="Final Location",
        help="Final destination location from procurement. "
             "Used when the immediate destination differs from final destination.",
    )

    # Make-to-Order detection (previously only in sale_stock)
    is_mto = fields.Boolean(
        string="Is MTO",
        compute="_compute_is_mto",
        help="True if this line uses Make-to-Order procurement",
    )

    # Lead time for delivery/receipt (previously customer_lead in sale only)
    lead_time = fields.Float(
        string="Lead Time",
        digits="Stock Delay",
        default=0.0,
        help="Number of days for delivery/receipt lead time",
    )

    # Forecasted issue warning (previously only in purchase_stock)
    forecasted_issue = fields.Boolean(
        string="Forecasted Issue",
        compute="_compute_forecasted_issue",
        help="Warning: negative stock forecast at planned date",
    )
    ...
    def _compute_is_mto(self):
        """Compute if line uses Make-to-Order procurement.

        A line is MTO if its product has an MTO route configured.
        Child models can override for custom MTO detection logic.
        """
        for line in self:
            line.is_mto = line._is_mto_line()

    def _compute_forecasted_issue(self):
        """Compute if there's a negative stock forecast at planned date.

        Default implementation sets to False. Child models should override
        to implement actual forecast checking based on warehouse context.
        """
        for line in self:
    ...
    def _is_mto_line(self):
        """Check if this line uses Make-to-Order procurement.

        Default implementation checks for MTO route on product.
        Child models can override for custom logic.

        Returns:
            bool: True if MTO line
        """
        self.ensure_one()
        ...
        return mto_route in self.product_id.route_ids
