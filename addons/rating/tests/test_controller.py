...
class TestControllersRoute(HttpCaseWithUserDemo, HttpCaseWithUserPortal):
    ...
    def test_controller_rating(self):
        [rating_test_1, rating_test_2, rating_test_3] = (
        ...
        for login, rating_test, access_token, expected_consume in details:
            with self.subTest(
                ...
                message = (
                    b"You cannot rate this"
                    if login == self.user_demo.login
                    else b"Feel free to share feedback on your experience:"
                )
