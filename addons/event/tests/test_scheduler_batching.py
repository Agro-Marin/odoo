...
class TestSchedulerBatching(EventCase):
    ...
    def test_attendee_scheduler_mails_each_attendee_once_past_the_chunk_size(self):
        ...
        self.env["event.registration"].with_context(install_mode=True).create(
            [
                {
                    "email": f"a{idx}@test.example.com",
                    "event_id": event.id,
                    "name": f"A{idx}",
                }
        ...
        self.assertEqual(
            self.env["event.mail.registration"].search_count(
                [("scheduler_id", "=", scheduler.id)]
            ),
            count,
        ...
        self.assertEqual(
            sorted(set(sent.values())), [1], "each attendee mailed exactly once"
        )
    ...
    def test_slot_scheduler_mails_every_attendee_across_cron_runs(self):
        ...
        for multi in (False, True):
            ...
            with mock_send:
                for _ in range(8):
                    ...
                    self.env.flush_all()
            results[multi] = [
                sent.get(registration.id, 0) for registration in registrations
            ]
        ...
        self.assertEqual(
            sorted(set(results[True])), [1], "each slot attendee mailed exactly once"
        )
