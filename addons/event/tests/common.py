...
class EventCase(common.TransactionCase):
        ...
        cls.event_organizer = cls.env["res.partner"].create(
            {
                ...
                "name": "Organizer",
                "phone_ids": [
                    Command.create({"number": "+32455123456", "type": "landline"})
                ],
        ...
        cls.event_customer2 = cls.env["res.partner"].create(
            {
                ...
                "country_id": cls.env.ref("base.be").id,
                "phone_ids": [
                    Command.create({"number": "0456987654", "type": "landline"})
                ],
            }
        )
            ...
            [
                {
                    ...
                    "phone_ids": [
                        Command.create(
                            {"number": f"04560000{idx}{idx}", "type": "landline"}
                        )
                    ],
                }
    ...
    @classmethod
    def _create_registrations_for_slot_and_ticket(
        cls, event, slot, ticket, count, **add_values
    ):
        return cls.env["event.registration"].create(
            [
                dict(
