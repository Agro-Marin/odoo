...
class EventMailCommon(EventCase, MailCase, CronMixinCase):
    ...
    def setUpClass(cls):
        ...
        cls.env["ir.config_parameter"].sudo().set_param(
            "crm.iap.lead.enrich.setting", "no"
        )
        ...
        with cls.mock_datetime_and_now(cls, cls.reference_now):
            ...
            cls.test_event = (
                cls.env["event.event"]
                .with_user(cls.user_eventmanager)
                .create(
                    {
                        "name": "TestEventMail",
...
class TestMailSchedule(EventMailCommon):
    def test_assert_initial_values(self):
        ...
        self.assertEqual(
            test_event.date_begin, self.event_date_begin, "Expressed in current user TZ"
        )
        self.assertEqual(
            test_event.date_end, self.event_date_end, "Expressed in current user TZ"
        )
        self.assertEqual(test_event.date_tz, "Europe/Brussels")
        self.assertEqual(
            test_event.organizer_id, self.user_eventmanager.company_id.partner_id
        )
        ...
        self.assertEqual(
            after_sub_scheduler.scheduled_date,
            test_event.create_date.replace(microsecond=0),
        )
        ...
        self.assertEqual(
            len(after_sub_scheduler_2), 1, "event: wrong scheduler creation"
        )
        self.assertFalse(after_sub_scheduler_2.error_datetime)
        self.assertEqual(
            after_sub_scheduler_2.scheduled_date,
        ...
        self.assertEqual(
            len(event_prev_scheduler), 1, "event: wrong scheduler creation"
        )
        self.assertFalse(event_prev_scheduler.error_datetime)
        self.assertEqual(
            event_prev_scheduler.scheduled_date,
            self.event_date_begin + relativedelta(days=-1),
        )
        ...
        self.assertEqual(
            len(event_next_scheduler), 1, "event: wrong scheduler creation"
        )
        self.assertFalse(event_next_scheduler.error_datetime)
        self.assertEqual(
            event_next_scheduler.scheduled_date,
            self.event_date_end + relativedelta(hours=1),
        )
    ...
    def test_event_mail_schedule(self):
        ...
        after_sub_scheduler = schedulers.filtered(
        ...
        event_next_scheduler = schedulers.filtered(
            lambda s: s.interval_type == "after_event"
        )
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "mail.render.cron.limit", render_limit
        )
        ...
        self.assertTrue(
            all(reg.state == "open" for reg in attendees[:15]),
            "Registrations: should be auto-confirmed",
        )
        self.assertListEqual(attendees[15:].mapped("state"), ["draft", "cancel"])
        self.assertTrue(
            all(reg.create_date == now for reg in attendees),
            "Registrations: should have open date set to confirm date",
        )

        # verify that subscription scheduler was auto-executed after each registration
        self.assertEqual(
            len(after_sub_scheduler.mail_registration_ids),
            15,
            "Should have 15 scheduled communication (1 / registration), as we schedule more"
            "than cron limit as create should be quick.",
        )
        for idx, mail_registration in enumerate(
            after_sub_scheduler.mail_registration_ids
        ):
            self.assertEqual(
                mail_registration.scheduled_date, now.replace(microsecond=0)
            )
            if idx < 10:
                self.assertTrue(
        ...
        self.assertEqual(after_sub_scheduler.mail_state, "running")
        self.assertEqual(
            after_sub_scheduler.mail_count_done,
            10,
        ...
        self.assertEqual(
            len(self._new_mails),
            10,
            "event: should have 10 scheduled emails (1 / executed registration)",
        )
        ...
        self.assertEqual(
            len(after_sub_scheduler_2.mail_registration_ids),
            15,
        ...
        for mail_registration in after_sub_scheduler_2.mail_registration_ids:
            self.assertEqual(
                mail_registration.scheduled_date,
                now.replace(microsecond=0) + relativedelta(hours=1),
            )
            self.assertFalse(
                mail_registration.mail_sent,
                "event: registration mail should be scheduled, not sent",
            )
        self.assertEqual(
            after_sub_scheduler_2.mail_count_done,
            0,
        ...
        self.assertEqual(
            len(self._new_mails),
            5,
            "event: should have 5 scheduled emails (1 / executed registration)",
        )
        self.assertMailMailWEmails(
        ...
        self.assertFalse(
            any(
                mail_reg.mail_sent
                for mail_reg in after_sub_scheduler_2.mail_registration_ids
            )
        )
        self.assertEqual(after_sub_scheduler_2.mail_count_done, 0)
        self.assertEqual(
            len(self._new_mails),
            0,
            "event: should not send mails before scheduled date",
        )
        ...
        self.assertEqual(
            len(self._new_mails),
            15,
            "event: should have 15 scheduled emails (1 / registration)",
        )
        self.assertMailMailWEmails(
        ...
        self.assertTrue(
            event_prev_scheduler.mail_done, "event: reminder scheduler should have run"
        )
        self.assertEqual(
            event_prev_scheduler.mail_state,
            "sent",
            "event: reminder scheduler should have run",
        )
        ...
        self.assertEqual(
            len(self._new_mails),
            15,
            "event: should have scheduled 15 mails (1 / registration)",
        )
        self.assertMailMailWEmails(
            [formataddr((reg.name, reg.email)) for reg in attendees[:15]],
            "outgoing",
        ...
        with (
            self.mock_datetime_and_now(now_start + relativedelta(hours=1)),
            self.mock_mail_gateway(),
        ):
        ...
        self.assertEqual(
            new_mail_reg.scheduled_date,
            now_start.replace(microsecond=0) + relativedelta(hours=1),
        )
        self.assertTrue(
            new_mail_reg.mail_sent,
            "event: registration mail should be sent at registration creation",
        ...
        self.assertEqual(
            len(self._new_mails),
            2,
            "event: should have 1 scheduled emails (new registration only)",
        )
        # manual check because 2 identical mails are sent and mail tools do not support it easily
        for mail in self._new_mails:
            self.assertEqual(
                mail.email_from, self.user_eventmanager.company_id.email_formatted
            )
            self.assertEqual(mail.subject, f"Confirmation for {test_event.name}")
            self.assertEqual(mail.state, "outgoing")
            self.assertEqual(
                mail.email_to, formataddr((new_attendee.name, new_attendee.email))
            )
        ...
        # check that scheduler is finished
        self.assertTrue(
            event_next_scheduler.mail_done,
            "event: reminder scheduler should should have run",
        )
        self.assertEqual(
            event_next_scheduler.mail_state,
            "sent",
            "event: reminder scheduler should have run",
        )
        ...
        self.assertEqual(
            len(self._new_mails),
            16,
            "event: should have scheduled 3 mails, one for each registration",
        )
        self.assertMailMailWEmails(
            [
                formataddr((reg.name, reg.email))
                for reg in attendees[:15] + new_attendee
            ],
            "outgoing",
            content=None,
            fields_values={
    ...
    def test_event_mail_schedule_fail_global_composer(self):
        ...
        before_scheduler = self.test_event.event_mail_ids.filtered(
            lambda s: s.interval_type == "before_event"
        )
        ...
        with patch.object(
            self.env.registry["mail.compose.message"],
            ...
            [
                {
                    ...
                    "notif": [
                        {
                            "partner": user_admin.partner_id,
                            "status": "sent",
                            "type": "inbox",
                        },
                        {
                            "partner": self.user_eventmanager.partner_id,
                            "status": "sent",
                            "type": "inbox",
                        },
                        {
                            "partner": self.user_eventmanager.company_id.partner_id,
                            "status": "ready",
                            "type": "email",
                        },
                    ],
        ...
        self.assertEqual(
            before_scheduler.error_datetime, current_dt.replace(microsecond=0)
        )
    ...
    @mute_logger(
        ...
        "odoo.addons.mail.models.mixin_mail_render",
    )
    @users("user_eventmanager")
    def test_event_mail_schedule_fail_global_composer_message(self):
        ...
        # value in DB
        with patch.object(
            type(self.template_reminder), "_check_rendering", return_value=True
        ):
    ...
    def test_event_mail_schedule_fail_registration_composer(self):
        ...
        # set template write_uid
        user_admin = self.env.ref("base.user_admin")
        onsub_scheduler.template_ref.with_user(user_admin).write(
            {"name": "Take Ownership"}
        )
        ...
        self.assertTrue(
            registration.exists(), "Registration record should exist after creation."
        )
        self.assertEqual(onsub_scheduler.mail_count_done, 0)
        self.assertMailNotifications(
            self._new_msgs,
        ...
        self.assertEqual(
            onsub_scheduler.error_datetime,
            self.reference_now.replace(microsecond=0) + relativedelta(days=3),
        )
    ...
    def test_event_mail_schedule_fail_registration_template_removed(self):
        ...
        after_sub_scheduler = self.test_event.event_mail_ids.filtered(
            lambda s: s.interval_type == "after_sub"
        )
        self.assertTrue(after_sub_scheduler)
        self.template_subscription.sudo().unlink()
        self.assertFalse(
            after_sub_scheduler.exists(),
            "When removing template, scheduler should be removed",
        )
    ...
    def test_event_mail_schedule_on_slot(self):
        ...
        event_after_scheduler = self.env["event.mail"].search(
            [("event_id", "=", test_event.id), ("interval_type", "=", "after_event")]
        )
        (
            test_event.event_mail_ids - (event_prev_scheduler + event_after_scheduler)
        ).unlink()
            ...
            registrations = (
                ...
                .create(
                    [
                        ...
                        for slot in [
                            test_event.event_slot_ids[0],
                            test_event.event_slot_ids[1],
                        ]
                        for idx in range(5)
                    ]
                )
        ...
        registrations_slot_1 = registrations.filtered(
            lambda r: r.event_slot_id == test_event.event_slot_ids[0]
        )
        ...
        self.assertEqual(
            event_prev_scheduler.mail_slot_ids.event_slot_id, test_event.event_slot_ids
        )
        ...
        mail_slot_1 = event_prev_scheduler.mail_slot_ids.filtered(
            lambda s: (
                s.event_slot_id.date
                == self.event_date_end.date() - relativedelta(days=1)
            )
        )
        self.assertEqual(mail_slot_1.scheduled_date, datetime(2021, 3, 22, 7, 0, 0))
        mail_slot_2 = event_prev_scheduler.mail_slot_ids.filtered(
            lambda s: s.event_slot_id.date == self.event_date_end.date()
        )
        ...
        with patch.object(
            EventMail,
            "_execute_event_based_for_registrations",
            autospec=True,
            wraps=EventMail,
            side_effect=exec_origin,
        ) as mock_exec:
        ...
        self.assertEqual(
            mock_exec.call_count, 2, "2 calls: 2x2registrations, limit of 4"
        )
        self.assertMailMailWEmails(
    ...
    def test_event_mail_schedule_on_subscription(self):
        ...
        schedulers = self.env["event.mail"].search([("event_id", "=", test_event.id)])
        _sub_scheduler = schedulers.filtered(
            lambda s: s.interval_type == "after_sub" and s.interval_unit == "now"
        )
        ...
        # nightly: 31 :shrug:
        with (
            self.assertQueryCount(31),
            self.mock_datetime_and_now(reference_now),
            self.mock_mail_gateway(),
        ):
        ...
        self.assertEqual(
            len(self._new_mails), 2, "EventMail: should be limited to new registrations"
        )
        self.assertEqual(
            self.mail_mail_create_mocked.call_count,
            1,
    ...
    def test_event_mail_schedule_on_subscription_async(self):
        ...
        with (
            self.capture_triggers(cron_event.id) as capt_event,
...
class TestMailScheduleInternals(EventMailCommon):
    ...
    def test_unique_event_mail_ids(self):
        ...
        aftersub = test_event.event_mail_ids.filtered(
            lambda mail: mail.interval_type == "after_sub"
        )
        self.assertTrue(aftersub)

        self.assertEqual(
            len(test_event.event_mail_ids), 3, "Should have 3 communication lines"
        )
        self.assertEqual(
            aftersub.mail_count_done, 1, "Should have sent first mail immediately"
        )
        ...
        self.assertTrue(
            aftersub in test_event.event_mail_ids,
            "Sent communication should not have been removed",
        )
        mail_not_done = event_mail_ids_initial - aftersub
        self.assertFalse(
            test_event.event_mail_ids & mail_not_done,
            "Other default communication lines should have been removed",
        )

        self.assertEqual(
            len(test_event.event_mail_ids),
            2,
            "Should now have only two communication lines",
        )
        mails_to_send = test_event.event_mail_ids - aftersub
        duplicate_mails = mails_to_send.filtered(
            lambda mail: (
                mail.notification_type == "mail"
                and mail.interval_nbr == 0
                and mail.interval_unit == "now"
                and mail.interval_type == "after_sub"
                and mail.template_ref.id
                == self.env["ir.model.data"]._xmlid_to_res_id(
                    "event.event_subscription"
                )
            )
    ...
    def test_scheduler_on_archived_cancelled_event(self):
        ...
        self.assertEqual(
            len(self._new_mails),
            2,
            "event: should have 2 scheduled emails (1 / registration)",
        )
        ...
        self.assertFalse(
            event_prev_scheduler.mail_done,
            "event: reminder scheduler should should have run",
        )
        ...
        for mail_id in test_event.event_mail_ids:
            self.assertEqual(
                mail_id.mail_state, "cancelled", "Mail state should be cancelled"
            )
        ...
        self.assertEqual(
            len(self._new_mails), 0, "No mail should be sent for cancelled event"
        )
        self.assertFalse(
            event_prev_scheduler.mail_done,
            "event: reminder scheduler should should have run",
    ...
    def test_scheduler_on_done_event(self):
        ...
        self.assertEqual(
            len(self._new_mails),
            0,
            "Registration emails should not be send once event is over, used mainly for tickets / reminders",
        )
