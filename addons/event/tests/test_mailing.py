    ...
    def test_event_mail_attendees(self):
        template_form = Form(
            self.env["mail.template"].with_context(
                default_model="event.registration", default_name="Test Template"
            )
        )
        ...
        for event_values, exp_mail_values in [
            (
                {
                    "organizer_id": self.event_organizer.id,
            ...
            with self.subTest(event_values=event_values):
                ...
                with (
                    ...
                    self.mock_datetime_and_now(self.event_date_begin),
                ):
    ...
    def test_event_mail_recipients(self):
        ...
        for event_values, exp_followers, exp_defaults, exp_suggested in [
            (
                {
                    "organizer_id": self.event_organizer.id,
                    "user_id": self.user_eventuser.id,
                },
