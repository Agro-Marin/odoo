...
class TestExpensesStates(TestExpenseCommon, MailCase):
    ...
    def setUpClass(cls):
        ...
        cls.paid_or_in_payment_state = cls.env[
            "account.move"
    ...
    def test_expense_state_synchro_1_regular_flow(self):
        ...
        self.assertRecordValues(
            self.expenses_all.account_move_id,
            [
        ...
        self.assertEqual(
            "in_process", self.expenses_company.account_move_id.origin_payment_id.state
        )
    ...
    def test_expense_state_synchro_2_employee_specific_flow_1(self):
        ...
        self.assertRecordValues(
            self.expenses_employee.account_move_id,
            [
    ...
    def test_expense_state_synchro_2_employee_specific_flow_2(self):
        ...
        self.get_new_payment(
            self.expenses_employee, self.expenses_employee.total_amount
        )
    ...
    def test_expense_state_synchro_2_employee_specific_flow_3(self):
        ...
        self.get_new_payment(
            self.expenses_employee, self.expenses_employee.total_amount - 1
        )
    ...
    def test_expense_state_synchro_3_company_specific_flow(self):
        ...
        self.assertRecordValues(
            self.expenses_company.account_move_id,
            [
                {"state": "draft", "payment_state": "not_paid"},
            ],
        )
    ...
    def test_expense_next_activity(self):
        ...
        mail_activity = self.env["mail.activity"].search(
            [
                ...
                ("res_id", "=", self.expenses_employee.id),
            ]
        )
        self.assertEqual(mail_activity.user_id.id, self.expense_user_manager_2.id)
        # No notification should be sent
        notification_message = self.env["mail.message"].search(
            [
                ("partner_ids", "in", self.expense_user_manager_2.partner_id.ids),
                ("display_name", "=", mail_activity.res_name),
            ]
        )
