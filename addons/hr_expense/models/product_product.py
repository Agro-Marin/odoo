...
class ProductProduct(models.Model):
    ...
    standard_price_update_warning = fields.Char(
    ...
    def _compute_standard_price_update_warning(self):
        ...
        for product in self:
            ...
            if undone_expenses:
                rounded_price = self.env.company.currency_id.round(
                    product.standard_price
                )
                if rounded_price and (
                    len(unit_amounts_no_warning) > 1
                    or (
                        len(unit_amounts_no_warning) == 1
        ...
        if "standard_price" in vals:
            ...
            for expense_sudo in expenses_sudo:
                ...
                product_has_cost = (
                    ...
                    and not expense_sudo.company_currency_id.is_zero(
                        expense_product_sudo.standard_price
                    )
                )
                expense_vals = {
                ...
                if product_has_cost:
                    ...
                    expense_vals.update(
                        {"quantity": 1, "price_unit": expense_sudo.total_amount}
                    )
