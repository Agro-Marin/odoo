...
class ResConfigSettings(models.TransientModel):
    ...
    module_document_extract_hr_expense = fields.Boolean(
    ...
    def get_values(self):
        ...
        expense_alias = self.env.ref(
            "hr_expense.mail_alias_expense", raise_if_not_found=False
        )
        res.update(
            hr_expense_alias_prefix=expense_alias.alias_name
            if expense_alias
            else False,
            hr_expense_alias_domain_id=expense_alias.alias_domain_id
            if expense_alias
            else False,
        )
        return res
    ...
    def _compute_hr_expense_alias_prefix(self):
        self.filtered(
            lambda w: not w.hr_expense_use_mailgateway
        ).hr_expense_alias_prefix = False
