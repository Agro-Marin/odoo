...
_ref_vat = {
    ...
    "cr": "3101012009",
    "ch": _lt(
        "CHE-123.456.788 TVA or CHE-123.456.788 MWST or CHE-123.456.788 IVA"
...
class ResPartner(models.Model):
    ...
    def _run_vat_checks(self, country, vat, partner_name="", validation="error"):
        ...
        if (
            vat_prefix == "EU"
            and country not in self.env.ref("base.europe").country_ids
        ):
        ...
        if not self._check_vat_number(code_to_check, vat) or double_prefix:
            ...
            if do_eu_check:
                try:
                    ...
                    msg = self._prepare_vat_error_message(
    ...
    _check_tin1_ro_natural_persons = re.compile(
        r"[1-9]\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\d{6}"
    )
    ...
    _check_vat_ch_re = re.compile(
        r"E([0-9]{9}|-[0-9]{3}\.[0-9]{3}\.[0-9]{3})( )?(MWST|TVA|IVA)$"
    )
    ...
    _check_vat_mx_re = re.compile(
        r"(?P<primeras>[A-Za-z\xd1\xf1&]{3,4})"
        r"[ \-_]?"
    ...
    def check_vat_pe(self, vat):
        if len(vat) != 11 or not vat.isdigit():
            return False
        dig_check = 11 - (
            sum(int("5432765432"[f]) * int(vat[f]) for f in range(10)) % 11
        )
        if dig_check == 10:
            dig_check = 0
        elif dig_check == 11:
    ...
    def check_vat_tr(self, vat):
        return stdnum.util.get_cc_module("tr", "tckimlik").is_valid(
            vat
    ...
    def check_vat_ve(self, vat):
        ...
        checksum += sum(
            map(lambda n, m: int(n) * m, identifier_number, multipliers, strict=True)
        )
    ...
    def check_vat_tw(self, vat):
        ...
        logic_multiplier = [
            ...
            4,
            1,
        ]  # This multiplier is set by the official validation logic.
    ...
    def _convert_hu_local_to_eu_vat(self, local_vat):
        if self._check_tin_hu_companies_re.match(
            local_vat
        ) or self._check_tin_hu_european_re.match(local_vat):
