...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _is_tw_edi_applicable(self, move):
        return (
            ...
            and (
                "debit_origin_id" not in self.env["account.move"]._fields
                or not move.debit_origin_id
            )
    ...
    def _get_all_extra_edis(self):
        ...
        res.update(
            {
                "tw_ecpay_send": {
                    "label": self.env._("Send to Ecpay"),
                    "is_applicable": self._is_tw_edi_applicable,
                },
                "tw_ecpay_issue_allowance": {
                    "label": self.env._("Send to Ecpay (Issue Allowance)"),
    ...
    def _l10n_tw_edi_generate_ecpay_json(self, invoice, invoice_data):
        ...
        if need_file:
            if "tw_ecpay_send" in invoice_data["extra_edis"]:
                ...
                file_name = (
                    f"{invoice.name.replace('/', '_')}_ecpay_issue_allowance.json"
                )
    ...
    def _call_web_service_before_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            if any(
                key in invoice_data["extra_edis"]
                for key in ("tw_ecpay_send", "tw_ecpay_issue_allowance")
            ):
                ...
                # If the invoice was downloaded but not sent, the json file could already be there.
                    ...
                    json_content = json.loads(
                        base64.b64decode(invoice.l10n_tw_edi_file)
                    )
            ...
            if "tw_ecpay_send" in invoice_data["extra_edis"]:
                if errors := invoice._l10n_tw_edi_send(json_content):
                    invoice_data["error"] = {
                        "error_title": self.env._(
                            "Error when sending the invoices to ECPay."
                        ),
                        "errors": errors,
                    }
            elif "tw_ecpay_issue_allowance" in invoice_data["extra_edis"]:
                if errors := invoice._l10n_tw_edi_issue_allowance(json_content):
                    invoice_data["error"] = {
                        "error_title": self.env._(
                            "Error when sending the allowances to ECPay."
                        ),
                        "errors": errors,
                    }
            if (
                "tw_ecpay_send" in invoice_data["extra_edis"]
                or "tw_ecpay_issue_allowance" in invoice_data["extra_edis"]
            ):
                if self._can_commit():
                    self.env.cr.commit()
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            if (
                "tw_ecpay_send" not in invoice_data["extra_edis"]
                or invoice.l10n_tw_edi_state != "invoiced"
            ):
                continue
            if errors := invoice._l10n_tw_edi_update_ecpay_invoice_info():
                invoice_data["error"] = {
                    "error_title": self.env._(
                        "Error when getting the invoice data from ECPay."
                    ),
                    "errors": errors,
                }
    ...
    def _link_invoice_documents(self, invoices_data):
        ...
        if attachments_vals:
            ...
            self.env["account.move"].browse(res_ids).invalidate_recordset(
                fnames=["l10n_tw_edi_file_id", "l10n_tw_edi_file"]
            )
