...
class AccountMove(models.Model):
    ...
    l10n_tw_edi_zero_tax_rate_reason = fields.Selection(
        ...
        selection=[
            ...
            (
                "72",
                "72: No.2 Services related to export sales, or services provided domestically but used abroad",
            ),
            (
                "73",
                "73: No.3 Duty-free shops established by law for the sale and transit or departure of passengers",
            ),
            (
                "74",
                "74: No.4 Sale of goods or services for operation by the operator of the FREE Trade Zone",
            ),
            (
                "75",
            ...
            (
                "76",
                "76: No.6 Ships, aircraft and distant-water fishing vessels for international transportation",
            ),
            (
                "77",
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        if self.move_type == "out_invoice":
            self.filtered(
                lambda m: m.l10n_tw_edi_state and m.l10n_tw_edi_state != "invalid"
    ...
    def _compute_is_print(self):
        for move in self:
            if move.l10n_tw_edi_love_code or (
                move.partner_id.vat and move.l10n_tw_edi_carrier_type in [1, 2]
            ):
    ...
    def _compute_love_code(self):
        for move in self:
            if (
                move.l10n_tw_edi_is_print
                or move.l10n_tw_edi_carrier_type
                or move.partner_id.vat
            ):
    ...
    def _l10n_tw_edi_check_tax_type_on_invoice_lines(self):
        ...
        if product_lines.filtered(
            lambda line: not line.tax_ids or len(line.tax_ids) > 1
        ):
            errors.append(
                self.env._(
                    "Invoice lines without taxes or more than one tax are not allowed."
                )
            )
        ...
        if len(set(product_lines.tax_ids.mapped("price_include"))) > 1:
            errors.append(
                self.env._(
                    "Invoice lines with different tax include/exclude are not allowed."
                )
            )
        ...
        invoice_lines_tax_types = set(
            product_lines.tax_ids.mapped("l10n_tw_edi_tax_type")
        )
        ...
        if invoice_lines_tax_types:
            ...
            if "4" in invoice_lines_tax_types and len(invoice_lines_tax_types) > 1:
                errors.append(
                    self.env._("Special tax type cannot be mixed with other tax types.")
                )

            special_tax_types_4 = set(
                product_lines.tax_ids.filtered(
            ...
            if (
                "3" in invoice_lines_tax_types
                and next(iter(special_tax_types_3)) == "8"
                and len(invoice_lines_tax_types) > 1
            ):
                errors.append(
                    self.env._(
            ...
            if "1" in invoice_lines_tax_types and any(
                rate != 5 for rate in tax_type_1_rates
            ):
                errors.append(self.env._("Amount for Taxable tax type must be 5%."))
        else:
            errors.append(
                self.env._(
    ...
    def _l10n_tw_edi_determine_tax_types(self):
        ...
        if invoice_lines_tax_types:
            if len(invoice_lines_tax_types) > 1:  # mixed tax types
                ...
                tax_type = "4"
                special_tax_type = int(
                    product_lines.tax_ids.mapped("l10n_tw_edi_special_tax_type")[0]
                )
            elif "3" in invoice_lines_tax_types:
                tax_type = "3"
                special_tax_type = (
                    8
                    if product_lines.tax_ids[0].l10n_tw_edi_special_tax_type == "8"
                    else False
                )
            else:
    ...
    def _l10n_tw_edi_check_before_generate_invoice_json(self):
        ...
        if (
            self.l10n_tw_edi_is_print or self.partner_id.vat
        ) and not self.partner_id.contact_address:
            errors.append(
                self.env._(
                    "Please fill in the customer address for printing Ecpay invoice."
                )
            )

        if not self.partner_id.email and not self.partner_id.phone_ids:
            errors.append(
                self.env._(
        ...
        if partner_phone := self.partner_id.phone_ids._primary().number:
            formatted_phone = self._reformat_phone_number(partner_phone)
            if not re.fullmatch(r"[\d]+", formatted_phone):
                errors.append(
                    self.env._(
                        "Phone number contains invalid characters! It should be in the format: '+886 0997624293'."
                    )
                )
        ...
        if self.l10n_tw_edi_is_b2b and not self.partner_id.vat:
            errors.append(
                self.env._(
                    "A tax ID is required for company contact or individual contact under a company."
                )
            )
        ...
        if (
            ...
            and (not self.partner_id.vat.isdigit() or len(self.partner_id.vat) != 8)
        ):
            errors.append(
                self.env._(
                    "The tax ID is invalid. It should be in the format: '12345678'."
                )
            )
        ...
        if is_zero_tax_rate:
            if (
                not self.l10n_tw_edi_clearance_mark
                or not self.l10n_tw_edi_zero_tax_rate_reason
            ):
                errors.append(
                    self.env._(
    ...
    def _l10n_tw_edi_prepare_item_list(self, json_data, is_allowance=False):
        ...
        for index, line in enumerate(
            ...
            if is_allowance and self.reversed_entry_id.currency_id == self.currency_id:
                ...
                )  # replace the rate by the original invoice's rate
            ...
            if self.l10n_tw_edi_is_b2b:
                item_price = float_round(
                    twd_excluded_amount / quantity, precision_rounding=0.01
                )
                ...
                if (
                    not is_allowance
                    and line.tax_ids
                    and not line.tax_ids[0].price_include
                ):
                    item_price = float_round(
                        twd_excluded_amount / quantity, precision_rounding=0.01
                    )
                    item_amount = float_round(
                        twd_excluded_amount, precision_rounding=0.01
                    )
                else:
                    item_price = float_round(
                        twd_included_amount / quantity, precision_rounding=0.01
                    )
                    item_amount = float_round(
                        twd_included_amount, precision_rounding=0.01
                    )

                item_amount_taxed = float_round(
                    twd_included_amount, precision_rounding=0.01
                )
                ...
                item_list.append(
                    {
                        ...
                        "OriginalInvoiceDate": self.l10n_tw_edi_invoice_create_date.strftime(
                            "%Y-%m-%d"
                        ),
                ...
                item_list.append(
                    {
                        ...
                        "ItemPrice": item_price,
                        "ItemTaxType": line.tax_ids[0].l10n_tw_edi_tax_type
                        if tax_type != "4" and line.tax_ids
                        else "",
                        "ItemAmount": item_amount,
                    }
                )
            if self.l10n_tw_edi_is_b2b:
                ...
                tax_amount += base_line["tax_details"]["taxes_data"][0][
                    "raw_tax_amount"
                ]
            else:
        ...
        amount_on_invoice = (
            ...
            if self.l10n_tw_edi_is_b2b and tax_type != "4"
            else self.amount_total_signed
        )
        difference = self.company_id.currency_id.round(sale_amount) - abs(
            amount_on_invoice
        )
        if difference != 0:
            ...
            item_list[-1]["ItemPrice"] = float_round(
                item_list[-1]["ItemAmount"] / item_list[-1]["ItemCount"],
        ...
        if is_allowance:
            ...
            exchange_difference = sum(
                item["amount"]
                for item in reconciled_partials
                if item.get("is_exchange")
            )
            if exchange_difference and self.l10n_tw_edi_is_b2b and tax_type != "4":
                tax_rate = abs(self.amount_untaxed_signed) / abs(
                    self.amount_total_signed
                )
                exchange_difference = self.company_id.currency_id.round(
                    exchange_difference * tax_rate
                )  # Use untaxed amount for B2B and non special type credit notes
            if (
                self.reversed_entry_id.invoice_currency_rate
        ...
        elif self.l10n_tw_edi_is_b2b:
            json_data["TotalAmount"] = self.company_id.currency_id.round(sale_amount)
        else:
            json_data["AllowanceAmount"] = self.company_id.currency_id.round(
                sale_amount
            )

        if self.l10n_tw_edi_is_b2b:
            json_data["TaxAmount"] = (
                self.company_id.currency_id.round(tax_amount) if tax_type != "4" else 0
            )
    ...
    def _l10n_tw_edi_generate_invoice_json(self):
        ...
        partner_phone = self.partner_id.phone_ids._primary().number
        formatted_phone = (
            self._reformat_phone_number(partner_phone) if partner_phone else ""
        )
        product_lines = self.invoice_line_ids.filtered(
            lambda line: line.display_type == "product"
        )
        vat = (
            "1"
            if product_lines[0].tax_ids and product_lines[0].tax_ids[0].price_include
            else "0"
        )
            ...
            json_data.update(
                {
                    ...
                    "Print": "1"
                    if self.l10n_tw_edi_is_print or self.l10n_tw_edi_is_b2b
                    else "0",
                    ...
                    "CarrierNum": self.l10n_tw_edi_carrier_number
                    if self.l10n_tw_edi_carrier_type in ["2", "3", "4", "5"]
                    else "",
                    "CarrierNum2": self.l10n_tw_edi_carrier_number_2
                    if self.l10n_tw_edi_carrier_type in ["4", "5"]
                    else "",
                    "vat": vat,
                }
            )
    ...
    def _l10n_tw_edi_check_before_generate_issue_allowance_json(self):
        ...
        if (
            self.l10n_tw_edi_is_b2b
            or self.l10n_tw_edi_refund_agreement_type == "online"
        ) and not self.partner_id.email:
        ...
        if not self.l10n_tw_edi_is_b2b and (
            ...
            or (
                self.l10n_tw_edi_allowance_notify_way == "phone"
                and not self.partner_id.phone_ids
            )
        ):
            raise UserError(
                self.env._(
                    "Customer %(notify_way)s is needed for notification",
    ...
    def _l10n_tw_edi_generate_issue_allowance_json(self):
        ...
        if not self.l10n_tw_edi_is_b2b:
            ...
            elif self.l10n_tw_edi_allowance_notify_way == "phone" and (
                notify_phone := self.partner_id.phone_ids._primary().number
            ):
                json_data["AllowanceNotify"] = "S"
                json_data["NotifyPhone"] = notify_phone.replace("+", "").replace(
                    " ", ""
                )
    ...
    def _l10n_tw_edi_send_create_buyer(self):
        ...
        if number := self.partner_id.commercial_partner_id.phone_ids._primary().number:
            buyer_json_data["TelephoneNumber"] = self._reformat_phone_number(number)

        return call_ecpay_api(
            "/MaintainMerchantCustomerData",
            buyer_json_data,
            self.company_id,
            self.l10n_tw_edi_is_b2b,
        )
    ...
    def _l10n_tw_edi_send(self, json_content):
        ...
        response_data = call_ecpay_api(
            "/Issue", json_content, self.company_id, self.l10n_tw_edi_is_b2b
        )
        if int(response_data.get("RtnCode")) != 1:
            return response_data.get("RtnMsg").split("\r\n")
        invoice_number = (
            response_data.get("InvoiceNumber")
            if self.l10n_tw_edi_is_b2b
            else response_data.get("InvoiceNo")
        )
        self.write(
            {
                "l10n_tw_edi_ecpay_invoice_id": invoice_number,
                # The date return from Ecpay API used "+" instead of " "
                "l10n_tw_edi_invoice_create_date": fields.Datetime.now()
                if self.l10n_tw_edi_is_b2b
                else transfer_time(response_data.get("InvoiceDate").replace("+", " ")),
    ...
    def _l10n_tw_edi_update_ecpay_invoice_info(self):
        ...
        if not self.l10n_tw_edi_related_number:
            raise UserError(
                self.env._(
                    "The invoice: %(invoice_name)s has no related number",
        ...
        response_data = call_ecpay_api(
            "/GetIssue", json_data, self.company_id, self.l10n_tw_edi_is_b2b
        )
        if int(response_data.get("RtnCode")) != 1:
            return response_data.get("RtnMsg").split("\r\n")

        invalid_status = (
            int(response_data.get("RtnData").get("Invalid_Status"))
            if self.l10n_tw_edi_is_b2b
    ...
    def _l10n_tw_edi_run_invoice_invalid(self):
        ...
        if not self.l10n_tw_edi_ecpay_invoice_id:
            raise UserError(
                self.env._(
                    "You cannot invalidate an invoice that was not sent to Ecpay."
                )
            )
        if self.l10n_tw_edi_state == "invalid":
            raise UserError(
                self.env._(
                    "The invoice: %(invoice_id)s has already been invalidated",
        ...
        response_data = call_ecpay_api(
            "/Invalid", json_data, self.company_id, self.l10n_tw_edi_is_b2b
        )

        if int(response_data.get("RtnCode")) != 1:
            raise UserError(
                self.env._(
                    "Fail to invalidate invoice. Error message: %(error_message)s",
    ...
    def _l10n_tw_edi_issue_allowance(self, json_content):
        ...
        query_param = (
            ...
            if not self.l10n_tw_edi_is_b2b
            and self.l10n_tw_edi_refund_agreement_type == "online"
            else "/Allowance"
        )

        response_data = call_ecpay_api(
            query_param, json_content, self.company_id, self.l10n_tw_edi_is_b2b
        )
        ...
        if self.l10n_tw_edi_is_b2b:
            ...
            self.write(
                {
                    "l10n_tw_edi_refund_invoice_number": response_data.get(
                        "IA_Allow_No"
                    ),
        ...
        self._message_log(
            body=self.env._(
                "Ecpay invoice number %(invoice_number)s has been issued allowance successfully.",
                invoice_number=self.l10n_tw_edi_ecpay_invoice_id,
            ),
        )
    ...
    def _l10n_tw_edi_print_invoice(self):
        self.check_singleton()
        if (
            self.l10n_tw_edi_state not in ["invoiced", "valid"]
            or not self.l10n_tw_edi_ecpay_invoice_id
            or not (self.l10n_tw_edi_is_print or self.l10n_tw_edi_is_b2b)
        ):
            raise UserError(
                self.env._(
