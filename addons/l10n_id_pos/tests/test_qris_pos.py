...
class TestPosQris(AccountTestInvoicingHttpCommon):
    ...
    def test_qris_transaction_allow_pos_order(self):
        ...
        self.env["l10n_id.qris.transaction"].create(
            {"model": "pos.order", "model_id": "1234512345"}
        )
        self.env["l10n_id.qris.transaction"].create(
            {"model": "account.move", "model_id": "1"}
        )
    ...
    def test_tour_qris_payment_fail(self):
        ...
        with patch(
            ...
            side_effect=_patched_make_qris_request,
        ):
            self.start_tour(
                "/pos/ui/%d" % self.main_pos_config.id,
                "PaymentScreenQRISPaymentFail",
                login="pos_user",
            )
    ...
    def test_tour_qris_payment_success(self):
        ...
        with patch(
            ...
            self.start_tour(
                "/pos/ui/%d" % self.main_pos_config.id,
                "PaymentScreenQRISPaymentSuccess",
                login="pos_user",
            )
    ...
    def test_qris_change_amount(self):
        ...
        with patch(
            ...
            self.start_tour(
                "/pos/ui/%d" % self.main_pos_config.id,
                "PayementScreenQRISChangeAmount",
                login="pos_user",
            )
