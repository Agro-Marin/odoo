...
class TestSaleExpense(TestExpenseCommon, TestSaleCommon):
    def test_sale_expense(self):
        ...
        analytic_account = self.env["account.analytic.account"].create(
            so._prepare_analytic_account_data()
        )
        ...
        sol = so.line_ids.filtered(
            lambda sol: (
                sol.product_id.id == self.company_data["product_delivery_cost"].id
            )
        )
        self.assertEqual(
            (sol.price_unit, sol.qty_transferred),
            (55.0, 11.3),
            "Sale Expense: error when invoicing an expense at cost",
        )
        self.assertEqual(
            so.amount_total,
            init_price + expense.total_amount,
        ...
        self.assertEqual(
            (sol.price_unit, sol.qty_transferred),
            (prod_exp_2.list_price, 100.0),
            "Sale Expense: error when invoicing an expense at cost",
        )
        self.assertEqual(
            so.amount_untaxed,
            init_price + (prod_exp_2.list_price * 100.0),
            "Sale Expense: price of so should be updated after adding expense",
        )
        ...
        self.assertEqual(
            inv.amount_untaxed,
            621.5 + (prod_exp_2.list_price * 100.0),
    ...
    def test_expense_multi_id_analytic_distribution(self):
        ...
        self.assertTrue(
            self.env["account.move"].search([("expense_ids", "=", expense.id)], limit=1)
        )
