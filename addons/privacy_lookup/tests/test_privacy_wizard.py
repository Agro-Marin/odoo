...
class TestPrivacyWizard(TransactionCase):
    ...
    def test_wizard(self):
        ...
        self.assertEqual(
            wizard.log_id.records_description, "Contact (1): #%s" % (self.partner.id)
        )
    ...
    def test_wizard_multi_company(self):
        ...
        partner_line = wizard.line_ids.filtered(
            lambda l: l.resource_ref == self.partner
        )
    ...
    def test_wizard_indirect_reference(self):
        ...
        self.assertTrue(
            wizard.line_ids.filtered(
                lambda l: l.res_model == "res.partner" and l.res_id == self.partner.id
            )
        )
        self.assertTrue(
            wizard.line_ids.filtered(
                lambda l: (
                    l.res_model == "res.company" and l.res_id == self.env.company.id
                )
            )
        )
    ...
    def test_wizard_lookup_with_invalid_email(self):
        ...
        with self.assertRaises(
            UserError, msg='Invalid email address "%s"' % wizard.email
        ):
            wizard.action_lookup()
