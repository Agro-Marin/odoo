...
class TestPacking(TestPackingCommon):
    ...
    def test_put_in_pack_weight_wizard(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_bw, self.stock_location, 20.0
        )
    ...
    def test_pack_in_pack_weight_wizard(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_bw, self.stock_location, 5.0
        )
    ...
    def test_send_to_shipper_without_sale_order(self):
        ...
        self.assertEqual(
            picking_ship.state, "assigned", "Delivery state should be assigned."
        )
    ...
    def test_batch_picking_delivery(self):
        ...
        self.assertEqual(
            ...
            2,
            'There should be move lines that can be "put in pack"',
        )
        ...
        self.assertEqual(
            delivery_1.move_line_ids.result_package_id,
            package,
            "Delivery 1 moves should have been put in package.",
        )
        self.assertEqual(
            delivery_2.move_line_ids.result_package_id,
            package,
    ...
    def test_picking_access_error_on_package(self):
        ...
        wh_a = self.env["stock.warehouse"].search(
        ...
        company_a_user.group_ids = [
            Command.unlink(self.env.ref("stock.group_stock_multi_warehouses").id)
        ]
    ...
    def test_put_in_pack_applies_only_to_selected_move_line(self):
        ...
        pack_wiz.action_put_in_pack()
        self.assertTrue(
            move_line_1.result_package_id,
            "A package should have been created for the selected move line",
        )
        self.assertFalse(
            move_line_2.result_package_id, "The other move line should not be packed"
        )
    ...
    def test_multi_level_package_weight(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productB, self.stock_location, 3, package_id=boxB
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productB, self.stock_location, 1, package_id=pallet
        )
        ...
        delivery.action_confirm()
        res_boxA = delivery.move_ids[0].move_line_ids.action_put_in_pack(
            package_type_id=sbox_type.id
        )
        res_boxB = delivery.move_ids[1].move_line_ids.action_put_in_pack(
            package_type_id=sbox_type.id
        )
        res_big_box = delivery.move_ids.move_line_ids.action_put_in_pack(
            package_type_id=bbox_type.id
        )
        res_pallet = delivery.move_ids.move_line_ids.action_put_in_pack(
