...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _l10n_ec_setup_location_accounts(self, companies):
        loss_locs = dict(
            self.env["stock.location"]._read_group(
                domain=[("usage", "=", "inventory")], groupby=["company_id", "id"]
            )
        )
        prod_locs = dict(
            self.env["stock.location"]._read_group(
                domain=[("usage", "=", "production")], groupby=["company_id", "id"]
            )
        )
        for company in companies:
            ...
            if (loss_loc := loss_locs.get(company)) and (
                loss_loc_account := ref and Template.ref(ref, raise_if_not_found=False)
            ):
                loss_loc.write(
                    {
            ...
            if (prod_loc := prod_locs.get(company)) and (
                prod_loc_account := ref and Template.ref(ref, raise_if_not_found=False)
            ):
                prod_loc.write(
                    {
