...
class TestSaEdiCommon(AccountEdiTestCommon):
        ...
        return cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create({"number": "+966556666666", "type": "landline"})
                ],
                # Tax info
                "vat": "311111111111113",
    ...
    def _setup_products(cls):
        ...
        cls.product_a = cls._create_product(
            name="Product A", standard_price=320.0, default_code="P0001"
        )
        cls.product_b = cls._create_product(
            name="Product B", standard_price=15.8, default_code="P0002"
        )
    ...
    def _setup_taxes(cls):
        ...
        cls.tax_15 = cls.env["account.tax"].search(
            [("company_ids", "in", [cls.company.id]), ("amount", "=", 15.0)], limit=1
        )
    ...
    def _setup_journal(cls):
        ...
        PCSID_Data = json.loads(
            cls.customer_invoice_journal.l10n_sa_production_csid_json
        )
        ...
        cls.customer_invoice_journal.l10n_sa_production_csid_certificate_id = (
            pcsid_certificate
        )
    ...
    def _setup_xpath_templates(cls):
        ...
        cls.remove_ubl_extensions_xpath = (
            """<xpath expr="//*[local-name()='UBLExtensions']" position="replace"/>"""
        )
    ...
    def _get_or_create_state(cls, name, code, country):
        ...
        if not state:
            state = cls.env["res.country.state"].create(
                {"name": name, "code": code, "country_id": country.id}
            )
