...
class TestEdiZatca(TestSaEdiCommon):
        ...
        with freeze_time(freeze_time_at):
            ...
            if move:
                ...
                raise ValidationError(
                    "Either move or document_type + move_data need to be given"
                )
            ...
            generated_file = self.env[
                "account.edi.format"
            ]._l10n_sa_generate_zatca_template(final_move)
            current_tree = self.get_xml_tree_from_string(generated_file)
            current_tree = self.with_applied_xpath(
                current_tree, self.remove_ubl_extensions_xpath
            )
    ...
    def testCreditNoteSimplified(self):
        ...
        self._test_document_generation(
            ...
            move_data=move_data,
            freeze_time_at=datetime(
                2023, 3, 10, 14, 59, 38, tzinfo=timezone("Etc/GMT-3")
            ),
        )
    ...
    def testCreditNoteStandard(self):
        ...
        self._test_document_generation(
            ...
            move_data=move_data,
            freeze_time_at=datetime(
                2022, 9, 5, 9, 39, 15, tzinfo=timezone("Etc/GMT-3")
            ),
    ...
    def testDebitNoteSimplified(self):
        ...
        self._test_document_generation(
            ...
            move_data=move_data,
            freeze_time_at=datetime(
                2023, 3, 10, 15, 1, 46, tzinfo=timezone("Etc/GMT-3")
            ),
        )
    ...
    def testDebitNoteStandard(self):
        ...
        self._test_document_generation(
            ...
            move_data=move_data,
            freeze_time_at=datetime(
                2022, 9, 5, 9, 45, 27, tzinfo=timezone("Etc/GMT-3")
            ),
            additional_xpath=additional_xpath,
        )
    ...
    def testInvoiceSimplified(self):
        ...
        self._test_document_generation(
            ...
            move_data=move_data,
            freeze_time_at=datetime(
                2023, 3, 10, 14, 56, 55, tzinfo=timezone("Etc/GMT-3")
            ),
        )
    ...
    def testInvoiceWithDownpayment(self):
        ...
        saudi_pricelist = self.env["product.pricelist"].create(
            {"name": "SAR", "currency_id": self.env.ref("base.SAR").id}
        )
    ...
    def testCompanyOnSimplifiedInvoiceQR(self):
        ...
        invoice.action_post()
        xml_content = self.env["account.edi.format"]._l10n_sa_generate_zatca_template(
            invoice
        )
    ...
    def test_zatca_xml_line_rounding_amount_consistency(self):
        ...
        for line in xml_root.xpath("//cac:InvoiceLine", namespaces=namespaces):
            line_ext = float(
                line.xpath("cbc:LineExtensionAmount/text()", namespaces=namespaces)[0]
            )
            tax_amt = float(
                line.xpath("cac:TaxTotal/cbc:TaxAmount/text()", namespaces=namespaces)[
                    0
                ]
            )
            rounding_amt = float(
                line.xpath(
                    "cac:TaxTotal/cbc:RoundingAmount/text()", namespaces=namespaces
                )[0]
            )
            self.assertEqual(
                line_ext + tax_amt,
