...
class BaseDocumentLayout(models.TransientModel):
    ...
    def _default_company_details(self):
        ...
        if self.env.company.vat:
            additional_company_details += (
                Markup("VAT Number: %s <br/>") % self.env.company.vat
            )
        if (
            code := self.env.company.l10n_sa_edi_additional_identification_scheme
        ) and self.env.company.l10n_sa_edi_additional_identification_number:
            code_value = dict(
                self._fields[
        ...
        return (
            super()._default_company_details()
            + Markup("<br/><br/>%s") % additional_company_details
        )
