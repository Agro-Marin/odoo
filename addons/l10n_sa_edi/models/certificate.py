    ...
    def _l10n_sa_get_csr_str(self, journal):
        ...
        subject_names = (
            ...
            (
                ...
                company_id.name if parent_company_id else company_id.vat[:10],
            ),
            # Organization Name
            (
                NameOID.ORGANIZATION_NAME,
                parent_company_id.name if parent_company_id else company_id.name,
            ),
            # Subject Common Name
            (
                NameOID.COMMON_NAME,
                "%s-%s-%s" % (journal.code, journal.name, company_id.name),
            ),
            # Organization Identifier
            (
                ObjectIdentifier("2.5.4.97"),
                parent_company_id.vat if parent_company_id else company_id.vat,
            ),
            # State/Province Name
        ...
        builder = builder.subject_name(
            x509.Name([x509.NameAttribute(n[0], "%s" % n[1]) for n in subject_names])
        )
                ...
                x509.DirectoryName(
                    x509.Name(
                        [
                            ...
                            x509.NameAttribute(
                                NameOID.TITLE,
                                company_id._l10n_sa_get_csr_invoice_type(),
                            ),
                            # Location
                            x509.NameAttribute(
                                ObjectIdentifier("2.5.4.26"), company_id.street
                            ),
                            # Industry
                            x509.NameAttribute(
