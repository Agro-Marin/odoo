...
class AccountMove(models.Model):
            ...
            if (
                ...
                if move._l10n_sa_is_simplified():
                    ...
                    xml_content = self.env.ref(
                        "l10n_sa_edi.edi_sa_zatca"
                    )._l10n_sa_generate_zatca_template(move)
                    ...
                    zatca_document.state == "sent"
                    and zatca_document.attachment_id.datas
                ):
                    document_xml = zatca_document.attachment_id.with_context(
                        bin_size=False
                    ).datas.decode()
    ...
    def _l10n_sa_get_qr_code_encoding(self, tag, field, int_length=1):
        ...
        company_name_length_encoding = len(field).to_bytes(
            length=int_length, byteorder="big"
        )
    ...
    @api.model
    def _l10n_sa_get_qr_code(
        self, company_id, unsigned_xml, certificate, signature, is_b2c=False
    ):
        """
        ...
        def xpath_ns(expr):
            return root.xpath(expr, namespaces=edi_format._l10n_sa_get_namespaces())[
                0
            ].text.strip()
        ...
        invoice_datetime = datetime.strptime(
            invoice_date + " " + invoice_time, "%Y-%m-%d %H:%M:%S"
        )

        if invoice_datetime and company_id.vat and certificate and signature:
            prehash_content = etree.tostring(root)
            invoice_hash = edi_format._l10n_sa_generate_invoice_xml_hash(
                prehash_content, "digest"
            )

            amount_total = float(xpath_ns("//cbc:PayableAmount"))
            amount_tax = float(xpath_ns("//cac:TaxTotal/cbc:TaxAmount"))
            seller_name_enc = self._l10n_sa_get_qr_code_encoding(
                1, company_id.display_name.encode()
            )
            seller_vat_enc = self._l10n_sa_get_qr_code_encoding(
                2, company_id.vat.encode()
            )
            timestamp_enc = self._l10n_sa_get_qr_code_encoding(
                3, invoice_datetime.strftime("%Y-%m-%dT%H:%M:%S").encode()
            )
            amount_total_enc = self._l10n_sa_get_qr_code_encoding(
                4, float_repr(abs(amount_total), 2).encode()
            )
            amount_tax_enc = self._l10n_sa_get_qr_code_encoding(
                5, float_repr(abs(amount_tax), 2).encode()
            )
            ...
            public_key_enc = self._l10n_sa_get_qr_code_encoding(
                8,
                base64.b64decode(
            ...
            if is_b2c:
                qr_code_str += self._l10n_sa_get_qr_code_encoding(
                    9,
                    base64.b64decode(
    ...
    def _l10n_sa_reset_confirmation_datetime(self):
        ...
        for move in self.filtered(lambda m: m.country_code == "SA"):
            zatca_doc = move.edi_document_ids.filtered(
                lambda d: d.edi_format_id.code == "sa_zatca"
            )
            if (
                not zatca_doc or zatca_doc[0].blocking_level != "error"
            ):  # Error is the rejection case
    ...
    def _l10n_sa_generate_unsigned_data(self):
        ...
        invoice_hash_hex = (
            ...
            ._l10n_sa_generate_invoice_xml_hash(xml_content)
            .decode()
        )
    ...
    def _l10n_sa_log_results(self, xml_content, response_data=None, error=False):
        ...
        attachment = False
        if error:
            xml_filename = self.env[
                "account.edi.xml.ubl_21.zatca"
            ]._export_invoice_filename(self)
        ...
        if response_data and response_data.get("validationResults", {}).get(
            "warningMessages"
        ):
            bootstrap_cls, title = (
                "warning",
    ...
    def _l10n_sa_is_legal(self):
        ...
        zatca_document = self.edi_document_ids.filtered(
            lambda d: d.edi_format_id.code == "sa_zatca"
        )
        return result or (
    ...
    def _get_report_base_filename(self):
        ...
        if self._is_l10n_sa_eligibile_invoice():
            return (
                self.with_context(l10n_sa_file_format=False)
                .env["account.edi.xml.ubl_21.zatca"]
                ._export_invoice_filename(self)
            )
    ...
    def _get_invoice_report_filename(self, extension="pdf", report=None):
        if self._is_l10n_sa_eligibile_invoice():
            return (
                self.with_context(l10n_sa_file_format=extension)
                .env["account.edi.xml.ubl_21.zatca"]
                ._export_invoice_filename(self)
            )
    ...
    def _l10n_sa_is_in_chain(self):
        ...
        """
        zatca_doc_ids = self.edi_document_ids.filtered(
            lambda d: d.edi_format_id.code == "sa_zatca"
        )
        return len(zatca_doc_ids) > 0 and not any(
            zatca_doc_ids.filtered(lambda d: d.state == "to_send")
        )
    ...
    def _prepare_tax_lines_for_taxes_computation(self, tax_amls, round_from_tax_lines):
        ...
        if (
            ...
            and self.line_ids._get_downpayment_lines()
        ):
            return []
        return super()._prepare_tax_lines_for_taxes_computation(
            tax_amls, round_from_tax_lines
        )

    def _get_l10n_sa_totals(self):
        self.check_singleton()
        invoice_node = self.env["account.edi.xml.ubl_21.zatca"]._get_invoice_node(
            {"invoice": self}
        )
        return {
            "total_amount": invoice_node["cac:LegalMonetaryTotal"][
    ...
    def _retry_edi_documents_error(self):
        ...
        """
        self.filtered(lambda m: m.country_code == "SA").write(
    ...
    def _compute_tax_amount(self):
        ...
        for line in self:
            if (
                ...
                base_line = (
                    line.move_id._prepare_product_base_line_for_taxes_computation(line)
                )
