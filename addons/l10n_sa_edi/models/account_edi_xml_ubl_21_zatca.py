...
PAYMENT_MEANS_CODE = {"bank": 42, "card": 48, "cash": 10, "transfer": 30, "unknown": 1}
...
class AccountEdiXmlUbl_21Zatca(models.AbstractModel):
    ...
    def _export_invoice_filename(self, invoice):
        ...
        invoice_date = fields.Datetime.context_timestamp(
            self.with_context(tz="Asia/Riyadh"), invoice.l10n_sa_confirmation_datetime
        )
    ...
    def _add_invoice_config_vals(self, vals):
        ...
        vals["document_type"] = (
            "invoice"  # Only use Invoice in ZATCA, even for credit and debit notes
        )
    ...
    def _add_invoice_base_lines_vals(self, vals):
        ...
        if not invoice._is_downpayment():
            prepayment_moves = (
                invoice.line_ids._get_downpayment_lines().move_id.filtered(
        ...
        for prepayment_move in prepayment_moves:
            prepayment_move_base_lines, _dummy = (
                prepayment_move._get_rounded_base_and_tax_lines()
            )
        ...
        def tax_grouping_function(base_line, tax_data):
            ...
            return {
                "tax_category_code": self._get_tax_category_code(
                    vals["customer"].commercial_partner_id, vals["supplier"], tax
                ),
        ...
        if supplier.country_id.code == "SA":
            if tax and tax.amount == 0:
                exemption_reason_by_code = dict(
                    tax._fields["l10n_sa_exemption_reason_code"]._description_selection(
                        self.env
                    )
                )
    ...
    def _is_document_allowance_charge(self, base_line):
        return (
            base_line["special_type"] == "early_payment"
            or base_line["tax_details"]["total_excluded_currency"] < 0
        )
        ...
        document_node.update(
            {
                ...
                "cbc:InvoiceTypeCode": {
                    ...
                    % (
                        ...
                        "1"
                        if invoice.commercial_partner_id.country_id
                        != invoice.company_id.country_id
                        and not invoice._l10n_sa_is_simplified()
                        else "0",
                    ),
                },
                ...
                "cac:Signature": {
                    "cbc:ID": {
                        "_text": "urn:oasis:names:specification:ubl:signature:Invoice"
                    },
                    "cbc:SignatureMethod": {
                        "_text": "urn:oasis:names:specification:ubl:dsig:enveloped:xades"
                    },
                }
                if invoice._l10n_sa_is_simplified()
                else None,
    ...
    def _add_invoice_delivery_nodes(self, document_node, vals):
        ...
        if "cac:Delivery" in document_node:
            ...
            if not document_node["cac:Delivery"]["cbc:ActualDeliveryDate"]["_text"]:
                document_node["cac:Delivery"]["cbc:ActualDeliveryDate"] = {
                    "_text": invoice.invoice_date
                }
    ...
    def _add_invoice_payment_means_nodes(self, document_node, vals):
        ...
        payment_means_node["cbc:InstructionNote"] = {
            "_text": invoice._l10n_sa_get_adjustment_reason()
        }
    ...
    def _get_address_node(self, vals):
        ...
        edi_plot_identification = (
            partner.l10n_sa_edi_plot_identification
            if partner._name == "res.partner"
            else ""
        )
    ...
    def _get_party_node(self, vals):
        ...
        if identification_number := self._get_partner_party_identification_number(
            commercial_partner
        ):
        ...
        party_node.update(
            {
                ...
                "cac:PartyLegalEntity": {
                    "cbc:RegistrationName": {"_text": commercial_partner.name},
                    "cbc:CompanyID": {"_text": commercial_partner.vat}
                    if commercial_partner.country_code == "SA"
                    else None,
                    "cac:RegistrationAddress": self._get_address_node(
                        {"partner": commercial_partner}
                    ),
                ...
                "cac:Contact": {
                    ...
                    "cbc:Telephone": {
                        "_text": re.sub(r"[^+\d]", "", partner_phone)
    ...
    def _add_invoice_monetary_total_nodes(self, document_node, vals):
        ...
        for prepayment_move_base_lines in vals["prepayment_moves_base_lines"].values():
            ...
            for (
                grouping_key,
                values,
            ) in prepayment_moves_total_aggregated_tax_details.items():
                if grouping_key:
                    prepaid_amount += (
        ...
        monetary_total_node["cbc:AllowanceTotalAmount"] = {
            "_text": self.format_float(
                vals["total_allowance_currency"], vals["currency_dp"]
            ),
            "currencyID": vals["currency_name"],
        }
        monetary_total_node["cbc:PrepaidAmount"]["_text"] = self.format_float(
            prepaid_amount, vals["currency_dp"]
        )
        monetary_total_node["cbc:PayableAmount"]["_text"] = self.format_float(
            vals["tax_inclusive_amount_currency"] - prepaid_amount, vals["currency_dp"]
        )
    ...
    def _add_invoice_line_nodes(self, document_node, vals):
        ...
        for prepayment_move, prepayment_move_base_lines in vals[
            "prepayment_moves_base_lines"
        ].items():
    ...
    def _get_prepayment_line_node(self, vals):
        ...
        aggregated_tax_details = AccountTax._aggregate_base_lines_aggregated_values(
            prepayment_moves_base_lines_aggregated_tax_details
        )
        ...
        return {
            ...
            "cac:DocumentReference": {
                ...
                "cbc:IssueDate": {
                    "_text": prepayment_move_issue_date.strftime("%Y-%m-%d")
                    if prepayment_move_issue_date
                    else None
                },
                "cbc:IssueTime": {
                    "_text": prepayment_move_issue_date.strftime("%H:%M:%S")
                    if prepayment_move_issue_date
                    else None
                },
                "cbc:DocumentTypeCode": {"_text": "386"},
            },
            "cac:TaxTotal": self._get_prepayment_line_tax_total_node(
                {**vals, "aggregated_tax_details": aggregated_tax_details}
            ),
            "cac:Item": {
            ...
            "cac:TaxSubtotal": [
                {
                    "cbc:TaxableAmount": {
                        "_text": self.format_float(
                            values["base_amount_currency"], vals["currency_dp"]
                        ),
                        "currencyID": vals["currency_name"],
                    },
                    "cbc:TaxAmount": {
                        "_text": self.format_float(
                            values["tax_amount_currency"], vals["currency_dp"]
                        ),
                        "currencyID": vals["currency_name"],
                    },
    ...
    def _l10n_sa_generate_invoice_xml_sha(self, xml_content):
        ...
        def _transform_and_canonicalize_xml(content):
            ...
            invoice_xsl = etree.parse(
                file_path("l10n_sa_edi/data/pre-hash_invoice.xsl")
            )
