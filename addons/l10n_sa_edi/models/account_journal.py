...
class AccountJournal(models.Model):
    ...
    l10n_sa_production_csid_validity = fields.Datetime(
    ...
    def _l10n_sa_api_onboard_sanity_checks(self):
        ...
        move_ids = self.env["account.move"].search(
            [("journal_id", "=", self.id), ("l10n_sa_chain_index", "!=", 0)]
        )
        stuck_moves = [move for move in move_ids if not move._l10n_sa_is_in_chain()]
        if stuck_moves:
            raise UserError(
                _(
                    "Oops! The journal is stuck. Please submit the pending invoices to ZATCA and try again."
                )
            )
    ...
    def _l10n_sa_generate_csr(self):
        ...
        self.l10n_sa_csr = (
            self.env["certificate.certificate"].sudo()._l10n_sa_get_csr_str(self)
        )
    ...
    def _l10n_sa_get_compliance_CSID(self, otp):
        ...
        if CCSID_data.get("errors") or CCSID_data.get("error"):
            error = (
                CCSID_data["errors"][0]["message"]
                if CCSID_data.get("errors")
                else CCSID_data["error"]
            )
            raise UserError(
                Markup("%s<br/>%s")
                % (
                    _("Please check the details below and onboard the journal again:"),
                    error,
                )
            )
        cert_id = (
    ...
    def _l10n_sa_get_production_CSID(self, OTP=None):
        ...
        if (
            not self_sudo.l10n_sa_compliance_csid_json
            or not self_sudo.l10n_sa_compliance_csid_certificate_id
            or not self_sudo.l10n_sa_compliance_checks_passed
        ):
        ...
        if self_sudo.l10n_sa_production_csid_json:
            ...
            if zatca_format._l10n_sa_get_zatca_datetime(validity_time) < time_now:
                ...
                raise UserError(
                    _(
                        "The Journal is valid until (%s) and can only be renewed upon expiry.",
                        validity_time,
                    )
                )
        ...
        if PCSID_data.get("error") or PCSID_data.get("errors"):
            raise UserError(
                _(
                    ...
                    PCSID_data["errors"][0]
                    if PCSID_data.get("errors")
                    else PCSID_data["error"],
                )
            )
    ...
    def _l10n_sa_get_compliance_files(self):
        ...
        file_names, compliance_files = (
            [
                ...
                "simplified/credit.xml",
                "simplified/debit.xml",
            ],
            {},
        )
    ...
    def _l10n_sa_run_compliance_checks(self):
        ...
        if self.country_code != "SA":
            raise UserError(
                _(
                    "Please change the (%s)'s country to Saudi Arabia and try again.",
        ...
        for fname, fval in compliance_files.items():
            invoice_hash_hex = (
                self.env["account.edi.xml.ubl_21.zatca"]
                ._l10n_sa_generate_invoice_xml_hash(fval)
                .decode()
            )
            digital_signature = (
                self.env.ref("l10n_sa_edi.edi_sa_zatca")
                ._l10n_sa_get_digital_signature(self.company_id, invoice_hash_hex)
                .decode()
            )
            prepared_xml = self._l10n_sa_prepare_compliance_xml(
                fname,
                fval,
                self_sudo.l10n_sa_compliance_csid_certificate_id,
                digital_signature,
            )
            result = self._l10n_sa_api_compliance_checks(
                prepared_xml.decode(), CCSID_data
            )
            if result.get("error"):
                raise UserError(Markup("<p class='mb-0'>%s</p>") % (str(ERROR_MESSAGE)))
            if result["validationResults"]["status"] == "WARNING":
                warnings = Markup().join(
                    Markup("<li><b>%(code)s</b>: %(message)s </li>") % e
                    for e in result["validationResults"]["warningMessages"]
                )
                self.l10n_sa_csr_errors = Markup(
                    "<br/><br/><ul class='pl-3'><b>%s</b>%s</ul>"
                ) % (_("Warnings:"), warnings)
            elif result["validationResults"]["status"] != "PASS":
                raise UserError(Markup("<p class='mb-0'>%s</p>") % (str(ERROR_MESSAGE)))
        self.l10n_sa_compliance_checks_passed = True

    def _l10n_sa_prepare_compliance_xml(
        self, xml_name, xml_raw, certificate, signature
    ):
        """
        Prepare XML content to be used for Compliance checks
        """
        xml_content = self._l10n_sa_prepare_invoice_xml(xml_raw)
        signed_xml = self.env.ref("l10n_sa_edi.edi_sa_zatca")._l10n_sa_sign_xml(
            xml_content, certificate, signature
        )
        if xml_name.startswith("simplified"):
            qr_code_str = self.env["account.move"]._l10n_sa_get_qr_code(
                self.company_id, signed_xml, certificate, signature, True
            )
    ...
    def _l10n_sa_get_last_posted_invoice(self):
        ...
        return self.env["account.move"].search(
            [("journal_id", "=", self.id), ("l10n_sa_chain_index", "!=", 0)],
            limit=1,
            order="l10n_sa_chain_index desc",
        )
    ...
    def _l10n_sa_api_get_compliance_CSID(self, otp):
        ...
        return self._l10n_sa_call_api(
            request_data, ZATCA_API_URLS["apis"]["ccsid"], "POST"
        )
    ...
    def _l10n_sa_api_get_production_CSID(self, CCSID_data):
        ...
        return self._l10n_sa_call_api(
            request_data, ZATCA_API_URLS["apis"]["pcsid"], "POST"
        )
    ...
    def _l10n_sa_api_renew_production_CSID(self, PCSID_data, OTP):
        ...
        return self._l10n_sa_call_api(
            request_data, ZATCA_API_URLS["apis"]["pcsid"], "PATCH"
        )
    ...
    def _l10n_sa_api_compliance_checks(self, xml_content, CCSID_data):
        ...
        invoice_hash_node = invoice_tree.xpath(
            '//*[@Id="invoiceSignedData"]/*[local-name()="DigestValue"]'
        )[0]
        ...
        return self._l10n_sa_call_api(
            request_data, ZATCA_API_URLS["apis"]["compliance"], "POST"
        )
    ...
    def _l10n_sa_get_api_clearance_url(self, invoice):
        ...
        """
        return ZATCA_API_URLS["apis"][
            "reporting" if invoice._l10n_sa_is_simplified() else "clearance"
        ]
    ...
    def _l10n_sa_api_get_pcsid(self):
        ...
        self_sudo = self.sudo()
        if (
            not self_sudo.l10n_sa_production_csid_json
            or not self_sudo.l10n_sa_production_csid_certificate_id
        ):
            raise UserError(str(ERROR_MESSAGE))
    ...
    def _l10n_sa_call_api(self, request_data, request_url, method):
        ...
        try:
            ...
            if (status_code := ex.response.status_code) not in {400, 409}:
                return {
                    "error": (Markup("<b>[%s]</b>") % status_code)
                    + _(
        ...
        if request_response.status_code == "303":
            return {
                "error": _("Clearance and reporting seem to have been mixed up. "),
                "blocking_level": "warning",
                "excepted": True,
            }
        ...
        if not request_response.ok and (
            val_res.get("errorMessages") or val_res.get("warningMessages")
        ):
