...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_sa_get_zatca_datetime(self, timestamp):
        return fields.Datetime.context_timestamp(
            self.with_context(tz="Asia/Riyadh"), timestamp
        )

    def _l10n_sa_xml_node_content(self, root, xpath, namespaces=None):
        namespaces = (
            namespaces
            or self.env["account.edi.xml.ubl_21.zatca"]._l10n_sa_get_namespaces()
        )
        return etree.tostring(
            root.xpath(xpath, namespaces=namespaces)[0],
            with_tail=False,
            encoding="utf-8",
            method="xml",
        )
    ...
    def _l10n_sa_calculate_signed_properties_hash(
        ...
        signed_properties = etree.fromstring(
            self.env["ir.qweb"]._render(
                "l10n_sa_edi.export_sa_zatca_ubl_signed_properties",
                {
        ...
        for index, line in enumerate(signed_properties_split):
            ...
            if index != len(signed_properties_final) - 1:
                signed_properties_final += "\n"
        signed_properties_final = etree.tostring(
            etree.fromstring(signed_properties_final)
        )
    ...
    def _l10n_sa_sign_xml(self, xml_content, certificate, signature):
        ...
        signing_time = self._l10n_sa_get_zatca_datetime(datetime.now()).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
        ...
        _set_content(
            "//*[local-name()='SignedSignatureProperties']//*[local-name()='DigestValue']",
            public_key_hashing,
        )

        prehash_content = etree.tostring(root)
        invoice_hash = self.env[
            "account.edi.xml.ubl_21.zatca"
        ]._l10n_sa_generate_invoice_xml_hash(prehash_content, "digest")

        _set_content("//*[local-name()='SignatureValue']", signature)
        _set_content("//*[local-name()='X509Certificate']", der_cert.decode())
        _set_content(
            "//*[local-name()='SignatureInformation']//*[local-name()='DigestValue']",
            invoice_hash,
        )
        _set_content(
            "//*[@URI='#xadesSignedProperties']/*[local-name()='DigestValue']",
    ...
    def _l10n_sa_postprocess_zatca_template(self, xml_content):
        ...
        ubl_extensions = etree.fromstring(
            self.env["ir.qweb"]._render("l10n_sa_edi.export_sa_zatca_ubl_extensions")
        )
        root = etree.fromstring(xml_content)
    ...
    def _l10n_sa_generate_zatca_template(self, invoice):
        ...
        """
        xml_content, errors = self.env["account.edi.xml.ubl_21.zatca"]._export_invoice(
            invoice
        )
        if errors:
            return {
    ...
    def _l10n_sa_submit_einvoice(self, invoice, signed_xml, PCSID_data):
        ...
        """
        clearance_data = invoice.journal_id._l10n_sa_api_clearance(
            invoice, signed_xml.decode(), PCSID_data
        )
    ...
    def _l10n_sa_postprocess_einvoice_submission(
        self, invoice, signed_xml, clearance_data
    ):
        """
    ...
    def _l10n_sa_apply_qr_code(self, invoice, xml_content):
        ...
        qr_node = root.xpath(
            '//*[local-name()="ID"][text()="QR"]/following-sibling::*/*'
        )[0]
    ...
    def _l10n_sa_get_signed_xml(self, invoice, unsigned_xml, certificate):
        ...
        """
        signed_xml = self._l10n_sa_sign_xml(
            unsigned_xml, certificate, invoice.l10n_sa_invoice_signature
        )
        if invoice._l10n_sa_is_simplified():
    ...
    def _l10n_sa_export_zatca_invoice(self, invoice, xml_content=None):
        ...
        certificate_sudo = (
            self.env["certificate.certificate"].sudo().browse(certificate)
        )

        # Apply Signature/QR code on the generated XML document
        try:
            signed_xml = self._l10n_sa_get_signed_xml(
                invoice, unsigned_xml, certificate_sudo
            )
        except UserError:
            return (
                {
        ...
        return self._l10n_sa_submit_einvoice(
            invoice, signed_xml, PCSID_data
        ), signed_xml
    ...
    def _l10n_sa_post_zatca_edi(
        self, invoice
    ):  # no batch ensure that there is only one invoice
        """
        ...
        if (
            chain_head
            and chain_head != invoice
            and not chain_head._l10n_sa_is_in_chain()
        ):
        ...
        if not invoice.l10n_sa_chain_index:
            ...
            invoice.l10n_sa_chain_index = (
                invoice.journal_id._l10n_sa_edi_get_next_chain_index()
            )
            xml_content = invoice._l10n_sa_generate_unsigned_data()

        # Generate Invoice name for attachment
        attachment_name = self.env[
            "account.edi.xml.ubl_21.zatca"
        ]._export_invoice_filename(invoice)

        # Generate XML, sign it, then submit it to ZATCA
        response_data, submitted_xml = self._l10n_sa_export_zatca_invoice(
            invoice, xml_content
        )
        ...
        if response_data.get("error"):
            ...
            invoice._l10n_sa_log_results(
                submitted_xml, response_data, error=response_data.get("rejected")
            )
            ...
            return {invoice: {**response_data, "response": submitted_xml}}

        # Once submission is done with no errors, check submission status
        cleared_xml = self._l10n_sa_postprocess_einvoice_submission(
            invoice, submitted_xml, response_data
        )
    ...
    def _check_move_configuration(self, invoice):
        ...
        if self.code != "sa_zatca" or (
            company.country_id and company.country_id.code != "SA"
        ):
            return errors
        ...
        if invoice.invoice_date > fields.Date.context_today(
    ...
    def _l10n_sa_get_invoice_content_edi(self, invoice):
        ...
        """
        doc = invoice.edi_document_ids.filtered(
            lambda d: d.edi_format_id.code == "sa_zatca" and d.state == "sent"
        )
        return (
            doc.attachment_id.raw
            or self._l10n_sa_generate_zatca_template(invoice).encode()
        )
    ...
    def _prepare_invoice_report(self, pdf_writer, edi_document):
        ...
        if not attachment or not attachment.datas:
            _logger.warning(
                "No attachment found for invoice %s", edi_document.move_id.name
            )
            return
