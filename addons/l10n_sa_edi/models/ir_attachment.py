...
class IrAttachment(models.Model):
    ...
    def _unlink_except_rejected_zatca_document(self):
        ...
        for attach in self.filtered(
            ...
            if move and move.country_code == "SA":
                raise UserError(
                    _(
                        "You can't unlink an attachment being an EDI document refused by the government."
                    )
                )
    ...
    def _unlink_except_validated_pdf_invoices(self):
        ...
        res = self.env["account.edi.document"]._read_group(
            domain=[
                ("move_id", "in", attachments_to_check.mapped("res_id")),
                ("state", "=", "sent"),
        ...
        for attachment in attachments_to_check:
            if (
                document_date := edi_documents.get(attachment.res_id)
            ) and attachment.create_date >= document_date:
    ...
    def _get_posted_pdf_moves_to_check(self):
        ...
        return (
            super()
            ._get_posted_pdf_moves_to_check()
            .filtered(lambda rec: not rec.edi_state)
        )
