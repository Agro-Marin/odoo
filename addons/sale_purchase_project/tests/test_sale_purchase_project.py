...
class TestSalePurchaseProject(TestSalePurchase):
    def test_pol_analytic_distribution(self):
        ...
        self.sale_order_2.line_ids.analytic_distribution = {
            str(self.test_analytic_account_2.id): 100
        }
        ...
        self.assertEqual(
            purchase_line1.analytic_distribution,
            {
                str(
                    self.sale_order_1.project_id[self.analytic_plan._column_name()].id
                ): 100
            },
            "Analytic Distribution in PO should be same as Analytic Account set in SO",
        )
        self.assertEqual(
            purchase_line2.analytic_distribution,
            {str(self.test_analytic_account_2.id): 100},
            "Analytic Distribution in PO should be same as Analytic Distribution set in SOL",
        )
