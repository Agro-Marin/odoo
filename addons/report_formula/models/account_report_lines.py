...
class AccountReportLines(models.Model):
    ...
    def _expand_unfoldable_line(
        ...
        if not expand_function_name:
            raise UserError(
                self.env._("Trying to expand a line without an expansion function.")
            )
    ...
    def _regroup_lines_by_name_prefix(
        ...
        for prefix_key, prefix_sublines in sorted(
            ...
            if sublines_nber == 1:
                prefix_group_line_name = (
                    f"{matched_prefix}{prefix_to_display} " + self.env._("(1 line)")
                )
            else:
                prefix_group_line_name = (
                    f"{matched_prefix}{prefix_to_display} "
                    + self.env._("(%s lines)", sublines_nber)
                )
    ...
    def _get_custom_report_function(self, function_name, prefix):
        ...
        if not hasattr(self, function_name):
            ...
            raise UserError(self.env._("Invalid method “%s”", function_name))
    ...
    def _format_value(self, options, value, figure_type, format_params=None):
        ...
        if figure_type == "monetary":
            ...
            return self.env._("Yes") if bool(value) else self.env._("No")
    ...
    def _parse_line_id(self, line_id, markup_as_string=False):
        ...
        def parse_segment(key):
            ...
            if len(parts) != 3:
                raise UserError(
                    self.env._("Malformed report line id: %(line_id)s", line_id=line_id)
                )
    ...
    def _generate_total_below_section_line(self, section_line_dict):
        return {
            **section_line_dict,
            ...
            "name": self.env._("Total %s", section_line_dict["name"]),
    ...
    def _get_column_percent_comparison_data(
        ...
        if (
            ...
            return {"name": self.env._("n/a"), "mode": "muted"}
