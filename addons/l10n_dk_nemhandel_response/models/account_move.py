...
class AccountMove(models.Model):
    ...
    def _compute_nemhandel_move_state(self):
        ...
        for move in self:
            valid_status = move.nemhandel_response_ids.filtered(
                lambda r: r.nemhandel_state == "done"
            ).mapped("response_code")
            if not valid_status:
                continue
        ...
        for move in self:
            move.nemhandel_can_send_response = (
                ...
                and not move.nemhandel_response_ids.filtered(
                    lambda r: (
                        r.nemhandel_state == "not_serviced"
                        or r.nemhandel_state != "error"
                    )
                )
    ...
    def action_nemhandel_send_approval_response(self):
        moves_to_respond_by_company = self.filtered(
    ...
    def action_nemhandel_open_rejection_wizard(self):
        ...
        if nemhandel_moves:
            return {
                ...
                "target": "new",
                "res_id": self.env["nemhandel.rejection.wizard"]
