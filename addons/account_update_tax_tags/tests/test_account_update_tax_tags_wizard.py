...
class TestAccountUpdateTaxTagsWizard(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.wizard = cls.env["account.update.tax.tags.wizard"].create(
    ...
    def _create_tax(
        ...
        cash_basis_transfer_account=None,
        **kwargs,
    ):
        if not tag_names:
            tag_names = {}
        ...
        vals = {
            ...
            if cash_basis_transfer_account
            else None,
            "children_tax_ids": [Command.set(children_taxes.ids)]
            if children_taxes
            else None,
    ...
    def _change_tax_tag(cls, tax, new_tag, invoice=True, base=True):
        ...
        filtered_rep_lines.write(
            {"tag_ids": [Command.set(cls._create_or_get_tax_tag(new_tag).ids)]}
        )
    ...
    def test_update_tax_tags(self):
        ...
        """
        moves = self._create_invoice(taxes=self.tax_1) + self._create_invoice(
            taxes=self.tax_1
        )
        self._change_tax_tag(
            self.tax_1, "invoice_tax_tag_changed", invoice=True, base=False
        )
        self.wizard.update_amls_tax_tags()

        invoice_lines, tax_lines, counterpart_lines = self._get_amls_by_type(moves)
        self.assertEqual(
            invoice_lines.tax_tag_ids.name,
            "invoice_base_tag",
            "Base lines tags should not have changed.",
        )
        self.assertEqual(
            tax_lines.tax_tag_ids.name,
            "invoice_tax_tag_changed",
            "Tax lines tags should have changed.",
        )
        self.assertFalse(
            counterpart_lines.tax_tag_ids, "Counterpart lines should not have changed."
        )

    def test_update_date_from(self):
        """Only the amls that are concerned by the date_from constraint should be updated."""
        move_included = self._create_invoice(
            date_invoice="2023-02-23", taxes=self.tax_1
        )
        move_not_included = self._create_invoice(
            date_invoice="2023-01-23", taxes=self.tax_1
        )
        self._change_tax_tag(
            self.tax_1, "invoice_tax_tag_changed", invoice=True, base=False
        )
        ...
        self.assertEqual(
            tax_line_included.tax_tag_ids.name,
            "invoice_tax_tag_changed",
            "Move within the date constraint should be updated.",
        )
        tax_line_not_included = move_not_included.line_ids.filtered(
            lambda aml: aml.tax_line_id
        )
        self.assertEqual(
            tax_line_not_included.tax_tag_ids.name,
            "invoice_tax_tag",
    ...
    def test_update_multiple_taxes(self):
        ...
        self._change_tax_tag(
            tax_2, "invoice_tax_tag_changed_2", invoice=True, base=False
        )
        self.wizard.update_amls_tax_tags()
    ...
    def test_update_multi_company(self):
        ...
        self._change_tax_tag(
            self.tax_1,
            "invoice_tax_tag_changed_for_company_1",
            invoice=True,
            base=False,
        )
        ...
        self._change_tax_tag(
            tax_2, "invoice_tax_tag_changed_for_company_2", invoice=True, base=False
        )
        self.wizard.update_amls_tax_tags()
        ...
        self.assertEqual(
            tax_line_1.tax_tag_ids.name, "invoice_tax_tag_changed_for_company_1"
        )
        self.assertEqual(
            tax_line_2.tax_tag_ids.name, "update_test_invoice_tax_tag_company_2"
        )
        ...
        for move_type in move_types:
            for line_type in ["base", "tax"]:
                with self.subTest(f"Update tax tag on {move_type}-{line_type}"):
                    ...
                    tax_2 = self._create_tax(
                        f"update_test_tax_2_{move_type}_{line_type}",
                        15,
                        type_tax_use=type_tax_use,
                        tag_names={
                    ...
                    if super_type == "receipt":
                        super_type = (
                            "invoice"  # receipt type acts just like invoice one
                        )
                    self._change_tax_tag(
                        tax_2,
                        f"{move_type}_{line_type}_tag_changed",
                        invoice=super_type == "invoice",
                        base=line_type == "base",
                    )
                    self.wizard.update_amls_tax_tags()

                    invoice_line, tax_line, counterpart_line = self._get_amls_by_type(
                        move
                    )
                    if line_type == "base":
                        self.assertEqual(
                            invoice_line.tax_tag_ids.name,
                            f"{move_type}_{line_type}_tag_changed",
                            "Base lines tags should have changed.",
                        )
                        self.assertEqual(
                            tax_line.tax_tag_ids.name,
                            f"test_tag_{super_type}_tax",
    ...
    def test_update_move_type_entry(self):
        ...
        ]  # Account is not relevant for the test but must be set.
        for tax_type in ["sale", "purchase"]:
            tax = self._create_tax(
                f"test_{tax_type}_tax",
                10,
            ...
            for balance in [-1000, 1000]:
                with self.subTest(f"Testing move type entry {tax_type}: {balance}"):
                    ...
                    self._change_tax_tag(
                        tax, "invoice_base_tag_changed", invoice=True, base=True
                    )
                    self._change_tax_tag(
                        tax, "refund_base_tag_changed", invoice=False, base=True
                    )

                    self.wizard.update_amls_tax_tags()
                    invoice_line, tax_line, _ = self._get_amls_by_type(move)
                    if (balance < 0 and tax_type == "sale") or (
                        balance > 0 and tax_type == "purchase"
                    ):
                        self.assertEqual(
                            invoice_line.tax_tag_ids.name, "invoice_base_tag_changed"
                        )
                        self.assertEqual(tax_line.tax_tag_ids.name, "invoice_tax_tag")
                    elif (balance < 0 and tax_type == "purchase") or (
                        balance > 0 and tax_type == "sale"
                    ):
                        self.assertEqual(
                            invoice_line.tax_tag_ids.name, "refund_base_tag_changed"
                        )
    ...
    def test_update_amls_all_states(self):
        ...
        moves = [
            self._create_invoice(
                partner_id=self.partner_a.id, taxes=self.tax_1, state=state
            )
            for state in move_states
        ]
        self._change_tax_tag(
            self.tax_1, "invoice_base_tag_changed", invoice=True, base=True
        )
        self._change_tax_tag(
            self.tax_1, "invoice_tax_tag_changed", invoice=True, base=False
        )
    ...
    def test_update_no_tag_after(self):
        ...
        )  # Command.CLEAR both base and tax lines
        self.wizard.update_amls_tax_tags()
    ...
    def test_update_child_tax(self):
        ...
        self.assertEqual(
            tax_lines.tax_tag_ids.sorted("name").mapped("name"),
            ["invoice_tax_tag_child_1", "invoice_tax_tag_child_2"],
        )
        self.assertFalse(counterpart_line.tax_tag_ids)

        self._change_tax_tag(
            tax_child_1, "invoice_base_tag_1_changed", invoice=True, base=True
        )
        self._change_tax_tag(
            tax_child_1, "invoice_tax_tag_1_changed", invoice=True, base=False
        )
        self._change_tax_tag(
            tax_child_2, "invoice_base_tag_2_changed", invoice=True, base=True
        )
        self._change_tax_tag(
            tax_child_2, "invoice_tax_tag_2_changed", invoice=True, base=False
        )
        self.wizard.update_amls_tax_tags()
    ...
    def test_update_with_caba_taxes(self):
        ...
        self.assertEqual(
            caba_base_line.tax_tag_ids.name,
            "invoice_base_tag_changed",
            "CABA base lines tags should have changed.",
        )
        self.assertEqual(
            caba_tax_line.tax_tag_ids.name,
            "invoice_tax_tag_changed",
            "CABA tax lines tags should have changed.",
        )
        self.assertFalse(
            caba_counterpart_lines.tax_tag_ids,
            "CABA counterpart lines should not have changed.",
        )
    ...
    def test_child_tax_multiple_parent_raises(self):
        ...
        tax_parent_1 = self._create_tax(
            "tax_parent", 15, amount_type="group", children_taxes=tax_child
        )
        self._create_tax(
            "tax_parent_2", 15, amount_type="group", children_taxes=tax_child
        )
