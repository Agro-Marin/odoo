...
class SaleReport(models.Model):
    ...
    team_id = fields.Many2one(
        comodel_name="crm.team", string="Sales Team", readonly=True
    )
    ...
    def _select_sale(self):
        select_ = f"""
            ...
            CASE WHEN l.product_id IS NOT NULL THEN SUM(l.qty_transfered * u.factor / u2.factor) ELSE 0 END AS qty_transfered,
            CASE WHEN l.product_id IS NOT NULL THEN SUM((l.product_uom_qty - l.qty_transfered) * u.factor / u2.factor) ELSE 0 END AS qty_to_deliver,
            ...
            CASE WHEN l.product_id IS NOT NULL OR l.is_downpayment THEN SUM(l.amount_to_invoice_taxexc
                / {self._case_value_or_one('s.currency_rate')}
                * {self._case_value_or_one('account_currency_table.rate')}
                ) ELSE 0
            ...
            s.state AS state,
            s.invoice_state as invoice_state,
