...
class SaleOrderLine(models.Model):
    ...
    @api.depends("qty_invoiced", "qty_transfered", "product_uom_qty", "state")
    ...
    @api.depends(
        "state", "product_uom_qty", "qty_transfered", "qty_to_invoice", "qty_invoiced"
    )
    def _compute_invoice_state(self):
        ...
        for line in self:
            if line.state != "sale":
                ...
                line.invoice_state = "invoiced"
            elif not float_is_zero(line.qty_to_invoice, precision_digits=precision):
                line.invoice_state = "to invoice"
            elif (
                line.state == "sale"
    ...
    @api.depends(
        "invoice_line_ids", "invoice_line_ids.price_total", "invoice_line_ids.move_id.state"
    )
        ...
        for line in self:
            ...
            for invoice_line in line._get_invoice_lines():
                ...
                if (
                    ...
                    invoice_date = invoice.invoice_date or fields.Date.context_today(
                        self
                    )
                    ...
                    amount_invoiced += (
                        amount_invoiced_unsigned * -invoice.direction_sign
                    )
    ...
    def _compute_amount_to_invoice_taxexc(self):
        ...
        for line in self:
            ...
            line.amount_to_invoice_taxexc = amount_to_invoice
    ...
    @api.depends("product_id", "state", "qty_invoiced", "qty_transfered")
    ...
    def write(self, vals):
        ...
        if "product_id" in values and any(
            sol.product_id.id != values["product_id"] and not sol.product_updatable
            for sol in self
        ):
        ...
        for order in orders:
            ...
            for line in order_lines:
                ...
                if line.product_id.type == "consu":
                    msg += _("Delivered Quantity: %s", line.qty_transfered) + Markup(
                        "<br/>"
                    )
