...
class SaleOrderLine(models.Model):
    ...
    @api.depends("product_id", "product_uom_id", "product_uom_qty", "display_type")
    def _compute_price_unit(self):
        ...
        - Display lines (sections, notes) - these must have NULL price
        - Down payments
        ...
        # Display type lines (notes, sections) must have NULL price
        for line in self.filtered(lambda x: x.display_type):
        ...
        for line in self.filtered(lambda x: not x.display_type):
            ...
            # If product changed, reset manual price flag (new product = new automatic pricing)
            origin = line._origin
    ...
    def _compute_invoice_state(self):
        ...
        for line in self.filtered(lambda l: not l.display_type):
            if line.is_downpayment and line.amount_taxexc_to_invoice == 0:
                ...
                # For order-based products, compare to product_uom_qty (what was ordered)
                qty_to_compare = (
                    line.qty_transferred
                    if line.product_id.invoice_policy == "transferred"
    ...
    def _should_update_price(self, force_recompute=False):
        ...
        if not force_recompute and self.price_unit_shadow is not False:
            ...
            if product_unchanged:
                ...
                if abs(self.price_unit - self.price_unit_shadow) > 0.001:
                    return False
