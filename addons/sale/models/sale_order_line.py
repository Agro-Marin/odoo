...
class SaleOrderLine(models.Model):
    ...
    def _compute_price_and_discount(self):
        ...
        for line in self:
            ...
            if not is_special_line:
                # If product changed, reset manual price flag
                origin = line._origin
                if origin and origin.product_id != line.product_id:
                    line.price_is_manual = False

                # Check if price update is allowed
                should_update = line._should_update_price(force_recompute)
                ...
                if line.product_id and line.product_uom_id:
                    ...
                    if should_update:
                        ...
                        if price_unit and line.order_id.fiscal_position_id:
                        ...
                        line.price_unit_shadow = price_unit
                        line.price_is_manual = False
                    else:
                        # Price is protected, only update shadow
                        line.price_unit_shadow = display_price
    ...
    def _compute_invoice_amounts(self):
        ...
        for line in self.filtered(lambda x: not x.display_type):
            ...
            has_different_discount = False  # Track if any invoice line has different discount
            ...
            for invoice_line in invoice_lines:
                ...
                if not has_different_discount and invoice_line.discount != line.discount:
            ...
            price_subtotal = line.currency_id.round(price_unit_discounted * qty_to_consider)
            ...
            unit_price_total = (
                0.0 if float_is_zero(line.product_qty, precision_digits=precision)
    ...
    def reset_to_pricelist_price(self):
        ...
        return self.with_context(force_price_recomputation=True)._compute_price_and_discount()
    ...
    def _should_update_price(self, force_recompute=False):
        ...
        # Also protect prices that differ from shadow price (manual change in progress)
        # This handles the case where price_unit was changed via Form/onchange
        # but price_is_manual hasn't been committed yet
        # Only apply this protection if the product hasn't changed (otherwise shadow is stale)
        if not force_recompute and self.price_unit_shadow is not False:
            origin = self._origin
            # For existing records: only protect if product hasn't changed
            # For new records (no origin): protect if price differs from shadow
            product_unchanged = (
                not origin  # New record - no origin to compare
                or (origin.product_id == self.product_id and origin.product_uom_id == self.product_uom_id)
            )
            if product_unchanged:
                # Use a small tolerance to handle floating point comparison
                if abs(self.price_unit - self.price_unit_shadow) > const.PRICE_COMPARISON_TOLERANCE:
                    return False
    ...
    def _validate_write_display_type(self, write_vals):
        ...
        if len(lines) == 1:
            ...
            raise UserError(
                _(
                    "You cannot change the type of %s sales order lines (%s). "
