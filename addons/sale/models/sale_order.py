...
from odoo.tools import float_is_zero, format_amount, is_html_empty, OrderedSet, SQL
...
class SaleOrder(models.Model):
    ...
    def _check_prepayment_percent(self):
        for order in self:
            if order.require_payment and not (0 < order.prepayment_percent <= 1.0):
                raise ValidationError(
                    _("Prepayment percentage must be greater than 0%% and at most 100%%."),
                )
    ...
    def _compute_has_active_pricelist(self):
        ...
        has_global_pricelist = bool(
            Pricelist.search([("company_id", "=", False), ("active", "=", True)], limit=1)
        )
        ...
        if company_ids:
            ...
            companies_with_pricelist = {data[0].id for data in pricelist_data if data[0]}
    ...
    def _compute_notes(self):
        ...
        for order in self:
            ...
            if order_company.terms_type == "html" and self.env.company.invoice_terms_html:
    ...
    def _onchange_pricelist_id_show_update_prices(self):
        self.show_update_pricelist = bool(self.line_ids and self._origin.pricelist_id != self.pricelist_id)
