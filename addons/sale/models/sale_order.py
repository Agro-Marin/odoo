...
class SaleOrder(models.Model):
    ...
    create_date = fields.Datetime(
        ...
        readonly=True,
        # Override of default create_date field from ORM
        index=True,
    )
    ...
    @api.depends(
        "line_ids.price_subtotal", "currency_id", "company_id", "payment_term_id"
    )
    ...
    def _compute_invoice_state(self):
        ...
        for order in confirmed_orders:
            ...
            if order.state != "sale":
                ...
                invoice_state == "to invoice" for invoice_state in line_invoice_state
            ):
                if any(
                ...
                invoice_state == "invoiced" for invoice_state in line_invoice_state
            ):
    ...
    @api.depends("line_ids.customer_lead", "date_order", "state")
    ...
    @api.depends("line_ids.amount_to_invoice")
    def _compute_amount_to_invoice(self):
        for order in self:
            order.amount_to_invoice = sum(order.line_ids.mapped("amount_to_invoice"))

    @api.depends("line_ids.amount_invoiced")
    def _compute_amount_invoiced(self):
        for order in self:
            order.amount_invoiced = sum(order.line_ids.mapped("amount_invoiced"))
    ...
    @api.depends(
        "partner_id.name", "partner_id.sale_warn_msg", "line_ids.sale_line_warn_msg"
    )
    ...
    def _onchange_date_commitment(self):
        ...
        if (
            ...
            and self.date_commitment < self.expected_date
        ):
            return {
                "warning": {
    ...
    def _recompute_taxes(self):
        lines_to_recompute = self.line_ids.filtered(
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        for order in self:
            ...
            for line in invoiceable_lines:
                ...
                for vals in line._prepare_invoice_line_ids_vals_list(**optional_values):
    ...
    def _compute_field_value(self, field):
        ...
        filtered_self = self.filtered(
            ...
            and so._origin.invoice_state != "upselling"
        )
        super()._compute_field_value(field)

        upselling_orders = filtered_self.filtered(
            lambda so: so.invoice_state == "upselling"
        )
