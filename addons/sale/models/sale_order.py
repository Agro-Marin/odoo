...
class SaleOrder(models.Model):
    ...
    def _get_default_user_from_partner(self):
        """Return salesperson from partner or current user."""
        self.ensure_one()
        return (
    ...
    def _should_be_locked(self):
        """Sale uses sale.group_auto_done_setting for auto-lock."""
        self.ensure_one()
    ...
    def _compute_access_url(self):
        """Sale orders use /my/orders/ instead of /my/sale/."""
    ...
    def _get_additional_base_lines(self):
        """Add early payment discount lines to the tax computation."""
    ...
    def action_confirm(self):
        """Extend base confirmation to clear context keys and send email."""
        # Context key 'default_name' is sometimes propagated up to here.
        # We don't need it and it creates issues in the creation of linked records.
        context = self.env.context.copy()
        context.pop("default_name", None)
        context.pop("default_user_id", None)
        clean_self = self.with_context(context)
        super(SaleOrder, clean_self).action_confirm()
    ...
    def action_draft(self):
        """Extend base to also clear signature fields."""
        ...
        return super(SaleOrder, orders).action_draft()
    ...
    def _prepare_grouped_data(self, order):
        """Add partner_shipping_id to the merge grouping key."""
        return super()._prepare_grouped_data(order) + (
            order.partner_shipping_id.id,
        )
    ...
    def _merge_metadata_refs(self, target, sources):
        """Merge client_order_ref instead of partner_ref."""
        all_refs = [target.client_order_ref] + list(
            sources.mapped("client_order_ref"),
        )
    ...
    def _get_invoice_action_context(self):
        """Add partner_shipping_id to invoice context."""
    ...
    def _get_invoice_grouping_keys(self):
        ...
        keys.insert(2, "partner_shipping_id")
        return keys
    ...
    def _prepare_invoice_vals(self):
        """Extend base invoice values with sale-specific fields."""
