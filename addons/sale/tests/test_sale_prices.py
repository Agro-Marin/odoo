...
class TestSalePrices(SaleCommon):
    ...
    def test_pricelist_minimal_qty(self):
        ...
        discounted_lines = self.empty_order.line_ids.filtered("pricelist_item_id")
        self.assertEqual(discounted_lines, self.empty_order.line_ids[1:5])
        self.assertEqual(discounted_lines.pricelist_item_id, pricelist_rule)
        self.assertTrue(
            all(
                not line.discount
                for line in self.empty_order.line_ids - discounted_lines
            )
        )
        self.assertEqual(
        ...
        discounted_lines[0].product_qty = 3.0
    ...
    def test_pricelist_product_context(self):
        ...
        order_lines = self.empty_order.line_ids
    ...
    def test_no_pricelist_rules(self):
        ...
        self.assertEqual(self.empty_order.line_ids.price_unit, 240.0)
    ...
    def test_pricelist_based_on_another(self):
        ...
        self.assertEqual(
            order_line.price_subtotal, 81, "Second pricelist rule not applied"
        )
        self.assertEqual(
            order_line.discount,
            19,
    ...
    def test_pricelist_with_another_currency(self):
        ...
        self.assertEqual(
            order_line.price_total, 1800, "First pricelist rule not applied"
        )
    ...
    def test_pricelist_price_recompute_on_quantity_change(self):
        ...
        with Form(self.env["sale.order"]) as order_form:
            ...
            with order_form.line_ids.new() as line_form:
                line_form.product_id = self.product
                self.assertAlmostEqual(line_form.price_unit, 0.825)
                line_form.product_qty = 1000
    ...
    def test_multi_currency_discount(self):
            ...
            .create(
                {
                    ...
                    "line_ids": [
                        Command.create(
                            {"product_id": product_1.id, "product_qty": 1.0}
                        ),
                        Command.create(
                            {"product_id": product_2.id, "product_qty": 1.0}
                        ),
                    ],
                }
            )
        ...
        so_line_1 = sales_order.line_ids[0]
        so_line_2 = sales_order.line_ids[1]
    ...
    def test_update_prices(self):
        ...
        pricelist.item_ids = [
            Command.create({"percent_price": 5.0, "compute_price": "percentage"})
        ]
        sale_order._recompute_prices()

        self.assertTrue(all(line.discount == 5 for line in sale_order.line_ids))
        amount_undiscounted = sum(
            line.price_unit * line.product_uom_qty for line in sale_order.line_ids
        )
        self.assertEqual(amount_undiscounted, so_amount)
        ...
        # With formula discount, the discount is applied to price_unit (not discount field)
        self.assertTrue(all(line.discount == 0 for line in sale_order.line_ids))
        amount_undiscounted = sum(
            line.price_unit * line.product_uom_qty for line in sale_order.line_ids
        )
        # price_unit is reduced by 5%, so amount_undiscounted = 0.95 * so_amount
        self.assertEqual(amount_undiscounted, 0.95 * so_amount)
        self.assertEqual(sale_order.amount_total, 0.95 * so_amount)
    ...
    def test_manual_price_prevents_recompute(self):
        sale_order_line = self.sale_order.line_ids[0]
        ...
        self.assertEqual(so_line.price_unit_auto, 0.0)

        with Form(so_line) as so_line:
            so_line.price_unit = 10.0
            so_line.product_qty = 2.0
            so_line.save()
    ...
    def test_fiscalposition_application(self):
        ...
        self.assertRecordValues(
            sale_order.line_ids, [{"price_unit": 106, "price_subtotal": 100}]
        )
        ...
        sale_order = order_form.save()
        self.assertRecordValues(
            sale_order.line_ids, [{"price_unit": 100, "price_subtotal": 94.34}]
        )
        ...
        sale_order = order_form.save()
        self.assertRecordValues(
            sale_order.line_ids, [{"price_unit": 100, "price_subtotal": 100}]
        )
        ...
        sale_order = order_form.save()
        self.assertRecordValues(
            sale_order.line_ids, [{"price_unit": 100, "price_subtotal": 84.91}]
        )
    ...
    def test_so_tax_mapping(self):
        ...
        self.product.write(
            {"list_price": 121, "taxes_id": [Command.set(tax_include.ids)]}
        )
    ...
    def test_so_tax_mapping_multicompany(self):
        ...
        self.assertEqual(
            order.line_ids.tax_ids, tax_exclude, "Line tax should be mapped"
        )
        self.assertAlmostEqual(
            order.line_ids.price_unit,
            100.0,
            ...
            Command.create(
                {
                    ...
                    "product_qty": 1,
                    "price_unit": 0.0,
                    "price_unit_auto": 1.0,  # Different from price_unit to mark as manual
                    "tax_ids": [
    ...
    def test_sale_with_taxes(self):
        ...
        self.sale_order.line_ids[0].write({"tax_ids": [Command.link(tax_include.id)]})
        self.sale_order.line_ids[1].write({"tax_ids": [Command.link(tax_exclude.id)]})
    ...
    def test_discount_and_untaxed_subtotal(self):
        ...
        line = order.line_ids
        self.assertEqual(line.amount_taxexc_to_invoice, 0)

        line.qty_transferred = 38
        # (541.26 - 0.02 * 541.26) * 38 = 20156.5224 ~= 20156.52
        self.assertEqual(line.price_subtotal, 20156.52)
        self.assertEqual(line.amount_taxexc_to_invoice, line.price_subtotal)
    ...
    def test_discount_calculation(self):
        """When adding a discount on a SO line, this test ensures that discounts
        are applied correctly and price_subtotal is consistent"""
        order = self.empty_order
        ...
        order.action_confirm()
        order_line = order.line_ids

        # test discount and qty 1: undiscounted = 100.0, subtotal = 99.0 (1% off)
        amount_undiscounted = sum(
            line.price_unit * line.product_uom_qty for line in order.line_ids
        )
        self.assertEqual(amount_undiscounted, 100.0)
        ...
        self.assertEqual(amount_undiscounted, 300.0)
    ...
    def test_product_quantity_rounding(self):
        ...
        self.assertEqual(
            ...
            float_round(product_uom_qty, precision_digits=quantity_precision),
        )
    ...
    def test_show_discount(self):
        ...
        sale_order._recompute_prices()
        show_discount_line = sale_order.line_ids[0]
        included_discount_line = sale_order.line_ids[1]

        self.assertEqual(show_discount_line.price_unit, 100)
        self.assertEqual(
            show_discount_line.price_subtotal, show_discount_line.price_unit * 0.9
        )
        self.assertEqual(show_discount_line.discount, 10)
        self.assertEqual(
            included_discount_line.price_unit, included_discount_line.price_subtotal
        )
        ...
        self.assertEqual(show_discount_line.price_unit, 100)
        self.assertEqual(
            show_discount_line.price_subtotal, show_discount_line.price_unit * 0.81
        )
    ...
    def test_combo_product_discount(self):
        ...
        self._create_discount_pricelist_rule(
            product_tmpl_id=product_combo.product_tmpl_id.id
        )
        ...
        self.assertAlmostEqual(
            ...
            amount_undiscounted * (100 - self.discount) / 100,
