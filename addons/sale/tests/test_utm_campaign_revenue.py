...
class TestUtmCampaignRevenue(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product = cls.env['product.product'].create({
            'name': 'Campaign product', 'type': 'consu', 'list_price': 100.0,
        })
    ...
    def test_quotation_redirect_is_scoped_to_the_campaign(self):
        ...
        self.assertEqual(
            action['context']['default_campaign_id'], self.campaign.id,
        )
    ...
    def test_invoiced_redirect_lists_only_live_invoices(self):
        ...
        domain = {
            item[0]: item[2] for item in action['domain'] if len(item) == 3
        }
