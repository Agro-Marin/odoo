...
class TestProductConfiguratorData(
                    ...
                    Command.create(
                        {
                            ...
                            "value_ids": [
                                Command.set([self.no_variant_attribute_extra.id]),
                            ],
                        }
                    ),
                    Command.create(
    ...
    def test_dropped_value_isnt_shown(self):
        ...
        self.product_template_sofa.attribute_line_ids.value_ids -= (
            self.color_attribute_red
        )
        self.assertEqual(
            len(self.product_template_sofa.product_variant_ids.filtered("active")), 2
        )
        ...
        self.assertEqual(
            len(result["products"][0]["attribute_lines"][0]["attribute_values"]), 2
        )
    ...
    def test_excluded_inactive_ptav(self):
        ...
        ptav_with_exclusion = product_template.attribute_line_ids[
            0
        ].product_template_value_ids[0]
        ptav_excluded = product_template.attribute_line_ids[
            1
        ].product_template_value_ids[0]
        ...
        self.assertEqual(
            result["products"][0]["exclusions"][str(ptav_with_exclusion.id)],
            [ptav_excluded.id],
        )
        self.assertEqual(
            result["products"][0]["exclusions"][str(ptav_excluded.id)],
            [ptav_with_exclusion.id],
        )
        ...
        self.assertFalse(
            str(ptav_with_exclusion.id) in result["products"][0]["exclusions"]
        )
    ...
    def test_ptal_values_set_for_no_variant_atribute(self):
        ...
        self.assertTrue(
            result["products"][0]["attribute_lines"][1]["selected_attribute_value_ids"]
        )
    ...
    def test_attribute_removal(self):
        ...
        exclusions_data = (
            product_template_2lines_2attributes._get_attribute_exclusions()
        )
        self.assertTrue(
            all(
