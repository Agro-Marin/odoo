...
class TestSaleComboConfigurator(HttpCase, SaleCommon):
        ...
        no_variant_attribute = self.env["product.attribute"].create(
            {
                ...
                "value_ids": [
                    ...
                    Command.create(
                        {"name": "B", "is_custom": True, "default_extra_price": 1}
                    ),
                ],
            }
        )
        ...
        combo_a = self.env["product.combo"].create(
            {
                ...
                "combo_item_ids": [
                    Command.create(
                        {
                            "product_id": product_a1.product_variant_id.id,
                            "extra_price": 5,
                        }
                    ),
                    Command.create(
                        {"product_id": self._create_product(name="Product A2").id}
                    ),
                ],
            }
        )
        combo_b = self.env["product.combo"].create(
            {
                ...
                "combo_item_ids": [
                    Command.create(
                        {"product_id": self._create_product(name="Product B1").id}
                    ),
                    Command.create(
                        {"product_id": self._create_product(name="Product B2").id}
                    ),
                ],
            }
        )
    ...
    def test_sale_combo_configurator_with_optional_products(self):
        ...
        combo_a = self.env["product.combo"].create(
            {
                ...
                "combo_item_ids": [
                    Command.create(
                        {"product_id": self._create_product(name="Product A1").id}
                    ),
                ],
            }
        )
        ...
        self.start_tour(
            "/", "sale_combo_configurator_with_optional_products", login="salesman"
        )

        order = self.env["sale.order"].search(
            [("partner_id.name", "=", "Test Partner")], limit=1
        )
        ...
        combo_with_multiple_unconfigurable_items = self.env["product.combo"].create(
            {
                ...
                "combo_item_ids": [
                    Command.create(
                        {"product_id": self._create_product(name="Product E1").id}
                    ),
                    Command.create(
                        {"product_id": self._create_product(name="Product E2").id}
                    ),
                ],
            }
        )
    ...
    def test_sale_combo_configurator_preconfigure_unconfigurable_ptals(self):
                    ...
                    Command.create(
                        {
                            ...
                            "value_ids": [
                                Command.set(
                                    unconfigurable_no_variant_attribute.value_ids.ids
                                )
                            ],
                        }
                    ),
                    Command.create(
                        {
                            ...
                            "value_ids": [
                                Command.set(
                                    configurable_no_variant_attribute.value_ids.ids
                                )
                            ],
                        }
                    ),
        ...
        combo = self.env["product.combo"].create(
            {
                "name": "Test combo",
                "combo_item_ids": [
                    Command.create({"product_id": product.product_variant_id.id})
                ],
            }
        )
        self._create_product(
        ...
        self.start_tour(
            "/",
            "sale_combo_configurator_preconfigure_unconfigurable_ptals",
            login="salesman",
        )
