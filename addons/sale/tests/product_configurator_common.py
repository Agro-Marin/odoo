...
class TestProductConfiguratorCommon(UomCommon, HttpCase):
    ...
    def setUpClass(cls):
                    ...
                    Command.create(
                        {
                            "attribute_id": cls.product_attribute_1.id,
                            "value_ids": [
                                Command.link(product_attribute_value_1.id),
                                Command.link(product_attribute_value_2.id),
                            ],
                        }
                    ),
                    Command.create(
                        {
                            "attribute_id": product_attribute_2.id,
                            "value_ids": [
                                Command.link(product_attribute_value_3.id),
                                Command.link(product_attribute_value_4.id),
                            ],
                        }
                    ),
                ],
        ...
        cls.product_product_conf_chair.attribute_line_ids[0].product_template_value_ids[
            1
        ].price_extra = 6.40
        ...
        cls.product_product_conf_chair.optional_product_ids = [
            (4, cls.product_product_conf_chair_floor_protect.id)
        ]
