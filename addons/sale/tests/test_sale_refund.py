...
class TestSaleRefund(TestSaleCommon):
    ...
    def test_refund_create(self):
        ...
        for line in self.sale_order.line_ids:
            if line.product_id.invoice_policy == "transferred":
                ...
                self.assertFalse(
                    ...
                    "The line based on delivered qty are not invoiced, so they should not be linked to invoice line",
                )
            else:
        ...
        self.assertEqual(
            ...
            "out_refund",
            "The last created invoiced should be a refund",
        )
        self.assertEqual(
            invoice_refund.state, "draft", "Last Customer invoices should be in draft"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(
                self.sale_order.invoice_ids.filtered(
        ...
        for line in self.sale_order.line_ids:
            if line.product_id.invoice_policy == "transferred":
                ...
                self.assertEqual(
                    line.qty_invoiced,
                    0.0,
                    "Invoiced quantity should be zero as no any invoice created for SO line based on delivered qty",
                )
                self.assertEqual(
                    line.amount_taxexc_to_invoice,
                    0.0,
    ...
    def test_refund_invoice_with_downpayment(self):
        ...
        credit_note_wizard.refund_moves()
        invoice_refund = sale_order_refund.invoice_ids.sorted(
            key=lambda inv: inv.id, reverse=False
        )[-1]
        invoice_refund.action_post()

        self.assertEqual(
            sol_product.qty_to_invoice,
            5.0,
            "As the refund still exists, the quantity to invoice is the ordered quantity",
        )
        self.assertEqual(
            sol_product.qty_invoiced,
            0.0,
            "The qty invoiced should be zero as, with the refund, the quantities cancel each other",
        )
        self.assertEqual(
            sol_product.amount_taxexc_to_invoice,
            sol_product.price_unit * 5,
            "Amount to invoice is now set as qty to invoice * unit price since no price change on invoice, as refund is validated",
        )
        self.assertEqual(
            sol_product.amount_taxexc_invoiced,
            0.0,
            "Amount invoiced decreased as the refund is now confirmed",
        )
        self.assertEqual(
            len(sol_product.invoice_line_ids),
            2,
            "The product line is invoiced, so it should be linked to 2 invoice lines (invoice and refund)",
        )

        self.assertEqual(
            sol_downpayment.qty_to_invoice,
            -1.0,
            "As the downpayment was invoiced separately, it will still have to be deducted from the total invoice (hence -1.0), after the refund.",
        )
        self.assertEqual(
            sol_downpayment.qty_invoiced,
            1.0,
            "The qty to invoice should be 1 as, with the refund, the products are not invoiced anymore, but the downpayment still is",
        )
        self.assertEqual(
            sol_downpayment.amount_taxexc_to_invoice,
            -(sol_product.price_unit * 5) / 2,
            "Amount to invoice decreased as the refund is now confirmed",
        )
        self.assertEqual(
            sol_downpayment.amount_taxexc_invoiced,
            (sol_product.price_unit * 5) / 2,
            "Amount invoiced is now set as half of all products' total amount to invoke, as refund is validated",
        )
        self.assertEqual(
            len(sol_downpayment.invoice_line_ids),
            3,
            "The product line is invoiced, so it should be linked to 3 invoice lines (downpayment invoice, partial invoice and refund)",
        )
