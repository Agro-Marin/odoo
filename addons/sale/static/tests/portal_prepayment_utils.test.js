...
test("explicit amount_selection wins", () => {
    expect(computeIsDownPayment(params("amount_selection=down_payment"), ORDER_TOTAL)).toBe(true);
    expect(computeIsDownPayment(params("amount_selection=full_amount"), ORDER_TOTAL)).toBe(false);
});
...
test("malformed amount_selection falls through to the heuristic (server parity)", () => {
    ...
    expect(computeIsDownPayment(params("amount_selection=xyz&payment_amount=40"), ORDER_TOTAL)).toBe(true);
    expect(computeIsDownPayment(params("amount_selection=xyz"), ORDER_TOTAL)).toBe(true);
});
