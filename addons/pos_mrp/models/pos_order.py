...
class PosOrderLine(models.Model):
    ...
    def _get_stock_moves_to_consider(self, stock_moves, product):
        ...
        bom_line_ids = [
            item.id
            for x in boms
            for item in x[0].bom_line_ids
            if set(item.bom_product_template_attribute_value_ids.ids).issubset(
        ...
        return stock_moves.filtered(
            lambda ml: (
                ml.product_id.id in ml_product_to_consider
...
class PosOrder(models.Model):
    ...
    def _get_pos_anglo_saxon_price_unit(self, product, quantity):
        bom = product.env["mrp.bom"]._get_bom_by_product(
            product,
            company_id=self.mapped("picking_ids.move_line_ids").company_id.id,
            bom_type="phantom",
        )[product]
        if not bom:
            return super()._get_pos_anglo_saxon_price_unit(product, quantity)
