...
class TestUi(TestPointOfSaleHttpCommon):
    ...
    def setUpClass(cls):
        ...
        cls.pos_user.group_ids = [
            Command.set([group_internal_user.id, group_pos_user.id])
        ]

        cls.basic_kit, cls.finished, cls.component_a, cls.component_b = cls.env[
            "product.product"
        ].create(
            [
                {
    ...
    def test_ship_later_kit_and_mto_manufactured_product(self):
        ...
        customer = self.env["res.partner"].search(
            [("email", "=", "partner.full@example.com")], limit=1
        )
        ...
        self.start_tour(
            url, "test_ship_later_kit_and_mto_manufactured_product", login="pos_user"
        )

        picking = self.env["stock.picking"].search(
            [("partner_id", "=", self.partner_full.id)], limit=1
        )
        ...
        self.assertEqual(
            finished_sm.move_orig_ids.production_id.product_id, self.finished
        )
