...
class TestPosMrp(CommonPosMrpTest):
    ...
    def test_bom_kit_with_kit_invoice_valuation(self):
        ...
        self.assertEqual(
            order.lines.filtered(
                lambda l: l.product_id == self.product_product_kit_three
            ).total_cost,
            30.0,
        )
        accounts = (
            self.product_product_kit_three.product_tmpl_id._get_product_accounts()
        )
        ...
        expense_line = order.account_move.line_ids.filtered(
            lambda l: l.account_id.id == credit_expense_account.id
        )
        self.assertEqual(
            expense_line.filtered(
                lambda l: l.product_id == self.product_product_kit_three
            ).credit,
            0.0,
        )
        ...
        interim_line = order.account_move.line_ids.filtered(
            lambda l: l.account_id.id == debit_interim_account.id
        )
        self.assertEqual(
            interim_line.filtered(
                lambda l: l.product_id == self.product_product_kit_three
            ).credit,
            30.0,
        )
    ...
    def test_bom_kit_different_uom_invoice_valuation(self):
        ...
        self.bom_one_line.bom_line_ids[0].product_uom_id = self.env.ref(
        ...
        expense_line = expense_line.filtered(
            lambda l: l.product_id == self.product_product_kit_one
        )
        interim_line = interim_line.filtered(
            lambda l: l.product_id == self.product_product_kit_one
        )
    ...
    def test_bom_kit_order_total_cost_with_shared_component(self):
        ...
        order, _ = self.create_backend_pos_order(
            {
                "line_data": [{"product_id": kit_1.id}, {"product_id": kit_2.id}],
                "payment_data": [{"payment_method_id": self.cash_payment_method.id}],
            }
        )
                    ...
                    Command.create(
                        {
                            ...
                            "bom_product_template_attribute_value_ids": [
                                Command.link(
                                    self.configurable_product.product_tmpl_id.attribute_line_ids[
                                        0
                                    ]
                    ...
                    Command.create(
                        {
                            ...
                            "product_qty": 1,
                            "bom_product_template_attribute_value_ids": product_test.product_variant_ids[
                                0
