...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        if account_info := self.env["ir.config_parameter"].get_param(
            "cloud_storage_google_account_info"
        ):
            res["cloud_storage_google_service_account_key"] = base64.b64encode(
                account_info.encode()
            )
        return res
    ...
    def _compute_cloud_storage_google_account_info(self):
        for setting in self:
            key = setting.with_context(
                bin_size=False
            ).cloud_storage_google_service_account_key
            setting.cloud_storage_google_account_info = (
                base64.b64decode(key) if key else False
            )
    ...
    def _setup_cloud_storage_provider(self):
        ...
        download_url = IrAttachment._generate_cloud_storage_google_signed_url(
            bucket_name,
            blob_name,
            method="GET",
