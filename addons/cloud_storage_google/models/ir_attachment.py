...
def get_cloud_storage_google_credential(env):
    ...
    """
    cached_account_info, cached_credential = CloudStorageGoogleCredentials.get(
...
class IrAttachment(models.Model):
    ...
    def _get_cloud_storage_google_info(self):
        ...
        if not match:
            raise ValidationError(
                self.env._("%s is not a valid Google Cloud Storage URL.", self.url)
            )
        return {
    ...
    def _generate_cloud_storage_google_signed_url(
        self, bucket_name, blob_name, **kwargs
    ):
