...
class TestResUsers(TestMailFullCommon):
    ...
    def test_deactivate_portal_users_blacklist(self):
        ...
        """
        (
            self.portal_user | self.portal_user_2 | self.portal_user_3
        )._deactivate_portal_user(request_blacklist=True)

        self.assertFalse(self.portal_user.active, "Should have archived the user")
        self.assertFalse(
            self.portal_user.partner_id.active, "Should have archived the partner"
        )
        self.assertFalse(self.portal_user_2.active, "Should have archived the user")
        self.assertFalse(
            self.portal_user_2.partner_id.active, "Should have archived the partner"
        )
        self.assertFalse(self.portal_user_3.active, "Should have archived the user")
        self.assertFalse(
            self.portal_user_3.partner_id.active, "Should have archived the partner"
        )
            ...
            [
                (
                    ...
                    (
                        ...
                        "portal_2@test.example.com",
                        "portal_3@test.example.com",
                    ),
                ),
            ]
        )
        ...
        blacklist = self.env["phone.blacklist"].search(
    ...
    def test_deactivate_portal_users_no_blacklist(self):
        ...
        (self.portal_user | self.portal_user_2)._deactivate_portal_user(
        ...
        blacklists = self.env["phone.blacklist"].search([])
        self.assertFalse(
            blacklists, "Should not have blacklisted the user phone and mobile"
        )
