...
class TestOdoobot(MailCommon, TestRecipients):
    ...
    def assertNoAnswer(self, record, message):
        ...
        self.assertFalse(
            later, f"odoobot answered on {record._name}: {later.mapped('body')}"
        )

    @mute_logger("odoo.addons.mail.models.mail_mail")
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_odoobot()
        odoobot_in_fetch_listeners = self.env["discuss.channel.member"].search(
            [("channel_id", "=", channel.id), ("partner_id", "=", self.odoobot.id)]
        )
        self.assertEqual(
            len(odoobot_in_fetch_listeners),
            1,
            "odoobot should appear only once in channel_fetch_listeners",
        )
    ...
    def test_odoobot_ping(self):
        ...
        kwargs.update(
            {
                "body": self.odoobot_ping_body,
                "partner_ids": [self.odoobot.id, self.user_admin.partner_id.id],
            }
        )

        message = self.test_record_employe.with_context(
    ...
    def test_odoobot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update(
            {"body": "I'm not talking to @odoobot right now", "partner_ids": []}
        )
