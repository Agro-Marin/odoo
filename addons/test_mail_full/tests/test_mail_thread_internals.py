...
class TestMailThreadInternalsCommon(TestMailFullCommon, TestSMSRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.test_portal_records, cls.test_portal_partners = (
            cls._create_records_for_batch(
                "mail.test.portal",
                2,
            )
        )
        ...
        cls.test_rating_records, cls.test_rating_partners = (
            cls._create_records_for_batch(
                "mail.test.rating",
                2,
            )
        ...
        for test_record in test_records:
            with self.subTest(test_record=test_record):
                ...
                portal_customer_group = next(
                    (group for group in groups if group[0] == "portal_customer"), False
                )

                if is_portal and has_customer:
                    # should have generated the access token, required for portal links
                    self.assertTrue(
                        test_record.access_token, "Portal should generate access token"
                    )
                    ...
                    parameters = dict(parse_qsl(urlsplit(portal_url).query))
                    self.assertEqual(
