...
class FullBaseMailPerformance(BaseMailPostPerformance):
    ...
    def setUpClass(cls):
        ...
        cls.record_ticket_mc.message_subscribe(
            cls.customers.ids
            + cls.user_admin.partner_id.ids
            + cls.user_follower_portal.partner_id.ids
        )
...
class TestMailPerformance(FullBaseMailPerformance):
    def test_assert_initial_values(self):
        ...
        record_ticket = self.env["mail.test.ticket.mc"].browse(
            self.record_ticket_mc.ids
        )
        self.assertEqual(
            record_ticket.message_partner_ids,
            self.user_follower_emp_email.partner_id
            + self.user_admin.partner_id
            + self.customers
            + self.user_follower_portal.partner_id,
        )
...
class TestPortalFormatPerformance(FullBaseMailPerformance):
    ...
    def setUpClass(cls):
        ...
        cls.test_users = (
            ...
            + cls.user_emp_inbox
            + cls.user_emp_email
            + cls.user_follower_emp_email
            + cls.user_follower_portal
        )
        ...
        user_id_field = cls.env["ir.model.fields"]._get(
            cls.record_ratings._name, "user_id"
        )
        ...
        cls.messages_records = [
            cls.env[message.model].browse(message.res_id)
            for message in cls.messages_all
        ]
            ...
            .create(
                [
                    ...
                    for message, record in zip(
                        cls.messages_all, cls.messages_records, strict=True
                    )
                ]
            )
        )
    ...
    def test_portal_message_format_norating(self):
        ...
        for format_res, message, record in zip(
            res, messages_all, self.messages_records, strict=True
        ):
                ...
                [
                    {
                        ...
                        "mimetype": "text/plain",
                        "name": "Test file 1",
                        "raw_access_token": message.attachment_ids[
                            0
                        ]._get_raw_access_token(),
                        "res_id": record.id,
                        "res_model": record._name,
                        "thumbnail_access_token": message.attachment_ids[
                            0
            ...
            self.assertEqual(
                format_res["author_id"]["name"], record.customer_id.display_name
            )
            self.assertEqual(
            ...
            self.assertEqual(
                " ".join(format_res["published_date_str"].split()),
                "05/15/2023 10:30:05 AM",
            )
            ...
            self.assertEqual(
                format_res["subtype_id"], (comment_subtype.id, comment_subtype.name)
            )
    ...
    def test_portal_message_format_rating(self):
        ...
        self.assertEqual(len(res), len(messages_all))
        for format_res, _message, _record in zip(
            res, messages_all, self.messages_records, strict=True
        ):
            self.assertEqual(
                format_res["rating_id"]["publisher_avatar"],
            ...
            self.assertEqual(
                format_res["rating_id"]["publisher_id"], self.partner_admin.id
            )
            self.assertEqual(
                " ".join(format_res["rating_id"]["publisher_datetime"].split()),
                "05/13/2023 10:30:05 AM",
            )
            self.assertEqual(
                format_res["rating_id"]["publisher_name"],
                self.partner_admin.display_name,
            )
...
class TestRatingPerformance(FullBaseMailPerformance):
    ...
    def test_rating_api_rating_get_operator(self):
        ...
        expected_names = ["Mitchell Admin", "Ernest Employee", False] * 6 + [
            "Mitchell Admin",
            "Ernest Employee",
        ]
    ...
    def test_rating_api_rating_get_partner(self):
        ...
        with self.assertQueryCount(employee=3):  # tmf: 3
            ratings = self.record_ratings.with_env(self.env)
            partner_names.extend(
                rating._rating_get_partner().name for rating in ratings
            )
    ...
    def test_rating_get_stats_perfs(self):
        ...
        self.assertDictEqual(
            stats,
            {
                "avg": 3.75,
                "total": 40,
                "percent": {1: 10.0, 2: 10.0, 3: 10.0, 4: 35.0, 5: 35.0},
            },
        )
    ...
    def test_rating_stat_fields(self):
        expected_texts = ["ok", "ok", "ok", "top", "top"] * 2 + [
            "ok",
            "ok",
            "top",
            "top",
        ...
        with self.assertQueryCount(employee=2):
            ratings = self.record_ratings.with_env(self.env)
            for rating, text, satisfaction in zip(
                ratings, expected_texts, expected_satis, strict=True
            ):
