...
class TestUBLDK(TestUBLCommon, TestAccountMoveSendCommon):
    ...
    def setUpClass(cls):
        ...
        cls.company_data["company"].write(
            {
                ...
                "vat": "DK12345674",
                "phone_ids": [
                    Command.create({"number": "+45 32 12 34 56", "type": "landline"})
                ],
        ...
        cls.partner_a.write(
            {
                ...
                "vat": "DK12345674",
                "phone_ids": [
                    Command.create({"number": "+45 32 12 35 56", "type": "landline"})
                ],
        ...
        cls.partner_b.write(
            {
                ...
                "phone_ids": [
                    Command.create({"number": "061928374", "type": "landline"})
                ],
                "vat": "BE0897223670",
                "invoice_edi_format": "oioubl_201",
                "peppol_endpoint": False,
        ...
        cls.partner_c = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create({"number": "+33 1 23 45 67 89", "type": "landline"})
                ],
                "vat": "FR23334175221",
        ...
        cls.dk_local_purchase_tax_goods = cls.env["account.chart.template"].ref(
            "tax_k1"
        )
    ...
    def test_export_credit_note_two_line_partner_fr(self):
        refund = self.create_post_and_send_invoice(
            partner=self.partner_c, move_type="out_refund"
        )
        self.assertTrue(refund.ubl_cii_xml_id)
        self._assert_invoice_attachment(
            refund.ubl_cii_xml_id,
    ...
    @freeze_time("2017-01-01")
    def test_oioubl_export_should_still_be_valid_when_currency_has_more_precision_digit(
        self,
    ):
    ...
    @freeze_time("2017-01-01")
    def test_oioubl_export_should_raise_an_error_when_partner_building_number_is_missing(
        self,
    ):
        self.partner_a.street = (
            "Paradisæblevej"  # remove the street number from the address
        )
        with self.assertRaisesRegex(
            UserError, "The following partner's street number is missing"
        ):
            self.create_post_and_send_invoice()

    @freeze_time("2017-01-01")
    def test_oioubl_export_should_raise_an_error_when_company_building_number_is_missing(
        self,
    ):
    ...
    @freeze_time("2017-01-01")
    def test_export_invoice_company_and_partner_without_country_code_prefix_in_vat(
        self,
    ):
    ...
    def test_export_partner_fr_without_siret_should_raise_an_error(self):
        ...
        with self.assertRaisesRegex(
            UserError, "The company registry is required for french partner:"
        ):
    ...
    def test_oioubl_export_partner_without_vat_number(self):
        ...
        self.partner_b.invoice_edi_format = (
            "oioubl_201"  # default format recomputes when vat is changed
        )
