...
class Crawler(HttpCaseWithUserDemo):
        ...
        if r.headers["Content-Type"].startswith("text/html"):
            ...
            for link in doc.xpath("//a[@href]"):
                ...
                if (
                    ...
                    or parts.path == "/odoo"
                    or parts.path.startswith("/web/")
                    or parts.path.startswith("/en/")
    ...
    def test_05_test_clean_url(self):
        urls_to_check = [
            ...
            (
                "/my/slug-19/?access_token=www-xxx-yyy-zzz",
                "/my/<slug>?access_token=<param>",
            ),
            (
                "/my/slug-19?access_token=www-xxx-yyy-zzz",
                "/my/<slug>?access_token=<param>",
            ...
            (
                "/controller?date=upcoming&tags=%5B5%5D",
                "/controller?date=<param>&tags=<param>",
            ),
            (
                "/controller?tags=%5B%5D&date=upcoming",
                "/controller?date=<param>&tags=<param>",
            ),
            (
                "/controller?tags=%5B%5D&from=/a/b/c",
                "/controller?from=<param>&tags=<param>",
            ),
            (
                "/controller?tags=%5B%5D&from=d/e/f&to=/a/b",
                "/controller?from=<param>&tags=<param>&to=<param>",
            ),
            (
                "/controller?tags=%5B%5D&from=d/e/f&to=/c/d",
                "/controller?from=<param>&tags=<param>&to=<param>",
            ),
        ]
        uniq = set()
