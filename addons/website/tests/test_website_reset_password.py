...
class TestWebsiteResetPassword(HttpCase):
    def test_01_website_reset_password_tour(self):
        ...
        with (
            ...
            patch.object(MailTemplate, "send_mail", my_send_mail),
        ):
            ...
            self.start_tour(
                user.partner_id._get_signup_url(), "website_reset_password", login=None
            )
    ...
    def test_multi_website_reset_password_user_specific_user_account(self):
        ...
        self.env["res.users"].with_context(website_id=website_1.id).reset_password(
            login
        )
