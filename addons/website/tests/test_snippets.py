...
class TestSnippets(HttpCase):
    ...
    def test_03_snippets_all_drag_and_drop(self):
        with MockRequest(self.env, website=self.env["website"].browse(1)):
            snippets_template = self.env["ir.ui.view"].render_public_asset(
                "website.snippets"
            )
        html_template = html.fromstring(snippets_template)
        data_snippet_els = html_template.xpath(
            "//*[snippets and not(hasclass('d-none'))]//*[@data-oe-snippet-key]"
        )
        ...
        path = urlencode({"path": "/?" + snippets_names_encoded})
        ...
        self.start_tour(
            f"/odoo/action-website.website_preview?{path}",
    ...
    def test_05_social_media(self):
        ...
        create_image_attachment(
            self.env,
            "/web/image/website.s_banner_default_image",
            "s_banner_default_image.jpg",
        )
        self.start_tour(
            self.env["website"].get_client_action_url("/"),
    ...
    def test_09_snippet_image_gallery(self):
        create_image_attachment(
            self.env, "/web/image/website.s_banner_default_image", "s_default_image.jpg"
        )
        create_image_attachment(
            self.env,
            "/web/image/website.s_banner_default_image_2",
            "s_default_image2.webp",
        )
    ...
    def test_cookie_bar_updates_gtag_consent(self):
        ...
        website.cookies_bar = True
        self.start_tour(
            website.get_client_action_url("/"), "cookie_bar_updates_gtag_consent"
        )
