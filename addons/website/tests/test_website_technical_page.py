...
class TestWebsiteTechnicalPage(TransactionCase):
    def _validate_routes(self, expected_routes):
        ...
        routes = TechnicalPage.search([("website_url", "in", expected_routes)]).mapped(
            "website_url"
        )
        ...
        for route in expected_routes:
            self.assertIn(
                route,
                routes,
                f"Route '{route}' is missing from technical page records.",
            )
    ...
    def test_load_website_escaping_title(self):
        ...
        with patch.object(
            WebsiteTechnicalPage, "get_static_routes", return_value=patched_route
        ):
