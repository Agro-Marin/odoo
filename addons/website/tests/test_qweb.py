...
class TestQweb(TransactionCaseWithUserDemo):
    ...
    def test_qweb_post_processing_att(self):
        ...
        rendered = self.env["ir.qweb"]._render(
            t.id, {"url": "http://test.external.img/img2.png"}, website_id=website.id
        )
    ...
    def test_render_context_website(self):
        ...
        self.assertEqual(
            rendered.strip(), result.strip(), "First rendering (without website_id)"
        )

        rendered = self.env["ir.qweb"].with_context(website_id=1)._render(template.id)
        self.assertEqual(
            rendered.strip(), result.strip(), "Second rendering (with website_id=1)"
        )

        rendered = (
            self.env["ir.qweb"].with_context(website_id=None)._render(template.id)
        )
        self.assertEqual(
            rendered.strip(), result.strip(), "Third rendering (with website_id=None)"
        )

        rendered = self.env["ir.qweb"].with_context(website_id=1)._render(template.id)
        self.assertEqual(
            rendered.strip(), result.strip(), "Fourth rendering (with website_id=1)"
        )
...
class TestQwebProcessAtt(TransactionCase):
    def setUp(self):
        ...
        self.website.language_ids = self.env.ref("base.lang_en") + self.env.ref(
            "base.lang_fr"
        )
        self.website.default_lang_id = self.env.ref("base.lang_en")
        self.website.cdn_activated = True
        self.website.cdn_url = "http://test.cdn"
        self.website.cdn_filters = "^(/[a-z]{2}_[A-Z]{2})?/a$\n^(/[a-z]{2})?/a$\n^/b$"

    def _test_att(self, url, expect, tag="a", attribute="href"):
        self.assertEqual(
            self.env["ir.qweb"]._post_processing_att(tag, {attribute: url}), expect
        )
...
class TestQwebDataSnippet(TransactionCase):
    ...
    def test_t_call_inside_snippet(self):
        ...
        self.assertEqual(
            self._normalize_xml(rendered), self._normalize_xml(expected_output)
        )
    ...
    def test_call_query_count_snippets_template(self):
        ...
        self.assertEqual(
            ...
            all_ir_ui_view_queries,
            f"ir_ui_view queries: {all_ir_ui_view_queries}",
        )
