...
class TestWebsiteWebsiteBuilderAssetsBundle(HttpCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.bundle = cls.env["ir.qweb"]._get_asset_bundle(
            "website.website_builder_assets", True
        )
    ...
    def test_website_builder_assets_bundle_no_edit_scss(self):
        for file in self.bundle.files:
            ...
            self.assertFalse(
                ...
                msg="website.website_builder_assets must not contain *.edit.scss files. Remove "
                + filename,
            )
