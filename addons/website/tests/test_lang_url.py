...
class TestLangUrl(TestLangUrlCommon):
    def test_01_url_lang(self):
        with MockRequest(self.env, website=self.website):
            self.assertEqual(
                self.env["ir.http"]._url_for("", "[lang]"),
                "/[lang]/mockrequest",
    ...
    def test_03_url_cook_lang_not_available(self):
        ...
        if 'lang="en-US"' not in r.text:
            ...
            self.assertEqual(
                doc.get("lang"),
                "en-US",
                "french should not be displayed as not a frontend lang",
            )
    ...
    def test_04_url_cook_lang_not_available(self):
        ...
        for line in r.text.splitlines():
            ...
            if match:
                ...
                self.assertEqual(
                    session_info["user_context"]["lang"],
                    "en_US",
                    "ensure english was loaded",
                )
                self.assertEqual(
                    session_info["bundle_params"]["lang"],
                    "en_US",
                    "ensure bundle use english",
                )
                break
        else:
        ...
        if 'lang="fr-FR"' not in r.text:
            ...
            self.assertEqual(
                doc.get("lang"),
                "fr-FR",
                "Ensure contactus did not soft crash + loaded in correct lang",
            )
    ...
    def test_05_invalid_ipv6_url(self):
        ...
        self.assertEqual(
            r.status_code, 200, "The page must still load despite the invalid link"
        )
        doc = lxml.html.document_fromstring(r.text)
        [anchor] = doc.xpath('//a[@id="foo"]')
        self.assertEqual(
            anchor.get("href"), "http://]", "The invalid IP URL must be left untouched"
        )

    def test_06_reroute_unicode(self):
        res = self.url_open("/fr/привет")
        self.assertEqual(
            res.status_code, 404, "Rerouting didn't crash because of unicode path"
        )
        ...
        self.assertEqual(
            res.status_code,
            404,
            "Rerouting didn't crash because of unicode query-string",
        )

    def test_07_nolang_prefix_underscore(self):
        res = self.url_open("/_not_a_lang", allow_redirects=False)
        self.assertEqual(
            res.status_code, 404, "Should not consider /_not_a_lang as a lang"
        )
        self.assertURLEqual(
            res.url,
            "/_not_a_lang",
            "Should use /_not_a_lang as the path and not a lang",
        )
