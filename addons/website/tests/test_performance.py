...
class UtilPerf(HttpCaseWithUserPortal, HttpCaseWithUserDemo):
    ...
    def _get_url_hot_query(self, url, query_list=False, nocache=False):
        ...
        self.assertEqual(
            len(profiler.sub_profilers), 1, "we expect to have only one accessed url"
        )  # if not adapt the code below
    ...
    def _check_url_hot_query(
        self,
        url,
        expected_query_count,
        select_tables_perf=None,
        insert_tables_perf=None,
        nocache=False,
    ):
        query_count, sql_queries = self._get_url_hot_query(
            url, query_list=True, nocache=nocache
        )
        ...
        for query in sql_queries:
            ...
            if query_type == "into":
                ...
                _logger.warning(
                    "Query type %s for query %s is not supported by _check_url_hot_query",
                    query_type,
                    query,
                )
        ...
        self.assertDictEqual(
            sql_from_tables,
            select_tables_perf,
            f"Select queries does not match: {select}{query_separator}{queries}{query_separator}",
        )
        self.assertDictEqual(
            sql_into_tables,
            insert_tables_perf,
            f"Insert queries does not match: {insert}{query_separator}{queries}{query_separator}",
        )
...
class TestStandardPerformance(UtilPerf):
    ...
    def setUpClass(cls):
        ...
        cls.env["res.users"].browse(
            2
        ).image_1920 = b"iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGNgYGAAAAAEAAH2FzhVAAAAAElFTkSuQmCC"
...
class TestWebsitePerformance(TestWebsitePerformanceCommon):
    def test_10_perf_sql_queries_page(self):
        ...
        for readonly_enabled in (True, False):
            ...
            with (
                self.subTest(readonly_enabled=readonly_enabled),
                ...
                self._check_url_hot_query(
                    self.page.url, expected_query_count, select_tables_perf
                )
                self.assertEqual(
                    self._get_url_hot_query(self.page.url), expected_query_count
                )
                ...
                self.assertEqual(
                    self._get_url_hot_query(self.page.url, nocache=True),
                    expected_query_count,
                )

    def test_15_perf_sql_queries_page(self):
        # standard tracked website.page
        for readonly_enabled, cache in (
            (True, True),
            (False, True),
            (True, False),
            (False, False),
            ...
            with (
                ...
                self.assertEqual(
                    self._get_url_hot_query(self.page.url, nocache=not cache),
                    expected_query_count,
                )
    ...
    def test_20_perf_sql_queries_homepage(self):
        ...
        for readonly_enabled in (True, False):
            ...
            with (
                ...
                self.assertEqual(
                    self._get_url_hot_query("/", nocache=True), expected_query_count
                )
