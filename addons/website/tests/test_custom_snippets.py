...
class TestCustomSnippet(TransactionCase):
    def test_translations_custom_snippet(self):
        ...
        custom_snippet_view = View.search(
            [("name", "=", "Test Translation MegaMenu")], limit=1
        )
        self.assertIn(
            "Texte Francais",
            custom_snippet_view.with_context(lang=parseltongue.code).arch,
        ...
        self.assertIn(
            "Parseltongue Text", view.with_context(lang=parseltongue.code).arch
        )
...
class TestHttpCustomSnippet(HttpCase):
    def test_editable_root_as_custom_snippet(self):
        ...
        self.start_tour(
            f"{custom_page.url}", "editable_root_as_custom_snippet", login="admin"
        )
