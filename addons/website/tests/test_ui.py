...
class TestUiCustomizeTheme(odoo.tests.HttpCase):
    def test_01_attachment_website_unlink(self):
        ...
        website_test.unlink()
        self.assertEqual(
            Attachment.search_count([("url", "=", custom_url)]),
            1,
...
class TestUiHtmlEditor(HttpCaseWithUserDemo):
    def test_html_editor_language(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/test_page"),
    ...
    def test_html_editor_multiple_templates(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/generic"),
            "html_editor_multiple_templates",
            login="admin",
        )
        self.assertEqual(
            View.search_count([("key", "=", "test.generic_view")]),
            2,
            "homepage view should have been COW'd",
        )
        self.assertTrue(
            generic_page.arch == oe_structure_layout,
            "Generic homepage view should be untouched",
        )
        self.assertEqual(
            len(
                generic_page.inherit_children_ids.filtered(
                    lambda v: "oe_structure" in v.name
                )
            ),
            0,
        ...
        self.assertTrue(
            specific_page.arch != oe_structure_layout,
            "Specific homepage view should have been changed",
        )
        self.assertEqual(
            len(
                specific_page.inherit_children_ids.filtered(
                    lambda v: "oe_structure" in v.name
                )
            ),
            1,
...
class TestUiTranslate(odoo.tests.HttpCase):
    ...
    def test_translate_menu_name(self):
        ...
        self.assertEqual(
            new_menu.with_context(lang=parseltongue.code).name,
            "value pa-GB",
    ...
    def test_snippet_translation(self):
        ...
        website_2 = self.env["website"].create(
            {
                "name": "website en_US",
                "language_ids": [
                    (6, 0, [self.env.ref("base.lang_en").id, parseltongue.id])
                ],
                "default_lang_id": parseltongue.id,
            }
        )
        ...
        self.start_tour(
            f"/website/force/{website.id}", "snippet_translation", login="admin"
        )
        self.start_tour(
            f"/website/force/{website_2.id}",
        ...
        self.start_tour(
            ...
            login="admin",
            cookies={"websiteIdMapping": json.dumps({website_3.name: website_3.id})},
        )
        self.start_tour(
            f"/website/force/{website.id}", "snippet_dialog_rtl", login="admin"
        )
...
class TestUi(HttpCaseWithWebsiteUser):
    ...
    def test_05_specific_website_editor(self):
        ...
        base_website_bundle = self.env["ir.qweb"]._get_asset_bundle(
        ...
        new_website_bundle_modified = self.env["ir.qweb"]._get_asset_bundle(
            "website.assets_inside_builder_iframe",
            assets_params={"website_id": new_website.id},
        )
        self.assertIn(custom_url, [f["url"] for f in new_website_bundle_modified.files])
        self.assertEqual(
            new_website_bundle_modified.get_version("css"), base_website_css_version
        )
        self.assertNotEqual(
            new_website_bundle_modified.get_version("js"),
            base_website_js_version,
            "js version for new website should now have been changed",
        )

        url_params = urlencode({"path": "/@/"})
        self.start_tour(
            f"/website/force/{website_default.id}?{url_params}",
            "generic_website_editor",
            login="website_user",
        )
        self.start_tour(
            f"/website/force/{new_website.id}?{url_params}",
            "specific_website_editor",
            login="website_user",
        )
    ...
    def test_17_website_edit_menus(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "edit_menus", login="admin"
        )
    ...
    def test_24_snippet_cache_across_websites(self):
        default_website = self.env.ref("website.default_website")
        website = self.env["website"].create(
            {"name": "Test Website", "domain": "", "sequence": 20}
        )
        ...
        self.start_tour(
            ...
            login="admin",
            cookies={"websiteIdMapping": json.dumps({"Test Website": website.id})},
        )
        ...
        Menu.create(
            {
                ...
                % (self.env.ref("base.open_module_tree").id,),
            }
        )
        self.env.ref("base.user_admin").action_id = self.env.ref(
    ...
    def test_33_website_menus(self):
        ...
        self.start_tour(
            "/odoo/action-website.action_website_menu",
            "parent_child_menu",
            login="admin",
        )
    ...
    def test_website_no_dirty_lazy_image(self):
        ...
        for key, active in [
            ...
            self.env["website"].with_context(website_id=website.id).viewref(
                key
            ).active = active
    ...
    def test_anchor_on_accordion_item(self):
        ...
        self.start_tour(
            "/#What-services-does-your-company-offer-%3F",
