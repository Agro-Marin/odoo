...
class TestWebsitePageManager(odoo.tests.HttpCase):
    def test_01_page_manager(self):
        ...
        self.start_tour(
            "/odoo#action=website.action_website_pages_list",
            "website_page_manager_direct_access",
            login="admin",
        )
    ...
    def test_generic_page_diverged_not_shown(self):
        ...
        specific_page = Page.search(
            [("url", "=", "/test_diverged"), ("website_id", "=", website.id)], limit=1
        )
        self.assertNotEqual(generic_page, specific_page)
        locs = website.with_context(website_id=website.id)._enumerate_pages(
            query_string="/test_diverged"
        )
        self.assertEqual(
            len(list(locs)), 1, "Specific page should be shown as same url"
        )
