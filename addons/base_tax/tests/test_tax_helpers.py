...
class TestFilterTaxesByCompany(BaseTaxCommon):
    ...
    def test_unrelated_company_yields_empty_recordset(self):
        """A company with no parent-chain link to the tax's company finds nothing."""
        tax = self._tax(21.0)
        unrelated_company = self.env["res.company"].create(
            {"name": "BT unrelated company"},
        )

        result = tax._filter_taxes_by_company(unrelated_company)

        self.assertFalse(result)
        self.assertEqual(result, self.env["account.tax"])
