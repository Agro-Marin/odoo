...
class TestMailingStatistics(TestMassMailCommon):
    ...
    def test_mailing_statistics(self):
        target_records = self._create_mailing_test_records(
            model="mailing.test.blacklist", count=13
        )
        target_records[10]["email_from"] = (
            False  # void email should lead to a 'cancel' trace_status
        )
        target_records[11]["email_from"] = (
            "raoul@example¢¡.com"  # wrong email should lead to a 'exception' trace_status
        )
        mailing = self.env["mailing.mailing"].browse(self.mailing_bl.ids)
        mailing.write(
            {
                "mailing_domain": [("id", "in", target_records.ids)],
                "user_id": self.user_marketing_2.id,
            }
        )
        ...
        self.gateway_mail_reply_wrecord(
            MAIL_TEMPLATE, target_records[0], use_in_reply_to=True
        )
        self.gateway_mail_reply_wrecord(
            MAIL_TEMPLATE, target_records[1], use_in_reply_to=True
        )
        self.gateway_mail_reply_wrecord(
            MAIL_TEMPLATE, target_records[2], use_in_reply_to=True
        )
        ...
        self.assertEqual(
            len(self._new_mails),
            1,
            "Mailing: a mail should have been created for statistics",
        )
        ...
        kpi_values = body_html.xpath(
            '//table[@data-field="mail"]//*[hasclass("kpi_value")]/text()'
        )
        self.assertEqual(
            [t.strip().strip("%") for t in kpi_values],
            [
                "83.33",
                str(mailing.opened_ratio),
                str(mailing.replied_ratio),
            ],  # first value is received_ratio
        )
    ...
    def test_mailing_statistics_wo_user(self):
        target_records = self._create_mailing_test_records(
            model="mailing.test.blacklist", count=10
        )
        mailing = self.env["mailing.mailing"].browse(self.mailing_bl.ids)
        mailing.write(
            {"mailing_domain": [("id", "in", target_records.ids)], "user_id": False}
        )
        mailing.action_put_in_queue()
        with self.mock_mail_gateway(mail_unlink_sent=False):
            mailing.action_send_mail()
