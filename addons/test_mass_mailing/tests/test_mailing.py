...
class TestMassMailing(TestMassMailCommon):
    ...
    def test_mailing_author(self):
        ...
        with self.mock_mail_gateway(mail_unlink_sent=False):
            (mailing | mailing_2).with_user(
                self.env.ref("base.user_root")
            ).action_send_mail()
        self.assertEqual(
            self._new_mails.filtered(lambda m: m.model == "res.partner").mapped(
                "author_id"
            ),
        ...
        with self.mock_mail_gateway(mail_unlink_sent=False):
            (mailing | mailing_2).copy().with_user(
                self.user_marketing_1
            ).action_send_mail()
        self.assertEqual(
            self._new_mails.filtered(lambda m: m.model == "res.partner").mapped(
                "author_id"
    ...
    def test_mailing_gateway_reply(self):
        ...
        mailing = self.env["mailing.mailing"].create(
            {
                ...
                "reply_to_mode": "new",
                "reply_to": "%s@%s"
                % (self.test_alias.alias_name, self.test_alias.alias_domain),
                "keep_archives": True,
        ...
        self.gateway_mail_reply_wrecord(
            MAIL_TEMPLATE, customers[0], use_in_reply_to=True
        )
        self.gateway_mail_reply_wrecord(
            MAIL_TEMPLATE, customers[1], use_in_reply_to=False
        )
        ...
        self.format_and_process(
            ...
            extra="",
            msg_id="<123456.%s.%d@test.example.com>"
            % (customers[2]._name, customers[2].id),
            target_model=customers[2]._name,
            target_field=customers[2]._rec_name,
        )
    ...
    def test_mailing_gateway_update(self):
        ...
        recipients = self._create_mailing_test_records(
            model="mailing.test.optout", count=5
        )
        ...
        self.assertMailTraces(
            ...
            mail_links_info=[
                [
                    ...
                    (
                        "url5",
                        "%s/event/dummy-event-0" % mailing.get_base_url(),
                        True,
                        {},
                    ),
                    ...
                    (
                        "url8",
                        "%s/unsubscribe_from_list" % mailing.get_base_url(),
                        False,
                        {},
                    ),
                ]
        ...
        self.assertMailingStatistics(
            mailing, expected=5, delivered=5, sent=5, opened=1, clicked=1
        )
        ...
        mailing.invalidate_recordset()
        self.assertMailingStatistics(
            mailing, expected=5, delivered=4, sent=5, opened=1, clicked=1, bounced=1
        )
                    ...
                    [
                        {
                            ...
                            "email_to_recipients": [
                                [
                                    f'"{customer_mult.name}" <customer.multi.1@example.com>',
                        ...
                        },
                        {
                            ...
                            "email_to_recipients": [
                                [
                                    f'"{customer_fmt.name}" <test.customer.format@example.com>'
                                ]
                            ],
                            "failure_type": False,
                        ...
                        },
                        {
                            ...
                            "email_to_recipients": [
                                [
                                    f'"{customer_unic.name}" <test.customer.😊@example.com>'
                                ]
                            ],
                            "failure_type": False,
                        ...
                        },
                        {
                            ...
                            "email_to_recipients": [
                                [
                                    f'"{customer_case.name}" <test.customer.case@example.com>'
                                ]
                            ],
                            "failure_type": False,
                        ...
                        },  # lower cased
                        {
                            ...
                            "trace_status": "sent",
                        },  # concatenates everything after domain
                        {
                            "email": "test.customer.weird.2@example.com",
                            "email_to_mail": "",  # using recipient_ids, not email_to
                            "email_to_recipients": [
                                [
                                    f'"{customer_weird_2.name}" <test.customer.weird.2@example.com>'
                                ]
                            ],
                            "failure_type": False,
                        ...
                        },
                        {
                            ...
                            "email_to_recipients": [
                                [
                                    "record.multi.1@example.com",
                                    '"Record Multi 2" <record.multi.2@example.com>',
                                ]
                            ],
                            "failure_type": False,
                            "trace_status": "sent",
                        },
                        {
                            ...
                            "email_to_recipients": [
                                ['"Formatted Record" <record.format@example.com>']
                            ],
                            "failure_type": False,
                            "trace_status": "sent",
                        },
                        {
                            ...
                            "email_to_recipients": [
                                ['"Unicode Record" <record.😊@example.com>']
                            ],
                            "failure_type": False,
                            "trace_status": "sent",
                        },
                        ...
                        },
                        {
                            ...
                            "email_to_recipients": [
                                ["test.record.weird@example.comweirdformat"]
                            ],
                            "failure_type": False,
                            "trace_status": "sent",
                        },
                        {
                            ...
                            "email_to_recipients": [
                                ['"Weird Format2" <test.record.weird.2@example.com>']
                            ],
                            "failure_type": False,
                            "trace_status": "sent",
                        },
    ...
    def test_mailing_reply_to_mode_new(self):
        mailing = self.env["mailing.mailing"].browse(self.mailing_bl.ids)
        recipients = self._create_mailing_test_records(
            model="mailing.test.blacklist", count=5
        )
        self.assertEqual(len(recipients), 5)
        ...
        self.assertEqual(
            answer_rec.message_ids.subject,
            "Re: %s" % mailing.subject,
            "Answer should be logged",
        )
        self.assertEqual(recipients.message_ids, initial_messages)

        self.assertMailingStatistics(
            mailing, expected=5, delivered=5, sent=5, opened=1, replied=1
        )
    ...
    def test_mailing_reply_to_mode_update(self):
        ...
        self.assertEqual(
            recipients[0].message_ids[1].subject,
            mailing.subject,
            "Should have keep a log (to enable thread-based answer)",
        )
        self.assertEqual(
            recipients[0].message_ids[0].subject,
            "Re: %s" % mailing.subject,
            "Answer should be logged",
        )
    ...
    def test_mailing_trace_utm(self):
        ...
        traces = self.env["mailing.trace"].search(
            [
                ("model", "=", self.mailing_list_1.contact_ids._name),
        ...
        self.gateway_mail_reply_wrecord(
            MAIL_TEMPLATE, self.mailing_list_1.contact_ids[0], use_in_reply_to=True
        )
        self.gateway_mail_reply_wrecord(
            MAIL_TEMPLATE, self.mailing_list_1.contact_ids[1], use_in_reply_to=False
        )

        mailing_test_utms = self.env["mailing.test.utm"].search(
            [("name", "=", "Re: %s" % subject)]
        )
    ...
    def test_mailing_w_blacklist(self):
        ...
        self.assertEqual(
            len(self.env["mail.mail"].sudo().search([("mailing_id", "=", mailing.id)])),
            3,
    ...
    def test_mailing_w_blacklist_nomixin(self):
        ...
        test_records = self._create_mailing_test_records(
            model="mailing.test.simple", count=2
        )
        ...
        self.assertEqual(
            len(
                self.env["mail.mail"]
                .sudo()
                .search([("mailing_id", "=", self.mailing_bl.id)])
            ),
            1,
    ...
    def test_mailing_w_seenlist(self):
        ...
        records = self.env["mailing.test.partner"].create(
            [
                {
                    "name": f"test_duplicates {i}",
                    "email_from": f'"{names[i % 2]}" <{emails[i % BATCH_SIZE]}>',
                }
                for i in range(20)
            ]
        )
        ...
        mailing = self.env["mailing.mailing"].create(
            {
                ...
                "mailing_model_id": self.env.ref(
                    "test_mass_mailing.model_mailing_test_partner"
                ).id,
    ...
    def test_mailing_w_seenlist_unstored_partner(self):
        ...
        mailing = self.env["mailing.mailing"].create(
            {
                ...
                "mailing_domain": [("id", "in", test_records.ids)],
                "mailing_model_id": self.env["ir.model"]._get_id(
        ...
        traces = self._create_sent_traces(mailing, test_records[:3])
        traces.flush_model()
        ...
        with self.mock_mail_gateway(mail_unlink_sent=False):
            mailing.action_send_mail()
        self.assertEqual(
            len(self._mails), 7, "Mailing: seen list should contain 3 existing traces"
        )
    ...
    def test_mailing_mailing_list_optout(self):
        ...
        mailing_contact_1 = self.env["mailing.contact"].create(
            {"name": "test 1", "email": "test@test.example.com"}
        )
        mailing_contact_2 = self.env["mailing.contact"].create(
            {"name": "test 1", "email": "test@test.example.com"}
        )
        mailing_contact_3 = self.env["mailing.contact"].create(
            {"name": "test 3", "email": "test3@test.example.com"}
        )
        mailing_contact_4 = self.env["mailing.contact"].create(
            {"name": "test 4", "email": "test4@test.example.com"}
        )
        mailing_contact_5 = self.env["mailing.contact"].create(
        ...
        subs = self.env["mailing.subscription"].search(
            [
                ...
                "|",
                "&",
                ("contact_id", "=", mailing_contact_1.id),
                ("list_id", "=", mailing_list_1.id),
                "&",
                ("contact_id", "=", mailing_contact_3.id),
                ("list_id", "=", mailing_list_1.id),
                "&",
                ("contact_id", "=", mailing_contact_5.id),
                ("list_id", "=", mailing_list_1.id),
            ]
        )
        ...
        mailing = self.env["mailing.mailing"].create(
            {
                ...
                "mailing_model_id": self.env["ir.model"]._get("mailing.list").id,
                "contact_list_ids": [
                    (4, ml.id) for ml in mailing_list_1 | mailing_list_2
                ],
            }
        )
        with self.mock_mail_gateway(mail_unlink_sent=False):
