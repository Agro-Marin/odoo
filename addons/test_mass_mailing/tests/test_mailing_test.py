...
class TestMailingTest(TestMassMailCommon):
        ...
        cls.test_mailing_bl = cls.env["mailing.mailing"].create(
            {
                ...
                "mailing_model_id": cls.env["ir.model"]._get_id(
    ...
    def test_mailing_test_equals_reality(self):
        ...
        self.assertEqual(
            expected_test_record, self.test_records[0], "Should take first found one"
        )
        expected_subject = (
            f'Subject {expected_test_record.name} <t t-out="object.name"/>'
        )
    ...
    def test_mass_sms_test_button_twilio(self):
        ...
        for error_type, exp_state, exp_msg in [
            (
                False,
                "outgoing",
                "<ul><li>Test SMS successfully sent to +32456001122</li></ul>",
            ),
            (
                "wrong_number_format",
                "outgoing",  # not sure why outgoing but hey
