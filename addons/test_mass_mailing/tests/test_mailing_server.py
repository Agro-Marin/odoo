...
class TestMassMailingServer(TestMassMailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.recipients = cls._create_mailing_test_records(
            model="mailing.test.optout", count=8
        )
    ...
    def test_mass_mailing_server_archived_usage_protection(self):
        ...
        self.env["ir.config_parameter"].set_param(
            "mass_mailing.mail_server_id", servers[0].id
        )
        ...
        self.assertEqual(
            mailing_clone.mail_server_id.id,
            servers[0].id,
            "The clone of a mailing inherits from the server of the copied mailing",
        )
        with self.assertRaises(
            UserError, msg="Servers still used as default and for 2 mailings"
        ):
            servers.action_archive()
        self.assertTrue(
            all(server.active for server in servers), "All servers must be active"
        )
    ...
    def test_mass_mailing_server_find(self):
        ...
        for expected_smtp_from, expected_msg_from, expected_mail_server in [
            ...
            (
                ...
                f'"Testing" <{self.default_from}@{self.alias_domain}>',
                self.mail_server_notification,
            ),
            # forced sever
            (
                "unknow_email@unknow_domain.com",
                "unknow_email@unknow_domain.com",
                self.mail_server_notification,
            ),
        ]:
