...
from odoo.addons.mass_mailing.models.mixin_mail_thread import (
    BLACKLIST_MAX_BOUNCED_LIMIT,
)
...
class TestAutoBlacklist(common.TestMassMailCommon):
    ...
    def _test_mailing_bounce_w_auto_bl(self, bounce_base_values):
        ...
        for idx in range(4):
            ...
            with (
                ...
                traces += self._create_bounce_trace(
                    new_mailing,
                    target,
                    dt=datetime.datetime.now() - datetime.timedelta(weeks=idx + 2),
                )
        ...
        blacklist_record = (
            ...
            .sudo()
            .search([("email", "=", target.email_normalized)])
        )
