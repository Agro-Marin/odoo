...
class AccountMove(models.Model):
    ...
    def _get_name_invoice_report(self):
        if (
            self.company_id.tax_config_id.account_fiscal_country_id.code == "AU"
            and self.company_id.l10n_au_config_id.l10n_au_is_gst_registered
        ):
    ...
    def _get_automatic_balancing_account(self):
        ...
        has_single_line = (
            len(self.invoice_line_ids) == 1 and len(self.invoice_line_ids.tax_ids) == 1
        )
        if has_single_line and self.move_type == "entry":
            ...
            with_dgst_account = (
                self.invoice_line_ids.account_id.tag_ids
                == self.env.ref("l10n_au.account_tag_dgst")
            )
