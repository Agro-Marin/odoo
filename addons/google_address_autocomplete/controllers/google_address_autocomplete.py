...
FIELDS_MAPPING = {
    ...
    "locality": [
        "city"
    ],  # If locality exists, use it instead of the more general administrative area
...
class AutoCompleteController(http.Controller):
    def _translate_google_to_standard(self, google_fields):
        ...
        for google_field in google_fields:
            ...
            for field_standard in fields_standard:
                if (
                    field_standard in standard_data
    ...
    def _guess_number_from_input(self, source_input, standard_address):
        ...
        guessed_house_number = (
            source_input.replace(standard_address.get("zip", ""), "")
    ...
    def _perform_complete_place_search(
        ...
        results.sort(
            key=lambda r: (
                FIELDS_PRIORITY.index(r["type"])
                if r["type"] in FIELDS_PRIORITY
                else 100
            )
        )
        ...
        if "number" not in standard_address:
            standard_address["number"] = self._guess_number_from_input(
                address, standard_address
            )
            standard_address["formatted_street_number"] = (
                f"{standard_address['number']} {standard_address.get('street', '')}".strip()
            )
        else:
    ...
    def _autocomplete_address(
        self, partial_address, session_id=None, use_employees_key=None
    ):
        try:
        ...
        if not api_key:
            return {"results": [], "session_id": session_id}
        return self._perform_place_search(
            partial_address, session_id=session_id, api_key=api_key
        )
    ...
    def _autocomplete_address_full(
        self,
        address,
        session_id=None,
        google_place_id=None,
        use_employees_key=None,
        **kwargs,
    ):
        try:
            api_key = self._get_api_key(use_employees_key)
