...
class TestSMSSchedule(EventCase, SMSCase):
    ...
    def test_sms_schedule(self):
        ...
        self.assertTrue(
            all(m.mail_sent is True for m in sub_scheduler.mail_registration_ids)
        )
        self.assertEqual(
            sub_scheduler.mapped("mail_registration_ids.registration_id"),
            test_event.registration_ids,
        )
        ...
        self.assertEqual(
            before_scheduler.scheduled_date, test_event.date_begin + timedelta(days=-3)
        )
    ...
    def test_sms_schedule_fail_registration_template_removed(self):
        ...
        self.assertFalse(
            after_sub_scheduler, "When removing template, scheduler should be removed"
        )
