...
class LunchSupplier(models.Model):
    ...
    def _sync_cron(self):
        for supplier in self:
            ...
            ).replace(tzinfo=timezone(supplier.tz))
            cron = supplier.cron_id.sudo()
            lc = cron.lastcall
            if (
                lc
                and sendat_tz.date()
                <= fields.Datetime.context_timestamp(supplier, lc).date()
            ) or (
                not lc
                and sendat_tz
                <= fields.Datetime.context_timestamp(supplier, fields.Datetime.now())
            ):
    ...
    def write(self, vals):
        ...
        if "active" in values:
            ...
            Product.search([("supplier_id", "in", active_suppliers.ids)]).write(
                {"active": True}
            )
            Product.search([("supplier_id", "in", inactive_suppliers.ids)]).write(
                {"active": False}
            )
        ...
        days_removed = [
            val
            for val in values
            if val in ("mon", "tue", "wed", "thu", "fri", "sat", "sun")
            and not values[val]
        ]
        if days_removed:
        ...
        self.env["lunch.order"].search(
            [
                ...
                (
                    "date",
                    ">=",
                    fields.Date.context_today(self.with_context(tz=self.tz)),
                ),
            ]
        ).filtered(lambda lo: lo.date.weekday() in weekdays_n).write(
            {"state": "cancelled"}
        )
    ...
    def _compute_available_today(self):
        ...
        for supplier in self:
            supplier_date = now.astimezone(timezone(supplier.tz))
    ...
    def _available_on_date(self, date):
        ...
        return (
            not (self.recurrency_end_date and date.date() >= self.recurrency_end_date)
            and self[fieldname]
        )
    ...
    def _compute_order_deadline_passed(self):
        ...
        for supplier in self:
            if supplier.send_by == "mail":
                now = now.astimezone(timezone(supplier.tz))
                email_time = datetime.combine(
                    fields.Date.context_today(supplier),
                    float_to_time(supplier.automatic_email_time, supplier.moment),
                ).replace(tzinfo=timezone(supplier.tz))
                supplier.order_deadline_passed = (
                    supplier.available_today and now > email_time
                )
            else:
    ...
    def _search_available_today(self, operator, value):
        ...
        recurrency_domain = Domain("recurrency_end_date", "=", False) | Domain(
            "recurrency_end_date", ">" if truth else "<", today
        )
    ...
    def _compute_buttons(self):
        ...
        for supplier in self:
            supplier.show_order_button = supplier_orders[supplier.id].get(
                "ordered", False
            )
            supplier.show_confirm_button = supplier_orders[supplier.id].get(
                "sent", False
            )
