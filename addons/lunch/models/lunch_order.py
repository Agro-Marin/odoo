...
class LunchOrder(models.Model):
    ...
    def _compute_display_add_button(self):
        new_orders = dict(
            self.env["lunch.order"]._read_group(
                [
        ...
        for order in self:
            ...
            wallet_amount = (
                self.env["lunch.cashmove"].get_wallet_balance(order.user_id) - price
            )
    ...
    def _compute_display_reorder_button(self):
        ...
        for order in self:
            order.display_reorder_button = (
                show_button
                and order.state == "confirmed"
                and order.supplier_id.available_today
            )
    ...
    def _extract_toppings(self, values):
        ...
        for i in range(1, 4):
            ...
            if self.ids:
                ...
                topping_ids += (
                    self._get_topping_ids(topping_field, values.pop(topping_field))
                    if topping_values
                    else self[:1][topping_field].ids
                )
            else:
                topping_ids += (
                    self._get_topping_ids(topping_field, topping_values)
                    if topping_values
                    else []
                )
    ...
    def write(self, vals):
        values = vals
        change_topping = (
            "topping_ids_1" in values
            or "topping_ids_2" in values
            or "topping_ids_3" in values
        )
        merge_needed = "note" in values or change_topping or "state" in values
        default_location_id = (
            self.env.user.last_lunch_location_id
            and self.env.user.last_lunch_location_id.id
        ) or False
                ...
                matching_lines = self._get_matching_lines(
                    {
                        ...
                        "toppings": toppings,
                        "lunch_location_id": values.get(
                            "lunch_location_id", default_location_id
                        ),
    ...
    def _get_matching_lines(self, values):
        ...
        domain = [
            (
                "user_id",
                "=",
                values.get("user_id", self.default_get(["user_id"])["user_id"]),
            ),
            ...
            (
                "lunch_location_id",
                "=",
                values.get("lunch_location_id", default_location_id),
            ),
        ]
        if values.get("state"):
        ...
        return self.search(domain).filtered(
            lambda line: (
                (line.topping_ids_1 | line.topping_ids_2 | line.topping_ids_3).ids
                == toppings
            )
        )
    ...
    def _compute_total_price(self):
        for line in self:
            line.price = line.quantity * (
                ...
                + sum(
                    (
                        line.topping_ids_1 | line.topping_ids_2 | line.topping_ids_3
                    ).mapped("price")
                )
            )
