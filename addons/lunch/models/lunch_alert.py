...
class LunchAlert(models.Model):
    ...
    def _compute_available_today(self):
        ...
        for alert in self:
            alert.available_today = (
                alert.until > today if alert.until else True
            ) and alert[fieldname]
    ...
    def _sync_cron(self):
        ...
        for alert in self:
            ...
            ).replace(tzinfo=timezone(alert.tz))
            cron = alert.cron_id.sudo()
            lc = cron.lastcall
            if (
                lc
                and sendat_tz.date()
                <= fields.Datetime.context_timestamp(alert, lc).date()
            ) or (
                not lc
                and sendat_tz
                <= fields.Datetime.context_timestamp(alert, fields.Datetime.now())
            ):
    ...
    def _notify_chat(self):
        ...
        if not self.available_today:
            ...
            if (
                self.cron_id
                and self.until
                and fields.Date.context_today(self) > self.until
            ):
                self.cron_id.unlink()
                self.cron_id = False
            return
        ...
        if self.location_ids.ids:
            order_domain &= Domain(
                "user_id.last_lunch_location_id", "in", self.location_ids.ids
            )

        if self.recipients != "everyone":
            weeksago = fields.Date.today() - timedelta(
                weeks=(
                    1
                    if self.recipients == "last_week"
                    else 4
                    if self.recipients == "last_month"
                    else 52  # if self.recipients == 'last_year'
                )
            )
