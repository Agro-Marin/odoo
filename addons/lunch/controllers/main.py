...
class LunchController(http.Controller):
    ...
    def infos(self, user_id=None, context=None):
        ...
        if lines:
            ...
            lines = [
                {
                    ...
                    "toppings": [
                        (
                            topping.name,
                            float_repr(
                                float_round(topping.price, 2) * line.quantity, 2
                            ),
            ...
            paid_subtotal = float_round(
                sum(line["price"] for line in lines if line["raw_state"] != "new"), 2
            )
    ...
    def set_user_location(self, location_id=None, user_id=None, context=None):
        ...
        user.sudo().last_lunch_location_id = request.env["lunch.location"].browse(
            location_id
        )
        return True
    ...
    def get_user_location(self, user_id=None, context=None):
        ...
        company_ids = request.env.context.get(
            "allowed_company_ids", request.env.company.ids
        )
        user_location = user.last_lunch_location_id
        has_multi_company_access = (
            not user_location.company_id or user_location.company_id.id in company_ids
        )

        if not user_location or not has_multi_company_access:
            return (
                request.env["lunch.location"]
                .search([("company_id", "in", [False] + company_ids)], limit=1)
                .id
            )
    ...
    def _get_infos(self, user, **kwargs):
        ...
        res.update(
            {
                ...
                "userimage": "/web/image?model=res.users&id=%s&field=avatar_128"
                % user.id,
                "wallet": request.env["lunch.cashmove"].get_wallet_balance(user, False),
                "wallet_with_config": request.env["lunch.cashmove"].get_wallet_balance(
                    user
                ),
        ...
        user_location = user.last_lunch_location_id
        has_multi_company_access = (
            not user_location.company_id
            or user_location.company_id.id
            in request.env.context.get("allowed_company_ids", request.env.company.ids)
        )

        if not user_location or not has_multi_company_access:
            user.last_lunch_location_id = user_location = (
                request.env["lunch.location"].search([], limit=1) or user_location
            )
        ...
        res.update(
            {
                ...
                "alerts": request.env["lunch.alert"].search_read(
                    alert_domain, ["message"]
                ),
            }
        )
