...
class TestSupplier(TestsCommon):
    ...
    def test_send_email_cron(self):
        ...
        self.assertEqual(
            self.supplier_kothai.cron_id.sudo().code,
            f"""\
...
env['lunch.supplier'].browse([{self.supplier_kothai.id}])._send_auto_email()""",
    ...
    def test_compute_available_today(self):
        tests = [
            (self.monday_1am, True),
            (self.monday_10am, True),
            (self.monday_1pm, True),
            (self.monday_8pm, True),
            (self.saturday_3am, False),
            (self.saturday_10am, False),
            (self.saturday_1pm, False),
            (self.saturday_8pm, False),
        ]
        ...
        for value, result in tests:
            with patch.object(fields.Datetime, "now", return_value=value) as _:
                assert self.supplier_pizza_inn.available_today == result, (
                    "supplier pizza inn should %s considered available on %s"
                    % ("be" if result else "not be", value)
                )
    ...
    def test_search_available_today(self):
        ...
        tests = [
            (self.monday_1am, 1.0, "mon"),
            (self.monday_10am, 10.0, "mon"),
            (self.monday_1pm, 13.0, "mon"),
            (self.monday_8pm, 20.0, "mon"),
            (self.saturday_3am, 3.0, "sat"),
            (self.saturday_10am, 10.0, "sat"),
            (self.saturday_1pm, 13.0, "sat"),
                ...
                self.assertEqual(
                    ...
                    [
                        ...
                        (
                            "recurrency_end_date",
                            ">",
                            value.astimezone(timezone(self.env.user.tz)).date(),
                        ),
                        (dayname, "in", [True]),
                    ],
                )
        ...
        with patch.object(fields.Datetime, "now", return_value=self.monday_10am):
            self.assertIn(
                self.supplier_pizza_inn,
                Supplier.search([("available_today", "=", True)]),
            )
    ...
    def test_auto_email_send(self):
        with patch.object(fields.Datetime, "now", return_value=self.monday_1pm) as _:
            with patch.object(
                fields.Date, "today", return_value=self.monday_1pm.date()
            ) as _:
                with patch.object(
                    fields.Date, "context_today", return_value=self.monday_1pm.date()
                ) as _:
                    ...
                    assert all(
                        line.state == "ordered"
                        for line in [line_pizza_2, line_pizza_olive_2]
                    )
    ...
    def test_cron_sync_create(self):
        ...
        self.assertEqual(
            [
                line
                for line in cron_ny.sudo().code.splitlines()
                if not line.lstrip().startswith("#")
            ],
            [
                "env['lunch.supplier'].browse([%i])._send_auto_email()"
                % self.supplier_kothai.id
            ],
        )
        self.assertEqual(
            cron_ny.nextcall, datetime(2021, 1, 29, 15, 0)
        )  # New-york is UTC-5
    ...
    def test_cron_sync_nextcall(self):
        ...
        self.assertEqual(
            cron_ny.nextcall, old_nextcall - timedelta(hours=5) + timedelta(days=1)
        )
        ...
        supplier = self.env["lunch.supplier"].create(
            {
                ...
                "topping_ids_2": [
                    (
                        0,
                        0,
                        {"name": "salt", "price": 7, "company_id": self.env.company.id},
                    ),
                ],
                "topping_ids_3": [
                    (
