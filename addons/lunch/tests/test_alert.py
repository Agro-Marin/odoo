...
class TestAlarm(TestsCommon):
    ...
    def test_cron_sync_create(self):
        ...
        self.assertEqual(
            cron_ny.name, "Lunch: alert chat notification (New York UTC-5)"
        )
        self.assertEqual(
            [
                line
                for line in cron_ny.code.splitlines()
        ...
        self.assertEqual(
            tokyo_cron.nextcall, datetime(2021, 1, 29, 23, 0)
        )  # Tokyo is UTC+9 but the cron is posponed
