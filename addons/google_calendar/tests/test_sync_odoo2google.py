...
class TestSyncOdoo2Google(TestSyncGoogle):
        ...
        self.assertGoogleEventInserted(
            {
                ...
                "reminders": {
                    "useDefault": False,
                    "overrides": [
                        {"method": "popup", "minutes": alarm.duration_minutes}
                    ],
                },
                "organizer": {"email": "odoobot@example.com", "self": True},
                "attendees": [
                    {"email": "jean-luc@opoo.com", "responseStatus": "needsAction"}
                ],
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: event.id}
                },
                "transparency": "opaque",
            }
        )
    ...
    def test_event_creation_perf(self):
        ...
        partners = self.env["res.partner"].create(
            [
                {"name": "Jean-Luc %s" % (i), "email": "jean-luc-%s@opoo.com" % (i)}
    ...
    def test_event_without_user(self):
        ...
        self.assertFalse(
            "%s_owner_id" % self.env.cr.dbname
            in values.get("extendedProperties", {}).get("shared", {})
        )
    ...
    def test_event_without_attendee_state(self):
        ...
        partner_2 = self.env["res.partner"].create(
            {"name": "Phineas", "email": "phineas@opoo.com"}
        )
        ...
        attendee_2 = event.attendee_ids.filtered(
            lambda a: a.partner_id.id == partner_2.id
        )
        attendee_2.write(
            {
                "state": False,
        ...
        self.assertGoogleEventInserted(
            {
                ...
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: recurrence.id}
                },
                "transparency": "opaque",
            }
        )
    ...
    def test_event_added_to_recurrence(self):
        ...
        self.assertFalse(
            event.google_id, "The google id will be set after the API call"
        )
        self.assertGoogleEventInserted(
            {
                "id": False,
                ...
                "extendedProperties": {
                    "shared": {
                        "%s_odoo_id" % self.env.cr.dbname: event.recurrence_id.id
                    }
                },
                "transparency": "opaque",
            },
    ...
    def test_stop_synchronization(self):
        ...
        self.assertTrue(
            self.env.user.google_synchronization_stopped,
            "The google synchronization flag should be switched on",
        )
        self.assertFalse(
            self.env.user._sync_google_calendar(self.google_service),
    ...
    def test_restart_synchronization(self):
        ...
        self.assertGoogleEventPatched(
            event.google_id,
            {
                ...
                "attendees": [
                    {"email": "jean-luc@opoo.com", "responseStatus": "accepted"}
                ],
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: event.id}
                },
        ...
        self.assertGoogleEventInserted(
            {
                ...
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: new_recurrence.id}
                },
                "reminders": {"overrides": [], "useDefault": False},
                "transparency": "opaque",
            },
    ...
    def test_event_until_utc(self):
        ...
        recurrence._apply_recurrence()
        self.assertEqual(
            recurrence._google_values()["recurrence"][0],
            "RRULE:FREQ=DAILY;UNTIL=20200117T235959Z",
        ...
        self.assertEqual(
            recurrence._google_values()["recurrence"][0],
            "RRULE:FREQ=DAILY;UNTIL=20200119T235959Z",
    ...
    def test_attendee_state(self):
        ...
        self.assertGoogleEventPatched(
            event.google_id,
            {
                ...
                "organizer": {"email": "odoobot@example.com", "self": True},
                "attendees": [
                    {"email": "jean-luc@opoo.com", "responseStatus": "declined"}
                ],
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: event.id}
                },
        ...
        self.assertGoogleEventInserted(
            {
                "id": False,
                "start": {
                    "dateTime": "2020-01-15T08:00:00+00:00",
                    "timeZone": "Europe/Brussels",
                    "date": None,
                },
                "end": {
                    "dateTime": "2020-01-15T09:00:00+00:00",
                    "timeZone": "Europe/Brussels",
                    "date": None,
                },
                "summary": "New name",
    ...
    def test_recurrence_delete_single_events(self):
        ...
        self.assertGoogleEventPatched(
            event_1.google_id,
            {
                ...
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: event_1.id}
                },
                "reminders": {"overrides": [], "useDefault": False},
                "status": "cancelled",
                "transparency": "opaque",
    ...
    def test_update_synced_event_with_sync_config_paused(self):
        ...
        self.assertEqual(
            record.name,
            "Updated Event",
            "Assert that event name was updated in Odoo Calendar",
        )
        self.assertTrue(
            record.need_sync,
            "Sync variable must be true for updating event when sync re-activates",
    ...
    def test_delete_synced_event_with_sync_config_paused(self):
        ...
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertFalse(
            record.active, "Event must be archived in Odoo after unlinking it"
        )
        self.assertTrue(
            record.need_sync,
    ...
    def test_event_sync_after_pause_period(self, mock_sync_request):
        ...
        self.assertFalse(
            self.organizer_user.google_synchronization_stopped,
            "Synchronization should not be stopped, only paused.",
        )
        self.assertEqual(
            self.organizer_user._get_google_sync_status(),
            "sync_paused",
        ...
        self.organizer_user.with_user(self.organizer_user).sudo()._sync_google_calendar(
            self.google_service
        )
        self.assertGoogleEventInserted(
            {
                ...
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: record.id}
                },
            }
        )
    ...
    def test_allday_duplicated_first_event_in_recurrence(self):
        ...
        self.assertFalse(
            event.need_sync,
            "Variable 'need_sync' must be falsy after event's 'create' call.",
        )
        self.assertGoogleAPINotCalled()
        ...
        self.assertFalse(
            event.need_sync,
            "Variable 'need_sync' must be falsy after recurrence's 'create' call.",
        )
        self.assertGoogleEventInserted(
            {
                "id": False,
    ...
    @patch_api
    @patch.object(ResUsers, "_sync_request")
    def test_skip_google_sync_for_non_synchronized_users_new_events(
        self, mock_sync_request
    ):
        """
        ...
        with self.mock_datetime_and_now("2023-01-10"):
            ...
            self.attendee_user.with_user(
                self.attendee_user
            ).sudo()._sync_google_calendar(self.google_service)
            self.assertGoogleAPINotCalled()
            ...
            self.assertGoogleEventInserted(
                {
                    ...
                    "summary": "Event",
                    "description": (
                        "<div><strong>Organized by</strong><br>"
                    ...
                    "attendees": [
                        ...
                        {
                            "email": self.organizer_user.email,
        ...
        self.assertGoogleEventInsertedMultiTime(
            {
                **event_response_data,
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: event2.id}
                },
            }
        )
    ...
    def test_event_over_send_updates(self):
        ...
        with self.mock_datetime_and_now("2023-01-10"):
            ...
            self.assertTrue(
                past_event._is_event_over(), "Event should be considered over"
            )
    ...
    def test_event_not_over_send_updates(self):
        ...
        with self.mock_datetime_and_now("2023-01-10"):
            ...
            with self.mock_google_service():
                ...
                self.call_post_commit_hooks()
            self.assertFalse(
                future_event._is_event_over(),
                "Future event should not be considered over",
            )
    ...
    def test_recurrence_over_send_updates(self):
        ...
        with self.mock_datetime_and_now("2023-01-10"):
            ...
            with self.mock_google_service():
                ...
                self.call_post_commit_hooks()
            self.assertTrue(
                past_recurrence._is_event_over(),
                "Past recurrence should be considered over",
            )
            self.assertGoogleEventSendUpdates("none")
    ...
    def test_recurrence_not_over_send_updates(self):
        ...
        with self.mock_datetime_and_now("2023-01-10"):
            ...
            with self.mock_google_service():
                ...
                self.call_post_commit_hooks()
            self.assertFalse(
                future_recurrence._is_event_over(),
                "Future recurrence should not be considered over",
            )
            self.assertGoogleEventSendUpdates("all")
    ...
    def test_eliminate_remote_meet_link_when_removed_locally(self):
        partner = self.env["res.partner"].create(
            {"name": "Name", "email": "email@emailprovider.com"}
        )
        ...
        self.assertGoogleEventInserted(
            {
                ...
                "attendees": [
                    {"email": "a.a@example.com", "responseStatus": "accepted"},
                    {"email": "o.o@example.com", "responseStatus": "needsAction"},
                ],
                "extendedProperties": {
                    "shared": {"%s_odoo_id" % self.env.cr.dbname: event.id}
                },
