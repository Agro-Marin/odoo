...
class TestSyncGoogle2Odoo(TestSyncGoogle):
    ...
    def generate_recurring_event(self, mock_dt, **values):
        ...
        with self.mock_datetime_and_now(mock_dt):
            base_event = (
                ...
                .create({"name": "coucou", "need_sync": False, **values})
            )
            recurrence = (
    ...
    def google_respond_to_recurrent_event_with_option_this_event(
        self, recurrence, event_index, response_status
    ):
        """
        ...
        """
        update_time = (recurrence.write_date + timedelta(seconds=30)).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
        recurrence_google_values = recurrence._google_values()
        recurrence_google_values["updated"] = update_time
        updated_event_google_values = recurrence.calendar_event_ids.sorted("start")[
            event_index
        ]._google_values()
    ...
    def google_respond_to_recurrent_event_with_option_all_events(
        self, recurrence, response_status
    ):
        """
    ...
    def google_respond_to_recurrent_event_with_option_following_events(
        self, recurrence, event_index, response_status, rrule1, rrule2
    ):
        """
        Function returns google api response simulating 'self.attendee_user' responding to the event number 'event_index' (0-indexed)
        ...
        updated_event_google_values["id"] = (
            updated_event_id[: updated_event_id.index("_") + 1]
            + "R"
            + updated_event_id[updated_event_id.index("_") + 1 :]
        )
    ...
    def test_new_google_event(self):
        ...
        admin_attendee = event.attendee_ids.filtered(
            lambda e: e.email == self.public_partner.email
        )
        ...
        event = self.env["calendar.event"].create(
            {
                ...
                "need_sync": False,
                "partner_ids": [
                    (6, 0, self.env.user.partner_id.ids)
    ...
    def test_cancelled_with_portal_attendee(self):
        ...
        portal_user = new_test_user(
            self.env, login="portal-user", groups="base.group_portal"
        )
    ...
    def test_private_extended_properties(self):
        ...
        self.assertGoogleEventPatched(
            event.google_id,
            {
                ...
                "end": {
                    "date": str(event.stop_date + relativedelta(days=1)),
                    "dateTime": None,
                },
                "attendees": [
                    {"email": "odoobot@example.com", "responseStatus": "declined"}
                ],
                "extendedProperties": {
                    "private": {"%s_odoo_id" % self.env.cr.dbname: event.id}
                },
    ...
    def test_recurrence(self):
        ...
        events = recurrence.calendar_event_ids.sorted("start")
        self.assertEqual(
            len(events), 3, "it should have created a recurrence with 3 events"
        )
    ...
    def test_recurrence_exdate(self):
        ...
        events = recurrence.calendar_event_ids.sorted("start")
        self.assertEqual(
            len(events), 2, "it should have created a recurrence with 2 events"
        )
        self.assertEqual(events[0].start_date, date(2020, 1, 6))
    ...
    def test_recurrence_outlier(self):
        ...
        self.assertEqual(
            events[1].start_date,
            date(2020, 1, 18),
            "It should not be in sync with the recurrence",
        )
        self.assertEqual(events[2].start_date, date(2020, 1, 20))
        self.assertGoogleAPINotCalled()
    ...
    def test_recurrence_write_with_outliers(self):
        ...
        self.assertEqual(
            events.mapped("name"), ["coucou again", "coucou again", "coucou again"]
        )
    ...
    def test_recurrence_write_time_fields(self):
        ...
        values = {
            ...
            "start": {
                "dateTime": "2021-02-15T12:00:00+01:00",
                "date": None,
            },  # 11:00 UTC
    ...
    def test_recurrence_timezone(self):
        ...
        self.assertEqual(
            recurrence.event_tz,
            "Pacific/Auckland",
            "The Google event Timezone should be saved on the recurrency",
        )
        self.assertGoogleAPINotCalled()
    ...
    def test_recurrence_no_duplicate(self):
        values = [
            {
                ...
                "end": {
                    "dateTime": "2023-02-25T16:20:00+01:00",
                ...
                "sequence": 0,
                "start": {
                    "dateTime": "2023-02-25T15:30:00+01:00",
                    "timeZone": "Europe/Zurich",
                },
                "status": "confirmed",
                "summary": "Weekly test",
            ...
            },
            {
                ...
                "creator": {"email": "john.doe@example.com"},
                "end": {
                    "dateTime": "2023-02-26T15:20:00+01:00",
                    "timeZone": "Europe/Zurich",
                },
                ...
                "sequence": 0,
                "start": {
                    "dateTime": "2023-02-26T14:30:00+01:00",
                    "timeZone": "Europe/Zurich",
                },
                "status": "confirmed",
        ...
        no_duplicate_gevent = google_events.filter(
            lambda e: e.id == "9lxiofipomymx2yr1yt0hpep99"
        )
        dt_start = (
            datetime.fromisoformat(no_duplicate_gevent.start["dateTime"])
            .astimezone(UTC)
            .replace(tzinfo=None)
            .replace(hour=0)
        )
        dt_end = (
            datetime.fromisoformat(no_duplicate_gevent.end["dateTime"])
            .astimezone(UTC)
            .replace(tzinfo=None)
            .replace(hour=23)
        )
    ...
    def test_recurrence_list_contains_more_items(self):
        ...
        values = {
            ...
            "visibility": "public",
            "recurrence": [
                "EXDATE;TZID=Europe/Rome:20200113",
    ...
    def test_new_google_notifications(self):
        ...
        triggers_before = self.env["ir.cron.trigger"].search(
            [("cron_id", "=", cron_id)]
        )
        google_id = "oj44nep1ldf8a3ll02uip0c9aa"
    ...
    def test_event_reminder_emails_with_google_id(self):
        ...
        self.env["calendar.event"]._sync_google2odoo(GoogleEvent([values]))
        events_by_alarm = self.env[
            "calendar.alarm_manager"
        ]._get_events_by_alarm_to_notify("email")
        self.assertFalse(
            events_by_alarm, "Events with google_id should not trigger reminders"
        )
    ...
    def test_attendees_same_event_both_share(self):
        ...
        event = self.env["calendar.event"].create(
            {
                ...
                "partner_ids": [
                    (
                        6,
                        0,
                        [self.env.user.partner_id.id, other_user.partner_id.id],
                    )
                ],  # current user is attendee
            }
        )
        ...
        self.assertGoogleEventPatched(
            event.google_id,
            {
                ...
                "extendedProperties": {
                    "shared": {
                        "%s_odoo_id" % self.env.cr.dbname: event.id,
    ...
    def test_several_users_have_the_same_mail(self):
        ...
        user1 = new_test_user(
            self.env, login="test@example.com", groups="base.group_portal"
        )
        ...
        values = {
            ...
            "attendees": [
                ...
                {"email": "test2@example.com", "responseStatus": "accepted"},
            ],
            "reminders": {"useDefault": True},
            "start": {
                "dateTime": "2020-01-13T16:00:00+01:00",
        ...
        self.assertEqual(
            event.partner_ids.ids,
            [user2.partner_id.id, new_partner.id],
            "The internal user should be chosen",
        )
    ...
    def test_event_with_meeting_url(self):
        ...
        self.assertTrue(event, "It should have created an event")
        self.assertEqual(
            event.videocall_location, "https://meet.google.com/odoo-random-test"
        )
        self.assertGoogleAPINotCalled()
    ...
    def test_event_with_local_videocall(self):
        ...
        values = {
            ...
            "conferenceData": {
                "entryPoints": [
                    {"entryPointType": "video", "uri": False, "label": "no label"}
                ]
            },
            "updated": self.now,
        }
        ...
        self.assertEqual(
            event.videocall_location, "https://meet.google.com/odoo_local_videocall"
        )

        # now google has meet URL and make sure local video call is updated accordingly
        values["conferenceData"]["entryPoints"][0]["uri"] = (
            "https://meet.google.com/odoo-random-test"
        )
        gevent = GoogleEvent([values])
        self.env["calendar.event"]._sync_google2odoo(gevent)
        self.assertEqual(
    ...
    def test_private_partner_single_event(self):
        ...
        private_attendee = event.attendee_ids.filtered(
            lambda e: e.email == self.private_partner.email
        )
    ...
    def test_recurrence_private_contact(self):
        ...
        events = recurrence.calendar_event_ids
        private_attendees = events.mapped("attendee_ids").filtered(
            lambda e: e.email == self.private_partner.email
        )
        self.assertTrue(
            all(a.partner_id == self.private_partner for a in private_attendees)
        )
    ...
    def test_alias_email_sync_recurrence(self):
        ...
        mail_alias = self.env["mail.alias"].create(
            {"alias_name": "sale", "alias_model_id": alias_model.id}
        )
    ...
    def test_partner_order(self):
        ...
        self.env["calendar.event"].with_user(user)._sync_google2odoo(
            GoogleEvent([values])
        )
        event = self.env["calendar.event"].search(
            [("google_id", "=", values.get("id"))]
        ...
        self.assertEqual(
            2,
            len(event.partner_ids),
            "Two attendees and two partners should be associated to the event",
        )
        self.assertGoogleAPINotCalled()
    ...
    def test_recurrence_range_start_date_in_other_dst_period(self):
        ...
        google_value = [
            {
                ...
                "start": {
                    "dateTime": "2023-03-27T09:00:00+02:00",
                    "timeZone": "Europe/Brussels",
                },
                "end": {
                    "dateTime": "2023-03-27T10:00:00+02:00",
                    "timeZone": "Europe/Brussels",
                },
        ...
        day_start = (
            datetime.fromisoformat(google_event.start["dateTime"])
            .astimezone(UTC)
            .replace(tzinfo=None)
            .replace(hour=0)
        )
        day_end = (
            datetime.fromisoformat(google_event.end["dateTime"])
            .astimezone(UTC)
            .replace(tzinfo=None)
            .replace(hour=23)
        )
    ...
    def test_recurrence_edit_specific_event(self):
        google_values = [
            {
                ...
                "start": {
                    "dateTime": "2023-05-12T09:00:00+02:00",
                    "timeZone": "Europe/Brussels",
                    "date": None,
                },
                "end": {
                    "dateTime": "2023-05-12T10:00:00+02:00",
                    "timeZone": "Europe/Brussels",
                    "date": None,
                },
                "recurrence": [
                    "RRULE:FREQ=WEEKLY;WKST=SU;UNTIL=20230518T215959Z;BYDAY=FR"
                ],
            ...
            },
            {
                ...
                "organizer": {"email": "john.doe@example.com", "self": True},
                "start": {
                    "dateTime": "2023-05-19T09:00:00+02:00",
                    "timeZone": "Europe/Brussels",
                    "date": None,
                },
                "end": {
                    "dateTime": "2023-05-19T10:00:00+02:00",
                    "timeZone": "Europe/Brussels",
                    "date": None,
                },
            ...
            },
            {
                ...
                "organizer": {"email": "john.doe@example.com", "self": True},
                "start": {
                    "dateTime": "2023-05-26T08:00:00+02:00",
                    "timeZone": "Europe/Brussels",
                    "date": None,
                },
                "end": {
                    "dateTime": "2023-05-26T09:00:00+02:00",
                    "timeZone": "Europe/Brussels",
                    "date": None,
                },
        ...
        # Range to check
        day_start = (
            datetime.fromisoformat(specific_event.start["dateTime"])
            .astimezone(UTC)
            .replace(tzinfo=None)
            .replace(hour=0)
        )
        day_end = (
            datetime.fromisoformat(specific_event.end["dateTime"])
            .astimezone(UTC)
            .replace(tzinfo=None)
            .replace(hour=23)
        )
    ...
    def test_recurrence_edit_specific_event_backward_compatibility(self):
        ...
        self.assertEqual(
            len(events), 2
        )  # Two because in this case we does not detect the existing event
    ...
    def test_event_guest_modify_permission(self):
        ...
        self.env["calendar.event"]._sync_google2odoo(
            GoogleEvent([not_editable_event_values, editable_event_values])
        )
        not_editable_event = self.env["calendar.event"].search(
            [("google_id", "=", not_editable_event_values.get("id"))]
        )
        editable_event = self.env["calendar.event"].search(
            [("google_id", "=", editable_event_values.get("id"))]
        )

        # Assert that event is created in Odoo with proper values for guests_readonly variable.
        self.assertFalse(
            editable_event.guests_readonly,
            "Value 'guestCanModify' received from Google must be True.",
        )
        self.assertTrue(
            not_editable_event.guests_readonly,
            "Value 'guestCanModify' received from Google must be False.",
        )

        # Assert that organizer can edit both events.
        self.assertTrue(
            editable_event.with_user(editable_event.user_id).write(
                {"name": "Edited by organizer!"}
            )
        )
        self.assertTrue(
            not_editable_event.with_user(not_editable_event.user_id).write(
                {"name": "Edited by organizer!"}
            )
        )
        ...
        with self.assertRaises(ValidationError):
            self.assertTrue(
                not_editable_event.with_user(guest_user).write(
                    {"name": "Edited by attendee!"}
                ),
        ...
        self.assertTrue(
            editable_event.with_user(guest_user).write({"name": "Edited by attendee!"}),
    ...
    @patch_api
    def test_attendee_status_is_not_updated_when_syncing_and_time_data_is_not_changed(
        self,
    ):
            ...
            .create(
                {
                    ...
                    "need_sync": False,
                    "partner_ids": [
                        Command.set([organizer.partner_id.id, other_user.partner_id.id])
                    ],
                }
            )
        )
        ...
        self.assertTrue(
            all(
                len(event.attendee_ids) == 2 for event in recurrence.calendar_event_ids
            ),
            "should have 2 attendees in all recurring events",
        )
        organizer_state = (
            recurrence.calendar_event_ids.sorted("start")[0]
            .attendee_ids.filtered(
                lambda attendee: attendee.partner_id.email == organizer.partner_id.email
            )
            .state
        )
        ...
        values = [
            {
                ...
                "attendees": [
                    {"email": organizer.partner_id.email, "responseStatus": "accepted"},
                    {
                        "email": other_user.partner_id.email,
                        "responseStatus": "accepted",
                    },
                ],
                "updated": self.now,
        ...
        self.env["calendar.recurrence"].with_user(other_user)._sync_google2odoo(
            GoogleEvent(values)
        )
        events = recurrence.calendar_event_ids.sorted("start")
        self.assertEqual(
        ...
        self.assertEqual(
            events[0].attendee_ids[0].state,
            "accepted",
            "after google sync, organizer should have accepted status still",
        )
        self.assertGoogleAPINotCalled()
    ...
    def test_create_event_with_default_and_undefined_privacy(self):
        ...
        self.env["calendar.event"]._sync_google2odoo(
            GoogleEvent([undefined_privacy_event, default_privacy_event])
        )

        # Ensure that synced events have the correct privacy field in Odoo.
        undefined_privacy_odoo_event = self.env["calendar.event"].search(
            [("google_id", "=", 1)]
        )
        default_privacy_odoo_event = self.env["calendar.event"].search(
            [("google_id", "=", 2)]
        )
        self.assertFalse(
            undefined_privacy_odoo_event.privacy,
            "Event with undefined privacy must have False value in privacy field.",
        )
        self.assertFalse(
            default_privacy_odoo_event.privacy,
            "Event with default privacy must have False value in privacy field.",
        )

    @patch.object(GoogleCalendarService, "get_events")
    def test_accepting_recurrent_event_with_this_event_option_synced_by_attendee(
        ...
        google_events = self.google_respond_to_recurrent_event_with_option_this_event(
            recurrence, 2, "accepted"
        )
        ...
        with self.mock_datetime_and_now("2024-04-22"):
            ...
            attendees = (
                ...
                .search([("partner_id", "=", self.attendee_user.partner_id.id)])
    ...
    @patch.object(GoogleCalendarService, "get_events")
    def test_accepting_recurrent_event_with_this_event_option_synced_by_organizer(
        self, mock_get_events
    ):
        """
    ...
    @patch.object(GoogleCalendarService, "get_events")
    def test_accepting_recurrent_event_with_all_events_option_synced_by_attendee(
        self, mock_get_events
    ):
        """
        ...
        google_events = self.google_respond_to_recurrent_event_with_option_all_events(
            recurrence, "accepted"
        )
        mock_get_events.return_value = (
            GoogleEvent(google_events),
    ...
    @patch.object(GoogleCalendarService, "get_events")
    def test_accepting_recurrent_event_with_all_events_option_synced_by_organizer(
        self, mock_get_events
    ):
        """
    ...
    @patch.object(GoogleCalendarService, "get_events")
    def test_accepting_recurrent_event_with_following_events_option_synced_by_attendee(
        self, mock_get_events
    ):
        """
        ...
        google_events = (
            self.google_respond_to_recurrent_event_with_option_following_events(
    ...
    @patch.object(GoogleCalendarService, "get_events")
    def test_accepting_recurrent_event_with_all_following_option_synced_by_organizer(
        self, mock_get_events
    ):
        """
    ...
    def test_keep_organizer_attendee_writing_recurrence_from_google(self):
        ...
        def check_organizer_as_single_attendee(self, recurrence, organizer):
            ...
            for event in recurrence.calendar_event_ids:
                self.assertTrue(
                    len(event.attendee_ids) == 1, "Should have only one attendee."
                )
                self.assertEqual(
                    event.attendee_ids[0].partner_id,
                    organizer.partner_id,
                    "The single attendee must be the organizer.",
                )
        ...
        self.env["calendar.recurrence"].with_user(
            self.organizer_user
        )._sync_google2odoo(GoogleEvent(values))
        events = recurrence.calendar_event_ids.sorted("start")
