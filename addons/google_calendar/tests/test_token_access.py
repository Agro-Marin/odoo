...
class TestTokenAccess(EncryptionKeyCase, TransactionCase):
    ...
    def test_normal_user_should_be_able_to_reset_his_own_token(self):
        ...
        user.with_user(user).res_users_settings_id._set_google_auth_tokens(
            "my_new_token", "my_new_rtoken", 3600
        )

        self.assertEqual(
            user.res_users_settings_id.google_calendar_rtoken, "my_new_rtoken"
        )
        self.assertEqual(
            user.res_users_settings_id.google_calendar_token, "my_new_token"
        )
        self.assertNotEqual(
            user.res_users_settings_id.google_calendar_token_validity, old_validity
        )
