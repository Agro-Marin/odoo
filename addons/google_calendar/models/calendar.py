...
class CalendarEvent(models.Model):
    ...
    def _odoo_values(self, google_event, default_reminders=()):
        ...
        if google_event.start.get('dateTime'):
            ...
            start = parse(google_event.start.get('dateTime')).astimezone(UTC).replace(tzinfo=None)
            stop = parse(google_event.end.get('dateTime')).astimezone(UTC).replace(tzinfo=None)
            values['allday'] = False
        else:
