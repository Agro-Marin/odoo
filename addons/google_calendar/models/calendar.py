...
class CalendarEvent(models.Model):
    ...
    _inherit = ["calendar.event", "mixin.google.calendar.sync"]
    ...
    def _compute_videocall_source(self):
        events_with_google_url = self.filtered(
            lambda event: self.MEET_ROUTE in (event.videocall_location or "")
        )
    ...
    def create(self, vals_list):
        ...
        return super(
            CalendarEvent,
            self.with_context(
                dont_notify=notify_context, skip_contact_description=description_context
            ),
        ).create(
            [
                dict(vals, need_sync=False)
    ...
    def write(self, vals):
        ...
        if not notify_context and (
            [self.env.user.id != record.user_id.id for record in self]
        ):
            self._check_modify_event_permission(vals)
        res = super(CalendarEvent, self.with_context(dont_notify=notify_context)).write(
            vals
        )
        if (
    ...
    def _check_modify_event_permission(self, values):
        ...
        if google_sync_restart or skip_event_permission or not is_synchronizable:
            return
        if any(
            event.guests_readonly and self.env.user.id != event.user_id.id
            for event in self
        ):
    ...
    def _skip_send_mail_status_update(self):
        ...
        if (
            user_id.is_google_calendar_synced()
    ...
    def _odoo_values(self, google_event, default_reminders=()):
        ...
        if not reminder_command:
            reminder_command = (
                google_event.reminders.get("useDefault") and default_reminders
            ) or ()
        ...
        values = {
            ...
            and tools.html_sanitize(google_event.description),
        ...
        if google_event.start.get("dateTime"):
            ...
            start = (
                parse(google_event.start.get("dateTime"))
                .astimezone(UTC)
                .replace(tzinfo=None)
            )
            stop = (
                parse(google_event.end.get("dateTime"))
                .astimezone(UTC)
                .replace(tzinfo=None)
            )
            values["allday"] = False
    ...
    def _odoo_attendee_commands(self, google_event):
        ...
        if (
            len(google_attendees) == 0
            and google_event.organizer
            and google_event.organizer.get("self", False)
        ):
    ...
    def _google_values(self):
        ...
        if (
            ...
            and not bool(self.user_id.sudo().google_calendar_token)
        ):
            # The organizer is an Odoo user that do not sync his calendar
            values["extendedProperties"]["shared"][
    ...
    def _cancel(self):
        ...
        attendees = (self - my_cancelled_records).attendee_ids.filtered(
