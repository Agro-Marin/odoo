...
class MixinGoogleCalendarSync(models.AbstractModel):
    ...
    def _sync_google2odoo(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        ...
        for gevent in pending:
            ...
            if not odoo_record_write_date or updated >= odoo_record_write_date.replace(tzinfo=UTC):
    ...
    def _get_sync_partner(self, emails):
        ...
        partners = self.env['mixin.mail.thread']._partner_find_from_emails_single(normalized_emails)
