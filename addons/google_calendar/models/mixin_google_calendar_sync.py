...
class MixinGoogleCalendarSync(models.AbstractModel):
    ...
    def write(self, vals):
        ...
        if (
            "need_sync" not in vals
            and vals.keys() & synced_fields
            and not self.env.user.google_synchronization_stopped
        ):
            vals["need_sync"] = True
    ...
    def create(self, vals_list):
        ...
        users_with_sync = (
            self.env["res.users"]
            .browse(user_ids)
            .filtered(lambda u: not u.sudo().google_synchronization_stopped)
        )
        ...
        for vals in vals_list:
            if (
                ...
                and vals["user_id"] not in users_with_sync_set
            ):
        ...
        if self.env.user._get_google_sync_status() != "sync_paused":
            for record in records:
                if record.need_sync and record.active:
                    record.with_user(record._get_event_user())._google_insert(
                        google_service, record._google_values(), timeout=3
                    )
        return records
    ...
    def _handle_allday_recurrences_edge_case(self, records, vals_list):
        ...
        if vals_list and self._name == "calendar.event":
            ...
            records_to_skip = records.filtered(
                lambda r: (
                    r.need_sync and r.allday and r.recurrency and not r.recurrence_id
                )
            )
        ...
        if self.env.user._get_google_sync_status() != "sync_paused":
            for record in cancelled_records:
                if record.google_id and record.need_sync:
                    record.with_user(record._get_event_user())._google_delete(
            ...
            for record in new_records:
                if record._is_google_insertion_blocked(sender_user=self.env.user):
                    continue
                record.with_user(record._get_event_user())._google_insert(
                    google_service, record._google_values()
                )
            for record in updated_records:
                record.with_user(record._get_event_user())._google_patch(
                    google_service, record.google_id, record._google_values()
                )
    ...
    def _sync_google2odoo(
        ...
        odoo_values = [
            dict(self._odoo_values(e, default_reminders), need_sync=False) for e in new
        ]
        ...
        rescheduled_events = new.filter(
            lambda gevent: not gevent.is_recurrence_follower()
        )
        if rescheduled_events:
            google_ids_to_remove = [
                event.full_recurring_event_id() for event in rescheduled_events
            ]
            cancelled_odoo += self.env["calendar.event"].search(
                [("google_id", "in", google_ids_to_remove)]
            )
        ...
        for gevent in pending:
            ...
            odoo_record_write_date = write_dates.get(
                odoo_record.id, odoo_record.write_date
            )
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not odoo_record_write_date or updated >= odoo_record_write_date.replace(
                tzinfo=UTC
            ):
                vals = dict(
                    self._odoo_values(gevent, default_reminders), need_sync=False
                )
                odoo_record.with_context(dont_notify=True)._write_from_google(
                    gevent, vals
                )
    ...
    def _google_error_handling(self, http_error):
        ...
        if http_error.response.status_code in (403, 400):
            ...
            if not self.exists():
                reason = "Google gave the following explanation: %s" % response[
                    "error"
                ].get("message")
                error_log = (
                    "Error while syncing record. It does not exists anymore in the database. %s"
                    % reason
                )
            ...
            if self._name == "calendar.event":
                ...
                event = self.base_event_id or self._get_first_event(
    ...
    @after_commit
    def _google_delete(
        self, google_service: GoogleCalendarService, google_id, timeout=TIMEOUT
    ):
        with google_calendar_token(self.env.user.sudo()) as token:
            if token:
                is_recurrence = self.env.context.get("is_recurrence", False)
                google_service.google_service = (
                    google_service.google_service.with_context(
    ...
    @after_commit
    def _google_patch(
        self, google_service: GoogleCalendarService, google_id, values, timeout=TIMEOUT
    ):
        with google_calendar_token(self.env.user.sudo()) as token:
            if token:
                try:
                    ...
                    google_service.patch(
                        google_id, values, token=token, timeout=timeout
                    )
    ...
    def _google_insert(
        ...
        with google_calendar_token(self.env.user.sudo()) as token:
            if token:
                try:
                    send_updates = (
                        self.env.context.get("send_updates", True)
                        and not self._is_event_over()
                    )
                    google_service.google_service = (
                        google_service.google_service.with_context(
                            send_updates=send_updates
                    ...
                    google_values = google_service.insert(
                        values,
                        token=token,
                        timeout=timeout,
