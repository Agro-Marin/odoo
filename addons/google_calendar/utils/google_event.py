...
class GoogleEvent(abc.Set):
    ...
    def _meta_odoo_id(self, dbname):
        ...
        properties = (
            ...
            and (
                self.extendedProperties.get("shared", {})
                or self.extendedProperties.get("private", {})
            )
        ) or {}
    ...
    def owner(self, env):
        ...
        try:
            ...
            real_owner_id = False
        real_owner = (real_owner_id and env["res.users"].browse(real_owner_id)) or env[
            "res.users"
        ]
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_odoo_ids_from_db(
            env, env["calendar.recurrence"]
        )
    ...
    def full_recurring_event_id(self):
        ...
        if (
            not id_value
            or not recurringEventId_value
            or id_value.group(1) != recurringEventId_value.group(1)
        ):
            return None
    ...
    def get_meeting_url(self):
        ...
        video_meeting = list(
            filter(
                lambda entryPoints: entryPoints.get("entryPointType") == "video",
    ...
    def get_odoo_event(self, env):
        if self._get_model(env)._name == "calendar.event":
            ...
            return (
                env["calendar.recurrence"].browse(self.odoo_id(self.env)).base_event_id
            )
