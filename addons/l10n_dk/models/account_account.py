...
class AccountAccount(models.Model):
    ...
    def _unlink_bank_cash_accounts(self):
        nb_account_to_delete_per_company = defaultdict(
        ...
        nb_account_per_company = {
            self.env["res.company"].browse(entry["company_ids"][0]): entry[
                "company_ids_count"
            ]
        ...
        for company_id, count in nb_account_per_company.items():
            nb_to_delete = sum(
                1
                for account in nb_account_to_delete_per_company.get(company_id)
