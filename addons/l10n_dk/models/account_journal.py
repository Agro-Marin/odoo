...
class AccountJournal(models.Model):
    ...
    def _prepare_liquidity_account_vals(self, company, code, vals):
        ...
        if company.tax_config_id.account_fiscal_country_id.code == "DK":
            ...
            if vals.get("type") == "bank":
                account_vals["tag_ids"].append(
                    Command.link(self.env.ref("l10n_dk.account_tag_6481").id)
                )
            elif vals.get("type") == "cash":
                account_vals["tag_ids"].append(
                    Command.link(self.env.ref("l10n_dk.account_tag_6471").id)
                )
