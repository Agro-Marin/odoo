...
class TestStructure(TransactionCase):
    ...
    def test_vat_country_difference(self):
        ...
        self.assertEqual(
            partner.vat, "RORO790707I47", "Partner VAT should not be altered"
        )
    ...
    def test_vat_syntactic_validation(self):
        ...
        test_partner.write(
            {"vat": "BE0477472701", "country_id": self.env.ref("base.fr").id}
        )
        ...
        with self.assertRaises(ValidationError):
            ...
            test_partner.write(
                {"vat": "BE23334175221", "country_id": self.env.ref("base.fr").id}
            )
        ...
        test_partner.write(
            {"vat": "BE0477472701", "country_id": None}
        )  # Even with BE prefix, it should not be checked
        test_partner.write(
            {"vat": "BE0477472702", "country_id": None}
        )  # also invalid in BE
    ...
    def test_vat_eu(self):
        ...
        test_partner = self.env["res.partner"].create(
            {"name": "Turlututu", "country_id": self.env.ref("base.fr").id}
        )
        test_partner.write({"vat": "EU528003646", "country_id": None})

        test_partner.write(
            {"vat": "EU528003646", "country_id": self.env.ref("base.ca").id}
        )

        with self.assertRaises(ValidationError):
            test_partner.write(
                {"vat": "EU528003646", "country_id": self.env.ref("base.be").id}
            )
    ...
    def test_rut_uy(self):
        test_partner = self.env["res.partner"].create(
            {"name": "UY Company", "country_id": self.env.ref("base.uy").id}
        )
        # Set a valid Number
    ...
    def test_company_changes(self):
        ...
        test_partner_1 = self.env["res.company"].create(
            {"name": "Roman", "country_id": self.env.ref("base.es").id}
        )
        test_partner_2 = self.env["res.company"].create(
            {"name": "Roman2", "country_id": ro_country, "vat": "1234567897"}
        )
    ...
    def test_cl_hyphen(self):
        ...
        test_partner_1 = self.env["res.partner"].create(
            {"name": "Roman", "country_id": cl_country, "vat": "CL 760864285"}
        )
    ...
    def test_co_hyphen(self):
        ...
        test_partner_1 = self.env["res.partner"].create(
            {"name": "Roman", "country_id": co_country, "vat": "213.123.4321"}
        )
    ...
    def test_xi_works(self):
        ...
        test_partner_1 = self.env["res.partner"].create(
            {"name": "Roman", "country_id": uk_country, "vat": "XI123456782"}
        )
    ...
    def test_gr_changes(self):
        ...
        test_partner_1 = self.env["res.partner"].create(
            {"name": "Roman", "country_id": gr_country, "vat": "GR123456783"}
        )
        self.assertEqual(test_partner_1.vat, "EL123456783")
        test_partner_1 = self.env["res.partner"].create(
            {"name": "Roman", "country_id": gr_country, "vat": "EL123456783"}
        )
        self.assertEqual(test_partner_1.vat, "EL123456783")
    ...
    def test_weird_roro_input(self):
        ...
        with self.assertRaises(ValidationError):
            self.env["res.partner"].create(
                {"name": "Roman", "country_id": be_country, "vat": "RORORORO1234567897"}
            )
    ...
    def test_vat_tw(self):
        test_partner = self.env["res.partner"].create(
            {"name": "TW Company", "country_id": self.env.ref("base.tw").id}
        )
    ...
    def test_vat_notEU_with_EU_vat(self):
        test_partner = self.env["res.partner"].create(
            {"name": "CN Company", "country_id": self.env.ref("base.cn").id}
        )
    ...
    def test_vat_do(self):
        test_partner = self.env["res.partner"].create(
            {"name": "DO Company", "country_id": self.env.ref("base.do").id}
        )
        # Valid do vat
