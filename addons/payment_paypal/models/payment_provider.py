...
class PaymentProvider(models.Model):
    ...
    def action_paypal_create_webhook(self):
        ...
        data = {
            ...
            "event_types": [
                {"name": event_type} for event_type in const.HANDLED_WEBHOOK_EVENTS
            ],
        }
        webhook_data = self._send_api_request(
            "POST", "/v1/notifications/webhooks", json=data
        )
    ...
    def _paypal_fetch_access_token(self):
        ...
        """
        if fields.Datetime.now() > self.paypal_access_token_expiry - timedelta(
            minutes=5
        ):
