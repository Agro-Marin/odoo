...
class PaypalTest(PaypalCommon, PaymentHttpCommon):
    ...
    def test_order_payload_values_for_public_user(self):
        ...
        tx = self._create_transaction(
            flow="direct", partner_id=self.public_user.partner_id.id
        )
    ...
    def test_provide_shipping_address(self):
        if "sale.order" not in self.env:
            self.skipTest(
                "Skipping shipping address test because sale is not installed."
            )
        ...
        tx = self._create_transaction(
            flow="direct", sale_order_ids=[Command.set(order.ids)]
        )
        ...
        shipping_partner = tx.sale_order_ids.partner_shipping_id = (
            self.partner.child_ids
        )
        payload = tx._paypal_prepare_order_payload()
        self.assertEqual(
