...
class PaymentProvider(models.Model):
    ...
    def _check_razorpay_credentials_are_set_before_enabling(self):
        ...
        for provider in self.filtered(
            lambda p: p.code == "razorpay" and p.state != "disabled"
        ):
    ...
    def action_start_onboarding(self, menu_id=None):
        ...
        params = {
            "return_url": tools.urls.urljoin(
                self.get_base_url(), RazorpayController.OAUTH_RETURN_URL
            ),
            "provider_id": self.id,
    ...
    def _razorpay_calculate_signature(self, data, is_redirect=True):
        ...
        if is_redirect:
            ...
            if not secret:
                _logger.warning(
                    "Missing webhook secret; aborting signature calculation."
                )
                return None
            return hmac.new(
                secret.encode(), msg=data, digestmod=hashlib.sha256
            ).hexdigest()
    ...
    def _razorpay_refresh_access_token(self):
        ...
        if response_content.get("access_token"):
            expiry = fields.Datetime.now() + timedelta(
                seconds=int(response_content["expires_in"])
            )
            self.write(
                {
    ...
    def _prepare_request_url(
        self, endpoint, *, api_version="v1", is_proxy_request=False, **kwargs
    ):
        if self.code != "razorpay":
    ...
    def _prepare_request_headers(self, *args, is_proxy_request=False, **kwargs):
        ...
        headers = None
        if (
            not is_proxy_request
            and self.razorpay_access_token
            and not self.razorpay_key_id
        ):
