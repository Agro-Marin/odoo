...
class PaymentTransaction(models.Model):
    ...
    def _razorpay_create_customer(self):
        ...
        payload = {
            ...
            "contact": (
                self.partner_phone and self._format_phone_number(self.partner_phone)
            )
            or "",
    ...
    def _razorpay_prepare_order_payload(self, customer_id=None):
        ...
        payload = {
            ...
            **(
                {"method": pm_code}
                if pm_code not in const.FALLBACK_PAYMENT_METHOD_CODES
                else {}
            ),
        }
        if self.operation in ["online_direct", "validation"]:
            payload["customer_id"] = (
                customer_id  # Required for only non-subsequent payments.
            )
            if self.tokenize:
                payload["token"] = {
        ...
        if (
            self.provider_id.capture_manually
        ):  # The related payment must be only authorized.
            payload.update(
                {
                    "payment": {
                        "capture": "manual",
    ...
    def _razorpay_get_mandate_max_amount(self):
        ...
        if "amount" in mandate_values and "MRR" in mandate_values:
            max_amount = min(
                pm_max_amount,
                max(mandate_values["amount"] * 1.5, mandate_values["MRR"] * 5),
            )
        else:
    ...
    def _send_void_request(self):
        ...
        raise UserError(
            _("Transactions processed by Razorpay can't be manually voided from Odoo.")
        )
    ...
    def _search_by_reference(self, provider_code, payment_data):
        ...
        if entity_type == "payment":
            ...
            if not reference:
                ...
                return tx
            tx = self.search(
                [("reference", "=", reference), ("provider_code", "=", "razorpay")]
            )
        else:  # 'refund'
    ...
    def _apply_updates(self, payment_data):
        ...
        if (
            "id" in payment_data
        ):  # We have the full entity data (S2S request or webhook).
    ...
    def _extract_token_values(self, payment_data):
        ...
        if (
            self.token_id
            or not self.provider_id.allow_tokenization
            or not has_token_data
        ):
            return {}
