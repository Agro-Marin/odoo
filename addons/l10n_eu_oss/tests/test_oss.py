...
class TestOSSBelgium(AccountTestInvoicingCommon):
    ...
    def test_oss_tax_should_be_instantiated_on_root_company_if_no_tax_id_on_sub_branch(
        self,
    ):
        self.sub_child_company._map_eu_taxes()
        # simulate sub child selection in the switcher
        self.env.user.company_id, self.env.user.company_ids = (
            self.sub_child_company,
            self.sub_child_company,
        )

        another_eu_country_code = (
            self.env.ref("base.europe").country_ids - self.sub_child_company.country_id
        )[0].code
        tax_oss = self.env["account.tax"].search(
            [("name", "ilike", f'%"{another_eu_country_code}"%')], limit=1
        )
    ...
    def test_country_tag_from_belgium(self):
        ...
        another_eu_country_code = (
            ...
            - self.company_data["company"].country_id
        )[0].code
        tax_oss = self.env["account.tax"].search(
            [("name", "ilike", f'%"{another_eu_country_code}"%')], limit=1
        )
        ...
        for doc_type, report_expression_xml_id in (
            ...
            with self.subTest(
                doc_type=doc_type, report_expression_xml_id=report_expression_xml_id
            ):
                oss_tag_id = (
                    tax_oss[f"{doc_type}_repartition_line_ids"]
                    .filtered(lambda x: x.repartition_type == "base")
                    .tag_ids
                )

                expected_tag_id = self.env.ref(
                    report_expression_xml_id
                )._get_matching_tags()

                self.assertIn(
                    expected_tag_id,
                    oss_tag_id,
                    f"{doc_type} tag from Belgian CoA not correctly linked",
                )
    ...
    def test_oss_tax_copied_name(self):
        ...
        fpos = self.env["account.fiscal.position"].search(
            [("country_id", "=", another_eu_country.id)], limit=1
        )
        ...
        for doc_type, tag_xml_id in (
            ...
            with self.subTest(doc_type=doc_type, report_line_xml_id=tag_xml_id):
                ...
                self.assertIn(
                    expected_tag_id,
                    oss_tag_id,
                    f"{doc_type} tag from Spanish CoA not correctly linked",
                )
...
class TestOSSUSA(AccountTestInvoicingCommon):
    ...
    def test_oss_tax_on_eu_branch(self):
        ...
        if not self.env["ir.module.module"].search_count(
            [("name", "=", "l10n_be"), ("state", "=", "installed")], limit=1
        ):
            self.skipTest(
        ...
        self.sub_child_company.tax_config_id.account_fiscal_country_id = (
        ...
        for chart_template, template_vals in chart_templates.items():
            ...
            for tax_report_line_xml_id in filter(lambda d: d, oss_tags.values()):
                with self.subTest(
                    ...
                    tax_report_line_xml_id=tax_report_line_xml_id,
                ):
