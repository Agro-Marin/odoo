...
class ResCompany(models.Model):
    ...
    def _map_all_eu_companies_taxes(self):
        ...
        companies = self.search(
            [("tax_config_id.account_fiscal_country_id", "in", eu_countries.ids)]
        )
    ...
    def _map_eu_taxes(self):
        ...
        for company in self:
            ...
            company = (
                company.parent_ids.filtered(lambda c: c.vat)[-1:] or company.root_id
            )
                ...
                for domestic_tax in taxes:
                    ...
                    if tax_amount and domestic_tax not in fpos.tax_ids.original_tax_ids:
                        if not foreign_taxes.get(tax_amount):
                            oss_tax_group_local_xml_id = f"{company.id}_oss_tax_group_{str(tax_amount).replace('.', '_')}_{company.tax_config_id.account_fiscal_country_id.code}"
                            if not self.env.ref(
                                f"account.{oss_tax_group_local_xml_id}",
                                raise_if_not_found=False,
                            ):
                                if not default_oss_payable_account:
                                    default_oss_payable_account = self.env[
                                        "account.account"
                                    ].create(
                                        [
                                            {
                                                ...
                                                "code": self.env[
                                                    ...
                                                    tg.tax_payable_account_id.with_company(
                                                        company
                                                    ).code
                                                ),
                                    ...
                                    default_oss_receivable_account = self.env[
                                        "account.account"
                                    ].create(
                                        [
                                            {
                                                ...
                                                "code": self.env[
                                                    ...
                                                    tg.tax_receivable_account_id.with_company(
                                                        company
                                                    ).code
                                                ),
                                ...
                                self.env["ir.model.data"].create(
                                    {
                                        ...
                                        "module": "account",
                                        "model": "account.tax.group",
                                        "res_id": self.env["account.tax.group"]
                                        .create(
                                            {
                            ...
                            extra_fields = (
                                company._get_fields_country_specific_account_tax()
                            )
    ...
    def _get_repartition_lines_oss(self):
        ...
        for doc_type, rep_type in product(("invoice", "refund"), ("base", "tax")):
            vals = {
                ...
                "tag_ids": [
                    Command.link(tag.id)
                    for tag in oss_tags[f"{doc_type}_{rep_type}_tag"]
                ],
            }
            if oss_account:
    ...
    def _get_oss_account(self):
        self.check_singleton()
        if not (
            oss_account := self.env.ref(
                f"l10n_eu_oss.oss_tax_account_company_{self.id}",
                raise_if_not_found=False,
            )
        ):
    ...
    def _create_oss_account(self):
        if self.account_config_id.chart_template in EU_ACCOUNT_MAP and (
            ...
            new_code = (
                ...
                .with_company(self)
                ._search_new_account_code(sales_tax_accounts[0].with_company(self).code)
            )
    ...
    def _get_oss_tags(self):
        ...
        chart_template = self.env["account.chart.template"]._guess_chart_template(
            country
        )
        ...
        is_coa_module_installed = self.env[
            "account.chart.template"
        ]._get_chart_template_mapping()[chart_template]["installed"]
        if not is_coa_module_installed:
            chart_template = None

        tag_for_country = EU_TAG_MAP.get(
            chart_template,
            {
        ...
        if self.vat:
            ...
            if country_prefix:
                country = self.env["res.country"].search(
                    [("code", "=", country_prefix)], limit=1
                )
        # otherwise fallback on the fiscal country
        if not country:
            country = self.tax_config_id.account_fiscal_country_id
        return country
