...
export class BuilderList extends Component {
    ...
    /**
     * Brings the list back in line with the records that exist now, keeping
     * whatever the user had already customised on the ones still there.
     */
    updateRecords() {
        ...
        const newRecords = this.allRecords
            .map((record) => selectedRecordsById.get(record.id) || record)
