...
class LayoutColumnOptionPlugin extends Plugin {
    ...
    resources = {
        ...
        selection_placeholder_container_predicates: (container) => {
            if (
                ...
                container.parentElement.classList.contains("row")
            ) {
                return true;
            }
        },
    ...
    onCloned({ cloneEl }) {
        ...
        const offsetRegex = new RegExp(
            `^offset-(${this.config.mobileBreakpoint}-)?([0-9]{1,2})$`,
        );
        const offsetClasses = [...cloneElClassList].filter((cls) =>
            cls.match(offsetRegex),
        );
...
export class ChangeColumnCountAction extends BuilderAction {
    ...
    _resizeColumns(columnEls, nbColumns) {
        ...
        const replacingRegex =
            ...
            new RegExp(
                `(?:^|\\s+)(col|offset)${resolutionModifier}(-\\d{1,2})?(?!\\S)`,
                "g",
            );
        ...
        for (const columnEl of columnEls) {
            ...
            if (firstItemOffset && columnEl === firstItem) {
                columnEl.classList.add(
                    `offset${resolutionModifier}-${firstItemOffset}`,
                );
            }
            const hasMobileOffset = columnEl.className.match(
                /(^|\s+)offset-\d{1,2}(?!\S)/,
            );
...
registry
    ...
    .add(LayoutColumnOptionPlugin.id, LayoutColumnOptionPlugin);
