...
class AccountMove(models.Model):
    ...
    def _get_domain_l10n_latam_documents(self):
        ...
        if (
            self.journal_id.company_id.tax_config_id.account_fiscal_country_id
        ...
        if self.journal_id.type == "sale":
            ...
            if self.move_type in ["in_invoice", "out_invoice"]:
                domain += [
                    ("internal_type", "in", ["invoice", "debit_note", "invoice_in"])
                ]
        ...
        if (
            ...
            and self.partner_id_vat != "60805000-0"
        ):
            ...
            self.partner_id.country_id.code != "CL"
    ...
    def _check_document_types_post(self):
        for rec in self.filtered(
            lambda r: (
                r.company_id.tax_config_id.account_fiscal_country_id.code == "CL"
            ...
            if (
                ...
                and tax_payer_type == "4"
                and country_id.code != "CL"
                and latam_document_type_code == "61"
                and "46"
                in rec.l10n_cl_reference_ids.mapped(
    ...
    def _get_starting_sequence(self):
        ...
        if (
            ...
            and self.company_id.tax_config_id.account_fiscal_country_id.code == "CL"
        ):
    ...
    def _uses_cl_documents(self):
        return (
            self.company_id.tax_config_id.account_fiscal_country_id.code == "CL"
    ...
    def _l10n_cl_get_invoice_totals_for_report(self):
        ...
        tax_group_ids_to_exclude = (
            ...
            .filtered(lambda x: x.l10n_cl_sii_code == 14)
            .ids
        )
        if tax_group_ids_to_exclude:
            return self.env["account.tax"]._exclude_tax_groups_from_tax_totals_summary(
    ...
    def _l10n_cl_include_sii(self):
        self.check_singleton()
        return self.l10n_latam_document_type_id.code in [
            "39",
            "41",
            "110",
            "111",
    ...
    def _is_manual_document_number(self):
        if self.journal_id.company_id.country_id.code == "CL":
            return (
                ...
                and not self.l10n_latam_document_type_id._is_doc_type_vendor()
            )
    ...
    def _l10n_cl_get_amounts(self):
        ...
        """
        self.check_singleton()
        global_discounts = self.invoice_line_ids.filtered(
            lambda x: x.price_subtotal < 0
        )
        ...
        currency_round_main_currency = (
            self.currency_id if export else self.company_id.currency_id
        )
        currency_round_other_currency = (
            self.company_id.currency_id if export else self.currency_id
        )
        total_amount_main_currency = (
            currency_round_main_currency.round(self.amount_total)
            if export
        ...
        values = {
            ...
            "main_currency_name": self._l10n_cl_normalize_currency_name(
                currency_round_main_currency.name
            )
            if export
            else False,
        }
        ...
        if other_currency:
            ...
            values["second_currency"] = {
                ...
                "subtotal_amount_exempt": 0,
                "vat_amount": 0,
                "total_amount": currency_round_other_currency.round(
                    abs(self.amount_total_signed)
                )
                if export
                else currency_round_other_currency.round(self.amount_total),
                "round_currency": currency_round_other_currency.decimal_places,
                "name": self._l10n_cl_normalize_currency_name(
                    currency_round_other_currency.name
                ),
        ...
        for line in self.line_ids:
            if line.tax_line_id and line.tax_line_id.l10n_cl_sii_code == 14:
                ...
                if other_currency:
                    values["second_currency"]["vat_amount"] += (
                        line[key_other_currency] * sign_main_currency
                    )
                vat_percent = max(vat_percent, line.tax_line_id.amount)
            if line.display_type == "product":
                if line.tax_ids.filtered(lambda x: x.l10n_cl_sii_code == 14):
                    values["subtotal_amount_taxable"] += (
                        line[key_main_currency] * sign_main_currency
                    )
                    if other_currency:
                        values["second_currency"]["subtotal_amount_taxable"] += (
                            line[key_other_currency] * sign_main_currency
                        )
                elif not line.tax_ids:
                    values["subtotal_amount_exempt"] += (
                        line[key_main_currency] * sign_main_currency
                    )
                    if other_currency:
                        values["second_currency"]["subtotal_amount_exempt"] += (
                            line[key_other_currency] * sign_main_currency
                        )
        values["global_discounts"] = []
        for gd in global_discounts:
            main_value = (
                currency_round_main_currency.round(abs(gd.price_subtotal))
                if (not other_currency and not export) or (other_currency and export)
                else currency_round_main_currency.round(abs(gd.balance))
            )
            second_value = (
                currency_round_other_currency.round(abs(gd.balance))
                if other_currency and export
                else currency_round_other_currency.round(abs(gd.price_subtotal))
            )
            values["global_discounts"].append(
                {
                    "name": gd.name,
                    "global_discount_main_value": main_value,
                    "global_discount_second_value": second_value
                    if second_value != main_value
                    else False,
                    "tax_ids": gd.tax_ids,
                }
            )
        ...
        return [
            {
                ...
                "tax_amount_currency": self.currency_id.round(
                    abs(line.amount_currency)
                ),
                "tax_amount": self.currency_id.round(abs(line.balance)),
            }
    ...
    def _compute_tax_totals(self):
        ...
        for move in self:
            if (
                move.tax_totals
                and move._get_name_invoice_report() == "l10n_cl.report_invoice_document"
            ):
