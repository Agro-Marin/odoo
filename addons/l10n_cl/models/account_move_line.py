...
class AccountMoveLine(models.Model):
    ...
    def _l10n_cl_get_line_amounts(self):
        ...
        line_sign = (
            self.price_subtotal / abs(self.price_subtotal) if self.price_subtotal else 0
        )
        domestic_invoice_other_currency = (
            self.move_id.currency_id != self.move_id.company_id.currency_id
            and not self.move_id.l10n_latam_document_type_id._is_doc_type_export()
        )
        ...
        if not export:
            ...
            second_currency_rate = (
                1 / self.move_id.invoice_currency_rate
                if self.move_id.invoice_currency_rate
                else 1
            )
            inverse_rate = (
                second_currency_rate
                if domestic_invoice_other_currency
                else main_currency_rate
            )
        else:
        ...
        if self.quantity and self.discount != 100.0:
            ...
            if self.move_id.l10n_latam_document_type_id._is_doc_type_electronic_ticket():
                price_item_document = (self.price_total / abs(self.quantity)) / (
                    1 - self.discount / 100
                )
        ...
        if domestic_invoice_other_currency or export:
            ...
            discount_amount_second = (
                price_unit_second * self.quantity - price_subtotal_second
            )
