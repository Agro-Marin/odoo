...
class AccountMoveReversal(models.TransientModel):
    ...
    def _compute_l10n_latam_manual_document_number(self):
        ...
        for wiz in self.filtered(
            ...
            wiz.l10n_latam_manual_document_number = (
    ...
    def _compute_document_type(self):
        for record in self.filtered(
            lambda x: (
                not x.l10n_latam_document_type_id
                or x.l10n_latam_document_type_id
                not in x.l10n_latam_available_document_type_ids
            )
        ):
            document_types = record.l10n_latam_available_document_type_ids._origin
            record.l10n_latam_document_type_id = (
                document_types[0] if document_types else False
            )
