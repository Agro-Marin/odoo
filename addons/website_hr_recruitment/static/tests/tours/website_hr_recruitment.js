...
registerWebsitePreviewTour(
    "website_hr_recruitment_tour_edit_form",
    {
        url: "/jobs",
    },
    () => [
        ...
        },
        {
            ...
            trigger: ":iframe form",
            run() {
                const departmentEl = this.anchor.querySelector(
                ...
                departmentEl
                    ...
                    .classList.remove("s_website_form_dnone");
            },
        },
        {
        ...
        },
        {
            ...
            trigger:
                ":iframe form input[name=job_id]:not(:visible):not([value='']):not([value=FAKE_JOB_ID_DEFAULT_VAL])",
        },
        {
            content: "Check that a department_id has been loaded",
            trigger:
                ':iframe input[name="department_id"][value="FAKE_DEPARTMENT_ID_DEFAULT_VAL"]',
            run: function () {
                if (this.anchor.value === "FAKE_DEPARTMENT_ID_DEFAULT_VAL") {
                    console.error(
                        "The department_id data-for should have been applied",
                    );
                }
            },
        },
        ...
        {
            content:
                "Verify that the department_id shown field has kept its default value",
            trigger:
                ':iframe form input[name="department_id"][value="FAKE_DEPARTMENT_ID_DEFAULT_VAL"]',
            run: function () {
                if (this.anchor.value !== "FAKE_DEPARTMENT_ID_DEFAULT_VAL") {
