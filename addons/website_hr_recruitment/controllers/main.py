...
class WebsiteHrRecruitment(WebsiteForm):
    ...
    @http.route(
        ...
        list_as_website_content=_lt("Jobs"),
    )
    def jobs(
        self,
        country_id=None,
        ...
        **kwargs,
        ...
        def job_filtering_condition(job, filter_to_disable=False):
            country_filter = (
                country if (country and filter_to_disable != "country_id") else None
            )
            field_filters = {
            ...
            if not all_fields or (
                country_filter
                and not (job.address_id and job.address_id.country_id == country)
            ):
                return False
            not_exist_filter = {
        ...
        def compute_filter_selection_counters(
            filtered_jobs, grouping_field, key_getter
        ):
        ...
        def get_filter_snippets_display_values(jobs):
            ...
            for field, alias in fields_and_filters:
                filtered_jobs = jobs.filtered(
                    partial(job_filtering_condition, filter_to_disable=field)
                )
            ...
            filtered_jobs = jobs.filtered(
                partial(job_filtering_condition, filter_to_disable="country_id")
            )
            counter_by_object_by_field["count_per_country"] = (
                compute_filter_selection_counters(
                    filtered_jobs,
                    "address_id",
                    lambda address_id: (
                        address_id.country_id
                        if address_id and address_id.country_id
                        else None
                    ),
                )
            )
        ...
        contract_type = (
            env["hr.contract.type"].browse(to_int(contract_type_id)).exists().sudo()
        )
        industry = (
            env["res.partner.industry"].browse(to_int(industry_id)).exists().sudo()
        )

        if (
            not (country or department or office or contract_type or all_countries)
    ...
    def check_recent_application(self, field, value, job_id):
        ...
        refused_applicants = applications_by_status.get(
            "refused", http.request.env["hr.applicant"]
        )
        if any(
            applicant
            for applicant in refused_applicants
            ...
            recruiter_contact = (
                ...
                if not ongoing_application.user_id
                    ...
                    contact_infos=", ".join(
                        [
                            value
                            for value in itemgetter("name", "email", "phone")(
                                ongoing_application.user_id
                            )
                            if value
                        ]
                    ),
    ...
    def extract_data(self, model_sudo, values):
        ...
        if short_introduction:
            ...
            data["custom"] = data["custom"].replace(
                "short_introduction", introduction_label
            )
        return data
