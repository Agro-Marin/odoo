...
class AccountMove(models.Model):
    ...
    l10n_in_show_gstin_status = fields.Boolean(
    ...
    def _compute_l10n_in_gst_treatment(self):
        for invoice in self.filtered(
            lambda m: m.country_code == "IN" and m.state == "draft"
        ):
            partner = invoice.partner_id
            invoice.l10n_in_gst_treatment = partner.l10n_in_gst_treatment or (
                "overseas"
    ...
    def _compute_l10n_in_state_id(self):
        ...
        for move in self:
            if move.country_code == "IN" and move.is_sale_document(
                ...
                partner_state = (
                    partner.state_id
                    or move.partner_id.commercial_partner_id.state_id
                    or move.company_id.state_id
                )
                country_code = partner_state.country_id.code or move.country_code
                move.l10n_in_state_id = (
                    partner_state if country_code == "IN" else foreign_state
                )
    ...
    def _compute_fiscal_position_id(self):
        ...
        def _get_fiscal_state(move):
            ...
            if (
                ...
                return (
                    move.l10n_in_state_id
                    if move.l10n_in_state_id.l10n_in_tin != "96"
                    else foreign_state
                )
            elif (
    ...
    def _compute_l10n_in_warning(self):
        ...
        for move in indian_invoice:
            ...
            if company.l10n_in_tcs_feature or company.l10n_in_tds_feature:
                    ...
                    warnings["lower_tcs_tax"] = {
                        ...
                        "actions": invalid_tax_lines.with_context(
                            ...
                            views=[
                                (
                                    _xmlid_to_res_id(
                                        "l10n_in.view_stock_move_line_list_hsn_l10n_in"
                                    ),
                                    "list",
                                )
                            ],
                ...
                if (
                    applicable_sections
                    := move._get_l10n_in_tds_tcs_applicable_sections()
                ):
                    ...
                    warnings["tds_tcs_threshold_alert"] = {
                        ...
                        "action": tds_tcs_applicable_lines.with_context(
                            ...
                            views=[
                                (
                                    _xmlid_to_res_id(
                                        "l10n_in.view_move_line_list_l10n_in_withholding"
                                    ),
                                    "list",
                                )
                            ],
    ...
    def _compute_l10n_in_partner_gstin_status_and_date(self):
        for move in self:
            if (
                ...
                move.l10n_in_partner_gstin_status = (
                    move.partner_id.l10n_in_gstin_verified_status
                )
                move.l10n_in_gstin_verified_date = (
                    move.partner_id.l10n_in_gstin_verified_date
                )
            else:
    ...
    def action_l10n_in_apply_higher_tax(self):
        ...
        for line in invalid_lines:
            ...
            if set(line.tax_ids.ids) != set(updated_tax_ids):
                line.write(
                    {"tax_ids": [Command.clear()] + [Command.set(updated_tax_ids)]}
                )
    ...
    def _get_l10n_in_invalid_tax_lines(self):
        ...
        if (
            self.country_code == "IN"
            and not self.commercial_partner_id.l10n_in_pan_entity_id
        ):
            lines = self.env["account.move.line"]
    ...
    def _get_sections_aggregate_sum_by_pan(self, section_alert, commercial_partner_id):
        ...
        company_fiscalyear_dates = self.company_id.sudo().compute_fiscalyear_dates(
            self.date
        )
        ...
        if commercial_partner_id.l10n_in_pan_entity_id:
            default_domain += [
                (
                    "move_id.commercial_partner_id.l10n_in_pan_entity_id",
                    "=",
                    commercial_partner_id.l10n_in_pan_entity_id.id,
                )
            ]
        else:
            default_domain += [
        ...
        for frequency, frequency_domain in frequency_domains.items():
            query = self.env["account.move.line"]._search(
                default_domain + frequency_domain, bypass_access=True, active_test=False
            )
    ...
    def _get_l10n_in_tds_tcs_applicable_sections(self):
        ...
        def _is_section_applicable(
            section_alert, threshold_sums, invoice_currency_rate, lines
        ):
            lines_total = sum(
                (line.price_total * invoice_currency_rate)
            ...
            if section_alert.is_aggregate_limit:
                aggregate_period_key = (
                    section_alert.consider_amount == "total_amount" and "price_total"
                ) or "balance"
                aggregate_total = threshold_sums.get(
                    section_alert.aggregate_period, {}
                ).get(aggregate_period_key)
                if self.state == "draft":
        ...
        if self.country_code == "IN" and self.move_type in [
            "in_invoice",
            "out_invoice",
        ]:
            warning = set()
            ...
            if commercial_partner_id.l10n_in_pan_entity_id.tds_deduction == "no":
                invoice_lines = self.invoice_line_ids.filtered(
                    lambda l: (
                        l.account_id.l10n_in_tds_tcs_section_id.tax_source_type != "tds"
                    )
                )
            else:
                ...
                if (
                    ...
                    and _is_section_applicable(
                        ...
                        self._get_sections_aggregate_sum_by_pan(
                            section_alert, commercial_partner_id
                        ),
    ...
    def l10n_in_verify_partner_gstin_status(self):
        ...
        return self.with_company(
            self.company_id
        ).partner_id.action_l10n_in_verify_gstin_status()
    ...
    def _can_be_unlinked(self):
        self.check_singleton()
        return (
            self.country_code != "IN" or not self.posted_before
        ) and super()._can_be_unlinked()
    ...
    def _generate_qr_code(self, silent_errors=False):
        ...
        if self.company_id.country_code == "IN" and self.company_id.l10n_in_upi_id:
            ...
            barcode = self.env["ir.actions.report"].prepare_barcode(
                barcode_type="QR", value=payment_url, width=120, height=120, quiet=False
            )
    ...
    def _l10n_in_get_hsn_summary_table(self):
        ...
        return self.env["account.tax"]._l10n_in_get_hsn_summary_table(
            base_lines, display_uom
        )
    ...
    def _l10n_in_prepare_tax_details(self):
        def l10n_in_grouping_key_generator(base_line, tax_data):
            ...
            tax = tax_data["tax"]
            if self.l10n_in_gst_treatment in (
                "overseas",
                "special_economic_zone",
            ) and all(
                ...
                if xmlid_to_res_id("l10n_in.tax_tag_cess") in tag_ids:
                    ...
                    for gst in ["cgst", "sgst", "igst"]:
                        if xmlid_to_res_id("l10n_in.tax_tag_%s" % (gst)) in tag_ids:
                            ...
                            line_code = (
                                f"{gst}_rc" if tax_data["is_reverse_charge"] else gst
                            )
            return {
                "tax": tax,
    ...
    def _get_l10n_in_seller_buyer_party(self):
        ...
        return {
            ...
            "dispatch_details": self._l10n_in_get_warehouse_address()
    ...
    def _get_l10n_in_tax_details_by_line_code(self, tax_details):
        ...
        for tax_detail in tax_details.values():
            ...
            l10n_in_tax_details.setdefault(
                "%s_rate" % (line_code), tax_detail["tax"].amount
            )
            ...
            l10n_in_tax_details["%s_amount_currency" % (line_code)] += tax_detail[
                "tax_amount_currency"
            ]
    ...
    def _get_l10n_in_invoice_label(self):
        ...
        if self.country_code != "IN" or not self.is_sale_document(
