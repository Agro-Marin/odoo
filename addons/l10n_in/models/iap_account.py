...
class IapAccount(models.Model):
    ...
    @api.model
    def _l10n_in_connect_to_server(
        self, is_production, params, url_path, config_parameter, timeout=25
    ):
        ...
        if params.get("gstin") == TEST_GST_NUMBER:
            ...
            default_endpoint = (
                DEFAULT_IAP_ENDPOINT if is_production else DEFAULT_IAP_TEST_ENDPOINT
            )
        endpoint = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param(config_parameter, default_endpoint)
        )
