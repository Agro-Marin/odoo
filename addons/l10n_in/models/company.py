...
class ResCompany(models.Model):
    ...
    @api.depends(
        "parent_id.l10n_in_tds_feature",
        "parent_id.l10n_in_tcs_feature",
        "parent_id.l10n_in_is_gst_registered",
    )
        ...
        for company in self:
            if company.parent_id:
                ...
                company.l10n_in_is_gst_registered = (
                    company.parent_id.l10n_in_is_gst_registered
                )
    ...
    def _update_l10n_in_is_gst_registered(self):
        for company in self:
            if company.country_code == "IN" and company.vat:
                company.l10n_in_is_gst_registered = company.partner_id.check_vat_in(
                    company.vat
                )
