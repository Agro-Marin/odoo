...
class AccountMoveLine(models.Model):
    ...
    def _compute_l10n_in_withhold_tax_amount(self):
        ...
        for line in withholding_lines:
            line.l10n_in_withhold_tax_amount = line.currency_id.round(
                abs(line.price_total - line.price_subtotal)
            )
    ...
    def _l10n_in_check_invalid_hsn_code(self):
        ...
        hsn_code = self.env["account.move"]._l10n_in_extract_digits(
    ...
    def _get_l10n_in_gstr_section(self, tax_tags_dict):
        ...
        def tags_have_categ(line_tax_tags, categories):
            return any(
                tag in line_tax_tags
                for category in categories
                for tag in tax_tags_dict.get(category, [])
            )
        ...
        def get_transaction_type(move):
            return (
                "intra_state"
                if move.l10n_in_state_id == move.company_id.state_id
                else "inter_state"
            )

        def is_reverse_charge_tax(line):
            return any(
                tax.l10n_in_reverse_charge for tax in line.tax_ids | line.tax_line_id
            )
        ...
        def get_sales_section(line):
            ...
            amt_limit = (
                100000
                if not line.invoice_date or line.invoice_date >= date(2024, 11, 1)
                else 250000
            )
            ...
            if gst_treatment in (
                "unregistered",
                "consumer",
            ) and not is_reverse_charge_tax(line):
                if (
                    transaction_type == "intra_state"
                    and tags_have_categ(line_tags, ["sgst", "cgst", "cess"])
                ) or (
                    ...
                    and (
                        ...
                        or (
                            ...
                            and move.debit_origin_id.amount_total <= amt_limit
                        )
                        or (
                            move.reversed_entry_id
                            and move.reversed_entry_id.amount_total <= amt_limit
                        )
                    )
                ):
                    return "sale_b2cs"
            ...
            if not line_tags or not tags_have_categ(
                line_tags, ["sgst", "cgst", "igst", "cess", "eco_9_5"]
            ):
            ...
            if is_inv:
                ...
                if (
                    gst_treatment in ("regular", "composition", "uin_holders")
                    and tags_have_categ(line_tags, ["sgst", "cgst", "igst", "cess"])
                    and not is_lut_tax(line)
                ):
                ...
                if not is_reverse_charge_tax(line):
                    ...
                    if gst_treatment == "overseas" and tags_have_categ(
                        line_tags, ["igst", "cess"]
                    ):
                        if is_lut_tax(line):
                            return "sale_exp_wop"
                    ...
                    if gst_treatment == "special_economic_zone" and tags_have_categ(
                        line_tags, ["igst", "cess"]
                    ):
                        if is_lut_tax(line):
                            return "sale_sez_wop"
        ...
        def get_purchase_section(line):
            ...
            if is_bill:
                ...
                if (
                    gst_treatment in ("unregistered", "consumer")
                    and tags_have_categ(line_tags, ["sgst", "cgst", "igst", "cess"])
                    and is_reverse_charge_tax(line)
                ):
                ...
                if (
                    ...
                    and any(
                        tax.tax_scope == "service"
                        for tax in line.tax_ids | line.tax_line_id
                    )
                    and tags_have_categ(line_tags, ["igst", "cess"])
                ):
