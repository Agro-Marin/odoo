...
class AccountJournal(models.Model):
    ...
    def _update_payment_channels(self, payment_type):
        ...
        lines_to_update = bank_journals[f"{payment_type}_payment_channel_ids"].filtered(
            lambda l: l.payment_method_id.code == "manual"
        )
        for company, lines in lines_to_update.grouped("company_id").items():
            if (
                account := self.env["account.chart.template"]
    ...
    def _compute_inbound_payment_channel_ids(self):
        super()._compute_inbound_payment_channel_ids()
        self._update_payment_channels("inbound")

    def _compute_outbound_payment_channel_ids(self):
        super()._compute_outbound_payment_channel_ids()
        self._update_payment_channels("outbound")
