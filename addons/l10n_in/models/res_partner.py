...
class ResPartner(models.Model):
    ...
    def _compute_display_pan_warning(self):
        for partner in self:
            partner.display_pan_warning = (
                partner.vat
                and partner.l10n_in_pan_entity_id
                and partner.l10n_in_pan_entity_id.name != partner.vat[2:12]
            )
    ...
    def _onchange_l10n_in_gst_status(self):
        ...
        """
        for partner in self:
            if partner.country_code == "IN" and (
                partner.l10n_in_gstin_verified_status
                or partner.l10n_in_gstin_verified_date
            ):
    ...
    def create(self, vals_list):
        ...
        if "import_file" in self.env.context:
            return res
        for partner in res.filtered(
            lambda p: p.country_code == "IN" and p.vat and p.check_vat_in(p.vat)
        ):
    ...
    def _set_l10n_in_pan_tan_from_vat(self):
        ...
        if pan.is_valid(identifier):
            self.l10n_in_pan_entity_id = (
                self._l10n_in_search_create_pan_entity_from_vat(self.vat).id
            )
    ...
    def _l10n_in_search_create_pan_entity_from_vat(self, vat):
        ...
        pan_entity = self.env["l10n_in.pan.entity"].search(
            [("name", "=", pan_number)], limit=1
        )
        if not pan_entity:
            context = clean_context(self.env.context)
            pan_entity = (
                self.env["l10n_in.pan.entity"]
                .with_context(context)
                .create({"name": pan_number})
            )
        return pan_entity
    ...
    def action_l10n_in_verify_gstin_status(self):
        ...
        if self.env.company.sudo().tax_config_id.account_fiscal_country_id.code != "IN":
            raise UserError(
                self.env._(
    ...
    def onchange_vat(self):
        if self.vat and self.check_vat_in(self.vat):
            ...
            state_id = self.env["res.country.state"].search(
                [("l10n_in_tin", "=", self.vat[:2])], limit=1
            )
            if state_id:
                self.state_id = state_id
            pan_entity = self.env["l10n_in.pan.entity"].search(
                [("name", "=", self.vat[2:12])], limit=1
            )
            if pan_entity:
        ...
        partner_data.update(
            {
                ...
                "is_company": True,
                "l10n_in_gst_treatment": partner_data.get(
