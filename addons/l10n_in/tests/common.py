...
class L10nInTestInvoicingCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.igst_sale_18_sez_exp_lut = AccountChartTemplate.ref(
            "igst_sale_18_sez_exp_lut"
        )
        cls.igst_sale_18_sez_exp = AccountChartTemplate.ref("igst_sale_18_sez_exp")
        cls.igst_sale_18_sez_exp_inc = cls.igst_sale_18_sez_exp.copy(
            {"price_include_override": "tax_included"}
        )
        cls.gst_with_cess = AccountChartTemplate.ref(
            "sgst_sale_12"
        ...
        # === Fiscal Positions === #
        cls.fp_in_intra_state = cls.env["account.chart.template"].ref(
            "fiscal_position_in_intra_state"
        )
        cls.fp_in_inter_state = cls.env["account.chart.template"].ref(
            "fiscal_position_in_inter_state"
        )
        cls.fp_in_export = cls.env["account.chart.template"].ref(
    ...
    def _set_vals_and_post(cls, move, ref=None, line_vals=None, post=True, irn=None):
        ...
        if line_vals:
            move.write(
                {
                    "invoice_line_ids": [
                        Command.update(line.id, line_vals) for line in move.line_ids
                    ]
                }
            )
    ...
    def _create_credit_note(
        cls, inv, ref=None, credit_note_date=None, line_vals=None, post=True
    ):
        move = inv._reverse_moves()
        move.invoice_date = credit_note_date or cls.test_date

        return cls._set_vals_and_post(
            move=move, ref=ref, line_vals=line_vals, post=post
        )
    ...
    def _create_debit_note(cls, inv, ref=None, debit_note_date=None, line_vals=None):
        ...
        return cls._set_vals_and_post(
            move=inv.debit_note_ids[0], ref=ref, line_vals=line_vals
        )
