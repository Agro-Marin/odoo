...
class TestL10nInHSNSummary(TestTaxCommon):
    ...
    def _create_py_sub_test_l10n_in_hsn_summary(self, document, display_uom):
        return {
            "hsn": self.env["account.tax"]._l10n_in_get_hsn_summary_table(
                document["lines"], display_uom
            ),
        }
                ...
                [
                    {
                        ...
                        "price_unit": 15.80,
                        "product_uom_id": self.uom_unit,
                        "tax_ids": self.gst_18 + self.cess_5_plus_1591,
                    },
                ]
            )
        )
                ...
                [
                    {
                        ...
                        "price_unit": 15.80,
                        "product_uom_id": self.uom_unit,
                        "tax_ids": self.igst_18 + self.cess_5_plus_1591,
                    },
                ]
            )
        )
    ...
    def test_l10n_in_hsn_summary_manual_edit_invoice_taxes(self):
        ...
        sgst_tax = self.gst_5.children_tax_ids.filtered(
            lambda tax: tax.l10n_in_gst_tax_type == "sgst"
        )
        cgst_tax = self.gst_5.children_tax_ids.filtered(
            lambda tax: tax.l10n_in_gst_tax_type == "cgst"
        )
        tax_line_sgst = invoice.line_ids.filtered(
            lambda aml: aml.tax_line_id == sgst_tax
        )
        tax_line_cgst = invoice.line_ids.filtered(
            lambda aml: aml.tax_line_id == cgst_tax
        )
        payment_term = invoice.line_ids.filtered(
            lambda aml: aml.display_type == "payment_term"
        )
        invoice.line_ids = [
            Command.update(
                tax_line_sgst.id,
                {"amount_currency": tax_line_sgst.amount_currency + 1.0},
            ),
            Command.update(
                tax_line_cgst.id,
                {"amount_currency": tax_line_cgst.amount_currency + 1.0},
            ),
            Command.update(
                payment_term.id, {"amount_currency": payment_term.amount_currency - 2.0}
            ),
        ]

        self.assert_invoice_l10n_in_hsn_summary(
            invoice,
            {
                "has_igst": False,
                "has_gst": True,
