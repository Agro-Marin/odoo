...
class TestFiscal(L10nInTestInvoicingCommon):
    ...
    def _assert_in_intra_state_fiscal_with_company(self, companies):
        for company in companies:
            ...
            self.assertRecordValues(
                ...
                .ref("fiscal_position_in_intra_state"),
                [
                    {
                        "name": name,
                        "state_ids": state.ids,
    ...
    def _assert_invoice_fiscal_position(
        self,
        fiscal_position_ref,
        partner,
        taxes=None,
        ...
        self.assertEqual(
            test_invoice.fiscal_position_id,
            self.env["account.chart.template"].ref(fiscal_position_ref),
        )
        return test_invoice
    ...
    def test_l10n_in_fiscal_in_bill_to_ship_to(self):
        with self._set_env_company(self.default_company):
            ...
            out_invoice.write(
                {
                    "l10n_in_state_id": self.env.ref(
                        "l10n_in.state_in_oc"
                    ).id,  # Other Country State
                }
            )
            self.assertEqual(
                out_invoice.fiscal_position_id,
                ...
                self.partner_a.write(
                    {
                        ...
                        "property_account_position_id": template.ref(
                            "fiscal_position_in_export_sez_in"
                        ).id,
                    }
                )
