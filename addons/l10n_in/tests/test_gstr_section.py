...
class TestGstrSection(L10nInTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.sez_partner = cls.partner_a.copy(
            {"l10n_in_gst_treatment": "special_economic_zone"}
        )
        cls.large_unregistered_partner = cls.partner_a.copy(
            {
    ...
    def test_gstr_sections(self):
        def assert_line_sections(lines, expected_sections):
            for line in lines.filtered(lambda l: l.display_type in ("product", "tax")):
                if line.display_type == "product":
                    ...
                    expected = (
                        expected_sections["with_gst_tag"]
                        if line.tax_tag_ids
                        else expected_sections["no_tag"]
                    )
        ...
        assert_line_sections(
            sez_invoice.line_ids,
            {
        ...
        b2cl_credit_note = self._create_credit_note(
            inv=b2cl_invoice, line_vals={"quantity": 0.5}
        )
        assert_line_sections(
            b2cl_credit_note.line_ids,
            {
