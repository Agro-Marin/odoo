...
class L10n_InWithholdWizard(models.TransientModel):
    ...
    def _compute_tds_deduction(self):
        for wizard in self:
            related_partner = (
                wizard.related_move_id.commercial_partner_id
                if wizard.related_move_id
                else wizard.related_payment_id.partner_id.commercial_partner_id
            )
            wizard.tds_deduction = (
                related_partner.l10n_in_pan_entity_id.tds_deduction
                if related_partner and related_partner.l10n_in_pan_entity_id
                else "higher"
            )
    ...
    def _compute_company_id(self):
        for wizard in self:
            wizard.company_id = (
                wizard.related_move_id.company_id
                or wizard.related_payment_id.company_id
            )
    ...
    def _compute_l10n_in_withholding_warning(self):
        for wizard in self:
            ...
            if (
                wizard.tax_id
                and wizard.l10n_in_tds_tax_type == "tds_purchase"
            ...
            if (
                wizard.related_move_id
                and float_compare(
                    wizard.related_move_id.amount_untaxed,
                    wizard.base,
                    ...
                    withhold_move_line = self.env["account.move.line"].search(
                        [
                            (
                                ...
                                "=",
                                wizard.related_move_id.commercial_partner_id.l10n_in_pan_entity_id.id,
                            ),
                            (
                                "move_id.l10n_in_withholding_ref_move_id.line_ids.account_id",
                                "in",
                                account.id,
                            ),
                            (
                                "move_id.l10n_in_withholding_ref_move_id.line_ids.l10n_in_tds_tcs_section_id",
                                "in",
                                section.id,
                            ),
    ...
    def _compute_base(self):
        for wizard in self:
            if (
                wizard.tax_id
                and wizard.related_move_id
                and wizard.tax_id.l10n_in_section_id
                not in (
                    self.env.ref("l10n_in.tds_section_194q"),
                    self.env.ref("l10n_in.tds_section_194n"),
                )
            ):
        ...
        for wizard in self:
            ...
            if wizard.tax_id:
                taxes_res = wizard.tax_id.compute_all(
                    wizard.base,
                    currency=wizard.env.company.currency_id,
                    quantity=1.0,
                    product=False,
                    partner=False,
                    is_refund=False,
    ...
    def _get_withhold_type(self):
        if self.related_move_id:
            ...
            withhold_type = (
                "in_withhold"
                if self.related_payment_id.partner_type == "supplier"
                else "out_withhold"
            )
    ...
    def _prepare_withhold_header(self):
        ...
        vals = {
            ...
            or self.related_payment_id.partner_id.id,
            "move_type": "entry",
            "ref": self.reference,
            "l10n_in_is_withholding": True,
            "l10n_in_withholding_ref_move_id": self.related_move_id.id
