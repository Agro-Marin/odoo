...
class TestUi(TestPointOfSaleHttpCommon):
    ...
    def test_spanish_pos(self):
        ...
        def get_number_of_regular_invoices():
            return self.env["account.move"].search_count(
                [
                    ("journal_id", "=", self.main_pos_config.invoice_journal_id.id),
        ...
        num_of_regular_invoices = (
            get_number_of_regular_invoices() - initial_number_of_regular_invoices
        )
    ...
    def test_l10n_es_pos_reconcile(self):
        ...
        self.main_pos_config.write(
            {
                "payment_method_ids": [
                    Command.link(self.customer_account_payment_method.id)
                ],
            }
        )
        ...
        self.start_tour(
            ...
            "l10n_es_pos_settle_account_due",
            login="accountman",
        )
    ...
    def test_spanish_pos_invoice_no_certificate(self):
        ...
        invoice_str = str(
            self.pos_order_pos0.account_move._get_invoice_legal_documents(
    ...
    def test_simplified_invoice_not_override_set_pricelist(self):
        ...
        self.main_pos_config.l10n_es_simplified_invoice_journal_id = (
            self.main_pos_config.journal_id
        )
        ...
        order = self.env["pos.order"].search(
            [("partner_id", "=", self.main_pos_config.simplified_partner_id.id)]
        )
        self.assertNotEqual(
            order.pricelist_id,
            self.main_pos_config.simplified_partner_id.property_product_pricelist,
        )
