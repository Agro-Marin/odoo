...
class TestSystray(HttpCase):
    ...
    def test_01_admin(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/test_model/1"),
            "test_systray_admin",
            login="admin",
        )
    ...
    def test_03_reditor_not_tester(self):
        self.user_test.group_ids |= self.group_restricted_editor
        self.user_test.group_ids = self.user_test.group_ids.filtered(
            lambda group: group != self.group_tester
        )
        self.assertNotIn(
            self.group_tester.id,
            self.user_test.group_ids.ids,
            "User should not be a group_tester",
        )
        self.start_tour(
            self.env["website"].get_client_action_url("/test_model/1"),
    ...
    def test_04_not_reditor_tester(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/test_model/1"),
            "test_systray_not_reditor_tester",
            login="testtest",
        )
    ...
    def test_05_not_reditor_not_tester(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/test_model/1"),
            "test_systray_not_reditor_not_tester",
            login="testtest",
        )
