...
class TestRedirect(HttpCase):
    ...
    def test_01_redirect_308_model_converter(self):
        ...
        self.assertTrue(
            country_ad.name in r.text,
            "Ensure the controller returned the expected value",
        )
        self.assertTrue(
            redirect_url in r.text,
            "Ensure the url_for has replaced the href URL in the DOM",
        )
        ...
        self.assertTrue(
            country_ad.name in r.text,
            "Ensure the controller returned the expected value (2)",
        )
        self.assertTrue(
            redirect_url in r.text,
            "Ensure the url_for has replaced the href URL in the DOM",
    ...
    def test_02_redirect_308_RequestUID(self):
        ...
        rec_published = self.env["test.model"].create(
        ...
        with patch.object(self.registry["ir.http"], "_get_error_html", _get_error_html):
            ...
            # published
            resp = self.url_open(
                f"/test_website/200/name-{rec_published.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertURLEqual(
                resp.headers.get("Location"),
                f"/test_website/308/name-{rec_published.id}",
            )

            resp = self.url_open(
                f"/test_website/308/name-{rec_published.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 200)

            resp = self.url_open(
                f"/test_website/200/xx-{rec_published.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertURLEqual(
                resp.headers.get("Location"), f"/test_website/308/xx-{rec_published.id}"
            )

            resp = self.url_open(
                f"/test_website/308/xx-{rec_published.id}", allow_redirects=False
            )
            ...
            resp = self.url_open(
                f"/test_website/200/xx-{rec_published.id}", allow_redirects=True
            )
            self.assertEqual(resp.status_code, 200)
            ...
            self.assertEqual(resp.status_code, 308)
            self.assertURLEqual(
                resp.headers.get("Location"), "/test_website/308/name-100"
            )
            ...
            self.assertEqual(resp.status_code, 308)
            self.assertURLEqual(
                resp.headers.get("Location"), "/test_website/308/xx-100"
            )
            ...
            # unpublish
            resp = self.url_open(
                f"/test_website/200/name-{rec_unpublished.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertURLEqual(
                resp.headers.get("Location"),
                f"/test_website/308/name-{rec_unpublished.id}",
            )

            resp = self.url_open(
                f"/test_website/308/name-{rec_unpublished.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 404)
            self.assertEqual(resp.text, "CUSTOM 404")

            resp = self.url_open(
                f"/test_website/200/xx-{rec_unpublished.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertURLEqual(
                resp.headers.get("Location"),
                f"/test_website/308/xx-{rec_unpublished.id}",
            )

            resp = self.url_open(
                f"/test_website/308/xx-{rec_unpublished.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 404)
            self.assertEqual(resp.text, "CUSTOM 404")
            ...
            resp = self.url_open(
                f"/test_website/200/seo-name-{rec_published.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertURLEqual(
                resp.headers.get("Location"),
                f"/test_website/308/seo-name-{rec_published.id}",
            )

            resp = self.url_open(
                f"/test_website/308/seo-name-{rec_published.id}", allow_redirects=False
            )
            self.assertEqual(resp.status_code, 200)
    ...
    def test_03_redirect_308_qs(self):
        ...
        self.assertEqual(r.status_code, 200)
        self.assertIn(
            'href="/empty_controller_test_redirected?a=a"',
            r.text,
    ...
    def test_redirect_308_multiple_url_endpoint(self):
        ...
        self.assertEqual(resp.status_code, 308)
        self.assertURLEqual(
            resp.headers.get("Location"), "/test_countries_308_redirected"
        )
