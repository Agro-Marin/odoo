...
class TestWebsiteControllerArgs(odoo.tests.HttpCase):
    ...
    def test_crawl_args(self):
        ...
        self.assertEqual(
            req.json(), {"a": "valueA", "b": "valueB", "kw": {"c": "valueC"}}
        )
        ...
        with patch.object(
            self.registry["ir.http"], "_get_error_html", lambda e, code, v: (code, "")
        ):
...
class TestWebsiteControllers(odoo.tests.TransactionCase):
    def test_01_sitemap(self):
        ...
        locs = website.with_user(website.user_id)._enumerate_pages(
            query_string="test_website_sitemap"
        )
