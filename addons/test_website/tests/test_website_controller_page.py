...
class TestWebsiteControllerPage(HttpCase):
    ...
    def setUpClass(cls):
        ...
        records_to_create = [
            dict(name=f"test_partner_{i}", **partner_data) for i in range(2)
        ]
    ...
    def test_access_rights_and_rules(self):
        ...
        with mute_logger("odoo.http"):
            response = self.url_open(
                f"/model/{self.listing_controller_page.name_slugified}"
            )
    ...
    def test_expose_model(self):
        ...
        rec_nodes = tree.xpath("//a[@class='test_record_listing']")
        self.assertEqual(len(rec_nodes), 2)
        for n, record in zip(rec_nodes, self.exposed_records, strict=True):
            self.assertEqual(
                n.get("href"),
                f"/model/{self.listing_controller_page.name_slugified}/{slug(record)}",
            )
        ...
        non_reachable_record = self.env[self.model.model].create(
            {"name": "non_reachable"}
        )
        response = self.url_open(
            f"/model/{self.listing_controller_page.name_slugified}/{slug(non_reachable_record)}"
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_search_listing(self):
        ...
        slug = self.env["ir.http"]._slug
        response = self.url_open(
            f"/model/{self.listing_controller_page.name_slugified}?search=1"
        )
        tree = html.fromstring(response.content.decode())
        rec_nodes = tree.xpath("//a[@class='test_record_listing']")
        self.assertEqual(len(rec_nodes), 1)
        self.assertEqual(
            rec_nodes[0].get("href"),
        ...
        response = self.url_open(
            f"/model/{self.listing_controller_page.name_slugified}/page/2"
        )
        tree = html.fromstring(response.content.decode())
        rec_nodes = tree.xpath("//a[@class='test_record_listing']")
        self.assertEqual(len(rec_nodes), 1)
        ...
        response = self.url_open(
            f"/model/{self.listing_controller_page.name_slugified}/page/2?search={self.exposed_records[0].name}"
        )
        self.assertEqual(
            response.url.partition("/model/")[2],
            f"exposed-model?search={self.exposed_records[0].name}&order=create_date+desc",
        )
        tree = html.fromstring(response.content.decode())
        rec_nodes = tree.xpath("//a[@class='test_record_listing']")
        self.assertEqual(len(rec_nodes), 1)
    ...
    def test_model_constrains(self):
        ...
        # Abstract Model
        self.assertEqual(
            get_model_meta_params("mixin.website.published"), (True, False, False)
        )
        with self.assertRaises(ValidationError) as cm:
            self.env["website.controller.page"].create(
                {**default_page_vals, "model": "mixin.website.published"}
            )
        self.assertEqual(
            str(cm.exception), "A page must be set to display a concrete model."
        )

        # Transient Model
        self.assertEqual(
            get_model_meta_params("base.partner.merge.automatic.wizard"),
            (False, True, True),
        )
        with self.assertRaises(ValidationError) as cm:
            self.env["website.controller.page"].create(
                {**default_page_vals, "model": "base.partner.merge.automatic.wizard"}
            )
        self.assertEqual(
            str(cm.exception), "A page must be set to display a concrete model."
        )
        ...
        with self.assertRaises(ValidationError) as cm:
            self.env["website.controller.page"].create(
                {**default_page_vals, "model": "res.device"}
            )
        self.assertEqual(
            str(cm.exception), "A page must be set to display a concrete model."
        )
