...
class TestIsMultiLang(odoo.tests.HttpCase):
    def test_01_is_multilang_url(self):
        ...
        for data in [None, {"post": True}]:  # GET / POST
            body = lxml.html.fromstring(
                self.url_open("/fr/multi_url", data=data).content
            )

            self.assertEqual(
                fr_prefix + "/get", body.find('./a[@id="get"]').get("href")
            )
            self.assertEqual(
                fr_prefix + "/post", body.find('./form[@id="post"]').get("action")
            )
            self.assertEqual(
                fr_prefix + "/get_post", body.find('./a[@id="get_post"]').get("href")
            )
            self.assertEqual(
    ...
    def test_02_url_lang_code_underscore(self):
        ...
        country1.update_field_translations(
            "name",
            {it.code: country1.name + " Italia", be.code: country1.name + " Belgium"},
        )

        r = self.url_open(f"/test_lang_url/{country1.id}")
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            urlparse(r.url).path, f"/test_lang_url/my-super-country-{country1.id}"
        )
        ...
        self.assertEqual(
            urlparse(r.url).path,
            f"/{it.url_code}/test_lang_url/my-super-country-italia-{country1.id}",
        )
        ...
        self.assertEqual(
            urlparse(it_href).path,
            f"/{it.url_code}/test_lang_url/my-super-country-italia-{country1.id}",
        )
        self.assertEqual(
            urlparse(fr_href).path,
            f"/{be.url_code}/test_lang_url/my-super-country-belgium-{country1.id}",
        )
        self.assertEqual(
            urlparse(en_href).path, f"/test_lang_url/my-super-country-{country1.id}"
        )
    ...
    def test_03_head_alternate_href(self):
        ...
        self.assertRegex(
            r.text, r'<link rel="alternate" hreflang="en" href="http://[^"]+/"/>'
        )
        r = self.url_open(be_prefix + "/contactus")
        self.assertRegex(
            r.text,
            r'<link rel="alternate" hreflang="en" href="http://[^"]+/contactus"/>',
        )
