...
class TestThemeAsset(TransactionCase):
    def test_theme_asset_tag(self):
        ...
        # Verify initial load
        self.assertEqual(
            prepend_asset._name, "theme.ir.asset", "Model should be theme.ir.asset"
        )
        self.assertEqual(
            prepend_asset.name, "Test asset tag with directive", "Name not loaded"
        )
        ...
        self.assertEqual(
            prepend_asset.path, "theme_default/tests/something.scss", "Path not loaded"
        )
        ...
        # Verify updated load
        self.assertEqual(
            prepend_asset.name, "Test asset tag with directive", "Name not restored"
        )
        self.assertEqual(prepend_asset.directive, "prepend", "Directive not restored")
        self.assertEqual(
            prepend_asset.bundle, "test_asset_bundle", "Bundle not restored"
        )
        self.assertEqual(
            prepend_asset.path,
            "theme_default/tests/something.scss",
            "Path not restored",
        )
