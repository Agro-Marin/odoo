...
class TestWebsitePageManager(HttpCaseWithWebsiteUser):
    def test_page_manager_test_model(self):
        ...
        self.env["test.model.multi.website"].create(
            {"name": "Test Model Multi Website 2", "website_id": website2.id}
        )
        self.assertTrue(
            len(
                {
                    t.website_id.id
                    for t in self.env["test.model.multi.website"].search([])
                }
            )
            >= 3,
        ...
        self.start_tour(
            "/odoo/action-test_website.action_test_model_multi_website",
            "test_website_page_manager",
            login="website_user",
        )
        ...
        self.start_tour(
            "/odoo/action-test_website.action_test_model_multi_website_js_class_bug",
            "test_website_page_manager_js_class_bug",
            login="website_user",
        )
        self.start_tour(
            "/odoo/action-test_website.action_test_model",
            "test_website_page_manager_no_website_id",
            login="website_user",
        )
