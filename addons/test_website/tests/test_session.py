...
class TestWebsiteSession(HttpCaseWithUserDemo):
    ...
    def test_02_inactive_session_lang(self):
        ...
        current_website_id = self.env["website"]._get_current_website_id(
            odoo.tests.HOST
        )
    ...
    def test_04_ensure_website_get_cached_values_can_be_called(self):
        ...
        with patch.object(
            Website,
            "_get_cached_values",
            side_effect=get_cached_values_without_cache,
            autospec=True,
        ):
    ...
    def test_branding_cache(self):
        ...
        self.user_demo.group_ids += self.env.ref(
            "website.group_website_restricted_editor"
        )
        self.user_demo.group_ids += self.env.ref(
            "test_website.group_test_website_admin"
        )
        ...
        self.opener.cookies.set(
            "session_id", public_session.sid, domain=odoo.tests.common.HOST
        )
        result = self.url_open(f"/test_website/model_item_sudo/{record.id}")
        self.assertFalse(
            has_branding(result.text), "Should have no branding for public user"
        )

        # Back to demo user.
        self.opener.cookies.set(
            "session_id", demo_session.sid, domain=odoo.tests.common.HOST
        )
        result = self.url_open(f"/test_website/model_item_sudo/{record.id}")
        self.assertTrue(has_branding(result.text), "Should have branding for user demo")
