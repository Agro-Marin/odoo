...
class StockRequestCount(models.TransientModel):
    ...
    def _compute_show_expected_quantity(self):
        show_quantity_count = (
            ...
            .sudo()
            .get_param("stock.show_expected_quantity_count", default="False")
            == "True"
        )
        for record in self:
    ...
    def _set_show_expected_quantity(self):
        for record in self:
            if record.show_expected_quantity:
                ...
                self.env["ir.config_parameter"].sudo().set_param(
                    "stock.show_expected_quantity_count", "False"
                )
    ...
    def _get_quants_to_count(self):
        ...
        if (
            not self.env.user.has_group("stock.group_production_lot")
