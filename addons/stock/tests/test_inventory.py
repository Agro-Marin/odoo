...
class TestInventory(TransactionCase):
    ...
    def test_inventory_1(self):
        ...
        self.assertEqual(
            len(self.env["stock.quant"]._gather(self.product1, self.stock_location)),
            1.0,
        )
        self.assertEqual(
    ...
    def test_inventory_4(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product2, self.stock_location, strict=True
            ),
            10.0,
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product2, self.stock_location
            ),
            11.0,
        )
        self.assertEqual(
            len(
                self.env["stock.quant"]
                ._gather(self.product2, self.stock_location, lot_id=lot1, strict=True)
        ...
        self.assertEqual(
            len(self.env["stock.quant"]._gather(self.product2, self.stock_location)),
            2.0,
        )
    ...
    def test_inventory_6(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._gather(self.product1, self.pack_location).quantity,
            10.0,
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product1, self.pack_location
            ),
            0.0,
        )
    ...
    def test_inventory_outdate_1(self):
        ...
        conflict_wizard_form = Form(
            self.env["stock.inventory.conflict"].with_context(
    ...
    def test_cyclic_inventory(self):
        ...
        no_cyclic_loc.company_id.write(
            {
                "annual_inventory_day": str(today.day),
                "annual_inventory_month": str(today.month),
            }
        )
        ...
        quant_existing_loc = self.env["stock.quant"].search(
        ...
        self.assertEqual(
            existing_loc2.next_inventory_date, date.today() + timedelta(days=2)
        )
        self.assertEqual(quant_new_loc.inventory_date, date.today() + timedelta(days=2))
        self.assertEqual(
            quant_existing_loc.inventory_date, date.today() + timedelta(days=2)
        )
        self.assertEqual(
