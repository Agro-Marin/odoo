...
class TestStockQuant(TestStockCommon):
    ...
    def test_get_available_quantity_7(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product_lot, self.stock_location, lot_id=lot1, allow_negative=True
            ),
            -10.0,
        )
    ...
    def test_get_available_quantity_8(self):
        ...
        self.assertEqual(
            len(self.gather_relevant(self.product_consu, self.stock_location)), 0
        )
        with self.assertRaises(ValidationError):
            self.env["stock.quant"]._update_available_quantity(
                self.product_consu, self.stock_location, 1.0
            )
    ...
    def test_increase_available_quantity_5(self):
        ...
        self.env["stock.quant"]._update_available_quantity(product2, self.shelf_1, 1.0)
        self.env["stock.quant"]._update_available_quantity(
            product2, self.stock_location, 1.0
        )

        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                product2, self.stock_location
            ),
            2.0,
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(product2, self.shelf_1), 1.0
        )
    ...
    def test_decrease_available_quantity_4(self):
        ...
        self.assertEqual(
            len(self.gather_relevant(self.productA, self.stock_location)), 0
        )
    ...
    def test_increase_reserved_quantity_3(self):
        ...
        self.assertEqual(
            len(self.gather_relevant(self.productA, self.stock_location)), 4
        )
        self.env["stock.quant"]._update_reserved_quantity(
            self.productA, self.stock_location, 10.0
        )
    ...
    def test_action_done_1(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productA, self.pack_location, 2.0
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
    ...
    def test_mix_tracked_untracked_1(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.product_serial, self.stock_location, lot_id=lot1
            ),
            2.0,
        )
    ...
    def test_access_rights_1(self):
        ...
        quant.with_user(self.user_stock_user).with_context(inventory_mode=True).write(
            {"quantity": 3.0}
        )
        with self.assertRaises(AccessError):
    ...
    def test_quant_in_date_5(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_lot, self.stock_location, 1.0, lot_id=lot1, in_date=in_date1
        )
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_lot, self.stock_location, 1.0, lot_id=lot1, in_date=in_date2
        )
    ...
    def test_quant_rounding(self):
        ...
        quants = (
            ...
            .with_context(packaging_uom_id=product.uom_id)
            ._get_reserve_quantity(product, self.stock_location, QUANTITY)
        )

        self.assertEqual(
            quants[0][1],
            QUANTITY,
            "Reserved Quantity should be rounded to UOM precision.",
        )
    ...
    def test_quant_creation(self):
        ...
        quant = self.gather_relevant(self.productA, self.shelf_2)
    ...
    def test_quant_display_name(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_lot, self.stock_location, 1.0, lot_id=lot1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product_serial, self.stock_location, 1.0, lot_id=sn1
        )
        ...
        for q in quants:
            if q.lot_id:
                self.assertEqual(
                    q.display_name,
                    "%s - %s" % (q.location_id.display_name, q.lot_id.name),
                )
            else:
    ...
    def test_serial_constraint_with_package_and_return(self):
        ...
        receipt01.move_line_ids.write({"lot_name": "Michel", "quantity": 1.0})
        ...
        wizard_form = Form(
            self.env["stock.return.picking"].with_context(
                active_ids=receipt01.ids,
                active_id=receipt01.ids[0],
        ...
        receipt02.move_line_ids.write({"lot_name": "Michel", "quantity": 1.0})
    ...
    def test_update_quant_with_forbidden_field(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product, self.shelf_2, 1.0, lot_id=sn1
        )
        self.assertEqual(len(product.stock_quant_ids), 1)
        self.env["stock.quant"]._update_available_quantity(
            product, self.shelf_1, 1.0, lot_id=sn1
        )
        ...
        with self.assertRaises(UserError):
            quant_2.with_context(inventory_mode=True).write(
                {"location_id": self.shelf_2}
            )
        ...
        self.assertRecordValues(
            product.stock_quant_ids.sorted("id"),
            [
                {
                    "location_id": self.shelf_2.id,
                    "quantity": 0.0,
    ...
    def test_update_quant_with_forbidden_field_02(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productA, self.stock_location, 1.0, package_id=package
        )
        quant = self.productA.stock_quant_ids
    ...
    def test_relocate(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productA, self.stock_location, 10, package_id=package_01
        )
        ...
        new_quant_a = self.env["stock.quant"].search(
            [("product_id", "=", self.productA.id), ("quantity", "=", 10)]
        )
        ...
        new_quant_a_bis = self.env["stock.quant"].search(
            [("product_id", "=", self.productA.id), ("quantity", "=", 10)]
        )
        ...
        self.env["stock.quant"]._update_available_quantity(
            product_b, self.stock_location, 10, package_id=package_01
        )
        product_c = self.env["product.product"].create(
            {"name": "product C", "is_storable": True}
        )
        self.env["stock.quant"]._update_available_quantity(
            product_c, self.stock_location, 10, package_id=package_01
        )

        quants_ab = self.env["stock.quant"].search(
            [
        ...
        relocate_wizard.save().action_relocate_quants()
        new_quants_abc = self.env["stock.quant"].search(
            [
                ("product_id", "in", (self.productA.id, product_b.id, product_c.id)),
                ("quantity", "=", 10),
            ],
            order="product_id",
        )
        ...
        company_B = self.env["res.company"].create(
            {"name": "company B", "currency_id": self.env.ref("base.USD").id}
        )
        ...
        self.env["stock.quant"]._update_available_quantity(
            product_a_company_B, location_company_B, 10, package_id=package_03
        )
        self.env["stock.quant"]._update_available_quantity(
        ...
        self.assertEqual(
            relocate_wizard.dest_package_id.search(
                literal_eval(relocate_wizard.dest_package_id_domain)
            ),
    ...
    def test_inventory_adjustment_package(self):
        ...
        creation_move_line = self.env["stock.move.line"].search(
            [("product_id", "=", dummy_product.id)]
        )
        self.assertEqual(
            creation_move_line.package_id.id, False, "There should be no origin package"
        )
        self.assertEqual(
            creation_move_line.result_package_id.id,
            dummy_package.id,
            "The destination package should be the dummy package",
        )
        self.assertEqual(
            creation_move_line.location_dest_id.id,
            self.stock_location.id,
            "The destination location should be the stock location",
        )

        dummy_quant.inventory_quantity = 0
        dummy_quant.action_apply_inventory()

        destruction_move_line = self.env["stock.move.line"].search(
            [("product_id", "=", dummy_product.id), ("id", "!=", creation_move_line.id)]
        )
        self.assertEqual(
            destruction_move_line.package_id.id,
            dummy_package.id,
            "The origin package should be the dummy package",
        )
        self.assertEqual(
            destruction_move_line.result_package_id.id,
            False,
            "The destination package should be False",
        )
        self.assertEqual(
            destruction_move_line.location_id.id,
            self.stock_location.id,
            "The origin location should be the stock location",
        )
        self.assertEqual(
            destruction_move_line.location_dest_id.id, creation_move_line.location_id.id
        )
    ...
    def test_quant_gs1_barcode(self):
        ...
        quants_product_serial_ean13 = quants.filtered(
            lambda q: q.product_id == product_serial_ean13
        )
        quants_product_lot_ean13 = quants.filtered(
        ...
        self.assertEqual(
            quants_product_serial_ean13[3]._get_gs1_barcode(),
            "010020002220202821tsn-003",
        )
        self.assertEqual(
            quants_product_lot_ean13[1]._get_gs1_barcode(),
    ...
    def test_quant_aggregate_barcode(self):
        ...
        quants_product_serial = quants.filtered(
            lambda q: q.product_id == product_serial
        )
    ...
    def test_unpack_and_quants_history(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product, stock_location, 5.0, lot_id=lot_a, package_id=package
        )
    ...
    def test_set_on_hand_quantity_tracked_product(self):
        ...
        quant_without_lot.with_context(
            {"inventory_mode": True}
...
class TestStockQuantRemovalStrategy(TestStockCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        least_package_strategy = cls.env["product.removal"].search(
        ...
        cls.product.categ_id.removal_strategy_id = least_package_strategy.id
        cls.picking_type_out.reservation_method = "at_confirm"
    ...
    def test_quant_cache_for_packages(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            products[1], self.stock_location, 1.0, package_id=packages[0]
        )
        self.env["stock.quant"]._update_available_quantity(
            products[2], self.stock_location, 1.0, package_id=packages[1]
        )
