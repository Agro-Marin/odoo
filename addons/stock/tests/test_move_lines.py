...
class TestStockMoveLine(TestStockCommon):
    ...
    def test_multi_edit_quant_and_lot(self):
        ...
        quant_productA = self.env["stock.quant"]._update_available_quantity(
            self.productA, self.shelf_1, 20, owner_id=self.partner
        )
        picking1 = self.env["stock.picking"].create(
            {
                "name": "Picking 1",
    ...
    def test_move_line_date(self):
        ...
        with freeze_time() as freeze:
            ...
            self.assertEqual(
                ml.date,
                create_date,
                "Increasing a quantity that isn't 'picked' shouldn't update its date",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ml.picked = True
            update_date_1 = ml.date
            self.assertTrue(
                update_date_1 > create_date,
                "Marking a ml as 'picked' should update its date",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ml.quantity = 3
            update_date_2 = ml.date
            self.assertTrue(
                update_date_2 > update_date_1,
                "Increasing a ml's quantity should update its date",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ml.product_uom_id = self.uom_dozen
            update_date_3 = ml.date
            self.assertTrue(
                update_date_3 > update_date_2,
                "Increasing a ml's quantity (via UoM type) should update its date",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
            ml.quantity = 2
            self.assertEqual(
                update_date_3,
                ml.date,
                "Decreasing a ml's quantity shouldn't update its date",
            )
            freeze.tick(delta=datetime.timedelta(seconds=2))
