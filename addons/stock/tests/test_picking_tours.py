...
class TestStockPickingTour(HttpCase):
    ...
    def test_generate_serial_2(self):
        ...
        lots_batch_1 = self.env["stock.lot"].search(
            [("name", "ilike", "lot_n_1_%"), ("product_id", "=", product_lot_1.id)],
            order="name asc",
        )
        lots_batch_2 = self.env["stock.lot"].search(
            [("name", "ilike", "lot_n_2_%"), ("product_id", "=", product_lot_1.id)],
            order="name asc",
        )
    ...
    def test_add_new_line_in_detailled_op(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product_lot, warehouse.lot_stock_id, quantity=10, lot_id=lot_1
        )
        self.env["stock.quant"]._update_available_quantity(
            product_lot, warehouse.lot_stock_id, quantity=15, lot_id=lot_2
        )
        self.env["stock.quant"]._update_available_quantity(
            product_lot, warehouse.lot_stock_id, quantity=10, lot_id=lot_3
        )
        ...
        self.start_tour(
            url, "test_add_new_line_in_detailled_op", login="admin", timeout=100
        )
        self.assertRecordValues(
            delivery.move_line_ids.sorted("quantity"),
            [
