...
class TestGenerateLotNames(TransactionCase):
    def _names(self, base, count):
        return self.env["stock.lot"].prepare_lot_names(base, count)
...
class StockGenerateCommon(TransactionCase):
    ...
    def test_receipt_import_lots(self):
        ...
        def_lot_id = self.env["stock.lot"].search(
            [("name", "=", "def"), ("product_id", "=", product_lot.id)]
        )
    ...
    def test_receipt_generate_serial_numbers(self):
        ...
        sn_t1_01 = self.env["stock.lot"].create(
            {"product_id": product_lot.id, "name": "sn-t1-01"}
        )
        sn_t1_02 = self.env["stock.lot"].create(
            {"product_id": product_lot.id, "name": "sn-t1-02"}
        )
        ...
        sn_t1_03, sn_t1_04, sn_t1_05 = self.env["stock.lot"].search(
            [
                ("name", "in", ["sn-t1-03", "sn-t1-04", "sn-t1-05"]),
                ("product_id", "=", product_lot.id),
            ]
        )
        self.assert_move_line_vals_values(
            ...
            [
                {
                    "quantity": 1,
                    "lot_id": {"id": sn_t1_01.id, "display_name": "sn-t1-01"},
                },
                {
                    "quantity": 1,
                    "lot_id": {"id": sn_t1_02.id, "display_name": "sn-t1-02"},
                },
                {
                    "quantity": 1,
                    "lot_id": {"id": sn_t1_03.id, "display_name": "sn-t1-03"},
                },
                {
                    "quantity": 1,
                    "lot_id": {"id": sn_t1_04.id, "display_name": "sn-t1-04"},
                },
                {
                    "quantity": 1,
                    "lot_id": {"id": sn_t1_05.id, "display_name": "sn-t1-05"},
                },
            ],
        )
        ...
        sn_t2_01, sn_t2_02, sn_t2_03, sn_t2_04, sn_t2_05 = self.env["stock.lot"].search(
            [
                (
                    "name",
                    "in",
                    ["sn-t2-01", "sn-t2-02", "sn-t2-03", "sn-t2-04", "sn-t2-05"],
                ),
                ("product_id", "=", product_lot.id),
            ]
        )
    ...
    def test_sequence_serial_numbers_access_rights(self):
        ...
        move_line_vals = (
            self.env["stock.move"]
            .with_user(inventory_user.id)
            .action_generate_lot_line_vals(
                action_context,
                "generate",
                self.product_serial.lot_sequence_id.next_by_id(),
                5,
                False,
            )
        )
        self.assert_move_line_vals_values(
            ...
            [
                {
                    "quantity": 1,
                    "lot_name": self.product_serial.lot_sequence_id.get_next_char(
                        first_num
                    ),
                },
                {
