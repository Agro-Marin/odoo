...
class TestMoveCreateWriteEdgeCases(TestStockCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
...
@tagged("post_install", "-at_install")
class TestMoveQuantityOnchange(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.product = cls.env["product.product"].create(
            {"name": "onchange quantity", "is_storable": True}
        )
        cls.env["stock.quant"]._update_available_quantity(
            cls.product, cls.stock_location, 20
        )
    ...
    def test_a_quantity_set_in_an_onchange_writes_no_move_line(self):
        MoveLine = self.env["stock.move.line"]
        rows_before = MoveLine.search_count([("move_id", "=", self.move.id)])
        draft = self.env["stock.move"].new(origin=self.move)

        draft._update_quantity_done(4)
        self.env.flush_all()

        self.assertEqual(
            MoveLine.search_count([("move_id", "=", self.move.id)]),
            rows_before,
            "an onchange must not persist move lines",
        )
        self.assertEqual(sum(draft.move_line_ids.mapped("quantity")), 4)

    def test_a_quantity_set_on_a_saved_move_still_creates_its_lines(self):
        self.move._update_quantity_done(4)

        self.assertEqual(sum(self.move.move_line_ids.mapped("quantity")), 4)
        self.assertTrue(all(line.id for line in self.move.move_line_ids))
