...
class TestStockMove(TestStockCommon):
    ...
    def test_edit_reserved_move_line_7(self):
        ...
        move_line.lot_id = lot1
        self.assertEqual(
            move_line.quantity_product_uom, 5
        )  # don't change reservation when assgning a lot now
        move1.picked = True
        move1._action_done()
    ...
    def test_edit_reserved_move_line_8(self):
        ...
        self.assertEqual(
            untracked_move_line.quantity_product_uom, 3
        )  # don't change reservation
        ...
        self.assertEqual(
            untracked_move_line.quantity_product_uom, 3
        )  # change reservation to 0 for done move
        self.assertEqual(
            tracked_move_line.quantity_product_uom, 2
        )  # change reservation to 0 for done move
        self.assertEqual(move1.state, "done")
    ...
    def test_move_line_aggregated_product_quantities(self):
        ...
        aggregate_val_2 = aggregate_values[
            f"{product2.id}_{product2.name}_Description2_{sm2.product_uom.id}_{sml2.move_id.packaging_uom_id.id}"
        ]
    ...
    def test_set_quantity_done_with_rounding_issues(self):
        ...
        self.assertEqual(
            delivery.move_ids.quantity, 149.97
        )  # 150 g -> 5.29109 oz -> 5.29 oz -> 149.97 g
