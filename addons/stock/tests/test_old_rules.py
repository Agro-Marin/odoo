...
class TestOldRules(TestStockCommon):
    ...
    def test_mtso_multi_pg(self):
        ...
        pickings_pg1 = self.env["stock.picking"].search(
            [("reference_ids", "in", reference_1.id)]
        )
        pickings_pg2 = self.env["stock.picking"].search(
            [("reference_ids", "in", reference_2.id)]
        )
        pickings_pg3 = self.env["stock.picking"].search(
    ...
    def test_2_steps_and_backorder(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productA, self.warehouse_2_steps.lot_stock_id, 1
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productB, self.warehouse_2_steps.lot_stock_id, 1
        )
        ...
        picking, _, _ = create_picking(
            delivery_type, delivery_type.default_location_src_id, customer_location
        )
    ...
    def test_pack_delivery_three_step_propagate_package_consumable_old(self):
        ...
        * The automatically generated internal transfer should have package set by default.
        ...
        picking.button_validate()
        self.assertEqual(
            pack_move.move_line_ids.result_package_id,
    ...
    def test_report_reception_4_pick_pack(self):
        ...
        self.assertEqual(
            ...
            1,
            "There should only be 1 line (pick move)",
        )
    ...
    def test_update_picking_origin(self):
        ...
        moves = self.env["stock.move"].create(
            [
                ...
                for product, origin in [
                    (self.productA, "origin1"),
                    (self.productA, "origin2"),
                    (self.productB, "origin2"),
                    (self.productB, "origin1"),
                ]
            ]
        )
    ...
    def test_propagate_cancel_in_pull_setup(self):
        ...
        pick_move = move_chain.filtered(
            lambda m: m.picking_type_id == self.warehouse_3_steps.pick_type_id
        )
