...
class TestReportStockQuantity(tests.TransactionCase):
    ...
    def test_inter_warehouse_transfer(self):
        ...
        for (date_day, warehouse, qty_rd), qty in zip(
            ...
            self.assertEqual(
                qty_rd,
                qty,
                f"Incorrect qty for Date '{date_day}' Warehouse '{warehouse.display_name}'",
            )
    ...
    def test_past_date_quantity_with_multistep_delivery(self):
        ...
        warehouse.resupply_route_ids.rule_ids.filtered(
            lambda r: r.location_src_id == transit_loc
        ).action = "push"
        product = self.env["product.product"].create(
