...
class StockWarehouseOrderpoint(models.Model):
            ...
            if (
                ...
                orderpoint.unwanted_replenish = (
                    orderpoint.product_uom.compare(
                        after_replenish_qty, orderpoint.product_max_qty
                    )
                    > 0
                )
    ...
    def _search_qty_to_order(self, operator, value):
        records = self.search_fetch(
            [("qty_to_order_manual", "in", [0, False])], ["qty_to_order_computed"]
        )
    ...
    def _get_qty_to_order(self, qty_in_progress_by_orderpoint=None):
        ...
        if (
            float_compare(
                self.qty_forecast, self.product_min_qty, precision_rounding=rounding
            )
            < 0
        ):
    ...
    def _get_orderpoint_action(self):
        ...
        for item in Move._read_group(
            domain_move_out, ["product_id", "location_id"], ["product_qty:sum"]
        ):
            moves_out[item[0]].append((item[1], item[2]))

        quants = defaultdict(list)
        for item in Quant._read_group(
            domain_quant, ["product_id", "location_id"], ["quantity:sum"]
        ):
            ...
            for product in all_product_ids:
                ...
                if (
                    product.uom_id.compare(
                        qty_available + incoming_qty - outgoing_qty, 0
                    )
                    < 0
                ):
                    ...
                    ploc_per_day[
                        lead_days["total_delay"] + lead_days["horizon_time"], loc
                    ].add(product.id)
        ...
        for (days, loc), prod_ids in ploc_per_day.items():
            ...
            qties = products.with_context(
                location=loc.id, to_date=today + relativedelta.relativedelta(days=days)
    ...
    def _prepare_procurement_vals(self, date=False):
        ...
        dates_info = self.product_id._get_dates_info(
            date_deadline, self.location_id, route_ids=self.route_id
        )
        values = {
            "route_ids": self.route_id,
    ...
    def _get_multiple_rounded_qty(self, qty_to_order):
        ...
        if replenishment_multiple and replenishment_multiple != self.product_id.uom_id:
            ...
            qty_to_order = self.product_id.uom_id._compute_quantity(
                qty_to_order, replenishment_multiple
            )
            qty_to_order = fields.Float.round(
                qty_to_order, precision_digits=0, rounding_method="UP"
            )
            qty_to_order = replenishment_multiple._compute_quantity(
                qty_to_order, self.product_id.uom_id
            )
        return qty_to_order
    ...
    def get_horizon_days(self):
        ...
        return self.env.context.get(
            "global_horizon_days", (self.company_id or self.env.company).horizon_days
        )
