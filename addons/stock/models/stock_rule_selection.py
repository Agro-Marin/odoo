...
class StockRuleSelection(models.Model):
    ...
    @api.model
    def _get_procurement_warehouse(self, values, default):
    ...
    def _get_rule_from_hierarchy(self, candidates, product_id, locations, values):
        ...
        for location in locations:
            ...
            for candidate_location in candidate_locations:
                warehouse_id = self._get_procurement_warehouse(
                    values, candidate_location.warehouse_id
                )
    ...
    def _get_rule(self, product_id, location_id, values):
        ...
        warehouse_ids = self._get_procurement_warehouse(values, locations.warehouse_id)
    ...
    def _get_rules_batch(self, procurements):
        ...
        for index, procurement in enumerate(procurements):
            ...
            warehouse_ids = self._get_procurement_warehouse(
                values, locations.warehouse_id
            )
            valid_route_ids = self._get_valid_route_ids(
                values.get("route_ids", False),
                values.get("packaging_uom_id", False),
        ...
        for group in groups.values():
            ...
            warehouse_ids = self._get_procurement_warehouse(
                representative.values, group[0][2].warehouse_id
            )
            candidates = self._get_rule_candidates(
