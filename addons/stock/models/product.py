...
class ProductProduct(models.Model):
    ...
    stock_quant_ids = fields.One2many(
        comodel_name="stock.quant", inverse_name="product_id"
    ...
    def _compute_quantities_dict(
        ...
        moves_in_res = {
            ...
            for product, product_qty in Move._read_group(
                domain_move_in_todo, ["product_id"], ["product_qty:sum"]
            )
        }
        moves_out_res = {
            product.id: product_qty
            for product, product_qty in Move._read_group(
                domain_move_out_todo, ["product_id"], ["product_qty:sum"]
            )
        }
        quants_res = {
        ...
        if self.env.context.get("with_expiration"):
            domain_quant += [
                ("removal_date", "<=", self.env.context["with_expiration"])
            ]
            expired_unreserved_quants_res = {
                product.id: quantity - reserved_quantity
                for product, quantity, reserved_quantity in Quant._read_group(
        ...
        if dates_in_the_past:
            ...
            for product, uom, quantity in Move._read_group(
                domain_move_in_done, groupby, ["quantity:sum"]
            ):
            ...
            for product, uom, quantity in Move._read_group(
                domain_move_out_done, groupby, ["quantity:sum"]
            ):
    ...
    def _get_domain_locations_new(self, location_ids) -> tuple[Domain, Domain, Domain]:
        ...
        if self.env.context.get("strict"):
            ...
            paths_domain = Domain.OR(
                Domain("parent_path", "=like", loc.parent_path + "%")
            ...
            dest_loc_domain_in_progress = Domain(
                [
                    ...
                    ("location_final_id", "any", paths_domain),
                    "&",
                    ("location_final_id", "=", False),
...
class ProductTemplate(models.Model):
    ...
    description_pickingout = fields.Text(
        string="Description on Delivery Orders", translate=True
    )
    description_pickingin = fields.Text(
        string="Description on Receptions", translate=True
    )
    qty_available = fields.Float(
        string="Quantity On Hand",
        digits="Product Unit",
...
class UomUom(models.Model):
    ...
    def _adjust_uom_quantities(self, qty, quant_uom):
        ...
        if get_param("stock.propagate_uom") != "1":
            ...
            computed_qty = self._compute_quantity(
                qty, procurement_uom, rounding_method="HALF-UP"
            )
