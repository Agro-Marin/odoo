...
class StockMoveLine(models.Model):
        ...
        for package, smls in groupby(self, lambda sml: sml.result_package_id):
            ...
            if package.package_type_id:
                best_loc = smls.move_id.location_dest_id.with_context(
                    exclude_sml_ids=excluded_smls, products=smls.product_id
        ...
        for move_line in mls:
            ...
            if move_line.quantity_product_uom and reservation:
                self.env.context.get(
                    "reserved_quant", self.env["stock.quant"]
    ...
    def write(self, vals):
        ...
        if updates or "quantity" in vals:
            ...
            for ml in mls:
                ...
                if ml.picking_id:
                    ml._log_message(
                        ml.picking_id, ml, "stock.track_move_template", vals
                    )
    ...
    def _create_and_assign_production_lot(self):
        ...
        for key, mls in key_to_mls.items():
            lot = lots[key_to_index[key]].with_prefetch(
                lots._ids
            )  # With prefetch to reconstruct the ones broke by accessing by index
    ...
    def _get_aggregated_product_quantities(self, **kwargs):
        ...
        for move_line in self:
            ...
            packaging_quantity = move_line.product_uom_id._compute_quantity(
            ...
            if line_key not in aggregated_move_lines:
                ...
                if backorders and not kwargs.get("strict"):
                    ...
                    packaging_qty_ordered = move_line.product_uom_id._compute_quantity(
    ...
    def _to_pack(self, without_pack=True):
        ...
        quantity_move_line_ids = self.filtered(
            ...
            and (without_pack != bool(ml.result_package_id))
    ...
    def action_put_in_pack(
        ...
        if done_pack and not self.env.context.get("force_move_lines"):
            ...
            packs_to_pack = lines_with_pack_to_pack.result_package_id.mapped(
            ...
            if packs_to_pack:
                return packs_to_pack.action_put_in_pack(
        ...
        if not done_pack:
            raise UserError(
                _(
                    "There is nothing eligible to put in a pack. Either there are no quantities to put in a pack or moves are already done."
                )
            )
