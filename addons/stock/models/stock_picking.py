...
from odoo.tools import format_date, format_datetime, groupby, OrderedSet, SQL
...
class StockPicking(models.Model):
    ...
    has_scrap_move = fields.Boolean(string="Has Scrap Moves", compute="_has_scrap_move")
    ...
    packages_count = fields.Integer(
        string="Packages Count", compute="_compute_packages_count"
    )
    package_history_ids = fields.Many2many(
        comodel_name="stock.package.history",
        string="Transfered Packages",
        copy=False,
    )
            ...
            if any(
                ...
                if forecast_date:
                    ...
                    picking.products_availability_state = (
                        "late"
                        if picking.date_planned
                        and picking.date_planned < forecast_date
                        else "expected"
                    )
            ...
            picking.json_popover = json.dumps(
                {
                    ...
                    "date_delay_alert": format_datetime(
                        self.env, picking.date_delay_alert, dt_format=False
                    ),
                    "late_elements": [
                        {
                            "id": late_move.id,
        ...
        for picking in self:
            ...
            if picking.move_type == "direct":
                picking.date_planned = min(
                    moves_dates, default=picking.date_planned or fields.Datetime.now()
                )
            else:
    ...
    def _compute_shipping_weight(self):
        for picking in self:
            ...
            relevant_packages = picking.move_line_ids.result_package_id.mapped(
        ...
        def _filter_picking_moves(picking):
            try:
                return picking.move_ids._match_searched_availability(
                    operator, value, _get_comparison_date
                )
            except UserError:
    ...
    def _log_activity_get_documents(
        self, orig_obj_changes, stream_field, stream, groupby_method=False
    ):
        ...
        if stream == "DOWN":
            if groupby_method:
                grouped_moves = groupby(
                    origin_objects.mapped(stream_field), key=groupby_method
                )
            else:
    ...
    def _get_autoprint_report_actions(self):
        ...
        if pickings_print_return_slip:
            action = self.env.ref("stock.return_label_report").report_action(
                pickings_print_return_slip.ids, config=False
            )
            clean_action(action, self.env)
            report_actions.append(action)
