...
class StockMove(models.Model):
    ...
    forecast_date_planned = fields.Datetime(
    ...
    display_assign_serial = fields.Boolean(
        compute="_compute_display_assign_serial",
    )
    display_import_lot = fields.Boolean(
        compute="_compute_display_assign_serial",
    )
    ...
    def default_get(self, fields_list):
        ...
        if self.env.context.get("default_picking_id"):
            ...
            if picking_id.state == "done":
                ...
                # to trigger `_autoconfirm_picking`
                defaults["additional"] = True
        return defaults
    ...
    @api.depends("picking_id", "picking_id.name", "name")
    ...
    def _compute_allowed_uom_ids(self):
        for move in self:
            move.allowed_uom_ids = (
                ...
                | move.product_id.seller_ids.product_uom_id
            )
    ...
    @api.depends("state", "move_line_ids.picked")
    ...
    def _compute_forecast_information(self):
        ...
        self.forecast_date_planned = False
        ...
        for warehouse, moves_ids in outgoing_unreserved_moves_per_warehouse.items():
            ...
            for location, mvs in moves_per_location.items():
                ...
                for move in mvs:
                    move.forecast_availability, move.forecast_date_planned = (
                        forecast_info[move]
                    )
    ...
    @api.depends("picking_id", "location_id", "location_dest_id", "product_id")
    ...
    def _set_quantity(self):
        def _process_decrease(move, quantity):
            ...
            for ml in reversed(move.move_line_ids.sorted("id")):
                ...
                if float_is_zero(
                    qty_ml_dec, precision_rounding=ml.product_uom_id.rounding
                ):
                    continue
...
Please change the quantity done or the rounding precision in your settings.""",
            ...
            if (
                float_compare(
                    delta_qty, 0, precision_rounding=move.product_uom.rounding
                )
                > 0
            ):
            ...
            if move._should_bypass_reservation():
                ...
                if (
                    ...
                    for (
                        ...
                        taken_quantity = move.with_context(
        ...
        for move in self:
            ...
            while location:
                ...
                rule = self.env["procurement.group"]._search_rule(
                    False, move.packaging_uom_id, product_id, move.warehouse_id, domain
                )
                if rule:
                    break
        ...
        def get_stock_moves(moves, state):
            if state == "available":
                return moves.filtered(
                    ...
                    and not m.forecast_date_planned
                )

            elif state == "expected":
                return moves.filtered(
                    lambda m: m.forecast_availability == m.product_qty
                    and m.forecast_date_planned
                    and m.forecast_date_planned <= get_comparison_date(m)
                )
    ...
    def _merge_moves(self, merge_into=False):
        ...
        neg_qty_moves = self.filtered(
            lambda m: float_compare(
                m.product_qty, 0.0, precision_rounding=m.product_uom.rounding
            )
            < 0
        )
    ...
    def _merge_move_itemgetter(self, distinct_fields, excluded_fields=None):
        field_names = [
            f_name
            for f_name in distinct_fields
            if f_name != "price_unit"
            and (excluded_fields is None or f_name not in excluded_fields)
        ]
        ...
        def _get_formatted_price_unit(move):
            ...
            rounded_price_unit = float_round(
                move.price_unit, precision_digits=price_precision
            )
    ...
    def _default_group_id(self):
        if self.env.context.get("default_picking_id"):
            return (
                ...
                .browse(self.env.context["default_picking_id"])
                .group_id.id
            )
    ...
    def _push_apply(self):
        ...
        for move in self:
            ...
            rule = ProcurementGroup._get_push_rule(
                move.product_id,
                move.location_dest_id,
                {
    ...
    def split_lots(self, lots):
        ...
        if not lots:
            # Skip if the `lot_name` doesn't contain multiple values.
            return []
    ...
    def _get_upstream_documents_and_responsibles(self, visited):
        if self.move_orig_ids and any(
            m.state not in ("done", "cancel") for m in self.move_orig_ids
        ):
            visited |= self
            return set(
    ...
    def _prepare_procurement_origin(self):
        ...
        return (
            self.group_id
            and self.group_id.name
            or (self.origin or self.picking_id.name or "/")
        )
    ...
    def _recompute_state(self):
        ...
        for move in self:
            ...
            if move.state in ("cancel", "done") or (
                ...
                and any(
                    float_compare(
                        orig.product_uom_qty,
                        0,
                        precision_rounding=orig.product_uom.rounding,
                    )
                    > 0
    ...
    def _set_quantity_done_prepare_vals(self, qty):
        ...
        for ml in self.move_line_ids:
            ...
            if (
                float_compare(ml_qty, 0, precision_rounding=ml.product_uom_id.rounding)
                <= 0
            ):
                continue
            ...
            # ml.qty_done and qty.
            taken_qty = float_round(
                taken_qty, precision_rounding=ml.product_uom_id.rounding
            )
