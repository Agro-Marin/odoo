...
Please change the quantity done or the rounding precision in your settings.""",
    ...
    def _compute_product_availability(self):
        ...
        # Batch: one _get_available_quantity call per unique (product, location)
        # instead of one per move. Multiple moves sharing the same product+location
        # get the same total_availability value.
    ...
    def _set_lot_ids(self):
        ...
        for move in self:
            ...
            free_uom_qty = move.product_uom._compute_quantity(
                max(move.quantity, move.product_uom_qty),
                product.uom_id,
            )
            ...
            for lot in move.lot_ids:
                ...
                if should_bypass_reservation:
                    if available_move_lines:
                        ...
                        new_vals = {
                            ...
                            "quantity": (
                                1.0
                                if move.product_id.tracking == "serial"
                                else move_line.quantity
                            ),
                        }
                        move_lines_commands.append(
                            Command.update(move_line.id, new_vals),
                        )
                        available_move_lines -= move_line
                        extra_uom_qty -= (
                            uom._compute_quantity(new_vals["quantity"], product.uom_id)
                            - 1
                        )
                    else:
                    ...
                    reserved = False
                    quants_to_reserve = quants_by_lot.get(
                        lot,
                        self.env["stock.quant"],
                    )
                    for quant in quants_to_reserve:
                        ...
                        if (
                            not quant.lot_id
                            or product.uom_id.compare(quant.available_quantity, 0.0)
                            <= 0
                        ):
                            continue
            ...
            if not should_bypass_reservation and available_move_line_ids:
                ...
                move_lines_commands.extend(
                    Command.delete(id) for id in available_move_line_ids
                )
    ...
    def _get_relevant_state_among_moves(self):
        ...
        if moves_todo[:1].picking_id and moves_todo[:1].picking_id.move_type == "one":
            ...
            if (
                most_important_move.state == "confirmed"
                or most_important_move.state == "partially_available"
            ):
                return "confirmed"
    ...
    def _onchange_lot_ids(self):
        ...
        if self._should_bypass_reservation():
            ...
            for lot, total_quantity, reserved_quantity in quant_by_lot:
                available_quantity_by_lot_name[lot.name] += product.uom_id._compute_quantity(
    ...
    def _get_product_catalog_lines_data(self, parent_record=False, **kwargs):
        ...
        return {
            **parent_record._get_product_price_and_data(self.product_id),
            ...
            "readOnly": len(self) > 1,
            "uomDisplayName": (len(self) == 1 and self.product_uom.display_name)
