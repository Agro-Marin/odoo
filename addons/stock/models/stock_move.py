...
from odoo.tools.misc import clean_context, groupby, OrderedSet
...
class StockMove(models.Model):
    ...
    move_line_ids = fields.One2many(
        comodel_name="stock.move.line", inverse_name="move_id"
    )
    ...
    show_operations = fields.Boolean(
        related="picking_id.picking_type_id.show_operations",
    )
    ...
    show_quant = fields.Boolean(string="Show Quant", compute="_compute_show_info")
    show_lots_m2o = fields.Boolean(string="Show lot_id", compute="_compute_show_info")
    show_lots_text = fields.Boolean(
        string="Show lot_name", compute="_compute_show_info"
    )
    ...
    def _compute_package_ids(self):
        for move in self:
            if move.state in ["done", "cancel"]:
                move.package_ids = (
                    move.move_line_ids.package_history_id.outermost_dest_id
                )
            else:
                # Only display the top-level packages until the move is done.
                move.package_ids = move.move_line_ids.result_package_id.mapped(
                    lambda p: p.outermost_package_id or p
                )
    ...
    def _set_quantity(self):
            ...
            for ml in reversed(move.move_line_ids.sorted("id")):
                ...
                qty_ml_dec = min(
                    ...
                    ml.product_uom_id._compute_quantity(
                        quantity, ml.product_uom_id, round=False
                    ),
                )
                ...
                quantity -= move.product_uom._compute_quantity(
                    qty_ml_dec, move.product_uom, round=False
                )
        ...
        for move in self:
            ...
            if (
                float_compare(
                    rounded_qty, move.quantity, precision_digits=precision_digits
                )
                != 0
            ):
                err.append(
...
Please change the quantity done or the rounding precision in your settings.""",
    ...
    def _compute_forecast_information(self):
        ...
        for move in product_moves:
            if (
                key_virtual_available(move) in virtual_available_dict
                and move.product_id.id
                in virtual_available_dict[key_virtual_available(move)]
            ):
                free_qty = virtual_available_dict[key_virtual_available(move)][
                    move.product_id.id
                ][1]
            else:
                free_qty = 0.0
    ...
    def _set_date_deadline(self, new_deadline):
        ...
        already_propagate_ids = self.env.context.get(
            "date_deadline_propagate_ids", set()
        )
        ...
        for move in self:
            ...
            for lot in move.lot_ids:
                if lot not in ls:
                    if mls_without_lots[
                        :1
                    ]:  # Updates an existing line without serial number.
    ...
    @api.depends(
        ...
        "move_orig_ids",
        "move_dest_ids",
        "move_orig_ids.packaging_uom_id",
        "move_dest_ids.packaging_uom_id",
    )
    ...
    def _compute_packaging_uom_qty(self):
        for move in self:
            if move.packaging_uom_id:
                move.packaging_uom_qty = move.product_uom._compute_quantity(
                    move.product_uom_qty, move.packaging_uom_id
                )
    ...
    def _generate_serial_numbers(
        ...
        if not count:
            raise ValidationError(
                _("The number of Serial Numbers to generate must be greater than zero.")
            )
        ...
        if self._can_create_lot():
            self._create_lot_ids_from_move_line_vals(
                field_data, self.product_id.id, self.company_id.id
            )
        ...
        for move in self:
            ...
            rule = StockRule._get_push_rule(
                ...
                move.location_dest_id,
                {
                    ...
                    | move.move_line_ids.result_package_id.package_type_id.route_ids,
    ...
    def _merge_move_itemgetter(self, distinct_fields, excluded_fields=None):
        ...
        def _get_formatted_float_fields(move, f_name, precision):
            ...
            rounded_value = float_round(
                move[f_name], precision_digits=precision[f_name]
            )
            return "{:.{precision}f}".format(rounded_value, precision=precision[f_name])

        return lambda move: base_getter(move) + tuple(
    ...
    def _merge_moves(self, merge_into=False):
        ...
        neg_qty_moves = self.filtered(
            lambda m: m.product_uom.compare(m.product_qty, 0.0) < 0
        )
        ...
        for candidate_moves in candidate_moves_set:
            ...
            for __, g in groupby(
                candidate_moves, key=self._merge_move_itemgetter(distinct_fields)
            ):
        ...
        for neg_move in neg_qty_moves:
            ...
            for pos_move in moves_by_neg_key.get(neg_key(neg_move), []):
                ...
                if (
                    pos_move.product_uom.compare(
                        pos_move.product_uom_qty, abs(neg_move.product_uom_qty)
                    )
                    >= 0
                ):
                    ...
                    pos_move.write(
                        {
                            "price_unit": (
                                float_round(
                                    new_total_value / pos_move.product_qty,
                                    precision_digits=price_unit_prec,
                                )
                                if pos_move.product_qty
                                else 0
                            ),
                            "move_dest_ids": [
                                Command.link(m.id)
                ...
                neg_move.price_unit = float_round(
                    new_total_value / neg_move.product_qty,
                    precision_digits=price_unit_prec,
                )
    ...
    def _onchange_lot_ids(self):
        quantity = sum(
            ...
            for ml in self.move_line_ids.filtered(
                lambda ml: not ml.lot_id and ml.lot_name
            )
        )
        quantity += self.product_id.uom_id._compute_quantity(
            len(self.lot_ids), self.product_uom
        )
        ...
        for _group, moves in grouped_moves:
            ...
            if picking:
                ...
                moves = moves.filtered(
                    lambda m: m.product_uom.compare(m.product_uom_qty, 0.0) >= 0
                )
                if not moves:
                    continue
                new_picking = True
    ...
    def _assign_picking_values(self, picking):
        ...
        if any(picking.origin != m.origin for m in self):
            current_origins = (
                set(picking.origin.split(",") + [False]) if picking.origin else {False}
            )
        ...
        for command_vals in field_data:
            ...
            # ... or create a new move line with the serial name.
                ...
                loc = loc_dest or self.location_dest_id._get_putaway_strategy(
                    self.product_id, quantity=quantity, additional_qty=qty_by_location
                )
                new_move_line_vals = {
                    **move_line_vals,
                    **command_vals,
    ...
    def _action_confirm(self, merge=True, merge_into=False, create_proc=True):
        ...
        move_to_confirm, move_waiting = self.browse(move_to_confirm).filtered(
        ...
        neg_r_moves = moves.filtered(
            lambda move: move.product_uom.compare(move.product_uom_qty, 0) < 0
        )

        # Push remaining quantities to next step
        neg_to_push = neg_r_moves.filtered(
            lambda move: move.location_final_id
        ...
        for move in neg_r_moves:
            ...
            for m in move.move_orig_ids | move.move_dest_ids:
                ...
                if to_loc == move.location_id:
                    ...
                    dest_move_ids += m.ids
            move.move_orig_ids, move.move_dest_ids = [Command.set(orig_move_ids)], [
        ...
        if new_push_moves:
            neg_push_moves = new_push_moves.filtered(
                lambda sm: sm.product_uom.compare(sm.product_uom_qty, 0) < 0
            )
    ...
    def _prepare_procurement_qty(self):
        ...
        for move in self:
            ...
            forecasted_qties_by_loc[move.location_id][move.product_id.id] -= min(
                move.product_qty, free_qty
            )
    ...
    def _prepare_procurement_values(self):
        ...
        if (
            ...
            dates_info = self.product_id._get_dates_info(
                self.date, self.location_id, route_ids=self.route_ids
            )
        ...
        if not self.location_id.warehouse_id:
            ...
            route = (
                self.route_ids
                or self.move_line_ids.result_package_id.package_type_id.route_ids
            )
        move_dest_ids = False
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        ...
        if quantity:
            ...
            if (
                ...
                vals = dict(
                    vals, quantity=quantity, product_uom_id=self.product_id.uom_id.id
                )
        package = None
        if reserved_quant:
    ...
    def _update_reserved_quantity_vals(
        ...
        for line in self.move_line_ids:
            if line.result_package_id or line.product_id.tracking == "serial":
                continue
            candidate_lines[
                line.location_id, line.lot_id, line.package_id, line.owner_id
            ] = line
        move_line_vals = []
        grouped_quants = {}
        ...
        for reserved_quant, quantity in grouped_quants.values():
            ...
            if to_update:
                ...
                uom_quantity = float_round(uom_quantity, precision_digits=rounding)
                uom_quantity_back_to_product_uom = (
                    to_update.product_uom_id._compute_quantity(
                        uom_quantity, self.product_id.uom_id, rounding_method="HALF-UP"
                    )
                )
            if (
    ...
    def _action_assign(self, force_qty=False):
        ...
        if not force_qty:
            moves_to_assign = moves_to_assign.filtered(
                lambda m: not m.picked
                and m.state in ["confirmed", "waiting", "partially_available"]
            )
        moves_mto = moves_to_assign.filtered(
        ...
        quants_cache = self.env["stock.quant"]._get_quants_by_products_locations(
            moves_mto.product_id, moves_mto.location_id
        )
        for move in moves_to_assign:
            ...
            if (
                float_compare(
                    missing_reserved_uom_quantity, 0, precision_rounding=rounding
                )
                <= 0
            ):
            ...
            if move._should_bypass_reservation():
                ...
                if move.product_uom.is_zero(move.product_uom_qty) and not force_qty:
                    ...
                    taken_quantity = move._update_reserved_quantity(
                        need, move.location_id, strict=False
                    )
                    ...
                    for (
                        ...
                        need = (
                            move.product_qty
                            - sum(move.move_line_ids.mapped("quantity_product_uom"))
                            - sum(taken_quantities.values())
                        )
                        move_line_vals, taken_quantity = (
                            move._update_reserved_quantity_vals(
                                min(quantity, need),
                                location_id,
                                lot_id,
                                package_id,
                        ...
                        taken_quantities[
                            need, location_id, lot_id, package_id, owner_id
                        ] = taken_quantity
    ...
    def _action_done(self, cancel_backorder=False):
        ...
        for move in moves:
            ...
            if (move.quantity <= 0 or not move.picked) and not move.is_inventory:
                if (
                    move.product_uom.compare(move.product_uom_qty, 0.0) == 0
                    or cancel_backorder
                ):
                    move._action_cancel()
    ...
    def _create_backorder(self):
        ...
        for move in self:
            ...
            if (
                float_compare(
                    move.quantity, move.product_uom_qty, precision_digits=rounding
                )
                < 0
            ):
    ...
    def _unlink_if_draft_or_cancel(self):
        if any(
            ...
            and (move.move_orig_ids or move.move_dest_ids)
            for move in self
        ):
    ...
    def _recompute_state(self):
        ...
        for move in self:
            ...
            if move.state in ("cancel", "done") or (
                ...
                move.move_orig_ids
                and any(
                    orig.product_uom.compare(orig.product_uom_qty, 0) > 0
    ...
    def _set_quantity_done_prepare_vals(self, qty):
        ...
        for ml in self.move_line_ids:
            ...
            if ml.product_uom_id != self.product_uom:
                taken_qty = ml.product_uom_id._compute_quantity(
                    taken_qty, self.product_uom, round=False
                )
            qty -= taken_qty
        ...
        if self.product_uom.compare(qty, 0.0) > 0:
            if self.product_id.tracking != "serial":
                ...
                uom_qty = self.product_uom._compute_quantity(
    ...
    def _trigger_assign(self):
        ...
        moves_to_reserve = moves_to_reserve.sorted(
            key=lambda m: any(r in self.reference_ids.ids for r in m.reference_ids.ids),
            reverse=True,
        )
    ...
    def _rollup_moves(self, origin=True, seen=False) -> OrderedSet[int]:
        ...
        if not unseen:
            return seen
        seen.update(unseen)
        self.filtered(lambda m: m.id in unseen)[target_field]._rollup_moves(
            origin, seen
        )
        return seen
        ...
        def get_stock_moves(moves, state):
            if state == "available":
                return moves.filtered(
                    lambda m: m.forecast_availability == m.product_qty
    ...
    def _get_product_catalog_lines_data(self, parent_record=False, **kwargs):
        ...
        return {
            **parent_record._get_product_price_and_data(self.product_id),
            ...
            and self.product_uom.display_name
