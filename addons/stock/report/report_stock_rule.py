...
class ReportStockReport_Stock_Rule(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        locations |= reordering_rules.mapped("location_id").filtered(
            lambda l: l not in locations
        )
        ...
        for location in locations:
            ...
            putaways = product.putaway_rule_ids.filtered(
                lambda p: p.location_in_id.id == location.id
            )
            if putaways or rr:
    ...
    def _get_routes(self, data):
        ...
        return (
            ...
            | warehouse_ids.mapped("route_ids")
        )
    ...
    def _sort_locations(self, rules_and_loc, warehouses):
        ...
        """
        all_src = self.env["stock.location"].concat(
            *([r["source"] for r in rules_and_loc])
        )
        all_dest = self.env["stock.location"].concat(
        ...
        locations = all_locations.filtered(
            lambda l: l.usage in ("supplier", "production")
        )
        for warehouse_id in warehouses:
            all_warehouse_locations = all_locations.filtered(
                lambda l: l.warehouse_id == warehouse_id
            )
            starting_rules = [
                d for d in rules_and_loc if d["source"] not in all_warehouse_locations
            ]
            if starting_rules:
                start_locations = self.env["stock.location"].concat(
                    *([r["destination"] for r in starting_rules])
                )
            else:
                starting_rules = [
                    d for d in rules_and_loc if d["source"] not in all_dest
                ]
                start_locations = self.env["stock.location"].concat(
                    *([r["source"] for r in starting_rules])
                )
            used_rules = self.env["stock.rule"]
            locations |= self._sort_locations_by_warehouse(
                rules_and_loc,
                used_rules,
                start_locations,
                ordered_locations,
            ...
            if any(location not in locations for location in all_warehouse_locations):
                remaining_locations = (
                    ...
                    .concat(*([r["source"] for r in rules_and_loc]))
                    .filtered(lambda l: l not in locations)
                )
                locations |= self._sort_locations_by_warehouse(
                    rules_and_loc,
                    used_rules,
    ...
    @api.model
    def _sort_locations_by_warehouse(
        self,
        rules_and_loc,
        used_rules,
        start_locations,
        ...
        start_locations = start_locations.filtered(
            lambda l: l.warehouse_id == warehouse_id
        )
        ...
        if rules_start:
            rules_start_dest_locations = self.env["stock.location"].concat(
                *([r["destination"] for r in rules_start])
            )
            remaining_rules = (
                self.env["stock.rule"].concat(*([r["rule"] for r in rules_and_loc]))
                - used_rules
            )
            ...
            start_locations = (
                rules_start_dest_locations
                - ordered_locations
                - remaining_rules_location
            )
            ordered_locations = self._sort_locations_by_warehouse(
                rules_and_loc,
                used_rules,
                start_locations,
                ordered_locations,
