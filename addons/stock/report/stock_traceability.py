...
class StockTraceabilityReport(models.TransientModel):
    ...
    def _get_move_lines(self, move_lines, line_id=None):
        ...
        while lines_todo:
            ...
            if move_line.move_id.move_orig_ids:
                lines = (
                    move_line.move_id.move_orig_ids.mapped("move_line_ids").filtered(
    ...
    def get_lines(self, line_id=False, **kw):
        ...
        if rec_id and model == "stock.lot":
            ...
            if model == "stock.picking":
                lines = record.move_ids.move_line_ids.filtered(
                    lambda m: m.lot_id and m.state == "done"
                )
            else:
                lines = record.move_finished_ids.mapped("move_line_ids").filtered(
    ...
    def _get_usage(self, move_line):
        ...
        if (move_line.location_id.usage == "internal") and (
            ...
            usage = "internal"
        elif (move_line.location_id.usage != "internal") and (
            move_line.location_dest_id.usage == "internal"
        ):
            usage = "in"
        else:
            ...
            lines.append(
                {
                    ...
                    "columns": [
                        ...
                        format_datetime(
                            self.env, data.get("date", False), tz=False, dt_format=False
                        ),
    ...
    @api.model
    def _lines(
        self, line_id=False, model_id=False, model=False, level=0, move_lines=None, **kw
    ):
        final_vals = []
        lines = move_lines or []
        ...
        for line in lines:
            unfoldable = False
            if line.consume_line_ids or (
                model != "stock.lot" and line.lot_id and self._get_move_lines(line)
            ):
                unfoldable = True
            final_vals += self._make_dict_move(
    ...
    def get_pdf(self, line_data=None):
        ...
        if context.get("active_id") and context.get("active_model"):
            rcontext["reference"] = (
                ...
                .browse(int(context.get("active_id")))
                .display_name
            )
