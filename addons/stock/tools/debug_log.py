"""Temporary debug instrumentation for the stock quality-improvement campaign.

Four loggers under ``odoo.addons.stock.debug``; enable one or all with
``--log-handler odoo.addons.stock.debug:DEBUG`` (or ``.logic``, ``.performance``,
``.pipeline``, ``.lifecycle``). Every call site imports this module as ``dbg``,
so ``grep -rn 'dbg\\.' addons/stock`` lists the whole instrumentation for removal.
...
- ``pipeline``: hand-offs between stages of a flow (procurement -> rule -> move ->
  picking; confirm -> assign -> done), tagged ``[<kind>:<ref>]`` for correlation.
...
def names(records, field="display_name"):
    return _Lazy(
        lambda: ",".join(str(value) for value in records.mapped(field)[:_MAX_IDS])
    )
