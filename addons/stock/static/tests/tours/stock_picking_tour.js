...
registry.category("web_tour.tours").add("test_generate_serial_1", {
    steps: () => [
        {
        ...
        },
        {
            ...
            run: () => {
                const nbLines = document.querySelectorAll(
                    ".o_field_cell[name=lot_name]",
                ).length;
                if (nbLines !== 5) {
                    console.error(
                        "wrong number of move lines generated. " +
                            nbLines +
                            " instead of 5",
                    );
                }
...
registry.category("web_tour.tours").add("test_generate_serial_2", {
    steps: () => [
        {
            trigger: ".o_field_x2many_list_row_add > a",
            run: "click",
...
registry.category("web_tour.tours").add("test_inventory_adjustment_apply_all", {
    steps: () => [
        {
...
registry.category("web_tour.tours").add("test_add_new_line_in_detailled_op", {
    steps: () => [
        {
            trigger:
                ".o_list_view.o_field_x2many .o_data_row button[name='action_show_details']",
            run: "click",
        },
        {
        ...
        {
            content:
                "Check that the first line is associated with LOT001 for a quantity of 0.00",
            trigger:
        ...
        },
        {
            ...
            run: () => {
                const lines = document.querySelectorAll(
                    ".o_data_row .o_data_cell[name=lot_id]",
                );
                if (lines.length !== 2) {
                    console.error(
                        "Wrong number of available quants: " +
                            lines.length +
                            " instead of 2.",
                    );
                }
        ...
        },
        {
            ...
            trigger:
                ".modal-header:has(.modal-title:contains(Add line: Product Lot)) .btn-close",
            run: "click",
        },
        {
        ...
        {
            content:
                "Modify the quant associated to the second line to fully use LOT003",
        ...
        },
        {
            ...
            trigger:
                ".modal-content .o_data_row:nth-child(1) .o_data_cell[name=quantity]",
            run: "click",
        },
        {
            trigger:
                ".modal-content .o_data_row:nth-child(1) .o_field_widget[name=quantity] input",
            run: "edit 8",
        },
        {
        ...
        {
            trigger:
                ".modal-content .o_data_row:nth-child(3) .o_field_pick_from:contains(LOT002)",
        },
        {
            content:
                "Modify the quantity of the first line from 10 to 15 to change the demand",
            trigger:
                ".modal-content .o_data_row:nth-child(3) .o_data_cell[name=quantity]",
            run: "click",
        },
        {
            trigger:
                ".modal-content .o_data_row:nth-child(3) .o_field_widget[name=quantity] input",
            run: "edit 15",
        },
        {
