...
class TestLeadMine(TestCrmCommon, MockIAPReveal):
    ...
    def setUpClass(cls):
        ...
        ) + cls.env.ref("crm_iap_mine.crm_iap_mine_industry_148")
    ...
    def test_reveal(self):
        ...
        with self.mock_IAP_reveal(
            self.ip_to_rules, name_list=["Heinrich", "Rivil", "LidGen"]
        ):
            self.env["crm.reveal.rule"]._process_lead_generation(autocommit=False)
        ...
        self.assertEqual(
            ...
            self.env["crm.reveal.view"],
            "Views should have been unlinked after completion",
        )

        self.assertEqual(
            len(self._new_leads), 3, "Number of leads should match IPs addresses"
        )
