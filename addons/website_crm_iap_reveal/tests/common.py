...
class MockIAPReveal(MockIAPEnrich):
    ...
    @contextmanager
    def mock_IAP_reveal(
        self, ip_to_rules, name_list=None, default_data=None, sim_error=None
    ):
        ...
        def _iap_contact_reveal(params, timeout):
            ...
            for counter, ip_values in enumerate(ip_to_rules):
                ...
                company_data = self._get_iap_company_data(
                    base_name, service="reveal", add_values={"ip": ip, "rule": rule}
                )
                if default_data:
                ...
                if rule.lead_for == "people":
                    people_data = self._get_iap_contact_data(
                        base_name, service="reveal"
                    )
                    iap_payload["people_data"] = people_data

                iap_payload["credit"] = 1 + (
                    len(people_data) if rule.lead_for == "people" else 0
                )

                response.append(iap_payload)

            return {"reveal_data": response, "credit_error": False}

        with (
            patch.object(
                CrmRevealRule, "_iap_contact_reveal", side_effect=_iap_contact_reveal
            ),
            patch.object(
                CrmLead,
                "create",
