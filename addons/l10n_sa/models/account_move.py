...
class AccountMove(models.Model):
        ...
        for record in self:
            ...
            if record.l10n_sa_confirmation_datetime and record.company_id.vat:
                ...
                timestamp_enc = get_qr_encoding(
                    3, time_sa.strftime(self._get_iso_format_asia_riyadh_date("T"))
                )
                totals = record._get_l10n_sa_totals()
                invoice_total_enc = get_qr_encoding(
                    4, float_repr(abs(totals["total_amount"]), 2)
                )
                total_vat_enc = get_qr_encoding(
                    5, float_repr(abs(totals["total_tax"]), 2)
                )
        ...
        for move in self:
            if move.country_code == "SA" and move.is_sale_document():
                ...
                if not move.l10n_sa_confirmation_datetime:
                    vals["l10n_sa_confirmation_datetime"] = datetime.combine(
                        move.invoice_date, fields.Datetime.now().time()
                    ).strftime(self._get_iso_format_asia_riyadh_date())
    ...
    def _l10n_sa_get_adjustment_reason(self):
        self.check_singleton()
        readable_zatca_reason = dict(
            self._fields["l10n_sa_reason"]._description_selection(self.env)
        ).get(self.l10n_sa_reason)
        return readable_zatca_reason if self.l10n_sa_show_reason else self.ref

    def _compute_show_l10n_sa_reason(self):
        for record in self:
            record.l10n_sa_show_reason = record.country_code == "SA" and (
                record.move_type == "out_refund"
                or (record.move_type == "out_invoice" and record.debit_origin_id)
            )
    ...
    def write(self, vals):
        ...
        for move in self.filtered("l10n_sa_confirmation_datetime"):
            move.l10n_sa_confirmation_datetime = datetime.combine(
                fields.Date.from_string(invoice_date),
                move.l10n_sa_confirmation_datetime.time(),
            )
        return result
