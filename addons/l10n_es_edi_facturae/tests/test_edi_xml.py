...
@patch(
    "odoo.addons.certificate.models.certificate_certificate.CertificateCertificate._compute_is_valid",
    _compute_is_valid,
)
class TestEdiFacturaeXmls(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        partner_b_ac.write(
            {
                ...
                "l10n_es_edi_facturae_ac_role_type_ids": [
                    Command.link(
                        cls.env.ref("l10n_es_edi_facturae.ac_role_type_01").id
                    ),
                    Command.link(
                        cls.env.ref("l10n_es_edi_facturae.ac_role_type_02").id
                    ),
                    Command.link(
                        cls.env.ref("l10n_es_edi_facturae.ac_role_type_03").id
                    ),
                ],
            }
        )
        ...
        cls.certificate_module = (
            "odoo.addons.certificate.models.certificate_certificate"
        )
        cls.move_module = "odoo.addons.l10n_es_edi_facturae.models.account_move"
        with (
            freeze_time(cls.frozen_today),
            patch(
                f"{cls.certificate_module}.fields.Datetime.now",
            ...
            cls.certificate = cls.env["certificate.certificate"].create(
                {
                    ...
                    "content": b64encode(
                        file_open(
                            "l10n_es_edi_facturae/tests/data/certificate_test.pfx", "rb"
                        ).read()
                    ),
    ...
    def test_generate_signed_xml(self, date=None):
        ...
        with (
            freeze_time(date),
            patch(
                f"{self.certificate_module}.fields.Datetime.now", lambda x=None: date
            ),
            self._mock_sha1(),
        ):
            ...
            with file_open(
                "l10n_es_edi_facturae/tests/data/expected_signed_document.xml", "rt"
            ) as f:
    ...
    def test_cannot_generate_unsigned_xml(self):
        ...
        with (
            ...
            patch(
                f"{self.certificate_module}.fields.Datetime.now",
                lambda x=None: self.frozen_today,
            ),
            patch(
                f"{self.certificate_module}.CertificateCertificate._compute_is_valid",
                _compute_is_valid,
            ),
            self._mock_sha1(),
        ):
    ...
    def test_in_invoice(self):
        ...
        with (
            ...
            with file_open(
                "l10n_es_edi_facturae/tests/data/expected_in_invoice_document.xml", "rt"
            ) as f:
                expected_xml = lxml.etree.fromstring(f.read().encode())
            self.assertXmlTreeEqual(lxml.etree.fromstring(generated_file), expected_xml)
    ...
    def test_out_invoice_decimals(self):
        ...
        with freeze_time(self.frozen_today):
            ...
            with file_open(
                "l10n_es_edi_facturae/tests/data/expected_out_invoice_4_decimals.xml",
                "rt",
            ) as f:
                expected_xml = lxml.etree.fromstring(f.read().encode())
    ...
    def test_out_invoice_withhold(self):
        ...
        with freeze_time(self.frozen_today):
            ...
            with file_open(
                "l10n_es_edi_facturae/tests/data/expected_out_invoice_withhold.xml",
                "rt",
            ) as f:
                expected_xml = lxml.etree.fromstring(f.read().encode())
    ...
    def test_refund_invoice(self):
        ...
        with (
            ...
            with file_open(
                "l10n_es_edi_facturae/tests/data/expected_refund_document.xml", "rt"
            ) as f:
                expected_xml = lxml.etree.fromstring(f.read().encode())
            self.assertXmlTreeEqual(lxml.etree.fromstring(generated_file), expected_xml)
    ...
    def test_import_multiple_invoices(self):
        with file_open(
            "l10n_es_edi_facturae/tests/data/import_multiple_invoices.xml", "rt"
        ) as f:
            content = f.read().encode()

        attachment = self.env["ir.attachment"].create(
            {"name": "invoice.xml", "raw": content}
        )
        sale_journal = self.company_data["default_journal_sale"].with_context(
    ...
    def test_import_withheld_taxes(self):
        with file_open(
            "l10n_es_edi_facturae/tests/data/import_withholding_invoice.xml", "rt"
        ) as f:
            content = f.read().encode()
        attachment = self.env["ir.attachment"].create(
            {"name": "invoice.xml", "raw": content}
    ...
    def test_generate_with_administrative_centers(self):
        ...
        with file_open(
            "l10n_es_edi_facturae/tests/data/expected_ac_document.xml", "rt"
        ) as f:
            expected_xml = lxml.etree.fromstring(f.read().encode())
        self.assertXmlTreeEqual(lxml.etree.fromstring(generated_file), expected_xml)
    ...
    def test_generate_with_invoice_period(self):
        ...
        with file_open(
            "l10n_es_edi_facturae/tests/data/expected_invoice_period_document.xml", "rt"
        ) as f:
            expected_xml = lxml.etree.fromstring(f.read().encode())
        self.assertXmlTreeEqual(lxml.etree.fromstring(generated_file), expected_xml)
    ...
    def test_generate_with_payment_means(self):
        ...
        with file_open(
            "l10n_es_edi_facturae/tests/data/expected_invoice_payment_means.xml", "rt"
        ) as f:
            expected_xml = lxml.etree.fromstring(f.read().encode())
        self.assertXmlTreeEqual(lxml.etree.fromstring(generated_file), expected_xml)
    ...
    def test_simplified_invoice(self):
        ...
        with freeze_time(datetime(2023, 1, 1)):
            ...
            with file_open(
                "l10n_es_edi_facturae/tests/data/expected_simplified_document.xml", "rt"
            ) as f:
                expected_xml = lxml.etree.fromstring(f.read().encode())
            self.assertXmlTreeEqual(lxml.etree.fromstring(generated_file), expected_xml)
    ...
    def test_download_facturae_xml_functionality(self):
        ...
        with (
            ...
            facturae_items = [
                ...
                if item.get("key") == "download_xml_facturae"
            ]
            self.assertEqual(
                len(facturae_items),
                1,
                "Should have exactly one Factura-e download option",
            )
    ...
    def test_download_facturae_xml_legal_documents(self):
        ...
        with (
            ...
            self.assertEqual(
                invoice._get_invoice_legal_documents("facturae"),
                {
    ...
    def test_download_facturae_xml_batch_scenario(self):
        ...
        with (
            ...
            self.assertEqual(
                len(facturae_items),
                1,
                "Should have one Factura-e download option for batch",
            )
            ...
            self.assertEqual(
                invoices.action_invoice_download_facturae(), expected_action
            )
    ...
    def test_out_invoice_rounding(self):
        ...
        with freeze_time(self.frozen_today):
            ...
            with file_open(
                "l10n_es_edi_facturae/tests/data/expected_out_invoice_round_glob.xml",
                "rt",
            ) as f:
                expected_xml = lxml.etree.fromstring(f.read().encode())
            self.assertXmlTreeEqual(lxml.etree.fromstring(generated_file), expected_xml)
