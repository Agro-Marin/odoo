...
class AccountMove(models.Model):
    ...
    l10n_es_invoicing_period_start_date = fields.Date(
    ...
    def _auto_init(self):
        ...
        if not column_exists(
            self.env.cr, "account_move", "l10n_es_edi_facturae_reason_code"
        ):
            create_column(
                self.env.cr,
                "account_move",
        ...
        if not column_exists(self.env.cr, "account_move", "l10n_es_payment_means"):
            create_column(
                self.env.cr, "account_move", "l10n_es_payment_means", "varchar"
            )
    ...
    def _compute_l10n_es_edi_facturae_reason_code(self):
        for move in self.filtered(lambda move: move.country_code == "ES"):
            move.l10n_es_edi_facturae_reason_code = (
                move.l10n_es_edi_facturae_reason_code or "10"
            )
    ...
    def _l10n_es_edi_facturae_get_tax_period(self):
        ...
        if self.env["res.company"].fields_get(["account_return_periodicity"]):
            ...
            )._get_period_boundaries(self.company_id, self.date)
        else:
    ...
    def _l10n_es_edi_facturae_get_refunded_invoices(self):
        ...
        invoices_refunded_mapping = {
            invoice.id: invoice.reversed_entry_id.id for invoice in self
        }
        ...
        for invoice in self:
            ...
            for move_id in (
                record_dict["counterpart_move_id"]
                for record_dict in payment_data.get(invoice.id, [])
            ):
    ...
    def _l10n_es_edi_facturae_get_corrective_data(self):
        ...
        if self.move_type.endswith("refund"):
            if not self.reversed_entry_id:
                raise UserError(
                    _(
                        "The credit note/refund appears to have been issued manually. For the purpose of "
    ...
    def _l10n_es_edi_facturae_get_tax_node_from_tax_data(self, values, round=False):
        ...
        return {
            ...
            "TaxableBase": {
                "TotalAmount": self.currency_id.round(
                    values[f"{prefix}base_amount_currency"]
                ),
                "EquivalentInEuros": self.company_currency_id.round(
                    values[f"{prefix}base_amount"]
                ),
            },
            "TaxAmount": {
                "TotalAmount": self.currency_id.round(
                    tax_sign * values[f"{prefix}tax_amount_currency"]
                ),
                "EquivalentInEuros": self.company_currency_id.round(
                    tax_sign * values[f"{prefix}tax_amount"]
                ),
            },
        }
    ...
    def _l10n_es_edi_facturae_prepare_inv_line(self, base_line, aggregated_values):
        ...
        receiver_transaction_reference = (
            ...
            and line.sale_line_ids.order_id.client_order_ref
            else invoice_ref
        )
        ...
        xml_values = {
            ...
            "DiscountsAndRebates": [],
            "Charges": [],
            "GrossAmount": float_round(
                tax_details["raw_total_excluded_currency"], precision_digits=8
            ),
        }
        ...
        if line.quantity:
            xml_values["UnitPriceWithoutTax"] = float_round(
                raw_total_cost / line.quantity, precision_digits=8
            )
        else:
    ...
    def _l10n_es_edi_facturae_export_facturae(self):
        ...
        def extract_party_name(party):
            ...
            if not party.is_company:
                name_split = [
                    part for part in party.name.replace(", ", " ").split(" ") if part
                ]
                if len(name_split) > 2:
        ...
        if not company.vat:
            raise UserError(
                _("The company needs a set tax identification number or VAT number")
            )
        if not partner.vat:
            raise UserError(
                _("The partner needs a set tax identification number or VAT number")
            )
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, grouping_function_per_tax
        )
        for base_line, aggregated_values in base_lines_aggregated_values:
            invoice_line_values = self._l10n_es_edi_facturae_prepare_inv_line(
                base_line, aggregated_values
            )
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, grouping_function_per_base_line_tax
        )
        ...
        for grouping_key, values in values_per_grouping_key.items():
            ...
            tax_data = self._l10n_es_edi_facturae_get_tax_node_from_tax_data(
                {**values, "grouping_key": tax}
            )
            invoice_values["TaxesWithheld" if is_withholding else "TaxOutputs"].append(
                tax_data
            )
            invoice_values[
                "TotalTaxesWithheld" if is_withholding else "TotalTaxOutputs"
            ] += abs(values["tax_amount_currency"])
        ...
        if (
            self.l10n_es_invoicing_period_start_date
        ...
        invoice_issuer_signature_type = (
            "supplier" if self.move_type == "out_invoice" else "customer"
        )
        signature_values = {
            "SigningTime": "",
    ...
    def _l10n_es_edi_facturae_render_facturae(self):
        ...
        xml_content = cleanup_xml_node(
            self.env["ir.qweb"]._render(
                "l10n_es_edi_facturae.account_invoice_facturae_export", template_values
            )
        )
        ...
        try:
            ...
            errors.append(
                _(
                    "No valid certificate found for this company, Facturae EDI file will not be signed.\n"
                )
            )
    ...
    def _import_create_or_retrieve_partner(self, partner_vals):
        ...
        partner = self.env["res.partner"]._get_matching_partner(
    ...
    def _import_invoice_facturae_invoice(self, invoice, partner, tree):
        ...
        if is_refund:
            invoice.move_type = (
                "in_refund" if invoice.move_type.startswith("in_") else "out_refund"
            )
        ...
        if partner:
            ...
            logs.append(
                _(
                    "Customer/Vendor could not be found and could not be created due to missing data in the XML."
                )
            )
        ...
        # ==== narration ====
        invoice.narration = "\n".join(
            ref.text for ref in tree.xpath(".//LegalReference") if ref.text
        )
        ...
        for line in lines:
            ...
            if item_description := get_xml_value(".//ItemDescription", line):
                ...
                if product:
                    ...
                    logs.append(
                        _("The product '%s' could not be found.", item_description)
                    )
            ...
            if tax_rate:
                ...
                if (
                    ...
                    and invoice.currency_id.compare_amounts(
                        float(taxable_base) * (float(tax_rate) / 100), float(tax_amount)
                    )
                    != 0
                ):
                    is_fixed = True

                tax_excl = self._search_tax_for_import(
                    invoice.company_id,
                    float(tax_rate),
                    is_fixed,
                    is_withheld,
                ...
                if tax_excl:
                ...
                elif tax_incl := self._search_tax_for_import(
                    invoice.company_id,
                    float(tax_rate),
                    is_fixed,
                    is_withheld,
                    ...
                    logs.append(
                        _(
                            "Could not retrieve the tax: %(tax_rate)s %% for line '%(line)s'.",
                            tax_rate=tax_rate,
    ...
    def _l10n_es_facturae_sign_xml(self, edi_data, signature_data):
        ...
        signature = self.env["ir.qweb"]._render(
            "l10n_es_edi_facturae.template_xades_signature", signature_data
        )
        ...
        signature.find(
            "ds:SignatureValue", namespaces=NS_MAP
        ).text = certificate_sudo._sign(
