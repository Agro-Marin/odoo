    ...
    def _compute_stage_id(self):
        ...
        for task in self:
            if not task.stage_id and task.state == "draft":
                task.stage_id = Test_Base_AutomationStage.search(
...
class BaseAutomationModelWithRecnameM2o(models.Model):
    ...
    user_id = fields.Many2one(
        "base.automation.model.with.recname.char", string="Responsible"
    )

    @api.model
    def name_create(self, name):
        name = name.strip()
        user = self.env["base.automation.model.with.recname.char"].search(
