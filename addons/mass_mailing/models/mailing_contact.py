...
class MailingContact(models.Model):
    ...
    def _compute_name(self):
        for record in self:
            if record.first_name or record.last_name:
                record.name = " ".join(
                    name_part
                    for name_part in (record.first_name, record.last_name)
                    if name_part
                )
    ...
    def _compute_opt_out(self):
        if (
            ...
            [active_list_id] = self.env.context["default_list_ids"]
            for record in self:
                active_subscription_list = record.subscription_ids.filtered(
                    lambda l: l.list_id.id == active_list_id
                )
    ...
    def create(self, vals_list):
        ...
        default_list_ids = (
            default_list_ids if isinstance(default_list_ids, (list, tuple)) else []
        )
        ...
        records = super(
            MailingContact, self.with_context(default_list_ids=False)
        ).create(vals_list)
    ...
    def action_import(self):
        ...
        if not context.get("default_mailing_list_ids") and context.get(
            "from_mailing_list_ids"
        ):
    ...
    def _is_name_split_activated(self):
        ...
        view = self.env.ref(
            "mass_mailing.mailing_contact_view_tree_split_name",
