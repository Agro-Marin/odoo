...
class MixinMailThread(models.AbstractModel):
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        if routes:
            ...
            if msg_references:
                self.env["mailing.trace"].set_opened(
                    domain=[("message_id", "in", msg_references)]
                )
                self.env["mailing.trace"].set_replied(
                    domain=[("message_id", "in", msg_references)]
                )
    ...
    def _routing_handle_bounce(self, email_message, message_dict):
        ...
        if bounced_email:
            three_months_ago = fields.Datetime.to_string(
                datetime.datetime.now() - datetime.timedelta(weeks=13)
            )
            stats = (
            ...
            if len(stats) >= BLACKLIST_MAX_BOUNCED_LIMIT and (
                not bounced_partner
                or any(
                    p.message_bounce >= BLACKLIST_MAX_BOUNCED_LIMIT
                    for p in bounced_partner
                )
            ):
