...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update(
            mass_mailing_split_contact_name=self.env[
                "mailing.contact"
            ]._is_name_split_activated(),
        )
        return res

    def set_values(self):
        super().set_values()
        ab_test_cron = self.env.ref(
        ...
        if (
            self.env["mailing.contact"]._is_name_split_activated()
            != self.mass_mailing_split_contact_name
        ):
            self.env.ref(
                "mass_mailing.mailing_contact_view_tree_split_name"
            ).active = self.mass_mailing_split_contact_name
            self.env.ref(
                "mass_mailing.mailing_contact_view_form_split_name"
            ).active = self.mass_mailing_split_contact_name
