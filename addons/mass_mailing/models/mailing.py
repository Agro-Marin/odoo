...
class MailingMailing(models.Model):
    ...
    def default_get(self, fields):
        ...
        if (
            ...
            and ("schedule_type" in fields and "schedule_date" in fields)
            and Datetime.to_datetime(default_calendar_date) > Datetime.now()
        ):
            vals.update(
                {"schedule_type": "scheduled", "schedule_date": default_calendar_date}
            )

        if (
            "contact_list_ids" in fields
            and not vals.get("contact_list_ids")
            and vals.get("mailing_model_id")
        ):
            if vals.get("mailing_model_id") == self.env["ir.model"]._get_id(
                "mailing.list"
            ):
    ...
    def _default_mail_server_id(self):
        server_id = (
            ...
            .sudo()
            .get_param("mass_mailing.mail_server_id")
        )
        try:
    ...
    body_html = fields.Html(
        string="Body converted to be sent by mail",
        sanitize="email_outgoing",
        render_engine="qweb",
        render_options={"post_process": True},
    )
    ...
    )  # used with SMS when not yet 'delivered'
    ...
    def _check_mailing_filter_model(self):
        ...
        for mailing in self:
            if (
                mailing.mailing_filter_id
                and mailing.mailing_model_id
                != mailing.mailing_filter_id.mailing_model_id
            ):
                raise ValidationError(
                    _(
                        "The saved filter targets different recipients and is incompatible with this mailing."
                    )
                )
    ...
    def _compute_ab_testing_is_winner_mailing(self):
        for mailing in self:
            mailing.ab_testing_is_winner_mailing = (
                mailing.campaign_id.ab_testing_winner_mailing_id == mailing
            )
    ...
    def _compute_email_from(self):
        ...
        for mailing in self:
            user_email = (
                mailing.create_uid.email_formatted or self.env.user.email_formatted
            )
            server = mailing.mail_server_id
            if not server:
                mailing.email_from = mailing.email_from or user_email
            elif mailing.email_from and server._match_from_filter(
                mailing.email_from, server.from_filter
            ):
    ...
    def _compute_total(self):
        for mass_mailing in self:
            ...
            if (
                total
                and mass_mailing.ab_testing_enabled
                and mass_mailing.ab_testing_pc < 100
            ):
    ...
    def _compute_clicks_ratio(self):
        ...
        mapped_data = {
            m["id"]: float_round(
                100 * m["nb_clicks"] / m["nb_mails"], precision_digits=2
            )
        ...
        for mailing in self:
            ...
            values = {
                ...
                "expected": sum(
                    ...
                    if k not in ("links_click_datetime", "sent_datetime")
                ),
            ...
            total_sent = (
                values["expected"] - values["canceled"] - values["failed"]
            ) or 1
            values["received_ratio"] = float_round(
                100.0 * values["delivered"] / total, precision_digits=2
            )
            values["opened_ratio"] = float_round(
                100.0 * values["opened"] / total_no_error, precision_digits=2
            )
            values["replied_ratio"] = float_round(
                100.0 * values["replied"] / total_no_error, precision_digits=2
            )
            values["bounced_ratio"] = float_round(
                100.0 * values["bounced"] / total_sent, precision_digits=2
            )
    ...
    def _compute_next_departure(self):
        ...
        for mass_mailing in self:
            if mass_mailing.schedule_date:
                ...
                mass_mailing.next_departure = max(
                    mass_mailing.schedule_date, fields.Datetime.now()
                )
            else:
        ...
        past = self.filtered(
            lambda mailing: (
                ...
                and mailing.next_departure < fields.Datetime.now()
            )
        )
    ...
    def _compute_warning_message(self):
        ...
        for mailing in self.filtered(lambda mailing: mailing.mailing_type == "mail"):
            ...
            if mail_server and not mail_server._match_from_filter(
                mailing.email_from, mail_server.from_filter
            ):
    ...
    def _compute_mailing_filter_count(self):
        ...
        for mailing in self:
            mailing.mailing_filter_count = mapped_data.get(
                mailing.mailing_model_id.id, 0
            )
    ...
    def _compute_mailing_model_real(self):
        for mailing in self:
            mailing.mailing_model_real = (
                "mailing.contact"
                if mailing.mailing_model_id.model == "mailing.list"
    ...
    def _compute_mailing_on_mailing_list(self):
        ...
        self.filtered(
            lambda m: m.mailing_model_id == mailing_list_model_id
        ).mailing_on_mailing_list = True

    @api.depends(
        "mailing_model_id", "contact_list_ids", "mailing_type", "mailing_filter_id"
    )
    ...
    def _compute_mailing_type_description(self):
        for mailing in self:
            mailing.mailing_type_description = dict(
    ...
    def _compute_is_ab_test_sent(self):
        for rec in self:
            ...
            selected_mailings = ab_testing_mailings.filtered(
    ...
    def create(self, vals_list):
        ...
        for values, mailing in zip(vals_list, mailings, strict=False):
            if values.get("body_arch"):
                mailing.body_arch = mailing._convert_inline_images_to_urls(
                    mailing.body_arch
                )
            if values.get("body_html"):
                mailing.body_html = mailing._convert_inline_images_to_urls(
                    mailing.body_html
                )
        return mailings
    ...
    def write(self, vals):
        values = vals
        if values.get("body_arch"):
            values["body_arch"] = self._convert_inline_images_to_urls(
                values["body_arch"]
            )
        if values.get("body_html"):
        ...
        if (
            values.get("campaign_id") is False
            and any(mailing.ab_testing_enabled for mailing in self)
            and "ab_testing_enabled" not in values
        ):
            raise ValidationError(
                _("A campaign should be set when A/B test is enabled")
            )
        ...
        if any(self.mapped("ab_testing_schedule_datetime")):
            schedule_date = min(
                m.ab_testing_schedule_datetime
                for m in self
    ...
    def _create_ab_testing_utm_campaigns(self):
        ...
        campaign_vals = [
            ...
            for mailing in self.filtered(
                lambda mailing: mailing.ab_testing_enabled and not mailing.campaign_id
            )
        ]
    ...
    def _action_view_documents_filtered(self, view_filter):
        def _fetch_trace_res_ids(trace_domain):
            ...
            return (
                self.env["mailing.trace"]
                .search_fetch(domain=trace_domain, field_names=["res_id"])
                .mapped("res_id")
            )
        ...
        if helper_header and helper_message:
            action["help"] = (
                Markup('<p class="o_view_nocontent_smiling_face">%s</p><p>%s</p>')
                % (
                    helper_header,
                    helper_message,
                ),
    ...
    def action_send_winner_mailing(self):
        ...
        if sorted_by != "manual":
            ab_testing_mailings = (
                final_mailing._get_ab_testing_siblings_mailings().sudo()
            )
            selected_mailings = ab_testing_mailings.filtered(
                lambda m: m.state == "done"
            ).sorted(sorted_by, reverse=True)
            if selected_mailings:
                final_mailing = selected_mailings[0]
            else:
                raise ValidationError(
                    _(
                        "No mailing for this A/B testing campaign has been sent yet! Send one first and try again later."
                    )
                )
        ...
        final_mailing = self.copy(
            {
                "ab_testing_pc": 100,
                "name": _(
                    " %(subject)s (final)", subject=self.name
    ...
    def _get_ab_testing_description_values(self):
        ...
        return {
            ...
            "ab_testing_winner_selection_description": self._get_ab_testing_winner_selection()[
                "description"
            ],
            "total_ab_testing_pc": sum(
                mailing.ab_testing_pc
                for mailing in self._get_ab_testing_siblings_mailings()
            ),
        }

    def _get_ab_testing_siblings_mailings(self):
        return self.campaign_id.mailing_mail_ids.filtered(
    ...
    def _get_default_ab_testing_campaign_values(self, values=None):
        values = values or {}
        return {
            "ab_testing_schedule_datetime": values.get("ab_testing_schedule_datetime")
            or self.ab_testing_schedule_datetime,
            "ab_testing_winner_selection": values.get("ab_testing_winner_selection")
            or self.ab_testing_winner_selection,
            "mailing_mail_ids": self.ids,
            "name": _(
                "A/B Test: %s",
                values.get("subject") or self.subject or fields.Datetime.now(),
            ),
    ...
    def _get_opt_out_list(self):
        ...
        if hasattr(self.env[self.mailing_model_name], "_mailing_get_opt_out_list"):
            ...
            _logger.info(
                "Mass-mailing %s targets %s, no opt out list available",
                self,
                target._name,
            )
        return opt_out
    ...
    def _get_seen_list(self):
        ...
        _logger.info(
            "Mass-mailing %s has already reached %s %s emails",
            self,
            len(seen_list),
            target._name,
        )
    ...
    def _get_remaining_recipients(self):
        ...
        if self.ab_testing_enabled and self.ab_testing_is_winner_mailing:
            trace_domain &= Domain(
                "mass_mailing_id", "in", self._get_ab_testing_siblings_mailings().ids
            )
        else:
            trace_domain &= Domain("res_id", "in", res_ids) & Domain(
    ...
    def _get_unsubscribe_oneclick_url(self, email_to, res_id):
        return tools.urls.urljoin(
            self.get_base_url(),
            "mailing/%(mailing_id)s/unsubscribe_oneclick?%(params)s"
            % {
                "mailing_id": self.id,
                "params": urlencode(
                    {
                        "document_id": res_id,
                        "email": email_to,
                        "hash_token": self._generate_mailing_recipient_token(
                            res_id, email_to
                        ),
                    }
                ),
    ...
    def _get_unsubscribe_url(self, email_to, res_id):
        return tools.urls.urljoin(
            self.get_base_url(),
            "mailing/%(mailing_id)s/confirm_unsubscribe?%(params)s"
            % {
                "mailing_id": self.id,
                "params": urlencode(
                    {
    ...
    def _action_send_mail(self, res_ids=None):
        ...
        for mailing in self:
            ...
            composer_values = {
                ...
                "author_id": mailing.user_id.partner_id.id
    ...
    def convert_links(self):
        ...
        for mass_mailing in self:
            ...
            res[mass_mailing.id] = mass_mailing._shorten_links(
                html, vals, blacklist=["/unsubscribe_from_list", "/view", "/cards/"]
            )
    ...
    def _process_mass_mailing_queue(self):
        ...
        for mass_mailing in mass_mailings:
            context_user = (
                mass_mailing.user_id or mass_mailing.write_uid or self.env.user
            )
    ...
    def _action_send_statistics(self):
        ...
        for mailing in self:
            ...
            if mail_user.has_group("mass_mailing.group_mass_mailing_user"):
                rendering_data["mailing_report_token"] = (
                    self._generate_mailing_report_token(mail_user.id)
                )
    ...
    def _prepare_statistics_email_values(self):
        ...
        if self.mailing_type == "mail":
            kpi = {
                "kpi_fullname": _(
                    "Engagement on %(expected)i %(mailing_type)s Sent",
        ...
        return {
            ...
            "top_button_url": tools.urls.urljoin(
                web_base_url, f"/odoo/mailing.mailing/{self.id}"
            ),
            "kpi_data": [
                kpi,
                {
                    "kpi_fullname": _(
                        "Business Benefits on %(expected)i %(mailing_type)s Sent",
                        expected=self.expected,
                        mailing_type=mailing_type,
                    ),
                    "kpi_action": None,
    ...
    def _convert_inline_images_to_urls(self, html_content):
            ...
            for node in root.iter(lxml.etree.Element, lxml.etree.Comment):
                if node.tag == "img":
                    ...
                    for match in re.findall(
                        r"data:image/[A-Za-z]+;base64,.+?(?=&\#34;|\"|\'|&quot;|\))",
                        node.attrib.get("style"),
                    ):
                        image = re.sub(
                            r"data:image/[A-Za-z]+;base64,", "", match
                        ).encode()  # base64 image as bytes
                        conversion_info.append(
                            (
                                image,
                                node,
                    ...
                    base64_in_element_regex = re.compile(
                        r"""
                    ...
                    for match in re.findall(r"<v:image[^>]*>", node.text):
                        ...
                        absolute_url = (
                            url
                            if "//" in url
                            else f"{self.get_base_url()}{url if url.startswith('/') else f'/{url}'}"
                        )
                        target_width_match = re.search(
                            r"width:\s*([0-9\.]+)\s*px", match
                        )
                        target_height_match = re.search(
                            r"height:\s*([0-9\.]+)\s*px", match
                        )
                        if target_width_match and target_height_match:
                            ...
                            try:
                                ...
                                image = image_processor.crop_resize(
                                    target_width, target_height, 0, 0
                                )
                                conversion_info.append(
                                    (
                                        base64.b64encode(image.source),
                                        node,
                                        url,
                                        int(node.attrib.get("data-original-id") or "0"),
                                    )
        ...
        urls = self._create_attachments_from_inline_images(
            [(image, original_id) for (image, _, _, original_id) in conversion_info]
        )
        for (_image, node, old_url, _original_id), new_url in zip(
            conversion_info, urls, strict=False
        ):
    ...
    def _get_image_by_url(self, url, session):
        ...
        try:
            ...
            _logger.warning(
                "This file could not be decoded as an image file.", exc_info=True
            )
            raise
