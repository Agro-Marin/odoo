...
class MassMailController(http.Controller):
    def _check_mailing_email_token(
        self, mailing_id, document_id, email, hash_token, required_mailing_id=False
    ):
        ...
        if not hash_token:
            if request.env.user._is_public():
                raise BadRequest
            if mailing_id and not request.env.user.has_group(
        ...
        if mailing_id:
            ...
            if not mailing_sudo.exists():
                raise NotFound
            if hash_token and not consteq(
                mailing_sudo._generate_mailing_recipient_token(document_id, email),
                hash_token,
            ):
                raise Unauthorized
        else:
    ...
    def _get_blocklist_record(self, email):
        if not email or not tools.email_normalize(email):
            return None
        return (
            request.env["mail.blacklist"]
            .sudo()
            .with_context(active_test=False)
            .search([("email", "=", tools.email_normalize(email))])
        )

    def _get_contacts(self, email):
        if not email or not tools.email_normalize(email):
            return request.env["mailing.contact"]
        return (
            request.env["mailing.contact"]
            .sudo()
            .search([("email_normalized", "=", tools.email_normalize(email))])
        )

    def _get_subscription_optouts(self):
        return request.env["mailing.subscription.optout"].sudo().search([])

    def _get_user_information(self, email, hash_token):
    ...
    def mailing_my(self):
        email, _hash_token = self._get_user_information(None, None)
        if not email:
            raise Unauthorized
    ...
    @http.route(
        "/mailing/<int:mailing_id>/confirm_unsubscribe",
        type="http",
        website=True,
        auth="public",
    )
    def mailing_confirm_unsubscribe(
        ...
        email_found, hash_token_found = self._get_user_information(email, hash_token)
        try:
        ...
        return request.render(
            "mass_mailing.page_mailing_confirm_unsubscribe",
            {
    ...
    @http.route(
        "/mailing/confirm_unsubscribe",
        type="http",
        website=True,
        auth="public",
        methods=["POST"],
    )
    def mailing_confirm_unsubscribe_post(
        self, mailing_id, document_id=None, email=None, hash_token=None
    ):
        # Unsubscribe user
        email_found, hash_token_found = self._get_user_information(email, hash_token)
        ...
        if mailing_sudo.mailing_on_mailing_list:
            self._mailing_unsubscribe_from_list(
                mailing_sudo, document_id, email_found, hash_token_found
            )
        else:
            self._mailing_unsubscribe_from_document(
                mailing_sudo, document_id, email_found, hash_token_found
            )
        ...
        return request.render(
            "mass_mailing.page_mailing_has_unsubscribed",
            {
    ...
    @http.route(
        ["/mailing/<int:mailing_id>/unsubscribe"],
        type="http",
        website=True,
        auth="public",
    )
    def mailing_unsubscribe(
        ...
        if mailing_sudo.mailing_on_mailing_list:
            return self._mailing_unsubscribe_from_list(
                mailing_sudo, document_id, email_found, hash_token_found
            )
        return self._mailing_unsubscribe_from_document(
            mailing_sudo, document_id, email_found, hash_token_found
        )
    ...
    def _mailing_unsubscribe_from_document(
        ...
        _blocklist_rec = (
            request.env["mail.blacklist"]
            .sudo()
            ._add(email, message=Markup("<p>%s</p>") % message)
        )
    ...
    def _prepare_mailing_subscription_values(
        ...
        mail_blocklist = self._get_blocklist_record(email)
        email_normalized = tools.email_normalize(email)

        # fetch optout/blacklist reasons
        opt_out_reasons = self._get_subscription_optouts()

        # as there may be several contacts / email -> consider any opt-in overrides
        # opt-out
        contacts = self._get_contacts(email)
    ...
    def mailing_update_list_subscription(
        ...
        **post,
        ...
        contacts = self._get_contacts(email_found)
        lists_optin = (
            request.env["mailing.list"].sudo().browse(lists_optin_ids or []).exists()
        )
    ...
    def mailing_send_feedback(
        ...
        **post,
        ...
        if last_action in {"subscription_updated", "subscription_updated_optout"} or (
            not last_action
            and (not mailing_sudo or mailing_sudo.mailing_on_mailing_list)
        ):
            contacts = self._get_contacts(email_found)
            contacts.subscription_ids.filtered(
                lambda sub: (
                    sub.opt_out
                    and sub.opt_out_datetime
        ...
        # feedback coming from a mailing, without additional context information: log
            ...
            documents_for_post = (
                request.env[mailing_sudo.mailing_model_real]
                .sudo()
                .search([("id", "=", document_id)])
            )
    ...
    @http.route(
        "/mail/track/<int:mail_id>/<string:token>/blank.gif", type="http", auth="public"
    )
    def track_mail_open(self, mail_id, token, **post):
        """Email tracking."""
        expected_token = request.env["mail.mail"]._generate_mail_recipient_token(
            mail_id
        )
        if not consteq(token, expected_token):
            raise Unauthorized

        request.env["mailing.trace"].sudo().set_opened(
            domain=[("mail_mail_id_int", "in", [mail_id])]
        )
        response = Response()
    ...
    def mailing_report_deactivate(self, token, user_id):
        ...
        if (
            not user
            or not user.has_group("mass_mailing.group_mass_mailing_user")
            or not consteq(
                token,
                request.env["mailing.mailing"]._generate_mailing_report_token(user.id),
            )
        ):
            raise Unauthorized

        request.env["ir.config_parameter"].sudo().set_param(
            "mass_mailing.mass_mailing_reports", False
        )
        render_vals = {}
        if user.has_group("base.group_system"):
            render_vals = {
                "menu_id": request.env.ref(
                    "mass_mailing.menu_mass_mailing_global_settings"
                ).id
            }
        return request.render("mass_mailing.mailing_report_deactivated", render_vals)

    @http.route(
        ["/mailing/<int:mailing_id>/view"], type="http", website=True, auth="public"
    )
    def mailing_view_in_browser(
        self, mailing_id, email=None, document_id=None, hash_token=None, **kwargs
    ):
    ...
    def _format_bl_request(self, mailing, document_id):
        mailing_model_name = (
            request.env["ir.model"]._get(mailing.mailing_model_real).display_name
        )
        return {
            "mailing_link": Markup(
