...
class TestMailingContactAccess(MassMailCommon):
    ...
    def test_mailing_contact_properties_access(self):
        ...
        contact = self.env["mailing.contact"].create(
            {"properties": copy.deepcopy(delete_value)}
        )
        ...
        with self.assertRaises(exceptions.AccessError):
            self.env["res.partner"].create(
                {"properties": copy.deepcopy(value), "name": "test"}
            )

        partner = (
            self.env["res.partner"]
            .sudo()
            .create({"properties": copy.deepcopy(value), "name": "test"})
        )
        self.assertEqual(dict(partner.properties), {"test": "test"})

        base_definition = self.env[
            "properties.base.definition"
        ]._get_definition_for_property_field("mailing.contact", "properties")
        self.assertTrue(base_definition)
        with self.assertRaises(exceptions.AccessError):
            base_definition.properties_field_id = (
                self.env["ir.model.fields"].sudo()._get("res.partner", "properties").id
            )

        partner_base_definition = self.env[
            "properties.base.definition"
...
class TestMailingContactToList(MassMailCommon):
    ...
    def test_mailing_contact_to_list(self):
        ...
        wizard_form = Form(
            self.env["mailing.contact.to.list"].with_context(
                default_contact_ids=contacts.ids
            )
        )
        ...
        with self.mock_datetime_and_now(frozen_time):
            ...
            self.assertTrue(
                all(date == frozen_time for date in create_dates),
                "All create dates should be equal to frozen datetime",
            )
        self.assertEqual(action["type"], "ir.actions.client")
        self.assertTrue(
            action.get("params", {}).get("next"),
            "Should return a notification with a next action",
        )
        ...
        self.assertEqual(
            subaction["context"]["default_contact_list_ids"], [mailing2.id]
        )
...
class TestMailingListMerge(MassMailCommon):
    ...
    def test_mailing_contact_create(self):
        ...
        new = (
            ...
            .with_context(default_list_ids=default_list_ids)
            .create(
                [
                    {
                        "name": "Contact_%d" % x,
        ...
        self.assertEqual(
            new.list_ids,
            (self.mailing_list_1 | self.mailing_list_2 | self.mailing_list_3),
        )
        ...
        self.assertEqual(
            contact.subscription_ids.filtered(
        ...
        self.assertFalse(
            contact.subscription_ids.filtered(
                lambda s: s.list_id == self.mailing_list_2
            ).opt_out_datetime
        )
    ...
    def test_mailing_list_action_send_mailing(self):
        ...
        self.assertEqual(
            ...
            self.mailing_list_1,
            "Should have correct mailing list set",
        )
        self.assertEqual(
            mailing.mailing_type, "mail", "Should have correct mailing_type"
        )
    ...
    def test_mailing_list_contact_copy_in_context_of_mailing_list(self):
        ...
        contact_2 = contact_1.with_context(
            default_list_ids=self.mailing_list_3.ids
        ).copy()
        self.assertEqual(
            contact_1.list_ids,
            contact_2.list_ids,
            "Should copy the existing mailing list(s)",
        )
    ...
    def test_mailing_list_merge(self):
        ...
        merge_form = Form(
            self.env["mailing.list.merge"].with_context(
        ...
        self.assertEqual(
            len(result_list.contact_ids.ids),
            5,
        ...
        self.assertEqual(
            len(list(set(result_list.contact_ids.mapped("email")))),
            5,
...
class TestMailingContactImport(MassMailCommon):
    ...
    def test_mailing_contact_import(self):
        ...
        contact_import = Form(
            self.env["mailing.contact.import"].with_context(
        ...
        self.assertEqual(
            len(first_list.contact_ids), 2, "Should not yet create the contact"
        )
        self.assertEqual(
            len(second_list.contact_ids), 2, "Should not yet create the contact"
        )
        self.assertEqual(
            len(third_list.contact_ids), 0, "Should not yet create the contact"
        )

        # Test that the context key "default_list_ids" is ignored (because we manually set list_ids)
        contact_import.with_context(
            default_list_ids=(first_list | second_list).ids
        ).action_import()

        self.env["mailing.list"].invalidate_model(["contact_ids"])
        # Check the contact of the first mailing list
        contacts = [(contact.name, contact.email) for contact in first_list.contact_ids]
        self.assertIn(
            ("", "alice@example.com"),
            contacts,
            "Should have imported the right email address",
        )
        self.assertIn(
            ("Bob", "bob@example.com"),
            contacts,
            "Should have imported the name of the contact",
        )
        self.assertIn(
            ("", "already_exists_list_2@example.com"),
            contacts,
        ...
        self.assertEqual(
            len(second_list.contact_ids), 2, "Should have ignored default_list_ids"
        )
        self.assertNotIn(
            ("Test", "bob@example.com"), contacts, "Should have ignored duplicated"
        )
        self.assertNotIn(
            ("", "bob@example.com"), contacts, "Should have ignored duplicated"
        )
        self.assertNotIn(
            ("Test", "already_exists_list_1_and_2@example.com"),
            contacts,
        ...
        contacts = [(contact.name, contact.email) for contact in third_list.contact_ids]
        self.assertIn(
            ("", "alice@example.com"),
            contacts,
            "Should have imported the right email address",
        ...
        self.assertIn(
            ("Already Exists", "already_exists_list_1@example.com"),
            contacts,
        ...
        self.assertEqual(
            len(contact),
            1,
            "Should have updated the existing contact instead of creating a new one",
        )
...
class TestSubscriptionManagement(MassMailCommon):
    ...
    def test_mailing_update_optout(self):
        ...
        ml_1, ml_2 = (
            self.mailing_list_1.with_env(self.env),
            self.mailing_list_2.with_env(self.env),
        )
        ...
        (ml_1 + ml_2)._update_subscription_from_email(
            _email_formatted_upd, opt_out=False
        )
        ...
        (ml_2 + ml_3)._update_subscription_from_email(
            _email_formatted_upd, opt_out=True
        )
        subs = self.env["mailing.subscription"].search(
            [("contact_id", "=", contact.id)]
        )
