        ...
        cls.test_mailing_on_lists = cls.env["mailing.mailing"].create(
            {
                "body_html": '<p>Hello <t t-out="object.name"/><br />Go to <a id="url" href="https://www.example.com/foo/bar?baz=qux">this link</a></p>',
                "contact_list_ids": [
                    (4, cls.mailing_list_1.id),
                    (4, cls.mailing_list_2.id),
                ],
...
class TestMailingControllers(TestMailingControllersCommon):
    ...
    def test_assert_initial_values(self):
        ...
        self.assertFalse(
            self.env["mail.blacklist"].search(
                [("email", "=", self.test_email_normalized)]
            )
        )
    ...
    def test_mailing_report_unsubscribe(self):
        ...
        self.user_marketing.write(
            {
                "group_ids": [
                    (3, self.env.ref("mass_mailing.group_mass_mailing_user").id)
                ],
            }
        )
        res = self.url_open(
    ...
    def test_mailing_unsubscribe_from_document_tour(self):
        ...
        for test_email, tour_name in [
            (
                '"Not Déboulonneur" <not.fleurus@example.com>',
            ...
            with self.subTest(test_email=test_email, tour_name=tour_name):
                test_partner = self.env["res.partner"].create(
                    {"email": test_email, "name": "Test Déboulonneur"}
                )
                ...
                hash_token = test_mailing._generate_mailing_recipient_token(
                    test_partner.id, test_partner.email_normalized
                )
                ...
                self.assertEqual(
                    len(test_partner.message_ids), len(previous_messages) + 1
                )
                msg_fb = test_partner.message_ids[0]
                self.assertEqual(
                    msg_fb.body,
                    Markup(
                        f"<p>Feedback from {test_email_normalized}<br>{test_feedback}</p>"
                    ),
                )

                # posted messages on exclusion list record: activated, feedback, deactivated, activated again
                bl_record = self.env["mail.blacklist"].search(
                    [("email", "=", test_partner.email_normalized)]
                )
                ...
                self.assertTracking(
                    msg_fb,
                    [("opt_out_reason_id", "many2one", False, opt_out_reasons[-1])],
                )
                self.assertEqual(
                    msg_bl.body,
                    Markup(
                ...
                self.assertEqual(
                    msg_create.body, Markup("<p>Mail Blacklist created</p>")
                )
    ...
    def test_mailing_unsubscribe_from_list_tour(self):
        ...
        hash_token = test_mailing._generate_mailing_recipient_token(
            contact_l1.id, contact_l1.email
        )
        with freeze_time(self._reference_now):
            self.start_tour(
        ...
        self.assertEqual(
            ...
            Markup(
                f"<p>Feedback from {contact_l1.email_normalized}<br>{test_feedback}</p>"
            ),
        )
        ...
        bl_record = self.env["mail.blacklist"].search(
            [("email", "=", contact_l1.email_normalized)]
        )
    ...
    def test_mailing_unsubscribe_from_list_with_update_tour(self):
        ...
        self.assertEqual(
            message_update.body,
            Markup(
                f"<p>{contact_l1.display_name} subscribed to the following mailing list(s)</p>"
        ...
        self.assertEqual(
            message_unsub.body,
            Markup(
                f"<p>{contact_l1.display_name} unsubscribed from the following mailing list(s)</p>"
        ...
        self.assertEqual(
            message_sub.body,
            Markup(
                f"<p>{contact_l3.display_name} subscribed to the following mailing list(s)</p>"
        ...
        self.assertTracking(
            msg_fb, [("opt_out_reason_id", "many2one", False, opt_out_reasons[0])]
        )
    ...
    def test_mailing_unsubscribe_from_my(self):
        ...
        _test_email, test_email_normalized = (
            portal_user.email,
            portal_user.email_normalized,
        )
        opt_out_reasons = self.env["mailing.subscription.optout"].search([])
        ...
        self.assertTrue(subscription_l1.opt_out)
        self.assertEqual(
            subscription_l1.opt_out_datetime,
            self._reference_now,
        ...
        self.assertFalse(subscription_l3.opt_out)
        self.assertFalse(
            subscription_l3.opt_out_datetime,
        ...
        self.assertEqual(
            msg_sub.body,
            Markup(
                f"<p>{contact_l1.name} subscribed to the following mailing list(s)</p>"
                f"<ul><li>{self.mailing_list_2.name}</li></ul>"
            ),
        )
        self.assertEqual(
    ...
    def test_mailing_view(self):
        ...
        doc_id, email_normalized = (
            self.user_marketing.partner_id.id,
            self.user_marketing.email_normalized,
        )
        hash_token = test_mailing._generate_mailing_recipient_token(
            doc_id, email_normalized
        )
        ...
        for test_mid, test_doc_id, test_email, test_token, error_code in [
            ...
            (
                ...
                hash_token,
                401,
            ),  # valid credentials but wrong mailing_id
            (
                0,
                doc_id,
                email_normalized,
                hash_token,
                400,
            ),  # valid credentials but missing mailing_id
        ]:
            with self.subTest(
                test_mid=test_mid,
...
class TestMailingTracking(TestMailingControllersCommon):
    @mute_logger(
        "odoo.addons.mail.models.mail_mail", "odoo.addons.mass_mailing.models.mailing"
    )
    def test_tracking_short_code(self):
        ...
        short_link_url = tools.urls.urljoin(
            mail.get_base_url(), f"r/{link_tracker_code.code}/m/{mailing_trace.id}"
        )
        with freeze_time(self._reference_now):
    ...
    def test_tracking_url_token(self):
        ...
        track_url = tools.urls.urljoin(
            mailing.get_base_url(), f"mail/track/{mail_id_int}/fake_token/blank.gif"
        )
