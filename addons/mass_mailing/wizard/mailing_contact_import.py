...
class MailingContactImport(models.TransientModel):
    ...
    contact_list = fields.Text(
        "Contact List", help="Contact list that will be imported, one contact per line"
    )
    ...
    def action_import(self):
        ...
        existing_contacts = {
            contact.email_normalized: contact for contact in existing_contacts
        }
        ...
        for name, email in contacts:
            ...
            if (
                email in existing_contacts
                and not self.mailing_list_ids < existing_contacts[email].list_ids
            ):
