...
class MailingMailingTest(models.TransientModel):
    ...
    def send_mail_test(self):
        ...
        if record:
            ...
            body = mailing._render_field(
                "body_html",
                record.ids,
                compute_lang=False,
                options={"preserve_comments": True},
            )[record.id]
            preview = mailing._render_field("preview", record.ids, compute_lang=False)[
                record.id
            ]
            full_body = mailing._prepend_preview(Markup(body), preview)
            subject = mailing._render_field("subject", record.ids, compute_lang=False)[
                record.id
            ]
        else:
        ...
        if notification_messages:
            self.mass_mailing_id._message_log(
                ...
                % Markup().join(
                    [
                        Markup("<li>%s</li>") % notification_message
