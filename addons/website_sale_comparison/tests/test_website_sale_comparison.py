...
class TestWebsiteSaleComparison(TransactionCase):
    def test_01_website_sale_comparison_remove(self):
        ...
        product_attributes_body = Website0.viewref(
        ...
        website_sale_comparison = self.env["ir.module.module"].search(
...
class TestWebsiteSaleComparisonUi(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.attribute_line_varieties = cls.env[
        ...
        for variant, price in zip(
            cls.variants_margaux, [487.32, 394.05, 532.44, 1047.84], strict=True
        ):
            variant.product_template_attribute_value_ids.filtered(
                lambda ptav: ptav.attribute_id == cls.attribute_vintage
            ).price_extra = price
    ...
    def test_02_attribute_multiple_lines(self):
        ...
        tr_varieties_simple_att = root.xpath(
            '//div[@id="product_attributes_simple"]//tr'
        )[0]
        text = etree.tostring(
            tr_varieties_simple_att, encoding="unicode", method="text"
        )
        self.assertEqual(
        ...
        # Case compare page
        res = self.url_open(
            "/shop/compare?products=%s"
            % ",".join(str(id) for id in self.variants_margaux.ids)
        )
        self.assertEqual(res.status_code, 200)
        root = etree.fromstring(res.content, etree.HTMLParser())
        ...
        text_vintage = etree.tostring(
            attribute_vintage, encoding="unicode", method="text"
        )
        self.assertEqual(text_vintage.replace(" ", "").replace("\n", ""), "Vintage2018")

        attribute_varieties = table.xpath('//div[@id="o_comparelist_attribute"]')[4]
        text_varieties = etree.tostring(
            attribute_varieties, encoding="unicode", method="text"
        )
        self.assertEqual(
            text_varieties.replace(" ", "").replace("\n", ""),
            "GrapeVarietiesCabernetSauvignon,Merlot,CabernetFranc,PetitVerdot",
                ...
                [
                    (
                        ...
                        OrderedDict(
                            [
                                (
                                    self.attribute_varieties,
                                    OrderedDict(
                                        [
                                            (
                                                variant_margaux,
                                                variant_ptavs.filtered(
                                                    lambda ptav: (
                                                        ptav.attribute_id
                                                        == self.attribute_varieties
                                                    )
