...
class AccountMove(models.Model):
    ...
    @api.depends(
        "country_code", "l10n_ke_cu_qrcode", "state", "move_type", "company_id"
    )
    ...
    def _l10n_ke_fmt(self, string, length, ljust=True):
        ...
        if not string:
            string = ""
        return (
            re.sub(r"[^A-Za-z0-9 ]+", "", str(string))
            .encode("cp1251")
            .ljust(length if ljust else 0)[:length]
        )
    ...
    def _l10n_ke_validate_move(self):
        ...
        for move in self:
            ...
            if move.country_code != "KE":
                move_errors.append(
                    _(
                        "This invoice is not a Kenyan invoice and therefore can not be sent to the device."
                    )
                )
            ...
            if move.company_id.currency_id != self.env.ref("base.KES"):
                move_errors.append(
                    _(
                        "This invoice's company currency is not in Kenyan Shillings, conversion to KES is not possible."
                    )
                )
            ...
            if move.move_type not in ("out_refund", "out_invoice"):
                move_errors.append(
                    _("The document being sent should be an invoice or credit note.")
                )

            if any(
                [
                    move.l10n_ke_cu_invoice_number,
                    move.l10n_ke_cu_serial_number,
                    move.l10n_ke_cu_qrcode,
                    move.l10n_ke_cu_datetime,
                ]
            ):
                move_errors.append(
                    _(
                        "The document already has details related to the fiscal device. Please make sure that the invoice has not already been sent."
                    )
                )
            ...
            if (
                ...
                and not move.reversed_entry_id.l10n_ke_cu_invoice_number
            ):
                move_errors.append(
                    _(
                        "This credit note must reference the previous invoice, and this previous invoice must have already been submitted."
                    )
                )
            ...
            for line in self.invoice_line_ids.filtered(
                ...
                if not vat_taxes or len(vat_taxes) > 1:
                    move_errors.append(
                        _(
                            "On line %s, you must select one and only one VAT tax.",
                            line.name,
                        )
                    )
                elif (
                    vat_taxes[0].amount == 0
                    and not line.tax_ids[0].l10n_ke_item_code_id
                ):
                    move_errors.append(
                        _(
                            "On line %s, a tax with a KRA item code must be selected, since the tax is 0%% or exempt.",
                            line.name,
                        )
                    )
    ...
    def _l10n_ke_cu_open_invoice_message(self):
        ...
        """
        headquarter_address = (self.commercial_partner_id.street or "") + (
            self.commercial_partner_id.street2 or ""
        )
        customer_address = (self.partner_id.street or "") + (
            self.partner_id.street2 or ""
        )
        postcode_and_city = (
            (self.partner_id.zip or "") + "" + (self.partner_id.city or "")
        )
        vat = (
            (self.commercial_partner_id.vat or "").strip()
            if self.commercial_partner_id.country_id.code == "KE"
            else ""
        )
        invoice_elements = [
            ...
            else b"A",  # 1 symbol with value '1' (new invoice), 'A' (credit note), or '@' (debit note)
            self._l10n_ke_fmt(
                self.commercial_partner_id.name, 30
            ),  # 30 symbols for Company name
            self._l10n_ke_fmt(vat, 14),  # 14 Symbols for the client PIN number
            self._l10n_ke_fmt(
                headquarter_address, 30
            ),  # 30 Symbols for customer headquarters
            self._l10n_ke_fmt(customer_address, 30),  # 30 Symbols for the address
            self._l10n_ke_fmt(
                postcode_and_city, 30
            ),  # 30 symbols for the customer post code and city
        ...
        if self.move_type == "out_refund":
            (
                invoice_elements.append(
                    self._l10n_ke_fmt(
                        self.reversed_entry_id.l10n_ke_cu_invoice_number, 19
                    )
                ),
            )  # 19 symbols for related invoice number
        invoice_elements.append(
            re.sub(r"[^A-Za-z0-9 ]+", "", self.name)[-15:].ljust(15).encode("cp1251")
    ...
    def _l10n_ke_cu_lines_messages(self):
        ...
        lines = self.invoice_line_ids.filtered(
            lambda l: l.display_type == "product" and l.quantity and l.price_total
        )
        ...
        # In the case of a refund, use the currency rate of the original invoice
            ...
            currency_rate = abs(
                self.reversed_entry_id.amount_total_signed
                / self.reversed_entry_id.amount_total
            )
        else:
            currency_rate = abs(self.amount_total_signed / self.amount_total)

        discount_dict = {
            line.id: line.discount for line in lines if line.price_total > 0
        }
        for line in lines:
            ...
            candidate_vals_list = [
                l for l in lines if not is_discount_line(l) and is_candidate(l, line)
            ]
            candidate_vals_list = sorted(
                candidate_vals_list,
                key=lambda x: x.price_unit * x.quantity,
                reverse=True,
            )
            ...
            for candidate in candidate_vals_list:
                still_to_discount = abs(
                    ...
                    * candidate.quantity
                    * (100.0 - discount_dict[candidate.id])
                    / 100.0
                )
                if line_to_discount >= still_to_discount:
                    ...
                    rest_to_discount = abs(
                        (line_to_discount / (candidate.price_unit * candidate.quantity))
                        * 100.0
                    )
        ...
        for line in self.invoice_line_ids.filtered(
            lambda l: (
                ...
                and l.quantity
                and l.price_total > 0
                and not discount_dict.get(l.id) >= 100
            )
        ):
            ...
            for tax in tax_details["tax_details_per_record"][line]["tax_details"]:
                if tax.amount in (16, 8, 0):  # This should only occur once
                    line_tax_details = tax_details["tax_details_per_record"][line][
                        "tax_details"
                    ][tax]
            ...
            price = (
                round(
                    price_total / abs(line.quantity) * 100 / (100 - line.discount),
            ...
            line_data = b";".join(
                [
                    self._l10n_ke_fmt(
                        line.name, 36
                    ),  # 36 symbols for the article's name
                    self._l10n_ke_fmt(
                        item_code.tax_rate or "A", 1
                    ...
                    self._l10n_ke_fmt(
                        item_code.description or "", 20
                    ),  # 20 symbols for KRA item code description (can be empty)
                    str(percentage).encode("cp1251")[
                        :5
                    ],  # up to 5 symbols for vat rate
                ]
            )
            ...
            if discount_dict.get(line.id):
                ...
                discount = (
                    discount_sign
                    + str(abs(discount_dict[line.id])).encode("cp1251")[:6]
                )
    ...
    def l10n_ke_action_cu_post(self):
        ...
        if self.company_id.l10n_ke_oscu_is_active:
            raise UserError(
                _(
                    "An OSCU has been initialized for this company. Please send the e-invoice via Send and Print -> Send to eTIMS instead."
                )
            )
            ...
            move.update(
                {
                    ...
                    "l10n_ke_cu_datetime": datetime.strptime(
                        replies[-1], "%d-%m-%Y %H:%M"
                    ),
                }
            )
