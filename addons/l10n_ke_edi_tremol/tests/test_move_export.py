...
class TestKeMoveExport(AccountTestInvoicingCommon):
        ...
        cls.spaceship_tax = cls.env["account.tax"].create(
            {
                ...
                "amount": 0,
                "amount_type": "percent",
                "l10n_ke_item_code_id": cls.env.ref(
                    "l10n_ke.item_code_2023_00391153"
                ).id,
            }
        )
    ...
    def line_dict_to_bytes(cls, line_dict):
        ...
        msg = (
            b"1"
            + b";".join(
                [  # 0x31, command to add a line
                    line_dict.get("name", b"".ljust(36)),  # 36 characters for the name
                    line_dict.get(
                        "vat_class", b"A"
                    ),  # 1 symbol for vat class (a because the tax is 16.0%)
                    line_dict.get(
                        "price", b"1"
                    ),  # up to 15 symbols for the unit price, tax included (up to 5 decimal places)
                    line_dict.get("uom", b"Uni"),  # 3 symbols for uom
                    line_dict.get(
                        "item_code", b"".ljust(10)
                    ),  # 10 symbols for item code (only reported when the tax is not 16.0%)
                    line_dict.get(
                        "item_desc", b"".ljust(20)
                    ),  # item description (only reported when the tex is not 16.0%)
        ...
        if line_dict.get("discount"):
            msg += b"," + line_dict.get(
                "discount"
            )  # 1 to 7 symbols for discount/addition
        return msg
    ...
    def test_export_global_discount_invoice(self):
                            ...
                            "tax_ids": [
                                (
                                    ...
                                    [
                                        self.company_data[
                                            "company"
                                        ].tax_config_id.account_sale_tax_id.id
                                    ],
                                )
                            ],
                            "discount": 10,
        ...
        expected_double_negative_header = [
            b"01;     0;0;1;Sirius Cybernetics Corporation;A000123456F   ;Test StreetFurther Test Street;Test StreetFurther Test Street;00500Nairobi                  ;                              ;INV202300002   "
        ]
