...
class TestMarketingCardMail(MailCase, MarketingCardCommon):
    ...
    def test_campaign_send_mailing(self):
        ...
        self.env.user.sudo().group_ids += self.env.ref(
    ...
    def test_campaign_send_mailing_with_duplicates(self):
        ...
        partners = (
            ...
            .create(
                [
                    {"name": f"Part{n}", "email": f"email{n % 3}@test.lan"}
                    for n in range(10)
                ]
            )
        )
...
class TestMarketingCardRender(MarketingCardCommon):
    ...
    def test_campaign(self):
        ...
        role_values = _extract_values_from_document(
            html.fromstring(self._render_image_bodies[0])
        )
        self.assertEqual(
            role_values["body"].attrib["style"],
        ...
        card = self.env["card.card"].search(
            [("campaign_id", "=", campaign.id), ("active", "=", False)]
        )
        self.assertEqual(len(card), 1)
        ...
        self.assertFalse(
            ...
            "Updating the preview record does not refresh the preview.",
        )
        ...
        with patch(
            ...
            # mismatch with card
            self.env["card.card"].sudo().create(
                {"campaign_id": self.static_campaign.id, "res_id": 1}
            )
    ...
    def test_fetch_datetime(self):
        ...
        self.assertEqual(utc_header, datetime(2020, 5, 5, 12, 0, 0))
        self.assertEqual(brussels_header, datetime(2020, 5, 5, 14, 0, 0))
        self.assertEqual(tokyo_header, datetime(2020, 5, 5, 21, 0, 0))
...
class TestMarketingCardRouting(HttpCase, MarketingCardCommon):
    ...
    def test_campaign_stats(self):
        ...
        cards = self.campaign._update_cards([("id", "in", partners.ids)]).sorted(
            "res_id"
        )
        ...
        self.assertEqual(
            self.campaign.action_preview()["url"], card._get_path("preview")
        )
        ...
        self.assertEqual(
            card.active,
            False,
            "preview card was updated and is thus considered not valid",
        )
        self.campaign.flush_recordset()
        self.assertEqual(self.campaign.card_count, 19)
        self.assertEqual(self.campaign.card_click_count, 0)
        self.assertEqual(
            self.campaign.card_share_count,
            0,
        ...
        opengraph_view = html.fromstring(
            self.url_open(card._get_redirect_url()).content
        )
        self.assertTrue(
            opengraph_view is not None,
            "Crawler should get a valid html page as response",
        )
        opengraph_image_url_element = opengraph_view.find(
            './/meta[@property="og:image"]'
        )
        self.assertTrue(
            opengraph_image_url_element is not None,
            "page should contain image opengraph node",
        )
        ...
        self.assertEqual(
            self.campaign.card_share_count,
            1,
            "A crawler fetching the card is considered a share.",
        )
        ...
        redirect_response = self.url_open(
            card._get_redirect_url(), allow_redirects=False
        )
        self.assertEqual(redirect_response.status_code, 303)
        self.assertEqual(
            redirect_response._next.url, self.campaign.link_tracker_id.short_url
        )
        ...
        self.assertEqual(self.campaign.card_count, 20)
        self.assertEqual(
            self.campaign.card_click_count,
            20,
            "Shared cards are considered implicitly visited",
        )
...
class TestMarketingCardSecurity(MarketingCardCommon):
    ...
    def test_campaign_field_paths(self):
        ...
        self.marketing_card_user.partner_id.state_id = (
        ...
        with self.assertRaises(exceptions.UserError), self.mock_image_renderer():
            campaign._update_cards(
                [("id", "=", self.marketing_card_user.partner_id.id)]
            )
        self.assertFalse(
            self._render_image_bodies,
            "There should have been no render on illegal fields",
        )

        with self.mock_image_renderer():
            campaign.with_user(self.system_admin)._update_cards(
                [("id", "=", self.marketing_card_user.partner_id.id)]
            )
    ...
    def test_mail_render_security_body_html(self):
        ...
        with self.assertRaisesRegex(
            exceptions.AccessError, "You are not allowed to modify"
        ):
    ...
    def test_mail_render_security_render_field_write_access(self):
        ...
        self.assertTrue(
            all(
                ...
                for field in CardCampaign._fields.values()
                if hasattr(field, "render_engine")
            )
        )
