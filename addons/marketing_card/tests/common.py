...
class MockImageRender(BaseCase):
    ...
    def mock_image_renderer(self, collect_params=True):
        ...
        def _mock_render_html_to_image(
            model, bodies, width, height, image_format="jpg"
        ):
            if collect_params:
        ...
        with patch.object(
            IrActionsReport, "_render_html_to_image", _mock_render_html_to_image
        ):
            yield
    ...
    def setUpClass(cls):
        ...
        cls.partners = cls.env["res.partner"].create(
            [
                ...
                {
                    "name": "Bob",
                    "email": "bob@justbob.me",
                    "phone_ids": [Command.create({"number": "+32 123 446 789"})],
                    "image_1920": base64.b64encode(VALID_JPEG),
                },
            ]
        )
...
<html>
    ...
    <div id="body" t-attf-style="background-image: url('data:image/png;base64,{{card_campaign.content_background or card_campaign.card_template_id.default_background}}');">
        ...
        cls.campaign = (
            cls.env["card.campaign"]
            .with_user(cls.marketing_card_user)
            .create(
                {
                    "name": "Test Campaign",
