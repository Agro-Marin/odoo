...
const setupWebRtc = () => {
    ...
    const mockBusService = {
        ...
        unsubscribe: (type, callback) =>
            bus.removeEventListener(type, busCallbacks.get(callback)),
...
describe("opening connection", () => {
    ...
    test("saves connection to connections dict", async () => {
        ...
        expect(webRtc.connections["iot"].connection).toBeInstanceOf(
    ...
    test("sets remote offer received via bus", async () => {
        ...
        expect(webRtc.connections["iot"].connection.remoteDescription).toBe(
            "mockAnswer",
        );
    });
    ...
    test("opens separate connections per IoT box", async () => {
        ...
        expect(webRtc.connections["iot2"].connection.remoteDescription).toBe(
            "mockAnswer2",
        );
    });
});
...
describe("sending message", () => {
    test("message is sent", async () => {
        ...
        const sentMessage = JSON.parse(
            webRtc.connections["iot"].channel._messagesSent[0],
        );
    ...
    test("large messages are chunked", async () => {
        ...
        expect(webRtc.connections["iot"].channel._messagesSent[0]).toBe(
            "chunked_start",
        );
