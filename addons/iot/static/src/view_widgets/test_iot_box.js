...
export class TestIotBox extends Component {
    ...
    async onClick() {
        ...
        this.removeTestingNotification = this.notification.add(
            _t(
                "Testing communication with IoT Box and network quality, please wait...",
            ),
            {
                type: "info",
        ...
        if (!/\d{4}\.\d{2}\.\d{2}/.test(version)) {
            try {
                await this.iotHttpService.webRtc.onMessage(
                    identifier,
                    identifier,
                    requestId,
                    () => {},
                ...
                await this.iotHttpService.webRtc.sendMessage(
                    identifier,
                    {},
                    requestId,
                    "test_protocol",
    ...
    onConnectionTestSuccess(data) {
        ...
        if (this.completeSuccess) {
            this.notification.add(
                _t("All communication protocols are working properly."),
                { type: "success" },
            );
        }
        if (!data.result) {
