    ...
    def test_iot_device_test_button(self):
        ...
        self.assertEqual(
            ...
            (
                ...
                % [
                    next(iter(message.keys()))
                    for message in self.iot_websocket_messages
                ]
            ),
        )
        self.assertIn(
            "webrtc_offer",
            self.iot_websocket_messages[0],
            "First ws message should be of type 'webrtc_offer'.",
        )
        self.assertIn(
            "iot_action",
            self.iot_websocket_messages[1],
            "Second ws message should be of type 'iot_action'.",
        )
