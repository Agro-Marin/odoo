...
class TestSurveyRandomize(TransactionCase):
    def test_01_generate_randomized_questions(self):
        ...
        self.assertEqual(
            len(generated_questions.ids), 10, msg="Expected 10 unique questions"
        )
        self.assertEqual(
            len(
                generated_questions.filtered(
                    lambda question: question.page_id == page_1
                )
            ),
            3,
        ...
        self.assertEqual(
            len(
                generated_questions.filtered(
                    lambda question: question.page_id == page_2
                )
            ),
            5,
        ...
        self.assertEqual(
            len(
                generated_questions.filtered(
                    lambda question: question.page_id == page_3
                )
            ),
            2,
