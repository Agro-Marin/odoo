...
class TestCertificationBadge(common.TestSurveyCommon):
    ...
    def test_badge_configuration(self):
        ...
        self.assertEqual(
            len(challenge),
            0,
            """A challenge should not exist or be linked to the certification badge
        ...
        challenge_line = self.env["gamification.challenge.line"].search(
        ...
        self.assertEqual(
            ...
            self.certification_survey.certification_badge_id.id,
        ...
        self.assertEqual(
            duplicate_survey.certification_badge_id,
            self.env["gamification.badge"],
    ...
    def test_certification_badge_access(self):
        ...
        with self.assertRaises(AccessError):
            self.certification_badge.with_user(self.user_portal).write(
                {"description": "Guy, you just can't do that!"}
            )
        with self.assertRaises(AccessError):
    ...
    def test_badge_configuration_multi(self):
        ...
        certification_surveys = self.env["survey.survey"].browse(
            [survey_1.id, survey_2.id, survey_3.id]
        )
        self.assertEqual(
            len(certification_surveys),
            3,
            "There should be 3 certification survey created",
        )
        ...
        challenges = self.env["gamification.challenge"].search(
            [
                (
                    "reward_id",
                    "in",
                    certification_surveys.mapped("certification_badge_id").ids,
                )
            ]
        )
        self.assertEqual(len(challenges), 3, "3 challenges should be created")
        challenge_lines = self.env["gamification.challenge.line"].search(
        ...
        for survey in certification_surveys:
            self.assertEqual(
                survey.certification_badge_id.active,
                False,
        ...
        self.assertEqual(
            len(challenge_lines), 0, "The 3 challenge_lines should be deleted"
        )
