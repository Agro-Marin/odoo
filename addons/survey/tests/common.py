...
class SurveyCase(common.TransactionCase):
    ...
    def _add_question(self, page, name, qtype, **kwargs):
        ...
        if qtype in ("simple_choice", "dropdown", "multiple_choice"):
    ...
    def _access_page(self, survey, token, query_count=None):
        with self.assertQueryCount(query_count) if query_count else nullcontext():
            return self.url_open(f"/survey/{survey.access_token}/{token}")
    ...
    def _access_submit(self, survey, token, post_data, query_count=None):
        url = survey.get_base_url() + f"/survey/submit/{survey.access_token}/{token}"
        with self.assertQueryCount(query_count) if query_count else nullcontext():
            return self.url_open(url, json={"params": post_data})

    def _find_csrf_token(self, text):
        csrf_token_re = re.compile(
            r'(input.+csrf_token.+value=")([a-f0-9]{64}o[0-9]*)', re.MULTILINE
        )
    ...
    def _answer_question(
        ...
        response = self._access_submit(
            question.survey_id, answer_token, post_data, query_count=submit_query_count
        )
    ...
    def _assert_skipped_question(self, question, survey_user):
        ...
        self.assertEqual(
            len(
                question_data.get("answer_input_ids")
                - question_data.get("answer_input_done_ids")
            ),
            1,
        )
    ...
    def _create_one_question_per_type(self):
        ...
        for question_type, _dummy in (
            ...
            elif question_type in ("matrix", "likert"):
                kwargs["labels"] = [{"value": "Column0"}, {"value": "Column1"}]
                kwargs["labels_2"] = [{"value": "Row0"}, {"value": "Row1"}]
            all_questions |= self._add_question(
                self.page_0, "Q0", question_type, **kwargs
            )
...
class TestSurveyResultsCommon(SurveyCase):
    ...
    def setUpClass(cls):
        ...
        cls.question_mx1 = cls._add_question(
            cls,
            None,
            "When do you harvest those fruits",
            "matrix",
            survey_id=cls.survey.id,
            sequence="5",
        ...
        cls.question_mx2 = cls._add_question(
            cls,
            None,
            "How often should you water those plants",
            "matrix",
            survey_id=cls.survey.id,
            sequence="6",
            labels=[{"value": "Once a month"}, {"value": "Once a week"}],
            labels_2=[{"value": "Cactus"}, {"value": "Ficus"}],
        )
        ...
        [cls.apples_row_id, cls.strawberries_row_id] = (
            cls.question_mx1.matrix_row_ids.ids
        )
        ...
        [cls.once_a_month_id, cls.once_a_week_id] = (
        ...
        cls.answer_lukas = cls._add_answer_line(
            cls, cls.question_char_box, cls.user_input_1, "Lukas"
        )
        cls.answer_24 = cls._add_answer_line(
            cls, cls.question_numerical_box, cls.user_input_1, 24
        )
        cls.answer_cat = cls._add_answer_line(
            cls, cls.question_sc, cls.user_input_1, cls.cat_id
        )
        ...
        cls.user_input_2 = cls._add_answer(
            cls, cls.survey, cls.survey_manager.partner_id
        )
        cls.answer_pauline = cls._add_answer_line(
            cls, cls.question_char_box, cls.user_input_2, "Pauline"
        )
        cls._add_answer_line(cls, cls.question_numerical_box, cls.user_input_2, 24)
        cls.answer_dog = cls._add_answer_line(
            cls, cls.question_sc, cls.user_input_2, cls.dog_id
        )
        ...
        cls.scale_answer_line_2 = cls._add_answer_line(
            cls, cls.question_scale, cls.user_input_1, "7"
        )
