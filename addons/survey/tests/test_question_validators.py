...
class TestQuestionValidators(common.TestSurveyCommon):
    ...
    def test_ranking_requires_every_item(self):
        ...
        question = self._question('ranking', suggested_answer_ids=[
        ...
        self.assertFalse(question._validate_ranking({
            str(answers[0].id): 1, str(answers[1].id): 2,
        }))
    ...
    def test_file_upload_accepts_allowed_extension(self):
        ...
        question = self._question(
            'file_upload', file_upload_types='.pdf,.png', file_upload_max_size=5,
        )
        self.assertFalse(
    ...
    def test_file_upload_rejects_other_extension(self):
        ...
        self.assertIn(
            'file type',
            str(question._validate_file_upload(
                self._attachment('payload.exe').id)).lower(),
        )
    ...
    def test_file_upload_default_types_are_enforced(self):
        ...
        self.assertIn(
            'file type',
            str(question._validate_file_upload(
                self._attachment('payload.bin').id)).lower(),
        )
