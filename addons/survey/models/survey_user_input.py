...
class SurveyUser_Input(models.Model):
    ...
    def _fire_completion_webhook(self) -> None:
        """POST survey response data to the configured webhook URL after commit.

        Uses the postcommit hook pattern from ir.actions.server to ensure the
        webhook only fires after the transaction is successfully committed.
        """
        self.ensure_one()
        webhook_url = self.survey_id.webhook_url
        ...
        def do_post():
            try:
                ...
                _logger.warning(
                    "Survey webhook failed for input %s to %s",
                    self.id,
                    webhook_url,
                    exc_info=True,
                )
    ...
    def _prepare_webhook_payload(self) -> dict[str, Any]:
        """Build the JSON payload for the completion webhook."""
        self.ensure_one()
        answers = []
