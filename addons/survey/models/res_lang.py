...
class ResLang(models.Model):
    ...
    def write(self, vals: ValuesType) -> Literal[True]:
        ...
        if "active" in vals and not vals["active"]:
            ...
            surveys_sudo = (
                self.env["survey.survey"].sudo().search([("lang_ids", "in", self.ids)])
            )
            if will_be_all_lang_survey_sudo := surveys_sudo.filtered(
                ...
                if (
                    self.env["survey.survey"].search(
                        [("id", "in", will_be_all_lang_survey_sudo.ids)]
                    )
                    ...
                    error += _(
                        ...
                        surveys_list=", ".join(
                            f'"{survey.title}"'
                            for survey in will_be_all_lang_survey_sudo
                        ),
                    )
