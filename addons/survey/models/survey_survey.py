...
class SurveySurvey(models.Model):
    ...
    def _compute_scoring_max_obtainable(self) -> None:
        ...
        Must mirror the per-type logic of ``_compute_scoring_values`` on
        ``survey.user_input``:

        - simple_choice / dropdown: only ONE answer can be selected → max
        - multiple_choice: ALL correct answers can be selected → sum
        - other scored types (numerical_box, date, datetime): question-level score
        """
        for survey in self:
            total = 0.0
            ...
            survey.scoring_max_obtainable = total
    ...
    def _compute_survey_statistic(self) -> None:
        ...
        for survey_stats in stat.values():
            ...
            # Average score and success ratio should only consider completed
            # responses — in-progress inputs have incomplete (zero) scores
    ...
    def action_end_session(self) -> None:
        ...
        # Use _mark_done() instead of a bare write so that end_datetime,
        # calculated fields, webhooks, and follow-up rules are processed.
        # Certification/badge logic is already guarded by session_available
        # excluding certifications, so those branches are safely skipped.
        if session_inputs:
