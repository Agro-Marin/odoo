...
class Survey(http.Controller):
    ...
    @http.route(
        "/s/<string:slug_or_token>", type="http", auth="public", website=True
    )
    def survey_start_by_slug(
        self, slug_or_token: str, **post: Any
    ) -> Response:
        """Start a survey using a custom slug or short token.

        Looks up the survey by ``slug`` field first, then falls back to
        matching the first 6 characters of ``access_token`` (short URL).
        """
        survey = request.env["survey.survey"].sudo().search(
            [("slug", "=", slug_or_token), ("active", "=", True)], limit=1
        )
        if not survey:
            # Fallback to short token (first 6 chars of access_token)
            survey = request.env["survey.survey"].sudo().search(
                [("access_token", "=like", f"{slug_or_token}%"), ("active", "=", True)],
                limit=1,
            )
        if not survey:
            return request.redirect("/")
        return self.survey_start(survey.access_token, **post)
        ...
        for line in answer_sudo.user_input_line_ids.filtered(
            ...
            if answer.skip_action == "end_survey":
                ...
                if target.id in survey_sudo.question_and_page_ids.ids:
                    answer_sudo.last_displayed_page_id = target.id
                    return correct_answers, self._prepare_question_html(
                        survey_sudo, answer_sudo
                    )
        return None
