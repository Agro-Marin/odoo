...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        def filter_peppol_state(moves, states):
            return peppol_partner(
                moves.filtered(
                    lambda m: peppol_partner(m).peppol_verification_state in states
                )
            )

        alerts = super()._get_alerts(moves, moves_data)
        # Check for invalid peppol partners.
        peppol_moves = moves.filtered(
            lambda m: "peppol" in moves_data[m]["sending_methods"]
        )
        ...
        if invalid_partners and "account_edi_ubl_cii_configure_partner" not in alerts:
            alerts["account_peppol_warning_partner"] = {
                "message": _(
                    "Customer is on Peppol but did not enable receiving documents."
                ),
                "action_text": _("View Partner(s)"),
                "action": invalid_partners._get_records_action(
                    name=_("Check Partner(s)")
                ),
            }
        not_peppol_moves = moves.filtered(
            lambda m: "peppol" not in moves_data[m]["sending_methods"]
        )
        ...
        any_moves_not_sent_peppol = any(
            move.peppol_move_state not in ("processing", "done") for move in moves
        )
        ...
        if all(
            ...
            (
                (
                    peppol_not_selected_partners := filter_peppol_state(
                        not_peppol_moves, ["valid"]
                    )
                ),
                any_moves_not_sent_peppol,
                len(peppol_not_selected_partners)
                == 1,  # Check for not peppol partners that are on the network
            )
        ):
    ...
    def _get_default_invoice_edi_format(self, move, **kwargs) -> str:
        ...
        if "peppol" in kwargs.get("sending_methods", []):
            return move.partner_id.with_company(
                move.company_id
        ...
        if len(moves.company_id) == 1:
            if not moves.company_id.peppol_can_send:
                return (
                    ...
                    ._action_view_peppol_form(reopen=False)
                )
    ...
    def _is_applicable_to_move(self, method, move, **move_data):
        # EXTENDS 'account'
        if method == "peppol":
            partner = move.partner_id.commercial_partner_id.with_company(
                move.company_id
            )
            invoice_edi_format = (
                move_data.get("invoice_edi_format") or partner._get_peppol_edi_format()
            )
    ...
    def _hook_if_errors(self, moves_data, allow_raising=True):
        ...
        for move, move_data in moves_data.items():
            if "peppol" in move_data["sending_methods"] and move_data.get(
                "blocking_error"
            ):
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            partner = invoice.partner_id.commercial_partner_id.with_company(
                invoice.company_id
            )
            if "peppol" in invoice_data[
                "sending_methods"
            ] and self._is_applicable_to_move("peppol", invoice, **invoice_data):
                if invoice_data.get("ubl_cii_xml_attachment_values"):
                    xml_file = invoice_data["ubl_cii_xml_attachment_values"]["raw"]
                    filename = invoice_data["ubl_cii_xml_attachment_values"]["name"]
                elif invoice.ubl_cii_xml_id and invoice.peppol_move_state not in (
                    "processing",
                    "done",
                ):
                ...
                if len(xml_file) > 64000000:
                    invoice_data["error"] = _(
                        "Invoice %s is too big to send via peppol (64MB limit)",
                        invoice.name,
                    )
                    continue

                receiver_identification = (
                    f"{partner.peppol_eas}:{partner.peppol_endpoint}"
                )
        ...
        try:
            ...
            if error_vals := response.get("error"):
                ...
                attachments_linked_message = _(
                    "The invoice has been sent to the Peppol Access Point. The following attachments were sent with the XML:"
                )
                attachments_not_linked_message = _(
                ...
                for message, (invoice, invoice_data) in zip(
                    ...
                    attachments_linked, attachments_not_linked = (
                        self._get_ubl_available_attachments(
                    ...
                    if attachments_not_linked:
                        invoice._message_log(
                            ...
                            attachment_ids=attachments_not_linked.mapped("id"),
                        )
                    ...
                    if new_message.attachment_ids.ids:
                        if (
                            ...
                            in new_message.attachment_ids
                        ):
                            invoice.message_main_attachment_id = None
                        # = ANY(%s)/list, not IN %s/tuple: under psycopg3 the
                        # tuple binds as a single $1 → "IN $1" syntax error.
                        self.env.cr.execute(
    ...
    def action_what_is_peppol_activate(self, moves):
        ...
        if len(companies) == 1 and not companies.peppol_can_send:
            ...
            return {
                ...
                "target": "new",
                "context": {"active_model": "account.move", "active_ids": moves.ids},
            }
