...
class AccountMove(models.Model):
    ...
    peppol_move_state = fields.Selection(
        selection=[
            ...
            (
                "skipped",
                "Skipped",
            ),  # TODO remove this state in master, we now put a regular error.
    ...
    def _notify_by_email_prepare_rendering_context(
        ...
        if (
            ...
            and invoice_country in PEPPOL_MAILING_COUNTRIES
        ):
            render_context["peppol_info"] = {
                "peppol_country": invoice_country,
                "is_peppol_sent": invoice.peppol_move_state in ("processing", "done"),
                "partner_on_peppol": invoice.commercial_partner_id.peppol_verification_state
                in ("valid", "not_valid_format"),
            }
