...
class ResPartner(models.Model):
    ...
    available_peppol_sending_methods = fields.Json(
    ...
    peppol_verification_state = fields.Selection(
        selection=[
            ...
            ),  # does not exist on Peppol at all
            (
                "not_valid_format",
                "Partner cannot receive format",
    ...
    def _compute_available_peppol_edi_formats(self):
        for partner in self:
            if partner.invoice_sending_method == "peppol":
                ...
                partner.available_peppol_edi_formats = list(
                    dict(self._fields["invoice_edi_format"].selection)
                )
    ...
    def _log_verification_state_update(self, company, old_value, new_value):
        ...
        old_label = (
            selection_values[old_value] if old_value else False
    ...
    def _check_peppol_participant_exists(self, participant_info, edi_identification):
        ...
        if isinstance(participant_info, dict):
            ...
            participant_identifier = (
                participant_info.findtext("{*}ParticipantIdentifier") or ""
            )
        ...
        return (
            edi_identification.lower() == participant_identifier.lower()
    ...
    def _peppol_lookup_participant(self, edi_identification):
        ...
        origin = self.env["account_edi_proxy_client.user"]._get_proxy_urls()["peppol"][
            edi_mode
        ]
        ...
        try:
            ...
            _logger.error(
                "invalid JSON response %s when querying peppol participant %s",
        ...
        if error := decoded_response.get("error"):
            if error.get("code") != "NOT_FOUND":
                _logger.error(
                    "error when querying peppol participant %s: %s",
                    edi_identification,
        ...
        if not response.ok:
            _logger.error(
                "unsuccessful response %s when querying peppol participant %s",
                response.status_code,
                edi_identification,
            )
            return None
    ...
    def _check_document_type_support(
        self, participant_info, ubl_cii_format, process_type="billing"
    ):
        edi_builder = self._get_edi_builder(ubl_cii_format)
        expected_customization_id = edi_builder._get_customization_id(
            process_type=process_type
        )
        if isinstance(participant_info, dict):
            return any(
                expected_customization_id in (service.get("document_id") or "")
        ...
        for service in service_references:
            if expected_customization_id in parse.unquote_plus(
                service.attrib.get("href", "")
            ):
                return True
        return False
            ...
            partners = self.filtered(
                lambda p: all(
                    [
                        ...
                        p.peppol_endpoint,
                        p.is_ubl_format,
                        p.country_code in PEPPOL_LIST,
                    ]
                )
            )
        elif {"peppol_eas", "peppol_endpoint", "invoice_edi_format"}.intersection(
            vals.keys()
        ):
    ...
    def button_account_peppol_check_partner_endpoint(self, company=None):
        ...
        if new_value != "valid" and self_partner.peppol_eas in ("0208", "9925"):
            ...
            inverse_endpoint = (
                f"BE{self_partner.peppol_endpoint}"
                if self_partner.peppol_eas == "0208"
                else self_partner.peppol_endpoint[2:]
            )
            if (
                peppol_state := self._get_peppol_verification_state(
                    inverse_endpoint, inverse_eas, self_partner._get_peppol_edi_format()
                )
            ) == "valid":
                self_partner.write(
                    {
        ...
        if old_value != new_value:
            ...
            self._log_verification_state_update(
                company, old_value, self_partner.peppol_verification_state
            )
        return False

    @api.model
    @handle_demo
    def _get_peppol_verification_state(
        self, peppol_endpoint, peppol_eas, invoice_edi_format, process_type="billing"
    ):
        if (
            not (peppol_eas and peppol_endpoint)
            or invoice_edi_format not in self._get_peppol_formats()
        ):
            return "not_verified"
        ...
        if participant_info is None:
            ...
            is_participant_on_network = self._check_peppol_participant_exists(
                participant_info, edi_identification
            )
            if is_participant_on_network:
                is_valid_format = self._check_document_type_support(
                    participant_info, invoice_edi_format, process_type=process_type
                )
                if is_valid_format:
                    return "valid"
                else:
