...
class ResConfigSettings(models.TransientModel):
    ...
    peppol_parent_company_name = fields.Char(
        compute="_compute_peppol_use_parent_company"
    )
    ...
    def _inverse_peppol_participation_role(self):
        for record in self:
            if record.account_peppol_edi_user:
                if (
                    record.peppol_participation_role == "sending_only"
                    and record.account_peppol_proxy_state != "sender"
                ):
                    ...
                    record.peppol_participation_role == "sending_and_receiving"
                    and record.account_peppol_proxy_state
                    not in ("smp_registration", "receiver")
                ):
    ...
    def action_view_peppol_form(self):
        registration_wizard = self.env["peppol.registration"].create(
    ...
    def button_peppol_disconnect_branch_from_parent(self):
        ...
        return {
            ...
            "params": {
                ...
                "type": "success",
                "message": _(
                    "Disconnected this branch company peppol configuration from %s.",
