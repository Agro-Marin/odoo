...
class Account_Edi_Proxy_ClientUser(models.Model):
    ...
    proxy_type = fields.Selection(
        selection_add=[("peppol", "PEPPOL")], ondelete={"peppol": "cascade"}
    )
    ...
    def _call_peppol_proxy(self, endpoint, params=None):
        ...
        try:
            ...
            if (
                e.code == "no_such_user"
                and not self.active
                and not self.company_id.account_edi_proxy_client_ids.filtered(
    ...
    def _peppol_out_of_sync_reconnect_this_database(self):
        ...
        if response.get("error"):
            ...
            raise AccountEdiProxyError(
                ...
                response["error"].get(
                    "message",
                    "An unknown error occurred while authenticating with IAP server.",
                ),
            )
        self.write(
            {
    ...
    def _peppol_import_invoice(self, attachment, peppol_state, uuid, journal=None):
        ...
        if (
            file_data["xml_tree"].findtext(".//{*}InvoiceTypeCode") in ["389", "527"]
            or file_data["xml_tree"].findtext(".//{*}CreditNoteTypeCode") == "261"
        ):
            is_self_billed = True
        ...
        if not is_self_billed:
            ...
            journal = journal or self.env["account.journal"].search(
                [
        ...
        try:
            ...
            _logger.exception(
                "Unexpected error occurred during the import of bill with id %s",
                move.id,
            )
    ...
    def _peppol_get_new_documents(self, skip_no_journal=False):
        ...
        for edi_user in self:
            ...
            try:
                ...
                _logger.error(
                    "Error while receiving the document from Peppol Proxy: %s",
                    e.message,
                )
                continue
            ...
            for uuid, content in all_messages.items():
                ...
                vals_to_ack = edi_user._peppol_import_invoice(
                    attachment, content["state"], uuid
                )
    ...
    def _peppol_get_message_status(self):
        ...
        for edi_user in self:
            ...
            message_uuids = {
                move.peppol_message_uuid: move for move in edi_user_moves[:job_count]
            }
    ...
    def _peppol_get_participant_status(self):
        for edi_user in self:
            ...
            if edi_user.proxy_type != "peppol":
                continue
            try:
                proxy_user = edi_user._prepare_request(
                ...
                if e.code == "client_gone":
                    ...
                    _logger.error(
                        "Error while updating Peppol participant status: %s", e
                    )
                continue

            if "error" in proxy_user:
                error_message = proxy_user["error"].get("message") or proxy_user[
                    "error"
                ].get("data", {}).get("message")
                _logger.error(
                    "Error while updating Peppol participant status: %s", error_message
                )
                continue
    ...
    def _peppol_register_sender_as_receiver(self):
        ...
        is_on_peppol, external_provider, error_msg = (
            peppol_info["is_on_peppol"],
            peppol_info["external_provider"],
        ...
        self._call_peppol_proxy(
            ...
            params={
                ...
                "supported_identifiers": list(
                    company._peppol_supported_document_types()
                ),
            },
        )
    ...
    def _peppol_deregister_participant(self):
        ...
        try:
            ...
            proxy_user = self._prepare_request(
                f"{self._get_server_url()}/api/peppol/2/participant_status"
            )
        ...
        try:
            if not (
                payload := tools.resolve_hash_signed(
                    self.sudo().env, "account_peppol_webhook", token
                )
            ):
                return None
        except ValueError:
    ...
    def _peppol_reset_webhook(self):
        for edi_user in self:
            edi_user._call_peppol_proxy(
                "/api/peppol/2/set_webhook",
                params={
                    "webhook_url": edi_user.company_id._get_peppol_webhook_endpoint(),
                    "token": self._generate_webhook_token(edi_user.company_id),
                },
            )
