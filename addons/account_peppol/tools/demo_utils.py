...
def _mock_call_peppol_proxy(func, self, endpoint, params=None):
    ...
    return {
        ...
        "get_services": lambda _user: {
            "services": self.env["res.company"]._peppol_supported_document_types()
        },
...
def _mock_create_connection(func, self, *args, **kwargs):
    ...
    dummy_response = {
        "id_client": f"demo4peppol_{company.id}",
        "refresh_token": "demo",
        "peppol_state": "receiver",
    }
    private_key_sudo = (
    ...
    content_to_key_id = {
        attachment.datas: attachment.res_id for attachment in attachments
    }
