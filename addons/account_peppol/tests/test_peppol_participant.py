...
class TestPeppolParticipant(PeppolConnectorCommon):
            ...
            [
                {
                    ...
                    "content": b64encode(
                        file_open(
                            "account_peppol/tests/assets/private_key.pem", "rb"
                        ).read()
                    ),
                }
            ]
    ...
    def test_ignore_archived_edi_users(self):
            ...
            [
                {
                    ...
                    "private_key_id": self.env["certificate.key"]
                    .sudo()
                    ._generate_rsa_private_key(self.env.company)
                    .id,
                    "refresh_token": False,
                    "proxy_type": "peppol",
                    "edi_mode": "demo",
        ...
        with self._mock_requests(
            ...
            self.env.company.with_context(
                active_test=False
            ).partner_id.button_account_peppol_check_partner_endpoint()
    ...
    def test_register_participant_for_the_first_time_as_sender_then_receiver_then_unregister(
        self,
    ):
    ...
    def test_register_participant_rejected(self):
        with (
            ...
            self.assertRaisesRegex(
                UserError, "There was an issue with the Peppol Participant"
            ),
        ):
            wizard = self.env["peppol.registration"].create({})
            self.assertRecordValues(wizard, [{"smp_registration": True}])
    ...
    def test_peppol_registration_register_as_self(self):
        ...
        wizard = (
            self.env["peppol.registration"]
            .with_context(allowed_company_ids=(branch + self.env.company).ids)
            .create({})
        )
        self.assertRecordValues(
            wizard,
    ...
    def test_peppol_registration_register_as_parent(self):
        ...
        with self._mock_requests(
            [
                self._mock_can_connect(),
                self._mock_connect(
                    peppol_state="sender", id_client="test_id_client_branch"
                ),
            ]
        ):
            wizard.button_register_peppol_participant()
