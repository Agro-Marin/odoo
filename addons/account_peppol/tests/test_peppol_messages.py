...
class TestPeppolMessage(TestAccountMoveSendCommon, MailCommon):
    ...
    def _request_handler(cls, s: Session, r: PreparedRequest, /, **kw):
        ...
        if r.path_url.startswith("/api/peppol/1/lookup"):
            peppol_identifier = parse.parse_qs(r.path_url.rsplit("?")[1])[
                "peppol_identifier"
            ][0].lower()
        ...
        if url == "/api/peppol/1/send_document":
            ...
            response.json = lambda: {
                "result": {"messages": [{"message_uuid": FAKE_UUID[0]}] * num_invoices}
            }
            return response
                    ...
                    "messages": [
                        {
                            ...
                            "error": False
                            if not cls.env.context.get("error")
                            else "Test error",
                        }
                    ],
                }
        ...
        if url == "/api/peppol/1/get_document":
            ...
            if uuid == FAKE_UUID[0]:
                ...
                response_content = {
                    ...
                    "document": b64encode(
                        file_open(
                            f"{FILE_PATH}/{cls.mocked_incoming_invoice_fname}",
                            mode="rb",
                        ).read()
                    ),
                    "state": "done" if not cls.env.context.get("error") else "error",
    ...
    def test_non_xml_compatible_characters(self):
        ...
        product_a = self._create_product(
            name="Test\x02Product", lst_price=1000.0, standard_price=800.0
        )
        ...
        self.assertEqual(
            self._get_mail_message(move).preview,
    ...
    def test_send_peppol_alerts_not_valid_partner(self):
        ...
        self.assertEqual(wizard.invoice_edi_format, "ubl_bis3")
        self.assertEqual(
            self.invalid_partner.peppol_verification_state, "not_valid"
        )  # not on peppol at all
        self.assertFalse(
            wizard.sending_methods and "peppol" in wizard.sending_methods
        )  # peppol is not checked
        self.assertTrue(
            wizard.sending_method_checkboxes["peppol"]["readonly"]
        )  # peppol is not possible to select
        self.assertFalse(wizard.alerts)  # there is no alerts

    @patch(
        "odoo.addons.account_peppol.models.res_partner.ResPartner._check_document_type_support",
        return_value=False,
    )
    def test_send_peppol_alerts_not_valid_format_partner(self, mocked_check):
        ...
        self.assertEqual(wizard.invoice_edi_format, "ubl_bis3")
        self.assertEqual(
            self.valid_partner.peppol_verification_state, "not_valid_format"
        )  # on peppol but can't receive bis3
    ...
    def test_send_peppol_alerts_invalid_partner(self):
        ...
        self.assertFalse(
            wizard.sending_methods and "peppol" in wizard.sending_methods
        )  # by default peppol is not selected for non-valid partners
    ...
    def test_peppol_send_multi_async(self):
        ...
        self.assertRecordValues(
            new_partner.with_company(company_2),
            [
                {
                    "peppol_verification_state": "not_valid_format",
                    "peppol_eas": "0208",
    ...
    def test_available_peppol_sending_methods(self):
        ...
        self.assertTrue(
            "peppol"
            in self.valid_partner.with_company(
                self.env.company
    ...
    def test_peppol_default_ubl_bis3_multi(self):
        ...
        self.assertEqual(
            moves.mapped("peppol_move_state"), ["processing", "processing"]
        )
    ...
    def test_peppol_branch_company_send(self):
            ...
            [
                {
                    ...
                    "proxy_type": "peppol",
                    "edi_mode": "test",
                    "edi_identification": self.env[
                        "account_edi_proxy_client.user"
                    ]._get_proxy_identification(branch_spoiled, "peppol"),
                ...
                },
                {
                    ...
                    "edi_identification": self.env[
                        "account_edi_proxy_client.user"
                    ]._get_proxy_identification(branch_independent, "peppol"),
                    "private_key_id": self.private_key.id,
                    "refresh_token": FAKE_UUID[1],
                },
            ]
        ...
        independent_move = self.create_move(
            self.valid_partner, company=branch_independent
        )
        independent_move.action_post()
        wizard = self.create_send_and_print(
            independent_move, sending_methods=["peppol"]
        )
        wizard.action_send_and_print()
