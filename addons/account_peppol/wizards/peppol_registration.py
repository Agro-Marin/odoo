...
class PeppolRegistration(models.TransientModel):
    ...
    def _onchange_peppol_endpoint(self):
        for wizard in self:
            if wizard.peppol_endpoint:
                wizard.peppol_endpoint = "".join(
                    char for char in wizard.peppol_endpoint if char.isalnum()
                )
    ...
    def _compute_edi_user_id(self):
        for wizard in self:
            wizard.edi_user_id = (
                wizard.company_id.account_edi_proxy_client_ids.filtered(
        ...
        for wizard in self:
            ...
            if all(
                (
                    ...
                    not wizard.selected_company_id._check_peppol_endpoint_number(
                        warning=True
                    ),
                )
            ):
                peppol_warnings["company_peppol_endpoint_warning"] = {
                    "level": "warning",
                    "message": self.env._(
                        "The endpoint number might not be correct. "
            ...
            if all(
                ...
                peppol_warnings["company_already_on_smp"] = {
                    ...
                    "message": self.env._(
                        ...
                        wizard.peppol_external_provider,
                    ),
                }
            if wizard.peppol_eas == "9925":
                peppol_warnings["be_9925_warning"] = {
                    "level": "warning",
                    "message": self.env._(
                        "You are about to register with your VAT number. Make sure you register with your "
    ...
    def _compute_edi_mode(self):
        for wizard in self:
            wizard.edi_mode = wizard.company_id._get_peppol_edi_mode(
                temporary_eas=wizard.peppol_eas
            )
    ...
    def _compute_smp_registration_external_provider(self):
        for wizard in self:
            ...
            if wizard.peppol_eas and wizard.peppol_endpoint:
                ...
                peppol_info = wizard.company_id._get_company_info_on_peppol(
                    edi_identification
                )
                is_company_on_peppol = peppol_info["is_on_peppol"]
                external_provider = peppol_info["external_provider"]
            wizard.smp_registration = (
                not is_company_on_peppol
            )  # Register on smp if not on Peppol
    ...
    def _compute_peppol_can_connect_data(self):
        for wizard in self:
            ...
            wizard.display_itsme_login = bool(
                connect_vals.get("available_auths", {}).get("itsme")
            )
    ...
    def _check_mandatory_fields(self):
        if not self.selected_company_id.tax_config_id.account_fiscal_country_id.code:
            raise ValidationError(
                self.env._("Please select a country for your company.")
            )
        if not self.contact_email or not self.phone_number:
            raise ValidationError(
                self.env._("Contact email and phone number are required.")
            )
        ...
        if self._branch_with_same_address():
            raise ValidationError(
                self.env._("Peppol ID should be different from main company.")
            )
        if (
    ...
    def _action_send_notification(self, title, message):
        ...
        if move_ids and self.env.context.get("active_model") == "account.move":
            next_action = (
                self.env["account.move"].browse(move_ids).action_send_and_print()
            )
    ...
    def _check_can_connect(self, can_connect_vals, selected_auth=None):
        ...
        if identifier_invalid := can_connect_vals.get("identifier_invalid"):
            if identifier_invalid.get("code") == "IDENTIFIER_NOT_ON_PEPPOL":
                raise UserError(
                    self.env._("Your identifier you entered is invalid for Peppol.")
                )
            if identifier_invalid.get("code") == "IDENTIFIER_INCORRECT_FORMAT":
                example = (
                    self.env._(
                        " Expected format: %(expected_format)s.",
                        identifier_invalid["example"],
                    )
                ...
                raise UserError(
                    self.env._(
                        "Your identifier does not have a valid format.%s", example
                    )
                )
        ...
        if can_connect_vals.get("db_invalid"):
            raise UserError(
                self.env._(
                    "The database you are trying to connect to is not suitable for Peppol."
                )
            )
        ...
        if selected_auth and not can_connect_vals.get("available_auths", {}).get(
            selected_auth
        ):
            raise UserError(
                self.env._("Selected authentication method is not available.")
            )
    ...
    def _decode_connect_token(self, token: str):
        if not (
            payload := tools.resolve_hash_signed(
                self.sudo().env, "account_peppol_connect", token
            )
        ):
            return None
    ...
    def _get_company_details(self, company):
        return {
            ...
            ]._generate_webhook_token(company),
        }
    ...
    def button_register_peppol_participant(self, selected_auth=None):
        ...
        old_proxy_users = self.env["account_edi_proxy_client.user"].search(
            [
                ...
                (
                    "edi_identification",
                    "=",
                    f"{self.peppol_eas}:{self.peppol_endpoint}",
                ),
            ]
        )
        old_proxy_users.active = False
        _logger.debug(
            "De-registering existing Peppol proxy user for company %s",
        ...
        self._check_can_connect(
            self.peppol_can_connect_data, selected_auth=selected_auth
        )
        if self.peppol_can_connect_data.get("auth_required"):
            return {
                "type": "ir.actions.act_url",
                "url": self.peppol_can_connect_data["available_auths"][selected_auth][
                    "authorization_url"
                ],
                "target": "new",
            }
        ...
        notifications = {
            "sender": {
                "message": self.env._(
                    "You can now send electronic invoices via Peppol."
                ),
            },
            "smp_registration": {
                "message": self.env._(
                    "Your Peppol registration will be activated soon. You can already send invoices."
                ),
            },
            "receiver": {
                "message": self.env._(
                    "You can now send and receive electronic invoices via Peppol"
                ),
            },
            "rejected": {
                "title": self.env._("Registration rejected."),
                "message": self.env._(
                    "Your registration has been rejected. Please contact the support for further assistance."
                ),
            },
        }
