...
class PeppolAuthentication(http.Controller):
    @http.route(
        "/peppol/authentication/callback", type="http", methods=["GET"], auth="user"
    )
    def peppol_authentication_callback(self, auth_type, connect_token, auth_token=None):
        ...
        def redirect(success=True, partner=None, error_message=None):
            ...
            return request.redirect_query(
                "/odoo/peppol-auth-callback-action", query={"success": success}
            )

        connect_data = request.env["peppol.registration"]._decode_connect_token(
            connect_token
        )
        if not connect_data:
            _logger.warning(
                "Invalid request token auth_type=%s connect_token=%s auth_token=%s",
                auth_type,
                connect_token,
                auth_token,
            )
        ...
        if not auth_token:
            _logger.warning(
                "Invalid auth token auth_type=%s connect_token=%s auth_token=%s",
                auth_type,
                connect_token,
                auth_token,
            )
        ...
        try:
            request.env["peppol.registration"].sudo()._create_connection(
                peppol_identifier, db_uuid, company, auth_token=auth_token
            )
        except UserError as e:
            _logger.warning(
                "Could not create proxy user auth_type=%s connect_token=%s auth_token=%s",
                auth_type,
                connect_token,
                auth_token,
            )
