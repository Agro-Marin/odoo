    ...
    def _get_mandatory_billing_address_fields(self, country_sudo):
        ...
        if sending_method == "peppol":
            mandatory_fields.update(
                {"peppol_eas", "peppol_endpoint", "invoice_edi_format"}
            )
    ...
    def _get_address_errors(self, address_values, *args, **kwargs):
        ...
        if address_values.get("invoice_sending_method") == "peppol":
            ...
            if (
                ...
                .browse(int(address_values.get("country_id")))
                .code
                not in PEPPOL_LIST
            ):
            ...
            if (
                request.env["res.partner"]._get_peppol_verification_state(
                    peppol_endpoint, peppol_eas, edi_format
                )
                != "valid"
            ):
