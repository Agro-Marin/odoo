...
STANDARD_EXCEPTION_CODE_MESSAGES_MAP: dict[int, Callable[..., str]] = {
    ...
    102: lambda arg: _lt(
        'Proxy error, please contact Odoo (missing "%s" - please make sure that it was loaded from the configuration panel)',
        arg,
    ),
    103: lambda: _lt("The document could not be validated."),
    104: lambda arg: _lt(
        'Could not find an XSD with which to validate the document with identifier "%s".',
        arg,
    ),
    ...
    202: lambda: _lt(
        'This user\'s is not a Peppol User. The proxy_type associated must be "peppol". This is required to create a peppol participant.'
    ),
    203: lambda arg: _lt(
        "The Service Metadata Publisher associated to this participant is not reachable. url: %s",
        arg,
    ),
    204: lambda: _lt(
    ...
    206: lambda: _lt(
        "The Peppol Participant DNS lookup resulted in an URL address that does not belong to this server."
    ),
    ...
    302: lambda: _lt(
        "The UBL document provided to the Peppol Outbound Service is malformed."
    ),
    303: lambda: _lt("The document could not be sent to the Peppol Participant."),
    304: lambda: _lt(
        "There was an issue with the Hermes Migration Token Collection Interface (MTCI)."
    ),
    ...
    707: lambda: _lt(
        "You have reached the limit of documents you can send today. Retry later. Please contact the support if you think you need to increase that limit."
    ),
...
EBMS_EXCEPTION_CODE_MESSAGES_MAP: dict[int, Callable[..., str]] = {
    ...
    47: lambda: _lt(
        "The href attribute does not reference a valid MIME data part, MIME parts other than the fragment header and a data part are in the message. are added or the SOAP Body is not empty."
    ),
    48: lambda: _lt(
    ...
    50: lambda arg: _lt(
        "The size of the data part in a fragment message is greater than %s", arg
    ),
    51: lambda arg: _lt(
        "More time than %s has passed since the first fragment was received but not all other fragments are received.",
        arg,
    ),
    52: lambda arg: _lt(
        "Message properties were present in the fragment SOAP header that were not specified in %s",
        arg,
    ),
    53: lambda: _lt(
...
def _get_translation_lambda_message(
    translation_lambda: Callable[..., str], args: list[str]
):
...
def get_exception_message(env, error_vals: dict):
    ...
    return _get_translation_lambda_message(
        translation_lambda, error_vals.get("args", [])
    )
