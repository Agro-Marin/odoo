    ...
    def test_partner_create(self):
        ...
        for street, name, number, number2 in values:
            ...
            self.assertEqual(
                partner.street_name,
                name,
                "Wrongly formatted street name: expected %s, received %s"
                % (name, partner.street_name),
            )
            self.assertEqual(
                partner.street_number,
                number,
                "Wrongly formatted street number: expected %s, received %s"
                % (number, partner.street_number),
            )
            self.assertEqual(
                partner.street_number2,
                number2,
                "Wrongly formatted street number2: expected %s, received %s"
                % (number2, partner.street_number2),
            )
        ...
        for street, name, number, number2 in values:
            ...
            self.assertEqual(
                partner.street,
                street.strip(),
                "Wrongly formatted street: expected %s, received %s"
