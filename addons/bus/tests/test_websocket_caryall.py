...
class TestWebsocketCaryall(WebsocketCase):
    ...
    def test_no_duplicate_on_spurious_redispatch(self):
        """Re-triggering dispatch without new notifications yields nothing.

        Verifies that ``_notif_history`` prevents already-sent notifications
        from being delivered again when the dispatch loop wakes up spuriously
        (e.g. NOTIFY from an unrelated channel hitting the same worker).
        """
        ws = self.websocket_connect()
        self.subscribe(ws, ["dedup_ch"], self.env["bus.bus"]._bus_last_id())
        self.env["bus.bus"]._sendone("dedup_ch", "first", {"seq": 1})
        self.trigger_notification_dispatching(["dedup_ch"])
        first_batch = json.loads(ws.recv())
        self.assertEqual(len(first_batch), 1)
        self.assertEqual(first_batch[0]["message"]["type"], "first")
        # Re-trigger dispatch without any new notification. Because the
        # notification id is in _notif_history and _last_notif_sent_id
        # hasn't advanced past it yet (within MAX_NOTIFICATION_HISTORY_SEC),
        # the notification should be filtered out via ignore_ids.
        self.trigger_notification_dispatching(["dedup_ch"])
        # The websocket should NOT receive anything — set a short timeout
        # so we don't block forever.
        ws.settimeout(1)
        try:
            data = ws.recv()
            # If we do receive data, it must be an empty list or a control frame.
            # A non-empty notification list means dedup failed.
            if data:
                notifications = json.loads(data)
                self.assertEqual(
                    len(notifications), 0,
                    "Already-dispatched notification should not be sent again",
                )
        except Exception:
            # Timeout is the expected path — no data to receive
            pass

    def test_multiple_notifications_all_delivered(self):
        """Multiple notifications sent before dispatch are all delivered in one batch."""
        ws = self.websocket_connect()
        self.subscribe(ws, ["batch_ch"], self.env["bus.bus"]._bus_last_id())
        for i in range(5):
            self.env["bus.bus"]._sendone("batch_ch", f"type_{i}", {"seq": i})
        self.trigger_notification_dispatching(["batch_ch"])
        notifications = json.loads(ws.recv())
        self.assertEqual(len(notifications), 5)
        received_types = {n["message"]["type"] for n in notifications}
        self.assertEqual(received_types, {f"type_{i}" for i in range(5)})
