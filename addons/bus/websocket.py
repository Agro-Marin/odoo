...
def acquire_cursor(db):
    """Try to acquire a cursor up to ``MAX_TRY_ON_POOL_ERROR`` times.

    ``PoolError`` is suppressed only during cursor acquisition — never
    during consumer code execution (the ``yield``).  This prevents
    silently swallowing exceptions raised inside the ``with`` block.
    """
...
class PollablePriorityQueue(PriorityQueue):
    """A PriorityQueue that exposes a file descriptor for ``select``/``poll``.

    Internally uses a socket pair so that ``put()`` wakes any selector
    blocking on this queue's ``fileno()``.
    """
    ...
    def fileno(self) -> int:
        """Return a selectable file descriptor that becomes readable on ``put``."""
        return self._getsocket.fileno()

    def put(self, item, *args, **kwargs) -> None:
        """Enqueue *item* and wake any selector watching this queue."""
        super().put(item, *args, **kwargs)
        self._putsocket.send(b".")

    def get(self, *args, **kwargs):
        """Dequeue an item, consuming the wakeup byte."""
        self._getsocket.recv(1)
        return super().get(*args, **kwargs)

    def close(self) -> None:
        """Release the internal socket pair."""
...
class Frame:
    """A single WebSocket frame (RFC 6455 §5.2)."""
...
class CloseFrame(Frame):
    """A WebSocket close frame carrying a status code and optional reason."""
...
class Websocket:
    """Server-side representation of a single WebSocket connection.

    Handles the full RFC 6455 lifecycle: frame parsing/sending, ping/pong
    keep-alive, rate limiting, notification dispatching, and graceful close.
    Each instance owns a TCP socket and a command queue polled via
    ``selectors`` in the ``get_messages`` event loop.
    """
    ...
    def __init__(self, sock: socket.socket, session, cookies) -> None:
    ...
    def get_messages(self):
        """Yield decoded data messages until the connection closes.

        Blocks on the socket and command queue via ``selectors``.  Control
        frames (ping, pong, close) are handled internally; only data-frame
        payloads are yielded to the caller.
        """
    ...
    def close(self, code: int, reason: str | None = None) -> None:
    ...
    def subscribe(self, channels: set, last: int) -> None:
    ...
    def _send(self, message: str | bytes | bytearray | list | dict) -> None:
        if self.state is not ConnectionState.OPEN:
    ...
    def _send_close_frame(self, code: int, reason: str | None = None) -> None:
        """Send a close frame."""
        self._send_frame(CloseFrame(code, reason))

    def _send_ping_frame(self) -> None:
        """Send a ping frame."""
        self._send_frame(Frame(Opcode.PING))

    def _send_pong_frame(self, payload: bytes) -> None:
        """Send a pong frame."""
        self._send_frame(Frame(Opcode.PONG, payload))

    def _disconnect(self, code: int, reason: str | None = None) -> None:
    ...
    def _handle_control_frame(self, frame: Frame) -> None:
        """Process a ping, pong, or close control frame."""
    ...
    def _handle_transport_error(self, exc: Exception) -> None:
        """Map *exc* to a close code and disconnect cleanly."""
    ...
    def _limit_rate(self) -> None:
        """Enforce rate limiting on incoming frames.

        Allows up to ``RL_BURST`` requests in excess of the steady-state
        rate of one request per ``RL_DELAY`` seconds.  Raises
        ``RateLimitExceededError`` when exceeded.
        """
        now = time.time()
...
class TimeoutManager:
    ...
    def acknowledge_frame_receipt(self, frame: Frame) -> None:
        """Record that a frame was received, resetting inactivity timers."""
        self._next_ping_time = time.time() + self.INACTIVITY_TIMEOUT
        self._expiration_time_by_opcode.pop(frame.opcode, None)

    def acknowledge_frame_sent(self, frame: Frame) -> None:
        """Record that a frame was sent; start response timer for PING/CLOSE."""
        now = time.time()
    ...
    def has_keep_alive_timed_out(self) -> bool:
        """Return whether the connection exceeded its keep-alive lifetime."""
        return time.time() >= self._keep_alive_expiration_time

    def has_frame_response_timed_out(self) -> bool:
        """
    ...
    def should_send_ping_frame(self) -> bool:
        """Return whether the connection has been idle long enough to warrant a ping."""
        return (
...
class WebsocketRequest:
    """Per-message request context for WebSocket connections.

    Acts as a context manager that pushes itself onto ``_wsrequest_stack``
    so that ``wsrequest`` resolves to the active request, mirroring how
    ``odoo.http.request`` works for HTTP requests.
    """

    def __init__(self, db: str, httprequest, websocket: Websocket) -> None:
        self.db = db
    ...
    def serve_websocket_message(self, message: str | bytes) -> None:
        """Parse a WebSocket message and dispatch it to ``ir.websocket``."""
        try:
...
class WebsocketConnectionHandler:
    """Handles the HTTP→WebSocket upgrade handshake and connection lifecycle.

    Validates the ``Sec-WebSocket-*`` headers, performs the SHA-1 accept
    handshake (RFC 6455 §4.2), and spawns a ``Websocket`` instance that
    runs in the calling thread for the lifetime of the connection.
    """
    ...
    def websocket_allowed(cls, request):
        ...
        # instances both use the `TestCursor` class which is locked.
    ...
    def _assert_handshake_validity(cls, headers):
        ...
        try:
            ...
            raise BadRequest("Sec-WebSocket-Key should be b64 encoded") from exc
