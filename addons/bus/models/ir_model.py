...
class IrModel(models.Model):
    ...
    def _get_model_definitions(self, model_names_to_fetch: list[str]) -> dict:
        """Build field definitions for the given models for the JS client.

        Returns a dict mapping each model name to its field metadata,
        including inverse field relationships and relational links
        (only for models within *model_names_to_fetch*).
        """
        model_definitions = {}
