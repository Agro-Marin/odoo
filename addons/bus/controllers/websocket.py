...
class WebsocketController(Controller):
    """WebSocket lifecycle endpoints: handshake, polling, and worker bundle."""

    @route("/websocket", type="http", auth="public", cors="*", websocket=True)
    def websocket(self, version=None):
        """
        Handle the websocket handshake, upgrade the connection if successful.
    ...
    def peek_notifications(self, channels: list, last: int, is_first_poll: bool = False) -> dict:
        """Poll for pending notifications without a persistent WebSocket.

        Used as a fallback when the WebSocket connection is not yet established
        or as an HTTP-only alternative.
        """
        if is_first_poll:
    ...
    def get_websocket_worker_bundle(self, v=None):  # pylint: disable=unused-argument
        ...
        bundle = request.env["ir.qweb"]._get_asset_bundle(
            bundle_name, debug_assets="assets" in request.session.debug
        )
