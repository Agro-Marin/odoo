    ...
    def _compute_analytic_distribution(self):
        ...
        if not self.env.context.get('project_id'):
            ...
            for expense in self.filtered('sale_order_id'):
                ...
                prefetch_ids.update(self.env['mixin.analytic']._get_analytic_account_ids_from_distributions(expense.analytic_distribution))
                prefetch_ids.update(self.env['mixin.analytic']._get_analytic_account_ids_from_distributions(expense.sale_order_id.project_id._get_analytic_distribution()))
            ...
            if expenses_to_recompute:
                ...
                for expense in expenses_to_recompute:
                    expense_account_ids = self.env['mixin.analytic']._get_analytic_account_ids_from_distributions(expense.analytic_distribution)
                    project_analytic_distribution = expense.sale_order_id.project_id._get_analytic_distribution()
                    project_account_ids = self.env['mixin.analytic']._get_analytic_account_ids_from_distributions(project_analytic_distribution)
