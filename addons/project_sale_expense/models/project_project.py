...
class ProjectProject(models.Model):
    ...
    def _get_expenses_profitability_items(self, with_action=True):
        ...
        for (
            ...
            currency,
            ids,
            untaxed_amount_currency_sum,
        ) in expenses_read_group:
        ...
        dict_invoices_amount_per_currency = defaultdict(
            lambda: {"to_invoice": 0.0, "invoiced": 0.0}
        )
        ...
        for (
            ...
            product,
            currency,
            amount_taxexc_to_invoice_sum,
            amount_taxexc_invoiced_sum,
        ) in sol_read_group:
            ...
            if product_id in expense_data_per_product_id:
                dict_invoices_amount_per_currency[currency]["to_invoice"] += (
                    amount_taxexc_to_invoice_sum
                )
                dict_invoices_amount_per_currency[currency]["invoiced"] += (
                    amount_taxexc_invoiced_sum
                )
        ...
        for currency, revenues in dict_invoices_amount_per_currency.items():
            total_amount_expense_to_invoice += currency._convert(
                revenues["to_invoice"], self.currency_id, self.company_id
            )
            total_amount_expense_invoiced += currency._convert(
