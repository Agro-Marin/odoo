...
class AccountEdiXmlUBLMyInvoisMY(models.AbstractModel):
            ...
            vals.update(
                {
                    "total_paid_amount": sum(
                        order.amount_paid / order.currency_rate
                        for order in myinvois_document.pos_order_ids
                    ),
                    "total_paid_amount_currency": sum(
                        myinvois_document.pos_order_ids.mapped("amount_paid")
                    ),
                }
            )
    ...
    def _l10n_my_edi_get_refund_details(self, invoice):
        ...
        if not invoice.pos_order_ids:
            return super()._l10n_my_edi_get_refund_details(
                invoice
            )  # the existing logic is enough.
    ...
    def _get_consolidated_invoice_node(self, vals):
        ...
        self._add_consolidated_invoice_accounting_supplier_party_nodes(
            document_node, vals
        )
        self._add_consolidated_invoice_accounting_customer_party_nodes(
            document_node, vals
        )
    ...
    def _add_consolidated_invoice_base_lines_vals(self, vals):
        ...
        orders_per_line = next(
            iter(
                consolidated_invoice._split_pos_orders_in_lines(
                    consolidated_invoice.pos_order_ids
                ).values()
            )
        )  # Only one config in a same consolidated invoice
        tax_data_fields = (
        ...
        for index, orders in enumerate(orders_per_line):
            ...
            for order in orders:
                ...
                AccountTax._add_tax_details_in_base_lines(
                    order_base_lines, consolidated_invoice.company_id
                )
            ...
            new_base_line = AccountTax._prepare_base_line_for_taxes_computation(
                ...
                line_name=f"{orders[0].name}-{orders[-1].name}"
                if len(orders) > 1
                else orders[0].name,
            )
        ...
        vals.update(
            {
                "total_paid_amount": sum(
                    order.amount_paid / order.currency_rate
                    for order in consolidated_invoice.pos_order_ids
                ),
                "total_paid_amount_currency": sum(
                    consolidated_invoice.pos_order_ids.mapped("amount_paid")
                ),
            }
        )
    ...
    def _add_document_tax_grouping_function_vals(self, vals):
        ...
        def tax_grouping_function(_base_line, tax_data):
            ...
            return {
                ...
                if tax and tax.l10n_my_tax_type == "E"
                else None,
        ...
        document_node.update(
            {
                ...
                "cac:AdditionalDocumentReference": {
                    "cbc:ID": {"_text": vals["export_custom_form_reference"]}
                },
            }
        )
    ...
    def _add_consolidated_invoice_accounting_supplier_party_nodes(
    ...
    def _add_consolidated_invoice_accounting_customer_party_nodes(
        self, document_node, vals
    ):
                ...
                "cac:ItemPriceExtension": {
                    "cbc:Amount": {
                        "_text": self.format_float(
                            vals[f"total_excluded{vals['currency_suffix']}"],
                            vals["currency_dp"],
                        ),
                        "currencyID": vals["currency_name"],
                    }
