...
class PosOrder(models.Model):
    ...
    def _process_order(self, order, existing_order):
        ...
        refunded_orders = (
            self.env["pos.order.line"].browse(refunded_order_line_ids).order_id
        )
        ...
        for refunded_order in refunded_orders:
            submitted = (
                refunded_order.is_invoiced
                and refunded_order.account_move.l10n_my_edi_state
                in ["in_progress", "valid", "rejected"]
            ) or (
                refunded_order._get_active_consolidated_invoice()
                and refunded_order._get_active_consolidated_invoice().myinvois_state
                in ["in_progress", "valid", "rejected"]
            )

            if submitted and not to_invoice:
                raise UserError(
                    refunded_order.env._(
            ...
            if refunded_order._get_active_consolidated_invoice():
                if not to_invoice:
                    ...
                    order["consolidated_invoice_ids"] = (
                        refunded_order._get_active_consolidated_invoice().id
                    )
    ...
    def _generate_pos_order_invoice(self):
        ...
        if self.company_id._l10n_my_edi_enabled():
            for order in self:
                ...
                refunded_consolidated_invoice = (
                    order.refunded_order_id
                    and order.refunded_order_id._get_active_consolidated_invoice()
                )
                refunding_consolidated_invoice = (
                    refunded_consolidated_invoice
                    and refunded_consolidated_invoice.myinvois_state
                    in ["in_progress", "valid", "rejected"]
                )
                ...
                if not refunding_consolidated_invoice:
                    ...
                    if (
                        ...
                        raise UserError(
                            order.env._(
                                "You must set the identification information on the commercial partner."
                            )
                        )
                    if not partner._l10n_my_edi_get_tin_for_myinvois():
                        raise UserError(
                            order.env._(
                                "You must set a TIN number on the commercial partner."
                            )
                        )
    ...
    def _get_active_consolidated_invoice(self, including_in_progress=False):
        ...
        return self.env["myinvois.document"].union(
            *[
                order.consolidated_invoice_ids._get_active_myinvois_document(
