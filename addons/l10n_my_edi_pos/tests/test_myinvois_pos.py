...
class TestMyInvoisPoS(TestPoSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.product_one = cls.create_product(
            "Product 1", cls.categ_basic, 100, tax_ids=cls.taxes["tax7"].ids
        )
        cls.product_two = cls.create_product(
            "Product 2", cls.categ_basic, 500, tax_ids=cls.taxes["tax7"].ids
        )
        ...
        cls.env.company.write(
            {
                ...
                "city": "Kuala Lumpur",
                "phone_ids": [
                    Command.create({"number": "+60123456789", "type": "landline"})
                ],
        ...
        cls.invoicing_customer.write(
            {
                ...
                "city": "Main city",
                "phone_ids": [
                    Command.create({"number": "+60123456786", "type": "landline"})
                ],
            }
        )

        cls.proxy_user = cls.env["account_edi_proxy_client.user"]._register_proxy_user(
            cls.env.company, "l10n_my_edi", "demo"
        )
    ...
    def test_consolidate_invoices(self):
        ...
        with freeze_time("2025-01-01"):
            ...
            self.assertEqual(
                len(consolidated_invoice), 1
            )  # One consolidated invoice holds up to 100 lines
            ...
            self.assertEqual(
                len(xml_tree.xpath("cac:InvoiceLine", namespaces=NS_MAP)), 1
            )  # Both orders are continuous, so they are merged in a single line.
            # Finally, assert a few nodes to make sure the file make sense (line amount, customer tin (general one), ...
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine/cbc:LineExtensionAmount", "600.00"
            )
            self._assert_node_values(
                xml_tree,
    ...
    def test_consolidate_invoices_with_split(self):
        ...
        with freeze_time("2025-01-01"):
            # Create the orders
            with (
                self.with_pos_session(),
                patch(CONTACT_PROXY_METHOD, new=self._mock_successful_submission),
            ):
                first_order = self._create_order(
                    {"pos_order_lines_ui_args": [(self.product_one, 1.0)]}
                )
                ...
                third_order = self._create_order(
                    {"pos_order_lines_ui_args": [(self.product_two, 1.0)]}
                )
            # Consolidate them
            wizard = self.env["myinvois.consolidate.invoice.wizard"].create(
            ...
            self.assertEqual(
                len(xml_tree.xpath("cac:InvoiceLine", namespaces=NS_MAP)), 2
            )
            # Finally, ensure that the line values are correct.
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine[1]/cbc:LineExtensionAmount", "100.00"
            )
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine[2]/cbc:LineExtensionAmount", "500.00"
            )
    ...
    def test_consolidate_invoices_limit(self):
        ...
        with freeze_time("2025-01-01"):
            ...
            with patch(
                ...
                self.assertEqual(
                    len(consolidated_invoice), 2
                )  # Two consolidated invoices of a single line due to the MAX_LINE_COUNT_PER_INVOICE
                    ...
                    [
                        {
                            ...
                            "myinvois_validation_time": fields.Datetime.from_string(
                                "2025-01-01 01:00:00"
                            ),
                        }
                    ],
    ...
    def test_invoice_from_pos(self):
        with freeze_time("2025-01-01"):
            ...
            self.assertRecordValues(
                order.account_move._get_active_myinvois_document(),
                [
                    {
                        "myinvois_submission_uid": "123456789",
                        "myinvois_external_uuid": "123458974513518",
    ...
    def test_delete_consolidated_invoice(self):
        with freeze_time("2025-01-01"):
            ...
            # We cancel it
            cancellation_wizard = (
                self.env["myinvois.document.status.update.wizard"]
                .with_context(
                    default_document_id=consolidated_invoice.id,
    ...
    def test_consolidate_invoices_in_foreign_currency(self):
        ...
        with freeze_time("2025-01-01"):
            ...
            # Finally, assert a few nodes to make sure the file make sense (line amount, customer tin (general one), ...
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine/cbc:LineExtensionAmount", "1200.00"
            )
            self._assert_node_values(
                xml_tree, "cac:TaxExchangeRate/cbc:CalculationRate", "0.5"
            )
    ...
    def test_consolidate_invoices_with_different_discounts(self):
        ...
        with freeze_time("2025-01-01"):
            ...
            with self.with_pos_session():
                first_order = self._create_order(
                    {"pos_order_lines_ui_args": [(self.product_one, 1.0, 25)]}
                )
                second_order = self._create_order(
                    {"pos_order_lines_ui_args": [(self.product_two, 1.0, 15)]}
                )
            # Consolidate them
            wizard = self.env["myinvois.consolidate.invoice.wizard"].create(
                {
            ...
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine/cac:Price/cbc:PriceAmount", "600.0"
            )
            # Both 'extension' amounts are the subtotal after applying discounts
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine/cbc:LineExtensionAmount", "500.00"
            )
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine/cac:ItemPriceExtension/cbc:Amount", "500.00"
            )
            # And the discount should be 100
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine/cac:AllowanceCharge/cbc:Amount", "100.00"
            )
    ...
    def test_refund_order(self):
        with freeze_time("2025-01-01"):
            ...
            self._assert_node_values(
                xml_tree, "cac:InvoiceLine/cac:Price/cbc:PriceAmount", "500.0"
            )
    ...
    def test_refund_order_partially(self):
        with freeze_time("2025-01-01"):
            ...
            with self.with_pos_session():
                first_order = self._create_order(
                    {"pos_order_lines_ui_args": [(self.product_one, 2.0)]}
                )
                self._create_order(
                    {"pos_order_lines_ui_args": [(self.product_two, 1.0)]}
                )
    ...
    @mute_logger(
        "odoo.addons.point_of_sale.models.pos_order",
        "odoo.addons.point_of_sale.models.pos_session",
    )
    ...
    def test_consolidate_invoices_refund_with_customer(self):
        ...
        with freeze_time("2025-01-01"):
            ...
            refund = self.env["account.move"].search(
                [("move_type", "=", "out_refund")], limit=1, order="id desc"
            )
            self.assertEqual(
                refund.partner_id, self.invoicing_customer
            )  # We have the correct customer on the refund.
            xml_tree = etree.fromstring(
                refund._get_active_myinvois_document().myinvois_file_id.raw
            )
    ...
    def test_consolidate_invoices_export_xml(self):
        ...
        product_1 = self.create_product(
            "Product 1", self.categ_basic, 100, tax_ids=tax_5.ids
        )
        product_2 = self.create_product(
            "Product 1", self.categ_basic, 10, tax_ids=tax_10.ids
        )
        ...
        with freeze_time("2025-01-01"):
            with (
                ...
                first_order = self._create_order(
                    {"pos_order_lines_ui_args": [(product_1, 2.0)]}
                )
                second_order = self._create_order(
                    {"pos_order_lines_ui_args": [(product_1, 1.0), (product_2, 1.0)]}
                )
                # This one has a 25% discount
                third_order = self._create_order(
                    {"pos_order_lines_ui_args": [(product_1, 4.0, 25)]}
                )
                fourth_order = self._create_order(
                    {"pos_order_lines_ui_args": [(product_1, 1.0), (product_2, 2.0)]}
                )
                ...
                fifth_order = self._create_order(
                    {"pos_order_lines_ui_args": [(product_1, 1.0), (product_2, 1.0)]}
                )
            ...
            with file_open(
                "l10n_my_edi_pos/tests/expected_xmls/consolidated_invoice.xml", "rb"
            ) as f:
    ...
    def test_consolidate_invoices_refund_export_xml(self):
        ...
        with freeze_time("2025-01-01"):
            ...
            with file_open(
                "l10n_my_edi_pos/tests/expected_xmls/consolidated_invoice_refund.xml",
                "rb",
            ) as f:
                expected_xml = etree.fromstring(f.read())
            self.assertXmlTreeEqual(root, expected_xml)
