...
// generated by the conversion. `toInline` runs again on its own output at every
// save, so each pass recognizes its own products and leaves them alone.
...
export function classToStyle(element, cssRules) {
    ...
    // longhands, so a re-run does not prepend longhands the browser has since
    // merged into a shorthand (e.g. the four border widths into border-width).
    ...
    for (const node of nodeToRules.keys()) {
        ...
        for (const [key, value] of Object.entries(css)) {
            ...
            // KNOWN BUG: this guard skips any property the node already carries
            // inline, so a stylesheet `!important` declaration loses to a
            // non-important inline one. The fix belongs in the _getMatchedCSSRules
            // merge (carrying priority through to one authoritative declaration
            // set), not here: appending or overwriting in place breaks idempotency
            // or diverges on shorthand/longhand pairs.
        }
...
/**
 * Return an array of twelve `<td>` elements.
 *
 * @returns {Element[]}
 */
