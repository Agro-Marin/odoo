...
test("can quickly dismiss 'Turn on notification' suggestion", async () => {
    ...
    await contains(".o-mail-NotificationItem", { text: "Turn on notifications" });
    await click(
        ".o-mail-NotificationItem:contains(Turn on notifications) [title='Dismiss']",
    );
    await contains(".o-mail-NotificationItem", {
...
test("mark unread channel as read", async () => {
    ...
    pyEnv["discuss.channel.member"].write([currentMemberId], {
        seen_message_id: messagId_1,
    });
...
test("basic rendering", async () => {
    ...
    expect(
        '.o_menu_systray .dropdown-toggle:has(i[aria-label="Messages"]):first',
...
test("switch tab", async () => {
    ...
    await contains(".o-mail-MessagingMenu button:not(.fw-bold)", {
        text: "Notifications",
    });
...
test("Attachment-only message preview shows file type icon", async () => {
    ...
    await contains(".o-mail-NotificationItem:eq(0)", {
        text: "Partner1: Voice Message",
    });
...
test("render message preview with message seen indicator in messaging menu", async () => {
    ...
    const [thomasMemberId] = pyEnv["discuss.channel.member"].search([
        ["channel_id", "=", channelId],
        ["partner_id", "=", thomas],
    ]);
    pyEnv["discuss.channel.member"].write([thomasMemberId], {
        seen_message_id: messageId,
    });
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
...
test("single preview for channel if it has unread and needaction messages", async () => {
    ...
    await contains(".o-mail-NotificationItem-text", {
        text: "Partner1: Message with needaction",
    });
});
...
test("failure notifications are shown before channel preview", async () => {
    ...
    pyEnv["discuss.channel"].write([channelId], {
        message_ids: [messageId, failedMessageId],
    });
    pyEnv["mail.notification"].create({
...
test("user notification from inbox redirect to discuss inbox", async () => {
    ...
    await click(".o_menu_systray i[aria-label='Messages']");
    await click(
        ".o-mail-NotificationItem .o-mail-NotificationItem-text:text('You: Hello world!')",
    );
