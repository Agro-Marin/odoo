...
test("composer text input: basic rendering when posting a message", async () => {
    ...
    await contains(
        "textarea.o-mail-Composer-input[placeholder='Send a message to followers…']",
    );
});
...
test("composer text input: basic rendering when logging note", async () => {
    ...
    await contains(
        "textarea.o-mail-Composer-input[placeholder='Log an internal note…']",
    );
});
...
test("composer textarea content is retained when changing channel then going back", async () => {
    ...
    await contains(
        "textarea.o-mail-Composer-input[placeholder='Message #epic-shrek-lovers…']",
    );
    await contains(".o-mail-Composer-input", { value: "" });
...
test("removing attachment from composer should not delete it from template", async () => {
    ...
    await contains(".o_field_mail_composer_attachment_list", {
        text: "TemplateAttachment",
    });
    ...
    expect(updatedTemplate.attachment_ids).toEqual([attachmentId], {
        message:
            "The attachment must remain on the template after being removed from the composer",
    });
});
...
test("[technical] does not crash when an attachment is removed before its upload starts", async () => {
    ...
    uploadDef.resolve();
    await contains(
        ".o-mail-AttachmentContainer:not(.o-isUploading):contains(text1.txt)",
    );
});
...
test("[text composer] select a canned response suggestion with some text", async () => {
    ...
    await click(".o-mail-Composer-suggestion");
    await contains(".o-mail-Composer-input", {
        value: "bluhbluh Hello! How are you? ",
    });
});
...
test("Canned response last used changes on posting", async () => {
    ...
    await press("Enter");
    await contains(".o-mail-Message");
    cannedResponse = pyEnv["mail.canned.response"].search_read([
        ["id", "=", cannedResponseId],
    ])[0];
...
test("Tab to select of canned response suggestion works in chat window", async () => {
    ...
    await contains(".o-mail-NavigableList-active", {
        text: "GoodbyeGoodbye! See you soon!",
    });
...
test("composer reply-to message is restored on thread change", async () => {
    ...
    expect(
        browser.localStorage.getItem(
            store.Thread.get({ model: "discuss.channel", id: channelId }).composer
                .localId,
        ),
    ).toBe(
...
test("parse link correctly in html composer", async () => {
    ...
    await contains(editor.editable.querySelector("a"), {
        text: "www.google.com",
        count: 0,
    });
    await htmlInsertText(editor, " ");
    await contains(editor.editable.querySelector("a[target='_blank']"), {
        text: "www.google.com",
    });
    await press("Enter");
...
test("mention insertion adds FEFF markers for safe cursor placement", async () => {
    ...
    await contains(
        ".o-mail-Composer-html.odoo-editor-editable:text('@Mitchell Admin')",
    );
...
test("mentions can be correctly selected with ctrl+A and deleted", async () => {
    ...
    await htmlInsertText(editor, "Hello");
    await contains(
        ".o-mail-Composer-html.odoo-editor-editable:text('@Mitchell Admin Hello')",
    );
...
test("mentions can be correctly cut with ctrl+A and ctrl+X", async () => {
    ...
    await click(".o-mail-NavigableList-item:text('Mitchell Admin')");
    await contains(
        ".o-mail-Composer-html.odoo-editor-editable:text('Hello @Mitchell Admin')",
    );
    await htmlInsertText(editor, "nice to meet you!");
    await contains(
        ".o-mail-Composer-html.odoo-editor-editable:text('Hello @Mitchell Admin nice to meet you!')",
