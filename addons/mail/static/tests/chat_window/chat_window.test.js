...
test("the thread avatar of a group conversation stays inert", async () => {
    const pyEnv = await startServer();
    const channelId = pyEnv["discuss.channel"].create({
        channel_type: "group",
        name: "Group",
    ...
    setupChatHub({ opened: [channelId] });
    await start();
    await contains(".o-mail-ChatWindow-threadAvatar");
    await contains(".o-mail-ChatWindow-threadAvatar.cursor-pointer", { count: 0 });
});
