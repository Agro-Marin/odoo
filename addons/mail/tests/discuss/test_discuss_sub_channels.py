...
class TestDiscussSubChannels(HttpCase):
    ...
    def test_05_cannot_upate_first_message_nor_parent_channel(self):
        ...
        with self.assertRaises(
            UserError,
            msg="Cannot change initial message nor parent channel of: Hello there!.",
        ):
    ...
    def test_07_unlink_sub_channel(self):
        ...
        parent_1 = (
            self.env["discuss.channel"].with_user(bob_user).create({"name": "Parent 1"})
        )
        ...
        parent_2 = (
            self.env["discuss.channel"].with_user(baz_user).create({"name": "Parent 2"})
        )
        ...
        parent_3 = (
            self.env["discuss.channel"].with_user(bob_user).create({"name": "Parent 3"})
        )
        ...
        members_to_unlink = (
            parent_1_baz_member + parent_2_bob_member + parent_3_guest_member
        )
        ...
        self.assertIn(
            bob_user.partner_id, parent_1_sub_channel_1.channel_member_ids.partner_id
        )
        self.assertIn(
            bob_user.partner_id, parent_1_sub_channel_2.channel_member_ids.partner_id
        )
        self.assertIn(
            baz_user.partner_id, parent_2_sub_channel.channel_member_ids.partner_id
        )
        self.assertIn(
            bob_user.partner_id, parent_3_sub_channel.channel_member_ids.partner_id
        )
    ...
    def test_12_unlink_children_members_only_once(self):
        ...
        unlinked_member_ids = []
        expected_unlinked_member_ids = sorted(
            (parent.self_member_id | child.self_member_id).ids
        )
        ...
        with patch.object(
            self.env.registry["discuss.channel.member"], "unlink", _patched_unlink
        ):
    ...
    def test_13_mentioned_user_becomes_sub_channel_member(self):
        ...
        sub_channel = parent._create_sub_channel(from_message_id=message.id)
        self.assertNotIn(
            alice_user.partner_id, sub_channel.channel_member_ids.partner_id
        )
