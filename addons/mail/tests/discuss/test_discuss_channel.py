...
class TestChannelInternals(MailCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_channel = (
            ...
            ._create_channel(name="Channel", group_id=None)
        )
        cls.test_user = (
            cls.env["res.users"]
            .with_context(cls._test_context)
    ...
    def test_channel_members(self):
        ...
        emp_partner_write_date = fields.Datetime.to_string(
            self.env.user.partner_id.write_date
        )
        test_partner_write_date = fields.Datetime.to_string(
            self.env.user.partner_id.write_date
        )
            ...
            return (
                ...
                [
                    ...
                    },
                    {
                        ...
                        "payload": {
                            "discuss.channel": [
                                {"id": test_group.id, "member_count": 2}
                            ],
        ...
        self.assertEqual(test_group.message_partner_ids, self.env["res.partner"])
        self.assertEqual(
            test_group.channel_partner_ids, self.test_partner + self.partner_employee
        )

        self.env["discuss.channel.member"].sudo().search(
            [
                ("partner_id", "in", self.test_partner.ids),
                ("channel_id", "in", test_group.ids),
            ]
        ).unlink()
        self.assertEqual(test_group.message_partner_ids, self.env["res.partner"])
        self.assertEqual(test_group.channel_partner_ids, self.partner_employee)
    ...
    def test_channel_user_synchronize(self):
        ...
        group_restricted_channel._add_members(
            users=self.user_employee | self.user_employee_nomail
        )
    ...
    def test_channel_info_get(self):
        ...
        together_channel_info = (
            Store().add(together_channel).get_result()["discuss.channel"][0]
        )
        ...
        same_solo_channel_info = (
            Store().add(same_solo_channel).get_result()["discuss.channel"][0]
        )
    ...
    def test_get_or_create_chat_should_update_last_interest_dt(self):
        ...
        self.assertEqual(
            fields.Datetime.to_string(channel.last_interest_dt), "2025-06-18 10:40:21"
        )
        self.assertEqual(
    ...
    def test_channel_info_mark_as_read(self):
        ...
        self_member = chat.channel_member_ids.filtered(
            lambda m: m.partner_id == self.user_admin.partner_id
        )
        self_member._mark_as_read(msg_2.id)
        init_data = Store().add(chat).get_result()
        self_member_info = next(
            filter(
                lambda d: d["id"] == self_member.id, init_data["discuss.channel.member"]
            )
        )
        self.assertEqual(
    ...
    def test_channel_message_post_should_not_allow_adding_wrong_parent(self):
        ...
        self.assertFalse(
            message_2.parent_id, "should not allow parent from wrong thread"
        )
    ...
    def test_channel_message_post_with_voice_attachment(self):
        ...
        self.assertTrue(
            channel.message_ids.attachment_ids.voice_ids,
            "message's attachment should have voice metadata",
        )
    ...
    def test_channel_unsubscribe_auto(self):
        ...
        self.assertEqual(
            private_group.channel_partner_ids,
            self.user_employee.partner_id | test_partner,
        )
    ...
    def test_channel_private_unfollow(self):
        ...
        public_channel = self.env["discuss.channel"]._create_channel(
            name="Channel for Everyone", group_id=None
        )
        private_group = self.env["discuss.channel"]._create_group(
            partners_to=self.user_employee.partner_id.ids, name="Group"
        )
        chat_user_current = self.env["discuss.channel"]._get_or_create_chat(
    ...
    def test_channel_should_generate_correct_default_avatar(self):
        test_channel = self.env["discuss.channel"]._create_channel(
            name="Channel", group_id=self.env.ref("base.group_user").id
        )
        ...
        self.assertEqual(
            base64.b64decode(test_channel.avatar_128), expceted_avatar_channel
        )
        self.assertEqual(
            base64.b64decode(private_group.avatar_128), expected_avatar_group
        )
            ...
            [
                {
                    ...
                    "payload": {
                        "discuss.channel": [
                            {"avatar_cache_key": avatar_cache_key, "id": channel.id}
                        ],
                    },
                }
            ],
    ...
    def test_channel_notification(self):
        ...
        self.assertEqual(
            len(mentions_notif), 0, "mentions + normal message = no needaction"
        )
        self.assertEqual(
            len(nothing_notif), 0, "nothing + normal message = no needaction"
        )
        ...
        self.assertEqual(
            len(mentions_notif), 1, "mentions + mention message = needaction"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(all_notif), 0, "mute + all + normal message = no needaction"
        )
        self.assertEqual(
            len(mentions_notif), 0, "mute + mentions + normal message = no needaction"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(mentions_notif), 1, "mute + mentions + mention message = needaction"
        )
        self.assertEqual(
            len(nothing_notif), 1, "mute + nothing + mention message = needaction"
        )

    def test_mail_message_starred_group(self):
        self.authenticate(self.user_employee.login, self.user_employee.login)
        data = self.call_jsonrpc("/mail/data", {"fetch_params": ["init_messaging"]})
        ...
        test_group_own_message = test_group.with_user(
            self.user_employee.id
        ).message_post(body="TestingMessage")
        test_group_own_message.write(
            {"starred_partner_ids": [(6, 0, self.partner_employee.ids)]}
        )
        data = self.call_jsonrpc("/mail/data", {"fetch_params": ["init_messaging"]})
        self.assertEqual(data["Store"]["starred"]["counter"], 1)

        test_group_message = test_group.message_post(body="TestingMessage")
        test_group_message.write(
            {"starred_partner_ids": [(6, 0, self.partner_employee.ids)]}
        )
        data = self.call_jsonrpc("/mail/data", {"fetch_params": ["init_messaging"]})
            ...
            [
                {
                    ...
                    "payload": {
                        "body": "<span class='o_mail_notification'>"
                        ...
                        "mail.message": [
                            {
                                ...
                                "body": [
                                    "markup",
                                    '<p>Test update <span class="o-mail-Message-edited"></span></p>',
                                ],
                                "id": message.id,
                                "parent_id": False,
            ...
            with self.subTest(
                ...
                for user, operation, expected_users in flow:
                    ...
                    for notification in self.env["bus.bus"].search(
                        ...
                        matching_data = next(
                            (
                                ...
                                and "channel_name_member_ids" in data
                            ),
                            None,
                        )
                        if matching_data:
                    ...
                    if expected_users is False:
                        ...
                        self.assertEqual(
                            matching_data["channel_name_member_ids"],
                            expected_member_ids,
                        )
    ...
    def test_create_channel_with_partners_and_guests(self):
        ...
        actual_member_ids = [
            m.partner_id.id if m.partner_id else m.guest_id.id
            for m in channel.channel_member_ids
        ]
        expected_member_ids = [
            self.partner_employee.id,
            self.guest.id,
