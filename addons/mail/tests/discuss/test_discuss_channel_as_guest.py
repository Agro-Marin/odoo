...
class TestMailPublicPage(HttpCaseWithUserPortal, HttpCaseWithUserDemo):
    ...
    def setUp(self):
        ...
        self.channel = self.env["discuss.channel"]._create_channel(
            group_id=None, name="Test channel"
        )
        ...
        internal_member = self.channel.channel_member_ids.filtered(
            lambda m: internal_user.partner_id == m.partner_id
        )
        internal_member._rtc_join_call()

        self.group = self.env["discuss.channel"]._create_group(
            partners_to=(internal_user + portal_user).partner_id.ids, name="Test group"
        )
    ...
    def test_discuss_channel_public_page_as_guest(self):
        ...
        guest = self.env["mail.guest"].search(
            [("channel_ids", "in", self.channel.id)], limit=1, order="id desc"
        )
        self.start_tour(
            self.channel.invitation_url,
            self.tour,
    ...
    def test_discuss_channel_public_page_call_public(self):
        ...
        self.start_tour(
            self.channel.invitation_url, "discuss_channel_call_public_tour.js"
        )
    ...
    def test_channel_invitation_from_token(self):
        ...
        internal_channel = self.env["discuss.channel"]._create_channel(
            name="Internal Channel", group_id=self.env.ref("base.group_user").id
        )
