...
class MockEmail(common.BaseCase, MockSmtplibCase):
    ...
    def _assertMailMail(
        ...
        if status == "sent":
            if email_to_recipients:
                ...
                ]  # one partner -> list of a single email
    ...
    def assertMailMailWId(
        ...
        self._assertMailMail(
            found_mail,
            [],  # generally used when recipients are Falsy
            status,
            email_to_recipients=email_to_recipients,
            author=author,
            content=content,
...
class MailCommon(MailCase):
    ...
    def _activate_multi_lang(
        ...
        * template
        ...
        if test_template:
            test_template.with_context(lang=lang_code).subject = (
                "SpanishSubject for {{ object.name }}"
            )
            test_template.with_context(lang=lang_code).body_html = (
                '<p>SpanishBody for <t t-out="object.name" /></p>'
            )
