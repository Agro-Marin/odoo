...
class TestMenuRootLookupByModel(MailCommon):
    ...
    def setUpClass(cls):
        ...
        def new_action(
            name, res_model, path=None, view_mode=None, domain=None, context=None
        ):
        ...
        new_menu(
            "Contacts",
            parent_id=menu_root_contact.id,
            action=new_action(
                "Contacts",
        ...
        new_menu(
            "Companies",
            parent_id=menu_root_invoicing.id,
            action=new_action(
                "Companies", res_model="res.company", view_mode="list,kanban,form"
            ),
        )
        menu_invoicing_customer = new_menu(
            "Customers", parent_id=menu_root_invoicing.id
        )
        new_menu(
            "Customers",
            parent_id=menu_invoicing_customer.id,
            action=new_action(
                "Customers",
                res_model="res.partner",
                path="all-customers",
        ...
        new_menu(
            "Customers",
            parent_id=menu_sales_orders.id,
            action=new_action(
                "Customers",
                res_model="res.partner",
                view_mode="kanban,list,form",
        ...
        new_menu(
            "Companies",
            parent_id=menu_settings_user_and_companies.id,
            action=new_action(
                "Companies",
    ...
    def patch_get_backend_root_menu_ids(self, model, return_values):
        return patch.object(
            model.__class__, "_get_backend_root_menu_ids", return_value=return_values
        )
        ...
        for _idx, (return_values, expected_menu_root_id) in enumerate(
            (
                ...
                (
                    ...
                    self.menu_root_sales.id,
                ),
                (
                    [self.menu_root_contact.id, self.menu_root_sales.id],
                    self.menu_root_contact.id,
                ),
            )
        ):
            with (
                self.patch_get_backend_root_menu_ids(
                    self.env["res.partner"], return_values
                ),
                self.assertQueryCount(employee=0),
            ):
                self.assertEqual(
                    Menu._get_best_backend_root_menu_id_for_model("res.partner"),
                    expected_menu_root_id,
                )
    ...
    def test_look_for_existing_menu_root_user_no_access(self):
        ...
        with self.assertQueryCount(user_portal=2, user_public=2):
            self.assertEqual(
                Menu._get_best_backend_root_menu_id_for_model("res.partner"), None
            )
        with self.assertQueryCount(user_portal=1, user_public=1):
            self.assertEqual(
                Menu._get_best_backend_root_menu_id_for_model("res.company"), None
            )
        with (
            self.patch_get_backend_root_menu_ids(
    ...
    def test_look_for_non_existing_menu_root(self):
        with (
            self.patch_get_backend_root_menu_ids(self.env["res.bank"], []),
            self.assertQueryCount(__system__=2),
        ):
            self.assertEqual(
                self.env["ir.ui.menu"]._get_best_backend_root_menu_id_for_model(
                    "res.bank"
                ),
                None,
            )
