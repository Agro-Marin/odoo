...
class Base(models.AbstractModel):
    ...
    def _mail_group_by_operation_for_mail_message_operation(self, message_operation):
        ...
        for record_operation, records in operation_documents.items():
            operation_documents[record_operation] = records.with_prefetch(self.ids)
    ...
    def _mail_get_companies(self, default=False):
        ...
        return {
            record.id: (
                (record[company_fname] or default_company)
                if company_fname
    ...
    def _mail_get_partner_fields(self, introspect_fields=False):
        ...
        """
        partner_fnames = [
            fname for fname in ("partner_id", "partner_ids") if fname in self
        ]
    ...
    def _mail_get_primary_email(self):
        ...
        primary_email = getattr(self, "_primary_email", None)
        fname = (
            primary_email if primary_email and primary_email in self._fields else None
        )
    ...
    def _mail_track(self, tracked_fields, initial_values):
        ...
        for col_name, _sequence in fields_track_info:
            ...
            if new_value == initial_value or (
                not new_value and not initial_value
            ...
            if self._fields[col_name].type == "properties":
                ...
                # Build name→property maps for both states to track
                # additions, changes, and removals correctly.
                old_by_name = {
                    p["name"]: p
                    for p in (initial_value or [])
                    if p["type"] not in ("separator", "html")
                }
                new_by_name = {
                    p["name"]: p
                    for p in (new_value or [])
                    if p["type"] not in ("separator", "html")
                }
                TrackingValue = self.env["mail.tracking.value"]
                all_names = list(dict.fromkeys(
                    list(old_by_name) + list(new_by_name)
                ))
                for pname in reversed(all_names):
                    old_prop = old_by_name.get(pname)
                    new_prop = new_by_name.get(pname)
                    old_val = old_prop.get("value") if old_prop else False
                    new_val = new_prop.get("value") if new_prop else False
                    if old_val == new_val:
                        continue
                    if not old_val and not new_val:
                        continue
                    # Use whichever property definition is available for
                    # field metadata (prefer the new one for additions).
                    prop_def = new_prop or old_prop
                    col_info_prop = tracked_fields[col_name] | {
                        "type": prop_def["type"],
                        "selection": prop_def.get("selection"),
                    }
                    field_info = {
                        "desc": f"{col_info_prop['string']}: {prop_def['string']}",
                        "name": col_name,
                        "type": prop_def["type"],
                    }
                    # For tags, resolve to (id, label) pairs
                    if old_val and prop_def["type"] == "tags":
                        tags = old_prop.get("tags", []) if old_prop else []
                        old_val = [t for t in tags if t[0] in old_val]
                    if new_val and prop_def["type"] == "tags":
                        tags = new_prop.get("tags", []) if new_prop else []
                        new_val = [t for t in tags if t[0] in new_val]
                    tracking_vals = TrackingValue._create_tracking_values(
                        old_val, new_val, col_name, col_info_prop, self,
                    )
                    tracking_value_ids.append(
                        [0, 0, {**tracking_vals, "field_info": field_info}]
                    )
                continue
    ...
    def _message_add_default_recipients(self):
        ...
        for record in self:
            ...
            cc_fn = next(
                (
                    fname
                    for fname in ["email_cc", "partner_email_cc", "x_email_cc"]
    ...
    def _message_get_suggested_recipients_batch(
        ...
        suggested_record = self._message_add_suggested_recipients(
        ...
        if reply_message or messages:
            for record in self:
                ...
                if (
                    from_normalized
                    and from_normalized != record_msg.author_id.email_normalized
                ):
            ...
            email_to_lst = list(
                tools.misc.unique(
                    ...
                    if (
                        e
                        and e.strip()
                        and email_key(e) not in ban_emails
                        and email_key(e) not in existing_mails
                    )
                )
            )
                ...
                recipients.append(
                    {
                        ...
                        "name": emails_normalized_info.get(email_normalized, {}).pop(
                            "name", False
                        )
                        or name,
                        "partner_id": False,
                        "create_values": emails_normalized_info.get(
                            email_normalized, {}
                        ),
                    }
                )
    ...
    def _find_value_from_field_path(self, field_path):
        ...
        if any(isinstance(value, datetime) for value in field_value):
            ...
            return " ".join(
                [
                    f"{tools.format_datetime(self.env, value, tz=tz)} {tz}"
