...
class MailThread(models.AbstractModel):
    ...
    def _compute_has_message(self):
        ...
        for record in self:
            record.has_message = record.id in res_ids
            ...
            for thread in threads:
                ...
                if changes:
                    self.env.cr.precommit.add(
                        thread._track_post_template_finalize
                    )  # call to _track_post_template_finalize bound to this record
    ...
    def _routing_warn(self, error_message, message_id, route, raise_exception=True):
        ...
        full_message = (
            f"Routing mail with Message-Id {message_id}: route {route}: {error_message}"
        )
    ...
    def _routing_create_bounce_email(
        ...
        if not email_from:
            catchall_aliases = (
                self.env.context.get("mail_catchall_aliases")
                or self.env["mail.alias.domain"].search([]).mapped("catchall_email")
            )
            if not any(
    ...
    def _routing_handle_bounce(self, email_message, message_dict):
        ...
        if bounced_email:
            ...
            for model in [
                bl_model for bl_model in bl_models if bl_model.model in self.env
            ]:  # transient test mode
                rec_bounce_w_email = (
    ...
    def _routing_check_route(self, message, message_dict, route, raise_exception=True):
        ...
        if not isinstance(route, (list, tuple)):
            raise TypeError("A route should be a list or a tuple")
        if len(route) != 5:
            raise ValueError(
                "A route should contain 5 elements: model, thread_id, custom_values, uid, alias record"
            )
        ...
        if alias:
            ...
            if not message_dict.get("author_id"):
                ...
                if (
                    ...
                    link_doc = (
                        self.env[alias.alias_parent_model_id.model]
                ...
                link_doc = (
                    link_doc
                    if link_doc
                    and hasattr(link_doc, "_partner_find_from_emails_single")
    ...
    def _routing_reset_bounce(self, email_message, message_dict):
        ...
        if normalized_from:
            ...
            for model in [
                ...
                self.env[model.model].sudo().search(
                    [
                        ("message_bounce", ">", 0),
    ...
    def _detect_is_bounce(self, message, message_dict):
        ...
        bounce_aliases = (
            self.env.context.get("mail_bounce_aliases")
            or self.env["mail.alias.domain"].search([]).mapped("bounce_email")
        )
        email_to_list = [
    ...
    def _detect_write_to_catchall(self, msg_dict):
        ...
        if self.env.context.get("mail_catchall_write_any_to"):
            return catchall_aliases and any(
                email_to in catchall_aliases for email_to in email_to_list
            )
        return (
            catchall_aliases
            and email_to_list
    ...
    def message_route(
        ...
        if catchall_domains_allowed:
            catchall_domains_allowed += (
                self.env.context.get("mail_alias_domain_names")
                or self.env["mail.alias.domain"].search([]).mapped("name")
            )
        ...
        if rcpt_tos_list:
            ...
            if dest_aliases:
                ...
                for alias in dest_aliases:
                    ...
                    AliasModel = (
                        self.env[route[0]]
                        if route[0] in self.env
                        and hasattr(self.env[route[0]], "_routing_check_route")
                        else self
                    )
        ...
        if rcpt_tos_list and self.with_context(
            mail_catchall_write_any_to=True
        )._detect_write_to_catchall(message_dict):
            _logger.info(
        ...
        raise ValueError(
            f"No possible route found for incoming message from {email_from} to {message_dict['to']} (Message-Id {message_id}:). "
    ...
    def _message_route_process(self, message, message_dict, routes):
        self = self.with_context(
            attachments_mime_plainxml=True
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if not (
                (thread_id and hasattr(Model, "message_update"))
                or hasattr(Model, "message_new")
            ):
                raise ValueError(
                    f"Undeliverable mail with Message-Id {message_dict['message_id']}, model {model} does not accept incoming emails"
                )
            ...
            if thread_id and hasattr(ModelCtx, "message_update"):
                thread = ModelCtx.browse(thread_id).exists()
                if thread:
                    thread.message_update(message_dict)
                else:
                    _logger.info(
                        "Routing mail with Message-Id %s: thread %s(%s) no longer exists, creating new thread",
                        message_dict.get("message_id"),
                        model,
                        thread_id,
                    )
            ...
            for x in (
                ...
                "recipients",
                "cc",
                "to",  # use cc_filtered, to_filtered
                "references",
                "in_reply_to",
                "x_odoo_message_id",
                "is_bounce",
            ...
            new_msg = False
            if (
                thread_root._name == "mail.thread"
            ):  # message with parent_id not linked to record
                ...
                thread_root = thread_root.with_context(
                    mail_post_autofollow_author_skip=not message_dict.get("author_id")
                )
    ...
    def message_process(
        ...
        # prefetch alias domains once for the entire processing pipeline
        # (used by _detect_is_bounce, message_route, and sub-methods)
        if not self.env.context.get("mail_alias_domains_prefetched"):
            alias_domains = self.env["mail.alias.domain"].search([])
            self = self.with_context(
                mail_alias_domains_prefetched=True,
                mail_bounce_aliases=alias_domains.mapped("bounce_email"),
                mail_catchall_aliases=alias_domains.mapped("catchall_email"),
                mail_alias_domain_names=alias_domains.mapped("name"),
            )
        ...
        # update document-dependent values
        ...
        if message.get_content_maintype() == "text":
            ...
            for part in message.walk():
                ...
                if part.get_content_type() == "binary/octet-stream":
                    _logger.warning(
                        "Message containing an unexpected Content-Type 'binary/octet-stream', assuming 'application/octet-stream'"
                    )
                    part.replace_header("Content-Type", "application/octet-stream")
                if part.get_content_type() == "*/*":
                    _logger.warning(
                        "Message containing an unexpected Content-Type '*/*', assuming 'application/octet-stream'"
                    )
                    part.replace_header("Content-Type", "application/octet-stream")
    ...
    def _message_parse_extract_bounce(self, email_message, message_dict):
        ...
        if (
            ...
            # Keep the canonical bounced_email from the DSN headers — the
            # partner's current email may have changed since the bounce.
    ...
    def message_parse(self, message, save_original=False):
        ...
        """
        if not isinstance(message, EmailMessage):
            raise TypeError(_("Message should be a valid EmailMessage instance"))
        ...
        if message.get("Date"):
            try:
                ...
                stored_date = datetime.datetime.now(datetime.UTC)
    ...
    def _partner_find_from_emails(
        ...
        def sort_key(p):
            return (
                p.id == current_partner_id,  # prioritize user
                p.id in follower_ids,  # then followers
                not p.partner_share,  # prioritize internal users
                bool(p.user_ids),  # prioritize portal users
                p.company_id.id
                == emails_key_company_id.get(
    ...
    def _mail_find_user_for_gateway(self, email_value, alias=None):
        ...
        if alias and alias.alias_parent_model_id and alias.alias_parent_thread_id:
            ...
            record_su = (
                record_su
                if hasattr(record_su, "_partner_find_from_emails_single")
    ...
    def _mail_find_partner_from_emails(
        ...
        if records and isinstance(records, self.pool["mail.thread"]):
            ...
            all_partners = self.env["res.partner"].browse(
                {
                    partner.id
                    for partners in results.values()
                ...
                results.append(
                    next(
                        (
                            ...
                            if email_key in (p.email_normalized, p.email)
                        ),
                        self.env["res.partner"],
                    )
                )
    ...
    def _process_attachments_for_post(
        ...
        if attachments:  # generate
            ...
            for attachment in attachments:
                ...
                # keep cid, name list and token synced with attachment_values_list length to match ids later
                attachment_extra_list.append((cid, name, token, info))
            ...
            for attachment, (cid, name, token, _info) in zip(
                new_attachments, attachment_extra_list, strict=False
            ):
                if cid:
    ...
    def _message_log_with_view(
        self, view_ref, render_values=None, message_type="notification", **kwargs
    ):
    ...
    def _message_compute_real_author(self, author_id):
        ...
        ):  # note that odoobot is always inactive, there is a python check
            real_author = self.env.user.partner_id
        elif author_id:
            author = self.env["res.partner"].browse(author_id)
            if author.active and author != self.env.ref(
                "base.partner_root"
            ):  # that happened :(
                real_author = author
        return real_author
    ...
    def _message_compute_parent_id(self, parent_id):
        ...
        if self._mail_flat_thread and not current_ancestor:
            ...
            # Prioritize comment/email types in SQL to avoid a Python re-sort.
            self.env["mail.message"].flush_model(
                ["res_id", "model", "message_type", "date"]
            )
            self.env.cr.execute(
                """SELECT id FROM mail_message
                    ...
                    ORDER BY (message_type IN ('comment', 'email')) DESC,
                             COALESCE(date, create_date) DESC, id DESC
                    LIMIT 1""",
                [self.id, self._name],
            )
            row = self.env.cr.fetchone()
            current_ancestor = MailMessage_sudo.browse(row[0]) if row else MailMessage_sudo
            ...
            if not template and not view:
                raise ValueError(
                    _(
                        ...
                        source_type=(
                            _("template") if template is not False else _("view")
                        ),
                    )
                )
    ...
    def _raise_for_invalid_parameters(
        self, parameter_names, forbidden_names=None, restricting_names=None
    ):
    ...
    def _notify_by_email_get_base_mail_values(
            ...
            .search(
                [
                    ...
                    (
                        ...
                        "!=",
                        False,
                    ),  # ignore records that somehow don't have a message_id (non ORM created)
                ],
                limit=32,
                order="id DESC",  # take 32 last, hoping to find public discussions in it
            )
        )
        ...
        history_ancestors = ancestors.sorted(
            lambda m: (
                ...
                not in (
                    "user_notification",
                    "out_of_office",
                ),  # user notif / out of office -> avoid if possible
            ),
            reverse=True,
        )  # False before True unless reverse
        ...
        ):  # more than threshold = considered as public record (slide, forum, ...) -> do not leak
    ...
    def _notify_by_email_get_final_mail_values(
        self, recipient_ids, mail_values, additional_values=None
    ):
    ...
    def _notify_get_recipients(self, message, msg_vals=False, **kwargs):
        ...
        # get values from msg_vals or from message if msg_vals doesn't exist
        ...
        if not notify_author:
            notify_author_mention = kwargs.get(
                "notify_author_mention"
            ) or self.env.context.get("mail_notify_author_mention")
        ...
        for pid, pdata in res.items():
            ...
            if pdata["active"] is False:
                continue
            # Skip only email-type recipients already present in the
            # incoming email's To/CC — inbox users still need their
            # Odoo notification even if their address was in the
            # original email headers.
            if (
                pdata["email_normalized"] in emailed_normalized
                and pdata["notif"] == "email"
            ):
                continue
    ...
    def _notify_get_recipients_groups_fillup(
        self, groups, model_description, msg_vals=False
    ):
    ...
    def _notify_get_action_link(self, link_type, **kwargs):
        ...
        link = f"{base_link}?{urlencode(sorted(params.items()))}"
        if self:
    ...
    def _notify_thread_with_out_of_office(
        ...
        internal_uids = [
            ...
            if (
                ...
                and r["id"]
                and r["id"] in pids
                and r["id"] not in recipient.ids  # don't OOO myself
                and r["uid"]
                and not r["share"]
            )
        ]
        ...
        for user in ooo_users.filtered(lambda u: u.partner_id not in already_mailed):
            body = self.env["ir.qweb"]._render(
                "mail.message_notification_out_of_office",
                {
                    ...
                    "replied_body": (
                        msg_vals["body"] if "body" in (msg_vals or {}) else message.body
                    ),
    ...
    def _notify_thread_with_out_of_office_get_additional_users(
        self, message, recipients_data, ooo_author, msg_vals=False
    ):
    ...
    def _encode_link(self, base_link, params):
        ...
        token = (
            f"{base_link}?{' '.join(f'{key}={params[key]}' for key in sorted(params))}"
        )
        return hmac.new(
    ...
    def _get_model_description(self, model_name):
        ...
        return (
            self.env["ir.model"]._get(model_name).display_name
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        ...
        if (
            ...
            try:  # avoid to make an exists, lets be optimistic and try to read it.
                ...
                _logger.warning(
                    "Failed to auto-subscribe user %s on %s(%s)",
                    user_id,
                    self._name,
                    self.id,
                    exc_info=True,
    ...
    def _message_auto_subscribe_notify(self, partner_ids, template):
        ...
        for record in self:
            ...
            company = (
                record.company_id.sudo() if "company_id" in record else self.env.company
            )
            values = {
            ...
            assignation_msg = self.env["ir.qweb"]._render(
                template, values, minimal_qcontext=True
            )
    ...
    def _message_auto_subscribe(self, updated_values, followers_existing_policy="skip"):
        ...
        udpated_fields = [
            fname
            for fnames in updated_relation.values()
            for fname in fnames
        ...
        for partner_id, sids, template in res:
            sids_set = set(sids)
            # Strip internal subtypes for portal/public partners — same
            # logic applied by the parent-relation auto-subscribe above.
            if self.env["res.partner"].browse(partner_id).partner_share:
                sids_set -= set(all_int_ids)
            # Union subtypes: the partner may already have subtypes from
            # parent-relation auto-subscribe — merge rather than discard.
            if partner_id in new_partner_subtypes:
                new_partner_subtypes[partner_id] |= sids_set
            else:
                new_partner_subtypes[partner_id] = sids_set
            if template:
    ...
    def _message_auto_subscribe_batch(
        self, vals_per_record, followers_existing_policy="skip"
    ):
        ...
        parent_subscription_data = (
            {}
        )  # {parent_doc_id: [(partner_id, sids, pshare, active)]}
                    ...
                    for fname in fnames:
                        ...
                        for (
                            ...
                            sids += [
                                ...
                                if sid not in parent and sid in child_ids
                            ]
                            if partner_id and active:
                                if pshare:
                                    new_partner_subtypes[partner_id] = set(sids) - set(
                                        all_int_ids
                                    )
                                else:
                                    new_partner_subtypes[partner_id] = set(sids)
        ...
        if all_new_partner_subtypes:
            ...
            # ONE existence-check query for all records x all partners
            doc_pids = {rid: set() for rid in all_record_ids}
            data_fols = {}
            if all_partner_ids:
                for fid, rid, pid, sids in self.env[
                    "mail.followers"
                ]._get_subscription_data(
                    [(self._name, all_record_ids)],
        ...
        for record_id, notify_data in all_notify_data.items():
            ...
            for (template, lang), pids in notify_data.items():
                record.with_context(lang=lang)._message_auto_subscribe_notify(
                    pids, template
                )
        ...
        if body is not None:
            if body or not message._filter_empty():
                ...
                if len(children) > 0:  # body is a valid html
                    ...
                    msg_values["body"] = escape(body) + Markup(
    ...
    def _thread_to_store(self, store: Store, fields, *, request_list=None):
        ...
        for thread in self:
            ...
            if "followers" in request_list:
                ...
                # single query for followersCount, selfFollower id, recipientsCount
                self.env["mail.followers"].flush_model(
                    ["partner_id", "res_id", "res_model", "subtype_ids"]
                )
                self.env["res.partner"].flush_model(["active"])
                self.env.cr.execute(
                    """SELECT
                        COUNT(*) AS followers_count,
                        (SELECT id FROM mail_followers
                          WHERE res_id = %(res_id)s AND res_model = %(res_model)s
                            AND partner_id = %(pid)s
                          LIMIT 1
                        ) AS self_follower_id,
                        (SELECT COUNT(*) FROM mail_followers fol
                          JOIN mail_followers_mail_message_subtype_rel rel
                            ON rel.mail_followers_id = fol.id
                           AND rel.mail_message_subtype_id = %(subtype_id)s
                          JOIN res_partner p ON p.id = fol.partner_id AND p.active
                         WHERE fol.res_id = %(res_id)s AND fol.res_model = %(res_model)s
                           AND fol.partner_id != %(pid)s
                        ) AS recipients_count
                      FROM mail_followers
                     WHERE res_id = %(res_id)s AND res_model = %(res_model)s""",
                    {
                ...
                row = self.env.cr.fetchone()
                res["followersCount"] = row[0]
                self_follower = (
                    self.env["mail.followers"].browse(row[1]) if row[1] else
                    self.env["mail.followers"]
                )
                res["selfFollower"] = Store.One(self_follower)
                res["recipientsCount"] = row[2]
            ...
            if "scheduledMessages" in request_list:
                res["scheduledMessages"] = Store.Many(
