...
class MailPoll(models.Model):
    ...
    end_message_id: MailMessage = fields.Many2one(
        "mail.message", ondelete="cascade", index=True
    )
    option_ids: MailPollOption = fields.One2many(
        "mail.poll.option", "poll_id", required=True
    )
    poll_end_dt = fields.Datetime(index=True)
    poll_question = fields.Char(required=True)
    start_message_id: MailMessage = fields.Many2one(
        "mail.message", ondelete="cascade", required=True, index=True
    )
    winning_option_id: MailPollOption = fields.Many2one(
        "mail.poll.option", compute="_compute_winning_option_id"
    )
    ...
    def _compute_winning_option_id(self) -> None:
    ...
    @api.model
    def _end_expired_polls(self) -> None:
        self.env["mail.poll"].search_fetch(
            [("poll_end_dt", "<=", "now"), ("end_message_id", "=", False)]
        )._end_and_notify()

    @api.ondelete(at_uninstall=False)
    def _poll_on_delete(self) -> None:
        messages = self.start_message_id | self.end_message_id
        thread_by_message = messages._record_by_message()
