...
class MailMessage(models.Model):
    ...
    * snailmail.letter (snailmail addon)
    ...
    def _compute_needaction(self):
        ...
        self.env.cr.execute(
            """SELECT mail_message_id FROM mail_notification
                WHERE mail_message_id = ANY(%s)
                  AND res_partner_id = %s
                  AND (is_read = false OR is_read IS NULL)""",
            [list(self.ids), self.env.user.partner_id.id],
        )
        needaction_ids = {row[0] for row in self.env.cr.fetchall()}
        for message in self:
            message.needaction = message.id in needaction_ids
    ...
    def _compute_has_error(self):
        if not self.ids:
            ...
            return
        self.env["mail.notification"].flush_model(
            ["mail_message_id", "notification_status"]
        )
        self.env.cr.execute(
            """SELECT DISTINCT mail_message_id FROM mail_notification
                WHERE mail_message_id = ANY(%s)
                  AND notification_status IN ('bounce', 'exception')""",
            [list(self.ids)],
        )
        error_ids = {row[0] for row in self.env.cr.fetchall()}
        for message in self:
            message.has_error = message.id in error_ids
    ...
    def _compute_starred(self):
        ...
        self.env.cr.execute(
            """SELECT mail_message_id FROM mail_message_res_partner_starred_rel
                WHERE mail_message_id = ANY(%s)
                  AND res_partner_id = %s""",
            [list(self.ids), self.env.user.partner_id.id],
        )
        starred_ids = {row[0] for row in self.env.cr.fetchall()}
        for message in self:
            ...
            if "body" in values:
                ...
                def base64_to_boundary(
                    ...
                    if not data_to_url.get(key):
                        name = match.group(4) or f"image{len(data_to_url)}"
                        try:
                            ...
                            _logger.warning(
                                "Impossible to create an attachment out of badly formatted base64 embedded image. Image has been removed."
                            )
                            return match.group(
                                3
    ...
    def _is_thread_message(self, vals=False, thread=None):
        ...
        res_id = (
            vals["res_id"]
            if "res_id" in vals
            else thread.ids[0] if thread and thread.ids else self.res_id
        )
