...
if typing.TYPE_CHECKING:
    from .mail_poll import MailPoll
    from .mail_poll_vote import MailPollVote
...
class MailPollOption(models.Model):
    ...
    poll_id: MailPoll = fields.Many2one(
        "mail.poll", ondelete="cascade", required=True, index=True
    )
    selected_by_self = fields.Boolean(compute="_compute_selected_by_self")
    vote_ids: MailPollVote = fields.One2many("mail.poll.vote", "option_id")
    ...
    _check_option_label = models.Constraint(
        ...
        "Options must have a non-empty option label.",
    )
    ...
    def _compute_selected_by_self(self) -> None:
        user, guest = self.env["mail.poll.vote"]._get_current_voter()
    ...
    @api.depends("vote_ids")
    def _compute_number_of_votes(self) -> None:
    ...
    def _compute_vote_percentage(self) -> None:
    ...
    def _to_store_defaults(self, target: Store.Target) -> StoreFieldsInput:
        ...
        if target.is_current_user(self.env):
            field_names.append("selected_by_self")
        return field_names
