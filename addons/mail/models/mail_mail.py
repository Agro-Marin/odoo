...
_UNFOLLOW_REGEX = re.compile(
    r'<span\s*(t-if="show_unfollow")?\s*id="mail_unfollow".*?<\/span>', re.DOTALL
)
...
class MailMail(models.Model):
    ...
    def default_get(self, fields):
        ...
        if (
            self.env.context.get("default_type")
            not in self._fields["message_type"].base_field.selection
        ):
            self = self.with_context(dict(self.env.context, default_type=None))
        if (
            self.env.context.get("default_state")
            not in self._fields["state"].base_field.selection
        ):
    ...
    unrestricted_attachment_ids = fields.Many2many(
        "ir.attachment",
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            ...
            if values.get("scheduled_date"):
                parsed_datetime = self._parse_scheduled_datetime(
    ...
    def _estimate_email_size(self, headers, body, attachments_size):
        ...
        return (
            ...
            * 8
            / 6  # 8 / 6 factor because base 64 are 6 bits coded in 8 bits
            + 10 * 1024  # adding 10Ko as security
        )

    def _filter_mail_mail_servers(self, mail_servers):
        if len(
            self.mail_message_id.create_uid
        ) > 1 or self.env[  # multiple create_uids -> subset that's allowed for all
            "ir.config_parameter"
        ].sudo().get_param(
    ...
    def _prepare_outgoing_list(self, mail_server=False, doc_to_followers=None):
        ...
        if body and attachments:
            ...
            if link_ids:
                attachments = attachments - self.env["ir.attachment"].browse(
                    list(link_ids)
                )
    ...
    def _split_by_mail_configuration(self):
        ...
        for mail, values in zip(self, mail_values, strict=False):
            ...
            mail_server_id = (
                values["mail_server_id"][0] if values["mail_server_id"] else False
            )
            alias_domain_id = (
                values["record_alias_domain_id"][0]
                if values["record_alias_domain_id"]
                else False
            )
            key = (
                mail_server_id,
    ...
    def _split_by_delayed_batch(self, mail_server):
        ...
        # ORM Datetime fields return naive UTC datetimes, so current_minute
        # must also be naive to allow comparison with owner_limit_time.
        current_minute = datetime.datetime.now(datetime.UTC).replace(
        ...
        for mail in self.sorted(lambda k: (k.create_date, k.id)):
            if mail_server.owner_limit_count >= MAX_SEND:
                ...
                # Split the mail so the first batch respects the
                # per-minute limit.  The *copy* inherits email_to/cc
                # so direct addresses are delivered in the first
                # batch.  The *original* (delayed) clears email_to/cc
                # to avoid duplicate delivery when those addresses
                # overlap with partner recipients sent later.
    ...
    def _send(
        ...
        for mail_id in self.ids:
            ...
            mail = None
            try:
                # Atomically check state and acquire a row lock to prevent
                # concurrent cron workers from processing the same mail.
                # SKIP LOCKED avoids blocking: if another worker already
                # holds this row, we simply skip it.
                self.env.cr.execute(
                    "SELECT id FROM mail_mail WHERE id = %s AND state = 'outgoing' FOR UPDATE SKIP LOCKED",
                    [mail_id],
                )
                if not self.env.cr.fetchone():
                    continue
                # Invalidate cache for this record to ensure the ORM reads
                # fresh values after acquiring the lock.
                self.browse(mail_id).invalidate_recordset()
                mail = self.browse(mail_id)
                no_recipients = (
                    not (mail.email_to or "").strip()
                    and not mail.recipient_ids
                    and not (mail.email_cc or "").strip()
                )
                ...
                mail.write(
                    {
                        ...
                        "failure_type": (
                            "mail_email_missing" if no_recipients else "unknown"
                        ),
                    }
                )
                ...
                if isinstance(e, UserError):
                    # Handle UserError raised in IrMailServer for from-specific errors.
                    # NO_VALID_FROM now includes extra context in the message, so use `in`.
                    error_msg = str(e)
                    failure_reason = error_msg
                    if IrMailServer.NO_VALID_FROM in error_msg:
                ...
                if raise_exception:
                    if isinstance(e, UnicodeEncodeError):
                        raise MailDeliveryException(
                            "Invalid text: %s" % e.object
                        ) from e
                    if isinstance(e, UserError):
                        raise MailDeliveryException(str(e)) from e
                    raise
