...
class MailAliasDomain(models.Model):
    ...
    def _find_aliases(self, email_list):
        ...
        aliases += potential_aliases.filtered(
            lambda x: not x.alias_incoming_local
        ).mapped("alias_full_name")
        # compat-mode alias: left-part only (filter on allowed domains already done)
        local_alias_names = potential_aliases.filtered(
            lambda x: x.alias_incoming_local
        ).mapped("alias_name")
        return [
            email
            for email in filtered_emails
            if (email in aliases or email.partition("@")[0] in local_alias_names)
        ]
