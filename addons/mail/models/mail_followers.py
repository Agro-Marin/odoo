...
GROUP BY fol.id%s%s""" % (
    ...
    def _insert_followers(
        ...
        # batch updates: group followers with identical subtype commands
        if upd:
            grouped_updates = defaultdict(list)
            for fol_id, values in upd.items():
                # use tuple of sorted commands as grouping key
                key = tuple(sorted(
                    (cmd for cmd in values.get("subtype_ids", [])),
                    key=str,
                ))
                grouped_updates[key].append(fol_id)
            for _key, fol_ids in grouped_updates.items():
                # all followers in this group share the same update values
