...
class MailTemplate(models.Model):
    ...
    model_id = fields.Many2one(
        ...
        "Applies to",
        ondelete="cascade",
        domain=_get_non_abstract_models_domain,
    )
    model = fields.Char(
    ...
    def _check_abstract_models(self, vals_list):
        model_names = (
            self.sudo()
            .env["ir.model"]
            .browse(filter(None, (vals.get("model_id") for vals in vals_list)))
            .mapped("model")
        )
    ...
    def write(self, vals):
        ...
        super().write(vals)
        self._check_can_be_rendered(
            fnames=(
                vals.keys() if {"model", "model_id"}.isdisjoint(vals.keys()) else None
            )
        )
                    ...
                    copy.write(
                        {
                            "attachment_ids": [
                                ...
                                for att_copy in (
                                    ...
                                    for attachment in original.attachment_ids
                                )
                            ]
                        }
                    )
            ...
            if "report_template_ids" in render_fields and self.report_template_ids:
                for report in self.report_template_ids:
                    ...
                    if report.report_type in ["qweb-html", "qweb-pdf"]:
                        ...
                        render_res = self.env["ir.actions.report"]._render(
                            report, [res_id]
                        )
                        if not render_res:
                            raise UserError(
                    ...
                    values.setdefault("attachments", []).append(
        ...
        if hasattr(self.env[self.model], "_process_attachments_for_template_post"):
            ...
            for res_id, additional_attachments in records_attachments.items():
                ...
                if additional_attachments.get("attachments"):
                    render_results[res_id].setdefault("attachments", []).extend(
    ...
    def _generate_template_recipients(
        ...
        if self.use_default_to and self.model:
            if allow_suggested:
                suggested_recipients = Model.browse(
                    res_ids
                ...
                default_recipients = Model.browse(
                    res_ids
        ...
        if find_or_create_partners:
            ...
            if hasattr(Model, "_partner_find_from_emails"):
                ...
                records_partners = self.env["mail.thread"]._partner_find_from_emails(
                    records_emails
                )
        ...
        for record_values in render_results.values():
            ...
            # Deduplicate partner_ids: the same partner can appear from
            # both email_to → _partner_find_from_emails and partner_to.
            if "partner_ids" in record_values:
                record_values["partner_ids"] = list(set(record_values["partner_ids"]))
    ...
    def _generate_template_static_values(
        self, res_ids, render_fields, render_results=None
    ):
    ...
    def _parse_partner_to(cls, partner_to):
        ...
        return [
            int(pid.strip()) if isinstance(pid, str) else int(pid)
            for pid in partner_to
    ...
    def send_mail(
        ...
        return self.send_mail_batch(
            ...
            0
        ].id  # TDE CLEANME: return mail + api.returns ?
    ...
    def _expression_is_default(self, source, model, fname):
        ...
        model_defaults = (
            hasattr(Model, "_mail_template_default_values")
            and Model._mail_template_default_values()
        ) or {}
