...
class MailTrackingValue(models.Model):
    ...
    def _create_tracking_values(
        ...
        if col_info["type"] in {"integer", "float", "char", "text", "datetime"}:
            ...
            currency_field = col_info.get("currency_field")
            ...
            selection_map = dict(col_info["selection"])
            values.update(
                {
                    "old_value_char": (
                        initial_value
                        and selection_map.get(initial_value, initial_value)
                    )
                    or "",
                    "new_value_char": (
                        new_value and selection_map.get(new_value, new_value)
                    )
                    or "",
                }
            )
