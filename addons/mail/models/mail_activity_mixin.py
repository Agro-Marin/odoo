...
class MailActivityMixin(models.AbstractModel):
    ...
    def _compute_activity_exception_type(self):
        ...
        for record in self:
            ...
            record.activity_exception_decoration = (
                exception_activity_type_id
                and exception_activity_type_id.decoration_type
            )
            record.activity_exception_icon = (
                exception_activity_type_id and exception_activity_type_id.icon
            )
    ...
    def _compute_activity_user_id(self):
        for record in self:
            record.activity_user_id = (
                record.activity_ids[0].user_id if record.activity_ids else False
            )
    ...
    def _search_activity_state(self, operator, value):
        ...
        self.env["mail.activity"].flush_model(
            ["active", "date_deadline", "res_model", "user_id", "user_tz"]
        )
        query = SQL(
            """(
            SELECT res_id
    ...
    def _compute_activity_date_deadline(self):
        for record in self:
            ...
            record.activity_date_deadline = next(
                iter(activities), activities
            ).date_deadline

    def _search_activity_date_deadline(self, operator, operand):
        if operator == "in" and False in operand:
            return Domain("activity_ids", "=", False) | Domain(
                self._search_activity_date_deadline("in", operand - {False})
            )
    ...
    def _search_activity_user_id(self, operator, operand):
        ...
        return domain | Domain(
            "activity_ids",
            "any",
            [("active", "in", [True, False]), ("user_id", operator, operand)],
        )
    ...
    def _read_group_groupby(self, alias, groupby_spec, query):
        ...
        # Use 'utc' as the fallback timezone when the activity has no
        # user_tz — consistent with _search_activity_state which also
        # falls back to 'utc'.
        sql_join = SQL(
            """
            (SELECT res_id,
                CASE
                    WHEN min(EXTRACT(day from (mail_activity.date_deadline - DATE_TRUNC('day', %(today_utc)s AT TIME ZONE COALESCE(mail_activity.user_tz, 'utc'))))) > 0 THEN 'planned'
                    WHEN min(EXTRACT(day from (mail_activity.date_deadline - DATE_TRUNC('day', %(today_utc)s AT TIME ZONE COALESCE(mail_activity.user_tz, 'utc'))))) < 0 THEN 'overdue'
                    WHEN min(EXTRACT(day from (mail_activity.date_deadline - DATE_TRUNC('day', %(today_utc)s AT TIME ZONE COALESCE(mail_activity.user_tz, 'utc'))))) = 0 THEN 'today'
                    ELSE null
    ...
    def activity_schedule(
        ...
        invalid_model = (
            activity_type.res_model and activity_type.res_model != self._name
        )
    ...
    def activity_reschedule(
        ...
        activity_types_ids = [
            act_type_id for act_type_id in activity_types_ids if act_type_id
        ]
        if not any(activity_types_ids):
            return False
