...
# Increase POP3 line limit — some servers send response lines exceeding the
# 2048-byte default (e.g. long headers, large UID lists).
...
class OdooIMAP4(IMAP4):
    ...
    def retrieve_unread_messages(self):
        if self._unread_messages is None:
            raise RuntimeError(
                "Must call check_unread_messages() before retrieve_unread_messages()"
            )
...
class OdooPOP3(POP3):
    """POP3 wrapper that defers message deletion until disconnect.

    POP3's DELE command is only finalized on QUIT.  However, if the
    Odoo process crashes after DELE but before the database transaction
    commits, the email is permanently lost from the server while Odoo
    never recorded it.  By collecting handled message numbers and
    issuing DELE only at disconnect time (after the DB commit), we
    ensure that a crash before commit leaves messages on the server
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._unread_messages = None
        self._handled_messages: list[int] = []
    ...
    def handled_message(self, num):
        """Mark a message for deletion — the actual DELE is deferred
        until :meth:`disconnect` so that a crash before DB commit does
        not lose the email."""
        self._handled_messages.append(num)

    def disconnect(self):
        for num in self._handled_messages:
            self.dele(num)
        # Clear only after all DELEs succeed — if one fails, the
        # exception propagates and the caller can decide what to do.
        # QUIT finalizes the deletions on the server.
        self._handled_messages.clear()
        self.quit()
        # Note: POP3 QUIT commits all pending DELEs atomically on the
        # server side.  If QUIT itself fails, the server rolls back
        # all DELE commands, so no data is lost.
...
class FetchmailServer(models.Model):
    ...
    server = fields.Char(
        string="Server Name", readonly=False, help="Hostname or IP of the mail server"
    )
    port = fields.Integer()
...
odoo_mailgate: "|/path/to/odoo-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
    ...
    def _connect__(self, allow_archived=False):
        ...
        if connection_type == "imap":
            ...
            connection = (
                OdooIMAP4_SSL(server, port, timeout=MAIL_TIMEOUT)
                if is_ssl
                else OdooIMAP4(server, port, timeout=MAIL_TIMEOUT)
            )
            self._imap_login__(connection)
        elif connection_type == "pop":
            server, port, is_ssl = self.server, int(self.port), self.is_ssl
            connection = (
                OdooPOP3_SSL(server, port, timeout=MAIL_TIMEOUT)
                if is_ssl
    ...
    def button_confirm_login(self):
        for server in self:
            ...
            try:
                ...
                raise UserError(_("Connection test failed: %s", str(err))) from err
            finally:
                try:
                    if connection:
    ...
    def _fetch_mails(self, **kw):
        ...
        if (
            ...
            raise RuntimeError("_fetch_mails is meant for cron usage only")
    ...
    def _fetch_mail(self, batch_limit=50) -> Exception | None:
        ...
        total_remaining = len(
            servers
        )  # number of remaining messages + number of unchecked servers
        ...
        for server in servers:
            ...
            try:
                ...
                _logger.debug(
                    "%d unread messages on %s server %s.",
                ...
                for (
                    message_num,
                    message,
                ) in server_connection.retrieve_unread_messages():
                    _logger.debug(
                        "Fetched message %r on %s server %s.",
                        message_num,
                        *server_type_and_name,
                    )
                    count += 1
                    ...
                    try:
                        ...
                        # Only mark the message as handled (IMAP: Seen flag,
                        # POP3: queued for deletion) when processing succeeded.
                        # On failure the message stays unprocessed so it can be
                ...
                if not server.error_date:
                    ...
                    server.error_date
                    < fields.Datetime.now() - MAIL_SERVER_DEACTIVATE_TIME
                ):
                    message = f"Deactivating fetchmail {server_type_and_name[0]} server {server_type_and_name[1]} (too many failures)"
                    server.set_draft()
