...
class ResUsers(models.Model):
    ...
    def _notify_security_setting_update_prepare_values(self, content, **kwargs):
        ...
        values = {
            ...
            "suggest_password_reset": kwargs.get("suggest_password_reset", True)
            and reset_password_enabled,
            "user": self,
            "useros": False,
        }
        if not request:
    ...
    def _get_portal_access_update_body(self, access_granted):
        ...
        if self.partner_id.email:
            return f"{body} ({self.partner_id.email})"
        return body
    ...
    def _get_activity_groups(self):
        ...
        # collect activity IDs by (model, res_id) using plain lists to avoid
        # quadratic recordset concatenation
        ...
        # prefetch state for all activities at once (triggers _compute_state in batch)
        activities.mapped("state")

        model_activity_ids = defaultdict(list)
        ...
        for model_name, ids_by_record in activities_rec_ids.items():
            res_ids = list(ids_by_record.keys())
            ...
            for record_id, act_ids in ids_by_record.items():
                if record_id in unallowed_ids:
                    model_key = "mail.activity"
                    model_activity_ids["mail.activity"].extend(act_ids)
                elif record_id in allowed_ids:
                    model_key = model_name
                    model_activity_ids[model_name].extend(act_ids)
                elif record_id:
                    continue

                # state is already cached from the batch prefetch above
                record_activities = self.env["mail.activity"].browse(act_ids)
                states = record_activities.mapped("state")
        ...
        # build activity recordsets from collected IDs (single browse per model)
        activities_model_groups = {
            model_name: self.env["mail.activity"].browse(ids)
            for model_name, ids in model_activity_ids.items()
        }
        model_ids = [
