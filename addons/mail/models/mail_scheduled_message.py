    ...
    def _post_message(self, raise_exception=True):
        ...
        for scheduled_message in self:
            ...
            try:
                ...
                # Unlink before commit so that the posted message and
                # its schedule removal are in the same transaction.
                # This prevents duplicate posting if the server crashes
                # after commit but before a deferred unlink.
                ...
                try:
                    ...
                    # Also remove the failed schedule so it is not retried
                    # endlessly — the user gets notified about the failure.
                    scheduled_message.unlink()
                    if auto_commit:
                        self.env.cr.commit()
                except Exception:
