...
# Distinct from mail.message.schedule, which holds an already posted message
# and only delays its notification process.
class MailScheduledMessage(models.Model):
    """Post values generated by the composer, held to delay the posting itself."""
    ...
    # (making 'mail.compose.message' non-transient) to cancel batches together
    # and share the static notification parameters instead of duplicating them
    # per message; 'email_from' and 'force_email_lang' then become dynamic.
    ...
    # context used when posting the message to trigger some actions
    # (eg. change SO state when sending quotation)
    ...
    def _search(
        ...
        """Return only the messages scheduled on records the user may post on.

        The required right is the target model's ``_mail_post_access`` ("write"
        unless the model overrides it), checked on the related records.
        """
        ...
        # The access filter below runs in Python, so the caller's LIMIT/OFFSET must
        # NOT be pushed into SQL: doing so truncates the candidate set before
        # filtering and returns short / empty pages when inaccessible rows fill the
        # window. Materialize the matching ids, filter by document access, then
        # slice. Raw SQL rather than the ORM, to avoid cache pollution.
        ...
        for model, res_ids in model_ids.items():
            ...
            # Filter per record: has_access() is all-or-nothing on the batch, so one
            # inaccessible record would drop every scheduled message of that model,
            # including those on records the user can access.
    ...
    def _message_created_hook(self, message):
        """Hook called after a scheduled message has been posted."""
        self.ensure_one()

    def _post_message(self, raise_exception=True):
        """Post the scheduled messages of self, then delete them.

        Posting runs as the message creator: their post permission on the related
        record is re-checked, and attachments can be transferred to the message
        (see _process_attachments_for_post in mail.thread).

        :param bool raise_exception: if False (cron mode), a failing message is
          skipped and its creator notified instead of raising;
        """
    ...
    def _check(self, values=None):
        """Restrict access to a scheduled message, based on the record it targets.

        :param dict values: model and res_id of an extra record to check;
        """
