...
class DiscussChannel(models.Model):
    ...
    def _subscribe_users_automatically_get_members(self):
        ...
        return {
            channel.id: (
                channel.group_ids.all_user_ids.partner_id.filtered(lambda p: p.active)
    ...
    def _notify_by_web_push_prepare_payload(
        ...
        if self.channel_type == "chat":
            ...
            payload["title"] = f"#{record_name} - {author.name}"
            ...
            if not record_name:
                ...
                record_name = (
                    ...
                    else member_names[0] if member_names else ""
                )
            payload["title"] = f"{record_name} - {author.name}"
        else:
    ...
    def _message_post_after_hook(self, message, msg_vals):
        ...
        if self.parent_channel_id and message.partner_ids:
            ...
            if self.parent_channel_id.channel_type == "channel":
                # Only invite mentioned partners who are already members of
                # the parent channel — otherwise we'd bypass the parent's
                # membership requirements.
                parent_partner_ids = (
                    self.parent_channel_id.channel_member_ids.partner_id
                )
                to_invite |= (
                    (message.partner_ids & parent_partner_ids) - members.partner_id
                ).filtered(
                    lambda p: (
    ...
    def _get_or_create_chat(self, partners_to, pin=True):
        ...
        # Lock matching channels to prevent concurrent DM creation
        # between the same partners from producing duplicates.
        # PG18 prohibits FOR UPDATE with GROUP BY, so we use a CTE
        # to find the channel first, then lock it in the outer query.
        self.env.cr.execute(
            SQL(
                """
            WITH matching AS (
            ...
            JOIN matching ON matching.channel_id = C.id
            FOR UPDATE OF C
                """,
