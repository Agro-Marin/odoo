...
**Total: 69 `@http.route` handlers** — 36 in `controllers/`, 33 in `controllers/discuss/` —
...
|-------|----------|-------|
| `controllers/` | 36 | thread (9), poll (5), attachment (5), mail (4), mailbox (3), webclient (2), websocket (2), link_preview (2), message_reaction (1), google_translate (1), im_status (1), guest (1) — webmanifest = 0 |
| `controllers/discuss/` | 33 | channel (12), rtc (8), gif (5), public_page (4), settings (2), search (1), voice (1) |
| **Total** | **69** | **21** controller files (excluding the two `__init__.py`). One file contributes 0 routes — `webmanifest.py`; separately, the *class* `DiscussChannelWebclientController` in `channel.py` also declares none |
