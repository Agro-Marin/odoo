...
(`tests/`, 73 `test_*.py` files) and JavaScript HOOT (`static/tests/`, 156 `*.test.js`).
...
etc.). Of **143** tagged classes, **114** carry `post_install`/`-at_install`. Note both
...
**42 of the 73 test files carry no topic tag at all** and are reachable only by the module
