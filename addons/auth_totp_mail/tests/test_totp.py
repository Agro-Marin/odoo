...
class TestTOTPMail(TestTOTPMixin, HttpCase):
    def test_totp_rpc_api_keys_only(self):
        ...
        [result] = self.xmlrpc_object.execute_kw(
            db, uid, password, "res.users", "read", [uid, ["login"]]
        )
        self.assertEqual(result["login"], login)

        # Enable enforcing TOTP by mail
        self.env["res.config.settings"].create(
            {"auth_totp_enforce": True, "auth_totp_policy": "all_required"}
        ).execute()
        ...
        self.assertIn(
            "Invalid API key or password-based authentication", log_catcher.output[0]
        )

        # Create an API key for the user
        api_key = (
            self.env["res.users.apikeys"]
            .with_user(self.user_test)
            ._generate(None, "Foo", datetime.now() + timedelta(days=1))
        )

        # With TOTP by mail, xmlrpc using an API key is expected
        [result] = self.xmlrpc_object.execute_kw(
            db, uid, api_key, "res.users", "read", [uid, ["login"]]
        )
        self.assertEqual(result["login"], login)
