...
class ResPartnerBank(models.Model):
    ...
    def _get_qr_code_vals_list(
        ...
        if not currency.is_zero(amount):
            ...
            amount = None
        merchant_name = (
            self.partner_id.name and self._remove_accents(self.partner_id.name)[:25]
        ) or "NA"
        merchant_city = (
            self.partner_id.city and self._remove_accents(self.partner_id.city)[:15]
        ) or ""
    ...
    def _get_qr_vals(
        ...
        if qr_method == "emv_qr":
            ...
            qr_code_str = "".join(list(starmap(self._serialize, qr_code_vals)))
