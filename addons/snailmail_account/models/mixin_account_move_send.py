...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        if snailmail_moves_without_valid_address := moves.filtered(
            lambda m: (
                "snailmail" in moves_data[m]["sending_methods"]
                and not self.env["snailmail.letter"]._is_valid_address(m.partner_id)
            )
        ):
    ...
    def _prepare_snailmail_letter_values(self, move):
        return {
            ...
            "report_template": self.env["ir.actions.report"]
            ._get_report("account.account_invoices")
            .id,
        }
    ...
    def _hook_if_success(self, moves_data, from_cron=False):
        ...
        to_send = {
            ...
            if "snailmail" in move_data["sending_methods"]
            and self._is_applicable_to_move("snailmail", move, **move_data)
        }
        if to_send:
