...
class EventBoothCategory(models.Model):
    ...
    def _default_product_id(self):
        return self.env.ref(
            "event_booth_sale.product_product_event_booth", raise_if_not_found=False
        )
    ...
    def _compute_image_1920(self):
        for category in self:
            category.image_1920 = category.image_1920 or category.product_id.image_1920
    ...
    def _compute_price(self):
        ...
        for category in self:
            if category.product_id and category.product_id.list_price:
                category.price = (
                    category.product_id.list_price + category.product_id.price_extra
                )
    ...
    def _init_column(self, column_name):
        ...
        booth_category_ids = [row[0] for row in self.env.cr.fetchall()]
