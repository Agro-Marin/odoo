...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        for so in self:
            if not any(line.service_tracking == "event_booth" for line in so.line_ids):
                continue
            so_lines_missing_booth = so.line_ids.filtered(
                lambda line: (
                    line.service_tracking == "event_booth"
                    and not line.event_booth_pending_ids
                )
            )
            if so_lines_missing_booth:
                ...
                raise ValidationError(
                    _(
                        "Please make sure all your event-booth related lines are configured before confirming this order:%s",
                        so_lines_descriptions,
                    )
                )
            so.line_ids._update_event_booths()
        return res
