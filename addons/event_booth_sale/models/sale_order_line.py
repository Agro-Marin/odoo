...
class SaleOrderLine(models.Model):
    ...
    def _compute_event_booth_pending_ids(self):
        for so_line in self:
            so_line.event_booth_pending_ids = (
                so_line.event_booth_registration_ids.event_booth_id
            )
    ...
    def _inverse_event_booth_pending_ids(self):
        ...
        for so_line in self:
            existing_booths = (
                so_line.event_booth_registration_ids.event_booth_id
                or self.env["event.booth"]
            )
            selected_booths = so_line.event_booth_pending_ids

            so_line.event_booth_registration_ids.filtered_domain(
                [("event_booth_id", "not in", selected_booths.ids)]
            ).unlink()
    ...
    def _onchange_product_id_booth(self):
        ...
        if self.event_id and (
            not self.product_id
            or self.product_id not in self.event_booth_pending_ids.product_id
        ):
            self.event_id = None
    ...
    def _update_event_booths(self, set_paid=False):
        for so_line in self.filtered(
            lambda sol: sol.product_id.service_tracking == "event_booth"
        ):
            if so_line.event_booth_pending_ids and not so_line.event_booth_ids:
                unavailable = so_line.event_booth_pending_ids.filtered(
    ...
    def _get_price_display(self, pricelist_price=None, base_price=None):
        if self.event_booth_pending_ids and self.event_id:
            ...
            if not self.pricelist_item_id._is_discount_shown():
                ...
                total_price = sum(
                    booth.booth_category_id.price_reduce for booth in event_booths
                )
            else:
