...
class TestEventBoothSale(TestEventBoothSaleWData):
    ...
    def test_event_booth_prices_with_sale_order(self):
        ...
        self.assertEqual(
            float_compare(sale_order.amount_untaxed, 40.0, precision_rounding=0.1),
            0,
            "Untaxed amount should be the sum of the booths prices ($40.0).",
        )
        self.assertEqual(
            float_compare(sale_order.amount_total, 44.0, precision_rounding=0.1),
            0,
        ...
        self.assertEqual(
            float_compare(sale_order.amount_untaxed, 200.0, precision_rounding=0.1),
            0,
            "Untaxed amount should be the sum of the booths prices ($200.0).",
        )
        self.assertEqual(
            float_compare(sale_order.amount_total, 220.0, precision_rounding=0.1),
            0,
        ...
        self.assertEqual(
            float_compare(sale_order.amount_untaxed, 180.0, precision_rounding=0.1),
            0,
            "Untaxed amount should be the sum of the booths prices with discount 10% ($180.0).",
        )
        self.assertEqual(
            float_compare(sale_order.amount_total, 198.0, precision_rounding=0.1),
            0,
        ...
        for booth in self.booth_1 + self.booth_2:
            self.assertEqual(
                booth.sale_order_id.id,
                sale_order.id,
                "Booth sale order should be the same as the original sale order.",
            )
            self.assertEqual(
                booth.sale_order_line_id.id,
                sale_order.line_ids[0].id,
    ...
    def test_event_booth_registrations_inverse(self):
        ...
        self.assertEqual(
            sale_order.line_ids.event_booth_registration_ids.event_booth_id.ids,
        ...
        sale_order.write(
            {
                "line_ids": [
                    Command.update(
                        sale_order.line_ids.id,
                        {
                            "event_booth_pending_ids": [
                                Command.set((self.booth_2 + self.booth_3).ids)
                            ]
                        },
                    )
                ]
        ...
        sale_order.action_confirm()
        self.assertEqual(
            sale_order.event_booth_count, 2, "Event Booth Count should be equal to 2."
        )
        self.assertEqual(
            sale_order.line_ids.event_booth_registration_ids.event_booth_id.ids,
...
class TestEventBoothSaleInvoice(AccountTestInvoicingCommon, TestEventBoothSaleWData):
    ...
    def setUpClass(cls):
        ...
        cls.user_sales_salesman.group_ids += cls.env.ref(
