...
class L10n_It_Edi_DoiDeclaration_Of_Intent(models.Model):
    ...
    def _compute_display_name(self):
        for record in self:
            record.display_name = (
                f"{record.protocol_number_part1}-{record.protocol_number_part2}"
            )

    @api.depends(
        "invoice_ids", "invoice_ids.state", "invoice_ids.l10n_it_edi_doi_amount"
    )
    def _compute_invoiced(self):
        for declaration in self:
            ...
            declaration.invoiced = sum(
                relevant_invoices.mapped("l10n_it_edi_doi_amount")
            )

    @api.depends(
        "sale_order_ids",
        "sale_order_ids.state",
        "sale_order_ids.l10n_it_edi_doi_not_yet_invoiced",
    )
    ...
    def _compute_remaining(self):
        for record in self:
            record.remaining = (
                record.threshold - record.invoiced - record.not_yet_invoiced
            )
    ...
    def _prepare_threshold_warning_message(self, invoiced, not_yet_invoiced):
        ...
        return _(
            ...
            threshold=formatLang(
                self.env, self.threshold, currency_obj=self.currency_id
            ),
            exceeded=formatLang(
                self.env, -updated_remaining, currency_obj=self.currency_id
            ),
            invoiced=formatLang(self.env, invoiced, currency_obj=self.currency_id),
            not_yet_invoiced=formatLang(
                self.env, not_yet_invoiced, currency_obj=self.currency_id
            ),
        )
    ...
    def _get_validity_warnings(
        ...
        for declaration in self:
            ...
            if (
                declaration.currency_id.compare_amounts(invoiced_amount, 0) > 0
                or not only_blocking
            ):
                if declaration.state != "active":
                    errors.append(_("The Declaration of Intent must be active."))
                if not sales_order and (
                    not date
                    or declaration.start_date > date
                    or declaration.end_date < date
                ):
                    errors.append(
                        _(
                            "The Declaration of Intent is valid from %(start_date)s to %(end_date)s, not on %(date)s.",
    ...
    def action_view_sale_order_ids(self):
        ...
        return {
            ...
            "views": [
                (self.env.ref("l10n_it_edi_doi.view_quotation_tree").id, "list"),
                (False, "form"),
            ],
            "search_view_id": [
                self.env.ref("sale.sale_order_view_search_inherit_quotation").id
            ],
            "context": {
    ...
    def action_view_invoice_ids(self):
        ...
        return {
            ...
            "views": [
                (self.env.ref("l10n_it_edi_doi.view_move_tree").id, "list"),
                (False, "form"),
            ],
