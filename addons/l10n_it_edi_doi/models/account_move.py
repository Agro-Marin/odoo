...
class AccountMove(models.Model):
    ...
    def _compute_l10n_it_edi_doi_date(self):
        for move in self:
            move.l10n_it_edi_doi_date = move.invoice_date or fields.Date.context_today(
                self
            )
    ...
    def _compute_l10n_it_edi_doi_use(self):
        ...
        for move in self:
            move.l10n_it_edi_doi_use = move.l10n_it_edi_doi_id or (
                move.country_code == "IT" and move.move_type in sale_types
            )
    ...
    def _compute_l10n_it_edi_doi_warning(self):
        for move in self:
            ...
            if (
                move.state != "posted"
            ):  # exactly the 'posted' invoices are included in declaration.invoiced
                ...
                for invoice_line in move.invoice_line_ids:
                    for sale_line in invoice_line.sale_line_ids:
                        ...
                        sale_line_id = sale_line.ids[
                            0
                        ]  # do not just use `id` in case of NewId
                        additional_invoiced_qty[sale_line_id] = (
                            additional_invoiced_qty.get(sale_line_id, 0) + qty_invoiced
                        )
                for order in linked_orders:
                    not_yet_invoiced = order.l10n_it_edi_doi_not_yet_invoiced
                    not_yet_invoiced_after_posting = (
                        order._l10n_it_edi_doi_get_amount_not_yet_invoiced(
                            declaration,
                            additional_invoiced_qty=additional_invoiced_qty,
                        )
                    )
                    declaration_not_yet_invoiced -= (
                        not_yet_invoiced - not_yet_invoiced_after_posting
                    )
