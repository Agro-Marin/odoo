...
class TestReorderingRuleProjectPurchase(TransactionCase):
    def test_po_creation_and_reuse_based_on_project(self):
        ...
        self.assertEqual(
            len(second_po),
            1,
            "A new purchase order should be created as the first one has a project set",
        )
        self.assertFalse(
            second_po.project_id,
            "The new purchase order should have no project since the procurement has none",
        )
        ...
        self.assertFalse(
            extra_po,
            "No new purchase order should be created since the second one matches (no project)",
        )
