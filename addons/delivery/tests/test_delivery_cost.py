...
class TestDeliveryCost(DeliveryCommon, SaleCommon):
    ...
    def test_00_delivery_cost(self):
        ...
        self.a_sale = self.env["account.account"].create(
            {
                ...
                "account_type": "income",
                "tag_ids": [
                    Command.set(self.env.ref("account.account_tag_operating").ids)
                ],
            }
        )
        ...
        line = self.sale_normal_delivery_charges.line_ids.filtered_domain(
        ...
        self.assertEqual(
            ...
            0,
            "Delivery cost does not correspond to 10.0. %s %s"
            % (line.price_subtotal, zin),
        )
        ...
        line = self.delivery_sale_order_cost.line_ids.filtered_domain(
    ...
    def test_01_delivery_cost_from_pricelist(self):
        ...
        line = sale_pricelist_based_delivery_charges.line_ids.filtered_domain(
            [("product_id", "=", self.normal_delivery.product_id.id)]
        )
        self.assertEqual(len(line), 1, "Delivery cost hasn't been added to SO")
        self.assertEqual(
            line.price_subtotal, 5.0, "Delivery cost does not correspond to 5.0"
        )
    ...
    def test_01_taxes_on_delivery_cost(self):
        ...
        self.assertRecordValues(
            sale_order.line_ids, [{"price_subtotal": 9.09, "price_total": 10.45}]
        )
        ...
        line = sale_order.line_ids.filtered_domain(
            [
                ("product_id", "=", self.normal_delivery.product_id.id),
                ("is_delivery", "=", True),
            ]
    ...
    def test_get_invalid_delivery_weight_lines(self):
        ...
        combos = self.env["product.combo"].create(
            [
                ...
                },
                {
                    "name": "Combo B",
                    "combo_item_ids": [
                        Command.create({"product_id": weightless_product.id})
                    ],
                },
            ]
        )
        ...
        order.line_ids = [
            *[
                ...
                for product, combo in zip(
                    self.product + weightless_product, combos, strict=True
                )
            ],
            Command.create({"product_id": weightless_product.id, "product_qty": 0}),
        ...
        error_lines = order.line_ids._get_invalid_delivery_weight_lines()
        self.assertIn(
            weightless_product,
        ...
        self.assertEqual(
            len(error_lines), 1, "Only 1 line should have an invalid weight"
        )
        self.assertTrue(
            error_lines.combo_item_id, "The erroneous line should be part of a combo"
        )
    ...
    def test_fixed_price_margins(self):
        ...
        line = sale_order.line_ids.filtered("is_delivery")
    ...
    def test_delivery_product_taxes_on_branch(self):
        ...
        delivery_line = sale_order.line_ids.filtered(lambda l: l.is_delivery)
