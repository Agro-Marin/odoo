...
class ResPartner(models.Model):
    ...
    def _iap_replace_location_codes(self, iap_data):
        country_code, country_name = (
            iap_data.pop("country_code", False),
            iap_data.pop("country_name", False),
        )
        state_code, state_name = (
            iap_data.pop("state_code", False),
            iap_data.pop("state_name", False),
        )
            ...
            if (
                installed_lang := (
                    self.env["res.lang"].search(
                        [("code", "=", lang), ("iso_code", "=", lang)]
                    )  # specific lang (e.g.: fr_BE)
                    or self.env["res.lang"].search(
                        [("code", "ilike", lang[:2]), ("iso_code", "ilike", lang[:2])],
                        limit=1,
    ...
    def autocomplete_by_name(self, query, query_country_id, timeout=15):
        if (
            query_country_id is False
        ):  # If it's 0, we purposely do not want to filter on the country
        ...
        if response and not response.get("error"):
            return [
                self._format_data_company(suggestion)
        ...
        if response and not response.get("error"):
            ...
            if vies_result:
                ...
                if vies_result["valid"] and name != "---":
                    ...
                    street = address[0]
                    zip_city_record = next(
                        filter(lambda addr: re.match(r"^\d.*", addr), address[1:]), None
                    )
                    zip_city = (
                        zip_city_record.split(" ", 1)
                        if zip_city_record
                        else [None, None]
                    )
                    street2 = next(
                        (
                            addr
                            for addr in filter(
                                lambda addr: addr != zip_city_record, address[1:]
                            )
                        ),
                        None,
                    )
                    return [
                        self._iap_replace_location_codes(
                            {
                                "name": name,
                                "vat": vat,
                                "street": street,
    ...
    def enrich_company_message_post(self, data):
        ...
        template = self.env.ref(
            "iap_mail.enrich_company_by_dnb", raise_if_not_found=False
        )
        if not template:
            return
        company = {
