...
class PosSelfAdyenController(PosAdyenController):
    def _process_payment_response(self, data, adyen_pm_sudo):
        ...
        order_sudo = (
            ...
            .sudo()
            .search([("id", "=", self_order_id)], limit=1)
        )
        if not order_sudo:
            _logger.warning(
                "Received an Adyen event notification for the self order #%d that does not exist (anymore)",
                self_order_id,
            )
        ...
        order = (
            order_sudo.sudo(False)
            .with_user(order_sudo.session_id.config_id.self_ordering_default_user_id)
            .with_company(order_sudo.session_id.config_id.company_id)
        )

        payment_result = data["SaleToPOIResponse"]["PaymentResponse"]["Response"][
            "Result"
        ]

        if (
            payment_result == "Success"
            and order.config_id.self_ordering_mode == "kiosk"
        ):
            payment_amount = data["SaleToPOIResponse"]["PaymentResponse"][
                "PaymentResult"
            ]["AmountsResp"]["AuthorizedAmount"]
            card_type = data["SaleToPOIResponse"]["PaymentResponse"]["PaymentResult"][
