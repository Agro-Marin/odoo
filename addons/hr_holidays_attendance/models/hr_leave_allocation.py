...
class HrLeaveAllocation(models.Model):
    ...
    def default_get(self, fields):
        ...
        if "holiday_status_id" in fields and self.env.context.get("deduct_extra_hours"):
            domain = Domain("overtime_deductible", "=", True) & Domain(
    ...
    def _compute_overtime_deductible(self):
        for allocation in self:
            allocation.overtime_deductible = (
                allocation.holiday_status_id.overtime_deductible
            )
    ...
    def _compute_employee_overtime(self):
        ...
        for allocation in self:
            allocation.employee_overtime = diff_by_employee.get(
                allocation.employee_id, 0
            )
    ...
    def _get_accrual_plan_level_work_entry_prorata(
        ...
        for attendance in attendances:
            total_worked_hours += attendance._get_worked_hours_in_range(
                start_dt, end_dt
            )
