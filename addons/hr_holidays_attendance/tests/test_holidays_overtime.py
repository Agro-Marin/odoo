...
class TestHolidaysOvertime(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user = new_test_user(
            cls.env, login="user", groups="base.group_user", company_id=cls.company.id
        ...
        cls.employee.version_ids.sorted("date_version")[0].date_version = datetime(
            2020, 1, 1
        ).date()

        cls.manager.ruleset_id = cls.ruleset
        cls.manager.version_ids.sorted("date_version")[0].date_version = datetime(
            2020, 1, 1
        ).date()
    ...
    def test_check_overtime(self):
        with self.with_user("user"):
            ...
            self.assertEqual(
                self.employee.total_overtime, 8, "Should have 8 hours of overtime"
            )

            overtime_leave_data = self.leave_type_no_alloc.get_allocation_data(
                self.employee
            )
            self.assertEqual(
    ...
    def test_leave_check_overtime_write(self):
        ...
        self.new_attendance(
            check_in=datetime(2021, 1, 3, 8), check_out=datetime(2021, 1, 3, 16)
        )
    ...
    def test_allocation_check_overtime_write(self):
        ...
        self.assertEqual(
            self.employee.total_overtime, 16, "Should have 16 hours of overtime"
        )
    ...
    def test_leave_check_cancel(self):
        ...
        self.env["hr.holidays.cancel.leave"].with_user(self.user).with_context(
            default_leave_id=leave.id
    ...
    def test_public_leave_overtime_with_timing_rule(self):
        ...
        (
            self.employee.version_ids + self.manager.version_ids
        ...
        self.assertEqual(
            self.manager.total_overtime, 9, "Should have 9 hours of overtime"
        )
    ...
    def test_public_leave_overtime_without_timing_rule(self):
        ...
        self.assertEqual(
            self.manager.total_overtime,
            9,
            "Should have 9 hours of overtime (because of the quantity rule)",
        )
