...
class TestSwissQRCode(AccountTestInvoicingCommon):
    ...
    def test_qr_code_generation_with_newlines(self):
        ...
        vals = self.ch_qr_invoice.partner_bank_id._prepare_qr_code_vals(
            ...
            self.ch_qr_invoice.currency_id,
        ...
        self.assertEqual(
            ...
            0,
            "Each element of the Swiss QR-code must be contained on one line.",
        )
    ...
    def test_ch_qr_code_cross_mask(self):
        for width, height in ((64, 128), (128, 128), (256, 256), (512, 512)):
            barcode = createBarcodeDrawing(
                "QR", value="", format="png", width=width, height=height
            )
            mask_to_apply = self.env[
                "ir.actions.report"
            ]._get_barcode_masks_available()["ch_cross"]
