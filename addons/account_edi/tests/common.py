...
_mocked_needs_web_services_false = _generate_mocked_needs_web_services(False)
...
class AccountEdiTestCommon(AccountTestInvoicingCommon):
    ...
    def with_custom_method(self, method_name, method_content):
        ...
        with patch(
            path,
            new=method_content,
            create=not hasattr(self.env["account.edi.format"], method_name),
        ):
            yield
    ...
    def mock_edi(
        ...
        _needs_web_services_method=_mocked_needs_web_services_false,
        ...
        with (
            ...
            patch(
                "odoo.addons.account_edi.models.account_edi_format.AccountEdiFormat._get_move_applicability",
    ...
    def assert_generated_file_equal(self, invoice, expected_values, applied_xpath=None):
        ...
        invoice.edi_document_ids._process_documents_web_services(
            with_commit=False
        )  # synchronous are called in post, but there's no CRON in tests for asynchronous
    ...
    def create_edi_document(self, edi_format, state, move=None, move_type=None):
        ...
        """
        move = move or self.init_invoice(
            move_type or "out_invoice", products=self.product_a
        )
    ...
    def _process_documents_web_services(self, moves, formats_to_return=None):
        ...
        if formats_to_return is not None:
            documents_to_return = documents_to_return.filtered(
                lambda x: x.edi_format_id.code in formats_to_return
            )
