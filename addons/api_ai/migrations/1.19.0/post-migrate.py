...
def _retire_seed(env, old_xmlid, new_xmlid):
    old = env.ref(f"api_ai.{old_xmlid}", raise_if_not_found=False)
    if not old:
        return
    new = new_xmlid and env.ref(f"api_ai.{new_xmlid}", raise_if_not_found=False)
    moved_providers = env["ai.provider"]
    if new:
        moved_providers = env["ai.provider"].search([("default_model_id", "=", old.id)])
        moved_providers.write({"default_model_id": new.id})
        _carry_hops(env, old, new)
    if env["ai.provider"].search_count([("default_model_id", "=", old.id)]):
        _logger.info(
            "api_ai 19.0.1.19.0: %s is still a provider's default and has no "
    ...
    old.active = False
    _logger.info(
        "api_ai 19.0.1.19.0: archived %s; %s provider(s) moved to %s",
        old.code,
        len(moved_providers),
