...
def _retire_experimental_gemini(env):
    experimental = env.ref(
        "api_ai.ai_model_gemini_2_0_flash_exp", raise_if_not_found=False
    )
    current = env.ref("api_ai.ai_model_gemini_3_5_flash_lite", raise_if_not_found=False)
    if not experimental or not current:
        return
    still_default = env["ai.provider"].search(
        [("default_model_id", "=", experimental.id)]
    )
    still_default.write({"default_model_id": current.id})
    if not env["ai.provider"].search_count(
        [("default_model_id", "=", experimental.id)]
    ):
        experimental.active = False
    _logger.info(
        "api_ai 19.0.1.18.0: %s provider(s) moved from gemini-2.0-flash-exp, an "
...
def _carry_fallback_relation(cr):
    ...
    # ai_model_fallback the old table would only be a stale second chain.
    cr.execute(
        """
