...
class TestBarcodeNomenclature(common.TransactionCase):
    ...
    def test_barcode_nomenclature_parse_barcode_ean8_01(self):
        ...
        self.assertEqual(
            res["type"], "error", "Must fail because the barcode is too short"
        )
        ...
        self.assertEqual(
            res["type"], "error", "Must fail because the checksum digit is wrong"
        )
        self.assertEqual(res["encoding"], "")
    ...
    def test_barcode_nomenclature_parse_barcode_ean8_03_value(self):
        ...
        self.assertEqual(
            res["value"], 12345670.0, "The barcode must be converted into value"
        )
        ...
        self.assertEqual(
            res["value"], 2003405.0, "The barcode must be converted into value"
        )
