...
from odoo.addons.website_sale.tests.test_website_sale_pricelist import (
    TestWebsitePriceList,
)


class TestWebsiteAllPerformance(
    TestWebsitePerformanceCommon, TestWebsitePriceList, WebsiteSaleCommon
):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.product_images = (
            cls.env["product.image"]
            .with_context(default_product_tmpl_id=cls.productC.product_tmpl_id.id)
            .create(
                [
                    {
                        "name": "Template image",
                        ...
                        Command.create(
                            {
                                "attribute_id": cls.product_attribute.id,
                                "value_ids": [
                                    Command.set(cls.product_attribute.value_ids.ids)
                                ],
                            }
                        ),
                    ],
    ...
    def test_website_user_id_public_user(self):
        ...
        def _serve_page():
            ...
            self.assertTrue(
                request.env["website"].search([]).user_id._is_public(),
                "The public user of the website is not public!",
            )
            self.assertTrue(
                request.env.user._is_public(), "The visitor should not be logged"
            )
            return origin_serve_page()

        with patch(
            "odoo.addons.website.models.ir_http.IrHttp._serve_page", wraps=_serve_page
        ) as mocked:
    ...
    def _get_queries_shop(self):
        ...
        self.assertIn(
            f'<img src="/web/image/product.template/{self.productA.product_tmpl_id.id}/',
            html,
        )
        self.assertIn(
            f'<img src="/web/image/product.image/{self.product_images.ids[1]}/', html
        )
