...
class TestUBLCommon(AccountTestInvoicingCommon):
    ...
    def assert_same_invoice(self, invoice1, invoice2, **invoice_kwargs):
        ...
        invoice_line_kwargs_list = invoice_kwargs.get(
            "invoice_line_ids", default_invoice_line_kwargs_list
        )
        self.assertRecordValues(
            invoice2.invoice_line_ids,
            [
                {
    ...
    def _assert_imported_invoice_from_file(
        self, subfolder, filename, invoice_vals, move_type="in_invoice"
    ):
        ...
        invoice = self.env["account.move"].create(
            {"move_type": move_type, "journal_id": journal.id}
        )
        ...
        account_move = self.env["account.move"].create(
            {
                ...
                **invoice_kwargs,
                "invoice_line_ids": [
                    ...
                    for i, invoice_line_kwargs in enumerate(
                        invoice_kwargs.get("invoice_line_ids", [])
                    )
                ],
            }
        )
    ...
    def _assert_invoice_attachment(self, attachment, xpaths, expected_file_path):
        ...
        modified_etree = self.with_applied_xpath(expected_etree, xpaths)
    ...
    def _test_import_in_journal(self, attachment):
        ...
        journal3.default_account_id = self.company_data[
        ...
        new_invoice2 = (
            ...
            .with_context(default_journal_id=journal3.id)
        ...
        self.assertEqual(
            new_invoice3.journal_id, self.company_data["default_journal_sale"]
        )
