...
class TestEdiTbaiXmls(TestEsEdiTbaiCommon):
    ...
    def create_total_refund(self):
        move_reversal = (
            ...
            .with_context(active_model="account.move", active_ids=self.out_invoice.ids)
            .create(
                {
                    "date": "2020-02-01",
                    "reason": "no reason",
    ...
    def test_xml_tree_post_refund(self):
        ...
        with freeze_time(self.frozen_today):
            ...
            xml_expected = etree.fromstring(
                super()._get_sample_xml("xml_post_refund.xml")
            )
            self.assertXmlTreeEqual(xml_doc, xml_expected)
    ...
    def test_xml_tree_post_multicurrency(self):
        ...
        with freeze_time(self.frozen_today):
            ...
            xml_expected_base = etree.fromstring(
                super()._get_sample_xml("xml_post.xml")
            )
            xpath = """
    ...
    def test_xml_tree_post_retention(self):
        self.out_invoice.invoice_line_ids.tax_ids = [
            (4, self._get_tax_by_xml_id("s_irpf15").id)
        ]
        with freeze_time(self.frozen_today):
            edi_document = self.out_invoice._l10n_es_tbai_create_edi_document(
                cancel=False
    ...
    def test_xml_tree_post_multitax(self):
        self.out_invoice.invoice_line_ids.tax_ids = [
            self._get_tax_by_xml_id("s_req52").id,
                    ...
                    "invoice_line_ids": [
                        (
                            ...
                            0,
                            {
                                ...
                                "discount": 20.0,
                                "tax_ids": [
                                    (6, 0, self._get_tax_by_xml_id("p_iva21_bc").ids)
                                ],
                            },
                        )
                    ],
    ...
    def test_xml_tree_in_140_post(self):
        ...
        with freeze_time(self.frozen_today):
            ...
            xml_doc = edi_document._get_xml()
            xml_expected = etree.fromstring(
                super()._get_sample_xml("xml_post_in_140.xml")
            )
            self.assertXmlTreeEqual(xml_doc, xml_expected)
    ...
    def test_xml_tree_no_deducible_tax(self):
        ...
        with freeze_time(self.frozen_today):
                    ...
                    "invoice_line_ids": [
                        (
                            ...
                            0,
                            {
                                ...
                                "quantity": 1,
                                "tax_ids": [
                                    (6, 0, self._get_tax_by_xml_id("p_iva10_nd").ids)
                                ],
                            },
                        )
                    ],
            ...
            xml_doc = edi_document._get_xml()
            xml_expected = etree.fromstring(
                super()._get_sample_xml("xml_post_in_nd.xml")
            )
            self.assertXmlTreeEqual(xml_doc, xml_expected)
    ...
    def test_xml_tree_in_ic_post(self):
        ...
        with freeze_time(self.frozen_today):
            ...
            xml_doc = edi_document._get_xml()
            xml_expected = etree.fromstring(
                super()._get_sample_xml("xml_post_in_ic.xml")
            )
            self.assertXmlTreeEqual(xml_doc, xml_expected)
...
<ds:SignatureValue xmlns:ds="http://www.w3.org/2000/09/xmldsig#">TEXT</ds:SignatureValue>
    ...
    def test_xml_tree_fecha_operacion(self):
        ...
        with freeze_time(self.frozen_today):
            ...
            xml_doc.remove(xml_doc.find("Signature", namespaces=NS_MAP))
            xml_expected = etree.fromstring(
                super()._get_sample_xml("xml_fecha_operacion.xml")
            )
            self.assertXmlTreeEqual(xml_doc, xml_expected)
