...
class TestTbaiUserErrors(TestEsEdiTbaiCommonGipuzkoa):
    ...
    def test_no_certificate(self):
        ...
        self.assertEqual(
            str(e.exception),
            self.tbai_error_msg + "Please configure the certificate for TicketBAI.",
        )
    ...
    def test_pending_invoice(self):
        ...
        try:
            ...
            self.assertEqual(
                str(e),
                self.tbai_error_msg
                + f"TicketBAI: Cannot post invoice while chain head ({first_invoice.name}) has not been posted",
            )
            self.assertEqual(second_invoice.l10n_es_tbai_state, "to_send")
            self.assertFalse(second_invoice.l10n_es_tbai_chain_index)
            self.assertEqual(
                second_invoice.l10n_es_tbai_post_document_id.state, "to_send"
            )
        ...
        self.assertGreater(
            second_invoice.l10n_es_tbai_chain_index,
            first_invoice.l10n_es_tbai_chain_index,
        )
    ...
    def test_post_tbai_credit_note_before_reversed_invoice(self):
        ...
        invoice_already_sent = self.create_invoice(
            invoice_line_ids=[
                {
                    "quantity": 5,
                    "discount": 20.0,
        ...
        invoice_already_sent_wizard = self._get_invoice_send_wizard(
        ...
        move_reversal = (
            ...
            .with_context(
                active_model="account.move", active_ids=self.invoice_to_send.ids
            )
            .create(
                {
