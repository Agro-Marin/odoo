...
class TestEsEdiTbaiCommon(TestAccountMoveSendCommon):
        ...
        cls.certificate = cls.env["certificate.certificate"].create(
            {
                ...
                "content": base64.b64encode(
                    file_open(
                        "l10n_es_edi_tbai/demo/certificates/" + cert_name, "rb"
                    ).read()
                ),
    ...
    def _get_tax_by_xml_id(cls, trailing_xml_id):
        ...
        """
        return cls.env.ref(
            f"account.{cls.env.company.id}_account_tax_template_{trailing_xml_id}"
        )
                ...
                "invoice_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "discount": 20.0,
                            "tax_ids": [
                                (6, 0, cls._get_tax_by_xml_id("p_iva21_bc").ids)
                            ],
                        },
                    )
                ],
...
class TestEsEdiTbaiCommonGipuzkoa(TestEsEdiTbaiCommon):
    ...
    def setUpClass(cls):
        ...
        cls.mock_response_failure = create_mock_response(
            cls._get_response_xml("post_or_cancel_invoice_failure_gi.xml")
        )
        cls.mock_request_error = requests.exceptions.RequestException(
            "A request exception"
        )
...
class TestEsEdiTbaiCommonBizkaia(TestEsEdiTbaiCommon):
    ...
    def setUpClass(cls):
        ...
        cls.mock_response_post_bill_failure = create_mock_response(
            None, cls.RESPONSE_HEADERS_FAILURE
        )
