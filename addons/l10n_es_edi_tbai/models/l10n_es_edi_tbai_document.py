...
class L10n_Es_Edi_TbaiDocument(models.Model):
    ...
    def _check_can_post(self, values):
        ...
        if values["is_sale"] and not self.is_cancel:
            if any(not base_line["tax_ids"] for base_line in values["base_lines"]):
                return self.env._(
                    "There should be at least one tax set on each line in order to send to TicketBAI."
                )
            ...
            if values["is_refund"]:
                ...
                if not refunded_doc or refunded_doc.state == "to_send":
                    ...
                    if not refunded_doc and refunded_doc_invoice_date:
                        ...
                        invoice_sent_before_original = self.search(
    ...
    def _post_to_web_service(self, values):
        ...
        if not self.chain_index and not self.is_cancel and values["is_sale"]:
            ...
            self.sudo().chain_index = (
                self.company_id._get_l10n_es_tbai_next_chain_index()
            )
                ...
                "eus-bizkaia-n3-data": json.dumps(
                    {
                        "con": "LROE",
                        "apa": "2.1"
                        if freelancer and not is_sale
                        else "1.1"
                        if is_sale
                        else "2",
                        "inte": {
                            "nif": company.vat.removeprefix("ES"),
                            "nrs": company.name,
                        },
                        "drs": {
    ...
    def _generate_final_xml_bi(self, freelancer=False):
        ...
        lroe_values = {
            ...
            "sender": sender,
            "sender_vat": sender.vat.removeprefix("ES"),
            "fiscal_year": str(self.date.year),
            "freelancer": freelancer,
            "epigrafe": self.env["ir.config_parameter"]
            .sudo()
            .get_param("l10n_es_edi_tbai.epigrafe", ""),
        }
        lroe_values.update(
            {"tbai_b64_list": [base64.b64encode(self.xml_attachment_id.raw).decode()]}
        )
    ...
    def _process_post_response_xml_bi(self, env, response_xml):
        ...
        # Get message in basque if env is in basque
        error_msg_node_name = "DescripcionErrorRegistro" + (
            "EU" if get_lang(env).code == "eu_ES" else "ES"
        )
        error_msg = (
            error_code + ": " + response_xml.findtext(f".//{error_msg_node_name}", "")
        )
        ...
        values.update(
            {
                ...
                **(
                    self._get_recipient_values(
                        values["partner"], values["is_simplified"]
                    )
                    if (values["partner"] and not self.is_cancel)
                    or not values["is_sale"]
                    else {}
                ),
    ...
    def _get_recipient_values(self, partner, is_simplified=False):
        ...
        if not partner._l10n_es_is_foreign() and partner.vat:
            recipient_values["nif"] = partner.vat.removeprefix("ES")
            ...
            recipient_values["alt_id_country"] = (
    ...
    def _get_sale_values(self, values):
        ...
        if (
            values["partner"]
            and values["partner"].country_id.code == "ES"
            and values["partner"].state_id.code in ("TF", "GC", "CE", "ME")
        ):
        ...
        if (
            not values["partner"]
            or not values["partner"]._l10n_es_is_foreign()
            or values["is_simplified"]
        ):
            sale_values.update(
                **self._get_importe_desglose_es_partner(
                    values["base_lines"], values["is_refund"]
                )
            )
        else:
            sale_values.update(
                **self._get_importe_desglose_foreign_partner(
                    values["base_lines"], values["is_refund"]
                )
            )
    ...
    def _add_base_lines_tax_amounts(self, base_lines, company, tax_lines=None):
        ...
        for base_line in base_lines:
            ...
            price_subtotal = base_line["price_subtotal"] = base_line["tax_details"][
                "raw_total_excluded"
            ]
            ...
            base_line["discount_amount"] = (
                gross_price_subtotal_before_discount - price_subtotal
            )
            base_line["description"] = re.sub(
                r"[^0-9a-zA-Z ]+",
                "",
                base_line["name"] or base_line["product_id"].display_name or "",
            )[:250]

            if quantity:
                base_line["gross_price_unit"] = (
                    gross_price_subtotal_before_discount / quantity
                )
            else:
    ...
    def _prepare_tax_details_info(self, values_list):
        ...
        for values in values_list:
            ...
            if l10n_es_type in ("sujeto", "sujeto_isp"):
                ...
                sujeta_no_sujeta.setdefault("Sujeta", {}).setdefault(
                    "NoExenta", {}
                ).setdefault("DesgloseIVA", {"DetalleIVA": []})["DetalleIVA"].append(
                    tax_info
                )
                ...
                detalle_iva = (
                    sujeta_no_sujeta.get("Sujeta", {})
                    .get("NoExenta", {})
                    .get("DesgloseIVA", {})
                    .get("DetalleIVA")
                )
                if detalle_iva:
                    detalle_iva[-1]["CuotaRecargoEquivalencia"] = sign * float_round(
                        values["tax_amount"], 2
                    )
                    detalle_iva[-1]["TipoRecargoEquivalencia"] = (
                        sign * grouping_key["applied_tax_amount"]
                    )
                ...
                no_sujeta["ImportePorArticulos7_14_Otros"] += sign * float_round(
                    values["base_amount"], 2
                )
            elif l10n_es_type == "no_sujeto_loc":
                no_sujeta = sujeta_no_sujeta.setdefault("NoSujeta", {})
                no_sujeta.setdefault("ImporteTAIReglasLocalizacion", 0.0)
                no_sujeta["ImporteTAIReglasLocalizacion"] += sign * float_round(
                    values["base_amount"], 2
                )

        if (
            "sujeto" in encountered_l10n_es_type
            and "sujeto_isp" not in encountered_l10n_es_type
        ):
            sujeta_no_sujeta["Sujeta"]["NoExenta"]["TipoNoExenta"] = "S2"
        elif (
            "sujeto" not in encountered_l10n_es_type
            and "sujeto_isp" in encountered_l10n_es_type
        ):
    ...
    def _get_importe_desglose_es_partner(self, base_lines, is_refund):
        ...
        def tax_details_info_grouping_function(base_line, tax_data):
            ...
            return {
                ...
                "l10n_es_exempt_reason": tax.l10n_es_exempt_reason
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, tax_details_info_grouping_function
        )
        values_per_grouping_key = AccountTax._aggregate_base_lines_aggregated_values(
            base_lines_aggregated_values
        )

        tax_details_info = self._prepare_tax_details_info(
            values_per_grouping_key.values()
        )
        invoice_info = {
            "DesgloseFactura": {
        ...
        for values in values_per_grouping_key.values():
            if (
                values["grouping_key"]
                and values["grouping_key"]["l10n_es_type"] == "retencion"
            ):
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, totals_grouping_function
        )
        values_per_grouping_key = AccountTax._aggregate_base_lines_aggregated_values(
            base_lines_aggregated_values
        )
    ...
    def _get_importe_desglose_foreign_partner(self, base_lines, is_refund):
        ...
        invoice_info = {}
        for scope, target_key in (
            ("service", "PrestacionServicios"),
            ("consu", "Entrega"),
        ):
            service_values_list = [
                values
                ...
                and values["grouping_key"]["tax_scope"] == scope
            ]
    ...
    def _generate_sale_document_xml(self, values):
        template_name = "l10n_es_edi_tbai.template_invoice_main" + (
            "_cancel" if self.is_cancel else "_post"
        )
    ...
    def _sign_sale_document(self, xml_root):
        ...
        values = {
            "dsig": {
                ...
                "x509_certificate": base64.encodebytes(
                    base64.b64decode(certificate_sudo._get_der_certificate_bytes())
                ).decode(),
        ...
        xml_sig_str = self.env["ir.qweb"]._render(
            "l10n_es_edi_tbai.template_digital_signature", values
        )
        ...
        update_reference_digests(
            xml_sig.find("SignedInfo", namespaces=NS_MAP), algorithm="sha256"
        )

        # Sign (writes into SignatureValue)
        signed_info_xml = xml_sig.find("SignedInfo", namespaces=NS_MAP)
        xml_sig.find("SignatureValue", namespaces=NS_MAP).text = certificate_sudo._sign(
            canonicalize_signed_info(signed_info_xml)
        ).decode()
    ...
    def _get_tbai_sequence_and_number_purchase(self):
        ...
        self.check_singleton()
        original_vendor_bill = self.env["account.move"].search(
            [("l10n_es_tbai_post_document_id", "=", self.id)], limit=1
        )
        if (
            original_vendor_bill and self.is_cancel
        ):  # Normally it should be is_cancel in this case
            vals = (
                original_vendor_bill.l10n_es_tbai_post_document_id._get_values_from_xml(
                    {
    ...
    def _get_tbai_seq_from_name(self, name):
        ...
        seq_length = self.env["mixin.sequence"]._get_sequence_format_param(name)[1][
            "seq_length"
        ]
    ...
    def _get_tbai_id(self):
        ...
        tbai_id_no_crc = "-".join(
            [
                "TBAI",
                str(company.vat.removeprefix("ES")),
        ...
        tbai_qr_no_crc = (
            ...
            + "&".join(
                [
                    ...
                    "nf=" + number,
                    "i="
                    + self._get_values_from_xml({"importe": ".//ImporteTotalFactura"})[
                        "importe"
                    ],
                ]
            )
