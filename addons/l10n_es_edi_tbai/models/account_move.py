...
class AccountMove(models.Model):
    ...
    @api.depends(
        "l10n_es_tbai_post_document_id.state", "l10n_es_tbai_cancel_document_id.state"
    )
    def _compute_l10n_es_tbai_state(self):
        for move in self:
            state = "to_send" if move.l10n_es_tbai_is_required else None
            if (
                move.l10n_es_tbai_post_document_id
                and move.l10n_es_tbai_post_document_id.state == "accepted"
            ):
                state = "sent"
            if (
                move.l10n_es_tbai_cancel_document_id
                and move.l10n_es_tbai_cancel_document_id.state == "accepted"
            ):
                state = "cancelled"
    ...
    def _compute_l10n_es_tbai_is_required(self):
        for move in self:
            move.l10n_es_tbai_is_required = (
                ...
                and any(
                    not line._l10n_es_tbai_is_ignored()
    ...
    def _l10n_es_tbai_unlink_except_in_chain(self):
        ...
        if not self.env.context.get("force_delete") and any(
            m.l10n_es_tbai_chain_index for m in self
        ):
            raise UserError(
                _("You cannot delete a move that has a TicketBAI chain id.")
            )
    ...
    def _l10n_es_tbai_create_edi_document(self, cancel=False):
        ...
        if self.is_purchase_document():
            name = self.ref
        return (
            self.env["l10n_es_edi_tbai.document"]
            .sudo()
            .create(
                {
                    "name": name,
    ...
    def _l10n_es_tbai_post_document_in_chatter(self, message, cancel=False):
        ...
        self.message_post(
            ...
            attachment_ids=[self.l10n_es_tbai_post_document_id.xml_attachment_id.id]
            if not cancel
            else [self.l10n_es_tbai_cancel_document_id.xml_attachment_id.id],
        )
    ...
    def l10n_es_tbai_resend_bill(self):
        ...
        if error := self.with_context(batuz_correction=True)._l10n_es_tbai_post():
            error = (
                ...
                + "\n\n"
                + _(
                    "Be careful if you modified this vendor bill, "
                    "because the official version is still the previous one sent. "
                )
            )
            raise UserError(
                error
            )  # This way, we rollback when rejected and the old accepted document is kept
    ...
    def l10n_es_tbai_cancel(self):
        for invoice in self:
            ...
            if (
                invoice.l10n_es_tbai_cancel_document_id
                and invoice.l10n_es_tbai_cancel_document_id.state == "rejected"
            ):
                invoice.l10n_es_tbai_cancel_document_id.sudo().unlink()

            if not invoice.l10n_es_tbai_cancel_document_id:
                invoice.l10n_es_tbai_cancel_document_id = (
                    invoice._l10n_es_tbai_create_edi_document(cancel=True)
                )

            edi_document = invoice.l10n_es_tbai_cancel_document_id

            error = edi_document._post_to_web_service(
                invoice._l10n_es_tbai_get_values(cancel=True)
            )
            if error:
                raise UserError(error)
            ...
            if edi_document.state == "accepted":
                ...
                invoice._l10n_es_tbai_post_document_in_chatter(
                    edi_document.response_message, cancel=True
                )
    ...
    def _l10n_es_tbai_get_values(self, cancel=False):
        values = {
            ...
            if self.delivery_date != fields.Datetime.today()
            else None,
    ...
    def _l10n_es_tbai_get_invoice_values(self, cancel=False):
        ...
        return {
            **self._l10n_es_tbai_get_credit_note_values(),
            ...
            "taxes": taxes,
            "rate": abs(self.amount_total / self.amount_total_signed)
    ...
    def _l10n_es_tbai_get_credit_note_values(self):
        return {
            ...
            "refunded_doc_invoice_date": self.reversed_entry_id.invoice_date
            if self.reversed_entry_id
            else False,
            "refunded_name": self.reversed_entry_id.name
            if self.reversed_entry_id
            else False,
        }
    ...
    def _l10n_es_tbai_get_vendor_bill_values_batuz(self):
        ...
        reagyp = self.invoice_line_ids.tax_ids.filtered(
        ...
        if values["is_refund"]:
            ...
            values["credit_note_invoices"] = (
                self.reversed_entry_id | self.l10n_es_tbai_reversed_ids
            )
        if reagyp:
    ...
    def _l10n_es_tbai_get_vendor_bill_tax_values(self):
        ...
        for line in self.line_ids.filtered(
            ...
            if (
                any(t.l10n_es_type == "ignore" for t in line.tax_ids)
                or line.tax_line_id.l10n_es_type == "ignore"
            ):
                continue
            if line.tax_line_id.l10n_es_type != "retencion":
                amount_total += line.balance
            for tax in line.tax_ids.filtered(
                lambda t: t.l10n_es_type not in ("recargo", "retencion")
            ):
        ...
        for tax in results:
            ...
            iva_values.append(
                {
                    "base": results[tax]["base_amount"],
                    "code": code,
                    "tax": results[tax]["tax_amount"],
                    "rec": tax,
                }
            )
        return {"iva_values": iva_values, "amount_total": amount_total}
