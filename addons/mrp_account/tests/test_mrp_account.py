...
class TestMrpAccount(TestBomPriceCommon):
    ...
    def test_stock_user_without_account_permissions_can_create_bom(self):
        mrp_manager = new_test_user(
            self.env,
            "temp_mrp_manager",
            "mrp.group_mrp_manager,product.group_product_variant",
        )
    ...
    def test_unbuild_account_00(self):
        ...
        productA_credit_line = self.env["account.move.line"].search(
            [("product_id", "=", self.dining_table.id), ("debit", "=", 0)]
        )
        ...
        productB_credit_line = self.env["account.move.line"].search(
            [("product_id", "=", self.glass.id), ("debit", "=", 0)]
        )
...
class TestMrpAccountWorkorder(TestBomPriceOperationCommon):
    ...
    def test_labor_cost_posting_is_not_rounded_incorrectly(self):
        ...
        self.assertEqual(
            production.workorder_ids.mapped("time_ids")
    ...
    def test_wip_accounting_00(self):
        ...
        self.assertEqual(
            len(wip_manual_entry1),
            2,
            "Should be 2 journal entries: 1 for the WIP accounting + 1 for its reversal",
        )
        self.assertEqual(
            wip_manual_entry1[0].wip_production_count,
            0,
            "Non-WIP MOs shouldn't be linked to manual entry",
        )
        self.assertEqual(
            len(wip_manual_entry1.line_ids),
            6,
            "Should be 3 lines per journal entry: 1 for 'Component Value', 1 for '(WO) overhead', 1 for WIP",
        )
        self.assertRecordValues(
            wip_manual_entry1.line_ids,
            [
                {
        ...
        self.assertEqual(
            len(wip_empty_entries),
            2,
            "Should be 2 journal entries: 1 for the WIP accounting + 1 for its reversal",
        )
        self.assertEqual(
            wip_empty_entries[0].wip_production_count,
            1,
            "WIP MOs should be linked to entries even if no 'done' work",
        )
        self.assertEqual(
            len(wip_empty_entries.line_ids),
            6,
            "Should be 3 lines per journal entry: 1 for 'Component Value', 1 for '(WO) overhead', 1 for WIP",
        )
        self.assertRecordValues(
            wip_empty_entries.line_ids,
            [
                {
                    "account_id": self.account_stock_valuation.id,
                    "debit": 0.0,
        ...
        wip_entries1 = self.env["account.move"].search(
            [
                ("ref", "ilike", "WIP - " + mo.name),
                ("id", "not in", wip_empty_entries.ids),
            ]
        )
        self.assertEqual(
            len(wip_entries1),
        ...
        self.assertEqual(
            wip_entries1[0].wip_production_count, 1, "WIP MOs should be linked to entry"
        )
        self.assertEqual(
            len(wip_entries1.line_ids),
            6,
            "Should be 3 lines per journal entry: 1 for 'Component Value', 1 for '(WO) overhead', 1 for WIP",
        )
        total_component_price = sum(
            m.product_qty * m.product_id.standard_price for m in mo.move_raw_ids
        )
        self.assertRecordValues(
            wip_entries1.line_ids,
            [
                {
                    "account_id": self.account_stock_valuation.id,
                    "debit": total_component_price,
                    "credit": 0.0,
                },
                {
                    "account_id": self.env.company.account_production_wip_overhead_account_id.id,
        ...
        self.assertEqual(
            len(wip_entries2),
            2,
            "Should be 2 journal entries: 1 for the WIP accounting + 1 for its reversal, for 1 MO",
        )
        self.assertTrue(
            mo_2.name not in wip_entries2[0].ref,
            "Draft MO should be completely disregarded by wizard",
        )
        self.assertEqual(
            wip_entries2[0].wip_production_count,
            1,
            "Only WIP MOs should be linked to entry",
        )
        self.assertEqual(
            len(wip_entries2.line_ids),
            6,
            "Should be 3 lines per journal entry: 1 for 'Component Value', 1 for '(WO) overhead', 1 for WIP",
        )
        self.assertRecordValues(
            wip_entries2.line_ids,
            [
                {
                    "account_id": self.account_stock_valuation.id,
                    "debit": total_component_price,
        ...
        self.assertEqual(
            len(wip_entries3),
            2,
            "Should be 2 journal entries: 1 for the WIP accounting + 1 for its reversal, for both MOs",
        )
        self.assertTrue(
            mo_2.name in wip_entries3[0].ref, "Both MOs should have been considered"
        )
        self.assertEqual(
            wip_entries3[0].wip_production_count,
            2,
            "Both WIP MOs should be linked to entry",
        )
        self.assertEqual(
            len(wip_entries3.line_ids),
            6,
            "Should be 3 lines per journal entry: 1 for 'Component Value', 1 for '(WO) overhead', 1 for WIP",
        )
        total_component_price = sum(
            m.quantity * m.product_id.standard_price for m in mo_2.move_raw_ids
        )
        self.assertRecordValues(
            wip_entries3.line_ids,
            [
                {
                    "account_id": self.account_stock_valuation.id,
                    "debit": total_component_price,
        ...
        self.assertTrue(
            mo_2.name not in wip_entries4[0].ref,
            "Done MO should be completely disregarded by wizard",
        )
        self.assertEqual(
            wip_entries4[0].wip_production_count,
            1,
            "Only WIP MOs should be linked to entry",
        )
        self.assertEqual(
            len(wip_entries4.line_ids),
            6,
            "Should be 3 lines per journal entry: 1 for 'Component Value', 1 for '(WO) overhead', 1 for WIP",
        )
        total_component_price = sum(
        ...
        self.assertEqual(
            wip_entries5[0].wip_production_count, 1, "WIP MOs should be linked to entry"
        )
        self.assertEqual(
            len(wip_entries5.line_ids),
            6,
            "Should be 3 lines per journal entry: 1 for 'Component Value', 1 for '(WO) overhead', 1 for WIP",
        )
        self.assertRecordValues(
            wip_entries5.line_ids,
            [
                {
                    "account_id": self.account_stock_valuation.id,
                    "debit": 0.0,
    ...
    def test_labor_cost_over_consumption(self):
        ...
        mo_aml = self.env["account.move.line"].search(
            [("name", "like", production.name)]
        )
        labour = 600
        ...
        self.assertRecordValues(
            ...
            [
                ...
                {
                    "name": production.name + " - " + self.dining_table.name,
                    "debit": 0.0,
                    "credit": (labour + compo_price) * (1 - cost_share),
                },
                {
                    "name": production.name + " - " + self.dining_table.name,
                    "debit": (labour + compo_price) * (1 - cost_share),
                    "credit": 0.0,
                },
                {
                    "name": production.name + " - " + self.glass.name,
                    "debit": 0.0,
                    "credit": 100.0,
                },
                {
