...
| Withdraw | Approver who approved; through `action_withdraw_approver(approver_id)` also a manager or a pool member of a later step (`_check_withdraw_actor`) | `action_withdraw()` / `action_withdraw_approver()` | approver state=approved (pending or approved request), `_check_withdraw_allowed()` |
