...
| `_get_approval_outcome_value(kind)` | The state a request-side outcome lands the document in, whose verb the decider must hold. Default: the one value mapped to that kind |
| `_access_verbs` (on the adopter) | One verb per value of the state field, `at_create=False`, each with `ir.access` rows and a shipped `approval.binding` in mode `act_decides` |
...
| `_approval_move_decided(verb)` | Called by the `act_decides` obligation once a write has moved documents into `verb`'s state: a document without a request that enters `pending` raises one; one with a request is brought in line by `_sync_approval_request()`, unless its request is already being synced (the `approval.state_sync` admission) |
| `_check_approval_decider_holds(verb)` | The request-side decider holds the verb of the outcome's move: its permission rows and guards, not the operation it requires. Raises the verb's access error |
...
| `_on_approval_progress/approved/refused/cancelled()` | A decision taken on the request itself: checks `_check_approval_sync_policy` and `_check_approval_decider_holds` (skipped for the superuser, and for an engine cancellation, which is not a decision), then applies the outcome admitted for its verb and under the sync admission. Skipped when the document is already in that kind |
...
| `mode` | Selection(advise/block/request/act_decides) | Yes | **Yes** | default="advise". `advise` (labelled Observe) runs the operation and records it; `block` refuses unless an approved request covers the record; `request` raises the approval instead of running — and, with `run_on_approval`, runs the operation exactly once when it is approved, as the person who called it; `act_decides` (The Move Decides) is shipped on each state verb of a `mixin.approval.state.sync` document, refuses nothing, and makes the move the request's decision; it cannot be archived, retargeted, switched to another mode or deleted outside an uninstall |
| `sudo_policy` | Selection(enforce/superuser/bypass) | Yes | **Yes** | default="superuser". Who the gate does NOT apply to. `sudo()` flips `su` and keeps `uid`, so the real superuser and an ordinary user elevated by `sudo()` are separate risks and separate settings. Under every policy a call with `install_module` in the context -- a module's data or demo file, reviewed with the code -- passes and is recorded as an observation |
