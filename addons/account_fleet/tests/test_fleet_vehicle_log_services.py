...
class TestFleetVehicleLogServices(AccountTestInvoicingCommon):
    ...
    def test_service_bill_right_amount(self):
        ...
        self.assertEqual(
            self.car_1.log_services[0].account_move_line_id.move_id, self.bill
        )
        self.assertEqual(
            self.car_1.log_services[0].amount, self.service_line.price_subtotal
        )
        ...
        self.assertEqual(
            self.car_1.log_services[0].amount, self.service_line.price_unit
        )
    ...
    def test_service_bill_deletion(self):
        ...
        self.assertEqual(
            self.car_2.log_services[0].account_move_line_id.move_id, self.bill
        )
        self.assertEqual(
            self.car_2.log_services[0].amount, service_line_2.price_subtotal
        )
    ...
    def test_service_bill_change_vehicle(self):
        ...
        self.assertEqual(
            self.car_2.log_services[0].amount, self.service_line.price_subtotal
        )
    ...
    def test_fleet_log_services_amount(self):
        ...
        fleet_service = self.env["fleet.vehicle.log.services"].search(
            [("vendor_id", "=", partner.id), ("description", "=", False)]
        )
