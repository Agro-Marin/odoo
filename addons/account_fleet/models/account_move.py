...
class AccountMove(models.Model):
    ...
    def _post_entries(self):
        vendor_bill_service = self.env.ref(
        ...
        for line in posted.line_ids:
            if (
                not line.vehicle_id
                or line.vehicle_log_service_ids
...
class AccountMoveLine(models.Model):
    ...
    vehicle_id = fields.Many2one(
        "fleet.vehicle", string="Vehicle", index="btree_not_null"
    )
    ...
    def write(self, vals):
        if "vehicle_id" in vals and not vals["vehicle_id"]:
            self.sudo().vehicle_log_service_ids.with_context(
