...
class EventEvent(models.Model):
    ...
    def _get_booth_stat_count(self):
        elements = (
            self.env["event.booth"]
            .sudo()
            ._read_group(
                [("event_id", "in", self.ids)], ["event_id", "state"], ["__count"]
            )
        )
    ...
    def _compute_event_booth_count(self):
        if self.ids and all(
            bool(event.id) for event in self
        ):  # no new/onchange mode -> optimized
            booths_available_count, booths_total_count = self._get_booth_stat_count()
            for event in self:
                event.event_booth_count_available = booths_available_count.get(
                    event.id, 0
                )
    ...
    def _compute_event_booth_category_ids(self):
        for event in self:
            event.event_booth_category_ids = event.event_booth_ids.mapped(
                "booth_category_id"
            )
