...
class TestL10nArWithholdingArRi(TestAr):
    ...
    def setUpClass(cls):
        ...
        cls.tax_wth_test_1 = cls.env.ref(
            "account.%i_ex_tax_withholding_iibb_caba_applied" % cls.env.company.id
        )
        ...
        cls.tax_wth_test_2 = cls.env.ref(
            "account.%i_ex_tax_withholding_iibb_ba_applied" % cls.env.company.id
        )
        ...
        cls.other_currency = cls.setup_other_currency(
            "USD", rounding=0.001, rates=[("2023-01-01", 0.01), ("2023-05-01", 0.005)]
        )
        cls.tax_wth_earnings_incurred_scale_test_5 = cls.env.ref(
            "account.%i_ex_tax_withholding_profits_regimen_110_insc"
            % cls.env.company.id
        )
        cls.tax_wth_earnings_incurred_test_6 = cls.env.ref(
            "account.%i_ex_tax_withholding_profits_regimen_35_insc" % cls.env.company.id
        )
        ...
        in_third_party_check = self.env["account.payment"].create(
            {
                ...
                "l10n_latam_new_check_ids": [Command.clear()]
                + [
                    Command.create(
                        {
                            "name": 1,
    ...
    def new_payment_register(self, move_ids, taxes):
        wizard = (
            ...
            .create({"payment_date": "2023-01-01"})
        )
        wizard.l10n_ar_withholding_ids = [Command.clear()] + [
            Command.create(
                {"tax_id": x["id"], "base_amount": x["base_amount"], "amount": 0}
            )
            for x in taxes
        ]
    ...
    def test_01_simple_full_payment(self):
        ...
        taxes = [
            {
                "id": self.tax_wth_test_1.id,
                "base_amount": sum(moves.mapped("amount_untaxed")),
            }
        ]
        wizard = self.new_payment_register(moves, taxes)
        self.assertEqual(
            wizard.currency_id.round(
                sum(wizard.l10n_ar_withholding_ids.mapped("amount"))
            )
            + wizard.l10n_ar_net_amount,
            wizard.amount,
        )
        ...
        self.assertEqual(
            1210,
            payment.currency_id.round(
                sum(payment.l10n_ar_withholding_ids.mapped("amount_currency")) * -1
                + payment.amount
            ),
        )
    ...
    def test_02_two_payments_same_invoice(self):
        ...
        taxes = [
            {
                "id": self.tax_wth_test_1.id,
                "base_amount": sum(moves.mapped("amount_untaxed")) * 0.5,
            }
        ]
        ...
        self.assertEqual(
            wizard_1.currency_id.round(
                sum(wizard_1.l10n_ar_withholding_ids.mapped("amount"))
            )
            + wizard_1.l10n_ar_net_amount,
            wizard_1.amount,
        )
        ...
        self.assertEqual(
            wizard_2.currency_id.round(
                sum(wizard_2.l10n_ar_withholding_ids.mapped("amount"))
            )
            + wizard_2.l10n_ar_net_amount,
            wizard_2.amount,
        )
        action = wizard_2.action_create_payments()
        payment_2 = self.env["account.payment"].browse(action["res_id"])
        self.assertRecordValues(
            payment_1.move_id.line_ids.sorted("balance"),
            [
                # Liquidity line:
                {
                    "debit": 0.0,
                    "credit": 555.0,
                    "currency_id": wizard_1.currency_id.id,
                    "amount_currency": -555.0,
                    "reconciled": False,
                },
                # base line:
                {
        ...
        self.assertEqual(
            605,
            payment_1.currency_id.round(
                sum(payment_1.l10n_ar_withholding_ids.mapped("amount_currency")) * -1
                + payment_1.amount
            ),
        )

        self.assertRecordValues(
            payment_2.move_id.line_ids.sorted("balance"),
            [
                # Liquidity line:
                {
                    "debit": 0.0,
                    "credit": 555.0,
                    "currency_id": wizard_2.currency_id.id,
                    "amount_currency": -555.0,
                    "reconciled": False,
                },
                # base line:
        ...
        self.assertEqual(
            605,
            payment_2.currency_id.round(
                sum(payment_2.l10n_ar_withholding_ids.mapped("amount_currency")) * -1
                + payment_2.amount
            ),
        )
    ...
    def test_03_two_withholdings_one_payment(self):
        ...
        taxes = [
            ...
            {
                "id": self.tax_wth_test_2.id,
                "base_amount": sum(moves.mapped("amount_total")),
            },
        ]
        ...
        action = wizard.action_create_payments()
        payment = self.env["account.payment"].browse(action["res_id"])
        line_1 = payment.l10n_ar_withholding_ids.filtered(
            lambda x: x.tax_line_id.id == self.tax_wth_test_1.id
        )
        line_2 = payment.l10n_ar_withholding_ids.filtered(
            lambda x: x.tax_line_id.id == self.tax_wth_test_2.id
        )
    ...
    def test_04_two_withholdings_different_currency(self):
        ...
        taxes = [
            {"id": self.tax_wth_test_1.id, "base_amount": 5},
            {"id": self.tax_wth_test_2.id, "base_amount": 6.05},
        ]
        ...
        self.assertEqual(
            ...
            wizard.currency_id.round(wizard.amount),
        )
        action = wizard.action_create_payments()
    ...
    def test_05_earnings_withholding_applied_with_scale(self):
        ...
        self.tax_wth_earnings_incurred_scale_test_5.l10n_ar_withholding_sequence_id = (
            self.earnings_withholding_sequence
        )
        ...
        taxes = [
            {
                "id": invoice.partner_id.l10n_ar_partner_tax_ids.tax_id.id,
        ...
        taxes = [
            {
                "id": invoice2.partner_id.l10n_ar_partner_tax_ids.tax_id.id,
                "base_amount": invoice2.amount_untaxed,
            }
        ]
    ...
    def test_06_earnings_withholding_applied(self):
        ...
        invoice = self.in_invoice_3_wht()
        self.tax_wth_earnings_incurred_test_6.l10n_ar_withholding_sequence_id = (
            self.earnings_withholding_sequence
        )
        self.env["l10n_ar.partner.tax"].create(
            {
    ...
    def test_08_earnings_withholding_applied_with_scale_and_minimun_withholdable_amount_set(
        self,
    ):
                        ...
                        Command.create(
                            {
                                ...
                                "base_amount": sum(
                                    in_invoice_wht.mapped("amount_untaxed")
                                ),
                                "amount": 0,
                            }
                        )
