...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _post_load_demo_data(self, company=False):
        ...
        if company == self.env.ref("base.company_ri", raise_if_not_found=False):
            profits_wth_tax_119 = self.with_company(company).ref(
                "ex_tax_withholding_profits_regimen_119_insc"
            )
            pba_wth_applied = self.with_company(company).ref(
                "ex_tax_withholding_iibb_ba_applied"
            )
            ...
            earnings_wth_tax_78 = self.with_company(company).ref(
                "ex_tax_withholding_profits_regimen_78_insc"
            )
            self.env["l10n_ar.partner.tax"].create(
                {
                    "tax_id": earnings_wth_tax_78.id,
                    "partner_id": self.env.ref("l10n_ar.res_partner_mipyme").id,
                    "company_id": company.id,
                }
            )
