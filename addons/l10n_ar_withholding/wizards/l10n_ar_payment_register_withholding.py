    ...
    def _tax_compute_all_helper(self):
        ...
        if self.tax_id.l10n_ar_tax_type in ["earnings", "earnings_scale"]:
            ...
            domain_same_period_withholdings = [
                ...
                (
                    "partner_id",
                    "=",
                    self.payment_register_id.partner_id.commercial_partner_id.id,
                ),
                ("date", "<=", to_date),
                ("date", ">=", from_date),
            ]
            if (
                same_period_partner_withholdings := self.env["account.move.line"]
                .sudo()
                ._read_group(
            ...
            if (
                same_period_partner_base := self.env["account.move.line"]
                .sudo()
                ._read_group(domain_same_period_base, ["partner_id"], ["balance:sum"])
            ):
        ...
        if self.tax_id.l10n_ar_tax_type in ["earnings", "earnings_scale"]:
            ...
            if self.tax_id.l10n_ar_tax_type == "earnings_scale":
                ...
                tax_amount = (
                    (net_amount - escala.excess_amount) * escala.percentage / 100
                ) + escala.fixed_amount
    ...
    def _compute_base_amount(self):
        for wth in self:
            if wth.tax_id.l10n_ar_tax_type == "iibb_total":
                ...
                wth.base_amount = (
                    wth.payment_register_id.amount
                    * sum(
                        wth.payment_register_id.line_ids.mapped(
