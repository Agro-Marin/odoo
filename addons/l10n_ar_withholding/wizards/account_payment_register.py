...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _compute_amount(self):
        ...
        for wizard in self:
            checks = (
                wizard.l10n_latam_new_check_ids
                if wizard.filtered(
                    lambda x: x._is_latam_check_payment(check_subtype="new_check")
                )
                else wizard.l10n_latam_move_check_ids
            )
            checks_amount = sum(checks.mapped("amount"))
            if (
                not wizard.currency_id.is_zero(checks_amount)
                and wizard.currency_id.compare_amounts(
                    checks_amount, wizard.l10n_ar_net_amount
                )
                != 0
            ):
                if wizard.partner_type == "supplier":
                    ...
                    for i in range(201):
                        ...
                        if wizard.currency_id.is_zero(f_delta):
                            break
                        der = (
                            ((wizard.l10n_ar_net_amount - f_previous) / d)
                            if abs(d) >= 0.01
                            else 1.0
                        )
    ...
    def _compute_l10n_ar_net_amount(self):
        for rec in self:
            rec.l10n_ar_net_amount = rec.amount - sum(
                rec.l10n_ar_withholding_ids.mapped("amount")
            )
    ...
    def _compute_l10n_ar_withholding_ids(self):
        for wizard in self:
            ...
            partner_taxes = self.env["l10n_ar.partner.tax"].search(
                [
                    *self.env["l10n_ar.partner.tax"]._check_company_domain(
                    ...
                    (
                        "tax_id.l10n_ar_withholding_payment_type",
                        "=",
                        wizard.partner_type,
                    ),
                ]
            ...
            wizard.l10n_ar_withholding_ids = [Command.clear()] + [
                Command.create({"tax_id": x.tax_id.id}) for x in partner_taxes
            ]

    def action_create_payments(self):
        if (
            self.l10n_ar_withholding_ids
