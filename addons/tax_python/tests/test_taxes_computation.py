...
from odoo.addons.tax_python.tests.common import TestTaxCommonTaxPython
from odoo.addons.tax_python.tools.formula_utils import (
    check_formula,
    normalize_formula,
)
...
class TestTaxesComputation(TestTaxCommonTaxPython):
    ...
    def test_ast_transformer_normalizes(self):
        ...
        for formula, expected_normalized, expected_fields in valid_cases:
            with self.subTest(code=formula):
                normalized_formula, accessed_fields = normalize_formula(
                    self.env, formula
                )
