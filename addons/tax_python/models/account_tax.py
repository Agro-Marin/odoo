...
class AccountTax(models.Model):
    ...
    amount_type = fields.Selection(
        ...
        ondelete={
            "code": lambda recs: recs.write({"amount_type": "percent", "active": False})
        },
    )
    formula = fields.Text(
    ...
    def _eval_tax_amount_formula(self, raw_base, evaluation_context):
        ...
        assert accessed_fields["product"] <= formula_context["product"].keys(), (
            "product fields used in formula must be present in the product dict"
        )
        assert accessed_fields["uom"] <= formula_context["uom"].keys(), (
            "uom fields used in formula must be present in the product dict"
        )
        try:
