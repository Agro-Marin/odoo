...
class L10n_Eg_EdiThumbDrive(models.Model):
    ...
    def action_sign_invoices(self, invoice_ids):
        ...
        for invoice_id in invoice_ids:
            ...
            signed_attrs = self._generate_signed_attrs__(
                eta_invoice, invoice_id.l10n_eg_signing_time
            )
    ...
    def set_signature_data(self, invoices):
        ...
        for key, value in invoices.items():
            ...
            eta_invoice_json["request"]["signatures"] = [
                {"signatureType": "I", "value": signature}
            ]
            invoice_id.l10n_eg_eta_json_doc_file = base64.b64encode(
    ...
    def _generate_signed_attrs__(self, eta_invoice, signing_time):
        ...
        data = hashlib.sha256(
            self._serialize_for_signing(eta_invoice).encode()
        ).digest()
                            ...
                            cms.Time(
                                {
                                    "utc_time": core.UTCTime(
                                        signing_time.replace(tzinfo=UTC)
                                    )
                                }
                            ),
                        ),
