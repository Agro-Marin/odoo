...
class AccountMove(models.Model):
    ...
    def _auto_init(self):
        if not column_exists(self.env.cr, "account_move", "l10n_eg_uuid"):
            ...
            create_column(
                self.env.cr, "account_move", "l10n_eg_submission_number", "VARCHAR"
            )
    ...
    def _compute_eta_long_id(self):
        for rec in self:
            response_data = rec.l10n_eg_eta_json_doc_file and json.loads(
                base64.b64decode(rec.l10n_eg_eta_json_doc_file)
            ).get("response")
            if response_data:
    ...
    def _compute_eta_qr_code_str(self):
        for move in self:
            if move.invoice_date and move.l10n_eg_uuid and move.l10n_eg_long_id:
                ...
                qr_code_str = "%s/documents/%s/share/%s" % (
                    base_url,
                    move.l10n_eg_uuid,
                    move.l10n_eg_long_id,
                )
    ...
    def action_post_sign_invoices(self):
        ...
        invoices = self.filtered(
            lambda r: (
                r.country_code == "EG"
                and r.state == "posted"
                and not r.l10n_eg_submission_number
                and r.edi_document_ids.filtered(
        ...
        drive_id = self.env["l10n_eg_edi.thumb.drive"].search(
            [("user_id", "=", self.env.user.id), ("company_id", "=", company_id.id)]
        )
