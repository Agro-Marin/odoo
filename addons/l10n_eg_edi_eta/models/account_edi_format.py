...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_eg_get_eta_qr_domain(self, production_enviroment=False):
        return (
            production_enviroment and ETA_DOMAINS["invoice.production"]
        ) or ETA_DOMAINS["invoice.preproduction"]

    @api.model
    def _l10n_eg_get_eta_api_domain(self, production_enviroment=False):
        return (production_enviroment and ETA_DOMAINS["production"]) or ETA_DOMAINS[
            "preproduction"
        ]
    ...
    def _l10n_eg_get_eta_token_domain(self, production_enviroment=False):
        return (
            production_enviroment and ETA_DOMAINS["token.production"]
        ) or ETA_DOMAINS["token.preproduction"]

    @api.model
    def _l10n_eg_eta_connect_to_server(
        self,
        request_data,
        request_url,
        method,
        is_access_token_req=False,
        production_enviroment=False,
    ):
        api_domain = (
            is_access_token_req
            and self._l10n_eg_get_eta_token_domain(production_enviroment)
        ...
        try:
            ...
            return {"error": str(ex), "blocking_level": "warning"}
        if not request_response.ok:
            try:
                response_data = request_response.json()
            except JSONDecodeError as ex:
                return {"error": str(ex), "blocking_level": "error"}
            if response_data and response_data.get("error"):
                return {"error": response_data.get("error"), "blocking_level": "error"}
    ...
    def _l10n_eg_edi_post_invoice_web_service(self, invoice):
        ...
        request_data = {
            "body": json.dumps(
                {"documents": [invoice_json["request"]]}, ensure_ascii=False, indent=4
            ).encode("utf-8"),
            "header": {
        ...
        if response_data.get("rejectedDocuments", False) and isinstance(
            response_data.get("rejectedDocuments"), list
        ):
            return {
                "error": str(response_data.get("rejectedDocuments")[0].get("error")),
                "blocking_level": "error",
            }
        if response_data.get("submissionId") is not None and response_data.get(
            "acceptedDocuments"
        ):
            invoice_json["response"] = {
                ...
                "l10n_eg_internal_id": response_data["acceptedDocuments"][0].get(
                    "internalId"
                ),
                "l10n_eg_hash_key": response_data["acceptedDocuments"][0].get(
                    "hashKey"
                ),
                "l10n_eg_submission_number": response_data["submissionId"],
            }
            invoice.l10n_eg_eta_json_doc_file = base64.b64encode(
                json.dumps(invoice_json).encode()
            )
    ...
    def _cancel_invoice_edi_eta(self, invoice):
        ...
        if invoice.l10n_eg_submission_number:
            ...
            if document_summary.get("doc_data") and document_summary["doc_data"][0].get(
                "status"
            ) in ("Cancelled", "Rejected"):
                return {"success": True}
        request_url = (
            f"/api/v1/documents/state/{quote(invoice.l10n_eg_uuid, safe='/:')}/state"
        )
        request_data = {
    ...
    def _l10n_eg_get_einvoice_document_summary(self, invoice):
        ...
        document_summary = [
            doc
            for doc in response_data.get("documentSummary", [])
            if doc.get("uuid") == invoice.l10n_eg_uuid
        ]
    ...
    def _l10n_eg_get_einvoice_status(self, invoice):
        ...
        return_dict = {
            "Invalid": {
                "error": _(
                    "This invoice has been marked as invalid by the ETA. Please check the ETA website for more information"
                ),
                "blocking_level": "error",
            },
            "Submitted": {
        ...
        if document_summary.get("doc_data") and return_dict.get(
            document_summary["doc_data"][0].get("status")
        ):
    ...
    def _l10n_eg_eta_get_access_token(self, invoice):
        ...
        request_data = {
            ...
            "header": {"Authorization": f"Basic {user_and_pass}"},
        }
        response_data = self._l10n_eg_eta_connect_to_server(
            request_data,
            request_url,
    ...
    def _l10n_eg_get_eta_invoice_pdf(self, invoice):
        ...
        if access_data.get("error"):
            return access_data
        request_url = (
            f"/api/v1.0/documents/{quote(invoice.l10n_eg_uuid, safe='/:')}/pdf"
        )
        request_data = {
            "body": None,
            "header": {
    ...
    def _l10n_eg_eta_prepare_eta_invoice(self, invoice):
        ...
        base_lines = [
            invoice._prepare_product_base_line_for_taxes_computation(x)
            for x in base_amls
        ]
        tax_amls = invoice.line_ids.filtered("tax_repartition_line_id")
        tax_lines = [
            invoice._prepare_tax_line_for_taxes_computation(x) for x in tax_amls
        ]
        AccountTax._add_tax_details_in_base_lines(base_lines, invoice.company_id)
        AccountTax._round_base_lines_tax_details(
            base_lines, invoice.company_id, tax_lines=tax_lines
        )
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, grouping_function_base_line
        )
        invoice_line_data, totals = self._l10n_eg_eta_prepare_invoice_lines_data(
            invoice, base_lines_aggregated_values
        )
        ...
        base_lines_aggregated_values_total_amount = (
            AccountTax._aggregate_base_lines_tax_details(
                base_lines, grouping_function_total_amount
            )
        )
        values_per_grouping_key_total_amount = (
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, grouping_function_global
        )
        ...
        eta_invoice = {
            ...
            "receiver": self._l10n_eg_eta_prepare_address_data(
                invoice.partner_id, invoice
            ),
            "documentType": "i"
                ...
                "taxTotals": [
                    {
                        ...
                        "amount": self._l10n_eg_edi_round(
                            abs(tax_values["tax_amount"])
                        ),
                    }
                    for grouping_key, tax_values in values_per_grouping_key.items()
                    if grouping_key
                ],
                "totalDiscountAmount": self._l10n_eg_edi_round(
                    totals["discount_total"]
                ),
                "totalSalesAmount": self._l10n_eg_edi_round(
                    totals["total_price_subtotal_before_discount"]
                ),
                "netAmount": self._l10n_eg_edi_round(
                    sum(
                        x["base_amount"]
                        for x in values_per_grouping_key_total_amount.values()
                    )
                ),
                "totalAmount": self._l10n_eg_edi_round(
                    sum(
                        x["base_amount"] + x["tax_amount"]
                        for x in values_per_grouping_key_total_amount.values()
                    )
                ),
    ...
    @api.model
    def _l10n_eg_eta_prepare_invoice_lines_data(
        self, invoice, base_lines_aggregated_values
    ):
        lines = []
        totals = {
            "discount_total": 0.0,
        ...
        for base_line, aggregated_values in base_lines_aggregated_values:
            ...
            discount_amount = self._l10n_eg_edi_round(
                price_subtotal_before_discount - abs(line.balance)
            )
            ...
            lines.append(
                {
                    ...
                    "netTotal": self._l10n_eg_edi_round(
            ...
            if invoice.currency_id != self.env.ref("base.EGP"):
                lines[-1]["unitValue"]["currencyExchangeRate"] = (
                    self._l10n_eg_edi_round(
    ...
    def _l10n_eg_eta_prepare_address_data(self, partner, invoice, issuer=False):
        ...
        if issuer:
            address["address"]["branchID"] = (
                invoice.journal_id.l10n_eg_branch_identifier or ""
            )
    ...
    def _check_move_configuration(self, invoice):
        ...
        if invoice.journal_id.l10n_eg_branch_id.vat == invoice.partner_id.vat:
            errors.append(
                _(
                    "You cannot issue an invoice to a partner with the same VAT number as the branch."
                )
            )
        ...
        if not all(
            [
                invoice.journal_id.l10n_eg_branch_id,
                invoice.journal_id.l10n_eg_branch_identifier,
        ...
        if not all(
            aml.product_uom_id.l10n_eg_unit_code_id.code
            ...
            errors.append(
                _(
                    "Please make sure the invoice lines UoM codes are all set up correctly"
                )
            )
        if not all(
            tax.l10n_eg_eta_code
        ...
        if not all(
            aml.product_id.l10n_eg_eta_code or aml.product_id.barcode
            for aml in invoice.invoice_line_ids.filtered(
                lambda x: x.display_type not in NON_ACCOUNTABLE_DISPLAY_TYPES
            )
        ):
            errors.append(
                _(
                    "Please make sure the EGS/GS1 Barcode is set correctly on all products"
                )
            )
        return errors
    ...
    def _l10n_eg_edi_post_invoice(self, invoice):
        ...
        if not invoice.l10n_eg_eta_json_doc_file:
            return {
                invoice: {
                    "error": _(
                        "An error occured in created the ETA invoice, please retry signing"
                    ),
                    "blocking_level": "error",
                }
            }
