...
def mocked_action_post_sign_invoices(self):
    for invoice in self:
        eta_invoice = self.env["account.edi.format"]._l10n_eg_eta_prepare_eta_invoice(
            self
        )
...
def mocked_l10n_eg_edi_post_invoice_web_service(self, invoice):
    ...
    invoice.l10n_eg_eta_json_doc_file = base64.b64encode(
        json.dumps(eta_invoice_json).encode()
    )
    invoice.invalidate_recordset(fnames=["l10n_eg_eta_json_doc_file"])
...
class TestEdiJson(TestEGEdiCommon):
                ...
                invoice_line_ids=[
                    {
                        ...
                        "tax_ids": [
                            (
                                ...
                                0,
                                self.env.ref(
                                    f"account.{self.env.company.id}_eg_standard_sale_14"
                                ).ids,
                            )
                        ],
                    },
    ...
    def test_4_simple_test_local_parter_vat_14_discount_multiple_tax(self):
        with (
            ...
            ref_eg_standard_sale_14 = self.env.ref(
                f"account.{self.env.company.id}_eg_standard_sale_14"
            ).ids
            invoice = self.create_invoice(
                partner_id=self.partner_a.id,
                ...
                json_file,
                {
                    "request": {
                        **COMMON_REQUEST_DICT,
                        ...
                        "taxTotals": [
                            {"taxType": "T1", "amount": 153.78},
                            {"taxType": "T3", "amount": 10.0},
                        ],
                ...
                invoice_line_ids=[
                    {
                        ...
                        "tax_ids": [
                            (
                                ...
                                0,
                                self.env.ref(
                                    f"account.{self.env.company.id}_eg_exempt_sale"
                                ).ids,
                            )
                        ],
                    },
    ...
    def test_7_simple_test_foreign_parter_exempt_discount_foreign_currency_credit_note(
        self,
    ):
        with (
            freeze_time(self.frozen_today),
    ...
    def test_8_test_serialization_function(self):
        with (
            ...
            self.assertTrue(generated_files)
            json_file = json.loads(generated_files[0])
            serialized_string = self.env[
                "l10n_eg_edi.thumb.drive"
            ]._serialize_for_signing(json_file["request"])
            self.assertEqual(
                serialized_string,
    ...
    def test_9_test_withholding_tax(self):
        with (
            ...
            taxes = (
                self.env.ref(f"account.{self.env.company.id}_eg_standard_sale_14").ids
                + self.env.ref(
                    f"account.{self.env.company.id}_eg_withholding_3_sale"
                ).ids
            )
            invoice = self.create_invoice(
                move_type="out_invoice",
                ...
                json_file,
                {
                    "request": {
                        **COMMON_REQUEST_DICT,
                        ...
                        "taxTotals": [
                            {"taxType": "T1", "amount": 14.0},
                            {"amount": 3.0, "taxType": "T4"},
                        ],
