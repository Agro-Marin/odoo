...
function assertProductPrice(product, pricelist_name, quantity, expected_price) {
    return function () {
        const pricelist = posmodel.data.models["product.pricelist"].find(
            (pricelist) => pricelist.name === pricelist_name,
        );
        let frontend_price = product.getPrice(
            pricelist,
            quantity,
            0,
            false,
    ...
    return [
        ...
        // So, wait for everything to be loaded
        {
            ...
            run: function () {
                ...
                const product_small_shelf = posmodel.data.models["product.template"]
                    .getAll()
                    .find((p) => p.display_name === "Small Shelf");
                const product_magnetic_board = posmodel.data.models["product.template"]
                    .getAll()
                    .find((p) => p.display_name === "Magnetic Board");
                const product_monitor_stand = posmodel.data.models["product.template"]
                    .getAll()
                    .find((p) => p.display_name === "Monitor Stand");
                const product_desk_pad = posmodel.data.models["product.template"]
                    .getAll()
                    .find((p) => p.display_name === "Desk Pad");
                const product_letter_tray = posmodel.data.models["product.template"]
                    .getAll()
