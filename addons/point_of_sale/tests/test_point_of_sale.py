...
class TestPointOfSale(TransactionCase):
    ...
    def test_no_default_pricelist(self):
        ...
        new_config = self.env["pos.config"].create(
            {
                "name": "usd config",
                "available_pricelist_ids": [(6, 0, [company1_pricelist.id])],
            }
        )
        ...
        self.assertEqual(
            ...
            "POS config incorrectly has pricelist %s"
            % new_config.pricelist_id.display_name,
        )
                ...
                "attribute_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "value_ids": [
                                (6, 0, [attribute_value_1.id, attribute_value_2.id])
                            ],
                        },
                    )
                ],
    ...
    def test_session_filter_local_data(self):
        ...
        product2 = self.env["product.template"].create({"name": "product2"})
        product3 = self.env["product.template"].create({"name": "product3"})
        ...
        products_to_display = list(
            set(products_to_display)
            - set(session.filter_local_data(models_to_filter)["product.template"])
        )
