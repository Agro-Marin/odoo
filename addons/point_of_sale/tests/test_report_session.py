...
class TestReportSession(TestPoSCommon):
    ...
    def test_report_session(self):
        ...
        self.product1 = self.create_product(
            "Product A", self.categ_basic, 110, self.tax1.id
        )
        ...
        split_payment_bank = [
            p for p in report["payments"] if p.get("id", 0) == self.bank_split_pm1.id
        ]
        ...
        self.assertEqual(
            report["products"][0]["products"][0]["base_amount"],
            100,
    ...
    def test_report_session_2(self):
        ...
        pdf = self.env["ir.actions.report"]._render_qweb_pdf(
            "point_of_sale.sale_details_report", res_ids=session_id_2
        )
        self.assertTrue(pdf)
    ...
    def test_report_listing(self):
        ...
        order_payment = (
            ...
            .create(
                [
                    {"amount": am, "payment_method_id": pm}
                    for am in [65, 100]
                    for pm in [cash_payment_method.id, bank_payment_method.id]
                ]
            )
        )
        ...
        self.assertEqual(
            order_report_lines[0].payment_method_id.id,
            order_report_lines[1].payment_method_id.id,
        )
        ...
        order_report_lines_count_product1 = (
            ...
            .sudo()
            .search_count([("product_id", "=", product1.id)])
        )
        order_report_lines_count_product2 = (
            self.env["report.pos.order"]
            .sudo()
            .search_count([("product_id", "=", product2.id)])
        )
    ...
    def test_report_session_3(self):
        ...
        report = self.env["report.point_of_sale.report_saledetails"].get_sale_details()
        self.assertEqual(
            report["products"][0]["products"][0]["quantity"],
            74.6,
    ...
    def test_report_bank_expected_different_than_counted(self):
        ...
        self.product1 = self.create_product(
            "Product A", self.categ_basic, 100, self.tax1.id
        )
    ...
    def test_report_session_4(self):
        ...
        self.product1 = self.create_product(
            "Product A", self.categ_basic, 125, self.tax1.id
        )
        ...
        self.assertEqual(
            ...
            100,
            "Base amount should be equal to 100",
        )
    ...
    def test_report_sum_taxes_base_amounts(self):
        ...
        product = self.create_product(
            "Product A", self.categ_basic, 110, tax_included.id
        )
        ...
        self.assertAlmostEqual(
            report["taxes"][0]["base_amount"],
            95.24 * 5,
            msg="Base amount should be equal to 476.20",
        )

    def test_report_reports_the_cash_rounding_applied_in_the_session(self):
        """The X/Z report accounts for what rounding added or removed.

        A register that rounds to 0.05 never takes the amount it charged: the
        difference is real money the report has to name, or the drawer will not
        ...
        tax = self.env["account.tax"].create({"name": "Tax 1", "amount": 10})
        product = self.create_product(
        ...
        # `add_payment` creates the payment without recomputing; the client's
        # own write path calls this, and it is the tax engine that rounds.
        ...
        self.assertNotEqual(
            ...
            0.0,
            "the fixture must actually have been rounded",
        )
        self.assertAlmostEqual(
            report["cash_rounding_total"],
            order.amount_cash_rounding,
            places=2,
            msg="the report must carry the rounding difference of every order",
        )

    def test_report_omits_the_cash_rounding_line_when_nothing_was_rounded(self):
        """With no rounding there is nothing to report, and no empty line."""
        product = self.create_product("Product Whole", self.categ_basic, 100)
        self.config.open_ui()
        session = self.config.current_session_id
        order = self.env["pos.order"].create(
            {
        ...
        self.make_payment(order, self.cash_pm1, 100.0)
        order._recompute_prices()
        session.action_pos_session_closing_control()
        ...
        self.assertEqual(report["cash_rounding_total"], 0.0)
