...
class TestPoSSetup(TestPoSCommon):
    ...
    def setUp(self):
        ...
        self.products = [
            self.create_product(
                "Product 1", self.categ_basic, lst_price=10.0, standard_price=5
            ),
            self.create_product(
                "Product 2", self.categ_basic, lst_price=20.0, standard_price=10
            ),
            self.create_product(
                "Product 3", self.categ_basic, lst_price=30.0, standard_price=15
            ),
        ]
    ...
    def test_product_price(self):
        ...
        for product in self.products:
            self.assertAlmostEqual(
                get_price(pricelist, product), product.lst_price * 0.5
            )
    ...
    def test_taxes(self):
        ...
        self.assertEqual(
            tax7.invoice_repartition_line_ids.mapped("account_id").id,
            self.tax_received_account.id,
        )
        ...
        self.assertEqual(
            tax10.invoice_repartition_line_ids.mapped("account_id").id,
            self.tax_received_account.id,
        )
        ...
        self.assertEqual(
            sorted(tax_group_7_10.children_tax_ids.ids), sorted((tax7 | tax10).ids)
        )
    ...
    def test_archive_used_journal(self):
        ...
        payment_method = self.env["pos.payment.method"].create(
            {"name": "Lets Pay for Tests", "journal_id": journal.id}
        )
