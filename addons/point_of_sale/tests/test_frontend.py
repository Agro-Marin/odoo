...
class TestPointOfSaleHttpCommon(AccountTestInvoicingHttpCommon):
    ...
    def setUpClass(cls):
        ...
        env["product.template.attribute.line"].create(
            {
                ...
                "value_ids": [
                    (
                        ...
                        0,
                        [
                            chair_fabrics_leather.id,
                            cls.chair_fabrics_wool.id,
                            cls.chair_fabrics_other.id,
                        ],
                    )
                ],
            }
        ...
        env["product.template.attribute.line"].create(
            {
                ...
                "value_ids": [
                    (
                        ...
                        0,
                        [
                            cls.chair_addon_cushion.id,
                            cls.chair_addon_cupholder.id,
                            cls.chair_addon_headrest.id,
                        ],
                    )
                ],
            }
                ...
                "item_ids": [
                    (
                        ...
                        0,
                        {
                            "compute_price": "fixed",
                            "fixed_price": 1,
                            "date_start": two_weeks_ago.strftime(
                                DEFAULT_SERVER_DATE_FORMAT
                            ),
                            "date_end": one_week_ago.strftime(
                                DEFAULT_SERVER_DATE_FORMAT
                            ),
                        },
                    ),
                    (
                        ...
                        0,
                        {
                            ...
                            "date_end": one_week_from_now.strftime(
                                DEFAULT_SERVER_DATE_FORMAT
                            ),
                        },
                    ),
                    (
                        ...
                        0,
                        {
                            "compute_price": "fixed",
                            "fixed_price": 3,
                            "date_start": one_week_from_now.strftime(
                                DEFAULT_SERVER_DATE_FORMAT
                            ),
                            "date_end": two_weeks_from_now.strftime(
                                DEFAULT_SERVER_DATE_FORMAT
                            ),
                        },
                    ),
        ...
        excluded_pricelist = env["product.pricelist"].create({"name": "Not loaded"})
        ...
        all_pricelists.write({"currency_id": main_company.currency_id.id})
        ...
        env["account.tax"].create(
            {
                "name": "DST",
                "amount": 5,
                "fiscal_position_ids": [Command.link(FP_POS_2M.id)],
...
class TestUi(TestPointOfSaleHttpCommon):
    ...
    def test_receipt_screen_tour(self):
        ...
        for order in self.env["pos.order"].search([]):
            self.assertEqual(
                order.state,
                "paid",
                "Validated order has payment of "
    ...
    def test_02_pos_with_invoiced(self):
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id, "ChromeTour", login="pos_user"
        )
    ...
    def test_optional_product(self):
        ...
        self.desk_pad.write(
            {"pos_optional_product_ids": [Command.set([self.small_shelf.id])]}
        )
    ...
    def test_05_ticket_screen(self):
        ...
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id, "TicketScreenTour", login="pos_user"
        )
    ...
    def test_change_without_cash_method(self):
        ...
        self.main_pos_config.write(
            {"payment_method_ids": [(6, 0, bank_pm.ids)], "ship_later": True}
        )
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
    ...
    def test_rounding_down(self):
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "PaymentScreenTotalDueWithOverPayment",
            login="pos_user",
        )
    ...
    def test_cash_payments_should_reflect_on_next_opening(self):
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id, "OrderPaidInCash", login="pos_user"
        )
        ...
        self.env["account.tax"].create(
            {
                ...
                "type_tax_use": "sale",
                "fiscal_position_ids": [
                    Command.set((fiscal_position_1 | fiscal_position_4).ids)
                ],
                "original_tax_ids": [
                    Command.set((tax_inclusive_1 | tax_exclusive_1).ids)
                ],
            }
        )
        self.env["account.tax"].create(
            {
                ...
                "type_tax_use": "sale",
                "fiscal_position_ids": [
                    Command.set((fiscal_position_2 | fiscal_position_3).ids)
                ],
                "original_tax_ids": [
                    Command.set((tax_inclusive_1 | tax_exclusive_1).ids)
                ],
    ...
    def test_06_pos_discount_display_with_multiple_pricelist(self):
        ...
        self.main_pos_config.write(
            {
                "pricelist_id": base_pricelist.id,
                "available_pricelist_ids": [
                    (6, 0, [base_pricelist.id, special_pricelist.id])
                ],
            }
        )
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "ReceiptScreenDiscountWithPricelistTour",
            login="pos_user",
        )
    ...
    def test_07_product_combo(self):
        ...
        combo_line_ids = self.env["pos.order.line"].search(
            [("product_id.name", "!=", "Office Combo"), ("order_id", "=", order.id)]
        )
        self.assertEqual(
            parent_line_id.combo_line_ids,
            combo_line_ids,
            "The combo parent should have 3 combo lines",
        )
    ...
    def test_09_pos_barcodes_scan_product_packaging(self):
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "BarcodeScanningProductPackagingTour",
            login="pos_user",
        )

    def test_GS1_pos_barcodes_scan(self):
        barcodes_gs1_nomenclature = self.env.ref(
    ...
    def test_customer_display_popup(self):
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "customer_display_shows_qr_popup",
            login="pos_user",
        )
    ...
    def test_lot_refund(self):
        ...
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id, "LotRefundTour", login="pos_user"
        )
    ...
    def test_limited_product_pricelist_loading(self):
        ...
        self.main_pos_config.pricelist_id.write(
            {"item_ids": [(6, 0, pricelist_item.ids)]}
        )
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "limitedProductPricelistLoading",
            login="pos_user",
        )
    ...
    def test_restricted_categories_combo_product(self):
        ...
        self.env["pos.printer"].create(
            {
                ...
                "product_categories_ids": [
                    Command.set(self.env["pos.category"].search([]).ids)
                ],
            }
        )
        ...
        self.desks_combo.combo_item_ids[0].product_id.pos_categ_ids = [
            (6, 0, [pos_restricted_categ.id])
        ]
        self.desks_combo.combo_item_ids[1].product_id.pos_categ_ids = [
            (6, 0, [pos_other_categ.id])
        ]
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
    ...
    def test_printer_restricts_to_allowed_categories_for_combo(self):
        ...
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_pos_tour(
            "test_printer_restricts_to_allowed_categories_for_combo", login="pos_user"
        )
    ...
    def test_printer_not_linked_to_any_combo_category(self):
        ...
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_pos_tour(
            "test_printer_not_linked_to_any_combo_category", login="pos_user"
        )
    ...
    def test_allow_order_modification_after_validation_error(self):
        ...
        with patch.object(
            ...
            with self.assertLogs(level="WARNING") as log_catcher:
                ...
                self.start_tour(
                    "/pos/ui/%d" % self.main_pos_config.id,
                    "OrderModificationAfterValidationError",
                    login="pos_user",
                )
    ...
    def test_refund_multiple_products_amounts_compliance(self):
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "refund_multiple_products_amounts_compliance",
            login="pos_user",
        )
            ...
            [
                {
                    "name": product.name,
                    "combo_item_ids": [
                        Command.create({"product_id": product.id, "extra_price": 0})
                    ],
                }
                for product in (
                    self.desk_organizer.product_variant_id,
                    self.desk_pad.product_variant_id,
                    self.whiteboard_pen.product_variant_id,
                )
            ]
        )
        ...
        self.env["product.product"].create(
            {
                ...
                "type": "combo",
                "taxes_id": False,
                "combo_ids": [(6, 0, [combo.id for combo in combos])],
            }
        )
    ...
    def test_customer_display_as_public(self):
        ...
        response = self.url_open(
            f"/web/image/pos.config/{self.main_pos_config.id}/customer_display_bg_img"
        )
    ...
    def test_combo_item_image_display(self):
        ...
        self.start_tour(
            f"/pos/ui?config_id={self.main_pos_config.id}",
            "test_combo_item_image_not_display",
            login="pos_user",
        )
    ...
    def test_product_with_dynamic_attributes(self):
        ...
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}",
            "PosProductWithDynamicAttributes",
            login="pos_admin",
        )
    ...
    def test_product_search_2(self):
        ...
        self.main_pos_config.open_ui()
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}", "SearchProducts", login="pos_user"
        )
    ...
    def test_lot(self):
        ...
        self.env["stock.quant"].with_context(inventory_mode=True).create(
            {
                ...
                .create({"name": "1001", "product_id": product2.id})
                .id,
            }
        ...
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id, "LotTour", login="pos_user"
        )
        two_last_orders = self.env["pos.order"].search([], order="id desc", limit=2)
        order_lot_id = [
            lot_id.lot_name for lot_id in two_last_orders[1].lines.pack_lot_ids
        ]
        refund_lot_id = [
            lot_id.lot_name for lot_id in two_last_orders[0].lines.pack_lot_ids
        ]
        self.assertEqual(
            order_lot_id,
            refund_lot_id,
            ...
            [
                {
                    ...
                    "location_id": self.env.user._get_default_warehouse_id().lot_stock_id.id,
                    "lot_id": self.env["stock.lot"]
                    .create({"name": "1001", "product_id": product.id})
                    .id,
                },
                {
                    "product_id": product.id,
                    ...
                    "location_id": self.env.user._get_default_warehouse_id().lot_stock_id.id,
                    "lot_id": self.env["stock.lot"]
                    .create({"name": "1002", "product_id": product.id})
                    .id,
                },
            ]
        ).sudo().action_apply_inventory()
            ...
            self.env["stock.quant"].create(
                {
                    ...
                    "location_id": self.env.user._get_default_warehouse_id().lot_stock_id.id,
                    "lot_id": self.env["stock.lot"]
                    .create({"name": sn, "product_id": product.id})
                    .id,
                }
            ).sudo().action_apply_inventory()
    ...
    def test_product_search(self):
        ...
        product_with_variant.product_variant_ids[0].write(
            {"barcode": "variant_barcode_1", "default_code": "VARIANT_1"}
        )
        product_with_variant.product_variant_ids[1].write(
            {"barcode": "variant_barcode_2", "default_code": "VARIANT_2"}
        )
    ...
    def test_tracking_number_closing_session(self):
        ...
        for i in range(len(references) - 1):
            self.assertEqual(
                int(references[i + 1].split("-")[-1]),
                int(references[i].split("-")[-1]) + 1,
                "There is a gap in the pos references",
            )
    ...
    def test_reload_page_before_payment_with_customer_account(self):
        ...
        self.main_pos_config.write(
            {"payment_method_ids": [(6, 0, self.customer_account_payment_method.ids)]}
        )
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}",
    ...
    def test_cash_in_out(self):
        ...
        self.main_pos_config.with_user(self.pos_user).open_ui()
        self.start_tour(
            f"/pos/ui/{self.main_pos_config.id}", "test_cash_in_out", login="pos_user"
        )

        self.assertEqual(
            len(self.main_pos_config.current_session_id.statement_line_ids),
            1,
            "There should be one cash in/out statement line",
        )
        self.assertEqual(
            self.main_pos_config.current_session_id.statement_line_ids[0].amount,
            -5,
            "The cash in/out amount should be -5",
        )
    ...
    def test_order_and_invoice_amounts(self):
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "PaymentScreenInvoiceOrder",
            login="pos_user",
        )

        order = self.env["pos.order"].search(
            [("partner_id", "=", self.partner_test_1.id)], limit=1
        )
        ...
        invoice = self.env["account.move"].search(
            [("invoice_origin", "=", order.pos_reference)], limit=1
        )
        ...
        self.assertAlmostEqual(
            ...
            places=2,
            msg="Order and Invoice amounts do not match.",
        )
    ...
    def test_pricelist_parent_category_rule(self):
        ...
        self.start_tour(
            f"/pos/ui?config_id={self.main_pos_config.id}",
            "test_pricelist_parent_category_rule",
            login="pos_user",
        )
    ...
    def test_product_create_update_from_frontend(self):
        ...
        self.env["pos.category"].search(
            [("id", "!=", self.pos_cat_chair_test.id)]
        ).write({"sequence": 100})
        self.pos_cat_chair_test.write({"sequence": 1})
        self.main_pos_config.with_user(self.pos_admin).open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "test_product_create_update_from_frontend",
            login="pos_admin",
        )
        ...
        frontend_created_product_edited = self.env["product.product"].search(
            [("name", "=", "Test Frontend Product Edited")]
        )

        self.assertEqual(frontend_created_product, 0)
        self.assertEqual(
            frontend_created_product_edited.name, "Test Frontend Product Edited"
        )
    ...
    def test_one_attribute_value_scan_barcode(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_one_attribute_value_scan_barcode",
            login="pos_user",
        )
    ...
    def test_draft_orders_not_syncing(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_draft_orders_not_syncing",
            login="pos_user",
        )
        n_draft_order = self.env["pos.order"].search_count(
            [("state", "=", "draft")], limit=1
        )
    ...
    def test_zero_decimal_places_currency(self):
        ...
        self.main_pos_config.available_pricelist_ids.write(
            {"currency_id": zero_decimal_currency.id}
        )
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_zero_decimal_places_currency",
            login="pos_user",
        )
    ...
    def test_barcode_search_attributes_preset(self):
        ...
        attribute_1, attribute_2, attribute_3, attribute_4 = self.env[
            "product.attribute"
        ].create(
            [
                {
    ...
    def test_auto_validate_force_done(self):
        self.main_pos_config.write({"auto_validate_terminal_payment": True})
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_auto_validate_force_done",
            login="pos_user",
        )
    ...
    def test_pos_ui_round_globally(self):
        ...
        self.assertEqual(
            len(lines), 5, "There should be 5 lines in the session journal entry"
        )
    ...
    def test_quantity_package_of_non_basic_unit(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_quantity_package_of_non_basic_unit",
            login="pos_user",
        )
    ...
    def test_preset_timing_retail(self):
        ...
        self.preset_delivery.write(
            {"use_timing": True, "resource_calendar_id": resource_calendar}
        )
        self.start_pos_tour("test_preset_timing_retail")

    def test_pricelists_in_pos(self):
        pos_limited_category = self.env["pos.category"].create(
            {"name": "Limited Category"}
        )
        pos_category = self.env["pos.category"].create(
            {"name": "test_pricelists_in_pos"}
        )
        product_category = self.env["product.category"].create(
            {"name": "test_pricelists_in_pos"}
        )
        orange_category = self.env["product.category"].create(
            {"name": "Orange Category"}
        )

        def generate_pricelist_items(
            pricelist,
            fixed_price,
            product=None,
            product_tmpl=None,
        ...
        def generate_product_template_with_attributes(
            name, price, pos_category=None, product_category=None
        ):
            size_attribute = self.env["product.attribute"].create(
                {
                    "name": "Size",
        ...
        banana = generate_product_template_with_attributes(
            "banana", 10.00, pos_category
        )
        apple = generate_product_template_with_attributes(
            "apple", 5.00, False, product_category
        )
        pear = generate_product_template_with_attributes("pear", 2.00)
        lime = generate_product_template_with_attributes("lime", 1.00)
        orange = generate_product_template_with_attributes(
            "orange", 3.00, False, orange_category
        )
        ...
        self.main_pos_config.write(
            {
                ...
                "available_pricelist_ids": [
                    (6, 0, [test_pricelist.id, percentage_pricelist.id])
                ],
        ...
        color_attribute = cherry.attribute_line_ids.filtered(
            lambda l: l.attribute_id.name == "Color"
        )
        first_color_value = color_attribute.product_template_value_ids.filtered(
            lambda v: v.attribute_id.name == "Color" and v.name == "RED"
        )
        first_size_value = (
            cherry.product_variant_ids.product_template_attribute_value_ids.filtered(
                lambda v: v.attribute_id.name == "Size" and v.name == "BIG"
            )
        )
        first_color_value.exclude_for = [
            (
                0,
                0,
                {
        ...
        def load_product_from_pos_patch(self, config_id, domain, offset=0, limit=0):
            ...
            result = super(product_template, self).load_product_from_pos(
                config_id, domain, offset, limit
            )
            lowered_name = result["product.template"][0]["display_name"].lower()
            load_product_from_pos_stats["items"][lowered_name] = len(
                result["product.pricelist.item"]
            )
            return result

        with patch.object(
            product_template, "load_product_from_pos", load_product_from_pos_patch
        ):
        ...
        self.assertEqual(
            ...
            3,
            "Banana should have 3 pricelist items",
        )
        self.assertEqual(
            load_product_from_pos_stats["items"]["apple"],
            1,
            "Apple should have 1 pricelist item",
        )
        self.assertEqual(
            load_product_from_pos_stats["items"]["pear"],
            3,
            "Pear should have 3 pricelist items",
        )
        self.assertEqual(
            load_product_from_pos_stats["items"]["lime"],
            3,
            "Lime should have 3 pricelist items",
        )
        self.assertEqual(
            load_product_from_pos_stats["items"]["orange"],
            2,
            "Orange should have 2 pricelist items",
        )
        self.assertEqual(
            load_product_from_pos_stats["items"]["kiwi"],
            1,
            "Kiwi should have 1 pricelist item",
        )
    ...
    def test_available_children_categories(self):
        ...
        self.main_pos_config.write(
            {
                "limit_categories": True,
                "iface_available_categ_ids": [
                    (6, 0, [parent_categ.id, children_categs[1].id])
                ],
            }
        )
        self.main_pos_config.open_ui()
        ...
        self.assertNotIn(
            children_categs[0].id,
            category_id,
            "Child category is unavailable and shouldn't appear in the POS",
        )
        self.assertIn(
            children_categs[1].id,
            category_id,
            "Child category is available and should appear in the POS",
        )
    ...
    def test_available_product_uom_ids(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "point_of_sale.limited_product_count", "2"
        )
        ...
        loaded_data = self.main_pos_config.current_session_id.load_data([])
        loaded_product_uoms = [
            loaded_product_uom["id"]
            for loaded_product_uom in loaded_data["product.uom"]
        ]

        self.assertNotIn(
            product_uom_three.id,
            loaded_product_uoms,
            f"Product UOM {product_uom_three} shouldn't be loaded as its product {product_three} is not included in the results",
        )
    ...
    def test_fast_payment_validation_from_product_screen_without_automatic_receipt_printing(
        self,
    ):
        self.preset_delivery = self.env["pos.preset"].create(
            {
        ...
        self.assertEqual(
            order1.state,
            "paid",
            "The order should be paid after the fast payment validation",
        )
        self.assertEqual(
            len(order1.payment_ids),
            1,
            "There should be one payment line used for the fast payment",
        )
        self.assertEqual(
            order1.payment_ids.payment_method_id,
            self.bank_payment_method,
            "The payment method used should be the bank payment method",
        )
        ...
        self.assertEqual(
            order2.payment_ids.payment_method_id,
            self.bank_payment_method,
            "The payment method used should be the bank payment method",
        )

    def test_fast_payment_validation_from_product_screen_with_automatic_receipt_printing(
        self,
    ):
        self.main_pos_config.write(
            {
    ...
    def test_paid_order_with_archived_product_loads(self):
        ...
        self.start_tour(
            f"/pos/ui?config_id={self.main_pos_config.id}",
            "test_paid_order_with_archived_product_loads",
            login="pos_user",
        )
    ...
    def test_load_pos_demo_data(self):
        ...
        self.start_pos_tour(
            "test_load_pos_demo_data_with_member_role", login="pos_user"
        )
        products = self.env["product.template"].search_count(
            [("available_in_pos", "=", True)]
        )
        self.assertFalse(
            products, "Demo data should not be loaded by user with member role."
        )
                    ...
                    Command.create(
                        {
                            "attribute_id": color_attribute.id,
                            "value_ids": [
                                Command.link(id) for id in color_attribute.value_ids.ids
                            ],
                        }
                    ),
                    Command.create(
                        {
                            "attribute_id": size_attribute.id,
                            "value_ids": [
                                Command.link(id) for id in size_attribute.value_ids.ids
                            ],
                        }
                    ),
                ],
    ...
    def test_cross_exclusion_attribute_values(self):
                        ...
                        0,
                        {
                            ...
                            "value_ids": [
                                (
                                    ...
                                    0,
                                    [
                                        self.attribute_1_value_1.id,
                                        self.attribute_1_value_2.id,
                                    ],
                                )
                            ],
                        },
                        ...
                        0,
                        {
                            ...
                            "value_ids": [
                                (
                                    ...
                                    0,
                                    [
                                        self.attribute_2_value_1.id,
                                        self.attribute_2_value_2.id,
                                    ],
                                )
                            ],
                        },
        ...
        ptav_1_1 = self.test_product_1.attribute_line_ids.filtered(
            ...
            lambda v: v.product_attribute_value_id.id == self.attribute_1_value_1.id
        )
        ptav_1_2 = self.test_product_1.attribute_line_ids.filtered(
            lambda l: l.attribute_id.id == self.attribute_1.id
        ).product_template_value_ids.filtered(
            lambda v: v.product_attribute_value_id.id == self.attribute_1_value_2.id
        )
        ptav_2_2 = self.test_product_1.attribute_line_ids.filtered(
            lambda l: l.attribute_id.id == self.attribute_2.id
        ).product_template_value_ids.filtered(
            lambda v: v.product_attribute_value_id.id == self.attribute_2_value_2.id
        )
        ptav_2_1 = self.test_product_1.attribute_line_ids.filtered(
            lambda l: l.attribute_id.id == self.attribute_2.id
        ).product_template_value_ids.filtered(
            lambda v: v.product_attribute_value_id.id == self.attribute_2_value_1.id
        )
    ...
    def test_product_info_product_inventory(self):
        ...
        for variant in range(len(product_template.product_variant_ids)):
            self.env["stock.quant"]._update_available_quantity(
                product_template.product_variant_ids[variant],
                self.main_pos_config.warehouse_id.lot_stock_id,
                (variant + 1) * 100,
            )
            product_template.product_variant_ids[variant].write(
                {"barcode": f"product_variant_{variant}"}
            )
    ...
    def test_exclusion_attribute_values(self):
        chair_fabrics_other_ptav = self.configurable_chair.attribute_line_ids.filtered(
            lambda l: l.attribute_id.id == self.chair_fabrics_attribute.id
        ).product_template_value_ids.filtered(
            lambda v: v.product_attribute_value_id.id == self.chair_fabrics_other.id
        )
        chair_fabrics_wool_ptav = self.configurable_chair.attribute_line_ids.filtered(
            lambda l: l.attribute_id.id == self.chair_fabrics_attribute.id
        ).product_template_value_ids.filtered(
            lambda v: v.product_attribute_value_id.id == self.chair_fabrics_wool.id
        )
        chair_color_red_ptav = self.configurable_chair.attribute_line_ids.filtered(
            lambda l: l.attribute_id.id == self.chair_color_attribute.id
        ).product_template_value_ids.filtered(
    ...
    def test_product_configurator_price(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_product_configurator_price",
            login="pos_user",
        )
...
class TestTaxCommonPOS(TestPointOfSaleHttpCommon, TestTaxCommon):
    ...
    def ensure_products_on_document(self, document, product_prefix):
        for i, base_line in enumerate(document["lines"], start=1):
            base_line["product_id"] = self.create_base_line_product(
                base_line, name=f"{product_prefix}_{i}"
            )
    ...
    def assert_pos_orders_and_invoices(self, tour, tests_with_orders):
        ...
        ) in enumerate(zip(orders, tests_with_orders, strict=False)):
