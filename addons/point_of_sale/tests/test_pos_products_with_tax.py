...
class TestPoSProductsWithTax(TestPoSCommon):
    ...
    def test_orders_with_invoiced(self):
        ...
        def _before_closing_cb():
            ...
            invoiced_orders = self.pos_session.order_ids.filtered(
                lambda order: order.is_invoiced
            )
            self.assertEqual(
        ...
        def _after_closing_cb():
            ...
            tax_lines = session_move.line_ids.filtered(
                lambda line: line.account_id == self.tax_received_account
            )

            manually_calculated_taxes = (-6.81, -44.54)
            self.assertAlmostEqual(
                sum(manually_calculated_taxes), sum(tax_lines.mapped("balance"))
            )
            for t1, t2 in zip(
                sorted(manually_calculated_taxes),
                sorted(tax_lines.mapped("balance")),
                strict=False,
            ):
                self.assertAlmostEqual(
                    t1, t2, msg="Taxes should be correctly combined."
                )

            base_amounts = (
                97.27,
                445.46,
            )  # computation does not include invoiced order.
            self.assertAlmostEqual(
                sum(base_amounts), sum(tax_lines.mapped("tax_base_amount"))
            )
    ...
    def test_return_order(self):
        ...
        def _after_closing_cb():
            manually_calculated_taxes = (
                4.01,
                6.36,
            )  # should be positive since it is return order
            tax_lines = self.pos_session.move_id.line_ids.filtered(
                lambda line: line.account_id == self.tax_received_account
            )
            self.assertAlmostEqual(
                sum(manually_calculated_taxes), sum(tax_lines.mapped("balance"))
            ...
            for t1, t2 in zip(
                ...
                self.assertAlmostEqual(
                    t1,
                    t2,
                    msg="Taxes should be correctly combined and should be debit.",
                )
    ...
    def test_pos_create_correct_account_move(self):
        ...
        self.assertEqual(
            2,
            len(lines.filtered(lambda l: l.tax_ids)),
            "Taxes should have been set on 2 lines",
        )
        self.assertEqual(
            4,
            len(lines.filtered(lambda l: l.tax_tag_ids)),
            "Tags should have been set on 4 lines",
        )
        self.assertRecordValues(
            ...
            [
                # pylint: disable=bad-whitespace
                {
                    ...
                    "balance": -10.0,
                    "tax_ids": tax_21_incl.ids,
                    "tax_tag_ids": self.tax_tag_invoice_base.ids,
                },
                {
                    "account_id": self.tax_received_account.id,
                    "balance": -2.10,
                    "tax_ids": [],
                    "tax_tag_ids": self.tax_tag_invoice_tax.ids,
                },
                {
                    "account_id": self.tax_received_account.id,
                    "balance": 1.05,
                    "tax_ids": [],
                    "tax_tag_ids": self.tax_tag_refund_tax.ids,
                },
                {
                    "account_id": self.sale_account.id,
                    "balance": 5.00,
                    "tax_ids": tax_21_incl.ids,
                    "tax_tag_ids": self.tax_tag_refund_base.ids,
                },
                {
                    "account_id": self.cash_pm1.receivable_account_id.id,
                    "balance": 6.05,
    ...
    def test_pos_create_correct_account_move_round_globally_discount_real_use_case(
        self,
    ):
        """Test for orders with global rounding enabled
    ...
    def test_pos_loaded_product_taxes_on_branch(self):
        ...
        self.xx_bank_journal = (
            self.env["account.journal"]
            .with_company(branch_xx)
            .create(
                {
                    "name": "Bank",
                    "type": "bank",
        ...
        self.assertEqual(
            next(
                iter(
                    filter(
                        lambda p: p["id"] == product_all_taxes.product_tmpl_id.id,
                        pos_data["product.template"],
                    )
                )
            )["taxes_id"],
            tax_xx.ids,
        ...
        self.assertEqual(
            next(
                iter(
                    filter(
                        lambda p: p["id"] == product_no_xx_tax.product_tmpl_id.id,
                        pos_data["product.template"],
                    )
                )
            )["taxes_id"],
        ...
        tax_data_no_branch = next(
            iter(
                filter(
                    lambda p: p["id"] == product_no_branch_tax.product_tmpl_id.id,
                    pos_data["product.template"],
                )
            )
        )["taxes_id"]
        tax_data_no_branch.sort()
        self.assertEqual(tax_data_no_branch, (tax_a + tax_b).ids)
        self.assertEqual(
            next(
                iter(
                    filter(
                        lambda p: p["id"] == product_no_tax.product_tmpl_id.id,
                        pos_data["product.template"],
                    )
                )
            )["taxes_id"],
        ...
        def get_taxes_name_popup(product):
            ...
            return [
                tax["name"]
                for tax in product.with_user(pos_user)
                .with_context(allowed_company_ids=[branch_xx_id])
                .get_product_info_pos(product_all_taxes_lst_price, 1, xx_config_id)[
                    "all_prices"
                ]["tax_details"]
            ]
        ...
        self.assertEqual(
            get_taxes_name_popup(product_no_branch_tax), ["Tax A", "Tax B"]
        )
    ...
    def test_combo_product_variant_error(self):
        ...
        a1 = self.env["product.attribute.value"].create(
            {"name": "V0hFCg==", "attribute_id": size_attribute.id}
        )
