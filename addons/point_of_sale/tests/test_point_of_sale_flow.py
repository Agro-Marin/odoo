...
class TestPointOfSaleFlow(CommonPosTest):
    def setup_tags(self):
        ...
        ).write({"tag_ids": tags[0].ids})
        self.tax_sale_a.invoice_repartition_line_ids.filtered(
            lambda l: l.repartition_type == "tax"
        ).write({"tag_ids": tags[1].ids})
        self.tax_sale_a.refund_repartition_line_ids.filtered(
            lambda l: l.repartition_type == "base"
        ).write({"tag_ids": tags[2].ids})
        self.tax_sale_a.refund_repartition_line_ids.filtered(
            lambda l: l.repartition_type == "tax"
        ).write({"tag_ids": tags[3].ids})
    ...
    def test_order_partial_refund(self):
        ...
        payment_context = {
            "active_ids": remaining_refund.ids,
            "active_id": remaining_refund.id,
        }
        refund_payment = (
            self.env["pos.make.payment"]
            .with_context(**payment_context)
    ...
    def test_order_to_payment_currency(self):
        ...
        amount_currency_lines = current_session.move_id.mapped(
    ...
    def test_order_to_invoice_no_tax(self):
        ...
        self.assertFalse(
            order.account_move, "Invoice should not be attached to order yet."
        )
    ...
    def test_order_multi_step_route(self):
        ...
        picking_mls_no_stock = order.picking_ids.move_line_ids.filtered(
            lambda l: (
                l.product_id.id == self.ten_dollars_with_10_incl.product_variant_id.id
            )
        )
        picking_mls_stock = order.picking_ids.move_line_ids.filtered(
            lambda l: (
                l.product_id.id
                == self.twenty_dollars_with_10_incl.product_variant_id.id
            )
        )
        self.assertEqual(order.state, "paid")
    ...
    def test_pos_order_invoice_payment_term(self):
        ...
        payment_methods = (
            self.pos_config_usd.payment_method_ids
            | self.customer_account_payment_method
        )
        self.pos_config_usd.write(
    ...
    def test_sale_order_postponed_invoicing(self):
        ...
        self.assertRecordValues(
            reverse_closing_entries[0].line_ids.sorted(),
            [
                {
                    "balance": 3.0,
                    "account_id": self.company_data["default_account_tax_sale"].id,
        ...
        self.assertRecordValues(
            reverse_closing_entries[2].line_ids.sorted(),
            [
                {
                    "balance": -23.0,
                    "account_id": self.company_data["default_account_receivable"].id,
    ...
    def test_sale_order_postponed_invoicing_storno(self):
        ...
        self.assertTrue(
            all(
                amount >= 0
                for amount in reverse_closing_entries[1:].line_ids.mapped("debit")
                + reverse_closing_entries[1:].line_ids.mapped("credit")
            ),
    ...
    def test_tracked_product_with_owner(self):
        ...
        current_session.action_pos_session_closing_control()
        self.assertEqual(
            current_session.picking_ids.move_line_ids.owner_id.id, self.partner_adgu.id
        )
    ...
    def test_order_refund_with_invoice(self):
        ...
        invoices = self.env["account.move"].search(
            [("move_type", "=", "out_invoice")], order="id desc", limit=1
        )
    ...
    def test_pos_branch_account(self):
        ...
        self.assertEqual(
            ...
            "closed",
            msg="State of current session should be closed.",
        )
                ...
                for quant in self.env["stock.quant"].search(
                    [
                        (
                            "product_id",
                            "=",
                            self.ten_dollars_with_10_incl.product_variant_id.id,
                        )
                    ]
                )
            )
    ...
    def test_reordering_rules_triggered_closing_pos(self):
        ...
        self.env["stock.route"].search([("name", "=", "Buy")]).warehouse_ids = self.env[
            "stock.warehouse"
        ].search([])
        self.ten_dollars_with_15_incl.write(
            {
        ...
        self.assertEqual(
            purchase_order.line_ids.product_id.id,
    ...
    def test_state_when_closing_register(self):
        ...
        current_session = self.pos_config_usd.current_session_id
        current_session.action_pos_session_closing_control(
            bank_payment_method_diffs={self.bank_payment_method.id: 5.00}
        )
        self.assertEqual(current_session.state, "closed")
    ...
    def test_pos_order_refund_ship_delay_totalcost(self):
        ...
        refunded_order_line = self.env["pos.order.line"].search(
        ...
        order = self.env["pos.order"].create(
            {
                ...
                "lines": [
                    (
                        ...
                        0,
                        {
                            ...
                            .search([("available_in_pos", "=", True)], limit=1)
                            .id,
                            "price_unit": 49.99,
                            "discount": 0,
                            "qty": 1,
                ...
                "preset_time": fields.Datetime.to_string(
    ...
    def test_sum_only_pos_locations(self):
        ...
        result = self.env["pos.order.line"].get_existing_lots(
            self.env.company.id, self.pos_config.id, self.product.id
        )

        self.assertEqual(len(result), 1, "Should return exactly one lot")
        self.assertEqual(result[0]["name"], "TEST_LOT")
        self.assertEqual(
            result[0]["product_qty"],
            15.0,
            "Should sum only quantities from POS source locations",
        )
    ...
    def test_order_invoiced_after_session_closed(self):
        ...
        order_no_invoice, _ = self.create_backend_pos_order(
            {**order_data, "to_invoice": False, "partner_id": False}
        )
        order_invoiced_immediate, _ = self.create_backend_pos_order(
            {**order_data, "to_invoice": True, "partner_id": self.partner.id}
        )
        ...
        self.assertFalse(
            order_invoiced_immediate.reversed_move_ids,
    ...
    def test_payment_difference_accounting_items(self):
        ...
        self.assertRecordValues(
            self.bank_payment_move.line_ids.sorted("balance"),
            [
                {
                    "balance": -100.0,
    ...
    def test_search_paid_order_ids(self):
        ...
        order_ids = [
            oi[0]
            for oi in self.env["pos.order"].search_paid_order_ids(
                other_pos_config.id, [], 80, 0
            )["ordersInfo"]
        ]
        ...
        order_ids = [
            ...
            for oi in self.env["pos.order"].search_paid_order_ids(
                other_pos_config.id,
                [("partner_id.complete_name", "ilike", self.partner.complete_name)],
                80,
                0,
            )["ordersInfo"]
        ]
    ...
    def test_session_name_gap(self):
        ...
        with patch.object(
            self.env.registry.models["pos.session"],
            "_post_cash_details_message",
            _post_cash_details_message_patch,
        ):
                ...
                "group_ids": [
                    (
                        ...
                        [
                            ...
                            self.ref("point_of_sale.group_pos_user"),
                        ],
                    )
                ],
    ...
    def test_delete_res_partner_linked_to_pos_order(self):
        ...
        with self.assertRaises(
            ValidationError,
            msg="You cannot delete a customer that has point of sales orders. You can archive it instead.",
        ):
            partner.unlink()
    ...
    def test_split_payment_linked_to_accounting_partner(self):
        ...
        child_partner = self.env["res.partner"].create(
            {"name": "partner1 child", "parent_id": self.partner.id}
        )
        product_order = {
            "amount_paid": 750,
            "amount_tax": 0,
    ...
    def test_filter_local_data_no_errors(self):
        ...
        self.env.clear()
        data = current_session.with_company(self.env.company).filter_local_data(
    ...
    def test_order_invoiced_customer_account_after_session_closed(self):
        ...
        order_no_invoice.action_pos_order_invoice()
        customer_account_receivable_entry = current_session.move_id.line_ids.filtered(
            lambda l: l.partner_id == self.partner
        )
        reversal_receivable_entry = (
            order_no_invoice.reversed_move_ids.line_ids.filtered(
