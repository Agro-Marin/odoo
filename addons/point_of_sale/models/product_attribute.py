...
class ProductTemplateAttributeValue(models.Model):
    ...
    def _load_pos_data_domain(self, data, config):
        ptav_ids = {
            ...
            for ptav_id in p["product_template_variant_value_ids"]
        }
        ptav_ids.update(
            {
                ptav_id
                for ptal in data["product.template.attribute.line"]
                for ptav_id in ptal["product_template_value_ids"]
            }
        )
        return [
            ("ptav_active", "=", True),
...
class ProductTemplateAttributeExclusion(models.Model):
    ...
    def _load_pos_data_domain(self, data, config):
        loaded_product_tmpl_ids = list({p["id"] for p in data["product.template"]})
        loaded_ptav_ids = list(
            {ptav["id"] for ptav in data["product.template.attribute.value"]}
        )
        return [
            ("product_tmpl_id", "in", loaded_product_tmpl_ids),
            ("product_template_attribute_value_id", "in", loaded_ptav_ids),
        ]
