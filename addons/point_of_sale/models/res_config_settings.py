...
class ResConfigSettings(models.TransientModel):
    ...
    def _default_pos_config_id(self):
        ...
        return self.env["pos.config"].search(
            [("company_id", "=", self.env.company.id)], order="write_date desc", limit=1
        )
    ...
    sale_tax_id = fields.Many2one(
        ...
        related="company_id.tax_config_id.account_sale_tax_id",
        string="Default Sale Tax",
        readonly=False,
        check_company=True,
    )
    module_pos_adyen = fields.Boolean(
    ...
    module_pos_stripe = fields.Boolean(
    ...
    module_pos_mercado_pago = fields.Boolean(
    ...
    module_pos_pine_labs = fields.Boolean(
    ...
    module_pos_qfpay = fields.Boolean(
    ...
    module_pos_pricer = fields.Boolean(
    ...
    def action_open_payment_method_form(self):
        ...
        return {
            ...
            "target": "current",
            "context": {
                "default_config_ids": self.env.context.get("config_ids", False)
                or False,
                "default_payment_method_type": "terminal",
                "default_use_payment_terminal": self.env.context.get(
                    "selection", False
                ),
                    ...
                    if field.name.startswith("pos_") and val is not None:
                        ...
                        if pos_config_field_name not in self.env["pos.config"]._fields:
                            _logger.warning(
                                ...
                                " field '%s' is not a valid field in the pos.config model.",
                                field.name,
        ...
        for res_config in self:
            if not res_config.pos_limit_categories:
                ...
                res_config.pos_iface_available_categ_ids = (
                    res_config.pos_config_id.iface_available_categ_ids
                )
    ...
    def _compute_pos_selectable_categ_ids(self):
        for res_config in self:
            if res_config.pos_iface_available_categ_ids:
                res_config.pos_selectable_categ_ids = (
                    res_config.pos_iface_available_categ_ids
                )
            else:
    ...
    def _compute_pos_iface_cashdrawer(self):
        for res_config in self:
            if self._is_cashdrawer_displayed(res_config):
                res_config.pos_iface_cashdrawer = (
                    res_config.pos_config_id.iface_cashdrawer
                )
            else:
    ...
    def _compute_pos_fiscal_positions(self):
        for res_config in self:
            if res_config.pos_tax_regime_selection:
                res_config.pos_default_fiscal_position_id = (
                    res_config.pos_config_id.default_fiscal_position_id
                )
                res_config.pos_fiscal_position_ids = (
                    res_config.pos_config_id.fiscal_position_ids
                )
            else:
    ...
    def _compute_pos_pricelists(self):
        for res_config in self:
            ...
            if not res_config.pos_use_pricelist:
                ...
                p.currency_id.id != currency_id
                for p in res_config.pos_available_pricelist_ids
            ):
        ...
        for res_config in self:
            if res_config.pos_use_pricelist:
                res_config.pos_allowed_pricelist_ids = (
                    res_config.pos_available_pricelist_ids.ids
                )
            else:
                res_config.pos_allowed_pricelist_ids = (
    ...
    def _compute_pos_iface_print_via_proxy(self):
        for res_config in self:
            if not res_config.pos_is_posbox:
                ...
                res_config.pos_iface_print_via_proxy = (
                    res_config.pos_config_id.iface_print_via_proxy
                )
    ...
    def _compute_pos_iface_scan_via_proxy(self):
        for res_config in self:
            if not res_config.pos_is_posbox:
                ...
                res_config.pos_iface_scan_via_proxy = (
                    res_config.pos_config_id.iface_scan_via_proxy
                )
    ...
    def _compute_pos_iface_electronic_scale(self):
        for res_config in self:
            if not res_config.pos_is_posbox:
                ...
                res_config.pos_iface_electronic_scale = (
                    res_config.pos_config_id.iface_electronic_scale
                )

    @api.onchange("pos_trusted_config_ids")
    def _onchange_trusted_config_ids(self):
        for config in self:
            removed_trusted_configs = set(
                config.pos_config_id.trusted_config_ids.ids
            ) - set(config.pos_trusted_config_ids.ids)
    ...
    def _onchange_epson_printer_ip(self):
        for rec in self:
            if rec.pos_epson_printer_ip:
                rec.pos_epson_printer_ip = format_epson_certified_domain(
                    rec.pos_epson_printer_ip
                )
