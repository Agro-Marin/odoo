...
class ProductTemplate(models.Model):
    ...
    def create_product_variant_from_pos(self, attribute_value_ids, config_id):
        ...
        return {
            "product.product": product_variant.read(
                self.env["product.product"]._load_pos_data_fields(pos_config),
                load=False,
            ),
        }
    ...
    def load_product_from_pos(self, config_id, domain, offset=0, limit=0):
        ...
        combo_read = self.env["product.combo"]._load_pos_data_read(
            combo_records, config
        )
        ...
        combo_item_read = self.env["product.combo.item"]._load_pos_data_read(
            combo_item_records, config
        )
        ...
        product_tmpl_attr_value = (
            product_tmpls.attribute_line_ids.product_template_value_ids
        )
        product_tmpl_attr_value_read = product_tmpl_attr_value._load_pos_data_read(
        ...
        product_tmpl_exclusion = (
            ...
            + product_tmpl_excl.search(
                [
                    ("product_tmpl_id", "in", product_tmpls.ids),
                ]
            )
        )
        product_tmpl_exclusion_read = product_tmpl_excl._load_pos_data_read(
        ...
        if barcode_in_domain:
            barcode = [
                ...
                if "barcode" in condition.field_expr
            ]
        ...
        tax_read = account_tax._load_pos_data_read(
    ...
    def _load_pos_data_search_read(self, data, config):
        ...
        if limit_count and pos_limited_loading:
            query = self._search(
                self._load_pos_data_domain(data, config), bypass_access=True
            )
            sql = SQL(
                """
                    WITH pm AS (
        ...
        if data.get("pos.order.line"):
            products += (
                ...
                .browse([l["product_id"] for l in data["pos.order.line"]])
                .product_tmpl_id
            )
    ...
    def _process_pos_ui_product_product(self, products, config_id):
        ...
        for product in products:
            if different_currency:
                product["list_price"] = self.env.company.currency_id._convert(
                    product["list_price"],
            ...
            if len(taxes_by_company) > 1 and len(product["taxes_id"]) > 1:
                product["taxes_id"] = filter_taxes_on_company(
                    product["taxes_id"], taxes_by_company
                )
    ...
    def _check_is_special_product(self):
        special_products = (
            self.env["pos.config"].sudo()._get_special_products().product_tmpl_id
        )
        for product in self:
    ...
    def get_product_info_pos(
        ...
        for tax in taxes["taxes"]:
            if tax["id"] in grouped_taxes:
                grouped_taxes[tax["id"]]["amount"] += (
                    tax["amount"] / quantity if quantity else 0
                )
            else:
        ...
        price_per_pricelist_id = (
            pricelists._price_get(template_or_variant, quantity)
            if pricelists
            else False
        )
        pricelist_list = [
            {"name": pl.name, "price": price_per_pricelist_id[pl.id]}
        ...
        for _key, group in groupby(sorted(self.seller_ids, key=key), key=key):
            for s in group:
                if not (
                    (s.date_start and s.date_start > date.today())
                    or (s.date_end and s.date_end < date.today())
                    or (s.min_qty > quantity)
                ):
