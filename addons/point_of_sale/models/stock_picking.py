    ...
    def _check_active(self):
        for picking_type in self:
            ...
            pos_config = (
                self.env["pos.config"]
                .sudo()
                .search([("picking_type_id", "=", picking_type.id)], limit=1)
            )
            if pos_config:
...
class StockMove(models.Model):
    ...
    def _create_production_lots_for_pos_order(self, lines):
        ...
        if moves:
            ...
            lots = lines.pack_lot_ids.filtered(
                lambda l: l.lot_name and l.product_id.id in moves_product_ids
            )
            ...
            if moves:
                moves_product_ids = set(moves.mapped("product_id").ids)
                missing_lot_values = []
                for lot_product_id, lot_name in filter(
                    lambda l: l[0] in moves_product_ids, lots_data
                ):
    ...
    def _add_mls_related_to_order(self, related_order_lines, are_qties_done=True):
        ...
        moves_to_assign = self.filtered(
            lambda m: (
                ...
                or (
                    not m.picking_type_id.use_existing_lots
                    and not m.picking_type_id.use_create_lots
                )
            )
        )
        ...
        for move in moves_to_assign.filtered(
            ...
            converted_qty = move.product_uom_id._compute_quantity(
        ...
        existing_lots = moves_remaining._create_production_lots_for_pos_order(
            related_order_lines
        )
            ...
            for move in moves_remaining:
                ...
                for line in lines_data[move.product_id.id]["order_lines"]:
                    for lot in line.pack_lot_ids.filtered(lambda l: l.lot_name):
                        qty = (
                            1 if line.product_id.tracking == "serial" else abs(line.qty)
                        )
                        if existing_lots:
                            ...
                            if qty_left_to_assign > 0:
                                ml_vals = dict(
                                    move._prepare_move_line_vals(qty_left_to_assign)
                                )
                                ml_vals.update(
                                    {
                        ...
                        if existing_lots:
                            ...
                            if existing_lot:
                                move._update_reserved_quantity(
                                    qty, move.location_id, lot_id=existing_lot
                                )
                                continue
