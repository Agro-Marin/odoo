...
class HrLeave(models.Model):
    ...
    def _l10n_in_is_full_day_request(self, hours=None, default_hours=None):
        ...
        if self.leave_type_request_unit == "hour":
            return (
                bool(default_hours)
                and float_compare(hours, default_hours, precision_digits=2) >= 0
            )
        if (
            self.request_date_from_period != self.request_date_to_period
            and (
                self.request_date_from_period != "pm"
                or self.request_date_to_period != "am"
            )
    ...
    def _l10n_in_check_optional_holiday_request_dates(self):
        ...
        optional_holidays = dict(
            self.env["l10n.in.hr.leave.optional.holiday"]._read_group(
    ...
    def _l10n_in_is_working(self, on_date, public_holiday_dates, resource_calendar):
        return on_date not in public_holiday_dates and resource_calendar._works_on_date(
            on_date
        )
    ...
    def _l10n_in_count_adjacent_non_working(
        ...
        count = 0
        while (
            not self._l10n_in_is_working(
                current, public_holiday_dates, resource_calendar
            )
            and count < 30
        ):
            count += 1
    ...
    def _l10n_in_find_linked_leave(
        ...
        for _ in range(30):
            ...
            if self._l10n_in_is_working(
                current_date, public_holiday_dates, resource_calendar
            ):
                break
    ...
    def _l10n_in_get_linked_leaves(
        self, leaves_dates_by_employee, public_holidays_date_by_company
    ):
        linked_before = self.env["hr.leave"]
        linked_after = self.env["hr.leave"]

        for leave in self:
            public_holiday_dates = public_holidays_date_by_company.get(
                leave.company_id, {}
            )
    ...
    def _l10n_in_prepare_sandwich_context(self):
        ...
        indian_leaves = self.filtered(
            lambda leave: (
                leave.company_id.country_id.code == "IN"
        ...
        for emp_id, recs in grouped_leaves:
            valid_recs = recs.filtered(
                lambda leave: leave._l10n_in_is_full_day_request()
            )
            if not valid_recs:
                continue
        ...
        public_holidays_dates_by_company = {
            company_id: {
                (
                    datetime.date(holiday.date_from.astimezone(tz))
    ...
    def _l10n_in_apply_sandwich_rule(
        self, public_holidays_date_by_company, leaves_dates_by_employee
    ):
        self.check_singleton()
        ...
        is_non_working_from = not self._l10n_in_is_working(
            date_from, public_holiday_dates, self.resource_calendar_id
        )
        is_non_working_to = not self._l10n_in_is_working(
            date_to, public_holiday_dates, self.resource_calendar_id
        )

        if (
            is_non_working_from
            and is_non_working_to
            and not any(
                self._l10n_in_is_working(
        ...
        linked_before, linked_after = self._l10n_in_get_linked_leaves(
            leaves_dates_by_employee, public_holidays_date_by_company
        )
        ...
        has_previous_link = bool(
            linked_before_leave and linked_before_leave.request_date_from < date_from
        )
        has_next_link = bool(
    ...
    def _get_durations(self, check_leave_type=True, resource_calendar=None):
        ...
        indian_leaves, leaves_dates_by_employee, public_holidays_date_by_company = (
            self._l10n_in_prepare_sandwich_context()
        )
        if not indian_leaves:
        ...
        for leave in indian_leaves:
            ...
            if updated_days and updated_days != leave_days:
                updated_hours = (
                    (updated_days * (hours / leave_days)) if leave_days else hours
                )
    ...
    def _l10n_in_update_neighbors_duration_after_change(self):
        indian_leaves, leaves_dates_by_employee, public_holidays_dates_by_company = (
            self._l10n_in_prepare_sandwich_context()
        )
        if not indian_leaves:
            return
        ...
        for neighbor in neighbors:
            base_days, base_hours = base_map.get(
                neighbor.id, (neighbor.number_of_days, neighbor.number_of_hours)
            )
            default_hours = neighbor._l10n_in_get_default_leave_hours()
            if not neighbor._l10n_in_is_full_day_request(
                hours=base_hours, default_hours=default_hours
            ):
                neighbor.write(
                    {
            ...
            if updated_days and updated_days != base_days:
                new_hours = (
                    (updated_days * (base_hours / base_days))
                    if base_days
                    else base_hours
                )
                neighbor.write(
