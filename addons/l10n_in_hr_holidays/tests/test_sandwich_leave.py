...
class TestSandwichLeave(TransactionCase):
    def setUp(self):
        ...
        self.indian_company = self.env["res.company"].create(
            {"name": "Test Indian Company", "country_id": self.env.ref("base.in").id}
        )
        self.env = self.env(
            context=dict(self.env.context, allowed_company_ids=self.indian_company.ids)
        )
        self.demo_user = (
            self.env["res.users"]
            .with_company(self.indian_company)
            .create(
                {
                    "login": "piyush",
                    "name": "piyush_demo",
        ...
        self.leave_type_day, self.leave_type_half_day, self.leave_type_hours = self.env[
            "hr.leave.type"
        ].create(
            [
                {
    ...
    def test_approved_leave_does_not_raise_access_error(self):
        ...
        self.assertIsNotNone(
            approved_leave.with_user(self.demo_user).leave_type_increases_duration
        )
    ...
    def test_long_sandwich_leave(self):
        ...
        self.assertEqual(
            holiday_leave.number_of_days, 2, "The total leaves should be 2"
        )
