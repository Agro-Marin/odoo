...
def get_cloud_storage_azure_user_delegation_key(env):
    ...
    """
    cached_config, cached_user_delegation_key = CloudStorageAzureUserDelegationKeys.get(
    ...
    db_config["sequence"] = int(
        ICP.get_param("cloud_storage_azure_user_delegation_key_sequence", 0)
    )
    if db_config == cached_config:
        ...
        if cached_user_delegation_key:
            expiry = datetime.strptime(
                cached_user_delegation_key.signed_expiry, "%Y-%m-%dT%H:%M:%SZ"
            ).replace(tzinfo=UTC)
            if expiry > datetime.now(UTC) + timedelta(days=1):
    ...
    try:
        ...
        CloudStorageAzureUserDelegationKeys[env.registry.db_name] = (
            db_config,
            user_delegation_key,
        )
...
class IrAttachment(models.Model):
    ...
    def _get_cloud_storage_azure_info(self):
        ...
        if not match:
            raise ValidationError(
                self.env._("%s is not a valid Azure Blob Storage URL.", self.url)
            )
        return {
    ...
    def _generate_cloud_storage_download_info(self):
        ...
        expiry = datetime.now(UTC) + timedelta(
            seconds=self._cloud_storage_download_url_time_to_expiry
        )
        return {
    ...
    def _generate_cloud_storage_upload_info(self):
        ...
        info = self._get_cloud_storage_azure_info()
        expiry = datetime.now(UTC) + timedelta(
            seconds=self._cloud_storage_upload_url_time_to_expiry
        )
        url = self._generate_cloud_storage_azure_sas_url(
            **info, permission="c", expiry=expiry
        )
        return {
            "url": url,
            "method": "PUT",
