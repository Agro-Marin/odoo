...
class TestCloudStorageAzureCommon(TransactionCase):
    def setUp(self):
        ...
        self.env["ir.config_parameter"].set_param(
            "cloud_storage_azure_account_name", self.DUMMY_AZURE_ACCOUNT_NAME
        )
        self.env["ir.config_parameter"].set_param(
            "cloud_storage_azure_tenant_id", self.DUMMY_AZURE_TENANT_ID
        )
        self.env["ir.config_parameter"].set_param(
        ...
        self.env["ir.config_parameter"].set_param(
            "cloud_storage_azure_container_name", self.container_name
        )
        ...
        self.DUMMY_USER_DELEGATION_KEY.signed_start = datetime.now(UTC).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
        self.DUMMY_USER_DELEGATION_KEY.signed_expiry = (
            datetime.now(UTC) + timedelta(days=7)
        ).strftime("%Y-%m-%dT%H:%M:%SZ")
        self.DUMMY_USER_DELEGATION_KEY.signed_service = "b"
        self.DUMMY_USER_DELEGATION_KEY.signed_version = "2023-11-03"
        self.DUMMY_USER_DELEGATION_KEY.value = (
            "KEHG9q+1y6XGLHkDNv3pR2DhmbOfxeTf5KAJ5/ssNpU="
        )

        self.DUMMY_USER_DELEGATION_KEY_XML = bytes(
...
class TestCloudStorageAzure(TestCloudStorageAzureCommon, MockEmail):
    def test_get_user_delegation_key_success(self):
        ...
        with patch(
            ...
            self.assertEqual(
                request_num,
                2,
                "user_delegation_key should be reused if configuration is not changed",
            )
            ...
            self.env["ir.config_parameter"].set_param(
                "cloud_storage_azure_account_name", "newaccountname2"
            )
            self.env.registry.clear_cache()
            get_cloud_storage_azure_user_delegation_key(self.env)
            self.assertEqual(
    ...
    def test_get_user_delegation_key_wrong_info(self):
        ...
        self.assertEqual(
            request_num,
            1,
            "1 request to validate the validity of the client id and tenant id",
        )
    ...
    def test_get_user_delegation_key_wrong_secret(self):
        ...
        with (
            ...
            get_cloud_storage_azure_user_delegation_key(self.env)
        self.assertEqual(
            request_num, 1, "1 request to validate the validity of the secret"
        )
        ...
        self.assertEqual(
            request_num,
            1,
            "401 response should be cached in case the secret is expired",
        )
    ...
    def test_generate_sas_url(self):
        with patch(
            "odoo.addons.cloud_storage_azure.models.ir_attachment.get_cloud_storage_azure_user_delegation_key",
    ...
    def test_azure_url_validation(self):
        ...
        attachment.url = mk_url(
            account_name="admin4lyfe", container_name="1-c-o-n-t-a-i-n-e-r"
        )
        self.assertDictEqual(
            attachment._get_cloud_storage_azure_info(),
            {
    ...
    def test_cloud_storage_attachments(self):
        ...
        thread_model = self.env["res.partner"].create(
            {"name": "Cloud Test Partner", "email": "cloud@test.com"}
        )
        ...
        for mail in self._mails:
            ...
            self.assertIn(
                small_attachment.name,
                str(mail["attachments"]),
        ...
        for body, attachment in zip(
            ...
            cloud_attachment2_present = (
                body.count(cloud_attachment2.access_token)
                == body.count(cloud_attachment2.name)
                == 1
            )
            large_attachment_link = str(
                self.env["ir.qweb"]._render(
                    "mail.mail_attachment_links", {"attachments": large_attachment}
                )
            )
            self.assertTrue(
                body.count(large_attachment_link) == 1
                and cloud_attachment_present
                and cloud_attachment2_present,
    ...
    def test_uninstall_fail(self):
        with self.assertRaises(
            UserError,
            msg="Don't uninstall the module if there are Azure attachments in use",
        ):
            attachment = self.env["ir.attachment"].create(
                [
                    {
    ...
    def test_uninstall_success(self):
        ...
        self.assertFalse(
            self.env["ir.config_parameter"].get_param("cloud_storage_provider")
        )
        self.assertFalse(
        ...
        self.assertFalse(
            self.env["ir.config_parameter"].get_param("cloud_storage_azure_tenant_id")
        )
        self.assertFalse(
