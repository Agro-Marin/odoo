...
class EventRegistration(models.Model):
    ...
    @api.depends(
        "sale_order_id.state", "sale_order_id.currency_id", "sale_order_id.amount_total"
    )
    def _compute_registration_status(self):
        for sale_order, registrations in (
            ...
            if float_is_zero(
                ...
                sold_registrations = (
                    registrations.filtered(
                        lambda reg: reg.sale_order_id.state == "done"
                    )
                    - cancelled_registrations
                )
        ...
        for registration in registrations:
            if registration.sale_order_id:
                registration.message_post_with_source(
                    ...
                    render_values={
                        "self": registration,
                        "origin": registration.sale_order_id,
                    },
        ...
        res.update(
            {
                ...
                "sale_status_value": self.sale_status
                and dict(self._fields["sale_status"]._description_selection(self.env))[
                    self.sale_status
                ],
