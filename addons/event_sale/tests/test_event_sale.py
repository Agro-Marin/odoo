...
class TestEventSale(TestEventSaleCommon):
    ...
    def setUpClass(cls):
        ...
        cls.customer_so = (
            cls.env["sale.order"]
            .with_user(cls.user_sales_salesman)
            .create(
                {
    ...
    def test_adjusted_quantity_in_sale_order(self):
        ...
        editor = (
            ...
            .with_context({"default_sale_order_id": customer_so.id})
            .create({})
        )
        ...
        self.assertListEqual(
            actual_states,
            expected_states,
            "One of the registrations should be cancelled.",
        )
    ...
    def test_event_crm_sale(self):
        ...
        ticket1_line = customer_so.line_ids.filtered(
            lambda line: line.event_ticket_id == ticket1
        )
        ticket2_line = customer_so.line_ids.filtered(
            lambda line: line.event_ticket_id == ticket2
        )
        self.assertEqual(
            customer_so.amount_untaxed, TICKET1_COUNT * 10 + TICKET2_COUNT * 50
        )
        ...
        self.assertEqual(
            len(editor.event_registration_ids), TICKET1_COUNT + TICKET2_COUNT
        )
        self.assertEqual(editor.sale_order_id, customer_so)
        self.assertEqual(
            editor.event_registration_ids.sale_order_line_id,
            ticket1_line | ticket2_line,
        )

        # check line linked to existing registration (ticket1_reg1)
        ticket1_editor_reg1 = editor.event_registration_ids.filtered(
            lambda line: line.registration_id
        )
        for field in ["name", "email", "phone_ids"]:
        ...
        ticket1_editor_other[0].write(
            {
                ...
                "phone_ids": [
                    Command.create({"number": "+32456111111", "type": "landline"})
                ],
            }
        )
        ...
        self.assertEqual(
            len(self.event_0.registration_ids), TICKET1_COUNT + TICKET2_COUNT
        )
        new_registrations = self.event_0.registration_ids - ticket1_reg1
        self.assertEqual(new_registrations.sale_order_id, customer_so)
        ticket1_new_reg = new_registrations.filtered(
            lambda reg: reg.event_ticket_id == ticket1
        )
        ticket2_new_reg = new_registrations.filtered(
            lambda reg: reg.event_ticket_id == ticket2
        )
        self.assertEqual(len(ticket1_new_reg), 2)
        self.assertEqual(len(ticket2_new_reg), 1)
        self.assertEqual(
            set(ticket1_new_reg.mapped("name")), {"ManualEntry1", "ManualEntry2"}
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ticket2_new_reg.phone_ids, self.event_customer._phone_get_number()
        )
        ...
        self.assertEqual(
            customer_so.amount_untaxed, TICKET1_COUNT * 10 + (TICKET2_COUNT + 2) * 50
        )
        ...
        new_registrations = self.event_0.registration_ids - (
            ticket1_reg1 | ticket1_new_reg | ticket2_new_reg
        )
    ...
    def test_event_sale_free_confirm(self):
        ...
        # Limiting seats
        self.event_0.write({"seats_limited": True, "seats_max": 5})
        ...
        self.assertTrue(
            all(reg.state == "open" for reg in self.event_0.registration_ids)
        )
    ...
    def test_event_sale_free_full_event_no_confirm(self):
        ...
        self.event_0.write({"seats_limited": True, "seats_max": 2})
    ...
    def test_compute_sale_status(self):
        ...
        self.assertEqual(
            registration.sale_status,
            "to_pay",
            "Free SO line on SO with non zero amount should be paid",
        )
        registration.sale_order_id.amount_total = 0.0
        self.assertEqual(
            registration.sale_status,
            "free",
            "Free SO line on SO with zero amount should be free",
        )
