...
from odoo.addons.iot_drivers.iot_handlers.interfaces.SocketInterface import (
    socket_devices,
)
    ...
    _const = type(
        ...
        (),
        {
            ...
            "tagType": [
                ...
                IngenicoTagType(
                    "CurrencyExponent", "52", "i", 2
                ),  # Named Decimal in IDD.
                ...
                IngenicoTagType(
                    "Group_TableRecord", "EF", "REC", False
                ...
                IngenicoTagType(
                    "SendOrDelete", "C2", "", False
                ),  # used# for# pending# messages# :# 1=send# 2=delete
                IngenicoTagType(
                    "Group_AcquirerList", "E7", "", False
                ),  # group: AcquirerIdentifier=0xDF68, AcquirerLabelName=0xDF69
                IngenicoTagType(
                    "Group_SecuritySchemeList", "BF01", "", False
...
class OutgoingIngenicoMessage(IngenicoMessage):
    ...
    def _generateTag(cls, tagName, content):
        ...
        if tag.len:
            return (
                unhexlify(tag.tag)
                + chr(tag.len).encode()
                + cls._withLength(content, tag.len)
            )
    ...
    def __init__(
        self, dev, terminalId, ecrId, protocolId, messageType, sequence, **kwargs
    ):
    ...
    def _generateBody(self, messageTypeId):
        ...
        if messageTypeId == messageTypes["HelloResponse"]:
            innerBody = self._generateTag(
                "ResultCode", self.resultCode
            ) + self._generateTag(
                "Group_ConnectionParameters",
                self._generateTag(
                    "KeepAliveInterval",
            ...
            innerBody = self._generateTag(
                "KeepAliveReason", self.reason
            ) + self._generateTag("ResultCode", self.resultCode)
        elif messageTypeId == messageTypes["TransactionRequest"]:
            innerBody = (
                self._generateTag(
                    "TransactionId", unhexlify(f"{int(self.transactionId):016x}")
                )
                + self._generateTag(
                    "Group_TransactionData",
                    self._generateTag(
                        "Amount", int(str(self.amount), 16).to_bytes(4, byteorder="big")
                    ),
                )
                + self._generateTag(
                    "Group_PrintData", self._generateTag("PrintOrigin", b"\x02")
                )
            )
        elif messageTypeId == messageTypes["CancelRequest"]:
            innerBody = self._generateTag(
                "CancelReason", self._const.cancelReasons[self.reason]
            )
...
class IncomingIngenicoMessage(IngenicoMessage):
    ...
    def _getMsg(self, length):
        ...
        if tag["format"] in ["GRP", "REC"]:
            ...
            while xMsgLength > 0:
                ...
                if xTag["format"] != "REC":
                    _logger.warning(
                        "Expected REC field but got %s with tag %s",
                        xTag["format"],
                        xTag["tag"],
                    )
    ...
    def getTransactionTicket(self):
        ...
        if "Group_TicketData" in self._tagTree["msg"]["Group_Body"]:
            ...
            ticket_bodies = [
                r["TicketBody"] for r in ticket_data if r.get("TicketBody")
            ]
            return b"\n".join(ticket_bodies)
        return False
...
class IngenicoDriver(Driver):
    ...
    def supported(cls, device):
        ...
        try:
            ...
            if (
                msg
                and msg.magic == b"P4Y-ECR!"
                and msg.getMessageType() == "HelloRequest"
            ):
                ...
                msg
                and msg.magic == b"P4Y-ECR!"
                and msg.getMessageType() == "KeepAliveRequest"
            ):
                device.terminalId = msg.getTerminalId()
                device.protocolId = msg.getProtocolId()
                OutgoingIngenicoMessage(
            ...
            while not self._stopped.is_set():
                ...
                if msg and msg.magic == b"P4Y-ECR!":
                    ...
                    if msgType == "KeepAliveRequest":
                        self._outgoingMessage(
                            "KeepAliveResponse", reason=msg.getKeepAliveReasonId()
                        )
