...
class PosOrder(models.Model):
    ...
    def _compute_previous_order_id(self):
        ...
        for company_id, orders in orders_by_company.items():
            ...
            prev_seq = [
                o.l10n_fr_secure_sequence_number - 1
                for o in orders
    ...
    def _compute_string_to_hash(self):
        ...
        def collect_sorted_relational_ids(
            orders_data, lines_data, order_field_defs, line_field_defs
        ):
        ...
        fields_to_fetch = list(
            set(ORDER_FIELDS_BEFORE_17_4) | set(ORDER_FIELDS_FROM_17_4)
        )
        ...
        order_field_defs = {
            field: {
                ...
                if hasattr(self._fields[field], "comodel_name")
                else None,
            }
        ...
        line_field_defs = {
            field: {
                ...
                "comodel": self.lines._fields[field].comodel_name
                if hasattr(self.lines._fields[field], "comodel_name")
                else None,
            }
            for field in LINE_FIELDS
        }

        sorted_relational_ids = collect_sorted_relational_ids(
            orders_data, lines_data, order_field_defs, line_field_defs
        )
        ...
        for order in self:
            ...
            for field in order_fields:
                ...
                values[field] = _getattrstring(
                    order_data.get(field), field_def["type"], field_def["comodel"]
                )
            ...
            for line in lines_by_order[order.id]:
                for field in LINE_FIELDS:
                    ...
                    values[k] = _getattrstring(
                        line.get(field), field_def["type"], field_def["comodel"]
                    )

            # make the json serialization canonical
            #  (https://tools.ietf.org/html/draft-staykov-hu-json-canonical-form-00)
            order.l10n_fr_string_to_hash = dumps(
                values,
                sort_keys=True,
                ensure_ascii=True,
                indent=None,
    ...
    def write(self, vals):
        ...
        for order in self:
            if order.company_id._is_accounting_unalterable():
                ...
                if order.pos_version:
                    ...
                    ORDER_FIELDS = ORDER_FIELDS_BEFORE_17_4
                if order.state in ["paid", "done"] and set(vals).intersection(
                    ORDER_FIELDS
                ):
                    raise UserError(
                        _(
        ...
        if has_been_posted:
            for order in self.filtered(
                lambda o: (
                    o.company_id._is_accounting_unalterable()
                ...
                res |= super(PosOrder, order).write(
                    {"l10n_fr_hash": order._get_new_hash()}
                )
        return res
...
class PosOrderLine(models.Model):
    ...
    def write(self, vals):
        ...
        if set(vals).intersection(LINE_FIELDS):
            if any(
                l.company_id._is_accounting_unalterable()
                and (l.order_id.account_move or l.order_id.state == "done")
                for l in self
            ):
                raise UserError(
                    _(
