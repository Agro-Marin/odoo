...
class TestBarcodeGS1Nomenclature(TransactionCase):
    ...
    def test_gs1_extanded_barcode_1(self):
        barcode_nomenclature = self.env["barcode.nomenclature"].browse(
    ...
    def test_gs1_extanded_barcode_2_decimal(self):
        ...
        barcode_rule = (
            self.env["barcode.rule"]
            .with_context(default_barcode_rule_vals)
            .create(
                {
    ...
    def test_preprocess_gs1_search_args_product(self):
        ...
        domain = [("barcode", "in", [])]
        res = self.env["barcode.nomenclature"]._preprocess_gs1_search_args(
            domain, ["product"]
        )
