...
import { defineDocumentsModels } from "./document_test_helpers.js";
...
test("systray open documents activity loads custom action", async function () {
    ...
    mockService("action", {
        async loadAction(xmlId) {
            if (xmlId === "document.document_action") {
                expect.step("load_document_action");
                return {
                    xml_id: "document.document_action",
                    res_model: "document.document",
        ...
        doAction(action, options) {
            if (action.xml_id === "document.document_action") {
    ...
    await click(".o-mail-ActivityGroup", { text: "document.document" });
