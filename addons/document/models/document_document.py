...
# the links that stand for a document's own "anyone with the link" setting
# (access_via_link), as opposed to links issued to a person
DOCUMENT_LINK_DOMAIN = (
    Domain("audience", "=", "anyone")
    & Domain("partner_id", "=", False)
    & Domain("cause", "in", ("share", "migration", "system"))
)
...
class DocumentsDocument(models.Model):
    ...
    link_date_to = fields.Datetime(
        string="Link Expires",
        compute="_compute_link_date_to",
        help="When the link anyone can use stops working; set when it is shared.",
    )
    ...
    def create(self, vals_list: list[ValuesType]) -> DocumentsDocument:
        ...
        if linked := documents.filtered(lambda d: d.access_via_link != "none"):
            linked._sync_document_links(cause="system")
    ...
    def write(self, vals: dict) -> bool:
        ...
        self._write_check_access_settings_before(vals)
        ...
        if "access_via_link" in vals:
            self._sync_document_links()
    ...
    def _write_check_access_settings_before(self, vals: dict) -> None:
        if self.env.su:
            return
        ...
        changed = [
            fname
            for fname in SHARE_USER_LOCKED_FIELDS
            if fname in vals
            and (
                fname == "access_ids"
                or any(document[fname] != vals[fname] for document in self)
            )
        ]
        if changed:
            _debug.logic(
    ...
    def action_rotate_document_token(self) -> bool:
        ...
        # a new link replaces the old one, which stops opening anything
        self._revoke_document_links("the link was rotated")
        self._sync_document_links()
        _debug.lifecycle("document_link_rotated", documents=self)
        return True

    # ------------------------------------------------------------------
    # The link anyone can use (access.link)
    # ------------------------------------------------------------------

    def _document_links(self) -> Any:
        return self.env["access.link"]._record_links(self, DOCUMENT_LINK_DOMAIN)

    def _revoke_document_links(self, reason: str) -> None:
        self.env["access.link"]._revoke_record_links(
            self, reason, domain=DOCUMENT_LINK_DOMAIN
        )
        self.invalidate_recordset(["access_token", "access_url", "link_date_to"])

    def _sync_document_links(
        self,
        date_to: Any = None,
        cause: str = "share",
        roles: dict[int, str] | None = None,
    ) -> None:
        """Hold each document's links to its access_via_link: a live link with
        that role when it is on, none when it is off. A document created shared
        (a payslip, a product sheet, a file uploaded into a shared folder) is
        the system's doing and keeps its link a year; one a user shares by hand
        30 days, unless they chose a date. `roles` (id: access_via_link) is
        given by code that holds base.privilege_manage_links for documents its
        user may not read."""
        Link = self.env["access.link"]
        if roles is None:
            documents = self.with_context(active_test=False).exists()
            roles = {document.id: document.access_via_link for document in documents}
        records = self.browse(list(roles))
        links_by_doc = records._document_links().grouped("res_id")
        turned_off = self.browse(
            [
                doc_id
                for doc_id, role in roles.items()
                if role == "none" and doc_id in links_by_doc
            ]
        )
        if turned_off:
            Link._revoke_record_links(
                turned_off, "link access turned off", domain=DOCUMENT_LINK_DOMAIN
            )
        for doc_id, role in roles.items():
            if role == "none":
                continue
            document = self.browse(doc_id)
            links = links_by_doc.get(doc_id, Link)
            changes = {
                **({"role": role} if any(link.role != role for link in links) else {}),
                **({"date_to": date_to} if date_to else {}),
            }
            if links and changes:
                Link._update_record_links(
                    document, changes, domain=DOCUMENT_LINK_DOMAIN
                )
            if not any(link._token() for link in links):
                # a link carried over from before can open, not be shown again
                Link._issue(
                    document,
                    role=role,
                    date_to=date_to or links[:1].date_to or None,
                    cause=cause,
                )
        records.invalidate_recordset(["access_token", "access_url", "link_date_to"])
    ...
    def _write_sync_moved_after(self, vals: dict, move: DocumentMovePlan) -> None:
        if move.new_parent_folder and (
            ...
            documents_to_sync._update_access_rights(
                access_internal=move.new_parent_folder.access_internal,
                access_via_link=move.new_parent_folder.access_via_link,
                partners={
    ...
    def _compute_access_token(self) -> None:
        # the token part is the live link's, shown again from the sealed
        # secret; a document without a link is addressed by its id, which
        # opens it to those who may read it and to nobody else
        saved = self.filtered(lambda document: isinstance(document.id, int))
        tokens = {}
        for link in saved._document_links().sorted("id"):
            if token := link._token():
                tokens[link.res_id] = token
        for document in self:
            document.access_token = (
                f"{tokens.get(document.id, '')}o{document.id or 0:x}"
            )

    def _compute_link_date_to(self) -> None:
        dates = {
            link.res_id: link.date_to
            for link in self.filtered("id")._document_links().sorted("id")
        }
        for document in self:
            document.link_date_to = dates.get(document.id, False)
    ...
    def _get_is_multipage(self) -> bool | None:
        ...
        if modules.module.current_test and not decoded.lstrip().startswith(b"%PDF"):
            # A test run renders reports as HTML (_is_pdf_rendering_enabled);
            # a document with no PDF header cannot have pages to count.
            _logger.info(
