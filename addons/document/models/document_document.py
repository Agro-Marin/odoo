...
class DocumentsDocument(models.Model):
    ...
    document_token = fields.Char(
        default=lambda __: new_document_token(),
        copy=False,
        required=True,
    )
    ...
    def create(self, vals_list: list[dict]) -> DocumentsDocument:
        ...
        for position, vals in enumerate(vals_list):
            ...
            if not self.env.su:
                if "document_token" in vals:
                    self._raise_document_token_write()
    ...
    def action_rotate_document_token(self) -> bool:
        ...
        _debug.lifecycle("document_token_rotated", documents=self)
        return True
