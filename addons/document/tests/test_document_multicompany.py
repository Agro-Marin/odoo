...
(
    MEMBER_VIEW,
    INTERNAL_VIEW,
    MEMBER_INTERNAL_VIEW,
    OWNER,
    MEMBER_VIEW_LINK_EDIT,
    INTERNAL_VIEW_LINK_EDIT,
) = range(6)
...
class TestDocumentsMulticompany(TransactionCaseDocuments):
    ...
    def setUpClass(cls):
        ...
        (
            cls.company_allowed,
            cls.company_other,
            cls.company_disabled,
            cls.company_archived,
        ) = cls.env["res.company"].create(
            [
                {"name": f"Company {name}"}
                for name in ["Allowed", "Other", "Disabled", "Archived"]
            ]
        )
        ...
        (
            cls.internal_user + cls.document_manager + cls.portal_user + cls.admin_user
        ).write(
            {
                "company_ids": [
        ...
        documents = (
            ...
            .create(
                [
                    ...
                    common
                    | {"name": "member + internal view", "access_internal": "view"},
                    common | {"name": "owner", "owner_id": user.id},
                    common
                    | {"name": "member view + link edit", "access_via_link": "edit"},
                    common
                    | {
                        "name": "internal view + link edit",
    ...
    def test_company_access_simple(self):
        ...
        self.folder_b.with_user(self.internal_user).with_company(
    ...
    def _test_company_with_user(self, cases, user):
        for company, expected_edit_indices, expected_view_indices in cases:
            with self.subTest(company_name=company.name):
                ...
                expected_view = Documents_with_ctx.browse(
                    docs[idx].id for idx in expected_view_indices
                )
                expected_edit = Documents_with_ctx.browse(
                    docs[idx].id for idx in expected_edit_indices
                )

                search_view = Documents_with_ctx.search(
                    [("id", "in", docs.ids), ("user_permission", "=", "view")]
                )
                with self.subTest(permission="view"):
                    self._assert_documents_equal(search_view, expected_view)

                search_edit = Documents_with_ctx.search(
                    [("id", "in", docs.ids), ("user_permission", "=", "edit")]
                )
                ...
                expected_user_permissions = [
                    "view"
                    if document in expected_view
                    else "edit"
                    if document in expected_edit
                    else "none"
    ...
    def test_company_access_manager(self):
        cases = [
            ...
            (
                self.company_other,
                [OWNER, MEMBER_VIEW_LINK_EDIT],
                [MEMBER_VIEW, MEMBER_INTERNAL_VIEW],
            ),
            (self.company_disabled, [], []),
    ...
    def test_company_access_internal(self):
        cases = [
            (
                self.env["res.company"],
                [OWNER, MEMBER_VIEW_LINK_EDIT, INTERNAL_VIEW_LINK_EDIT],
                [MEMBER_VIEW, INTERNAL_VIEW, MEMBER_INTERNAL_VIEW],
            ),
            (
                self.company_allowed,
                [OWNER, MEMBER_VIEW_LINK_EDIT, INTERNAL_VIEW_LINK_EDIT],
    ...
    def test_company_shortcut_mismatch(self):
        ...
        shortcut = member_view_link_edit.with_user(
            self.internal_user
        ).action_create_shortcut(location_user_folder_id="MY")
        ...
        self.assertEqual(shortcut.company_id, self.company_allowed)
        self.assertEqual(
            Documents_with_ctx.search([("id", "=", shortcut.id)]), shortcut
        )
    ...
    def test_company_access_inherit(self):
        ...
        folder = Documents_with_ctx.browse(
            self.env["document.document"]
            .create(
                {
                    "name": "Folder",
        ...
        document = Documents_with_ctx.browse(
            self.env["document.document"]
            .create(
                {
                    "name": "Document",
        ...
        self.assertTrue(
            self.env["document.document"].search([("id", "=", document.id)])
        )
    ...
    def test_folder_company_inheritance(self):
        ...
        self.assertFalse(
            self.env["document.document"].create(folder_a_child_vals).company_id
        )
        ...
        self.assertEqual(
            self.env["document.document"]
            .with_context(default_company_id=self.company_disabled.id)
            .create({"name": "doc without folder"})
            .company_id,
            self.company_disabled,
        )
        self.assertEqual(
            self.env["document.document"]
            .with_context(default_company_id=self.company_disabled.id)
            .create(folder_a_child_vals)
            .company_id,
            self.company_allowed,
        )
        self.assertEqual(
            self.env["document.document"]
            .create(folder_a_child_vals | {"company_id": self.company_disabled.id})
            .company_id,
            self.company_disabled,
        )
        ...
        restricted_document = self.env["document.document"].create(
            folder_a_child_vals
            | {
        ...
        self.assertEqual(
            restricted_document.with_user(self.doc_user).user_permission, "none"
        )
        ...
        self.assertFalse(
            restricted_document.company_id,
            "Inaccessible document shouldn't have been updated",
        )
        self.assertEqual(
            shortcut.company_id,
            self.company_allowed,
    ...
    def test_folder_move_company_propagation(self):
        self.assertFalse(
            (self.folder_a | self.folder_b | self.folder_b.children_ids).company_id
        )
        self.folder_a.company_id = self.company_allowed
        self.folder_b.folder_id = self.folder_a
        document_with_other_company = self.env["document.document"].create(
            {
                "name": "doc",
                "company_id": self.company_other.id,
            }
        )
        shortcut_with_other_company = (
            document_with_other_company.action_create_shortcut()
        )
        self.assertEqual(shortcut_with_other_company.company_id, self.company_other)
        self.assertEqual(
            (self.folder_b | self.folder_b.children_ids).company_id,
            self.company_allowed,
        )
        shortcut_with_other_company.folder_id = self.folder_a
        self.assertEqual(
            shortcut_with_other_company.company_id,
            self.company_other,
        ...
        self.assertFalse(document_without_company.company_id)
        document_without_company.write(
            {"folder_id": self.folder_b.id, "company_id": None}
        )
        self.assertFalse(document_without_company.company_id)
        document_without_company.write(
            {"folder_id": self.folder_a.id, "company_id": self.company_other.id}
        )
