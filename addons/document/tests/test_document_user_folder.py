...
class TestDocumentsUserFolder(TransactionCaseDocuments):
    ...
    def setUpClass(cls):
        ...
        cls.document_sys_admin = cls.env["res.users"].create(
            [
                {
                    "group_ids": [
                        Command.link(cls.env.ref("document.group_documents_system").id)
                    ],
                    "login": "doc_system",
        ...
        cls.sub_company_admin, cls.sub_drive_admin = (
            cls.env["document.document"]
            .sudo()
            .create(
                [
                    ...
                    for label, folder in (
                        ("Company", cls.company_folder),
                        ("Drive", cls.internal_drive),
                    )
                ]
            )
        )
        (cls.shared_doc_company_sub, cls.shared_doc_drive, cls.shared_doc_company) = (
            cls.env["document.document"].create(
                [
                    {
    ...
    def test_create_with_default_user_folder_id(self):
        ...
        no_folder = self.env["document.document"]
        ...
        cases = [
            ...
            (
                {
                    "name": "In folder A > A from folder_id in vals",
                    "folder_id": self.folder_a_a.id,
                },
                {
                    "folder_id": self.folder_a_a,
            ...
            (
                {
                    "name": "In folder A from user_folder_id in vals",
            ...
            (
                {
                    "name": "In My Drive from user_folder_id in vals",
                    "user_folder_id": "MY",
                },
                {
                    "folder_id": no_folder,
            ...
            (
                {
                    "name": "In My Drive from owner_id in vals",
        ...
        for vals, expected in cases:
            with self.subTest(vals=vals):
                doc = self.env["document.document"].with_context(context).create(vals)
                for key, value in expected.items():
                    self.assertEqual(doc[key], value)

    @users("internal_user")
    def test_create_write_user_folder_id(self):
        defaults, company, my, folder_a_b, folder_a_b_2 = (
    ...
    def test_search_child_of(self):
        my_subfolder, company_subfolder, sub_sub_company_admin = (
            self.env["document.document"]
            .sudo()
            .create(
                [
                    ...
                    {
                        "name": "Company Subfolder",
                        "type": "folder",
                        "folder_id": self.company_folder.id,
                    },
                    {
        ...
        sub_company_admin_folder_doc = self.env["document.document"].create(
            {
        ...
        cases = [
            (
                ...
                | self.folder_a_a
                | self.folder_b
                | self.shared_doc_company_sub
                | self.shared_doc_drive
                | self.shared_doc_company
                | sub_sub_company_admin
                | sub_company_admin_folder_doc,
            ),
        ...
        accessible_documents = self.env["document.document"].search([])
        for user_folder_id, expected in cases:
            with self.subTest(user_folder_id=user_folder_id):
                domain = [("user_folder_id", "child_of", user_folder_id)]
                actual = self.env["document.document"].search(domain)
    ...
    def test_search_folder_id_child_of(self):
        my_subfolder, company_subfolder = self.env["document.document"].create(
            [
                {"name": "My Subfolder", "folder_id": self.internal_drive.id},
                {
                    "name": "Company Subfolder",
        ...
        cases = [
            (
                self.internal_user,
                {
                    ...
                    False: self.company_restr_doc
                    | self.company_restr_folder
                    | self.sub_company_admin
                    | self.sub_drive_admin,
                },
            ),
            (
                self.doc_user,
                {
                    ...
                    | self.company_folder
                    | self.company_restr_doc,
                    "MY": self.folder_a | self.folder_b,
                    "SHARED": self.shared_doc_company_sub
                    | self.shared_doc_drive
                    | self.shared_doc_company,
                    "RECENT": self.folder_a | self.folder_a_a | self.folder_b,
                    str(self.folder_a.id): self.folder_a_a,
                    False: self.internal_drive
                    | self.sub_company_admin
                    | self.sub_drive_admin,
                },
            ),
            ...
            for user_folder_id, documents in expected.items():
                with self.subTest(user=user.name, user_folder_id=user_folder_id):
                    ...
                    if (
                        ...
                        self.assertEqual(
                            set(documents.with_user(user).mapped("user_folder_id")),
                            {user_folder_id},
                        )
    ...
    def test_search_user_folder_id_in_and_not_in(self):
        ...
        actual_shared = self.env["document.document"].search(
            [("user_folder_id", "in", "SHARED")]
        )
        self.assertDocumentsEqual(actual_shared, expected_shared)

        expected_not_shared = expected_all - expected_shared
        actual_not_shared = self.env["document.document"].search(
            [("user_folder_id", "not in", "SHARED")]
        )
        self.assertDocumentsEqual(actual_not_shared, expected_not_shared)

        expected_folder_a = self.folder_a_a
        actual_folder_a = self.env["document.document"].search(
            [("user_folder_id", "in", str(self.folder_a.id))]
        )
        self.assertDocumentsEqual(actual_folder_a, expected_folder_a)

        expected_not_folder_a = expected_all - expected_folder_a
        actual_not_folder_a = self.env["document.document"].search(
            [("user_folder_id", "not in", str(self.folder_a.id))]
        )
    ...
    def test_documents_search_panel(self):
        ...
        for context, expected in cases:
            with self.subTest(context=context):
                Documents = self.env["document.document"].with_context(context)
                actual = Documents.search_panel_select_range("user_folder_id")["values"]
                user_folder_ids = {
                    vals["display_name"]: vals.get("user_folder_id") for vals in actual
                }
    ...
    def test_numeric_user_folder_id_search_requires_a_reachable_folder(self):
        ...
        inside.action_update_access_rights(
            partners={self.internal_user.partner_id.id: ("view", False)}
        )
        as_internal = Document.with_user(self.internal_user)
        self.assertEqual(
            inside.with_user(self.internal_user).user_folder_id,
            "SHARED",
            "the containing folder is not reachable",
        )
        self.assertFalse(
            as_internal.search(
                [
                    ("id", "=", inside.id),
                    ("user_folder_id", "=", str(hidden_folder.id)),
                ]
            ),
            "an unreachable folder is not this document's virtual folder",
        )
