...
class TestDocumentsEmbeddedActions(TransactionCaseDocuments):
    ...
    def test_a_listable_action_still_pins_and_appears(self):
        ...
        self.env["document.document"].action_folder_embed_action(folder.id, action.id)
        listed = self.env["document.document"]._get_folder_embedded_actions(folder.ids)
        self.assertIn(
            action.id,
...
class TestDocumentsEmbeddedActionsGc(TransactionCaseDocuments):
    ...
    def test_gc_keeps_pins_of_actions_the_vacuum_user_cannot_see(self):
        ...
        self.env["document.document"].with_user(pinner).action_folder_embed_action(
            folder.id, action.id
        )
    ...
    def test_gc_still_removes_a_child_action_pin(self):
        ...
        self.env["document.document"].action_folder_embed_action(folder.id, child.id)
        pinned = self.env["ir.embedded.actions"].search(
            [
                ("parent_res_model", "=", "document.document"),
                ("parent_res_id", "=", folder.id),
