...
class TestDocumentRequest(MailCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.folder_a = cls.env["document.document"].create(
            {
                "name": "folder A",
                "type": "folder",
            }
    ...
    def test_request_document_from_partner_with_user(self):
        ...
        self.assertEqual(
            document.request_activity_id.user_id,
            self.doc_user,
            "Activity assigned to the requestee",
        )
        self.assertEqual(
            document.owner_id,
            self.user_employee,
            "Owner of the document is the requester",
        )
        self.assertSentEmail(
            '"Ernest Employee" <e.e@example.com>',
            self.doc_partner_1,
            subject="Document Request : Wizard Request",
        )
        ...
        self.assertEqual(
            access_doc_partner_1.expiration_date,
            datetime.combine(
                fields.Date.today() + relativedelta(days=3), datetime.max.time()
            ),
        )
        ...
        new_expiration_date = datetime.combine(
            fields.Date.today() + relativedelta(days=5), datetime.max.time()
        )
    ...
    def test_request_document_from_partner_without_user(self):
        ...
        self.assertEqual(
            document.request_activity_id.user_id,
            self.user_employee,
            "Activity assigned to the requester because the requestee has no user",
        )
        self.assertEqual(
            document.owner_id,
            self.user_employee,
        ...
        self.assertSentEmail(
            '"Ernest Employee" <e.e@example.com>',
            self.doc_partner_2,
            subject="Document Request : Wizard Request 2",
        )
    ...
    def test_send_message_with_attachment_on_doc_request(self):
        ...
        with (
            ...
            RecordCapturer(self.env["mail.message"], []) as capture,
        ):
    ...
    def test_request_document_upload_through_activity_popover(self):
        ...
        document.action_update_access_rights(
            partners={self.user_admin.partner_id: ("edit", False)}
        )
        ...
        with io.StringIO("Hello world!") as file:
            response = self.url_open(
                ...
                data={
                    ...
                    "thread_model": "document.document",
        ...
        self.assertTrue(
            data_attachment_id,
            "We should have the id of the attachment upload inside the response.",
        )
        self.assertEqual(
            data_attachment_id, response_content.get("data", {}).get("attachment_id")
        )
