...
class TestDocumentsDocumentFolder(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.parent_folder = cls.env["document.document"].create(
            {
                "name": "Parent",
                "type": "folder",
            }
        ...
        cls.child_folder = cls.env["document.document"].create(
            {
                "name": "Child",
                "type": "folder",
    ...
    def test_folder_archive_unlink(self):
        ...
        self.assertEqual(
            self.folders.exists(),
            self.parent_folder,
            "Folder and Child Folder should have been deleted",
        )
    ...
    def test_folder_create_subfolder(self):
        ...
        new_folder = self.env["document.document"].create(
            {
    ...
    def test_folder_copy(self):
        ...
        self.assertEqual(
            folder_copy_2.children_ids.children_ids.owner_id, self.user_portal
        )

    def test_folder_copy_embedded_actions(self):
        original_folder = self.env["document.document"].create(
            {
                "type": "folder",
                "name": "A Folder",
        ...
        server_action = self.env["ir.actions.server"].create(
            {
                ...
                "model_id": self.env.ref("document.model_document_document").id,
                "type": "ir.actions.server",
                "group_ids": self.env.ref("base.group_user").ids,
                "update_path": "folder_id",
                "usage": "documents_embedded",
                "state": "object_write",
                "resource_ref": f"document.document,{self.parent_folder.id}",
            }
        )
    ...
    def test_action_move_folder(self):
            ...
            [
                {
                    ...
                    "group_ids": [
                        Command.link(
                            self.env.ref("document.group_documents_manager").id
                        )
                    ],
                    "login": "dtdm",
        ...
        self.folder_cpy_1, self.folder_cpy_2, self.folder_cpy_3 = self.env[
            "document.document"
        ].create(
            [
                {
        ...
        for company_folder in self.company_folders:
            ...
            self.assertTrue(
                company_folder.with_user(self.document_manager).user_permission
                == "edit"
            )
            self.assertTrue(
                company_folder.with_user(self.internal_user).user_permission == "view"
            )

        self.assertTrue(
            self.folder_cpy_1.sequence
            < self.folder_cpy_2.sequence
            < self.folder_cpy_3.sequence
        )
        ...
        for my_drive_folder in self.my_drive_folders:
            self.assertTrue(
                my_drive_folder.with_user(self.document_manager).user_permission
                == "none"
            )
            self.assertTrue(
                my_drive_folder.with_user(self.internal_user).user_permission == "edit"
            )

        with self.assertRaises(AccessError):
            self.folder_my_2.with_user(self.document_manager).action_move_folder(
