...
class TestAttachmentAccess(TransactionCase):
        ...
        cls.user = cls.env["res.users"].create(
            {
                ...
                "email": "foo@bar.com",
                "group_ids": [
                    (6, 0, [cls.env.ref("document.group_documents_user").id])
                ],
            }
        )
        folder = cls.env["document.document"].create(
    ...
    def test_user_document_attachment_without_res_fields(self):
        ...
        document = env_user["document.document"].create(
    ...
    def test_create_shortcut(self):
        ...
        with self.assertRaises(AccessError):
            self.env["document.document"].with_user(self.user).create(
