...
class PosOrder(models.Model):
    ...
    use_self_order_online_payment = fields.Boolean(
        compute="_compute_use_self_order_online_payment", store=True, readonly=True
    )
    ...
    def get_order_to_print(self):
        ...
        # Lock the line
        self.env.cr.execute(
            "SELECT id FROM pos_order WHERE id = %s FOR UPDATE NOWAIT", (self.id,)
        )
    ...
    def _compute_use_self_order_online_payment(self):
        for order in self:
            order.use_self_order_online_payment = bool(
                order.config_id.self_order_online_payment_method_id
            )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if (
                "use_self_order_online_payment" not in vals
                or vals["use_self_order_online_payment"]
            ):
                session = self.env["pos.session"].browse(vals.get("session_id"))
                config = session.config_id
                vals = {
                    **vals,
                    "use_self_order_online_payment": bool(
                        config.self_order_online_payment_method_id
                    ),
                }
    ...
    def write(self, vals):
        ...
        if vals["use_self_order_online_payment"]:
            can_change_self_order_domain += [
                ("config_id.self_order_online_payment_method_id", "!=", False)
            ]

        can_change_self_order_orders = self.filtered_domain(
            can_change_self_order_domain
        )
        ...
        if cannot_change_self_order_orders:
            ...
            res = (
                super(PosOrder, cannot_change_self_order_orders).write(clean_vals)
                and res
            )
    ...
    @api.depends(
        ...
        "config_id.self_order_online_payment_method_id",
    ...
    def get_and_set_online_payments_data(self, next_online_payment_amount=False):
        ...
        if (
            "paid_order" not in res
            and not res.get("deleted", False)
            and not isinstance(next_online_payment_amount, bool)
        ):
