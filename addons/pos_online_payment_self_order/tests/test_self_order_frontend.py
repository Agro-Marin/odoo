...
class TestSelfOrderOnlinePayment(TestUi):
    ...
    def test_02_online_payment_with_multi_website_company(self):
        ...
        with patch.object(
            ...
            classmethod(patched_frontend_pre_dispatch),
        ):
            # Create a new order
            self.start_tour(
                pos_config._get_self_order_route(),
                "test_online_payment_self_multi_company",
                login="testuser",
            )
            draft_order = self.env["pos.order"].search(
                [("state", "=", "draft")], limit=1
            )
            payment_route = PaymentPortal._get_pay_route(
                draft_order.id, draft_order.access_token
            )

            # Test payment page access for different users
            self.start_tour(
                payment_route,
                "test_online_payment_self_multi_company_payment",
                login="testuser",
            )
            self.start_tour(
                payment_route,
            ...
            self.start_tour(
                payment_route, "test_online_payment_self_multi_company_payment"
            )
