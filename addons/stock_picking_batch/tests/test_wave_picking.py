...
class TestBatchPicking(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env["stock.move"].create(
            {
                "product_id": cls.productB.id,
                "product_uom_qty": 5,
                "product_uom_id": cls.productB.uom_id.id,
                "picking_id": cls.picking_client_1.id,
                "location_id": cls.stock_location.id,
                "location_dest_id": cls.customer_location.id,
            }
        ...
        cls.all_pickings = (
            cls.picking_client_1 | cls.picking_client_2 | cls.picking_client_3
        )
    ...
    def test_creation_from_pickings(self):
        ...
        action["context"] = {
            ...
            "active_ids": self.all_pickings.ids,
        }
    ...
    def test_add_to_existing_wave_from_lines(self):
        ...
        res_dict["context"] = {
            "active_model": "stock.move.line",
            "active_ids": self.picking_client_1.move_line_ids.ids,
        }
        wizard_form = Form.from_action(self.env, res_dict)
        wizard_form.mode = "new"
        wizard_form.user_id = self.user_demo
        ...
        res_dict["context"] = {
            "active_model": "stock.move.line",
            "active_ids": self.picking_client_2.move_line_ids.ids,
        }
        wizard_form = Form.from_action(self.env, res_dict)
        ...
        self.assertEqual(
            wave.picking_ids, self.picking_client_1 | self.picking_client_2
        )
    ...
    def test_wave_split_picking(self):
        lines = self.picking_client_1.move_ids.filtered(
        ...
        self.assertEqual(
            new_picking.date_planned, fields.Datetime.to_datetime("2025-01-01 00:00:00")
        )

    def test_wave_split_move(self):
        lines = self.picking_internal.move_ids.filtered(
            lambda m: m.product_id == self.productB
        ).move_line_ids[0:2]
        move = lines.move_id
        all_db_pickings = self.env["stock.picking"].search([])
    ...
    def test_wave_split_move_uom(self):
        ...
        for i in range(12):
            self.env["stock.quant"]._update_available_quantity(
                self.productB, self.stock_location, 1.0, lot_id=sns[i]
            )
        ...
        self.assertEqual(
            dozen_move.move_line_ids.product_uom_id,
    ...
    def test_wave_picking_vals_link_each_line_once(self):
        ...
        self.assertEqual(
            len(vals["move_line_ids"]),
            len(waved),
            "one link per line going to the wave, not one per (move, line)",
        )
        self.assertEqual(
            {command[1] for command in vals["move_line_ids"]},
            set(waved.ids),
    ...
    def test_wave_trigger_errors(self):
        ...
        with self.assertRaises(UserError):
            ...
            self.picking_client_2.company_id = self.env["res.company"].create(
                {"name": "Company 2"}
            )
    ...
    def test_not_assign_to_wave(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.productA, location, 5.0, lot_id=self.lots_p_a[0]
        )
        self.env["stock.quant"]._update_available_quantity(
            self.productA, location, 5.0, lot_id=self.lots_p_a[1]
        )
        ...
        self.assertTrue(
            ...
            not in [picking_1.id, move_line_to_wave.picking_id.id]
        )
    ...
    def test_validatation_of_partially_empty_picking(self):
        ...
        self.assertEqual(
            [picking_1.state, picking_1.move_ids.quantity, picking_1.move_ids.picked],
            ["done", 2.0, True],
        )
        self.assertEqual(
            [picking_2.state, picking_2.move_ids.quantity, picking_2.move_ids.picked],
            ["assigned", 0.0, False],
        )
    ...
    def test_add_partially_assigned_move_to_batch(self):
        ...
        self.assertRecordValues(
            picking.move_ids,
            [
                {"product_uom_qty": 15.0, "quantity": 10.0},
    ...
    def test_dont_add_empty_move_to_batch(self):
        ...
        self.assertRecordValues(
            picking.move_ids,
            [
                {"product_uom_qty": 15.0, "quantity": 0.0},
                {"product_uom_qty": 5.0, "quantity": 5.0},
            ],
        )
        lines = picking.move_ids.move_line_ids
    ...
    def test_validate_wave_with_zero_qty_picking(self):
        ...
        self.assertEqual(
            (picking_1 | picking_2).mapped("state"), ["assigned", "assigned"]
        )
        ...
        self.assertRecordValues(
            picking_1.move_ids,
            [{"state": "confirmed", "quantity": 0.0, "picked": False}],
        )
        self.assertRecordValues(
            picking_2.move_ids, [{"state": "done", "quantity": 3.0, "picked": True}]
        )
