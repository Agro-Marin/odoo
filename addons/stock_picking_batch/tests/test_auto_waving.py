...
class TestAutoWaving(TransactionCase):
    ...
    def test_group_by_partner_and_location(self):
        ...
        all_batches = self.env["stock.picking.batch"].search(
            [("picking_ids.partner_id", "in", self.demo_partners.ids)]
        )
        ...
        wave_1 = waves.filtered(
            lambda w: (
                w.description
                == f"{self.us_client.name}, {self.child_location_2.complete_name}"
            )
        )
        ...
        wave_2 = waves.filtered(
            lambda w: (
                w.description
                == f"{self.be_client.name}, {self.child_location_2.complete_name}"
            )
        )
        ...
        wave_3 = waves.filtered(
            lambda w: (
                w.description
                == f"{self.fr_client.name}, {self.child_location_2.complete_name}"
            )
        )
    ...
    def test_group_by_locations(self):
        ...
        wave_1 = waves.filtered(
            lambda w: w.description == self.child_location_1.complete_name
        )
        self.assertEqual(len(wave_1), 1)
        ...
        wave_2 = waves.filtered(
            lambda w: w.description == self.stock_location.complete_name
        )
        self.assertEqual(len(wave_2), 1)
    ...
    def test_group_by_country_and_product(self):
        ...
        wave_1 = waves.filtered(
            lambda w: w.description == f"United States, {self.product_1.name}"
        )
        self.assertEqual(len(wave_1), 1)
        self.assertEqual(len(wave_1.picking_ids), 2)
        ...
        wave_2 = waves.filtered(
            lambda w: w.description == f"United States, {self.product_2.name}"
        )
        self.assertEqual(len(wave_2), 1)
        ...
        wave_3 = waves.filtered(
            lambda w: w.description == f"United States, {self.product_3.name}"
        )
        self.assertEqual(len(wave_3), 1)
        self.assertEqual(len(wave_3.picking_ids), 1)
        self.assertEqual(len(wave_3.move_line_ids), 1)
        ...
        wave_4 = waves.filtered(
            lambda w: w.description == f"Belgium, {self.product_1.name}"
        )
        ...
        wave_5 = waves.filtered(
            lambda w: w.description == f"Belgium, {self.product_2.name}"
        )
        ...
        wave_6 = waves.filtered(
            lambda w: w.description == f"Belgium, {self.product_4.name}"
        )
        ...
        wave_7 = waves.filtered(
            lambda w: w.description == f"France, {self.product_1.name}"
        )
        ...
        wave_8 = waves.filtered(
            lambda w: w.description == f"France, {self.product_2.name}"
        )
    ...
    def test_auto_wave_skip_current_batch(self):
        ...
        wave_domain = [
            ("is_wave", "=", True),
            ("move_line_ids.product_id", "=", self.product_2.id),
            ("state", "=", "in_progress"),
        ]
