...
class StockMoveLine(models.Model):
        ...
        for picking_type, lines in self.grouped(lambda l: l.picking_type_id).items():
            if lines:
                ...
                if picking_type.batch_group_by_partner:
                    domains.append(
                        Domain(
                            "picking_ids.partner_id", "in", lines.move_id.partner_id.ids
                        )
                    )
                ...
                if picking_type.batch_group_by_src_loc:
                    domains.append(
                        Domain("picking_ids.location_id", "in", lines.location_id.ids)
                    )
                if picking_type.batch_group_by_dest_loc:
                    domains.append(
                        Domain(
                            ...
                            "in",
                            lines.location_dest_id.ids,
                        )
                    )
                if batches_to_validate_ids:
                    domains.append(Domain("id", "not in", batches_to_validate_ids))
                domains = lines._get_potential_existing_waves_extra_domain(
                    domains, picking_type
                )

                potential_waves = self.env["stock.picking.batch"].search(
                    Domain.AND(domains)
                )
                    ...
                    for wave in potential_waves:
                        for wave_location in reversed(picking_type.wave_location_ids):
                            if all(
                                ...
                                break
                    potential_waves = self.env["stock.picking.batch"].browse(
                        valid_wave_ids
                    )
                ...
                for line in lines:
                    ...
                    for wave in potential_waves:
                        ...
                        wave_picking_ids = set(
                            wave.move_line_ids.mapped("picking_id.id")
                        )
                        ...
                        if not wave._is_line_auto_mergeable(
                            line.move_id.id not in wave_move_ids
                            and line.move_id.id not in wave_new_move_ids
                            and len(wave_new_move_ids) + 1,
                            line.picking_id.id not in wave_picking_ids
                            and line.picking_id.id not in wave_new_picking_ids
                            and len(wave_new_picking_ids) + 1,
                            waves_new_extra_weight[wave]
    ...
    def _auto_wave_lines_into_new_waves(self, nearest_parent_locations=False):
        ...
        for picking_type in picking_types:
            ...
            if picking_type.batch_group_by_dest_loc:
                domains.append(
                    Domain("location_dest_id", "in", lines.location_dest_id.ids)
                )
            ...
            if picking_type.wave_group_by_location:
                domains.append(
                    Domain(
                        "location_id", "child_of", picking_type.wave_location_ids.ids
                    )
                )
            ...
            for line in lines:
                ...
                for potential_line in potential_lines:
                    if (
                        line.id == potential_line.id
                        or line.company_id != potential_line.company_id
                        or (
                            picking_type.batch_group_by_partner
                            and line.move_id.partner_id
                            != potential_line.move_id.partner_id
                        )
                        or (
                            picking_type.batch_group_by_destination
                            and line.move_id.partner_id.country_id
                        ...
                        or (
                            picking_type.batch_group_by_src_loc
                            and line.location_id != potential_line.location_id
                        )
                        or (
                            picking_type.batch_group_by_dest_loc
                            and line.location_dest_id != potential_line.location_dest_id
                        )
                        or (
                            picking_type.wave_group_by_product
                            and line.product_id != potential_line.product_id
                        )
                        or (
                            picking_type.wave_group_by_category
                            and line.product_id.categ_id
                            != potential_line.product_id.categ_id
                        )
                        or (
                            picking_type.wave_group_by_location
            ...
            for line, potential_line_ids in line_to_lines.items():
                ...
                potential_lines = self.env["stock.move.line"].browse(
                    potential_line_ids | {line.id}
                )
                    ...
                    new_wave = self.env["stock.picking.batch"].create(
                        {
                            "is_wave": True,
                            "picking_type_id": picking_type.id,
                            "description": line._get_auto_wave_description(
                                nearest_parent_locations[line]
                            ),
                        }
                    )
                    wave_move_ids = set()
