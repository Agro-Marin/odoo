...
class ResPartnerBank(models.Model):
    ...
    def _check_br_proxy(self):
        for bank in self.filtered(
            lambda bank: bank.country_code == "BR" and bank.proxy_type != "none"
        ):
            ...
            regex = r"%(char)s{8}-%(char)s{4}-%(char)s{4}-%(char)s{4}-%(char)s{12}" % {
                "char": "[a-fA-F0-9]"
            }
            if bank.proxy_type == "br_random" and not re.fullmatch(
    ...
    def _get_qr_code_vals_list(self, *args, **kwargs):
        ...
        if self.country_code == "BR":
            res[5] = (
                res[5][0],
                float_repr(res[5][1], 2) if res[5][1] else None,
            )  # amount
            ...
            if not res[9][1]:
                ...
                )  # default comment if none is set
        return res
