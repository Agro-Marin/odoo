        ...
        **kw,
        ...
        def get_timesheets():
            ...
            timesheets = Timesheet_sudo.search(
                domain, order=orderby, limit=items_per_page, offset=pager["offset"]
            )
            if field:
                if groupby == "date":
                    ...
                    grouped_timesheets = [
                        (records, unit_amount)
                        for __, unit_amount, records in raw_timesheets_group
                    ]

                else:
                    time_data = Timesheet_sudo._read_group(
                        domain, [field], ["unit_amount:sum"]
                    )
                    mapped_time = {
                        field.id: unit_amount for field, unit_amount in time_data
                    }
                    grouped_timesheets = [
            ...
            grouped_timesheets = (
                [
                    (
                        timesheets,
                        Timesheet_sudo._read_group(
                            domain, aggregates=["unit_amount:sum"]
                        )[0][0],
                    )
                ]
                if timesheets
...
class TimesheetProjectCustomerPortal(ProjectCustomerPortal):
    def _show_task_report(self, task_sudo, report_type, download):
        ...
        return self._show_report(
            ...
            report_ref="hr_timesheet.timesheet_report_task_timesheets",
            download=download,
        )
