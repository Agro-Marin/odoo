        ...
        cls.user_manager = cls.env["res.users"].create(
            {
                ...
                "group_ids": [
                    (6, 0, [cls.env.ref("hr_timesheet.group_timesheet_manager").id])
                ],
            }
        )
        # employees
...
class TestTimesheet(TestCommonTimesheet):
    ...
    def test_log_timesheet(self):
        ...
        self.assertEqual(
            timesheet1.account_id,
            self.project_customer.account_id,
            "Analytic account should be the same as the project",
        )
        self.assertEqual(
            timesheet1.employee_id,
            self.empl_employee,
            "Employee should be the one of the current user",
        )
        self.assertEqual(
            timesheet1.partner_id,
            self.task1.partner_id,
            "Customer of task should be the same of the one set on new timesheet",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            timesheet3.user_id,
            self.user_employee2,
            "Timesheet user should be the one linked to the given employee",
        )
        self.assertEqual(
            timesheet3.product_uom_id,
            timesheet_uom,
        ...
        self.assertEqual(
            timesheet4.partner_id,
            self.project_customer.partner_id,
            "Customer of new timesheet should be the same of the one set project (since no task on timesheet)",
        )
    ...
    def test_create_unlink_project(self):
        ...
        self.assertFalse(
            non_tracked_project.account_id,
        ...
        self.assertTrue(
            tracked_project.account_id,
            "A time-tracked project should generate an analytic account",
        )
        self.assertTrue(
            tracked_project.account_id.active,
            "A time-tracked project should generate an active analytic account",
        )
        self.assertEqual(
            tracked_project.partner_id,
            tracked_project.account_id.partner_id,
            "The generated AA should have the same partner as the project",
        )
        self.assertEqual(
            tracked_project.name,
            tracked_project.account_id.name,
            "The generated AA should have the same name as the project",
        )
        self.assertEqual(
            tracked_project.account_id.project_count,
            1,
        ...
        self.assertNotEqual(
            analytic_project.name,
            tracked_project.account_id.name,
            "The name of the associated AA can be different from the project",
        )
        self.assertEqual(
            tracked_project.account_id.project_count,
            2,
    ...
    def test_transfert_project(self):
        ...
        self.assertEqual(
            timesheet_count1, 3, "3 timesheets should be linked to Project1"
        )
        self.assertEqual(
            timesheet_count2, 0, "No timesheets should be linked to Project2"
        )
        self.assertEqual(
            len(self.task1.timesheet_ids), 1, "The timesheet should be linked to task1"
        )
        self.assertEqual(
            len(task_child.timesheet_ids),
            1,
            "The timesheet should be linked to task_child",
        )
        self.assertEqual(
            len(task_grandchild.timesheet_ids),
            1,
        ...
        self.assertEqual(
            timesheet_count1, 0, "No timesheets should be linked to Project1"
        )
        self.assertEqual(
            timesheet_count2, 3, "3 timesheets should be linked to Project2"
        )
        self.assertEqual(
            len(self.task1.timesheet_ids),
            1,
            "The timesheet still should be linked to task1",
        )
        self.assertEqual(
            len(task_child.timesheet_ids),
            1,
            "The timesheet still should be linked to task_child",
        )
        self.assertEqual(
            len(task_grandchild.timesheet_ids),
            1,
    ...
    def test_favorite_project_id(self):
        ...
        self.assertFalse(
            favorite_project,
            "A user without timesheet and without access to the internal project should have no favorite project.",
        )
    ...
    def test_recompute_partner_from_task_customer_change(self):
        ...
        self.assertEqual(
            timesheet_entry.partner_id,
            self.partner,
            "The timesheet entry's partner should be equal to the task's partner/customer",
        )
        ...
        self.assertEqual(
            timesheet_entry.partner_id,
            partner2,
            "The timesheet entry's partner should still be equal to the task's partner/customer, after the change",
        )
    ...
    def test_task_with_timesheet_project_change(self):
        ...
        task_1.with_user(project_manager).write({"project_id": second_project.id})

        self.assertEqual(
            timesheet.project_id,
            second_project,
            "The project_id of non-validated timesheet should have changed",
        )

    def test_compute_display_name(self):
        self.timesheet.with_user(
            self.timesheet_manager_no_project_user
    ...
    def test_create_timesheet_employee_not_in_company(self):
        ...
        Timesheet = self.env["account.analytic.line"].with_context(
            allowed_company_ids=[company_3.id, company_2.id, self.env.company.id]
        )
        ...
        self.assertEqual(
            ...
            "As there is a unique employee for this user, it must be found",
        )
    ...
    def test_create_timesheet_with_multi_company(self):
        ...
        Timesheet = self.env["account.analytic.line"].with_context(
            allowed_company_ids=[company_4.id, self.env.company.id]
        )
        ...
        with self.assertRaises(
            UserError, msg="The employee must be active to encode a timesheet"
        ):
            Timesheet.create(
                {
                    "project_id": self.project_customer.id,
    ...
    def test_subtask_log_timesheet(self):
        ...
        self.assertTrue(
            self.project_customer.allow_timesheets,
            "The project should be timesheetable",
        )
        self.assertEqual(
            subtask_1.allow_timesheets,
            self.project_customer.allow_timesheets,
            "The subtask should follow the settings of its project linked.",
        )
        Timesheet = self.env["account.analytic.line"]
        Timesheet.create(
            {
        ...
        self.assertEqual(
            self.task1.subtask_effective_hours,
            8,
    ...
    def test_ensure_product_uom_set_in_timesheet(self):
        self.assertFalse(
            self.project_customer.timesheet_ids,
            "No timesheet should be recorded in this project",
        )
        self.assertFalse(
            self.project_customer.total_timesheet_time,
            "The total time recorded should be equal to 0 since no timesheet is recorded.",
        )
    ...
    def test_create_timesheet_with_archived_employee(self):
        ...
        self.assertRaises(
            UserError, self.env["account.analytic.line"].create, batch_vals
        )
    ...
    def test_create_timesheet_with_companyless_analytic_account(self):
        ...
        timesheet_with_project = (
            self.env["account.analytic.line"]
            .with_user(self.user_employee)
            .create({"unit_amount": 1.0, "project_id": self.project_customer.id})
        )
        self.assertEqual(
            timesheet_with_project.product_uom_id,
            self.project_customer.company_id.project_time_mode_id,
        ...
        timesheet_with_task = (
            self.env["account.analytic.line"]
            .with_user(self.user_employee)
            .create({"unit_amount": 1.0, "task_id": self.task1.id})
        )
        self.assertEqual(
            timesheet_with_task.product_uom_id,
            self.task1.company_id.project_time_mode_id,
    ...
    def test_create_timesheet_with_default_employee_in_context(self):
        timesheet = (
            self.env["account.analytic.line"]
            .with_context(default_employee_id=self.empl_employee.id)
            .create(
                {
                    "project_id": self.project_customer.id,
                    "task_id": self.task1.id,
    ...
    def test_uom_change_timesheet(self):
        ...
        self.assertEqual(
            project_update_hrs.timesheet_time,
            8,
        ...
        self.assertEqual(
            project.total_timesheet_time, 1, "Total timesheet time should be 1 day"
        )
        project.allocated_hours = 0.0
        panel_data = project.get_panel_data()
        self.assertEqual(
            panel_data["buttons"][1]["number"], "1 Days", "Should display '1 Days'"
        )
        self.assertEqual(
            project.timesheet_encode_uom_id,
            self.env.company.timesheet_encode_uom_id,
            "Timesheet encode uom should be the one from the company of the env, since the project has no company.",
        )
        ...
        self.assertEqual(
            project_update_days.timesheet_time,
            1,
            "Timesheet time should be 1 day for new project update",
        )
    ...
    def test_percentage_of_allocated_hours(self):
        ...
        self.assertEqual(
            self.task1.progress, 1, "The progress of allocated hours should be 1."
        )
        ...
        def create_project_update():
            update_form = Form(
                self.env["project.update"].with_context(
                    {"default_project_id": self.project_customer.id}
                )
            )
            ...
            return [
                update_project.allocated_time,
                update_project.timesheet_time,
                update_project.timesheet_percentage,
            ]
    ...
    def test_calendar_display_name(self):
        ...
        self.assertEqual(
            timesheet.calendar_display_name, f"{self.project_customer.name} (1h)"
        )
        timesheet.unit_amount = 1.5
        timesheet.invalidate_recordset(["calendar_display_name"])
        self.assertEqual(
            timesheet.calendar_display_name, f"{self.project_customer.name} (1h30)"
        )
        timesheet.unit_amount = 8
        timesheet.company_id.timesheet_encode_uom_id = self.env.ref(
            "uom.product_uom_day"
        )
        timesheet.invalidate_recordset(["calendar_display_name"])
        self.assertEqual(
            timesheet.calendar_display_name, f"{self.project_customer.name} (1d)"
        )
        timesheet.unit_amount = 12
        timesheet.invalidate_recordset(["calendar_display_name"])
        self.assertEqual(
            timesheet.calendar_display_name, f"{self.project_customer.name} (1.5d)"
        )
    ...
    def test_multi_create_timesheets_from_calendar(self):
        ...
        with patch.object(self.env.registry["bus.bus"], "_sendone") as mock_send:
            ...
            def create_timesheet_records(days):
                return HrTimesheet.create(
                    [
                        ...
                        for day in days
                        for employee in (self.empl_employee, self.empl_employee2)
                    ]
                )
            ...
            self.assertEqual(
                fields.Date.to_string(timesheets[0].date),
                "2025-05-26",
                "The timesheet creation should skip non-working days",
            )
            self.assertEqual(
                fields.Date.to_string(timesheets[1].date),
                "2025-05-26",
                "The timesheet creation should skip non-working days",
            )
            assert_notification(
    ...
    def test_keep_create_account_values_at_timesheet_creation(self):
        ...
        self.assertEqual(
            line[field_name].id,
            analytic_account.id,
            f"The value of {field_name} shouldn't get overwritten by the project's account",
        )
    ...
    def test_keep_write_account_values_at_timesheet_update(self):
        ...
        line.write(
            {"project_id": self.project_customer.id, field_name: analytic_account.id}
        )
        self.assertEqual(
            line[field_name].id,
            analytic_account.id,
