...
class ResPartner(models.Model):
    ...
    def _compute_website_url(self):
        ...
        for partner in self:
            if partner.id:
                partner.website_url = "/partners/%s" % self.env["ir.http"]._slug(
                    partner
                )
    ...
    def _track_subtype(self, init_values):
        ...
        if "is_published" in init_values:
            if self.is_published:
                return self.env.ref(
                    "website_partner.mt_partner_published", raise_if_not_found=False
                )
            return self.env.ref(
                "website_partner.mt_partner_unpublished", raise_if_not_found=False
            )
