...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_es_edi_get_invoices_tax_details_info(
        self, invoice, filter_invl_to_apply=None
    ):
        ...
        def full_filter_invl_to_apply(invoice_line):
            if all(
                t == "ignore"
                for t in invoice_line.tax_ids.flatten_taxes_hierarchy().mapped(
                    "l10n_es_type"
                )
            ):
                return False
        ...
        for base_line in tax_details["base_lines"]:
            ...
            if recargo_tax and taxes:
                recargo_main_tax = taxes.filtered(
                    lambda x: x.l10n_es_type in ("sujeto", "sujeto_isp")
                )[:1]
                ...
                recargo_values = next(
                    iter(
                        ...
                        if (
                            values["grouping_key"]
                            and values["grouping_key"]["l10n_es_type"]
                            == recargo_tax.l10n_es_type
                            and values["grouping_key"]["applied_tax_amount"]
                            == recargo_tax.amount
                        )
                    )
                )
                recargo_tax_details[
                    recargo_main_tax.l10n_es_type, recargo_main_tax.amount
                ]["tax_amount"] += recargo_values["tax_amount"]
                recargo_tax_details[
                    recargo_main_tax.l10n_es_type, recargo_main_tax.amount
                ]["applied_tax_amount"] = recargo_values["applied_tax_amount"]
        ...
        for tax_values in tax_details["tax_details"].values():
            recargo = recargo_tax_details.get(
                (tax_values["l10n_es_type"], tax_values["applied_tax_amount"])
            )
            if invoice.is_sale_document():
                ...
                if tax_values["l10n_es_type"] in ("sujeto", "sujeto_isp"):
                    ...
                    tax_info = {
                        ...
                        "CuotaRepercutida": float_round(
                            math.copysign(tax_values["tax_amount"], base_amount), 2
                        ),
                    }

                    if recargo:
                        tax_info["CuotaRecargoEquivalencia"] = float_round(
                            sign * recargo["tax_amount"], 2
                        )
                        tax_info["TipoRecargoEquivalencia"] = recargo[
                            "applied_tax_amount"
                        ]
                    ...
                    tax_details_info["Sujeta"].setdefault(
                        "Exenta", {"DetalleExenta": []}
                    )
                    tax_details_info["Sujeta"]["Exenta"]["DetalleExenta"].append(
                        {
                            "BaseImponible": float_round(
                                sign * tax_values["base_amount"], 2
                            ),
                ...
                if tax_values["l10n_es_type"] not in [
                    "retencion",
                    "recargo",
                ]:  # = in sujeto/sujeto_isp/no_deducible
                    base_amount = sign * tax_values["base_amount"]
                    ...
                    if tax_values["l10n_es_type"] == "sujeto_agricultura":
                        tax_info.update(
                            {
                                "PorcentCompensacionREAGYP": tax_values[
                                    "applied_tax_amount"
                                ],
                                "ImporteCompensacionREAGYP": round(
                                    math.copysign(
                                        tax_values["tax_amount"], base_amount
                                    ),
                                    2,
                                ),
                            }
        ...
        if (
            tax_subject_isp_info_list and not tax_subject_info_list
        ):  # Only for sale_invoices
        ...
        if tax_subject_info_list:
            ...
            tax_details_info["Sujeta"]["NoExenta"]["DesgloseIVA"].setdefault(
                "DetalleIVA", []
            )
            tax_details_info["Sujeta"]["NoExenta"]["DesgloseIVA"]["DetalleIVA"] += (
                tax_subject_info_list
            )
        ...
        if (
            not invoice.company_id.currency_id.is_zero(base_amount_not_subject)
            and invoice.is_sale_document()
        ):
            tax_details_info["NoSujeta"]["ImportePorArticulos7_14_Otros"] = float_round(
                sign * base_amount_not_subject, 2
            )
        if (
            not invoice.company_id.currency_id.is_zero(base_amount_not_subject_loc)
            and invoice.is_sale_document()
        ):
            tax_details_info["NoSujeta"]["ImporteTAIReglasLocalizacion"] = float_round(
                sign * base_amount_not_subject_loc, 2
            )
        if not tax_details_info and invoice.is_sale_document():
            if any(
                t["l10n_es_type"] == "no_sujeto"
                for t in tax_details["tax_details"].values()
            ):
                tax_details_info["NoSujeta"]["ImportePorArticulos7_14_Otros"] = 0
            if any(
                t["l10n_es_type"] == "no_sujeto_loc"
                for t in tax_details["tax_details"].values()
            ):
    ...
    def _l10n_es_edi_get_invoices_info(self, invoices):
        ...
        for invoice in invoices:
            ...
            if invoice.is_sale_document():
                nif = invoice.company_id.vat.removeprefix("ES")
                ...
                invoice_node["ClaveRegimenEspecialOTrascendencia"] = (
                    invoice.invoice_line_ids.tax_ids._l10n_es_get_regime_code()
                )
            else:
            ...
            if invoice.is_sale_document():
                ...
                if tax_details_info_isp_vals["tax_details_info"]:
                    invoice_node["DesgloseFactura"]["InversionSujetoPasivo"] = (
                        tax_details_info_isp_vals["tax_details_info"]
                    )
                if tax_details_info_other_vals["tax_details_info"]:
                    invoice_node["DesgloseFactura"]["DesgloseIVA"] = (
                ...
                invoice_node["CuotaDeducible"] = float_round(
                    sign
                    * (
    ...
    def _l10n_es_edi_call_web_service_sign_common(
        ...
        try:
            ...
            elif invoices[0].is_sale_document():
        ...
        for respl in res.RespuestaLinea:
            ...
            if resp_line_state in ("Correcto", "AceptadoConErrores"):
                ...
                if resp_line_state == "AceptadoConErrores":
                    inv.message_post(
                        body=self.env._("This was accepted with errors: ")
    ...
    def _get_move_applicability(self, move):
        ...
        if move.l10n_es_edi_is_required:
            return {
                ...
                "post_batching": lambda invoice: (
                    invoice.move_type,
                    invoice.l10n_es_edi_csv,
                ),
    ...
    def _check_move_configuration(self, move):
        ...
        if not move.company_id.vat:
            res.append(
                self.env._(
                    "VAT number is missing on company %s", move.company_id.display_name
                )
            )
        ...
        for line in move.invoice_line_ids.filtered(
            ...
            if retention_count > 1:
                res.append(
                    self.env._(
                        "Line %s should only have one retention tax.", line.display_name
                    )
                )
            if recargo_count > 1:
                res.append(
                    self.env._(
                        "Line %s should only have one recargo tax.", line.display_name
                    )
                )
            if sujeto_count > 1:
                res.append(
                    self.env._(
                        "Line %s should only have one sujeto tax.", line.display_name
                    )
                )
            if no_sujeto_count > 1:
                res.append(
                    self.env._(
                        "Line %s should only have one no sujeto tax.", line.display_name
                    )
                )
            if no_sujeto_loc_count > 1:
                res.append(
                    self.env._(
                        "Line %s should only have one no sujeto (localizations) tax.",
                        line.display_name,
                    )
                )
            if sujeto_count + no_sujeto_loc_count + no_sujeto_count > 1:
                res.append(
                    self.env._(
                        "Line %s should only have one main tax.", line.display_name
                    )
                )
        if (
            move.is_inbound()
            and move.commercial_partner_id._l10n_es_is_foreign()
        ...
        if move.move_type in ("in_invoice", "in_refund"):
            if not move.ref:
                res.append(
                    self.env._(
                        "You should put a vendor reference on this vendor bill. "
                    )
                )
        return res
    ...
    def _l10n_es_edi_sii_send(self, invoices, cancel=False):
        ...
        if not cancel:  # retrocompatibility and mocks in tests
            ...
            res = self._l10n_es_edi_call_web_service_sign_common(
                invoices, info_list, cancel=True
            )
