...
class AccountMove(models.Model):
    ...
    def _compute_l10n_es_edi_is_required(self):
        for move in self:
            ...
            if move.is_purchase_document():
                ...
                has_tax = any(
                    t.l10n_es_type and t.l10n_es_type != "ignore" for t in taxes
                )
    ...
    def _l10n_es_is_dua(self):
        ...
        return any(
            t.l10n_es_type == "dua"
            for t in self.invoice_line_ids.tax_ids.flatten_taxes_hierarchy()
        )
    ...
    def _check_edi_documents_for_reset_to_draft(self):
        ...
        if (
            len(docs) == 1
            and docs.edi_format_id.code == "es_sii"
