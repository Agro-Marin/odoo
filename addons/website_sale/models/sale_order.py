...
class SaleOrder(models.Model):
    ...
    def _compute_is_abandoned_cart(self):
        for order in self:
            ...
            if order.website_id and order.state == 'draft' and order.date_order:
                ...
                abandoned_datetime = datetime.now(UTC).replace(tzinfo=None) - relativedelta(hours=abandoned_delay)
                order.is_abandoned_cart = bool(order.date_order <= abandoned_datetime and order.partner_id != public_partner_id and order.line_ids)
            else:
    ...
    def _verify_cart(self):
        ...
        self.line_ids.filtered(lambda sol: sol.product_id and not sol.product_id.active).unlink()
    ...
    def _filter_can_send_abandoned_cart_mail(self):
        ...
        abandoned_datetime = datetime.now(UTC) - relativedelta(hours=self.website_id.cart_abandoned_delay)
