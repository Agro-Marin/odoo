...
class WebsiteSnippetFilter(models.Model):
    ...
    def _filter_records_to_values(self, records, **options):
        hide_variants = self.env.context.get("hide_variants") and not isinstance(
            records, list
        )
        if hide_variants:
    ...
    def _prepare_category_list_data(self, parent_id=None):
        ...
        default_img_path = request.env[
            "product.template"
        ]._get_product_placeholder_filename()
    ...
    def _get_products(self, mode, **kwargs):
        ...
        handler = getattr(
            self, "_get_products_%s" % mode, self._get_products_latest_sold
        )
        website = self.env["website"].get_current_website()
        ...
        if sale_orders:
            if self.env.context.get("hide_variants"):
                sold_products = Counter(
                    ...
                    for sol in sale_orders.line_ids
                )
            else:
                sold_products = Counter(sol.product_id for sol in sale_orders.line_ids)
            if sold_products:
    ...
    def _get_products_latest_viewed(self, website, limit, domain, **kwargs):
        ...
        if visitor:
            excluded_products = request.cart.line_ids.product_id.ids
            tracked_products = (
    ...
    def _get_products_recently_sold_with(
        ...
        **kwargs,
        ...
        if current_template:
            ...
            if sale_orders:
                cart_products = request.cart.line_ids.product_id
                ...
                included_products = sale_orders.line_ids.product_id
                if self.env.context.get("hide_variants"):
                    included_products = (
                        included_products.product_tmpl_id.product_variant_id
                    )
    ...
    def default_get(self, fields):
        ...
        if (
            "field_names" in defaults
            and self.env.context.get("model") == "product.product"
        ):
