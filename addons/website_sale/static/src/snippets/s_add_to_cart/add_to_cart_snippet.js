...
export class AddToCartSnippet extends Interaction {
    ...
    async onClickAddToCartButton(ev) {
        ...
        if (productId) {
            const isAddToCartAllowed = await this.waitFor(
                rpc("/shop/product/is_add_to_cart_allowed", { product_id: productId }),
            );
            if (!isAddToCartAllowed) {
                this.services.notification.add(
                    _t(
                        "This product does not exist therefore it cannot be added to cart.",
                    ),
