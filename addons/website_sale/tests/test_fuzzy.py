...
class TestFuzzy(ProductVariantsCommon):
    def test_variant_default_code(self):
        ...
        product_blue = self.product_template_sofa._get_variant_for_combination(
            value_blue
        )
        product_green = self.product_template_sofa._get_variant_for_combination(
            value_green
        )
