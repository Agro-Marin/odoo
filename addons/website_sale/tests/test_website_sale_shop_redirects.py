...
class TestWebsiteSaleShopRedirects(HttpCase, WebsiteSaleCommon):
    ...
    def test_ecommerce_product_page_url_unpublished_product(self):
        ...
        self.assertEqual(
            len(res.history), 0, "Unpublished products shouldn't redirect."
        )
        self.assertURLEqual(
            res.url, url, "Unpublished products shouldn't slug the URL."
        )
        self.assertEqual(
            res.status_code, 404, "Unpublished products should return a 404 page."
        )
