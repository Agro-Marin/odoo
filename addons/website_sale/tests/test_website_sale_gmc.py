...
class TestWebsiteSaleGMC(WebsiteSaleGMCCommon, HttpCase):
    ...
    def test_gmc_items_sorted_types(self):
        ...
        self.assertListEqual(
            [
                name.replace("/", ">")
                for name in self.public_categories.sorted("sequence").mapped(
                    "display_name"
                )
            ],
    ...
    def test_gmc_items_sorted_labels(self):
        ...
        self.assertListEqual(
            tags, [name for _, name in self.red_sofa_item["custom_label"]]
        )
    ...
    def test_gmc_items_unit_pricing_iff_product_reference_enabled(self):
        ...
        self.assertEqual(
            "6.0l", self.items[six_pack]["unit_pricing_measure"], "$12 / 6l"
        )
