...
class TestWebsiteSaleCheckoutSteps(WebsiteSaleCommon):
    ...
    def test_translate_checkout_steps(self):
        ...
        website_1_step_FR, website_2_step_FR = CheckoutStep.with_context(
            lang="fr_FR"
        ).search(
            [
                ("website_id", "!=", False),
        ...
        for fname, field in CheckoutStep._fields.items():
            if field.translate:
                self.assertEqual(
                    website_1_step_FR[fname], default_payment_step_FR[fname]
                )
                self.assertEqual(
                    website_2_step_FR[fname], default_payment_step_FR[fname]
                )
