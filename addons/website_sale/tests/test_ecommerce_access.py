...
class TestEcommerceAccess(HttpCaseWithUserDemo, WebsiteSaleCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.filled_category, cls.empty_category = cls.env[
    ...
    def test_ecommerce_access_public_user(self):
        ...
        self.assertFalse(
            self.website.with_user(self.public_user).has_ecommerce_access()
        )
    ...
    def test_ecommerce_access_shop_redirection(self):
        ...
        response = self.url_open(
            f"/shop/category/{category_slug}/page/1", allow_redirects=True
        )
