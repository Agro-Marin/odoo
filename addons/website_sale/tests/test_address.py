    ...
    * address choice (/shop/checkout)
    ...
    def test_02_demo_address_and_company(self):
        ...
        with MockRequest(website.env, website=website, sale_order_id=so.id) as req:
            ...
            self.assertNotEqual(
                self.demo_partner.name,
                self.default_address_values["name"],
                "Employee cannot change their name during the checkout process.",
            )
            self.assertNotEqual(
                self.demo_partner.email,
                self.default_address_values["email"],
    ...
    def test_03_carrier_rate_on_shipping_address_change(self):
        ...
        self.cart.write(
            {"carrier_id": self.carrier.id, "partner_shipping_id": shipping_partner.id}
        )
        website = self.website.with_user(self.public_user).with_context({})
        with (
    ...
    def test_05_create_so_with_website_and_multi_company(self):
        ...
        with self.assertRaises(
            ...
            self._create_so(
                partner_id=self.demo_partner.id, company_id=self.company_a.id
            )

        # Same company on the SO and the website company
        so = self._create_so(
            partner_id=self.demo_partner.id, company_id=self.company_b.id
        )
        self.assertEqual(so.company_id, self.website.company_id)
        ...
        with self.assertRaises(
            ...
            self._create_so(
                partner_id=self.demo_partner.id, company_id=self.company_c.id
            )
    ...
    def test_07_change_fiscal_position(self):
        ...
        tax_10_incl, tax_20_excl, tax_15_incl = self.env["account.tax"].create(
            [
                ...
                },
                {
                    ...
                    "fiscal_position_ids": fpos_be,
                },
                {
                    "name": "Tax 15% incl",
                    "amount": 15,
                    "price_include_override": "tax_included",
                    "fiscal_position_ids": fpos_nl,
                },
            ]
        )
        ...
        self.assertEqual(
            [so.amount_untaxed, so.amount_tax, so.amount_total], [90.91, 9.09, 100.0]
        )
    ...
    def test_08_new_user_address_state(self):
        ...
        with MockRequest(website.env, website=website, sale_order_id=so.id) as req:
            ...
            use_delivery_as_billing = self.default_address_values | {
    ...
    def test_09_shop_update_address(self):
        ...
        invoicing, shipping, bad_invoicing, bad_shipping = self.env[
            "res.partner"
        ].create(
            [
                {
        ...
        with MockRequest(website.env, website=website, sale_order_id=so.id):
            self.assertFalse(
                colleague._can_be_edited_by_current_customer(order_sudo=so)
            )
    ...
    def test_11_payment_term_when_address_change(self):
        ...
        self.assertTrue(
            so.payment_term_id,
            "A payment term should be set by default on the sale order",
        )
        ...
        with MockRequest(website.env, website=website) as req:
            ...
            self.assertTrue(
                so.payment_term_id,
                "A payment term should still be set on the sale order",
            )
    ...
    def test_12_recompute_taxes_on_address_change(self):
        ...
        self.assertEqual(
            cart.amount_untaxed, amount_untaxed, "Untaxed amount should not change"
        )

        cart.action_confirm()
        self.partner.country_id = self.country_id
        self.assertEqual(
            cart.line_ids.tax_ids,
            tax_15_incl,
    ...
    def test_imported_user_with_trailing_name_can_checkout(self):
        ...
        with MockRequest(website.env, website=website, sale_order_id=so.id) as req:
            ...
            self.assertIsNotNone(
                json.loads(res).get("redirectUrl"),
                "We should get a 'redirectUrl' in the response",
            )
