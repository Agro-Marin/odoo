...
class TestWebsiteSaleProductFilters(
    WebsiteSaleCommon, TestProductAttributeValueCommon, HttpCase
):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.accessories = (
            cls.computer_case.product_variant_ids + cls.monitor.product_variant_id
        )
        cls.computer.write(
            {
                "company_id": False,
    ...
    def test_latest_sold_filter(self):
        ...
        self.empty_cart.write(
            {
                ...
                "line_ids": [
                    ...
                    for product_id in (
                        computer + self.pink_case_M + self.pink_case_L
                    ).ids
                ],
            }
        )
        ...
        with MockRequest(self.env, website=self.website):
            ...
            self.assertSetEqual(
                ...
                {
                    computer.id,
                    *(self.pink_case_L + self.pink_case_M + self.black_case_M).ids,
                },
    ...
    def test_latest_viewed_filter(self):
        ...
        with MockRequest(self.env, website=self.website):
            ...
            self.assertSetEqual(
                ...
                {
                    self.computer_case.product_variant_id.id,
                    self.computer.product_variant_id.id,
                },
    ...
    def test_recently_sold_with_filter(self):
        ...
        dyn_filter = self.env.ref(
            "website_sale.dynamic_filter_cross_selling_recently_sold_with"
        )
        with MockRequest(self.env, website=self.website):
            with_variants = self.WebsiteSnippetFilter.with_context(
                dynamic_filter=dyn_filter,
                ...
                "recently_sold_with", product_template_id=str(self.computer.id)
            )
            self.assertSetEqual(
                {p["product_id"] for p in with_variants},
            ...
            self.assertSetEqual(
                {p["product_id"] for p in no_variants},
                {
                    self.monitor.product_variant_id.id,
                    self.computer_case.product_variant_id.id,
                },
    ...
    def test_alternative_products_filter(self):
        ...
        with MockRequest(self.env, website=self.website):
            with_variants = self.WebsiteSnippetFilter.with_context(
                ...
                "alternative_products", product_template_id=str(self.mac.id)
            )
            self.assertListEqual(
                [p["product_id"] for p in with_variants],
            ...
            self.assertListEqual(
                ...
                [
                    self.computer.product_variant_id.id,
                    self.windows_pc.product_variant_id.id,
                ],
