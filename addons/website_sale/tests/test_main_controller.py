...
class TestPaymentProviderVisibility(PaymentHttpCommon, SaleCommon):
    def test_payment_provider_visibility_with_portal(self):
        ...
        restricted_provider = (
            ...
            .sudo()
            .search(
                [("name", "=", "Demo"), ("company_id", "=", website_shop.company_id.id)]
            )
        )
        ...
        with patch(
            ...
            self.assertEqual(
                mock_method.call_args.kwargs["website_id"], website_portal.id
            )
        ...
        self.assertIn(
            self.provider.id, providers.ids, "The visible provider should be visible."
        )

        self.assertNotIn(
            restricted_provider.id,
            providers.ids,
            "The restricted provider shouldn't be visible.",
        )
