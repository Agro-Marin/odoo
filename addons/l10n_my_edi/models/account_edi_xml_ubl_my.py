...
class AccountEdiXmlUBLMyInvoisMY(models.AbstractModel):
    ...
    * MyInvois API formats doc: https://sdk.myinvois.hasil.gov.my/documents
    ...
    def _add_myinvois_document_config_vals(self, vals):
        ...
        if (
            myinvois_document._is_consolidated_invoice()
            or myinvois_document._is_consolidated_invoice_refund()
        ):
            customer = self.env["res.partner"].search(
                domain=[
                    *self.env["res.partner"]._check_company_domain(
                        myinvois_document.company_id
                    ),
                    "|",
                    ("vat", "=", "EI00000000010"),
            ...
            invoice = myinvois_document.invoice_ids[
                0
        ...
        document_type_code, original_document = (
            self._l10n_my_edi_get_document_type_code(myinvois_document)
        )
            ...
            vals.update(
                {
                    ...
                    "total_paid_amount_currency": sum(
                        invoice.amount_total - invoice.amount_residual
                        for invoice in myinvois_document.invoice_ids
                    ),
                }
    ...
    def _get_myinvois_document_address_node(self, vals):
        ...
        if partner.state_id:
            ...
            subentity_code = (
                subentity_code.split("-")[1]
                if "MY-" in subentity_code
                else subentity_code
            )
        ...
        return {
            ...
            "cac:Country": {
                ...
                "cbc:Name": {
                    "_text": country.name,
                    "languageID": vals.get("country_vals", {})
                    .get("name_attrs", {})
                    .get("languageID"),
                },
            },
    ...
    def _get_myinvois_document_party_identification_node(self, vals):
        ...
        if (
            vals["document_type_code"] in ("01", "02", "03", "04")
            and tin == "EI00000000030"
            and vals["role"] == "customer"
        ):
            tin = (
                "EI00000000020"  # For normal invoices, this partner TIN should be used
            )
        elif (
            vals["document_type_code"] in ("11", "12", "13", "14")
            and tin == "EI00000000020"
            and vals["role"] == "supplier"
        ):
        ...
        if (
            partner.l10n_my_identification_type
            and partner.l10n_my_identification_number
        ):
    ...
    def _get_myinvois_document_party_node(self, vals):
        ...
        return {
            ...
            "cac:PartyIdentification": self._get_myinvois_document_party_identification_node(
                {**vals, "partner": partner.commercial_partner_id}
            ),
            ...
            "cac:Contact": {
                ...
                "cbc:Telephone": {
                    "_text": self._l10n_my_edi_get_formatted_phone_number(
    ...
    def _add_myinvois_document_header_nodes(self, document_node, vals):
        ...
        if (
            vals["document_type_code"] in {"02", "03", "04", "12", "13", "14"}
            and vals["original_document"]
        ):
            if vals["original_document"].myinvois_file_id:
                decoded_vals = self._l10n_my_edi_decode_myinvois_attachment(
                    vals["original_document"].myinvois_file_id
                )
                original_document_id = decoded_vals.get("original_document_id")
            original_invoice = vals["original_document"].invoice_ids[:1]
            if (
                not original_document_id
                and vals["document_type_code"] in {"12", "13", "14"}
                and original_invoice.ref
            ):
        ...
        document_node.update(
            {
                ...
                "cbc:BuyerReference": {
                    "_text": vals["customer"].commercial_partner_id.ref
                },
                ...
                "cac:BillingReference": {
                    "cac:InvoiceDocumentReference": {
                        ...
                        "cbc:UUID": {
                            "_text": (
                                vals["original_document"]
                                and vals["original_document"].myinvois_external_uuid
                            )
                            or "NA"
                        },
                    }
                ...
                "cac:AdditionalDocumentReference": [
                    ...
                    if vals["document_type_code"] in {"11", "12", "13", "14"}
                    and vals["custom_form_reference"]
                    else None,
                    ...
                    if vals["document_type_code"] in {"01", "02", "03", "04"}
                    and vals["custom_form_reference"]
                    else None,
                ],
            }
        ...
        if (
            vals["document_type_code"] in ("11", "12", "13", "14")
            and vals["document_ref"]
        ):
            document_node["cbc:ID"]["_text"] = vals["document_ref"]

    def _add_myinvois_document_accounting_supplier_party_nodes(
        self, document_node, vals
    ):
        document_node["cac:AccountingSupplierParty"] = {
            "cac:Party": self._get_myinvois_document_party_node(
                {**vals, "partner": vals["supplier"], "role": "supplier"}
            ),
        }

    def _add_myinvois_document_accounting_customer_party_nodes(
        self, document_node, vals
    ):
    ...
    def _add_myinvois_document_delivery_nodes(self, document_node, vals):
        document_node["cac:Delivery"] = {
            "cac:DeliveryParty": self._get_myinvois_document_party_node(
                {**vals, "partner": vals["customer"], "role": "delivery"}
            ),
        }
    ...
    def _add_myinvois_document_payment_terms_nodes(self, document_node, vals):
        if vals["invoice_payment_term_id"]:
            document_node["cac:PaymentTerms"] = {
                ...
                "cbc:Note": {
                    "_text": html2plaintext(vals["invoice_payment_term_id"].note)
                }
            }
    ...
    def _add_myinvois_document_exchange_rate_nodes(self, document_node, vals):
        if vals["currency_id"].name != "MYR":
            ...
            for base_line in vals["base_lines"]:
                total_amount_in_company_currency += base_line["tax_details"][
                    "raw_total_included"
                ]
                total_amount_in_currency += base_line["tax_details"][
    ...
    def _add_myinvois_document_monetary_total_nodes(self, document_node, vals):
        ...
        payable_amount = self.format_float(
            vals[f"tax_inclusive_amount{currency_suffix}"] - amount_paid,
            vals["currency_dp"],
        )
        ...
        if myinvois_document._is_consolidated_invoice():
            ...
            for currency_suffix in ["", "_currency"]:
                ...
                vals[f"gross_price_unit{currency_suffix}"] += (
                    discount_amount  # Price unit should be excluding discounts.
                )
    ...
    def _add_myinvois_document_line_item_nodes(self, line_node, vals):
        ...
        if (
            ...
            base_line = vals["base_line"]
            class_code = (
                base_line["record"].l10n_my_edi_classification_code
                or base_line[
                    "record"
                ].product_id.product_tmpl_id.l10n_my_edi_classification_code
            )
    ...
    def _export_myinvois_document_constraints(self, vals):
        constraints = {
            "myinvois_supplier_name_required": self._check_required_fields(
                vals["supplier"], "name"
            ),
            "myinvois_customer_name_required": self._check_required_fields(
                vals["customer"].commercial_partner_id, "name"
            ),
            "myinvois_document_name_required": self._check_required_fields(
                vals, "document_name"
            ),
        }
        ...
        if not vals[
            "supplier"
        ].commercial_partner_id.l10n_my_edi_industrial_classification:
            self._l10n_my_edi_make_validation_error(
                constraints,
                "industrial_classification_required",
                "supplier",
        ...
        for partner_type in ("supplier", "customer"):
            ...
            if (
                not partner.commercial_partner_id.l10n_my_identification_type
                or not partner.commercial_partner_id.l10n_my_identification_number
            ):
                self._l10n_my_edi_make_validation_error(
                    constraints,
                    "required_id",
            ...
            if not partner.state_id:
                self._l10n_my_edi_make_validation_error(
                    constraints, "no_state", partner_type, partner.display_name
                )
            if not partner.city:
                self._l10n_my_edi_make_validation_error(
                    constraints, "no_city", partner_type, partner.display_name
                )
            if not partner.country_id:
                self._l10n_my_edi_make_validation_error(
                    constraints, "no_country", partner_type, partner.display_name
                )
            if not partner.street:
                self._l10n_my_edi_make_validation_error(
                    constraints, "no_street", partner_type, partner.display_name
                )

            if (
                partner.commercial_partner_id.sst_registration_number
                and len(
                    partner.commercial_partner_id.sst_registration_number.split(";")
                )
                > 2
            ):
                self._l10n_my_edi_make_validation_error(
        ...
        for line_vals in vals["document_node"]["cac:InvoiceLine"]:
            ...
            if "cac:CommodityClassification" not in line_item:
                self._l10n_my_edi_make_validation_error(
                    constraints,
                    "class_code_required",
                    line_vals["cbc:ID"]["_text"],
                    line_item["cbc:Name"]["_text"],
                )
            ...
            for tax_category in line_item["cac:ClassifiedTaxCategory"]:
                if (
                    tax_category["cbc:ID"]["_text"] == "E"
                    and not tax_category["cbc:TaxExemptionReason"]["_text"]
                ):
                    self._l10n_my_edi_make_validation_error(
                        constraints,
                        "tax_exemption_required",
                        line_vals["cbc:ID"]["_text"],
                        line_item["cbc:Name"]["_text"],
                    )
            ...
            if (
                ...
                customer_vat = vals["document_node"]["cac:AccountingCustomerParty"][
                    "cac:Party"
                ]["cac:PartyIdentification"][0]["cbc:ID"]["_text"]
                if customer_vat != "EI00000000010":
                    self._l10n_my_edi_make_validation_error(
                        constraints,
                        "missing_general_public",
                        vals["customer"].id,
                        vals["customer"].name,
                    )
    ...
    @api.model
    def _l10n_my_edi_make_validation_error(
        self, constraints, code, record_identifier, record_name
    ):
    ...
    def _l10n_my_edi_decode_myinvois_attachment(self, attachment):
        ...
        try:
            ...
            invoice_id_node = get_node(
                root, "//*[local-name()='Invoice']/*[local-name()='ID']"
            )
    ...
    def _l10n_my_edi_get_document_type_code(self, myinvois_document):
        ...
        if not myinvois_document._is_consolidated_invoice():
            ...
            if (
                "debit_origin_id" in self.env["account.move"]._fields
                and invoice.debit_origin_id
            ):
                document_type_code = (
                    "03" if invoice.move_type == "out_invoice" else "13"
                )
                original_document = (
                    invoice.debit_origin_id._get_active_myinvois_document()
                )
            elif invoice.move_type in ("out_refund", "in_refund"):
                is_refund, refunded_document = self._l10n_my_edi_get_refund_details(
                    invoice
                )
                if is_refund:
                    document_type_code = (
                        "04" if invoice.move_type == "out_refund" else "14"
                    )
                else:
                    document_type_code = (
                        "02" if invoice.move_type == "out_refund" else "12"
                    )

                original_document = refunded_document
            else:
                document_type_code = (
                    "01" if invoice.move_type == "out_invoice" else "11"
                )

        return (
            document_type_code,
            original_document,
        )  # Consolidated invoices are fixed to '01'
    ...
    def _l10n_my_edi_get_refund_details(self, invoice):
        ...
        counterpart_move_type = (
            "out_invoice" if invoice.move_type == "out_refund" else "in_invoice"
        )
        has_payments = bool(
            counterpart_amls.move_id.filtered(
                lambda move: move.move_type != counterpart_move_type
            )
        )
