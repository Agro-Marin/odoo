...
class AccountMove(models.Model):
    ...
    def _compute_l10n_my_edi_state(self):
        for move in self:
            myinvois_document = move._get_active_myinvois_document(
    ...
    def _compute_l10n_my_edi_display_tax_exemption_reason(self):
        ...
        for proxy_user, moves in moves_per_proxy_users.items():
            for move in moves:
                should_display = proxy_user and any(
    ...
    @api.depends(
        "move_type", "state", "country_code", "l10n_my_edi_state", "company_id"
    )
    ...
    def action_l10n_my_edi_update_status(self):
        ...
        self.l10n_my_edi_document_ids._get_active_myinvois_document(
            including_in_progress=True
    ...
    def action_l10n_my_edi_send_invoice(self):
        ...
        for move in self.filtered(lambda m: m.state == "posted"):
            ...
            if valid_document := move.l10n_my_edi_document_ids.filtered(
                lambda d: d.myinvois_state in [False, "invalid"]
            ):
    ...
    def _get_active_myinvois_document(self, including_in_progress=False):
        ...
        return self.env["myinvois.document"].union(
            *[
                invoice.l10n_my_edi_document_ids._get_active_myinvois_document(
