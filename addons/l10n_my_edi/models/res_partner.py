...
class ResPartner(models.Model):
    ...
    def _compute_l10n_my_edi_display_tin_warning(self):
        ...
        for partner in self:
            ...
            partner.l10n_my_edi_display_tin_warning = (
                is_edi_used and partner.l10n_my_identification_number
            )
    ...
    def _compute_l10n_my_edi_industrial_classification(self):
        default_classification = self.env.ref(
            "l10n_my_edi.class_00000", raise_if_not_found=False
        )
        self.filtered(
            lambda p: not p.l10n_my_edi_industrial_classification
        ).l10n_my_edi_industrial_classification = default_classification
    ...
    def action_validate_tin(self):
        ...
        if (
            not self._l10n_my_edi_get_tin_for_myinvois()
            or not self.l10n_my_identification_type
            or not self.l10n_my_identification_number
        ):
            raise UserError(
                self.env._(
                    "In order to validate the TIN, you must provide the Identification type and number."
                )
            )
        ...
        response = proxy_user._l10n_my_edi_contact_proxy(
            "api/l10n_my_edi/1/validate_tin",
            params={
        ...
        if "error" in response:
            ...
            if ref == "document_tin_not_found":
                ...
                self._message_log(
                    body=self.env._(
                        "An unexpected error occurred while validating the TIN. Please try again later."
                    )
                )
        else:
            self.l10n_my_tin_validation_state = (
                "valid" if response.get("success") else "invalid"
            )
