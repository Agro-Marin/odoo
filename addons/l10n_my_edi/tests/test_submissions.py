...
class L10nMyEDITestNewSubmission(TestAccountMoveSendCommon):
    ...
    def test_01_new_basic_submission(self):
        ...
        myinvois_document = (
            self.basic_invoice.l10n_my_edi_document_ids._get_active_myinvois_document()
        )
            ...
            [
                {
                    ...
                    "myinvois_validation_time": datetime.strptime(
                        "2024-07-15 05:00:00", "%Y-%m-%d %H:%M:%S"
                    ),
    ...
    def test_04_new_cancellation(self):
        ...
        self.assertEqual(
            self.basic_invoice.l10n_my_edi_document_ids[0].myinvois_state, "cancelled"
        )
    ...
    def test_05_new_cancellation_failures(self):
        ...
        with patch(CONTACT_PROXY_METHOD, new=self._test_05_mock):
            ...
            self.basic_invoice.l10n_my_edi_document_ids[
                0
            ].myinvois_validation_time = datetime.strptime(
                "2024-07-12 10:00:00", "%Y-%m-%d %H:%M:%S"
            )
            ...
            self.basic_invoice.l10n_my_edi_document_ids[
                0
            ].myinvois_validation_time = datetime.now()
            action = self.basic_invoice.button_request_cancel()
            wizard = (
                self.env[action["res_model"]]
                .with_context(action["context"])
            ...
            wizard.button_request_update()
            self.assertEqual(
                self.basic_invoice.message_ids[0].preview,
    ...
    def test_08_new_mass_submission(self):
        ...
        with (
            patch(CONTACT_PROXY_METHOD, new=self._test_08_mock),
            patch(
                "odoo.addons.l10n_my_edi.models.myinvois_document.SUBMISSION_MAX_SIZE",
                2,
            ),
        ):
    ...
    def test_10_new_full_rejection_flow_invoice(self):
        ...
        with patch(CONTACT_PROXY_METHOD, new=self._test_10_mock):
            ...
            wizard.button_request_update()
            self.assertEqual(
                self.basic_invoice.l10n_my_edi_document_ids[0].myinvois_state,
                "cancelled",
            )
    ...
    def test_12_multiple_moves_with_one_failed_submission(self):
        ...
        with (
            patch(CONTACT_PROXY_METHOD, new=self._test_12_mock),
            patch(
                "odoo.addons.l10n_my_edi.models.myinvois_document.SUBMISSION_MAX_SIZE",
                1,
            ),
        ...
        self.assertEqual(self.submission_count, 5)
        valid_invoices = self.submission_invoice.filtered(
            lambda inv: inv.l10n_my_edi_state == "valid"
        )
        self.assertEqual(
            len(valid_invoices), 4, "The four invoices are in a valid state."
        )

        failed_invoice = self.submission_invoice.filtered(
            lambda inv: not inv.l10n_my_edi_state
        )
    ...
    def test_13_multiple_cron_runs(self):
        ...
        with patch(CONTACT_PROXY_METHOD, new=self._test_13_mock):
            ...
            with freeze_time("2024-07-15 10:01:00"):
                ...
                self.env[
                    "myinvois.document"
                ]._myinvois_statuses_update_cron()  # If failed to avoid the query, the mock method will raise.
                self.submission_status_count += 1
    ...
    def _test_06_mock(self, endpoint, params):
        ...
        if (
            ...
            and not params["documents"][0]["error_document_hash"]
        ):
            return {
                "submission_uid": "123456789",
                "documents": [
        ...
        if (
            endpoint == "api/l10n_my_edi/1/submit_invoices"
            and params["documents"][0]["error_document_hash"]
        ):
            return {
                "submission_uid": "123456789",
                "documents": [
    ...
    def _test_07_mock(self, endpoint, params):
        ...
        if (
            ...
            and self.get_submission_status_count < 3
        ):
    ...
    def _test_08_mock(self, endpoint, params):
        ...
        if endpoint == "api/l10n_my_edi/1/get_submission_statuses":
            documents = self.submission_invoice.l10n_my_edi_document_ids.grouped(
                "myinvois_submission_uid"
            ).get(params["submission_uid"])
            return {
                "statuses": {
    ...
    def _test_13_mock(self, endpoint, params):
        ...
        if (
            endpoint == "api/l10n_my_edi/1/get_submission_statuses"
            and self.submission_status_count == 0
        ):
            return {
                "statuses": {
                    "123458974513510": {
        ...
        if (
            endpoint == "api/l10n_my_edi/1/get_submission_statuses"
            and self.submission_status_count in [1, 3]
        ):
