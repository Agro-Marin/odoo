...
class L10nMyEDITestFileGeneration(AccountTestInvoicingCommon):
        ...
        cls.company_data["company"].write(
            {
                ...
                "l10n_my_edi_industrial_classification": cls.env[
        ...
        cls.partner_b.write(
            {
                ...
                "city": "Main city",
                "phone_ids": [
                    Command.create({"number": "+60123456785", "type": "landline"})
                ],
                "l10n_my_edi_industrial_classification": cls.env.ref(
    ...
    def test_01_can_generate_file(self):
        ...
        # 2. Supplier party
        supplier_root = root.xpath(
            "cac:AccountingSupplierParty/cac:Party", namespaces=NS_MAP
        )[0]
        ...
        self._assert_node_values(
            ...
            attributes={
                "name": self.company_data[
                    "company"
                ].l10n_my_edi_industrial_classification.name
            },
        )
        ...
        # 3. Customer level
        customer_root = root.xpath(
            "cac:AccountingCustomerParty/cac:Party", namespaces=NS_MAP
        )[0]
    ...
    def test_02_multicurrency(self):
        ...
        with file_open(
            "l10n_my_edi/tests/expected_xmls/invoice_multicurrency.xml", "rb"
        ) as f:
            expected_xml = etree.fromstring(f.read())
        self.assertXmlTreeEqual(root, expected_xml)
    ...
    def test_03_optional_fields(self):
        ...
        with file_open(
            "l10n_my_edi/tests/expected_xmls/invoice_optional_fields.xml", "rb"
        ) as f:
            expected_xml = etree.fromstring(f.read())
        self.assertXmlTreeEqual(root, expected_xml)
    ...
    def test_05_invoice_with_so(self):
        ...
        with file_open(
            "l10n_my_edi/tests/expected_xmls/invoice_with_so.xml", "rb"
        ) as f:
            expected_xml = etree.fromstring(f.read())
        self.assertXmlTreeEqual(root, expected_xml)
    ...
    def test_06_foreigner(self):
        ...
        with file_open(
            "l10n_my_edi/tests/expected_xmls/invoice_foreigner.xml", "rb"
        ) as f:
            expected_xml = etree.fromstring(f.read())
        self.assertXmlTreeEqual(root, expected_xml)
    ...
    def test_07_tax_exempt(self):
        ...
        tax_subtotal_root = root.xpath(
            "cac:TaxTotal/cac:TaxSubtotal", namespaces=NS_MAP
        )[0]
        self._assert_node_values(
            tax_subtotal_root,
        ...
        with file_open(
            "l10n_my_edi/tests/expected_xmls/invoice_tax_exempt.xml", "rb"
        ) as f:
            expected_xml = etree.fromstring(f.read())
        self.assertXmlTreeEqual(root, expected_xml)
    ...
    def test_08_self_billing(self):
        ...
        with file_open(
            "l10n_my_edi/tests/expected_xmls/invoice_self_billing.xml", "rb"
        ) as f:
            expected_xml = etree.fromstring(f.read())
        self.assertXmlTreeEqual(root, expected_xml)
    ...
    def test_09_refund_note(self):
        ...
        with file_open(
            "l10n_my_edi/tests/expected_xmls/refund_note_company_currency.xml", "rb"
        ) as f:
            expected_xml = etree.fromstring(f.read())
        self.assertXmlTreeEqual(root, expected_xml)
    ...
    def test_10_credit_note(self):
        ...
        with file_open(
            "l10n_my_edi/tests/expected_xmls/credit_note_company_currency.xml", "rb"
        ) as f:
            expected_xml = etree.fromstring(f.read())
        self.assertXmlTreeEqual(root, expected_xml)
    ...
    def test_12_partner_ref_not_in_party_id(self):
        ...
        node = customer_root.xpath(
            "cac:PartyIdentification/cbc:ID[count(@*)=0]", namespaces=NS_MAP
        )
        self.assertEqual(node, [])
    ...
    def test_13_prepaid_amount_present(self):
        ...
        self.env["account.payment.register"].with_context(
            active_model="account.move", active_ids=invoice.ids
        ).create({"amount": 2200.00, "payment_date": "2024-07-15"})._create_payments()
    ...
    def test_14_prioritize_invoice_line_classification_code(self):
        ...
        root = etree.fromstring(file)
        class_root = root.xpath(
            "cac:InvoiceLine/cac:Item/cac:CommodityClassification", namespaces=NS_MAP
        )[0]
    ...
    def test_16_original_document_id(self):
        ...
        myinvois_document.write(
            {
                "myinvois_file_id": self.env["ir.attachment"]
                .create({"name": "test myinvois", "raw": bill_file})
                .id,
                "myinvois_state": "valid",
            }
        )
    ...
    def _assert_node_values(self, root, node_path, text, attributes=None):
        ...
        self.assertEqual(
            len(node),
            1,
            f"The node {node[0].tag} has been found {len(node)} time in the file",
        )
