...
class TestMailGateway(MailCommon):
            ...
            .create(
                {
                    ...
                    "name": "file.gif",
                    "mimetype": "image/gif",
                    "folder_id": cls.env["documents.document"]
    ...
    def test_alias_access(self):
        ...
        manager = new_test_user(
            ...
            groups="base.group_user,documents.group_documents_manager",
        )
        ...
        alias = self.env["mail.alias"].create(
            {
                ...
                "alias_model_id": self.env["ir.model"]._get("documents.document").id,
            }
        )
        with self.assertRaises(AccessError):
            Doc.with_user(user).create(
    ...
    def test_documents_email_system_parameter(self):
        ...
        self.env["ir.config_parameter"].set_param(
            "documents.disable_mail_to_document", "1"
        )
        with self.mock_mail_gateway():
            document = self.format_and_process(
                MAIL_TEMPLATE,
