...
class TestDocumentDeletion(HttpCase):
    ...
    def test_delete_folder_and_documents_tour(self):
        ...
        self.start_tour(
            f"/odoo/documents/{self.document.access_token}",
            "document_delete_tour",
            login="admin",
        )
        self.assertTrue(folder_copy.exists(), "The folder should still exist")
        self.assertFalse(
            document_copy.exists(), "The document should not exist anymore"
        )

    def test_tour_default_action_view(self):
        self.start_tour(
            f"/odoo/documents.document/{self.document.id}",
            "document_default_access_view",
            login="admin",
        )
