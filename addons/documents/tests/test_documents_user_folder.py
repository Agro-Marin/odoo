...
class TestDocumentsUserFolder(TransactionCaseDocuments):
    ...
    def test_numeric_user_folder_id_search_requires_a_reachable_folder(self):
        ...
        hidden_folder = Document.create({
            "name": "Hidden folder", "type": "folder", "access_internal": "none",
        })
        inside = Document.create({
            "name": "inside.txt", "type": "binary", "folder_id": hidden_folder.id,
        })
        ...
        self.assertEqual(
            inside.with_user(self.internal_user).user_folder_id, "SHARED",
