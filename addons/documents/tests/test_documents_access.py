...
class TestDocumentsAccess(TransactionCaseDocuments, MockEmail):
    ...
    def test_forbidden_partner_members(self):
        ...
        with self.assertRaises(ValidationError, msg="public user should not be a member"):
        ...
        self.assertTrue(self.env["documents.access"].create({
            "document_id": self.folder_a.id,
            "partner_id": public_partner.id,
    ...
    def test_action_update_access_rights_partners(self):
        ...
        partners = dict.fromkeys((self.portal_user | self.internal_user).partner_id.ids, ("edit", False))
        self.env.invalidate_all()
        with self.assertQueryCount(10):
            self.folder_a.action_update_access_rights(partners=partners)
        folder_a_as_portal.check_access("write")
    ...
    def test_tracking_creation_on_access_rights_changes(self):
        ...
        for message in folder_a_d_message_ids:
            ...
            self.assertIn("Portal user has gained access as Viewer", " ".join(html2plaintext(message.body).split()))
        ...
        for message in message_ids:
            ...
            self.assertIn("Portal user has gained access as Editor", " ".join(html2plaintext(message.body).split()))
        ...
        for message in folder_a_d_latest_message_ids:
            ...
            self.assertIn("Portal user rights changed from Viewer to Editor", " ".join(html2plaintext(message.body).split()))
