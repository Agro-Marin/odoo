...
class TestScoreCeilingCascade(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.env["partner.profile"].search([]).write({"active": False})
        ...
        cls.bystander._update_profile_scores()

    def _refresh_queued(self):
        self.Partner.search([("score_line_ids", "!=", False)])._update_profile_scores()
        self.env.flush_all()

    def test_a_new_weighted_value_moves_the_denominator(self):
        before = self.Partner._get_score_max_possible()
        self.Value.create(
            {"name": "Added", "attribute_id": self.attribute.id, "score_value": 5.0}
        )
        self.assertEqual(self.Partner._get_score_max_possible(), before + 5.0)

        self._refresh_queued()
        self.assertEqual(self.holder.score_max_possible, before + 5.0)
        self.assertEqual(self.bystander.score_max_possible, before + 5.0)
    ...
    def test_deleting_a_weighted_value_moves_the_denominator(self):
        ...
        with_extra = self.Partner._get_score_max_possible()
        extra.unlink()
        self.assertEqual(self.Partner._get_score_max_possible(), with_extra - 4.0)

        self._refresh_queued()
        self.assertEqual(self.bystander.score_max_possible, with_extra - 4.0)
    ...
    def test_a_weight_edit_restales_the_non_holders_too(self):
        ...
        self.assertEqual(
            self.bystander.score_max_possible, self.Partner._get_score_max_possible()
        )
    ...
    def test_a_new_attribute_moves_the_denominator(self):
        ...
        self.assertEqual(self.Partner._get_score_max_possible(), before + 6.0)

        self._refresh_queued()
        self.assertEqual(self.bystander.score_max_possible, before + 6.0)
    ...
    def test_archiving_a_weighted_value_moves_the_denominator(self):
        ...
        with_spare = self.Partner._get_score_max_possible()
        spare.action_archive()
        self.assertEqual(self.Partner._get_score_max_possible(), with_spare - 7.0)

        spare.action_unarchive()
        self.assertEqual(self.Partner._get_score_max_possible(), with_spare)
    ...
    def test_rehoming_a_value_moves_both_ceilings(self):
        ...
        self.assertEqual(self.Partner._get_score_max_possible(), before)

        self._refresh_queued()
        self.assertEqual(self.bystander.score_max_possible, before)
        keys = self.bystander.score_line_ids.mapped("source_key")
    ...
    def test_the_ceiling_ignores_the_callers_active_test(self):
        ...
        spare.action_archive()
        as_default = self.Partner._get_score_max_possible()
    ...
    def test_the_requeue_reaches_partners_the_editor_cannot_see(self):
        ...
        hidden._update_profile_scores()
        archived = self.Partner.create({"name": "Cascade Archived", "is_company": True})
        archived._update_profile_scores()
        archived.action_archive()

        partner_class = type(self.Partner)
        with patch.object(
            partner_class, "_delay_profile_scores_recompute", autospec=True
        ) as delayed:
    ...
    def test_a_second_wave_collapses_onto_the_queued_job(self):
        ...
        scored._delay_profile_scores_recompute()
