...
class ResPartnerBank(models.Model):
    ...
    proxy_type = fields.Selection(
        ...
        ondelete={
            "merchant_id": "set default",
            "payment_service": "set default",
            "atm_card": "set default",
            "bank_acc": "set default",
        },
    )
    ...
    def _get_merchant_account_info(self):
        ...
        vals = [
            (0, "A000000727"),
            (1, "".join(list(starmap(self._serialize, payment_network)))),
            (2, proxy_type_mapping[self.proxy_type]),
        ]
        return (38, "".join(list(starmap(self._serialize, vals))))
        ...
        if self.country_code == "VN":
            merchant_city = (
                ...
                or (
                    ...
                    and self._remove_accents(self.partner_id.state_id.name)[:15]
                )
                or ""
            )
