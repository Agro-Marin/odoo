...
class TestMailGroupModeration(TestMailListCommon):
    ...
    @mute_logger(
        "odoo.models.unlink", "odoo.addons.mail_group.models.mail_group_message"
    )
    @users("employee")
    def test_moderation_rule_api(self):
        ...
        self.assertEqual(
            len(mail_group.moderation_rule_ids),
            4,
            "Should have created only one moderation rule",
        )
        self.assertEqual(
            set(mail_group.moderation_rule_ids.mapped("email")),
            set(
        ...
        self.assertEqual(
            moderation_2.status,
            "allow",
            "Should have write on the existing moderation rule",
        )
        self.assertEqual(
            moderation_3.status,
            "ban",
            "Should not have changed moderation of the other group",
        )
        new_moderation = mail_group.moderation_rule_ids.filtered(
            lambda rule: rule.email == "bob@test.com"
        )
        self.assertEqual(
            new_moderation.status,
            "allow",
            "Should have created the moderation with the right status",
        )
    ...
    def test_moderation_rule_security(self):
        with self.assertRaises(
            AccessError, msg="Portal should not have access to moderation rules"
        ):
            self.env["mail.group.moderation"].with_user(self.user_portal).browse(
                self.moderation.ids
            ).email
        ...
        with self.assertRaises(
            AccessError, msg="Non moderators should not have access to moderation rules"
        ):
            self.env["mail.group.moderation"].with_user(self.user_employee).browse(
                self.moderation.ids
            ).email
...
class TestModeration(TestMailListCommon):
    ...
    def test_moderation_flow_accept(self):
        ...
        self.assertEqual(
            new_email_message.moderation_status,
            "accepted",
            "Should have accepted the message",
        )
        self.assertEqual(
            old_email_message.moderation_status,
            "pending_moderation",
    ...
    def test_moderation_flow_allow(self):
        ...
        mail_group_2_as2 = (
            self.env["mail.group"]
            .with_user(self.user_employee_2)
            .browse(self.test_group_2.ids)
        )
        self.assertEqual(len(mail_group.mail_group_message_ids), 3)
        ...
        self.assertEqual(
            self.test_group_2_msg_1_pending.moderation_status,
            "pending_moderation",
            "Should not have accepted message in the other group",
        )
        self.assertEqual(
            len(mail_group_2_as2.mail_group_message_ids),
            group_2_message_count,
    ...
    def test_moderation_flow_reject(self):
        ...
        with self.mock_mail_gateway():
            new_email_message.action_moderate_reject_with_comment(
                "Test Rejected", "Bad email"
            )

        self.assertEqual(
            new_email_message.moderation_status,
            "rejected",
            "Should have rejected the message",
        )
        self.assertEqual(
            old_email_message.moderation_status,
            "pending_moderation",
