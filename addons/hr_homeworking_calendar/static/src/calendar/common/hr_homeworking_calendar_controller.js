...
patch(AttendeeCalendarController.prototype, {
    setup() {
        ...
        this._baseRendererProps.openWorkLocationWizard =
            this.openWorkLocationWizard.bind(this);
    },
    async editRecord(record) {
        if (record.homeworking && "start" in record) {
            return this.action.doAction(
                "hr_homeworking_calendar.set_location_wizard_action",
                {
                    additionalContext: {
        ...
        if (record.id && record.homeworking) {
            if (record.ghostRecord) {
                ...
                this.displayDialog(ConfirmationDialog, {
                    ...
                    confirm: async () => {
                        await this.orm.unlink("hr.employee.location", [
                            parseInt(record.id),
                        ]);
                        this.model.load();
                    },
