...
class SaleEdiXmlUbl_Bis3(models.AbstractModel):
    ...
    def _export_order_vals(self, sale_order):
        ...
        order_line_vals = self._get_order_line_vals(
            sale_order.line_ids, customer, supplier
        )
        ...
        vals["vals"].update(
            {
                ...
                "customer_party_vals": self._get_partner_party_vals(
                    customer, role="customer"
                ),
                "supplier_party_vals": self._get_partner_party_vals(
                    supplier, role="supplier"
                ),
                "delivery_party_vals": self._get_partner_party_vals(
                    delivery, role="delivery"
                ),
                "anticipated_monetary_total_vals": self._get_anticipated_monetary_total_vals(
                    order_line_vals,
