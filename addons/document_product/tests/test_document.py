...
class TestCaseDocumentsBridgeProduct(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.folder_test = cls.env['document.document'].create({'name': 'folder_test', 'type': 'folder'})
    ...
    def test_bridge_folder_product_settings_on_write(self):
        ...
        txt_doc = self.env['document.document'].search([('attachment_id', '=', self.attachment_txt_two.id)])
        gif_doc = self.env['document.document'].search([('attachment_id', '=', self.attachment_gif_two.id)])
    ...
    def _products_search(self, record, field_name):
        ...
        docs = self.env['document.document'].search([('res_id', 'in', record.ids), ('res_model', '=', record._name)], order='id')
        ...
        cases = [
            ...
            ([(field_name, 'not in', record.ids + [False])], self.env['document.document']),
            (['|', (field_name, 'in', [record[2].id]), (field_name, 'ilike', 'template')], docs),
        ]
        for domain, result in cases:
            with self.subTest(domain=domain):
                self.assertEqual(self.env['document.document'].search(owned & Domain(domain)), result)
    ...
    def test_bridge_folder_product_settings_default_company(self):
        ...
        test_user = self.env['res.users'].create({
            ...
            'group_ids': [(6, 0, [self.ref('document.group_documents_user'), self.ref('base.group_system')])],
    ...
    def test_default_res_id_model(self):
        ...
        document = self.env['document.document'].search([('attachment_id', '=', attachment.id)])
