...
class ResCompany(models.Model):
    ...
    product_folder_id = fields.Many2one(
        'document.document', string="Product Folder", check_company=True,
    ...
    def _compute_product_folder_id(self):
        folder_id = self.env.ref('document_product.document_product_folder', raise_if_not_found=False)
    ...
    def _get_used_folder_ids_domain(self, folder_ids):
        return super()._get_used_folder_ids_domain(folder_ids) | (
            Domain('product_folder_id', 'in', folder_ids) & Domain('documents_product_settings', '=', True)
        )
