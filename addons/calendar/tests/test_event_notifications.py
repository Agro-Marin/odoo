...
class TestCalendarMail(CalendarMailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.event.write(
            {
                "partner_ids": [
                    (4, p.id) for p in cls.user_employee_2.partner_id + cls.customers
                ],
            }
        )
    ...
    def test_event_get_default_recipients(self):
        ...
        self.assertDictEqual(
            defaults[event.id],
            {
                ...
                "email_to": "",
                "partner_ids": (
                    self.customers[0] + self.user_employee_2.partner_id
                ).ids,
            },
    ...
    def test_event_template(self):
        event, template = (
            self.event.with_user(self.user_employee),
        ...
        self.assertEqual(
            ...
            self.customers[0] + self.customers[1] + self.user_employee_2.partner_id,
...
class TestEventNotifications(CalendarMailCommon):
    ...
    def test_message_invite(self):
        self.env["ir.config_parameter"].sudo().set_param(
            "mail.mail_force_send_limit", None
        )
        with self.assertSinglePostNotifications(
            [{"partner": self.partner, "type": "inbox"}],
            {
        ...
        email_partner = self.env["res.partner"].create(
            {"name": "bob invitee", "email": "bob.invitee@test.lan"}
        )
        with self.mock_mail_gateway(mail_unlink_sent=False):
            self.event.partner_ids += email_partner
        self.assertMailMail(
            email_partner, "sent", author=self.env.ref("base.partner_root")
        )
    ...
    def test_message_invite_email_notif_mass_queued(self):
        ...
        additional_attendees = self.env["res.partner"].create(
            [{"name": f"test{n}", "email": f"test{n}@example.com"} for n in range(101)]
        )
                        ...
                        "payload": [
                            {
                                ...
                                "timer": 20 * 60,
                                "notify_at": fields.Datetime.to_string(
    ...
    def test_email_alarm(self):
        ...
        with patch.object(fields.Datetime, "now", lambda: now):
            ...
            user_message = new_messages.filtered(
                lambda x: self.event.user_id.partner_id in x.partner_ids
            )
        ...
        with self.capture_triggers("calendar.ir_cron_scheduler_alarm") as capt:
            with freeze_time("2022-04-13 10:00+0000"):
                ...
                self.assertEqual(
                    len(capt.records),
                    1,
                    "1 trigger should have been created for the whole recurrence",
                )
            ...
            with freeze_time("2022-05-16 10:00+0000"):
                self.env["calendar.alarm_manager"]._send_reminder()
                self.assertEqual(
                    capt.records.mapped("call_at"), [datetime(2022, 6, 13, 10, 14)]
                )
                self.assertEqual(
                    len(capt.records), 1, "1 more trigger should have been created"
                )
        ...
        with self.capture_triggers("calendar.ir_cron_scheduler_alarm") as capt:
            ...
            with freeze_time("2024-04-16 10:00+0000"):
                ...
                self.assertEqual(
                    len(capt.records),
                    1,
                    "Only one trigger must be created for the entire recurrence.",
                )
                self.assertEqual(
    ...
    def test_notification_event_timezone(self):
        ...
        def search_event():
            return self.env["calendar.event"].search(
                self.env["res.users"]._systray_get_calendar_event_domain()
            )
    ...
    def test_get_next_potential_limit_alarm(self):
        ...
        self.env.flush_all()
        result = self.env["calendar.alarm_manager"]._get_next_potential_limit_alarm(
        ...
        expected_alarms = sorted(
            [
                stop + relativedelta(days=offset) - relativedelta(minutes=15)
