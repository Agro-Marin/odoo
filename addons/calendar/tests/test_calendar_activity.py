...
class TestCalendarActivity(ActivityScheduleCase):
    def test_event_activity(self):
        ...
        # test with escaping
        test_name, test_description, test_description2 = (
            "Test-Meeting",
            "Test-Description",
            "Test & <br> Description",
        )
        test_note, test_note2 = (
            "<p>Test-Description</p>",
            "<p>Test &amp; <br> Description</p>",
        )
        ...
        self.assertEqual(
            test_record.activity_ids.date_deadline, (now + timedelta(days=-1)).date()
        )
    ...
    def test_activity_event_multiple_meetings(self):
        ...
        event_1_1 = (
            self.env["calendar.event"]
            .with_context(default_activity_ids=[(6, 0, activity_1.ids)])
            .create(
                {
                    "name": "Meeting 1",
        ...
        self.assertEqual(
            event_1_2.activity_ids, activity_1, "Changes activity ownership"
        )
        ...
        calendar_action = activity_2.with_context(
            default_res_model="res.partner", default_res_id=test_record.id
        ...
        self.assertEqual(
            event_2_1.activity_ids,
            activity_2,
            "Event 1's activity should still be the first activity",
        )
        self.assertEqual(
            activity_2.calendar_event_id,
            event_2_1,
            "The first activity's event should still be event 1",
        )

        self.assertEqual(
            len(new_activity),
            1,
            "1 more activity record should have been created (by event 2)",
        )
        self.assertEqual(
            event_2_2.activity_ids,
            new_activity,
            "Event 2's activity should not be the first activity",
        )
        self.assertEqual(
            event_2_2.activity_ids.activity_type_id,
            activity_2.activity_type_id,
            "Event 2's activity should be the same activity type as the first activity",
        )
        self.assertEqual(
            test_record.activity_ids,
            activity_1 + activity_2 + new_activity,
    ...
    def test_event_activity_user_sync(self):
        ...
        self.assertEqual(
            event_from_activity.attendee_ids.partner_id, activity.user_id.partner_id
        )
    ...
    def test_synchronize_activity_timezone(self):
        ...
        self.assertEqual(
            activity.date_deadline, date(2025, 6, 28), "Next day due to timezone"
        )

        # now change from activity, timezone should be taken into account when
        # converting into UTC for event
        activity.with_context({"tz": "Australia/Brisbane"}).date_deadline = date(
            2025, 6, 30
        )
        self.assertEqual(
            calendar_event.start,
            datetime(2025, 6, 29, 21, 0, 0),
