...
class AppointmentCalendarEventTest(AppointmentCommon):
    ...
    def test_appointment_calendar_event_inverse_capacity(self):
        ...
        test_cases = [
            (
                user_appointment,
                {"manage_capacity": False, "user_capacity": 5},
                {},
                5,
                1,
            ),
            ...
            (
                user_appointment,
                {"manage_capacity": True, "user_capacity": 5},
                {},
                6,
                None,
            ),
            (
                ...
                {"resource_ids": resource_appointment.resource_ids},
                2,
                3,
            ),
            (
        ...
        for (
            ...
            expected_total_capacity_reserved,
        ) in test_cases:
            with self.subTest(
                appointment=appointment_type.name,
                appointment_vals=appointment_vals,
                event_vals=event_vals,
                total_capacity_reserved=total_capacity_reserved,
                expected_total_capacity_reserved=expected_total_capacity_reserved,
            ):
                ...
                if expected_total_capacity_reserved is None:
                    ...
                    event.invalidate_recordset(
                        ["total_capacity_reserved", "user_id", "resource_ids"]
                    )
                    self.assertEqual(
                        event.total_capacity_reserved, expected_total_capacity_reserved
                    )
                    event.unlink()
