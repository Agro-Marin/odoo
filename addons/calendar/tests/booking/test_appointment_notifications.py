...
class AppointmentTestTracking(AppointmentCommon, MailCase):
            ...
            [
                {
                    ...
                    "phone_ids": [
                        Command.create({"number": "+32 81 212 220", "type": "landline"})
                    ],
                }
            ]
        )
    ...
    def test_archive_message(self):
        ...
        self.assertEqual(
            self.ref("calendar.mt_calendar_event_canceled"),
        ...
        self.assertEqual(
            len(self._new_msgs), 4, "Expected a tracking message and confirmation mails"
        )  # 1 track message + 3 mails(1 apt_manager + 2 customers)
        self.assertTracking(self._new_msgs[3], [("active", "boolean", False, True)])
        #  Check all mails are sent after confirming the booking
        for attendee in meeting.partner_ids:
            self.assertSentEmail(
                self.apt_manager.partner_id,
                attendee,
                subject="Invitation to Test Tracking Appointment",
            )
    ...
    def test_cancel_meeting_message(self):
        ...
        with self.mock_mail_gateway(), self.mock_mail_app():
            meeting1.with_context(mail_notify_force_send=True).action_cancel_meeting(
    ...
    def test_request_meeting_message_for_manual_confirmation(self):
        ...
        slot_str = self.reference_monday.replace(hour=11).strftime(DTF)
        with self.mock_mail_gateway(), self.mock_mail_app():
            apt_data = {
                "allday": 0,
                "asked_capacity": 1,
            ...
            response = self.url_open(
                f"/appointment/{apt_type.id}/submit", data=apt_data
            )
        ...
        calendar_event = self.env["calendar.event"].search(
            [("name", "=", "Test Online Meeting - Bxls Appt Type Booking")], limit=1
        )
        ...
        organizer_mail = self.assertMailMail(
            self.staff_user_bxls.partner_id,
            "sent",
            author=self.staff_user_bxls.partner_id,
            email_values={
                "subject": "Invitation to Test Online Meeting - Bxls Appt Type Booking",
        ...
        self.assertEqual(
            booked_follower_mail | attendee_mail | organizer_mail, self._new_mails
        )
