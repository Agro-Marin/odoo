...
class TestCalendar(SavepointCaseWithUserDemo):
    ...
    def test_event_order(self):
        ...
        self.assertEqual(
            events.mapped("start"), (foo1 + bar1 + foo2 + bar2).mapped("start")
        )
        events = self.CalendarEvent.search(domain, order="start desc")
        self.assertEqual(
            events.mapped("start"), (foo2 + bar2 + bar1 + foo1).mapped("start")
        )
    ...
    def test_recurring_around_dst(self):
        ...
        start_recurring_dates = m.recurrence_id.calendar_event_ids.sorted(
            "start"
        ).mapped("start")
    ...
    def test_recurring_ny(self):
        ...
        f.recurrency = True
        self.assertEqual(
            f.end_type, "forever", "An unbounded recurrence is the default"
        )
        # The rest of this test is about the bounded case, and `count` is only
        # writable once `end_type` says the recurrence is bounded.
        f.end_type = "count"
    ...
    def test_event_creation_mail(self):
        ...
        def _test_emails_has_attachment(self, partners, attachments_names=None):
            attachments_names = attachments_names or ["fileText_attachment.txt"]
        ...
        partners = [
            self.env["res.partner"].create(
                {"name": "testuser0", "email": "bob@example.com"}
            ),
            self.env["res.partner"].create(
                {"name": "testuser1", "email": "alice@example.com"}
            ),
        ]
        partner_ids = [
        ...
        partners.extend(
            [
                self.env["res.partner"].create(
                    {"name": "testuser2", "email": "marc@example.com"}
                ),
                self.env["res.partner"].create(
                    {"name": "testuser3", "email": "carl@example.com"}
                ),
                self.env["res.partner"].create(
                    {"name": "testuser4", "email": "alain@example.com"}
                ),
            ]
        )
        partner_ids = [
        ...
        _test_emails_has_attachment(
            self,
            partners=[partner_staff, new_partner],
    ...
    def test_event_creation_internal_user_invitation_ics(self):
        ...
        internal_user = new_test_user(
            self.env, login="internal_user", groups="base.group_user"
        )
        ...
        self.assertEqual(
            msg.with_user(internal_user).attachment_ids.name, "invitation.ics"
        )
    ...
    def test_event_creation_sudo_other_company(self):
        ...
        now = fields.Datetime.context_timestamp(
            self.partner_demo, fields.Datetime.now()
        )
    ...
    def test_meeting_creation_from_partner_form(self):
        ...
        event = (
            ...
            .with_context(calendar_action["context"])
            .create(
                {
                    "name": "Super Meeting",
        ...
        self.assertTrue(
            self.env.user.partner_id in event.attendee_ids.mapped("partner_id")
        )

    def test_discuss_videocall(self):
        self.event_tech_presentation._set_discuss_videocall_location()
        self.assertFalse(
            self.event_tech_presentation.videocall_channel_id.id,
        ...
        self.assertTrue(
            set(new_partners)
            == set(
                self.event_tech_presentation.videocall_channel_id.channel_partner_ids.ids
            ),
    ...
    def test_search_current_attendee_status(self):
        ...
        event_accepted = self.env["calendar.event"].create(
            {
                ...
                "attendee_ids": [
                    ...
                    Command.create(
                        {"partner_id": user_test.partner_id.id, "state": "needsAction"}
                    ),
                ],
            }
        )
        event_declined = self.env["calendar.event"].create(
            {
                ...
                "attendee_ids": [
                    ...
                    Command.create(
                        {"partner_id": user_test.partner_id.id, "state": "accepted"}
                    ),
                ],
            }
        )
        event_tentative = self.env["calendar.event"].create(
            {
                ...
                "attendee_ids": [
                    ...
                    Command.create(
                        {"partner_id": user_test.partner_id.id, "state": "declined"}
                    ),
                ],
            }
        )
        ...
        accepted_events = CalendarEvent_Demo.search(
            [("current_status", "=", "accepted")]
        )
        self.assertEqual(
            accepted_events,
            event_accepted,
            "Should find only the event where the demo user has accepted.",
        )

        # Search for 'No' (declined)
        declined_events = CalendarEvent_Demo.search(
            [("current_status", "=", "declined")]
        )
        self.assertEqual(
            declined_events,
            event_declined,
            "Should find only the event where the demo user has declined.",
        )

        # Search for 'Maybe' (tentative)
        tentative_events = CalendarEvent_Demo.search(
            [("current_status", "=", "tentative")]
        )
        self.assertEqual(
            tentative_events,
            event_tentative,
            "Should find only the event where the demo user is tentative.",
        )

        # Search for events where status is not 'No' (declined)
        not_declined_events = CalendarEvent_Demo.search(
            [("current_status", "!=", "declined")]
        )
        self.assertIn(
            event_accepted,
            not_declined_events,
            "Accepted events should be in the result.",
        )
        self.assertIn(
            event_tentative,
            not_declined_events,
            "Tentative events should be in the result.",
        )
        self.assertNotIn(
            event_declined,
            not_declined_events,
            "Declined events should NOT be in the result.",
        )
        self.assertNotIn(
            event_other_user,
            not_declined_events,
            "Events where the user is not an attendee should NOT be in the result.",
        )

        # Search using the 'in' operator
        in_events = CalendarEvent_Demo.search(
            [("current_status", "in", ["accepted", "tentative"])]
        )
        self.assertEqual(
            len(in_events), 2, "Should find two events for 'accepted' or 'tentative'."
        )
        self.assertIn(
            event_accepted,
            in_events,
            "Should find the accepted event in the 'in' search.",
        )
        self.assertIn(
            event_tentative,
            in_events,
            "Should find the tentative event in the 'in' search.",
        )
    ...
    def test_event_duplication_allday(self):
        ...
        self.assertEqual(
            new_calendar_event.start_date,
            calendar_event.start_date,
            "Start date should match the original.",
        )
        self.assertEqual(
            new_calendar_event.stop_date,
            calendar_event.stop_date,
            "Stop date should match the original.",
        )
    ...
    def test_unauthorized_user_cannot_add_attendee(self):
        ...
        with self.assertRaises(AccessError):
            attendee_model.with_context(
                default_event_id=self.event_tech_presentation.id
            ).create(
                [
                    {
                        "partner_id": self.partner_demo.id,
    ...
    def test_default_duration(self):
        ...
        second_company = (
            self.env["res.company"].sudo().create({"name": "Second Company"})
        )
        ...
        IrDefault.with_user(user_demo).set(
            "calendar.event", "duration", 2, user_id=True, company_id=False
        )
        IrDefault.with_company(second_company).set(
            "calendar.event", "duration", 8, company_id=True
        )

        duration = (
            self.env["calendar.event"].with_user(user_demo).get_default_duration()
        )
    ...
    def test_discuss_videocall_not_ringing_with_event(self):
        ...
        channel_member = (
            self.event_tech_presentation.videocall_channel_id.channel_member_ids[0]
        )
        channel_member_2 = (
            self.event_tech_presentation.videocall_channel_id.channel_member_ids[1]
        )
