...
class TestEventNotifications(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user = new_test_user(
            cls.env, "xav", email="em@il.com", notification_type="inbox"
        )
        cls.event = (
    ...
    def test_attendee_added(self):
        ...
        self.assertEqual(
            self.event.attendee_ids.partner_id,
            self.partner,
            "It should be linked to the partner",
        )
        self.assertIn(
            self.partner,
            self.event.message_follower_ids.partner_id,
            "He should be follower of the event",
        )
    ...
    def test_attendee_added_create(self):
        ...
        self.assertNotIn(
            self.partner,
            event.message_follower_ids.partner_id,
            "He should not be automatically added in followers, no need",
        )
    ...
    def test_attendee_added_create_with_specific_states(self):
        ...
        """
        organizer_partner = self.env["res.partner"].create(
            {"name": "orga", "email": "orga@google.com"}
        )
        event = (
            ...
            .create(
                {
                    ...
                    "attendee_ids": [
                        ...
                        (
                            0,
                            0,
                            {"partner_id": organizer_partner.id, "state": "accepted"},
                        ),
                    ],
    ...
    def test_attendee_added_multi(self):
        ...
        self.assertEqual(
            len(events.attendee_ids), 2, "It should have created one attendee per event"
        )
    ...
    def test_existing_attendee_added(self):
        ...
        self.assertEqual(
            self.event.attendee_ids,
            attendee,
            "It should not have created an new attendee record",
        )
    ...
    def test_attendee_add_self(self):
        ...
        self.assertEqual(
            self.event.attendee_ids.state,
            "accepted",
            "It should be accepted for the current user",
        )
    ...
    def test_attendee_removed(self):
        ...
        self.assertEqual(
            attendee,
            self.event.attendee_ids,
            "It should not have re-created an attendee record",
        )
        self.assertNotIn(
            self.partner,
            self.event.attendee_ids.partner_id,
            "It should have removed the attendee",
        )
        self.assertNotIn(
            self.partner,
            self.event.message_follower_ids.partner_id,
        ...
        self.assertIn(
            partner_bis,
            self.event.attendee_ids.partner_id,
            "It should have left the attendee",
        )
    ...
    def test_default_attendee(self):
        ...
        self.assertIn(
            self.partner, event.attendee_ids.partner_id, "Partner should be in attendee"
        )
        self.assertNotIn(
            partner_bis,
            event.attendee_ids.partner_id,
