...
class TestRecurrentEvents(TransactionCase):
    ...
    def assertEventDates(self, events, dates):
        ...
        for event, (start, stop) in zip(events, dates, strict=True):
...
class TestCreateRecurrentEvents(TestRecurrentEvents):
    ...
    def test_weekly_count(self):
        ...
        self.assertFalse(
            self.event.recurrence_id, "It should be detached from the recurrence"
        )
    ...
    def test_yearly_count(self):
        ...
        self.assertEventDates(
            ...
            [
                ...
                (
                    self.event.start + relativedelta(years=2),
                    self.event.stop + relativedelta(years=2),
                ),
            ],
        )
    ...
    def test_videocall_recurrency(self):
        ...
        self.assertEqual(
            len(rec_events_videocall_locations),
            len(set(rec_events_videocall_locations)),
            "Recurrent events should have different videocall locations",
        )
        self.assertEqual(
            not any(recurrent_events.videocall_channel_id),
            True,
        ...
        self.assertEqual(
            detached_events[0].videocall_channel_id.id,
            self.event.videocall_channel_id.id,
        )
    ...
    def test_backward_pass_dst(self):
        ...
        self.assertEqual(
            self.event.start, datetime(2023, 3, 27, 7, 0, 00)
        )  # Because 2023-03-27 in Europe/Brussels is UTC+2
    ...
    def test_all_day_date(self):
        ...
        self.assertEqual(
            events[0].start_date,
            date(2019, 10, 22),
            "The first event has the initial start date",
        )
        self.assertEqual(
            events[1].start_date,
            date(2019, 10, 29),
    ...
    def test_recurrency_with_this_event(self):
        ...
        updated_events = (
            self.env["calendar.recurrence"]
            .search([("base_event_id", "=", events[5].id)])
            .calendar_event_ids
        )
        self.assertEqual(
            len(updated_events), 2, "It should have 2 events in the recurrence"
        )
        self.assertTrue(
            updated_events[1].recurrency,
            "It should have recurrency in the updated events",
        )
...
class TestUpdateRecurrentEvents(TestRecurrentEvents):
    ...
    def test_shift_future(self):
        ...
        self.assertEventDates(
            new_recurrence.calendar_event_ids,
            [
    ...
    def test_rrule_x_params_no_rrule_prefix(self):
        self.recurrence.rrule = (
            "X-EVOLUTION-ENDDATE=20371102T114500Z:FREQ=WEEKLY;COUNT=720;BYDAY=MO"
        )
        self.assertFalse(self.recurrence.tue)
    ...
    def test_shift_all_base_inactive(self):
        ...
        self.assertFalse(
            self.recurrence.exists(),
            "Inactive event should not create recurrent events",
        )
    ...
    def test_shift_all_with_outlier(self):
        ...
        self.assertTrue(
            outlier.exists(),
            "The outlier should have its date and time updated according to the change.",
        )
    ...
    def test_update_recurrence_future(self):
        ...
        self.assertEqual(
            events[0], self.events[1], "Events on Tuesdays should not have changed"
        )
        self.assertEqual(
            events[2].start,
            self.events[2].start,
            "Events on Tuesdays should not have changed",
        )
        self.assertNotEqual(
    ...
    def test_update_name_all(self):
        ...
        self.events[0].write(
            {"name": "New name", "recurrence_update": "all", "count": "5"}
        )
        new_recurrence = self.env["calendar.recurrence"].search(
    ...
    def test_unlink_recurrence_wizard_next(self):
        ...
        wizard = (
            ...
            .with_context(form_view_ref="calendar.calendar_popover_delete_view")
...
class TestUpdateMonthlyByDate(TestRecurrentEvents):
    ...
    def test_attendees_state_after_update(self):
        ...
        attendee_partner = self.env["res.partner"].create(
            {"name": "attendee", "email": "attendee@email.com"}
        )
        first_event = (
            self.env["calendar.event"]
            .with_user(organizer)
            .create(
                {
                    ...
                    "recurrency": True,
                    "partner_ids": [
                        (4, organizer.partner_id.id),
                        (4, attendee_partner.id),
                    ],
                }
            )
        )
        ...
        self.assertTrue(
            all(attendee.state == "accepted" for attendee in first_event.attendee_ids),
