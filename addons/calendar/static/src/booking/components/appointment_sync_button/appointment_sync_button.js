...
export class AppointmentSyncButton extends Component {
    ...
    static template = "calendar.booking.AppointmentSyncButton";
    ...
    async onConnectCalendar() {
        if (!this.pendingSync) {
            ...
            try {
                const syncResult = await rpc(calendarData.syncRoute, {
                ...
                if (syncResult.status === "need_auth") {
                    ...
                    syncResult.status === calendarData.noNewEventKey ||
                    syncResult.status === "need_refresh"
                ) {
                    this.notification.add(
                        _t(
                            "Your calendar is already configured and was successfully synchronized.",
                        ),
                        { type: "success" },
                    );
                } else {
                    this.notification.add(
                        _t(
                            "The configuration has changed and synchronization is not possible anymore. Please reload the page.",
                        ),
                        { type: "warning" },
                    );
                }
