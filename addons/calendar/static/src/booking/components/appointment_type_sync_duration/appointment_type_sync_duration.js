...
export class AppointmentTypeSyncDuration extends Component {
    static template = "calendar.booking.AppointmentTypeSyncDuration";
    ...
    setup() {
        ...
        onWillStart(async () => {
            if (this.appointmentTypeId) {
                ...
                this.isDefaultDuration =
                    this.props.record.data.duration ===
                    appointmentDuration?.[0].appointment_duration;
            }
        });

        useRecordObserver(async (record) => {
            if (
                record.data.appointment_type_id.id !== this.appointmentTypeId &&
                this.isDefaultDuration
            ) {
                ...
                if (this.appointmentTypeId) {
                    ...
                    if (appointmentDuration.length !== 0) {
                        record.update({
                            duration: appointmentDuration[0].appointment_duration,
                        });
                    }
                }
            }
