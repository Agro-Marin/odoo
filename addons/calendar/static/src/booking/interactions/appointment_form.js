...
export class AppointmentForm extends Interaction {
    ...
    start() {
        this.hasFormDefaultValues = this.getAttendeeFormData().some(
            ([, value]) => value !== "",
        );
        this.mainPhoneQuestion = this.el.querySelector(
            'input[data-is-main-phone-question="True"]',
        );
        if (
            !this.hasFormDefaultValues &&
            localStorage.getItem("appointment.form.values")
        ) {
    ...
    getAttendeeFormData() {
        const formData = new FormData(
            this.el.querySelector("form.appointment_submit_form"),
        );
            ...
            if (emailInfo.invalidEmails.length || emailInfo.emailList.length > 10) {
                this.errorMessage =
                    emailInfo.invalidEmails.length > 0
                        ? _t("Invalid Email")
                        : _t("You cannot invite more than 10 people");
                return;
            } else {
                this.errorMessage = "";
            }
            ...
            if (!this.hasFormDefaultValues) {
                ...
                if (attendeeData.length) {
                    ...
                    if (this.mainPhoneQuestion) {
                        attendeeDataObject["phone"] =
                            attendeeDataObject[this.mainPhoneQuestion.name];
                        delete attendeeDataObject[this.mainPhoneQuestion.name];
                    }
                    localStorage.setItem(
                        "appointment.form.values",
                        JSON.stringify(attendeeDataObject),
                    );
                }
            }
...
registry
    ...
    .add("calendar.appointment_form", AppointmentForm);
