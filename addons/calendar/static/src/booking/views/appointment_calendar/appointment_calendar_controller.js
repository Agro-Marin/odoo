...
patch(AttendeeCalendarController.prototype, {
    setup() {
        ...
        onWillStart(async () => {
            [
                this.appointmentState.data,
                this.isAppointmentManager,
                this.isAppointmentUser,
            ] = await Promise.all([
                rpc("/appointment/appointment_type/get_staff_user_appointment_types"),
                user.hasGroup("calendar.group_appointment_manager"),
                user.hasGroup("calendar.group_appointment_user"),
            ]);
            // Using this context key is a stable workaround, to be improved in master
            this.isResourceView =
                this.props.context.appointment_default_assign_user_attendees === false;
        });
    },
    ...
    async _updateCustomAppointmentSlots() {
        await rpc("/appointment/appointment_type/update_custom", {
        ...
        setTimeout(
            async () =>
                await navigator.clipboard.writeText(
                    this.appointmentState.lastAppointment.url,
                ),
        );
    },
    ...
    async onClickSearchCreateAnytimeAppointment() {
        const anytimeAppointment = await rpc(
