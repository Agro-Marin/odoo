...
patch(AttendeeCalendarModel.prototype, {
    ...
    buildRawRecord(partialRecord) {
        if ("default_duration" in this.meta.context) {
            ...
            if (
                partialRecord.start &&
                (!partialRecord.end || !partialRecord.end.isValid) &&
                !partialRecord.isAllDay
            ) {
                partialRecord.end = partialRecord.start.plus({
    ...
    processPartialSlotRecord(record) {
        if (!record.end || !record.end.isValid) {
            if (record.isAllDay) {
                ...
                record.end = record.start.plus({
                    minutes: this.defaultSlotDurationMinutes,
                });
            }
        }
        if (!record.isAllDay) {
            ...
            if (record.end.diff(record.start, "hours").hours >= 24) {
                record.title =
                    record.start.toFormat("EEE d, t") +
                    " - " +
                    record.end.toFormat("EEE d, t");
            }
            if (record.start && record.end) {
                const datesInterval = luxon.Interval.fromDateTimes(
                    record.start,
                    record.end,
                );
            ...
            if (!isSameDay && record.start.hasSame(record.end, "month")) {
                ...
                record.title =
                    record.start.toFormat("LLLL d") + "-" + record.end.toFormat("d, y");
            } else {
