...
from odoo.addons.sms_twilio.tools.sms_twilio import (
    ...
    get_twilio_status_callback_url,
)
...
class SmsApiTwilio(SmsApiBase):
    ...
    def _sms_twilio_send_request(self, session, to_number, body, uuid):
        ...
        data = {
            ...
            or "",  # avoid 'False', to have clear missing From error
            "To": to_number,
            "Body": body,
    ...
    def _send_sms_batch(self, messages, delivery_reports_url=False):
        ...
        for message in messages:
            ...
            for number_info in message.get("numbers") or []:
                ...
                response = self._sms_twilio_send_request(
                ...
                if response is not None:
                    ...
                    if not response.ok or response_json.get("error"):
                        failure_type = self._twilio_error_code_to_odoo_state(
                            response_json
                        )
                        error_message = (
                            response_json.get("message")
                            or response_json.get("error_message")
                            or self._get_sms_api_error_messages().get(failure_type)
                        )
    ...
    def _twilio_error_code_to_odoo_state(self, response_json):
        ...
        if error_code in (
            ...
            21614,
            21265,
        ):  # See https://www.twilio.com/docs/errors/xxxxx
        ...
        _logger.warning(
            'Twilio SMS: Unknown error "%s" (code: %s)',
            response_json.get("message"),
            error_code,
        )
        return "unknown"
