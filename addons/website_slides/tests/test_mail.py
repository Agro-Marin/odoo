...
class TestSlidesMail(SlidesCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_template_slides = (
            cls.env["mail.template"]
            .with_user(cls.user_manager)
            .create(
                {
                    **common_vals,
        ...
        cls.slide.write(
            {
                "partner_ids": [
                    (
                        ...
                        [
                            cls.customer.id,
                            cls.user_emp.partner_id.id,
                            cls.user_portal.partner_id.id,
                        ],
                    )
                ],
            }
    ...
    def test_slide_slide_get_suggested_recipients(self):
        ...
        self.assertFalse(
            suggested_recipient,
            "The user_id is already subscribed => no suggested_recipients",
        )

    def test_slide_and_channel_templates(self):
        values = [
            self.channel.with_user(self.user_manager),
            self.slide.with_user(self.user_manager),
        ]
        templates = [
        ...
        for channel_or_slide, template, expected_value, error_message in zip(
            ...
            with self.subTest(
                ...
                self.assertEqual(
                    message.notified_partner_ids, expected_value, error_message
                )
