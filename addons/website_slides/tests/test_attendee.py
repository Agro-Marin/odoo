...
class TestAttendee(common.SlidesCase):
    ...
    def test_attendee_course_completion_values(self):
        ...
        (self.slide | self.slide_2 | self.slide_3).with_user(
            self.user_portal
        ...
        self.channel._action_add_members(
            self.user_portal.partner_id, member_status="invited"
        )
    ...
    def test_enroll_to_course(self):
        ...
        self.assertIn(
            user_portal_channel_partner.id, self.channel.channel_partner_ids.ids
        )
    ...
    def test_invite_to_course(self):
        ...
        self.assertFalse(
            user_portal_channel_partner.id in self.channel.channel_partner_ids.ids
        )
    ...
    def test_join_invite_enroll_channel(self):
        ...
        self.channel.with_user(self.user_portal)._action_add_members(
            user_portal_partner
        )
        self.assertFalse(user_portal_partner.id in self.channel.partner_ids.ids)
        ...
        with self.assertRaises(AccessError):
            self.channel.with_user(self.user_portal)._action_add_members(
                user_portal_partner, raise_on_access=True
            )
        ...
        self.assertIn(
            user_portal_partner, self.channel.channel_partner_all_ids.partner_id
        )
        self.assertFalse(user_portal_partner.id in self.channel.partner_ids.ids)
        # Invited partner can join the course and enroll itself. Sudo is used in controller if invited.
        self.assertTrue(self.channel.with_user(self.user_portal).is_member_invited)
        self.channel.with_user(self.user_portal).sudo()._action_add_members(
            user_portal_partner
        )
        self.assertEqual(user_portal_channel_partner.member_status, "joined")
        self.assertIn(user_portal_partner.id, self.channel.partner_ids.ids)
        self.assertIn(
            self.user_portal.partner_id.id, self.channel.message_partner_ids.ids
        )
    ...
    def test_partners_and_search_on_slide_channel(self):
        ...
        invited_cp_channel_ids = self.env["slide.channel"].search(
            [("partner_ids", "=", invited_cp.partner_id.id)]
        )
        self.assertFalse(self.channel in invited_cp_channel_ids)
        joined_cp_channel_ids = self.env["slide.channel"].search(
            [("partner_ids", "=", joined_cp.partner_id.id)]
        )
    ...
    def test_copy_partner_not_course_member(self):
        ...
        self.assertEqual(
            member_before,
            member_after,
            "Duplicating the contact should not create a new member",
        )
...
class TestAttendeeCase(HttpCaseWithUserPortal):
    def setUp(self):
        ...
        self.channel_connect, self.channel_members, self.channel_link = (
            self.env["slide.channel"]
            .with_user(self.user_admin)
            .create(
                [
        ...
        # Enrolled by default
        self.channel_partner_emp, self.channel_partner_no_user = self.env[
            ...
            [
                ...
                {"channel_id": self.channel.id, "partner_id": self.partner_no_user.id},
            ]
        )
    ...
    def test_direct_enroll_link_redirection(self):
        ...
        self.assertEqual(
            (res.status_code, url.path),
            (HTTPStatus.SEE_OTHER, "/web/login"),
        ...
        self.assertEqual(
            (res.status_code, url.path),
            (HTTPStatus.SEE_OTHER, "/web/signup"),
        ...
        self.assertEqual(
            url.path,
            f"/slides/{self.env['ir.http']._slug(self.channel)}",
        ...
        self.assertEqual(res.status_code, 200)
        self.assertFalse(
            f"slides/{self.env['ir.http']._slug(self.channel)}" in res.url,
    ...
    def test_invite_route_errors_handling(self):
        ...
        self.assertURLEqual(
            res.url,
            "/slides?invite_error=partner_fail",
        ...
        # No such channel
        max_channel_id = (
            self.env["slide.channel"].search([], order="id desc", limit=1).id
        )
        invite_url = f"/slides/{max_channel_id + 1}"
        res = self.url_open(invite_url)
        self.assertEqual(res.status_code, 200)
        self.assertIn(
            "/slides?invite_error=no_channel",
            res.url,
    ...
    def test_members_invitation_expiration(self):
        ...
        self.channel_partner_emp.write(
            {
                "member_status": "invited",
                "last_invitation_date": fields.Datetime.subtract(
                    fields.Datetime.now(), months=3, days=5
                ),
            }
        )
        self.channel.visibility = "members"
        ...
        self.assertTrue(
            self.channel_partner_emp.exists(),
            "Memberships with progress should never be removed.",
        )
        self.assertFalse(
            channel_partner_portal.exists(),
            "Expired invitations with no progress should be removed by the GC.",
        )
        self.assertTrue(
            self.channel_partner_no_user.exists(),
            "Joined members should never be removed.",
        )
        ...
        self.assertFalse(
            channel_partner_portal.exists(),
            "Expired invitations should be removed, even if archived",
        )
        self.assertTrue(
            self.channel_partner_emp.exists(),
