        ...
        def can_read_slides_content(user, can_read):
            ...
            for url in [
                ...
                if can_read:
                    ...
                    self.assertTrue(
                        ...
                        or response.content in (image_placeholder, b""),
            ...
            for url in [
                ...
                if can_read:
                    ...
                    self.assertTrue(
                        response.status_code == 404
                        or response.url.endswith("/slides?invite_error=no_rights"),
...
class TestAccessFeatures(common.SlidesCase):
    ...
    def test_channel_auto_subscription(self):
        ...
        self.assertEqual(
            channel.partner_ids,
            user_employees.mapped("partner_id") | new_user.partner_id,
        )

        new_user_2 = self.env["res.users"].create(
            {"name": "NewUser2", "login": "NewUser2", "group_ids": [(5, 0)]}
        )
        channel.invalidate_model()
        self.assertEqual(
            channel.partner_ids,
        ...
        self.assertEqual(
            channel.partner_ids,
            user_employees.mapped("partner_id")
            | new_user.partner_id
            | new_user_2.partner_id,
        )

        new_user_3 = self.env["res.users"].create(
            {"name": "NewUser3", "login": "NewUser3", "group_ids": [(5, 0)]}
        )
        channel.invalidate_model()
        self.assertEqual(
            channel.partner_ids,
    ...
    def test_channel_access_fields_officer(self):
        ...
        channel_manager = self.channel.with_user(self.user_manager)
        channel_manager.sudo().write(
            {"upload_group_ids": [(5, 0)], "user_id": self.user_manager.id}
        )
    ...
    def test_resource_access(self):
        ...
        resource1, resource2 = (
            self.env["slide.slide.resource"]
            .with_user(self.user_officer)
            .create([resource_values for _ in range(2)])
        )
        resource3 = (
            self.env["slide.slide.resource"]
            .with_user(self.user_officer)
        ...
        with self.assertRaises(AccessError):
            self.env["slide.slide.resource"].with_user(user_officer_other).create(
                resource_values
            )
        with self.assertRaises(AccessError):
