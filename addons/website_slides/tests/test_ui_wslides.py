...
class TestUICommon(HttpCaseGamification, HttpCaseWithUserPortal):
    ...
    def setUp(self):
        ...
        img_content = base64.b64encode(
            file_open(
                "website_slides/static/src/img/slide_demo_gardening_1.jpg", "rb"
            ).read()
        )
...
class TestUi(TestUICommon):
    ...
    def test_course_member_employee(self):
        ...
        user_demo.write(
            {"karma": 1, "group_ids": [(6, 0, self.env.ref("base.group_user").ids)]}
        )
    ...
    def test_course_review_comment(self):
        ...
        self.start_tour(
            "/slides", "course_reviews_comment", login=self.user_admin.login
        )
    ...
    def test_course_review_modification(self):
        ...
        self.start_tour(
            "/slides", "course_review_modification", login=self.user_portal.login
        )
...
class TestUiMemberInvited(TestUICommon):
    ...
    def test_invite_check_channel_preview_as_public_connected_on_invite(self):
        ...
        self.channel.visibility = "connected"
        self.start_tour(
            self.portal_invite_url, "invite_check_channel_preview_as_public", login=None
        )
