...
class TestSlidesManagement(slides_common.SlidesCase, HttpCase):
    ...
    def test_archive(self):
        ...
        for slide in self.channel.slide_ids:
            self.assertTrue(
                slide.active, "All slide should be archived when a channel is archived"
            )
            self.assertTrue(
                slide.is_published,
                "All slide should be unpublished when a channel is archived",
            )
        ...
        for slide in self.channel.slide_ids:
            self.assertFalse(
                slide.active, "All slides should be archived when a channel is archived"
            )
    ...
    def test_mail_completed(self):
        ...
        for user in self.user_officer | self.user_emp:
            self.assertTrue(
                any(
                    ...
                    and user.partner_id in mail.recipient_ids
        ...
        self.assertFalse(
            any(
                mail.model == "slide.channel.partner"
                and self.user_portal.partner_id in mail.recipient_ids
                for mail in created_mails
            )
        )
    ...
    def test_mail_completed_with_different_templates(self):
        ...
        channel_2 = self.env["slide.channel"].create(
            {
                "name": "Test Course 2",
                "slide_ids": [(0, 0, {"name": "Test Slide 2", "is_published": True})],
    ...
    def test_slides_prepare_preview(self):
        ...
        slide = self.channel.slide_ids.filtered(
            lambda slide: slide.name == "memory_record_for_computed_fields"
        )
        self.assertFalse(slide)

    def test_unlink_slide_channel(self):
        self.assertTrue(
            any(s.has_questions for s in self.channel.slide_content_ids),
    ...
    def test_mail_completed_not_on_unpublishing_or_unlinking_slides(self):
        ...
        self.assertGreaterEqual(
            len(slides_initially_published),
            2,
        ...
        self.assertFalse(
            were_emails_sent(),
            "Participants should not receive emails when a slide is unpublished.",
        )
        ...
        self.assertFalse(
            were_emails_sent(),
            "Participants should not receive emails when a slide is deleted.",
        )
...
class TestSequencing(slides_common.SlidesCase):
    ...
    def test_category_update(self):
        ...
        self.assertEqual(
            self.channel.slide_content_ids, self.slide | self.slide_2 | self.slide_3
        )
        ...
        self.assertEqual(
            [s.id for s in self.channel.slide_ids],
        ...
        channel_2 = self.env["slide.channel"].create({"name": "Test2"})
        new_category = self.env["slide.slide"].create(
            {
    ...
    def test_channel_enroll_policy(self):
        channel = self.env["slide.channel"].create(
            {
                "name": "Test Course 2",
                "slide_ids": [(0, 0, {"name": "Test Slide 1"})],
            }
        )
        ...
        self.assertEqual(
            copied_channel.enroll,
            "invite",
            "Copied channel should have the same enroll field value",
        )
    ...
    def test_duplicate_slide_sets_sequence_to_zero(self):
        ...
        self.assertEqual(
            copied_slide.sequence,
            0,
            "When copying a single slide its sequence should be set to 0",
        )
