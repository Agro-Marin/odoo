 ...
 */
const VideoPlayerYouTube = publicWidget.Widget.extend({
    ...
    _onPlayerStateChange: function (event) {
        ...
        if (event.data !== YT.PlayerState.ENDED) {
            ...
            self.tid = setInterval(function () {
                ...
                if (
                    self.totalVideoTime &&
                    self.currentVideoTime > self.totalVideoTime - 30
                ) {
                    clearInterval(self.tid);
                    if (
                        self.slide.isMember &&
                        !self.slide.hasQuestion &&
                        !self.slide.completed
                    ) {
 ...
 */
const VideoPlayerVimeo = publicWidget.Widget.extend({
...
const Sidebar = publicWidget.Widget.extend({
    ...
    _getCurrentIndex: function () {
        ...
        const currentIndex = this.slideEntries.findIndex(
 ...
 */
const Fullscreen = SlideCoursePage.extend({
    ...
    attachTo: function () {
        ...
        defs.push(
            this.sidebar.attachTo(this.el.querySelector(".o_wslides_fs_sidebar")),
        );
    ...
    _renderSlide: async function () {
        ...
        try {
            ...
            if (slide.category === "quiz" || slide.isQuiz) {
                ...
                const QuizWidget = new Quiz(this, slide, this.channel);
...
publicWidget.registry.websiteSlidesFullscreenPlayer = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        const fullscreen = new Fullscreen(
            this,
            this._getSlides(),
            this._getCurrentSlideID(),
            this._extractChannelData(),
        );
