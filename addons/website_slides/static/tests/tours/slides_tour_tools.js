...
const addSection = function (sectionName, backend = false) {
    ...
    return [
        ...
        },
        {
            ...
            trigger:
                prefix +
                'div.o_wslides_slide_list_category_header:contains("' +
                sectionName +
                '")',
            run: "click",
        },
    ...
    if (saveAsDraft) {
        base_steps = [].concat(base_steps, [
            ...
            },
            {
                ...
                trigger:
                    prefix + 'footer.modal-footer button:contains("Save as Draft")',
                run: "click",
            },
        ]);
    } else {
...
const addArticleToSection = function (sectionName, pageName, backend) {
    ...
    return [
        ...
        },
        {
            ...
            trigger: prefix + ".o_select_menu_toggler:last",
            run: "click",
        },
        {
...
const compareBase64Content = async (url, name, type, expectedContent) => {
    ...
    const file = new File([blob], name, { type: type });
    const actualContent = await getDataURLFromFile(file).then(
            ...
            run: async () => {
                const uploadInput = backend
                    ? document
                          ...
                          .contentWindow.document.getElementById("upload")
                    : document.getElementById("upload");
                await fillInFileInput(
                    uploadInput,
                    "Overview.png",
                    "image/png",
                    testPngImage,
...
const addPdfToSection = function (sectionName, pageName, backend) {
    ...
    return [
        ...
        stepUtils.waitIframeIsReady(),
        {
            ...
            trigger:
                (backend ? ":iframe " : "") +
                ".o_wslides_fs_content :iframe #PDFSlideViewer",
            run: async (helpers) => {
                if (
                    await compareBase64Content(
                        helpers.anchor.getAttribute("data-slideurl"),
                        "Exercise.pdf",
                        "application/pdf",
                        testPdf,
                    )
...
const addExistingCourseTag = function (backend = false) {
    const prefix = backend ? ":iframe " : "";
    return [
...
const addNewCourseTag = function (courseTagName, backend) {
    const prefix = backend ? ":iframe " : "";
    return [
        ...clickOnAddTagDropdown(prefix),
        {
