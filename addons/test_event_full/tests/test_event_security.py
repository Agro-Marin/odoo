...
class TestEventSecurity(TestEventFullCommon):
    ...
    def test_event_access_event_registration(self):
        ...
        self.assertEqual(
            registration.event_id.name,
            event.name,
            "Registration users should be able to read",
        )
    ...
    def test_event_question_access(self):
        ...
        for user in restricted_users:
            with self.assertRaises(
                AccessError,
                msg=f"{user.name} should not have access to questions of unpublished events",
            ):
                question.with_user(user).read(["title"])
            with self.assertRaises(
                AccessError,
                msg=f"{user.name} should not have access to answers of unpublished events",
            ):
    ...
    def test_multi_companies(self):
        ...
        registration_company_1, registration_company_2 = self.env[
            "event.registration"
        ].create(
            [
                {
        ...
        self.assertIn(
            event_company_1,
            result,
            "You must be able to read the events in your company",
        )
        self.assertNotIn(
            event_company_2,
            result,
            "You must not be able to read events outside of your company",
        )

        result = self.env["event.registration"].with_user(user_company_1).search([])
        self.assertIn(
            registration_company_1,
            result,
            "You must be able to read the registrations in your company",
        )
        self.assertNotIn(
            registration_company_2,
            result,
