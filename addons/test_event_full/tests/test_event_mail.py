                ...
                "event_type_mail_ids": [
                    ...
                    (
                        ...
                        0,
                        {
                            ...
                            % self.env["ir.model.data"]._xmlid_to_res_id(
                                "event_sms.sms_template_data_event_registration"
                            ),
                        },
                    ),
                ],
...
class TestEventMailSchedule(TestEventMailCommon):
    def test_event_mail_before_trigger_sent_count(self):
        ...
        with (
            self.mock_datetime_and_now(self.event_date_begin - timedelta(days=2)),
        ...
        self.assertEqual(
            len(self._new_mails),
            2,
            "Mails should not be sent to draft or cancel registrations",
        )
        self.assertEqual(
            len(self._new_sms),
            2,
            "SMS should not be sent to draft or cancel registrations",
        )
    ...
    def test_schedule_event_scalability(self):
        ...
        after_mail = test_event.event_mail_ids.filtered(
            lambda s: s.interval_type == "after_event" and s.notification_type == "mail"
        )
        self.assertEqual(len(after_mail), 1)
        self.assertEqual(after_mail.mail_count_done, 0)
        self.assertFalse(after_mail.mail_done)
        after_sms = test_event.event_mail_ids.filtered(
            lambda s: s.interval_type == "after_event" and s.notification_type == "sms"
        )
        self.assertEqual(len(after_sms), 1)
        self.assertEqual(after_sms.mail_count_done, 0)
        self.assertFalse(after_sms.mail_done)
        before_mail = test_event.event_mail_ids.filtered(
            lambda s: (
                s.interval_type == "before_event" and s.notification_type == "mail"
            )
        )
        self.assertEqual(len(before_mail), 1)
        self.assertEqual(before_mail.mail_count_done, 0)
        self.assertFalse(before_mail.mail_done)
        before_sms = test_event.event_mail_ids.filtered(
            lambda s: s.interval_type == "before_event" and s.notification_type == "sms"
        )
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "mail.render.cron.limit", cron_limit
        )
        ...
        self.assertEqual(
            ...
            8,
            "Batch of 5 to make 20 registrations: 4 calls / scheduler",
        )
    ...
    def test_schedule_subscription_scalability(self):
        ...
        sub_mail = test_event.event_mail_ids.filtered(
            lambda s: (
                s.interval_type == "after_sub"
                and s.interval_unit == "now"
                and s.notification_type == "mail"
            )
        )
        self.assertEqual(len(sub_mail), 1)
        self.assertEqual(sub_mail.mail_count_done, 0)
        sub_sms = test_event.event_mail_ids.filtered(
            lambda s: (
                s.interval_type == "after_sub"
                and s.interval_unit == "now"
                and s.notification_type == "sms"
            )
        )
        ...
        # cron should have been triggered for the remaining registrations
        self.assertSchedulerCronTriggers(
            capture, [self.reference_now + timedelta(hours=1)] * 2
        )
        ...
        self.assertEqual(
            mock_exec.call_count,
            4,
            "Batch of 5 to make 10 remaining registrations: 2 calls / scheduler",
        )
...
class TestEventSaleMail(TestEventFullCommon):
    def test_event_mail_on_sale_confirmation(self):
        ...
        self.assertEqual(
            self.test_event.organizer_id, self.test_event.env.company.partner_id
        )
    ...
    def test_registration_template_body_translation(self):
        ...
        self.env.ref("event.event_subscription").with_context(
            lang="fr_BE"
        ).body_html = "Bonjour"
        with self.mock_mail_gateway(mail_unlink_sent=False):
            self.env["event.registration"].create(
                {"event_id": test_event.id, "partner_id": self.partners[0].id}
            )
