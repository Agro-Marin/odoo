...
class TestEventEvent(TestEventFullCommon):
        ...
        event.write(
            {
                "event_ticket_ids": [
                    Command.update(
                        ...
                        {
                            "start_sale_datetime": self.reference_now
                            + timedelta(hours=1)
                        },
                    ),
                    Command.update(
    ...
    def test_event_kanban_state_on_stage_change(self):
        ...
        self.assertEqual(
            ...
            "normal",
            'kanban state should reset to "normal" on stage change',
        )
        self.assertEqual(
            test_event_2.kanban_state,
            "cancel",
            "kanban state should not reset on stage change",
        )
    ...
    def test_event_seats_and_schedulers(self):
        ...
        ticket_1 = test_event.event_ticket_ids.filtered(
            lambda ticket: ticket.name == "Ticket1"
        )
        ticket_2 = test_event.event_ticket_ids.filtered(
            lambda ticket: ticket.name == "Ticket2"
        )
