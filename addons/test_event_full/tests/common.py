    ...
    def setUpClass(cls):
        ...
        cls.tags = cls.env["event.tag"].create(
            [
                ...
                {
                    "category_id": cls.tag_categories[0].id,
                    "color": 0,
                    "name": "PubTag2",
                },
        ...
        # Event type
        # ------------------------------------------------------------
        test_registration_report = cls.env.ref(
            "test_event_full.event_registration_report_test"
        )
        subscription_template = cls.env.ref("event.event_subscription")
        subscription_template.write(
            {"report_template_ids": [(6, 0, test_registration_report.ids)]}
        )
        cls.test_event_type = cls.env["event.type"].create(
            {
                ...
                "event_type_booth_ids": [
                    (
                        ...
                        0,
                        {
                            "booth_category_id": cls.event_booth_categories[0].id,
                            "name": "Standard Booth",
                        },
                    ),
                    (
                    ...
                    (
                        ...
                        0,
                        {
                            "booth_category_id": cls.event_booth_categories[1].id,
                            "name": "Premium Booth",
                        },
                    ),
                    (
                ...
                "event_type_mail_ids": [
                    (
                        ...
                        0,
                        {
                            "interval_unit": "now",  # right at subscription
        ...
        ticket_1 = cls.test_event.event_ticket_ids.filtered(
            lambda t: t.name == "Ticket1"
        )
        ticket_2 = cls.test_event.event_ticket_ids.filtered(
            lambda t: t.name == "Ticket2"
        )
        ...
        cls.website = cls.env["website"].search(
            [("company_id", "=", cls.company_admin.id)], limit=1
        )
                ...
                "registration_answer_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "value_answer_id": cls.test_event.question_ids[0]
                            .answer_ids[(idx % 2)]
                            .id,
                        },
                    ),
                    (
                        ...
                        0,
                        {
                            ...
                            "value_answer_id": cls.test_event.question_ids[1]
                            .answer_ids[(idx % 2)]
                            .id,
                        },
                    ),
        ...
        for registration in registrations:
            ...
            for answer in registration.registration_answer_ids:
                ...
                if answer.question_type == "simple_choice":
                    ...
                    self.assertIn(
                        answer.value_text_box, lead.description
                    )  # better: check multi line
...
class TestWEventCommon(HttpCaseWithUserDemo, HttpCaseWithUserPortal, MockVisitor):
    def setUp(self):
        ...
        self.env["event.event"].search([("name", "like", "Online Reveal")]).write(
        ...
        self.event = self.env["event.event"].create(
            {
                ...
                "date_tz": "Europe/Brussels",
                "date_begin": datetime.combine(self.reference_now, time(5, 0))
                - timedelta(days=1),
                "date_end": datetime.combine(self.reference_now, time(15, 0))
                + timedelta(days=1),
                # ticket informations
                "event_ticket_ids": [
                    (
