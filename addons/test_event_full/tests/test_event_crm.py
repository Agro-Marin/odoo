...
class TestEventCrm(TestEventFullCommon):
    ...
    def setUpClass(cls):
        ...
        (cls.test_rule_attendee + cls.test_rule_order).write(
    ...
    def test_event_crm_sale_customer(self):
        ...
        self.assertEqual(
            len(self.test_event.registration_ids),
            self.TICKET1_COUNT + self.TICKET2_COUNT,
        )
        self.assertEqual(len(self.test_rule_order.lead_ids), 1)  # no new lead created
        self.assertEqual(
            self.test_rule_order_done.lead_ids, self.env["crm.lead"]
        )  # this one still not triggered

        # check existing lead has been updated with new registrations
        self.assertLeadConvertion(
            self.test_rule_order,
            self.test_event.registration_ids,
    ...
    def test_event_crm_sale_mixed_group(self):
        ...
        mixed_reg_vals = [
            ...
            dict(
                self.website_customer_data[1],
                partner_id=customer_so.partner_id.id,
                sale_order_line_id=customer_so.line_ids[0].id,
            ),
            dict(
                self.website_customer_data[2],
                partner_id=public_so.partner_id.id,
                sale_order_line_id=public_so.line_ids[0].id,
            ),
            dict(
                self.website_customer_data[3],
                partner_id=public_so.partner_id.id,
                sale_order_line_id=public_so.line_ids[0].id,
            ),
        ]
        ...
        public_regs = self.test_event.registration_ids.filtered(
            lambda reg: reg.sale_order_id == public_so
        )
        self.assertEqual(len(public_regs), 2)
        customer_regs = self.test_event.registration_ids.filtered(
            lambda reg: reg.sale_order_id == customer_so
        )
    ...
    def test_event_update_lead(self):
                ...
                "registration_answer_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "value_answer_id": self.test_event.question_ids[1]
                            .answer_ids[0]
                            .id,
                        },
                    )
        ...
        self.assertIn(
            self.test_event.question_ids[1].answer_ids[0].name,
            event_registration.lead_ids[0].description,
