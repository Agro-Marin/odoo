...
class TestWebsiteBlogFlow(TestWebsiteBlogCommon):
    ...
    def test_website_blog_followers(self):
        ...
        self.assertIn(
            self.user_blogmanager.partner_id,
            self.test_blog.message_partner_ids,
            "website_blog: blog create should be in the blog followers",
        )
        self.test_blog.message_subscribe(
        ...
        publish_message = next(
            (
                m
                for m in self.test_blog_post.blog_id.message_ids
                if m.subtype_id.id == self.ref("website_blog.mt_blog_blog_published")
            ),
            None,
        )
        self.assertEqual(
...
class TestWebsiteBlogTranslationFlow(HttpCase, TestWebsiteBlogCommon):
    ...
    def test_teaser_manual(self):
        blog_post_parseltongue = self.test_blog_post.with_context(
            lang=self.parseltongue.code
        )

        # No manual teaser, ensure everything works as expected in multi langs
        self.test_blog_post.content = "English Content"
        self.test_blog_post.update_field_translations(
            "content",
            {
        ...
        self.assertFalse(self.test_blog_post.teaser_manual)
        self.assertEqual(
            blog_post_parseltongue.teaser_manual, "Parseltongue Teaser Manual"
        )
        ...
        self.assertEqual(
            blog_post_parseltongue.teaser,
            "Parseltongue Content...",
            "Should fallback again to content",
        )
        self.assertEqual(self.test_blog_post.teaser_manual, "English Teaser Manual")
        self.assertFalse(
            blog_post_parseltongue.teaser_manual, "Should have been emptied"
        )
    ...
    def test_update_field_translation(self):
        ...
        blog_post = (
            self.env["blog.post"]
            .with_context(lang=br_lang.code)
            .create(
                {
                    "name": "Test Blog",
        ...
        blog_post.update_field_translations(
            "content", {en_lang.code: {"Todos os blogs": "All blogs"}}
        )
        self.assertEqual(
            "Todos os blogs", blog_post.with_context(lang=br_lang.code).content
        )
        ...
        self.assertEqual(
            "Updated blogs", blog_post.with_context(lang=en_lang.code).content
        )
