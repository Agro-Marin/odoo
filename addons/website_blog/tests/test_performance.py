...
class TestBlogPerformance(UtilPerf):
    ...
    def test_20_perf_sql_blog_bigger_data_scaling(self):
        ...
        blog_tags = BlogTag.create(
            [{"name": "Blog Tag Test %s" % i} for i in range(1, 20)]
        )
        BlogPost.create(
            [
                {
                    "name": "Blog Post Test %s" % i,
                    "is_published": True,
                    "blog_id": blogs[i % 2].id,
                }
                for i in range(1, 20)
            ]
        )
    ...
    def test_30_perf_sql_blog_bigger_data_scaling(self):
        ...
        blogs = self.env["blog.blog"].search([])
        blog_tags = BlogTag.create(
            [{"name": "New Blog Tag Test %s" % i} for i in range(1, 50)]
        )
        BlogPost.create(
            [
                {
                    "name": "New Blog Post Test %s" % i,
                    "is_published": True,
                    "blog_id": blogs[random.randint(0, 1)].id,
                }
            ...
            blog_post.write(
                {
                    "tag_ids": [
                        [
                            ...
                            0,
                            random.choices(
                                blog_tags.ids, k=random.randint(0, len(blog_tags))
                            ),
                        ]
                    ]
                }
