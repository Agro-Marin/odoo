...
class TestWebsiteBlogUi(odoo.tests.HttpCase, TestWebsiteBlogCommon):
    ...
    def test_admin(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url("/"), "blog", login="admin"
        )

    def test_blog_post_tags(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/blog"),
            "blog_tags",
            login="admin",
        )
    ...
    def test_blog_posts_dynamic_snippet_visibility(self):
        ...
        def start_visibility_tour(blog_posts, publish):
            ...
            tour_before, tour_after = (
                ("empty", "visible") if publish else ("visible", "empty")
            )
            self.start_tour(
                url, f"blog_posts_dynamic_snippet_{tour_before}", login="admin"
            )
