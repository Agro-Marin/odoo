...
class WebsiteBlog(http.Controller):
    ...
    def nav_list(self, blog=None):
        ...
        groups = request.env["blog.post"]._read_group(dom, groupby=["post_date:month"])
        ...
        for [start] in groups:
            year = babel.dates.format_datetime(
                start, format="yyyy", tzinfo=tzinfo, locale=locale
            )
            res[year].append(
                {
                    "date_begin": start.strftime(fmt),
                    "date_end": (
                        start + models.READ_GROUP_TIME_GRANULARITY["month"]
                    ).strftime(fmt),
                    "month": babel.dates.format_datetime(
                        start, format="MMMM", tzinfo=tzinfo, locale=locale
                    ),
                    "year": year,
                }
            )
    ...
    def _get_blog_post_search_options(
        self,
        blog=None,
        active_tags=None,
        date_begin=None,
    ...
    def _prepare_blog_values(
        ...
        **post,
        ...
        if date_begin and date_end:
            domain &= Domain("post_date", ">=", date_begin) & Domain(
                "post_date", "<=", date_end
            )
        active_tag_ids = (
            tags and [request.env["ir.http"]._unslug(tag)[1] for tag in tags.split(",")]
        ) or []
        active_tags = BlogTag
        if active_tag_ids:
            active_tags = BlogTag.browse(active_tag_ids).exists()
            fixed_tag_slug = ",".join(
                request.env["ir.http"]._slug(t) for t in active_tags
            )
        ...
        if request.env.user.has_group("website.group_website_designer"):
            ...
            if state == "published":
                domain &= Domain("website_published", "=", True) & Domain(
                    "post_date", "<=", fields.Datetime.now()
                )
            elif state == "unpublished":
                domain &= Domain("website_published", "=", False) | (
                    "post_date",
                    ">",
                    fields.Datetime.now(),
                )
        ...
        tag_category = tools.lazy(
            lambda: sorted(
                all_tags.mapped("category_id"),
                key=lambda category: category.name.upper(),
            )
        )
        other_tags = tools.lazy(
            lambda: sorted(
                all_tags.filtered(lambda x: not x.category_id),
                key=lambda tag: tag.name.upper(),
            )
        )
    ...
    def blog(self, blog=None, tag=None, page=1, search=None, **opt):
        ...
        blogs = tools.lazy(
            lambda: Blog.search(request.website.website_domain(), order="sequence")
        )

        if not blog and len(blogs) == 1:
            url = QueryURL(
                "/blog/%s" % request.env["ir.http"]._slug(blogs[0]),
                search=search,
                **opt,
            )()
    ...
    def old_blog_post(self, blog, blog_post, **post):
        ...
        return request.redirect(
            "/blog/%s/%s"
            % (
                request.env["ir.http"]._slug(blog),
                request.env["ir.http"]._slug(blog_post),
            ),
            code=301,
        )
    ...
    def blog_post(
        ...
        if blog_post.blog_id.id != blog.id:
            return request.redirect(
                "/blog/%s/%s"
                % (
                    request.env["ir.http"]._slug(blog_post.blog_id),
                    request.env["ir.http"]._slug(blog_post),
                ),
                code=301,
            )
        ...
        next_post_id = (
            all_post_ids[(current_blog_post_index + 1) % nb_posts]
            if nb_posts > 1
            else None
        )
        ...
        if blog_post.id not in request.session.get("posts_viewed", []):
            if blog_post._increment_fields_skiplock("visits"):
