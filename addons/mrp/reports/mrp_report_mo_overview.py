...
class ReportMrpReport_Mo_Overview(models.AbstractModel):
    ...
    def _get_move_bom_demand(self, production, move_raw):
        plan = self._get_bom_plan(production)
        line = move_raw.bom_line_id
        if line not in plan["demands"]:
            return False
        return line.product_uom_id._get_quantity_report(
            plan["demands"][line] / max(plan["moves_per_line"][line], 1),
    ...
    def _get_report_data(self, production_id):
        ...
        if production.company_id != self.env.company:
            self = self.with_company(production.company_id)
        self = self.with_context(
            bom_cost_share_cache=self.env["mrp.bom"]._get_explosion_scratch()
        )
        production = production.with_env(self.env).with_context(
            warehouse_id=production.warehouse_id.id
        )
        ...
        initial_mo_cost, initial_bom_cost, initial_real_cost = self._get_cost_sums(
    ...
    def _get_report_extra_lines(self, summary, components, operations, production):
        ...
        unit_mo_cost = currency.round(
            summary.get("mo_cost", 0) / (summary.get("quantity") or 1)
        )
        unit_bom_cost = currency.round(
            summary.get("bom_cost", 0) / (summary.get("quantity") or 1)
        )
        unit_real_cost = currency.round(
            summary.get("real_cost", 0) / (summary.get("quantity") or 1)
        )
        extras = {
        ...
        if production.state == "done":
            ...
            extras["total_bom_cost_components"] = sum(
                compo.get("summary", {}).get("bom_cost", 0.0) for compo in components
            )
            extras["total_real_cost_components"] = sum(
                compo.get("summary", {}).get("real_cost", 0.0) for compo in components
            )
            extras["unit_mo_cost_components"] = (
                extras["total_mo_cost_components"] / production_qty
            )
            extras["unit_bom_cost_components"] = (
                extras["total_bom_cost_components"] / production_qty
            )
            extras["unit_real_cost_components"] = (
                extras["total_real_cost_components"] / production_qty
            )
            extras["total_mo_cost_operations"] = operations.get("summary", {}).get(
                "mo_cost", 0.0
            )
            extras["total_bom_cost_operations"] = operations.get("summary", {}).get(
                "bom_cost", 0.0
            )
            extras["total_real_cost_operations"] = operations.get("summary", {}).get(
                "real_cost", 0.0
            )
            extras["unit_mo_cost_operations"] = (
                extras["total_mo_cost_operations"] / production_qty
            )
            extras["unit_bom_cost_operations"] = (
                extras["total_bom_cost_operations"] / production_qty
            )
            extras["unit_real_cost_operations"] = (
                extras["total_real_cost_operations"] / production_qty
            )
            extras["total_mo_cost"] = (
                extras["total_mo_cost_components"] + extras["total_mo_cost_operations"]
            )
            extras["total_bom_cost"] = (
                extras["total_bom_cost_components"]
                + extras["total_bom_cost_operations"]
            )
            extras["total_real_cost"] = (
                extras["total_real_cost_components"]
                + extras["total_real_cost_operations"]
            )
        return extras
    ...
    def _get_cost_breakdown_data(self, production, extras, remaining_cost_share):
        ...
        for bp_move in production.move_byproduct_ids:
            if bp_move.state == "cancel" or float_is_zero(
                bp_move.cost_share, precision_digits=2
            ):
                continue
            quantities_by_product[bp_move.product_id] += (
                bp_move.product_uom_id._get_quantity_report(
                    bp_move.quantity,
            ...
            total_cost_by_product[bp_move.product_id] += (
                extras["total_real_cost"] * cost_share
            )
            component_cost_by_product[bp_move.product_id] += (
                extras["total_real_cost_components"] * cost_share
            )
            operation_cost_by_product[bp_move.product_id] += (
                extras["total_real_cost_operations"] * cost_share
            )

        produced = (
            production.product_uom_id._get_quantity_report(
                production.qty_produced, production.product_id.uom_id, round=False
            )
            or 1.0
        )
        breakdown_lines = [
            self._format_cost_breakdown_lines(
                ...
                production.product_id.uom_id.display_name,
                extras["total_real_cost_components"] * remaining_cost_share / produced,
                extras["total_real_cost_operations"] * remaining_cost_share / produced,
                extras["total_real_cost"] * remaining_cost_share / produced,
            )
        ]
        index = 0
    ...
    def _format_cost_breakdown_lines(
        self, index, product_name, uom_name, component_cost, operation_cost, total_cost
    ):
        return {
            "index": f"BR{index}",
            "name": product_name,
    ...
    def _get_mo_summary(
        ...
        decorator = self._get_comparison_decorator(
            real_cost if self._is_production_started(production) else bom_cost,
            mo_cost,
            currency.rounding,
        )
        mo_cost_decorator = (
            decorator
            if any(
                compo["summary"]["mo_cost_decorator"] == decorator
                for compo in (components + [operations])
            )
            else False
        )
        real_cost_temp_decorator = (
            self._get_comparison_decorator(mo_cost, real_cost, currency.rounding)
        ...
        real_cost_decorator = (
            real_cost_temp_decorator
            if any(
                compo["summary"]["real_cost_decorator"] == real_cost_temp_decorator
                for compo in (components + [operations])
            )
            else False
        )
        stock = self._get_stock_in_unit(product, production.product_uom_id)
        return {
            "level": 0,
    ...
    def _format_state(self, record, components=False):
        if (
            record._name != "mrp.production"
            or record.state not in ("draft", "confirmed")
            or not components
        ):
            return dict(record._fields["state"]._description_selection(self.env)).get(
                record.state
            )
        to_consume = defaultdict(float)
        ...
        for component in components:
            ...
            if not product.is_storable:
                continue
            uom = component["uom"]
            to_consume[product] += uom._get_quantity_report(
                component["quantity"], product.uom_id, round=False
            )
            reserved[product] += uom._get_quantity_report(
                component["quantity_reserved"], product.uom_id, round=False
            )
            free[product] = uom._get_quantity_report(
                component["quantity_free"], product.uom_id, round=False
            )
        producible_qty = record._get_producible_qty(
            {
                product: (quantity, reserved[product] + free[product])
                for product, quantity in to_consume.items()
            }
        )
    ...
    def _get_stock_in_unit(self, product, unit):
        if not product.is_storable:
            return {"free": False, "on_hand": False}
        return product._get_stock_in_unit(unit)
    ...
    def _get_operations_data(self, production, level=0, current_index=False):
        ...
        for index, workorder in enumerate(production.workorder_ids):
            ...
            wo_duration = workorder._get_cost_minutes(until=fields.Datetime.now())
            ...
            if self._is_production_started(production):
                ...
                mo_cost_decorator = (
                    "danger"
                    if isinstance(bom_cost, bool) and not bom_cost
                    else self._get_comparison_decorator(bom_cost, mo_cost, 0.01)
                )
            ...
            operations.append(
                {
                    ...
                    "quantity": workorder.duration_expected
                    if float_is_zero(wo_duration, precision_digits=2)
                    else wo_duration,
        ...
        if not self._is_production_started(production):
            mo_cost_decorator = self._get_comparison_decorator(
                total_bom_cost or 0.0, total_expected_cost, 0.01
            )
        ...
        return {
            "summary": {
                ...
                "real_cost_decorator": self._get_comparison_decorator(
                    total_expected_cost, total_real_cost, 0.01
                )
                if self._is_production_started(production)
                else False,
                "uom_name": operation_uom,
    ...
    def _get_finished_operation_data(self, production, level=0, current_index=False):
        ...
        for index, workorder in enumerate(production.workorder_ids):
            ...
            minutes = workorder._get_cost_minutes()
            duration = minutes / 60
            operation_cost = workorder._get_machine_cost(minutes)
            mo_cost = (
        ...
        return {
            "summary": {
                ...
                "real_cost_decorator": self._get_comparison_decorator(
                    total_mo_cost, total_cost, currency.rounding
                ),
                "uom_name": done_operation_uom,
                "currency_id": currency.id,
                "currency": currency,
    ...
    def _get_byproducts_data(
            ...
            byproducts.append(
                {
                    ...
                    if move_bp.state != "done"
                    else move_bp.quantity,
                    "uom_name": move_bp.product_uom_id.display_name,
                    "uom_precision": self._get_uom_precision(
                        move_bp.product_uom_id.rounding
                    ),
        ...
        if self._is_production_started(production):
            mo_cost_decorator = self._get_comparison_decorator(
                total_real_cost, total_mo_cost, currency.rounding
            )
        else:
            mo_cost_decorator = self._get_comparison_decorator(
                total_bom_cost, total_mo_cost, currency.rounding
            )
    ...
    def _get_cost_sums(self, components, operations=False):
    ...
    def _get_components_data(
        self, production, replenish_data=False, level=0, current_index=False
    ):
        ...
        components = []
        production.move_raw_ids.fetch(["move_orig_ids"])
        _debug.logic(
        ...
        if production.state == "done":
            ...
            replenish_data = self._get_replenishments_from_forecast(
                production, replenish_data
            )
        for count, move_raw in enumerate(production.move_raw_ids):
            if production.state == "done" and move_raw.product_uom_id.is_zero(
                move_raw.quantity
            ):
                continue
            component_index = f"{current_index}{count}"
            replenishments = self._get_replenishment_lines(
                production, move_raw, replenish_data, level, component_index
            )
            components.append(
                {
                    "summary": self._format_component_move(
                        production,
                        move_raw,
                        replenishments,
                        replenish_data,
    ...
    def _format_component_move(
        self, production, move_raw, replenishments, replenish_data, level, index
    ):
        currency = (production.company_id or self.env.company).currency_id
        product = move_raw.product_id
        expected_quantity = move_raw.product_uom_qty
        current_quantity = move_raw.quantity
        replenish_mo_cost, _dummy_bom_cost, _dummy_real_cost = self._get_cost_sums(
            replenishments
        )
        replenish_quantity = sum(
            rep.get("summary", {}).get("quantity", 0.0) for rep in replenishments
        )
        mo_quantity = (
            current_quantity if production.state == "done" else expected_quantity
        )
        missing_quantity = mo_quantity - replenish_quantity
        missing_quantity_cost = self._get_component_real_cost(
            move_raw, missing_quantity
        )
        mo_cost = currency.round(replenish_mo_cost + missing_quantity_cost)
        real_cost = currency.round(
            self._get_component_real_cost(
                move_raw, current_quantity if move_raw.picked else 0
            )
        )
        if production.bom_id:
        ...
        if production.state == "draft":
            ...
            mo_cost_decorator = self._get_comparison_decorator(
                cost_to_compare, mo_cost, currency.rounding
            )
        stock = self._get_stock_in_unit(product, move_raw.product_uom_id)
        component = {
            "level": level,
            "index": index,
            "id": product.id,
            ...
            "mo_cost": mo_cost,
            "mo_cost_decorator": "danger"
            if isinstance(bom_cost, bool)
            and not bom_cost
            and not self._is_production_started(production)
            else mo_cost_decorator,
            "bom_cost": bom_cost,
            "real_cost": real_cost,
            "real_cost_decorator": False,
            "currency_id": currency.id,
        ...
        if not product.is_storable:
            return component
        if any(
            rep.get("summary", {}).get("model") == "to_order" for rep in replenishments
        ):
    ...
    def _get_full_bom_demand(self, production, move_raw, default):
        if not production.bom_id:
            return default
        bom_demand = self._get_move_bom_demand(production, move_raw)
    ...
    def _update_receipt_decorator(self, mo_planned_start, receipt):
        if (
            ...
            and receipt["date"] > mo_planned_start
        ):
    ...
    def _get_component_receipt(
        ...
        qty_free = product._get_stock_in_unit(move.product_uom_id)["free"]
        ...
        replenishments_with_date = list(
            filter(
                lambda r: r.get("summary", {}).get("receipt", {}).get("date"),
                replenishments,
            )
        )
        max_date = max(
    ...
    def _get_replenishment_lines(
        self, production, move_raw, replenish_data, level, current_index
    ):
        product = move_raw.product_id
        quantity = (
            move_raw.product_uom_qty if move_raw.state != "done" else move_raw.quantity
        )
        reserved_quantity = self._get_reserved_qty(
            move_raw, production.warehouse_id, replenish_data
        )
        currency = (production.company_id or self.env.company).currency_id
        forecast = replenish_data["products"][product.id].get("forecast", [])
        current_lines = filter(
            lambda line: (
                line.get("document_in", False)
                and line.get("document_out", False)
                and line["document_out"].get("id", False) == production.id
                and not line.get("already_used")
            ),
            forecast,
        )
        total_ordered = 0
        ...
        for count, forecast_line in enumerate(current_lines):
            if (
                move_raw.product_uom_id.compare(
                    total_ordered, quantity - reserved_quantity
                )
                >= 0
            ):
                break
            doc_in = self.env[forecast_line["document_in"]["_name"]].browse(
                forecast_line["document_in"]["id"]
            )
            ...
            line_quantity = min(
                quantity,
                forecast_uom_id._get_quantity_report(
                    forecast_line["quantity"], move_raw.product_uom_id
                ),
            )
            full_bom_demand = self._get_full_bom_demand(production, move_raw, quantity)
            ...
            replenishment["summary"] = {
                ...
                "uom_name": move_raw.product_uom_id.display_name,
                "uom_precision": self._get_uom_precision(
                    forecast_line["uom_id"]["rounding"]
                ),
                "unit_cost": self._get_unit_cost(move_raw),
                "mo_cost": forecast_line.get(
                    "cost",
                ...
                "real_cost": currency.round(
                    self._get_component_real_cost(move_raw, line_quantity)
                ),
                "currency_id": currency.id,
                "currency": currency,
            }
            ...
            if doc_in._name == "mrp.production":
                replenishment["components"] = self._get_components_data(
                    doc_in, replenish_data, level + 2, replenishment_index
                )
                replenishment["operations"] = self._get_operations_data(
                    doc_in, level + 2, replenishment_index
                )
                initial_mo_cost, initial_bom_cost, initial_real_cost = (
                    self._get_cost_sums(
                        replenishment["components"], replenishment["operations"]
                    )
                )
                remaining_cost_share, byproducts = self._get_byproducts_data(
                    doc_in,
                ...
                replenishment["summary"]["mo_cost"] = (
                    initial_mo_cost * remaining_cost_share
                )
                replenishment["summary"]["bom_cost"] = (
                    initial_bom_cost * remaining_cost_share
                )
                replenishment["summary"]["real_cost"] = (
                    initial_real_cost * remaining_cost_share
                )

            if self._is_doc_in_done(doc_in):
                replenishment["summary"]["receipt"] = self._format_receipt_date(
                    "available"
                )
            else:
            ...
            if self._is_production_started(production):
                replenishment["summary"]["mo_cost_decorator"] = (
                    self._get_comparison_decorator(
                        replenishment["summary"]["real_cost"],
                        replenishment["summary"]["mo_cost"],
                        replenishment["summary"]["currency"].rounding,
                    )
                )
            else:
                replenishment["summary"]["mo_cost_decorator"] = (
            ...
            replenishment["summary"]["formatted_state"] = (
                self._format_state(doc_in, replenishment["components"])
                if doc_in._name == "mrp.production"
                else self._format_state(doc_in)
            )
        ...
        qty_free = product._get_stock_in_unit(move_raw.product_uom_id)["free"]
        ...
        if (
            ...
            and move_raw.product_uom_id.compare(missing_quantity, 0) > 0
        ):
            resupply_rules = self._get_resupply_rules(
                production, product, replenish_data
            )
            resupply_data = self._get_resupply_data(
                resupply_rules,
                missing_quantity,
                move_raw.product_uom_id,
                product,
            ...
            to_order_line = {
                "summary": {
                    ...
                    "replenish_quantity": move_raw.product_uom_id._get_quantity_report(
                    ...
                    "real_cost": currency.round(
                        ...
                        * move_raw.product_uom_id._get_quantity_report(
            ...
            if resupply_data:
                mo_cost = resupply_data["currency"]._convert(
                    resupply_data["cost"],
                    currency,
                ...
                to_order_line["summary"]["receipt"] = self._update_receipt_decorator(
                    ...
                    self._format_receipt_date(
                        ...
                        + timedelta(days=resupply_data["delay"]),
                    ),
                )
            else:
            ...
            to_order_line["summary"]["unit_cost"] = currency.round(
                to_order_line["summary"]["mo_cost"] / missing_quantity
            )

            if self._is_production_started(production):
                to_order_line["summary"]["mo_cost_decorator"] = (
                    self._get_comparison_decorator(
                        to_order_line["summary"]["real_cost"],
                        to_order_line["summary"]["mo_cost"],
                        currency.rounding,
                    )
                )
            else:
                to_order_line["summary"]["mo_cost_decorator"] = (
                    self._get_comparison_decorator(
                        to_order_line["summary"]["bom_cost"],
                        to_order_line["summary"]["mo_cost"],
                        currency.rounding,
                    )
                )
    ...
    def _prepare_transit_line(
        ...
        def is_related_to_production(document, production):
            if not document:
                return False
            return (
                document.get("_name") == production._name
                and document.get("id") == production.id
            )
        ...
        in_transit = next(
            filter(
                lambda line: (
                    line.get("in_transit")
                    and is_related_to_production(line.get("document_out"), production)
                ),
                forecast,
            ),
            None,
        )
        if not in_transit or is_related_to_production(
            in_transit.get("reservation"), production
        ):
            return None

        product = move_raw.product_id
        currency = (production.company_id or self.env.company).currency_id
        lg = self.env["res.lang"]._get_data(code=self.env.user.lang) or get_lang(
            self.env
        )
        receipt_date = datetime.strptime(in_transit["delivery_date"], lg.date_format)
        full_bom_demand = self._get_full_bom_demand(production, move_raw, 0)
        bom_missing_qty = max(
            0,
        ...
        return {
            "summary": {
                ...
                "quantity": min(
                    ...
                    in_transit["uom_id"]._get_quantity_report(
    ...
    def _get_replenishment_mo_cost(
        self, product, quantity, uom_id, currency, move_in=False
    ):
        return currency.round(
            product.standard_price
            * uom_id._get_quantity_report(quantity, product.uom_id)
        )
        ...
        if doc_in._name == "mrp.production":
            ...
            for component in components:
                if component["summary"]["receipt"]["date"]:
                    max_date_start = max(
                        max_date_start, component["summary"]["receipt"]["date"]
                    )
                all_available = (
                    all_available
                    and component["summary"]["receipt"]["type"] == "available"
                )
                some_unavailable = (
                    some_unavailable
                    or component["summary"]["receipt"]["type"] == "unavailable"
                )
                some_estimated = (
                    some_estimated
                    or component["summary"]["receipt"]["type"] == "estimated"
                )
    ...
    def _format_receipt_date(self, state, date=False):
        if state == "available":
            ...
            return {
                "display": self.env._("Estimated %s", format_date(self.env, date)),
                "type": "estimated",
                "decorator": False,
                "date": date,
            }
            ...
            return {
                "display": self.env._("Expected %s", format_date(self.env, date)),
                "type": "expected",
                "decorator": "warning",
                "date": date,
            }
            ...
            return {
                "display": self.env._("Not Available"),
                "type": "unavailable",
                "decorator": "danger",
                "date": False,
            }
    ...
    def _get_replenishments_from_forecast(self, production, replenish_data):
        ...
        unknown_products = products.filtered(
            lambda product: product.id not in replenish_data.get("products", {})
        )
        if unknown_products:
            warehouse = production.warehouse_id
            wh_location_ids = self._get_warehouse_locations(warehouse, replenish_data)
            forecast_lines = self.env[
                "stock.forecasted_product_product"
            ]._get_report_lines(
                False,
                unknown_products.ids,
                wh_location_ids,
            ...
            for product in unknown_products:
                ...
                extra_docs.sort(
                    key=lambda ex: ex.get("production_id", False), reverse=True
                )
                product_forecast_lines = list(
                    filter(
                        lambda line: line.get("product", {}).get("id") == product.id,
                        forecast_lines,
                    )
                )
    ...
    def _get_replenishment_from_moves(self, production, replenish_data):
        for component_move in production.move_raw_ids:
            ...
            for move_origin in self.env["stock.move"].browse(
                ...
                if doc_origin:
                    to_uom_qty = move_origin.product_uom_id._get_quantity_report(
                    ...
                    product_lines.append(
                        {
                            ...
                            "quantity": used_qty,
                            "uom_id": component_move.product_uom_id,
                            "move_in": move_origin,
                            "product": product,
                        }
                    )
    ...
    def _get_resupply_rules(self, production, product, replenish_data):
        if not replenish_data["products"][product.id].get("resupply_rules"):
            replenish_data["products"][product.id]["resupply_rules"] = (
                product._get_rules_from_location(production.warehouse_id.lot_stock_id)
            )
    ...
    def _add_forecast_origins(self, forecast_lines):
        ...
        for line in filter(
            lambda line: (
                not line.get("document_in", False)
                and line.get("move_out", False)
                and line.get("replenishment_filled", False)
            ),
            forecast_lines,
        ):
            move_out_qty = line["move_out"].product_uom_id._get_quantity_report(
            ...
            for move_origin in self.env["stock.move"].browse(
                ...
                if doc_origin:
                    ...
                    if line["move_out"].product_uom_id.compare(move_out_qty, 0) <= 0:
                        break
    ...
    def _add_forecast_extras(self, forecast_lines, extras, product_rounding):
        ...
        for forecast_line in forecast_lines:
            if forecast_line.get("document_in", False) or forecast_line.get(
            ...
            if (
                forecast_line.get("document_out", False)
                and forecast_line["document_out"]["_name"] == "mrp.production"
            ):
            ...
            for index, extra in enumerate(extras):
                if float_is_zero(
                    extra["quantity"], precision_rounding=product_rounding
                ):
                    index_to_remove.append(index)
                    continue
                if (
                    production_id
                    and extra.get("production_id", False)
                    and extra["production_id"] != production_id
                ):
                    continue
                if "init_quantity" not in extra:
                    extra["init_quantity"] = extra["quantity"]
                converted_qty = extra["uom"]._get_quantity_report(
                    extra["quantity"], forecast_line["uom_id"]
                )
                taken_from_extra = min(line_qty, converted_qty)
                ratio = taken_from_extra / extra["uom"]._get_quantity_report(
                ...
                extra["quantity"] -= forecast_line["uom_id"]._get_quantity_report(
                    taken_from_extra, extra["uom"]
                )
                if (
                    float_compare(
                        extra["quantity"], 0, precision_rounding=product_rounding
                    )
                    <= 0
                ):
                    index_to_remove.append(index)
    ...
    def _get_resupply_data(self, rules, quantity, uom_id, product, production):
        ...
        if not manufacture_rule:
            return False
        bom = manufacture_rule._get_matching_bom(product, production.company_id, {})
        if not bom:
            return False
    ...
    def _get_warehouse_locations(self, warehouse, replenish_data):
        if not replenish_data["warehouses"].get(warehouse.id):
            replenish_data["warehouses"][warehouse.id] = [
                loc["id"]
