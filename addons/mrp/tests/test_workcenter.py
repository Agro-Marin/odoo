...
class TestWorkcenterOverview(common.TestMrpCommon):
    ...
    def test_workcenter_graph_data(self):
        ...
        wc_load_data = self.workcenter_2._get_workcenter_load_per_week(
            week_range, date_start, date_stop
        )
        self.assertListEqual(
            list(wc_load_data[self.workcenter_2].values()), [20.0, 60.0]
        )
        self.assertListEqual(
            list(wc_load_data[self.workcenter_2].keys()),
            [datetime(2020, 3, 11), datetime(2020, 3, 18)],
        )
        load_graph_data = self.workcenter_2._prepare_graph_data(
            wc_load_data, week_range
        )
        self.assertEqual(
            load_graph_data[self.workcenter_2.id][0]["is_sample_data"], False
        )
        self.assertListEqual(
            load_graph_data[self.workcenter_2.id][0]["labels"],
            list(week_range.values()),
        )
        self.assertListEqual(
            load_graph_data[self.workcenter_2.id][0]["values"],
