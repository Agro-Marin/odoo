...
class TestWarehouseMrp(common.TestMrpCommon):
    ...
    def test_manufacturing_rule_other_dest(self):
        ...
        self.assertEqual(
            production.picking_type_id.default_location_dest_id,
            self.warehouse_1.lot_stock_id,
        )
    ...
    def test_manufacturing_scrap(self):
        ...
        self.assertEqual(
            production_3.reservation_state,
            "assigned",
        ...
        scrap_id = (
            ...
            .with_context(active_model="mrp.production", active_id=production_3.id)
            .create(
                {
                    "product_id": self.product_2.id,
                    "scrap_qty": 1.0,
    ...
    def test_backorder_unpacking(self):
        ...
        self.assertEqual(
            mo.move_raw_ids.move_line_ids.mapped("quantity_product_uom"), [20, 80]
        )
...
class TestKitPicking(common.TestMrpCommon):
    ...
    def setUpClass(cls):
        ...
        BomLine.create(
            {"product_id": component_a.id, "product_qty": 2.0, "bom_id": bom_kit_1.id}
        )
        BomLine.create(
            {"product_id": component_b.id, "product_qty": 1.0, "bom_id": bom_kit_1.id}
        )
        BomLine.create(
            {"product_id": component_c.id, "product_qty": 3.0, "bom_id": bom_kit_1.id}
        )
        ...
        BomLine.create(
            {"product_id": component_d.id, "product_qty": 1.0, "bom_id": bom_kit_2.id}
        )
        BomLine.create(
        ...
        BomLine.create(
            {"product_id": component_f.id, "product_qty": 1.0, "bom_id": bom_kit_3.id}
        )
        BomLine.create(
            {"product_id": component_g.id, "product_qty": 2.0, "bom_id": bom_kit_3.id}
        )
        BomLine.create(
            {
    ...
    def test_move_line_aggregated_product_quantities_with_kit(self):
        kit, kit_component_1, kit_component_2, not_kit_1, not_kit_2 = self.env[
            "product.product"
        ].create(
            [
                {
        ...
        delivery.move_line_ids.filtered(
            lambda ml: ml.product_id == kit_component_1
        ).quantity = 3
        delivery.move_line_ids.filtered(
            lambda ml: ml.product_id == kit_component_2
        ).quantity = 3
        delivery.move_line_ids.filtered(
            lambda ml: ml.product_id == not_kit_1
        ).quantity = 4
        delivery.move_line_ids.filtered(
            lambda ml: ml.product_id == not_kit_2
        ).quantity = 2
        ...
        self.assertTrue(
            all("Not" in val for val in aggregate_not_kit_values),
            "Only non kit products should be included",
        )

        aggregate_kit_values = (
            delivery.move_line_ids._get_aggregated_product_quantities(
        ...
        self.assertTrue(
            all("Component" in val for val in aggregate_kit_values),
            "Only kit products should be included",
        )
    ...
    def test_search_kit_on_quantity(self):
        self.env["stock.quant"].create(
            [
                ...
                for product, qty in self.expected_quantities.items()
            ]
        ).action_apply_inventory()
        ...
        self.assertNotIn(self.kit_1, products)
        self.assertIn(self.kit_2, products)
        self.assertNotIn(self.kit_3, products)
    ...
    def test_scrap_change_product(self):
        ...
        product_c = self.env["product.product"].create(
            {"name": "product_c", "is_storable": True}
        )
