...
class TestMrpReplenish(TestMrpCommon):
    ...
    def test_mrp_delay_bom(self):
        ...
        with freeze_time("2023-01-01"):
            ...
            self.assertEqual(
                fields.Datetime.from_string("2023-01-03 00:00:00"), wizard2.date_planned
            )
            bom.days_to_prepare_mo = 4
            wizard3 = self._create_wizard(product, self.warehouse_1)
            self.assertEqual(
                fields.Datetime.from_string("2023-01-07 00:00:00"), wizard3.date_planned
            )
    ...
    def test_replenish_from_scrap(self):
        ...
        replenish_picking = basic_mo.picking_ids.filtered(
    ...
    def test_scrap_replenishment_reassigns_required_qty_to_component(self):
        ...
        move_to_scrap = basic_mo.move_raw_ids.filtered(
            lambda m: m.product_id == product_to_scrap
        )
        move_other = basic_mo.move_raw_ids.filtered(
            lambda m: m.product_id == other_product
        )
        self.assertEqual(
            move_to_scrap.quantity, 5, "Scrapped component should have qty 5"
        )
        self.assertEqual(
            move_other.quantity, 10, "Other component should remain qty 10"
        )
        self.assertEqual(len(basic_mo.picking_ids), 2)
        ...
        self.assertEqual(
            move_to_scrap.quantity, 10, "Scrapped component should return to qty 10"
        )
        self.assertEqual(
            move_other.quantity, 10, "Other component should still be qty 10"
        )
    ...
    def test_global_horizon_days_affect_lead_time_manufacture_rule(self):
        ...
        finished_product.route_ids = [
            Command.set(self.warehouse_1.manufacture_pull_id.route_id.ids)
        ]
        ...
        lead_horizon_date = datetime.strptime(
            loads(repl_info.json_lead_days)["lead_horizon_date"], "%m/%d/%Y"
        ).date()
    ...
    def test_set_bom_on_orderpoint(self):
        ...
        self.product_4.bom_ids.with_context(
            orderpoint_id=orderpoint.id
        ).action_set_bom_on_orderpoint()
