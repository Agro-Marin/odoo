...
class TestTraceability(TestMrpCommon):
    ...
    def test_tracking_types_on_mo(self):
        ...
        quants |= self.env["stock.quant"].create(
            {
                ...
                "lot_id": Lot.create(
                    {"name": "S1", "product_id": consumed_serial.id}
                ).id,
            }
        )
        quants |= self.env["stock.quant"].create(
            {
                ...
                "inventory_quantity": 1,
                "lot_id": Lot.create(
                    {"name": "S2", "product_id": consumed_serial.id}
                ).id,
            }
        )
        quants |= self.env["stock.quant"].create(
            {
                ...
                "inventory_quantity": 1,
                "lot_id": Lot.create(
                    {"name": "S3", "product_id": consumed_serial.id}
                ).id,
            }
        )
        ...
        for finished_product in [finished_no_track, finished_lot, finished_serial]:
            bom = self.env["mrp.bom"].create(
                {
                    ...
                    "bom_line_ids": [
                        Command.create(
                            {"product_id": consumed_no_track.id, "product_qty": 1}
                        ),
                        Command.create(
                            {"product_id": consumed_lot.id, "product_qty": 1}
                        ),
                        Command.create(
                            {"product_id": consumed_serial.id, "product_qty": 1}
                        ),
                    ],
                }
            )
            ...
            self.assertEqual(
                len(lines), 1, "Should always return 1 line : the final product"
            )
            final_product = lines[0]
            self.assertEqual(
                final_product["unfoldable"],
                True,
    ...
    def test_tracking_on_byproducts(self):
        ...
        self.assertEqual(
            finished_move_line_lot_1.consume_line_ids.filtered(lambda l: l.quantity),
        ...
        self.assertEqual(
            finished_move_line_lot_2.consume_line_ids,
        ...
        self.assertEqual(
            byproduct_move_line_1_lot_1.consume_line_ids.filtered(lambda l: l.quantity),
            raw_line_raw_1_lot_1 | raw_line_raw_2_lot_1,
        )
        ...
        self.assertEqual(
            byproduct_move_line_1_lot_2.consume_line_ids,
            raw_line_raw_1_lot_2 | raw_line_raw_2_lot_2,
        )
        ...
        self.assertEqual(
            byproduct_move_line_2_lot_1.consume_line_ids.filtered(lambda l: l.quantity),
            raw_line_raw_1_lot_1 | raw_line_raw_2_lot_1,
        )
        ...
        self.assertEqual(
            byproduct_move_line_2_lot_2.consume_line_ids,
            raw_line_raw_1_lot_2 | raw_line_raw_2_lot_2,
        )
    ...
    def test_tracked_and_manufactured_component(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            productB, self.stock_location, 10, lot_id=lot_B01
        )
        self.env["stock.quant"]._update_available_quantity(
    ...
    def test_last_delivery_traceability(self):
        ...
        self.env["mrp.bom"].create(
            [
                ...
                for finished, component in [
                    (endproductA, componentA),
                    (componentA, subcomponentA),
                ]
            ]
        )
    ...
    def test_unbuild_scrap_and_unscrap_tracked_component(self):
        ...
        self.assertRecordValues(
            (mo.move_finished_ids + mo.move_raw_ids).move_line_ids,
            [
                {
    ...
    def test_use_customized_serial_sequence(self):
        ...
        self.assertRecordValues(
            mo.lot_producing_ids.sorted("name"),
            [
                {"name": "TEST0000001"},
                {"name": "TEST0000002"},
            ],
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                final_product, self.stock_location
            ),
            2,
        )
        self.assertRecordValues(
            self.env["stock.lot"]
            .search([("product_id", "=", final_product.id)])
            .sorted("name"),
            [
                {"name": "TEST0000001"},
                {"name": "TEST0000002"},
        ...
        self.assertRecordValues(
            second_mo.lot_producing_ids.sorted("name"),
            [
                {"name": "IPSUM101"},
                {"name": "LOREM002"},
                {"name": "TEST0000003"},
    ...
    def test_use_lot_already_consumed(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            component, self.stock_location, 1, lot_id=sn_lot02
        )
    ...
    def test_produce_consume_unbuild_and_consume(self):
        ...
        self.assertRecordValues(
            mo_consume_sn.move_raw_ids.move_line_ids,
            [
                {
                    "product_id": component.id,
                    "lot_id": sn.id,
    ...
    def test_produce_consume_unbuild_all_and_consume(self):
        ...
        unbuild_form.lot_ids.add(sn)
        unbuild_form.save().action_unbuild()

        self.env["stock.quant"]._update_available_quantity(
            component, self.stock_location, 1, lot_id=sn
        )
