...
class TestMultistepManufacturingWarehouse(TestMrpCommon):
    ...
    def setUpClass(cls):
        ...
        # visible in the form (which would need product_selectable routes to be
        # present, an assumption that doesn't hold on DBs without demo data).
    ...
    def _check_location_and_routes(self):
        ...
        self.assertTrue(
            self.warehouse.manufacture_pull_id.active,
            self.warehouse.manufacture_to_resupply,
        )
        ...
        self.assertEqual(
            self.warehouse.pbm_loc_id.active,
            self.warehouse.manufacture_steps != "mrp_one_step",
            "Input location must be de-active for single step only.",
        )
    ...
    def test_00_create_warehouse(self):
        ...
        self.assertEqual(
            self.warehouse.manufacture_pull_id.location_dest_id.id,
    ...
    def test_manufacturing_flow(self):
        ...
        picking_customer = self.env["stock.picking"].create(
            {
                ...
                "partner_id": self.env["ir.model.data"]._xmlid_to_res_id(
                    "base.res_partner_4"
                ),
        ...
        self.assertEqual(
            production_order.origin,
            "SOURCEDOCUMENT",
            "The MO origin should be the SO name",
        )
        self.assertNotEqual(
            production_order.name,
            "SOURCEDOCUMENT",
            "The MO name should not be the origin of the move",
        )

        picking_stock_preprod = (
            self.env["stock.move"]
            .search(
                [
        ...
        self.assertEqual(
            picking_stock_preprod.origin,
            production_order.name,
            "The pre-prod origin should be the MO name",
        )
        ...
        self.assertFalse(
            sum(
                ...
                ._gather(self.raw_product, self.warehouse.lot_stock_id)
                .mapped("quantity")
            )
        )
        self.assertTrue(
            self.env["stock.quant"]._gather(self.raw_product, self.warehouse.pbm_loc_id)
        )
        ...
        self.assertFalse(
            sum(
                self.env["stock.quant"]
                ._gather(self.raw_product, self.warehouse.pbm_loc_id)
                .mapped("quantity")
            )
        )
        ...
        self.assertEqual(
            picking_customer.move_ids.move_orig_ids.picking_id, picking_stock_postprod
        )
    ...
    def test_3_steps_and_byproduct(self):
        ...
        preprod_picking = (
            pickings[0]
            if pickings[0].location_id == warehouse_stock_location
            else pickings[1]
        )
    ...
    def test_2_steps_and_additional_moves(self):
        ...
        self.assertEqual(
            self.bom.bom_line_ids.product_id + self.product_2,
            mo.picking_ids.move_ids.product_id,
        )
    ...
    def test_child_parent_relationship_on_backorder_creation(self):
        ...
        self.assertEqual(
            ...
            0,
            "Children MOs counted as existing where there should be none",
        )
        self.assertEqual(
            mo.mrp_production_source_count,
            0,
            "Source MOs counted as existing where there should be none",
        )
    ...
    def test_component_and_byproduct_on_transfers(self):
        ...
        self.assertEqual(
            len(mo.picking_ids),
            2,
            "Should have 2 pickings: Components + (Final product and byproducts)",
        )
        for picking in mo.picking_ids:
            if demo in [m.product_id for m in picking.move_ids]:
                self.assertEqual(
                    ...
                    3,
                    "Should have 3 moves for: Demo, Bprod1 and Bprod2",
                )
                self.assertEqual(
                    [
                        move.product_id
                        for move in picking.move_ids.sorted(
                            lambda m: bool(m.product_id)
                        )
                    ],
                    [demo, bprod1, bprod2],
                )
                ...
                self.assertEqual(
                    len(picking.move_ids), 2, "Should have 2 moves for: Comp1 and Comp2"
                )
                self.assertEqual(
                    [
                        move.product_id
    ...
    def test_update_mo_from_bom_forecast(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_2, self.warehouse_1.lot_stock_id, 10
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product_3, self.warehouse_1.lot_stock_id, 20
        )
        mo = self.env["mrp.production"].create(
            {
        ...
        self.assertRecordValues(
            pre_prod_pick.move_ids,
            [
                {
                    "product_id": self.product_2.id,
                    "product_uom_qty": 3,
                    "product_qty_available": 10,
                },
                {
                    "product_id": self.product_1.id,
                    "product_uom_qty": 0,
