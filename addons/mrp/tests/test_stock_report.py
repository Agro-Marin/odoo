...
class TestMrpStockReports(TestReportsCommon):
    ...
    def test_report_forecast_3_report_line_corresponding_to_mo_highlighted(self):
        ...
        for mo in [mo_1, mo_2]:
            ...
            _, _, lines = self.get_report_forecast(
                product_template_ids=product_banana.product_tmpl_id.ids, context=context
            )
            for line in lines[1:]:
    ...
    def test_kit_packaging_delivery_slip(self):
        ...
        for back_order, expected_vals in [("never", [24, 12]), ("always", [24, 12])]:
            ...
            non_kit_aggregate_values = (
                picking.move_line_ids._get_aggregated_product_quantities()
            )
    ...
    def test_subkit_in_delivery_slip(self):
        ...
        move = picking.move_ids.filtered(
            lambda m: m.description_picking == "Super Kit" and m.product_id == compo03
        )
        move.move_line_ids.result_package_id = self.env["stock.package"].create(
            {"name": "Package0001"}
        )
        picking.button_validate()
        ...
        bom = self.env["mrp.bom"].create(
            {
                ...
                "bom_line_ids": [
                    Command.create(
                        {"product_id": self.product1.id, "product_qty": 1.0}
                    ),
                ],
                "byproduct_ids": [
    ...
    def test_report_price_variants(self):
        ...
        black_white_product, black_product, white_product = self.env[
            "product.product"
        ].create(
            [
                {
            ...
            [
                {
                    ...
                    "bom_product_template_attribute_value_ids": [
                        Command.set([black_tmpl.id, white_tmpl.id])
                    ],
                },
                {
                    "bom_id": bom_normal.id,
