...
"""Direct coverage for `mrp.workcenter._get_first_available_slot`.

That method is the whole of the planner: `_plan_workorder` calls it, and
everything a user does through "Plan" reaches scheduling only through it. It is
~100 lines of two mirrored forward/backward interval walks, with a conflict
restart inside a `while` and a bounded outer loop, and until this file it had no
test of its own -- a wrong answer surfaced only as "planning produced a strange
date" somewhere downstream.

Each test states a calendar, a set of occupied intervals and a duration, then
asserts the exact slot returned.
"""
...
class TestWorkcenterFirstAvailableSlot(common.TestMrpCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        # A calendar with one predictable shift a day, 08:00-16:00 UTC Monday to
        # Friday, so every expected datetime below can be written out in full.
    ...
    @freeze_time("2024-01-01 06:00:00")  # a Monday, before the shift opens
    ...
    def test_forward_starts_when_the_shift_opens_not_before_it(self):
        """Asked at 05:00, the slot begins at 08:00 -- outside working hours is
        not available time."""
        start, stop = self.workcenter._get_first_available_slot(
    ...
    def test_forward_rejects_a_gap_that_is_too_short(self):
        """A 60-minute gap cannot hold 90 minutes: the answer is the next free
        stretch, not a partial fit."""
        self._reserve(datetime(2024, 1, 1, 8, 0), datetime(2024, 1, 1, 10, 0))
    ...
    def test_forward_rolls_over_to_the_next_working_day(self):
        """The shift has 8 hours; 10 hours of work cannot start on Monday, and
        ...
        # Saturday the 6th and Sunday the 7th are closed.
    ...
    def test_forward_spans_two_days_when_the_shift_is_too_short(self):
        """A 10-hour job does not fit in one 8-hour shift, so it carries over:
        the returned window opens on the first day and closes on the second."""
        start, stop = self.workcenter._get_first_available_slot(
    ...
    def test_forward_ignores_the_reservations_it_is_told_to(self):
        """Replanning a work order must not collide with its own booking."""
    ...
    def test_forward_avoids_extra_leave_slots(self):
        """`extra_leaves_slots` blocks time that is not a reservation -- what
        the shop floor uses to hold a workcenter without booking it."""
        start, stop = self.workcenter._get_first_available_slot(
    ...
    def test_forward_rounds_a_zero_duration_up_to_a_second(self):
        """`max(duration, 1/60)` -- a zero-length job still needs a position in
        the schedule, and a zero-length interval matches nothing."""
        start, stop = self.workcenter._get_first_available_slot(
    ...
    def test_a_workcenter_that_never_works_reports_no_slot(self):
        """No attendance at all: the search exhausts its iterations and says so
        rather than looping or returning a nonsense date."""
    ...
    def test_the_iteration_bound_is_configurable(self):
        """`mrp.workcenter_max_planning_iterations` bounds the horizon; one
        iteration covers a fortnight, so a job pushed past it is not found."""
    ...
    def test_backward_takes_the_end_of_the_window(self):
        """Backward scheduling lands the job so that it *finishes* at the
        requested moment."""
        start, stop = self.workcenter._get_first_available_slot(
    ...
    def test_backward_walks_back_over_a_closed_day(self):
        """Monday the 8th is the working day before Saturday the 6th."""
        start, stop = self.workcenter._get_first_available_slot(
