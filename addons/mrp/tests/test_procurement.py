...
class TestProcurement(TestMrpCommon):
    ...
    def test_procurement_3(self):
        ...
        warehouse.reception_route_id.rule_ids.filtered(
            lambda p: (
                p.location_src_id == warehouse.wh_input_stock_loc_id
                and p.location_dest_id == warehouse.wh_qc_stock_loc_id
            )
        ).write(
            {
                "action": "pull",
        ...
        warehouse.reception_route_id.rule_ids.filtered(
            lambda p: (
                p.location_src_id == warehouse.wh_qc_stock_loc_id
                and p.location_dest_id == warehouse.lot_stock_id
            )
        ).write(
            {
                "action": "pull",
    ...
    def test_link_date_mo_moves(self):
        ...
        self.assertAlmostEqual(
            mo.move_finished_ids.date,
            mo.move_raw_ids.date + timedelta(hours=1),
        ...
        self.assertEqual(
            mo_form.product_qty,
            10,
            "the quantity to produce is not good relative to the move",
        )
        ...
        self.assertAlmostEqual(
            mo.move_raw_ids.date, mo.date_start, delta=timedelta(seconds=1)
        )
        self.assertAlmostEqual(
            mo.move_finished_ids.date, mo.date_end, delta=timedelta(seconds=1)
        )
    ...
    def test_finished_move_cancellation(self):
        ...
        self.assertEqual(
            mo.move_finished_ids[0].state,
            "cancel",
            "Finished move should be cancelled if mo is cancelled.",
        )
        self.assertEqual(
            mo.move_dest_ids[0].state,
            "confirmed",
    ...
    def test_mtso_with_empty_bom(self):
        ...
        product = self.env["product.product"].create(
            {
                "name": "Product",
                "route_ids": [
                    Command.link(route_manufacture.id),
        ...
        self.assertEqual(
            len(production), 1, "The manufacturing order was not automatically created."
        )
        self.assertFalse(production.move_raw_ids)
        ...
        self.assertEqual(
            production.state,
            "draft",
            "MO with empty BoM created via MTSO should remain in draft state.",
        )
    ...
    def test_auto_assign(self):
        ...
        info_p2 = self.env["stock.replenishment.info"].create(
            {"orderpoint_id": orderpoint_p2.id}
        )
        info_p2.write(
            {
                "based_on": "one_week",
        ...
        mo.is_locked = False
        self.assertEqual(
            mo.move_raw_ids.quantity, 0, "No components should be reserved yet"
        )
        self.assertEqual(
            mo.product_qty,
            15,
        ...
        self.assertEqual(
            len(mo3), 1, "Third manufacture order for added component was not created"
        )
        self.assertEqual(
            mo3.product_qty,
            6,
        ...
        self.assertEqual(
            mo_assign_at_confirm.move_raw_ids.quantity,
            5,
    ...
    def test_check_update_qty_mto_chain(self):
        ...
        customer_move = self.env["stock.move"].search(
            [
        ...
        self.env["mrp.bom"].create(
            {
                ...
                "type": "normal",
                "bom_line_ids": [
                    Command.create({"product_id": product_4.id, "product_qty": 1})
                ],
            }
        )
    ...
    def test_update_mo_component_qty(self):
        ...
        mo.write(
            {
                "move_raw_ids": [
                    Command.create({"product_id": comp3.id, "product_uom_qty": 3})
                ]
            }
        )
    ...
    def test_update_merged_mo_component_qty(self):
        ...
        self.assertEqual(len(mo.picking_ids.move_ids), 5, "Picking should have 3 moves")
        self.assertEqual(
            mo.picking_ids.move_ids.filtered(
                lambda m: m.product_id == comp3
            ).product_uom_qty,
            2,
            "Comp3 move should have same qty as MO",
        )
        ...
        self.env["mrp.bom"].create(
            {
                ...
                "operation_ids": [
                    Command.create(
                        {"name": "OP1", "workcenter_id": self.workcenter_2.id}
                    ),
                ],
            }
        )
    ...
    def test_mo_split_with_batch_size_mto(self):
        ...
        self.assertEqual(
            len(manufacturing_orders),
            2,
            "Expected 2 manufacturing orders to be created.",
        )
        self.assertEqual(
            manufacturing_orders.mapped("product_qty"),
            [200.0, 200.0],
            "Each manufacturing order should have a quantity of 200.0, as defined by the BoM batch size.",
        )
    ...
    def test_update_mo_producing_qty_with_mtso_rule_and_some_available_stock(self):
        ...
        mo.action_confirm()
        replenishment = self.env["stock.move"].search(
            [
                ("product_id", "=", self.productB.id),
                ("product_uom_qty", "=", 3),
