...
class MrpProduction(models.Model):
    ...
    )  # Only used for search in view_mrp_production_filter
    ...
    date_end = fields.Datetime(
        "End",
        copy=False,
        default=_get_default_date_finished,
    ...
    unbuild_count = fields.Integer(
        compute="_compute_unbuild_count", string="Number of Unbuilds"
    )
    ...
    mrp_production_backorder_count = fields.Integer(
        "Count of linked backorder", compute="_compute_mrp_production_backorder"
    )
    ...
    def _compute_product_id(self):
        for production in self:
            ...
            if bom and (
                ...
                or (bom.product_id
                and bom.product_id != production.product_id)
            ):
                    ...
                    moves.mapped(
                        lambda move: (
                            move.product_id.uom_id._compute_quantity(
                                move.product_id.qty_available, move.product_uom
                            )
                            / move.unit_factor
                        )
                    )
            ...
            if not default or "move_finished_ids" not in default:
                ...
                if production.state != "cancel":
                    move_finished_ids = production.move_finished_ids.filtered(
                        lambda m: m.state != "cancel" and m.product_qty != 0.0
                    )
                vals["move_finished_ids"] = [
    ...
    def _get_bom_values(self, ratio=1):
        ...
        def get_uom_and_quantity(move):
            ...
            ).product_uom_id or move.product_uom
    ...
    def _get_move_finished_values(
        ...
        if len(group_orders) > 1:
            move_dest_ids |= (
                group_orders[0]
                .move_finished_ids.filtered(lambda m: m.product_id == self.product_id)
                .move_dest_ids
            )
        return {
            "product_id": product_id,
    ...
    def _set_qty_producing(self, pick_manual_consumption_moves=True):
        ...
        is_waiting = (
            self.warehouse_id.manufacture_steps != "mrp_one_step"
            and self.picking_ids.filtered(
                lambda p: (
                    p.picking_type_id == self.warehouse_id.pbm_type_id
    ...
    def action_confirm(self):
        ...
        moves_to_confirm._action_confirm(
            merge=False, create_proc=not self.env.context.get("no_procurement")
        )
    ...
    def action_merge(self):
        ...
        products = set(
            [(production.product_id, production.bom_id) for production in self]
        )
    ...
    def _link_bom(self, bom):
        ...
        for move_byproduct in self.move_byproduct_ids:
            ...
            move_byproduct_qty = (
                bom_byproduct
                and move_byproduct.product_uom._compute_quantity(
    ...
    def _check_sn_uniqueness(self):
            ...
            for move_line in move.move_line_ids:
                ...
                duplicates = (
                    co_prod_move_lines.filtered(
                        lambda ml: ml.quantity and ml.lot_id == sml_sn
                    )
                    - move_line
                )
                if duplicates:
        ...
        cancelled_sml_groups = self.env["stock.move.line"]._read_group(
            [  # SML that cancels the SN consumption
    ...
    def _set_quantities(self):
        ...
        for move in self.move_raw_ids:
            ...
            if move.manual_consumption:
                if move.has_tracking in ("serial", "lot") and (
                    not move.picked
                    or any(
                        not line.lot_id
                        for line in move.move_line_ids
                        if line.quantity and line.picked
                    )
                ):
