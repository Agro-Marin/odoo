...
class MrpProduction(models.Model):
    ...
    def write(self, vals):
        ...
        for move_str in ("move_raw_ids", "move_finished_ids"):
            if move_str not in vals or self.state in ["cancel", "done"]:
                continue
            # A move raw/finished must carry the source location's `warehouse_id`; force
            # it when vals does not already provide one.
    ...
    def _split_productions(
        ...
        # Split the reserved amounts manually instead of do_unreserve -> action_assign:
        # re-reserving is slower (`stock.quant` updates) and, in mass production, a new
        # lot arriving higher in FIFO in between would change what gets reserved.
