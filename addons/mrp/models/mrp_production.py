...
class MrpProduction(models.Model):
    ...
    def _get_default_date_start(self):
        if self.env.context.get('default_date_deadline'):
            date_end = fields.Datetime.to_datetime(self.env.context.get('default_date_deadline'))
    ...
    def _get_default_date_end(self):
        ...
        date_end = date_start + relativedelta(hours=1)
        return date_end
    ...
    date_end = fields.Datetime(
        'End', copy=False, default=_get_default_date_end,
    ...
    date_delay_alert = fields.Datetime('Delay Alert Date', compute='_compute_date_delay_alert', search='_search_date_delay_alert')
    ...
    @api.depends('state', 'reservation_state', 'date_start', 'move_raw_ids', 'move_raw_ids.forecast_availability', 'move_raw_ids.date_planned_forecast')
    def _compute_components_availability(self):
        ...
        for production in productions:
            if any(
                ...
                forecast_date = max(production.move_raw_ids.filtered('date_planned_forecast').mapped('date_planned_forecast'), default=False)
                if forecast_date:
    ...
    def _compute_is_planned(self):
        for production in self:
            if production.workorder_ids:
                production.is_planned = any(wo.date_start and wo.date_end for wo in production.workorder_ids)
            else:
                production.is_planned = False

    @api.depends('move_raw_ids.date_delay_alert')
    def _compute_date_delay_alert(self):
        date_delay_alert_data = self.env['stock.move']._read_group([('id', 'in', self.move_raw_ids.ids), ('date_delay_alert', '!=', False)], ['raw_material_production_id'], ['date_delay_alert:max'])
        date_delay_alert_data = {raw_material_production.id: date_delay_alert_max for raw_material_production, date_delay_alert_max in date_delay_alert_data}
        for production in self:
            production.date_delay_alert = date_delay_alert_data.get(production.id, False)

    def _compute_json_popover(self):
        production_no_alert = self.filtered(lambda m: m.state in ('done', 'cancel') or not m.date_delay_alert)
        ...
        for production in (self - production_no_alert):
            production.json_popover = json.dumps({
                ...
                'date_delay_alert': format_datetime(self.env, production.date_delay_alert, dt_format=False),
                'late_elements': [{
                    'id': late_document.id,
                    'name': late_document.display_name,
                    'model': late_document._name,
                } for late_document in production.move_raw_ids.filtered(lambda m: m.date_delay_alert).move_orig_ids._delay_alert_get_documents()
                ]
            })
    ...
    def _search_date_delay_alert(self, operator, value):
        if operator in Domain.NEGATIVE_OPERATORS:
            return NotImplemented
        late_stock_moves = self.env['stock.move'].search([('date_delay_alert', operator, value)])
    ...
    def _compute_date_end(self):
        for production in self:
            ...
            if production._should_postpone_date_end(date_end):
                ...
                date_end = date_end + relativedelta(minutes=workorder_expected_duration or 60)
    ...
    @api.depends('product_id', 'bom_id', 'product_qty', 'product_uom_id', 'location_dest_id', 'date_end', 'move_dest_ids', 'never_product_template_attribute_value_ids')
        ...
        for record in self:
            record.is_delayed = bool(
                record.state in ['confirmed', 'progress', 'to_close'] and (
                    record.date_deadline and (record.date_deadline < datetime.datetime.now() or record.date_deadline < record.date_end))
            )
    ...
    def write(self, vals):
        ...
        for production in self:
            ...
            if self._has_workorders() and not production.workorder_ids.operation_id and vals.get('date_start') and not vals.get('date_end'):
    ...
    def _plan_workorders(self, replan=False):
        ...
        self.with_context(force_date=True).write({
            ...
            'date_end': max((wo.reservation_id.date_end for wo in workorders if wo.reservation_id), default=None),
        })
    ...
    def action_plan_with_components_availability(self):
        for production in self.filtered(lambda p: p.state in ('draft', 'confirmed')):
            if production.state == 'draft':
                production.action_confirm()
            move_expected_date = production.move_raw_ids.filtered('date_planned_forecast').mapped('date_planned_forecast')
    ...
    def action_view_serial_numbers(self):
        action = self.env["ir.actions.actions"]._for_xml_id("stock.action_stock_lot_form")
