...
class MrpWorkcenter(models.Model):
    ...
    def _get_first_available_slot(self, start_datetime, duration, forward=True, reservations_to_ignore=False, extra_leaves_slots=[]):
        ...
        for n in range(max_planning_iterations):  # 50 * 14 = 700 days in advance
            if forward:
                ...
                occupied = Reservation._reservation_intervals_batch(
                    date_start, date_stop, resource, domain=reservation_domain,
                ...
                for start, stop, _records in available_intervals:
                    ...
                    while (interval := Intervals([(start_interval or start, start + timedelta(minutes=min(remaining, interval_minutes)), _records)])) \
                      and (conflict := interval & occupied or interval & extra_leaves_slots_intervals):
