...
class TestMassMailValues(MassSMSCommon):
    ...
    def test_mailing_get_sms_link_replacements_placeholders(self):
        ...
        link_trackers = bool(
            self.env["link.tracker"].search([], limit=1)
        )  # depends on demo
        ...
        self.assertDictEqual(
            no_mailing.get_sms_link_replacements_placeholders(), expected
        )

        new_mailing = self.env["mailing.mailing"].new()
        self.assertDictEqual(
            new_mailing.get_sms_link_replacements_placeholders(), expected
        )
