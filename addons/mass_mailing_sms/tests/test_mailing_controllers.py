...
class TestMailingListSms(MassSMSCommon):
    ...
    def test_controller_unsubscribe(self):
        ...
        partner = self.env["res.partner"].create(
            {
                "name": "Test Partner",
                "phone_ids": [
                    Command.create({"number": "+91 1234657890", "type": "landline"})
                ],
            }
        )
        ...
        unsubscribe_url = url_join(
            mailing.get_base_url(), f"/sms/{mailing.id}/unsubscribe/{trace.sms_code}"
        )
        response = self.url_open(
            url=unsubscribe_url, data={"sms_number": trace.sms_number}, method="GET"
        )
