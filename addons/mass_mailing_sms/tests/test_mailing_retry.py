...
class TestMailingRetrySMS(MassSMSCommon, CronMixinCase):
    ...
    def test_sms_retry_immediate_trigger(self):
        ...
        def patched_sms_sms_send(
            sms_records, unlink_failed=False, unlink_sent=True, raise_exception=False
        ):
        ...
        self.assertEqual(
            len(captured_triggers.records),
            1,
            "Should have created an additional trigger immediately",
        )
