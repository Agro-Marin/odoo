...
class MailingSMSController(http.Controller):
    ...
    def blacklist_page(self, mailing_id, trace_code, **post):
        ...
        if sms_number:
            ...
            if sanitized_number:
                valid_trace = found_traces.filtered(
                    lambda t: t.sms_number == sanitized_number
                )
    ...
    def blacklist_number(self, mailing_id, trace_code, **post):
        ...
        trace = (
            check_res["trace"].filtered(lambda r: r.sms_number == tocheck_number)[:1]
            if tocheck_number
            else False
        )
    ...
    def sms_short_link_redirect(self, code, sms_id_int, **post):
        if sms_id_int:
            trace_id = (
                request.env["mailing.trace"]
                .sudo()
                .search([("sms_id_int", "=", int(sms_id_int))])
                .id
            )
        else:
            trace_id = False
