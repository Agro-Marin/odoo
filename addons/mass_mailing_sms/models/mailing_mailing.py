...
class MailingMailing(models.Model):
    ...
    def default_get(self, fields):
        ...
        if (
            fields is not None
            and "keep_archives" in fields
            and res.get("mailing_type") == "sms"
        ):
    ...
    def _compute_medium_id(self):
        ...
        for mailing in self:
            if mailing.mailing_type == "sms" and (
                not mailing.medium_id
                or mailing.medium_id
                == self.env["utm.medium"]._get_or_create_utm_medium("email")
            ):
                mailing.medium_id = (
                    self.env["utm.medium"]
                    ._get_or_create_utm_medium("sms", module="mass_mailing_sms")
                    .id
                )
            elif mailing.mailing_type == "mail" and (
                not mailing.medium_id
                or mailing.medium_id
                == self.env["utm.medium"]._get_or_create_utm_medium(
    ...
    def action_retry_failed_sms(self):
        failed_sms = (
            self.env["sms.sms"]
            .sudo()
            .search([("mailing_id", "in", self.ids), ("state", "=", "error")])
        )
    ...
    def _action_view_traces_filtered(self, view_filter):
        ...
        if self.mailing_type == "sms":
            action["views"] = [
                (
                    self.env.ref("mass_mailing_sms.mailing_trace_view_tree_sms").id,
                    "list",
                ),
                (
                    self.env.ref("mass_mailing_sms.mailing_trace_view_form_sms").id,
                    "form",
                ),
            ]
        return action
    ...
    def _get_opt_out_list_sms(self):
        ...
        if hasattr(self.env[self.mailing_model_name], "_mailing_get_opt_out_list_sms"):
            opt_out = self.env[self.mailing_model_name]._mailing_get_opt_out_list_sms(
                self
            )
            _logger.info(
                "Mass SMS %s targets %s: optout: %s contacts",
                self,
                target._name,
                len(opt_out),
            )
            ...
            _logger.info(
                "Mass SMS %s targets %s: no opt out list available", self, target._name
            )
        return opt_out
    ...
    def _get_seen_list_sms(self):
        ...
        phone_fields = [
            fname
            for fname in target._get_phone_number_fields()
        ...
        if not phone_fields:
            partner_field = next(
                (
                    fname
                    for fname in target._mail_get_partner_fields()
                    if target._fields[fname].store
                    and target._fields[fname].type == "many2one"
                ),
                False,
            )
        ...
        if phone_fields:
            owner_alias = "target"
            rel_fields = [target._fields[fname] for fname in phone_fields]
            join_add_query = ""
        else:
            owner_alias = "partner"
            rel_fields = [self.env["res.partner"]._fields["phone_ids"]]
            join_add_query = (
                f'JOIN res_partner partner ON (target."{partner_field}" = partner.id)'
            )
        rel_joins = " UNION ".join(
            f"""
            SELECT rel."{field.column1}" AS owner_id, pn.sanitized AS sanitized
              FROM "{field.relation}" rel
              JOIN phone_number pn ON (pn.id = rel."{field.column2}")
            """
            for field in rel_fields
        )
        query = f"""
            SELECT target.id, numbers.sanitized
              FROM mailing_trace trace
              JOIN "{target._table}" target ON (trace.res_id = target.id)
              {join_add_query}
              JOIN ({rel_joins}) numbers ON (numbers.owner_id = {owner_alias}.id)
             WHERE trace.mass_mailing_id = %(mailing_id)s
               AND trace.model = %(target_model)s
        """
        params = {"mailing_id": self.id, "target_model": self.mailing_model_real}
        self.env.cr.execute(query, params)
        query_res = self.env.cr.fetchall()
        seen_list = {number for _res_id, number in query_res if number}
        seen_ids = {item[0] for item in query_res}
        _logger.info(
            "Mass SMS %s targets %s: already reached %s SMS",
            self,
            target._name,
            len(seen_list),
        )
    ...
    def get_sms_link_replacements_placeholders(self):
        ...
        * unsubscribe is formatted as: ``"STOP SMS : [base_url]/sms/[mailing_id]/[trace_code]"``.
        ...
        max_sms = (
            self.env["sms.sms"].sudo().search_read([], ["id"], order="id desc", limit=1)
        )
        sms_id_length = (
            max(len(str(max_sms[0]["id"])), 5) if max_sms else 5
        )  # Assumes a mailing won't be more than 10⁵ sms at once
        max_code = (
            self.env["link.tracker.code"]
            .sudo()
            .search_read([], ["code"], order="id DESC", limit=1)
        )
        code_length = (
            len(max_code[0]["code"]) + 1 if max_code else LINK_TRACKER_MIN_CODE_LENGTH
        )
        ...
        if self.id:
            ...
            max_mailing = (
                ...
                .sudo()
                .search_read([], ["id"], order="id DESC", limit=1)
            )
            mailing_id_placeholder_length = (
                len(str(max_mailing[0]["id"] + 1)) if max_mailing else 1
            )
        ...
        opt_out_url = urljoin(
            base_url,
            f"sms/{mailing_id_placeholder}/{'x' * self.env['mailing.trace'].CODE_SIZE}",
        )
        return {
