...
class MailingMailing(models.Model):
    ...
    def _get_seen_list_sms(self):
        ...
        if not phone_fields:
            partner_field = next(
                (
                    fname for fname in target._mail_get_partner_fields()
                    if target._fields[fname].store and target._fields[fname].type == 'many2one'
                ),
                False
            )
        ...
        if phone_fields:
            ...
            join_add_query = f'JOIN res_partner partner ON (target."{partner_field}" = partner.id)'
