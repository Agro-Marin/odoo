...
class MailingList(models.Model):
    ...
    def action_view_contacts_sms(self):
        ...
        action["context"] = dict(
            action.get("context", {}), search_default_filter_valid_sms_recipient=1
        )
        return action
    ...
    def _get_fields_contact_statistics(self):
        ...
        values = super()._get_fields_contact_statistics()
        values.update(
            {
                "contact_count_sms": """
                SUM(CASE WHEN
    ...
    def _mailing_get_opt_out_list_sms(self, mailing):
        ...
        opt_out_contacts = subscriptions.filtered(lambda sub: sub.opt_out).mapped(
            "contact_id"
        )
