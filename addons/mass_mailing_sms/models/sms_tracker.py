...
class SmsTracker(models.Model):
    ...
    def _action_update_from_provider_error(self, provider_error):
        error_status, failure_type, failure_reason = (
            super()._action_update_from_provider_error(provider_error)
        )
        self._update_sms_traces(
            error_status or "error",
    ...
    def _action_update_from_sms_state(
        ...
        traces = self._update_sms_traces(
            trace_status, failure_type=failure_type, failure_reason=failure_reason
        )
        self._update_sms_mailings(trace_status, traces)

    def _update_sms_traces(
        self, trace_status, failure_type=False, failure_reason=False
    ):
        ...
        traces = self.mailing_trace_id.filtered(
            lambda t: t.trace_status not in statuses_to_ignore
        )
        if traces:
            ...
            traces.filtered(
                lambda t: (
                    t.trace_status not in ["outgoing", "process", "error", "cancel"]
                    and not t.sent_datetime
                )
    ...
    def _update_sms_mailings(self, trace_status, traces):
        ...
        mailings_to_mark_done = self.env["mailing.mailing"].search(
            [
                ...
                "!",
                (
                    "mailing_trace_ids.trace_status",
                    "=",
                    "process",
                ),  # = not any trace with 'process' status
                ("state", "!=", "done"),
            ]
        )
        ...
        if mailings_to_mark_done:
            if self.env.user.is_public:  # From webhook event
                mailings_to_mark_done._track_set_author(
