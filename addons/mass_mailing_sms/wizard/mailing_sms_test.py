...
class MailingSmsTest(models.TransientModel):
    ...
    def _default_numbers(self):
        previous_numbers = (
            ...
            .search(
                [("create_uid", "=", self.env.uid)], order="create_date desc", limit=1
            )
            .numbers
        )
    ...
    def _prepare_test_trace_values(self, record, sms_number, sms_uuid, body):
        ...
        unsubscribe_info = self.env["sms.composer"]._get_unsubscribe_info(
            self.env["sms.composer"]._get_unsubscribe_url(
                self.mailing_id.id, trace_code
            )
        )
    ...
    def action_send_sms(self):
        ...
        if record:
            ...
            body = self.env["mixin.mail.render"]._render_template(
                body, self.mailing_id.mailing_model_real, record.ids
            )[record.id]
        ...
        for sanitized_number in valid_numbers:
            ...
            if self.mailing_id.sms_allow_unsubscribe:
                trace_values, sms_body = self._prepare_test_trace_values(
                    record, sanitized_number, sms_values["uuid"], sms_body
                )
            ...
            [
                {
                    "content": body,
                    "numbers": [
                        {"number": sms.number, "uuid": sms.uuid} for sms in sms_sudo
                    ],
                }
            ],
        ...
        for sent_sms in sent_sms_list:
            ...
            if sent_sms.get("state") in ("success", "sent"):
                ...
                failure_explanation = sms_api._get_sms_api_error_messages().get(
                    sent_sms["state"]
                )
                ...
                notification_messages.append(
                    _(
                        ...
                        state=failure_explanation
                        or failure_reason
                        or _("An error occurred."),
                    )
                )
        if invalid_numbers:
            notification_messages.append(
