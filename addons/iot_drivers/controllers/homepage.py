...
def _render_page(view):
    ...
    return http.request.prepare_response(
        page,
        headers=[
...
class IotBoxOwlHomePage(http.Controller):
    ...
    def get_homepage_data(self):
            ...
            for iface_id in netifaces.interfaces():
                ...
                network_interfaces.extend(
                    [
                        ...
                        for conf in netifaces.ifaddresses(iface_id).get(
                            netifaces.AF_INET, []
                        )
                    ]
                )
        ...
        grouped_devices = {
            ...
            for device_type, devices in groupby(
                sorted(devices, key=device_type_key), device_type_key
            )
        }
        ...
        return http.request.prepare_json_response(
            {
                ...
                "pairing_code_expired": connection_manager.pairing_code_expired
    ...
    @route.iot_route(
        "/iot_drivers/version_info", type="http", cors="*", linux_only=True
    )
    def get_version_info(self):
        ...
        if not last_available_commit:
            _logger.error(
                "Failed to retrieve last commit available for branch origin/%s",
                current_branch,
            )
            return http.request.prepare_json_response(
                {
                    "status": "error",
                    "message": "Failed to retrieve last commit available for branch origin/"
                    + current_branch,
                }
            )
        ...
        return http.request.prepare_json_response(
            {
                ...
                "odooIsUpToDate": current_commit == last_available_commit
        ...
        return http.request.prepare_json_response(
            {
                ...
                "root_logger_log_level": self._get_logger_effective_level_str(
                    logging.getLogger()
                ),
                "odoo_current_log_level": self._get_logger_effective_level_str(
                    logging.getLogger("odoo")
                ),
                "recommended_log_level": "warning",
                "available_log_levels": AVAILABLE_LOG_LEVELS,
                "drivers_logger_info": self._get_iot_handlers_logger(
                    drivers_list, "drivers"
                ),
                "interfaces_logger_info": self._get_iot_handlers_logger(
    ...
    def is_ngrok_enabled(self):
        return http.request.prepare_json_response(
            {"enabled": helpers.is_ngrok_enabled()}
        )
    ...
    # ---------------------------------------------------------- #
    @route.iot_route(
        "/iot_drivers/six_payment_terminal_add",
        type="jsonrpc",
        methods=["POST"],
        cors="*",
    )
    def add_six_terminal(self, terminal_id):
        if terminal_id.isdigit():
            ...
            _logger.warning(
                'Ignoring invalid Six TID: "%s". Only digits are allowed', terminal_id
            )
    ...
    @route.iot_route(
        "/iot_drivers/save_credential", type="jsonrpc", methods=["POST"], cors="*"
    )
    ...
    @route.iot_route(
        "/iot_drivers/enable_ngrok", type="jsonrpc", methods=["POST"], linux_only=True
    )
    def enable_remote_connection(self, auth_token):
        return {
            "status": "success"
            if helpers.toggle_remote_connection(auth_token)
            else "failure"
        }

    @route.iot_route(
        "/iot_drivers/disable_ngrok", type="jsonrpc", methods=["POST"], linux_only=True
    )
    ...
    @route.iot_route(
        "/iot_drivers/connect_to_server", type="jsonrpc", methods=["POST"], cors="*"
    )
            ...
            try:
                if len(token.split("|")) == 4:
                    ...
                    helpers.save_conf_server(
                        configuration["url"], token, db_uuid, enterprise_code
                    )
                else:
    ...
    @route.iot_route(
        "/iot_drivers/log_levels_update", type="jsonrpc", methods=["POST"], cors="*"
    )
    ...
    def _update_logger_level(
        ...
        if new_level not in available_log_levels:
            _logger.warning(
                "Unknown level to set on logger %s: %s", logger_name, new_level
            )
            return
        ...
        if handler_folder:
            ...
            if not logger:
                _logger.warning(
                    "Unable to change log level for logger %s as logger missing",
                    logger_name,
                )
                return
        ...
        LOG_HANDLERS = (
            helpers.get_conf(ODOO_TOOL_CONFIG_HANDLER_NAME, section="options") or []
        ).split(",")
        ...
        log_handlers_without_logger = [
            ...
            if not log_handler.startswith(LOGGER_PREFIX)
        ]
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        ...
        """
        odoo_addon_handler_path = helpers.compute_iot_handlers_addon_name(
            handler_folder_name, handler_name
        )
        return (
            odoo_addon_handler_path in logging.Logger.manager.loggerDict
            and logging.getLogger(odoo_addon_handler_path)
        )
