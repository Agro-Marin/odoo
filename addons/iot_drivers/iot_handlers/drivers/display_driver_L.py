...
class DisplayDriver(Driver):
    ...
    def __init__(self, identifier, device):
        ...
        self.url = (
            saved_url or self.get_url_from_db() or "http://localhost:8069/status/"
        )
    ...
    def get_default_display(cls):
        displays = list(
            filter(lambda d: iot_devices[d].device_type == "display", iot_devices)
        )
    ...
    def run(self):
        while not self._stopped.is_set() and "pos_customer_display" not in self.url:
            time.sleep(60)
            if (
                self.url != "http://localhost:8069/status/"
                and self.browser.state != BrowserState.KIOSK
            ):
    ...
    def update_url(self, url=None):
        self.url = (
            url or helpers.load_browser_state()[0] or "http://localhost:8069/status/"
        )

        browser_state = (
            BrowserState.KIOSK if "/pos-self/" in self.url else BrowserState.FULLSCREEN
        )
    ...
    def _action_view_kiosk(self, data):
        ...
        self.update_url(
            f"{origin}/pos-self/{data.get('pos_id')}?access_token={data.get('access_token')}"
        )
    ...
    def set_orientation(self, orientation=Orientation.NORMAL):
        ...
        subprocess.run(
            [
                "sed",
                "-i",
                f"s/HDMI-A-[12]/{self.device_identifier}/",
                "/home/odoo/.config/labwc/rc.xml",
            ],
            check=False,
        )
    ...
    def get_display(self):
        ...
        if not display:
            raise werkzeug.exceptions.ServiceUnavailable(
                description="No display connected"
            )
        return display
