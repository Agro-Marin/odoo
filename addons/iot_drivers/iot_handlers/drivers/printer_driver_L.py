...
class PrinterDriver(PrinterDriverBase):
    def __init__(self, identifier, device):
        ...
        if (
            any(cmd in device["device-id"] for cmd in ["CMD:STAR;", "CMD:ESC/POS;"])
            or "tm-m30" in self.device_name.lower()
        ):
            self.device_subtype = "receipt_printer"
        elif (
            any(
                cmd in device["device-id"]
                for cmd in ["COMMAND SET:ZPL;", "CMD:ESCLABEL;"]
            )
            or "zpl" in self.device_name.lower()
        ):
        ...
        if (
            self.device_subtype == "receipt_printer"
            and self.receipt_protocol == "escpos"
        ):
    ...
    def _init_escpos(self, device):
        try:
            if device.get("usb_product"):
                ...
                def usb_matcher(usb_device):
                    return (
                        ...
                        and usb_device.manufacturer.lower()
                        == device["usb_manufacturer"]
                        and usb_device.product == device["usb_product"]
                        and usb_device.serial_number == device["usb_serial_number"]
                    )

                self.escpos_device = printer.Usb(
                    usb_args={"custom_match": usb_matcher}, timeout=5
                )
            elif device.get("ip"):
            ...
            _logger.info(
                "%s - Could not initialize escpos class: %s", self.device_name, e
            )
    ...
    def print_raw(self, data, action_unique_id=None):
        ...
        if self.escpos_device:
            try:
                ...
                _logger.warning(
                    "Failed to print via python-escpos, falling back to CUPS"
                )

        try:
            with self.cups_lock:
                job_id = self.conn.createJob(
                    self.device_identifier,
                    "Odoo print job",
                    {"document-format": CUPS_FORMAT_AUTO},
                )
    ...
    def _get_iot_status(cls):
        ...
        ssid = (
            wifi.get_access_point_ssid()
            if wifi.is_access_point()
            else wifi.get_current()
        )
        ...
        if self.escpos_device:
            try:
                with EscposIO(self.escpos_device) as dev:
                    if not self._check_status_escpos(
                        dev.printer, action_unique_id=None
                    ):
                        return
                    dev.printer.set(
                        align="center", double_height=True, double_width=True
                    )
                    dev.printer.textln(title.decode())
                    dev.printer.set_with_default(
                        align="center", double_height=False, double_width=False
                    )
                ...
                _logger.warning(
                    "Failed to print QR status receipt, falling back to simple receipt"
                )
    ...
    def print_status_zpl(self):
        ...
        title = (
            "IoT Box Connected" if helpers.get_odoo_server_url() else "IoT Box Status"
        )
        ...
        if iot_status["pairing_code"]:
            ...
            p += 35
            command += (
                f"^FT35,{p} ^A0N,25 ^FDPairing code: {iot_status['pairing_code']}^FS"
            )
            p += 35
        if iot_status["ssid"]:
    ...
    def _printer_status_content(self):
        ...
        title = (
            b"IoT Box Connected" if helpers.get_odoo_server_url() else b"IoT Box Status"
        )
        body = (
            pairing_code
            + wlan
    ...
    def _action_default(self, data):
        ...
        self.print_raw(
            b64decode(data["document"]), action_unique_id=data.get("action_unique_id")
        )
    ...
    def _check_job_status(self, job_id):
        try:
            with self.cups_lock:
                job = self.conn.getJobAttributes(
                    job_id,
                ...
                # Cannot reach network printer
                    ...
                    and "printer is unreachable"
                    in job.get("job-printer-state-message", "")
                ):
    ...
    def default_printer_action(self, data):
        printer = next(
            (
                ...
                if iot_devices[d].device_type == "printer"
