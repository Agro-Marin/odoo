...
class TremolG03Driver(SerialDriver):
    ...
    def get_default_device(cls):
        fiscal_devices = list(
            filter(
                lambda d: iot_devices[d].device_type == "fiscal_data_module",
                iot_devices,
            )
        )
    ...
    def send(self, msgs):
        ...
        with self._device_lock:
            ...
            for msg in msgs:
                ...
                core_message = struct.pack(
                    "BB%ds" % (len(msg)), len(msg) + 34, self.message_number + 32, msg
                )
                request = struct.pack(
                    "B%ds2sB" % (len(core_message)),
                    STX,
                    core_message,
                    self.generate_checksum(core_message),
                    ETX,
                )
                ...
                if output_size:
                    try:
                        ...
                        _logger.exception(
                            "Timeout error while reading response to command %s", msg
                        )
                ...
                if response[0] == ACK:
                    ...
                    if response[2] != 0x30 or response[3] != 0x30:
                        ...
                        _logger.error(
                            ...
                            request,
                            FD_ERRORS.get(response[2], "Unknown fiscal device error"),
    ...
    def l10n_ke_cu_send(self, messages, company_vat):
        ...
        if device:
            ...
            if device_vat != company_vat:
                return json.dumps(
                    {
                        "status": "The company vat number does not match that of the device"
                    }
                )
