...
class PrinterDriver(PrinterDriverBase):
    ...
    def print_raw(self, data, action_unique_id=None):
        if self.escpos_device:
            try:
                ...
                _logger.warning(
                    "Failed to print via python-escpos, falling back to Windows printing"
                )
        ...
        try:
            with win32print_lock:
                job_id = win32print.StartDocPrinter(
                    self.printer_handle, 1, ("", None, "RAW")
                )
            ...
            _logger.error(
                "Error while printing raw data to printer %s: %s",
                self.device_name,
                error,
            )
            if job_id or page_started:
                try:
                    ...
                    _logger.error(
                        "Error while finalizing print job to printer %s after failure: %s",
                        self.device_name,
                        err,
                    )
    ...
    def print_report(self, data):
        with win32print_lock:
            ...
            _logger.debug(
                "Printing report with %s using %s",
                "SumatraPDF" if use_sumatra else "Ghostscript",
                args,
            )
            if use_sumatra:
                try:
                    ...
                    _logger.exception(
                        "Error while printing report, SumatraPDF args: %s, exit code: %s",
                        args,
                        error.returncode,
                    )
            else:
                ...
                try:
                    ...
                    stdout_log_level = (
                        logging.ERROR
                    )  # some stdout value might contains relevant error information
                    self.send_status(status="error", message="ERROR_FAILED")
                    raise
                finally:
                    _logger.log(
                        stdout_log_level,
                        "Ghostscript stdout: %s",
                        stdout_buf.getvalue(),
                    )
    ...
    def _action_default(self, data):
        ...
        _logger.debug(
            "_action_default finished with mimetype %s for printer %s",
            mimetype,
            self.device_name,
        )
    ...
    def print_status(self, _data=None):
        ...
        if self.device_subtype == "receipt_printer":
            ...
            self.print_raw(
                commands["center"]
                + (commands["title"] % b"IoT Box Test Receipt")
                + commands["cut"]
            )
    ...
    def _cancel_job_with_error(self, job_id, error_message):
        ...
        win32print.SetJob(
            self.printer_handle, job_id, 0, None, win32print.JOB_CONTROL_DELETE
        )
        self.send_status(
            status="error",
    ...
    def _check_job_status(self, job_id):
        try:
            ...
            elapsed_time = datetime.now(UTC) - job["Submitted"]
