...
class EtaUsbController(http.Controller):
    ...
    def eta_certificate(self, pin, access_token):
        ...
        if error:
            return error
        try:
            cert = session.findObjects([(PyKCS11.CKA_CLASS, PyKCS11.CKO_CERTIFICATE)])[
                0
            ]
            cert_bytes = bytes(session.getAttributeValue(cert, [PyKCS11.CKA_VALUE])[0])
            payload = {"certificate": base64.b64encode(cert_bytes).decode()}
            return http.request.prepare_json_response(payload)
        ...
        try:
            ...
            for invoice, eta_inv in invoices.items():
                ...
                signed_data = session.sign(
                    priv_key, to_sign, PyKCS11.Mechanism(PyKCS11.CKM_SHA256_RSA_PKCS)
                )
