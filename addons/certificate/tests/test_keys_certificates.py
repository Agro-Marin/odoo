...
class TestKeysCertificates(TransactionCase):
    ...
    def test_ec_key_generated(self):
        ...
        private_key_obj = serialization.load_pem_private_key(
            base64.b64decode(private_key.pem_key), None
        )
    ...
    def test_ed25519_key_generated(self):
        private_key = self.env["certificate.key"]._generate_ed25519_private_key(
            self.env.company
        )
        private_key_obj = serialization.load_pem_private_key(
            base64.b64decode(private_key.pem_key), None
        ...
        certificate = self.env["certificate.certificate"].create(
            {
                ...
                "content": base64.b64encode(
                    self.certificate_1.public_bytes(encoding=serialization.Encoding.DER)
                ),
        ...
        certificate = self.env["certificate.certificate"].create(
            {
                ...
                "content": base64.b64encode(
                    self.certificate_1.public_bytes(encoding=serialization.Encoding.PEM)
                ),
                "private_key_id": self.test_key_1.id,
            }
        )
        ...
        certificate = self.env["certificate.certificate"].create(
            {
                ...
                "content": base64.b64encode(
                    file_open("certificate/tests/data/cert.pfx", "rb").read()
                ),
            }
        )
    ...
    def test_is_valid(self):
        private_key = serialization.load_pem_private_key(
            base64.b64decode(self.test_key_1.pem_key), None
        )
        ...
        expired_cert = self.env["certificate.certificate"].create(
            {
                ...
                "content": base64.b64encode(
                    old_certificate.public_bytes(encoding=serialization.Encoding.PEM)
                ),
                "private_key_id": self.test_key_1.id,
            }
        )
    ...
    def test_keys_certificate_not_matching(self):
        with self.assertRaises(
            UserError, msg="The certificate and private key are not compatible."
        ):
            self.env["certificate.certificate"].create(
                {
                    "name": "Test PEM Certificate and key don't match",
                    "content": base64.b64encode(
                        self.certificate_1.public_bytes(
                            encoding=serialization.Encoding.PEM
                        )
                    ),
        ...
        with self.assertRaises(
            UserError, msg="The certificate and public key are not compatible."
        ):
            self.env["certificate.certificate"].create(
                {
                    "name": "Test PEM Certificate and key don't match",
