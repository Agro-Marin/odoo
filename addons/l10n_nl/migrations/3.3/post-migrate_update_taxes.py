...
def migrate(cr, version):
    ...
    goods_tax_ids = [
        tax_id
        for xml_id in ["btw_X0_producten", "btw_X0"]
        for tax_id in _get_tax_ids_for_xml_id(env, xml_id)
    ]

    goods_taxes = env["account.tax"].browse(goods_tax_ids)
    services_taxes = env["account.tax"].browse(
        _get_tax_ids_for_xml_id(env, "btw_X0_diensten")
    )
    ...
    services_plus_tax_tag_id = services_tax_tags.filtered(
        lambda tag: not tag.tax_negate
    ).id
    services_minus_tax_tag_id = services_tax_tags.filtered(
        lambda tag: tag.tax_negate
    ).id

    insert_query_params = [
        (
            goods_plus_tax_tag_id,
            goods_taxes.ids,
            old_plus_tax_tag_ids,
            goods_taxes.invoice_repartition_line_ids.ids,
        ),
        (
            services_plus_tax_tag_id,
            services_taxes.ids,
            old_plus_tax_tag_ids,
            services_taxes.invoice_repartition_line_ids.ids,
        ),
        (
            goods_minus_tax_tag_id,
            goods_taxes.ids,
            old_minus_tax_tag_ids,
            goods_taxes.refund_repartition_line_ids.ids,
        ),
        (
            services_minus_tax_tag_id,
            services_taxes.ids,
            old_minus_tax_tag_ids,
