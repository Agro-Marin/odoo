...
class TestAccountMoveImport(AccountTestInvoicingCommon):
    ...
    def test_import_purchase_order_reference_from_provided_field(self):
        ...
        self.assertTrue(
            all(
                [
                    line.purchase_line_ids.order_id == self.purchase_order
                    for line in bill.line_ids
                    if line.purchase_line_ids.order_id
                ]
                or [False]
            )
        )
    ...
    def test_multiple_purchase_order_references(self):
        ...
        self.assertTrue(
            all(not line.purchase_line_ids.order_id for line in bill_2.line_ids)
        )
