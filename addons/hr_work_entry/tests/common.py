...
class TestWorkEntryBase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.work_entry_type_leave = cls.env["hr.work.entry.type"].create(
            {"name": "Time Off", "is_leave": True, "code": "LEAVETEST100"}
        )
    ...
    def create_work_entries(self, intervals):
        ...
        for interval in intervals:
            ...
            stop = interval[1]
            work_entry_type = (
                interval[2] if len(interval) == 3 else default_work_entry_type
            )
            create_vals.append(
                {
        ...
        create_vals = self.env["hr.version"]._generate_work_entries_postprocess(
            create_vals
        )
