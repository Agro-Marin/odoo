...
class TestWorkEntry(TestWorkEntryBase):
    ...
    def test_work_entry(self):
        self.richard_emp.generate_work_entries(self.start, self.end)
        attendance_nb = len(
            self.resource_calendar_id._attendance_intervals_batch(
                self.start.replace(tzinfo=UTC), self.end.replace(tzinfo=UTC)
            )[False]
        )
        ...
        self.assertEqual(
            attendance_nb / 2,
            work_entry_nb,
            "One work_entry should be generated for each pair of calendar attendance per day",
        )
    ...
    def test_validate_undefined_work_entry(self):
        ...
        self.assertFalse(
            work_entry1.action_validate(),
            "It should not validate work_entries without a type",
        )
        self.assertEqual(
            work_entry1.state, "conflict", "It should change to conflict state"
        )
        work_entry1.work_entry_type_id = self.work_entry_type
        self.assertTrue(
            work_entry1.action_validate(), "It should validate work_entries"
        )
    ...
    def test_outside_calendar(self):
        work_entries = self.create_work_entries(
            [
                ...
                (
                    datetime(2018, 10, 13, 1, 0),
                    datetime(2018, 10, 13, 2, 0),
                    self.work_entry_type_leave,
                ),
            ]
        )
        ...
        self.assertEqual(
            len(work_entries),
            1,
            "The second work entry should not be generated. As it is a leave outside of the working schedule",
        )
        self.assertNotEqual(work_entries.state, "conflict", "It should not conflict")

    def test_work_entry_timezone(self):
        hk_resource_calendar_id = self._create_calendar(
            "HK Calendar", 7, 11, 13, 17, tz="Asia/Hong_Kong"
        )
        ...
        self.env["resource.calendar.leaves"].create(
            {
                ...
                .astimezone(UTC)
                .replace(tzinfo=None),
                "date_to": datetime(2023, 8, 2, 23, 59, 59)
                .replace(tzinfo=timezone("Asia/Hong_Kong"))
                .astimezone(UTC)
                .replace(tzinfo=None),
    ...
    def test_separate_overlapping_work_entries_by_type(self):
        calendar = self.env["resource.calendar"].create(
            {"name": "Calendar", "tz": "Europe/Brussels"}
        )
        ...
        calendar.attendance_ids -= calendar.attendance_ids.filtered(
        ...
        result_entries = self.env["hr.work.entry"].search(
        ...
        self.assertEqual(
            len(result_entries),
            2,
            "A shift should be created for each pair of attendance by day",
        )
    ...
    def test_work_entry_different_calendars(self):
        ...
        emp_flex_std, emp_std_flex, emp_fullyflex_std, emp_std_fullyflex = self.env[
            "hr.employee"
        ].create(
            [
                {"name": "emp flex std"},
        ...
        all_work_entries = (
            emp_flex_std + emp_std_flex + emp_fullyflex_std + emp_std_fullyflex
        ).generate_work_entries(date(2025, 9, 1), date(2025, 9, 30))
        flex_std_work_entries = all_work_entries.filtered(
            lambda e: e.employee_id == emp_flex_std
        )
        self.assertEqual(len(flex_std_work_entries), 26)
        half1_entries = flex_std_work_entries.filtered(
            lambda e: e.date >= half1_date_start and e.date <= half1_date_end
        )
        half2_entries = flex_std_work_entries.filtered(
            lambda e: e.date >= half2_date_start and e.date <= half2_date_end
        )
        self.assertEqual(len(half1_entries), 15)
        self.assertTrue(all(entry.duration == 3 for entry in half1_entries))
        self.assertTrue(
            all(entry.version_id.name == "Flex Contract" for entry in half1_entries)
        )
        self.assertEqual(len(half2_entries), 11)
        self.assertTrue(all(entry.duration == 8 for entry in half2_entries))
        self.assertTrue(
            all(entry.version_id.name == "Std Contract" for entry in half2_entries)
        )

        std_flex_work_entries = all_work_entries.filtered(
            lambda e: e.employee_id == emp_std_flex
        )
        self.assertEqual(len(std_flex_work_entries), 26)
        half1_entries = std_flex_work_entries.filtered(
            lambda e: e.date >= half1_date_start and e.date <= half1_date_end
        )
        half2_entries = std_flex_work_entries.filtered(
            lambda e: e.date >= half2_date_start and e.date <= half2_date_end
        )
        self.assertEqual(len(half1_entries), 11)
        self.assertTrue(all(entry.duration == 8 for entry in half1_entries))
        self.assertTrue(
            all(entry.version_id.name == "Std Contract" for entry in half1_entries)
        )
        self.assertEqual(len(half2_entries), 15)
        self.assertTrue(all(entry.duration == 3 for entry in half2_entries))
        self.assertTrue(
            all(entry.version_id.name == "Flex Contract" for entry in half2_entries)
        )

        fullyflex_std_work_entries = all_work_entries.filtered(
            lambda e: e.employee_id == emp_fullyflex_std
        )
        self.assertEqual(len(fullyflex_std_work_entries), 26)
        half1_entries = fullyflex_std_work_entries.filtered(
            lambda e: e.date >= half1_date_start and e.date <= half1_date_end
        )
        half2_entries = fullyflex_std_work_entries.filtered(
            lambda e: e.date >= half2_date_start and e.date <= half2_date_end
        )
        self.assertEqual(len(half1_entries), 15)
        ...
        self.assertTrue(
            all(
                entry.version_id.name == "FullyFlex Contract" for entry in half1_entries
            )
        )
        self.assertEqual(len(half2_entries), 11)
        self.assertTrue(all(entry.duration == 8 for entry in half2_entries))
        ...
        std_fullyflex_work_entries = all_work_entries.filtered(
            lambda e: e.employee_id == emp_std_fullyflex
        )
        self.assertEqual(len(std_fullyflex_work_entries), 26)
        half1_entries = std_fullyflex_work_entries.filtered(
            lambda e: e.date >= half1_date_start and e.date <= half1_date_end
        )
        half2_entries = std_fullyflex_work_entries.filtered(
            lambda e: e.date >= half2_date_start and e.date <= half2_date_end
        )
        self.assertEqual(len(half1_entries), 11)
        self.assertTrue(all(entry.duration == 8 for entry in half1_entries))
        ...
        self.assertTrue(
            all(
                entry.version_id.name == "FullyFlex Contract" for entry in half2_entries
            )
        )
    ...
    def test_work_entry_version_changed_after_generation(self):
        ...
        self.assertEqual(len(work_entries), 23, "23 attendance")
        self.assertEqual(
            sum(work_entries.mapped("duration")), 178, "7 * 8h + 6 * 7h + 10 * 8h"
        )
