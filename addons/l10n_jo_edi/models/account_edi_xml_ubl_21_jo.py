...
class AccountEdiXmlUBL21JO(models.AbstractModel):
    ...
    def _add_invoice_config_vals(self, vals):
        ...
        vals["document_type"] = (
            "invoice"  # Only use Invoice in JO, even for credit and debit notes
        )
        vals["fixed_taxes_as_allowance_charges"] = (
            False  # In JO, fixed taxes should be reported as taxes, not as AllowanceCharges
        )
    ...
    def _add_invoice_base_lines_vals(self, vals):
        ...
        base_amls = invoice.line_ids.filtered(
            lambda line: line.display_type == "product"
        )
        base_lines = [
            invoice._prepare_product_base_line_for_taxes_computation(line)
            for line in base_amls
        ]
        epd_amls = invoice.line_ids.filtered(lambda line: line.display_type == "epd")
        base_lines += [
            invoice._prepare_epd_base_line_for_taxes_computation(line)
            for line in epd_amls
        ]
        cash_rounding_amls = invoice.line_ids.filtered(
            lambda line: (
                line.display_type == "rounding" and not line.tax_repartition_line_id
            )
        )
        base_lines += [
            invoice._prepare_cash_rounding_base_line_for_taxes_computation(line)
        ...
        for base_line in base_lines:
            ...
            AccountTax._add_tax_details_in_base_line(
                base_line, base_line["record"].company_id, "round_per_line"
            )
        ...
        document_node.update(
            {
                ...
                "cbc:InvoiceTypeCode": {
                    ...
                    "name": invoice._get_invoice_scope_code()
                    + invoice._get_invoice_payment_method_code()
                    + invoice._get_invoice_tax_payer_type_code(),
                },
        ...
        if invoice.reversed_entry_id:
            document_node["cac:BillingReference"] = {
                "cac:InvoiceDocumentReference": {
                    "cbc:ID": {
                        "_text": (invoice.reversed_entry_id.name or "").replace(
                            "/", "_"
                        )
                    },
    ...
    def _add_invoice_accounting_customer_party_nodes(self, document_node, vals):
        ...
        if invoice.move_type == "out_refund":
            document_node["cac:AccountingCustomerParty"] = {
                "cac:Party": {
                    "cac:PostalAddress": {
                        "cac:Country": {"cbc:IdentificationCode": {"_text": "JO"}}
                    },
                    "cac:PartyTaxScheme": {
                        "cac:TaxScheme": {"cbc:ID": {"_text": "VAT"}}
                    },
                },
                "cac:AccountingContact": {"cbc:Telephone": {"_text": ""}},
            }
        else:
            # For non-refund invoices, use the standard party node
            document_node["cac:AccountingCustomerParty"]["cac:AccountingContact"] = {
                "cbc:Telephone": {
                    "_text": self._sanitize_phone(
                        invoice.partner_id.phone_ids._primary().number
                    )
                }
            }
    ...
    def _add_invoice_payment_means_nodes(self, document_node, vals):
        ...
        if invoice.move_type == "out_refund":
            document_node["cac:PaymentMeans"] = {
                ...
                "cbc:InstructionNote": {"_text": (invoice.ref or "").replace("/", "_")},
            }
    ...
    def _add_document_tax_total_nodes(self, document_node, vals):
        ...
        super()._add_document_tax_total_nodes(
            document_node, {**vals, "tax_grouping_function": tax_grouping_function}
        )
    ...
    def _add_invoice_monetary_total_nodes(self, document_node, vals):
        ...
        base_lines_aggregated_tax_details = self.env[
            ...
            vals["base_lines"], vals["total_grouping_function"]
        )
        ...
        document_node[monetary_total_tag] = {
            ...
            if allowance_total_amount
            else None,
            "cbc:PrepaidAmount": {
                "_text": self.format_float(0.0, vals["currency_dp"]),
                "currencyID": vals["currency_name"],
            }
            if invoice._is_sales_refund()
            else None,
            "cbc:PayableAmount": {
                "_text": self.format_float(tax_inclusive_amount, vals["currency_dp"]),
                "currencyID": vals["currency_name"],
    ...
    def _add_document_allowance_charge_nodes(self, document_node, vals):
        ...
        document_node["cac:AllowanceCharge"] = self._get_document_allowance_charge_node(
            vals
        )
    ...
    def _add_document_line_gross_subtotal_and_discount_vals(self, vals):
        ...
        vals["gross_price_unit_currency"] = (
            currency_9_dp.round(gross_subtotal_currency / base_line["quantity"])
            if base_line["quantity"]
            else 0.0
        )

        # Then compute the gross subtotal from the rounded unit price
        vals["gross_subtotal_currency"] = currency_9_dp.round(
            vals["gross_price_unit_currency"] * base_line["quantity"]
        )

        # Then compute the discount from the gross subtotal
        vals["discount_amount_currency"] = (
            vals["gross_subtotal_currency"]
    ...
    def _add_document_line_tax_total_nodes(self, line_node, vals):
        ...
        line_node["cac:TaxTotal"] = {
            ...
            "cbc:TaxAmount": {
                "_text": self.format_float(
                    total_tax_amount_percent, vals["currency_dp"]
                ),
                "currencyID": vals["currency_name"],
            },
            "cbc:RoundingAmount": {
    ...
    def _add_invoice_line_price_nodes(self, line_node, vals):
        ...
        line_node["cac:Price"] = {
            "cbc:PriceAmount": {
                "_text": self.format_float(
                    vals["gross_price_unit_currency"], vals["currency_dp"]
                ),
                "currencyID": vals["currency_name"],
            },
    ...
    def _get_line_discount_allowance_charge_node(self, vals):
        ...
        return {
            "cbc:ChargeIndicator": {
                "_text": "false" if vals["discount_amount_currency"] > 0 else "true"
            },
            "cbc:AllowanceChargeReason": {"_text": "DISCOUNT"},
            "cbc:Amount": {
                "_text": self.format_float(
