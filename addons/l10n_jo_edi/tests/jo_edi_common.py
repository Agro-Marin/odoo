...
class JoEdiCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.jo_general_tax_16_included = cls.jo_general_tax_16.copy(
            {"name": "Tax 16% included"}
        )
        ...
        cls.partner_jo = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create({"number": "+962 795-5585-949", "type": "landline"})
                ],
                "is_company": True,
            }
        )
    ...
    def _l10n_jo_create_refund(self, invoice, return_reason, refund_vals):
        invoice = (
            self._l10n_jo_create_invoice(invoice)
