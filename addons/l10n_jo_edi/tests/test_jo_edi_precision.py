...
class TestJoEdiPrecision(JoEdiCommon):
    def _equality_check(self, vals_dict, up_to_jo_max_dp=True):
        ...
        for label, value in vals_dict.items():
            ...
            elif not equals(value, first_tuple[1]):
                error_message += (
                    f"{label} ({value}) != {first_tuple[0]} ({first_tuple[1]})\n"
                )
    ...
    def _extract_vals_from_subtotals(self, subtotals, defaults):
        for subtotal in subtotals:
            tax_percent = float(
                subtotal.findtext("{*}TaxCategory/{*}Percent", default=-1)
            )
            if tax_percent == -1:  # special (fixed amount) tax
                defaults.update(
                    {
                        "taxable_amount_special": float(
                            subtotal.findtext("{*}TaxableAmount")
                        ),
                ...
                defaults.update(
                    {
                        "taxable_amount_general": float(
                            subtotal.findtext("{*}TaxableAmount")
                        ),
                        "tax_amount_general_subtotal": float(
                            subtotal.findtext("{*}TaxAmount")
                        ),
    ...
    def _validate_jo_edi_numbers(self, xml_string, invoice):
        ...
        tax_exclusive_amount = float(
            root.findtext("./{*}LegalMonetaryTotal/{*}TaxExclusiveAmount")
        )
        tax_inclusive_amount = float(
            root.findtext("./{*}LegalMonetaryTotal/{*}TaxInclusiveAmount")
        )
        self.assertEqual(
            float_compare(tax_inclusive_amount, invoice.amount_total, 2),
            0,
            f"{tax_inclusive_amount} != {invoice.amount_total}",
        )
        monetary_values_discount = float(
            root.findtext("./{*}LegalMonetaryTotal/{*}AllowanceTotalAmount") or 0.0
        )
        ...
        error_message += self._equality_check(
            {  # They have to be exactly the same, no decimal difference is tolerated
        ...
        for xml_line in root.findall("./{*}InvoiceLine"):
            ...
            line = {
                ...
                "tax_amount_general": float(
                    xml_line.findtext("{*}TaxTotal/{*}TaxAmount", default=0)
                ),
                "rounding_amount": float(
                    xml_line.findtext(
                        "{*}TaxTotal/{*}RoundingAmount", default=line_extension_amount
                    )
                ...
                **self._extract_vals_from_subtotals(
                ...
                "discount": float(
                    xml_line.findtext("{*}Price/{*}AllowanceCharge/{*}Amount") or 0.0
                ),
            }
            lines.append(line)
            line_errors = (
                self._equality_check(
                    {
                        ...
                        "Price Unit * Quantity - Discount": line["price_unit"]
                        * line["quantity"]
                        - line["discount"],
                    }
                )
                + self._equality_check(
                    {
                        ...
                        "Line Extension Amount + Total Tax": line[
                ...
                + self._equality_check(
                    {
                        ...
                        "General Tax Amount in subtotal": line[
                            "tax_amount_general_subtotal"
                        ],
                        "Taxable Amount * Tax Percent": (
                            line["taxable_amount_general"] + line["tax_amount_special"]
                        )
                        * line["tax_percent"],
                    }
                )
        ...
        aggregated_tax_exclusive_amount = self._sum_max_dp(
            line["price_unit"] * line["quantity"] for line in lines
        )
            ...
            + self._equality_check(
                {
                    ...
                    "Tax Exclusive Amount - Total Discount + Total Tax": tax_exclusive_amount
                    - total_discount
                    + sum(line["total_tax_amount"] for line in lines),
                }
            )
            + self._equality_check(
                {
    ...
    def _validate_invoice_vals_jo_edi_numbers(self, invoice_vals):
        with self.subTest(sub_test_name=invoice_vals["name"]):
            ...
            generated_file = self.env["account.edi.xml.ubl_21.jo"]._export_invoice(
                invoice
            )[0]
                    ...
                    Command.create(
                        {
                            ...
                            "discount": 2.5,
                            "tax_ids": [
                                Command.set(self.jo_general_tax_16_included.ids)
                            ],
                        }
                    ),
                    Command.create(
                    ...
                    Command.create(
                        {
                            ...
                            "tax_ids": [
                                Command.set(
                                    (
                                        self.jo_general_tax_10 | self.jo_special_tax_10
                                    ).ids
                                )
                            ],
                        }
                    ...
                    Command.create(
                        {
                            ...
                            "tax_ids": [
                                Command.set(
                                    (self.jo_general_tax_10 | self.jo_special_tax_5).ids
                                )
                            ],
                        }
                    ),
                    Command.create(
                        {
                            ...
                            "tax_ids": [
                                Command.set(
                                    (self.jo_general_tax_16 | self.jo_special_tax_5).ids
                                )
                            ],
                        }
                    ),
                    Command.create(
                        {
                            ...
                            "tax_ids": [
                                Command.set(
                                    (
                                        self.jo_general_tax_16 | self.jo_special_tax_10
                                    ).ids
                                )
                            ],
                        }
    ...
    def test_jo_credit_notes_price_unit(self):
        ...
        refund = self._l10n_jo_create_refund(
            invoice,
            "return reason",
            {
                "invoice_line_ids": [
