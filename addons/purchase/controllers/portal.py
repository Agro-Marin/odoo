...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if "rfq_count" in counters:
            values["rfq_count"] = (
                PurchaseOrder.search_count([("state", "in", ["sent"])])
                if PurchaseOrder.has_access("read")
                else 0
            )
        if "purchase_count" in counters:
            values["purchase_count"] = (
                PurchaseOrder.search_count([("state", "in", ["purchase", "cancel"])])
                if PurchaseOrder.has_access("read")
                else 0
            )
        return values
    ...
    def _purchase_order_get_page_view_values(self, order, access_token, **kwargs):
        ...
        return self._get_page_view_values(
            order, access_token, values, history, False, **kwargs
        )

    @http.route(
        ["/my/rfq", "/my/rfq/page/<int:page>"], type="http", auth="user", website=True
    )
    ...
    def portal_my_purchase_order(self, order_id=None, access_token=None, **kw):
        ...
        if report_type in ("html", "pdf", "text"):
            report_ref = (
                ...
                else "purchase.action_report_purchase_order"
            )
    ...
    def portal_my_purchase_order_update_dates(
        ...
        for id_str, date_str in kw.items():
            ...
            line = order_sudo.line_ids.filtered(lambda l: l.id == line_id)
            if not line:
                return request.redirect(order_sudo.get_portal_url())
    ...
    @http.route(
        ["/my/purchase/<int:order_id>/download_edi"], auth="public", website=True
    )
