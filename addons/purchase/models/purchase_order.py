...
class PurchaseOrder(models.Model):
    ...
    # ------------------------------------------------------------
    # FIELDS — purchase-specific (shared fields come from mixins)
    # ------------------------------------------------------------
    ...
    # Invoice block — purchase-specific overrides
    invoice_ids = fields.Many2many(string="Bills")
    invoice_count = fields.Integer(string="Bill Count")
    invoice_state = fields.Selection(selection=const.INVOICE_STATE)
    ...
    # ------------------------------------------------------------
    # ORDER TYPE ROUTING — required by order.mixin
    # ------------------------------------------------------------
    ...
    def _get_default_user_from_partner(self):
        """Override: use vendor's purchase representative."""
        self.ensure_one()
        return (
    ...
    def _prepare_confirmation_values(self):
        """Override: also set date_confirmed."""
        return {"state": "done", "date_confirmed": fields.Datetime.now()}

    def _prepare_grouped_data(self, order):
        """Override: add dest_address_id to merge grouping key."""
        return (order.partner_id.id, order.currency_id.id, order.dest_address_id.id)
    ...
    def _compute_payment_term_id(self):
        """Override: use vendor payment terms."""
        for order in self:
    ...
    def _compute_currency_id(self):
        """Override: use vendor purchase currency if set."""
        for order in self:
            order = order.with_company(order.company_id)
            order.currency_id = (
    ...
    # _compute_amounts, _compute_tax_totals, _compute_amounts_invoice
    # inherited from order.amount.mixin
    ...
    # _compute_invoice_ids, _compute_invoice_state
    # inherited from order.invoice.mixin
    ...
    def _action_cancel(self):
        """Override: also cancel related draft invoices."""
    ...
    def _action_confirm(self):
        """Override: create supplier info records on confirmation."""
        for order in self:
            order._create_supplier_to_product()

    def action_lock(self):
        """Override: also reset priority on lock."""
        self.write({"locked": True, "priority": "0"})

    # -------------------------------------------------------------------------
    # MERGE — overrides of order.merge.mixin hooks
    # -------------------------------------------------------------------------

    def _merge_get_eligible_orders(self):
        """Override: include 'sent' state for RFQs."""
    ...
    def _merge_lines_match_date(self, line1, line2):
        """Override: require both dates to be set for purchase merge."""
    ...
    def _merge_finalize(self, target, sources):
        """Override: also handle alternative PO references."""
    ...
    def _get_invoice_action_context(self):
        """Override: add invoice_origin for purchase bills."""
    ...
    # _is_readonly inherited from order.mixin
    ...
    # _get_report_base_filename inherited from order.mixin
    ...
    def _prepare_invoice_vals(self):
        """Extend base invoice values with purchase-specific fields."""
    ...
    # ------------------------------------------------------------
    # VALIDATIONS — overrides of order.mixin validation registry
    # ------------------------------------------------------------

    def _get_can_confirm_validation_methods(self):
        """Override: identical to base, kept for extensibility documentation."""
        return super()._get_can_confirm_validation_methods()

    def _can_confirm_analytic_distribution(self):
        """Override: validate analytic distributions with purchase business domain."""
    ...
    def _get_can_cancel_validation_methods(self):
        """Override: add purchase-specific ``_can_cancel_except_invoiced``."""
        methods = super()._get_can_cancel_validation_methods()
        methods.append("_can_cancel_except_invoiced")
        return methods
