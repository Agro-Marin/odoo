...
class PurchaseOrderLine(models.Model):
    ...
    @api.depends("product_id", "partner_id", "selected_seller_id")
    ...
    def _compute_price_unit(self):
        """Compute unit price from seller pricelist or product cost.

        Price updates are skipped for:
        - Display lines (sections, notes) - these must have NULL price
        - Lines without product
        - Invoiced lines
        - Lines with manual price override (price_unit != price_unit_shadow)
        """
        for line in self:
            # Non-accountable lines (sections, notes) must have NULL for constraint
            if line.display_type:
    ...
    @api.depends("selected_seller_id")
    def _compute_discount(self):
        """Compute discount percentage from selected seller.

        Sets discount from the seller's pricelist. If no seller is selected
        or no discount is defined, defaults to 0%.
        """
        for line in self:
    ...
    @api.depends("date_order", "selected_seller_id")
    ...
    def _set_price_from_seller(self):
        """Set price_unit from selected seller's pricelist.
        ...
        Note: Discount is now computed separately by _compute_discount.
        """
        self.ensure_one()
    ...
    def _set_product_description(self):
        ...
        Only updates if name is empty or matches what the old default was.
        """
        self.ensure_one()

        lang = get_lang(self.env, self.partner_id.lang).code

        # Build product context with NEW seller
        seller_id = self.selected_seller_id.id if self.selected_seller_id else None
        product_ctx = {"seller_id": seller_id, "lang": lang}

        # Check if current name was customized (to preserve it)
        # Compare against OLD default (what it should have been before changes)
        if self.name:
            # Get the old seller to compute what the old default would have been
            old_seller_id = (
                self._origin.selected_seller_id.id
                if self._origin.selected_seller_id
                else None
            )
            old_product_ctx = {"seller_id": old_seller_id, "lang": lang}
            old_default = self._get_line_description(
                self.product_id.with_context(old_product_ctx),
            )
            # Only skip if name was manually customized (differs from old default)
            if self.name != old_default:
                return

        # Set description with new seller context
        self.name = self._get_line_description(
