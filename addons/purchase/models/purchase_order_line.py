...
class PurchaseOrderLine(models.Model):
    ...
    qty_transfered = fields.Float(
        ...
        compute_sudo=True,
        store=True,
        inverse="_inverse_qty_transfered",
    )
    qty_transfered_manual = fields.Float(
    ...
    def _compute_price_unit_discounted_taxexc(self):
        for line in self:
            line.price_unit_discounted_taxexc = line.price_unit * (1 - line.discount / 100)
    ...
    def _get_invoice_lines(self):
        ...
        if self.env.context.get("accrual_entry_date"):
            return self.invoice_line_ids.filtered(
                ...
                and l.move_id.invoice_date <= self.env.context["accrual_entry_date"]
            )
        else:
    ...
    @api.depends("qty_transfered_method", "qty_transfered_manual")
    def _compute_qty_transfered(self):
        for line in self:
            if line.qty_transfered_method == "manual":
                line.qty_transfered = line.qty_transfered_manual or 0.0
            else:
                line.qty_transfered = 0.0

    @api.onchange("qty_transfered")
    def _inverse_qty_transfered(self):
        """When writing on qty_transfered, if the value should be modify manually (`qty_transfered_method` = 'manual' only),
        then we put the value in `qty_transfered_manual`. Otherwise, `qty_transfered_manual` should be False since the
        received qty is automatically compute by other mecanisms.
        """
        for line in self:
            if line.qty_transfered_method == "manual":
                line.qty_transfered_manual = line.qty_transfered
            else:
                line.qty_transfered_manual = 0.0
    ...
    def write(self, vals):
        ...
        if "qty_transfered" in values:
            for line in self:
                line._track_qty_transfered(values["qty_transfered"])
        return super().write(values)

    @api.ondelete(at_uninstall=False)
    def _unlink_except_purchase(self):
        for line in self:
            if line.order_id.state == "purchase" and line.display_type not in [
                "line_section",
                "line_subsection",
                "line_note",
            ]:
    ...
    def _compute_price_unit_and_date_planned_and_name(self):
        for line in self:
            if (
                ...
                or (line.technical_price_unit != line.price_unit)
            ):
                continue
            params = line._get_select_sellers_params()
            ...
            if not line.selected_seller_id:
                ...
                line.price_unit = line.technical_price_unit = float_round(
                    price_unit,
                ...
                line.price_unit = line.technical_price_unit = (
                    line.selected_seller_id.product_uom_id._compute_price(
