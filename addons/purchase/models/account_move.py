...
# tolerance applied to the total when searching for a matching purchase order
TOLERANCE = 0.02
...
class AccountMove(models.Model):
    ...
    def write(self, vals):
        # OVERRIDE
        old_purchases = [
            move.mapped("line_ids.purchase_line_id.order_id") for move in self
        ]
    ...
    def _compute_is_purchase_matched(self):
        for move in self:
            if any(
                il.display_type == "product" and not bool(il.purchase_line_id)
        ...
        for move in self:
            if move.purchase_order_count == 1:
                move.purchase_order_name = (
                    move.invoice_line_ids.purchase_order_id.display_name
                )
            else:
    ...
    def _onchange_purchase_auto_complete(self):
        ...
        When setting a 'purchase.bill.match' in 'purchase_vendor_bill_id':
    ...
    def action_view_source_purchase_orders(self):
        ...
        if len(source_orders) > 1:
            ...
            result["views"] = [
                (self.env.ref("purchase.purchase_order_form", False).id, "form")
            ]
    ...
    def _add_purchase_order_lines(self, purchase_order_lines):
        ...
        for po_line in purchase_order_lines:
            new_line_values = po_line._prepare_aml_vals(self)
            ...
            for invoice_line in invoice_lines:
                    ...
                    if (
                        ...
                        purchase_line_candidates[purchase_line] = (
                            difflib.SequenceMatcher(
                                None, invoice_line.name, purchase_line.name
                            ).ratio()
                        )
                ...
                if len(purchase_line_candidates) > 0:
                    ...
                    purchase_line_match = max(
                        purchase_line_candidates, key=purchase_line_candidates.get
                    )
    ...
    def _match_purchase_orders(
        ...
        common_domain = [
            ...
            ("state", "=", "purchase"),
            ("invoice_state", "in", ("to invoice", "no")),
        ]
