...
class TestPurchaseToInvoice(TestPurchaseToInvoiceCommon):
    ...
    def test_vendor_bill_analytic_account_model_change(self):
        ...
        aml = self.env["account.move.line"].search(
            [("purchase_line_ids", "=", purchase_order.line_ids.id)]
        )
        self.assertRecordValues(
            aml, [{"analytic_distribution": analytic_distribution_manual}]
        )
    ...
    def test_purchase_order_analytic_account_product_change(self):
        ...
        self.assertEqual(
            ...
            "The analytic account should be set to 'Super Account'",
        )
        purchase_order_line.write({"product_id": great_product.id})
        self.assertEqual(
            purchase_order_line.analytic_distribution,
            {str(analytic_account_great.id): 100},
            "The analytic account should be set to 'Great Account'",
        )
        ...
        self.assertFalse(
            pol_no_analytic_distribution.analytic_distribution,
    ...
    def test_purchase_order_to_invoice_analytic_rule_with_account_prefix(self):
        ...
        self.env["purchase.order.line"].create(
            {"order_id": po.id, "name": "test", "product_id": self.product_a.id}
        )
        self.assertFalse(
            po.line_ids.analytic_distribution, "There should be no analytic set."
        )
    ...
    def test_sequence_invoice_lines_from_multiple_purchases(self):
        ...
        for line in invoice.invoice_line_ids.sorted("sequence"):
            self.assertEqual(
                line.purchase_line_ids.order_id[:1], expected_purchase.pop(0)
            )
    ...
    def test_on_change_quantity_price_unit(self):
        ...
        self.assertEqual(
            10.0, po_line.price_unit, "Unit price should be set to 10.0 for 1 quantity"
        )
        ...
        self.assertEqual(
            20.0, po_line.price_unit, "Unit price should be set to 20.0 for 2 quantity"
        )
        ...
        self.assertEqual(
            30.0, po_line.price_unit, "Unit price should be set to 30.0 for 3 quantity"
        )
        ...
        self.assertEqual(
            235.0,
            po_line.price_unit,
            "Unit price should be reset to 235.0 since the supplier supplies minimum of 4 quantities",
        )

        # Ensure price unit is updated when changing quantity on PO confirmed and line NOT linked to an invoice line
        po_line.write({"product_qty": 4})
        self.assertEqual(
            40.0, po_line.price_unit, "Unit price should be set to 40.0 for 4 quantity"
        )
    ...
    def test_supplier_discounted_price(self):
        ...
        self.assertEqual(
            840.0, po_line.price_unit_discounted_taxexc, "1680.0 * 0.5 = 840.0"
        )
    ...
    def test_invoice_line_name_has_product_name(self):
        ...
        self.assertEqual(
            ...
            f"{pol_prod_no_redundancy.product_id.display_name}\n{pol_prod_no_redundancy.name}",
        ...
        self.assertEqual(
            ...
            f"{pol_prod_name_in_product.product_id.display_name}\n{pol_prod_name_in_product.name}",
            "When the product name contains the description, the invoice line name should be the product name and the description",
        )
...
class TestInvoicePurchaseMatch(TestPurchaseToInvoiceCommon):
    ...
    def test_subset_total_match_from_ocr(self):
        po = self.init_purchase(
            confirm=True, products=[self.product_order, self.service_order]
        )
        invoice = self.init_invoice(
            "in_invoice", partner=self.partner_a, products=[self.product_order]
        )
        ...
        additional_unmatch_po_line = po.line_ids.filtered(
            lambda l: l.product_id == self.service_order
        )

        self.assertTrue(invoice.id in po.invoice_ids.ids)
        self.assertTrue(
            additional_unmatch_po_line.id in invoice.line_ids.purchase_line_ids.ids
        )
        self.assertTrue(
            invoice.line_ids.filtered(
                lambda l: l.purchase_line_ids == additional_unmatch_po_line
            ).quantity
            == 0
        )
    ...
    def test_subset_match_from_edi_full(self):
        ...
        invoice = self.init_invoice(
            ...
            products=[self.product_order, self.service_order],
        )
    ...
    def test_subset_match_from_edi_same_unit_price(self):
        ...
        po = self.init_purchase(
            confirm=True, products=[self.product_order_var_name, self.product_order]
        )
        invoice = self.init_invoice(
            "in_invoice", partner=self.partner_a, products=[self.product_order]
        )
    ...
    def test_manual_matching_restrict_no_pol(self):
        ...
        with self.assertRaisesRegex(
            UserError, "must select at least one Purchase Order line"
        ):
            match_lines = self.env["purchase.bill.line.match"].search(
                [("partner_id", "=", self.partner_a.id)]
            )
    ...
    def test_manual_matching_allow_multi_bill(self):
        ...
        bill_2 = self.init_invoice(
            move_type="in_invoice",
            partner=self.partner_a,
            products=[self.product_order, self.product_order_var_name],
        )
        ...
        self.assertFalse(
            self.env["account.move.line"].search(
                [("purchase_line_ids", "=", po.line_ids[2].id)]
            )
        )
    ...
    def test_manual_matching_create_bill(self):
        ...
        self.init_purchase(
            confirm=True, products=[self.product_order, self.product_order_var_name]
        )
        self.env["purchase.order.line"].flush_model()
    ...
    def test_manual_matching_multi_po(self):
        ...
        self.env.flush_all()
        self.assertEqual(
            bill.invoice_line_ids.purchase_line_ids, (po_1 + po_2 + po_3).line_ids
        )
        self.assertRecordValues(
    ...
    def test_add_bill_to_po(self):
        bill = self.init_invoice(
            "in_invoice",
            partner=self.partner_a,
            products=[self.product_order_var_name, self.service_deliver],
            post=True,
        )
        self.env["account.move.line"].flush_model()
        ...
        self.assertEqual(
            po.line_ids.product_id, self.product_order_var_name + self.service_deliver
        )

        bill_2 = self.init_invoice(
            "in_invoice",
            partner=self.partner_a,
            products=[self.product_order_other_price],
            post=False,
        )
        ...
        wizard = (
            self.env["bill.to.po.wizard"]
            .with_context(
                {"default_purchase_order_id": po.id, "active_ids": match_lines.ids}
            )
            .create({})
        )
        ...
        self.assertListEqual(
            po.line_ids.mapped("price_total"),
            (bill + bill_2).invoice_line_ids.mapped("price_total"),
        )
    ...
    def test_onchange_partner_currency(self):
        ...
        self.assertEqual(
            ...
            "The currency of the Bill should be the same as the currency of the partner",
        )
        self.assertEqual(
            bill.invoice_line_ids.currency_id,
            self.env.ref("base.EUR"),
            "The currency of the Bill lines should be the same as the currency of the partner",
        )
        ...
        self.assertEqual(
            ...
            "The currency of the Bill should be the same as the currency of the company",
        )
        self.assertEqual(
            bill.invoice_line_ids.currency_id,
            self.env.company.currency_id,
            "The currency of the Bill lines should be the same as the currency of the company",
        )
    ...
    def test_onchange_partner_no_currency(self):
        ...
        self.assertEqual(
            bill.currency_id,
            self.env.ref("base.EUR"),
            "The currency of the Bill should be the one set on the Bill",
        )
        self.assertEqual(
            bill.invoice_line_ids.currency_id,
            self.env.ref("base.EUR"),
            "The currency of the Bill lines should be the same as the currency of the Bill",
        )
        ...
        self.assertEqual(
            bill.currency_id,
            self.other_currency,
            "The currency of the Bill should be the one of the context",
        )
        self.assertEqual(
            bill.invoice_line_ids.currency_id,
            self.other_currency,
    ...
    def test_create_invoice_from_multiple_purchase_orders(self):
        ...
        self.assertEqual(
            len(purchase_orders.invoice_ids), 2, "Each PO should generate one invoice"
        )
        self.assertEqual(
    ...
    def test_po_matching_credit_note(self):
        ...
        credit_note = self.init_invoice(
            move_type="in_refund", partner=self.partner_a, amounts=[0]
        )
