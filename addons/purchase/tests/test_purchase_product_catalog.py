...
class TestPurchaseProductCatalog(AccountTestInvoicingCommon, HttpCase):
    def test_catalog_price(self):
        ...
        other_currency = self.setup_other_currency(
            "HRK", rates=[(fields.Date.today(), 0.5)]
        )
        ...
        self.assertNotEqual(
            other_product.seller_ids[0].currency_id.id, purchase_order.currency_id.id
        )
        ...
        catalog_price_company_cur = resp.json()["result"][str(company_product.id)][
            "price"
        ]
    ...
    def test_seller_price_discounted_with_template(self):
                    ...
                    Command.create(
                        {
                            ...
                            "value_ids": [
                                Command.link(att_color_red.id),
                                Command.link(att_color_blue.id),
                            ],
                        }
                    ),
                ],
        ...
        catalog_info = purchase_order._get_product_price_and_data(
