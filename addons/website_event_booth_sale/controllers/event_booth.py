...
class WebsiteEventBoothController(WebsiteEventController):
    ...
    def event_booth_registration_confirm(
        ...
        if order_sudo.amount_total:
            ...
            return self._prepare_booth_registration_success_values(
                event.name, booth_values
            )
    ...
    def _prepare_booth_contact_form_values(self, event, booth_ids, booth_category_id):
        ...
        values["has_payment_step"] = (
            request.cart.amount_total
            or values.get("booth_category", request.env["event.booth.category"]).price
        )
        return values
    ...
    def _prepare_booth_main_values(
        ...
        values["has_payment_step"] = request.cart.amount_total or any(
            booth_category.price
            for booth_category in values.get(
                "available_booth_category_ids", request.env["event.booth.category"]
            )
        )
        return values
