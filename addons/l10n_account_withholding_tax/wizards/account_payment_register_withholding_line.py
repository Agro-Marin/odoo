...
class AccountPaymentRegisterWithholdingLine(models.TransientModel):
    ...
    def _compute_type_tax_use(self):
        for line in self:
            if line.tax_id:
                ...
                line.type_tax_use = (
                    "sale"
                    if line.payment_register_id.payment_type == "inbound"
                    else "purchase"
                )
    ...
    def _compute_comodel_percentage_paid_factor(self):
        ...
        for wizard, lines in self.grouped("payment_register_id").items():
            ...
            if total_amounts_to_pay["full_amount"]:
                ...
                split_factor = abs(
                    total_amounts_to_pay["full_amount"] / moves_total_amount
                )
                lines.comodel_percentage_paid_factor = (
                    abs(wizard.amount / total_amounts_to_pay["full_amount"])
                    * split_factor
                )
            else:
                lines.comodel_percentage_paid_factor = 0.0
    ...
    def _get_valid_liquidity_accounts(self):
        ...
        return (
            ...
            | self.payment_register_id.payment_channel_id.payment_account_id
            | self.payment_register_id.journal_id.inbound_payment_channel_ids.payment_account_id
            | self.payment_register_id.journal_id.outbound_payment_channel_ids.payment_account_id
