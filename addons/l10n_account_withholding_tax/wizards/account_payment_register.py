...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _compute_withholding_net_amount(self):
        ...
        for wizard in self:
            if wizard.can_edit_wizard:
                wizard.withholding_net_amount = wizard.amount - sum(
                    wizard.withholding_line_ids.mapped("amount")
                )
            else:
    ...
    def _compute_withholding_outstanding_account_id(self):
        ...
        for wizard in self:
            ...
            if not latest_payment:
                ...
                wizard.withholding_outstanding_account_id = latest_payment[0][
    ...
    def _compute_display_withholding(self):
        ...
        for company, wizards in self.grouped("company_id").items():
            ...
            for wizard in self:
                ...
                wizard.display_withholding = (
                    bool(wizard_withholding_taxes) and not will_create_multiple_entry
                )
        ...
        for wizard in self:
            ...
            if not wizard.withholding_line_ids:
                ...
                for move in batch["lines"].move_id:
                    move_base_lines, _move_tax_lines = (
                        move._get_rounded_base_and_tax_lines()
                    )
                    base_lines += move_base_lines

                wizard.withholding_line_ids = (
                    wizard.withholding_line_ids._prepare_withholding_lines_commands(
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        if self.withholding_net_amount < 0:
            raise UserError(
                self.env._("The withholding net amount cannot be negative.")
            )
        ...
        if withholding_account:
            ...
            if (
                not withholding_account.reconcile
                and withholding_account.account_type
                not in ("asset_cash", "liability_credit_card", "off_balance")
            ):
        ...
        for withholding_line_values in self.withholding_line_ids.with_context(
            active_test=False
        ).copy_data():
            del withholding_line_values["payment_register_id"]
            del withholding_line_values["placeholder_value"]
            payment_vals["withholding_line_ids"].append(
                Command.create(withholding_line_values)
            )
        return payment_vals
    ...
    def _get_total_amount_in_wizard_currency(self):
        ...
        moves = self.batches[0][
            "lines"
        ].move_id  # Use the move to get the TOTAL amount, including all installment.
        wizard_curr = self.currency_id
        comp_curr = self.company_currency_id

        total = 0.0
        for line in moves.line_ids.filtered(
            lambda batch_line: batch_line.display_type == "payment_term"
        ):
            ...
            if currency == wizard_curr:
                ...
                total += currency._convert(
                    line.amount_currency, comp_curr, self.company_id, self.payment_date
                )
            elif currency == comp_curr and wizard_curr != comp_curr:
                # Company currency on source line but a foreign currency one on the opposite line.
                total += comp_curr._convert(
                    line.balance, wizard_curr, self.company_id, self.payment_date
                )
            else:
