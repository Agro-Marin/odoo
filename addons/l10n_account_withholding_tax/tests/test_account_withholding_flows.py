...
class TestL10nAccountWithholdingTaxesFlows(TestTaxCommon, AnalyticCommon):
                    ...
                    Command.create(
                        {
                            "product_id": self.product_a.id,
                            "tax_ids": [
                                Command.set((invoice_tax + withholding_tax1).ids)
                            ],
                        }
                    )
                ],
    ...
    def test_withholding_tax_default_tax_on_product(self):
        """
        Test that an invoice having a product with a default withholding tax will cause
        that tax to appear on a default line in the wizard.
        """
        self.product_b.taxes_id = self.percent_tax(
            -1,
            is_withholding_tax_on_payment=True,
            withholding_sequence_id=self.withholding_sequence.id,
        )
    ...
    def test_withholding_not_payment_account_on_method_line(self):
        ...
        with Form(payment_register) as payment_register_form:
            ...
            with payment_register_form.withholding_line_ids.new() as line:
                line.tax_id = withholding_tax
                line.base_amount = 1000.0
            payment_register_form.withholding_outstanding_account_id = (
                self.outstanding_account
            )
    ...
    def test_display_withholding(self):
        ...
        available_withholding_taxes = self.percent_tax(
            -1,
            is_withholding_tax_on_payment=True,
            withholding_sequence_id=self.withholding_sequence.id,
        )
    ...
    def test_withholding_line_base_amount(self):
        ...
        self.product_a.taxes_id = self.percent_tax(
            -1,
            is_withholding_tax_on_payment=True,
            withholding_sequence_id=self.withholding_sequence.id,
        )
    ...
    def test_compute_outstanding_account_id(self):
        """Test the correct behavior of the _compute_withholding_outstanding_account_id method on the wizard.
    ...
    def test_cannot_register_negative_payment(self):
        ...
        with self.assertRaisesRegex(
            UserError, "The withholding net amount cannot be negative."
        ):
            payment_register._create_payments()
    ...
    def test_tax_repartition_on_refund(self):
        """Test that a withholding tax's repartition line tags are applied on both the payment and its refund."""
        ...
        self.assertRecordValues(
            ...
            [
                # Liquidity line:
                {"balance": -990.0, "tax_tag_ids": []},
                # Receivable line:
                {"balance": 1000.0, "tax_tag_ids": []},
                # withholding line:
                {"balance": -10.0, "tax_tag_ids": tax_tag.ids},
                # base lines:
                {"balance": -1000.0, "tax_tag_ids": []},
        ...
        move_reversal = (
            ...
            .create(
                {
                    "date": fields.Date.from_string("2025-02-01"),
