...
class MixinAccountWithholdingLine(models.AbstractModel):
    ...
    _name = "mixin.account.withholding.line"
    ...
    def _compute_base_amount(self):
        ...
        for line in self:
            ...
            if line.source_currency_id:
                ...
                line.base_amount = line_curr.round(
                    line.original_base_amount * percentage_paid_factor
                )
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            if line.original_base_amount:
                line.amount = line_curr.round(
                    line.original_tax_amount
                    * line.base_amount
                    / line.original_base_amount
                )
            else:
                line.amount = 0.0
    ...
    def _update_placeholders(self):
        ...
        lines_per_sequence = self.sorted().grouped(
            lambda l: (
                l.placeholder_type == "given_by_sequence" and l.withholding_sequence_id
            )
        )
        for sequence, lines in lines_per_sequence.items():
            if sequence:
                for i, line in enumerate(lines):
                    line.write(
                        {
                            "placeholder_value": sequence.get_next_char(
                                sequence.number_next_actual + i
                            ),
    ...
    def _constrains_account_id(self):
        ...
        for line in self:
            if (
                ...
                raise UserError(
                    line.env._(
                        'The account "%(account_name)s" is not valid to use on withholding lines.',
    ...
    def _prepare_base_line_for_taxes_computation(self):
        ...
        conversion_rate = self.env["res.currency"]._get_conversion_rate(
            company.currency_id, currency, company, conversion_date
        )
        ...
        manual_tax_amounts = {
            str(self.tax_id.id): {
                ...
                "base_amount": company.currency_id.round(
                    self.base_amount / conversion_rate
                )
                if conversion_rate
                else 0.0,
                "tax_amount_currency": -self.amount,
                "tax_amount": company.currency_id.round(-self.amount / conversion_rate)
                if conversion_rate
                else 0.0,
            }
        }
        return self.env["account.tax"]._prepare_base_line_for_taxes_computation(
            ...
            manual_total_excluded=company.currency_id.round(
                self.base_amount / conversion_rate
            )
            if conversion_rate
            else 0.0,
    ...
    def _prepare_withholding_amls_create_values(self):
        ...
        for line in self:
            if not line.name and not line.withholding_sequence_id:
                raise UserError(
                    self.env._(
                        "Please enter the withholding number for the tax %(tax_name)s",
            ...
            aml_create_values_list.append(
                {
                    **grouping_key,
                    "name": self.env._(
                        "WH Base: %(names)s", names=", ".join(amounts["names"])
                    ),
                    "tax_ids": [],
                    "tax_tag_ids": [],
            ...
            aml_create_values_list.append(
                {
                    **grouping_key,
                    "name": self.env._(
                        "WH Base Counterpart: %(names)s",
                        names=", ".join(amounts["names"]),
                    ),
    ...
    def _get_withholding_tax_domain(self, company, payment_type):
        ...
        return Domain.AND(
            [
                filter_domain,
                Domain("type_tax_use", "=", payment_type),
    ...
    def _prepare_withholding_lines_commands(self, base_lines, company):
        ...
        existing_withholding_line_map = self.grouped(
            key=lambda l: l._get_grouping_key()
        )
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            new_base_lines, grouping_function
        )
        ...
        for grouping_key, values in values_per_grouping_key.items():
            ...
            if existing_line:
                ...
                withholding_line_commands.append(
                    Command.update(
                        existing_line.id,
                        {
                            "source_base_amount_currency": values[
                                "base_amount_currency"
                            ],
                            "source_base_amount": values["base_amount"],
                            "source_tax_amount_currency": -values[
                                "tax_amount_currency"
                            ],
                    ...
                    Command.create(
                        {
                            ...
                            "analytic_distribution": grouping_key[
    ...
    def _is_refund(self):
        ...
        """
        return (
            self.type_tax_use == "sale" and self.comodel_payment_type == "outbound"
        ) or (
            self.type_tax_use == "purchase" and self.comodel_payment_type == "inbound"
        )
