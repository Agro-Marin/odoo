...
class TestCourseCertificationStats(TestSurveyCommon):
    ...
    def setUpClass(cls):
        ...
        cls.slide_partners = cls.env["slide.slide.partner"].create(
            [
                ...
                for slide in cls.courses.slide_ids
                for partner in cls.participants.partner_id
            ]
        )
    ...
    def test_membership_certification_fields(self):
        ...
        certified_partners_memberships = self.partner_memberships.filtered(
            lambda membership: (
                (
                    membership.channel_id == self.course_1
                    and membership.partner_id == self.survey_user.partner_id
                )
                or (
                    membership.channel_id == self.course_3
                    and membership.partner_id == self.user_portal.partner_id
                )
            )
        )
        non_certified_partners_memberships = (
            self.partner_memberships - certified_partners_memberships
        )

        self.assertTrue(
            all(certified_partners_memberships.mapped("survey_certification_success"))
        )
        self.assertFalse(
            any(
                non_certified_partners_memberships.mapped(
