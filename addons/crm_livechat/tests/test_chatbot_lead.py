...
class CrmChatbotCase(chatbot_common.CrmChatbotCase):
    ...
    def test_chatbot_create_lead_company(self):
        self.user_portal.write(
            {"company_ids": self.company_2, "company_id": self.company_2}
        )
    ...
    def test_chatbot_create_lead_and_forward_company(self):
        self.step_create_lead.sudo().step_type = "create_lead_and_forward"
        self.user_portal.write(
            {"company_ids": self.company_2.ids, "company_id": self.company_2.id}
        )
        self.user_employee.write(
            {"company_ids": self.company_3.ids, "company_id": self.company_3.id}
        )
    ...
    def _play_session_with_lead(self):
        ...
        self.assertEqual(
            discuss_channel.chatbot_current_step_id, self.step_create_lead_email
        )
        self._post_answer_and_trigger_next_step(
            discuss_channel, email="test2@example.com"
        )
        self.assertEqual(
            discuss_channel.chatbot_current_step_id, self.step_create_lead_phone
        )
    ...
    def test_create_lead_from_chatbot(self):
        ...
        self.start_tour(
            ...
            "crm_livechat.create_lead_from_chatbot",
        )
        lead = self.env["crm.lead"].search(
            [("origin_channel_id", "=", livechat_channel.channel_ids.id)]
        )
