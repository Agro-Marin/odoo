...
class ChatbotScriptStep(models.Model):
    ...
    def _compute_is_forward_operator(self):
        ...
        self.filtered(
            lambda s: s.step_type == "create_lead_and_forward"
        ).is_forward_operator = True
    ...
    def _process_step_create_lead(self, discuss_channel):
        ...
        create_values.update(
            self._chatbot_crm_prepare_lead_values(
    ...
    def _process_step_create_lead_and_forward(self, discuss_channel):
        ...
        if previous_operator != discuss_channel.livechat_operator_id:
            user = next(
                user
                for user in users
                if user.partner_id == discuss_channel.livechat_operator_id
            )
            lead.user_id = user
            lead.team_id = next(
