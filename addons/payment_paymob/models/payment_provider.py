...
class PaymentProvider(models.Model):
        ...
        for provider in self.filtered(lambda p: p.code == "paymob"):
            if len(provider.available_currency_ids) > 1:
                raise ValidationError(
                    _("Only one currency can be selected per Paymob account.")
                )
            if (
                provider.available_currency_ids
                and provider.available_currency_ids.name
                not in const.CURRENCY_MAPPING.values()
            ):
                raise ValidationError(
                    _("Only currencies supported by Paymob can be selected.")
                )
    ...
    def _inverse_paymob_account_country_id(self):
        for provider in self.filtered(lambda p: p.code == "paymob"):
            if self.paymob_account_country_id.code:
                currency_code = const.CURRENCY_MAPPING.get(
    ...
    def _match_paymob_payment_methods(self, paymob_gateways_data):
        ...
        for gateway_data in sorted_gateways_data:
            if (
                not available_payment_method_codes
            ):  # All available payment methods are now matched.
                break
    ...
    def _update_payment_method_integration_names(self, matched_gateways_data):
        ...
        for gateway_data in matched_gateways_data:
            payment_method_code = const.PAYMENT_METHODS_MAPPING[
            ...
            payload = {
                "integration_name": f"{payment_method_code.replace('_', '')}{environment}"
            }
