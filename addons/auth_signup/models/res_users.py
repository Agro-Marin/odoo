...
class ResUsers(models.Model):
    ...
    def _action_reset_password(self, signup_type="reset"):
        ...
        if create_mode:
            if any(user._is_internal() for user in self):
                internal_account_created_template = self.env.ref(
                    "auth_signup.set_password_email", raise_if_not_found=False
                )
                if (
                    internal_account_created_template
                    and internal_account_created_template._name != "mail.template"
                ):
                    _logger.error(
                        "Wrong set password template %r",
                        internal_account_created_template,
                    )
                    return None

            if any(not user._is_internal() for user in self):
                portal_account_created_template = self.env.ref(
                    "auth_signup.portal_set_password_email", raise_if_not_found=False
                )
                if (
                    portal_account_created_template
                    and portal_account_created_template._name != "mail.template"
                ):
                    _logger.error(
                        "Wrong set password template %r",
                        portal_account_created_template,
        ...
        for user in self:
            ...
            with contextlib.closing(self.env.cr.savepoint()):
                ...
                account_created_template = (
                    internal_account_created_template
                    if is_internal
                    else portal_account_created_template
                )
                        ...
                        .create(
                            {
                                ...
                                "email_from": user.company_id.email_formatted
    ...
    def send_unregistered_user_reminder(self, *, after_days=5, batch_size=100):
        ...
        for user, invited_users in invited_by_users.items():
            ...
            template = email_template.with_context(
                dbname=self.env.cr.dbname, invited_users=invited_user_emails
            )
            template.send_mail(
                user.id,
            ...
            if not self.env["ir.cron"]._commit_progress(len(invited_users)):
                _logger.info(
                    "send_unregistered_user_reminder: timeout reached, stopping"
                )
                break
