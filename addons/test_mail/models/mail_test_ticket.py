...
class MailTestTicketMc(models.Model):
    ...
    def _notify_get_reply_to_addresses(self):
        ...
        res = {
            ticket.id: addresses[ticket.container_id.id]
            for ticket in self
            if ticket.container_id and ticket.container_id.id in addresses
        }
        leftover = self.filtered(lambda rec: not rec.container_id)
        if leftover:
            res.update(super()._notify_get_reply_to_addresses())
        return res
...
class MailTestContainer(models.Model):
    ...
    _inherit = ["mixin.mail.thread", "mixin.mail.alias"]
    _mail_partner_fields = ("customer_id",)
