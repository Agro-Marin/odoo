...
class MailPerformanceThread(models.Model):
    ...
    _description = "Performance: mixin.mail.thread"
...
class MailPerformanceThreadRecipients(models.Model):
    ...
    _description = "Performance: mixin.mail.thread, for recipients"
    _inherit = ["mixin.mail.thread"]
...
class MailTestTrackAll(models.Model):
    ...
    many2many_field = fields.Many2many(
        "mail.test.track.all.m2m", string="Many2Many", tracking=8
    )
    ...
    one2many_field = fields.One2many(
        "mail.test.track.all.o2m", "mail_track_all_id", string="One2Many", tracking=11
    )
    properties_parent_id = fields.Many2one(
        "mail.test.track.all.properties.parent", tracking=True
    )
    properties = fields.Properties(
        "Properties", definition="properties_parent_id.definition_properties"
    )
...
class MailTestTrackDurationMixin(models.Model):
    _description = "Fake model to test the mixin mixin.mail.tracking.duration"
    _name = "mail.test.track.duration.mixin"
    _track_duration_field = "customer_id"
    _inherit = ["mixin.mail.tracking.duration"]
    _mail_partner_fields = ("customer_id",)
...
class MailTestTrackMonetary(models.Model):
    ...
    revenue = fields.Monetary(
        "Revenue", currency_field="company_currency", tracking=True
    )
...
class MailTestTrackSelection(models.Model):
    ...
    selection_type = fields.Selection(
        [("first", "First"), ("second", "Second")], tracking=True
    )
...
class MailTestMultiCompanyRead(models.Model):
    ...
    _inherit = ["mail.test.multi.company", "mixin.mail.activity"]
