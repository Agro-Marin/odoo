...
class MailTestTLead(models.Model):
    ...
    def _mail_get_customer_information(self):
        email_normalized_to_values = super()._mail_get_customer_information()
    ...
    def _message_post_after_hook(self, message, msg_vals):
        if self.email_from and not self.partner_id:
            ...
            if new_partner:
                ...
                self.search([("partner_id", "=", False), email_domain]).write(
                    {"partner_id": new_partner[0].id}
                )
