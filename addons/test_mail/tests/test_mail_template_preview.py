...
class TestMailTemplateTools(TestMailTemplateCommon):
    ...
    def test_mail_template_preview_fields(self):
        ...
        preview = Form(
            self.env["mail.template.preview"].with_context(
                default_mail_template_id=test_template.id
            )
        )
        ...
        self.assertEqual(
            ...
            test_record_ref,
            "Should take first (only) record by default",
        )
    ...
    def test_mail_template_preview_dynamic_attachment(self):
        ...
        preview = (
            self.env["mail.template.preview"]
            .with_context(
                {
        ...
        self.assertEqual(
            preview.body_html, f"<p>EnglishBody for {test_record.name}</p>"
        )
        self.assertFalse(
            preview.attachment_ids, "Reports should not be listed in attachments"
        )
    ...
    def test_mail_template_preview_force_lang(self):
        ...
        self.assertEqual(
            preview.body_html, "<p>SpanishBody for %s</p>" % test_record.name
        )

        preview.write({"lang": "en_US"})
        self.assertEqual(
            preview.body_html, "<p>EnglishBody for %s</p>" % test_record.name
        )
