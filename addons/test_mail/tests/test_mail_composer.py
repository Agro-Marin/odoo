...
class TestMailComposer(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.test_report, cls.test_report_2, cls.test_report_3 = cls.env[
            "ir.actions.report"
        ].create(
            [
                {
...
class TestComposerForm(TestMailComposer):
    ...
    def test_composer_comment_without_res_ids(self):
        ...
        (browse([])._message_compute_subject() used to raise check_singleton())."""
    ...
    def test_mail_composer_comment(self):
        ...
        self.assertTrue(
            composer_form.force_send,
            "MailComposer: single record post send notifications right away",
        )
        ...
        self.assertFalse(
            composer_form.reply_to_force_new, "By default, replies land on same thread"
        )
        self.assertFalse(composer_form.scheduled_date)
        self.assertEqual(literal_eval(composer_form.res_ids), self.test_record.ids)
        self.assertEqual(
            composer_form.subject, self.test_record._message_compute_subject()
        )
        self.assertIn(
            f"Ticket for {self.test_record.name}",
            composer_form.subject,
    ...
    def test_mail_composer_comment_attachments(self):
        ...
        attachment_data = self._generate_attachments_data(
            2, self.template._name, self.template.id
        )
        template_1 = self.template.copy(
            {
                "attachment_ids": [(0, 0, a) for a in attachment_data],
                "report_template_ids": [
                    (6, 0, (self.test_report + self.test_report_2).ids)
                ],
            }
        )
        ...
        composer_form = Form(
            self.env["mail.compose.message"].with_context(
                self._get_web_context(
                    self.test_record, add_web=True, default_attachment_ids=[]
                )
            )
        )
        ...
        report_attachments = [
            att
            for att in composer_form.attachment_ids
            if att not in template_1_attachments
        ]
        self.assertEqual(len(report_attachments), 2)
        tpl_attachments = composer_form.attachment_ids[:] - self.env[
            "ir.attachment"
        ].concat(*report_attachments)
    ...
    def test_mail_composer_comment_batch(self):
        ...
        composer_form = Form(
            self.env["mail.compose.message"].with_context(
                self._get_web_context(
                    self.test_records, add_web=True, default_composition_mode="comment"
                )
            )
        )
        self.assertTrue(
        ...
        self.assertFalse(composer_form.email_layout_xmlid)
        self.assertFalse(
            composer_form.force_send, "MailComposer: batch mode uses queue"
        )
        self.assertFalse(composer_form.mail_server_id)
        self.assertEqual(composer_form.model, self.test_record._name)
        self.assertFalse(composer_form.notify_author)
        ...
        self.assertFalse(
            ...
            "MailComposer: batch mode does not precompute alias domain (MC, ...)",
        )
        self.assertFalse(
            composer_form.record_company_id,
            "MailComposer: batch mode does not precompute company environment (MC, ...)",
        )
        self.assertFalse(composer_form.reply_to)
        self.assertFalse(
            composer_form.reply_to_force_new, "By default, replies land on same thread"
    ...
    def test_mail_composer_comment_wtpl(self):
        composer_form = Form(
            self.env["mail.compose.message"].with_context(
                self._get_web_context(
                    self.test_record, add_web=True, default_template_id=self.template.id
                )
            )
        )
        ...
        self.assertTrue(
            composer_form.reply_to_force_new,
            "Reply-To on template forces its usage for new thread creation",
        )
        self.assertEqual(literal_eval(composer_form.res_ids), self.test_record.ids)
        self.assertEqual(
            composer_form.scheduled_date,
            FieldDatetime.to_string(self.reference_now + timedelta(days=2)),
        )
        self.assertEqual(
            composer_form.subject, f"TemplateSubject {self.test_record.name}"
        )
        self.assertEqual(composer_form.subtype_id, self.env.ref("mail.mt_comment"))
        self.assertFalse(composer_form.subtype_is_log)
    ...
    def test_mail_composer_comment_wtpl_batch(self):
        ...
        self.assertEqual(composer_form.composition_mode, "comment")
        self.assertEqual(
            composer_form.email_from,
            self.template.email_from,
            "MailComposer: comment in batch mode should have template raw email_from if template",
        )
        self.assertEqual(composer_form.email_layout_xmlid, "mail.test_layout")
        self.assertFalse(
            composer_form.force_send,
            "MailComposer: batch record post use email queue for notifications",
        )
        self.assertEqual(composer_form.mail_server_id, self.mail_server_domain)
        self.assertEqual(composer_form.model, self.test_record._name)
        self.assertFalse(composer_form.notify_author)
    ...
    def test_mail_composer_comment_wtpl_domain(self):
        ...
        self.assertEqual(
            literal_eval(composer_form.res_domain),
            [("id", "in", self.test_records.ids)],
        )
        self.assertEqual(composer_form.scheduled_date, self.template.scheduled_date)
        self.assertEqual(
            composer_form.subject,
    ...
    def test_mail_composer_comment_wtpl_norecords(self):
        ...
        self.assertEqual(composer_form.composition_mode, "comment")
        self.assertEqual(
            composer_form.email_from, self.env.user.partner_id.email_formatted
        )
        self.assertEqual(composer_form.email_layout_xmlid, "mail.test_layout")
    ...
    def test_mail_composer_mass(self):
        ...
        self.assertFalse(composer_form.email_layout_xmlid)
        self.assertTrue(
            composer_form.force_send, "MailComposer: mass mode sends emails right away"
        )
        self.assertFalse(composer_form.mail_server_id)
        self.assertFalse(composer_form.notify_author)
        self.assertFalse(composer_form.notify_author_mention)
        ...
        self.assertFalse(
            composer_form.record_alias_domain_id,
            "MailComposer: mass mode should have void alias domain",
        )
        self.assertFalse(
            composer_form.record_company_id,
            "MailComposer: mass mode should have void company",
        )
        self.assertFalse(composer_form.reply_to)
        self.assertFalse(composer_form.reply_to_force_new)
        self.assertEqual(
            sorted(literal_eval(composer_form.res_ids)), sorted(self.test_records.ids)
        )
        self.assertFalse(composer_form.scheduled_date)
        self.assertFalse(
            composer_form.subject,
        ...
        self.assertFalse(
            composer_form.subtype_id, "MailComposer: subtype is not used in mail mode"
        )
        self.assertFalse(
            composer_form.subtype_is_log,
            "MailComposer: subtype is log has no meaning in mail mode",
        )
    ...
    def test_mail_composer_mass_wtpl_domain(self):
        ...
        self.assertFalse(
            composer_form.force_send,
            "MailComposer: mass mode with domain uses email queue",
        )
        self.assertEqual(composer_form.mail_server_id, self.mail_server_domain)
        self.assertEqual(composer_form.model, self.test_records._name)
        self.assertFalse(composer_form.notify_author)
    ...
    def test_mail_composer_template_switching(self):
        ...
        for composition_mode, form in forms.items():
            with self.subTest(composition_mode=composition_mode):
                ...
                self.assertEqual(
                    form.email_from,
                    template_complete.email_from,
                    "email_from not set correctly to form this test",
                )
                self.assertEqual(
                    form.subject,
                    template_complete.subject,
                    "subject not set correctly to form this test",
                )
                ...
                self.assertEqual(
                    form.subject,
                    template_complete.subject,
                    "subject should be kept unchanged",
                )
...
class TestComposerInternals(TestMailComposer):
    ...
    def test_mail_composer_attachments(self):
        ...
        attachment_data = self._generate_attachments_data(
            3, self.template._name, self.template.id
        )
        self.template.write(
            {
                "attachment_ids": [(0, 0, a) for a in attachment_data],
        ...
        template_void = self.template.copy(
            default={
                "attachment_ids": False,
        ...
        attachs = (
            ...
            .sudo()
            .search([("name", "in", [a["name"] for a in attachment_data])])
        )
        ...
        for composition_mode, batch_mode in product(
            ("comment", "mass_mail"), (False, True, "domain")
        ):
            with self.subTest(composition_mode=composition_mode, batch_mode=batch_mode):
                ...
                if composition_mode == "comment" and not batch:
                    self.assertEqual(
                        composer.attachment_ids, attachs + extra_attach + generated
                    )
                else:
    ...
    def test_mail_composer_configuration(self):
        ...
        template_falsy = self.template.copy(
            default={
                "auto_delete": False,
            }
        )
        ...
        for composition_mode, batch_mode in product(
            ...
            with self.subTest(composition_mode=composition_mode, batch_mode=batch_mode):
                ...
                if composition_mode == "comment":
                    self.assertTrue(
                        composer.auto_delete, "By default, remove notification emails"
                    )
                    self.assertFalse(
                        composer.auto_delete_keep_log, "Not used in comment mode"
                    )
                    self.assertTrue(
                        composer.email_add_signature, "Default value in comment mode"
                    )
                    self.assertEqual(
                        composer.subtype_id, self.env.ref("mail.mt_comment")
                    )
                else:
                    self.assertFalse(
                        composer.auto_delete, "By default, keep mailing emails"
                    )
                    self.assertFalse(
                        composer.auto_delete_keep_log,
                        "Emails are not unlinked, logs are already kept",
                    )
                    self.assertFalse(
                        composer.email_add_signature,
                        "Not supported in mass mailing mode",
                    )
                ...
                if composition_mode == "comment":
                    ...
                    self.assertFalse(
                        composer.email_add_signature,
                        "Template is considered as complete",
                    )
                    self.assertEqual(composer.email_layout_xmlid, "mail.test_layout")
                    self.assertEqual(composer.message_type, "comment")
                    self.assertEqual(
    ...
    def test_mail_composer_content(self):
        ...
        for composition_mode, batch_mode in product(
            ...
            with self.subTest(composition_mode=composition_mode, batch_mode=batch_mode):
                ...
                self.assertEqual(
                    composer.body, '<p>Test Body <t t-out="record.name"/></p>'
                )
                self.assertEqual(composer.mail_server_id, self.mail_server_default)
                self.assertEqual(
                    composer.scheduled_date,
                    "{{ datetime.datetime(2023, 1, 10, 10, 0, 0) }}",
                )
                self.assertEqual(
                    composer.subject, "My amazing subject for {{ record.name }}"
                )
                ...
                if composition_mode == "comment" and not batch:
                    self.assertEqual(
                        composer.body, f"<p>TemplateBody {self.test_record.name}</p>"
                    )
                    self.assertEqual(
                        composer.mail_server_id, self.template.mail_server_id
                    )
                    self.assertEqual(
                        FieldDatetime.from_string(composer.scheduled_date),
                        self.reference_now + timedelta(days=2),
                    )
                    self.assertEqual(
                        composer.subject, f"TemplateSubject {self.test_record.name}"
                    )
                else:
                    ...
                    self.assertEqual(
                        composer.scheduled_date, self.template.scheduled_date
                    )
                ...
                self.assertEqual(
                    composer.body,
                    '<p>Back to my amazing body <t t-out="record.name"/></p>',
                )
                self.assertEqual(composer.mail_server_id, self.mail_server_default)
                self.assertEqual(
                    composer.scheduled_date,
                ...
                self.assertEqual(
                    composer.subject, "Back to my amazing subject for {{ record.name }}"
                )
                ...
                if composition_mode == "comment" and not batch:
                    ...
                    self.assertEqual(
                        composer.subject,
                        "Back to my amazing subject for {{ record.name }}",
                    )
                else:
                    self.assertEqual(
                        composer.body,
                ...
                if composition_mode == "comment" and not batch:
                    ...
                    self.assertEqual(
                        composer.subject, self.test_record._message_compute_subject()
                    )
                    self.assertIn(
                        f"Ticket for {self.test_record.name}",
                        composer.subject,
    ...
    def test_mail_composer_recipients(self):
        ...
        for composition_mode, batch_mode in product(
            ...
            with self.subTest(composition_mode=composition_mode, batch_mode=batch_mode):
                ...
                self.assertEqual(
                    self.test_record.message_partner_ids, self.partner_employee_2
                )
                ...
                self.assertTrue(
                    composer.reply_to_force_new,
                    "Forced reply_to -> consider it is not going to land in same thread",
                )
                ...
                if composition_mode == "comment" and not batch:
                    ...
                    self.assertEqual(
                        composer.partner_ids,
                        base_recipients,
                        "Mass mode: kept original values",
                    )
                    self.assertEqual(
                        composer.reply_to,
                        self.template.reply_to,
                        "Mass mode: raw template value",
                    )
                    self.assertTrue(
                        composer.reply_to_force_new,
                        "Mass mode: reply_to -> consider this is for new threads",
                    )
                ...
                if composition_mode == "comment" and not batch:
                    self.assertEqual(
                        composer.partner_ids, self.partner_1 + new_partners
                    )
                else:
                    self.assertFalse(composer.partner_ids)
                if composition_mode == "comment" and not batch:
                ...
                self.assertTrue(
                    composer.reply_to_force_new
                )  # note: this should be updated with reply-to
                self.assertEqual(
                    composer.reply_to_mode, "new"
                )  # note: this should be updated with reply-to
    ...
    def test_mail_composer_parent(self):
        ...
        composer = (
            ...
            .with_context(
                self._get_web_context(
                    self.test_record, add_web=False, default_parent_id=parent.id
                )
            )
            .create(
                {
    ...
    def test_mail_composer_rights_attachments(self):
        ...
        template_1.write(
            {
                "attachment_ids": [
                    (0, 0, dict(a, res_model="mail.template", res_id=template_1.id))
        ...
        composer = (
            self.env["mail.compose.message"]
            .with_context(self._get_web_context(self.test_record))
            .create(
                {
    ...
    def test_mail_composer_save_template(self):
        ...
        template = self.env["mail.template"].search(
            [("model", "=", self.test_record._name), ("name", "=", "My Template")],
            limit=1,
        )
    ...
    def test_mail_composer_schedule_message(self):
            ...
            .create(
                {
                    ...
                    "scheduled_date": FieldDatetime.to_string(
        ...
        attachment_data = self._generate_attachments_data(1, "mail.compose.message", 0)[
            0
        ]
        composer = (
            self.env["mail.compose.message"]
        ...
        self.assertEqual(
            scheduled_message.scheduled_date, self.reference_now + timedelta(hours=2)
        )
    ...
    def test_mail_composer_wtpl_populate_new_recipients_mc(self):
        ...
        for composition_mode, batch_mode in product(
            ...
            with self.subTest(composition_mode=composition_mode, batch_mode=batch_mode):
                ...
                new_partners = self.env["res.partner"].search(
                    [("email_normalized", "in", manual_recipients)], order="email"
                )
                try:
                    self.assertEqual(len(new_partners), len(test_records))
                    self.assertEqual(
    ...
    def test_mail_composer_wtpl_schedule_message(self):
        ...
        for additional_params, exp_notif_params in zip(
            ...
            with self.subTest(params=additional_params):
                ...
                self.assertEqual(
                    scheduled_message.body, "<p>TemplateBody TestRecord</p>"
                )
                self.assertEqual(
                    scheduled_message.subject, "TemplateSubject TestRecord"
                )
                self.assertEqual(
                    scheduled_message.scheduled_date,
                    self.test_record.create_date + timedelta(days=2),
                )
                self.assertEqual(scheduled_message.model, self.test_record._name)
                self.assertEqual(scheduled_message.res_id, self.test_record.id)
                self.assertEqual(
                    scheduled_message.author_id, self.test_record.user_id.partner_id
                )
                self.assertEqual(
                    scheduled_message.partner_ids, self.test_record.customer_id
                )
                self.assertFalse(scheduled_message.is_note)
                notification_parameters = json.loads(
                    scheduled_message.notification_parameters
                )
                self.assertEqual(
                    notification_parameters["email_from"],
                    self.test_record.user_id.email_formatted,
                )
                self.assertEqual(
                    notification_parameters["force_email_lang"],
                    self.test_record.customer_id.lang,
                )
                self.assertEqual(
                    notification_parameters["mail_server_id"],
                    self.mail_server_domain.id,
                )
...
class TestComposerResultsComment(TestMailComposer, CronMixinCase):
    ...
    def test_mail_composer_default_subject(self):
        ...
        nonthread_record = self.env["mail.test.nothread"].create(
            {"name": "TestNoThread"}
        )

        # default behavior: use the record name
        ctx = self._get_web_context(
            simple_record, add_web=False, default_composition_mode="comment"
        )
        _, message = (
            self.env["mail.compose.message"]
            .with_context(ctx)
        ...
        ctx = self._get_web_context(
            nonthread_record, add_web=False, default_composition_mode="comment"
        )
        _, message = (
            self.env["mail.compose.message"]
            .with_context(ctx)
        ...
        # custom subject
        ctx = self._get_web_context(
            ticket_record, add_web=False, default_composition_mode="comment"
        )
        _, message = (
            self.env["mail.compose.message"]
            .with_context(ctx)
    ...
    def test_mail_composer_notifications_delete(self):
        ...
        self.assertTrue(
            composer.auto_delete, "Comment mode removes notification emails by default"
        )
        ...
        self.assertEqual(
            ...
            self.partner_employee_2 + self.partner_1 + self.partner_2,
        )

        # global outgoing
        self.assertEqual(
            len(self._mails), 3, "Should have sent an email each recipient"
        )
        self.assertEqual(
            len(self._new_mails),
            2,
            "Should have created 2 mail.mail (1 for users, 1 for customers)",
        )
        self.assertFalse(
            self._new_mails.exists(), "Should have deleted mail.mail records"
        )
        ...
        self.assertEqual(
            ...
            2,
            "Should not have deleted mail.mail records",
        )
    ...
    def test_mail_composer_post_parameters(self):
        ...
        self.assertEqual(
            ...
            "comment",
            "Mail: default message type with composer is user comment",
        )
        self.assertEqual(
            message.subtype_id,
            self.env.ref("mail.mt_comment", "Mail: default subtype is comment"),
        )
        ...
        self.assertTrue(
            self.user_employee.partner_id in self.test_record.message_partner_ids
        )
        for notify_author, notify_author_mention, add_pid, should_mention in [
            ...
            with self.subTest(
                ...
                if should_mention:
                    self.assertTrue(
                        self.user_employee.partner_id in message.notified_partner_ids
                    )
                else:
                    self.assertFalse(
                        self.user_employee.partner_id in message.notified_partner_ids
        ...
        self.assertEqual(
            self.test_record.message_partner_ids,
            self.partner_employee + self.partner_employee_2,
        )
        for notify_skip_followers in (False, True):
            with self.subTest(notify_skip_followers=notify_skip_followers):
                ...
                if notify_skip_followers:
                    ...
                    self.assertEqual(
                        message.notified_partner_ids,
                        self.partner_employee_2 + self.partner_admin,
    ...
    def test_mail_composer_recipients_email_only(self):
        ...
        email_addrs = [
            "test1@test.lan",
            "test2@test.lan",
            '"Formatted" <FORMATTED.EMAIL@TEST.LAN>',
            'test.multi.1@test.lan, test.multi.2@test.lan, "Formatted" <FORMATTED.EMAIL@TEST.LAN>, InvalidEmailMulti',
            "InvalidEmail",
            "",
        ]
        notified_emails = {
            "test1@test.lan",
            "test2@test.lan",
            "test.multi.1@test.lan",
            "test.multi.2@test.lan",
            '"Formatted" <formatted.email@test.lan>',
            "InvalidEmail",
            False,
        }
        mail_to_vals = {
            "test1@test.lan",
            "test2@test.lan",
            'test.multi.1@test.lan,test.multi.2@test.lan,"Formatted" <formatted.email@test.lan>',
            '"Formatted" <formatted.email@test.lan>',
            "InvalidEmail",
            "",
        }
        records = self.env["mail.test.ticket"].create(
            [{"name": f"Test {email}", "email_from": email} for email in email_addrs]
        )
        composer = (
            self.env["mail.compose.message"]
            .with_context(self._get_web_context(records))
            .create({})
        )
        ...
        self.assertEqual(
            len(self._new_mails), 6, "Should have created 1 mail.mail per record"
        )
        self.assertEqual(
            len(self._mails),
            4,
            "Should have sent 1 email per record with a valid email",
        )
        self.assertEqual(
            self._new_mails.notification_ids.mapped("notification_status"),
            ["sent", "sent", "sent", "sent", "sent", "sent", "exception", "exception"],
        )
        self.assertEqual(
            set(self._new_mails.notification_ids.mapped("mail_email_address")),
            set(notified_emails),
            "Email address should be retained on the notification",
        )
        self.assertEqual(
            set(self._new_mails.mapped("email_to")),
            mail_to_vals,
            "Should have sent emails to the default recipients",
        )
    ...
    def test_mail_composer_recipients_status(self):
        ...
        external_partner = (
            ...
            .sudo()
            .create(
                {"name": "Customer", "partner_share": True, "email": "custo@kuzco.com"}
            )
        )
        ctx = {
        ...
        for is_customer_subscribed in [True, False]:
            ...
            with self.subTest(
                is_customer_subscribed=is_customer_subscribed,
                partner_id=external_partner,
            ):
                if is_customer_subscribed:
                    test_record.message_subscribe(
                        partner_ids=external_partner.ids
                    )  # add external follower
                    self.assertTrue(
                        composer.notified_bcc_contains_share,
                        "The created partner is external so should be True",
                    )
                else:
                    test_record.message_unsubscribe(
                        external_partner.ids
                    )  # remove external follower
                    self.assertFalse(
                        composer.notified_bcc_contains_share,
                        "External follower should be unsubscribed",
                    )

    def test_notified_bcc_contains_share_sees_partners_the_user_cannot_read(self):
        """The flag must answer from the recipient data, not from an ACL-filtered
        search: a follower in another company is invisible to `search` yet is
        still going to be mailed."""
        company_b = self.env["res.company"].sudo().create({"name": "BCC Company B"})
        share_partner = (
            self.env["res.partner"]
            .sudo()
            .create(
                {
                    ...
                    "email": "share.other.company@test.example.com",
                    "name": "Share In Other Company",
                }
            )
        )
        self.assertTrue(share_partner.partner_share)
        single_company_user = mail_new_test_user(
            self.env,
            company_id=self.company_admin.id,
            company_ids=[(6, 0, self.company_admin.ids)],
            groups="base.group_user",
            login="bcc_single_company",
            name="Single Company User",
        )
        test_record = self.env["mail.test.ticket"].sudo().create({"name": "BCC doc"})
        test_record.message_subscribe(partner_ids=share_partner.ids)
        self.assertFalse(
            self.env["res.partner"]
            .with_user(single_company_user)
            .search([("id", "=", share_partner.id)]),
            "the follower must be unreadable for this user, else the test proves nothing",
        )
        composer = (
            self.env["mail.compose.message"]
            .with_user(single_company_user)
            ...
            .create({"body": "<p>Test Body</p>"})
        )
        self.assertTrue(
            composer.notified_bcc_contains_share,
            "a share follower must be reported even when the composing user "
            "cannot read the partner record",
        )
    ...
    def test_mail_composer_server_config(self):
        ...
        for (emails_from, servers_active, mail_config), (
            exp_smtp_from_lst,
            exp_msg_from_lst,
            exp_from_filter,
        ) in zip(
            [
                (
                    ...
                    self.env["ir.mail_server"],
                    {
                        "default_from": f"notifications@{self.alias_domain}",
            ...
            with self.subTest(emails_from=emails_from, servers_active=servers_active):
                ...
                self.env["ir.config_parameter"].sudo().set_param(
                    "mail.default.from_filter", from_filter
                )
    ...
    def test_mail_composer_wtpl_complete(self):
        ...
        email_to_3 = (
            "test.to.1@test.example.com"  # duplicate: should not sent twice the email
        )
        ...
        self.template.write(
            {
                ...
                "email_cc": email_cc_1,
                "partner_to": '%s, {{ object.customer_id.id if object.customer_id else "" }}'
        ...
        for (
            ...
            with self.subTest(
                ...
                self.assertEqual(
                    test_records.message_partner_ids, self.partner_employee_2
                )
                ...
                if batch:
                    ...
                    self.assertEqual(
                        composer.email_from, self.partner_employee_2.email_formatted
                    )
                if reply_to:
                    if batch:
                        self.assertEqual(
                            composer.reply_to,
                            '{{ ctx.get("custom_reply_to") or "info@test.example.com" }}',
                        )
                    else:
                        self.assertEqual(
                            composer.reply_to, "custom.reply.to@test.example.com"
                        )
                    self.assertTrue(
                        composer.reply_to_force_new,
                        "Template forces reply_to -> consider it is a new thread",
                    )
                else:
                    self.assertFalse(composer.reply_to)
                    self.assertFalse(composer.reply_to_force_new)
                ...
                with (
                    ...
                    # monorecord: force_send notifications
                        ...
                        self.assertEqual(
                            self._new_mails.mapped("state"),
                            ["sent"] * len(self._new_mails),
                        )
                        self.assertEqual(
                            len(self._mails),
                            5,
                ...
                if scheduled_date:
                    self.assertEqual(
                        ...
                        [self.reference_now + timedelta(days=2)] * len(test_records),
                ...
                self.assertEqual(
                    ...
                    {
                        "test.to.1@test.example.com",
                        "test.to.2@test.example.com",
                        "test.cc.1@test.example.com",
                    },
                )
                self.assertEqual(
                ...
                if scheduled_date:
                    ...
                    with (
                        ...
                        self.mock_datetime_and_now(
                            self.reference_now + timedelta(days=3)
                        ),
                    ):
                        self.env[
                            "mail.message.schedule"
                        ].sudo()._send_notifications_cron()
                ...
                for test_record, exp_lang in zip(test_records, langs, strict=True):
                    ...
                    if reply_to:
                        ...
                        exp_reply_to = formataddr(
                            (author.name, f"{self.alias_catchall}@{self.alias_domain}")
                        )
                    self.assertMailMail(
                    ...
                    email = self._find_sent_email(
                        test_record.user_id.email_formatted,
                    ...
                    self.assertEqual(
                        message.subtype_id, self.env.ref("mail.mt_comment")
                    )
                    self.assertNotified(
                        message,
                        [
                    ...
                    self.assertTrue(
                        all(attach not in message.attachment_ids for attach in attachs),
        ...
        for batch, companies, expected_companies, expected_alias_domains in [
            ...
            with self.subTest(batch=batch, companies=companies):
                ...
                new_partner = self.env["res.partner"].search(
                    [("email_normalized", "=", "test.to.1@test.example.com")]
                )
    ...
    def test_mail_composer_wtpl_recipients_email_fields(self):
        ...
        partner_format_tofind, partner_multi_tofind, partner_at_tofind = self.env[
            "res.partner"
        ].create(
            [
                {
        ...
        email_ccs = [
            '"Raoul" <test.cc.1@example.com>',
            '"Raoulette" <test.cc.2@example.com>',
            "test.cc.2.2@example.com>",
            "invalid",
            "  ",
        ]
        email_tos = [
            '"Micheline, l\'Immense" <test.to.1@example.com>',
        ...
        self.template.write(
            {
                ...
                "email_to": ", ".join(
                    email_tos
                    + (
                        partner_format_tofind + partner_multi_tofind + partner_at_tofind
                    ).mapped("email")
                ),
        ...
        self.user_employee.write(
            {
                "email": "email.from.1@test.mycompany.com, email.from.2@test.mycompany.com"
            }
        )
        self.partner_1.write(
            {"email": '"Valid Formatted" <valid.lelitre@agrolait.com>'}
        )
        self.partner_2.write(
            {"email": "valid.other.1@agrolait.com, valid.other.cc@agrolait.com"}
        )
        ...
        new_partners = (
            self.env["res.partner"]
            .search([])
            .search([("id", "not in", existing_partners.ids)])
        )
        self.assertEqual(
            len(new_partners),
            9,
        ...
        self.assertEqual(
            new_partners[0:3].ids,
            (partner_format_tofind + partner_multi_tofind + partner_at_tofind).ids,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    '"NotBike@Home" <find.me.at@test.example.com>',
                    '"FindMe Format" <find.me.format@test.example.com>',
        ...
        self.assertMailMail(
            ...
            email_values={
                ...
                "email_from": formataddr(
                    (self.user_employee.name, "email.from.1@test.mycompany.com")
                ),
        ...
        for recipient in recipients:
            ...
            if recipient == self.partner_2:
                smtp_to_list = [
                    "valid.other.1@agrolait.com",
                    "valid.other.cc@agrolait.com",
                ]
            # find.me.format
...
class TestComposerResultsCommentStatus(TestMailComposer):
    ...
    def setUpClass(cls):
        ...
        cls.env["mail.blacklist"]._add(cls.test_partners[0].email_formatted)
    ...
    def test_assert_initial_data(self):
        ...
        self.assertEqual(self.test_records.message_partner_ids, self.partner_employee_2)
        self.assertEqual(
            self.test_records[1].email_from, self.test_records[4].email_from
        )
        self.assertEqual(
            self.test_records[5].email_from, self.test_records[6].email_from
        )
    ...
    def test_comment_blacklist(self):
        ...
        composer_form = Form(
            self.env["mail.compose.message"].with_context(
                self._get_web_context(
                    test_record, add_web=True, default_template_id=self.template.id
                )
            )
        )
        composer = composer_form.save()
        ...
        self.assertEqual(
            len(self._mails),
            2,
            "Should have sent 2 emails, skipping the exclusion list",
        )
...
class TestComposerResultsMass(TestMailComposer):
    ...
    def test_mail_composer_delete(self):
        ...
        self.assertEqual(
            len(self._new_mails), 2, "Should have created 1 mail.mail per record"
        )
        self.assertFalse(
            self._new_mails.exists(), "Should have deleted mail.mail records"
        )
        self.assertEqual(
            len(self._new_msgs), 2, "Should have created 1 mail.mail per record"
        )
        self.assertEqual(
            self._new_msgs.exists(),
            self._new_msgs,
            "Should not have deleted mail.message records",
        )
        self.assertEqual(
            len(self._new_notifs),
            2,
            "Should have created 1 mail.notification per record",
        )
        self.assertEqual(
            self._new_notifs.exists(),
            self._new_notifs,
            "Should not have deleted mail.notification",
        )
        ...
        self.assertEqual(
            len(self._new_notifs),
            2,
            "Should have created 1 mail.notification per recipient per record",
        )
        self.assertEqual(
            self._new_notifs.exists(),
            self._new_notifs,
        ...
        self.assertEqual(
            len(self._new_msgs), 2, "Should have created 1 mail.message per record"
        )
        self.assertFalse(
            self._new_msgs.exists(), "Should have deleted mail.message records"
        )
        self.assertFalse(
            self._new_notifs,
            "Should not create notifications when removing logs in mass mode",
        )
    ...
    def test_mail_composer_duplicates(self):
        ...
        customer_ids.write({"email": "test@test.lan"})
        ...
        diff_combinations = product(
            [
                [diff_body],
                [diff_subject],
                [diff_attachment_same_content],
                [diff_attachment_diff_content],
        ...
        for template_changes, customer_changes in diff_combinations:
            ...
            with self.subTest(
                template_values=test_template_values, customer_changes=customer_changes
            ):
                ...
                for record in self.test_records[:2]:
                    ...
                    if (
                        test_template_values["subject"]
                        != base_template_values["subject"]
                    ):
                        expected_subject = record.name
                    expected_body = base_template_values["body_html"]
                    if (
                        test_template_values["body_html"]
                        != base_template_values["body_html"]
                    ):
                    ...
                    if self.template.attachment_ids:
                        expected_attachment_info.append(
                            {
                                "name": "mail.template_Common_Attachment.txt",
                                "type": "text/plain",
                            }
                        )
                    if self.template.report_template_ids:
                        test_report_name = (
                            "TestReport2"
                            if self.template.report_template_ids == self.test_report_2
                            else "TestReport3"
                        )
                        expected_attachment_info.append(
                            {
                                "name": f"{test_report_name} for {record.name}.html",
                                "type": "text/plain",
                            },
                        )
    ...
    def test_mail_composer_wtpl(self):
        ...
        composer = composer_form.save()
        composer.attachment_ids = (
            self.env["ir.attachment"]
            .sudo()
            .create(
                self._generate_attachments_data(
                    1, res_model=composer._name, res_id=composer.id
                )
            )
        )
        self.assertTrue(
            composer.reply_to_force_new, "Should use template reply-to value"
        )
        with self.mock_mail_gateway(mail_unlink_sent=True):
            composer._action_send_mail()
        ...
        for record in self.test_records:
            ...
            self.assertEqual(
                composer.attachment_ids.datas, message.attachment_ids.datas
            )
            ...
            with self.subTest(
                ...
                with (
                    ...
                    new_partners = self.env["res.partner"].search(
                        [
                            (
                                "email",
                                "in",
                                [email_to_1, email_to_2, email_to_3, email_cc_1],
                            )
                        ]
                    )
                    self.assertEqual(len(new_partners), 3)
                    self.assertEqual(
                        new_partners.mapped("lang"),
                        ["en_US"] * 3,
                    ...
                    self.assertEqual(
                        len(self._new_mails),
                        2,
                        "Should have created 1 mail.mail per record",
                    )
                    if not scheduled_date:
                        # emails sent directly
                        self.assertEqual(
                            len(self._mails), 10, "Should have sent emails"
                        )
                        self.assertEqual(
                            self._new_mails.mapped("scheduled_date"), [False] * 2
                        )
                    else:
                        ...
                        self.assertEqual(
                            len(self._mails),
                            0,
                            "Should not send emails, scheduled in the future",
                        )
                        self.assertEqual(
                            self._new_mails.mapped("scheduled_date"),
                        ...
                        self.assertEqual(
                            len(self._mails), 10, "Should have sent 5 emails per record"
                        )
                ...
                for record, exp_lang in zip(self.test_records, langs, strict=True):
                    ...
                    # template is sent only to partners (email_to are transformed)
                    self.assertMailMail(
                        record.customer_id + new_partners + self.partner_admin,
                        "sent",
                        mail_message=message,
                        author=author,
                        email_values={
                            "attachments_info": [
                                ...
                                {
                                    "name": "TestReport for %s.html" % record.name,
                                    "type": "text/plain",
                                },
                            ],
                            "body_content": exp_body,
                        ...
                        [
                            formataddr(
                                (
                                    ...
                                    email_normalize(
                                        record.customer_id.email, strict=False
                                    ),
                                )
                            )
                        ],
                    ...
                    if record == self.test_records[0]:
                        self.assertEqual(
                            sent_mail["email_to"],
                            ['"Partner_0" <test_partner_0@example.com>'],
                            "Should take email normalized in to",
                        )
                    else:
                        self.assertEqual(
                            sent_mail["email_to"],
                            ['"Partner_1" <test_partner_1@example.com>'],
                            "Should take email normalized in to",
                        )

                    if not email_layout_xmlid:
                        self.assertEqual(sent_mail["body"], f"<p>{exp_body}</p>")
                    else:
                        exp_layout_content_en = "English Layout for Ticket-like model"
                        exp_layout_content_es = (
                            "Spanish Layout para Spanish Model Description"
    ...
    def test_mail_composer_wtpl_recipients(self):
        ...
        composer = composer_form.save()
        self.assertEqual(
            sorted(literal_eval(composer.res_ids)), sorted(self.test_records.ids)
        )
        ...
        self.assertEqual(
            self.build_email_mocked.call_count, 2, "One build email per outgoing email"
        )
        self.assertEqual(
            self.mail_mail_create_mocked.call_count, 1, "Emails are created in batch"
        )
        # global outgoing
        self.assertEqual(
            len(self._new_mails),
            2,
            "Should have created 1 mail.mail per record based on active_ids",
        )
        self.assertEqual(
            len(self._mails),
            2,
            "Should have sent 1 email per record based on active_ids",
        )

        for record in self.test_records:
            # template is sent directly using customer field, even if author is partner_employee
            self.assertSentEmail(
                self.partner_employee_2.email_formatted, record.customer_id
            )
        ...
        # global outgoing
        self.assertEqual(
            len(self._new_mails), 1, "Should have taken default_res_ids (1 record)"
        )
        self.assertEqual(
            len(self._mails), 1, "Should have taken default_res_ids (1 record)"
        )

        # template is sent directly using customer field, even if author is partner_employee
        self.assertSentEmail(
            self.partner_employee_2.email_formatted, self.test_record.customer_id
        )
        ...
        # global outgoing
        self.assertEqual(
            len(self._new_mails), 1, "Should have created 1 mail.mail per record"
        )
        ...
        self.assertEqual(
            len(self._mails),
            2,
            "Should have sent 1 email per record based on  on active_ids",
        )
        ...
        # 7: invalid email in mass mode is exception, unless deleting messages
        # If keeping logs, then it is useful to let users know that the logged message was not actually sent
        # If not keeping logs, then the message would not appear anyway and we can cancel it to avoid creating it
        invalid_partners = self.env["res.partner"].create(
            [
                {"name": "Invalid", "email": "InvalidEmail"},
                {"name": "NoEmail", "email": ""},
            ]
        )
        invalid_records = self.env[self.test_record._name].create(
            [
                {"name": f"Test {customer.email}", "customer_id": customer.id}
                for customer in invalid_partners
            ]
        )
        composer = (
            self.env["mail.compose.message"]
            .with_context(self._get_web_context(invalid_records))
            .create({})
        )
        with self.mock_mail_gateway(mail_unlink_sent=False):
            composer._action_send_mail()
        ...
        self.assertEqual(
            self._new_mails.notification_ids.mapped("failure_type"),
            ["mail_email_invalid", "mail_email_missing"],
        ...
        with self.mock_mail_gateway(mail_unlink_sent=False):
            composer._action_send_mail()
        self.assertEqual(len(self._new_mails), 2)
        self.assertEqual(self._new_mails.mapped("state"), ["cancel", "cancel"])
        self.assertFalse(self._new_mails.notification_ids)
        ...
        for record in self.test_records:
            self.assertMailMail(
                ...
                email_values={
                    ...
                    "email_from": self.partner_employee_2.email_formatted,
                    "reply_to": formataddr(
                        (f"{record.name}", "dynamic.reply.to@test.mycompany.com")
                    ),
...
class TestComposerResultsMassStatus(TestMailComposer):
    ...
    def test_mailing_blacklist_mixin(self):
        ...
        composer_form = Form(
            self.env["mail.compose.message"].with_context(
                self._get_web_context(
                    test_records, add_web=True, default_template_id=self.template.id
                )
            )
        )
        composer = composer_form.save()
        ...
        self.assertEqual(
            len(self._mails),
            1,
            "Should have sent 1 email, and skipped an excluded email.",
        )
        ...
        self.assertEqual(
            len(self._mails), 2, "Should have sent 2 emails, even to excluded email."
        )
