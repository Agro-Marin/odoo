...
class MessageAccessCommon(MailCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        (
            cls.record_public,
            cls.record_portal,
            cls.record_portal_ro,
            cls.record_followers,
            cls.record_internal,
            cls.record_internal_ro,
            cls.record_admin,
...
class TestMailMessageAccess(MessageAccessCommon):
    ...
    def test_assert_initial_values(self):
        ...
        for record in (
            self.record_public
            + self.record_portal
            + self.record_portal_ro
            + self.record_followers
            + self.record_internal
            + self.record_internal_ro
            + self.record_admin
        ):
    ...
    def test_access_create(self):
        ...
        for record, msg_vals, should_crash, reason in [
            ...
            (
                ...
                False,
                "W Access on record, notification does not change anything",
            ),
            ...
            (
                ...
                {},
                True,
                "No access on record (and not notified on first message)",
            ),
            (
                record_admin_fol,
                {
                ...
                False,
                "Followers > no access on record",
            ),
            # parent based
            (
                self.record_admin,
                {  # note: force reply_to normally computed by message_post avoiding ACLs issues
                    "parent_id": admin_msg.id,
                },
                False,
                "No access on record but reply to notified parent",
            ),
        ]:
    ...
    def test_access_create_customized(self):
        ...
        )  # avoid acl conflict with those follower-based
        ...
        for post_value, should_crash in [
            ...
            with self.subTest(post_value=post_value):
                with patch.object(MailTestAccess, "_mail_post_access", post_value):
                    if should_crash:
                        with self.assertRaises(AccessError):
                            self.env["mail.message"].with_user(
                                self.user_employee
                            ).create(
                                {
    ...
    def test_access_create_portal(self):
        ...
        for record, msg_vals, should_crash, reason in [
            ...
            (
                ...
                {"is_internal": True},
                True,
                "Internal subtype always forbidden",
            ),
            (
                self.record_portal,
                {"is_internal": True, "message_type": "notification"},
                False,
                "Automatic log accepted",
            ),
            (
                self.record_portal,
            ...
            (
                ...
                True,
                "Internal flag (custom subtype) always forbidden",
            ),
            (
                self.record_portal,
                {
            ...
            (
                ...
                {"subtype_id": False},
                True,
                "No subtype = internal = always forbidden",
            ),
        ]:
            with self.subTest(record=record, msg_vals=msg_vals, reason=reason):
                if should_crash:
    ...
    def test_access_create_wo_parent_access(self):
        ...
        self.assertEqual(
            new_mail.references,
            f"{message.message_id} {new_msg.message_id}",
    ...
    def test_access_read(self):
        ...
        for msg, msg_vals, should_crash, reason in [
            ...
            (
                ...
                False,
                "Recipients > no access on record",
            ),
        ]:
            original_vals = {
    ...
    def test_access_read_customized(self):
        ...
        with self.assertRaises(
            AccessError
        ):  # write access not granted on locked -> cannot read message
            messages_all[2].with_user(self.user_employee).read(["subject"])
        with self.assertRaises(
            AccessError
        ):  # also working in case of batch ok / not ok
            messages_all.with_user(self.user_employee).read(["subject"])
        messages_all[0].with_user(self.user_employee).read(["subject"])
        messages_all[1].with_user(self.user_employee).read(
            ["subject"]
        )  # can read message of readonly
    ...
    def test_access_read_portal(self):
        ...
        for msg, msg_vals, should_crash, reason in [
            ...
            (
                ...
                True,
                "Note (comment) cannot be read by portal users",
            ),
            (
                self.record_portal.message_ids[0],
                {
                ...
                True,
                "Internal subtype (comment) cannot be read by portal users",
            ),
            (
                self.record_portal.message_ids[0],
                {
                ...
                False,
                "Note (email_outgoing) can be read by portal users",
            ),
            (
                self.record_portal.message_ids[0],
                {
                ...
                True,
                "Pure log (no subtype, even comment) cannot be read by portal users",
            ),
            (
                self.record_portal.message_ids[0],
                {
                ...
                True,
                "Internal message (comment) cannot be read by portal users",
            ),
            (
                self.record_portal.message_ids[0],
                {
                ...
                False,
                "Internal message (notification) can be read by portal users",
            ),
            # forbidden: other
            (
                self.record_portal.message_ids[0],
                ...
                True,
                "User notifications for other people can never be read by portal users",
            ),
        ]:
            original_vals = {
                "author_id": msg.author_id.id,
    ...
    def test_access_read_public(self):
        ...
        for msg, msg_vals, should_crash, reason in [
            ...
            (
                ...
                True,
                "Note cannot be read by public users",
            ),
            (
                self.record_public.message_ids[0],
                {
                ...
                True,
                "Internal message cannot be read by public users",
            ),
        ]:
            original_vals = {
                "author_id": msg.author_id.id,
    ...
    def test_access_unlink_portal(self):
        ...
        for msg, msg_vals, should_crash, reason in [
            ...
            (
                ...
                {},
                False,
                "W Access on record but unlink limited",
            ),
    ...
    def test_access_write(self):
        ...
        for msg, msg_vals, should_crash, reason in [
            ...
            (
                ...
                True,
                "Notified is read-only, not write",
            ),
        ]:
            original_vals = {
                "author_id": msg.author_id.id,
    ...
    def test_search(self):
        ...
        msg_record_admin = self.env["mail.message"].create(
            dict(
                base_msg_vals,
                body="Admin Comment",
        ...
        msg_record_portal = self.env["mail.message"].create(
            dict(
                base_msg_vals,
                body="Portal Comment",
                model=self.record_portal._name,
        ...
        msg_record_portal_internal = self.env["mail.message"].create(
            dict(
                base_msg_vals,
        ...
        msg_record_public = self.env["mail.message"].create(
            dict(
                base_msg_vals,
                body="Public Comment",
        ...
        msg_record_public_internal = self.env["mail.message"].create(
            dict(
                base_msg_vals,
    ...
    def test_search_customized(self):
        ...
        found_emp = (
            self.env["mail.message"]
            .with_user(self.user_employee)
            .search([("body", "ilike", "AnchorForSearch")])
        )
        ...
        self.assertEqual(
            found_emp,
            messages_all.filtered(lambda m: m.res_id != records[2].id),
            "Should filter like read",
        )
...
class TestMessageSubModelAccess(MessageAccessCommon):
    ...
    def test_mail_follower(self):
        ...
        internal_record.message_subscribe(partner_ids=self.user_portal.partner_id.ids)
        ...
        follower.with_user(self.user_admin).write(
    ...
    def test_mail_notification(self):
        ...
        message = internal_record.message_post(
            ...
            message_type="comment",
            partner_ids=(
                self.user_portal.partner_id + self.user_employee.partner_id
            ).ids,
            subtype_id=self.env.ref("mail.mt_comment").id,
        )
        ...
        notif_other = notifications.filtered(
            lambda n: n.res_partner_id == self.user_portal.partner_id
        )
        with self.assertRaises(AccessError):
            notif_other.write({"is_read": True})

        notif_own = notifications.filtered(
            lambda n: n.res_partner_id == self.user_employee.partner_id
        )
    ...
    def test_mail_notification_portal(self):
        ...
        message = portal_record.message_post(
            ...
            message_type="comment",
            partner_ids=(
                self.user_portal_2.partner_id + self.user_employee.partner_id
            ).ids,
            subtype_id=self.env.ref("mail.mt_comment").id,
        )
