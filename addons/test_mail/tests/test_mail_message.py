...
class TestMessageHelpersRobustness(MailCommon, HttpCase):
    ...
    def setUp(self):
        ...
        self.env.cr.execute(
            f"DELETE FROM {self.test_records_simple._table} WHERE id = %s",
            (self.deleted_record.id,),
        )
        self.env.invalidate_all()

    def test_assert_initial_values(self):
        notifs_by_employee = self.env["mail.notification"].search(
            [("author_id", "=", self.partner_employee.id)]
        )
        self.assertEqual(
        ...
        self.assertTrue(
            all(
                notif.notification_status == "exception" for notif in notifs_by_employee
            )
        )
        self.assertTrue(
            all(
                notif.res_partner_id == self.partner_employee_2
                for notif in notifs_by_employee
            )
        )
    ...
    def test_load_message_failures(self):
        ...
        self.assertEqual(
            ...
            sorted(self.test_records_simple[:2].ids),
        )
        self.assertEqual(
            sorted(
                self.env["mail.notification"]
                .search([("author_id", "=", self.partner_employee.id)])
    ...
    def test_load_message_failures_use_display_name(self):
        test_record = self.env["mail.test.simple.unnamed"].create(
        ...
        msg = test_record.message_post(
            body="Some body", author_id=self.partner_employee.id
        )
        # simulate failure
        ...
        with (
            ...
            self.assertEqual(
                ...
                sorted(
                    ["Some description"]
                    + (self.test_records_simple - self.deleted_record).mapped(
                        "display_name"
                    )
                ),
            )
    ...
    def test_message_fetch(self):
        ...
        p2_notifications = self.env["mail.notification"].search(
            [("res_partner_id", "=", self.partner_employee_2.id)]
        )
    ...
    def test_message_link_by_employee(self):
        ...
        thread_message = record.message_post(
            body="Thread Message", message_type="comment"
        )
        ...
        with self.subTest(thread_message=thread_message):
            expected_url = (
                self.base_url()
                + f"/odoo/{thread_message.model}/{thread_message.res_id}?highlight_message_id={thread_message.id}"
            )
    ...
    def test_notify_cancel_by_type(self):
        ...
        self.test_records_simple.with_user(self.user_employee).notify_cancel_by_type(
            "email"
        )
        self.env.invalidate_all()
        ...
        self.assertEqual(
            len(notifs_by_employee),
            3,
            "Currently keep notifications for missing records",
        )
        self.assertTrue(
            all(notif.notification_status == "canceled" for notif in notifs_by_employee)
        )
...
class TestMessageValues(MailCommon):
    ...
    def test_empty_message(self):
        ...
        record.message_subscribe(
            partner_ids=self.partner_admin.ids, subtype_ids=note_subtype.ids
        )
        ...
        )  # message still notified (albeit content is removed)
        self.assertEqual(
            message.starred_partner_ids, self.partner_admin
        )  # starred messages stay (albeit content is removed)
    ...
    def test_mail_message_to_store_access(self):
        ...
        message = record1.message_post(
            body="", partner_ids=[self.user_employee.partner_id.id]
        )
        ...
        # check model not inheriting from mixin.mail.thread -> should not crash
    ...
    def test_records_by_message(self):
        ...
        self.assertEqual(
            test_simple_records._prefetch_ids, tuple((record1 + record2).ids)
        )
    ...
    def test_mail_message_values_fromto_long_name(self):
        ...
        msg = self.env["mail.message"].create(
            {"model": test_record._name, "res_id": test_record.id}
        )
        ...
        self.assertEqual(
            msg.reply_to,
            f"{sanitized_alias_name}@{self.alias_domain}",
    ...
    @mute_logger("odoo.models.unlink")
    @users("employee")
    def test_the_three_reply_to_spellings_agree(self):
        """A model's reply-to redirect must apply however the question is asked.

        Three code paths ask for a record's reply-to and they used to disagree,
        because the redirect was overridden one level too high:

          `_notify_get_reply_to`          -> honoured the override
          `_notify_get_reply_to_batch`    -> skipped it   (mail.compose.message)
          `_notify_get_reply_to_addresses`-> skipped it   (mail.message)

        So a reply to a mass mail sent from the composer on a lead, a task, an
        applicant or a ticket went to the **generic catchall** instead of the
        team, project or job alias it was addressed from. Reproduced on
        `mail.test.ticket`, whose override points at its container:

            _notify_get_reply_to        aliased-container@...   correct
            _notify_get_reply_to_batch  catchall@...            wrong
            _addresses + formatted      catchall@...            wrong

        The override point is now `_notify_get_reply_to_addresses`, the deepest
        of the three, so all of them inherit it. Asserted as "the three agree"
        rather than "each returns X" because that is the invariant -- they are
        three spellings of one question.
        """
        # `mail.test.ticket.mc` is the fixture carrying the redirect, and it
        # points at `mail.test.container.mc`
        container = (
            self.env["mail.test.container.mc"]
            .sudo()
            .create({"name": "Redirecting container", "alias_name": "redirected"})
        )
        ticket = self.env["mail.test.ticket.mc"].create(
            {"name": "Redirected", "container_id": container.id}
        )
        self.env.flush_all()
        self.env.invalidate_all()

        author = self.user_employee.partner_id
        default = '"Fallback" <fallback@test.example.com>'
        expected = formataddr(
            (author.name, f"{container.alias_name}@{self.alias_domain}")
        )

        through_default = ticket._notify_get_reply_to(
            default=default, author_id=author.id
        )[ticket.id]
        through_batch = ticket._notify_get_reply_to_batch(
            defaults={ticket.id: default}, author_ids={ticket.id: author.id}
        )[ticket.id]
        address = ticket.sudo()._notify_get_reply_to_addresses().get(ticket.id)
        through_addresses = ticket.sudo()._notify_get_reply_to_formatted_email(
            address, author_id=author.id
        )

        self.assertEqual(
            through_default, expected, "the container alias, not the catchall"
        )
        self.assertEqual(through_batch, expected, "mail.compose.message asks this one")
        self.assertEqual(through_addresses, expected, "mail.message asks this one")

    @users("employee")
    def test_a_record_without_a_parent_still_falls_back(self):
        """Moving the override point must not lose the fallback for a record
        that names no parent: it keeps its own alias, then the catchall."""
        lone = self.env["mail.test.ticket.mc"].create({"name": "No container"})
        self.env.flush_all()
        author = self.user_employee.partner_id
        self.assertEqual(
            lone._notify_get_reply_to(default=False, author_id=author.id)[lone.id],
            formataddr((author.name, self.env.company.catchall_email)),
        )

    def test_mail_message_values_fromto_document_alias(self):
        msg = self.Message.create(
            {"model": "mail.test.container", "res_id": self.alias_record.id}
        )
        self.assertIn(
            "-odoo-%d-mail.test" % self.alias_record.id, msg.message_id.split("@")[0]
        )
        reply_to_name = self.user_employee.name
    ...
    def test_mail_message_values_fromto_document_no_alias(self):
        ...
        msg = self.Message.create(
            {"model": "mail.test.simple", "res_id": test_record.id}
        )
        self.assertIn(
            "-odoo-%d-mail.test.simple" % test_record.id, msg.message_id.split("@")[0]
        )
        reply_to_name = self.user_employee.name
        reply_to_email = "%s@%s" % (self.alias_catchall, self.alias_domain)
        self.assertEqual(msg.reply_to, formataddr((reply_to_name, reply_to_email)))
    ...
    def test_mail_message_values_misc(self):
        ...
        msg = self.env["mail.message"].create(
            {"model": self.alias_record._name, "res_id": self.alias_record.id}
        )
        self.assertEqual(
