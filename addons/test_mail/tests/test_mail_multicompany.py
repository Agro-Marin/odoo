...
from odoo.addons.test_mail.models.test_mail_corner_case_models import (
        ...
        cls.fake_email = cls.env["mail.message"].create(
            {
                ...
                "message_id": "<123456-odoo-%s-mail.test.gateway@%s>"
                % (cls.test_record.id, socket.gethostname()),
            }
        )
...
class TestMultiCompanySetup(TestMailMCCommon, HttpCase):
    ...
    def test_post_with_read_access(self):
        ...
        test_record_c1_su = (
            self.env["mail.test.multi.company.read"]
            .sudo()
            .create(
                [
                    {
    ...
    def test_post_wo_access(self):
        ...
        with patch.object(
            MailMessage, "_check_access", autospec=True, side_effect=_original_car
        ) as mock_msg_car:
            with self.assertRaises(AccessError):
        ...
        attachments = self.env["ir.attachment"].create(
            self._generate_attachments_data(2, "mail.compose.message", 0, prefix="Same")
        )
        ...
        self.assertEqual(
            ...
            [
                "ReportLike1",
                "ReportLike2",
                "SameAttFileName_00.txt",
                "SameAttFileName_01.txt",
            ],
        )
        self.assertEqual(
    ...
    def test_recipients_multi_company(self):
        ...
        with self.assertBus(
            [(self.cr.dbname, "res.partner", self.user_employee_c3.partner_id.id)]
        ):
...
class TestMultiCompanyControllers(TestMailMCCommon, HttpCase):
    ...
    def test_mail_thread_data(self):
        ...
        self.assertTrue(result["mixin.mail.thread"][0]["hasWriteAccess"])
        self.assertTrue(result["mixin.mail.thread"][0]["hasReadAccess"])
        self.assertTrue(result["mixin.mail.thread"][0]["canPostOnReadonly"])
            ...
            with self.subTest(login=login, mc_record=mc_record):
                ...
                if not login:
                    ...
                    if mc_error:
                        ...
                        self.assertEqual(
                            parsed_url.path, "/odoo/action-mail.action_discuss"
                        )
                    else:
                        ...
                        if mc_record.company_id == user.company_id:
                            ...
                            self.assertEqual(
                                cids, f"{user.company_id.id}-{mc_record.company_id.id}"
                            )
    ...
    def test_redirect_to_records_nothread(self):
        ...
        for test_record in nothreads:
            for user_company in self.company_admin, self.company_2:
                with self.subTest(
                    record_name=test_record.name, user_company=user_company
                ):
                    self.authenticate(self.user_admin.login, self.user_admin.login)
                    ...
                    self.assertEqual(
                        response.request._cookies.get("cids"), str(user_company.id)
                    )
