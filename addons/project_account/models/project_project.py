...
class ProjectProject(models.Model):
    ...
    def action_profitability_items(self, section_name, domain=None, res_id=False):
        if section_name in ["other_revenues_aal", "other_costs_aal", "other_costs"]:
            ...
            if res_id:
                ...
                action["views"] = [
                    (pivot_view_id, view_type)
                    if view_type == "pivot"
                    else (graph_view_id, view_type)
                    if view_type == "graph"
    ...
    def _get_items_from_aal(self, with_action=True):
        ...
        aal_other_search = (
            ...
            .sudo()
            .search_read(domain, ["id", "amount", "currency_id"])
        )
        ...
        for aal in aal_other_search:
            ...
            if aal_amount < 0.0:
                ...
                dict_amount_per_currency_id[aal["currency_id"][0]]["revenues"] += (
                    aal_amount
                )
        ...
        for currency_id, dict_amounts in dict_amount_per_currency_id.items():
            currency = (
                ...
                .browse(currency_id)
                .with_prefetch(dict_amount_per_currency_id)
            )
            total_revenues += currency._convert(
                dict_amounts["revenues"], self.currency_id, self.company_id
            )
            total_costs += currency._convert(
        ...
        if with_action and self.env.user.has_group("account.group_account_readonly"):
            ...
            revenues["action"] = self._get_action_for_profitability_section(
                revenue_ids, "other_revenues_aal"
            )

        return {
            "revenues": {
                "data": [revenues],
                "total": {"invoiced": total_revenues, "to_invoice": 0.0},
            },
            "costs": {
                "data": [costs],
                "total": {"billed": total_costs, "to_bill": 0.0},
            },
        }
