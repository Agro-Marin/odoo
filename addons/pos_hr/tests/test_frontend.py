...
class TestPosHrHttpCommon(TestPointOfSaleHttpCommon):
    ...
    def setUpClass(cls):
        ...
        cls.manager2 = cls.env["hr.employee"].create(
            {"name": "Test Manager 2", "company_id": cls.env.company.id, "pin": "5652"}
        )
        ...
        cls.emp1.write(
            {"name": "Pos Employee1", "pin": "2580", "user_id": emp1_user.id}
        )
        ...
        cls.main_pos_config.write(
            {
                "basic_employee_ids": [
                    Command.link(cls.emp1.id),
                    Command.link(cls.emp2.id),
                    Command.link(cls.emp3.id),
                ],
                "minimal_employee_ids": [Command.link(cls.emp4.id)],
                "advanced_employee_ids": [
                    Command.link(cls.manager1.id),
                    Command.link(cls.manager2.id),
                ],
            }
        )
...
class TestUi(TestPosHrHttpCommon):
    ...
    def test_minimal_employee_refund(self):
        ...
        order_payment = (
            ...
            .create({"amount": 100, "payment_method_id": self.bank_payment_method.id})
        )
    ...
    def test_go_backend(self):
        ...
        self.start_pos_tour(
            "pos_hr_go_backend_opened_registered_different_user_logged",
            login="emp1_user",
        )
