...
class TestSeller(TransactionCase):
    ...
    def test_10_sellers(self):
        self.product_service.write(
            {
                "seller_ids": [
                    ...
                    (
                        0,
                        0,
                        {"partner_id": self.camptocamp.id, "product_code": "C2CCODE"},
                    ),
                ]
            }
        )
        ...
        self.assertEqual(
            "DEFCODE", default_code, "Default code not used in product name"
        )

        context_code = self.product_service.with_context(
            partner_id=self.camptocamp.id
        ).code
        self.assertEqual(
            "C2CCODE",
            context_code,
            "Partner's code not used in product name with context set",
        )
    ...
    def test_20_sellers_company(self):
        ...
        self.assertEqual(
            name,
            "[A] Boudin, [B] Boudin, [NO] Boudin",
            "Incorrect vendor reference list",
        )
        name = self.product_consu.with_context(
            partner_id=self.asustec.id,
            company_id=company_a.id,
        ...
        self.assertEqual(
            name, "[A] Boudin, [NO] Boudin", "Incorrect vendor reference list"
        )
        name = self.product_consu.with_context(
            partner_id=self.asustec.id,
            company_id=company_b.id,
        ...
        self.assertEqual(
            name, "[B] Boudin, [NO] Boudin", "Incorrect vendor reference list"
        )
    ...
    def test_30_select_seller(self):
        ...
        msg = (
            "Wrong cost price: LCD Monitor if more than 3 Unit.should be 785 instead of %s"
            % price
        )
    ...
    def test_31_select_seller(self):
        ...
        self.assertAlmostEqual(
            price, 0.02, places=3, msg="Lowest price should be returned"
        )
