...
class TestVariantsSearch(ProductVariantsCommon):
    def test_attribute_line_search(self):
        ...
        self.assertNotIn(
            product_template_shirt,
            search_not_to_be_found,
        ...
        self.assertIn(
            product_template_shirt, search_value, "Shirt should be found searching L"
        )
    ...
    def test_name_search(self):
        ...
        self.assertIn(
            self.product_slip_template.product_variant_ids.id,
            res_ids,
        ...
        templates = self.product_slip_template.with_context(
...
class TestVariants(ProductVariantsCommon):
    def test_variants_is_product_variant(self):
        ...
        self.assertFalse(
            template.is_product_variant, "Product template is not a variant"
        )
        self.assertEqual(
            {True},
            {v.is_product_variant for v in variants},
            "Product variants are variants",
        )

    def test_variants_pricelist_code(self):
        vendor = self.env["res.partner"].create(
            {"name": "Bidou", "email": "bidou@odoo.com"}
        )
        ...
        self.env["product.supplierinfo"].create(
            [
                ...
                for product, code in zip(
                    self.product_template_sofa.product_variant_ids, codes, strict=False
                )
            ]
        )
        variants = self.product_template_sofa.product_variant_ids.with_context(
            partner_id=vendor.id
        )
        self.assertEqual(
            variants[0].code, codes[0], "sofa red should have code bidou-red"
        )
        self.assertEqual(
            variants[1].code, codes[1], "sofa green should have code bidou-green"
        )
        self.assertEqual(
            variants[2].code, codes[2], "sofa blue should have code bidou-blue"
        )
    ...
    def test_variants_creation_mono_double(self):
        ...
        self.assertEqual(
            test_template.product_variant_ids.product_template_attribute_value_ids.product_attribute_value_id,
            self.size_attribute_s + self.color_attribute_blue,
        )
                    ...
                    Command.create(
                        {
                            "attribute_id": self.size_attribute.id,
                            "value_ids": [
                                Command.set(self.size_attribute.value_ids.ids)
                            ],
                        }
                    ),
                ],
    ...
    def test_dynamic_variants_copy(self):
        ...
        self.color_attr_value_r = self.env["product.attribute.value"].create(
            {"name": "Red", "attribute_id": self.color_attr.id}
        )
        self.color_attr_value_b = self.env["product.attribute.value"].create(
                ...
                "attribute_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "value_ids": [
                                (4, self.color_attr_value_r.id),
                                (4, self.color_attr_value_b.id),
                            ],
                        },
                    )
                ],
        ...
        variant_dyn = template_dyn._create_product_variant(
            template_dyn._get_first_possible_combination()
        )
    ...
    def test_standard_price(self):
        ...
        self.assertEqual(
            one_variant_product.with_company(company_b).cost_currency_id,
            company_b.currency_id,
        )

        one_variant_template = one_variant_product.product_tmpl_id
        self.assertEqual(
            one_variant_product.standard_price, one_variant_template.standard_price
        )
        ...
        self.assertEqual(
            500.0, one_variant_template.with_company(company_b).standard_price
        )
    ...
    def test_archive_variant(self):
        ...
        self.assertEqual(len(template.product_variant_ids), 1)
        self.assertEqual(
            len(template.with_context(active_test=False).product_variant_ids), 2
        )
...
class TestVariantsArchive(ProductVariantsCommon):
    ...
    def test_04_from_to_single_values(self):
        ...
        self._assert_2color_x_1size()
        self.assertEqual(
            self.template.product_variant_ids, variants_2x2[0] + variants_2x2[2]
        )
        archived_variants = self._get_archived_variants()
    ...
    def test_dynamic_attributes_archiving(self):
        ...
        pa_size = ProductAttribute.create(
            {"sequence": 2, "name": "size", "create_variant": "dynamic"}
        )
        ...
        pa_material = ProductAttribute.create(
            {"sequence": 3, "name": "material", "create_variant": "no_variant"}
        )
        ...
        self.assertFalse(
            template._is_combination_possible_by_config(
    ...
    def _update_color_vars(self, ptal):
        ...
        self.assertEqual(
            self.ptav_color_red.product_attribute_value_id, self.color_attribute_red
        )
        self.ptav_color_blue = self.ptal_color.product_template_value_ids[1]
        self.assertEqual(
            self.ptav_color_blue.product_attribute_value_id, self.color_attribute_blue
        )
    ...
    def _update_size_vars(self, ptal):
        ...
        self.assertEqual(
            self.ptav_size_s.product_attribute_value_id, self.size_attribute_s
        )
        if len(self.ptal_size.product_template_value_ids) > 1:
            self.ptav_size_m = self.ptal_size.product_template_value_ids[1]
            self.assertEqual(
                self.ptav_size_m.product_attribute_value_id, self.size_attribute_m
            )
            ...
            Command.create(
                {
                    ...
                    "value_ids": [
                        Command.set(
                            [cls.color_attribute_red.id, cls.color_attribute_blue.id]
                        )
                    ],
                }
            ),
            Command.create(
                {
                    ...
                    "value_ids": [
                        Command.set([cls.size_attribute_s.id, cls.size_attribute_m.id])
                    ],
                }
            ),
        ]
    ...
    def _assert_required_combinations(self, variants, required_values):
        ...
        self.assertEqual(
            ...
            "\nRequired: %s\nActual:   %s"
            % (
                self._get_combinations_names(required_values),
                self._get_combinations_names(actual_values),
            ),
        )
    ...
    def _assert_0color_x_1size(self, variants=None):
        ...
        self.assertEqual(
            variants[0].product_template_attribute_value_ids, self.ptav_size_s
        )
    ...
    def test_add_attribute_to_archived_template(self):
        ...
        for variant in all_variants:
            self.assertFalse(
                variant.active,
                "Variants should remain archived when template is archived",
            )
        ...
        self.env["product.pricelist"].create(
            {
                "name": "Foo",
                "item_ids": [
                    Command.create({"product_id": product.id, "fixed_price": 1})
                ],
            }
        )
...
class TestVariantsExclusion(ProductVariantsCommon):
    ...
    def setUpClass(cls):
        ...
        cls.storage_attr_value_128 = cls.env["product.attribute.value"].create(
            {"name": "128", "attribute_id": cls.storage_attr.id}
        )
        cls.storage_attr_value_256 = cls.env["product.attribute.value"].create(
            ...
            [
                {
                    ...
                    "attribute_id": cls.size_attribute.id,
                    "value_ids": [
                        (6, 0, [cls.size_attribute_s.id, cls.size_attribute_l.id])
                    ],
                },
                {
                    "product_tmpl_id": cls.smartphone.id,
                    ...
                    "value_ids": [
                        (
                            ...
                            0,
                            [
                                cls.storage_attr_value_128.id,
                                cls.storage_attr_value_256.id,
                            ],
                        )
                    ],
                },
    ...
    def test_variants_1_exclusion(self):
        ...
        self.assertEqual(
            len(self.smartphone.product_variant_ids),
            3,
        ...
        self.assertEqual(
            len(self.smartphone.product_variant_ids),
            4,
            "With no exclusion, the smartphone should have 4 active different variants",
        )
                    ...
                    Command.create(
                        {
                            "product_tmpl_id": self.smartphone.id,
                            "value_ids": [
                                (6, 0, [self.smartphone_128.id, self.smartphone_256.id])
                            ],
                        }
                    )
                ]
        ...
        self.smartphone_s.write(
            {
                "exclude_for": [
                    (
                        1,
                        self.smartphone_s.exclude_for.id,
                        {
                            "product_tmpl_id": self.smartphone.id,
    ...
    def test_variants_2_exclusions_different_lines(self):
        ...
        self.assertEqual(
            len(self.smartphone.product_variant_ids),
            3,
            "With one exclusion, the smartphone should have 3 active different variants",
        )
    ...
    def test_exclusions_crud(self):
        ...
        self.assertNotIn(
            ...
            [
                ...
                for product in self.smartphone.product_variant_ids
            ],
        )
    ...
    def test_dynamic_variants_unarchive(self):
                    ...
                    Command.create(
                        {
                            ...
                            "value_ids": [
                                Command.set(self.dynamic_attribute.value_ids.ids)
                            ],
                        }
                    ),
                    Command.create(
        ...
        first_line_ptavs = product_template.attribute_line_ids[
            0
        ].product_template_value_ids
        second_line_ptavs = product_template.attribute_line_ids[
            1
        ].product_template_value_ids
        ...
        variant_to_archive = product_template.product_variant_ids.filtered(
            lambda pp: (
                pav_to_remove
                in pp.product_template_attribute_value_ids.product_attribute_value_id
            )
        )
    ...
    def test_supplierinfo_with_dynamic_attribute(self):
        ...
        product_template._create_product_variant(
            product_template.attribute_line_ids.product_template_value_ids[0]
        )
        self.assertEqual(len(product_template.product_variant_ids), 1)
        self.assertFalse(supplierinfo.product_id)

        product_template._create_product_variant(
            product_template.attribute_line_ids.product_template_value_ids[1]
        )
        self.assertEqual(len(product_template.product_variant_ids), 2)
        self.assertFalse(supplierinfo.product_id)
