...
class TestProduct(ProductCommon):
    ...
    def test_any_user_can_print_product_labels(self):
        ...
        product_template_print_label_action = self.env.ref(
            "product.action_product_template_print_labels"
        )
        ...
        try:
            product_template_print_label_action.with_user(base_user).with_context(
                product_template_context
            ).run()
        except AccessError:
            self.fail("AccessError raised while printing product label with base user.")

        product_product_print_label_action = self.env.ref(
            "product.action_product_print_labels"
        )
        ...
        try:
            product_product_print_label_action.with_user(base_user).with_context(
                product_product_context
            ).run()
        except AccessError:
            self.fail("AccessError raised while printing product label with base user.")
