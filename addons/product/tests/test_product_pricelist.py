...
class TestProductPricelist(ProductCommon):
    ...
    def setUpClass(cls):
        ...
        cls.category_5_id = (
            ...
            .create({"name": "Office Furniture", "parent_id": cls.product_category.id})
            .id
        )
        ...
        cls.new_currency = cls.env["res.currency"].create(
            {
                ...
                "symbol": ":)",
                "rate_ids": [
                    Command.create({"rate": 10, "name": time.strftime("%Y-%m-%d")})
                ],
            }
        )

        cls.ipad_retina_display.write(
            {"uom_id": cls.uom_unit.id, "categ_id": cls.category_5_id}
        )
    ...
    def test_10_calculation_price_of_products_pricelist(self):
        ...
        msg = "Wrong sale price: Customizable Desk. should be %s instead of %s" % (
            price,
        ...
        msg = (
            "Wrong sale price: IT component if more than 3 Unit. should be %s instead of %s"
            % (price, (product.lst_price - product.lst_price * (0.05)))
        )
        self.assertEqual(
            float_compare(
                price,
                product.lst_price - product.lst_price * (0.05),
                precision_digits=2,
            ),
            0,
            msg,
        ...
        msg = (
            "Wrong sale price: LCD Monitor on end of year. should be %s instead of %s"
            % (price, product.lst_price - product.lst_price * (0.30))
        )
        self.assertEqual(
            float_compare(
                price,
                product.lst_price - product.lst_price * (0.30),
                precision_digits=2,
            ),
            0,
            msg,
        ...
        context.update(
            {
                "quantity": 1,
                "date": datetime.strptime("2020-04-05 08:00:00", "%Y-%m-%d %H:%M:%S"),
            }
        )
        ...
        price = self.customer_pricelist._get_product_price(
            product, quantity=1.0, date="2020-04-06 10:00:00"
        )
    ...
    def test_price_without_pricelist_fallback_product_price(self):
        ...
        self.assertEqual(
            ProductPricelist._get_product_price(
                self.monitor, quantity=1.0, currency=self.new_currency
            ),
    ...
    def test_pricelist_sync_on_partners(self):
        ...
        test_partner_company.with_company(
            company_1
        ).property_product_pricelist = self.business_pricelist.id
        test_partner_company.with_company(
            company_2
        ).property_product_pricelist = self.customer_pricelist.id
    ...
    def test_batched_chained_pricing_matches_per_product(self):
        ...
        products = self.computer_SC234 | self.ipad_retina_display | self.ipad_mini
