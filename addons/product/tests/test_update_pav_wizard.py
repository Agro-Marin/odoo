...
class TestUpdateProductAttributeValueWizard(ProductVariantsCommon):
    ...
    def test_update_extra_prices(self):
        self.assertEqual(
            ...
            self.product_template_sofa.attribute_line_ids.product_template_value_ids.mapped(
                "price_extra"
            ),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.product_template_sofa.attribute_line_ids.product_template_value_ids.filtered(
                lambda ptav: (
                    ptav.product_attribute_value_id == self.color_attribute_red
                ),
            ).price_extra,
            20.0,
        )
        self.assertFalse(
            any(self.color_attribute.value_ids.mapped("default_extra_price_changed"))
        )
