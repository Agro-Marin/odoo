...
class ProductProduct(models.Model):
    ...
    code = fields.Char(string="Reference", compute="_compute_product_code")
    partner_ref = fields.Char(string="Customer Ref", compute="_compute_partner_ref")
    ...
    def _compute_write_date(self):
        ...
        for record in self:
            ...
            record.write_date = max(
                record.write_date or self.env.cr.now(),
    ...
    def _check_duplicated_packaging_barcodes(self, barcodes_within_company, company_id):
        packaging_domain = self._get_barcode_search_domain(
            barcodes_within_company, company_id
        )
    ...
    def _compute_product_code(self):
        read_access = self.env["ir.model.access"].check(
            "product.supplierinfo", "read", False
        )
        for product in self:
        ...
        if self.env["product.product"].search_count(domain, limit=1):
            return {
                "warning": {
                    "title": _("Note:"),
                    "message": _(
                        "The Reference '%s' already exists.", self.default_code
                    ),
                }
            }
    ...
    def unlink(self):
        ...
        for product in existing_products:
            ...
            has_other_products = product_ids_by_template_id.get(
                product.product_tmpl_id.id, set()
            ) - {product.id}
    ...
    def _search(self, domain, *args, **kwargs):
        ...
        if self.env.context.get("search_default_categ_id"):
            domain = Domain(domain) & Domain(
                "categ_id", "child_of", self.env.context["search_default_categ_id"]
            )
    ...
    def name_search(self, name="", domain=None, operator="ilike", limit=100):
        ...
        if operator in positive_operators:
            products = self.search_fetch(
                ...
                domain & Domain("barcode", "=", name), ["display_name"], limit=limit
            )
        if not products:
            if is_positive:
                ...
                if limit_rest is None or limit_rest > 0:
                    products_query = self._search(
                        domain & Domain("default_code", operator, name), limit=limit
                    )
                    products |= self.search_fetch(
                        domain
                ...
                products = self.search_fetch(
                    domain & domain_neg, ["display_name"], limit=limit
                )
        if (
            not products
            ...
            products = self.search_fetch(
                domain & match_domain, ["display_name"], limit=limit
            )
        ...
        for seller in sellers_filtered:
            ...
            if quantity_uom_seller and uom_id and uom_id != seller.product_uom_id:
                quantity_uom_seller = uom_id._compute_quantity(
                    quantity_uom_seller, seller.product_uom_id
                )
