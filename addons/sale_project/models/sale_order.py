...
class SaleOrder(models.Model):
    ...
    def default_get(self, fields):
        ...
        if "origin" in fields and (
            task_id := self.env.context.get("create_for_task_id")
        ):
    ...
    def _compute_visible_project(self):
        ...
        for order in self:
            order.visible_project = any(
                ...
                for service_tracking in order.line_ids.mapped(
    ...
    def _compute_project_ids(self):
        projects = self.env["project.project"].search(
            [
                ...
                ("reinvoiced_sale_order_id", "in", self.ids),
            ]
        )
        projects_per_so = defaultdict(lambda: self.env["project.project"])
        for project in projects:
            projects_per_so[
                project.sale_order_id.id or project.reinvoiced_sale_order_id.id
            ] |= project
        for order in self:
    ...
    def action_create_project(self):
        ...
        if not self.show_create_project_button:
            return {
                ...
                "params": {
                    "type": "danger",
                    "message": self.env._(
                        "The project couldn't be created as the Sales Order must be confirmed or is already linked to a project."
                    ),
                },
            }
        ...
        default_sale_line = next(
            (
                sol
                for sol in sorted_line
                if sol.product_id.type == "service" and not sol.is_downpayment
            ),
            self.env["sale.order.line"],
        )
        view_id = self.env.ref(
            "sale_project.sale_project_view_form_simplified_template",
        ...
        return {
            **self.env["project.template.create.wizard"].action_open_template_view(),
            ...
            "context": {
                ...
                "generate_milestone": default_sale_line.product_id.service_policy
                == "delivered_milestones",
                "default_name": self.name,
                "default_allow_milestones": "delivered_milestones"
                in self.line_ids.product_id.mapped("service_policy"),
            },
        }
    ...
    def action_view_milestone(self):
        ...
        default_sale_line = next(
            (
                ...
                for sol in sorted_line
                if sol.is_service
                and sol.product_id.service_policy == "delivered_milestones"
            ),
            self.env["sale.order.line"],
        )
        return {
    ...
    def create(self, vals_list):
        ...
        project = self.env["project.project"].browse(
            self.env.context.get("create_for_project_id")
        )
        ...
        if project or task:
            service_sol = next(
                (sol for sol in created_records.line_ids if sol.is_service),
                self.env["sale.order.line"],
            )
            if project and not project.sale_line_id:
                ...
                if not project.reinvoiced_sale_order_id:
                    project.reinvoiced_sale_order_id = (
                        service_sol.order_id or created_records[0]
                        if created_records
                        else False
                    )
    ...
    def _compute_completed_task_percentage(self):
        for so in self:
            so.completed_task_percentage = (
                so.tasks_count and so.closed_task_count / so.tasks_count
            )
