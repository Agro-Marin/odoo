...
class SaleOrderLine(models.Model):
    ...
    def default_get(self, fields):
        ...
        if (
            self.env.context.get("form_view_ref")
            ...
            if "order_id" in fields and not res.get("order_id"):
                ...
                if project_id:
                    try:
                        project_so = (
                            self.env["project.project"].browse(project_id).sale_order_id
                        )
                        project_so.check_access("write")
                        sale_order = project_so or self.env["sale.order"].search(
    ...
    def _compute_qty_transferred_method(self):
        milestones_lines = self.filtered(
            lambda sol: (
                not sol.is_expense
    ...
    def _compute_qty_transferred(self):
        lines_by_milestones = self.filtered(
            lambda sol: sol.qty_transferred_method == "milestones"
        )
        ...
        project_milestone_read_group = self.env["project.milestone"]._read_group(
            [
                ("sale_line_id", "in", lines_by_milestones.ids),
                ("is_reached", "=", True),
            ],
            ["sale_line_id"],
            ["quantity_percentage:sum"],
        )
        reached_milestones_per_sol = {
        ...
        for line in lines_by_milestones:
            ...
            line.qty_transferred = (
                reached_milestones_per_sol.get(sol_id, 0.0) * line.product_qty
            )
    ...
    def _prepare_qty_transferred(self):
        ...
        delivered_qties = super(
            SaleOrderLine, self - lines_by_milestones
                ...
                applied_root_plans = (
                    ...
                    .browse(
                        list(
                            {
                                int(account_id)
                                for ids in line.analytic_distribution
    ...
    def write(self, vals):
        ...
        if "product_qty" in vals and not self.env.context.get(
            "no_update_allocated_hours", False
        ):
            for line in self:
    ...
    def copy_data(self, default=None):
        ...
        for origin, datum in zip(self, data, strict=True):
            if (
                origin.analytic_distribution
                == origin.order_id.project_id.sudo()._get_analytic_distribution()
            ):
    ...
    def _timesheet_create_project_prepare_values(self):
        ...
        return {
            ...
            or self.order_id.project_account_id.id
            or self.env["account.analytic.account"]
            .create(self.order_id._prepare_analytic_account_data())
            .id,
    ...
    def _timesheet_create_project(self):
        ...
        if project_template:
            ...
            values.update(
                self._timesheet_create_project_account_vals(self.order_id.project_id)
            )
    ...
    def _timesheet_create_project_account_vals(self, project):
        return {
            ...
            for fname in project._get_plan_fnames()
            if fname != "account_id" and project[fname]
        }
    ...
    def _timesheet_create_task_prepare_values(self, project):
        ...
        if (
            sale_line_name_parts
            and sale_line_name_parts[0] == self.product_id.display_name
        ):
    ...
    def _get_so_lines_new_project(self):
        return self.filtered(
            lambda sol: (
                sol.is_service
                and sol.product_id.service_tracking
                in ["project_only", "task_in_project"]
            )
        )
    ...
    def _timesheet_service_generation(self):
        ...
        sale_order_lines = self.filtered(
            lambda sol: (
                sol.is_service
                and sol.product_id.service_tracking
                in ["project_only", "task_in_project", "task_global_project"]
        ...
        def _can_create_project(sol):
            if not sol.project_id:
                if sol.product_id.project_template_id:
                    return (
                        sol.order_id.id,
                        sol.product_id.project_template_id.id,
                    ) not in map_so_project_templates
        ...
        for so_line in so_line_new_project.sorted(lambda sol: (sol.sequence, sol.id)):
            ...
            if not project and _can_create_project(so_line):
                ...
                if not account:
                    ...
                    ].create(so_line.order_id._prepare_analytic_account_data())
            ...
            if so_line.product_id.service_tracking == "task_in_project":
                if not project:
                    if so_line.product_id.project_template_id:
                        project = map_so_project_templates[
                            (
                                so_line.order_id.id,
                                so_line.product_id.project_template_id.id,
                            )
                        ]
                    else:
                ...
                if (
                    not so_line.task_id
                    and so_line.product_id.task_template_id not in task_templates
                ):
        ...
        if not values.get("analytic_distribution") and not self.analytic_distribution:
            if self.task_id.project_id.account_id:
                values["analytic_distribution"] = {
                    self.task_id.project_id.account_id.id: 100
                }
