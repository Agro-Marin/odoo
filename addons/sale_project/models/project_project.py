...
class ProjectProject(models.Model):
    ...
    def _get_revenues_items_from_sol(self, domain=None, with_action=True):
        sale_line_read_group = self.env['sale.order.line'].sudo()._read_group(
            ...
            ['id:array_agg', 'amount_to_invoice_taxexc:sum', 'untaxed_amount_invoiced:sum'],
        )
        ...
        if sale_line_read_group:
            ...
            downpayment_sol_ids = []
            for currency, product, is_downpayment, sol_ids, amount_to_invoice_taxexc, untaxed_amount_invoiced in sale_line_read_group:
                if is_downpayment:
