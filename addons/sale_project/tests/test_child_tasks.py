...
class TestNestedTaskUpdate(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.line_ids = cls.env["sale.order.line"].create(
            {
                "name": "Order line",
        ...
        cls.project = cls.line_ids.project_id
    ...
    def test_default_values_creating_subtask(self):
        ...
        self.assertTrue(
            parent.allow_billable,
            "The parent task should be billable as the project linked is billable",
        )
        self.assertEqual(
            parent.partner_id,
            self.project.partner_id,
            "The partner set on the parent task should the one set on the project linked",
        )
        self.assertEqual(
            child.project_id,
            parent.project_id,
            "The project set on the subtask be inheritted from parent",
        )
        self.assertTrue(
            child.allow_billable,
            "The subtask should be billable since its parent task's project is billable",
        )
        self.assertEqual(
            child.partner_id,
            self.project.partner_id,
            "The partner set on the subtask should the one set on the project linked to the parent",
        )
    ...
    def test_creating_subtask_partner_id_on_parent_goes_on_child(self):
        ...
        child = (
            ...
            .with_context(
                default_parent_id=parent.id, default_project_id=self.project.id
            )
            .create({"name": "child"})
        )
        self.assertEqual(child.partner_id, self.user.partner_id)

    def test_creating_subtask_sale_line_id_on_parent_goes_on_child_if_same_partner_in_values(
        self,
    ):
        parent = self.env["project.task"].create(
            {
        ...
        self.assertEqual(child.sale_line_id, self.line_ids)

    def test_creating_subtask_sale_line_id_on_parent_goes_on_child_with_partner_if_not_in_values(
        self,
    ):
        parent = self.env["project.task"].create(
            {
    ...
    def test_creating_subtask_sale_line_id_on_parent_dont_go_on_child_if_other_partner(
        self,
    ):
        parent = self.env["project.task"].create(
            {
    ...
    def test_creating_subtask_sale_line_id_on_parent_go_on_child_if_same_commercial_partner(
        self,
    ):
        ...
        self.assertEqual(
            child.sale_line_id,
            self.line_ids,
            "Sale order line on parent should be transfered to child",
        )
        self.assertNotEqual(child.partner_id, parent.partner_id)
    ...
    def test_write_partner_id_on_parent_write_on_child(self):
        parent = self.env["project.task"].create(
            {"name": "parent", "partner_id": False, "project_id": self.project.id}
        )
        child = self.env["project.task"].create(
            {
                "name": "child",
    ...
    def test_linking_user_id_on_parent_dont_write_on_child(self):
        ...
        child = self.env["project.task"].create(
            {"name": "child", "user_ids": False, "project_id": self.project.id}
        )
        self.assertFalse(child.user_ids)
    ...
    def test_linking_partner_id_on_parent_write_on_child(self):
        ...
        child = self.env["project.task"].create(
            {"name": "child", "partner_id": False, "project_id": self.project.id}
        )
        self.assertFalse(child.partner_id)
    ...
    def test_linking_sale_line_id_on_parent_write_on_child_with_partner_if_not_set(
        self,
    ):
        parent = self.env["project.task"].create(
            {
    ...
    def test_linking_on_parent_with_multiple_tasks(self):
        ...
        children_values = [
            {"name": "child%s" % i, "user_ids": False, "project_id": self.project.id}
            for i in range(5)
        ]
        children = self.env["project.task"].create(children_values)
    ...
    def test_allow_billable_on_subtasks(self):
        ...
        self.assertTrue(
            all((parent + parent._get_all_subtasks()).mapped("allow_billable"))
        )
        ...
        project_non_billable = self.env["project.project"].create(
            {"name": "Non-billable project", "allow_billable": False}
        )
        ...
        child = self.env["project.task"].create(
            {
                "name": "child task",
                "parent_id": parent.id,
                "project_id": project_tempalte.id,
            }
        )
