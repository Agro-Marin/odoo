...
class TestProjectDashboardCommon(Common):
    ...
    def setUpClass(cls):
        ...
        cls.dashboardSaleOrderLine = cls.env["sale.order.line"].with_context(
            tracking_disable=True, default_order_id=cls.dashboard_sale_order.id
        )
        (
            cls.dashboard_product_delivery_service,
            cls.product_milestone,
            cls.product_prepaid,
...
class TestDashboardProject(TestProjectDashboardCommon):
    ...
    def test_get_sale_item_data_various_sols(self):
        ...
        sale_item_data = self.dashboard_project.get_sale_items_data(
            limit=5, with_action=False, section_id="materials"
        )
        ...
        sale_item_data = self.dashboard_project.get_sale_items_data(
            limit=5, with_action=False, section_id="service_revenues"
        )
        self.assertEqual(sale_item_data["sol_items"], expected_dict)
