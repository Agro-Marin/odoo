...
def _document_depends(model):
    ...
    depends = ["company_id.tax_config_id.account_fiscal_country_id.code"]
...
class MixinStockConsignment(models.AbstractModel):
    ...
    def _compute_l10n_ro_edi_stock_default_location_type(self):
        for record in self:
            if record.company_id.tax_config_id.account_fiscal_country_id.code == "RO":
    ...
    def _compute_l10n_ro_edi_stock_current_document_state(self):
        for record in self:
            if (
                record.company_id.tax_config_id.account_fiscal_country_id.code == "RO"
