...
class TestETransportFlows(TestL10nRoEdiStockCommon):
    ...
    def setUpClass(cls):
        ...
        if "intrastat_code_id" in cls.env["product.product"]._fields:
            cls.default_intrastat_code = cls.env.ref(
    ...
    def _assert_etransport_document(self, document, filename):
        with misc.file_open(
            f"{self.test_module}/tests/test_files/{filename}.xml", "rb"
        ) as file:
        ...
        if "intrastat_code_id" in self.env["product.product"]._fields:
            ...
            nsmap.pop(None)
            for tag in expected_tree.xpath(
                "//*/etr:bunuriTransportate", namespaces=nsmap
            ):
    ...
    def test_send_and_amend_etransport(self, make_request):
        ...
        self.delivery_picking.carrier_id.l10n_ro_edi_stock_partner_id = (
            self.shipping_partner
        )
        self.delivery_picking.button_validate()
        self._assert_picking_state(
            self.delivery_picking, enabled_fields=["enable", "enable_send"]
        )
        ...
        self._assert_picking_state(
            self.delivery_picking, "stock_sending_failed", 1, ("enable", "enable_send")
        )
        self.assertTrue(
            self.delivery_picking.l10n_ro_edi_stock_document_ids.message
            == "some error happened"
        )
        ...
        self.delivery_picking.action_l10n_ro_edi_stock_send_etransport()
        self._assert_picking_state(
            self.delivery_picking,
            "stock_sent",
            1,
            ("enable", "enable_fetch", "fields_readonly"),
        )
        self._assert_etransport_document(
            self.delivery_picking.l10n_ro_edi_stock_document_ids,
        ...
        self.delivery_picking.action_l10n_ro_edi_stock_fetch_status()
        self._assert_picking_state(
            self.delivery_picking, "stock_validated", 1, ("enable", "enable_amend")
        )
        ...
        make_request.return_value = self.successful_upload_response
        self.delivery_picking.with_context(
            test_send_and_amend_etransport="amend"
        ...
        self._assert_etransport_document(
            self.delivery_picking._l10n_ro_edi_stock_get_last_document("stock_sent"),
        ...
        self.delivery_picking.action_l10n_ro_edi_stock_fetch_status()
        self._assert_picking_state(
            self.delivery_picking, "stock_validated", 2, ("enable", "enable_amend")
        )
        self._assert_etransport_document(
            self.delivery_picking._l10n_ro_edi_stock_get_last_document(
                "stock_validated"
            ),
            "test_send_and_amend_etransport_2",
        )
    ...
    def test_intra_community_purchase(self, make_request):
        ...
        self.receipt_picking.carrier_id.l10n_ro_edi_stock_partner_id = (
            self.shipping_partner
        )
        ...
        self._assert_picking_state(
            self.receipt_picking,
            "stock_sent",
            1,
            ("enable", "enable_fetch", "fields_readonly"),
        )
        self._assert_etransport_document(
            self.receipt_picking.l10n_ro_edi_stock_document_ids,
    ...
    def test_export(self, make_request):
        ...
        self._assert_etransport_document(
            self.delivery_picking.l10n_ro_edi_stock_document_ids, "test_export_1"
        )
