...
class TestProgramWithoutCodeOperations(TestSaleCouponCommon):
    ...
    def test_immediate_program_basic_operation(self):
        ...
        self.assertEqual(len(order.line_ids.ids), 1, "The promo offer shouldn't have been applied as the product B isn't in the order")
        ...
        self.assertEqual(order.line_ids[0].product_id.id, self.product_A.id, "The wrong line has been removed")
        self.assertEqual(order.line_ids[1].product_id.id, self.product_B.id, "The wrong line has been removed")
        ...
        order._update_programs_and_rewards()
        self._claim_reward(order, self.immediate_promotion_program)
        self.assertEqual(len(order.line_ids.ids), 1, "The promo reward should have been removed as the rules are not matched anymore")
        self.assertEqual(order.line_ids.product_id.id, self.product_B.id, "The wrong line has been removed")
