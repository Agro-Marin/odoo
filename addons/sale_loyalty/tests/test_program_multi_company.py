...
class TestSaleCouponMultiCompany(TestSaleCouponCommon):
    def setUp(self):
        ...
        self.company_b = self.env["res.company"].create({"name": "TEST"})
    ...
    def test_applicable_programs(self):
        ...
        self.assertNotIn(
            self.immediate_promotion_program_c2, self._get_applicable_programs(order)
        )
        self.assertNotIn(
            self.immediate_promotion_program_c2, order._get_applied_programs()
        )
        ...
        self.assertNotIn(
            self.immediate_promotion_program, self._get_applicable_programs(order_b)
        )
        order_b._update_programs_and_rewards()
        self.assertIn(
            self.immediate_promotion_program_c2, order_b._get_applied_programs()
        )
        self.assertNotIn(
            self.immediate_promotion_program, order_b._get_applied_programs()
        )
    ...
    def test_applicable_programs_confirm_on_branch(self):
        ...
        order.with_user(self.sale_user).with_company(branch_a.id).sudo(
            False
        ).action_confirm()
