...
class SaleOrder(models.Model):
    ...
    def _best_global_discount_already_applied(
        ...
        if (
            discountable is None
        ):  # Only recompute if discountable is not given, not if its zero
    ...
    def _try_apply_code(self, code):
        ...
        if coupon and coupon in self.applied_coupon_ids and is_reward_program_applied:
            # The coupon is already on the order and has paid for a reward. If it kept
            # enough points for another one, offer it instead of refusing the program.
