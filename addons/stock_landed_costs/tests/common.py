...
class TestStockLandedCostsCommon(ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls):
        ...
        cls.categ_manual_periodic = cls.env.ref("product.product_category_goods").copy(
            {"property_valuation": "periodic", "property_cost_method": "fifo"}
        )
        ...
        cls.brokerage_quantity = cls.Product.create(
            {"name": "Brokerage Cost", "type": "service", "categ_id": cls.categ_all.id}
        )
        ...
        cls.packaging_volume = cls.Product.create(
            {"name": "Packaging Cost", "type": "service", "categ_id": cls.categ_all.id}
        )
