...
class TestStockLandedCosts(TestStockLandedCostsCommon):
    def test_stock_landed_costs(self):
        ...
        for valuation in stock_landed_cost_1.valuation_adjustment_lines:
            if valuation.cost_line_id.name == "equal split":
                ...
                valuation.cost_line_id.name == "split by quantity"
                and valuation.move_id.id == move_1_id
            ):
                self.assertEqual(valuation.additional_landed_cost, 50)
            elif (
                valuation.cost_line_id.name == "split by quantity"
                and valuation.move_id.id == move_2_id
            ):
                self.assertEqual(valuation.additional_landed_cost, 100)
            elif (
                valuation.cost_line_id.name == "split by weight"
                and valuation.move_id.id == move_1_id
            ):
                self.assertEqual(valuation.additional_landed_cost, 50)
            elif (
                ...
                valuation.cost_line_id.name == "split by volume"
                and valuation.move_id.id == move_1_id
            ):
                self.assertEqual(valuation.additional_landed_cost, 5)
            elif (
        ...
        lc_value = sum(
            stock_landed_cost_1.account_move_id.line_ids.filtered(
                lambda aml: aml.account_id.name.startswith("Expenses")
            ).mapped("debit")
        )
        product_value = abs(product_landed_cost_1.value_svl) + abs(
            product_landed_cost_2.value_svl
        )
        self.assertEqual(lc_value, product_value)

        self.assertEqual(
            len(picking_landed_cost_1.move_ids.stock_valuation_layer_ids), 5
        )
        self.assertEqual(
            len(picking_landed_cost_2.move_ids.stock_valuation_layer_ids), 5
        )
    ...
    def test_aml_account_selection(self):
        ...
        for valuation in ["periodic", "real_time"]:
            ...
            account = self.landed_cost.product_tmpl_id._get_product_accounts()[
                account_name
            ]
            ...
            self.assertEqual(
                bill.state, "posted", "Incorrect value with valuation %s" % valuation
            )
            self.assertEqual(
                landed_cost_aml.account_id,
                account,
                "Incorrect value with valuation %s" % valuation,
            )
    ...
    def test_landed_cost_in_move_line(self):
        ...
        account_move._update_order_line_info(product_id=self.landed_cost.id, quantity=1)
        self.assertTrue(
            account_move.invoice_line_ids.is_landed_costs_line,
            "The landed cost should appear in the move line.",
        )
        account_move._update_order_line_info(product_id=self.product.id, quantity=1)
        move_line_no_landed = account_move.line_ids.filtered(
            lambda line: line.product_id == self.product
        )
        self.assertFalse(
            move_line_no_landed.is_landed_costs_line,
            "The landed cost should not be set to True.",
        )
