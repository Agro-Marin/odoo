...
class TestStockLandedCostsLots(TestLotValuation):
    ...
    def test_stock_landed_costs_lots(self):
        ...
        picking_1.move_ids.move_line_ids = [Command.clear()] + [
            Command.create(
                {
                    "product_id": self.product1.id,
                    "lot_name": lot_name,
        ...
        picking_2.move_ids.move_line_ids = [Command.clear()] + [
            Command.create(
                {
                    "product_id": product2.id,
                    "lot_name": lot_name,
        ...
        lc_value = sum(
            lc.account_move_id.line_ids.filtered(
        ...
        self.assertRecordValues(
            lc.stock_valuation_layer_ids.sorted(lambda svl: svl.product_id.id),
            [
                {
                    "lot_id": lot_product_a[0].id,
                    "product_id": self.product1.id,
                    "stock_valuation_layer_id": og_p1_layers[0].id,
                    "quantity": 0,
                    "value": 1,
                },
                {
                    "lot_id": lot_product_a[1].id,
                    "product_id": self.product1.id,
                    "stock_valuation_layer_id": og_p1_layers[1].id,
                    "quantity": 0,
                    "value": 1,
                },
                {
                    "lot_id": lot_product_a[2].id,
                    "product_id": self.product1.id,
                    "stock_valuation_layer_id": og_p1_layers[2].id,
                    "quantity": 0,
                    "value": 1,
                },
                {
                    "lot_id": lot_product_b[0].id,
                    "product_id": product2.id,
                    "stock_valuation_layer_id": og_p2_layers[0].id,
                    "quantity": 0,
                    "value": 1.5,
                },
                {
                    "lot_id": lot_product_b[1].id,
                    "product_id": product2.id,
                    "stock_valuation_layer_id": og_p2_layers[1].id,
                    "quantity": 0,
                    "value": 1.5,
                },
            ],
        ...
        for l, price in zip(lot_product_b, [11.3, 11.3], strict=False):
            self.assertEqual(l.standard_price, price)
        outs = self._make_out_move(
            self.product1,
            9,
            lot_ids=[lot_product_a[0], lot_product_a[1], lot_product_a[2]],
        )
        self.assertRecordValues(
            outs.stock_valuation_layer_ids.sorted("id"),
            [
                {
                    "lot_id": lot_product_a[0].id,
                    "product_id": self.product1.id,
        ...
        outs = self._make_out_move(
            product2, 4, lot_ids=[lot_product_b[0], lot_product_b[1]]
        )
        self.assertRecordValues(
            outs.stock_valuation_layer_ids.sorted("id"),
            [
    ...
    def test_landed_cost_when_partially_sold(self):
        ...
        # deliver 2 products
        (lot1, lot2, lot3, lot4) = picking_1.move_ids.move_line_ids.mapped(
            "lot_id"
        ).sorted("id")
        ...
        self.assertRecordValues(
            lc.stock_valuation_layer_ids.sorted("id"),
            [
