...
class TestLandedCosts(TestStockLandedCostsCommon):
    ...
    def test_00_landed_costs_on_incoming_shipment(self):
        ...
        self.assertRecordValues(
            stock_landed_cost.account_move_id.line_ids,
            [
    ...
    def test_01_negative_landed_costs_on_incoming_shipment(self):
        ...
        self.assertEqual(
            ...
            "done",
            "Negative landed costs should be in done state",
        )
        self.assertTrue(
            stock_negative_landed_cost.account_move_id,
            "Landed costs should be available account move lines",
        )
        [balance] = self.env["account.move.line"]._read_group(
            [("move_id", "=", stock_negative_landed_cost.account_move_id.id)],
        ...
        move_lines = [
            ...
            {
                "name": "split by quantity - Microwave Oven",
                "debit": 33.33,
                "credit": 0.0,
            },
            {
            ...
            {
                "name": "split by weight - Refrigerator: 2.0 already out",
                "debit": 4.0,
                "credit": 0.0,
            },
            {
            ...
            {
                "name": "split by quantity - Refrigerator: 2.0 already out",
                "debit": 6.67,
                "credit": 0.0,
            },
            {
            ...
            {
                "name": "equal split - Refrigerator: 2.0 already out",
                "debit": 1.0,
                "credit": 0.0,
            },
            {
        ...
        self.assertRecordValues(
            stock_negative_landed_cost.account_move_id.line_ids.sorted(
                lambda d: (d["name"], d["debit"])
            ),
        ...
        for valuation in stock_landed_cost.valuation_adjustment_lines:
            ...
            if split_method == "equal":
                self.assertEqual(
                    add_cost,
                    valid_vals["equal"],
                    self._error_message(valid_vals["equal"], add_cost),
                )
            elif split_method == "by_quantity" and product == self.product_refrigerator:
                self.assertEqual(
                    add_cost,
                    valid_vals["by_quantity_refrigerator"],
                    self._error_message(
                        valid_vals["by_quantity_refrigerator"], add_cost
                    ),
                )
            elif split_method == "by_quantity" and product == self.product_oven:
                self.assertEqual(
                    add_cost,
                    valid_vals["by_quantity_oven"],
...
class TestLandedCostsWithPurchaseAndInv(TestStockValuationLCCommon):
    def test_invoice_after_lc(self):
        ...
        aml = self.env["account.move.line"].search(
            [("account_id", "=", stock_valuation_account.id)], order="id desc", limit=1
        )
    ...
    def test_invoice_after_lc_amls(self):
        ...
        landed_cost_aml = bill.invoice_line_ids.filtered(
            lambda l: l.product_id == self.landed_cost
        )
    ...
    def test_landed_cost_avco_partial_bill_rounding(self):
        ...
        backorder_picking = purchase_order.picking_ids.filtered(
            lambda p: p.backorder_id == picking
        )
        self.assertTrue(
            backorder_picking, "Backorder picking was not created or not found."
        )
        ...
        self.assertAlmostEqual(
            ...
            110,
            msg="Product AVCO should be 110 after first receipt.",
        )
        ...
        self.assertAlmostEqual(
            bill2.invoice_line_ids[0].quantity,
            120,
            msg="Bill 2 should be for the remaining 120 units.",
        )
        self.assertAlmostEqual(
            bill2.invoice_line_ids[0].price_unit,
            110,
