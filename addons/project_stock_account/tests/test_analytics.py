...
class TestAnalytics(TestStockCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.plan1, cls.plan2 = cls.env["account.analytic.plan"].create(
            [{"name": "Plan 1"}, {"name": "Plan 2"}]
        )
        cls.plan1_name = cls.plan1._column_name()
        cls.plan2_name = cls.plan2._column_name()
        cls.analytic_account1, cls.analytic_account2 = cls.env[
    ...
    def test_analytic_lines_generation_delivery(self):
        ...
        analytic_line1 = analytic_lines.filtered(
            lambda a: a.product_id == self.product1
        )
        ...
        analytic_line2 = analytic_lines.filtered(
            lambda a: a.product_id == self.product2
        )
