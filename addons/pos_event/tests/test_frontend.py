...
class TestUi(TestPointOfSaleHttpCommon):
    ...
    def test_selling_event_in_pos(self):
        ...
        event_answer_name = (
            event_registration.registration_answer_ids.value_answer_id.mapped("name")
        )
    ...
    def test_selling_multislot_event_in_pos(self):
        ...
        event_answer_names = (
            registrations.registration_answer_ids.value_answer_id.mapped("name")
        )
    ...
    def test_selling_multiple_ticket_saved(self):
        ...
        self.start_tour(
            ...
            "test_selling_multiple_ticket_saved",
            login="pos_user",
        )
    ...
    def test_orderline_price_remain_same_as_ticket_price(self):
        ...
        self.env["res.partner"].search([("name", "=", "Partner Test 1")]).write(
            {
                "property_product_pricelist": self.main_pos_config.available_pricelist_ids.filtered(
                    lambda pl: pl.item_ids
                )[0],
            }
        )
        self.main_pos_config.with_user(self.pos_user).open_ui()
    ...
    def test_pos_event_registration_not_mandatory(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_pos_event_registration_not_mandatory",
            login="pos_user",
        )

        registrations = self.env["event.registration"].search(
            [("event_id", "=", self.test_event_registration_not_mandatory.id)]
        )
        ...
        partner_registrations = registrations.filtered(
            lambda r: r.partner_id == event_partner
        )
