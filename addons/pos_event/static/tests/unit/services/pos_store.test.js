...
test("createDummyProductForEvents", async () => {
    ...
    const eventProducts = models["product.template"].filter(
        (product) => product._event_id,
    );
    ...
    for (const product of eventProducts) {
        expect(product.event_id).toEqual(
            events.find((event) => event.id === product._event_id),
        );
    }
});
