...
class TestThirdChecks(L10nLatamCheckTest):
    def create_third_party_check(
        ...
        vals = {
            ...
            "l10n_latam_new_check_ids": [
                ...
                Command.create(
                    {
                        "name": check_numbers[1],
                        "payment_date": fields.Date.add(fields.Date.today(), months=1),
                        "amount": 1,
                    }
                ),
            ],
            "payment_channel_id": journal._get_available_payment_channels("inbound")
            .filtered(lambda x: x.code == "new_third_party_checks")
            .id,
        }
    ...
    def test_01_get_paid_with_multiple_checks(self):
        ...
        self.assertEqual(
            len(payment.l10n_latam_new_check_ids),
            2,
    ...
    def test_02_third_party_check_delivery(self):
        ...
        vals = {
            ...
            "payment_channel_id": self.third_party_check_journal._get_available_payment_channels(
                "outbound"
            )
            .filtered(
                lambda x: (
                    x.code in ("out_third_party_checks", "return_third_party_checks")
                )
            )
            .id,
        }
        ...
        self.assertFalse(
            ...
            "Current journal was not computed properly on delivery",
        )
        # check dont delivery twice
        with self.assertRaisesRegex(
            ValidationError, "it seems it has been moved by another payment"
        ):
        ...
        vals = {
            ...
            "payment_channel_id": self.rejected_check_journal._get_available_payment_channels(
                "inbound"
            )
            .filtered(lambda x: x.code == "in_third_party_checks")
            .id,
        }
        ...
        self.assertEqual(
            check.current_journal_id,
            self.rejected_check_journal,
            "Current journal was not computed properly on return",
        )
        # check dont return twice
        with self.assertRaisesRegex(
            ValidationError,
            "Some checks are already in hand and can't be received again",
        ):
            self.env["account.payment"].create(vals).action_post()
        ...
        self.assertFalse(
            check.current_journal_id,
            "Current journal was not computed properly on customer return",
        )
        # check dont claim twice
        with self.assertRaisesRegex(
            ValidationError, "Some checks are not anymore in journal,"
        ):
            self.env["account.payment"].create(vals).action_post()

        operations = self.env["account.payment"].search(
            [("l10n_latam_move_check_ids", "=", check.id), ("state", "!=", "draft")],
            order="date desc, id desc",
        )
        self.assertEqual(
            len(operations), 3, "There should be 3 operations on the check"
        )
    ...
    def test_03_deposit(self):
        ...
        self.assertEqual(
            check.current_journal_id.id,
            bank_journal.id,
            "Current journal was not computed properly on delivery",
        )
        self.assertEqual(
            len(check.operation_ids + payment),
            3,
            "Check that all three payments were created",
        )
        ...
        self.assertEqual(
            check.current_journal_id.id,
            self.rejected_check_journal.id,
            "Current journal was not computed properly on delivery",
        )
        self.assertEqual(
            len(check.operation_ids + payment),
            5,
            "Check that all five payments were created",
        )
    ...
    def test_04_check_transfer(self):
        ...
        self.env["l10n_latam.payment.mass.transfer"].with_context(
            active_model="l10n_latam.check", active_ids=[check.id, check2.id]
        ).create(
    ...
    def test_05_check_current_journal_with_both_operations(self):
        ...
        outbound_payment = self.env["account.payment"].create(
            {
                ...
                .filtered(lambda x: x.code == "out_third_party_checks")
                .id,
            }
        )
        ...
        outbound_payment_2.write(
            {"l10n_latam_move_check_ids": [Command.set([check_2.id])]}
        )
