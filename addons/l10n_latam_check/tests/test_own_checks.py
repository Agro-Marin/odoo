...
class TestOwnChecks(L10nLatamCheckTest):
    def test_01_pay_with_own_checks(self):
        ...
        with Form(
            self.env["account.payment"].with_context(default_payment_type="outbound")
        ) as payment_form:
            payment_form.partner_id = self.partner_a
            payment_form.journal_id = self.bank_journal
            payment_form.payment_channel_id = (
                self.bank_journal._get_available_payment_channels("outbound").filtered(
        ...
        outstanding_line_ids = payment.l10n_latam_new_check_ids.mapped(
            "outstanding_line_id"
        )
        self.assertEqual(
            len(outstanding_line_ids), 2, "There should be a split line per check. (2)"
        )
        all_handed = any(
            s == "handed"
            for s in payment.l10n_latam_new_check_ids.mapped("issue_state")
        )
        ...
        self.assertTrue(
            first_check.issue_state == "voided",
            "First checks should be in voided status.",
        )
    ...
    def test_02_pay_with_own_check_and_cancel_payment(self):
        ...
        self.assertEqual(
            len(payment.l10n_latam_new_check_ids.outstanding_line_id),
            0,
    ...
    def test_post_own_check_with_3_lines(self):
        ...
        payment_channel = self.bank_journal._get_available_payment_channels(
            "outbound"
        ).filtered_domain([("code", "=", "own_checks")])[:1]
