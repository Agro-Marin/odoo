...
class L10nLatamCheckTest(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.bank_journal.outbound_payment_channel_ids = [
        ...
        third_party_checks_journals = cls.env['account.journal'].search([
            ('inbound_payment_channel_ids.code', '=', 'in_third_party_checks'),
            ('inbound_payment_channel_ids.code', '=', 'new_third_party_checks'),
            ('outbound_payment_channel_ids.code', 'in', ('out_third_party_checks', 'return_third_party_checks')),
        ])
        ...
        for company in third_party_checks_journals.grouped('company_id'):
            outstanding_account = cls.outbound_payment_channel.payment_account_id.copy({'company_ids': [Command.set(company.ids)]})
            cls.bank_journal.outbound_payment_channel_ids.filtered(lambda m: m.company_id == company).payment_account_id = outstanding_account
