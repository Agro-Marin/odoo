...
class AccountPayment(models.Model):
    ...
    def _compute_amount(self):
        for rec in self:
            checks = (
                rec.l10n_latam_new_check_ids
                if rec._is_latam_check_payment(check_subtype="new_check")
                else rec.l10n_latam_move_check_ids
            )
            if checks:
    ...
    def action_post(self):
        ...
        if not self.env.context.get("l10n_ar_skip_remove_check"):
            for payment in self.filtered(
                lambda x: (
                    x.l10n_latam_move_check_ids
                    and not x._is_latam_check_payment(check_subtype="move_check")
                )
            ):
    ...
    def _get_blocking_l10n_latam_warning_msg(self):
        ...
        for rec in self.filtered(
            lambda x: x.state == "draft" and x._is_latam_check_payment()
        ):
            if any(
                rec.currency_id != check.currency_id
            ...
            if not rec.currency_id.is_zero(
            ...
            if rec._is_latam_check_payment(check_subtype="move_check"):
                if any(
                ...
                elif rec.payment_type == "outbound" and any(
                    check.current_journal_id != rec.journal_id
                    for check in rec.l10n_latam_move_check_ids
                ):
                    ...
                    and not rec._is_latam_check_transfer()
                    ...
                    if last_operation and last_operation[0].date > date:
                        msgs.append(
                            self.env._(
                                ...
                                last_operation=last_operation.display_name,
                            )
                        )
        return msgs

    def _get_reconciled_checks_error(self):
        checks_reconciled = self.l10n_latam_new_check_ids.filtered(
            lambda x: x.issue_state in ["debited", "voided"]
        )
            ...
            raise UserError(
                self.env._(
                    ...
                    (
                        "\n".join(
                            [
                                "* %s (%s)" % (x.name, x.issue_state)
                                for x in checks_reconciled
                            ]
                        )
                    ),
                )
    ...
    def _l10n_latam_check_split_move(self):
        for payment in self.filtered(
            lambda x: (
                x.payment_method_code == "own_checks" and x.payment_type == "outbound"
            )
        ):
            ...
            for check in payment.l10n_latam_new_check_ids:
                ...
                if check == payment.l10n_latam_new_check_ids[-1]:
                    liquidity_balance = payment.currency_id.round(
                        payment_liquidity_line.balance - liquidity_balance_total
                    )
                else:
                    liquidity_balance = payment.currency_id.round(
                        payment_liquidity_line.balance * check.amount / checks_total
                    )
                    ...
                    Command.create(
                        {
                            "name": self.env._(
                                ...
                                suffix="".join(
                                    [
                                        item[1]
                                        for item in payment._get_aml_default_display_name_list()
                                    ]
                                ),
                            ),
            ...
            move_id.action_post()
            split_move_counterpart_line = move_id.line_ids._filtered_amount_currency(
                -payment_liquidity_line.amount_currency
            )
    ...
    @api.depends(
        ...
        "currency_id",
        "company_id",
        "l10n_latam_move_check_ids.issuer_vat",
        "l10n_latam_move_check_ids.bank_id",
        "l10n_latam_move_check_ids.payment_id.date",
        "l10n_latam_new_check_ids.amount",
        ...
        for rec in self.filtered(lambda x: x._is_latam_check_payment()):
            ...
            if rec.payment_method_code == "new_third_party_checks":
                ...
                for check in rec.l10n_latam_new_check_ids.filtered(
                    lambda x: (
                        x.name
                        and x.payment_channel_id.code == "new_third_party_checks"
                        and x.bank_id
                        and x.issuer_vat
                    )
                ):
            ...
            rec.l10n_latam_check_warning_msg = (
                msgs and "* %s" % "\n* ".join(msgs)
            ) or False
    ...
    def _prepare_move_line_default_vals(
            ...
            res[0].update(
                {
                    "name": self.env._(
                        ...
                        suffix="".join(
                            [
                                item[1]
                                for item in self._get_aml_default_display_name_list()
                            ]
                        ),
                    ),
        ...
        # we dont want this names on the own checks because it doesn't add value, already each split/check line will have it name
            ...
            self.l10n_latam_new_check_ids or self.l10n_latam_move_check_ids
            ...
            res[0].update(
                {
                    ...
                    + " - "
                    + "".join(
                        [item[1] for item in self._get_aml_default_display_name_list()]
                    ),
                }
            )
        return res
    ...
    def _compute_destination_account_id(self):
        ...
        for payment in self:
            if payment.l10n_latam_move_check_ids and (
                not payment.partner_id
