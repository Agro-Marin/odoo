...
class L10n_LatamCheck(models.Model):
    ...
    _unique = models.UniqueIndex(
        "(name, payment_method_line_id) WHERE outstanding_line_id IS NOT NULL"
    )
    ...
    def button_open_check_operations(self):
        ...
        action = {
            ...
            "views": [
                (
                    self.env.ref(
                        "l10n_latam_check.view_account_third_party_check_operations_tree"
                    ).id,
                    "list",
                ),
                (False, "form"),
    ...
    def _get_reconciled_move(self):
        reconciled_line = (
            self.outstanding_line_id.full_reconcile_id.reconciled_line_ids
            - self.outstanding_line_id
        )
    ...
    def _clean_issuer_vat(self):
        for rec in self.filtered(
            lambda x: x.issuer_vat and x.company_id.country_id.code
        ):
            stdnum_vat = stdnum.util.get_cc_module(
                rec.company_id.country_id.code, "vat"
            )
    ...
    def _check_issuer_vat(self):
        for rec in self.filtered(lambda x: x.issuer_vat and x.company_id.country_id):
            self.env["res.partner"]._run_vat_checks(
                rec.company_id.country_id,
                rec.issuer_vat,
                partner_name="Check Issuer VAT",
            )
    ...
    def _unlink_if_payment_is_draft(self):
        if any(check.payment_id.state != "draft" for check in self):
            raise UserError(
                self.env._("Can't delete a check if payment is In Process!")
            )
