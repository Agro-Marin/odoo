...
class L10n_LatamPaymentMassTransfer(models.TransientModel):
    ...
    journal_id = fields.Many2one(
        "account.journal", check_company=True, compute="_compute_journal_company"
    )
    company_id = fields.Many2one("res.company", compute="_compute_journal_company")
    check_ids = fields.Many2many(
        "l10n_latam.check",
        "latam_tranfer_check_reltransfer_id",
        "check_id",
        check_company=True,
    )
    ...
    def default_get(self, fields):
        ...
        if "check_ids" in fields and "check_ids" not in res:
            ...
            checks = self.env["l10n_latam.check"].browse(
            ...
            if not all(
                check.payment_id.state not in ("draft", "canceled") for check in checks
            ):
    ...
    def _create_payments(self):
        ...
        checks = self.check_ids.filtered(
            lambda x: (
                x.payment_channel_id.code == "new_third_party_checks"
                and x.currency_id == self.check_ids[0].currency_id
            )
        )
        currency_id = self.check_ids[0].currency_id

        pay_method_line = self.journal_id._get_available_payment_channels(
            "outbound"
        ).filtered(
        ...
        outbound_payment = self.env["account.payment"].create(
            {
                ...
                "payment_channel_id": pay_method_line.id if pay_method_line else False,
        ...
        dest_payment_method = (
            self.destination_journal_id.inbound_payment_channel_ids.filtered(
                lambda x: x.code == "in_third_party_checks"
            )
        )
        if dest_payment_method:
            inbound_payment.payment_channel_id = dest_payment_method
        ...
        body_inbound = (
            _("This payment has been created from: ")
            + outbound_payment._get_html_link()
        )
        inbound_payment.message_post(body=body_inbound)
        body_outbound = (
            _("A second payment has been created: ") + inbound_payment._get_html_link()
        )
        ...
        (outbound_payment.move_id.line_ids + inbound_payment.move_id.line_ids).filtered(
            lambda l: (
                l.account_id == outbound_payment.destination_account_id
