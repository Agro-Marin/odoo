...
class AccountPaymentRegister(models.TransientModel):
    ...
    l10n_latam_new_check_ids = fields.One2many(
        "l10n_latam.payment.register.check", "payment_register_id", string="New Checks"
    )
    ...
    def _compute_amount(self):
        ...
        for wizard in self.filtered(
            lambda x: x._is_latam_check_payment(check_subtype="move_check")
        ):
            ...
            vals.update(
                {
                    "l10n_latam_move_check_ids": [
                        Command.link(x.id) for x in self.l10n_latam_move_check_ids
                    ]
                }
            )
        return vals
    ...
    def action_create_payments(self):
        if self._is_latam_check_payment(check_subtype="move_check"):
            latam_check_currencies = self.l10n_latam_move_check_ids.mapped(
                "currency_id"
            )
            if latam_check_currencies and (
                len(latam_check_currencies) > 1
                or latam_check_currencies != self.currency_id
            ):
