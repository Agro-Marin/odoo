...
class TestCalendarSms(SMSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.partner_phone = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create({"number": "0477777777", "type": "landline"})
                ],
        ...
        cls.partner_phone_2 = cls.env["res.partner"].create(
            {
                "name": "Partner With Phone Number",
                "phone_ids": [
                    Command.create({"number": "0488888888", "type": "landline"})
                ],
                "country_id": cls.env.ref("base.be").id,
            }
        )
        ...
        cls.partner_phone_3 = cls.env["res.partner"].create(
            {
                "name": "Partner With Phone Number",
                "phone_ids": [
                    Command.create({"number": "0499999999", "type": "landline"})
                ],
                "country_id": cls.env.ref("base.be").id,
            }
        )
        ...
        cls.event = cls.env["calendar.event"].create(
            {
                ...
                "partner_ids": [
                    (6, 0, [cls.partner_phone.id, cls.partner_no_phone.id])
                ],
        ...
        cls.event_1h_dup = cls.event_1h.copy(
            default={
                "alarm_ids": [(4, cls.alarm_1h.id), (4, cls.alarm_24h.id)],
        ...
        cls.event_24h = cls.env["calendar.event"].create(
            {
                "name": "Event in 24h",
                "start": now
                + timedelta(hours=23, minutes=30),  # Start in 23 hours 30 minutes
    ...
    def test_attendees_with_number(self):
        ...
        self.assertEqual(
            len(self._sms), 1, "There should be only one partner retrieved"
        )
    ...
    def test_send_reminder_match_both_events(self):
        ...
        self.assertSMS(
            self.partner_phone_3,
            self.partner_phone_3.phone_sanitized,
            "pending",
            content=self.sms_template_1h.body,
        )
        self.assertSMS(
            self.partner_phone_2,
            self.partner_phone_2.phone_sanitized,
            "pending",
