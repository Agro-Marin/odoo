...
class TestTaxesTaxTotalsSummaryAccountTaxPython(
    TestTaxCommonPOS, TestTaxesTaxTotalsSummary
):
    def test_point_of_sale_custom_tax_with_extra_product_field(self):
        assert "weight" not in self.env["product.template"]._load_pos_data_fields(
    ...
    def test_point_of_sale_custom_tax_with_extra_product_uom_field(self):
        assert "relative_factor" not in self.env["uom.uom"]._load_pos_data_fields(
            self.main_pos_config
        )
