...
class TestTodoOnboardingUsers(TransactionCase):
    ...
    def test_onboarding_stages_and_task_created_for_new_users(self):
        ...
        onboarding_tasks = ProjectTaskSudo.search(
            [("user_ids", "in", internal_user.ids)]
        )

        self.assertEqual(
            len(onboarding_tasks),
            1,
            "Exactly 1 onboarding task should be created for internal users upon creation.",
        )
        self.assertFalse(
            onboarding_tasks.project_id,
            "Onboarding task should not be linked to any project.",
        )
        ...
        self.assertEqual(
            len(onboarding_tasks),
            0,
            "Portal users should not receive onboarding tasks upon creation.",
        )
        ...
        self.assertEqual(
            len(onboarding_tasks),
            0,
            "Public users should not receive onboarding tasks upon creation.",
        )
        ...
        onboarding_tasks = ProjectTaskSudo.search(
            [("user_ids", "in", other_internal_user.ids)]
        )
        self.assertEqual(
            len(onboarding_tasks),
            1,
