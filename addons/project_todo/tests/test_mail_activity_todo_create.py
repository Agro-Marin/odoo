        ...
        *previous* evening anywhere west of UTC — the to-do says "Yesterday" for
        ...
        for tz in ('UTC', 'America/Mexico_City', 'Pacific/Kiritimati', 'Asia/Tokyo'):
            with self.subTest(tz=tz):
                ...
                wizard = self.env['mail.activity.todo.create'].with_user(self.user_admin).create({
                    'summary': f'tz {tz}',
                    'date_deadline': picked,
                    'user_id': self.user_admin.id,
                })
                wizard.create_todo_activity()
                todo = self.env['project.task'].search([('name', '=', f'tz {tz}')], limit=1)
                ...
                activity = self.env['mail.activity'].search([('summary', '=', f'tz {tz}')], limit=1)
                self.assertEqual(
                    activity.date_deadline, shown,
