...
registry.category("web_tour.tours").add("project_task_activities_split", {
    ...
    steps: () => [
        ...
        {
            content:
                "Click on `Show Sub-Tasks` button to see sub-tasks in the main view",
            ...
            run: () => {
                ...
                for (const node of nodes) {
                    if (node.textContent === "New To-do!") {
                        console.error(
                            "Private task records with no parent task should not appear in this view",
                        );
                    }
                }
            },
        ...
        },
        {
            ...
            run: () => {
                ...
                for (const node of nodes) {
                    if (
                        node.textContent === "New Task!" ||
                        node.textContent === "New Sub-Task!"
                    ) {
                        console.error(
                            "Tasks linked to a project should not appear in this view",
                        );
                    }
                }
            },
