...
SINVOICE_TIMEOUT = (
    60  # They recommend between 60 and 90 seconds, but 60s is already quite long.
)
...
class AccountMove(models.Model):
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        self.filtered(
            lambda m: m._l10n_vn_need_cancel_request()
    ...
    def _compute_l10n_vn_edi_invoice_state(self):
        ...
        for invoice in self:
            if (
                ...
                and invoice.l10n_vn_edi_invoice_state == "sent"
            ):
    ...
    def _compute_l10n_vn_edi_invoice_symbol(self):
        ...
        for invoice in self:
            if invoice.country_code == "VN":
                ...
                invoice.l10n_vn_edi_invoice_symbol = (
                    invoice.partner_id.l10n_vn_edi_symbol
                )
            else:
    ...
    def action_l10n_vn_edi_update_payment_status(self):
        ...
        invoices = self.filtered(
            lambda i: i.l10n_vn_edi_invoice_state == "payment_state_to_update"
        )
        if not invoices:
            return
        ...
        for invoice in invoices:
            ...
            if "result" in invoice_lookup:
                ...
                if (
                    invoice_data["status"] == "Chưa thanh toán"
                ):  # Vietnamese for 'unpaid'
                    sinvoice_status = "unpaid"
                else:
            ...
            params = {
                ...
                "strIssueDate": invoice._l10n_vn_edi_format_date(
                    invoice.l10n_vn_edi_issue_date
                ),
            }

            if (
                invoice.payment_state in {"in_payment", "paid"}
                and sinvoice_status == "unpaid"
            ):
                # Mark the invoice as paid
                endpoint = f"{SINVOICE_API_URL}InvoiceAPI/InvoiceWS/updatePaymentStatus"
                params["templateCode"] = (
                    invoice.l10n_vn_edi_invoice_symbol.invoice_template_id.name
                )
            elif (
                invoice.payment_state not in {"in_payment", "paid"}
                and sinvoice_status == "paid"
            ):
    ...
    def _l10n_vn_edi_check_invoice_configuration(self):
        ...
        commercial_partner_phone = (
            commercial_partner_phone
            and self._l10n_vn_edi_format_phone_number(commercial_partner_phone)
        )
    ...
    def _l10n_vn_edi_send_invoice(self, invoice_json_data):
        ...
        if self.l10n_vn_edi_invoice_transaction_id:
            ...
            self.l10n_vn_edi_invoice_transaction_id = invoice_json_data[
                "generalInvoiceInfo"
            ]["transactionUuid"]
    ...
    def _l10n_vn_edi_cancel_invoice(
        ...
        _request_response, error_message = _l10n_vn_edi_send_request(
            ...
            params={
                ...
                "templateCode": self.l10n_vn_edi_invoice_symbol.invoice_template_id.name,
                "invoiceNo": self.l10n_vn_edi_invoice_number,
                "strIssueDate": self._l10n_vn_edi_format_date(
                    self.l10n_vn_edi_issue_date
                ),
                "additionalReferenceDesc": agreement_document_name,
                "additionalReferenceDate": self._l10n_vn_edi_format_date(
    ...
    def action_draft(self):
        ...
        cancelled_sinvoices = self.filtered(
            lambda i: (
                i.country_code == "VN"
                and i.l10n_vn_edi_invoice_state == "canceled"
                and i.state == "cancel"
            )
        )
    ...
    def _l10n_vn_edi_add_general_invoice_information(self, json_values):
        ...
        invoice_data = {
            ...
            "invoiceIssuedDate": self._l10n_vn_edi_format_date(
                self.l10n_vn_edi_issue_date
            ),
        ...
        ):  # Credit note are used to adjust an existing invoice
            adjustment_origin_invoice = self.reversed_entry_id
        elif (
            self.l10n_vn_edi_replacement_origin_id
        ):  # 'Reverse and create invoice' is used to issue a replacement invoice
            ...
            invoice_data.update(
                {
                    ...
                    else "3",  # Adjustment or replacement
                    ...
                    "additionalReferenceDate": self._l10n_vn_edi_format_date(
                        self.l10n_vn_edi_agreement_document_date
                    ),
                }
            )
    ...
    def _l10n_vn_edi_add_buyer_information(self, json_values):
        ...
        commercial_partner_phone = (
            self.commercial_partner_id.phone_ids._primary().number
        )
        commercial_partner_phone = (
            commercial_partner_phone
            and self._l10n_vn_edi_format_phone_number(commercial_partner_phone)
    ...
    def _l10n_vn_edi_add_item_information(self, json_values):
        ...
        for line in self.invoice_line_ids.filtered(
            ...
            item_information = {
                ...
                "itemTotalAmountWithoutTax": line.currency_id.round(
                ...
                "taxAmount": line.currency_id.round(
    ...
    def _l10n_vn_edi_add_tax_breakdowns(self, json_values):
        ...
        tax_details_grouped = self._prepare_invoice_aggregated_taxes(
            grouping_key_generator=grouping_key_generator
        )
        for tax_percentage, tax_percentage_values in tax_details_grouped[
            "tax_details"
        ].items():
            tax_breakdowns.append(
                {
                    ...
                    "taxableAmountPos": self.move_type
                    == "out_invoice",  # For adjustment invoice, the amount should be considered as negative.
