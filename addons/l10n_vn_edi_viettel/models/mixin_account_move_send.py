...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _is_vn_edi_applicable(self, move):
        return bool(
            move.l10n_vn_edi_invoice_state == "ready_to_send"
            and move._l10n_vn_edi_get_credentials_company()
        )
    ...
    def _get_placeholder_mail_attachments_data(
        ...
        if (
            invoice_edi_format == "vn_sinvoice"
            and move._l10n_vn_edi_get_credentials_company()
        ):
            results.extend(
                [
    ...
    def _update_invoice_data_sinvoice_file(self, invoice, invoice_data):
        ...
        need_file = (
            ...
            and invoice._l10n_vn_edi_get_credentials_company()
                ...
                if not errors:
                    if "sinvoice_attachments" in invoice_data:
                        json_data = json.loads(
                            invoice_data["sinvoice_attachments"][0]["raw"].decode(
                                "utf-8"
                            )
                        )
                    # If the invoice was downloaded but not sent, the json file could already be there.
                        ...
                        json_data = json.loads(
                            base64.b64decode(invoice.l10n_vn_edi_sinvoice_file).decode(
                                "utf-8"
                            )
                        )
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            json_file_data = [
                file
                for file in invoice_data.get("sinvoice_attachments", [])
