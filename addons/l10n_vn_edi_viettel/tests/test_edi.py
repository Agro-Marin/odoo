...
class TestVNEDI(AccountTestInvoicingCommon):
        ...
        cls.partner_a.write(
            {
                ...
                "vat": "0100109106-505",
                "phone_ids": [
                    Command.create({"number": "3825 7670", "type": "landline"})
                ],
        ...
        cls.env.company.write(
            {
                ...
                "vat": "0100109106-506",
                "phone_ids": [
                    Command.create({"number": "6266 1275", "type": "landline"})
                ],
    ...
    def test_invoice_creation(self):
        ...
        self.assertFalse(
            invoice.l10n_vn_edi_invoice_state
        )  # State should be False before posting.
    ...
    def test_default_symbol_on_partner(self):
        ...
        self.partner_a.l10n_vn_edi_symbol = self.env[
        ...
        self.assertEqual(
            invoice.l10n_vn_edi_invoice_symbol.id, self.partner_a.l10n_vn_edi_symbol.id
        )
    ...
    def test_adjustment_invoice(self):
        ...
        invoice.write(
            {  # Would be set by sending it to the edi
    ...
    def test_replacement_invoice(self):
        ...
        reverse_move = self.env["account.move"].browse(reversal["res_id"])
        reverse_move.invoice_line_ids[
            0
        ].price_unit = 900  # New price is 900 and not 1000
        json_data = reverse_move._l10n_vn_edi_generate_invoice_json()
        # 1. Check the general info values, ensure correct adjustment type, and that the data were correctly fetched from the original invoice.
        expected = {
            "adjustmentType": "3",
    ...
    def test_decimal_rounding(self):
        ...
        self.assertEqual(
            itemInfo["taxAmount"],
            100.03,
            "Tax amount should be correctly rounded to 2 decimals.",
        )
