...
class SaleReport(models.Model):
        ...
        for table_name, alias, join_type, on_condition in tables:
            if join_type is None:
                ...
                if isinstance(table_name, SQL):
                    from_parts.append(
                        SQL(
                            "%s %s ON %s", SQL(join_type), table_name, SQL(on_condition)
                        )
                    )
                else:
                    from_parts.append(
    ...
    def _get_pos_select_fields(self) -> dict:
        ...
        fields = {
            ...
            "qty_invoiced": f"""CASE WHEN pos.account_move IS NOT NULL
                    THEN SUM(l.qty)
                    ELSE 0
                END""",
            "qty_to_invoice": f"""CASE WHEN pos.account_move IS NULL
                    THEN SUM(l.qty)
                    ELSE 0
                END""",
            ...
            "price_subtotal": f"""SUM(l.price_subtotal)
                    / MIN({currency_rate_pos})
                    * {currency_rate_table}""",
            "price_total": f"""SUM(l.price_subtotal_incl)
                    / MIN({currency_rate_pos})
                    * {currency_rate_table}""",
            "discount": "l.discount",
