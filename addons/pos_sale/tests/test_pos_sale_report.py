...
class TestPoSSaleReport(TestPoSCommon, TestPointOfSaleHttpCommon):
    ...
    def test_weight_and_volume(self):
        ...
        reports = (
            ...
            .sudo()
            .search([("product_id", "=", self.product0.id)], order="id", limit=2)
        )
    ...
    def test_refund_line_report_prices_sign(self):
        ...
        report = (
            ...
            .sudo()
            .search(
                [("product_id", "=", test_product.id), ("name", "ilike", "% REFUND")],
                order="id",
                limit=1,
            )
        )
        ...
        self.assertEqual(
            report.price_subtotal, report.product_uom_qty * test_product.list_price
        )
    ...
    def test_weight_and_volume_product_variant(self):
        ...
        report = (
            self.env["sale.report"]
            .sudo()
            .search([("product_id", "=", product_1.id)], order="id", limit=1)
        )
        ...
        report = (
            self.env["sale.report"]
            .sudo()
            .search([("product_id", "=", product_2.id)], order="id", limit=1)
        )
    ...
    def test_different_shipping_address(self):
        ...
        move_id = self.env["account.move"].browse(
            order_ids["pos.order"][0]["account_move"]
        )
    ...
    def test_warehouse(self):
        ...
        self.assertEqual(
            reports[0].warehouse_id.id, self.config.picking_type_id.warehouse_id.id
        )
    ...
    def test_qty_deliverd_qty_to_deliver_in_sales_report(self):
        ...
        orders.append(
            self.create_ui_order_data(
                [(self.product0, 5, 100), (self.product0, 3)], {}, self.partner_1
            )
        )
