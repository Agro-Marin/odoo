...
class TestPoSSale(TestPointOfSaleHttpCommon):
    ...
    def test_settle_order_with_kit(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.component_a, self.location, 100000
        )
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id, "PosSettleOrder", login="pos_user"
        )

        # assert that sales order qty are correctly updated
        self.assertEqual(sale_order.line_ids.qty_transferred, 3)
        self.assertEqual(
            sale_order.picking_ids[0].move_ids.product_qty, 2100
        )  # 7 left to deliver => 300 * 7 = 2100
        ...
        self.assertEqual(
            sale_order.picking_ids[1].move_ids.quantity, 300
        )  # 1 delivered => 300 * 2 = 600
    ...
    def test_settle_order_with_different_product(self):
        ...
        self.assertEqual(sale_order.line_ids[1].qty_transferred, 0)
        orderline_product_a = sale_order.line_ids.filtered(
            lambda l: l.product_id.id == product_a.id
        )
        orderline_product_b = sale_order.line_ids.filtered(
            lambda l: l.product_id.id == product_b.id
        )
    ...
    def test_settle_order_unreserve_order_lines(self):
        ...
        self.removal_strategy = self.env["product.removal"].search(
        ...
        self.assertEqual(
            sale_order.line_ids.move_ids.move_line_ids[0].location_id.id,
            self.shelf_1.id,
        )
        self.assertEqual(sale_order.line_ids.move_ids.move_line_ids[1].quantity, 2)
        self.assertEqual(
            sale_order.line_ids.move_ids.move_line_ids[1].location_id.id,
            self.shelf_2.id,
        )
        ...
        self.assertEqual(
            pos_order.picking_ids.move_line_ids[0].location_id.id, self.shelf_1.id
        )
        self.assertEqual(pos_order.picking_ids.move_line_ids[1].quantity, 2)
        self.assertEqual(
            pos_order.picking_ids.move_line_ids[1].location_id.id, self.shelf_2.id
        )
    ...
    def test_settle_order_with_multistep_delivery(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            product_a, warehouse.lot_stock_id, 1
        )
    ...
    def test_import_lot_groupable_and_non_groupable(self):
        ...
        self.env["stock.quant"].with_context(inventory_mode=True).create(
            [
                ...
                for lot in (non_groupable_lot, groupable_lot)
            ]
        ...
        self.start_tour(
            ...
            "test_import_lot_groupable_and_non_groupable",
            login="accountman",
        )
    ...
    def test_so_with_downpayment(self):
        ...
        self.main_pos_config.down_payment_product_id = self.env.ref(
            "pos_sale.default_downpayment_product"
        )
        self.main_pos_config.down_payment_product_id.write({"active": True})
        self.main_pos_config.open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "PoSSaleOrderWithDownpayment",
            login="accountman",
        )
    ...
    def test_downpayment_with_taxed_product(self):
        ...
        invoice_pdf_content = str(
            self.env["pos.order"]
            .search([])
            .account_move._get_invoice_legal_documents("pdf", allow_fallback=True)
            .get("content")
        )
        ...
        for order_line in sale_order.line_ids.filtered(
            lambda l: l.product_id == self.downpayment_product
        ):
            order_line = order_line.with_context(lang=partner_test.lang)
            self.assertIn(
                format_date(order_line.env, order_line.order_id.date_order),
                order_line.name,
            )
    ...
    def test_show_orders_for_pos_currency_only(self):
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "PosOrdersListDifferentCurrency",
            login="pos_admin",
        )
    ...
    def test_settle_order_with_multistep_delivery_receipt(self):
        ...
        self.assertEqual(
            sale_order.pos_order_line_ids.order_id.picking_ids.state, "assigned"
        )
        self.assertEqual(
    ...
    def test_pos_sale_warnings(self):
        self.env["res.partner"].create(
            [
                {
                    "name": "A Test Customer 1",
                    "sale_warn_msg": "Highly infectious disease",
                },
                {
                    "name": "A Test Customer 2",
                    "sale_warn_msg": "Cannot afford our services",
                },
            ]
        )
        self.main_pos_config.open_ui()
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id, "PosSaleWarning", login="accountman"
        )
    ...
    def test_downpayment_invoice_link(self):
        ...
        final_invoice_downpayment_line = (
            sale_order.invoice_ids.invoice_line_ids.filtered(lambda r: r.quantity < 0)
        )
        ...
        final_invoice_downpayment_line = sale_order.pos_order_line_ids[
            -1
        ].order_id.account_move.invoice_line_ids.filtered(lambda r: r.quantity < 0)
    ...
    def test_settle_order_ship_later_effect_on_so(self):
        ...
        self.assertEqual(
            len(sale_order_single.pos_order_line_ids.order_id.picking_ids), 1
        )
        self.assertEqual(
            sale_order_single.pos_order_line_ids.order_id.picking_ids.state, "assigned"
        )
        ...
        self.assertEqual(
            len(sale_order_multi.pos_order_line_ids.order_id.picking_ids), 1
        )
        self.assertEqual(
            sale_order_multi.pos_order_line_ids.order_id.picking_ids.state, "assigned"
        )
    ...
    def test_get_special_products_and_ensure_downpayment_product(self):
        ...
        self.assertIn(
            ...
            special_products,
            "_get_special_products should include every pos.config's down"
            " payment product",
        )
    ...
    def test_pos_order_and_invoice_amounts(self):
        ...
        self.start_tour(
            "/pos/ui/%d" % self.main_pos_config.id,
            "POSSalePaymentScreenInvoiceOrder",
            login="accountman",
        )

        order = self.env["pos.order"].search(
            [("partner_id", "=", partner_test.id)], limit=1
        )
    ...
    def test_payment_terms_with_early_discount(self):
        ...
        pos_order_id = self.env["pos.order"].sync_from_ui([pos_order])["pos.order"][0][
            "id"
        ]
    ...
    def test_sale_order_fp_different_from_partner_one(self):
        ...
        self.assertEqual(
            sale_a.fiscal_position_id,
            fp_1,
            "Sale order should have the fiscal position of the partner",
        )
        self.assertEqual(
            sale_a.amount_total,
            20,
            "Sale order amount should be 20 with the tax override 1",
        )
        self.assertEqual(
            sale_a.amount_untaxed,
            10,
            "Sale order untaxed amount should be 10 with the tax override 1",
        )
        self.assertEqual(
            sale_b.fiscal_position_id,
            fp_2,
            "Sale order should have the fiscal position set on the sale order",
        )
        self.assertEqual(
            sale_b.amount_total,
            10,
            "Sale order amount should be 10 with the tax override 2",
        )
        self.assertEqual(
            sale_b.amount_untaxed,
            10,
            "Sale order untaxed amount should be 10 with the tax override 2",
        )
        self.start_pos_tour(
            "test_sale_order_fp_different_from_partner_one", login="accountman"
        )

        pos_order_a = self.env["pos.order"].search(
            [("fiscal_position_id", "=", fp_1.id)], limit=1, order="id desc"
        )
        pos_order_b = self.env["pos.order"].search(
            [("fiscal_position_id", "=", fp_2.id)], limit=1, order="id desc"
        )
        self.assertEqual(
            pos_order_a.amount_total,
            20,
            "PoS order amount should be 20 with the tax override 1",
        )
        self.assertEqual(
            pos_order_a.amount_tax,
            10,
            "PoS order untaxed amount should be 10 with the tax override 1",
        )
        self.assertEqual(
            pos_order_a.lines[0].tax_ids,
            tax_override_1,
            "PoS order should have the tax override 1",
        )
        self.assertEqual(
            pos_order_b.amount_total,
            10,
            "PoS order amount should be 10 with the tax override 2",
        )
        self.assertEqual(
            pos_order_b.amount_tax,
            0,
            "PoS order untaxed amount should be 10 with the tax override 2",
        )
        self.assertEqual(
            pos_order_b.lines[0].tax_ids,
            tax_override_2,
            "PoS order should have the tax override 2",
        )
    ...
    def test_settle_order_with_different_uom(self):
        ...
        pos_order = self.env["pos.order"].search(
            [("partner_id", "=", test_partner.id)], limit=1
        )

        self.assertEqual(pos_order.lines[0].qty, 12.0, "quantity should be 12.0")
        self.assertEqual(
            pos_order.lines[0].price_unit, 0.83, "price of product should be 0.83"
        )
    ...
    def test_multiple_lots_sale_order(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_multiple_lots_sale_order_1",
            login="accountman",
        )
        sale_order.action_confirm()
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_multiple_lots_sale_order_2",
            login="accountman",
        )
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_multiple_lots_sale_order_3",
            login="accountman",
        )
    ...
    def test_selected_partner_quotation_loading(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_selected_partner_quotation_loading",
            login="accountman",
        )
    ...
    def test_ecommerce_paid_order_is_hidden_in_pos(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_ecommerce_paid_order_is_hidden_in_pos",
            login="accountman",
        )
    ...
    def test_ecommerce_unpaid_order_is_shown_in_pos(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_ecommerce_unpaid_order_is_shown_in_pos",
            login="accountman",
        )
    ...
    def test_settle_groupable_lot_total_amount(self):
        ...
        self.start_tour(
            "/pos/ui?config_id=%d" % self.main_pos_config.id,
            "test_settle_groupable_lot_total_amount",
            login="accountman",
        )
