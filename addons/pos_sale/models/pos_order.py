        ...
        for pos_order in pos_orders:
            ...
            for so_line in so_lines:
                ...
                for stock_move in so_line.move_ids:
                    ...
                    qty_transfered = max(so_line.qty_transfered, get_expected_qty_to_ship_later())
                    new_qty = so_line.product_uom_qty - qty_transfered
        ...
        for order_line in self:
            if order_line.order_id.state in ['paid', 'done']:
                ...
                if outgoing_pickings and order_line.order_id.shipping_date:
                    ...
                    if (qty_left):
                        order_line.qty_transfered = min(order_line.qty, qty_left)
                        product_qty_left_to_assign[order_line.product_id.id] -= order_line.qty_transfered
                    else:
                        order_line.qty_transfered = min(order_line.qty, sum(moves.mapped('quantity')))
                        product_qty_left_to_assign[order_line.product_id.id] = sum(moves.mapped('quantity')) - order_line.qty_transfered

                elif outgoing_pickings:
                    # If the order is not delivered later, and in a "paid", "done" or "invoiced" state, it fully delivered
                    order_line.qty_transfered = order_line.qty
                else:
