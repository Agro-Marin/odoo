...
class SaleOrderLine(models.Model):
    ...
    pos_order_line_ids = fields.One2many('pos.order.line', 'sale_order_line_id', string="Order lines Transferred to Point of Sale", readonly=True, groups="point_of_sale.group_pos_user")
    ...
    def _load_pos_data_fields(self, config):
        return ['discount', 'display_name', 'price_total', 'price_unit', 'product_id', 'product_uom_qty', 'qty_transferred',
    ...
    def _compute_qty_transferred(self):
        ...
        for sale_line in self:
            ...
            if all(picking.state == 'done' for picking in pos_lines.order_id.picking_ids):
                sale_line.qty_transferred += sum((self._convert_qty(sale_line, pos_line.qty, 'p2s') for pos_line in pos_lines if sale_line.product_id.type != 'service'), 0)
    ...
    @api.depends('pos_order_line_ids.qty', 'pos_order_line_ids')
    ...
    def _get_sale_order_fields(self):
        return ["product_id", "display_name", "price_unit", "product_uom_qty", "tax_ids", "qty_transferred", "qty_invoiced", "discount", "qty_to_invoice", "price_total", "is_downpayment"]
    ...
    def read_converted(self):
        ...
        for sale_line in self:
            if sale_line.product_type or (sale_line.is_downpayment and sale_line.price_unit != 0):
                ...
                item['qty_transferred'] = self._convert_qty(sale_line, item['qty_transferred'], 's2p')
