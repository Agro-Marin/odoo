...
class SaleOrder(models.Model):
    ...
    def load_sale_order_from_pos(self, config_id):
        ...
        product_tmpls = self.env["product.template"].load_product_from_pos(
            config_id, [("product_variant_ids.id", "in", product_ids)]
        )
        ...
        sale_order_fp_fields = self.env[
            "account.fiscal.position"
        ]._load_pos_data_fields(config_id)
        sale_order_fp_read = self.fiscal_position_id.read(
            sale_order_fp_fields, load=False
        )
    ...
    @api.depends(
        ...
        "line_ids",
        "amount_total",
        "line_ids.invoice_line_ids.parent_state",
        "line_ids.invoice_line_ids.price_total",
        "line_ids.pos_order_line_ids",
    )
    def _compute_amount_unpaid(self):
        for sale_order in self:
            invoices = sale_order.line_ids.invoice_line_ids.move_id.filtered(
        ...
        for order in self:
            ...
            order_amount = sum(
                ...
                .pos_order_line_ids.filtered(
                    lambda pol: (
                        pol.order_id.state in ["paid", "done", "invoiced"]
    ...
    def _prepare_down_payment_line_values_from_base_line(self, base_line):
        # EXTENDS 'sale'
        so_line_values = super()._prepare_down_payment_line_values_from_base_line(
            base_line
        )
        if (
            base_line
            ...
            so_line_values["name"] = _(
                ...
                date=format_date(
                    pos_order_line.env, pos_order_line.order_id.date_order
                ),
            )
