"""Mixin for Odoo models backed by PostgreSQL materialized views.

This mixin provides safe refresh() and helper methods for models that use
materialized views as their underlying storage. It handles edge cases during
module upgrades when views are dropped and recreated.
"""
...
class MaterializedViewMixin(models.AbstractModel):
    ...
    def _create_materialized_view(self, with_data=False, index_field=None):
        ...
        _logger.info("Dropping materialized view %s if exists", self._table)
        self.env.cr.execute(
