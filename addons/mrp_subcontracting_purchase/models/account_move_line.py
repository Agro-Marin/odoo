...
class AccountMoveLine(models.Model):
    ...
    def _get_price_unit_val_dif_and_relevant_qty(self):
        price_unit_val_dif, relevant_qty = (
            super()._get_price_unit_val_dif_and_relevant_qty()
        )
        if self.product_id.cost_method == "standard" and self.purchase_line_ids:
            components_cost = 0
            subcontract_production = (
                self.purchase_line_ids.move_ids._get_subcontract_production()
            )
            valuation_date = (
                subcontract_production.move_raw_ids
                and max(subcontract_production.move_raw_ids.mapped("date"))
            ) or self.date
            ...
            if not self.product_uom_id.is_zero(qty):
                price_unit_val_dif += components_cost / qty
            ...
            if mo := m._get_subcontract_production():
                finished_moves |= set(
                    mo.move_finished_ids.filtered(
                        lambda mf: mf.product_id == m.product_id
                    ).ids
                )
