...
class LoyaltyProgram(models.Model):
    ...
    def _compute_is_payment_program(self):
        for program in self:
            program.is_payment_program = program.program_type in self._PAYMENT_PROGRAM_TYPES
    ...
    def _program_type_default_values(self):
        ...
        first_sale_product = self.env['product.product'].search(
            [('company_id', 'in', [False, self.env.company.id]), ('sale_ok', '=', True)],
            limit=1,
        )
        return {
            ...
            'next_order_coupons': self._next_order_coupons_default_values(first_sale_product),
        }
    ...
    def _get_child_default_values(self, program_type, field_name):
        ...
        creates = [
            command[2] for command in defaults.get(field_name) or ()
    ...
    def action_open_loyalty_cards(self):
        ...
        action['context'] = {
            ...
            # For the wizard
            'default_mode': 'selected' if self.program_type == 'ewallet' else 'anonymous',
        }
        return action
    ...
    def write(self, vals):
        if 'trigger_product_ids' in vals:
            ...
            accepts = self.filtered(
                lambda program: (target_type or program.program_type) in self._PAYMENT_PROGRAM_TYPES
            )
            if accepts != self:
                rest_vals = {k: v for k, v in vals.items() if k != 'trigger_product_ids'}
        ...
        if 'reward_ids' in vals and self._commands_leave_a_reward(vals['reward_ids']):
            ...
            if 'program_type' in vals:
                res = super(LoyaltyProgram, self.with_context(program_type=vals['program_type'])).write(vals)
            else:
