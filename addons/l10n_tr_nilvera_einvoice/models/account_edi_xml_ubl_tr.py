...
class AccountEdiXmlUblTr(models.AbstractModel):
    ...
    def _add_invoice_header_nodes(self, document_node, vals):
        ...
        document_node.update(
            {
                ...
                "cbc:ProfileID": {
                    "_text": "TEMELFATURA"
                    if invoice.partner_id.l10n_tr_nilvera_customer_status == "einvoice"
                    else "EARSIVFATURA"
                },
                ...
                "cbc:DueDate": None,
                "cbc:InvoiceTypeCode": {"_text": "SATIS"}
                if vals["document_type"] == "invoice"
                else None,
                "cbc:CreditNoteTypeCode": {"_text": "IADE"}
                ...
                "cbc:Note": {
                    "_text": html2plaintext(invoice.narration, include_references=False)
        ...
        document_node["cac:OrderReference"]["cbc:IssueDate"] = {
        ...
        document_node["cbc:Note"] = [
            document_node["cbc:Note"],
            {
                "_text": self._l10n_tr_get_amount_integer_partn_text_note(
                    invoice.amount_residual_signed, self.env.ref("base.TRY")
                ),
                "note_attrs": {},
            },
        ]
        if vals["invoice"].currency_id.name != "TRY":
            document_node["cbc:Note"].append(
                {
                    "_text": self._l10n_tr_get_amount_integer_partn_text_note(
                        invoice.amount_residual, vals["invoice"].currency_id
                    ),
                    "note_attrs": {},
                }
            )
            document_node["cbc:Note"].append(
                {"_text": self._l10n_tr_get_invoice_currency_exchange_rate(invoice)}
            )
    ...
    def _add_invoice_delivery_nodes(self, document_node, vals):
        ...
        if "picking_ids" in invoice._fields and invoice.picking_ids:
            document_node["cac:Delivery"]["cbc:ID"] = {
                "_text": invoice.picking_ids[0].name
            }
            document_node["cac:Delivery"]["cbc:ActualDeliveryDate"] = {
                "_text": invoice.delivery_date
            }
        else:
            ...
            document_node["cac:AllowanceCharge"].append(
                {
                    ...
                    "cbc:Amount": {
                        "_text": self.format_float(
                            total_discount_amount, vals["currency_dp"]
                        ),
    ...
    def _get_address_node(self, vals):
        ...
        return {
            "cbc:StreetName": {
                "_text": " ".join(s for s in [partner.street, partner.street2] if s)
            },
    ...
    def _get_party_node(self, vals):
        ...
        party_node = {
            "cac:PartyIdentification": self._get_party_identification_node_list(
                partner
            ),
            "cac:PartyName": {"cbc:Name": {"_text": partner.display_name}},
            "cac:PostalAddress": self._get_address_node(vals),
            "cac:PartyTaxScheme": {
                "cac:TaxScheme": {"cbc:Name": {"_text": (commercial_partner.ref)}}
            },
        ...
        if not partner.is_company:
            ...
            party_node["cac:Person"] = {
                ...
                "cbc:FamilyName": {
                    "_text": name_parts[1] if len(name_parts) > 1 else "\u200b"
                },
            }
        return party_node
    ...
    def _add_document_line_allowance_charge_nodes(self, line_node, vals):
        ...
        for allowance_charge_node in line_node["cac:AllowanceCharge"]:
            ...
            discount_percentage = (
                vals.get("discount_amount") / vals.get("gross_subtotal")
                if vals.get("gross_subtotal")
                else 0
            )
