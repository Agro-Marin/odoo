...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _get_all_extra_edis(self) -> dict:
        ...
        res.update(
            {
                "tr_nilvera": {
                    "label": self.env._("by Nilvera"),
                    "is_applicable": self._is_tr_nilvera_applicable,
                }
            }
        )
        return res
    ...
    def _get_invoice_extra_attachments(self, move):
        ...
        if (
            move.l10n_tr_nilvera_send_status == "succeed"
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        tr_nilvera_moves = moves.filtered(
        ...
        if (
            self.env.company.tax_config_id.account_fiscal_country_id.code == "TR"
            and self.env.company.l10n_tr_nilvera_config_id.l10n_tr_nilvera_use_test_env
        ):
            alerts["l10n_tr_nilvera_einvoice_test_mode"] = {
                "level": "info",
                "message": self.env._("Testing mode is enabled."),
            }
        ...
        if tr_companies_missing_required_codes := tr_nilvera_moves.company_id.filtered(
            lambda c: (
                c.country_code == "TR"
                and not (
                    c.partner_id.tag_ids.parent_id
                    and self.env[
                        "res.partner.tag"
                    ]._get_l10n_tr_official_mandatory_categories()
                )
            )
        ):
            alerts["tr_companies_missing_required_codes"] = {
                "message": self.env._(
                    "Please ensure that your company contact has either the 'MERSISNO' or 'TICARETSICILNO' tag with a value assigned."
                ),
                "action_text": self.env._("View Company(s)"),
                "action": tr_companies_missing_required_codes.partner_id._get_records_action(
                    name=self.env._("Check tags on company(s)")
                ),
                "level": "danger",
            }
        ...
        if (
            tr_partners_missing_required_fields
            := self._get_l10n_tr_tax_partner_address_alert(tr_nilvera_moves)
        ):
        ...
        if (
            tr_partners_missing_tax_office
            := self._get_l10n_tr_tax_partner_tax_office_alert(tr_nilvera_moves)
        ):
            alerts["tr_partners_missing_tax_office"] = tr_partners_missing_tax_office

        # Alert if TR company is missing required tax office
        if (
            tr_companies_missing_tax_office
            := self._get_l10n_tr_tax_company_tax_office_alert(tr_nilvera_moves)
        ):
        ...
        if tr_partners_invalid_edi_or_status := tr_nilvera_moves.filtered(
            ...
            alerts["tr_partners_invalid_edi_or_status"] = {
                ...
                "action": tr_partners_invalid_edi_or_status._get_records_action(
                    name=self.env._(
                        "Check e-Invoice Format or Nilvera Status on Partner(s)"
                    )
                ),
            }
        ...
        if tr_invalid_subscription_dates := moves.filtered(
            lambda move: (
                move._l10n_tr_nilvera_einvoice_check_invalid_subscription_dates()
            )
        ):
        ...
        if invalid_negative_lines := tr_nilvera_moves.filtered(
            ...
            alerts["critical_invalid_negative_lines"] = {
                "level": "danger",
                "message": self.env._(
                    "Nilvera portal cannot process negative quantity nor negative price on invoice lines"
                ),
                "action_text": self.env._("View Invoice(s)"),
                "action": invalid_negative_lines._get_records_action(
                    name=self.env._("Check data on Invoice(s)")
                ),
            }

        if moves_with_invalid_name := tr_nilvera_moves.filtered(
            lambda move: not _is_valid_nilvera_name(move)
        ):
            alerts["tr_moves_with_invalid_name"] = {
                ...
                "action_text": self.env._("View Invoice(s)"),
                "action": moves_with_invalid_name._get_records_action(
                    name=self.env._("Check name on Invoice(s)")
                ),
            }
    ...
    def _get_l10n_tr_tax_partner_address_alert(self, moves):
        ...
        if tr_partners_missing_required_fields := moves.filtered(
            ...
            return {
                ...
                "action_text": self.env._("View Partner(s)"),
                "action": tr_partners_missing_required_fields._get_records_action(
                    name=self.env._(
    ...
    def _get_l10n_tr_tax_company_tax_office_alert(self, moves):
        # Overriden in l10n_tr_nilvera_einvoice_extended to give error based on l10n_tax_office field
        if tr_companies_missing_tax_office := moves.company_id.partner_id.filtered(
            lambda p: not p.reference and p.country_code == "TR"
        ):
            return {
                "message": self.env._(
                    "The following TR Company(s) must have the reference field set to the tax office name."
                ),
                "action_text": self.env._("View Company(s)"),
    ...
    def _call_web_service_before_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            if "tr_nilvera" in invoice_data["extra_edis"]:
                if attachment_values := invoice_data.get(
                    "ubl_cii_xml_attachment_values"
                ):
                ...
                customer_alias = (
                    invoice._get_partner_l10n_tr_nilvera_customer_alias_name()
                )
