...
class PaymentTransaction(models.Model):
    ...
    def _send_payment_request(self):
        ...
        partner_country_code = (
            ...
            or self.provider_id.company_id.country_id.code
            or "NL"
        )
        data = {
    ...
    def _search_by_reference(self, provider_code, payment_data):
        ...
        event_code = payment_data.get(
            "eventCode", "AUTHORISATION"
        )  # Fallback on auth if S2S.
        ...
        if event_code == "AUTHORISATION":
            ...
            if not tx:  # The refund was initiated from Adyen
                ...
                if source_tx:
                    ...
                    tx = self._adyen_create_child_tx(
                        source_tx, payment_data, is_refund=True
                    )
    ...
    def _adyen_create_child_tx(self, source_tx, payment_data, is_refund=False):
        ...
        if not provider_reference or amount is None:  # amount == 0 if success == False
            _logger.warning(
                "Received data for child transaction with missing transaction values."
            )
    ...
    def _apply_updates(self, payment_data):
        ...
        ):  # Not from webhook: the data contain the PM code.
        ...
        if not payment_state:
            ...
            if event_code in ["AUTHORISATION", "REFUND"]:
                ...
                ):  # child tx => The event can't be retried.
                    self._set_error(
                ...
                if (
                    self.source_transaction_id
                ):  # child_tx => The event can't be retried.
                    self._set_error(
