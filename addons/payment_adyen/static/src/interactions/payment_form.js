...
patch(PaymentForm.prototype, {
    ...
    async _prepareInlineForm(
        ...
        if (flow === "token") {
            ...
            if (paymentMethodCode === "paypal") {
                // PayPal uses its own button to submit.
                this._hideInputs();
            }
        ...
        await this.waitFor(
            loadJS(
                "https://checkoutshopper-live.adyen.com/checkoutshopper/sdk/6.9.0/adyen.js",
            ),
        );
        await this.waitFor(
            loadCSS(
                "https://checkoutshopper-live.adyen.com/checkoutshopper/sdk/6.9.0/adyen.css",
            ),
        );
        ...
        if (!this.adyenCheckout) {
            try {
                ...
                const response = await this.waitFor(
                    rpc("/payment/adyen/payment_methods", {
                ...
                const configuration = {
                    ...
                    locale:
                        pyToJsLocale(document.documentElement.getAttribute("lang")) ||
                        "en-US",
                    countryCode: response["country_code"],
                    environment: providerState === "enabled" ? "live" : "test",
                    onAdditionalDetails:
                        this._adyenOnSubmitAdditionalDetails.bind(this),
        ...
        const adyenContainer = inlineForm.querySelector(
            '[name="o_adyen_component_container"]',
        );
    ...
    async _initiatePaymentFlow(providerCode, paymentOptionId, paymentMethodCode, flow) {
        ...
        if (!this.adyenComponents[paymentOptionId]) {
    ...
    async _adyenOnSubmit(state, component, actions) {
        try {
            ...
            const paymentResponse = await this.waitFor(
                rpc("/payment/adyen/payments", {
            ...
            const error_message =
                error instanceof RPCError ? error.data.message : error.message;
    ...
    async _adyenOnSubmitAdditionalDetails(state, component, actions) {
        try {
            const paymentDetails = await this.waitFor(
