...
class TestHrHolidaysCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_responsible = mail_new_test_user(
            cls.env,
            login="Titus",
            groups="base.group_user,hr_holidays.group_hr_holidays_responsible",
        )
        cls.user_responsible_id = cls.user_responsible.id
        cls.user_employee = mail_new_test_user(
            cls.env, login="enguerran", password="enguerran", groups="base.group_user"
        )
    ...
    def assert_remaining_leaves_equal(
        self, leave_type, value, employee, date=None, digits=None
    ):
        ...
        if digits:
            self.assertAlmostEqual(
                allocation_data[employee][0][1]["remaining_leaves"],
                value,
                digits,
                f"Remaining leaves for date '{date}' are incorrect.",
            )
        else:
            self.assertEqual(
                allocation_data[employee][0][1]["remaining_leaves"],
    ...
    def _create_form_test_accrual_allocation(
        self,
        leave_type,
        date_from,
        employee,
        ...
        with Form(
            allocation, "hr_holidays.hr_leave_allocation_view_form_manager"
        ) as form:
