...
class TestChangeDepartment(TestHrHolidaysCommon):
    def test_employee_change_department_request_change_department(self):
        self.HolidaysEmployeeGroup = self.env["hr.leave"].with_user(
            self.user_employee_id
        )

        HolidayStatusManagerGroup = self.env["hr.leave.type"].with_user(
            ...
            return self.HolidaysEmployeeGroup.create(
                {
                    ...
                    + relativedelta(weekday=(MO(2) if start > 0 else FR(-1)))
                    + relativedelta(days=start),
                    "request_date_to": date.today()
                    + relativedelta(weekday=(MO(2) if start > 0 else FR(-1)))
                    + relativedelta(days=end),
                }
            )
        ...
        self.employee_emp.department_id = self.rd_dept
        self.assertEqual(
            hol2_employee_group.department_id,
            self.hr_dept,
        ...
        self.employee_emp.department_id = self.rd_dept
        self.assertEqual(
            hol22_employee_group.department_id,
            self.rd_dept,
        ...
        self.employee_emp.department_id = self.hr_dept  # Change department
        self.assertEqual(
            hol32_employee_group.department_id,
            self.hr_dept,
