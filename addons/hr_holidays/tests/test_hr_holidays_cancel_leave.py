...
class TestHrHolidaysCancelLeave(TestHrHolidaysCommon):
    ...
    def test_action_cancel_leave(self):
        ...
        self.assertEqual(
            self.holiday.state, "cancel", "The validated leave should be canceled."
        )
    ...
    def test_action_cancel_leave_in_past(self):
        ...
        with self.assertRaises(
            ...
            self.env["hr.holidays.cancel.leave"].with_user(
                self.user_employee
            ).with_context(default_leave_id=self.holiday.id).new(
    ...
    def test_user_cannot_unarchive_leave(self):
        ...
        with self.assertRaises(
            UserError, msg="Only a manager can modify a canceled leave."
        ):
