...
class TestHolidaysCalendar(HttpCase, TestHrHolidaysCommon):
    ...
    def test_hours_time_off_request_calendar_view(self):
        ...
        last_leave = (
            self.env["hr.leave"]
            .search([("employee_id.id", "=", self.employee_emp.id)])
            .sorted(lambda leave: leave.create_date)[-1]
        )
        self.assertEqual(last_leave.date_from.weekday(), 3, "It should be Thursday")
        self.assertEqual(
            last_leave.date_from.hour, expected_leave_start, "Wrong start of the day"
        )
        self.assertEqual(
    ...
    def test_timezone_calendar_event_single_day(self):
        ...
        self.assertEqual(
            leave.meeting_id.start_date,
            test_date,
            f"Meeting start date should be {test_date}",
        )
        self.assertEqual(
            leave.meeting_id.stop_date,
            test_date,
