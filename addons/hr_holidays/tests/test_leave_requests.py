...
class TestLeaveRequests(TestHrHolidaysCommon):
    def _check_holidays_status(self, holiday_status, employee, ml, lt, rl, vrl):
        ...
        self.assertEqual(
            result["leaves_taken"], lt, "hr_holidays: wrong type days computation"
        )
        self.assertEqual(
            result["remaining_leaves"], rl, "hr_holidays: wrong type days computation"
        )
        self.assertEqual(
    ...
    def setUpClass(cls):
        ...
        LeaveType = (
            ...
            .with_user(cls.user_hrmanager_id)
    ...
    def test_limited_type_days_left(self):
        with freeze_time("2022-01-05"):
            ...
            self._check_holidays_status(
                holiday_status, self.employee_emp, 2.0, 0.0, 2.0, 2.0
            )
            ...
            self._check_holidays_status(
                holiday_status, self.employee_emp, 2.0, 0.0, 2.0, 0.0
            )
            ...
            self._check_holidays_status(
                holiday_status, self.employee_emp, 2.0, 2.0, 0.0, 0.0
            )
    ...
    def test_department_leave(self):
        ...
        leave_wizard_form = Form(
            self.env["hr.leave.generate.multi.wizard"].with_user(self.user_hrmanager)
        )
    ...
    def test_allocation_request(self):
        allocation_form = Form(
            self.env["hr.leave.allocation"].with_user(self.user_employee)
        )
    ...
    def test_employee_is_absent(self):
        ...
        (self.employee_emp | self.employee_hrmanager).mapped("is_absent")
        self.assertFalse(
            self.employee_emp.is_absent, "He should not be considered absent"
        )
        self.assertFalse(
            self.employee_hrmanager.is_absent, "He should not be considered absent"
        )
    ...
    def test_timezone_employee_leave_request(self):
        self.employee_emp.tz = "Pacific/Auckland"
    ...
    def test_timezone_company_leave_request(self):
        ...
        company.resource_calendar_id.tz = "Australia/Sydney"
        leave = self.env["hr.leave"].new(
            {
                "employee_id": self.employee_emp.id,
                "holiday_status_id": self.holidays_type_1.id,
    ...
    def test_timezone_company_validated(self):
        ...
        self.assertEqual(
            employee_leave.request_date_from,
            date(2019, 5, 6),
    ...
    def test_several_allocations_split(self):
        ...
        for unit in ["hour", "day"]:
            ...
            allocation_days = self.employee_emp._get_consumed_leaves(
                self.holidays_type_2
            )[0]

            self.assertEqual(
                allocation_days[self.employee_emp][self.holidays_type_2][
                    allocation_4days
                ]["leaves_taken"],
    ...
    def test_time_off_irregular_working_schedule(self):
        ...
        next_tuesday = date_utils.start_of(
            fields.Date.today() + relativedelta(days=7), "week"
    ...
    def test_holiday_type_allocation(self):
        with freeze_time("2020-09-15"):
            ...
            self._check_holidays_count(
                self.employee_emp._get_consumed_leaves(self.holidays_type_2)[0][
                    self.employee_emp
    ...
    def test_archived_allocation(self):
        with freeze_time("2022-09-15"):
            ...
            self._check_holidays_count(
                self.holidays_type_2.get_allocation_data(
                    self.employee_emp, target_date=date(2021, 12, 1)
            ...
            self.assertEqual(
                self.holidays_type_2.get_allocation_data(
                    self.employee_emp, target_date=date(2021, 12, 31)
            ...
            self._check_holidays_count(
                self.holidays_type_2.get_allocation_data(self.employee_emp)[
                    self.employee_emp
                ][0][1],
                ml=20,
                lt=4,
    ...
    def test_current_leave_status(self):
        ...
        def run_validation_flow(leave_validation_type):
            ...
            if leave_validation_type in ("manager", "both"):
                ...
                self.assertEqual(
                    employee.filtered_domain([("is_absent", "=", False)]), employee
                )
            ...
            self.assertEqual(
                employee.filtered_domain([("is_absent", "=", True)]), employee
            )
    ...
    def test_default_request_date_timezone(self):
        self.user_employee.tz = "Asia/Hong_Kong"
        context = {
            "default_date_from": "2024-03-27 23:00:00",
            "default_date_to": "2024-03-28 08:00:00",
        }
        leave_form = Form(
            self.env["hr.leave"].with_user(self.user_employee).with_context(context)
        )
    ...
    def test_filter_time_off_type_multiple_employees(self):
        ...
        search_result = (
            self.env["hr.leave.type"]
            .with_context(employee_id=False)
            .name_search(domain=search_domain)
        )
        self.assertFalse(
            self.holidays_type_2.id in [alloc_id for (alloc_id, _) in search_result]
        )
    ...
    def test_holiday_type_allocation_requirement_edit(self):
        ...
        self.assertFalse(
            ...
            "Allocations should no longer be required",
        )
    ...
    def test_activity_update_with_time_off_officer(self):
        self.holidays_type_1.responsible_ids = False
        ...
        self.assertFalse(
            activities,
            "No activity should be created if no Time Off Officer is set for approval.",
        )
        ...
        self.assertEqual(
            len(activities),
            1,
            "One activity should be created for the Employee's Approver.",
        )
        self.assertEqual(
            activities.activity_type_id,
            self.env.ref("hr_holidays.mail_act_leave_approval"),
        ...
        self.assertEqual(
            activities.user_id,
            self.employee_hrmanager.leave_manager_id,
            "The activity should be assigned to the Employee's Approver.",
        )
    ...
    def test_get_default_leave_type(self):
        ...
        hr_leave_default_value = (
            ...
            .default_get(
                list(self.env["hr.leave"].fields_get()) + ["holiday_status_id"]
            )
        )
        self.assertEqual(
            hr_leave_default_value.get("holiday_status_id"), hour_leave_type.id
        )
        ...
        self.assertEqual(
            hr_leave_default_value.get("holiday_status_id"), half_day_leave_type.id
        )
    ...
    def test_leave_duration_on_public_holiday_with_flexible_request(self):
        ...
        for leave, expected_days, data in zip(
            leaves, expected_days_list, leave_data, strict=False
        ):
            self.assertEqual(
    ...
    def test_coextensive_holidays_one_include_public_leave(self):
        ...
        self.assertEqual(
            sick_leave_hr.duration_display,
            "2 days",
            "hr_holidays: duration_display should update after adding an overlapping holiday",
        )
    ...
    def test_flexible_schedule_full_day_off(self):
        ...
        self.employee_hruser.write(
            {"is_flexible": True, "resource_calendar_id": calendar.id}
        )
