...
class TestHolidaysMultiContract(TestHolidayContract):
    ...
    def test_leave_request_next_contracts(self):
        ...
        self.assertEqual(
            ...
            14,
            "It should count hours according to the future contract.",
        )
    ...
    def test_leave_same_contract_multiple_versions_with_different_schedules(self):
        ...
        leave = self.create_leave(
            datetime(2022, 5, 1, 7, 0, 0),
            datetime(2022, 5, 10, 18, 0, 0),
            name="Doctor Appointment",
            employee_id=self.jules_emp.id,
        )
        leave.action_approve()
        ...
        leave = self.create_leave(
            datetime(2022, 6, 15, 7, 0, 0),
            datetime(2022, 7, 20, 18, 0, 0),
            name="Doctor Appointment",
            employee_id=self.jules_emp.id,
        )
        leave.action_approve()
    ...
    def test_leave_multi_contracts_split(self):
        ...
        first_leave = leaves.filtered(
            lambda l: l.date_from.day == 1 and l.date_to.day == 15
        )
        self.assertEqual(first_leave.state, "validate")
        self.assertEqual(first_leave.number_of_days, 11)

        second_leave = leaves.filtered(
            lambda l: l.date_from.day == 16 and l.date_to.day == 30
        )
