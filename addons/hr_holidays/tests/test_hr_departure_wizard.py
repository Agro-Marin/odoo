...
class TestHolidaysFlow(TestHrHolidaysCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.employee = cls.env["hr.employee"].create({"name": "Sky"})
    ...
    def test_departure_leave_with_departure_date(self):
        ...
        self._check_action_departure()
        message = (
            "<p>End date has been updated because the employee will leave the company on %(departure_date)s.</p>"
            % {"departure_date": self.departure_date}
        )
        self.assertTrue(message in leave.message_ids.mapped("body"))

        cancel_message = (
            "<p>The time off request has been cancelled for the following reason:</p><p>The employee will leave the company on %(departure_date)s.</p>"
            % {"departure_date": self.departure_date}
        )
        self.assertTrue(
            cancel_message
            in self.env["hr.leave"]
            .search(
                [
    ...
    def test_departure_allocation_with_departure_date(self):
        ...
        allocation_msg = (
            "<p>Validity End date has been updated because the employee will leave the company on %(departure_date)s.</p>"
            % {"departure_date": self.departure_date}
        )
    ...
    def _check_employee_leave(self):
        ...
        self.assertFalse(
            any(
                leave.date_to.date() > self.departure_date
                for leave in leaves_before_departure_date
            )
        )
