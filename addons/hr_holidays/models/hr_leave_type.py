    ...
    def _compute_valid(self):
        ...
        for leave_type in self:
            if leave_type.requires_allocation:
                ...
                allocations = allocations.filtered(lambda alloc, allowed_excess=allowed_excess:
        ...
        for employee in employees:
            for leave_type in leave_type_requires_allocation:
                ...
                if closest_expiration_date:
                    ...
                    start_datetime = datetime.combine(target_date, time.min).replace(tzinfo=UTC)
                    end_datetime = datetime.combine(closest_expiration_date, time.max).replace(tzinfo=UTC)
