...
class WebsiteSaleL10nTW(WebsiteSale):
    def _l10n_tw_is_extra_info_needed(self):
        ...
        invoicing_info_needed = invoicing_step.sudo().is_published = (
            order_sudo.company_id._is_ecpay_enabled()
    ...
    def shop_checkout(self, try_skip_step=False, **query_params):
        ...
        return super().shop_checkout(
            try_skip_step=try_skip_step, query_params=query_params
        )
    ...
    def _is_valid_tax_id(self, tax_id, order):
        ...
        response_data = call_ecpay_api(
            "/GetCompanyNameByTaxID", json_data, order.company_id
        )
    ...
    def l10n_tw_invoicing_info_get(self, **kw):
        ...
        default_vals = {
            ...
            "carrier_type": order_sudo.l10n_tw_edi_carrier_type or "1",
        ...
        return request.render(
            "l10n_tw_edi_ecpay_website_sale.l10n_tw_edi_invoicing_info", values
        )
    ...
    def l10n_tw_invoicing_info_post(self, **kw):
        ...
        if kw.get("l10n_tw_edi_is_donate") != "on":
            if kw.get("l10n_tw_edi_carrier_type") == "2" and not kw.get(
                "l10n_tw_edi_carrier_number"
            ):
                errors["carrier_number"] = request.env._(
                    "Please enter the carrier number"
                )
            if kw.get(
                "l10n_tw_edi_carrier_type"
            ) == "3" and not self._is_valid_mobile_barcode(
                kw.get("l10n_tw_edi_carrier_number"), order_sudo
            ):
                errors["carrier_number"] = request.env._("Mobile Barcode is invalid")
            if kw.get("l10n_tw_edi_carrier_type") in ["4", "5"] and (
                not kw.get("l10n_tw_edi_carrier_number")
                or not kw.get("l10n_tw_edi_carrier_number_2")
            ):
                errors["carrier_number"] = request.env._(
                    "Please enter the carrier number and carrier number 2"
                )
        elif kw.get("l10n_tw_edi_is_donate") == "on" and not self._is_valid_love_code(
            kw.get("l10n_tw_edi_love_code"), order_sudo
        ):
        ...
        if not is_donate and "carrier_number" not in errors:
            ...
            if carrier_type in ["2", "3", "4", "5"]:
                vals_to_write["l10n_tw_edi_carrier_number"] = default_vals.get(
                    "carrier_number"
                )

            if carrier_type in ["4", "5"]:
                vals_to_write["l10n_tw_edi_carrier_number_2"] = default_vals.get(
                    "carrier_number_2"
                )
    ...
    def check_mobile_barcode(self, sale_order_id, **kwargs):
        try:
            order = CustomerPortal._document_check_access(
    ...
    def _prepare_address_form_values(
        ...
        if order_sudo:
            rendering_values["l10n_tw_edi_require_paper_format"] = (
                order_sudo.partner_id.l10n_tw_edi_require_paper_format
            )
    ...
    def _validate_address_values(
        ...
        if (
            ...
            and request.website.sudo().company_id.country_id.code == "TW"
            and request.website.sudo().company_id._is_ecpay_enabled()
        ):
            phone = address_values.get("phone")
            if phone:
                formatted_phone = request.env["account.move"]._reformat_phone_number(
                    phone
                )
    ...
    def _handle_extra_form_data(self, extra_form_data, address_values):
        ...
        if (
            request.website.sudo().company_id.country_id.code == "TW"
            and request.website.sudo().company_id._is_ecpay_enabled()
        ):
            order_sudo = request.cart
            if extra_form_data.get("company_name"):
                ...
                l10n_tw_edi_is_print = (
                    extra_form_data.get("l10n_tw_edi_require_paper_format") == "1"
                )
