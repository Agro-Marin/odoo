...
class TestStockLandedCostsMrp(ValuationReconciliationTestCommon):
    ...
    def test_landed_cost_on_mrp(self):
        ...
        # Check that the finished move was already valued before the landed cost
        ...
        # Link to one adjustment line for product_refrigerator
        self.assertEqual(len(landed_cost.valuation_adjustment_lines), 1)
        self.assertEqual(
            landed_cost.valuation_adjustment_lines.product_id,
            self.product_refrigerator,
        )
        self.assertEqual(
            landed_cost.valuation_adjustment_lines.additional_landed_cost, 5.0
        )
    ...
    def test_landed_cost_on_mrp_02(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.product_component1, self.warehouse_1.lot_stock_id, 10
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product_component2, self.warehouse_1.lot_stock_id, 10
        )
        ...
        # Check that the MO's finished move was already valued
    ...
    def test_landed_cost_on_mrp_03(self):
        ...
        self.assertFalse(
            byproduct2 in landed_cost.valuation_adjustment_lines.product_id
        )
