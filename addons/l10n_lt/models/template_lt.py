...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _setup_utility_bank_accounts(self, template_code, company, template_data):
        ...
        if template_code == "lt":
            ...
            other_operating_results_tags = self.env.ref(
                "l10n_lt.account_account_tag_6_other_operating_results"
            )
            company.default_cash_difference_income_account_id.tag_ids |= (
                other_operating_results_tags
            )
            company.default_cash_difference_expense_account_id.tag_ids |= (
                other_operating_results_tags
            )
