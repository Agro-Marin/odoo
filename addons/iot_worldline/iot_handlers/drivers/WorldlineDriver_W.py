...
class WorldlineDriver(CtypesTerminalDriver):
    ...
    def processTransaction(self, transaction):
        ...
        try:
            result = easyCTEP.startTransaction(
                ...
                ctypes.c_char_p(
                    str(transaction_amount).encode("utf-8")
                ),  # char const* amount
                ctypes.c_char_p(
                ...
                ctypes.c_ulong(
                    transaction_action_identifier
                ),  # unsigned long action_identifier
            ...
            if result == 1:
                ...
                # Transaction was cancelled
                    ...
                    _logger.info(
                        "transaction #%d cancelled by PoS user", transaction_id
                    )
            ...
            _logger.exception(
                "Failed to perform Worldline transaction. Check for potential segmentation faults"
            )
            self.send_status(
    ...
    def cancelTransaction(self, transaction):
        ...
        try:
            result = easyCTEP.abortTransaction(
                ctypes.cast(self.dev, ctypes.c_void_p), error_code
            )
            ...
            if not result:
                ...
                error_msg = "%s (Error code: %s)" % (
                    TERMINAL_ERRORS.get(
                        error_code, "Transaction could not be cancelled"
                    ),
                    error_code,
                )
                _logger.info(error_msg)
                self.send_status(
                    stage="Cancel", error=error_msg, request_data=transaction
                )
        except OSError:
            _logger.exception(
                "Failed to cancel Worldline transaction. Check for potential segmentation faults."
            )
            self.send_status(
                stage="Cancel",
