...
if not path_file(
    "odoo/addons/iot_drivers/iot_handlers/lib/ctep/libeasyctep.so"
).exists():
    load_worldline_library_script = path_file(
        "odoo/addons/iot_drivers/iot_handlers/lib/load_worldline_library.sh"
    )
    try:
        subprocess.run(["sudo", "sh", load_worldline_library_script], check=True)
    except subprocess.CalledProcessError:
        _logger.exception(
            "An error encountered while downloading / setting up Worldline CTEP library"
        )
...
easyCTEP.connectedTerminal.argtypes = [
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.c_void_p,
]
