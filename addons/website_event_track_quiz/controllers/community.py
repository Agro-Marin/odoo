...
class WebsiteEventTrackQuizCommunityController(EventCommunityController):
    ...
    def community(self, event, **kwargs):
        ...
        return request.render(
            ...
            values | {"seo_object": event.community_menu_ids},
        )
    ...
    def _get_community_leaderboard_render_values(self, event, search_term, page):
        ...
        if user_count:
            ...
            url = "/event/%s/community/leaderboard/results" % (
                request.env["ir.http"]._slug(event)
            )
            if values.get("current_visitor_position") and not page:
                values["scroll_to_position"] = True
                page = math.ceil(
                    values["current_visitor_position"] / self._visitors_per_page
                )
            elif not page:
                page = 1
    ...
    def _get_leaderboard(self, event, searched_name=None):
        ...
        track_visitor_data = (
            request.env["event.track.visitor"]
            .sudo()
            ._read_group(
                [
                ...
                ["visitor_id"],
                ["quiz_points:sum"],
                order="quiz_points:sum DESC, visitor_id ASC",
            )
        )
        ...
        for visitor_id, points in data_map.items():
            ...
            if not visitor:
                continue
            if (
                searched_name and searched_name.lower() in visitor.display_name.lower()
            ) or not searched_name:
                leaderboard.append(
                    {"visitor": visitor, "points": points, "position": position}
                )
