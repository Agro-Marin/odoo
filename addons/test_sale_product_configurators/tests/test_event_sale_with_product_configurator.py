...
class TestEventProductConfiguratorUi(AccountTestInvoicingCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.event_product_template.attribute_line_ids[0].product_template_value_ids[
            0
        ].price_extra = -20.00
        cls.event_product_template.attribute_line_ids[0].product_template_value_ids[
            2
        ...
        for variant in cls.event_product_template.attribute_line_ids[
            0
            ...
            cls.event_product_template.optional_product_ids = [
                cls.product_product_memorabilia.id,
            ]

    def test_event_using_product_configurator(self):
        self.start_tour(
            "/odoo", "event_sale_with_product_configurator_tour", login="salesman"
        )

        sale_order = self.env["sale.order"].search(
            [("create_uid", "=", self.salesman.id)]
        )
