...
registry.category("web_tour.tours").add("sale_product_configurator_advanced_tour", {
    ...
    steps: () => [
        ...
        {
            trigger:
                ".o_field_widget[name=partner_shipping_id] .o_external_button:not(:visible)", // Wait for onchange_partner_id
        },
        ...
        configuratorTourUtils.assertProductNameContains(
            "Custom, White, PAV9, PAV5, PAV1",
        ),
        {
            trigger: configuratorTourUtils.optionalProductSelector(
        ...
        configuratorTourUtils.selectAttribute("Conference Chair", "Legs", "Aluminium"),
        {
            ...
            run({ queryOne }) {
                const newOptionVariantImage =
                    configuratorTourUtils.optionalProductImageSrc(
                        queryOne,
                        "Conference Chair (TEST) (Aluminium)",
                    );
