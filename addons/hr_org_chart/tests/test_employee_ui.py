...
class TestEmployeeUi(TestHrCommon, HttpCase):
    ...
    def test_indirect_subordinates(self):
        ...
        self.start_tour(
            f"/odoo/employees/{self.employee_georges.id}",
            "indirect_subordinates_tour",
            login="admin",
        )
        indirect_subordinates = len(
            self.employee_georges.subordinate_ids - self.employee_georges.child_ids
        )
        self.assertEqual(
    ...
    def test_employee_view_access_multicompany(self):
        ...
        self.env = self.env(
            context=dict(
                self.env.context, allowed_company_ids=[company_0.id, company_1.id]
            )
        )

        employee_a = (
            self.env["hr.employee"]
            .with_company(company_0)
            .create(
                {
                    "name": "Employee A",
                }
        ...
        self.start_tour(
            "/odoo/employees",
            "employee_view_access_multicompany",
            login="restricted_user",
        )
