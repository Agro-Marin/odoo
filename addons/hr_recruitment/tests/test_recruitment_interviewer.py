...
class TestRecruitmentInterviewer(MailCase):
    ...
    def test_interviewer_group(self):
        ...
        self.assertFalse(
            interviewer_group.id in self.simple_user.all_group_ids.ids,
            "Simple User should not be interviewer",
        )

        self.job.interviewer_ids = self.simple_user.ids
        self.assertTrue(
            interviewer_group.id in self.simple_user.all_group_ids.ids,
            "Simple User should be added as interviewer",
        )

        self.job.write({"interviewer_ids": [(5, 0, 0)]})
        self.assertFalse(
            interviewer_group.id in self.simple_user.all_group_ids.ids,
            "Simple User should be removed from interviewer",
        )
        ...
        self.assertTrue(
            interviewer_group.id in self.simple_user.all_group_ids.ids,
            "Simple User should stay interviewer",
        )
    ...
    def test_update_interviewer_for_multiple_applicants(self):
        ...
        interviewer_user_1 = new_test_user(
            ...
            "sma",
            groups="base.group_user,hr_recruitment.group_hr_recruitment_interviewer",
            name="Recruitment Interviewer1",
            email="sma@example.com",
        )

        interviewer_user_2 = new_test_user(
            self.env,
            "jab",
            groups="base.group_user,hr_recruitment.group_hr_recruitment_interviewer",
            name="Recruitment Interviewer2",
            email="jab@example.com",
        )

        interviewer_user_3 = new_test_user(
            self.env,
            "aad",
            groups="base.group_user,hr_recruitment.group_hr_recruitment_interviewer",
            name="Recruitment Interviewer3",
            email="aad@example.com",
        )
        ...
        applicants = applicant + applicant.copy(
        ...
        self.assertEqual(
            message.subject,
            f"You have been assigned as an interviewer for {applicant.display_name}",
        )

    def test_update_recruiter_for_ongoing_application(self):
        Application = self.env["hr.applicant"]
        new_manager_user = new_test_user(
            self.env,
            "thala",
            groups="base.group_user,hr_recruitment.group_hr_recruitment_manager",
            name="New Recruitment Manager",
