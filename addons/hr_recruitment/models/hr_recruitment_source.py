    ...
    def _compute_has_domain(self):
        for source in self:
            if source.alias_id:
                ...
                source.has_domain = bool(
                    source.job_id.company_id.alias_domain_id
    ...
    def create_alias(self):
        ...
        for source in self.filtered(lambda s: not s.alias_id):
            vals = {
                ...
                "alias_domain_id": source.job_id.company_id.alias_domain_id.id
                or self.env.company.alias_domain_id.id,
