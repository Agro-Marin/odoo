...
class TestPartnerAssign(TransactionCase):
    ...
    def test_partner_assign(self):
        ...
        self.assertTrue(
            3 < partner_be.partner_longitude < 5,
        ...
        self.assertTrue(
            50 < lead.partner_latitude < 55,
            "Latitude is wrong: 50 < %s < 55" % lead.partner_latitude,
        )
        self.assertTrue(
            -4 < lead.partner_longitude < -1,
            "Longitude is wrong: -4 < %s < -1" % lead.partner_longitude,
        )
        self.assertTrue(
...
class TestPartnerLeadPortal(TestCrmCommon, HttpCase):
    ...
    def test_partner_lead_decline(self):
        ...
        self.lead_portal.with_user(self.user_portal).partner_desinterested(
            comment="No thanks, I have enough leads !", contacted=True, spam=False
        )

        self.assertFalse(
            self.lead_portal.partner_assigned_id.id,
            "The partner_assigned_id of the declined lead should be False.",
        )
        self.assertTrue(
            self.user_portal.partner_id in self.lead_portal.sudo().partner_declined_ids,
    ...
    def test_partner_lead_decline_spam(self):
        ...
        self.assertIn(
            self.env.ref("website_crm_partner_assign.tag_portal_lead_is_spam"),
            self.lead_portal.tag_ids,
            "The lead must be tagged as spam.",
        )
    ...
    def test_lead_creation(self):
        ...
        self.assertEqual(
            opportunity.partner_assigned_id,
    ...
    def test_lead_update(self):
        ...
        self.assertEqual(
            ...
            9999.0,
            "Portal user should be able to update revenue or other details via portal method",
        )
        ...
        self.assertEqual(
            opportunity.email_from,
            email_2,
            "Address email on the opportunity must be updated",
        )
        self.assertEqual(
            test_partner.email,
            email_1,
            "Adress email on the partner should not be updated",
        )
    ...
    def test_portal_mixin_url(self):
        record_action = self.lead_portal._get_access_action(
            access_uid=self.user_portal.id
        )
        self.assertEqual(
            record_action["url"], "/my/opportunity/%s" % self.lead_portal.id
        )
    ...
    def test_route_portal_my_opportunities_as_portal(self):
        ...
        with (
            self.with_user(self.user_portal.login),
            MockRequest(
    ...
    def test_03_crm_partner_assign_geolocalization(self, GeoIpMock):
        ...
        with MockRequest(
            ...
            self.assertEqual(
                [b"rendered"], res.response, "render_function wasn't called"
            )
...
class TestPublish(HttpCase):
    ...
    def test_01_admin(self):
        self.start_tour(
            self.env["website"].get_client_action_url("/partners"),
            "test_can_publish_partner",
            login="admin",
        )
        self.assertTrue(
            self.partner.website_published, "Partner should have been published"
        )
    ...
    def test_04_not_reditor_salesman(self):
        ...
        self.assertNotIn(
            self.group_restricted_editor.id,
            self.user_test.group_ids.ids,
