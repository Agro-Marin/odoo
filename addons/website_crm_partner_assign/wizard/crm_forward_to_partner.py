...
class CrmLeadForwardToPartner(models.TransientModel):
        ...
        if active_ids:
            ...
            if default_composition_mode == "mass_mail":
                ...
                partner_assigned_dict = {
                    lead.id: lead.partner_assigned_id.id for lead in leads
                }
    ...
    def action_forward(self):
        ...
        for lead in self.assignation_lines:
            partner = (
                self.forward_type == "single" and self.partner_id
            ) or lead.partner_assigned_id
            lead_details = {
        ...
        for partner_id, partner_leads in partners_leads.items():
            in_portal = False
            if portal_group:
                for contact in (partner.child_ids or partner).filtered(
                    lambda contact: contact.user_ids
                ):
                    in_portal = portal_group.id in [
                        g.id for g in contact.user_ids[0].all_group_ids
                    ]
