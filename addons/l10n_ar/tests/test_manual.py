...
from odoo.libs.numbers import float_split_str
...
class TestArManual(common.TestAr):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_01_create_invoice(self):
        ...
        * Proper fiscal position (this case not fiscal position is selected)
        ...
        self.assertEqual(
            invoice.company_id, self.company_ri, "created with wrong company"
        )
        self.assertEqual(invoice.amount_tax, 21, "invoice taxes are not properly set")
        self.assertEqual(
            invoice.amount_total,
            121.0,
            "invoice taxes has not been applied to the total",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            invoice.name,
            "FA-A %05d-00000001" % self.journal.l10n_ar_afip_pos_number,
            "Invoice number is wrong",
        )
        self.assertEqual(
            invoice.sequence_prefix, "FA-A %05d-" % self.journal.l10n_ar_afip_pos_number
        )
    ...
    def test_02_fiscal_position(self):
        ...
        self.assertFalse(
            invoice.fiscal_position_id, "Fiscal position should be set to empty"
        )
        ...
        self.assertEqual(
            ...
            self._search_fp("Purchases / Sales Free Trade Zone"),
        )
        ...
        self.assertEqual(
            invoice.fiscal_position_id, self._search_fp("Purchases / Sales abroad")
        )
    ...
    def test_15_liquido_producto_sales(self):
        ...
        self.journal = self._create_journal(
            "preprinted", data={"l10n_ar_is_pos": False}
        )
        ...
        with self.assertRaisesRegex(
            AssertionError, "l10n_latam_document_number is a required field"
        ):
            with Form(
                self.env["account.move"].with_context(default_move_type="out_invoice")
            ) as invoice_form:
                invoice_form.ref = (
                    "demo_liquido_producto_1: Vendor bill liquido producto (DOC 186)"
                )
    ...
    def test_16_liquido_producto_purchase(self):
        ...
        purchase_pos_journal = self._create_journal(
            "preprinted", data={"type": "purchase", "l10n_ar_is_pos": True}
        )
    ...
    def test_17_corner_cases(self):
        ...
        for line in invoice_line_ids:
            ...
            _unitary_part, l10n_ar_price_unit_decimal_part = float_split_str(
                l10n_ar_line_prices["price_unit"], decimal_price_digits_setting
            )
            len_l10n_ar_price_unit_digits = len(l10n_ar_price_unit_decimal_part)
            _unitary_part, line_price_unit_decimal_part = float_split_str(
                line.price_unit, decimal_price_digits_setting
            )
            len_line_price_unit_digits = len(line_price_unit_decimal_part)
            if (
                len_l10n_ar_price_unit_digits
                == len_line_price_unit_digits
                == decimal_price_digits_setting
            ):
                self.assertEqual(
                    l10n_ar_price_unit_decimal_part, line_price_unit_decimal_part
                )
    ...
    def test_18_invoice_b_tax_breakdown_1(self):
        ...
        invoice = self._create_invoice_ar(
            ...
            invoice_line_ids=[
                self._prepare_invoice_line(
                    ...
                    tax_ids=self.tax_21 + self.tax_perc_iibb,
                ),
                self._prepare_invoice_line(
                    product_id=self.service_iva_27,
                    price_unit=2250.0,
                    tax_ids=self.tax_27 + self.tax_national,
                ),
                self._prepare_invoice_line(
                    product_id=self.product_iva_105_perc,
                    price_unit=1740.0,
                    tax_ids=self.tax_10_5 + self.tax_internal,
                ),
                self._prepare_invoice_line(
                    product_id=self.product_iva_105_perc,
                    price_unit=10000.0,
                    tax_ids=self.tax_0 + self.tax_other,
                ),
            ],
        )
        ...
        self._assert_tax_totals_summary(
            invoice._l10n_ar_get_invoice_totals_for_report(),
            {
                "same_tax_base": False,
        ...
        invoice = self._create_invoice_ar(
            ...
            invoice_line_ids=[
                self._prepare_invoice_line(
                    product_id=self.product_iva_105_perc,
                    price_unit=10000.0,
                    tax_ids=self.tax_no_gravado + self.tax_internal,
                )
            ],
        )
        results = invoice._l10n_ar_get_invoice_custom_tax_summary_for_report()
    ...
    def test_l10n_ar_vat_with_non_numeric_value(self):
        with self.assertRaises(ValidationError) as e:
            with Form(self.partner) as partner_form:
                partner_form.l10n_latam_identification_type_id = self.env.ref(
                    "l10n_ar.it_dni"
                )
    ...
    def test_l10n_ar_rounding_01(self):
        ...
        invoice_a = self._create_invoice_ar(
            ...
            invoice_line_ids=[
                self._prepare_invoice_line(price_unit=124, tax_ids=tax_02)
            ],
        )
        self.assertEqual(
            invoice_a.amount_total,
            invoice_a.invoice_line_ids.price_total,
            "The invoice total should match the line total since there is only one line.",
        )

        tax_lines_a = invoice_a.line_ids.filtered(lambda x: x.tax_line_id).sorted(
            lambda x: (
                x.move_id.id,
                x.tax_line_id.id,
                x.tax_ids.ids,
        ...
        self.assertEqual(
            invoice_b.amount_total,
            invoice_b.invoice_line_ids.price_total,
            "The invoice total should match the line total since there is only one line.",
        )

        tax_lines_b = invoice_b.line_ids.filtered(lambda x: x.tax_line_id).sorted(
            lambda x: (
                x.move_id.id,
                x.tax_line_id.id,
                x.tax_ids.ids,
        ...
        self.assertEqual(
            tax_lines_a.balance,
            tax_lines_b.balance,
            "Tax balances should be equal since both invoices have a single line and the total matches the line amount.",
        )
