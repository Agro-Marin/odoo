...
class TestAr(AccountTestInvoicingCommon):
        ...
        cls.company_ri.partner_id.write(
            {
                ...
                "zip": "2000",
                "phone_ids": [
                    Command.create({"number": "+1 555 123 8069", "type": "landline"})
                ],
        ...
        cls.res_partner_adhoc = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create(
                        {"number": "(+54) (341) 208 0203", "type": "landline"}
                    )
                ],
        ...
        cls.res_partner_gritti_mono = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create(
                        {"number": "(+54) (341) 111 2222", "type": "landline"}
                    )
                ],
        ...
        cls.res_partner_montana_sur = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create(
                        {"number": "(+54) (297) 4876 123", "type": "landline"}
                    )
                ],
        ...
        cls.res_partner_servicios_globales = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create(
                        {"number": "(+54) (341) 333 4444", "type": "landline"}
                    )
                ],
        ...
        cls.res_partner_barcelona_food = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create(
                        {"number": "(+00) (11) 222 3333", "type": "landline"}
                    )
                ],
        ...
        cls.partner_mipyme = cls.env["res.partner"].create(
            {
                ...
                "phone_ids": [
                    Command.create({"number": "(123)-456-7890", "type": "landline"})
                ],
        ...
        cls.tax_national = cls.env["account.tax"].create(
            {
                ...
                "tax_group_id": cls.env.ref(
                    f"account.{cls.company_ri.id}_tax_group_national_taxes"
                ).id,
            }
        )
        cls.tax_internal = cls.env["account.tax"].create(
            {
                ...
                "company_ids": [Command.set(cls.company_ri.ids)],
                "tax_group_id": cls.env.ref(
                    f"account.{cls.company_ri.id}_tax_impuestos_internos"
                ).id,
            }
        )
        cls.tax_other = cls.env["account.tax"].create(
            {
                ...
                "company_ids": [Command.set(cls.company_ri.ids)],
                "tax_group_id": cls.env.ref(
                    f"account.{cls.company_ri.id}_tax_group_otros_impuestos"
                ).id,
            }
        )

        cls.tax_21_purchase = cls._search_tax(cls, "iva_21", type_tax_use="purchase")
        cls.tax_no_gravado_purchase = cls._search_tax(
            cls, "iva_no_gravado", type_tax_use="purchase"
        )
    ...
    def _get_ar_multi_invoice_line_ids(cls):
        return [
            cls._prepare_invoice_line(
                product_id=cls.product_iva_105, price_unit=642.0, quantity=5
            ),
            cls._prepare_invoice_line(
                product_id=cls.service_iva_27, price_unit=250.0, quantity=1
            ),
            cls._prepare_invoice_line(
                product_id=cls.product_iva_105_perc, price_unit=3245.0, quantity=2
            ),
            cls._prepare_invoice_line(
                product_id=cls.product_no_gravado, price_unit=50.0, quantity=10
            ),
            cls._prepare_invoice_line(
                product_id=cls.product_iva_cero, price_unit=200.0, quantity=1
            ),
            cls._prepare_invoice_line(
                product_id=cls.product_iva_exento, price_unit=100.0, quantity=1
            ),
        ]
    ...
    def _create_test_invoices_like_demo(cls, use_current_date=True):
        ...
        payment_term = cls.env.ref("payment_term.account_payment_term_end_following_month")
        ...
        test_invoices_map = {
            ...
            "test_invoice_2": {
                ...
                "invoice_line_ids": [
                    ...
                    cls._prepare_invoice_line(
                        product_id=cls.product_iva_105_perc,
                        price_unit=3245.0,
                        quantity=2,
                    ),
                ],
            ...
            "test_invoice_4": {
                ...
                "invoice_line_ids": [
                    cls._prepare_invoice_line(
                        product_id=cls.product_iva_105, price_unit=642.1234, quantity=5
                    ),
                    cls._prepare_invoice_line(
                        product_id=cls.product_iva_exento,
                        price_unit=100.5678,
                        quantity=1,
                    ),
                ],
            ...
            "test_invoice_8": {
                ...
                "invoice_line_ids": [
                    cls._prepare_invoice_line(
                        product_id=cls.service_iva_21, price_unit=642.0, quantity=1
                    ),
                ],
            },
            "test_invoice_10": {
                ...
                "invoice_line_ids": [
                    cls._prepare_invoice_line(
                        product_id=cls.product_iva_105, price_unit=1000.0, quantity=5
                    ),
                ],
        ...
        if use_current_date:
            for invoice_key, invoice_values in test_invoices_map.items():
                ...
                cls.demo_invoices[invoice_key] = cls._create_invoice_ar(
                    **invoice_values
                )
        else:
            for key, values in test_invoices_map.items():
                values["invoice_line_ids"] = [
                    line_data for _, _, line_data in values["invoice_line_ids"]
                ]
                with Form(
                    cls.env["account.move"].with_context(
                        default_move_type=values["move_type"]
                    )
                ) as invoice_form:
                    invoice_form.ref = values["ref"]
                    invoice_form.partner_id = values["partner_id"]
                    invoice_form.invoice_payment_term_id = values[
                    ...
                    for line in values["invoice_line_ids"]:
                        with invoice_form.invoice_line_ids.new() as line_form:
                            line_form.product_id = cls.env["product.product"].browse(
                                line.get("product_id")
                            )
                            line_form.price_unit = line.get("price_unit")
                            line_form.quantity = line.get("quantity")
                            if line.get("tax_ids"):
                                line_form.tax_ids = cls.env["account.tax"].browse(
                                    line.get("tax_ids")
                                )
                            line_form.name = "xxxx"
                            line_form.account_id = cls.company_data[
    ...
    def _create_journal(cls, afip_ws, data=None):
        ...
        values = {
            "name": "%s %s" % (afip_ws.replace("WS", ""), pos_number),
            "type": "sale",
            "code": pos_number,
            "l10n_ar_afip_pos_system": cls._get_afip_pos_system_real_name().get(
                afip_ws
            ),
        ...
        _logger.info(
            "Created journal %s for company %s", journal.name, cls.env.company.name
        )
        return journal
    ...
    def _create_invoice_ar(cls, **invoice_args):
        ...
        invoice_args.setdefault(
            "invoice_line_ids",
            [cls._prepare_invoice_line(price_unit=100, product_id=cls.product_iva_21)],
        )
            ...
            .search(
                [
                    ...
                    (
                        ...
                        "=",
                        self.env.ref(
                            f"account.{self.env.company.id}_tax_group_{tax_type}"
                        ).id,
                    ),
                ],
                limit=1,
    ...
    def _prepare_multicurrency_values(self):
        ...
        self.env.user.write(
            {"group_ids": [(4, self.env.ref("base.group_multi_currency").id)]}
        )
