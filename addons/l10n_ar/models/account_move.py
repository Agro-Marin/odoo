...
class AccountMove(models.Model):
    ...
    def _is_manual_document_number(self):
        ...
        * if purchase journal and not a ARCA pos (regular case of vendor bills)
        ...
        return (
            ...
            and self.journal_id.type in ["purchase", "sale"]
            and not self.journal_id.l10n_ar_is_pos
        )
    ...
    def _check_moves_use_documents(self):
        ...
        not_invoices = self.filtered(
            lambda x: (
                x.company_id.tax_config_id.account_fiscal_country_id.code == "AR"
    ...
    def _check_invoice_type_document_type(self):
        ...
        docs_used_for_inv_and_ref = self.filtered(
            lambda x: (
                ...
                in self._get_l10n_ar_codes_used_for_inv_and_ref()
                and x.move_type in ["out_refund", "in_refund"]
            )
        )
    ...
    def _get_concept(self):
        ...
        is_zona_franca = (
            ...
            == self.env.ref("l10n_ar.res_IVA_LIB")
        )
        ...
        if expo_invoice and is_zona_franca:
            ...
            product_types - consumable and product_types - service and not expo_invoice
        ):
            afip_concept = "3"
        return afip_concept
    ...
    def _get_domain_l10n_latam_documents(self):
        ...
        if (
            self.journal_id.company_id.tax_config_id.account_fiscal_country_id.code
            == "AR"
        ):
    ...
    def _check_argentinean_invoice_taxes(self):
        ...
        for inv in self.filtered(lambda x: x.company_id.l10n_ar_company_requires_vat):
            ...
            if (
                ...
                and inv.l10n_latam_document_type_id.purchase_aliquots == "zero"
            ):
                purchase_aliquots = "zero"
            for line in inv.mapped("invoice_line_ids").filtered(
                lambda x: x.display_type not in NON_ACCOUNTABLE_DISPLAY_TYPES
            ):
                ...
                if len(vat_taxes) != 1:
                    raise UserError(
                        self.env._(
                            "There should be a single tax from the “VAT“ tax group per line, but this is not the case for line “%s”. Please add a tax to this line or check the tax configuration's advanced options for the corresponding field “Tax Group”.",
                            line.name,
                        )
                    )
                ...
                if (
                    purchase_aliquots == "zero"
                    and vat_taxes.tax_group_id.l10n_ar_vat_afip_code != "0"
                ):
                    raise UserError(
                        self.env._(
                            "On invoice id “%s” you must use VAT Not Applicable on every line.",
                            inv.id,
                        )
                    )
                if (
    ...
    def _set_afip_service_dates(self):
        for rec in self.filtered(
            lambda m: m.invoice_date and m.l10n_ar_afip_concept in ["2", "3", "4"]
        ):
            ...
            if not rec.l10n_ar_afip_service_end:
                rec.l10n_ar_afip_service_end = rec.invoice_date + relativedelta(
    ...
    def _set_afip_responsibility(self):
        ...
        for rec in self:
            rec.l10n_ar_afip_responsibility_type_id = (
                rec.commercial_partner_id.l10n_ar_afip_responsibility_type_id.id
            )

    @api.onchange("partner_id")
    def _onchange_afip_responsibility(self):
        if (
            self.company_id.tax_config_id.account_fiscal_country_id.code == "AR"
            ...
            return {
                "warning": {
                    "title": self.env._("Missing Partner Configuration"),
                    "message": self.env._(
    ...
    def _onchange_partner_journal(self):
        ...
        for rec in self.filtered(
            ...
            if (
                ...
                journal = journal.search(
                    domain + [("l10n_ar_afip_pos_system", "not in", expo_journals)],
                    limit=1,
                )
                msg = self.env._(
            ...
            if journal:
                ...
                raise RedirectWarning(msg, action.id, self.env._("Go to Journals"))
    ...
    def _compute_l10n_latam_document_type_id(self):
        ...
        doctype_fa_exterior = self.env.ref(
            "l10n_ar.fa_exterior", raise_if_not_found=False
        )
    ...
    @api.onchange(
        "l10n_latam_document_type_id", "l10n_latam_document_number", "partner_id"
    )
    def _inverse_l10n_latam_document_number(self):
        ...
        for rec in to_review:
            number = rec.l10n_latam_document_type_id._format_document_number(
    ...
    def _get_starting_sequence(self):
        ...
        if (
            self.l10n_latam_use_documents
            and self.company_id.tax_config_id.account_fiscal_country_id.code == "AR"
        ):
    ...
    def _l10n_ar_get_amounts(self, company_currency=False):
        ...
        vat_taxes = tax_lines.filtered(
            lambda r: r.tax_line_id.tax_group_id.l10n_ar_vat_afip_code
        )
        ...
        if not profits_tax_group:
            raise RedirectWarning(
                ...
                button_text=self.env._("Accounting Settings"),
            )

        return {
            "vat_amount": sign * sum(vat_taxes.mapped(amount_field)),
            ...
            "vat_exempt_base_amount": sign
            * sum(
                self.invoice_line_ids.filtered(
                    lambda x: x.tax_ids.filtered(
                        lambda y: y.tax_group_id.l10n_ar_vat_afip_code == "2"
                    )
                ).mapped(amount_field)
            ),
            "vat_untaxed_base_amount": sign
            * sum(
                self.invoice_line_ids.filtered(
                    lambda x: x.tax_ids.filtered(
                        lambda y: y.tax_group_id.l10n_ar_vat_afip_code == "1"
                    )
                ).mapped(amount_field)
            ),
            # used on FE
            "not_vat_taxes_amount": sign
            * sum((tax_lines - vat_taxes).mapped(amount_field)),
            # used on BFE + TXT
            "iibb_perc_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: (
                        r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "07"
                    )
                ).mapped(amount_field)
            ),
            "mun_perc_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: (
                        r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "08"
                    )
                ).mapped(amount_field)
            ),
            "intern_tax_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: (
                        r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "04"
                    )
                ).mapped(amount_field)
            ),
            "other_taxes_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: (
                        r.tax_line_id.tax_group_id.l10n_ar_tribute_afip_code == "99"
                    )
                ).mapped(amount_field)
            ),
            "profits_perc_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: r.tax_line_id.tax_group_id == profits_tax_group
                ).mapped(amount_field)
            ),
            "vat_perc_amount": sign
            * sum(
                tax_lines.filtered(
                    lambda r: (
    ...
    def _get_vat(self):
        ...
        for tax_group in vat_taxable.mapped("tax_group_id"):
            ...
            res += [
                {
                    "Id": tax_group.l10n_ar_vat_afip_code,
        ...
        vat_base_0 = sum(
            self.invoice_line_ids.filtered(
                lambda x: x.tax_ids.filtered(
                    lambda y: y.tax_group_id.l10n_ar_vat_afip_code == "3"
                )
    ...
    def _l10n_ar_get_invoice_custom_tax_summary_for_report(self):
        ...
        def grouping_function(base_line, tax_data):
            ...
            if self._l10n_ar_is_tax_group_other_national_ind_tax(tax_group):
                name = self.env._(
                    "Other National Ind. Taxes %s", base_line["currency_id"].symbol
                )
            elif self._l10n_ar_is_tax_group_vat(tax_group):
                name = self.env._("VAT Content %s", base_line["currency_id"].symbol)
            else:
                skip = True
            return {
                "name": name,
        ...
        for grouping_key, values in values_per_grouping_key.items():
            if grouping_key and not grouping_key["skip"]:
                results.append(
                    {
                        "name": grouping_key["name"],
                        "tax_amount_currency": values["tax_amount_currency"],
                        "formatted_tax_amount_currency": formatLang(
                            self.env, values["tax_amount_currency"]
                        ),
                    }
                )
        return results
