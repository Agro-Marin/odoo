...
class ResPartner(models.Model):
    ...
    def _compute_l10n_ar_vat(self):
        ...
        one is not found"""
        recs_ar_vat = self.filtered(
            lambda x: (
                x.l10n_latam_identification_type_id.l10n_ar_afip_code == "80" and x.vat
            )
        )
        for rec in recs_ar_vat:
    ...
    def _run_check_identification(self, validation="error"):
        ...
        return super(ResPartner, self - l10n_ar_partners)._run_check_identification(
    ...
    def _get_id_number_sanitize(self):
        ...
        if self.l10n_latam_identification_type_id.l10n_ar_afip_code in ["80", "86"]:
            ...
            id_number = re.sub(r"[^0-9]", "", self.vat)
