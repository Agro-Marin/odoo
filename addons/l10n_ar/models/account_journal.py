...
class AccountJournal(models.Model):
    ...
    def _get_journal_letter(self, counterpart_partner=False):
        ...
        if not self.company_id.l10n_ar_afip_responsibility_type_id:
            ...
            msg = self.env._(
                "Can not create chart of account until you configure your company ARCA Responsibility and VAT."
            )
        ...
        if counterpart_partner:
            counterpart_letters = letters_data[
                "issued" if not self.l10n_ar_is_pos else "received"
            ].get(counterpart_partner.l10n_ar_afip_responsibility_type_id.code, [])
    ...
    def _check_afip_pos_system(self):
        journals = self.filtered(
            lambda j: (
                j.l10n_ar_is_pos
                and j.type == "purchase"
                and j.l10n_ar_afip_pos_system not in ["II_IM", "RLI_RLM", "RAW_MAW"]
            )
        )
        if journals:
            raise ValidationError(
                "\n".join(
                    self.env._(
                        ...
                        system=x.l10n_ar_afip_pos_system,
                        id=x.id,
                    )
                    for x in journals
                )
    ...
    def _check_afip_pos_number(self):
        ...
        if self.filtered(
            lambda j: j.l10n_ar_is_pos and j.l10n_ar_afip_pos_number > 99999
        ):
            raise ValidationError(
                self.env._("Please define a valid ARCA POS number (5 digits max)")
            )
    ...
    def write(self, vals):
        ...
        if fields_to_check:
            self.env.cr.execute(
                "SELECT DISTINCT(journal_id) FROM account_move WHERE posted_before = True"
            )
            ...
            for journal in self:
                if (
                    journal.company_id.tax_config_id.account_fiscal_country_id.code
                    != "AR"
                ...
                for field in fields_to_check:
                    ...
                    if vals[field] != journal[field]:
                        raise UserError(
                            self.env._(
                                "You can not change %s journal's configuration if it already has validated invoices",
                                journal.name,
                            )
                        )
