...
class TestSaleStockMultiCompany(TestSaleCommon, ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls):
        ...
        ).property_warehouse_id = cls.warehouse_A.id
        cls.env.user.with_company(
            cls.company_data_2["company"]
        ).property_warehouse_id = cls.warehouse_B.id
    ...
    def test_warehouse_definition_on_so(self):
        ...
        so_company_A = sale_order.with_company(self.env.company).create(
            sale_order_vals2
        )
        ...
        so_company_B = sale_order.with_company(self.company_data_2["company"]).create(
            sale_order_vals3
        )
    ...
    def test_intercompany_show_lot_on_invoice(self):
        ...
        sn = self.env["stock.lot"].create(
            {"name": "SN0012", "product_id": self.product_a.id}
        )
        self.env["stock.quant"]._update_available_quantity(
            self.product_a, self.warehouse_A.lot_stock_id, 1.0, lot_id=sn
        )
        so = self.env["sale.order"].create(
            {
                "partner_id": company2.partner_id.id,
