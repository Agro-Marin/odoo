...
class TestSaleStock(TestSaleStockCommon, ValuationReconciliationTestCommon):
    ...
    def test_04_create_picking_update_saleorderline(self):
        ...
        self.so.write({
            'line_ids': [
                (1, self.so.line_ids.sorted()[0].id, {'product_uom_qty': 2}),
                (1, self.so.line_ids.sorted()[1].id, {'product_uom_qty': 2}),
            ]
        })
    ...
    def test_08_quantities(self):
        ...
        so1.write({
            'line_ids': [
                (1, so1.line_ids.sorted()[0].id, {'product_uom_qty': 15}),
            ]
        })
    ...
    def test_14_delivered_qty_in_multistep(self):
        ...
        self.assertEqual(len(sale_order.line_ids), 1)
        self.assertEqual(sale_order.line_ids.product_uom_qty, 10)
        self.assertEqual(sale_order.line_ids.qty_transferred, 10)
    ...
    def test_17_qty_update_propagation(self):
        ...
        sale_order.line_ids.write({'product_uom_qty': 30})
        ...
        sale_order.line_ids.write({'product_uom_qty': 40})
        ...
        sale_order.line_ids.write({'product_uom_qty': 50})
    ...
    def test_reduce_qty_ordered_no_backorder(self):
        ...
        so_1.action_confirm()
        self.assertEqual(so_1.line_ids.product_uom_qty, 3)
        ...
        so_1.write({'line_ids': [(1, so_1.line_ids.id, {'product_uom_qty': so_1.line_ids.qty_transferred})]})
        self.assertEqual(len(so_1.picking_ids), 1)
    ...
    def test_decrease_sol_qty_to_zero(self):
        ...
        so.line_ids[0].product_uom_qty = 0
    ...
    def test_update_picking_sale_order(self):
        ...
        self.assertRecordValues(sale_order.line_ids, [
            {'product_id': self.product_a.id, 'product_uom_qty': 1, 'qty_transferred': 2},
            {'product_id': self.new_product.id, 'product_uom_qty': 0, 'qty_transferred': 3},
        ])
