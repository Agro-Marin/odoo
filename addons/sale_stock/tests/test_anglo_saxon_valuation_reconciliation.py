...
class TestValuationReconciliationCommon(TestStockValuationCommon, TestSaleStockCommon):
    ...
    def test_invoice_shipment(self):
        ...
        new_invoice = self.env["account.move"].browse(
            refund_invoice_wiz.modify_moves()["res_id"]
        )
        self.assertEqual(
            invoice.payment_state, "reversed", "Invoice should be in 'reversed' state."
        )
        self.assertEqual(
            invoice.reversal_move_ids.payment_state,
            "paid",
            "Refund should be in 'paid' state.",
        )
        self.assertEqual(
            new_invoice.state, "draft", "New invoice should be in 'draft' state."
        )
    ...
    def test_fifo_multiple_products(self):
        ...
        in_moves = self.env["stock.move"].create(
            [
                ...
                for product, quantity in zip(
                    [product_1, product_2], [2.0, 2.0], strict=False
                )
            ]
        )
        ...
        amls = self.env["account.move.line"].search(
            [("product_id", "in", [product_1.id, product_2.id])]
        )
        self.assertRecordValues(
            amls,
            [
