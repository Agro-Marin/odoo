            ...
            line["move_out"].update(
                {
                    "picking_id": {
                        ...
                        "sale_id": {
                            ...
                            "currency_id": picking.sale_id.currency_id.read(
                                fields=["id", "name"]
                            )[0],
                            "partner_id": picking.sale_id.partner_id.read(
    ...
    def _get_report_header(self, product_template_ids, product_ids, wh_location_ids):
        ...
        for product in self._get_products(product_template_ids, product_ids):
            if product not in so_lines:
                continue
            res["product"][product.id]["draft_sale_orders"] = (
                so_lines[product]
                .mapped("order_id")
                .sorted(key=lambda so: so.name)
                .read(fields=["id", "name"])
            )
            res["product"][product.id]["draft_sale_orders_matched"] = (
                self.env.context.get("sale_line_to_match_id") in so_lines[product].ids
            )
        return res
