...
class SaleOrder(models.Model):
    ...
    show_json_popover = fields.Boolean(
        string="Has late picking", compute="_compute_json_popover"
    )
    ...
    def _init_column(self, column_name):
        ...
        _logger.debug(
            "Initializing column '%s' in table '%s'", column_name, self._table
        )
    ...
    def _compute_delivery_status(self):
        for order in self:
            if not order.picking_ids or all(
                ...
                l.qty_transfered for l in order.line_ids
            ):
    ...
    def write(self, vals):
        values = vals
        if values.get("line_ids") and self.state == "sale":
            for order in self:
                pre_order_line_qty = {
        ...
        if values.get("partner_shipping_id") and self.env.context.get(
    ...
    def _action_cancel(self):
        ...
        for sale_order in self:
            if sale_order.state == "sale" and sale_order.line_ids:
    ...
    def _get_action_view_picking(self, pickings):
        ...
        action["context"] = dict(
            ...
            default_reference_ids=self.stock_reference_ids[:-1].id,
        )
        return action
