...
class AccountMove(models.Model):
    ...
    def _get_invoiced_lot_values(self):
        ...
        all_invoices_amls = current_invoice_amls.sale_line_ids.invoice_line_ids.filtered(
        ...
        for sml in stock_move_lines:
            ...
            previous_qty_transfered = previous_qties_delivered[product]
            ...
            if (
                ...
                or product_uom.compare(previous_qty_transfered, previous_qty_invoiced)
                < 0
            ):
                previously_done = (
                    quantity
                    if is_stock_return
                    else min(previous_qty_invoiced - previous_qty_transfered, quantity)
                )
    ...
    def _compute_delivery_date(self):
        ...
        for move in self:
            sale_order_date_effective = list(
                filter(
                    None, move.line_ids.sale_line_ids.order_id.mapped("date_effective")
                )
            )
            date_effective_res = (
