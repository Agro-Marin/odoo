...
class SaleOrderLine(models.Model):
    ...
    virtual_available_at_date = fields.Float(
        digits="Product Unit", compute="_compute_qty_at_date"
    )
    ...
    qty_free_today = fields.Float(digits="Product Unit", compute="_compute_qty_at_date")
    ...
    qty_to_deliver = fields.Float(
        digits="Product Unit", compute="_compute_qty_to_deliver"
    )
    ...
    def _compute_qty_to_deliver(self):
        ...
        for line in self:
            line.qty_to_deliver = line.product_uom_qty - line.qty_transfered
            if (
                ...
                if line.state == "sale" and all(
                    m.state in ["done", "cancel"] for m in line.move_ids
                ):
    ...
    def _compute_qty_at_date(self):
        ...
        date_planned_forecast_per_move = dict(
            all_moves.mapped(lambda m: (m.id, m.date_planned_forecast))
        )
        ...
        for line in self.filtered(lambda l: l.state == "sale"):
            ...
            line.date_planned = line.order_id.date_commitment or line._expected_date()
        ...
        for line in self.filtered(lambda l: l.state in ("draft", "sent")):
            ...
            grouped_lines[
                (
                    ...
                    line.order_id.date_commitment or line._expected_date(),
                )
            ] |= line
        ...
        for (warehouse, date_planned), lines in grouped_lines.items():
            ...
            for line in lines:
                ...
                qty_available_today, qty_free_today, virtual_available_at_date = (
    ...
    def _compute_qty_transfered_method(self):
        ...
        """
        super(SaleOrderLine, self)._compute_qty_transfered_method()
    ...
    def _compute_qty_transfered(self):
        super(SaleOrderLine, self)._compute_qty_transfered()
    ...
    def _get_outgoing_incoming_moves(self, strict=True):
        ...
        return self.env["stock.move"].browse(outgoing_moves_ids), self.env[
            "stock.move"
        ].browse(incoming_moves_ids)
    ...
    def _prepare_procurement_values(self):
        ...
        date_deadline = self.order_id.date_commitment or self._expected_date()
