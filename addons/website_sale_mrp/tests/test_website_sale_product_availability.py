...
class TestWebsiteSaleMrpAvailability(
    HttpCase, AccountTestInvoicingCommon, TestProductAttributeValueCommon
):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
            ...
            [
                {
                    ...
                    "bom_line_ids": [
                        Command.create(
                            {"product_id": cls.component_A.id, "product_qty": 8}
                        ),
                        Command.create(
                            {"product_id": cls.kit_product.id, "product_qty": 2}
                        ),
                        Command.create(
                            {
                ...
                },
                {
                    ...
                    "bom_line_ids": [
                        Command.create(
                            {"product_id": cls.component_A.id, "product_qty": 1}
                        ),
                        Command.create(
                            {"product_id": cls.component_B.id, "product_qty": 5}
                        ),
                        Command.create(
                            {
                                "product_id": cls.consumable_component.id,
        ...
        cls.env["stock.quant"]._update_available_quantity(
            cls.component_B, cls.warehouse.lot_stock_id, 100
        )
