...
class SlideChannel(models.Model):
    ...
    enroll = fields.Selection(
        selection_add=[("payment", "On payment")],
    ...
    def _compute_product_sale_revenues(self):
        ...
        rg_data = {
            ...
            for product, price_total in self.env["sale.report"]._read_group(
                domain, ["product_id"], ["price_total:sum"]
            )
        }
        for channel in self:
    ...
    def _synchronize_product_publish(self):
        ...
        if not self:
            return
        self.filtered(
            lambda channel: channel.is_published and not channel.product_id.is_published
        ).sudo().product_id.write({"is_published": True})

        unpublished_channel_products = self.filtered(
            lambda channel: not channel.is_published
        ).product_id
        group_data = self._read_group(
            [
                ("is_published", "=", True),
                ("product_id", "in", unpublished_channel_products.ids),
            ],
            ["product_id"],
        )
        used_product_ids = {product.id for [product] in group_data}
        product_to_unpublish = unpublished_channel_products.filtered(
            lambda product: product.id not in used_product_ids
        )
