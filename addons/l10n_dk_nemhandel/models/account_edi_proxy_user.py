...
class AccountEdiProxyClientUser(models.Model):
    ...
    def _call_nemhandel_proxy(self, endpoint, params=None):
        ...
        errors = {
            ...
            "too_many_attempts": _(
                "Too many attempts to request an SMS code. Please try again later."
            ),
        }
        ...
        if "error" in response:
            ...
            error_message = response["error"].get("subject") or response["error"].get(
                "data", {}
            ).get("message")
            raise UserError(
                errors.get(error_code)
                or error_message
                or _("Connection error, please try again later.")
            )
        return response
    ...
    def _get_proxy_identification(self, company, proxy_type):
        ...
        if (
            not company.nemhandel_identifier_type
            or not company.nemhandel_identifier_value
        ):
            raise UserError(_("Please fill in the Identifier Type and Value."))
        return (
            f"{company.nemhandel_identifier_type}:{company.nemhandel_identifier_value}"
        )
    ...
    def _register_proxy_user(self, company, proxy_type, edi_mode):
        ...
        if edi_mode == "demo":
            ...
            response = {
                "id_client": f"demo{company.id}{proxy_type}",
                "refresh_token": "demo",
            }
        else:
            try:
                ...
                response = self._prepare_request(
                    self._get_server_url(proxy_type, edi_mode)
                    + "/api/nemhandel/1/connect",
                    params={
                        "dbuuid": company.env["ir.config_parameter"].get_param(
                            "database.uuid"
                        ),
                        "company_id": company.id,
                        "nemhandel_identifier": nemhandel_identifier,
                        "public_key": private_key_sudo._get_public_key_bytes(
                            encoding="pem"
                        ).decode(),
                    },
                )
    ...
    def _nemhandel_import_invoice(
        self, attachment, nemhandel_state, uuid, journal=None
    ):
    ...
    def _nemhandel_get_new_documents(self, skip_no_journal=True, batch_size=None):
        ...
        for edi_user in self:
            ...
            try:
                ...
                _logger.error(
                    "Error while receiving the document from Nemhandel Proxy: %s",
                    ", ".join(e.args),
                )
                continue

            message_uuids = [
                message["uuid"] for message in messages.get("messages", [])
            ]
            ...
            processed_uuids, moves = edi_user._nemhandel_process_new_messages(
                all_messages
            )
    ...
    def _nemhandel_process_new_messages(self, messages):
        ...
        for uuid, content in messages.items():
            if content["document_type"] == "ApplicationResponse":
                ...
                # because the receiver cannot unregister from that service when the
                # module is uninstalled, and an unacknowledged document is fetched again.
                processed_uuids.append(uuid)
                continue
            attachment = self.env["ir.attachment"].create(
                {
                    "name": f"{content['filename'] or 'attachment'}.xml",
                    "raw": self._decrypt_data(content["document"], content["enc_key"]),
                    "type": "binary",
                    "mimetype": "application/xml",
                }
            )
            if imported := self._nemhandel_import_invoice(
                attachment, content["state"], uuid, journal=journal
            ):
                processed_uuids.append(uuid)
                moves |= imported.get("move", self.env["account.move"])
    ...
    def _nemhandel_get_message_status(self, batch_size=None):
        ...
        for edi_user in self:
            ...
            uuid_to_record = {
                document.nemhandel_message_uuid: document
                for document in documents[:job_count]
            }
            messages_to_process = edi_user._call_nemhandel_proxy(
                "/api/nemhandel/1/get_document",
                params={"message_uuids": list(uuid_to_record)},
            )
            processed_message_uuids = edi_user._nemhandel_process_messages_status(
                messages_to_process, uuid_to_record
            )
            edi_user._call_nemhandel_proxy(
                "/api/nemhandel/1/ack",
    ...
    def _nemhandel_process_messages_status(self, messages, uuid_to_record):
        ...
        for uuid, content in messages.items():
            if uuid == "error":
                ...
                moves = self.env["account.move"].union(
                    *(
                        record
                ...
                moves._message_log_batch(bodies=dict.fromkeys(moves.ids, log_message))
    ...
    def _nemhandel_get_participant_status(self):
        for edi_user in self:
            edi_user = edi_user.with_company(edi_user.company_id)
            try:
                proxy_user = edi_user._call_nemhandel_proxy(
                    "/api/nemhandel/1/participant_status"
                )
            except AccountEdiProxyError as e:
                _logger.error(
                    "Error while updating Nemhandel participant status: %s", e
                )
                continue

            if proxy_user["nemhandel_state"] in {
                "not_registered",
                "receiver",
                "rejected",
            }:
    ...
    def _get_nemhandel_company_details(self):
        ...
        return {
            ...
            "nemhandel_company_cvr": self.company_id.vat[2:]
        ...
        if company.l10n_dk_nemhandel_proxy_state != "in_verification":
            ...
            raise UserError(
                _(
                    ...
                    nemhandel_state_translated,
                )
            )

        company_vat = (
            company.vat[2:]
            if company.vat and company.vat[:2].isalpha()
            else company.vat
        )
        if (
            company.nemhandel_identifier_type == "0184"
            and company_vat != company.nemhandel_identifier_value
        ):
            raise ValidationError(
                _(
                    "If you try to register with your CVR, please make sure your company has the same VAT"
                )
            )
    ...
    def _get_nemhandel_user_from_token(self, token: str, url: str):
        try:
            if not (
                payload := tools.resolve_hash_signed(
                    self.sudo().env, "account_nemhandel_webhook", token
                )
            ):
                return None
        except ValueError:
