...
class ResPartner(models.Model):
    ...
    def _compute_nemhandel_identifier_value(self):
        for partner in self:
            if (
                ...
                != partner._origin.nemhandel_identifier_value
            ):
    ...
    def _check_nemhandel_send_oioubl(self):
        if self.filtered(
            lambda partner: (
                partner.invoice_edi_format != "oioubl_21"
    ...
    def _nemhandel_lookup_participant(self, edi_identification):
        ...
        origin = self.env["account_edi_proxy_client.user"]._get_proxy_urls()[
            "nemhandel"
        ][edi_mode]
        query = parse.urlencode(
            {"peppol_identifier": edi_identification.lower(), "zone": sml_zone}
        )
        ...
        if not response.ok:
            _logger.info(
                "unsuccessful response %s when querying nemhandel participant %s",
        ...
        try:
            ...
            _logger.error(
                "invalid JSON response %s when querying nemhandel participant %s",
                response.status_code,
                edi_identification,
            )
            return None
        ...
        if error := decoded_response.get("error"):
            if error.get("code") != "NOT_FOUND":
                _logger.error(
                    "error when querying nemhandel participant %s: %s",
                    edi_identification,
    ...
    def _l10n_dk_nemhandel_log_verification_state_update(
        self, company, old_value, new_value
    ):
        ...
        nemhandel_verification_state_field = self._fields[
    ...
    def _check_nemhandel_participant_exists(self, participant_info, edi_identification):
        ...
        smp_nemhandel_url = (
            "smp-demo.nemhandel.dk" if edi_mode == "test" else "smp.nemhandel.dk"
        )

        return (
            edi_identification.lower() == participant_identifier.lower()
            and parse.urlsplit(service_href).netloc == smp_nemhandel_url
        )
    ...
    def _update_nemhandel_state_per_company(self, vals=None):
        ...
        if vals is None:
            partners = self.filtered(
                lambda p: all(
                    [
                        p.nemhandel_identifier_type,
                        p.nemhandel_identifier_value,
                        p.is_using_nemhandel,
                    ]
                )
            )
        elif {
    ...
    def button_nemhandel_check_partner_endpoint(self, company=None):
        ...
        self_partner.nemhandel_verification_state = (
            self._get_nemhandel_verification_state(self_partner.invoice_edi_format)
        )
        if (
            self_partner.nemhandel_verification_state == "valid"
            and not self_partner.invoice_sending_method
        ):
            self_partner.invoice_sending_method = "nemhandel"

        self._l10n_dk_nemhandel_log_verification_state_update(
            company, old_value, self_partner.nemhandel_verification_state
        )
        return False
    ...
    def _get_nemhandel_verification_state(self, invoice_edi_format):
        self.check_singleton()
        if (
            not self.nemhandel_identifier_type
            or not self.nemhandel_identifier_value
            or invoice_edi_format != "oioubl_21"
        ):
            return "not_verified"
        ...
        is_participant_on_network = self._check_nemhandel_participant_exists(
