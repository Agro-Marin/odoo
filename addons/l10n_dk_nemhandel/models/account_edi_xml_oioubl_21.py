...
class AccountEdiXmlOIOUBL21(models.AbstractModel):
    ...
    def _export_invoice_constraints(self, invoice, vals):
        ...
        for partner_type in ("supplier", "customer"):
            ...
            if not building_number:
                constraints[f"oioubl21_{partner_type}_building_number_required"] = _(
                    "The following partner's street number is missing: %s",
                    partner.display_name,
                )
            if (
                partner.country_code == "FR"
                and not partner.commercial_partner_id.company_registry
            ):
                constraints["oioubl21_company_registry_required_for_french_partner"] = (
                    _(
                        "The company registry is required for french partner: %s",
                        partner.display_name,
                    )
                )
            constraints[f"oioubl21_{partner_type}_vat_required"] = (
                self._check_required_fields(partner.commercial_partner_id, "vat")
            )
        constraints["oioubl21_supplier_company_registry_required"] = (
            self._check_required_fields(vals["supplier"], "company_registry")
        )
    ...
    def _add_invoice_payment_means_nodes(self, document_node, vals):
        ...
        if (
            ...
            and payment_means_node["cac:PayeeFinancialAccount"][
                "cac:FinancialInstitutionBranch"
            ].get("cac:FinancialInstitution")
        ):
            payment_means_node["cac:PayeeFinancialAccount"][
                "cac:FinancialInstitutionBranch"
            ]["cac:FinancialInstitution"]["cac:Address"] = None
    ...
    def _add_invoice_monetary_total_nodes(self, document_node, vals):
        ...
        document_node[monetary_total_tag]["cbc:TaxExclusiveAmount"] = {
            ...
            "_text": self.format_float(
                vals[f"tax_inclusive_amount{currency_suffix}"]
        ...
        if document_node[monetary_total_tag].get("cbc:PrepaidAmount") and document_node[
            monetary_total_tag
        ]["cbc:PrepaidAmount"].get("_text") == FloatFmt(0, 2):
    ...
    def _get_tax_category_node(self, vals):
        ...
        tax_category_node["cbc:ID"].update(
            {
                "_text": UBL_TO_OIOUBL_TAX_CATEGORY_ID_MAPPING.get(
                    tax_category_node["cbc:ID"]["_text"]
                ),
            ...
            document_node["cac:PaymentTerms"] = [
                {
                    ...
                    "cbc:Amount": {
                        "_text": self.format_float(
                            sign * line.amount_currency, 2
                        ),  # OIOUBL needs format to 2 decimals
