...
class TestNemhandelMessage(TestAccountMoveSendCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env["ir.config_parameter"].sudo().set_param(
            "l10n_dk_nemhandel.edi.mode", "test"
        )
        ...
        ]._get_proxy_identification(cls.env.company, "nemhandel")
    ...
    def _get_mock_data(cls, error=False, nr_invoices=1):
        ...
        responses = {
            "/api/nemhandel/1/send_document": {
                "result": {"messages": [{"message_uuid": FAKE_UUID[0]}] * nr_invoices}
            },
    ...
    def _request_handler(cls, s: Session, r: PreparedRequest, /, **kw):
        ...
        if r.path_url.startswith("/api/peppol/1/lookup"):
            nemhandel_identifier = parse.parse_qs(r.path_url.rsplit("?")[1])[
                "peppol_identifier"
            ][0].lower()
        ...
        if url == "/api/nemhandel/1/send_document":
            ...
            proxy_documents, responses = cls._get_mock_data(
                cls.env.context.get("error"),
    ...
    def test_nemhandel_attachment_placeholders(self):
        ...
        wizard = self.create_send_and_print(
            move, sending_methods=["email", "nemhandel"]
        )
    ...
    def test_send_nemhandel_alerts_not_valid_partner(self):
        ...
        self.assertEqual(
            self.invalid_partner.nemhandel_verification_state, "not_valid"
        )  # not on nemhandel at all
        self.assertFalse(
            wizard.sending_methods and "nemhandel" in wizard.sending_methods
        )  # nemhandel is not checked by default
        self.assertTrue(
            wizard.sending_method_checkboxes["nemhandel"]["readonly"]
        )  # can't select nemhandel
    ...
    def test_resend_error_nemhandel_message(self):
        ...
        with self._set_context({"error": True}):
            ...
            self.env[
                "account_edi_proxy_client.user"
            ]._cron_nemhandel_get_message_status()
            self.assertRecordValues(
                move,
                [
                    {
                        "nemhandel_move_state": "error",
    ...
    def test_receive_error_nemhandel(self):
        ...
        with self._set_context({"error": True}):
            self.env[
                "account_edi_proxy_client.user"
            ]._cron_nemhandel_get_new_documents()

            move = self.env["account.move"].search(
                [("nemhandel_message_uuid", "=", FAKE_UUID[1])]
            )
            self.assertRecordValues(
                move, [{"nemhandel_move_state": "error", "move_type": "in_invoice"}]
            )
    ...
    def test_receive_success_nemhandel(self):
        ...
        self.assertRecordValues(
            move, [{"nemhandel_move_state": "done", "move_type": "in_invoice"}]
        )
    ...
    def test_validate_partner_nemhandel(self):
        ...
        self.assertEqual(
            new_partner.nemhandel_verification_state, "valid"
