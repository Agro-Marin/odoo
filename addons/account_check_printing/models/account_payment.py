...
class AccountPayment(models.Model):
    ...
    def _compute_check_number(self):
        for pay in self:
            if (
                pay.journal_id.check_manual_sequencing
                and pay.payment_method_code == "check_printing"
            ):
    ...
    def action_post(self):
        payment_method_check = self.env.ref(
            "account_check_printing.account_payment_method_check"
        )
        for payment in self.filtered(
            lambda p: (
                p.payment_method_id == payment_method_check
    ...
    def print_checks(self):
        ...
        if any(
            payment.journal_id != valid_payments[0].journal_id
    ...
    def _check_build_page_info(self, i, p):
        ...
        return {
            ...
            "amount_in_word": self._check_fill_line(self.check_amount_in_words)
            if i == 0
            else "VOID",
            "memo": self.memo,
            "stub_cropped": not multi_stub
            and len(self.move_id._get_reconciled_invoices()) > INV_LINES_PER_STUB,
    ...
    def _check_make_stub_pages(self):
        ...
        def prepare_vals(invoice, partials=None, current_amount=0):
            ...
            number = " - ".join(
                [invoice_name, invoice.ref] if invoice.ref else [invoice_name]
            )
        ...
        if self.move_id:
            ...
            invoices = self.invoice_ids.filtered(
                lambda x: x.is_outbound(include_receipts=True)
            )
