...
class AccountJournal(models.Model):
    ...
    bank_check_printing_layout = fields.Selection(
        selection="_selection_check_printing_layouts",
        string="Check Layout",
    )

    def _selection_check_printing_layouts(self):
    ...
    def _inverse_check_next_number(self):
        for journal in self:
            ...
            if journal.check_next_number and not re.match(
            ...
            if journal.check_sequence_id:
                ...
                journal.check_sequence_id.sudo().padding = len(
                    journal.check_next_number
                )
    ...
    def _get_journal_dashboard_data_batched(self):
        ...
        self._update_dashboard_data_count(
            dashboard_data,
            "account.payment",
            "num_checks_to_print",
            [
    ...
    def action_checks_to_print(self):
        payment_channel_id = self.outbound_payment_channel_ids.filtered(
