...
class L10nInEwaybill(models.Model):
    ...
    def _get_ewaybill_document_details(self):
        ...
        return {
            "document_number": (move.is_purchase_document(True) and move.ref)
            or move.name,
    ...
    def _get_seller_buyer_details(self):
        ...
        if move.is_outbound():
            return {
                ...
                "ship_to_details": (
                    move._l10n_in_get_warehouse_address() or move.company_id.partner_id
                ),
            }
    ...
    def _compute_vehicle_type(self):
        ...
        for ewaybill in self.filtered(
            lambda ewb: ewb.state == "pending" and ewb.mode == "4"
        ):
    ...
    def _check_transporter(self):
        ...
        if (
            transporter
            and not transporter.check_vat_in(transporter.vat)
    ...
    def _l10n_in_ewaybill_handle_zero_distance_alert_if_present(self, response_data):
        if self.distance == 0 and (alert := response_data.get("alert")):
            ...
            if (match := re.search(pattern, alert)) and (
    ...
    def _create_and_post_response_attachment(self, ewb_name, response, is_cancel=False):
        ...
        name = (
            f"ewaybill_{ewb_name}_cancel.json"
            if is_cancel
            else f"ewaybill_{ewb_name}.json"
        )
        attachment_vals_list.append(
            _create_attachment_vals(name, response, res_field="attachment_file")
        )
        ...
        self.message_post(
            ...
            body=self.env._(
                "E-waybill has been successfully %s.",
                "cancelled" if is_cancel else "sent",
            ),
        )
        ...
        self._write_successfully_response(
            {
                ...
                **self._l10n_in_ewaybill_handle_zero_distance_alert_if_present(
                    response_data
                ),
            }
        )
    ...
    def _get_partner_state_code(self, partner):
        return (
            int(partner.state_id.l10n_in_tin) if partner.country_id.code == "IN" else 99
        )
    ...
    def _get_l10n_in_ewaybill_line_details(self, line, tax_details):
        ...
        tax_details_by_code = self.env[
            "account.move"
        ]._get_l10n_in_tax_details_by_line_code(tax_details.get("tax_details", {}))
        line_details = {
        ...
        line_details.update(
            gst_tax_rates
            or dict.fromkeys({f"{gst_type}Rate" for gst_type in gst_types}, 0.00)
        )
        if tax_details_by_code.get("cess_rate"):
            line_details.update(
                {"cessRate": round_value(tax_details_by_code.get("cess_rate"))}
            )
        return line_details
    ...
    def _prepare_ewaybill_base_json_payload(self):
        ...
        ewaybill_json = {
            ...
            "docDate": (self.document_date or fields.Datetime.now()).strftime(
                "%d/%m/%Y"
            ),
            # bill details
            **prepare_details(
                ...
                partner_detail={
                    "from": self.partner_bill_from_id,
                    "to": self.partner_bill_to_id,
                }.items(),
            ),
            # shipping details
            **prepare_details(
                key_paired_function={
                    ...
                    "Pincode": lambda p: (
                        int(p.zip) if p.country_id.code == "IN" else 999999
                    ),
                }.items(),
                partner_detail={
                    "from": self.partner_ship_from_id,
                    "to": self.partner_ship_to_id,
                }.items(),
            ),
        ...
        return dict(
            filter(
                lambda kv: kv[1],
                {
                    ...
                    and self.transportation_doc_date.strftime("%d/%m/%Y"),
    ...
    def _prepare_ewaybill_tax_details_json_payload(self):
        ...
        total_invoice_value = (
            ...
            + tax_details.get("tax_amount", 0.00)
            + rounding_amount
        )
        if (
            self.account_move_id.l10n_in_gst_treatment == "overseas"
            and self.partner_ship_to_id.country_id.code != "IN"
        ):
            ...
            adjusting_rc_amount = sum(
                tax_details_by_code.get(code, 0.00)
                for code in ("cgst_rc_amount", "sgst_rc_amount", "igst_rc_amount")
            )
    ...
    def _set_data_from_attachment(self):
        ...
        if self.attachment_id:
            ...
            if not ewb_name:
                return False
            ewb_date = self._convert_str_datetime_to_date(
                res_json.get("ewayBillDate") or res_json.get("EwbDt")
            )
            ewb_validity = self._convert_str_datetime_to_date(
                res_json.get("validUpto") or res_json.get("EwbValidTill")
            )
            self.write(
                {
                    "name": ewb_name,
