...
class TestEwaybillJson(L10nInTestInvoicingCommon):
    ...
    def test_edi_json(self):
        default_ewaybill_vals = {
            ...
            "type_id": self.env.ref(
                "l10n_in_ewaybill.type_tax_invoice_sub_type_supply"
            ).id,
            "mode": "1",
        ...
        ewaybill_invoice = Ewaybill.create(
            {"account_move_id": self.invoice.id, **default_ewaybill_vals}
        )
        ewaybill_invoice_full_discount = Ewaybill.create(
            {"account_move_id": self.invoice_full_discount.id, **default_ewaybill_vals}
        )
        ewaybill_invoice_zero_qty = Ewaybill.create(
            {"account_move_id": self.invoice_zero_qty.id, **default_ewaybill_vals}
        )
        ...
        self.invoice.invoice_line_ids.product_uom_id = self.env.ref(
    ...
    def test_ewaybill_zero_distance(self):
        ...
        with self.subTest(scenario="Extract distance when multiple alerts in response"):
            ...
            distance_val = (
                ewaybill._l10n_in_ewaybill_handle_zero_distance_alert_if_present(
                    response.get("data")
                )
            )
    ...
    def test_ewaybill_export_overseas(self):
        ewaybill_invoice_with_export_without_lut = self.env["l10n.in.ewaybill"].create(
            {
                "type_id": self.env.ref(
                    "l10n_in_ewaybill.type_tax_invoice_sub_type_export"
                ).id,
        ...
        json_value = (
            ewaybill_invoice_with_export_without_lut._ewaybill_generate_direct_json()
        )
        expected = {
            "supplyType": "O",
            "subSupplyType": "3",
        ...
        self.assertDictEqual(
            json_value,
            expected,
            "Indian EDI Export without LUT json value does not match",
        )
        ...
        self.assertDictEqual(
            json_value, expected, "Indian EDI Export with LUT json value does not match"
        )
    ...
    def test_ewaybill_transporter_gst(self):
        ...
        self.assertDictEqual(
            json_value,
            expected,
            "Indian EDI Ewaybill without transporter GST sent json value is not matched",
        )
