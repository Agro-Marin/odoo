...
class AccountTax(models.Model):
        ...
        for tax in self:
            if tax.country_id.code == "IT":
                if (
                    ...
                    and tax.amount == 0
                    and not (
                        tax.l10n_it_exempt_reason
                        and html2plaintext(tax.invoice_legal_notes)
                    )
                ):
                ...
                if (
                    tax.l10n_it_exempt_reason == "N6"
                    and tax._l10n_it_is_split_payment()
                ):
                    raise UserError(
                        _(
    ...
    def _l10n_it_filter_kind(self, kind):
        if kind == "vat":
            return self.flatten_taxes_hierarchy().filtered(
                lambda tax: float_compare(tax.amount, 0, precision_digits=2) >= 0
            )
    ...
    def _l10n_it_is_split_payment(self):
        ...
        tax_tags = self._get_repartition_tags(
            is_refund=False, repartition_type="tax"
        ) | self._get_repartition_tags(is_refund=False, repartition_type="base")
        if not tax_tags:
            return False
