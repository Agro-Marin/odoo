...
class TestPurchaseRequisitionSale(TransactionCase):
    ...
    def test_01_purchase_requisition_services(self):
        ...
        self.assertEqual(
            ...
            1,
            "A RFQ should be created, since `service_to_purchase` has been activated for this product",
        )
        ...
        self.assertEqual(
            len(purchase_order.alternative_po_ids),
            2,
        ...
        self.assertEqual(
            len(linked_so),
            1,
            "The Sale Order from the original Purchase Order should be linked",
        )
        self.assertEqual(
            linked_so.id,
            sale_order.id,
            "The Sale Order linked to the alternative PO must be the same as the original one",
        )
        self.assertEqual(
            sale_order.purchase_order_count,
            2,
            "Both the original PO and the alternative one should be there",
        )
