...
patch(ExpressCheckout.prototype, {
    setup() {
        ...
        patchDynamicContent(this.dynamicContent, {
            'button[name="o_payment_submit_button"]': {
                "t-on-click.stop.prevent": this.debounced(
                    this.initiateExpressPayment.bind(this),
                    500,
                    true,
                ),
            },
        ...
        document
            .querySelector('[name="o_payment_submit_button"]')
    ...
    async initiateExpressPayment(ev) {
        ...
        if (this.paymentContext.shippingInfoRequired) {
            ...
            expressDeliveryAddress = {
                ...
                email: shippingInfo.querySelector("#o_payment_demo_shipping_email")
                    .value,
                street: shippingInfo.querySelector("#o_payment_demo_shipping_address")
                    .value,
                street2: shippingInfo.querySelector("#o_payment_demo_shipping_address2")
                    .value,
                ...
                country: shippingInfo.querySelector("#o_payment_demo_shipping_country")
                    .value,
            };
            // Call the shipping address update route to fetch the shipping options.
            const { delivery_methods } = await this.waitFor(
        ...
        await this.waitFor(
            rpc(
                document.querySelector('[name="o_payment_express_checkout_form"]')
