...
class TestSurveyCrm(common.TestSurveyCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        q02 = cls._add_question(
            cls,
            None,
            "What programming languages do you use on a daily basis?",
            "multiple_choice",
            labels=[
                {"value": "Assembly"},
                {"value": "Java"},
    ...
    def _test_survey_crm(
        ...
        survey_sudo = self.survey_crm.sudo()
        ...
        self._answer_question(
            question_ids[3], answers[3], answer_token, csrf_token, "submit"
        )
        ...
        self.assertEqual(
            lead_created.name,
            "%(name)s survey results"
            % {"name": self.survey_user.display_name if login else answers[2]},
        )

        description_expected = Markup(
            "%s"
            % (
                "<div>Answers:</div>"
                "<ul>"
        ...
        self.assertEqual(
            lead_created.partner_id, self.survey_user.partner_id
        ) if login else self.assertFalse(lead_created.partner_id.id)
        self.assertEqual(
            lead_created.user_id.id, False if login else self.survey_crm.user_id.id
        )
        self.assertEqual(
        ...
        self.assertEqual(
            lead_created.contact_name,
            self.survey_user.partner_id.name if answers[3] else "",
        )

        self.assertTrue(
            lead_created.message_ids[0].author_id == self.survey_user.partner_id
            if login
            else self.env.ref("base.public_user").partner_id
        )
    ...
    def test_survey_with_lead_generation_logged_in(self):
        ...
        args = {
            "answers": [
                question_ids[0].suggested_answer_ids[0],
                question_ids[1].suggested_answer_ids[2],
                "beautiful_address@example.com",
                "",
            ],
            "sales_team": True,
            "survey_type": "custom",
    ...
    def test_survey_with_lead_generation_public(self):
        ...
        args = {
            "answers": [
                question_ids[0].suggested_answer_ids[0],
                question_ids[1].suggested_answer_ids[0],
                self.survey_user.email,
                "Martin Test",
            ],
