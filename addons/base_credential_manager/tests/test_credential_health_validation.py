...
class TestCronValidateCredentials(HealthValidationCommon):
    ...
    def test_cron_counts_expired_certificate_as_error(self):
        """An expired auto-validate certificate is reported and stamped 'error'."""
        expired_cert = self._make_certificate_credential(
            "Cron expired cert", days_valid=10, days_ago=40
        )
        expired_cert.auto_validate_health = True

        result = self.env["credential.credential"].cron_validate_credentials()

        self.assertEqual(result["errors"], 1)
        self.assertEqual(expired_cert.health_status, "error")
    ...
    def test_cron_swallows_validation_exception(self):
        ...
        Credential = self.env["credential.credential"]
        credential = self._make_certificate_credential("Cron crashing cert")
        credential.auto_validate_health = True
...
class TestGetCredentialValueByKey(HealthValidationCommon):
    ...
    def test_simple_storage_never_synthesizes_dict_view(self):
        ...
        credential = self.env["credential.credential"].create(
            {
                "name": "Simple storage for key lookup",
                "category_id": self.category_custom.id,
