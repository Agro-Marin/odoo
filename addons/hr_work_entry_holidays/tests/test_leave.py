...
class TestWorkEntryLeave(TestWorkEntryHolidaysBase):
    def test_resource_leave_has_work_entry_type(self):
        ...
        self.assertEqual(
            ...
            self.leave_type.work_entry_type_id,
            "it should have the corresponding work_entry type",
        )
    ...
    def test_resource_leave_in_contract_calendar(self):
        ...
        resource_leave = leave._create_resource_leave()
        self.assertEqual(
            len(resource_leave), 1, "it should have created only one resource leave"
        )
        self.assertEqual(
            resource_leave.work_entry_type_id,
            self.leave_type.work_entry_type_id,
    ...
    def test_validate_leave_without_overlap(self):
        ...
        leave = self.create_leave(
            datetime(2019, 10, 10, 9, 0), datetime(2019, 10, 12, 18, 0)
        )
        ...
        self.assertTrue(
            leave_work_entry.work_entry_type_id.is_leave,
            "It should have created a leave work entry",
        )
        self.assertNotEqual(
            leave_work_entry[:1].state,
            "conflict",
            "The leave work entry should not conflict",
        )
    ...
    def test_refuse_approved_leave(self):
        ...
        leave_work_entry = self.richard_emp.version_id.generate_work_entries(
            start.date(), end.date()
        )
        ...
        self.assertEqual(
            len(work_entries), 1, "Attendance work entry should have been re-created"
        )
        self.assertTrue(
            all(work_entries.mapped(lambda w: w.state != "conflict")),
            "Attendance work entries should not conflict",
        )

    def test_work_entry_create_leave(self):
        self.create_leave(datetime(2019, 10, 10, 9, 0), datetime(2019, 10, 10, 18, 0))
        work_entry = self.create_work_entry(
            datetime(2019, 10, 10, 9, 0), datetime(2019, 10, 10, 18, 0)
        )
        self.assertTrue(work_entry.active)
        self.assertEqual(
            work_entry.state,
            "draft",
            "Attendance work entries don't conflict with leave requests which are in draft state.",
        )
    ...
    def test_split_leaves_by_entry_type(self):
        ...
        entries = self.contract_cdi._generate_work_entries(
            datetime(2024, 9, 10, 0, 0, 0), datetime(2024, 9, 10, 23, 59, 59)
        )
        paid_leave_entry = entries.filtered_domain(
            [("work_entry_type_id", "=", entry_type_paid.id)]
        )
        unpaid_leave_entry = entries.filtered_domain(
            [("work_entry_type_id", "=", entry_type_unpaid.id)]
        )
    ...
    def test_create_work_entry_for_flexible_employee_leave(self):
        ...
        entries = self.jules_emp.generate_work_entries(
            date(2024, 9, 9), date(2024, 9, 14)
        )
        paid_leave_entry = entries.filtered_domain(
            [("work_entry_type_id", "=", entry_type_paid.id)]
        )
        self.assertEqual(
            len(paid_leave_entry),
            4,
            "Four work entries should be created for a flexible employee",
        )
        self.assertEqual(
            sum(paid_leave_entry.mapped("duration")),
            32,
            "The combined duration of the work entries for flexible employee should "
    ...
    def test_leave_change_working_schedule(self):
        ...
        self.assertEqual(
            work_entries.work_entry_type_id, self.leave_type.work_entry_type_id
        )
