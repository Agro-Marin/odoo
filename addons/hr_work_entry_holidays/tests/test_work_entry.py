...
class TestWorkeEntryHolidaysWorkEntry(TestWorkEntryHolidaysBase):
    ...
    def setUpClass(cls):
        ...
        cls.external_company = cls.env["res.company"].create(
            {"name": "External Test company"}
        )
        cls.external_user_employee = mail_new_test_user(
    ...
    def test_time_week_leave_work_entry(self):
        ...
        work_entries = self.richard_emp.generate_work_entries(
            self.start.date(), self.end.date()
        )
        work_entries.action_validate()
        leave_work_entry = work_entries.filtered(
            lambda we: we.work_entry_type_id in self.work_entry_type_leave
        )
        ...
        self.assertEqual(
            sum_hours,
            5.0,
            "It should equal the number of hours richard should have worked",
        )
    ...
    def test_work_entries_generation_if_parent_leave_zero_hours(self):
        ...
        work_entries = employee.version_id.generate_work_entries(
    ...
    def test_work_entries_leave_if_leave_conflict_with_public_holiday(self):
        ...
        self.assertEqual(
            leave_work_entry.leave_id.id,
            leave.id,
            "Leave work entry should have leave_id value",
        )

        public_holiday_work_entry = work_entries.filtered(
            lambda we: we.work_entry_type_id == work_entry_type_holiday
        )
        self.assertEqual(
            len(public_holiday_work_entry.leave_id),
            0,
    ...
    def test_work_entries_overlap_work_leaves(self):
        ...
        remote_work_entry = work_entries.filtered(
            lambda we: we.work_entry_type_id in self.work_entry_type_remote
        )
        leave_work_entry = work_entries.filtered(
            lambda we: we.work_entry_type_id in self.work_entry_type_leave
        )
        self.assertEqual(
            len(remote_work_entry), 5, "There should be five remote work entries"
        )
        self.assertEqual(
            len(leave_work_entry), 1, "There should be one leave work entry"
        )
        ...
        self.assertEqual(
            sum_leave_hours,
            5.0,
            "It should equal the number of hours richard was on leave",
        )
    ...
    def test_work_entries_overlap_half_day_leaves(self):
        ...
        attendance_work_entry = work_entries.filtered(
            lambda we: we.work_entry_type_id == attendance_work_entry_type
        )
        self.assertEqual(1, len(attendance_work_entry))
        self.assertEqual(4.0, attendance_work_entry.duration)
        self.assertEqual(
            date(2025, 11, 28),
            attendance_work_entry.date,
            "The 27/11 work entry should be replaced",
        )
        ...
        self.assertTrue(
            all(
                we.work_entry_type_id == self.work_entry_type_leave
                for we in leave_work_entries
            )
        )
        self.assertEqual(
            8.0,
            leave_work_entries.filtered(
                lambda we: we.date == date(2025, 11, 27)
            ).duration,
        )
        self.assertEqual(
            4.0,
            leave_work_entries.filtered(
                lambda we: we.date == date(2025, 11, 28)
            ).duration,
        )
    ...
    def test_work_entries_overlap_hours_leaves(self):
        ...
        attendance_work_entry = work_entry.filtered(
            lambda we: we.work_entry_type_id == attendance_work_entry_type
        )
        self.assertEqual(1, len(attendance_work_entry))
        ...
        self.assertTrue(
            leave_work_entry.work_entry_type_id == self.work_entry_type_leave
        )
