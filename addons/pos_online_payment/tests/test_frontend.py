...
class TestUi(TestPointOfSaleHttpCommon, OnlinePaymentCommon):
    ...
    def setUpClass(cls):
        ...
        cls.payment_provider = (
            cls.provider
        )  # The dummy_provider used by the tests of the 'payment' module.
    ...
    def _open_session_fake_cashier_unpaid_order(self):
        ...
        create_result = (
            self.env["pos.order"].with_user(self.pos_user).sync_from_ui([order_data])
        )
        self.assertEqual(len(current_session.order_ids), 1)
        order_id = next(
            result_order_data
            for result_order_data in create_result["pos.order"]
            if result_order_data["uuid"] == order_uid
        )["id"]
    ...
    def _test_fake_customer_online_payment(
        self, payments_amount=1, cashier_request_for_remaining=True
    ):
        ...
        for i in range(payments_amount):
            if i != payments_amount - 1 or cashier_request_for_remaining:
                ...
                op_data = order.with_user(
                    self.pos_user
                ).get_and_set_online_payments_data(amount_per_payment)
            ...
            self._fake_online_payment(
                order.id, order.access_token, self.payment_provider.id
            )
        ...
        total_cash_payment = sum(
            current_session.order_ids.filtered(lambda o: o.state != "cancel")
    ...
    def test_1_online_payment_with_cashier(self):
        self._test_fake_customer_online_payment(
            payments_amount=1, cashier_request_for_remaining=True
        )

    def test_1_online_payment_without_cashier(self):
        self._test_fake_customer_online_payment(
            payments_amount=1, cashier_request_for_remaining=False
        )

    def test_2_online_payments_with_cashier(self):
        self._test_fake_customer_online_payment(
            payments_amount=2, cashier_request_for_remaining=True
        )
    ...
    def test_invalid_access_token(self):
        ...
        with mute_logger(
            "odoo.http"
        ):  # Mutes "The provided order or access token is invalid." online payment portal error.
            self.assertRaises(
                AssertionError,
                self._fake_open_pos_order_pay_page,
                order.id,
                order.access_token[:-1],
            )
            self.assertRaises(
                AssertionError, self._fake_open_pos_order_pay_page, order.id, ""
            )
    ...
    def test_customer_display_online_payment(self):
        self.start_tour(
            ...
            "CustomerDisplayTourOnlinePayment",
            login="pos_user",
        )
    ...
    def test_selected_customer_after_adding_payment_sync(self):
        ...
        self.start_pos_tour(
            "test_selected_customer_after_adding_payment_sync", login="pos_admin"
        )
        order = self.pos_config.current_session_id.order_ids.sorted(
            lambda o: o.id, reverse=True
        )[0]
        self.assertEqual(
            order.partner_id.name,
            "A simple PoS man!",
            "The selected customer was not kept after adding an online payment.",
        )
        self.assertEqual(
            order.state, "draft", "The order should still be in draft state."
        )
        self.assertEqual(
            len(order.payment_ids), 0, "There should be no payment line in the order."
        )
    ...
    def tearDownClass(cls):
        ...
        cls.pos_config.payment_method_ids = [
            Command.unlink(cls.online_payment_method.id)
        ]
