...
class PosPaymentMethod(models.Model):
    ...
    def _get_online_payment_providers(self, pos_config_id=False, error_if_invalid=True):
        ...
        config_currency = (
            self.sudo().env["pos.config"].browse(pos_config_id).currency_id
        )
        valid_providers = providers_sudo.filtered(
            lambda p: (
                not p.journal_id.currency_id
                or p.journal_id.currency_id == config_currency
            )
        )
    ...
    def _compute_has_an_online_payment_provider(self):
        for pm in self:
            if pm.is_online_payment:
                pm.has_an_online_payment_provider = bool(
                    pm._get_online_payment_providers()
                )
            else:
    ...
    def _selection_payment_terminals(self):
        return (
            super()._selection_payment_terminals() if not self.is_online_payment else []
        )
    ...
    def _get_or_create_online_payment_method(self, company_id, pos_config_id):
        ...
        if not payment_method_id:
            payment_method_id = (
                self.env["pos.payment.method"]
                .search(
                    [("is_online_payment", "=", True), ("company_id", "=", company_id)],
                    limit=1,
                )
                .exists()
            )
