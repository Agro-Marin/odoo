...
class PosPayment(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            online_account_payments_by_pm[pm_id].add(
                vals.get("online_account_payment_id") or None
            )

        opms_read_id = self.env["pos.payment.method"].search_read(
            [
                "&",
                ("id", "in", list(online_account_payments_by_pm.keys())),
        ...
        if online_account_payments_to_check_id:
            valid_oap_amount = self.env["account.payment"].search_count(
                [("id", "in", list(online_account_payments_to_check_id))]
            )
    ...
    def write(self, vals):
        if vals.keys() & (
            ...
            payment.online_account_payment_id
    ...
    def _check_payment_method_id(self):
        ...
        if any(
            payment.payment_method_id != payment.pos_order_id.online_payment_method_id
            for payment in bypass_check_payments
        ):
            ...
            _logger.warning(
                "Allow to save a POS online payment with an unexpected online payment method"
            )
