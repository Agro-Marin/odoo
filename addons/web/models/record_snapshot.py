        ...
        for field_name, field_spec in x2many_fields_spec.items():
            ...
            for id_, line_snapshot in self_value.items():
                if not force and id_ in other_value:
                    ...
                    # Raw tuple form is intentional: Command.create(vals) always
                    # uses 0 for the second element, but we need to pass the
                    # virtual record's ref so the webclient can match responses.
