...
| `webclient/home_menu/` | pages | 7 | Home menu (app launcher): `HomeMenu`, the `home_menu` service and its `menu` client action, `QuickLauncher` (the popover the navbar opens on hover intent over its home toggle, with pinned and recent tiles and a search box), the `home_menu_badges` loader, the `enterprise_subscription` service, expiration and sysadmin banners, the home-menu background shared with the login page |
