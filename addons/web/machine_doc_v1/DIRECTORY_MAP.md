...
| `core/utils/` | shared | 31 | Cross-cutting utilities: reactivity (`SignalStore`, `effect`, `derived`), `patch`, concurrency, hooks, timing, IndexedDB, URLs, macros, PDF.js loader, render instrumentation |
...
| `model/` | entities | 11 | `Model` base + `useReactiveModel`, the sample-data server/generators/coordinator, search-param schema, shared model types, the `ViewScope` a view-scoped callback takes |
...
| `views/` | widgets | 35 | View infrastructure: the `view` service, `View` component, arch compiler, view utilities/measurements, standard props, action helper, view buttons, the shared multi-record renderer layer (`multi_record_selection.js`, `multi_record_group.js`), and the per-view faces (form, list, kanban, calendar, graph, pivot) |
...
| `webclient/home_menu/` | pages | 13 | Home menu (app launcher): `HomeMenu` (an iCloud-style board of tiles — profile, app widgets listing each featured app's menus, launcher tiles per category), the `home_menu` service and its `menu` client action, `QuickLauncher` (the popover the navbar opens on hover intent over its home toggle, with pinned and recent tiles and a search box), the `home_menu_badges` loader, the `enterprise_subscription` service, expiration and sysadmin banners, the home-menu background shared with the login page |
