...
| `static/tests/` | 814 `.js` (incl. 749 `*.test.js` Hoot suites), mirroring the `static/src/` tree | `TEST_TAGS.md` |
...
| **Primitives** | `core/` | Registry, utils, reactivity, browser abstraction, l10n, network + ORM, errors, py_js, tree, debug, hotkeys, navigation, `lib/` lazy ESM loaders | 203 JS |
...
| **Fields** | `fields/` | 68 widget directories in 7 subcategories (basic, display, media, relational, selection, specialized, temporal); 115 fork-wide `registerField` / `registerFallbackField` sites | 129 JS |
| **Views** | `views/` | View types: form, list, kanban, calendar, graph, pivot + view utilities + settings | 182 JS |
...
| **Model** | `model/` | Client-side relational data model (`RelationalRecord`, `StaticList`, groups, save orchestration) | 53 JS |
...
| `active_view` | `views/active_view.js` | Which view the user is looking at: every `WithSearch` registers its config and search model while mounted, its controller attaches through `useActiveViewController`, and `current` is the last one under `ui.activeElement` -- so a dialog's form wins over the list behind it. Read by `voice` and by enterprise `ai`'s view details |
...
Field widgets (68 widget directories across 7 subcategories; 115 fork-wide `registerField` / `registerFallbackField` sites, 81 plain and 34 through the typed spec form) live in `fields/` (top-level). Import path: `@web/fields/*`. Registration goes through `registerField()` / `registerFallbackField()` in `fields/_registry.js`, never `registry.category("fields").add()` directly.
...
| JavaScript (tests) | 814 (incl. 749 `*.test.js` Hoot suites) |
