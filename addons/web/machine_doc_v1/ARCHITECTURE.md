...
| `static/src/` | 855 JavaScript/OWL source files across 249 directories (FSD layers) | `DIRECTORY_MAP.md` |
| `static/lib/` | 18 directories (17 vendored libraries + generated `popper_compat/`) — DO NOT MODIFY | `static/lib/versions.json` |
| `static/tests/` | 780 `.js` (incl. 717 `*.test.js` Hoot suites), mirroring the `static/src/` tree | `TEST_TAGS.md` |
...
| **Fields** | `fields/` | 68 widget directories in 7 subcategories (basic, display, media, relational, selection, specialized, temporal); 115 fork-wide `registerField` / `registerFallbackField` sites | 128 JS |
| **Views** | `views/` | View types: form, list, kanban, calendar, graph, pivot + view utilities + settings | 178 JS |
| **Webclient** | `webclient/` | App shell: home menu, navbar, menus, actions, user menu, colour scheme, density, debug/profiling, Studio upsell | 90 JS |
...
Field widgets (68 widget directories across 7 subcategories; 115 fork-wide `registerField` / `registerFallbackField` sites, 82 plain and 33 through the typed spec form) live in `fields/` (top-level). Import path: `@web/fields/*`. Registration goes through `registerField()` / `registerFallbackField()` in `fields/_registry.js`, never `registry.category("fields").add()` directly.
...
| JavaScript (src) | 855 (853 carry `@ts-check`; `module_loader.js` + `service_worker.js` are the two exclusions) |
| JavaScript (tests) | 780 (incl. 717 `*.test.js` Hoot suites) |
