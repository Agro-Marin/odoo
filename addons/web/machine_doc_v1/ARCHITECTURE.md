...
| **Fields** | `fields/` | 68 widget directories in 7 subcategories (basic, display, media, relational, selection, specialized, temporal); 225 fork-wide `registerField` / `registerFallbackField` sites | 128 JS |
...
Field widgets (68 widget directories across 7 subcategories; 225 fork-wide `registerField` / `registerFallbackField` sites, 165 plain and 60 through the typed spec form) live in `fields/` (top-level). Import path: `@web/fields/*`. Registration goes through `registerField()` / `registerFallbackField()` in `fields/_registry.js`, never `registry.category("fields").add()` directly.
