...
def qunit_error_checker(message):
    ...
    return "QUnit test failed:" not in message
...
class QunitCommon(odoo.tests.HttpCase):
    ...
    def get_qunit_regex(self, test_params):
        ...
        if filters:
            ...
            pattern = f"^({negative_re})({positive_re})$"
        return pattern

    def get_qunit_filters(self):
        filter_param = ""
        pattern = self.get_qunit_regex(self._test_params)
        if pattern:
            url_filter = quote_plus(pattern)
            filter_param = f"&filter=/{url_filter}/"
        return filter_param

    def test_get_qunit_regex(self):
        f = self.get_qunit_regex([("+", "utils,mail,-utils > bl1,-utils > bl2")])
        f2 = self.get_qunit_regex(
            [("+", "utils"), ("-", "utils > bl1,utils > bl2"), ("+", "mail")]
        )
...
class WebSuite(QunitCommon, HOOTCommon):
    ...
    def test_qunit_desktop(self):
        # ! DEPRECATED
        self.browser_js(
            f"/web/tests/legacy?mod=web{self.qunit_filters}",
            "",
            "",
            login="admin",
            timeout=1800,
            ...
            with suppress(FileNotFoundError):
                with file_open(filename, "rb", filter_ext=(".js",)) as fp:
                    if RE_ONLY.search(fp.read().decode("utf-8")):
                        self.fail(
                            "`QUnit.only()` or `QUnit.debug()` used in file %r"
                            % asset["url"]
                        )
