...
@tagged("-at_install", "post_install", "web_http", "web_partner")
class TestPartnerVCard(HttpCase):
        ...
        self.partners = self.env["res.partner"].create(
            [
                ...
                },
                {
                    ...
                    "child_ids": [Command.create({"type": "other"})],
                },
            ]
        )
    ...
    def check_vcard_contents(self, vcard, partner):
        self.assertEqual(
            vcard.contents["n"][0].value.family,
            partner.name,
            "Vcard should have the same name",
        )
        self.assertEqual(
            vcard.contents["adr"][0].value.street,
            partner.street,
            "Vcard should have the same street",
        )
        self.assertEqual(
            vcard.contents["adr"][0].value.city,
            partner.city,
            "Vcard should have the same city",
        )
        self.assertEqual(
            vcard.contents["adr"][0].value.code,
            partner.zip,
            "Vcard should have the same zip",
        )
        self.assertEqual(
            vcard.contents["adr"][0].value.country,
            self.env.ref("base.us").name,
            "Vcard should have the same country",
        )
        self.assertEqual(
            vcard.contents["email"][0].value,
            partner.email,
            "Vcard should have the same email",
        )
        self.assertEqual(
            vcard.contents["url"][0].value,
            partner.website,
            "Vcard should have the same website",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            vcard.contents["title"][0].value,
            partner.function,
            "Vcard should have the same function",
        )
        self.assertEqual(
            len(vcard.contents["photo"][0].value),
            len(b64decode(partner.avatar_512)),
    ...
    def test_fetch_multiple_partners_vcard(self):
        ...
        with io.BytesIO(res.content) as buffer:
            with zipfile.ZipFile(buffer, "r") as zipf:
                vcf_file_list = zipf.namelist()
                for i, vcf_file in enumerate(vcf_file_list):
                    vcard_content = zipf.read(vcf_file).decode()
                    self.check_vcard_contents(
