...
## ADR-008: Move Remaining Pure Tree Logic to core/tree/ — APPROVED

**Date**: 2026-03-05
**Phase**: 10 (post-Phase-9 discovery)
**Context**: Post-Phase-9 codebase scan found 6 pure-logic files still in `components/tree_editor/`
that Phase 5 missed. These handle expression↔tree conversion (the "expression side" of the tree
pipeline, as opposed to the "domain side" moved in Phase 5). All 6 have zero OWL/UI imports.
One creates a concrete boundary violation: `fields/domain_field.js` imports `domainContainsExpressions`
from `@web/components/`, pulling a fields/ module into a cross-feature dependency.
**Options Considered**:
1. Move all 6 files to `core/tree/`
2. Keep in components/ — accept the fields/ → components/ coupling
3. Move only `domain_contains_expressions.js` (the file causing the concrete violation)

**Decision**: APPROVED Option 1 (move all 6).
**Reasoning**: The files are cohesive (expression↔tree pipeline), all pure, and Phase 5 established
`core/tree/` as the canonical home for exactly this category of logic. Moving all 6 completes the
separation cleanly. Option 3 would leave structural inconsistency.
**Rejected alternatives**: Option 2 rejected — the violation forces non-component code to import from
components/, inverting the expected dependency direction. Option 3 rejected — partial move leaves the
tree pipeline split across two directories without reason.
**Consequences**: After move, `core/tree/` is complete — the full domain↔tree↔expression pipeline
lives in a single shared directory. Components in `components/tree_editor/` import their own logic
from `core/tree/` (correct direction). Zero re-export shims — all consumers updated directly.
**Revisit if**: The expression parser needs OWL-specific behavior (unlikely — it's a pure transform).

## ADR-009: Accept with_search.js → getDefaultDomain Coupling — ACCEPTED

**Date**: 2026-03-05
**Phase**: 10 analysis
**Context**: `with_search.js` (search/ features layer) imports `getDefaultDomain` from
`@web/components/domain_selector/utils` — a peer-features dependency.
**Why accepted**: `getDefaultDomain` is NOT a pure utility. It calls `getDomainDisplayedOperators`
(from `domain_selector_operator_editor`, which resolves field-type-specific operator lists) and
`getDefaultValue` (from `tree_editor_value_editors`, which builds field-type-specific default values).
Both of these are component-layer concerns — operator display logic and value editor defaults are
inherently presentation-layer decisions. Moving `getDefaultDomain` to `core/` would pull component-
layer UI logic into the shared layer, which is worse than the current features→features coupling.
The injection pattern in `with_search.js` (importing and passing to SearchModel) is the optimal
architecture given this constraint: search logic stays pure (injected), and `with_search.js`
(already a component itself) is the natural injection point.
**Consequences**: This coupling is permanently accepted. Document as "injection boundary" pattern.
