            ...
            create(params = {}) {
                ...
                function getPopoverTarget() {
                    ...
                    if (pickerProps.range) {
                        ...
                        while (
                            parentElement &&
                            !inputEls.every((inputEl) =>
                                parentElement.contains(inputEl),
                            )
                        ) {
                ...
                function onInputChange(ev) {
                    ...
                    inputsChanged[inputTarget === getInput(1) ? 1 : 0] = true;
                ...
                function onInputKeydown(ev) {
                    ...
                    if (ev.key === "Enter" && ev.ctrlKey) {
                        ev.preventDefault();
                        updateValueFromInputs();
                        return open(inputTarget === getInput(1) ? 1 : 0);
                    }
                    switch (ev.key) {
                        case "Enter":
                        case "Escape": {
                        ...
                        case "Tab": {
                            if (
                                !getInput(0) ||
                                !getInput(1) ||
                                inputTarget !== getInput(ev.shiftKey ? 1 : 0)
                            ) {
                                return saveAndClose();
                            }
                            break;
                ...
                function updateValueFromInputs() {
                    const values = zipWith(
                        ...
                        (el, currentValue) => {
                            ...
                            const inputEl = /** @type {HTMLInputElement} */ (el);
                            const [parsedValue, error] = safeConvert(
                                "parse",
                                inputEl.value,
                            );
                            if (error) {
                                updateInput(inputEl, currentValue);
                                return currentValue;
                            } else {
                                return parsedValue;
                            }
                ...
                const rawPickerProps = {
                    ...
                    onReset: () => {
                        updateValue(
                            ensureArray(pickerProps.value).length === 2
                                ? [false, false]
                                : false,
                            "date",
                            "picker",
                ...
                const pickerProps = reactive(rawPickerProps, () => {
                    ...
                    for (const [el, value] of zip(
                        ...
                        if (el) {
                            updateInput(/** @type {HTMLInputElement} */ (el), value);
