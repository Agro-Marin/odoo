...
export async function annotateTraceback(error) {
    ...
    // code in a template) is not compatible with the stacktrace lib. Normalize
    // the stack in a local copy to avoid mutating the original error.
    let normalizedError = error;
    if (error.stack) {
        const normalizedStack = error.stack.replace(
            / line (\d*) > (Function):(\d*)/g,
            `:$1`,
        );
        if (normalizedStack !== error.stack) {
            normalizedError = { ...error, stack: normalizedStack };
        }
    }

    let frames;
    try {
        frames = await StackTrace.fromError(normalizedError);
    } catch (e) {
