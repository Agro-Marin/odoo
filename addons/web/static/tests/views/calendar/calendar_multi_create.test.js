...
test("multi_create: basic creation (datetime field)", async () => {
    ...
    await animationFrame();
    await expect(".o_popover").toHaveCount(1);
    await expect(".o_popover .fa-regular.fa-clock").toHaveCount(1);
    await expect(".o_popover .list-group-item:has(.fa-regular.fa-clock)").toHaveText(
...
test("multi_create: delete", async () => {
    ...
    await contains(".o_multi_selection_buttons .btn .fa-solid.fa-trash-can").click();
    await animationFrame();
...
test(`multi_create: no button "Delete" if no record selected`, async () => {
    ...
    expect(".o_multi_selection_buttons .btn .fa-solid.fa-trash-can").toHaveCount(0);
});
