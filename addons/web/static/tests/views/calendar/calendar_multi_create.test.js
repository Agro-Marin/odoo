...
test("multi_create: basic creation (datetime field)", async () => {
    ...
    await animationFrame();
    await expect(".o_popover").toHaveCount(1);
    await expect(".o_popover .fa-clock").toHaveCount(1);
    await expect(".o_popover .list-group-item:has(.fa-clock)").toHaveText(
...
test("multi_create: delete", async () => {
    ...
    await contains(".o_multi_selection_buttons .btn .fa-trash-can").click();
    await animationFrame();
