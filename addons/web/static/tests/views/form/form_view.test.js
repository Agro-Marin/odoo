...
test(`simple form rendering`, async () => {
    ...
    expect(`.o_group .o_inner_group:eq(0)`).toHaveStyle({
...
test(`duplicate fields rendered properly`, async () => {
    ...
    expect(`.o_field_widget[name=int_field].int_field_1`).not.toHaveClass(
        "o_readonly_modifier",
    );
    expect(`.o_field_widget[name=int_field].int_field_2`).toHaveClass(
        "o_readonly_modifier",
    );
    ...
    expect(`.o_field_widget[name=int_field].int_field_1`).toHaveClass(
        "o_readonly_modifier",
    );
    expect(`.o_field_widget[name=int_field].int_field_2`).not.toHaveClass(
        "o_readonly_modifier",
    );
...
test(`duplicate fields rendered properly (one2many)`, async () => {
    ...
    expect(
        `.o_field_one2many:eq(1) .o_data_row:eq(0) .o_data_cell[name=foo]`,
    ).toHaveText("yop");
    ...
    expect(
        `.o_field_one2many:eq(0) .o_selected_row .o_field_widget[name="foo"] input`,
    ).toHaveValue("My little Foo Value");
    expect(
        `.o_field_one2many:eq(1) .o_data_row:eq(1) .o_data_cell[name=foo]`,
...
test(`form with o2m having a field with fieldDependencies`, async () => {
    ...
    await contains(
        `.o_field_widget[name="child_ids"] .o_data_row .o_data_cell`,
    ).click();
...
test(`notebook page name and class transferred to DOM`, async () => {
    ...
    expect(`.o_notebook .nav .nav-link[name='choucroute']`).toHaveClass([
        "active",
        "sauerKraut",
    ]);
});
...
test.tags("desktop");
test(`rendering stat buttons without class on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `
...
test.tags("mobile");
test(`rendering stat buttons without class on mobile`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `
...
test.tags("desktop");
test(`rendering stat buttons without action on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `
...
test.tags("mobile");
test(`rendering stat buttons without action on mobile`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `
...
test(`input ids for multiple occurrences of fields in sub form view (inline)`, async () => {
    ...
    await contains(`.o_field_x2many_list_row_add a`).click();
    const fieldIdAttrs = queryAllAttributes(
        `.modal .o_form_view .o_field_widget input`,
        "id",
    );
    const labelForAttrs = queryAllAttributes(
        `.modal .o_form_view .o_form_label`,
        "for",
    );
    expect(new Set(fieldIdAttrs)).toHaveLength(4);
    expect(fieldIdAttrs).toEqual(labelForAttrs);
...
test.tags("desktop");
test(`tooltips on multiple occurrences of fields and labels`, async () => {
...
test.tags("desktop");
test(`readonly attrs on lines are re-evaluated on field change 2`, async () => {
    ...
    expect(`.o_field_one2many[name="product_ids"]`).not.toHaveClass(
        "o_readonly_modifier",
    );
...
test.tags("desktop");
test(`buttons in form view`, async () => {
    expect.errors(1);
...
test(`onchange only send present fields value`, async () => {
    ...
    onRpc("onchange", ({ args }) => {
        ...
        expect(args[1]).toEqual({
            foo: "tralala",
            child_ids: [
                [0, args[1].child_ids[0][1], { name: "valid line", float_field: 12.4 }],
            ],
        });
    });
    await mountView({
    ...
    onRpc("onchange", ({ args, kwargs }) => {
        ...
        expect(args[1]).toEqual({
            ...
            user_id: {
                ...
                partner_ids: [
                    [0, args[1].user_id.partner_ids[0][1], { float_field: 0 }],
                ],
            },
        });
        expect.step("onchange");
...
test(`reference field in one2many list`, async () => {
    ...
    await contains(
        `.o_dialog:not(.o_inactive_modal) .o_field_widget[name="name"] input`,
    ).edit("New name");
    await contains(
        `.o_dialog:not(.o_inactive_modal) footer .o_form_button_save`,
    ).click();
...
test.tags("desktop");
test(`buttons with attr "special" in dialog close the dialog`, async () => {
    Product._views = {
        form: `
            <form>
                <sheet>
...
test.tags("desktop");
test(`discard changes on relational data on new record`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `
...
test.tags("desktop");
test(`switching to another record from a dirty one on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `<form><field name="foo"></field></form>`,
...
test.tags("desktop");
test(`do not reload after save when using pager on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `<form><field name="foo"></field></form>`,
...
test.tags("desktop");
test(`switching to another record from an invalid one on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `<form><field name="foo" required="1"/></form>`,
...
test.tags("desktop");
test(`handling dirty state: switching to another record on desktop`, async () => {
    Partner._fields.priority = fields.Selection({
        default: 1,
        selection: [
            [1, "Low"],
...
test.tags("desktop");
test(`pager is hidden in create mode on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `<form><field name="foo"/></form>`,
...
test.tags("desktop");
test(`switching to another record on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `<form><field name="foo"></field></form>`,
...
test("empty required fields in an existing record are highlighted", async () => {
    ...
    await contains(".o_field_widget[name=int_field] input").edit("25", {
        confirm: false,
    });
...
test(`delete a line in a one2many while editing another line`, async () => {
    ...
    await contains(`.fa-solid.fa-trash-can:eq(1)`).click();
    expect(`.o_data_row`).toHaveCount(2);
...
test(`args of onchanges in o2m fields are correct (inline edition)`, async () => {
    ...
    await contains(`.o_field_widget[name=int_field] input`).edit("77", {
        confirm: false,
    });
...
test.tags("desktop");
test(`clicking on a stat button with x2many in context on desktop`, async () => {
...
test.tags("mobile");
test(`clicking on a stat button with x2many in context on mobile`, async () => {
    Partner._records[1].type_ids = [12];
...
test.tags("desktop");
test(`display something else than a button in a buttonbox on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `
...
test.tags("mobile");
test(`display something else than a button in a buttonbox on mobile`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `
    ...
    expect(`.o-form-buttonbox-small > .o-dropdown-item > .oe_stat_button`).toHaveCount(
        1,
    );
...
test.tags("desktop");
test(`many2manys inside one2manys are saved correctly`, async () => {
    onRpc("web_save", ({ args }) => {
        expect.step("web_save");
...
test.tags("desktop");
test(`*_view_ref in context are passed correctly`, async () => {
    PartnerType._views = {
        "list,module.list_view_ref": `<list/>`,
    };

    onRpc("partner.type", "get_views", ({ kwargs }) =>
        expect.step(kwargs.context.list_view_ref),
    );
...
test(`readonly sub fields fields with force_save attribute`, async () => {
    ...
    onRpc("web_save", ({ args }) => {
        ...
        expect(commands).toEqual([
            [0, commands[0][1], { foo: "some value", float_field: 6.5 }],
        ]);
    });
    onRpc("onchange", () => {
        expect.step("onchange");
        return {
            value: {
                child_ids: [
                    [0, false, { foo: "some value", int_field: 44, float_field: 6.5 }],
                ],
            },
        };
    });
...
test.tags("desktop");
test(`render stat button with string inline on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        resId: 1,
    ...
    expect(`.o_form_view .o-form-buttonbox button.oe_stat_button`).toHaveText(
        "Inventory Moves",
    );
});

test.tags("mobile");
test(`render stat button with string inline on mobile`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        resId: 1,
    ...
    await contains(".o-form-buttonbox .o_button_more").click();
    expect(`.o-form-buttonbox-small button.oe_stat_button`).toHaveText(
        "Inventory Moves",
    );
});
...
test.tags("desktop");
test(`in create mode, autofocus fields are focused`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
...
test(`check scroll on small height screens`, async () => {
    ...
    await contains(
        `.o_dialog:not(.o_inactive_modal) button[class="btn-close"]`,
    ).click();
    expect(queryFirst(`.o_content`).scrollTop).not.toBe(0);
    expect(`.modal-dialog`).toHaveCount(1);
});
...
test(`form rendering with groups with col/colspan`, async () => {
    ...
    expect(`.group_4 > div.o_cell:last-child`).toHaveAttribute(
        "style",
...
test(`form rendering innergroup: separator should take one line`, async () => {
    ...
    expect(`.o_inner_group > .o_cell:first-child .o_horizontal_separator`).toHaveCount(
        1,
    );
    expect(
        `.o_inner_group > .o_cell:nth-child(2) label[for=display_name_0]`,
    ).toHaveCount(1);
...
test(`outer and inner groups string attribute`, async () => {
    ...
    expect(
        `.parent_group > div.o_horizontal_separator:contains(parent group)`,
    ).toHaveCount(1);
});
...
test(`form group with newline tag inside`, async () => {
    ...
    expect(
        `.main_inner_group > .o_wrap_field_boolean:nth-child(3) > .o_wrap_label`,
    ).toHaveCount(1);
    expect(
        `.main_inner_group > .o_wrap_field_boolean:nth-child(3) > .o_wrap_input`,
    ).toHaveCount(1);
...
test(`translate event correctly handled with multiple controllers`, async () => {
    ...
    await contains(`[name="product_id"] .o_external_button`, {
        visible: false,
    }).click();
    await contains(`.o_field_translate`).click();
        ...
        if (checkOnchange) {
            ...
            if (args[2][0] === "name") {
                ...
                expect(args[1].child_ids).toEqual([
                    [
                        0,
                        args[1].child_ids[0][1],
                        { name: "readonly", foo: "foo value" },
                    ],
                ]);
            }
        }
...
test.tags("desktop");
test(`process the context for subview not inline`, async () => {
    Partner._records[0].child_ids = [2];
    Partner._views = {
        list: `
            <list>
...
test(`can toggle column in x2many in sub form view`, async () => {
    ...
    await contains(`.o_external_button`, { visible: false }).click();
    expect(queryAllTexts`.o_dialog:not(.o_inactive_modal) .o_data_cell`).toEqual([
        "yop",
        "blip",
    ]);
...
test.tags("desktop");
test(`support header button as widgets on form statusbar on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
...
test.tags("mobile");
test(`support header button as widgets on form statusbar on mobile`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `<form><header><widget name="attach_document" string="Attach document"/></header></form>`,
...
test(`proper stringification in debug mode tooltip`, async () => {
    ...
    expect(`.o-tooltip--technical > li[data-item="context"]`).toHaveText(
        /{'lang': 'en_US'}/,
    );
    ...
    expect(`.o-tooltip--technical > li[data-item="invisible"]`).toHaveText(
        /product_id == 33/,
    );
...
test.tags("desktop");
test(`do not change pager when discarding current record on desktop`, async () => {
    await mountView({
        resModel: "partner",
        type: "form",
        arch: `<form><field name="foo"/></form>`,
...
test.tags("desktop");
test(`coming to a form view from a grouped and sorted list`, async () => {
    defineActions([
        {
            id: 1,
            name: "test",
...
test.tags("desktop");
test(`keep editing after call_button fail`, async () => {
    expect.errors(1);
    ...
    await contains(
        `.o_form_view .o_field_one2many .o_data_row .o_data_cell:eq(1)`,
    ).click();
...
test(`saving with invalid uncommitted changes`, async () => {
    ...
    await contains(`.o_field_widget[name=float_field] input`).edit(
        "some float_field value",
    );
    await contains(`.o_form_button_save`).click();
    expect(`.o_form_editable`).toHaveCount(1);
...
test.tags("desktop");
test(`leave the form view while saving`, async () => {
    Partner._onChanges = {
        foo(record) {
...
test.tags("desktop");
test(`discard after a failed save (and close notifications)`, async () => {
    Partner._views = {
        form: `
            <form>
...
test.tags("desktop");
test(`company_dependent field in form view, in multi company group`, async () => {
...
test.tags("desktop");
test(`Action Button clicked with failing action on desktop`, async () => {
    expect.errors(1);
...
test.tags("mobile");
test(`Action Button clicked with failing action on mobile`, async () => {
    expect.errors(1);
...
test.tags("desktop");
test(`help on field is shown without debug mode -- form`, async () => {
    Partner._fields.bar = fields.Boolean({ help: "bar tooltip" });
...
test.tags("desktop");
test(`form view does not deactivate sample data on other views`, async () => {
    ResUsers._records = [];
    Partner._records = [];
    Partner._views = {
...
test.tags("desktop");
test(`empty x2manys when coming form a list with sample data`, async () => {
    ResUsers._records = [];
    Partner._records = [];
    Partner._views = {
        list: `<list sample="1"><field name="name"/></list>`,
...
test(`status indicator: field dirty state (datetime)`, async () => {
    ...
    await contains(".o_field_widget button").click();
    await contains(`.o_field_widget input`).edit("12/12/2012 11:55:05", {
        confirm: false,
    });
    expect(`.o_form_status_indicator_buttons.invisible`).toHaveCount(0);
});
...
test(`coming to an action with an error from a form view with a dirty x2m`, async () => {
    ...
    await contains(
        `.o_field_one2many[name="child_ids"] .o_field_x2many_list_row_add a`,
    ).click();
...
test(`commitChanges with a field input removed during an update`, async () => {
    ...
    await contains(`.o_data_cell[name="foo"] input`).edit("new foo", {
        confirm: "tab",
    });
    onchangeDeferred.resolve();
    await contains(`.o_form_button_save`).click();
    ...
    class MyWidget extends Component {
        ...
        setup() {
            this.specialData = useSpecialData((orm, props) => {
                const { record } = props;
                return orm.call("my.model", "get_special_data", [
                    record.data.int_field,
                ]);
            });
        }
    }
...
test(`an empty json object does not pass the required check`, async () => {
    ...
    class JsonField extends Component {
        ...
        onChange(ev) {
            this.props.record.update({
                [this.props.name]: JSON.parse(ev.target.value),
            });
        }
    }
...
test.tags("mobile");
test(`statusbar buttons are correctly rendered in mobile`, async () => {
    await mountView({
        type: "form",
        resModel: "partner",
        resId: 1,
    ...
    expect(".o-dropdown--menu:visible").toHaveCount(1, {
        message: "dropdown should be visible",
    });
...
test.tags("mobile");
test(`statusbar widgets should appear in the CogMenu dropdown`, async () => {
    await mountView({
        type: "form",
        resModel: "partner",
        resId: 2,
...
test.tags("mobile");
test(`CogMenu dropdown's open/close state shouldn't be modified after 'onchange'`, async () => {
    Partner._onChanges = {
        name() {},
    };
...
test(`preserve current scroll position on form view while closing dialog`, async () => {
    ...
    expect(window.scrollY).toBe(265, {
        message: "Should have scrolled 265 px vertically",
    });
...
test.tags("mobile");
test("attach_document widget also works inside a dropdown", async () => {
    let fileInput;
