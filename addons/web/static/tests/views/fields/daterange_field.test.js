...
test("Render with initial empty value: date field", async () => {
    ...
    await press("Enter");
    await animationFrame();
    expect("button[data-field=date]").toHaveValue(
        queryValue("button[data-field=date_end]"),
    );
});
...
test("required: related end date, both start date and end date empty", async () => {
    ...
    expect(".o_field_daterange input:eq(0)").toHaveAttribute("data-field", "datetime");
    expect(".o_field_daterange input:eq(0)").toHaveValue("");
    expect(".o_field_daterange input:eq(1)").toHaveAttribute(
        "data-field",
        "datetime_end",
    );
    ...
    expect(".o_field_daterange button:eq(1)").toHaveAttribute(
        "data-field",
        "datetime_end",
    );
...
test("related start date, both start date and end date empty", async () => {
    ...
    expect(".o_field_daterange input").toHaveCount(1);
    expect(".o_field_daterange input:eq(0)").toHaveAttribute(
        "data-field",
        "datetime_end",
    );
    expect(".o_field_daterange input:eq(0)").toHaveValue("");
...
test("related end date, start date empty and end date set", async () => {
    ...
    expect(".o_field_daterange button").toHaveCount(1);
    expect(".o_field_daterange button:eq(0)").toHaveAttribute(
        "data-field",
        "datetime_end",
    );
...
test("related start date, both start date and end date set", async () => {
    ...
    expect(".o_field_daterange button[data-field]").toHaveAttribute(
        "data-field",
        "datetime_end",
    );
...
test("list daterange: start date input width matches its span counterpart", async () => {
    ...
    await animationFrame();
    const inputWidth = queryFirst(".o_field_daterange input").offsetWidth;
    expect(Math.abs(inputWidth - initialWidth)).toBeLessThan(2, {
        message: `input width (${inputWidth}) should match span width (${initialWidth}) within 1px tolerance`,
    });
});
...
test("daterange field in kanban with show_time option", async () => {
    ...
    expect(queryAllTexts(".o_field_daterange span")).toEqual([
        "Feb 8, 2017",
        "Mar 13, 2017",
    ]);
});
