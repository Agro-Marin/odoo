...
test("PriorityField when not set", async () => {
    ...
    expect(".o_field_widget .o_priority a.o_priority_star.fa-solid").toHaveCount(0, {
        message: "should have no full star since there is no value",
    });

    expect(".o_field_widget .o_priority a.o_priority_star.fa-regular").toHaveCount(2, {
...
test("PriorityField in form view", async () => {
    ...
    expect(".o_field_widget .o_priority a.o_priority_star").toHaveCount(2);
    expect(".o_field_widget .o_priority a.o_priority_star.fa-solid").toHaveCount(2);
    expect(".o_field_widget .o_priority a.o_priority_star.fa-regular").toHaveCount(0);
});
...
test("PriorityField can write after adding a record -- kanban", async () => {
    ...
    expect(".o_kanban_record .fa-solid").toHaveCount(2);
});
...
test("PriorityField in editable list view", async () => {
    ...
    expect(".o_data_row:last-child .o_priority a.o_priority_star.fa-solid").toHaveCount(
        2,
        {
            message:
    ...
    expect(
        ".o_data_row:last-child .o_priority a.o_priority_star.fa-regular",
    ).toHaveCount(0, {
...
test("PriorityField with readonly attribute", async () => {
    ...
    expect("span.o_priority_star.fa-regular.o_disabled").toHaveCount(2, {
        message: "stars of priority widget should rendered with span tag if readonly",
    });
    await hover(".o_priority_star.fa-regular:last");
    await animationFrame();
    expect.step("hover");
    expect(".o_field_widget .o_priority a.o_priority_star.fa-solid").toHaveCount(0, {
        message: "should have no full stars on hover since the field is readonly",
    });
    await click(".o_priority_star.fa-regular:last");
    await animationFrame();
    expect.step("click");
    expect("span.o_priority_star.fa-regular.o_disabled").toHaveCount(2, {
        message: "should still have two stars",
...
test('PriorityField edited by the smart action "Set priority..."', async () => {
    ...
    expect("a.fa-solid").toHaveCount(1);
