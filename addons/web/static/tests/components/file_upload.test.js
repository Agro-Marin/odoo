...
import { FileUploadProgressRecord } from "@web/components/file_upload/file_upload_progress_record";
...
test("handles error", async () => {
    ...
    fileUploadService.upload("/test/", []);
    await waitFor(
        ".o_notification:has(.bg-danger):contains(An error occurred while uploading)",
    );
});
...
test("handles Odoo's jsonrpc error", async () => {
    ...
    fileUploadService.upload("/test/", []);
    await waitFor(
        ".o_notification:has(.bg-danger):contains(ValidationError: Boom Odoo)",
    );
});
