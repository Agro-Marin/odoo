...
test("a string group_by is normalised to an array, an empty one to []", async () => {
    ...
    const build = (group_by) =>
        prepareViewInfo(LIST, makeAction({ context: { group_by } }), [LIST], {}, am)
            .props.groupBy;
    expect(build("stage_id")).toEqual(["stage_id"]);
    expect(build("")).toEqual([]);
    expect(build(["a", "b"])).toEqual(["a", "b"]);
    expect(prepareViewInfo(LIST, makeAction(), [LIST], {}, am).props.groupBy).toEqual(
        [],
    );
});
...
test("display mode is inDialog for a dialog and the raw target otherwise", async () => {
    const am = makeFakeAm();
    expect(
        prepareViewInfo(LIST, makeAction({ target: "new" }), [LIST], {}, am).props
            .display,
    ).toEqual({ mode: "inDialog" });
    expect(
        prepareViewInfo(LIST, makeAction({ target: "fullscreen" }), [LIST], {}, am)
            .props.display,
...
test("action menus are suppressed for dialogs and for res.config.settings", async () => {
    const am = makeFakeAm();
    const load = (action) =>
        prepareViewInfo(LIST, action, [LIST], {}, am).props.loadActionMenus;
...
test("ir.filters are loaded only when the action declares a search view", async () => {
    const am = makeFakeAm();
    expect(
        prepareViewInfo(LIST, makeAction(), [LIST], {}, am).props.loadIrFilters,
    ).toBe(true);
    const noSearch = makeAction({ views: [[false, "list"]] });
    expect(prepareViewInfo(LIST, noSearch, [LIST], {}, am).props.loadIrFilters).toBe(
        false,
    );
});
...
test("help is renamed to noContentHelp; the other special keys keep their names", async () => {
    ...
    const { props } = prepareViewInfo(LIST, action, [LIST], {}, makeFakeAm());
...
test("special keys absent from the action are not invented", async () => {
    const { props } = prepareViewInfo(LIST, makeAction(), [LIST], {}, makeFakeAm());
...
test("resId falls back from props to action.res_id to false", async () => {
    const am = makeFakeAm();
    expect(
        prepareViewInfo(LIST, makeAction(), [LIST], { resId: 3 }, am).props.resId,
    ).toBe(3);
    expect(
        prepareViewInfo(LIST, makeAction({ res_id: 8 }), [LIST], {}, am).props.resId,
    ).toBe(8);
    expect(prepareViewInfo(LIST, makeAction(), [LIST], {}, am).props.resId).toBe(false);
});

test("noBreadcrumbs defaults to 'is a dialog' but an explicit flag wins", async () => {
    const am = makeFakeAm();
    const nb = (action) =>
        prepareViewInfo(LIST, action, [LIST], {}, am).props.noBreadcrumbs;
...
test("selecting a record switches to the form view when the action has one", async () => {
    ...
    const { props } = prepareViewInfo(LIST, makeAction(), [LIST, FORM], {}, am);
...
test("without a form view, selecting an existing record does nothing", async () => {
    ...
    const { props } = prepareViewInfo(LIST, makeAction(), [LIST], {}, am);
...
test("embedded actions are suppressed on a form view", async () => {
    ...
    const { config } = prepareViewInfo(FORM, action, [FORM], {}, am);
...
test("a multi-record view prefers the parent's embedded actions over its own", async () => {
    ...
    const { config } = prepareViewInfo(LIST, action, [LIST], {}, am);
...
test("the search view is skipped and the rest are described from session.view_info", async () => {
    const views = getActionViews(makeAction());
