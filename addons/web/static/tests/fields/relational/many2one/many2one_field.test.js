...
test("editing a many2one (with form view opened with external button)", async () => {
    ...
    await contains(
        ".o_dialog:not(.o_inactive_modal) .o_field_widget[name='foo'] input",
    ).edit("brandon");
...
test("show_address works in a view embedded in a view of another type", async () => {
    ...
    expect('[name="turtle_trululu"]').toHaveText(
        "second record\nrue morgue\nparis 75013",
    );
});
...
test("many2ones in list views: create in dialog keeps the input", async () => {
    ...
    await contains(".o_field_widget[name=trululu] input").edit("yy", {
        confirm: false,
    });
    await runAllTimers();
...
test("many2ones in list views: create a new record with a context", async () => {
    ...
    await contains(".o_data_cell:eq(0)").click();
    await contains(".o_field_widget[name=user_id] input").edit("yy", {
        confirm: false,
    });
    await runAllTimers();
...
test("..._view_ref keys are removed from many2one context on create and edit", async () => {
    ...
    await contains(".o_field_widget[name=trululu] input").edit("ABC", {
        confirm: false,
    });
    await runAllTimers();
    await contains(
        ".o_field_widget[name=trululu] .o_m2o_dropdown_option_create_edit",
    ).click();
...
test("empty many2one field", async () => {
    ...
    await contains(".o_field_many2one[name='trululu'] input").edit("abc", {
        confirm: false,
    });
    await runAllTimers();
...
test("empty many2one field with node options", async () => {
    ...
    expect(
        ".o_field_many2one[name='trululu'] .dropdown-menu li.o_m2o_start_typing",
    ).toHaveCount(0);

    await contains(".o_field_many2one[name='product_id'] input").click();
    expect(
        ".o_field_many2one[name='product_id'] .dropdown-menu li.o_m2o_start_typing",
    ).toHaveCount(0);
});
...
test("many2one in edit mode", async () => {
    ...
    expect(
        ".o_field_many2one[name='trululu'] .dropdown-menu li.o_m2o_dropdown_option",
    ).toHaveCount(1);
    expect(
        ".o_field_many2one[name='trululu'] .dropdown-menu li.o_m2o_start_typing",
    ).toHaveCount(0);
...
test("many2one in non edit mode (with value)", async () => {
    ...
    expect("div[name=res_trululu] a.o_form_uri").toHaveAttribute(
        "href",
        "/odoo/res.partner/1",
    );
    expect("div[name=trululu] a.o_form_uri").toHaveAttribute(
        "href",
        "/odoo/m-partner/4",
    );
});
...
test("form: quick create for field that returns false after name_create call", async () => {
    ...
    await contains(".o_field_widget[name=trululu] input").edit("beam", {
        confirm: false,
    });
    await runAllTimers();
    await contains(".o_m2o_dropdown_option_create").click();
...
test("name_create in form dialog", async () => {
    ...
    await runAllTimers();
    await contains(
        ".modal .o_field_widget[name=product_id] .o_m2o_dropdown_option_create",
    ).click();
...
test("list in form: call button in sub view", async () => {
    ...
    const buttons = queryAll(
        ".o_dialog:not(.o_inactive_modal) .o_form_statusbar button",
    );
    ...
    await contains(
        ".o_dialog:not(.o_inactive_modal) .o_form_statusbar button:eq(1)",
    ).click();
...
test("failing quick create on a many2one because ValidationError", async () => {
    ...
    await contains(".o_field_widget[name='product_id'] input").edit("abcd", {
        confirm: false,
    });
    await runAllTimers();
    await contains(
        ".o_field_widget[name='product_id'] .o_m2o_dropdown_option_create",
    ).click();
    await animationFrame();
...
test("can_create and can_write option on a many2one", async () => {
    ...
    await contains(
        ".o_dialog:not(.o_inactive_modal) .modal-footer .btn-primary",
    ).click();
...
test("create_name_field option on a many2one", async () => {
    ...
    await contains(".o_field_widget[name=trululu] input").edit("yz", {
        confirm: false,
    });
    await runAllTimers();
    await contains(".o_field_widget[name=trululu] input").click();
...
test("propagate can_create onto the search popup", async () => {
    ...
    await contains(".o_field_widget[name=product_id] input").edit("a", {
        confirm: false,
    });
    await runAllTimers();
    ...
    await contains(".o_field_many2one[name=product_id] input").edit("", {
        confirm: false,
    });
    await runAllTimers();
...
test("many2one with can_create=false shows no result item when searched something that doesn't exist", async () => {
    ...
    await contains(".o_field_many2one input").click();
    await contains(".o_field_many2one[name=product_id] input").edit("abc", {
        confirm: false,
    });
    await runAllTimers();
    expect(
...
test("search more in many2one: no text in input", async () => {
    ...
    await contains(`.o_field_widget[name="trululu"] input`).click();
    await contains(
        `.o_field_widget[name="trululu"] .o_m2o_dropdown_option_search_more`,
    ).click();
...
test("search more in many2one: text in input", async () => {
    ...
    await contains(".o_field_widget[name='trululu'] input").edit("test", {
        confirm: false,
    });
    await runAllTimers();
    await contains(
...
test("updating a many2one from a many2many", async () => {
    ...
    await contains(".o_dialog:not(.o_inactive_modal) div[name=name] input").edit(
        "test",
    );
    await contains(".modal:eq(1) .o_form_button_save").click();
    ...
    expect(
        ".o_dialog:not(.o_inactive_modal) div[name=turtle_trululu] input",
    ).toHaveValue("test");
});
...
test("keep changes when editing related record in a dialog", async () => {
    ...
    await contains(
        ".o_dialog:not(.o_inactive_modal) .o_field_widget[name=int_field] input",
    ).edit("5464");
    await contains(
...
test("create and edit, save and then discard", async () => {
    ...
    await contains(".o_field_widget[name=trululu] input").edit("new m2o", {
        confirm: false,
    });
    await runAllTimers();
    await contains(".o_field_widget[name=trululu] input").click();
...
test("search typeahead", async () => {
    ...
    await contains(".o_field_widget[name=trululu] input").edit("re", {
        confirm: false,
    });
    await runAllTimers();
    expect.verifySteps([]);
    ...
    await contains(".o_field_widget[name=trululu] input").edit("rec", {
        confirm: false,
    });
    await runAllTimers();
...
test("res_partner_many2one renders as a many2one", async () => {
    await mountView({
        ...
        resModel: "partner",
        resId: 1,
        arch: `<form><field name="res_trululu" widget="res_partner_many2one"/></form>`,
    });

    expect(".o_field_widget[name=res_trululu]").toHaveClass(
        "o_field_res_partner_many2one",
    );
    expect(".o_field_res_partner_many2one[name=res_trululu] input").toHaveValue(
        "res partner",
    );
});

test("res_partner_many2one links to the record when readonly", async () => {
    await mountView({
        type: "form",
        resModel: "partner",
        resId: 1,
        arch: `<form edit="0"><field name="res_trululu" widget="res_partner_many2one"/></form>`,
    });

    expect(
        ".o_field_res_partner_many2one[name=res_trululu] a.o_form_uri",
    ).toHaveAttribute("href", "/odoo/res.partner/1");
});

test("res_partner_many2one selects another partner", async () => {
    ResPartner._records.push({ id: 2, name: "other partner" });
    ...
    const input = ".o_field_res_partner_many2one[name=res_trululu] input";
    await contains(input).click();
    await runAllTimers();
    await clickFieldDropdownItem("res_trululu", "other partner");

    expect(input).toHaveValue("other partner");
});
