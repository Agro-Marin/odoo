...
import {
    useDebugCategory,
    useOwnDebugContext,
} from "@web/services/debug/debug_context";
import { DebugMenu } from "@web/services/debug/debug_menu";
import {
    becomeSuperuser,
    regenerateAssets,
} from "@web/services/debug/debug_menu_items";
...
describe("DebugMenu", () => {
    test("can be rendered", async () => {
        ...
        expect(queryAllTexts(children)).toEqual([
            "a",
            "Item 2",
            "Item 1",
            "b",
    ...
    test("get view: basic rendering", async () => {
        ...
        await contains(".o_debug_manager button").click();
        await contains(
            ".dropdown-menu .dropdown-item:contains('Computed Arch')",
        ).click();
    ...
    test("set defaults: basic rendering", async () => {
        ...
        expect(".modal #formview_default_fields option:nth-child(2)").toHaveText(
            "Name = p1",
        );
    });
    ...
    test("fetch raw data: basic rendering", async () => {
        ...
        const fnames = Object.keys(Custom._fields).map(
            (fname) => `<field name="${fname}"/>`,
        );
    ...
    test("set defaults: settings default value with a very long value", async () => {
        ...
        expect(
            queryAllProperties(".modal #formview_default_fields option", "value"),
