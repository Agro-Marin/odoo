...
function scorePlacement(d, v, m) {
  ...
  const [leftCompensation, topCompensation] = m.containerIsInIframe ? [m.iframeBox.left, m.iframeBox.top] : [0, 0];
  const [directionSize, variantSize] = vertical ? [m.popBox.height, m.popBox.width] : [m.popBox.width, m.popBox.height];
  let [directionMin, directionMax] = vertical ? [m.contBox.top + topCompensation, m.contBox.bottom + topCompensation] : [m.contBox.left + leftCompensation, m.contBox.right + leftCompensation];
  let [variantMin, variantMax] = vertical ? [m.contBox.left + leftCompensation, m.contBox.right + leftCompensation] : [m.contBox.top + topCompensation, m.contBox.bottom + topCompensation];
  if (m.containerIsHTMLNode) {
    const [directionScroll, variantScroll] = vertical ? [m.cont.scrollTop, m.cont.scrollLeft] : [m.cont.scrollLeft, m.cont.scrollTop];
