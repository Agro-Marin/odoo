...
var FullCalendar = (function (exports) {
  ...
  var classNames = {"popoverZ":"fc-Is","isolate":"fc-C9","borderBoxRoot":"fc-Cv","notAllowed":"fc-aL","noScrollbars":"fc-TK","noShrink":"fc-JL","calendarScreenRoot":"fc-bI","safeTiles":"fc-UT","calendarPrintRoot":"fc-eY","cursorPointer":"fc-2y","cursorResizeT":"fc-Gi","cursorResizeB":"fc-Qu","cursorResizeS":"fc-q7","cursorResizeE":"fc-2S","cursorColResizer":"fc-Nx","hit":"fc-3G","hitX":"fc-ba","hitY":"fc-E7","hitXSkinny":"fc-dv","selectNone":"fc-Ov","invisible":"fc-HL","borderNone":"fc-b8","borderOnlyT":"fc-3I","borderOnlyB":"fc-cE","borderOnlyS":"fc-4d","borderOnlyE":"fc-rn","borderlessX":"fc-xL","borderlessY":"fc-aJ","fakeBorderS":"fc-xA","flexRow":"fc-hm","flexCol":"fc-6y","grow":"fc-yq","liquid":"fc-7k","minHeight0":"fc-oj","liquidX":"fc-wm","printRoot":"fc-B7","printHeader":"fc-v3","noPadding":"fc-FD","noMargin":"fc-9k","noMarginY":"fc-oD","noMarginX":"fc-qX","whiteSpaceNoWrap":"fc-u7","whiteSpacePre":"fc-W5","overflowAnchorNone":"fc-AY","crop":"fc-PW","cropNowrap":"fc-fy","rel":"fc-Fz","abs":"fc-H3","start0":"fc-TZ","fill":"fc-wh","fillTop":"fc-cD","fillX":"fc-ww","fillY":"fc-1y","fillStart":"fc-Y6","sticky":"fc-mU","stickyT":"fc-yi","stickyS":"fc-y6","tableHeaderSticky":"fc-ZS","contentBox":"fc-BC","offscreen":"fc-fd","alignCenter":"fc-jD","alignStart":"fc-NW","alignEnd":"fc-mv","footerScrollbarSticky":"fc-Hw","footerScrollbar":"fc-KM","breakInsideAvoid":"fc-MP","printSiblingRow":"fc-vf","z0":"fc-iF","z1":"fc-mp","focusZ2":"fc-BY","internalTimelineSlot":"fc-v6","internalEvent":"fc-d4","internalEventMirror":"fc-c5","internalEventDraggable":"fc-87","internalEventSelected":"fc-em","internalEventResizable":"fc-WB","internalEventResizer":"fc-Ws","internalEventResizerStart":"fc-tb","internalEventResizerEnd":"fc-lh","internalBgEvent":"fc-FB","internalMoreLink":"fc-aS","internalNavLink":"fc-DO","internalPopover":"fc-i9","internalView":"fc-Yg","internalScroller":"fc-1i"};
  ...
  */
  function createViewHookComponent(contentGenerator) {
      return (viewProps) => (u$2(ViewContextType.Consumer, { children: (context) => {
              ...
              return (u$2(ContentContainer, { tag: "div", className: joinClassNames(generateClassName(options.viewClass, renderProps), 
  ...
  */
  ...
  function getAppendableRoot(el) {
      ...
      // ``data-fc-portal-host``; the calendar hook in
      // ``hooks/full_calendar_hook.js`` sets it on the FC root element.
      // Drag mirror styling uses ``position: fixed`` (line 14024) and
      // popover ``updateSize`` (line 9655) already accounts for a
      // non-body offsetParent, so positioning is preserved.
      const host = el.closest && el.closest('[data-fc-portal-host]');
      if (host) {
          return host;
      }
  ...
  */
  ...
  class ViewContainer extends BaseComponent {
      ...
      render() {
          ...
          return (u$2(ContentContainer, { elRef: props.elRef, tag: props.tag || 'div', attrs: props.attrs, style: props.style, className: joinClassNames(props.className, generateClassName(options.viewClass, renderProps), 
              // WORKAROUND for way calendar's className would get merged into view's className
  ...
  */
  ...
  class StandardEvent extends BaseComponent {
      ...
      render() {
          ...
          // AGROMARIN FORK PATCH (FC v7.0.0-rc.3): re-inject the v6
          // ``fc-event-mirror`` class so downstream selectors
          // (`.fc-event-mirror`, etc.) keep working alongside FC v7's
          // hashed ``classNames.internalEventMirror`` (= ``fc-c5``).
          // Same pattern the fork uses for slot lanes, day cells,
          // popovers, etc. — re-inject stable v6 names without
          // disturbing FC's internal styling.
          props.isMirror && 'fc-event-mirror',
          isDraggable && classNames.internalEventDraggable, renderProps.isSelected && classNames.internalEventSelected, (renderProps.isStartResizable || renderProps.isEndResizable) && classNames.internalEventResizable);
          ...
          return (u$2(ContentContainer, { tag: tag, attrs: {
                  ...
                  '--fc-event-contrast-color': renderProps.contrastColor,
                          ...
                          'fc-event-resizer', 'fc-event-resizer-end')), children: [afterContent && (u$2(ContentContainer, { tag: 'div', style: { display: 'contents' }, attrs: { 'aria-hidden': true }, renderProps: renderProps, generatorName: undefined, customGenerator: afterContent })), Boolean(renderProps.isEndResizable && renderProps.isSelected) && (u$2("div", { className: classNames.hit }))] }))] })) }));
      }
      componentDidUpdate(prevProps) {
  ...
  */
  ...
  scrollerHeight) {
      if (!slatInnerHeight || !scrollerHeight) {
          // AGROMARIN FORK PATCH (FC v7.0.0-rc.3): fall back to the caller-
          ...
          // ``innerHeightRef`` hasn't yet arrived).  Without this, FC
          // returns [undefined, false], inlines ``height: 0`` on every
          // ``.fc-timegrid-slot-lane``, adjacent half-hour rows share the
          // same Y coordinate, and FC's ``select`` event never fires
          // because the drag distance never clears ``selectMinDistance``.
          // The fall-back is gated on ``explicitSlatMinHeight > 0`` so
          // callers who do NOT set ``slotMinHeight`` get the original
          // behaviour unchanged.  Patch is co-located with the existing
          // ``Math.max(slatInnerHeight + 1, explicitSlatMinHeight)`` rule
          // below — both treat ``explicitSlatMinHeight`` as a floor.
          if (explicitSlatMinHeight > 0 && scrollerHeight) {
              return [explicitSlatMinHeight, false];
          }
          ...
          if (initialHit) {
              if (this.useSubjectCenter && subjectRect) {
                  ...
                  if (slicedSubjectRect) {
                      ...
                      // AGROMARIN FORK PATCH (FC v7.0.0-rc.3): re-query the
                      ...
                      // this, ``hit0`` is computed at the raw cursor and
                      // ``hit1`` is computed at cursor+coordAdjust, and the
                      // resulting snap delta is asymmetric by exactly one
                      // sub-slot snap whenever the cursor lands at slot
                      // boundary while ``useSubjectCenter`` is true — see
                      // ``queryHit`` rounding at ``floor(0.5 * 2) = 1``.
                      // Symptom: drag to the same slot on a new day shifts
                      // the time by ``snapDuration`` (e.g. +15 min for
                      // ``snapDuration: {minutes: 15}`` on 30-min slots).
                      initialHit = this.initialHit = this.queryHitForOffset(adjustedPoint.left, adjustedPoint.top) || initialHit;
                  }
              }
