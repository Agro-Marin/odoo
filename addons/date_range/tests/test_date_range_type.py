...
class DateRangeTypeTest(TransactionCase):
        ...
        dr_type = self.env["date.range.type"].create(
            {
                ...
                # so the constraint passes and the assertions below (which expect
    ...
    def test_archiving_a_type_archives_its_ranges(self):
        drt = self.type.create({"name": "Cascade", "allow_overlap": True})
        dr = self.env["date.range"].create(
            {
                "name": "R",
                "date_start": "2060-01-01",
                "date_end": "2060-12-31",
                "type_id": drt.id,
            }
        )
        self.env.flush_all()
        self.assertTrue(dr.active)
        drt.active = False
        self.env.flush_all()
        self.assertFalse(dr.active)
