...
class TestSaleTimesheet(TestCommonSaleTimesheet):
    ...
    def test_compute_commercial_partner(self):
        ...
        commercial_partner = self.env["res.partner"].create(
            {"name": "Commercial Partner", "is_company": True}
        )
        sub_partner = self.env["res.partner"].create(
            {"name": "Sub Partner", "parent_id": commercial_partner.id}
        )
        ...
        timesheet.with_user(
            timesheet_manager_no_project_user
        )._compute_commercial_partner_id()
        self.assertEqual(
    ...
    def test_timesheet_order(self):
        ...
        self.assertEqual(
            so_line_ordered_global_project.qty_transferred,
            10.5,
        ...
        self.assertEqual(
            timesheet2.timesheet_invoice_type,
            "billable_fixed",
            "Timesheets linked to SO line with ordered product shoulbe be billable fixed",
        )
        self.assertFalse(
        ...
        self.assertEqual(
            so_line_ordered_project_only.qty_transferred,
            0.0,
            "Timesheet directly on project does not increase delivered quantity on so line",
        )
        self.assertEqual(
            timesheet3.timesheet_invoice_type,
            "non_billable",
            "Timesheets without SO should be be 'non-billable'",
        )
        self.assertFalse(
        ...
        invoice_line_1 = so_line_ordered_global_project.invoice_line_ids.filtered(
            lambda line: line.move_id == invoice1
        )

        self.assertEqual(
            so_line_ordered_global_project.product_qty,
        ...
        self.assertTrue(
            float_is_zero(
                invoice2.amount_total - so_line_ordered_task_in_project.price_unit * 3,
                precision_digits=2,
            ),
    ...
    def test_timesheet_delivery(self):
        ...
        self.assertEqual(
            sale_order.project_account_id,
            task_serv2.project_id.account_id,
        ...
        self.assertEqual(
            so_line_deliver_task_project.invoice_state,
            "no",
        ...
        self.assertTrue(
            float_is_zero(
                so_line_deliver_only_project.qty_transferred, precision_digits=2
            ),
        ...
        self.assertEqual(
            timesheet3.timesheet_invoice_type,
            "billable_time",
            "Timesheets with an amount > 0 should be 'billable time'",
        )
        self.assertFalse(
            timesheet3.timesheet_invoice_id,
    ...
    def test_timesheet_manual(self):
        ...
        sale_order.action_confirm()
        self.assertTrue(
            sale_order.project_ids, "Sales Order should have create a project"
        )
        self.assertEqual(
            sale_order.invoice_state,
            "no",
        ...
        self.assertEqual(
            timesheet1.timesheet_invoice_type,
            "billable_manual",
            "Milestone timesheet goes in billable manual category",
        )
        self.assertTrue(
            float_is_zero(
                so_line_manual_global_project.qty_transferred, precision_digits=2
            ),
            "Milestone Timesheeting should not incremented the delivered quantity on the SO line",
        )
        self.assertEqual(
            so_line_manual_global_project.qty_to_invoice,
            0.0,
            "Manual service should not be affected by timesheet on their created task.",
        )
        self.assertEqual(
            so_line_manual_only_project.qty_to_invoice,
            0.0,
        ...
        self.assertEqual(
            timesheet1.timesheet_invoice_type,
            "billable_manual",
            "Timesheets linked to SO line with ordered product shoulbe be billable fixed since it is a prepaid product.",
        )
        self.assertEqual(
            timesheet2.timesheet_invoice_type,
            "non_billable",
    ...
    def test_timesheet_invoice(self):
        ...
        with self.assertRaises(
            AccessError, msg="The user should not have access to the SOL"
        ):
            so_line_deliver_timesheet.with_user(
                self.user_employee_without_sales_access
            ).read(["name"])
        ...
        self.assertEqual(
            so_line_deliver_global_project.qty_invoiced, timesheet1.unit_amount
        )
        ...
        self.assertEqual(
            so_line_deliver_global_project.qty_invoiced,
            timesheet1.unit_amount + timesheet2.unit_amount + timesheet3.unit_amount,
        )
        self.assertTrue(so_line_deliver_task_project.invoice_line_ids)
        self.assertEqual(
            so_line_deliver_task_project.qty_invoiced, timesheet4.unit_amount
        )
    ...
    def test_transfert_project(self):
        ...
        timesheet_count1 = Timesheet.search_count(
            [("project_id", "=", self.project_global.id)]
        )
        timesheet_count2 = Timesheet.search_count(
            [("project_id", "=", self.project_template.id)]
        )
        ...
        self.assertEqual(
            len(task.timesheet_ids), 1, "The timesheet should be linked to task"
        )
        ...
        self.assertEqual(
            len(task.timesheet_ids), 1, "The timesheet still should be linked to task"
        )
        ...
        self.assertEqual(
            ...
            2,
            "2 timesheets in project_global",
        )
        ...
        self.assertEqual(timesheet_count2, 1, "One timesheet in project_template")
        self.assertEqual(
            len(task.timesheet_ids),
            2,
            "The 2 timesheets still should be linked to task",
        )
    ...
    def test_change_customer_and_SOL_after_invoiced_timesheet(self):
        ...
        task_so1 = self.env["project.task"].search(
            [("sale_line_id", "=", so1_product_global_project_so_line.id)]
        )
        task_so2 = self.env["project.task"].search(
        ...
        self.assertEqual(
            self.partner_a,
            task_so1.partner_id,
            "The Customer of the first task should be equal to partner_a.",
        )
        self.assertEqual(
            self.partner_b,
            task_so2.partner_id,
            "The Customer of the second task should be equal to partner_b.",
        )
        self.assertEqual(
            sale_order1.partner_id,
            task_so1.partner_id,
            "The Customer of the first task should be equal to the Customer of the first Sales Order.",
        )
        self.assertEqual(
            sale_order2.partner_id,
            task_so2.partner_id,
            "The Customer of the second task should be equal to the Customer of the second Sales Order.",
        )
        ...
        self.assertEqual(
            self.partner_a,
            task_so1_timesheet1.partner_id,
            "The Task's Timesheet entry should have the same partner than on the task 1 and Sales Order 1.",
        )
        ...
        self.assertEqual(
            self.partner_a,
            task_so1_timesheet1.partner_id,
            "The Task's first Timesheet entry should not have changed as it was already invoiced (its partner should still be partner_a).",
        )
        self.assertEqual(
            self.partner_b,
            task_so1_timesheet2.partner_id,
            "The Task's second Timesheet entry should have its partner changed, as it was not invoiced and the Task's partner/customer changed.",
        )
        self.assertEqual(
            so1_product_global_project_so_line,
            task_so1_timesheet1.so_line,
            "The Task's first Timesheet entry should not have changed as it was already invoiced (its so_line should still be equal to the first Sales Order line).",
        )
        self.assertEqual(
            so2_product_global_project_so_line,
            task_so1_timesheet2.so_line,
            "The Task's second Timesheet entry should have it's so_line changed, as the Sales Order Item of the Task changed, and this entry was not invoiced.",
        )
    ...
    def test_timesheet_upsell(self):
        ...
        last_message_id = (
            ...
            .search(
                [("model", "=", "sale.order"), ("res_id", "=", sale_order.id)], limit=1
            )
            .id
            or 0
        )
        ...
        self.assertEqual(
            len(message_sent),
            1,
            "Sale Timesheet: An email should always be sent to the saleperson when the state of the sale order change to upselling",
        )
        ...
        self.assertEqual(
            len(message_sent),
            1,
            "Sale Timesheet: An email should only be sent to the saleperson when the state of the sale order change to upselling",
        )
    ...
    def test_unlink_timesheet(self):
        ...
        self.assertEqual(
            analytic_line.timesheet_invoice_id,
            move,
            "The timesheet should be linked to move",
        )

        move.with_context(check_move_validity=False).line_ids[0].unlink()
        self.assertFalse(
            analytic_line.timesheet_invoice_id,
            "The timesheet should have been unlinked from move",
        )
    ...
    def test_update_sol_price(self):
        ...
        self.assertEqual(
            items["revenues"]["data"][0]["to_invoice"],
            product_price,
            "The quantity to_invoice should be equal to the price of the product",
        )
        ...
        self.assertEqual(
            items["revenues"]["data"][0]["to_invoice"],
            2 * product_price,
            "The quantity to_invoice should be equal to twice the price of the product",
        )
    ...
    def test_sale_order_with_multiple_project_templates(self):
        ...
        sale_order_line_template_1, sale_order_line_template_2 = self.env[
            "sale.order.line"
        ].create(
            [
                {
    ...
    def test_onchange_uom_service_product(self):
        ...
        products = [
            product_1,
            product_2,
        ]  # perform the tests for both product and variants
        for product in products:
            # 1. product.template form: [uom: unit] --> change to service --> [uom: hour]
            with Form(
                product, view="sale_timesheet.view_product_timesheet_form"
            ) as product_form:
    ...
    def test_allocated_hours_copy(self):
        ...
        self.assertTrue(
            ...
            == project_template.allocated_hours
    ...
    def test_non_consolidated_billing_service_timesheet(self):
        ...
        advance_payment = (
            self.env["sale.advance.payment.inv"]
            .with_context(active_ids=sale_orders.ids)
            .create(
                {
        ...
        self.assertEqual(
            len(invoices),
            2,
            "The number of invoices created should be equal to the number of sales orders.",
        )
    ...
    def test_linked_timesheet_after_invoice_reversal(self):
        ...
        self.assertEqual(
            ...
            invoice,
            "Timesheet should be linked to the invoice",
        )
        reversal_wizard = (
        ...
        self.assertFalse(
            timesheet.timesheet_invoice_id,
            "Timesheet should not be linked to the invoice after reversal",
        )
        timesheet.write({"unit_amount": 7})
        self.assertEqual(
            timesheet.unit_amount,
            7,
            "It Should be possible to edit timesheet after invoice reversal",
        )
        ...
        self.assertEqual(
            timesheet1.timesheet_invoice_id,
            invoice2,
            "Timesheet1 should be linked to the invoice",
        )
        self.assertEqual(
            timesheet2.timesheet_invoice_id,
            invoice2,
            "Timesheet2 should be linked to the invoice",
        )
        ...
        self.assertFalse(
            timesheet1.timesheet_invoice_id,
            "Timesheet1 should be cleared after partial refund of its task",
        )
        self.assertEqual(
            timesheet2.timesheet_invoice_id,
            invoice2,
            "Timesheet2 should still be linked to the original invoice",
        )
...
class TestSaleTimesheetAnalyticPlan(TestCommonSaleTimesheet):
    ...
    def test_timesheet_get_accounts_from_sol(self):
        project_analytic_plan, _other_plans = self.env[
            "account.analytic.plan"
        ]._get_all_plans()
        other_analytic_plan2 = self.plan_b
        analytic_account1, analytic_account2 = self.env[
        ...
        so_line1 = self.env["sale.order.line"].create(
            {
                ...
                "analytic_distribution": {
                    f"{analytic_account1.id}, {analytic_account2.id}": 100
                },
            }
        )
        timesheet = self.env["account.analytic.line"].create(
        ...
        self.assertEqual(
            timesheet[other_analytic_plan2._column_name()], analytic_account2
        )
        ...
        so_line2 = self.env["sale.order.line"].create(
            {
                ...
                "order_id": sale_order.id,
                "analytic_distribution": {
                    f"{analytic_account1.id}, {analytic_account3.id}": 100
                },
            }
        )
        ...
        self.assertEqual(
            timesheet[other_analytic_plan3._column_name()], analytic_account3
        )

    def test_timesheet_get_accounts_from_sol_fallback_on_project(self):
        _project_analytic_plan, _other_plans = self.env[
            "account.analytic.plan"
        ]._get_all_plans()
        other_analytic_plan2 = self.plan_b
