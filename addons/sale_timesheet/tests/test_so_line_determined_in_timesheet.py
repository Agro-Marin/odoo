...
class TestSoLineDeterminedInTimesheet(TestCommonSaleTimesheet):
    ...
    def test_sol_determined_when_project_is_project_rate(self):
        ...
        self.assertTrue(timesheet.so_line == task.sale_line_id == self.so.line_ids[1], "The SOL in the timesheet should also change and be the same than the one in the task.")
    ...
    def test_sol_determined_when_project_is_employee_rate(self):
        ...
        self.assertTrue(timesheet.so_line == task.sale_line_id == self.so.line_ids[2], "The SOL in the timesheet should also change and be the same than the one in the task.")
        ...
        self.assertTrue(employee_user_timesheet.so_line == mapping.sale_line_id == self.so.line_ids[-1], "The SOL in the timesheet done by the employee user should be the one defined in the mapping.")
    ...
    def test_tranfer_project(self):
        ...
        # 2) Check if the task has the SOL which contain the prepaid service product,
        self.assertEqual(task.sale_line_id, self.so.line_ids[-1], 'The SOL with prepaid service product should be linked to the task.')
