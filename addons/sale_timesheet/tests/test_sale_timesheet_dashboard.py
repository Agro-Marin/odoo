...
from odoo.addons.sale_project.tests.test_sale_project_dashboard import (
    TestProjectDashboardCommon as Common,
)
...
class TestSaleTimesheetDashboard(Common):
    ...
    def test_get_sale_item_data_various_sol_with_timesheet_installed(self):
        ...
        sale_item_data = self.dashboard_project.get_sale_items_data(
            with_action=False, limit=5, section_id="materials"
        )
        ...
        self.assertEqual(sale_item_data["sol_items"][0], expected_dict[2])
        sale_item_data = self.dashboard_project.get_sale_items_data(
            with_action=False, limit=5, section_id="billable_fixed"
        )
        self.assertEqual(sale_item_data["sol_items"][0], expected_dict[1])
        sale_item_data = self.dashboard_project.get_sale_items_data(
            with_action=False, limit=5, section_id="billable_milestones"
        )
        self.assertEqual(sale_item_data["sol_items"][0], expected_dict[0])
        sale_item_data = self.dashboard_project.get_sale_items_data(
            with_action=False, limit=5, section_id="billable_time"
        )
        self.assertEqual(sale_item_data["sol_items"][0], expected_dict[3])
        sale_item_data = self.dashboard_project.get_sale_items_data(
            with_action=False, limit=5, section_id="billable_manual"
        )
        self.assertEqual(sale_item_data["sol_items"][0], expected_dict[4])
