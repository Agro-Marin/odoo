...
class TestProjectBilling(TestCommonSaleTimesheet):
    ...
    def setUpClass(cls):
        ...
        cls.project_task_rate2 = cls.env["project.project"].search(
            [("sale_line_id", "=", cls.so2_line_deliver_project_template.id)], limit=1
        )
        ...
        cls.project_employee_rate_manager = cls.env[
        ...
        cls.project_employee_rate_user = cls.env[
            "project.sale.line.employee.map"
        ].create(
            {
                "project_id": cls.project_employee_rate.id,
    ...
    def test_billing_employee_rate(self):
        ...
        self.assertTrue(
            task.allow_billable, "Task in project 'employee rate' should be billable"
        )
        self.assertEqual(
            task.pricing_type,
            "employee_rate",
            "Task in project 'employee rate' should be billed at employee rate",
        )
        self.assertEqual(
            task.sale_line_id,
            self.project_employee_rate.sale_line_id,
            "Task created in a project billed on 'employee rate' should be linked to the SOL defined in the project.",
        )
        self.assertEqual(
            task.partner_id,
        ...
        self.assertFalse(
            timesheet1.so_line,
            "The timesheet should be not linked to the project of the map entry since no SOL in the linked task.",
        )

        task.write({"sale_line_id": self.project_employee_rate_user.sale_line_id.id})

        self.assertEqual(
            self.project_employee_rate_manager.sale_line_id,
            timesheet1.so_line,
            "The timesheet should be linked to the SOL associated to the Employee manager in the map",
        )
        self.assertEqual(
            self.project_employee_rate_manager.project_id,
        ...
        self.assertFalse(
            subtask.allow_billable,
            "Subtask in non billable project should be non billable too",
        )
        self.assertFalse(
        ...
        self.assertEqual(
            subtask.project_id,
            timesheet2.project_id,
            "The timesheet is in the subtask project",
        )
        self.assertNotEqual(
            self.project_employee_rate_user.project_id,
            timesheet2.project_id,
            "The timesheet should not be linked to the billing project for the map",
        )
        self.assertFalse(
            timesheet2.so_line,
        ...
        self.assertTrue(
            task.allow_billable,
            "Task in project 'task rate' should be billed at task rate",
        )
        self.assertEqual(
            task.sale_line_id,
            self.so1_line_deliver_no_task,
            "The task should keep the same SOL since the partner_id has not changed when the project of the task has changed.",
        )
        self.assertEqual(
            task.partner_id,
            self.partner_a,
        ...
        self.assertTrue(
            subtask.allow_billable,
            "Subtask should keep the billable type from its parent, even when they are moved into another project",
        )
        self.assertEqual(
        ...
        self.assertFalse(
            timesheet4.so_line,
            "The timesheet should not be linked to SOL, as no entry for TDE in project map",
        )
    ...
    def test_billing_task_rate(self):
        ...
        self.assertEqual(
            task.sale_line_id,
            self.so2_line_deliver_project_task,
            "Task created in a project billed on 'task rate' should be linked to a SOL containing a prepaid service product and the remaining hours of this SOL should be greater than 0.",
        )
        self.assertEqual(
            task.partner_id,
            task.project_id.partner_id,
        ...
        self.assertEqual(
            task.sale_line_id,
            self.project_task_rate.sale_line_id,
            "Task moved in a employee rate billable project should keep its SOL because the partner_id has not changed too.",
        )
        self.assertEqual(
            task.partner_id,
            self.project_task_rate.partner_id,
            "Task created in a project billed on 'employee rate' should have the same customer as the one from its initial project.",
        )

        self.assertEqual(
            subtask.sale_line_id,
            subtask.parent_id.sale_line_id,
            "Subtask moved in a employee rate billable project should have the SOL of its parent since it keep its partner_id and this partner is different than the one in the destination project.",
        )
        self.assertEqual(
            subtask.partner_id,
            subtask.parent_id.partner_id,
            "Subtask moved in a project billed on 'employee rate' should keep its initial customer, that is the one of its parent.",
        )
    ...
    def test_customer_change_in_project(self):
        ...
        self.assertFalse(
            self.project_project_rate.sale_order_id,
            "The SO in the project should be False because the previous SO customer does not match the actual customer of the project.",
        )
        self.assertFalse(
            self.project_project_rate.sale_line_id,
        ...
        self.assertEqual(
            self.project_project_rate.pricing_type,
            "task_rate",
        ...
        self.assertFalse(
            self.project_employee_rate.sale_order_id,
            "The SO in the project should be False because the previous SO customer does not match the actual customer of the project.",
        )
        self.assertFalse(
            self.project_employee_rate.sale_line_id,
        ...
        self.assertFalse(
            self.project_employee_rate_user.sale_line_id,
            "The SOL in the mapping should be False because the actual customer in the project has not this SOL.",
        )
        self.assertEqual(
            self.project_employee_rate.pricing_type,
            "employee_rate",
            "Since the mappings have not been removed, the pricing type should remain the same, that is employee rate.",
        )
    ...
    def test_project_form_view(self):
        ...
        with Form(
            ...
            with project_form.sale_line_employee_ids.new() as mapping_form:
                ...
                mapping_form.sale_line_id = self.so.line_ids[:1]
            self.assertEqual(
                project_form.partner_id,
                self.so.partner_id,
                "The partner should be the one defined the SO linked to the SOL defined in the mapping.",
            )
            project = project_form.save()
            self.assertEqual(
                project.pricing_type,
                "employee_rate",
                "Since there is a mapping in this project, the pricing type should be employee rate.",
            )
    ...
    def test_take_into_account_invoicing_app_legacy(self):
        ...
        self.assertEqual(
            self.so1_line_deliver_no_task.qty_transferred, timesheet1.unit_amount
        )
        ...
        self.assertEqual(
            self.so1_line_deliver_no_task.qty_transferred,
            timesheet1.unit_amount + timesheet2.unit_amount,
        )
        invoice1.write({"state": "cancel", "payment_state": "invoicing_legacy"})
        self.assertEqual(
            self.so1_line_deliver_no_task.qty_invoiced, timesheet1.unit_amount
        )
        invoice2 = self.sale_order_1._create_invoices()[0]
        invoice2.action_post()
        self.assertEqual(
            self.so1_line_deliver_no_task.qty_invoiced,
            timesheet1.unit_amount + timesheet2.unit_amount,
        )
        self.assertEqual(timesheet1.timesheet_invoice_id, invoice1)
