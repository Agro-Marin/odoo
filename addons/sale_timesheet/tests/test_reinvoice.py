...
class TestReInvoice(TestCommonSaleTimesheet):
    ...
    def test_at_cost(self):
        ...
        self.assertEqual(
            sale_order_line1.qty_transferred_method,
            "timesheet",
            "Delivered quantity of 'service' SO line should be computed by timesheet amount",
        )
        self.assertEqual(
            sale_order_line2.qty_transferred_method,
            "timesheet",
            "Delivered quantity of 'service' SO line should be computed by timesheet amount",
        )
        ...
        sale_order_line3 = self.sale_order.line_ids.filtered(
            lambda sol: (
                sol != sale_order_line1
                and sol.product_id == self.company_data["product_order_cost"]
            )
        )
        sale_order_line4 = self.sale_order.line_ids.filtered(
            lambda sol: (
                sol != sale_order_line2
                and sol.product_id == self.company_data["product_delivery_cost"]
            )
        )
        ...
        self.assertEqual(
            len(self.sale_order.line_ids),
            4,
            "There should be 4 lines on the SO (2 vendor bill lines created)",
        )
        self.assertEqual(
            len(self.sale_order.line_ids.filtered(lambda sol: sol.is_expense)),
            2,
            "There should be 4 lines on the SO (2 vendor bill lines created)",
        )

        self.assertEqual(
            sale_order_line1.qty_transferred,
            1,
            "Exising SO line 1 should not be impacted by reinvoicing product at cost",
        )
        self.assertEqual(
            sale_order_line2.qty_transferred,
            0,
            "Exising SO line 2 should not be impacted by reinvoicing product at cost",
        )

        self.assertFalse(
            sale_order_line3.task_id,
            "Adding a new expense SO line should not create a task (sol3)",
        )
        self.assertFalse(
            sale_order_line4.task_id,
            "Adding a new expense SO line should not create a task (sol4)",
        )
        self.assertEqual(
            len(self.sale_order.line_ids.mapped("project_id")),
            1,
            "SO create only one project with its service line. Adding new expense SO line should not impact that",
        )

        self.assertEqual(
            (
                sale_order_line3.price_unit,
                sale_order_line3.qty_transferred,
                sale_order_line3.product_qty,
                sale_order_line3.qty_invoiced,
            ),
            (self.company_data["product_order_cost"].standard_price, 3.0, 3.0, 0),
            "Sale line is wrong after confirming vendor invoice",
        )
        self.assertEqual(
            (
                sale_order_line4.price_unit,
                sale_order_line4.qty_transferred,
                sale_order_line4.product_qty,
                sale_order_line4.qty_invoiced,
            ),
            (self.company_data["product_delivery_cost"].standard_price, 3.0, 3.0, 0),
            "Sale line is wrong after confirming vendor invoice",
        )

        self.assertEqual(
            sale_order_line3.qty_transferred_method,
            "analytic",
            "Delivered quantity of 'expense' SO line should be computed by analytic amount",
        )
        self.assertEqual(
            sale_order_line4.qty_transferred_method,
            "analytic",
            "Delivered quantity of 'expense' SO line should be computed by analytic amount",
        )
        ...
        sale_order_line5 = self.sale_order.line_ids.filtered(
            lambda sol: (
                sol not in (sale_order_line1, sale_order_line3)
                and sol.product_id == self.company_data["product_order_cost"]
            )
        )
        sale_order_line6 = self.sale_order.line_ids.filtered(
            lambda sol: (
                sol not in (sale_order_line2, sale_order_line4)
                and sol.product_id == self.company_data["product_delivery_cost"]
            )
        )
        ...
        self.assertEqual(
            (
                sale_order_line5.price_unit,
                sale_order_line5.qty_transferred,
                sale_order_line5.product_qty,
                sale_order_line5.qty_invoiced,
            ),
            (self.company_data["product_order_cost"].standard_price, 2.0, 2.0, 0),
            "Sale line 5 is wrong after confirming 2e vendor invoice",
        )
        self.assertEqual(
            (
                sale_order_line6.price_unit,
                sale_order_line6.qty_transferred,
                sale_order_line6.product_qty,
                sale_order_line6.qty_invoiced,
            ),
            (self.company_data["product_delivery_cost"].standard_price, 2.0, 2.0, 0),
    ...
    def test_sales_price(self):
        ...
        sale_order_line3 = self.sale_order.line_ids.filtered(
            lambda sol: (
                sol != sale_order_line1
                and sol.product_id == self.company_data["product_delivery_sales_price"]
            )
        )
        sale_order_line4 = self.sale_order.line_ids.filtered(
            lambda sol: (
                sol != sale_order_line2
                and sol.product_id == self.company_data["product_order_sales_price"]
            )
        )
        ...
        self.assertEqual(
            ...
            (self.company_data["product_delivery_sales_price"].list_price, 3.0, 3.0, 0),
            "Sale line is wrong after confirming vendor invoice",
        )
        self.assertEqual(
            (
            ...
            (self.company_data["product_order_sales_price"].list_price, 3.0, 3.0, 0),
            "Sale line is wrong after confirming vendor invoice",
        )
        ...
        self.assertFalse(
            sale_order_line5,
            "No new sale line should have been created with delivered product !!",
        )
        self.assertTrue(
            sale_order_line6,
            "A new sale line should have been created with ordered product",
        ...
        self.assertEqual(
            ...
            (self.company_data["product_order_sales_price"].list_price, 2.0, 3.0, 0),
    ...
    def test_no_expense(self):
        ...
        self.assertTrue(
            invoice_a.mapped("line_ids.analytic_line_ids"),
    ...
    def test_project_update_reinvoiced_vendor_bill_product(self):
        ...
        self.assertTrue(
            project, "Project should have been created on sale order confirmation"
        )
        ...
        vendor_bill = self.env["account.move"].create(
            {
                ...
                .search(
                    [("code", "=", "BILL"), ("company_id", "=", self.env.company.id)]
                )
                .id,
        ...
        self.assertEqual(
            ...
            1,
            "Vendor bill should be linked to project account",
        )
        self.assertTrue(
            vendor_bill.line_ids.analytic_line_ids,
            "Analytic line should be created for the account move line",
        )
        self.assertTrue(
            sale_order.line_ids.analytic_line_ids,
        ...
        self.assertEqual(
            updates["costs"]["total"]["billed"],
            -20,
            "Only the vendor bill should be deducted",
        )
