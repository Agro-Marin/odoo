...
class AccountAnalyticLine(models.Model):
    ...
    commercial_partner_id = fields.Many2one(
        "res.partner", compute="_compute_commercial_partner_id"
    )
    ...
    @api.depends(
        "project_id.partner_id.commercial_partner_id",
        "task_id.partner_id.commercial_partner_id",
    )
    def _compute_commercial_partner_id(self):
        for timesheet in self:
            timesheet.commercial_partner_id = (
                timesheet.task_id.sudo().partner_id.commercial_partner_id
                or timesheet.project_id.sudo().partner_id.commercial_partner_id
            )
        ...
        for timesheet in self:
            if timesheet.project_id:  # AAL will be set to False
                ...
                if not timesheet.so_line:
                    invoice_type = (
                        "non_billable"
                        if timesheet.project_id.billing_type != "manually"
                        else "billable_manual"
                    )
    ...
    def _compute_partner_id(self):
        super(
            AccountAnalyticLine, self.filtered(lambda t: t._is_not_billed())
        )._compute_partner_id()
    ...
    def _check_timesheet_can_be_billed(self):
        return (
            self.so_line
            in self.project_id.mapped("sale_line_employee_ids.sale_line_id")
    ...
    def _check_can_write(self, values):
        ...
        if self.sudo().filtered(
            ...
            lambda t: (
                t.timesheet_invoice_id and t.timesheet_invoice_id.state != "cancel"
            )
        ):
            if any(
            ...
            if self.task_id.pricing_type in ("task_rate", "fixed_rate"):
                ...
                map_entry = self.project_id.sale_line_employee_ids.filtered(
                    lambda map_entry: (
                        ...
                        == (self.employee_id or self.env.user.employee_id)
                        and map_entry.sale_line_id.partner_id.commercial_partner_id
    ...
    def _unlink_except_invoiced(self):
        if any(
            line.timesheet_invoice_id and line.timesheet_invoice_id.state == "posted"
            for line in self
        ):
            raise UserError(
    ...
    def _timesheet_convert_sol_uom(self, sol, to_unit):
        ...
        return round(
            sol.product_uom_id._compute_quantity(
                sol.product_qty, to_uom, raise_if_failure=False
            ),
            2,
        )
    ...
    def _timesheet_preprocess_get_accounts(self, vals):
        ...
        plan_column_names = {
            account.root_plan_id._column_name() for account in accounts
        }
        mandatory_plans = [
            plan
        ...
        missing_plan_names = [
            ...
            if plan["column_name"] not in plan_column_names
        ]
