...
class SaleOrderLine(models.Model):
    ...
    qty_transfered_method = fields.Selection(selection_add=[('timesheet', 'Timesheets')])
    ...
    @api.depends('qty_transfered', 'product_uom_qty', 'analytic_line_ids')
    def _compute_remaining_hours(self):
        uom_hour = self.env.ref('uom.product_uom_hour')
        for line in self:
            remaining_hours = None
            if line.remaining_hours_available:
                qty_left = line.product_uom_qty - line.qty_transfered
    ...
    def _compute_qty_transfered_method(self):
        ...
        for line in self:
            if not line.is_expense and line.product_id.type == 'service' and line.product_id.service_type == 'timesheet':
                line.qty_transfered_method = 'timesheet'
    ...
    def _compute_qty_transfered(self):
        ...
        lines_by_timesheet = self.filtered(lambda sol: sol.qty_transfered_method == 'timesheet')
        domain = lines_by_timesheet._timesheet_compute_delivered_quantity_domain()
        mapping = lines_by_timesheet.sudo()._get_delivered_quantity_by_analytic(domain)
        for line in lines_by_timesheet:
            line.qty_transfered = mapping.get(line.id or line._origin.id, 0.0)
