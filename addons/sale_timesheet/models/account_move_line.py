    ...
    def unlink(self):
        move_line_read_group = self.env["account.move.line"].search_read(
            [
                ...
                ("move_id.state", "=", "draft"),
                ("sale_line_ids.product_id.invoice_policy", "=", "transferred"),
        ...
        for move_line in move_line_read_group:
            sale_line_ids_per_move[move_line["move_id"][0]] += self.env[
                "sale.order.line"
            ].browse(move_line["sale_line_ids"])
        ...
        self.sudo().env["account.analytic.line"].browse(timesheet_ids).write(
            {"timesheet_invoice_id": False}
        )
        return super().unlink()
