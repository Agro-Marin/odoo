...
class ProjectProject(models.Model):
    ...
    def _search_pricing_type(self, operator, value):
        ...
        if "employee_rate" in value:
            domains.append(
                [("sale_line_employee_ids", "!=", False), ("allow_billable", "=", True)]
            )
        if False in value:
    ...
    def _compute_warning_employee_rate(self):
        ...
        dict_project_employee = {
            project.id: employee_ids for project, employee_ids in employees
        }
    ...
    def _compute_partner_id(self):
        ...
        for project in billable_projects:
            ...
            if (
                ...
                and project.allow_timesheets
                and project.pricing_type != "task_rate"
            ):
                sol = (
    ...
    def _compute_billing_type(self):
        self.filtered(
            lambda project: (
                (not project.allow_billable or not project.allow_timesheets)
                and project.billing_type == "manually"
            )
    ...
    def action_profitability_items(self, section_name, domain=None, res_id=False):
        ...
        if section_name in [
            ...
            action["context"].update(
                search_default_groupby_timesheet_invoice_type=False, **self.env.context
            )
            graph_view = False
            if section_name == "billable_time":
                graph_view = self.env.ref(
                    "sale_timesheet.view_hr_timesheet_line_graph_invoice_employee"
                ).id
            action["views"] = [
                (view_id, view_type)
                if view_type != "graph"
                else (graph_view or view_id, view_type)
    ...
    def _get_sale_order_items_query(self, domain_per_model=None):
        ...
        query._tables["project_sale_order_item"] = SQL(
            "(%s)",
            SQL(" UNION ").join(
                [
    ...
    def _get_domain_from_section_id(self, section_id):
        section_domains = {
            "materials": [("product_id.type", "!=", "service")],
            "billable_fixed": [
    ...
    def _get_profitability_items(self, with_action=True):
        return self._get_profitability_items_from_aal(
            super()._get_profitability_items(with_action), with_action
        )

    def _get_project_to_template_warnings(self):
        res = super()._get_project_to_template_warnings()
        timesheet_linked_count = self.env["account.analytic.line"].search_count(
