...
class ProductTemplate(models.Model):
    ...
    def _compute_service_upsell_threshold_ratio(self):
        ...
        for record in self:
            if (
                not record.uom_id
                or record.uom_id != uom_unit
                or product_uom_hour.factor == record.uom_id.factor
            ):
    ...
    def _onchange_service_fields(self):
        for record in self:
            if (
                record.type == "service"
                and record.service_type == "timesheet"
                and not (
                    record._origin.service_policy
                    and record.service_policy == record._origin.service_policy
                )
            ):
    ...
    @api.model
    def _get_onchange_service_policy_updates(
        self, service_tracking, service_policy, project_id, project_template_id
    ):
        vals = {}
    ...
    def _onchange_service_policy(self):
        ...
        vals = self._get_onchange_service_policy_updates(
