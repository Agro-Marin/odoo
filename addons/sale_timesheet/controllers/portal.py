...
class PortalProjectAccount(PortalAccount, ProjectCustomerPortal):
    def _invoice_get_page_view_values(self, invoice, access_token, /, **kwargs):
        ...
        values["timesheets"] = (
            request.env["account.analytic.line"].sudo().search(domain)
        )
        values["is_uom_day"] = (
            request.env["account.analytic.line"].sudo()._is_timesheet_encode_uom_day()
        )
        return values
...
class SaleTimesheetCustomerPortal(TimesheetCustomerPortal):
    ...
    def _get_search_domain(self, search_in, search):
        if search_in == "so":
            ...
            invoices = (
                ...
                .sudo()
                .search(["|", ("name", "ilike", search), ("id", "ilike", search)])
            )
            return Domain(
                request.env["account.analytic.line"]._timesheet_get_sale_domain(
    ...
    def _task_get_page_view_values(self, task, access_token, /, **kwargs):
        ...
        try:
            if task.sale_order_id and self._document_check_access(
                "sale.order", task.sale_order_id.id
            ):
