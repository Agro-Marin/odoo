...
registry.category("web_tour.tours").add("sale_timesheet_tour", {
    ...
    steps: () => [
        ...
        {
            trigger: 'div[name="product_qty"] input',
        ...
        },
        {
            ...
            content:
                "Click on Confirm button to create a sale order with this quotation.",
            run: "click",
        },
        {
            content:
                'Wait for the confirmation to finish. State should be "Sales Order"',
            trigger:
                '.o_field_widget[name=state] .o_arrow_button_current:contains("Sales Order")',
        },
        ...
        },
        {
            ...
            content:
                "Click on Create button to create and enter to this newest project.",
            run: "click",
        },
        {
        ...
        {
            content: "wait for the board column every project is seeded with",
            trigger:
                ".o_kanban_renderer .o_kanban_group .o_kanban_header_title:contains(New)",
        },
        {
            trigger: "button.o-kanban-button-new",
        ...
        },
        {
            ...
            content:
                "The so_line field should be hidden by default. We check if it is the case by adding this field in the timesheet list view",
            run: "click",
        },
        {
        ...
        },
        {
            ...
            content:
                "Manually save the records (sale order should be filled based on the partner picked for this task",
            run: "click",
        },
        {
            trigger: 'button[name="action_view_so"]',
            content:
                "Click on this stat button to see the SO linked to the SOL of the task.",
            run: "click",
        },
        {
            ...
            run({ queryFirst }) {
                ...
                for (let i = 0; i < tr.children.length; i++) {
                    ...
                    if (th.dataset && th.dataset.name === "qty_transferred") index = i;
                }
                const qtyDelivered = queryFirst(
                    `tbody > tr:first-child > td.o_data_cell:eq(${index})`,
                    { root: this.anchor },
                );
                if (qtyDelivered.textContent !== "1.00")
                    console.error(
                        "The quantity delivered on this Sales Order Item should be equal to 1.00 hour. qtyDelivered = " +
                            qtyDelivered,
                    );
            },
        },
        ...
        },
        {
            ...
            content:
                "Click on Create button to create a new project and see the different configuration available for the project.",
            run: "click",
        },
        {
        ...
        },
        {
            trigger: 'div[name="partner_id"] input',
            content: markup(
                "Add the customer for this project to select an SO and SOL for this customer <i>(e.g. Brandon Freeman)</i>.",
            ),
            run: "edit Brandon Freeman",
        },
        {
        ...
        },
        {
            ...
            content:
                "Select a Sales Order Item as Default Sales Order Item for each task in this project.",
            run: "edit S",
        },
        {
            trigger:
        ...
        },
        {
            ...
            content:
                "Click on Invoicing tab to configure the invoicing of this project.",
            run: "click",
        },
        {
            trigger:
                'div[name="sale_line_employee_ids"] td.o_field_x2many_list_row_add > a[role="button"]',
        ...
        },
        {
            ...
            content:
                "Select an employee to link a Sales Order Item on his timesheets into this project.",
            run: "click",
        },
        {
            trigger:
        ...
        {
            trigger:
                'div[name="sale_line_employee_ids"] div[name="sale_line_id"] input',
            content:
                "Select the Sales Order Item to link to the timesheets of this employee.",
            tooltipPosition: "bottom",
            run: "edit S",
        },
        {
        ...
        {
            trigger:
                ".o_rightpanel_section[name='profitability'] .o_rightpanel_title:contains('Profitability')",
            content: "Check the user sees Profitability section",
        },
        {
            trigger:
                ".o_rightpanel_section[name='profitability'] .o_rightpanel_data > div > .o_rightpanel_subsection:eq(0) > table > thead > tr > th:eq(0):contains('Revenues')",
        ...
        {
            trigger:
                ".o_rightpanel_section[name='profitability'] .o_rightpanel_data > div > .o_rightpanel_subsection:eq(1) > table > thead > tr > th:eq(0):contains('Costs')",
            content: "Check the user sees Profitability subsection row",
        },
        {
            trigger:
                ".o_rightpanel_section[name='profitability'] .o_rightpanel_data > div > .o_rightpanel_subsection:eq(2) > table > thead > tr > th:eq(0):contains('Total')",
            content: "Check the user sees Profitability subsection row",
        },
        {
            trigger:
                ".o_rightpanel_section[name='milestones'] .o_rightpanel_title:contains('Milestones')",
        ...
        // "Form views in edition mode are automatically saved when the page is closed, which leads to stray network requests and inconsistencies."
        {
            ...
            content:
                "Go back to the kanban view and the project update will be added on that view",
            run: "click",
        },
        {
            trigger: ".o_controller_with_rightpanel",
            content:
                "Check the kanban view of project update is rendered to be sure the user leaves the form view and the project update is created",
        },
        ...stepUtils.toggleHomeMenu(),
        ...stepUtils.goToAppSteps(
            "project.menu_project_root",
