...
class BaseDocumentLayout(models.TransientModel):
    ...
    account_fiscal_country_id = fields.Many2one(
        related="company_id.account_fiscal_country_id", readonly=True
    )
    l10n_din5008_invoice_date = fields.Date(default=fields.Date.today, store=False)
    l10n_din5008_due_date = fields.Date(
        default=fields.Date.today() + relativedelta(day=7), store=False
    )
    l10n_din5008_delivery_date = fields.Date(
        default=fields.Date.today() + relativedelta(day=7), store=False
    )
