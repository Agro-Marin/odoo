...
class DashboardShareRoute(http.Controller):
    def _get_active_share_or_not_found(self, share_id):
        # A revoked link must read as gone, not as forbidden: a 403 tells the
        # holder the share still exists.
        share = (
    ...
    def share_portal(self, share_id=None, token=None):
        share = self._get_active_share_or_not_found(share_id)
