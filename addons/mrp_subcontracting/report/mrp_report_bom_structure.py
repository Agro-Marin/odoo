...
class ReportMrpReport_Bom_Structure(models.AbstractModel):
    ...
    def _get_quantities_info(
        ...
        quantities_info = super()._get_quantities_info(
            product, bom_uom, product_info, parent_bom, parent_product
        )
        if (
            parent_product
            and parent_bom
            and parent_bom.type == "subcontract"
            ...
            if route_info and route_info["route_type"] == "subcontract":
                subcontracting_loc = route_info[
                    "supplier"
                ].partner_id.property_stock_subcontractor
                subloc_product = product.with_context(
                    location=subcontracting_loc.id, warehouse_id=False
                )
    ...
    def _get_resupply_availability(self, route_info, components):
        ...
        if route_info.get("route_type") == "subcontract":
            ...
            subcontract_delay += (
                max(vendor_lead_time, manufacture_lead_time) + max_component_delay
            )
            route_info["manufacture_delay"] = route_info["lead_time"] + max(
                vendor_lead_time, manufacture_lead_time
            )
            route_info["lead_time"] += max(
                vendor_lead_time,
