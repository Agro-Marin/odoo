...
class TestSubcontractingBasic(TransactionCase):
    ...
    def test_duplicating_warehouses_recreates_their_routes_and_operation_types(self):
        ...
        wh_copy = wh_original.copy(
            default={"name": "Dummy Warehouse (copy)", "code": "Dummy"}
        )
...
class TestSubcontractingFlows(TestMrpSubcontractingCommon):
    ...
    def test_flow_1(self):
        ...
        self.assertTrue(
            all(
                ...
                .mapped("subcontracting_type_id.use_create_components_lots")
            )
        )
        ...
        self.assertTrue(
            all(m.product_uom_qty == m.quantity for m in picking_receipt.move_ids)
        )
    ...
    def test_flow_2(self):
        ...
        self.assertEqual(
            picking_receipt.date_planned,
            picking.date_planned + relativedelta(days=mo.bom_id.produce_delay),
        )
    ...
    def test_flow_3(self):
        ...
        (self.comp1 + self.comp2).write(
            {"route_ids": [(6, None, [resupply_sub_on_order_route.id])]}
        )
        ...
        self.assertEqual(
            picking_delivery.partner_id, picking_receipt.partner_id.parent_id
        )
    ...
    def test_flow_4(self):
        ...
        self.comp2.write(
            {"route_ids": [(6, None, [manufacture_route.id, mto_route.id])]}
        )
    ...
    def test_flow_8(self):
        ...
        mo_done = picking_receipt.move_ids._get_subcontract_production().filtered(
            lambda p: p.state == "done"
        )
        backorder_mo = backorder.move_ids.move_orig_ids.production_id.filtered(
            lambda p: p.state != "done"
        )
        ...
        self.assertTrue(
            picking_receipt.move_ids.move_orig_ids[0].production_id.state == "done"
        )
    ...
    def test_flow_9(self):
        ...
        picking_delivery = (
            self.env["stock.move"]
            .search([("product_id", "in", (self.comp1 | self.comp2).ids)])
            .picking_id
        )
        self.assertTrue(picking_delivery)
    ...
    def test_mrp_report_bom_structure_subcontracting(self):
        ...
        report_values = self.env["report.mrp.report_bom_structure"]._get_report_data(
            self.bom.id, searchQty=1, searchVariant=False
        )
        subcontracting_values = report_values["lines"]["subcontracting"]
        self.assertEqual(
            subcontracting_values["name"], self.subcontractor_partner1.display_name
        )
        self.assertEqual(
            report_values["lines"]["bom_cost"], 20
        )  # 10 For subcontracting + 5 for comp1 + 5 for subcontracting of comp2_bom
        ...
        report_values = self.env["report.mrp.report_bom_structure"]._get_report_data(
            self.bom.id, searchQty=3, searchVariant=False
        )
        subcontracting_values = report_values["lines"]["subcontracting"]
        self.assertEqual(
            report_values["lines"]["bom_cost"], 60
        )  # 30 for subcontracting + 15 for comp1 + 15 for subcontracting of comp2_bom
        ...
        report_values = self.env["report.mrp.report_bom_structure"]._get_report_data(
            self.bom.id, searchQty=5, searchVariant=False
        )
        subcontracting_values = report_values["lines"]["subcontracting"]
        self.assertEqual(
            report_values["lines"]["bom_cost"], 80
        )  # 50 for subcontracting + 25 for comp1 + 5 for subcontracting of comp2_bom
    ...
    def test_several_backorders(self):
        ...
        supply_picking = (
            self.env["mrp.production"].search([("bom_id", "=", bom.id)]).picking_ids
        )
        ...
        self.assertEqual(
            self.env["mrp.production"].search_count([("bom_id", "=", bom.id)]), 3
        )
    ...
    def test_several_backorders_2(self):
        ...
        def check_quants(product, stock_qty, sub_qty, prod_qty):
            ...
            self.assertEqual(
                ...
                ._gather(product, subcontracting_location)
                .quantity,
            )
            self.assertEqual(
                stock_qty,
                self.env["stock.quant"]._gather(product, stock_location).quantity,
            )
            self.assertEqual(
                prod_qty,
                self.env["stock.quant"]._gather(product, production_location).quantity,
            )
    ...
    def test_subcontracting_set_quantity_done(self):
        ...
        for qty in quantities[1:]:
            move.quantity = qty
            subcontracted = move._get_subcontract_production().filtered(
    ...
    def test_change_reception_serial(self):
        ...
        self.assertEqual(
            action["name"],
            "Detailed Operations",
            "It should open the detailed operations view.",
        )
        with Form(
            subcontract_move.with_context(action["context"]), view=action["view_id"]
        ) as move_form:
        ...
        self.assertEqual(
            len(subcontract_move._get_subcontract_production().lot_producing_ids), 3
        )
        self.assertRecordValues(
            subcontract_move._get_subcontract_production().lot_producing_ids.sorted(
                "id"
            ),
            [
        ...
        self.assertEqual(
            len(subcontracted_mo.filtered(lambda p: p.lot_producing_ids == new_lot)), 1
        )
        self.assertEqual(
            len(subcontracted_mo.filtered(lambda p: p.lot_producing_ids != new_lot)), 2
        )
    ...
    def test_decrease_quantity_done(self):
        ...
        productions = self.env["mrp.production"].search(
            [("product_id", "=", self.finished.id)], order="id"
        )
        self.assertRecordValues(
            productions,
            [
    ...
    def test_change_partner_subcontracting_location(self):
        ...
        def create_picking(subcontractor):
            picking_form = Form(self.env["stock.picking"])
            picking_form.picking_type_id = (
                self.warehouse.subcontracting_resupply_type_id
            )
        ...
        self.assertEqual(
            picking_with_default_location.location_dest_id,
        ...
        self.assertEqual(
            picking_with_custom_location.location_dest_id, custom_subcontract_location
        )
    ...
    def test_subcontracted_product_return_locations(self):
        ...
        self.assertRecordValues(
            return_picking.move_ids.move_line_ids,
            [
                {
                    "product_id": self.finished.id,
    ...
    def test_flow_tracked_1(self):
        ...
        with Form(
            mo.move_raw_ids[0].with_context(action["context"]), view=action["view_id"]
        ) as move_form:
            with move_form.move_line_ids.new() as move_line:
        ...
        avail_qty_finished = self.env["stock.quant"]._get_available_quantity(
            self.finished, self.warehouse.lot_stock_id
        )
        self.assertEqual(avail_qty_comp1, -1)
        self.assertEqual(avail_qty_comp2, -1)
        self.assertEqual(avail_qty_finished, 1)
    ...
    def test_flow_tracked_only_finished(self):
        ...
        move_details = Form(
            picking_receipt.move_ids, view="stock.view_stock_move_form_operations"
        )
        for lot_id in lots:
        ...
        self.assertEqual(
            set(mos.lot_producing_ids.mapped("name")),
            {f"subtracked_{i}" for i in range(nb_finished_product)},
        )
    ...
    def test_flow_backorder_production(self):
        ...
        self.env["stock.quant"]._update_available_quantity(
            self.comp1, self.warehouse.lot_stock_id, todo_nb
        )
    ...
    def test_flow_subcontracting_portal(self):
                ...
                "group_ids": [
                    (
                        ...
                        [
                            ...
                            self.env.ref("stock.group_production_lot").id,
                        ],
                    )
                ],
            }
        ...
        action = (
            picking_receipt.with_user(self.portal_user)
            .with_context({"is_subcontracting_portal": 1})
            .move_ids.action_show_details()
        )
        with Form(
            picking_receipt.move_ids.with_context(action["context"]),
            view=action["view_id"],
            ...
            with move_form.move_line_ids.new() as move_line:
                ...
                move_line.quantity = 1
            move_form.save()
        mo_1, mo_2 = picking_receipt.with_user(
            self.portal_user
        ...
        action = picking_receipt.with_user(
            self.portal_user
        ).move_ids.action_show_subcontract_details()
        mo_form = Form(
            mo_2.with_context(**action["context"]), view=action["views"][1][0]
        )
...
class TestSubcontractingSerialMassReceipt(TransactionCase):
    def setUp(self):
        ...
        self.resupply_route = self.env["stock.route"].search(
            [("name", "=", "Resupply Subcontractor on Order")]
        )
        ...
        self.finished = self.env["product.product"].create(
            {"name": "Finished", "is_storable": True, "tracking": "serial"}
        )
        self.bom = self.env["mrp.bom"].create(
            {
                ...
                "bom_line_ids": [
                    Command.create(
                        {"product_id": self.raw_material.id, "product_qty": 1}
                    ),
                ],
            }
        )
    ...
    def test_receive_after_resupply(self):
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.raw_material, self.env.ref("stock.stock_location_stock")
            ),
            0,
        )
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.raw_material, self.subcontractor.property_stock_subcontractor
            ),
            0,
        )
    ...
    def test_bom_subcontracting_product_dynamic_attribute(self):
        ...
        report_values = self.env["report.mrp.report_bom_structure"]._get_report_data(
            bom_id=bom.id, searchVariant=False
        )
    ...
    def test_subcontracting_multiple_backorders(self):
                    ...
                    Command.create(
                        {
                            ...
                            "product_uom_qty": 100,
                            "product_uom_id": subcontracted_produt.uom_id.id,
                            "location_id": self.ref("stock.stock_location_suppliers"),
                            "location_dest_id": warehouse.lot_stock_id.id,
                        }
                    )
        ...
        mo = self.env["mrp.production"].browse(action["res_id"])
        with Form(
            mo.with_context(**action["context"]), view=action["views"][0][0]
        ) as mo_form:
        ...
        self.assertRecordValues(
            backorder_backorder.move_line_ids,
    ...
    def test_use_customized_serial_sequence_in_subcontracting_productions(self):
        ...
        self.assertRecordValues(
            mo._get_subcontract_move().lot_ids.sorted("name"),
            [
                {"name": "TEST0000001"},
                {"name": "TEST0000002"},
            ],
        ...
        self.assertRecordValues(
            receipt.move_ids[0].lot_ids.sorted("name"),
            [
                {"name": "TEST0000001"},
                {"name": "TEST0000002"},
            ],
        ...
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(
                self.finished, warehouse.lot_stock_id
            ),
            2,
        )
        self.assertRecordValues(
        ...
        second_receipt, second_mo = self.generate_subcontracting_receipt_and_mo(
            5, warehouse
        )
        second_serials_wizard = Form.from_action(
            self.env, second_mo.action_generate_serial()
        )
        self.assertEqual(second_serials_wizard.lot_name, "TEST0000003")
        second_serials_wizard.serial_numbers = (
            "TEST0000005\nLOREM002\nTEST0000003\nIPSUM101\nTEST0000004"
        )
        second_serials_wizard.save().action_apply()
        self.assertRecordValues(
            second_mo._get_subcontract_move().lot_ids.sorted("name"),
            [
                {"name": "IPSUM101"},
                {"name": "LOREM002"},
                {"name": "TEST0000003"},
        ...
        self.assertRecordValues(
            second_receipt.move_ids[0].lot_ids.sorted("name"),
            [
                {"name": "IPSUM101"},
                {"name": "LOREM002"},
                {"name": "TEST0000003"},
        ...
        _third_receipt, third_mo = self.generate_subcontracting_receipt_and_mo(
            2, warehouse
        )
        third_mo.action_confirm()
        third_serials_wizard = Form.from_action(
            self.env, third_mo.action_generate_serial()
        )
    ...
    def test_use_interpolated_prefix_in_subcontracting_productions(self):
        ...
        self.assertRecordValues(
            receipt.move_ids.lot_ids.sorted("name"),
            [
                {"name": "03-02-0000001"},
                {"name": "03-02-0000002"},
            ],
