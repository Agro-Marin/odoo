...
def _request_ciusro_send_invoice(
    company, xml_data, move_type="out_invoice", is_b2b=True
):
    """
    ...
    if res_status == "1":
        error_elements = root.findall(
            ".//{mfp:anaf:dgti:efactura:stareMesajFactura:v1}Errors"
        )
        error_messages = [
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(result["content"])) as zip_ref:
            for file in zip_ref.infolist():
                ...
                # Extract the invoice or the errors if there are any
                elif error_elements := root.findall(
                    ...
                    extracted_data["invoice"] = {
                        ...
                        "date": datetime.strptime(
                            root.findtext(".//cbc:IssueDate", namespaces=NS_DOWNLOAD),
                            "%Y-%m-%d",
                        ).date(),
