...
class AccountMove(models.Model):
    ...
    def _compute_l10n_ro_edi_state(self):
        ...
        for move in self:
            ...
            move.l10n_ro_edi_state = (
                move.l10n_ro_edi_document_ids
                and move.l10n_ro_edi_document_ids.sorted()[0].state
            )
        ...
        for invoice in invoices_to_fetch:
            ...
            document_data = {
                ...
                "attachment": base64.b64encode(
                    download_data["signature"]["attachment_raw"]
                ),
            }
    ...
    def _l10n_ro_edi_process_invoice_accepted_messages(
        self, sent_invoices_accepted_messages
    ):
        ...
        """
        invoice_names = {
            message["answer"]["invoice"]["name"]
            for message in sent_invoices_accepted_messages
            if "error" not in message["answer"]
        }
        invoice_indexes = [
            message["id_solicitare"] for message in sent_invoices_accepted_messages
        ]
        domain = (
            Domain("company_id", "=", self.env.company.id)
        ...
        for message in sent_invoices_accepted_messages:
            ...
            if not invoice:
                ...
                if "error" in message["answer"] or not name_to_move.get(
                    message["answer"]["invoice"]["name"]
                ):
                    continue
            ...
            document_ids_to_delete += invoice.l10n_ro_edi_document_ids.filtered(
    ...
    @api.model
    def _l10n_ro_edi_process_invoice_refused_messages(
        self, sent_invoices_refused_messages
    ):
        ...
        """
        refused_invoice_indexes = [
            message["id_solicitare"] for message in sent_invoices_refused_messages
        ]
        domain = (
            Domain("company_id", "=", self.env.company.id)
            & Domain("move_type", "in", self.get_sale_types())
