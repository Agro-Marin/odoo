...
class AccountEdiXmlUbl_Ro(models.AbstractModel):
    ...
    def _get_address_node(self, vals):
        ...
        if state:
            address_node["cbc:CountrySubentity"]["_text"] = (
                partner.country_code + "-" + state.code
            )
            ...
            if state.code == "B" and partner.city:
                address_node["cbc:CityName"]["_text"] = get_formatted_sector_ro(
                    partner.city
                )
    ...
    def _import_retrieve_partner_vals(self, tree, role):
        ...
        if "peppol_endpoint" in partner_vals:
            ...
            error = self.env["res.partner"]._prepare_error_peppol_endpoint(
                False, partner_vals["peppol_endpoint"]
            )
            if error:
