    ...
    answer_data = {
        ...
        "3030439533": {
            ...
            "invoice": {
                ...
                "attachment_raw": file_open(
                    "l10n_ro_edi/tests/test_files/from_odoo/ciusro_in_invoice.xml"
                ).read(),
            },
        },
        "3029027563": {
    ...
    sent_invoices_accepted_messages = [
        {
            ...
            "tip": "FACTURA TRIMISA",
            "id": "3029027561",
            "answer": _patch_request_ciusro_download_answer(
                company, "3029027561", None
            ),
        },
        {
            "data_creare": "202503271639",
...
class TestUBLRO(TestUBLCommon):
    ...
    def setUpClass(cls):
        ...
        cls.company_data["company"].write(
            {
                "country_id": cls.env.ref(
                    "base.ro"
                ).id,  # needed to compute peppol_endpoint based on VAT
    ...
    def test_export_invoice(self):
        invoice = self.create_move(
            "out_invoice", currency_id=self.company.currency_id.id
        )
        attachment = self.get_attachment(invoice)
        self._assert_invoice_attachment(
            attachment,
            xpaths=None,
            expected_file_path="from_odoo/ciusro_out_invoice.xml",
        )
    ...
    def test_export_credit_note(self):
        ...
        self._assert_invoice_attachment(
            attachment,
            xpaths=None,
            expected_file_path="from_odoo/ciusro_out_refund.xml",
        )
    ...
    def test_export_no_vat_but_have_company_registry(self):
        self.company_data["company"].write(
            {"vat": False, "company_registry": "RO1234567897"}
        )
        invoice = self.create_move(
            "out_invoice", currency_id=self.company.currency_id.id
        )
    ...
    def test_export_no_vat_but_have_company_registry_without_prefix(self):
        self.company_data["company"].write(
            {"vat": False, "company_registry": "1234567897"}
        )
        self.partner_a.write({"vat": False})
        invoice = self.create_move(
            "out_invoice", currency_id=self.company.currency_id.id
        ...
        self._assert_invoice_attachment(
            attachment,
            xpaths=None,
            expected_file_path="from_odoo/ciusro_out_invoice_no_prefix_company_registry.xml",
        )
    ...
    def test_export_no_vat_and_no_company_registry_raises_error(self):
        ...
        with self.assertRaisesRegex(UserError, "doesn't have a VAT nor Company ID"):
            invoice._generate_and_send(
                allow_fallback_pdf=False, template_id=self.move_template.id
            )
    ...
    def test_ciusro_synchronize_invoices_bill_found(self):
        ...
        bill = self.create_move(
            "in_invoice",
            send=False,
            l10n_ro_edi_index="5020704741",
    ...
    def test_ciusro_synchronize_invoices_validation(self):
        ...
        invoice = self.create_move(
            "out_invoice", l10n_ro_edi_index="5019882651", send=False
        )
    ...
    def test_ciusro_synchronize_invoices_validation_error(self):
        ...
        # Create an invoice that will match the success response returned by the server
        invoice = self.create_move(
            "out_invoice", l10n_ro_edi_index="5019882654", send=False
        )
        ...
        self.assertEqual(
            str(last_message.body),
            "<p>Error when trying to download the E-Factura data from the SPV: There has been an connection error when downloading the message content</p>",
        )
    ...
    def test_ciusro_synchronize_invoices_refusal(self):
        ...
        invoice = self.create_move(
            "out_invoice", l10n_ro_edi_index="5020592384", send=False
        )
        self.env["l10n_ro_edi.document"].create(
            {
                "invoice_id": invoice.id,
