...
class PurchaseOrder(models.Model):
    ...
    def _compute_default_location_dest_id_is_subcontracting_loc(self):
        for order in self:
            order.default_location_dest_id_is_subcontracting_loc = (
                order.picking_type_id.default_location_dest_id.is_subcontract()
            )
    ...
    def _compute_dest_address_id(self):
        ...
        for order in dropship_subcontract_pos:
            subcontractor_ids = (
                order.picking_type_id.default_location_dest_id.subcontractor_ids
            )
