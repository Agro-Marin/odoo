...
class TestSubcontractingDropshippingValuation(ValuationReconciliationTestCommon):
    ...
    def test_valuation_subcontracted_and_dropshipped(self):
        ...
        stock_out_acc_id = (
            self.categ_fifo_auto.property_stock_account_output_categ_id.id
        )
        stock_valu_acc_id = self.categ_fifo_auto.property_stock_valuation_account_id.id
        stock_cop_acc_id = (
            self.categ_fifo_auto.property_stock_account_production_cost_id.id
        )
        ...
        self.assertRecordValues(
            ...
            [
                ...
                {
                    "account_id": stock_in_acc_id,
                    "product_id": self.product_a.id,
                    "debit": 0.0,
                    "credit": 200.0,
                },
                {
                    "account_id": stock_cop_acc_id,
                    "product_id": self.product_a.id,
                    "debit": 0.0,
                    "credit": 20.0,
                },
    ...
    def test_avco_valuation_subcontract_and_dropshipped_and_backorder(self):
        ...
        dropship_transfer._action_done(cancel_backorder=False)
    ...
    def test_account_line_entry_kit_bom_dropship(self):
        ...
        product_c = self.env["product.product"].create(
            {
                ...
                "property_account_income_id": self.copy_account(
                    self.company_data["default_account_revenue"]
                ).id,
                "property_account_expense_id": self.copy_account(
                    self.company_data["default_account_expense"]
                ).id,
                "taxes_id": [Command.set((self.tax_sale_a + self.tax_sale_b).ids)],
                "supplier_taxes_id": [
                    Command.set((self.tax_purchase_a + self.tax_purchase_b).ids)
                ],
                "is_storable": True,
            }
        )
        ...
        self.assertRecordValues(
            ...
            [
                ...
                {
                    "name": "15% (copy)",
                    "account_name": "Tax Received",
                    "debit": 0.0,
                    "credit": 270.0,
                },
                {
                    "name": f"{sale_order.name} - {invoice.name} installment #1",
                    "account_name": "Account Receivable (copy)",
                    "debit": 621.0,
                    "credit": 0.0,
                },
                {
                    "name": f"{sale_order.name} - {invoice.name} installment #2",
                    "account_name": "Account Receivable (copy)",
                    "debit": 1449.0,
                    "credit": 0.0,
                },
