...
class TestSubcontractingDropshippingFlows(
    ...
    def test_mrp_subcontracting_purchase_2(self):
        ...
        self.assertEqual(
            picking_delivery.partner_id, self.subcontractor_partner1.parent_id
        )
        ...
        self.assertEqual(
            picking_deliveries.partner_id, self.subcontractor_partner1.parent_id
        )
        self.assertTrue(picking_deliveries.state != "cancel")
        comp1_moves = picking_deliveries.move_ids.filtered(
    ...
    def test_po_to_customer(self):
        ...
        self.assertEqual(
            self.finished.qty_available,
            1,
            "One product has been returned to the stock location, so it should be available",
        )
        self.assertEqual(
            po.line_ids.qty_transferred,
            2,
            "One product has been returned to the stock location, so we should still consider it as received",
        )
        ...
        delivery_return02.location_dest_id = (
            dropship_picking_type.default_location_src_id
        )
    ...
    def test_subcontracted_bom_routes(self):
            ...
            [
                {
                    ...
                    "seller_ids": [
                        Command.create(
                            {"partner_id": self.subcontractor_partner1.parent_id.id}
                        )
                    ],
        ...
        route_resupply = self.env["stock.route"].search(
            [
                ("name", "=like", "%Resupply Subcontractor"),
        ...
        self.comp1.write(
            {
                "seller_ids": [
                    Command.create({"partner_id": self.subcontractor_partner1.id})
                ]
            }
        )
        ...
        report = (
            ...
            ._get_report_data(bom_subcontract.id)
        )
        ...
        self.assertEqual(
            component_lines[1]["route_name"],
            "Buy",
            "Despite the RR linked to it, it should still display the Buy route",
        )

        report = (
            self.env["report.mrp.report_bom_structure"]
            .with_context(warehouse_id=warehouse.id)
            ._get_report_data(bom_local.id)
        )
        component_lines = report.get("lines", []).get("components", [])
        self.assertEqual(component_lines[0]["product_id"], compo_drop.id)
        self.assertEqual(
            component_lines[0]["route_name"],
            "Buy",
            "Outside of the subcontracted context, it should try to resupply stock.",
        )
        self.assertEqual(component_lines[1]["product_id"], compo_rr.id)
    ...
    def test_partner_id_no_overwrite(self):
        ...
        p1, _p2 = self.env["res.partner"].create(
            [
                {
                    "name": "partner 1",
                    "property_stock_subcontractor": subcontract_location.id,
                },
                {
                    "name": "partner 2",
                    "property_stock_subcontractor": subcontract_location.id,
        ...
        delivery = (
            self.env["stock.move"]
            .search([("product_id", "=", self.comp1.id)])
            .picking_id
        )
    ...
    def test_shared_purchase_from_so(self):
        ...
        self.assertEqual(len(po_vendor.line_ids), 1)
        self.assertEqual(sum(po_vendor.line_ids.mapped("product_qty")), 4)
        po_vendor.action_confirm()
        self.assertEqual(len(po_vendor.line_ids.move_ids), 1)
        self.assertFalse(po_vendor.line_ids.move_ids.production_group_id)
        self.assertEqual(
            po_vendor.line_ids.move_dest_ids.production_group_id,
            mo_1.production_group_id | mo_2.production_group_id,
        )
    ...
    def test_portal_subcontractor_record_production_with_dropship(self):
        ...
        po.action_confirm()
        subcontracted_mo = self.env["mrp.production"].search(
            [("bom_id", "=", self.bom.id)], limit=1
        )
        # confirm the po to resuply the subcontractor
        po_dropship_subcontractor = self.env["purchase.order"].search(
        ...
        action = (
            subcontracted_mo.incoming_picking.with_user(portal_user)
            .with_context(is_subcontracting_portal=True)
        ...
        self.assertRecordValues(
            move._get_subcontract_production()[1],
            [
                {
                    "qty_producing": 0.0,
                    "lot_producing_ids": finished_serial.ids,
                    "state": "confirmed",
                }
            ],
        )
