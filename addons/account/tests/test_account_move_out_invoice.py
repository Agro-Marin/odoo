    ...
    def test_auto_post_and_reset_to_draft(self):
        ...
        def recurrence():
            return self.env["account.move"].search(
                [("auto_post_origin_id", "=", inv1.id)], order="date"
            )
        ...
        jan, feb, mar, apr, may = (
            fields.Date.to_date(f"2026-0{month}-01") for month in range(1, 6)
        )

        inv1.action_post()
        self.assertRecordValues(
            recurrence(),
            [{"date": jan, "state": "posted"}, {"date": apr, "state": "draft"}],
        )
        ...
        self.assertRecordValues(
            recurrence(),
            [{"date": jan, "state": "posted"}, {"date": feb, "state": "draft"}],
        )
        ...
        # The pending April draft was generated by inv3, so resetting inv1 leaves it.
        inv1.action_draft()
        self.assertRecordValues(
            recurrence(),
            [
