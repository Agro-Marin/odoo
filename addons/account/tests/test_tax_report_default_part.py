...
class TestTaxReportDefaultPart(TestAccountReportsCommon):
    ...
    def setUpClass(cls):
        ...
        cls.report_grouped_account_tax = cls.env.ref(
            "account.generic_tax_report_account_tax"
        )
        cls.report_grouped_tax_account = cls.env.ref(
            "account.generic_tax_report_tax_account"
        )

    def checkAmlsRedirection(
        self,
        report,
        options,
        tax_lines_with_caret_options,
        ...
        for tax_line in tax_lines_with_caret_options:
            ...
            action = self.env[report.custom_handler_model_name].caret_option_audit_tax(
                options, {"line_id": tax_line["id"]}
            )
    ...
    def test_tax_affect_base(self):
        ...
        options = self._generate_options(
            self.report_generic, date_from_str, date_to_str
        )
        ...
        options = self._generate_options(
            self.report_grouped_account_tax, date_from_str, date_to_str
        )
        self.assertLinesValues(
        ...
        options = self._generate_options(
            self.report_grouped_tax_account, date_from_str, date_to_str
        )
        ...
        tax_lines_with_caret_options = [
            ...
            if report_line.get("caret_options") == "generic_tax_report"
        ]
        expected_amls_tax_10 = invoice.line_ids.filtered(
            lambda x: x.tax_line_id == tax_10 or tax_10 in x.tax_ids
        )
        expected_amls_tax_20 = invoice.line_ids.filtered(
            lambda x: (
                x.tax_line_id == tax_20_affect_base or tax_20_affect_base in x.tax_ids
            )
        )
        expected_amls_based_on_tax_dict = {
        ...
        self.checkAmlsRedirection(
            self.report_grouped_tax_account,
            options,
            tax_lines_with_caret_options,
            expected_amls_based_on_tax_dict,
        )
    ...
    def test_tax_group_shared_tax(self):
        ...
        expected_amls_tax_group_10_20 = invoice.line_ids.filtered(
            lambda x: x.group_tax_id == tax_group_10_20 or tax_group_10_20 in x.tax_ids
        )
        expected_amls_tax_group_10_30 = invoice.line_ids.filtered(
            lambda x: x.group_tax_id == tax_group_10_30 or tax_group_10_30 in x.tax_ids
        )
        expected_amls_based_on_tax_dict = {
    ...
    def test_tax_group_of_taxes_affected_by_other(self):
                    ...
                    Command.create(
                        {
                            ...
                            "price_unit": 1000.0,
                            "tax_ids": [
                                Command.set((tax_10_affect_base + tax_group).ids)
                            ],
                        }
                    ),
                ],
        ...
        expected_amls_tax_10_affect_base = invoice.line_ids.filtered(
            lambda x: (
                x.tax_line_id == tax_10_affect_base or tax_10_affect_base in x.tax_ids
            )
        )
        expected_amls_tax_group = invoice.line_ids.filtered(
            lambda x: x.tax_line_id or x.tax_ids
        )
        expected_amls_based_on_tax_dict = {
    ...
    def test_mixed_all_type_tax_use_same_line(self):
        ...
        expected_amls_tax_10 = move.line_ids.filtered(
            lambda x: x.tax_line_id == tax_10 or x.tax_ids
        )
        expected_amls_tax_20 = move.line_ids.filtered(
            lambda x: x.tax_line_id == tax_20 or x.tax_ids
        )
        expected_amls_based_on_tax_dict = {
            "tax_10 (10.0%)": expected_amls_tax_10,
    ...
    def test_mixed_all_type_tax_on_different_line(self):
        ...
        expected_amls_tax_20 = move.line_ids.filtered(
            lambda x: x.tax_line_id == tax_20 or tax_20 in x.tax_ids
        )
        expected_amls_based_on_tax_dict = {
            "tax_10 (10.0%)": expected_amls_tax_10,
            "tax_20 (20.0%)": expected_amls_tax_20,
    ...
    def test_tax_report_custom_edition_tax_line(self):
        """Check the tax report handles manually edited tax lines on a journal entry.

        Editing a tax line breaks the mapping between the original tax details and the line, so extra
        tax details are generated on it to reflect the edition.
        """
        tax_10 = self.env["account.tax"].create(
            {
                "name": "tax_10",
        ...
        tax_10_line = invoice.line_ids.filtered(
            lambda x: x.tax_repartition_line_id.tax_id == tax_10
        )
        tax_20_line = invoice.line_ids.filtered(
            lambda x: x.tax_repartition_line_id.tax_id == tax_20
        )
        ...
        invoice.write(
            {
                "line_ids": [
                    ...
                    Command.update(
                        tax_20_line.id,
                        {"account_id": self.revenue_2.id, "credit": 201.0},
                    ),
    ...
    def test_tax_report_comparisons(self):
        ...
        options = self._update_comparison_filter(
            options, self.report_generic, "previous_period", 2
        )
        self.assertLinesValues(
            self.report_generic._get_lines(options),
        ...
        options = self._update_comparison_filter(
            options, self.report_grouped_account_tax, "previous_period", 2
        )
        self.assertLinesValues(
            self.report_grouped_account_tax._get_lines(options),
            #   Name                                    NET         TAX         NET         TAX         NET         TAX
        ...
        options = self._update_comparison_filter(
            options, self.report_grouped_tax_account, "previous_period", 2
        )
        self.assertLinesValues(
    ...
    def test_tax_invoice_completely_refund(self):
        ...
        expected_amls = invoice.line_ids.filtered(
            lambda x: x.tax_line_id or x.tax_ids
        ) + invoice.reversal_move_ids.line_ids.filtered(
            lambda x: x.tax_line_id or x.tax_ids
        )
        expected_amls_based_on_tax_dict = {
            "tax (10.0%)": expected_amls,
    ...
    def test_tax_report_entry_move_2_opposite_invoice_lines(self):
        ...
        # 'invoice_date' is invisible on 'entry' moves, so the accounting date is the one to set here
        move_form.date = "2022-02-01"

        for name, account_id, debit, credit, tax_to_apply in (
            (
                "invoice line in entry",
