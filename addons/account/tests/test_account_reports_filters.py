...
class TestAccountReportsFilters(TestAccountReportsCommon, odoo.tests.HttpCase):
    def _assert_filter_date(self, report, previous_options, expected_date_values):
        """Compute the 'date' option for the given report and previous_options, and assert it matches expected_date_values."""
    ...
    def _assert_filter_comparison(
        ...
        """Compute the 'comparison' option for the given report and previous_options, and assert its periods match expected_period_values."""
        options = report.get_options(previous_options)

        self.assertEqual(
            len(options["comparison"]["periods"]), len(expected_period_values)
        )
    ...
    def test_filter_date_fiscalyear_range_custom_years(self):
        ...
        for i in range(9):
            quarter_df, quarter_dt = date_utils.get_quarter(
                today - relativedelta(months=i * 3)
            )
    ...
    def test_filter_date_custom_range_recognition(self):
        """Test the period_type inferred from a custom dates range: a range covering a whole month,
        quarter, fiscal year, or a custom account.fiscal.year must be recognized as such.
        """
        self._assert_filter_date(
            self.date_range_report,
            {
    ...
    def test_filter_date_today_single(self):
        ...
        self._assert_filter_comparison(
            ...
            {
                "date": {"filter": "today", "mode": "single"},
                "comparison": {"filter": "custom", "date_to": "2016-12-31"},
            },
            [
                {
                    "string": "As of %s" % format_date(self.env, "2016-12-31"),
    ...
    def test_filter_date_custom_single_period_type_month(self):
        """Test the filter_date with a custom date in 'single' mode, when the previous options carry a
        stale 'today' period_type that must be recomputed from the date range."""
        self._assert_filter_date(
            self.single_date_report,
            {
                "date": {
        ...
        self._assert_filter_comparison(
            self.single_date_report,
            {
                "date": {"filter": "custom", "mode": "single", "date_to": "2019-07-18"},
                "comparison": {"filter": "previous_period", "number_period": 2},
            },
            [
                {
    ...
    def test_filter_aml_ir_filters(self):
        ...
        moves = self.init_invoice(
            "out_invoice", self.partner_a, "2023-09-01", amounts=[1000]
        ) + self.init_invoice(
            "out_invoice", self.partner_a, "2023-09-01", amounts=[1000]
        )
        moves.action_post()
    ...
    def test_hide_line_at_0_tour(self):
        ...
        self.start_tour(
            "/odoo", "account_reports_hide_0_lines", login=self.env.user.login
        )
    ...
    def test_hide_line_at_0_tour_with_string_columns(self):
        ...
        self.start_tour(
            "/odoo",
            "account_reports_hide_0_lines_with_string_columns",
    ...
    def test_rounding_unit_tour(self):
        ...
        self.start_tour(
            "/odoo", "account_reports_rounding_unit", login=self.env.user.login
        )

    def test_filter_multi_company(self):
        def _check_company_filter(
            allowed_companies, expected_companies, message=None, match_active=True
        ):
            ...
            if match_active:
                ...
                self.assertEqual(
                    computed_company_ids[0], expected_companies[0].id, message
                )
            # Selected companies should match, whatever their order
            self.assertEqual(
                set(computed_company_ids), set(expected_companies.ids), message
            )

        main_company = self.company_data["company"]
        main_company.vat = "123"
        branch_1 = self.env["res.company"].create(
            {"name": "Branch 1", "parent_id": main_company.id, "vat": "123"}
        )
        branch_1_1 = self.env["res.company"].create(
            {"name": "Branch 1 sub-branch 1", "parent_id": branch_1.id}
        )
        branch_1_2 = self.env["res.company"].create(
            {"name": "Branch 1 sub-branch 2", "parent_id": branch_1.id, "vat": "123"}
        )
        branch_2 = self.env["res.company"].create(
            {"name": "Branch 2", "parent_id": main_company.id}
        )
        branch_2_1 = self.env["res.company"].create(
            {"name": "Branch 2 sub-branch 1", "parent_id": branch_2.id}
        )
    ...
    def test_available_variants_options(self):
        ...
        expected_reports = (
            root_report + report_always + report_generic_coa + report_us_country
        )
        ...
        options = self._generate_options(root_report, "2024-01-01", "2024-12-31")
        assert_available_variants_match(
            options, root_report + report_always + report_us_coa + report_us_country
        )
    ...
    def test_hide_line_at_0_tour_load_more_line(self):
        ...
        self.start_tour(
            "/odoo", "account_reports_hide_0_lines_load_more", login=self.env.user.login
        )
