...
class TestTaxesTaxTotalsSummary(TestTaxCommon):
    ...
    def setUpClass(cls):
        ...
        cls.tax_groups = cls.env["account.tax.group"].create(
            [{"name": str(i), "sequence": str(i)} for i in range(1, 10)]
        )
    ...
    def _test_random_case_tax_included(self):
        ...
        tax_1 = self.percent_tax(
            10.0,
            price_include_override="tax_included",
            tax_group_id=self.tax_groups[0].id,
        )
        tax_2 = self.percent_tax(
            10.0,
            price_include_override="tax_included",
            tax_group_id=self.tax_groups[1].id,
        )
        taxes = tax_1 + tax_2
        document_params = self.init_document(
            lines=[
    ...
    def test_random_case_tax_included_generic_helpers(self):
        for (
            ...
            document,
            expected_values,
        ) in self._test_random_case_tax_included():
            with self.subTest(test_index=test_index):
                self.assert_tax_totals_summary(document, expected_values)
        self._run_js_tests()
    ...
    def _test_cash_rounding(self):
        ...
        with (
            self.same_tax_group(taxes),
            self.with_tax_calculation_rounding_method("round_per_line"),
        ):
            cash_rounding = self.env["account.cash.rounding"].create(
                {
                    ...
                    "profit_account_id": self.company_data[
            ...
            yield 1, self.populate_document(document_params), expected_values
        with (
            self.different_tax_group(taxes),
            self.with_tax_calculation_rounding_method("round_per_line"),
        ):
            cash_rounding = self.env["account.cash.rounding"].create(
                {
    ...
    def test_cash_rounding_with_excluded_tax_groups(self):
        ...
        self.assert_tax_totals_summary(
            document, expected_values, excluded_tax_group_ids=self.tax_groups[2:5].ids
        )
    ...
    def _test_mixed_combined_standalone_taxes(self):
        ...
        document1 = self.populate_document(
            self.init_document(
                lines=[
        ...
        document2 = self.populate_document(
            self.init_document(
                lines=[
    ...
    def test_mixed_combined_standalone_taxes_generic_helpers(self):
        for (
            ...
            document,
            expected_values,
        ) in self._test_mixed_combined_standalone_taxes():
            with self.subTest(test_index=test_index):
                self.assert_tax_totals_summary(document, expected_values)
        self._run_js_tests()
    ...
    def test_preceding_subtotal_with_tax_group_generic_helpers(self):
        for (
            ...
            document,
            expected_values,
        ) in self._test_preceding_subtotal_with_tax_group():
            with self.subTest(test_index=test_index):
                self.assert_tax_totals_summary(document, expected_values)
        self._run_js_tests()
    ...
    def _test_preceding_subtotal_with_include_base_amount(self):
        ...
        self.tax_groups[2].preceding_subtotal = "PRE GROUP 2"
        tax_1 = self.percent_tax(
            10.0, include_base_amount=True, tax_group_id=self.tax_groups[1].id
        )
        tax_2 = self.percent_tax(
            20.0, include_base_amount=True, tax_group_id=self.tax_groups[1].id
        )
        tax_3 = self.percent_tax(
            30.0, include_base_amount=True, tax_group_id=self.tax_groups[1].id
        )
    ...
    def test_preceding_subtotal_with_include_base_amount_generic_helpers(self):
        document, expected_values = (
            self._test_preceding_subtotal_with_include_base_amount()
        )
        self.assert_tax_totals_summary(document, expected_values)
        self._run_js_tests()
    ...
    def test_reverse_charge_percent_tax_generic_helpers(self):
        for (
            ...
            document,
            expected_values,
        ) in self._test_reverse_charge_percent_tax():
            with self.subTest(test_index=test_index):
                self.assert_tax_totals_summary(document, expected_values)
        self._run_js_tests()
    ...
    def test_reverse_charge_division_tax_generic_helpers(self):
        for (
            ...
            document,
            expected_values,
        ) in self._test_reverse_charge_division_tax():
            with self.subTest(test_index=test_index):
                self.assert_tax_totals_summary(document, expected_values)
        self._run_js_tests()
    ...
    def test_discount_with_round_globally_generic_helpers(self):
        for (
            ...
            document,
            expected_values,
        ) in self._test_discount_with_round_globally():
            with self.subTest(test_index=test_index):
                self.assert_tax_totals_summary(document, expected_values)
        self._run_js_tests()
