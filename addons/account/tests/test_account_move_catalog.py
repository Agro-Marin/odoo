...
class TestAccountMoveCatalog(AccountTestInvoicingCommon):
    def test_posted_move_is_readonly_for_catalog(self):
        ...
        The controller (`product/controllers/catalog.py`) gates every
        catalog write on `_is_readonly()` alone, so a stale override here
        is a direct RPC path to mutate an already-posted, hash-protected
        entry."""
