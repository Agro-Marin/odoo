...
class TestAccountMoveDateAlgorithm(AccountTestInvoicingCommon):
    ...
    def test_out_refund_reverse_date_with_lock_date(self):
        ...
        self.assertRecordValues(
            reverse_move, [{"date": fields.Date.from_string("2017-01-12")}]
        )
    ...
    def test_reconcile_with_lock_date(self):
        invoice = self._create_invoice(
            "out_invoice", "2016-01-01", currency_id=self.other_currency.id
        )
        refund = self._create_invoice(
            "out_refund", "2017-01-01", currency_id=self.other_currency.id
        )
        (invoice + refund).action_post()
        self._set_lock_date("2017-01-31")

        amls = (invoice + refund).line_ids.filtered(
    ...
    def test_unreconcile_with_lock_date(self):
        ...
        reverse_exchange_move = (
            exchange_move.line_ids.matched_credit_ids.credit_move_id.move_id
        )
        self.assertRecordValues(
            reverse_exchange_move,
            [
