...
class TestResPartnerBankTrust(TransactionCase):
    ...
    def test_money_transfer_belgian_account_detected(self):
        acc = self.RPB.create(
            {"acc_number": "BE40967000000063", "partner_ids": [(4, self.partner_be.id)]}
        )
        ...
        acc = self.RPB.create(
            {
                ...
                "partner_ids": [(4, self.partner_fr.id)],
            }
        )
    ...
    def test_lock_trust_fields(self):
        new_rec = self.RPB.new({"partner_ids": [(4, self.partner_be.id)]})
        self.assertFalse(new_rec.lock_trust_fields, "new record is never locked")

        acc = self.RPB.create(
            {"acc_number": "BE71096123456769", "partner_ids": [(4, self.partner_be.id)]}
        )
    ...
    def test_create_rejects_archived_duplicate(self):
        acc = self.RPB.create(
            {"acc_number": "BE68539007547034", "partner_ids": [(4, self.partner_be.id)]}
        )
        acc.action_archive()
        with self.assertRaises(UserError):
            self.RPB.create(
            ...
            self.RPB.create(
                {
                    ...
                    "partner_ids": [(4, self.partner_be.id)],
                }
            )

    def test_create_multi_detects_archived_duplicate(self):
        acc = self.RPB.create(
            {"acc_number": "BE62510007547061", "partner_ids": [(4, self.partner_be.id)]}
        )
        acc.action_archive()
        with self.assertRaises(UserError):
            self.RPB.create(
