...
class TestAccountMoveInInvoiceOnchanges(AccountTestInvoicingCommon):
    ...
    def setup_armageddon_tax(cls, tax_name, company_data, **kwargs):
        return super().setup_armageddon_tax(
            tax_name, company_data, type_tax_use="purchase", **kwargs
        )
    ...
    def test_in_invoice_line_onchange_cash_rounding_1(self):
        ...
        repartition_line = self.tax_purchase_a.invoice_repartition_line_ids.filtered(
        ...
        self.assertInvoiceValues(
            ...
            [
                ...
                {
                    **self.tax_line_vals_2,
                    "tax_repartition_line_id": self.tax_purchase_b.invoice_repartition_line_ids.filtered(
                        lambda x: x.repartition_type == "tax"
                    ).id,
                    "tax_tag_ids": [],
                },
    ...
    def test_in_invoice_payment_register_wizard(self):
        ...
        )  # should not raise an error on non-posted move
        self.assertTrue(action_register_payment)
        wizard = (
            self.env[action_register_payment["res_model"]]
            .with_context(action_register_payment["context"])
            .create({})
        )
        ...
        with self.assertRaisesRegex(
            UserError,
            r"There's nothing left to pay for the selected journal items, so no payment registration is necessary. You've got your finances under control like a boss!",
        ):
            action_register_payment = move.action_force_register_payment()
            self.env[action_register_payment["res_model"]].with_context(
                action_register_payment["context"]
            ).create({})
    ...
    def _assert_payment_move_state(
        self, move_type, amount, counterpart_values_list, payment_state, post_move=True
    ):
        def assert_partial(line1, line2):
            partial = self.env["account.partial.reconcile"].search(
                Domain.OR(
                    [
                        [
                            ("debit_move_id", "=", line1.id),
                            ("credit_move_id", "=", line2.id),
                        ],
                        [
                            ("debit_move_id", "=", line2.id),
                            ("credit_move_id", "=", line1.id),
                        ],
                    ]
                ),
                limit=1,
        ...
        def create_move(move_type, amount, account=None):
            ...
            if move_type in self.env["account.move"].get_sale_types(
                ...
                if amount > 0.0:
                    debit_account = (
                        account or self.company_data["default_account_receivable"]
                    )
                ...
                reverse_move.write(
                    {
                        "invoice_line_ids": [
                            Command.update(
                                reverse_move.invoice_line_ids.id, {"price_unit": amount}
                            ),
                        ],
                    }
                )
        ...
        for counterpart_move_type, counterpart_amount in counterpart_values_list:
            if counterpart_move_type == "payment":
                ...
                reconciliation_info = self.pay_with_statement_line(
                    move,
                    self.company_data["default_journal_bank"].id,
                    "2020-01-10",
                    counterpart_amount,
                )
                assert_partial(
                    reconciliation_info["statement_line_reconciled"],
                    reconciliation_info["move_reconciled"],
                )
            else:
                counterpart_move = create_move(
                    counterpart_move_type, counterpart_amount, account=line.account_id
                )
        ...
        if (
            ...
            and move._get_invoice_in_payment_state() == "paid"
        ):
            payment_state = "paid"
    ...
    def test_payment_move_state(self):
        for move_type, amount, counterpart_values_list, payment_state in (
            ...
            (
                "out_invoice",
                1000.0,
                [("out_refund", 500.0), ("out_refund", 500.0)],
                "reversed",
            ),
            (
                "out_invoice",
                1000.0,
                [("out_refund", 500.0), ("entry", -500.0)],
                "reversed",
            ),
            ...
            (
                "in_invoice",
                1000.0,
                [("in_refund", 500.0), ("in_refund", 500.0)],
                "reversed",
            ),
            (
                "in_invoice",
                1000.0,
                [("in_refund", 500.0), ("entry", 500.0)],
                "reversed",
            ),
            ...
            (
                "out_invoice",
                1000.0,
                [("out_refund", 500.0), ("payment", 500.0)],
                "in_payment",
            ),
            (
                "out_invoice",
                1000.0,
                [("out_refund", 500.0), ("payment", 400.0)],
                "partial",
            ),
            (
                "out_invoice",
                1000.0,
                [("out_refund", 500.0), ("statement_line", 500.0)],
                "paid",
            ),
            (
                "out_invoice",
                1000.0,
                [("out_refund", 500.0), ("statement_line", 400.0)],
                "partial",
            ),
            ...
            (
                "in_invoice",
                1000.0,
                [("in_refund", 500.0), ("payment", 500.0)],
                "in_payment",
            ),
            (
                "in_invoice",
                1000.0,
                [("in_refund", 500.0), ("payment", 400.0)],
                "partial",
            ),
            (
                "in_invoice",
                1000.0,
                [("in_refund", 500.0), ("statement_line", -500.0)],
                "paid",
            ),
            (
                "in_invoice",
                1000.0,
                [("in_refund", 500.0), ("statement_line", -400.0)],
                "partial",
            ),
            ...
            with self.subTest(
                ...
                self._assert_payment_move_state(
                    move_type, amount, counterpart_values_list, payment_state
                )
    ...
    def test_manual_label_change_on_payment_term_line(self):
        ...
        self.invoice.action_post()
        self.assertEqual(
            payment_term_line.name,
            "XYZ",
            "Manual name of payment term line should be kept",
        )
    ...
    def test_duplicate_invoice_with_separate_discount_acccount(self):
        ...
        invoice.action_post()
        self.assertFalse(
            invoice.line_ids.filtered(lambda l: l.display_type == "discount").tax_ids
        )
    ...
    def test_default_tax_and_default_fiscal_position(self):
        """Tests that for invoices the default fp based on partner is used.
        In the case of receipts, the default receipt fp in the COA should take priority over the fp based on partner.
        And the tax will be applied according to the fp in both cases.
        """
        ...
        product_tax = self.env["account.tax"].create(
            {
                "name": "Default product tax",
                "type_tax_use": "purchase",
                "amount_type": "percent",
                "amount": 10,
                "company_id": self.env.company.id,
        ...
        # Default fiscal position (when fp set on the company)
        self.assertEqual(invoice.fiscal_position_id.id, False)
        self.assertEqual(receipt.fiscal_position_id, receipt_fiscal_position)
        ...
        # Default fiscal position (when fp not set on the company)
