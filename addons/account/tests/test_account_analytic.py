...
class TestAccountAnalyticAccount(AccountTestInvoicingCommon, AnalyticCommon):
    ...
    def test_mandatory_plan_validation(self):
        ...
        invoice.invoice_line_ids.analytic_distribution = {
            self.analytic_account_4.id: 100.01
        }
        with self.assertRaisesRegex(ValidationError, "100% analytic distribution."):
            invoice.with_context({"validate_analytic": True}).action_post()

        invoice.invoice_line_ids.analytic_distribution = {
            self.analytic_account_4.id: 99.9
        }
        with self.assertRaisesRegex(ValidationError, "100% analytic distribution."):
            invoice.with_context({"validate_analytic": True}).action_post()

        invoice.invoice_line_ids.analytic_distribution = {
            self.analytic_account_4.id: 100
        }
        invoice.with_context({"validate_analytic": True}).action_post()
    ...
    def test_applicability_score(self):
        ...
        applicability = self.analytic_plan_2._get_applicability(
            business_domain="invoice",
            company_id=self.env.company.id,
        ...
        score = applicability_with_company._get_score(
            business_domain="invoice", product=self.product_a.id
        )
    ...
    def test_model_sequence(self):
            ...
            [
                {
                    "account_prefix": "123",
                    "sequence": 10,
                    "analytic_distribution": {f"{aa_A1.id},{aa_B1.id}": 100},
                },
                {
                    "account_prefix": "123",
                    "sequence": 20,
                    "analytic_distribution": {f"{aa_A2.id},{aa_C2.id}": 100},
                },
                {
                    "account_prefix": "123",
                    "sequence": 30,
                    "analytic_distribution": {f"{aa_B3.id}": 100},
                },
            ]
        )
        criteria = {
        ...
        self.assertEqual(
            distribution,
            m2.analytic_distribution | m3.analytic_distribution,
    ...
    def test_analytic_applicability_multiple_prefixes(self):
        ...
        account_analytic = self.env["account.analytic.account"].search(
            [("plan_id", "=", self.analytic_plan_2.id)], limit=1
        )
        ...
        invoice.line_ids.filtered(lambda line: line.account_id.code.startswith("40"))[
            0
        ...
        invoice.line_ids.filtered(lambda line: line.account_id.code.startswith("41"))[
            0
        ].write({"analytic_distribution": {account_analytic.id: 100}})
    ...
    def test_analytic_lines_partner_compute(self):
        ...
        def get_analytic_lines():
            return (
                self.env["account.analytic.line"]
                .search([("move_line_id", "in", entry.line_ids.ids)])
                .sorted("amount")
            )
    ...
    def test_zero_balance_invoice_with_analytic_line(self):
        ...
        self.assertEqual(
            invoice.invoice_line_ids.analytic_distribution,
            {f"{self.analytic_account_1.id}": 100.0},
        )
