...
class TestUi(AccountTestInvoicingHttpCommon):
    ...
    def test_use_product_catalog_on_invoice(self):
        ...
        self.start_tour(
            ...
            "test_use_product_catalog_on_invoice",
            login="admin",
        )

    def test_deductible_amount_column(self):
        self.assertFalse(
            self.env.user.has_group("account.group_partial_purchase_deductibility")
        )
        partner = self.env["res.partner"].create(
            {"name": "Test Partner", "email": "test@test.odoo.com"}
        )
        move = self.env["account.move"].create(
            {
                ...
                "line_ids": [
                    Command.create({"name": "T-shirt", "deductible_amount": 50.0})
                ],
            }
        )
        move.action_post()
        ...
        self.start_tour(
            "/odoo/vendor-bills/new",
            "deductible_amount_column",
