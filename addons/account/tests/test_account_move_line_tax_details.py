...
class TestAccountTaxDetailsReport(AccountTestInvoicingCommon):
    ...
    def _dispatch_move_lines(self, moves):
        base_lines = moves.line_ids.filtered(
            lambda x: x.tax_ids and not x.tax_line_id
        ).sorted(lambda x: (x.move_id.id, x.id, -abs(x.amount_currency)))
        tax_lines = moves.line_ids.filtered(lambda x: x.tax_line_id).sorted(
            lambda x: (
                x.move_id.id,
                x.tax_line_id.id,
                x.tax_ids.ids,
    ...
    def _get_tax_details(self, fallback=False, extra_domain=None):
        ...
        tax_details_query = self.env[
            "account.move.line"
        ]._get_query_tax_details_from_domain(domain, fallback=fallback)
    ...
    def assertTaxDetailsValues(self, tax_details, expected_values_list):
        ...
        for i, expected_values in enumerate(expected_values_list):
            ...
            self.assertDictEqual(
                {k: v for k, v in tax_detail.items() if k in keys}, expected_values
            )
    ...
    def test_affect_base_amount_1(self):
        ...
        invoice_create_values["invoice_line_ids"][0][2]["tax_ids"] = [
            Command.set(tax_group.ids)
        ]
        invoice = self.env["account.move"].create(invoice_create_values)
                    ...
                    Command.create(
                        {
                            ...
                            "tax_ids": [
                                Command.set(
                                    (taxes_10_affect[0] + taxes_10_affect[2]).ids
                                )
                            ],
                        }
                    ),
    ...
    def test_affect_base_amount_6(self):
        ...
        invoice.write(
            {"invoice_line_ids": [Command.delete(invoice.invoice_line_ids.id)]}
        )
        base_lines, tax_lines = self._dispatch_move_lines(invoice)
    ...
    def test_multiple_same_tax_lines(self):
        ...
        tax_rep = percent_tax.refund_repartition_line_ids.filtered(
        ...
        percent_tax = self.env["account.tax"].create(
            {
                ...
                "cash_basis_transition_account_id": self.company_data[
