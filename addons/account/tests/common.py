...
class AccountTestInvoicingCommon(ProductCommon):
    ...
    def setUpClass(cls):
        ...
        cls.cash_rounding_a = cls.env["account.cash.rounding"].create(
            {
                ...
                "profit_account_id": cls.company_data["default_account_revenue"]
                .copy()
                .id,
                "loss_account_id": cls.company_data["default_account_expense"]
                .copy()
                .id,
                "rounding_method": "UP",
            }
        ...
        in_outstanding_account = cls.env["account.chart.template"].ref(
            "account_journal_payment_debit_account_id", raise_if_not_found=False
        )
        out_outstanding_account = cls.env["account.chart.template"].ref(
    ...
    def _create_product(cls, **create_values):
        ...
        create_values.setdefault(
            "property_account_income_id", cls.company_data["default_account_revenue"]
        )
        create_values.setdefault(
            "property_account_expense_id", cls.company_data["default_account_expense"]
        )
    ...
    def setup_other_currency(cls, code, **kwargs):
        if "rates" not in kwargs:
            return super().setup_other_currency(
                code,
                rates=[
                    ("1900-01-01", 1.0),
                    ("2016-01-01", 3.0),
    ...
    def _use_chart_template(cls, company, chart_template_ref=None):
        chart_template_ref = chart_template_ref or cls.env[
            "account.chart.template"
        ]._guess_chart_template(company.country_id)
        template_vals = cls.env["account.chart.template"]._get_chart_template_mapping()[
            chart_template_ref
        ]
    ...
    def collect_company_accounting_data(cls, company):
        ...
        account_company_domain = cls.env["account.account"]._check_company_domain(
            company
        )
        journal_company_domain = cls.env["account.journal"]._check_company_domain(
            company
        )
        return {
            "company": company,
            ...
            "default_account_receivable": cls.env["res.partner"]
            ...
            "default_account_deferred_expense": AccountAccount.search(
                [*account_company_domain, ("account_type", "=", "asset_current")],
                limit=1,
            ),
            ...
            "default_journal_misc": cls.env["account.journal"].search(
                [*journal_company_domain, ("type", "=", "general")], limit=1
            ),
            "default_journal_sale": cls.env["account.journal"].search(
                [*journal_company_domain, ("type", "=", "sale")], limit=1
            ),
            "default_journal_purchase": cls.env["account.journal"].search(
                [*journal_company_domain, ("type", "=", "purchase")], limit=1
            ),
            "default_journal_bank": cls.env["account.journal"].search(
                [*journal_company_domain, ("type", "=", "bank")], limit=1
            ),
    ...
    def copy_account(cls, account, default=None):
        ...
        while True:
            ...
            if account.search_count([("code", "=", new_code)]):
                ...
                return account.copy(
                    default={"code": new_code, "name": account.name, **(default or {})}
                )
    ...
    def setup_armageddon_tax(cls, tax_name, company_data, **kwargs):
        ...
        cash_basis_transition_account = (
            company_data["default_account_tax_sale"]
    ...
    def create_line_for_reconciliation(
        self,
        balance,
        amount_currency,
        currency,
        ...
        move.action_post()
        line = move.line_ids.filtered(
            lambda x: x.account_id == write_off_account_to_be_reconciled
        )
    ...
    def _prepare_invoice_line(
        cls, price_unit=None, product_id=None, quantity=1.0, tax_ids=None, **line_args
    ):
        assert price_unit is not None or product_id is not None, (
            "Either `price_unit` or `product_id` must be filled!"
        )
        invoice_line_args = {
    ...
    def _create_account_move_send_wizard_multi(cls, moves, **kwargs):
        return (
            cls.env["account.move.send.batch.wizard"]
    ...
    @classmethod
    def _create_sale_order_one_line(
        cls,
        price_unit=None,
        product_id=None,
        tax_ids=None,
    ...
    def _create_down_payment_invoice(
        ...
        down_payment_wizard = (
            ...
            .with_context(
                {"active_model": sale_order._name, "active_ids": sale_order.ids}
            )
    ...
    def _apply_sale_order_discount(cls, sale_order, amount_type: str, amount: float):
        ...
        if amount_type in ("percent", "all"):
            discount_type = (
                "so_discount" if amount_type == "percent" else "sol_discount"
            )
    ...
    def assertInvoiceValues(self, move, expected_lines_values, expected_move_values):
        ...
        self.assertRecordValues(
            sort_lines(move.line_ids.sorted()),
            expected_lines_values,
            field_names=expected_lines_values[0].keys(),
        )
        self.assertRecordValues(
            move, [expected_move_values], field_names=expected_move_values.keys()
        )
    ...
    def _assert_tax_totals_summary(
        self, tax_totals, expected_results, soft_checking=False
    ):
        ...
        def fix_monetary_value(current_values, expected_values, monetary_fields):
            for key, current_value in current_values.items():
                ...
                if current_value is not None and currency.is_zero(
                    current_value - expected_value
                ):
                    current_values[key] = expected_value

        currency = self.env["res.currency"].browse(tax_totals["currency_id"])
        company_currency = self.env["res.currency"].browse(
            tax_totals["company_currency_id"]
        )
        ...
        current_values = {
            k: len(v) if k == "subtotals" else v
            for k, v in tax_totals.items()
            if k not in excluded_fields
        }
        expected_values = {
            k: len(v) if k == "subtotals" else v for k, v in expected_results.items()
        }
        if soft_checking:
            current_values = {
                k: v for k, v in current_values.items() if k in expected_values
            }
        ...
        for subtotal, expected_subtotal in zip(
            ...
            expected_values = {
                k: len(v) if k == "tax_groups" else v
                for k, v in expected_subtotal.items()
            }
            fix_monetary_value(current_values, expected_values, monetary_fields)
            self.assertEqual(current_values, expected_values)
            for tax_group, expected_tax_group in zip(
                ...
                fix_monetary_value(
                    current_tax_group, expected_tax_group, monetary_fields
                )
    ...
    def _get_ignore_schema(
        ...
        ignore_schema_paths = []
        while subfolders:
            ignore_schema_paths.append(
                f"{cls.test_module}/tests/test_files/{'/'.join(subfolders)}/{ignore_schema_name}"
            )
            subfolders.pop()
        ignore_schema_paths.append(
            f"{cls.test_module}/tests/test_files/{ignore_schema_name}"
        )
    ...
    def _merge_two_xml(
        ...
        if secondary_xml.text and (
            (not primary_xml.text and overwrite_on_conflict)
            or (primary_xml.text and overwrite_on_conflict)
        ):
    ...
    @classmethod
    def _rebuild_xml_with_sorted_namespaces(
        cls, root: etree._Element
    ) -> etree._Element:
        # Collect all namespaces and prefixes
        all_nsmap = {
            prefix: uri for elem in root.iter() for prefix, uri in elem.nsmap.items()
        }
        ...
        sorted_nsmap = {
            nsmap_key: all_nsmap[nsmap_key] for nsmap_key in sorted_nsmap_keys
        }
    ...
    def _apply_json_ignore_schema(cls, data, ignore_schema):
        assert data.__class__ is ignore_schema.__class__, (
            "Type of data and ignore_schema must match"
        )
        ...
        if isinstance(ignore_schema, dict):
            for schema_key, schema_value in ignore_schema.items():
                if schema_key in data:
                    if schema_value == "___ignore___":
                        ...
                        schema_key
                    ]:  # schema_value is a dict or a list, and the corresponding value is not None
            ...
            if len(ignore_schema) == 1:
                ...
                assert len(ignore_schema) == len(data), (
                    "Length of list of ignore_schema and data must match"
                )
            ...
            with patch.object(re, "fullmatch", lambda _arg1, _arg2: True):
                save_test_file(
                    ...
                    content=etree.tostring(
                        xml_element, pretty_print=True, encoding="UTF-8"
                    ),
            ...
            if children != expected_children:
                for child in node_dict["children"]:
                    if child["tag"] not in expected_children:
                        _logger.warning(
                            "Non-expected child: \n%s",
                            etree.tostring(child["node"]).decode(),
                        )
...
class AccountTestInvoicingHttpCommon(
    AccountTestInvoicingCommon, AccountTestMockOnlineSyncCommon
):
    pass
...
class TestTaxCommon(AccountTestInvoicingHttpCommon):
    ...
    def _ensure_rate(self, currency, date, rate):
        currency_rate = currency.rate_ids.filtered(
            lambda x: x.name == fields.Date.from_string(date)
        )
        if currency_rate:
    ...
    def _jsonify_tax(self, tax):
        return {
            ...
            "children_tax_ids": [
                self._jsonify_tax(child) for child in tax.children_tax_ids
            ],
    ...
    def _jsonify_document_line(self, document, index, line):
        return {
            "record": None,
            "id": index,
            "currency_id": self._jsonify_currency(
                line.get("currency_id") or document["currency"]
            ),
            ...
            "product_uom_id": self._jsonify_product_uom(
                line["product_uom_id"], line["tax_ids"]
            ),
    ...
    def _jsonify_document(self, document):
        return {
            **document,
            ...
            "lines": [
                self._jsonify_document_line(document, index, line)
        ...
        return self.env["account.move"].create(
            {
                ...
                "currency_id": currency.id,
                "invoice_cash_rounding_id": document["cash_rounding"]
                and document["cash_rounding"].id,
                "invoice_line_ids": [
                    Command.create(
                        self.convert_base_line_to_invoice_line(document, base_line)
                    )
    ...
    def _assert_sub_test_taxes_computation(self, results, expected_values):
        ...
        def compare_taxes_computation_values(sub_results, rounding):
            self.assertEqual(
                ...
                float_round(
                    expected_values["total_included"], precision_rounding=rounding
                ),
            )
            self.assertEqual(
                float_round(sub_results["total_excluded"], precision_rounding=rounding),
                float_round(
                    expected_values["total_excluded"], precision_rounding=rounding
                ),
            )
            self.assertEqual(
        ...
        if is_round_globally:
            ...
            if "total_excluded" not in excluded_special_modes:
                compare_taxes_computation_values(
                    results["total_excluded_results"], rounding
                )
                delta = sum(
                    x["tax_amount"]
                ...
                self.assertEqual(
                    float_round(
                        (results["total_excluded_results"]["total_excluded"] + delta)
                        / results["quantity"],
            ...
            if "total_included" not in excluded_special_modes:
                compare_taxes_computation_values(
                    results["total_included_results"], rounding
                )
                delta = sum(
                    x["tax_amount"]
                ...
                self.assertEqual(
                    float_round(
                        (results["total_included_results"]["total_included"] - delta)
                        / results["quantity"],
                        precision_rounding=rounding,
                    ),
                    float_round(results["price_unit"], precision_rounding=rounding),
    ...
    def _create_py_sub_test_taxes_computation(
        self,
        taxes,
        price_unit,
        quantity,
    ...
    def _create_js_sub_test_taxes_computation(
        self,
        taxes,
        price_unit,
        quantity,
    ...
    def assert_adapt_price_unit_to_another_taxes(
        self,
        price_unit,
        original_taxes,
        new_taxes,
    ...
    def _assert_sub_test_base_lines_tax_details(self, results, expected_values):
        self.assertEqual(
            len(results["base_lines_tax_details"]),
    ...
    def _create_py_sub_test_tax_totals_summary(
        self, document, excluded_tax_group_ids, soft_checking
    ):
        AccountTax = self.env["account.tax"]
        ...
        if excluded_tax_group_ids:
            tax_totals = AccountTax._exclude_tax_groups_from_tax_totals_summary(
                tax_totals, excluded_tax_group_ids
            )
        return {"tax_totals": tax_totals, "soft_checking": soft_checking}

    def _create_js_sub_test_tax_totals_summary(
        self, document, excluded_tax_group_ids, soft_checking
    ):
        return {
    ...
    def assert_tax_totals_summary(
        self,
        document,
        expected_values,
        excluded_tax_group_ids=None,
        soft_checking=False,
    ):
        self._create_assert_test(
            expected_values,
    ...
    def assert_py_tax_totals_summary(
        self,
        document,
        expected_values,
        excluded_tax_group_ids=None,
        ...
        results = self._create_py_sub_test_tax_totals_summary(
            document, excluded_tax_group_ids, soft_checking
        )
    ...
    def _create_py_sub_test_global_discount(
        self, document, amount_type, amount, soft_checking
    ):
        AccountTax = self.env["account.tax"]
        ...
        AccountTax._add_tax_details_in_base_lines(
            new_document["lines"], self.env.company
        )
    ...
    def _create_js_sub_test_global_discount(
        self, document, amount_type, amount, soft_checking
    ):
        return {
    ...
    def _create_py_sub_test_down_payment(
        self, document, amount_type, amount, soft_checking
    ):
        AccountTax = self.env["account.tax"]
    ...
    def _create_js_sub_test_down_payment(
        self, document, amount_type, amount, soft_checking
    ):
        return {
            "test": "down_payment",
    ...
    def assert_invoice_totals(self, invoice, expected_values):
        ...
        if "base_amount_currency" in expected_values:
            expected_amounts["amount_untaxed"] = (
                expected_values["base_amount_currency"]
    ...
    def assert_invoice_tax_totals_summary(
        self, invoice, expected_values, soft_checking=False
    ):
        self._assert_tax_totals_summary(
...
class TestAccountMergeCommon(AccountTestInvoicingCommon):
    ...
    def _create_references_to_account(self, account):
        ...
        move = self.env["account.move"].create(
            {
                ...
                .search([("company_id", "=", account.company_ids.id)], limit=1)
                .id,
                "date": "2024-07-20",
                "line_ids": [
                    Command.create(
                    ...
                    Command.create(
                        {
                            ...
                            .search(
                                [("company_ids", "=", account.company_ids.id)], limit=1
                            )
                            .id,
                            "balance": -10.0,
                        }
        ...
        partner = (
            ...
            .with_company(account.company_ids)
            .create(
                {
...
class PatchRequestsMixin(TestCase):
    ...
    @contextmanager
    def assertRequests(
        self, expected_requests_and_responses: list[tuple[dict, requests.Response]]
    ):
    ...
    def assertMockRequests(self, expected_requests_and_responses):
        ...
        def mock_request(session, method, url, **kwargs):
            ...
            try:
                expected_request, mocked_response = next(
                    expected_requests_and_responses_iter
                )
        ...
        next_expected_request, _ = next(
            expected_requests_and_responses_iter, (None, None)
        )
    ...
    def patch_requests_warn(self, expected_requests_and_responses):
        ...
        def request_and_warn(session, method, url, **kwargs):
            ...
            try:
                expected_request, expected_response = next(
                    expected_requests_and_responses_iter
                )
            except StopIteration:
            ...
            try:
                ...
                _logger.warning(
                    "Request differs from expected request: \n%s", e.args[0]
                )

            actual_response = original_request_method(session, method, url, **kwargs)

            if diff_msg := self.difference_between_responses(
                actual_response, expected_response
            ):
                _logger.warning(
                    "Response differs from expected response:\n%s", diff_msg
                )
