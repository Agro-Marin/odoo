...
def test_all_l10n(env):
    ...
    def try_loading_patch(
        self, template_code, company, install_demo=True, force_create=True
    ):
    ...
    with (
        patch.object(AccountChartTemplate, "try_loading", try_loading_patch),
    ...
    if demo_failures:
        ...
        for failure in demo_failures:
            _logger.warning(
                "Demo data of module %s has failed: %s",
                failure.module_id.name,
                failure.error,
            )
        ...
        for field in model._fields.values():
            ...
            env.cr.execute(
                SQL(
                    ...
                    SQL("")
                    if field.required
                    else SQL(" WHERE %s IS NOT NULL", SQL.identifier(field.name)),
                )
            )
            idxs.append(idxname)
    ...
    not_loaded_codes = [
        ...
        for template_code, template in env["account.chart.template"]
            ...
            if company.fiscal_position_ids and not company.domestic_fiscal_position_id:
                ...
                if len(potential_domestic_fps) > 1:
                    potential_domestic_fps.sorted(
                        lambda x: x.country_id.id or float("inf")
                    ).sorted("sequence")
                    if (
                        potential_domestic_fps[0].country_id
                        == potential_domestic_fps[1].country_id
                    ) and (
                        potential_domestic_fps[0].sequence
                        == potential_domestic_fps[1].sequence
                    ):
                        _logger.warning(
                            "Several fiscal positions fitting for being tagged as domestic were found in fiscal data for %s %s.",
                            company.country_id.name,
                            template_code,
                        )
        except Exception:
            _logger.exception("Error when creating COA %s", template_code)
            env.cr.rollback()
