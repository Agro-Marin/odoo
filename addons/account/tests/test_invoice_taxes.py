...
class TestInvoiceTaxes(AccountTestInvoicingCommon):
    ...
    def test_one_tax_per_line(self):
        ...
        invoice.action_post()
        self.assertRecordValues(
            invoice.line_ids.filtered("tax_line_id").sorted(lambda x: x.name),
            [
                {
                    "name": self.percent_tax_2.name,
                    "tax_base_amount": -100,
                    "balance": -12,
                    "tax_ids": [],
                },
                ...
                {
                    "name": self.percent_tax_1_incl.name,
                    "tax_base_amount": -100,
                    "balance": -21,
                    "tax_ids": [],
                },
    ...
    def test_affecting_base_amount(self):
        ...
        invoice.action_post()
        self.assertRecordValues(
            invoice.line_ids.filtered("tax_line_id").sorted(lambda x: -x.balance),
            [
                {
                    "name": self.percent_tax_1_incl.name,
                    "tax_base_amount": -100,
                    "balance": -21,
                    "tax_ids": [self.percent_tax_2.id],
                },
                {
                    "name": self.percent_tax_2.name,
                    "tax_base_amount": -221,
    ...
    def test_tax_repartition(self):
        ...
        account_1 = self.env["account.account"].create(
            {"name": "test1", "code": "test1", "account_type": "asset_current"}
        )
        account_2 = self.env["account.account"].create(
            {"name": "test2", "code": "test2", "account_type": "asset_current"}
        )
        ...
        self.assertEqual(
            ...
            4,
            "There should be 4 account move lines created for the invoice: payable, base and 2 tax lines",
        )
        inv_base_line = invoice.line_ids.filtered(
            lambda x: (
                not x.tax_repartition_line_id
                and x.account_id.account_type != "asset_receivable"
            )
        )
        self.assertEqual(
            len(inv_base_line), 1, "There should be only one base line generated"
        )
        self.assertEqual(abs(inv_base_line.balance), 100, "Base amount should be 100")
        self.assertEqual(
            inv_base_line.tax_tag_ids,
            inv_base_tag,
            "Base line should have received base tag",
        )
        inv_tax_lines = invoice.line_ids.filtered(
            lambda x: x.tax_repartition_line_id.repartition_type == "tax"
        )
        self.assertEqual(
            len(inv_tax_lines),
            2,
            "There should be two tax lines, one for each repartition line.",
        )
        self.assertEqual(
            abs(inv_tax_lines.filtered(lambda x: x.account_id == account_1).balance),
            4.2,
            "Tax line on account 1 should amount to 4.2 (10% of 42)",
        )
        self.assertEqual(
            inv_tax_lines.filtered(lambda x: x.account_id == account_1).tax_tag_ids,
            inv_tax_tag_10,
            "Tax line on account 1 should have 10% tag",
        )
        self.assertAlmostEqual(
            abs(inv_tax_lines.filtered(lambda x: x.account_id == account_2).balance),
            37.8,
            2,
            "Tax line on account 2 should amount to 37.8 (90% of 42)",
        )
        self.assertEqual(
            inv_tax_lines.filtered(lambda x: x.account_id == account_2).tax_tag_ids,
            inv_tax_tag_90,
            "Tax line on account 2 should have 90% tag",
        )
        ...
        self.assertEqual(
            len(refund.line_ids),
            4,
            "There should be 4 account move lines created for the refund: payable, base and 2 tax lines",
        )
        ref_base_line = refund.line_ids.filtered(
            lambda x: (
                not x.tax_repartition_line_id
                and x.account_id.account_type != "asset_receivable"
            )
        ...
        self.assertEqual(
            len(ref_base_line), 1, "There should be only one base line generated"
        )
        self.assertEqual(abs(ref_base_line.balance), 100, "Base amount should be 100")
        self.assertEqual(
            ref_base_line.tax_tag_ids,
            ref_base_tag,
            "Base line should have received base tag",
        )
        ref_tax_lines = refund.line_ids.filtered(
            lambda x: x.tax_repartition_line_id.repartition_type == "tax"
        )
        self.assertEqual(len(ref_tax_lines), 2, "There should be two refund tax lines")
        self.assertEqual(
            abs(
                ref_tax_lines.filtered(
                    lambda x: x.account_id == ref_base_line.account_id
                ).balance
            ),
            4.2,
            "Refund tax line on base account should amount to 4.2 (10% of 42)",
        )
        self.assertAlmostEqual(
            abs(ref_tax_lines.filtered(lambda x: x.account_id == account_1).balance),
            37.8,
            2,
            "Refund tax line on account 1 should amount to 37.8 (90% of 42)",
        )
        self.assertEqual(
            ref_tax_lines.mapped("tax_tag_ids"),
            ref_tax_tag,
            "Refund tax lines should have the right tag",
        )
    ...
    def test_misc_journal_entry_tax_tags_sale(self):
        ...
        inv_tax_rep_ln = sale_tax.invoice_repartition_line_ids.filtered(
            lambda x: x.repartition_type == "tax"
        )
        ref_tax_rep_ln = sale_tax.refund_repartition_line_ids.filtered(
            lambda x: x.repartition_type == "tax"
        )
        ...
        self.assertRecordValues(
            ...
            [
                ...
                },
                {
                    ...
                    "tax_repartition_line_id": ref_tax_rep_ln.id,
                },
                {
                    "balance": 1000.0,
        ...
        self.assertRecordValues(
            ...
            [
                ...
                },
                {
                    ...
                    "tax_tag_ids": self.tax_tag.ids,
                    "tax_base_amount": -1000,
                    "tax_repartition_line_id": inv_tax_rep_ln.id,
                },
                {
                    "balance": 1100.0,
                    "tax_ids": [],
    ...
    def test_misc_journal_entry_tax_tags_purchase(self):
        ...
        inv_tax_rep_ln = purch_tax.invoice_repartition_line_ids.filtered(
            lambda x: x.repartition_type == "tax"
        )
        ref_tax_rep_ln = purch_tax.refund_repartition_line_ids.filtered(
            lambda x: x.repartition_type == "tax"
        )
    ...
    def test_misc_entry_tax_group_signs(self):
            ...
            return self.env["account.tax"].create(
                {
                    ...
                    "amount": 10,
                    "children_tax_ids": [
                        (6, 0, [child1_sale_tax.id, child2_sale_tax.id])
                    ],
                }
            )
        ...
        self.assertRecordValues(
            debit_sale_move.line_ids.sorted("balance"),
            [
                {
                    "balance": -1150.0,
                    "tax_ids": [],
        ...
        self.assertRecordValues(
            credit_sale_move.line_ids.sorted("balance"),
            [
                {
                    "balance": -1000.0,
                    "tax_ids": sale_group.ids,
        ...
        self.assertRecordValues(
            debit_purchase_move.line_ids.sorted("balance"),
            [
                {
                    "balance": -1150.0,
                    "tax_ids": [],
        ...
        self.assertRecordValues(
            credit_purchase_move.line_ids.sorted("balance"),
            [
                {
                    "balance": -1000.0,
                    "tax_ids": purchase_group.ids,
    ...
    def test_ensure_no_unbalanced_entry(self):
        ...
        invoice = self._create_invoice(
            ...
            invoice_payment_term_id=self.pay_terms_a,
        )
        invoice.action_post()
    ...
    def test_tax_calculation_multi_currency(self):
        ...
        self.assertRecordValues(
            invoice.line_ids.filtered(lambda l: not l.name),
            [
                {
                    "balance": 686.53,
                }
    ...
    def test_tax_repartition_lines_dispatch_amount_1(self):
        ...
        tax_reps = [
            Command.create({"repartition_type": "tax", "factor_percent": 10.0})
        ] * 10
        tax = self.env["account.tax"].create(
            {
        ...
        tax = self.env["account.tax"].create(
            {
                ...
                "amount_type": "percent",
                "amount": 5.0,
                "invoice_repartition_line_ids": [base_tax_rep]
                + plus_tax_reps
                + neg_tax_reps,
                "refund_repartition_line_ids": [base_tax_rep]
                + plus_tax_reps
                + neg_tax_reps,
            }
        )
    ...
    def test_product_account_tags(self):
        ...
        self.assertRecordValues(
            ...
            [
                ...
                {
                    "tax_ids": [],
                    "tax_line_id": self.percent_tax_1.id,
                    "tax_tag_ids": product_tag.ids,
                    "credit": 210,
                    "debit": 0,
                },
                ...
                {
                    "tax_ids": [],
                    "tax_line_id": self.percent_tax_2.id,
                    "tax_tag_ids": [],
                    "credit": 24,
                    "debit": 0,
                },
    ...
    def test_fiscal_position_tax_mapping_with_inactive_tax(self):
        ...
        move = self.init_invoice(
            "out_invoice", self.partner_a, post=True, products=[self.product]
        )
        line = move.invoice_line_ids
        self.assertEqual(
            line.tax_ids,
            active_tax,
            f"The tax should be {active_tax.name}, but is is currently {line.tax_ids.name}",
        )
        ...
        results = invoice.onchange(
            {
                "invoice_line_ids": [
                    Command.update(
                        ...
                        {"price_unit": 2000.0, "product_id": self.product_b.id},
                    )
                ]
            },
            ["invoice_line_ids"],
