...
class TestAccountPaymentRegister(AccountTestInvoicingCommon, PaymentCommon):
    ...
    def setUpClass(cls):
        ...
        cls.other_currency_2 = cls.setup_other_currency(
            "CAD", rates=[("2016-01-01", 3.0), ("2017-01-01", 0.01)]
        )
        ...
        cls.term_0_5_10_days = cls.env["account.payment.term"].create(
            {
                ...
                "line_ids": [
                    Command.create(
                        {"value": "percent", "value_amount": 10.0, "nb_days": 0}
                    ),
                    Command.create(
                        {"value": "percent", "value_amount": 30.0, "nb_days": 5}
                    ),
                    Command.create(
                        {"value": "percent", "value_amount": 60.0, "nb_days": 10}
                    ),
                ],
            }
        )
    ...
    def test_register_payment_single_batch_not_grouped(self):
        ...
        payment_register = (
            self.env["account.payment.register"]
            .with_context(active_model="account.move", active_ids=active_ids)
            .create({"group_payment": False, "amount": 1200.0})
        )
    ...
    def test_register_payment_multi_batches_grouped_with_credit_note(self):
        ...
        invoice_2 = invoice_1.copy(
            {"invoice_date": invoice_1.invoice_date, "partner_id": partner_b.id}
        )
    ...
    def test_register_payment_doesnt_send_email(self):
        ...
        if self.env["ir.module.module"]._get("payment_demo").state == "installed":
            payment_token = self._create_token(
                provider_id=self._prepare_provider(code="demo").id,
        ...
        payment_register = (
            self.env["account.payment.register"]
            .with_context(
                active_model="account.move", active_ids=self.out_invoice_4.ids
            )
    ...
    def test_register_payment_multi_currency_rounding_issue_positive_delta(self):
        ...
        payment = (
            self.env["account.payment.register"]
            .with_context(
                active_model="account.move", active_ids=self.out_invoice_3.ids
            )
            .create(
                {
    ...
    def test_register_payment_multi_currency_rounding_issue_writeoff_lower_amount_keep_open(
        self,
    ):
        payment = (
            self.env["account.payment.register"]
    ...
    def test_register_payment_multi_currency_rounding_issue_writeoff_lower_amount_reconcile_positive_delta(
        self,
    ):
        payment = (
            self.env["account.payment.register"]
    ...
    def test_register_payment_multi_currency_rounding_issue_writeoff_lower_amount_reconcile_negative_delta(
        self,
    ):
        payment = (
            self.env["account.payment.register"]
    ...
    def test_register_payment_multi_currency_rounding_issue_writeoff_higher_amount_reconcile_positive_delta(
        self,
    ):
        payment = (
            self.env["account.payment.register"]
    ...
    def test_register_payment_multi_currency_rounding_issue_writeoff_higher_amount_reconcile_negative_delta(
        self,
    ):
        payment = (
            self.env["account.payment.register"]
    ...
    def test_register_foreign_currency_on_invoice_exchange_writeoff_account(self):
        ...
        default_tax.cash_basis_transition_account_id = self.env[
            "account.account"
        ].create(
            {
        ...
        self.assertRecordValues(
            caba_move.line_ids.sorted("balance"),
            [
                {
                    "balance": -321.74,
                    "currency_id": self.other_currency.id,
    ...
    def test_register_payment_inbound_multiple_bank_account(self):
        ...
        wizard = (
            self.env["account.payment.register"]
            .with_context(**ctx)
            .create({"journal_id": self.bank_journal_2.id})
        )
        ...
        refund_1, refund_2 = self.env["account.move"].create(
            [
                ...
                },
                {
                    ...
                    "date": "2017-01-01",
                    "invoice_date": "2017-01-01",
                    "partner_id": self.partner_b.copy(
    ...
    def test_group_payment_method_with_and_without_discount_and_refund(self):
        ...
        self.assertEqual(
            wizard.installments_switch_amount, 39.50, "With the full refund and the epd"
        )
    ...
    def test_keep_user_amount(self):
        ...
        invoice.action_post()
        self.assertRecordValues(
            invoice.line_ids.sorted().filtered("date_maturity"),
            [
                {
                ...
                {
                    "date_maturity": fields.Date.from_string("2016-03-01"),
    ...
    def test_installment_mode_single_batch(self):
        ...
        self.assertRecordValues(
            ...
            [
                {
                    "date_maturity": fields.Date.from_string("2016-01-01"),
                    "amount_currency": 115.0,
                },
                {
                    "date_maturity": fields.Date.from_string("2016-01-06"),
                    "amount_currency": 345.0,
                },
                {
                    "date_maturity": fields.Date.from_string("2016-01-11"),
    ...
    def test_installment_mode_multiple_batches(self):
        ...
        wizard = (
            ...
            .with_context(
                active_model="account.move",
                active_ids=(self.in_invoice_1 + in_invoice_cad_with_payment_term).ids,
            )
            .create({"payment_date": "2016-01-01"})
        )
        ...
        wizard = (
            ...
            .with_context(
                active_model="account.move",
                active_ids=(in_invoice_cad_with_payment_term + in_invoice_cad_copy).ids,
            )
            .create({"payment_date": "2016-01-07"})
        )
        ...
        wizard = (
            ...
            .with_context(
                active_model="account.move",
                active_ids=(
                    self.in_invoice_epd_applied + in_invoice_cad_with_payment_term
                ).ids,
            )
            .create({"payment_date": "2016-01-01"})
        )
    ...
    def test_payment_register_with_next_payment_date(self):
        ...
        wizard = (
            ...
            .with_context(
                ...
                ],  # To mimick next_payment date in the search
                active_model="account.move",
    ...
    def test_payment_with_branch(self):
        ...
        def test_register_payment_flow(cases):
            for group_payment, case in product((False, True), cases):
                ...
                with self.subTest(
                    ...
                    selected_companies=selected_companies.mapped("name"),
                ):
                    wizard = (
                        self.env["account.payment.register"]
                        .with_context(
                    ...
                    if should_raise:
                        with self.assertRaisesRegex(
                            UserError, "got some company inconsistencies here:"
                        ):
        ...
        for branch in branches:
            ...
            branch_invoices |= self.init_invoice(
                "out_invoice", products=self.product_a, company=branch
            )

        parent_invoice = self.init_invoice("out_invoice", products=self.product_a)
        other_company_invoice = self.init_invoice(
            "out_invoice", products=self.product_a, company=company_2
        )
        ...
        with self.with_user("user_branch"):
            with self.assertRaisesRegex(
                ...
                self.env["account.payment.register"].with_context(
                    allowed_company_ids=branches.ids,
                    active_model="account.move",
    ...
    def test_epd_and_cash_rounding(self):
        cash_rounding = self.env["account.cash.rounding"].create(
            {
                ...
                "profit_account_id": self.company_data["default_account_revenue"]
                .copy()
                .id,
                "loss_account_id": self.company_data["default_account_expense"]
                .copy()
                .id,
                "rounding_method": "UP",
            }
        ...
        ).create({"payment_date": "2024-01-01"})._create_payments()
    ...
    def test_payment_register_wizard_without_receivable_line_due_date(self):
        ...
        self.assertRecordValues(
            ...
            [
                ...
                {
                    "amount_currency": 300,
                    "date_maturity": fields.Date.from_string("2017-01-06"),
                },
                {
                    "amount_currency": 600,
                    "date_maturity": fields.Date.from_string("2017-01-11"),
                },
            ],
        )
    ...
    def test_payment_register_misc_different_batches(self):
        ...
        self.assertEqual(
            len(payments),
            3,
            "We should get 2 payments from the first move and 1 for the second one",
        )
