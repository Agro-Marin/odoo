...
_AUTO_POST_LOGGER = "odoo.addons.account.models.account_move"
...
class TestAccountMove(AccountTestInvoicingCommon):
    ...
    def test_autopost_failure_keeps_entry_scheduled(self):
        ...
        for error, is_logged in (
            ...
            with self.subTest(error=type(error).__name__):
                expect_log = (
                    self.assertLogs(_AUTO_POST_LOGGER, "WARNING")
                    if is_logged
                    else self.assertNoLogs(_AUTO_POST_LOGGER, "WARNING")
                )
                with (
                    freeze_time(self.test_move.date + relativedelta(days=1)),
                    self.enter_registry_test_mode(),
                ...
                if is_logged:
                    self.assertEqual(len(logs.output), 1, "only the retry is logged")
                    self.assertIn(f"skipping move {self.test_move.id}", logs.output[0])
    ...
    def test_posting_future_invoice_fails(self):
        ...
        with self.assertRaisesRegex(
            UserError, "This move is configured to be auto-posted"
        ):
    ...
    def test_out_invoice_auto_post_monthly(self):
        ...
        new_invoices_2 = (
            self.env["account.move"].search(domain=[]) - prev_invoices - new_invoices_1
        )
    ...
    def test_modify_posted_move_readonly_fields(self):
        ...
        for field in readonly_fields:
            with self.assertRaisesRegex(
                UserError,
                "You cannot modify the following readonly fields on a posted move",
            ):
    ...
    def test_included_tax(self):
        ...
        self.assertRecordValues(
            move.line_ids.sorted(lambda x: -x.balance),
            [
                {
                    "name": "debit_line_1",
                    "debit": 1000.0,
    ...
    def test_misc_prevent_unlink_posted_items(self):
        def unlink_posted_items():
            self.test_move.line_ids.filtered(
    ...
    def test_entry_reverse_storno(self):
        ...
        self.assertRecordValues(
            reversed_move.line_ids,
            [
                {**self.entry_line_vals_1, "debit": -500.0, "credit": 0.0},
                {**self.entry_line_vals_2, "debit": 0.0, "credit": -500.0},
            ],
        )

        move_reversal_2 = self.env["account.move.reversal"].create(
            {
        ...
        reversal_2 = move_reversal_2.refund_moves()
        reversed_move_2 = self.env["account.move"].browse(reversal_2["res_id"])
        self.assertRecordValues(
            reversed_move_2.line_ids,
            [
                {**self.entry_line_vals_1, "debit": 500.0, "credit": 0.0},
                {**self.entry_line_vals_2, "debit": 0.0, "credit": 500.0},
            ],
    ...
    def test_manual_entries_storno(self):
        def manual_entry(
            ...
            def merge_overrides(base_vals, overrides):
                return [
                    {**(line or {}), **(override or {})}
                    for line, override in zip_longest(base_vals, overrides)
                ]
            ...
            input_assert_vals = [revenue_line, expense_line]
            normal_assert_vals = merge_overrides(
                input_assert_vals, normal_assert_overrides
            )
            reverse_assert_vals = merge_overrides(
                input_assert_vals, reverse_assert_overrides
            )
        ...
        manual_entry(
            is_storno=False,
            normal_assert_overrides=[
                {"debit": 1000.0, "credit": 0.0},
                {"debit": 0.0, "credit": 1000.0},
            ],
            reverse_assert_overrides=[
                {"debit": 0.0, "credit": 1000.0},
                {"debit": 1000.0, "credit": 0.0},
            ],
        )
    ...
    def test_misc_prevent_edit_tax_on_posted_moves(self):
        ...
        self.test_move.action_post()
        with self.assertRaisesRegex(
            UserError, "You cannot modify the taxes related to a posted journal item"
        ):
    ...
    def test_misc_with_taxes_reverse(self):
        ...
        sale_invoice_rep_line = sale_tax.invoice_repartition_line_ids.filtered(
            lambda x: x.repartition_type == "tax"
        )

        self.assertRecordValues(
            sale_move.line_ids.sorted(lambda x: -x.balance),
            [
                {
                    "name": "debit",
                    "debit": 115.0,
                ...
                },
                {
                    ...
                    "tax_ids": [],
                    "tax_base_amount": -100,
                    "tax_repartition_line_id": sale_invoice_rep_line.id,
                },
                {
                    "name": "credit",
                    "debit": 0.0,
        ...
        self.assertRecordValues(
            purchase_move.line_ids.sorted(lambda x: x.balance),
            [
                {
                    "name": "credit",
                    "credit": 115.0,
                ...
                },
                {
                    ...
                    "tax_ids": [],
                    "tax_base_amount": 100,
                    "tax_repartition_line_id": purchase_invoice_rep_line.id,
                },
                {
                    "name": "debit",
                    "credit": 0.0,
    ...
    def test_change_journal_posted_before(self):
        ...
        self.test_move.action_draft()
        self.assertEqual(
            self.test_move.journal_id, self.company_data["default_journal_misc"]
        )
    ...
    def test_change_journal_sequence_number(self):
        ...
        self.test_move.action_post()
        test_move_2 = self.test_move.copy(
            {"name": "TEST/2016/01/0002", "date": "2016-01-01"}
        )
        self.assertEqual(test_move_2.sequence_number, 2)
        self.assertEqual(
            test_move_2.journal_id, self.company_data["default_journal_misc"]
        )
        with self.assertRaisesRegex(
            UserError,
    ...
    def test_line_steal(self):
        ...
        with (
            self.assertRaisesRegex(UserError, "not balanced"),
            contextlib.closing(self.env.cr.savepoint()),
        ):
            self.env["account.move"].create(
                {"line_ids": [Command.set(honest_move.line_ids[0].ids)]}
            )
        ...
        with (
            ...
            self.env["account.move"].create(
                {"line_ids": [Command.link(honest_move.line_ids[0].id)]}
            )
    ...
    def test_validate_move_wizard_with_auto_post_entry(self):
        ...
        wizard = (
            ...
            .with_context(active_model="account.move", active_ids=self.test_move.ids)
            .create({})
        )
            ...
            return self.env["account.tax"].create(
                {
                    ...
                    "invoice_repartition_line_ids": [
                        Command.create(
                            {
                                "repartition_type": "base",
                                "tag_ids": _create_tax_tag(
                                    f"invoice_base_{tax_percent}"
                                ).ids,
                            }
                        ),
                        Command.create(
    ...
    def test_balance_modification_auto_balancing(self):
        ...
        move.write(
            {
                "line_ids": [
                    Command.update(line.id, {"debit": 10, "credit": 0, "balance": 10}),
                    Command.create(
                        {
                            "account_id": account.id,
                            "balance": -30,
    ...
    def test_no_recompute_when_company_address_changes(self):
        ...
        original_recompute = {
            model: self.env[model]._recompute_field for model in protected_models
        }
        fields_recomputed = []
        ...
        def mock_recompute(self, field, ids=None):
            ...
            if field.store and (
                ...
                or (
                    ...
                    and set(invoice.line_ids.ids) & set(ids)
                )
            ):
    ...
    def test_post_invoice_fails_with_account_and_journal_company_inconsistency(self):
        ...
        move = self.env["account.move"].create(
            {
                "line_ids": [
                    Command.create(
                        {"name": "debit_line", "debit": 100.0, "account_id": account.id}
                    ),
                    Command.create(
                        {
                            "name": "credit_line",
        ...
        move.write(
            {"name": "/", "company_id": company_b.id, "journal_id": journal_b.id}
        )
        with self.assertRaisesRegex(
            UserError,
            rf"The entry is using accounts \({account.display_name}\) from a different company\.",
        ):
            move.action_post()
            ...
            [
                {
                    ...
                    "code_mapping_ids": [
                        Command.create(
                            {"company_id": self.env.company.id, "code": "180001"}
                        ),
