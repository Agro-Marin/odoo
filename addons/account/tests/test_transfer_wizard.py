...
class TestTransferWizard(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.analytic_account_1, cls.analytic_account_2 = cls.env[
    ...
    def test_transfer_default_tax(self):
        ...
        with Form(
            self.env["account.automatic.entry.wizard"].with_context(context)
        ) as wizard_form:
    ...
    def test_transfer_wizard_reconcile(self):
        ...
        payable_transfer = transfer_move.line_ids.filtered(
            lambda x: x.account_id == self.payable_account
        )
        receivable_transfer = transfer_move.line_ids.filtered(
            lambda x: x.account_id == self.receivable_account
        )

        self.assertTrue(
            payable_transfer.reconciled,
            "Payable line of the transfer move should be fully reconciled",
        )
        self.assertAlmostEqual(
            self.move_1.line_ids.filtered(
                lambda x: x.account_id == self.receivable_account
            ).amount_residual,
            100,
    ...
    def test_transfer_wizard_grouping(self):
        ...
        self.assertAlmostEqual(
            groups[
                (self.accounts[0], self.partner_a, self.company_data["currency"])
            ].balance,
            -800,
            self.company.currency_id.decimal_places,
        )
        self.assertAlmostEqual(
            groups[(self.accounts[1], None, self.company_data["currency"])].balance,
            500,
            self.company.currency_id.decimal_places,
        )
        self.assertAlmostEqual(
            groups[
                (self.accounts[1], self.partner_b, self.company_data["currency"])
            ].balance,
            -480,
            self.company.currency_id.decimal_places,
        )
        self.assertAlmostEqual(
            groups[
                (self.accounts[2], self.partner_a, self.company_data["currency"])
            ].balance,
            1030,
            self.company.currency_id.decimal_places,
        )
        self.assertAlmostEqual(
            groups[(self.accounts[2], self.partner_a, self.test_currency_2)].balance,
            512,
            self.company.currency_id.decimal_places,
        )
        self.assertAlmostEqual(
            groups[
                (self.accounts[3], self.partner_a, self.company_data["currency"])
            ].balance,
            -250,
            self.company.currency_id.decimal_places,
        )
    ...
    def test_transfer_wizard_currency_conversion(self):
        ...
        destination_line = transfer_move.line_ids.filtered(
            lambda x: x.account_id == self.test_currency_account
        )
        self.assertEqual(
            destination_line.currency_id,
            self.test_currency_3,
    ...
    def test_transfer_wizard_no_currency_conversion(self):
        ...
        destination_lines = transfer_move.line_ids.filtered(
            lambda x: x.account_id == self.receivable_account
        )
        self.assertEqual(
        ...
        self.assertAlmostEqual(
            destination_lines.filtered(
                lambda x: x.currency_id == self.test_currency_1
            ).amount_currency,
            -10,
        ...
        self.assertAlmostEqual(
            destination_lines.filtered(
                lambda x: x.currency_id == self.test_currency_2
            ).amount_currency,
            -756,
    ...
    def test_period_change_lock_date(self):
        ...
        wizard = (
            ...
            .with_context(
                active_model="account.move.line", active_ids=move.line_ids[0].ids
            )
            .create(
                {
    ...
    def test_transfer_wizard_amount_currency_is_zero(self):
        ...
        active_move_lines = move.line_ids.filtered(
            lambda line: line.account_id.id == self.receivable_account.id
        )
        context = {
            "active_model": "account.move.line",
            "active_ids": active_move_lines.ids,
        ...
        source_line = transfer_move.line_ids.filtered(
            lambda x: x.account_id == self.receivable_account
        )
        destination_line = transfer_move.line_ids.filtered(
            lambda x: x.account_id == self.accounts[0]
        )
    ...
    def test_transfer_wizard_analytic(self):
        ...
        self.assertRecordValues(
            transfer_move.line_ids,
            [
                {
                    "balance": -4000,
                    "analytic_distribution": {
                        str(self.analytic_account_1.id): 50,
                        str(self.analytic_account_2.id): 25,
                    },
                },
                {
                    "balance": 1000,
