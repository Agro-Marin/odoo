...
class TestBudgetReport(TestAccountReportsCommon):
    ...
    @classmethod
    def _create_moves(
        cls, amount_per_account_ids=None, date_from=None, date_to=None, to_post=True
    ):
        date_from, date_to = (
            fields.Date.to_date(date_from),
            fields.Date.to_date(date_to),
    ...
    def test_reports_multiple_budgets(self):
        """Check the report renders 2 columns per selected budget: the budget amount and its comparison percentage."""
        self._create_moves(
            {
                self.account_1.id: 100,
                self.account_2.id: 200,
        ...
        options = self._generate_options(
            ...
            default_options={
                "budgets": [
                    {"id": self.budget_1.id, "selected": True},
                    {"id": self.budget_2.id, "selected": True},
                ]
            },
        )
    ...
    def test_reports_budget_with_comparison_period(self):
        budget_2024 = self._create_budget(
            {self.account_1.id: 200}, "2024-01-01", "2024-12-31"
        )
        moves = self._create_moves(
            {self.account_1.id: 200}, "2024-01-01", "2024-12-31", to_post=False
        )
        moves += self._create_moves(
            {self.account_1.id: 600}, "2024-06-22", "2024-06-22", to_post=False
        )
        moves.action_post()
    ...
    def test_reports_budget_with_comparison_period_and_multiple_budgets(self):
        ...
        budget_2023 = self._create_budget(
            {self.account_1.id: 400}, "2023-01-01", "2023-12-31"
        )
        moves = self._create_moves(
            {self.account_1.id: 200}, "2024-01-01", "2024-12-31", to_post=False
        )
        moves += self._create_moves(
            {self.account_1.id: 400}, "2023-01-01", "2023-12-31", to_post=False
        )
        moves.action_post()
        ...
        options = self._generate_options(
            ...
            default_options={
                "budgets": [
                    {"id": budget_2024.id, "selected": True},
                    {"id": budget_2023.id, "selected": True},
                ],
                "comparison": {"filter": "previous_period", "number_period": 1},
            },
        )
    ...
    def test_report_budget_items_with_different_date_filters(self):
        """Check the budget items reported match the selected date range."""
        budget_2024 = self._create_budget(
            {self.account_1.id: 300}, "2024-01-01", "2024-12-31"
        )
    ...
    def test_report_budget_edit_items(self):
        """Check how a budget item edition is spread over the months of the selected period.

        The difference between the old and the new value is split evenly over those months, the
        rounding remainder being added to the last one.
        """
        ...
        # Rounded value (1200) on a whole year: 100 per month
        ...
        # 900 over the same 3 months: +200 spread on each, bringing them to 300
        ...
        # Reduce the whole year to a non-rounded value: the remainder lands on the last month
    ...
    def test_report_budget_show_all_accounts_filter(self):
        """Check the 'show all accounts' filter returns every income account, budgeted or not.

        The expected universe is the income accounts because the test report only targets those.
        """
    ...
    def test_financial_budget_with_analytic_groupby(self):
        ...
        moves_2024 = self._create_moves(
            {self.account_1.id: 100}, "2024-01-01", "2024-12-31", to_post=False
        )
        moves_2025 = self._create_moves(
            {self.account_1.id: 150}, "2025-01-01", "2025-12-31", to_post=False
        )

        for move_line in (moves_2024 + moves_2025).line_ids:
            move_line.analytic_distribution = {
                str(analytic_account_1.id): 75.0,
                str(analytic_account_2.id): 25.0,
            }
        ...
        options = self._generate_options(
            ...
            default_options={
                ...
                "analytic_accounts_groupby": [
                    analytic_account_1.id,
                    analytic_account_2.id,
                ],
                "budgets": [
                    {"id": budget_a.id, "selected": True},
                    {"id": budget_b.id, "selected": True},
                ],
            },
        )
    ...
    def test_financial_budget_with_several_columns(self):
        ...
        and that the budget percentage column is hidden when multiple monetary columns exist"""
        self.report.write(
            {
                "column_ids": [
                    Command.create(
    ...
    def test_financial_budget_with_horizontal_and_analytic_groupbys(self):
        ...
        analytic_account_1 = self.env["account.analytic.account"].create(
            [{"name": "Account A1", "plan_id": analytic_plan.id}]
        )
        analytic_account_2 = self.env["account.analytic.account"].create(
            [{"name": "Account A2", "plan_id": analytic_plan.id}]
        )
        ...
        for move_line in moves_2025.line_ids:
            ...
            move_line.analytic_distribution = {
                str(analytic_account_1.id): 50.0,
                str(analytic_account_2.id): 25.0,
            }

        moves_2025.action_post()

        move_partner_b = self._create_moves(
            {self.account_1.id: 300}, "2025-01-01", "2025-01-31", to_post=False
        )
    ...
    def test_hierarchy_with_budget(self):
        ...
        profit_and_loss_report = self.env.ref("account.profit_and_loss")
        line_id = self._get_basic_line_dict_id_from_report_line_ref(
            "account.account_financial_report_revenue0"
        )
        options = self._generate_options(
        ...
        unfolded_lines = [
            {"name": line["name"], "level": line["level"]} for line in unfolded_lines
        ]
        ...
        report_with_percentage = self.env["account.report"].create(
            {
                ...
                "filter_date_range": True,
                "filter_budgets": True,
                "root_report_id": self.env.ref("account.profit_and_loss").id,
                "column_ids": [
                    Command.create(
                        {
                            "name": "Balance",
