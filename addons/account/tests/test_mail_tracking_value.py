...
class TestTracking(AccountTestInvoicingCommon, MailCase):
    ...
    def test_aml_change_tracking(self):
        ...
        account_move = self.env["account.move"].create(
            {
                ...
                "invoice_line_ids": [
                    Command.create(
                        {"product_id": self.product_a.id, "price_unit": 200.0}
                    )
                ],
            }
        )
        ...
        field = self.env[tracking_value.field_id.model]._fields[
            tracking_value.field_id.name
        ]
        self.assertFalse(
            field.groups, "There is no group on account.move.line.account_id"
        )
        ...
        multiple_account_moves = [
            {
                ...
                "account_moves": [
                    {"name": "Test Single", "partner_id": self.partner_a.id}
                ],
        ...
        for move in multiple_account_moves:
            with self.subTest(move["description"]):
                ...
                for account_move in account_moves:
                    self.assertEqual(
                        account_move.message_partner_ids, move["expected_partners"]
                    )
