...
class TestAccountEarlyPaymentDiscount(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.pay_30_percents_now_balance_60_days = cls.env[
    ...
    def test_early_payment_end_date(self):
        ...
        for line in inv_1200_10_percents_discount_no_tax.line_ids:
            if line.display_type == "payment_term":
                self.assertEqual(
                    line.discount_date, fields.Date.from_string("2019-01-11") or False
                )
    ...
    def test_early_payment_date_eligibility(self):
        ...
        inv.action_post()
        self.assertTrue(
            inv._is_eligible_for_early_payment_discount(
                inv.currency_id, fields.Date.from_string("2019-01-10")
            )
        )
        self.assertFalse(
            inv._is_eligible_for_early_payment_discount(
                inv.currency_id, fields.Date.from_string("2019-01-12")
            )
        )
                    ...
                    Command.create(
                        {
                            ...
                            "price_unit": 1000.0,
                            "tax_ids": [
                                Command.set(self.product_a.taxes_id.ids + fixed_tax.ids)
                            ],  # 15% tax + fixed 0.05
                        }
                    )
                ],
                    ...
                    Command.create(
                        {
                            ...
                            "tax_ids": [
                                Command.set(
                                    self.product_a.supplier_taxes_id.ids + fixed_tax.ids
                                )
                            ],
                        }
                    )
    ...
    def test_register_discounted_payment_on_single_invoice_with_tax(self):
        self.early_pay_10_percents_10_days.early_pay_discount_computation = "included"
        inv_1500_10_percents_discount_tax_incl_15_percents_tax = self.env[
            "account.move"
        ].create(
            {
                "move_type": "in_invoice",
                    ...
                    Command.create(
                        {
                            ...
                            "price_unit": 1500.0,
                            "tax_ids": [
                                Command.set(self.product_a.supplier_taxes_id.ids)
                            ],
                        }
                    )
                ],
    ...
    def test_register_discounted_payment_multi_line_discount(self):
        ...
        inv_mixed_lines_discount_and_no_discount = self.env["account.move"].create(
            {
                ...
                "invoice_line_ids": [
                    ...
                    Command.create(
                        {"name": "line", "price_unit": 2000.0, "tax_ids": None}
                    ),
                ],
                "invoice_payment_term_id": self.early_pay_10_percents_10_days.id,
            }
        ...
        payments = (
            self.env["account.payment.register"]
            .with_context(active_model="account.move", active_ids=active_ids)
            .create({"payment_date": "2017-01-01", "group_payment": True})
            ._create_payments()
        )
    ...
    def test_mixed_epd_with_tax_no_duplication(self):
        (self.pay_terms_a | self.early_pay_10_percents_10_days).write(
            {"early_pay_discount_computation": "mixed"}
        )
        inv = self.env["account.move"].create(
            {
        ...
        self.assertEqual(
            len(inv.line_ids), 5
        )  # 1 prod, 1 tax, 1 epd, 1 epd tax discount, 1 payment terms
    ...
    def test_mixed_epd_with_tax_deleted_line(self):
        ...
        self.assertEqual(
            len(inv.line_ids), 8
        )  # 2 prod, 2 tax, 1 epd, 2 epd tax discount, 1 payment terms
    ...
    def test_mixed_epd_with_tax_refund(self):
        ...
        self.assertRecordValues(
            reverse_move.line_ids.sorted("id"),
            [
                {
                    "balance": -100.0,
                    "tax_base_amount": 0.0,
        ...
        payment_term = self.env["account.payment.term"].create(
            {
                ...
                "line_ids": [
                    ...
                    Command.create(
                        {"value": "percent", "value_amount": 50, "nb_days": 60}
                    ),
                ],
            }
        )
