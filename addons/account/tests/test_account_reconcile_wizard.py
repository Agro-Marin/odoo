...
class TestAccountReconcileWizard(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.foreign_currency_2 = cls.setup_other_currency(
            "XAF", rates=[("2016-01-01", 6.0), ("2017-01-01", 4.0)]
        )

    def assertWizardReconcileValues(
        self,
        selected_lines,
        input_values,
        wo_expected_values,
        expected_transfer_values=None,
    ):
        wizard = (
        ...
        if expected_transfer_values:
            ...
            regex_match = re.findall(
                r"([+-]*\d*,*\d+\.*\d+)", wizard.transfer_warning_message
            )
            self.assertEqual(
                float(regex_match[0].replace(",", "")),
                transfer_move.amount_total_in_currency_signed
                or transfer_move.amount_total_signed,
            )
            transfer_from_account = transfer_move.line_ids.filtered(
                lambda aml: "Transfer from" in aml.name
            ).account_id
            transfer_to_account = (
                transfer_move.line_ids.account_id - transfer_from_account
            )
            transfer_from_amls = transfer_move.line_ids.filtered(
            ...
            if transfer_amount > 0:
                self.assertEqual(
                    regex_match[1:],
                    [transfer_from_account.code, transfer_to_account.code],
                )
            else:
                self.assertEqual(
                    regex_match[1:],
                    [transfer_to_account.code, transfer_from_account.code],
                )
        write_off_move = wizard.create_write_off()
        self.assertRecordValues(
            write_off_move.line_ids.sorted("balance"), wo_expected_values
        )
        wizard.reconcile()
        if wizard.allow_partials or (
            wizard.edit_mode
            and wizard.reco_currency_id.compare_amounts(
                wizard.edit_mode_amount_currency, wizard.amount_currency
            )
        ):
            self.assertTrue(
                len(selected_lines.matched_debit_ids) > 0
                or len(selected_lines.matched_credit_ids) > 0
            )
        else:
    ...
    def test_wizard_should_not_open(self):
        line_1 = self.create_line_for_reconciliation(
            1000.0, 1000.0, self.company_currency, "2016-01-01"
        )
    ...
    def test_wizard_should_open(self):
        ...
        line_2 = self.create_line_for_reconciliation(
            -500.0, -500.0, self.company_currency, "2016-01-01"
        )
        line_3 = self.create_line_for_reconciliation(
            -500.0, -1500.0, self.foreign_currency, "2016-01-01"
        )
        ...
        for batch, sub_test_name in (
            ...
            with self.subTest(sub_test_name=sub_test_name):
                ...
                self.assertEqual(
                    returned_action.get("res_model"), "account.reconcile.wizard"
                )
    ...
    def test_write_off_same_currency(self):
        ...
        self.assertWizardReconcileValues(
            line_1 + line_2, wizard_input_values, write_off_expected_values
        )
    ...
    def test_write_off_one_foreign_currency(self):
        ...
        self.assertWizardReconcileValues(
            line_1 + line_2, wizard_input_values, expected_values
        )
    ...
    def test_write_off_one_foreign_currency_rounding(self):
        foreign_currency = self.setup_other_currency(
            "CAD",
            rounding=0.01,
            rates=[("2016-01-01", 0.052972554919), ("2017-01-01", 4.0)],
        )

        self.assertFalse(
            self.payable_account_2.account_type
            in ("asset_receivable", "liability_payable")
        )
        self.assertTrue(
            self.receivable_account.account_type
            in ("asset_receivable", "liability_payable")
        )
        for foreign_amount_sign, account in [
            ...
            with self.subTest(
                sub_test_name=f"sign: {foreign_amount_sign}, account: {account.name}"
            ):
                line_1 = self.create_line_for_reconciliation(
                    -foreign_amount_sign * 372239.38,
                    -foreign_amount_sign * 372239.38,
                    self.company_currency,
                    "2016-01-01",
                    account_1=account,
                ...
                line_2 = self.create_line_for_reconciliation(
                    foreign_amount_sign * 377554.0,
                    foreign_amount_sign * 20000.0,
                    foreign_currency,
                    "2016-01-01",
                    account_1=account,
                )
                ...
                self.assertWizardReconcileValues(
                    lines, wizard_input_values, expected_values
                )
    ...
    def test_write_off_mixed_foreign_currencies(self):
        ...
        line_3 = self.create_line_for_reconciliation(
            -400.0, -2400.0, self.foreign_currency_2, "2016-01-01"
        )
        wizard_input_values = {
            "journal_id": self.misc_journal.id,
            "account_id": self.write_off_account.id,
        ...
        self.assertWizardReconcileValues(
            line_1 + line_2 + line_3, wizard_input_values, expected_values
        )

    def test_write_off_one_foreign_currency_change_rate(self):
        foreign_currency = self.setup_other_currency(
            "CAD", rounding=0.001, rates=[("2016-01-01", 0.5), ("2017-01-01", 1 / 3)]
        )
        new_date = fields.Date.from_string("2017-02-01")
        line_1 = self.create_line_for_reconciliation(
            -2000.0, -2000.0, self.company_currency, "2017-01-01"
        )
        line_2 = self.create_line_for_reconciliation(
            2000.0, 1000.0, foreign_currency, "2016-01-01"
        )
        wizard_input_values = {
            "journal_id": self.misc_journal.id,
            "account_id": self.write_off_account.id,
    ...
    def test_write_off_both_same_foreign_currency_ensure_no_exchange_diff(self):
        foreign_currency = self.setup_other_currency(
            "CAD", rounding=0.01, rates=[("2016-01-01", 1 / 0.225)]
        )
        new_date = fields.Date.from_string("2017-02-01")
        line_1 = self.create_line_for_reconciliation(
            21.38, 95.0, foreign_currency, "2016-01-01"
        )
        line_2 = self.create_line_for_reconciliation(
            1.13, 5.0, foreign_currency, "2016-01-01"
        )
        line_3 = self.create_line_for_reconciliation(
            1.13, 5.0, foreign_currency, "2016-01-01"
        )
        ...
        expected_transfer_values = [
            {
                ...
                "name": f"Transfer to {self.receivable_account.display_name}",
                "balance": -100.0,
                "amount_currency": -100.0,
                "currency_id": self.company_currency.id,
            },
            {
                "account_id": self.receivable_account.id,
                "name": f"Transfer from {self.payable_account.display_name}",
                "balance": 100.0,
                "amount_currency": 100.0,
                "currency_id": self.company_currency.id,
            },
    ...
    def test_write_off_with_transfer_account_one_foreign_currency(self):
        line_1 = self.create_line_for_reconciliation(
            1100.0, 1100.0, self.company_currency, "2016-01-01"
        )
        line_2 = self.create_line_for_reconciliation(
            100.0,
            300.0,
    ...
    def test_write_off_with_complex_transfer(self):
        ...
        line_1 = self.create_line_for_reconciliation(
            1000.0, 1000.0, self.company_currency, "2016-01-01", partner=partner_2
        )
        line_2 = self.create_line_for_reconciliation(
            -100.0,
            -300.0,
        ...
        tax_id = self.env["account.tax"].create(
            {
                ...
                "invoice_repartition_line_ids": [
                    ...
                    Command.create(
                        {
                            "factor_percent": 100,
                            "account_id": tax_recover_account_id.id,
    ...
    def test_reconcile_partials_allowed(self):
        ...
        wizard.reconcile()
        self.assertTrue(
            len(lines.matched_debit_ids) > 0 or len(lines.matched_credit_ids) > 0
        )
    ...
    def test_raise_lock_date_violation(self):
        ...
        line_2 = self.create_line_for_reconciliation(
            -500.0, -500.0, self.company_currency, "2016-06-01"
        )
        wizard = (
            self.env["account.reconcile.wizard"]
            .with_context(
    ...
    def test_raise_reconcile_too_many_accounts(self):
        ...
        line_3 = self.create_line_for_reconciliation(
            ...
            self.company_currency,
            "2016-01-01",
            account_1=self.payable_account_2,
        )
    ...
    def test_reconcile_no_receivable_no_payable_account(self):
        ...
        line_1 = self.create_line_for_reconciliation(
            1000.0, 1000.0, self.company_currency, "2016-01-01", account_1=account
        )
        line_2 = self.create_line_for_reconciliation(
            -500.0, -1500.0, self.foreign_currency, "2016-01-01", account_1=account
        )
        wizard_input_values = {
            "journal_id": self.misc_journal.id,
            "account_id": self.write_off_account.id,
    ...
    def test_reconcile_exchange_diff_foreign_currency(self):
        ...
        line_2 = self.create_line_for_reconciliation(
            ...
            self.foreign_currency,
            "2016-01-01",
            account_1=exchange_gain_account,
        )
        wizard_input_values = {
            "journal_id": self.misc_journal.id,
            "account_id": self.write_off_account.id,
        ...
        expected_transfer_values = [
            {
                "account_id": self.receivable_account.id,
                "name": f"Transfer from {exchange_gain_account.display_name}",
                "balance": -100.0,
                "amount_currency": 0.0,
                "currency_id": self.foreign_currency.id,
            },
    ...
    def test_write_off_on_same_account(self):
        ...
        line_2 = self.create_line_for_reconciliation(
            2000.0, 2000.0, self.company_currency, "2016-01-01"
        )
        wizard_input_values = {
            "journal_id": self.misc_journal.id,
            "account_id": self.receivable_account.id,
    ...
    def test_write_off_kpmg_case(self):
        new_date = fields.Date.from_string("2017-02-01")
        line_1 = self.create_line_for_reconciliation(
            1000.0, 1500.0, self.foreign_currency, "2016-01-01"
        )
        line_2 = self.create_line_for_reconciliation(
            -900.0, -5400.0, self.foreign_currency_2, "2016-01-01"
        )
        wizard_input_values = {
            "journal_id": self.misc_journal.id,
            "account_id": self.write_off_account.id,
    ...
    def test_write_off_multi_curr_multi_residuals_force_partials(self):
        debit_1 = self.create_line_for_reconciliation(
            2000.0, 12000.0, self.foreign_currency_2, "2016-01-01"
        )
        credit_1 = self.create_line_for_reconciliation(
            -1000.0, -6000.0, self.foreign_currency_2, "2016-01-01"
        )
        debit_2 = self.create_line_for_reconciliation(
            2000.0, 3000.0, self.foreign_currency, "2016-01-01"
        )
        wizard = (
            self.env["account.reconcile.wizard"]
            .with_context(
        ...
        self.assertRecordValues(
            wizard, [{"force_partials": True, "allow_partials": True}]
        )

    def test_write_off_multi_curr_multi_residuals_exch_diff_force_partials(self):
        debit_1 = self.create_line_for_reconciliation(
            2000.0, 0.0, self.foreign_currency_2, "2016-01-01"
        )
        credit_1 = self.create_line_for_reconciliation(
            -1000.0, 0.0, self.foreign_currency_2, "2016-01-01"
        )
        debit_2 = self.create_line_for_reconciliation(
            2000.0, 0.0, self.foreign_currency, "2016-01-01"
        )
        wizard = (
            self.env["account.reconcile.wizard"]
            .with_context(
    ...
    def test_reconcile_with_partner_change(self):
        ...
        partner_2 = self.env["res.partner"].create({"name": "Test Partner 2"})
        line_1 = self.create_line_for_reconciliation(
            -1000.0, -1000.0, self.company_currency, "2016-01-01", partner=partner_1
        )
        line_2 = self.create_line_for_reconciliation(
            2000.0, 2000.0, self.company_currency, "2016-01-01"
        )
    ...
    def test_reconcile_with_partner_change_and_transfer(self):
        ...
        line_2 = self.create_line_for_reconciliation(
            2000.0, 2000.0, self.company_currency, "2016-01-01", partner=partner_1
        )
        wizard_input_values = {
            "journal_id": self.misc_journal.id,
            "account_id": self.receivable_account.id,
    ...
    def test_reconcile_edit_mode_partial_foreign_curr(self):
        line_1 = self.create_line_for_reconciliation(
            100.0, 300.0, self.foreign_currency, "2016-01-01"
        )
        wizard_input_values = {
            "account_id": self.write_off_account.id,
            "label": "Write-Off Test Label",
    ...
    def test_reconcile_edit_mode_partial_company_curr(self):
        line_1 = self.create_line_for_reconciliation(
            300.0, 300.0, self.company_currency, "2016-01-01"
        )
        wizard_input_values = {
            "account_id": self.write_off_account.id,
            "label": "Write-Off Test Label",
    ...
    def test_reconcile_same_currency_same_side_not_recpay(self):
        current_assets_account = self.company_data["default_account_assets"].copy(
            {
                "name": "Current Assets",
        ...
        line_1 = self.create_line_for_reconciliation(
            200, 200, self.company_currency, "2016-01-01", current_assets_account
        )
        line_2 = self.create_line_for_reconciliation(
            200, 200, self.company_currency, "2016-01-01", current_assets_account
        )
        ...
        self.assertRecordValues(
            wizard,
            [{"is_write_off_required": True, "amount": 400, "amount_currency": 400}],
        )
    ...
    def test_reconcile_foreign_currency_same_side_not_recpay(self):
        ...
        line_1 = self.create_line_for_reconciliation(
            200, 300, self.foreign_currency, "2016-01-01", current_assets_account
        )
        line_2 = self.create_line_for_reconciliation(
            200, 200, self.company_currency, "2016-01-01", current_assets_account
        )
    ...
    def test_reconcile_transfer_with_different_partners(self):
        ...
        line_a1 = self.create_line_for_reconciliation(
            1000.0,
            1000.0,
            self.company_currency,
            "2016-01-01",
            account_1=self.receivable_account,
            partner=partner_a,
        )
        line_b1 = self.create_line_for_reconciliation(
            -1000.0,
            -1000.0,
            self.company_currency,
            "2016-01-01",
        ...
        lines.action_reconcile()
        transfer_lines = lines.full_reconcile_id.reconciled_line_ids.filtered(
            lambda line: line.id not in lines.ids
        )
        self.assertRecordValues(
            transfer_lines,
            [
        ...
        line_a2 = self.create_line_for_reconciliation(
            1000.0,
            1000.0,
            self.company_currency,
            "2016-01-01",
        ...
        line_b2 = self.create_line_for_reconciliation(
            -600.0,
            -600.0,
            self.company_currency,
            "2016-01-01",
        ...
        line_c2 = self.create_line_for_reconciliation(
            -400.0,
            -400.0,
            self.company_currency,
            "2016-01-01",
