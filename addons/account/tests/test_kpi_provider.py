...
class TestKpiProvider(TransactionCase):
    ...
    def test_kpi_summary(self):
        ...
        account_id = self.env["account.account"].search(
            [("company_ids", "=", company_id)], limit=1
        )
        base_move = {
            "company_id": company_id,
            "invoice_line_ids": [
                Command.create(
                    {"account_id": account_id.id, "quantity": 15, "price_unit": 10}
                )
            ],
        ...
        self.assertCountEqual(
            self.env["kpi.provider"].get_account_kpi_summary(),
            [
                {
                    "id": "account_journal_type.general",
                    "name": "Miscellaneous",
                    "type": "integer",
                    "value": 2,
                },
                {
                    "id": "account_journal_type.sale",
                    "name": "Sales",
                    "type": "integer",
                    "value": 3 + 4 + 7,
                },
                {
                    "id": "account_journal_type.purchase",
                    "name": "Purchase",
                    "type": "integer",
                    "value": 5 + 6 + 8,
                },
    ...
    def test_kpi_summary_shouldnt_report_posted_moves(self):
        ...
        account_id = (
            ...
            .search([("company_ids", "=", company_id)], limit=1)
            .id
        )
        move = self.env["account.move"].create(
            {
                ...
                "invoice_line_ids": [
                    Command.create(
                        {"account_id": account_id, "quantity": 15, "price_unit": 10}
                    )
                ],
                "partner_id": self.partner_id.id,
    ...
    def test_kpi_summary_reports_unreconciled_bank_statements(self):
        ...
        account_id = (
            self.env["account.account"]
            .search(
                [("company_ids", "=", company_id), ("account_type", "=", "income")],
                limit=1,
            )
            .id
        )
        ...
        _st_liquidity_lines, st_suspense_lines, _st_other_lines = (
            bank_statement.line_ids._seek_for_lines()
        )
