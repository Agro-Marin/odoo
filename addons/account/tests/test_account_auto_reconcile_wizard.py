...
class TestAccountAutoReconcileWizard(AccountTestInvoicingCommon):
    ...
    def _create_many_lines(self):
        self.line_1_group_1 = self.create_line_for_reconciliation(
            1000.0, 1000.0, self.comp_curr, "2016-01-01", partner=self.partner_a
        )
        self.line_2_group_1 = self.create_line_for_reconciliation(
            -1000.0, -1000.0, self.comp_curr, "2016-01-02", partner=self.partner_a
        )
        self.line_3_group_1 = self.create_line_for_reconciliation(
            1000.0, 1000.0, self.comp_curr, "2016-01-03", partner=self.partner_a
        )
        self.line_4_group_1 = self.create_line_for_reconciliation(
            -1000.0, -1000.0, self.comp_curr, "2016-01-04", partner=self.partner_a
        )
        self.line_5_group_1 = self.create_line_for_reconciliation(
            1000.0, 1000.0, self.comp_curr, "2016-01-05", partner=self.partner_a
        )
        self.group_1 = (
            self.line_1_group_1
            + self.line_2_group_1
        ...
        self.line_1_group_2 = self.create_line_for_reconciliation(
            500.0, 500.0, self.comp_curr, "2016-01-01", partner=self.partner_b
        )
        self.line_2_group_2 = self.create_line_for_reconciliation(
            -500.0, -500.0, self.comp_curr, "2016-01-01", partner=self.partner_b
        )
        self.line_3_group_2 = self.create_line_for_reconciliation(
            500.0, 500.0, self.comp_curr, "2017-01-02", partner=self.partner_b
        )
        self.line_4_group_2 = self.create_line_for_reconciliation(
            -500.0, -500.0, self.comp_curr, "2017-01-02", partner=self.partner_b
        )
        self.group_2 = (
            self.line_1_group_2
        ...
        self.line_1_group_3 = self.create_line_for_reconciliation(
            1500.0, 3000.0, self.foreign_curr, "2016-01-01", partner=self.partner_b
        )
        self.line_2_group_3 = self.create_line_for_reconciliation(
            -1000.0, -3000.0, self.foreign_curr, "2017-01-01", partner=self.partner_b
        )
        self.line_3_group_3 = self.create_line_for_reconciliation(
            3000.0, 3000.0, self.comp_curr, "2016-01-01", partner=self.partner_b
        )
        self.line_4_group_3 = self.create_line_for_reconciliation(
            -3000.0, -3000.0, self.comp_curr, "2016-01-01", partner=self.partner_b
        )
        self.group_3 = (
            self.line_1_group_3
        ...
        self.line_1_group_4 = self.create_line_for_reconciliation(
            1000.0,
            1000.0,
            self.comp_curr,
            "2016-01-01",
        ...
        self.line_2_group_4 = self.create_line_for_reconciliation(
            -1000.0,
            -1000.0,
            self.comp_curr,
            "2016-01-02",
    ...
    def test_auto_reconcile_one_to_one(self):
        ...
        self.assertEqual(
            self.line_1_group_1.full_reconcile_id,
            self.line_2_group_1.full_reconcile_id,
            "Entries should be reconciled together since they are in the same group and have closer dates.",
        )
        self.assertEqual(
            self.line_3_group_1.full_reconcile_id,
            self.line_4_group_1.full_reconcile_id,
            "Entries should be reconciled together since they are in the same group and have closer dates.",
        )
        self.assertEqual(
            self.line_1_group_2.full_reconcile_id,
            self.line_1_group_2.full_reconcile_id,
            "Entries should be reconciled together since they are in the same group and have closer dates.",
        )
        self.assertEqual(
            self.line_1_group_3.full_reconcile_id,
            self.line_2_group_3.full_reconcile_id,
            "Entries should be reconciled together since they are in the same group and have closer dates.",
        )
        self.assertEqual(
            self.line_3_group_3.full_reconcile_id,
            self.line_4_group_3.full_reconcile_id,
            "Entries should be reconciled together since they are in the same group and have closer dates.",
        )
        self.assertNotEqual(
            self.line_2_group_3.full_reconcile_id,
        ...
        self.assertFalse(
            self.line_5_group_1.reconciled,
        ...
        self.assertFalse(
            (self.line_3_group_2 + self.line_4_group_2).full_reconcile_id,
            "Entries shouldn't be reconciled since it's outside of accepted date range of the wizard.",
        )
        self.assertFalse(
            (self.line_1_group_4 + self.line_2_group_4).full_reconcile_id,
    ...
    def test_auto_reconcile_zero_balance(self):
        ...
        self.assertTrue(should_be_reconciled.full_reconcile_id)
        self.assertFalse(
            self.group_1.full_reconcile_id,
            "Entries shouldn't be reconciled since their total balance is not zero.",
        )
        self.assertEqual(
            (self.line_1_group_2 + self.line_2_group_2).mapped("matching_number"),
            [self.line_1_group_2.matching_number] * 2,
            "Entries should be reconciled together as their total balance is zero.",
        )
        self.assertEqual(
            (self.line_1_group_3 + self.line_2_group_3).mapped("matching_number"),
            [self.line_1_group_3.matching_number] * 2,
            "Entries should be reconciled together as their total balance is zero with the same currency.",
        )
        self.assertEqual(
            (self.line_3_group_3 + self.line_4_group_3).mapped("matching_number"),
            [self.line_3_group_3.matching_number] * 2,
            "Lines 3 and 4 are reconciled but not with two first lines since their currency is different.",
        )
        self.assertFalse(
            self.group_4.full_reconcile_id,
    ...
    def test_auto_reconcile_no_account_nor_partner_one_to_one(self):
        self.create_line_for_reconciliation(
            1000.0, 1000.0, self.comp_curr, "2016-01-01", partner=self.partner_a
        )
        self.create_line_for_reconciliation(
            -1000.0, -1000.0, self.comp_curr, "2016-01-02", partner=self.partner_a
    ...
    def test_preset_wizard(self):
        line_1 = self.create_line_for_reconciliation(
            1000.0, 1000.0, self.comp_curr, "2016-01-30", partner=self.partner_a
        )
        line_2 = self.create_line_for_reconciliation(
            -1000.0, -1000.0, self.comp_curr, "2016-01-31", partner=self.partner_a
        )
        wizard = (
            self.env["account.auto.reconcile.wizard"]
            .with_context(domain=[("id", "in", (line_1 + line_2).ids)])
            .create({})
        )
        ...
        line_4 = self.create_line_for_reconciliation(
            -500.0, -500.0, self.comp_curr, "2016-02-28", partner=None
        )
        wizard = (
            self.env["account.auto.reconcile.wizard"]
            .with_context(domain=[("id", "in", (line_3 + line_4).ids)])
            .create({})
        )
        self.assertRecordValues(
            wizard,
