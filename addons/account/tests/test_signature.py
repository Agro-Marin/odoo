...
class TestInvoiceSignature(AccountTestInvoicingCommon):
    ...
    def test_draft_invoice_shouldnt_have_signature(self):
        ...
        self.assertFalse(
            ...
            "the signature area shouldn't appear on a draft invoice",
        )
    ...
    def test_invoice_signing_user_should_be_the_user_that_posted_it(self):
        ...
        self.assertEqual(
            self.invoice.signature,
            False,
            "There shouldn't be any signature if there isn't a signing user",
        )
        self.invoice.action_post()
        self.assertEqual(
            self.invoice.signing_user,
        ...
        self.assertEqual(
            self.invoice.signature,
            self.signature_fake_2,
            "The signature should be from `self.another_user`",
        )
    ...
    def test_invoice_signing_user_should_be_reprensative_user_if_there_is_one(self):
        ...
        self.assertEqual(
            ...
            self.user,
            "The signing user should be the representative person set in the settings",
        )
        self.assertEqual(
            invoice.signature,
            self.signature_fake_1,
