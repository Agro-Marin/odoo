...
class TestBankRecWidgetCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.other_currency_2 = cls.setup_other_currency(
            "CAD", rounding=0.001, rates=[("2016-01-01", 6.0), ("2017-01-01", 4.0)]
        )
        cls.other_currency_3 = cls.setup_other_currency(
            "XAF", rounding=0.001, rates=[("2016-01-01", 12.0), ("2017-01-01", 8.0)]
        )
        ...
        invoice = cls.env["account.move"].create(
            {
                ...
                **kwargs,
                "invoice_line_ids": [
                    Command.create(x) for x in kwargs["invoice_line_ids"]
                ],
            }
        )
        invoice.action_post()
    ...
    def _create_st_line(
        ...
        amount,
        date="2019-01-01",
        payment_ref="turlututu",
        update_create_date=True,
        **kwargs,
    ):
        ...
        if update_create_date:
            cls.env.cr.execute(
                "UPDATE account_bank_statement_line SET create_date = %s WHERE id=%s",
