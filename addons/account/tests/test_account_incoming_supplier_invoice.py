...
class TestAccountInvoiceImportMixin:
    ...
    def assert_attachment_import(self, origin, attachments_vals, expected_invoices):
        ...
        for message in created_messages.filtered(lambda m: m.model == "account.move"):
            for attachment in message.attachment_ids:
                actual_invoices.setdefault(message.res_id, {}).setdefault(
                    attachment.name, {}
                )["on_message"] = True

        for attachment in created_attachments:
            if attachment.res_model == "account.move":
                actual_invoices.setdefault(attachment.res_id, {}).setdefault(
                    attachment.name, {}
                )["on_invoice"] = True
        ...
        for decoder_call in decoder_calls:
            ...
            actual_invoices.setdefault(invoice.id, {}).setdefault(filename, {})[
                "is_decoded"
            ] = True
    ...
    def _patch_import_methods(self):
        ...
        with (
            patch.object(
                ...
                "_get_import_file_type",
                patched_get_import_file_type,
            ),
            patch.object(
                self.env.registry["account.move"],
                "_get_edi_decoder",
                patched_get_edi_decoder,
            ),
        ):
            yield decoder_calls
    ...
    def _upload_and_import_attachments(self, origin, attachments_vals):
        with (
            ...
            match origin:
                case "chatter_message":
                    move.message_post(
                        message_type="comment", attachment_ids=attachments.ids
                    )
                case "chatter_upload":
        ...
        return (
            attachment_capturer.records,
            message_capturer.records,
            move_capturer.records,
        )
...
class TestAccountIncomingSupplierInvoice(
    AccountTestInvoicingCommon, TestAccountInvoiceImportMixin, MailCommon
):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.pdf3_vals = {
            "name": "invoice3.pdf",
            **cls._get_dummy_pdf_with_embedded_file_vals(),
        }
    ...
    def test_mail_gateway_bounce_uses_the_journal_company(self):
        ...
        other_domain = self.env["mail.alias.domain"].create(
            {
                "name": "bounce-co.example.com",
                "bounce_alias": "bounce.other",
                "catchall_alias": "catchall.other",
                "default_from": "notifications.other",
                "company_ids": [Command.link(other_company.id)],
            }
        )
        other_company.alias_domain_id = other_domain
        other_company.partner_id.email = "hello@bounce-co.example.com"

        message = message_from_string(
            "From: someone@example.com\n"
            "To: bills@bounce-co.example.com\n"
            "Subject: a bill with nothing attached\n"
            "Message-Id: <gateway-bounce-test>\n"
            "\n"
            "body\n"
        )
        message_dict = {
            "from": "someone@example.com",
            "message_id": "<gateway-bounce-test>",
            "attachments": [],
        }
        route = (
            "account.move",
        ...
        self.assertNotEqual(
            self.env.company, other_company, "the acting company must differ"
        )
        with RecordCapturer(self.env["mail.mail"].sudo(), []) as capture:
        ...
        self.assertIn(
            other_domain.name,
            bounce.email_from,
            f"bounce sent as {bounce.email_from!r}, not as the journal alias' company",
        )
    ...
    def test_supplier_invoice_forwarded_by_internal_with_supplier_in_body(self):
        message_parsed = {
            ...
            "to": "%s@%s"
            % (self.journal.alias_id.alias_name, self.journal.alias_id.alias_domain),
            "body": "Mail sent by %s <%s>:\nYou know, that thing that you bought."
            % (self.supplier_partner.name, self.supplier_partner.email),
            "attachments": [b"Hello, invoice"],
        }
    ...
    def test_supplier_invoice_forwarded_by_internal_with_internal_in_body(self):
        message_parsed = {
            ...
            % (self.journal.alias_id.alias_name, self.journal.alias_id.alias_domain),
            "body": "Mail sent by %s <%s>:\nYou know, that thing that you bought."
            % (self.internal_user.name, self.internal_user.email),
            "attachments": [b"Hello, invoice"],
        }
    ...
    def test_einvoice_notification(self):
        ...
        purchase_journal.incoming_einvoice_notification_email = (
