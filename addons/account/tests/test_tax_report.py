...
class TaxReportTest(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.tax_report_line_1_1 = cls._create_basic_tax_report_line(
            cls.tax_report_1, "Line 01", "01"
        )
        cls.tax_report_line_1_2 = cls._create_basic_tax_report_line(
            cls.tax_report_1, "Line 02", "02"
        )
        cls.tax_report_line_1_3 = cls._create_basic_tax_report_line(
            cls.tax_report_1, "Line 03", "03"
        )
        cls.tax_report_line_1_4 = cls._create_basic_tax_report_line(
            cls.tax_report_1, "Line 04", "04"
        )
        cls.tax_report_line_1_5 = cls._create_basic_tax_report_line(
            cls.tax_report_1, "Line 05", "05"
        )
        cls.tax_report_line_1_55 = cls._create_basic_tax_report_line(
            cls.tax_report_1, "Line 55", "55"
        )
        cls.tax_report_line_1_6 = cls._create_basic_tax_report_line(
        ...
        cls.tax_report_line_2_1 = cls._create_basic_tax_report_line(
            cls.tax_report_2, "Line 01, but in report 2", "01"
        )
        cls.tax_report_line_2_2 = cls._create_basic_tax_report_line(
            cls.tax_report_2, "Line 02, but in report 2", "02"
        )
        cls.tax_report_line_2_42 = cls._create_basic_tax_report_line(
            cls.tax_report_2, "Line 42", "42"
        )
        cls.tax_report_line_2_6 = cls._create_basic_tax_report_line(
            cls.tax_report_2, "Line 100, but in report 2", "100"
        )
    ...
    def test_create_shared_tags(self):
        self.assertEqual(
            len(self._get_tax_tags(self.test_country_1, tag_name="01")),
            1,
            "tax_tags expressions created for reports within the same countries using the same formula should create a single tag.",
        )
    ...
    def test_write_single_line_tagname_not_shared(self):
        ...
        self.assertEqual(
            len(self._get_tax_tags(self.test_country_1, tag_name=original_tag_name)),
            0,
            "The original formula of the expression should not correspond to any tag anymore.",
        )
        self.assertEqual(
            original_tags,
            new_tags,
            "The expression should still be linked to the same tags.",
        )
        self.assertEqual(
            len(self._get_tax_tags(self.test_country_1)),
            len(start_tags),
            "No new tag should have been created.",
        )
    ...
    def test_write_single_line_tagname_shared(self):
        ...
        self.assertEqual(
            self._get_tax_tags(self.test_country_1, tag_name=original_tag_name),
            original_tags,
        ...
        self.assertEqual(
            len(self._get_tax_tags(self.test_country_1)),
            len(start_tags) + 1,
            "A tag should have been created",
        )
        self.assertNotEqual(
            original_tags,
            new_tags,
            "New tags should have been assigned to the expression",
        )
    ...
    def test_write_multi_no_change(self):
        ...
        self.assertEqual(
            tags_before,
            tags_after,
            "Re-assigning the same formula to a tax_tags expression should keep the same tags.",
        )
    ...
    def test_edit_multi_line_tagname_all_different_new(self):
        ...
        self.assertEqual(
            len(tags_before) + 1, len(tags_after), "Only tag should have been created."
        )
        ...
        self.assertTrue(
            line_1_1_tags == line_2_2_tags == line_2_42_tags,
            "The impacted expressions should now all share the same tags.",
        )
    ...
    def test_tax_report_change_country(self):
        ...
        self.assertEqual(
            country_1_tags_before_copy,
            country_1_tags_after_copy,
        ...
        self.assertEqual(
            country_1_tags_after_change,
            country_1_tags_after_copy,
            "Modifying the country should not have changed the tags in the original country.",
        )
        self.assertEqual(
            len(country_2_tags_after_change),
            len(country_2_tags_before_change) + len(copied_report_1.line_ids),
        ...
        for original, copy in zip(
            ...
            self.assertNotEqual(
                original_tags,
                copy_tags,
                "Tags matched by original and copied expressions should now be different.",
            )
            self.assertEqual(
                set(original_tags.mapped("name")),
                set(copy_tags.mapped("name")),
                "Tags matched by original and copied expression should have the same names.",
            )
            self.assertNotEqual(
                original_tags.country_id,
                copy_tags.country_id,
        ...
        for line in self.tax_report_2.line_ids:
            ...
            if line == self.tax_report_line_2_42:
                ...
                for tag in line_tags:
                    self.assertIn(
                        tag,
                        country_2_tags_after_change,
                        "The tax_tags expressions sharing their tags with other report should not receive new tags since they already exist.",
                    )
    ...
    def test_unlink_report_line_tags_used_by_amls(self):
                ...
                "invoice_line_ids": [
                    (
                        ...
                        0,
                        {
                            "quantity": 1,
                            "price_unit": 42,
                            "tax_ids": [Command.set([test_tax.id])],
                        },
                    ),
                ],
            }
        ...
        self.assertEqual(
            ...
            [False],
            "Unlinking a tax_tags expression should archive the tag if it was used on move lines, and unlink it otherwise.",
        )
        self.assertEqual(
            len(test_tax.invoice_repartition_line_ids.tag_ids),
            0,
            "After a tag is archived it shouldn't be on tax repartition lines.",
        )
    ...
    def test_unlink_report_line_tags_used_by_other_expression(self):
        ...
        self.assertEqual(
            len(tags_after),
            len(tags_before),
            "Unlinking a report expression whose tags are used by another expression should not delete them.",
        )
        self.assertEqual(
            len(tags_archived_after),
            len(tags_archived_before),
            "Unlinking a report expression whose tags are used by another expression should not archive them.",
        )
    ...
    def test_change_engine_without_formula(self):
        ...
        self.assertEqual(
            len(tags_after), 1, "Changing the engine should have created a tag"
        )
    ...
    def test_change_engine_shared_tags(self):
        ...
        self.assertEqual(
            len(tags_before),
            1,
            "The tag should already exist because of another expression",
        )
    ...
    def test_change_formula_multiple_fields(self):
        ...
        self.assertEqual(
            len(tags_after), 1, "Changing the formula should have renamed the tag"
        )
        self.assertEqual(tags_after.mapped("name"), ["Buny"])
        self.assertEqual(
            tags_after,
            tags_to_rename,
            "Changing the formula should have renamed the tags",
        )
