...
class TestAccountMergeWizard(TestAccountMergeCommon):
            ...
            [
                {
                    ...
                    "values": {
                        ...
                        "tax_ids": [
                            Command.link(cls.company_data["default_tax_sale"].id)
                        ],
                        "tag_ids": [
                            Command.link(
                                cls.env.ref("account.account_tag_operating").id
                            )
                        ],
                    },
                },
                ...
                },
                {
                    ...
                    "values": {
                        ...
                        "tax_ids": [
                            Command.link(cls.company_data_2["default_tax_sale"].id)
                        ],
                        "tag_ids": [
                            Command.link(
                                cls.env.ref("account.account_tag_investing").id
                            )
                        ],
                    },
                },
    ...
    def test_merge(self):
        ...
        self.assertEqual(
            self.env["account.chart.template"].ref("test_account_2"), self.accounts[1]
        )
        self.assertEqual(
        ...
        self.assertRecordValues(
            self.accounts[0].with_context({"lang": "nl_NL"}),
            [{"name": "Mijn derde conto"}],
        )
    ...
    def test_merge_accounts_company_dependent_related(self):
        ...
        for company in self.env.companies:
            partner_payable_account = self.partner_a.with_company(
                company
            ).property_account_payable_id.exists()
