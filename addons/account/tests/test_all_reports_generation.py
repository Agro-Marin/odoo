...
class TestAllReportsGeneration(AccountTestInvoicingCommon):
    ...
    def test_generate_all_export_files(self):
        ...
        l10n_bd_corporate_tax_liability = self.env["account.account"].search(
            [("account_type", "=", "liability_current")], limit=1
        )
        l10n_ae_liability_account = self.env["account.account"].search(
            [("account_type", "=", "liability_current")], limit=1
        )
        l10n_cz_reports_tax_office = self.env.ref(
            "l10n_cz.tax_office_1", raise_if_not_found=False
        )
        l10n_ar_activity_default = self.env.ref(
            "l10n_ar_reports_simple.activity_620100", raise_if_not_found=False
        )
        company_test_values = {
            "LU": {"vat": "LU12345613"},
            ...
            "AR": {
                "vat": "30714295698",
                "l10n_ar_arca_activity_id": l10n_ar_activity_default
                and l10n_ar_activity_default.id,
            },
            "AU": {
                "vat": "11225459588",
                "street": "Arrow Street",
            ...
            "DE": {
                ...
                "state_id": self.env.ref("base.state_de_th").id,
            },
            "NO": {"vat": "NO123456785", "l10n_no_bronnoysund_number": "987654325"},
            "PL": {
                "l10n_pl_reports_tax_office_id": l10n_pl_reports_tax_office
                and l10n_pl_reports_tax_office.id
            },
            "BD": {
                "l10n_bd_corporate_tax_liability": l10n_bd_corporate_tax_liability,
                "l10n_bd_corporate_tax_expense": l10n_bd_corporate_tax_liability,
            },
            "AE": {
                "l10n_ae_tax_report_liabilities_account": l10n_ae_liability_account,
                "l10n_ae_tax_report_expenses_account": l10n_ae_liability_account,
                "l10n_ae_tax_report_asset_account": l10n_ae_liability_account,
            },
            "CZ": {
                "l10n_cz_tax_office_id": l10n_cz_reports_tax_office
                and l10n_cz_reports_tax_office.id
            },
            "SK": {
                "zip": "010 01",
                "city": "Žilina",
                "income_tax_id": "9999999999",
                "company_registry": "47686065",
                "l10n_sk_nace_code": "01110",
            },
        }
        ...
        partner_test_values = {
            "AR": {
                "l10n_latam_identification_type_id": (
                    self.env.ref("l10n_ar.it_cuit", raise_if_not_found=False)
                    or {"id": None}
                )["id"]
            },
        }
        ...
        for report in self.reports:
            with self.subTest(report=report.name, country=report.country_id.name):
                ...
                self.env.company.write(
                    {
                        ...
                        "phone_ids": [
                            Command.create(
                                {"number": "01234567890", "type": "landline"}
                            )
                        ],
                        "company_registry": "42",
                ...
                self.env.company.partner_id.write(
                    partner_test_values.get(report.country_id.code, {})
                )

                options = report.get_options(
                    {"selected_variant_id": report.id, "_running_export_test": True}
                )
            ...
            self.env.company.write(
                dict.fromkeys(
                    company_test_values.get(report.country_id.code, {}).keys()
                )
            )

            self.env.company.partner_id.write(
                dict.fromkeys(
                    partner_test_values.get(report.country_id.code, {}).keys()
                )
            )

    def test_custom_engines_related_groupby(self):
        blacklist_xmlids = [
            "account.account_financial_report_executivesummary_avdebt0_ndays",
            "account.account_financial_report_executivesummary_avgcre0_ndays",
            "account.last_statement_balance_amount",
        ...
        for expression in custom_engine_expressions:
            expressions_per_engine[
                (expression.report_line_id.report_id, expression.formula)
            ] += expression
            ...
            with self.subTest(report=report.name):
                ...
                (
                    expressions.report_line_id.expression_ids.filtered(
                        lambda x: x.engine in ("aggregation", "external")
                    )
                ).unlink()

                expressions.report_line_id.user_groupby = (
                    "account_code"  # account_code is a  non-stored related field on aml
                )
                self.env.flush_all()
                report._compute_expression_totals_for_each_column_group(
                    expressions, options, groupby_to_expand="account_code"
                )
                # This computation will fail if the custom engine forgot to handle such groupby, typically via a call to _field_to_sql
