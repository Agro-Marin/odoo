...
class TestReportEngines(TestAccountReportsCommon):
    ...
    def _prepare_test_account_move_line(
        self, balance, account_code=None, tax_tags=None, date="2020-01-01", **kwargs
    ):
        if tax_tags:
    ...
    def _create_test_account_moves(self, test_account_move_line_values_list):
        ...
        for test_account_move_line_values in test_account_move_line_values_list:
            if test_account_move_line_values.get("account_code"):
                accounts_to_create_by_code.add(
                    test_account_move_line_values["account_code"]
                )
        ...
        if accounts_to_create_by_code:
            ...
            for test_account_move_line_values in test_account_move_line_values_list:
                account = account_by_code.get(
                    test_account_move_line_values.get("account_code")
                )
                if account:
                    test_account_move_line_values["account_move_line_values"][
                        "account_id"
                    ] = account.id
        ...
        to_create = {}
        for test_account_move_line_values in test_account_move_line_values_list:
            account_move_key = frozendict(
                test_account_move_line_values["account_move_values"]
            )
            account_move_line_values = test_account_move_line_values[
                "account_move_line_values"
            ]
            account_move_to_create = to_create.setdefault(
                account_move_key,
                {
            ...
            account_move_to_create["account_move_values"]["line_ids"].append(
                Command.create(account_move_line_values)
            )
            account_move_to_create["balance"] += (
                account_move_line_values["debit"] - account_move_line_values["credit"]
            )
        ...
        for account_move_dict, account_move_to_create in to_create.items():
            ...
            if not self.env.company.currency_id.is_zero(open_balance):
                account_move_values["line_ids"].append(
                    Command.create(
                        {
                            "name": "open balance",
                            "account_id": self.garbage_account.id,
    ...
    def _prepare_test_expression_tax_tags(self, formula, **kwargs):
        return self._prepare_test_expression(
            engine="tax_tags", formula=formula, **kwargs
        )

    def _prepare_test_expression_domain(self, formula, subformula, **kwargs):
        return self._prepare_test_expression(
            engine="domain", formula=formula, subformula=subformula, **kwargs
        )

    def _prepare_test_expression_account_codes(self, formula, **kwargs):
        return self._prepare_test_expression(
            engine="account_codes", formula=formula, **kwargs
        )

    def _prepare_test_expression_external(
        self, formula, external_value_generators, **kwargs
    ):
        return {
            **self._prepare_test_expression(
                engine="external", formula=formula, **kwargs
            ),
    ...
    def _prepare_test_expression_aggregation(
        self, formula, subformula=None, column="balance", date_scope=None
    ):
        expression_values = {
            "label": column,
            "engine": "aggregation",
    ...
    def _create_report(self, test_report_line_values_list, columns=None, **kwargs):
        ...
        report = self.env["report.formula"].create(
            {
                ...
                **kwargs,
                ...
                "line_ids": [
                    ...
                    for i, test_report_line_values in enumerate(
                        test_report_line_values_list, start=1
                    )
                ],
            }
        )
        ...
        for report_line, test_report_line_values in zip(
            report.line_ids, test_report_line_values_list, strict=False
        ):
            for expression, expression_values in zip(
    ...
    def test_engine_tax_tags(self):
        self.env.company.tax_config_id.account_fiscal_country_id = self.fake_country
        ...
        move = self._create_test_account_moves(
            [
                self._prepare_test_account_move_line(
                    2000.0, account_code="101001", tax_tags=["11", "222T"]
                ),
                self._prepare_test_account_move_line(
                    1000.0, account_code="101001", tax_tags=["11", "222T"]
                ),
                self._prepare_test_account_move_line(
                    -3600.0, account_code="101001", tax_tags=["222T"]
                ),
                self._prepare_test_account_move_line(
                    600.0, account_code="101001", tax_tags=["222T", "3333"]
                ),
                self._prepare_test_account_move_line(
                    900.0, account_code="101002", tax_tags=["11"]
                ),
                self._prepare_test_account_move_line(
                    1500.0, account_code="101002", tax_tags=["11"]
                ),
            ]
        )
        ...
        for report_line, expected_amls in zip(
            report.line_ids, expected_redirection_values_list, strict=False
        ):
            report_line_dict = next(
                x for x in report_lines if x["name"] == report_line.name
            )
            with self.subTest(report_line=report_line.name):
                action_dict = report.action_audit_cell(
                    options,
                    self._get_audit_params_from_report_line(
                        options, report_line, report_line_dict
                    ),
                )
                self.assertEqual(
    ...
    def test_engine_domain_batching_matches_orm_semantics(self):
        ...
        the domain against the comodel. That shortcut must agree with what the same domain
        means on account.move.line -- including the lines where the many2one is NULL, which
        a search on the comodel can never return.
        """
        partner = self.env["res.partner"].create({"name": "Partner batched"})
        counterpart = self.env["res.partner"].create({"name": "Partner counterpart"})
        self._create_test_account_moves(
            [
                ...
                self._prepare_test_account_move_line(
                    250.0, account_code="101001", partner_id=False, date="2020-01-01"
                ),
                self._prepare_test_account_move_line(
                    -350.0,
                    account_code="101002",
        ...
        cases = [
            (
                [("partner_id", "=", partner.id)],
                "positive id match (already batched today)",
            ),
            ([("partner_id", "=", False)], "NULL many2one"),
            ([("partner_id", "!=", partner.id)], "negated id, must include NULL rows"),
            (
                [("partner_id", "not in", [partner.id])],
                "not in, must include NULL rows",
            ),
            (
                [("partner_id", "ilike", "Partner batched")],
                "bare m2o resolves against display_name",
            ),
        ]

        for domain, description in cases:
            with self.subTest(domain=domain, case=description):
                report = self._create_report(
                    [
                        self._prepare_test_report_line(
                            self._prepare_test_expression_domain(domain, "sum"),
                ...
                rendered = report._get_lines(options)[0]["columns"][0]["no_format"]
                ...
                self.assertAlmostEqual(rendered, expected, places=2)

    def test_engine_domain_count_rows_counts_the_null_group(self):
        """count_rows is documented as the number of distinct grouping keys. A NULL grouping
        key is a real group -- expanding the line renders it as "Unknown" -- but SQL COUNT
        skips NULLs, so it must be counted explicitly.
        """
        partner = self.env["res.partner"].create({"name": "Partner counted"})
        self._create_test_account_moves(
            [
                self._prepare_test_account_move_line(
                    100.0,
        ...
        cases = [
            ("[('account_id.code', '=', '101002')]", 1, "one non-null group"),
            (
                "[('account_id.code', '=', '101001')]",
                2,
                "one non-null group and the null group",
            ),
            (
                "[('account_id.code', '=', '101001'), ('partner_id', '=', False)]",
                1,
                "only the null group",
            ),
        ]

        for formula, expected_groups, description in cases:
            with self.subTest(case=description):
                report = self._create_report(
                    [
                        self._prepare_test_report_line(
                            self._prepare_test_expression_domain(formula, "count_rows"),
                            groupby="partner_id",
                        )
                    ]
                )
                options = self._generate_options(report, "2020-01-01", "2020-01-31")
                line = report._get_lines(options)[0]

                self.assertEqual(line["columns"][0]["no_format"], expected_groups)

                expanded = report.get_expanded_lines(
                    options,
                    line["id"],
                    line["groupby"],
                ...
                self.assertEqual(
                    len(expanded),
                    expected_groups,
                    "count_rows must agree with what expanding the line actually shows",
                )
    ...
    def test_engine_account_codes(self):
        ...
        account_tags = self.env["account.account.tag"]._load_records(
            [
                {
                    "xml_id": "account.account_codes_engine_test_tag1",
                    "noupdate": True,
                    "values": {
                ...
                {
                    "xml_id": "account.account_codes_engine_test_tag2",
                    "noupdate": True,
                    "values": {
        ...
        test_line_13 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes(
                rf"tag(account.account_codes_engine_test_tag1) + tag({account_tags[1].id})"
            ),
            groupby="account_id",
        )
        test_line_14 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes(
                r"tag(account.account_codes_engine_test_tag1)D"
            ),
            groupby="account_id",
        )
        test_line_15 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes(
                r"tag(account.account_codes_engine_test_tag1)C"
            ),
            groupby="account_id",
        )
        test_line_16 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes(
                rf"tag(account.account_codes_engine_test_tag1)\(101)D + 101003 + tag({account_tags[1].id})\(101)C"
            ),
            groupby="account_id",
        )
        ...
        self.env["account.account"].search(
            [("code", "in", ("100001", "101001"))]
        ).tag_ids = account_tags[0]
        self.env["account.account"].search(
            [("code", "in", ("10.20.0", "101002"))]
        ).tag_ids = account_tags[1]
    ...
    def test_engine_external(self):
        ...
        # Check redirection.
        for report_line, report_line_dict in zip(
            report.line_ids, report_lines, strict=False
        ):
            with self.subTest(report_line=report_line.name):
                action_dict = report.action_audit_cell(
                    options,
        ...
        for report_line, report_line_dict, expected_values in zip(
            report.line_ids,
            report_lines,
            expected_redirection_values_list,
            strict=False,
    ...
    def test_engine_external_editable_percentage(self):
        ...
        test_rounding_4 = self._prepare_test_report_line(
            self._prepare_test_expression_external(
                ...
                subformula="editable;rounding=4",
            ),
        ...
        report = self._create_report(
            [test_rounding_4, test_rounding_2, test_percentage_aggregate, test_float]
        )
        # Check the values at multiple dates.
        options = self._generate_options(report, "2020-01-01", "2020-01-01")
        self.assertLinesValues(
    ...
    def test_engine_custom(self):
        # Create the report.
        test_line_1 = self._prepare_test_report_line(
            self._prepare_test_expression_custom(
                "_custom_engine_test", subformula="sum"
            ),
            groupby="account_id",
        )
        ...
        def _custom_engine_test(
            ...
            return next(iter(domain_result.values()))
        ...
        with patch.object(
            type(report), "_get_custom_report_function", get_custom_report_function
        ):
            options = self._generate_options(report, "2020-01-01", "2020-01-01")
            self.assertLinesValues(
                # pylint: disable=bad-whitespace
    ...
    def test_engine_aggregation(self):
        ...
        test1 = self._prepare_test_report_line(
            ...
            self._prepare_test_expression_domain(
                [("account_id.code", "=", "101002")], "sum", label="domain"
            ),
            self._prepare_test_expression_external(
                "sum",
                [self._prepare_test_external_values(100.0, "2020-01-01")],
                label="external",
            ),
            self._prepare_test_expression_aggregation(
                "test1.tax_tags + test1.domain", column="aggregation"
            ),
            self._prepare_test_expression_aggregation(
                "test1.tax_tags / 0", subformula="ignore_zero_division"
            ),
            self._prepare_test_expression_external(
                "sum",
                [self._prepare_test_external_values(100.47, "2020-01-01")],
                label="external_decimal",
            ),
            name="test1",
            code="test1",
        )
        ...
        test2_1 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.tax_tags", subformula="if_above(USD(0))"
            ),
            name="test2_1",
            code="test2_1",
        )
        test2_2 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.tax_tags", subformula="if_above(USD(1999.9999999))"
            ),
            name="test2_2",
            code="test2_2",
        )
        test2_3 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.tax_tags", subformula="if_above(USD(2500.0))"
            ),
            name="test2_3",
            code="test2_3",
        )
        test2_4 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.tax_tags", subformula="if_above(CAD(3600.0))"
            ),
            name="test2_4",
            code="test2_4",
        )
        test3_1 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.domain", subformula="if_below(USD(0))"
            ),
            name="test3_1",
            code="test3_1",
        )
        test3_2 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.domain", subformula="if_below(USD(-300.00001))"
            ),
            name="test3_2",
            code="test3_2",
        )
        test3_3 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.domain", subformula="if_below(USD(- 350))"
            ),
            name="test3_3",
            code="test3_3",
        )
        test4_1 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.tax_tags + test1.domain",
                subformula="if_between(USD(0), USD(2000))",
            ),
            name="test4_1",
            code="test4_1",
        )
        test4_2 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.tax_tags + test1.domain",
                subformula="if_between(CAD(0), CAD(3000))",
            ),
            name="test4_2",
            code="test4_2",
        )
        ...
        test5 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes(
                "101003", label="account_codes"
            ),
            self._prepare_test_expression_aggregation(
                "test1.tax_tags + 9999.account_codes"
            ),
            name="9999",
            code="9999",
        )
        ...
        test10_1 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external_decimal", subformula="round(0)"
            ),
            name="test10_1",
            code="test10_1",
        )
        test10_2 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external_decimal", subformula="round(1)"
            ),
            name="test10_2",
            code="test10_2",
        )
        test10_3 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external_decimal", subformula="round(3)"
            ),
            name="test10_3",
            code="test10_3",
        )
        ...
        test11_1 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external",
                subformula="if_other_expr_above(test1.tax_tags, USD(3000.0))",
            ),
            name="test11_1",
            code="test11_1",
        )
        test11_2 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external",
                subformula="if_other_expr_below(test1.tax_tags, USD(3000.0))",
            ),
            name="test11_2",
            code="test11_2",
        )
        test11_3 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external",
                subformula="if_other_expr_above(test1.tax_tags, USD(1000.0))",
            ),
            name="test11_3",
            code="test11_3",
        )
        test11_4 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external",
                subformula="if_other_expr_below(test1.tax_tags, USD(1000.0))",
            ),
            name="test11_4",
            code="test11_4",
        )
        ...
        test11_5 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external",
                subformula="if_other_expr_above(test1.aggregation, USD(1000.0))",
            ),
            name="test11_5",
            code="test11_5",
        )
        test11_6 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test1.external",
                subformula="if_other_expr_below(test1.aggregation, USD(1000.0))",
            ),
            name="test11_6",
            code="test11_6",
        )
        ...
        moves = self._create_test_account_moves(
            [
                self._prepare_test_account_move_line(
                    100000.0, account_code="101002", date="2019-01-01"
                ),
                self._prepare_test_account_move_line(
                    2000.0, account_code="101001", tax_tags=["11"]
                ),
                self._prepare_test_account_move_line(-300.0, account_code="101002"),
        ...
        for report_line_name, expected_amls in expected_amls_to_test:
            ...
            with self.subTest(report_line=report_line.name):
                ...
                self.assertEqual(
                    moves.line_ids.filtered_domain(action_dict["domain"]), expected_amls
                )
    ...
    def test_engine_aggregation_cross_report(self):
        moves = self._create_test_account_moves(
            [
                self._prepare_test_account_move_line(
                    2.0, account_code="100000", date="2021-01-01"
                ),
                self._prepare_test_account_move_line(
                    4.0, account_code="200000", date="2021-01-01"
                ),
            ]
        )

        # First sub report
        report0_line_1 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes("1"),
            name="report0_line_1",
            code="report0_line_1",
        )
        report0_line_2 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes("2"),
            name="report0_line_2",
            code="report0_line_2",
        )
        report0_line_3 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "report0_line_1.balance + report0_line_2.balance"
            ),
            name="report_line_3",
            code="report_line_3",
        )
        report0 = self._create_report([report0_line_1, report0_line_2, report0_line_3])
        report0_options = self._generate_options(report0, "2021-01-01", "2021-01-01")

        # Second sub report
        report1_line_1 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes("1"),
            name="report1_line_1",
            code="report1_line_1",
        )
        report1_line_2 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes("2"),
            name="report1_line_2",
            code="report1_line_2",
        )
        report1_line_3 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "report1_line_1.balance - report1_line_2.balance"
            ),
            name="report_line_3",
            code="report_line_3",
        )

        report1 = self._create_report([report1_line_1, report1_line_2, report1_line_3])
        report1_options = self._generate_options(report1, "2021-01-01", "2021-01-01")

        # report_line_3 is made ambiguous on purpose to check that we can target the right report using cross_report aggregation
        # First target the first other report using the xmlID
        self.env["ir.model.data"]._update_xmlids(
            [
                {
                    "xml_id": "account.test_report_cross_report_0",
                    "record": report0,
                }
            ]
        )
        main_report_line_1 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "report_line_3.balance",
                subformula="cross_report(account.test_report_cross_report_0)",
                date_scope="strict_range",
            ),
            name="main_report_line",
            code="main_report_line",
        )
        main_report = self._create_report([main_report_line_1])
        main_report_options = self._generate_options(
            main_report, "2021-01-01", "2021-01-01"
        )

        self.assertLinesValues(
            report0._get_lines(report0_options),
            [0, 1],
            [
                ("report0_line_1", 2.0),
                ("report0_line_2", 4.0),
        ...
        self.assertLinesValues(
            ...
            [0, 1],
            [
                ("main_report_line", 6.0),
            ],
            main_report_options,
        )

        # Target the second other report using the model id
        main_report_line_1 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "report_line_3.balance",
                subformula=f"cross_report({report1.id})",
                date_scope="strict_range",
            ),
            name="main_report_line",
            code="main_report_line",
        ...
        self.assertLinesValues(
            report1._get_lines(report1_options),
            [0, 1],
            [
                ("report1_line_1", 2.0),
                ("report1_line_2", 4.0),
        ...
        for report_line_name, expected_amls in expected_amls_to_test:
            ...
            with self.subTest(report_line=report_line.name):
                action_dict = main_report.action_audit_cell(
                    main_report_options,
                    self._get_audit_params_from_report_line(
                        main_report_options, report_line, report_line_dict
                    ),
                )
                self.assertEqual(
                    moves.line_ids.filtered_domain(action_dict["domain"]), expected_amls
    ...
    def test_engine_aggregation_expansion(self):
        report = self._create_report(
            [
                ...
                self._prepare_test_report_line(
                    self._prepare_test_expression_aggregation(
                        "TAG_1.balance + TAG_2.balance"
                    ),
                    code="SIMPLE_AGG",
                ),
                self._prepare_test_report_line(
                    self._prepare_test_expression_aggregation(
                        "SIMPLE_AGG.balance + TAG_3.balance"
                    ),
                    code="COMPLEX_AGG",
                ),
                self._prepare_test_report_line(
                    self._prepare_test_expression_aggregation(
                        "TAG_1.balance + TAG_2.balance",
                        subformula="if_other_expr_above(TAG_3.balance, EUR(13))",
                    ),
                    code="BOUNDED_AGG",
                ),
                self._prepare_test_report_line(
        ...
        other_report = self._create_report(
            [
                self._prepare_test_report_line(
                    self._prepare_test_expression_aggregation(
                        "SIMPLE_AGG.balance + BOUNDED_AGG.balance",
                        subformula=f"cross_report({report.id})",
                    ),
        ...
        expr_map = {
            expression.report_line_id.code: expression
            for expression in (report + other_report).line_ids.expression_ids
        }
    ...
    def test_load_more(self):
        ...
        self._create_test_account_moves(
            [
                self._prepare_test_account_move_line(
                    1000.0, partner_id=partner_a.id, date="2020-01-01"
                ),
                self._prepare_test_account_move_line(
                    2000.0, partner_id=partner_b.id, date="2020-01-01"
                ),
                self._prepare_test_account_move_line(
                    3000.0, partner_id=partner_c.id, date="2020-01-01"
                ),
            ]
        )

        report = self._create_report(
            test_report_line_values_list=[
                self._prepare_test_report_line(
                    self._prepare_test_expression_domain(
                        [("partner_id", "!=", False)], "sum"
                    ),
                    groupby="partner_id",
                )
            ],
    ...
    def test_engine_external_string(self):
        ...
        test_line = self._prepare_test_report_line(
            self._prepare_test_expression_external(
                "most_recent",
                [
                    self._prepare_test_external_values(
                        "TARDIS", "2020-01-02", figure_type="string"
                    ),
                    self._prepare_test_external_values(
                        "Kris Kelvin", "2020-01-03", figure_type="string"
                    ),
                    self._prepare_test_external_values(
                        "Trisolaris", "2020-01-03", figure_type="string"
                    ),
                    self._prepare_test_external_values(
                        "5-ounce bird carrying a 1-pound coconut",
                        "2020-01-05",
                        figure_type="string",
                    ),
                ],
    ...
    def test_engine_external_default_value_tax_closing_fiscalyear_lock_date(self):
        def lock_via_fiscalyear_lock_date(
            non_tax_report, tax_report, report_options_map
        ):
        ...
        self._run_external_engine_default_test_case(
            False, True, lock_via_fiscalyear_lock_date
        )
    ...
    def _run_external_engine_default_test_case(
        self, impact_tax_report, impact_non_tax_report, lock_operation_function
    ):
        """Common helper to run the tests of _default expressions.

        :param impact_tax_report: whether the lock operation is expected to feed the _default expression of the tax report
        :param impact_non_tax_report: same as impact_tax_report, for the non-tax report
        :param lock_operation_function: callable(non_tax_report, tax_report, report_options_map) performing the lock
        """
        test_line_1 = self._prepare_test_report_line(
            self._prepare_test_expression_account_codes("10"),
            groupby="account_id",
        ...
        non_tax_report = self._create_report(
            [test_line_1, test_line_2], name="non_tax_report"
        )
        ...
        for report, impacted in [
            (non_tax_report, impact_non_tax_report),
            (tax_report, impact_tax_report),
        ]:
            with self.subTest(report=report.name):
                options = report_options_map[report]
                if impacted:
    ...
    def test_engine_aggregation_cross_bound(self):
        other_report = self._create_report(
            [
                self._prepare_test_report_line(
                    self._prepare_test_expression_aggregation(
                        "14.0", subformula="if_other_expr_above(line_2_1.dudu, EUR(0))"
                    ),
        ...
        report_1 = self._create_report(
            [
                self._prepare_test_report_line(
                    self._prepare_test_expression_aggregation(
                        "line_2_1.balance",
                        subformula=f"cross_report({other_report.id})",
                    ),
                    name="Line 1-1",
                    code="line_1_1",
                ),
    ...
    def test_integer_rounding(self):
        ...
        line_5 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test_1.balance + test_2.balance + test_3.balance + test_4.balance"
            ),
            code="test_5",
        )
        ...
        self._create_test_account_moves(
            [
                self._prepare_test_account_move_line(
                    5.4, account_code="101001", tax_tags=["42"], date="2023-01-01"
                ),
            ]
        )
        ...
        print_mode_options = self._generate_options(
            ...
            "2023-01-01",
            "2023-01-01",
            default_options={"integer_rounding_enabled": False, "export_mode": "file"},
        )
        self.assertLinesValues(
            # pylint: disable=bad-whitespace
        ...
        no_rounding_options = self._generate_options(
            report,
            "2023-01-01",
            "2023-01-01",
            default_options={"integer_rounding_enabled": False},
        )
        self.assertLinesValues(
            # pylint: disable=bad-whitespace
    ...
    def test_print_hide_0_lines(self):
        ...
        report = self._create_report(
            [test_line_1, test_line_2], country_id=self.fake_country.id
        )
        ...
        self._create_test_account_moves(
            [
                self._prepare_test_account_move_line(
                    3000.0, account_code="101001", tax_tags=["11", "222T"]
                ),
                self._prepare_test_account_move_line(
                    -3600.0, account_code="101001", tax_tags=["222T"]
                ),
                self._prepare_test_account_move_line(
                    600.0, account_code="101001", tax_tags=["222T", "11"]
                ),
            ]
        )
    ...
    def test_column_blank_if_zero(self):
        ...
        test_line = self._prepare_test_report_line(
            ...
            self._prepare_test_expression_external(
                "most_recent", [], label="percentage", figure_type="percentage"
            ),
            self._prepare_test_expression_external(
                "most_recent", [], label="integer", figure_type="integer"
            ),
            self._prepare_test_expression_external(
                "most_recent", [], label="float", figure_type="float"
            ),
            self._prepare_test_expression_external(
                "most_recent",
                [
                    self._prepare_test_external_values(
                        False, "2024-01-01", figure_type="boolean"
                    ),
                ],
                label="boolean",
                figure_type="boolean",
            ...
            self._prepare_test_expression_external(
                ...
                [
                    self._prepare_test_external_values(
                        "dudu", "2024-01-01", figure_type="string"
                    ),
                ],
                label="string",
                figure_type="string",
        ...
        report = self._create_report(
            [test_line],
            columns=["monetary", "percentage", "integer", "float", "boolean", "string"],
        )
    ...
    def test_column_groups_audit(self):
        ...
        main_options = self._generate_options(
            report,
            "2024-01-01",
            "2024-01-01",
            default_options={"selected_horizontal_group_id": horizontal_group.id},
        )
        lines = report._get_lines(main_options)
        for (col_group_key, col_group_options), expected_partner in zip(
            report._split_options_per_column_group(main_options).items(),
    ...
    def test_account_codes_load_more_limit_groupby(self):
        """The account_codes engine performs an additional groupby in its SQL query; make sure this
        does not break the load_more_limit behavior when grouping.
        """
        partner_a, partner_b, partner_c = self.env["res.partner"].create(
            [
                {"name": "Partner A"},
        ...
        self._create_test_account_moves(
            [
                ...
                self._prepare_test_account_move_line(
                    20, account_code="11", partner_id=partner_a.id
                ),
                self._prepare_test_account_move_line(
                    25, account_code="12", partner_id=partner_a.id
                ),
                self._prepare_test_account_move_line(30, account_code="11"),
                self._prepare_test_account_move_line(40, account_code="11"),
                self._prepare_test_account_move_line(
                    50, account_code="12", partner_id=partner_b.id
                ),
                self._prepare_test_account_move_line(
                    60, account_code="13", partner_id=partner_c.id
                ),
    ...
    def test_subformula_rounding(self):
        ...
        test_1 = self._prepare_test_report_line(
            self._prepare_test_expression_external(
                "sum",
                [self._prepare_test_external_values(98745.50, "2020-01-01")],
                label="external_decimal",
            ),
            self._prepare_test_expression_aggregation(
                "test_1.external_decimal", subformula="round(0)"
            ),
            name="test_1",
            code="test_1",
        )
        ...
        test_2 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test_1.external_decimal", subformula="round(0, HALF-UP)"
            ),
            name="test_2",
            code="test_2",
        )
        ...
        test_3 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test_1.external_decimal", subformula="round(0, HALF-EVEN)"
            ),
            name="test_3",
            code="test_3",
        )
        ...
        test_4 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test_1.external_decimal", subformula="round(0, UP)"
            ),
            name="test_4",
            code="test_4",
        )
        ...
        test_5 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test_1.external_decimal", subformula="round(0, DOWN)"
            ),
            name="test_5",
            code="test_5",
        )
        # Round to the nearest ten, using HALF-DOWN
        test_6 = self._prepare_test_report_line(
            self._prepare_test_expression_external(
                "sum",
                [self._prepare_test_external_values(98745.00, "2020-01-01")],
                label="external_decimal",
            ),
            self._prepare_test_expression_aggregation(
                "test_6.external_decimal", subformula="round(-1)"
            ),
            name="test_6",
            code="test_6",
        )
        # Round to the nearest ten, using HALF-UP
        test_7 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test_1.external_decimal", subformula="round(-1, HALF-UP)"
            ),
            name="test_7",
            code="test_7",
        )
        ...
        test_8 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test_1.external_decimal", subformula="round(-2)"
            ),
            name="test_8",
            code="test_8",
        )
    ...
    def test_engine_aggregation_if_other_expr_zero(self):
        ...
        test_1 = self._prepare_test_report_line(
            self._prepare_test_expression_external(
                "sum",
                [self._prepare_test_external_values(2500, "2020-01-01")],
                label="balance",
            ),
            name="test_1",
            code="test_1",
        ...
        test_2 = self._prepare_test_report_line(
            self._prepare_test_expression_external(
                "sum",
                [self._prepare_test_external_values(0, "2020-01-01")],
                label="balance",
            ),
            name="test_2",
            code="test_2",
        ...
        test_3 = self._prepare_test_report_line(
            self._prepare_test_expression_aggregation(
                "test_1.balance",
                subformula="if_other_expr_below(test_2.balance, USD(50))",
            ),
            name="test_3",
            code="test_3",
        )
    ...
    def test_account_name_added_in_account_code_grouping(self):
        ...
        AccountJournal.with_company(company_b).create(
            journal_vals | {"company_id": company_b.id}
        )
                                    ...
                                    Command.create(
                                        {
                                            ...
                                            "formula": "tag(account.account_codes_engine_test_tag1)",
                                        }
                                    )
                                ],
                            }
