...
class TestAccountMoveInalterableHash(AccountTestInvoicingCommon):
    ...
    def test_account_move_inalterable_hash(self):
        ...
        with self.assertRaisesRegex(
            UserError, f"{common}(.*Company.*Date.)|(.*Date.*Company.)"
        ):
            move.write(
                {"company_id": 666, "date": fields.Date.from_string("2023-01-03")}
            )

        with self.assertRaisesRegex(
            UserError, "You cannot edit the following fields.*Account.*"
        ):
            move.line_ids[0].account_id = move.line_ids[1]["account_id"]
        with self.assertRaisesRegex(
            UserError, "You cannot edit the following fields.*Partner.*"
        ):
    ...
    def test_account_move_hash_integrity_report(self):
        ...
        second_chain_moves = second_chain_moves_first_move | self._init_and_post(
            [
                {
                    "partner": self.partner_b,
                    "date": "2023-01-09",
        ...
        first_chain_moves_new_move.name = first_chain_moves[-1].name[:-1] + str(
            int(first_chain_moves[-1].name[-1]) + 1
        )
        ...
        self._verify_integrity(
            moves,
            "Entries are correctly hashed",
            second_chain_moves[0],
            second_chain_moves[-1],
            "XYZ/",
        )
        ...
        self._verify_integrity(
            moves,
            f"Corrupted data on journal entry with id {second_chain_moves[-1].id}.*",
            prefix="XYZ/",
        )
        ...
        self._verify_integrity(
            moves,
            f"Corrupted data on journal entry with id {first_chain_moves[-1].id}.*",
            prefix="INV/",
        )
    ...
    def test_account_move_hash_versioning_1(self):
        ...
        self._verify_integrity(
            moves,
            f"Corrupted data on journal entry with id {moves[1].id}.*",
            prefix=moves[0].sequence_prefix,
        )
    ...
    def test_account_move_hash_versioning_v1_to_v2(self):
        ...
        self._verify_integrity(
            moves,
            f"Corrupted data on journal entry with id {moves[4].id}.*",
            prefix=moves[0].sequence_prefix,
        )
        ...
        self._verify_integrity(
            moves,
            f"Corrupted data on journal entry with id {moves_v1_bis[0].id}.*",
            prefix=moves[0].sequence_prefix,
        )
    ...
    def test_retroactive_hashing(self):
        ...
        self._verify_integrity(
            move1 | move2, "Entries are correctly hashed", move1, move2
        )
    ...
    def test_no_hash_if_hole_in_sequence(self):
        ...
        with self._skip_hash_moves():
            ...
            move5 = self.init_invoice(
                "out_invoice", self.partner_a, "2024-01-02", amounts=[1000], post=False
            )
            move5.action_post()
        ...
        self._verify_integrity(
            moves, "Entries are correctly hashed", move1, move8, prefix="INV/"
        )
        self._verify_integrity(
            moves, "Entries are correctly hashed", move4, move7, prefix="RINV/"
        )
    ...
    def test_hash_multiyear(self):
        ...
        move2 = self.init_invoice(
            "out_invoice", self.partner_a, "2023-01-03", amounts=[1000], post=True
        )
        move3 = self.init_invoice(
            "out_invoice", self.partner_a, "2023-01-02", amounts=[1000], post=True
        )
        ...
        move5 = self.init_invoice(
            "out_invoice", self.partner_a, "2024-01-03", amounts=[1000], post=True
        )
        move6 = self.init_invoice(
            "out_invoice", self.partner_a, "2024-01-04", amounts=[1000], post=True
        )
    ...
    def test_retroactive_hashing_before_current(self):
        ...
        move5 = self.init_invoice(
            "out_invoice", self.partner_a, "2024-01-05", amounts=[1000], post=True
        )
        move6 = self.init_invoice(
            "out_invoice", self.partner_a, "2024-01-06", amounts=[1000], post=True
        )
    ...
    def test_account_move_hash_versioning_v3_to_v4(self):
        ...
        for _ in range(3):
            moves_v3_pre_restrict_mode |= self.init_invoice(
                "out_invoice",
                self.partner_a,
                "2024-01-01",
                amounts=[1000, 2000],
        ...
        with self.assertRaisesRegex(
            UserError, "This move could not be locked either because.*"
        ):
        ...
        self._verify_integrity(
            moves,
            "Entries are correctly hashed",
            moves_v3_post_restrict_mode[0],
            moves[-1],
            prefix=moves[0].sequence_prefix,
        )

        Model.write(
            moves_v3_post_restrict_mode[1],
            {"date": fields.Date.from_string("2024-11-07")},
        )
        self._verify_integrity(
            moves,
            f"Corrupted data on journal entry with id {moves_v3_post_restrict_mode[1].id}.*",
            prefix=moves[0].sequence_prefix,
        )
    ...
    def test_wizard_hashes_all_journals(self):
        ...
        moves = self.env["account.move"].create(
            [
                ...
                for idx, journal_type in enumerate(
    ...
    def test_wizard_ignores_sequence_prefixes_with_unreconciled_entries(self):
        ...
        wizard = self.env["account.secure.entries.wizard"].create(
            {"hash_date": "2017-01-01"}
        )
        reconciled_bank_statement_lines = (
            reconciled_bank_statement_line_2016 | reconciled_bank_statement_line_2017
        )
        ...
        self.assertEqual(
            ...
            unreconciled_bank_statement_line_2017,
        )
        self.assertEqual(
            wizard.move_to_hash_ids, reconciled_bank_statement_line_2016.move_id
        )
    ...
    def test_wizard_backwards_compatibility(self):
        ...
        moves_v4 = self.init_invoice(
            "out_invoice", self.partner_a, "2024-01-03", amounts=[1000], post=False
        )
        moves_v4 |= self.init_invoice(
            "out_invoice", self.partner_a, "2024-01-03", amounts=[1000], post=False
        )
        ...
        self.assertEqual(
            wizard.not_hashable_unlocked_move_ids, moves_v3_pre_restrict_mode
        )
