...
class TestAnalyticReport(TestAccountReportsCommon):
    ...
    def setUpClass(cls):
        ...
        cls.report = cls.env.ref("account.profit_and_loss")
    ...
    def test_report_audit_analytic_filter(self):
        ...
        self.assertEqual(
            audited_lines,
            out_invoice.invoice_line_ids[0],
            "Only the line with the parent account should be shown",
        )
    ...
    def test_report_analytic_groupby_and_filter(self):
        ...
        options = self._generate_options(
            ...
            default_options={
                "analytic_accounts_groupby": [
                    self.analytic_account_parent.id,
                    self.analytic_account_child.id,
                ],
            },
        )
        ...
        # Keep only the child analytic account on filter, the groupby column
        # for this account should still be filled, unlike the other
        options["analytic_accounts"] = [self.analytic_account_child.id]
    ...
    def test_audit_cell_analytic_groupby_and_filter(self):
        ...
        def _get_action_dict(options, column_index):
            ...
            report_line = self.report.line_ids[0]
            report_line_dict = next(x for x in lines if x["name"] == report_line.name)
            audit_param = self._get_audit_params_from_report_line(
                options,
                report_line,
                report_line_dict,
                column_group_key=list(options["column_groups"])[column_index],
            )
        ...
        other_account = self.env["account.analytic.account"].create(
            {"name": "Other Account", "plan_id": other_plan.id, "active": True}
        )
        ...
        out_invoices = out_invoices.with_context(
            analytic_plan_id=self.analytic_plan_parent.id
        )
        analytic_lines_parent = (
            out_invoices.invoice_line_ids.analytic_line_ids.filtered(
                lambda line: line.auto_account_id == self.analytic_account_parent
            )
        )
        analytic_lines_other = out_invoices.with_context(
            analytic_plan_id=other_plan.id
        ).invoice_line_ids.analytic_line_ids.filtered(
            lambda line: line.auto_account_id == other_account
        )
    ...
    def test_general_ledger_analytic_filter(self):
        ...
        general_ledger_report = self.env.ref("account.general_ledger_report")
        options = self._generate_options(
    ...
    def test_general_ledger_with_analytic_group_by(self):
        ...
        self.assertLinesValues(
            ...
            [
                ...
                [
                    "Total 121000 Account Receivable",
                    0.00,
                    0.00,
                    0.00,
                    300.00,
    ...
    def test_analytic_groupby_with_horizontal_groupby(self):
        ...
        options = self._generate_options(
            ...
            default_options={
                "analytic_accounts_groupby": [
                    self.analytic_account_parent.id,
                    self.analytic_account_parent_2.id,
                    self.analytic_account_parent_3.id,
                ],
    ...
    def test_analytic_groupby_plans_without_analytic_accounts(self):
        ...
        options = self._generate_options(
            ...
            "2019-12-31",
            default_options={
                "analytic_plans_groupby": analytic_plans_without_accounts.ids
            },
        )
    ...
    def test_profit_and_loss_multicompany_access_rights(self):
        ...
        test_user = self.env["res.users"].create(
            {
                ...
                "company_ids": [
                    Command.link(self.env.company.id),
                    Command.link(branch.id),
                ],
            }
        )
        ...
        options = self._generate_options(
            self.report.with_user(test_user).with_company(branch),
            "2019-01-01",
            "2019-12-31",
        )
        lines = self.report._get_lines(options)
    ...
    def test_audit_analytic_lines(self):
        ...
        analytic_lines = (
            parent_invoice.invoice_line_ids.analytic_line_ids
            + other_invoice.invoice_line_ids.analytic_line_ids
        )

        action_dict = _get_action_dict(
            options, 0
        )  # First column: analytic lines of parent plan
        self.assertEqual(action_dict["context"].get("group_by"), "move_line_id")
        self.assertEqual(
            self.env["account.analytic.line"]
            .with_context(action_dict["context"])
            .search(action_dict["domain"]),
            analytic_lines,
        )
        ...
        move_line_parent_product_a = move_lines.filtered(
            lambda line: (
                line.move_id.id == parent_invoice.id
                and line.product_id == self.product_a
            )
        )
        move_line_parent_product_b = move_lines.filtered(
            lambda line: (
                line.move_id.id == parent_invoice.id
                and line.product_id == self.product_b
            )
        )
        move_line_other = move_lines.filtered(
            lambda line: line.move_id.id == other_invoice.id
        )
