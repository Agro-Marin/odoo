...
class TestCompanyBranch(AccountTestInvoicingCommon):
    ...
    def test_chart_template_loading(self):
        ...
        branch_a_accounts = self.env["account.account"].search(
            [("company_ids", "parent_of", self.branch_a.id)]
        )
        ...
        branch_a_journals = self.env["account.journal"].search(
            [("company_id", "parent_of", self.branch_a.id)]
        )
    ...
    def test_reconciliation(self):
        ...
        payment_lines = (invoice + refund).line_ids.filtered(
    ...
    def test_reconciliation_foreign_currency(self):
        ...
        self.assertTrue(
            payment_lines.matched_debit_ids.exchange_move_id.journal_id.company_id,
            invoice.company_id,
        )
        ...
        for lock, lock_sale, lock_purchase in [
            ...
            for (
                ...
                with (
                    ...
                    check = (
                        partial(self.assertRaises, UserError)
                        if failure_expected
                        else nullcontext
                    )
    ...
    def test_branch_should_keep_parent_company_currency(self):
        ...
        self.env["account.chart.template"].try_loading(
            "generic_coa", company=root_company.child_ids[0], install_demo=False
        )
        self.assertEqual(
            root_company.currency_id, root_company.child_ids[0].currency_id
        )
