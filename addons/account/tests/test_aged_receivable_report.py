...
class TestAgedReceivableReport(TestAccountReportsCommon):
    ...
    def setUpClass(cls):
        ...
        receivable_5 = cls.copy_account(
            cls.company_data_2["default_account_receivable"]
        )
        receivable_6 = cls.copy_account(
            cls.company_data_2["default_account_receivable"]
        )
        ...
        (move_3 + move_4).line_ids.filtered(
            lambda line: line.account_id == receivable_4
        ).reconcile()
        (move_3 + move_4).line_ids.filtered(
            lambda line: line.account_id == receivable_5
        ).reconcile()
        ...
        cls.report = cls.env.ref("account.aged_receivable_report")
    ...
    def test_aged_receivable_prefix_groups(self):
        ...
        for name in partner_names:
            ...
            invoices_map[name] = self.env["account.move.line"]._format_aml_name(
                invoice.payment_reference, False, invoice.name
            )
