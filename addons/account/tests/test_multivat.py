...
def _tax_vals(
    name, amount, external_id_prefix, cash_basis=False, account_on_repartition=True
):
    return {
        "name": name,
        "amount": amount,
...
class TestMultiVAT(AccountTestInvoicingCommon):
    ...
    def _use_chart_template(cls, company, chart_template_ref=None):
        ...
        with patch.object(
            ...
            cls.env["account.chart.template"].try_loading(
                "local", company=company, install_demo=False
            )
    ...
    def test_tax_and_tax_group_should_be_reachable_using_standard_api(self):
        ...
        for xml_id in (
            ...
            with self.subTest(xml_id=xml_id):
                record = self.env["account.chart.template"].ref(
    ...
    def test_tax_group_data(self):
        ...
        foreign_tax_group = self.env["account.chart.template"].ref(
            "foreign_tax_group_taxes"
        )
        for field in (
            ...
            with self.subTest(field=field):
                self.assertTrue(
                    foreign_tax_group[field], "This account should have been set"
                )
                self.assertNotEqual(
                    foreign_tax_group[field],
                    local_tax_group[field],
        ...
        for xml_id in composite_taxes:
            with self.subTest(xml_id=xml_id):
                ...
                for i, child in enumerate(record.children_tax_ids):
                    child_tax = self.env["account.chart.template"].ref(
                        children_taxes[xml_id][i], raise_if_not_found=False
                    )
    ...
    def test_multivat_cash_basis(self):
        def wrap_data_getter_for_caba(data_getter):
            def caba_data_getter(self, template_code):
                ...
                rslt["account.tax"]["es.caba_0_tax"] = _tax_vals(
                    "Dudu 0", 0, "es", cash_basis=True, account_on_repartition=False
                )
                rslt["account.tax"]["es.caba_42_tax"] = _tax_vals(
                    "Dudu 42", 42, "es", cash_basis=True
                )
        ...
        created_taxes = self.env.ref("local_es.caba_0_tax") + self.env.ref(
            "local_es.caba_42_tax"
        )
