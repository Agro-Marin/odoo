...
class TestUnexpectedAmount(AccountTestInvoicingCommon):
    ...
    def test_higher_amount(self):
        base = self.env["account.move"].create(
            [self._invoice_vals(price_unit=100) for i in range(10)]
        )
        base.action_post()
        ...
        bills = bills.with_context(
            {
                ...
                if k != "disable_abnormal_invoice_detection"
            }
        )

        wizard_thrown = bills[0].action_post()
        self.assertFalse(
            wizard_thrown, "Invoice is not deviant, no wizard should be thrown"
        )
        ...
        wizard_thrown = (
            bills[2].with_context(disable_abnormal_invoice_detection=True).action_post()
        )
        self.assertFalse(
            wizard_thrown,
    ...
    def test_date_too_soon_year(self):
        base = self.env["account.move"].create(
            [self._invoice_vals(date=f"{year}-01-01") for year in range(2000, 2010)]
        )
        base.action_post()
