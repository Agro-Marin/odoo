...
class AccountDeferredReportHandler(models.AbstractModel):
    ...
    def _get_deferred_report_type(self):
        raise NotImplementedError(
            "This method is not implemented in the deferred report handler."
        )
    ...
    def _get_domain(
        ...
        domain &= Domain(self._get_domain_fully_inside_period(options))
    ...
    def _get_select(self, options):
        ...
        return [
            ...
            SQL(
                "account_move_line__product_template_id.categ_id AS product_category_id"
            ),
    ...
    def _get_grouping_fields_deferred_lines(
        self, filter_already_generated=False, grouping_field="account_id"
    ):
        return (grouping_field,)

    @api.model
    def _group_by_deferred_fields(
        self, line, filter_already_generated=False, grouping_field="account_id"
    ):
        return tuple(
            line[k]
            for k in self._get_grouping_fields_deferred_lines(
                filter_already_generated, grouping_field
            )
        )
    ...
    def _group_deferred_amounts_by_grouping_field(
        ...
        grouping_field="account_id",
    ):
        """Group the deferred amounts by grouping key and compute the totals per key and per period.

        Returns (totals_per_key, totals_aggregated_by_period), where period1 = (date1, date2, label1), ...:
        {
            self._group_by_deferred_fields(line): {
        ...
        """
        deferred_amounts_by_line = groupby(
            deferred_amounts_by_line,
            key=lambda x: self._group_by_deferred_fields(
                x, filter_already_generated, grouping_field
            ),
        )
        totals_per_key = {}  # {key: {**self._get_current_key_totals_dict(), <one entry per period>}}
        totals_aggregated_by_period = dict.fromkeys(periods + ["totals_aggregated"], 0)
        ...
        for key, lines_per_key in deferred_amounts_by_line:
            ...
            totals_aggregated_by_period["totals_aggregated"] += current_key_totals[
                "amount_total"
            ]
            for period in periods:
                current_key_totals[period] = sign * sum(
                    line[period] for line in lines_per_key
                )
                totals_aggregated_by_period[period] += (
    ...
    def _custom_options_initializer(self, report, options, previous_options):
        ...
        before_column = [
            {
                **options["columns"][0],
                ...
                "date_to": fields.Date.to_string(
                    fields.Date.to_date(options["columns"][0]["date_from"])
        ...
        recognized_column = [
            {
                **options["columns"][0],
                ...
                "date_to": options["columns"][-1][
                    "date_to"
                ],  # Covers up to end of current period
            }
        ]
        options["columns"] = (
            total_column
        ...
        options["deferred_grouping_field"] = (
            previous_options.get("deferred_grouping_field") or "account_id"
        )
        if (
    ...
    def action_audit_cell(self, options, params):
        ...
        if grouping_record_id:
            ...
            original_move_lines_domain &= Domain(
                options["deferred_grouping_field"], "=", grouping_record_id
            )
        ...
        # - Not Started: lines of the deferral entries whose original invoice/bill date is before or in the
        #   current period but whose deferral only starts after it, plus the lines of those invoices/bills
        # - Before: lines of the deferral entries dated before the current period, created by invoices/bills
        #   also being deferred in the current period, plus the lines of those invoices/bills
        # - Current: lines of the deferral entries in the current period, plus those of their original invoices/bills
        # - Later: lines of the deferral entries dated after the current period, created by invoices/bills also
        #   being deferred in the current period, plus the lines of those invoices/bills
        domain = [
        ...
        if (
            column_values["expression_label"] != "total"
            and original_moves.deferred_move_ids
        ):
            domain = (
                ["|"]
                + [("move_id", "in", original_moves.deferred_move_ids.ids)]
                + domain
            )
        ...
        elif not original_moves.deferred_move_ids:
            domain += [
    ...
    def _dynamic_lines_generator(
        ...
        deferred_amounts_by_line = self.env[
            "account.move"
        ]._get_deferred_amounts_by_line(
            lines, periods, self._get_deferred_report_type()
        )
        totals_per_grouping_field, totals_all_grouping_field = (
            self._group_deferred_amounts_by_grouping_field(
        ...
        for totals_grouping_field in totals_per_grouping_field.values():
            ...
            grouping_field_description = self.env["account.move.line"][
                options["deferred_grouping_field"]
            ]._description
                ...
                (
                    0,
                    {
                        "id": report._get_generic_line_id(
                            grouping_model, grouping_record.id
                        ),
                        "name": grouping_name,
    ...
    def _generate_deferral_entry(self, options):
        ...
        deferred_move = (
            ...
            .with_context(skip_account_deprecation_check=True)
            .create(
                {
                    "move_type": "entry",
    ...
    def _get_deferred_lines(self, lines, deferred_account, period, is_reverse, ref):
        ...
        :return: (deferred_lines, original_move_ids), the Command objects creating the deferred lines and the
                 ids of the moves that created them, kept to track the original invoice in the deferral entry.
        :rtype: tuple
        """
        ...
        deferred_amounts_by_line = self.env[
            ...
            lines, [period], self._get_deferred_report_type()
        )
        deferred_amounts_by_key, deferred_amounts_totals = (
            self._group_deferred_amounts_by_grouping_field(
                deferred_amounts_by_line,
                [period],
                is_reverse,
                filter_already_generated=True,
            )
        )
        ...
        # dict of keys: self._group_by_deferred_fields(line)
        ...
        for line in lines:
            if not line["analytic_distribution"]:
                continue
            # Analytic distribution should be computed from the lines with the same _group_by_deferred_fields(), except for
            # the deferred line with the deferral account which will use _group_by_deferral_fields()
            sign = 1 if is_reverse else -1
            key_amount = deferred_amounts_by_key.get(
                self._group_by_deferred_fields(line, True)
            )
            total_amount = key_amount.get("amount_total")
            key_ratio = sign * line["balance"] / total_amount if total_amount else 0
            full_ratio = (
                sign * line["balance"] / totals_aggregated if totals_aggregated else 0
            )

            for account_id, distribution in line["analytic_distribution"].items():
                anal_dist_by_key[self._group_by_deferred_fields(line, True)][
                    account_id
                ] += distribution * key_ratio
                deferred_anal_dist[self._group_by_deferral_fields(line)][
                    account_id
        ...
        for key, line in deferred_amounts_by_key.items():
            for balance in (-line["amount_total"], line[period]):
                if balance != 0 and line[period] != line["amount_total"]:
                    ...
                    deferred_balance = self.env.company.currency_id.round(
                        (1 if is_reverse else -1) * balance
                    )
        ...
        for key, lines_per_key in grouped_by_key.items():
            ...
            for line in lines_per_key:
                if line[period] != line["amount_total"]:
                    balance += self.env.company.currency_id.round(
                        (1 if is_reverse else -1)
                        * (line["amount_total"] - line[period])
                    )
