...
class AccountAccountTag(models.Model):
    ...
    # If the tag was generated by a report line for the ``tax_tags``
    # engine, these fields allow getting the sign of the report line
    # used to display the amount.
        ...
        for tag in self:
            ...
            if (
                ...
                name = _(
                    ...
                    tag=tag.name, country_code=tag.country_id.code,
                )
        ...
        id2expression = {
            ...
            for tag_id, *vals in self.env.execute_query(
                query.select(
                    ...
                    self._field_to_sql(
                        query.table, 'report_expression_id', query,
                    ),
                    self._field_to_sql(
                        query.table, 'balance_negate', query,
                    ),
                ),
            )
        }
        for tag in self:
            tag.report_expression_id, tag.balance_negate = (
                id2expression.get(tag._origin.id, (False, False))
            )

    def _field_to_sql(
        self, alias: str, field_expr: str,
    ...
    def _get_tax_tags(self, tag_name, country_id):
        """Return all tax tags matching the given name in the country."""
        domain = self._get_tax_tags_domain(tag_name, country_id)
        original_lang = self.env.context.get('lang', 'en_US')
        rslt_tags = self.env['account.account.tag'].with_context(
    ...
    def _get_tax_tags_domain(self, formula, country_id):
        """Return a domain for tax tags matching a formula in a country."""
        return [
            ('name', '=', formula.lstrip('-')),
    ...
    def _get_related_tax_report_expressions(self):
        ...
        return self.env['account.report.expression'].search(
            Domain('engine', '=', 'tax_tags') & Domain.OR(
                (
                    Domain(
