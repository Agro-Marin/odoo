...
class AccountMove(models.Model):
    ...
    def _compute_invoice_default_user(self):
        ...
        Base implementation sets current user for invoice documents.
        Extended by sale module for salesperson and purchase module for buyer.
        """
        for move in self:
    ...
    def _compute_partner_bank_id(self):
        ...
        for move in self:
            if (
                ...
                and payment_method.journal_id.bank_account_id.allow_out_payment
            ):
    ...
    def _fetch_duplicate_reference(self, matching_states=("draft", "posted")):
        ...
        return {
            self.env["account.move"].browse(move_id): self.env["account.move"].browse(
                duplicate_ids
            )
    ...
    def _compute_reconciled_payment_ids(self):
        ...
        for move in self:
            move.reconciled_payment_ids = (
                self.env["account.payment"].browse(invoice_payment_links.get(move.id))
    ...
    def _get_move_display_name(self, show_ref=False):
        ...
        if self.env.context.get("name_as_amount_total"):
            ...
            if self.name:
                return _(
                    "%(name)s - Draft at (%(currency_amount)s)",
                    name=(self.name),
                    currency_amount=currency_amount,
                )
            else:
    ...
    def _post(self, soft=True):
            ...
            if not invoice.invoice_date:
                if invoice.is_sale_document(include_receipts=True):
                    is_manual_rate = (
                        ...
                        != invoice._get_expected_currency_rate_at(
        ...
        if not self.env.user.has_group(
            ...
            lambda move: (
                ...
                and move.invoice_line_ids.filtered(lambda l: l.deductible_amount != 100)
            )
        ):
    ...
    def _link_bill_origin_to_purchase_orders(self, timeout=10):
        for move in self.filtered(lambda m: m.move_type in self.get_purchase_types()):
            references = (
                [ref.strip() for ref in re.split(r"[ ,]+", move.invoice_origin)]
    ...
    def _detach_attachments(self):
        ...
        if files_to_detach:
            ...
            for attachment in files_to_detach:
                attachment_path = PurePosixPath(attachment.name)
                attachment_name, attachment_extension = (
                    attachment_path.stem,
                    attachment_path.suffix,
                )
                attachment.name = _(
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        partners = self._partner_find_from_emails_single(
            from_mail_addresses,
            filter_found=lambda p: is_right_company(p) or not p.partner_share,
            no_create=True,
        )
        ...
        if partners and is_internal_partner(partners[0]):
            ...
            partners = (
                self._partner_find_from_emails_single(
                    body_mail_addresses,
                    filter_found=lambda p: is_right_company(p) or p.partner_share,
                    no_create=True,
                )
                if body_mail_addresses
