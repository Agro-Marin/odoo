...
class AccountMove(models.Model):
    ...
    made_sequence_gap = fields.Boolean(
        ...
        store=True,
    )  # store wether this is the first move breaking the natural sequencing
    ref = fields.Char(
        string="Reference",
        copy=False,
        tracking=True,
    ...
    needed_terms_dirty = fields.Boolean(
        compute="_compute_needed_terms",
    )
    ...
    show_discount_details = fields.Boolean(
        compute="_compute_show_payment_term_details",
    )

    abnormal_amount_warning = fields.Text(
        compute="_compute_abnormal_warnings",
    )
    abnormal_date_warning = fields.Text(
        compute="_compute_abnormal_warnings",
    )
    ...
    _checked_idx = models.Index(
        "(journal_id) WHERE (checked IS NOT TRUE)",
    )
    _payment_idx = models.Index(
        "(journal_id, state, payment_state, move_type, date)",
    )
    _journal_id_company_id_idx = models.Index(
        "(journal_id, company_id, date)",
    )
        ...
        for move in self:
            ...
            if not move.is_sale_document(include_receipts=True):
                accounting_date = move._get_accounting_date(
                    move.invoice_date, move._affect_tax_report()
                )
    ...
    def _compute_is_storno(self):
        for move in self:
            move.is_storno = move.is_storno or (
                ...
                and move.company_id.account_storno
            )
    ...
    def _compute_made_sequence_gap(self):
        ...
        for (journal, prefix), moves in (
            (self - unposted)
            .grouped(lambda m: (m.journal_id, m.sequence_prefix))
            .items()
        ):
        ...
        groups = self.grouped(
            lambda move: (
                ...
                if move.payment_state == "invoicing_legacy"
                    ...
                    else "blocked" if move.payment_state == "blocked" else "unpaid"
                )
            )
        )
    ...
    def _compute_status_in_payment(self):
        for move in self:
            move.status_in_payment = (
                move.state if move.state == "cancel" else move.payment_state
            )
                    ...
                    if is_draft:
                        ...
                        for _base_line, to_update in tax_results[
                            ...
                            untaxed_amount_currency += (
                                sign * to_update["amount_currency"]
                            )
                            untaxed_amount += sign * to_update["balance"]
                        for tax_line_vals in tax_results["tax_lines_to_add"]:
                            tax_amount_currency += (
                                sign * tax_line_vals["amount_currency"]
                            )
                    ...
                    reconciled_vals.append(
                        {
                            ...
                            "company_name": (
                                counterpart_line.journal_id.company_id.name
                                if counterpart_line.journal_id.company_id
                                != move.company_id
                                else False
                            ),
    ...
    def _compute_duplicated_ref_ids(self):
        ...
        for move in self:
            ...
            move.duplicated_ref_ids = move_to_duplicate_move.get(
                move._origin, self.env["account.move"]
            )
    ...
    def _fetch_duplicate_reference(self, matching_states=("draft", "posted")):
        ...
        if not moves[0].id:  # check if record is under creation/edition in UI
            ...
            values = {
                field_name: moves._fields[field_name].convert_to_write(
                    moves[field_name], moves
                )
                or None
            ...
            casted_values = SQL(", ").join(
                SQL(
                    "%s::%s",
                    value,
                    SQL.identifier(moves._fields[field_name].column_type[0]),
                )
            ...
            move_table_and_alias = SQL(
                "(VALUES (%s)) AS move(%s)", casted_values, column_names
            )
        ...
        return {
            self.env["account.move"]
            .browse(move_id): self.env["account.move"]
    ...
    def _compute_abnormal_warnings(self):
        ...
        if self.env.context.get("disable_abnormal_invoice_detection"):
            ...
            draft_invoices = self.filtered(
                lambda m: m.is_purchase_document()
                and m.state == "draft"
                and m.amount_total
                and not (
    ...
    def _sync_tax_lines(self, container):
        ...
        def get_value(record, field):
            return (
                ...
                ._fields[field]
                .convert_to_write(record[field], record)
            )
    ...
    def _sync_dynamic_line(
        ...
        if not needed_before and (
            filter_trivial(inv_existing_after) != filter_trivial(inv_existing_before)
        ):
    ...
    def copy(self, default=None):
        ...
        for old_move, new_move in zip(self, new_moves):
            ...
            message_content = (
                _("This entry has been reversed from %s", old_move._get_html_link())
                if default.get("reversed_entry_id")
                else _(
                    "This entry has been duplicated from %s", old_move._get_html_link()
                )
            )
    ...
    def write(self, vals):
        ...
        for move in self:
            ...
            if (
                not self._context.get("skip_readonly_check")
    ...
    def _get_unlink_logger_message(self):
        ...
        moves_details = []
        for move in self.filtered(
            lambda m: m.posted_before and m.company_id.check_account_audit_trail
        ):
    ...
    def _get_starting_sequence(self):
        ...
        if (
            self.journal_id.payment_sequence
    ...
    def _get_invoice_reference_euro_invoice(self):
        ...
        return format_structured_reference_iso(
            f"{self.journal_id.code}{str(self.id).zfill(6)}"
        )
    ...
    def _get_chain_info(
        ...
        moves_to_hash = (
            self.env["account.move"].sudo().search(domain, order="sequence_number")
        )
        warnings = set()
        if moves_to_hash:
            # gap warning
            ...
            unreconciled = False in moves_to_hash.statement_line_ids.mapped(
                "is_reconciled"
            )
            if unreconciled:
    ...
    def _get_edi_creation(self):
        ...
        """
        container = {"records": self}
        with self._check_balanced(
            container
        ), self._disable_discount_precision(), self._sync_dynamic_lines(container):
    ...
    def _extend_with_attachments(self, attachments, new=False):
        ...
        for file_data in file_data_list:
            ...
            if attachments_by_invoice.get(file_data["attachment"]):
                add_file_data_results(
                    file_data, attachments_by_invoice[file_data["attachment"]]
                )
                close_file(file_data)
                continue
            ...
            extend_with_existing_lines = file_data.get(
                "process_if_existing_lines", False
            )
            ...
            if decoder or file_data["type"] in ("pdf", "binary"):
                try:
                    with self.env.cr.savepoint():
                        ...
                        if (
                            success
                            or file_data["type"] == "pdf"
                            or file_data["attachment"].mimetype in ALLOWED_MIMETYPES
                        ):
                            (invoice.invoice_line_ids - existing_lines).is_imported = (
                                True
                            )
    ...
    def _get_invoice_counterpart_amls_for_early_payment_discount_per_payment_term_line(
        ...
        if early_pay_discount_computation == "included" and invoice_lines.tax_ids:
            ...
            percentage_paid = abs(
                payment_term_line.amount_residual_currency / self.amount_total
            )
            ...
            biggest_base_line = max(
                list(res["base_lines"][payment_term_line].values()),
    ...
    def _can_be_unlinked(self):
        ...
        posted_caba_entry = self.state == "posted" and (
            self.tax_cash_basis_rec_id or self.tax_cash_basis_origin_move_id
        )
        posted_exchange_diff_entry = (
            self.state == "posted" and self.exchange_diff_partial_ids
        )
        return (
    ...
    def _post(self, soft=True):
        ...
        for move in self:
            ...
            if move.line_ids.account_id.filtered(
                lambda account: account.deprecated
            ) and not self._context.get("skip_account_deprecation_check"):
                validation_msgs.add(
                    _(
        ...
        draft_reverse_moves = to_post.filtered(
            lambda move: move.reversed_entry_id
            and move.reversed_entry_id.state == "posted"
        )
                ...
                for partial in partials:
                    ...
                    if (
                        ...
                        if (
                            ...
                            to_post |= move.tax_cash_basis_created_move_ids.filtered(
        ...
        if non_deductible_lines := self.line_ids.filtered(
            lambda line: (
                line.display_type
        ...
        for invoice in to_post:
            ...
            subscribers = (
                [partner_id.id]
                if partner_id and partner_id not in invoice.sudo().message_partner_ids
                else None
            )
    ...
    def _set_next_made_sequence_gap(self, made_gap: bool):
        ...
        for (journal, prefix), moves in named.grouped(
            lambda move: (move.journal_id, move.sequence_prefix)
        ).items():
            next_moves += (
                self.env["account.move"]
                .sudo()
    ...
    def action_switch_move_type(self):
        if any(move.posted_before for move in self):
            raise ValidationError(
                _(
                    "You cannot switch the type of a document which has been posted once."
                )
            )
    ...
    def action_invoice_sent(self):
        ...
        if (
            ...
            report_action = self.env[
                ...
                report_action, "account.action_base_document_layout_configurator"
            )
    ...
    def _detach_attachments(self):
        ...
        if files_to_detach:
            ...
            for attachment in files_to_detach:
                attachment_name, attachment_extension = os.path.splitext(
                    attachment.name
                )
                attachment.name = _(
    ...
    def _notify_get_recipients_groups(self, message, model_description, msg_vals=False):
        ...
        if self.move_type != "entry":
            ...
            recipient_group = (
                ...
                lambda pdata: pdata["id"] in local_msg_vals.get("partner_ids", [])
    ...
    def _autopost_draft_entries(self):
        ...
        moves = self.search(
            [
                ...
                "|",
                ("checked", "=", True),
                ("journal_id.autocheck_on_post", "=", True),
            ],
            limit=100,
        )
    ...
    def _cron_account_move_send(self, job_count=10):
        ...
        def get_account_notification(moves, is_success: bool):
            ...
            return [
                "account_notification",
                {
                    ...
                    "title": (
                        _("Invoices sent") if is_success else _("Invoices in error")
                    ),
                    "message": (
                        _("Invoices sent successfully.")
                        if is_success
                    ...
                    "action_button": {
                        "name": _("Open"),
                        "action_name": (
                            _("Sent invoices") if is_success else _("Invoices in error")
                        ),
                        "model": "account.move",
                        "res_ids": moves.ids,
                    },
        ...
        if not self.env["res.company"]._with_locked_records(
            to_process, allow_raising=False
        ):
            return
        ...
        moves_by_partner = to_process.grouped(
            lambda m: m.sending_data["author_partner_id"]
        )
        ...
        for partner_id, partner_moves in moves_by_partner.items():
            ...
            partner_moves_error = partner_moves.filtered(
                lambda m: m.sending_data and m.sending_data.get("error")
            )
            ...
            if partner_moves_success:
                partner._bus_send(
                    *get_account_notification(partner_moves_success, True)
                )
    ...
    def _get_available_action_reports(self, is_invoice_report=True):
        ...
        available_reports = model_reports.filtered(
            lambda model_template: len(
                self.filtered_domain(ast.literal_eval(model_template.domain or "[]"))
            )
            == len(self)
        )
    ...
    def _is_action_report_available(self, action_report, is_invoice_report=True):
        ...
        if available_report := action_report.filtered(
            lambda available_report: not (
                is_invoice_report ^ available_report.is_invoice_report
            )
        ):
            return bool(
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if (custom_values or {}).get("move_type", "entry") not in (
            "out_invoice",
            "in_invoice",
            "entry",
        ):
        ...
        followers = self._partner_find_from_emails_single(
            cc_mail_addresses, filter_found=is_right_company, no_create=True
        )

        # Search for partner that sent the mail.
        from_mail_addresses = email_split(msg_dict.get("from", ""))
        senders = partners = self._partner_find_from_emails_single(
            from_mail_addresses, filter_found=is_right_company, no_create=True
        )
        ...
        if not senders:
            user_partners = (
                self.env["res.users"]
                .sudo()
                .search([("email_normalized", "in", from_mail_addresses)])
                .mapped("partner_id")
            )
            senders = partners = self.env["res.partner"].search(
                [("id", "in", user_partners.ids)]
            )
    ...
    def _message_post_after_hook(self, new_message, message_values):
        ...
        for invoice, attachments in attachments_per_invoice.items():
            if invoice == self:
                ...
                super(AccountMove, invoice)._message_post_after_hook(
                    new_message, message_values
                )
            else:
                ...
                super(AccountMove, invoice)._message_post_after_hook(
                    sub_new_message, sub_message_values
                )
    ...
    def _check_and_decode_attachment(self, attachments):
        ...
        if self.state != "draft":
            self.with_user(SUPERUSER_ID).message_post(
                body=_(
                    "The invoice is not a draft, it was not updated from the attachment."
                ),
        ...
        move_per_decodable_attachment = self._extend_with_attachments(
            attachments, new=bool(self._context.get("from_alias"))
        )
    ...
    def _set_reversed_entry(self, credit_note):
        ...
        original_invoice = self.filtered(
            ...
            and credit_note.invoice_line_ids.sale_line_ids
            in inv.invoice_line_ids.sale_line_ids
        )
