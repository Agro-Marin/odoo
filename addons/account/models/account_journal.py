...
class AccountJournal(models.Model):
        ...
        for journal in self:
            ...
            if journal.type == "sale":
                ...
                journal.profit_account_id = (
                    journal.company_id.default_cash_difference_income_account_id
                )
                journal.loss_account_id = (
                    journal.company_id.default_cash_difference_expense_account_id
                )
    ...
    def create(self, vals_list):
        ...
        for journal, vals in zip(journals, vals_list):
            ...
            if (
                journal.type == "bank"
                and not journal.bank_account_id
                and vals.get("bank_acc_number")
            ):
                journal.set_bank_account(
                    vals.get("bank_acc_number"), vals.get("bank_id")
                )
    ...
    def _create_document_from_attachment(self, attachment_ids):
        ...
        if not self:
            raise UserError(
                self.env["account.journal"]._build_no_journal_error_msg(
