...
class AccountMoveLine(models.Model):
        ...
        for line in product_lines:
            if line.product_id:
                ...
                if account_id:
                    line.account_id = account_id
        for line in self:
            if not line.account_id and line.display_type not in (
                "line_section",
                "line_subsection",
                "line_note",
            ):
    ...
    def _compute_tax_ids(self):
        for line in self:
            if (
                line.display_type
                in ("line_section", "line_subsection", "line_note", "payment_term")
                or line.is_imported
            ):
                continue
            ...
            if line.product_id or (
                ...
                and (line.account_id.tax_ids or not line.tax_ids)
            ):
    ...
    def _get_computed_taxes(self):
        ...
        if self.move_id.is_sale_document(include_receipts=True):
            ...
            tax_ids = filtered_taxes_id or self.account_id.tax_ids.filtered(
            ...
            filtered_supplier_taxes_id = (
                self.product_id.supplier_taxes_id.filtered_domain(company_domain)
            )
    ...
    def _compute_epd_needed(self):
        ...
        for line in self:
            ...
            discount_percentage = (
                line.move_id.invoice_payment_term_id.discount_percentage
            )

            if (
                not has_epd
                or line.display_type != "product"
                or not line.tax_ids.ids
                or line.move_id.invoice_payment_term_id.early_pay_discount_computation
                != "mixed"
            ):
                continue
            ...
            amount_currency = line.currency_id.round(
                sign * line.price_subtotal * percentage
            )
            balance = (
                line.company_currency_id.round(
                    sign * line.price_subtotal * percentage / rate
                )
                if rate
                else 0.0
            )
    ...
    def _compute_payment_date(self):
        for line in self:
            line.payment_date = (
                line.discount_date
                if line.discount_date and date.today() <= line.discount_date
    ...
    def _inverse_account_id(self):
        ...
        self._conditional_add_to_compute(
            ...
            lambda line: (
                ...
                and not line.product_id.taxes_id.filtered(
    ...
    def _check_constrains_account_id_journal_id(self):
        ...
        for line in self.filtered(
            lambda x: (
                x.display_type not in ("line_section", "line_subsection", "line_note")
            )
        ):
    ...
    def _constrains_deductible_amount(self):
        for line in self:
            if not line.move_id.is_purchase_document() and float_compare(
    ...
    def default_get(self, fields):
        ...
        if quick_encode_suggestion and self.env.context.get(
    ...
    def create(self, vals_list):
        ...
        if not self.env.context.get("tracking_disable"):
            ...
            tracked_fields = [
                ...
                if hasattr(f, "tracking")
                and f.tracking
                and not (hasattr(f, "related") and f.related)
            ]
    ...
    def write(self, vals):
        ...
        for line in self:
            ...
            if line.parent_state == "posted" and any(
                self.env["account.move"]._field_will_change(line, vals, field_name)
                for field_name in ("tax_ids", "tax_line_id")
            ):
                raise UserError(
                    _(
            ...
            if line.parent_state == "posted" and any(
                self.env["account.move"]._field_will_change(line, vals, field_name)
                for field_name in protected_fields["fiscal"]
            ):
            ...
            if line.parent_state == "posted" and any(
                self.env["account.move"]._field_will_change(line, vals, field_name)
                for field_name in protected_fields["tax"]
            ):
            ...
            if line.matching_number and (
                changing_fields := {
                    ...
                    if self.env["account.move"]._field_will_change(
    ...
    def _unlink_except_posted(self):
        ...
        if not self.env.context.get("force_delete") and any(
            m.state == "posted" for m in self.move_id
        ):
            raise UserError(
                _(
    ...
    def _compute_has_invalid_analytics(self):
        ...
        for line in lines_to_validate:
            ...
            try:
                business_domain = (
                    "invoice"
                    if line.move_id.is_sale_document(True)
                    else "bill"
                    if line.move_id.is_purchase_document(True)
                    else "general"
                )
    ...
    def _reconcile_plan_with_sync(self, plan_list, all_amls):
        ...
        for partial in partials:
            for exchange_move in exchange_moves:
                ...
                if (
                    any(
                        line == partial.debit_move_id or line == partial.credit_move_id
    ...
    def _get_child_lines(self):
        ...
        section_lines = self.move_id.invoice_line_ids.filtered(
            lambda l: l.parent_id == self or l.parent_id.parent_id == self
        )
        result = []
