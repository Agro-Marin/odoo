...
def _subselect_domain(model, field_name, domain):
    query = model._search(
        Domain(field_name, "!=", False) & domain, active_test=False, bypass_access=True
    )
    return Domain(
        "id", "in", query.subselect(model._field_to_sql(query.table, field_name, query))
    )
...
class MailMessage(models.Model):
    ...
    def _compute_account_audit_log_restricted(self):
        ...
        if potentially_restricted := self.filtered(lambda r: r.model in DOMAINS):
            ...
            restricted = self.search(
                Domain("id", "in", potentially_restricted.ids) & search_domain
            )
        ...
        return Domain("message_type", "=", "notification") & Domain.OR(
            [
                ...
                (
                    "res_id",
                    "in",
                    self.env[model]._search(domain_factory(self, operator, value)),
                ),
            ]
    ...
    def _search_audit_log_related_record_id(self, model, operator, value):
        if (
        ...
        ) or (operator in ("in", "not in") and any(isinstance(v, str) for v in value)):
            res_id_domain = [
                (
                    "res_id",
                    "in",
                    self.env[model]._search([("display_name", operator, value)]),
                )
            ]
            ...
            if isinstance(value, Domain):
                ...
                query = value
            res_id_domain = [
                ("res_id", "in" if operator in ("any", "any!") else "not in", query)
            ]
    ...
    def write(self, vals):
        ...
        if (
            ...
            or (
                "subject" in vals
                and any(
                    " ".join(s.subject.split()) != normalized_subject
                    for s in self
                    if s.subject
                )
            )
