...
class AccountEcSalesReportHandler(models.AbstractModel):
    ...
    def _dynamic_lines_generator(
        ...
        operation_categories = options["sales_report_taxes"].get(
            "operation_category", {}
        )
        ...
        for partner, results in self._query_partners(report, options, warnings):
            for tax_ec_category in ("goods", "triangular", "services"):
                ...
                for col_grp_key in options["column_groups"]:
                    ...
                    partner_values[col_grp_key]["vat_number"] = partner_sum.get(
                        "vat_number", "UNKNOWN"
                    )
                    partner_values[col_grp_key]["country_code"] = partner_sum.get(
                        "country_code", "UNKNOWN"
                    )
                    partner_values[col_grp_key]["sales_type_code"] = []
                    partner_values[col_grp_key]["balance"] = partner_sum.get(
                        tax_ec_category, 0.0
                    )
                    totals_by_column_group[col_grp_key]["balance"] += partner_sum.get(
                        tax_ec_category, 0.0
                    )
                    for i, operation_id in enumerate(
                        partner_sum.get("tax_element_id", [])
                    ):
                        if (
                            operation_id
                            in options["sales_report_taxes"][tax_ec_category]
                        ):
                            has_found_a_line = True
                            partner_values[col_grp_key]["sales_type_code"] += [
                                country_specific_code
                                or (
                                    partner_sum.get("sales_type_code")
                                    and partner_sum.get("sales_type_code")[i]
                                )
                                or None
                            ]
                    partner_values[col_grp_key]["sales_type_code"] = ", ".join(
                        set(partner_values[col_grp_key]["sales_type_code"])
                    )
                if has_found_a_line:
                    lines.append(
                        (
            ...
            lines.append(
                (
                    ...
                    self._get_report_line_total(
                        report, options, totals_by_column_group
                    ),
                )
            )
    ...
    def _custom_options_initializer(self, report, options, previous_options):
        ...
        country_ids = (
            ...
            .search([("code", "in", tuple(self._get_ec_country_codes(options)))])
            .ids
        )
    ...
    def _init_core_custom_options(self, report, options, previous_options):
        ...
        if ec_tax_filter_selection != default_tax_filter:
            filtered_ec_tax_filter_selection = [
                item
                for item in ec_tax_filter_selection
                if item["id"] in {item["id"] for item in default_tax_filter}
            ]
    ...
    def _get_report_line_partner(
        ...
        :return dict: the values for the report line
        """
        column_values = []
        for column in options["columns"]:
            value = partner_values[column["column_group_key"]].get(
    ...
    def _query_partners(self, report, options, warnings=None):
        ...
        (partner, fetched_values) sorted by the res.partner model _order:
            - partner is a res.partner record.
            - fetched_values is a dictionary keyed by column_group_key, each value containing:
        ...
        :return:                    A list of (res.partner record, fetched_values) tuples.
        :rtype:                     list
        """
        groupby_partners = {}
        vat_set = set()
        ...
        def update_partner_sums(row):
            ...
            :param dict row: One row of the _get_query_sums result.
            """
            # Balances are kept per operation type so the report can be rendered either vertically (up to 3
            # lines per partner, one per operation type) or horizontally (one line with 3 columns).
            if not company_currency.is_zero(row["balance"]):
                vat = row["vat_number"] or ""
                vat_country_code = vat[:2] if vat[:2].isalpha() else None
                duplicated_vat = (
                    vat and vat in vat_set and row["groupby"] not in groupby_partners
                )
                if vat:
                    vat_set.add(vat)
                ...
                groupby_partners_keyed = groupby_partners[row["groupby"]][
                    row["column_group_key"]
                ]
                for key in options["sales_report_taxes"]:
                    ...
                    if (
                        not isinstance(options["sales_report_taxes"][key], bool)
                        and row["tax_element_id"] in options["sales_report_taxes"][key]
                    ):
                        groupby_partners_keyed[key] += row["balance"]

                groupby_partners_keyed.setdefault("tax_element_id", []).append(
                    row["tax_element_id"]
                )
                groupby_partners_keyed.setdefault("sales_type_code", []).append(
                    row["sales_type_code"]
                )

                groupby_partners_keyed.setdefault(
                    "vat_number", vat if not vat_country_code else vat[2:]
                )
                groupby_partners_keyed.setdefault("full_vat_number", vat)
                groupby_partners_keyed.setdefault(
                    "country_code", vat_country_code or row.get("country_code")
                )
    ...
    def _get_query_sums(self, report, options) -> SQL:
        ...
        if options.get("sales_report_taxes", {}).get("use_taxes_instead_of_tags"):
            ...
            tax_elem_table_name = self.env["account.tax"]._field_to_sql(
                "account_tax", "name"
            )
        else:
        ...
        for (
            ...
            if allowed_ids:
                query.add_where(
                    SQL("%s.id = ANY(%s)", tax_elem_table, list(allowed_ids))
                )
                if service_ids:
                    query.add_where(
                        SQL(
                            "(%s.id != ALL(%s) OR LEFT(res_partner.vat, 2) IS DISTINCT FROM 'XI')",
                            tax_elem_table,
                            list(service_ids),
                        )
                    )
    ...
    def _get_tag_ids_filtered(self, options, operation_types=None):
        """Get all the ids concerned by the report for the given options.

        :param dict options: Report options
        :param list operation_types: only retrieve the taxes or tags of the specified types (if selected)
        :return set: the ids stored in options['sales_report_taxes'] for the selected operation types; tax
            tag ids, or tax ids when 'use_taxes_instead_of_tags' is set
        """
        return {
            tag_id
            for op in options.get("ec_tax_filter_selection", [])
            if op.get("selected")
            and (operation_types is None or op["id"] in operation_types)
    ...
    def _get_ec_country_codes(self, options):
        ...
        :param dict options: Report options
        :return set: the country codes applicable at the report date (see the GB and MC cases below)
        """
        rslt = {
            "AT",
            "BE",
        ...
        if self.env.company.tax_config_id.account_fiscal_country_id.code != "FR":
            rslt.add("MC")
    ...
    def get_warning_act_window(self, options, params):
        ...
        tax_or_tag_field = (
            "tax_ids.id" if use_taxes_instead_of_tags else "tax_tag_ids.id"
        )
    ...
    def _get_duplicated_vat_partners(self, duplicated_partners_vat):
        view_ref = self.env.ref(
            "account.duplicated_vat_partner_tree_view", raise_if_not_found=False
        )
        return {
            ...
            "context": {
                "group_by": "vat",
                "expand": 1,
                "duplicated_partners_vat": duplicated_partners_vat,
            },
