        ...
        for wizard in self:
            ...
            if wizard.unreconciled_bank_statement_line_ids:
                ignored_sequence_prefixes = list(
                    set(
                        wizard.unreconciled_bank_statement_line_ids.move_id.mapped(
                            "sequence_prefix"
                        )
                    )
                )
            ...
            draft_entries = self.env["account.move"].search_count(
                wizard._get_draft_moves_in_hashed_period_domain(), limit=1
            )
            if draft_entries:
            ...
            if wizard.chains_to_hash_with_gaps:
                ...
                warnings["account_sequence_gap"] = {
                    ...
                    "action_text": _("Review Entries"),
                    "action": {
                        **self.env["account.journal"]._show_sequence_holes(
                            list(domain)
                        ),
                        "views": [
                            [
            ...
            if moves_to_hash_after_selected_date:
                warnings["account_move_to_secure_after_selected_date"] = {
                    ...
                    "action_text": _("Review Entries"),
                    "action": wizard.action_show_moves(
                        moves_to_hash_after_selected_date
                    ),
                }
