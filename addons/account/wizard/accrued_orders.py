        ...
        for order in orders:
            ...
            if len(orders) == 1 and product_lines and self.amount and order.line_ids:
                ...
                if is_purchase:
                    ...
                    o.line_ids.with_context(accrual_entry_date=self.date)._compute_qty_transfered()
                    o.line_ids.with_context(accrual_entry_date=self.date)._compute_qty_invoiced()
                    o.line_ids.with_context(accrual_entry_date=self.date)._compute_untaxed_amount_invoiced()
                    o.line_ids.with_context(accrual_entry_date=self.date)._compute_qty_to_invoice()
