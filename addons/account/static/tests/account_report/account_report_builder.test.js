...
test("can edit a line", async () => {
    ...
    await contains(
        ".account_report_lines_list_x2many li[data-record_id='1'] .column",
    ).click();
    await waitFor(".o_dialog");
...
test("can delete a root", async () => {
    ...
    expect(
        ".account_report_lines_list_x2many li span:contains('Root with children')",
    ).toHaveCount(1);
    expect(
        ".account_report_lines_list_x2many li span:contains('Child #1')",
    ).toHaveCount(1);
    expect(
        ".account_report_lines_list_x2many li span:contains('Grandchild')",
    ).toHaveCount(1);
    expect(
        ".account_report_lines_list_x2many li span:contains('Child #2')",
    ).toHaveCount(1);
});
...
test("can display and hide 'Code' column when toggled in optional fields", async () => {
    ...
    expect(
        ".account_report_lines_list_x2many span.fw-bold.fixed:contains('Code')",
    ).toHaveCount(0);
