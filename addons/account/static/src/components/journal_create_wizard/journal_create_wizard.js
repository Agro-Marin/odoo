...
export class JournalCreateWizard extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            this.hasGroupAccountUser = await user.hasGroup(
    ...
    cardImages(cardType) {
        const result =
            cardType === "card" ? ["logo_visa", "logo_mastercard"] : [cardType];
        return result.map(
            (imageName) =>
                `/account/static/src/img/journal_create_wizard/${imageName}.svg`,
        );
    }
        ...
        const data = [
            {
                ...
                title: _t("Bank"),
                text: _t(
                    "Connect your bank and payment gateways (Paypal, Stripe, ...) or record your transactions manually",
                ),
            ...
            data.push(
                {
                    ...
                    text: _t(
                        "Payroll, depreciation, closing entries, deferred revenues, ...etc",
                    ),
