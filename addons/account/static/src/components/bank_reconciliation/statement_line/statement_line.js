...
export class BankRecStatementLine extends KanbanRecord {
    ...
    _updateLinesState() {
        ...
        this.state.linesToReconcile = allLines.filter(
            (line) =>
                line.account_id.id !== suspenseId && line.account_id.id !== defaultId,
        );
        this.state.suspenseAccountLine = allLines.find(
            (line) => line.account_id.id === suspenseId,
        );
    ...
    _computeReconciledLineName(linesToReconcile) {
        ...
        for (const line of linesToReconcile) {
            if (
                line.reconciled_lines_excluding_exchange_diff_ids.records.length ===
                    1 &&
                line.reconciled_lines_excluding_exchange_diff_ids.records[0].data
                    .move_name
            ) {
                reconciledLine[line.id] = {
                    move: line.reconciled_lines_excluding_exchange_diff_ids.records[0]
                        .data.move_name,
                };
    ...
    get hasForeignCurrencyAndSameCurrencyForAllLines() {
        return (
            ...
            this.linesToReconcile.every(
                (line) =>
                    line.currency_id.id === this.recordData.foreign_currency_id.id,
            )
        );
    }
            ...
            lines
                ...
                .reduce(
                    (accumulator, line) =>
                        ...
                        parseInt(line.data.move_attachment_ids.count),
                    0,
                ) +
            lines
                .filter(
                ...
                .reduce(
                    (accumulator, line) =>
                        parseInt(accumulator) +
                        parseInt(line.move_attachment_ids.count),
                    0,
                )
        );
    }
