...
export class BankRecButtonList extends Component {
    ...
    setup() {
        ...
        this.currencyDigits =
            getCurrency(this.statementLineData.currency_id.id)?.digits || 2;
    ...
    async _setPartnerOnReconcileLine(partner_id) {
        await this.orm.call(
            ...
            [this.statementLineData.id, partner_id],
        );
        const recordsToLoad = [];
        ...
        this.addDialog(
            SelectCreateDialog,
            {
                ...
                onSelected: async (partner) =>
                    await this._setPartnerOnReconcileLine(partner[0]),
            },
            {
                onClose: () => {
                    this.restoreFocus();
    ...
    setAccountOnReconcileLine() {
        const context = {
            ...
            search_view_ref: "account.view_account_search_bank_rec_widget",
    ...
    async setAccountReceivableOnReconcileLine() {
        ...
        if (this.statementLineData.partner_id.property_account_receivable_id.id) {
            accountId =
                this.statementLineData.partner_id.property_account_receivable_id.id;
        } else {
        ...
        await this._setAccountOnReconcileLine(
            this.lastAccountMoveLine.data.id,
            accountId,
        );
    ...
    async setAccountPayableOnReconcileLine() {
        ...
        if (this.statementLineData.partner_id.property_account_payable_id.id) {
            accountId =
                this.statementLineData.partner_id.property_account_payable_id.id;
        } else {
            const account = await this.orm.webSearchRead(
                "account.account",
                [
    ...
    get isLineSelected() {
        return (
            this.statementLineData.id === this.bankReconciliation.statementLine?.data.id
        );
    }
