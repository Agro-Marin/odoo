...
class PortalAccount(CustomerPortal):
    ...
    def _get_domain_invoices(self, m_type=None):
        ...
        return Domain("state", "not in", ("cancel", "draft")) & Domain(
    ...
    def _get_account_searchbar_sortings(self):
        return {
            ...
            "duedate": {
                "label": request.env._("Due Date"),
                "order": "invoice_date_due desc",
            },
            "name": {"label": request.env._("Reference"), "order": "name desc"},
            "state": {"label": request.env._("Status"), "order": "payment_state"},
        }
    ...
    def _get_account_searchbar_filters(self):
        return {
            ...
            "overdue_invoices": {
                "label": request.env._("Overdue invoices"),
                "domain": self._get_domain_overdue_invoices(),
            },
            "invoices": {
                "label": request.env._("Invoices"),
                "domain": [
                    ("move_type", "in", ("out_invoice", "out_refund", "out_receipt"))
                ],
            },
            "bills": {
                "label": request.env._("Bills"),
                "domain": [
    ...
    def portal_my_invoice_detail(
        ...
        if report_type == "pdf" and download and invoice_sudo.state == "posted":
            ...
            if (
                partner_lang
                and request.env["res.lang"]._get_data(code=partner_lang).direction
                == "rtl"
            ):
    ...
    def portal_my_journal_unsubscribe(self, journal_id, **kw):
        ...
        if access_token := kw.get("token"):
            try:
                token_data = resolve_hash_signed(
                    ...
                    request.env[
                        "account.journal"
                    ]._get_journal_notification_unsubscribe_scope(),
                    access_token,
                )
            except ValueError:
        ...
        all_recipients = email_normalize_all(
            journal.incoming_einvoice_notification_email or ""
        )
        ...
        if request.httprequest.method == "POST":
            ...
            return _render(
                {"journal": journal, "email": email_to_unsubscribe, "completed": True}
            )
