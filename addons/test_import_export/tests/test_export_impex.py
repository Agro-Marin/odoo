...
class test_integer_field(CreatorCase):
    ...
    def test_empty(self):
        self.assertEqual(
            self.model.search([]).ids, [], "Test model should have no records"
        )
...
class test_string_field(CreatorCase):
    ...
    def test_out_of_bounds(self):
        self.assertEqual(
            self.export(
                "C for Sinking, Java for Drinking, Smalltalk for Thinking. ...and Power to the Penguin!"
            ),
            [["C for Sinking, J"]],
        )
...
class test_date(CreatorCase):
    ...
    def test_basic(self):
        self.assertEqual(
            self.export("2011-11-07"), [[fields.Date.from_string("2011-11-07")]]
        )
...
class test_selection(CreatorCase):
    ...
    def test_localized_export(self):
        ...
        for sel_fr, sel in zip(
            selection.with_context(lang="fr_FR"), selection, strict=True
        ):
...
class test_o2m(CreatorCase):
    ...
    def test_single_subfield(self):
        self.assertEqual(
            self.export(
                [Command.create({"value": 42})], fields=["value", "value/value"]
            ),
    ...
    def test_integrate_one_in_parent(self):
        self.assertEqual(
            self.export(
                [Command.create({"value": 42})], fields=["const", "value/value"]
            ),
            [[4, 42]],
        )
...
class test_o2m_multiple(CreatorCase):
    ...
    def test_single_per_side(self):
        ...
        self.assertEqual(
            self.export(
                child1=[Command.create({"value": 43})],
    ...
    def test_single_integrate_subfield(self):
        ...
        self.assertEqual(
            self.export(
                child1=False, child2=[Command.create({"value": 42})], fields=fields
            ),
            [[36, "", 42]],
        )
        ...
        self.assertEqual(
            self.export(
                child1=[Command.create({"value": 43})], child2=False, fields=fields
            ),
            [[36, 43, ""]],
        )
    ...
    def test_multiple(self):
        ...
        fields = ["const", "child1/value", "child2/value"]
        child1 = [
            Command.create({"value": v, "str": "record%.02d" % index})
...
class test_m2m(CreatorCase):
    ...
    def test_multiple_records_subfield(self):
        ...
        self.assertEqual(
            r.with_context(import_compat=True)._export_rows(
                [["value"], ["value", "id"]]
            ),
            [
                [
        ...
        self.assertEqual(
            r.with_context(import_compat=False)._export_rows(
                [["id"], ["value", "id"], ["value", "value"]]
            ),
            [
                [xid, "__t__.record000", 4],
                ["", "__t__.record001", 42],
                ["", "__t__.record010", 36],
                ["", "__t__.record011", 4],
                ["", "__t__.record100", 13],
