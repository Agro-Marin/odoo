        ...
        for row in range(len(expected)):
            for column in range(len(expected[row])):
                ...
                self.assertEqual(
                    cell_value,
                    expected_value,
                    "Cell %s, %s have a wrong value" % (row, column),
                )
...
class TestExport(XlsxCreatorCase):
    def test_int_monetary_float(self):
        ...
        values = [
            {
                "int_sum": 1,
                "float_monetary": 60739.2000000004,
                "bool_and": True,
                "float_min": 60739.2000000004,
            },
            {
                "int_sum": 2,
                "float_monetary": 999.9995999,
                "bool_and": True,
                "float_min": 999.9995999,
            },
    ...
    def test_export_template(self):
        def get_namelist(export_id):
            res = self.url_open(
                "/web/export/namelist",
                data=json.dumps(
                    {"params": {"model": "res.users", "export_id": export_id}}
                ),
    ...
    def test_import_compatible_export(self):
        ...
        res = self.url_open(
            ...
            data=json.dumps(
                {"params": {"model": test_model, "import_compat": True, "domain": []}}
            ),
            headers={"Content-Type": "application/json"},
        )
        ...
        expected_fields = {
            f.name for f in model_fields.filtered(lambda field: not field.readonly)
        } | {"id"}
...
class TestGroupedExport(XlsxCreatorCase):
    ...
    def test_unarchived_grouped(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "active"], params={"groupby": ["int_sum"]}
        )
        self.assertExportEqual(
            export,
            [
    ...
    def test_archived_grouped_by_ids(self):
        ...
        export = self.export(
            fields=["int_sum", "active"], params={"groupby": ["int_sum"], "ids": ids}
        )
        self.assertExportEqual(
            export,
            [
    ...
    def test_archived_grouped_by_ids_one2many(self):
        sub_rec_active = self.env["export.aggregator.one2many"].create(
            {"name": "active", "active": True}
        )
        sub_rec_non_active = self.env["export.aggregator.one2many"].create(
            {"name": "non active", "active": False}
        )
        values = [
            {
                "int_sum": 1,
                "active": False,
                "one2many": [sub_rec_active.id, sub_rec_non_active.id],
            },
        ]
        ids = [rec.id for rec in self.make(values)]
        export = self.export(
            fields=["int_sum", "active", "one2many"],
            params={"groupby": ["int_sum"], "ids": ids},
        )
        self.assertExportEqual(
            export,
            [
    ...
    def test_int_sum_max(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "int_max"], params={"groupby": ["int_sum", "int_max"]}
        )
        self.assertExportEqual(
            export,
            [
        ...
        export = self.export(
            fields=["int_max", "int_sum"], params={"groupby": ["int_sum", "int_max"]}
        )
    ...
    def test_float_min(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "float_min"],
            params={"groupby": ["int_sum", "float_min"]},
        )
    ...
    def test_float_avg(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "float_avg"],
            params={"groupby": ["int_sum", "float_avg"]},
        )
    ...
    def test_float_avg_nested(self):
        ...
        export = self.export(
            fields=["int_sum", "float_avg"],
            params={"groupby": ["int_sum", "int_max", "float_avg"]},
        )
    ...
    def test_date_max(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "date_max"],
            params={"groupby": ["int_sum", "date_max:month"]},
        )
        ...
        self.assertExportEqual(
            ...
            [
                ["Int Sum", "Date Max"],
                ["10 (2) — 20", "2019-01-01"],
                ["    January 2000 (1) — 10", "2000-01-01"],
                ["10", "2000-01-01"],
                ["    January 2019 (1) — 10", "2019-01-01"],
                ["10", "2019-01-01"],
                ["20 (1) — 20", "1980-01-01"],
                ["    January 1980 (1) — 20", "1980-01-01"],
                ["20", "1980-01-01"],
            ],
        )
    ...
    def test_bool_and(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "bool_and"], params={"groupby": ["int_sum", "bool_and"]}
        )
    ...
    def test_bool_or(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "bool_or"], params={"groupby": ["int_sum", "bool_or"]}
        )
    ...
    def test_many2one(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "many2one"], params={"groupby": ["int_sum", "many2one"]}
        )
    ...
    def test_float_representation(self):
        ...
        self.make(values)
        export = self.export(
            fields=["int_sum", "float_monetary"],
            params={"groupby": ["int_sum", "float_monetary"]},
        )
        self.assertExportEqual(
            export,
            [
    ...
    def test_order(self):
        ...
        export = self.export(
            fields=["date_max"], params={"groupby": ["int_sum"], "domain": []}
        )
        self.assertExportEqual(
            export,
            [
    ...
    def test_order_many2many(self):
        ...
        self.make(
            [
                {
                    "many2many": [Command.link(partner1.id), Command.link(partner2.id)],
                    "date_max": "2025-04-01",
                },
                {
                    "many2many": [Command.link(partner2.id), Command.link(partner3.id)],
                    "date_max": "2025-12-12",
                },
                {
                    "many2many": [Command.link(partner3.id), Command.link(partner4.id)],
                    "date_max": "2025-12-12",
                },
                {
                    "many2many": [Command.link(partner4.id), Command.link(partner1.id)],
                    "date_max": "2025-04-01",
                },
            ]
        )
    ...
    def test_performance_export_data(self):
        ...
        with (
            ...
            export = self.export(
                fields=["int_sum", "int_max"],
                params={"groupby": ["int_sum"], "domain": []},
            )

        self.assertEqual(
            mock.call_count, 1, "`export_data` should be called only one time"
        )
    ...
    def test_groupby_properties_type_field(self):
        ...
        property_def = [
            {"name": "date", "type": "date", "string": "Date", "default": "2025-11-11"}
        ]
        record = self.env["export.aggregator"].create(
            {"definition_properties": property_def}
        )
        values = [
            {
                "int_sum": 10,
                "properties": {"date": "2025-11-09"},
                "parent_id": record.id,
                "float_min": 86.8,
            },
            {
                "int_sum": 10,
                "properties": {"date": "2025-11-12"},
                "parent_id": record.id,
                "float_min": 82.8,
            },
            {
                "int_sum": 10,
                "properties": {"date": "2025-09-09"},
                "parent_id": record.id,
                "float_min": 20.8,
            },
        ]
