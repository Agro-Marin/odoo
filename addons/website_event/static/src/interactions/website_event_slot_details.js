...
export class SlotDetails extends Interaction {
    ...
    dynamicContent = {
        ...
        ".o_wevent_selected_slot_title": {
            "t-out": () =>
                this.selectedSlotId ? _t("Selected Date:") : _t("Select a Date:"),
        },
    };
    ...
    _onChangePageClick(ev) {
        const numPage = ev.currentTarget.classList.contains("o_wevent_slot_next")
            ? this.currentSlotPage + 1
            : this.currentSlotPage - 1;
        const maxNumPage =
            Math.ceil(
                this.el.querySelectorAll(".o_wevent_slot_date").length /
                    MAX_DATES_PER_PAGE,
            ) - 1;
    ...
    _onSlotSelected(ev) {
        ...
        this.selectedSlotDatetime =
            deserializeDateTime(dataset.slotStart, {
                tz: dataset.eventTz,
            }).toLocaleString(DateTime.DATETIME_MED_WITH_WEEKDAY) +
            " - " +
            deserializeDateTime(dataset.slotEnd, {
                tz: dataset.eventTz,
            }).toLocaleString(DateTime.TIME_SIMPLE);
        this.form.setAttribute(
            "data-selected-slot-id",
            parseInt(ev.currentTarget.dataset.slotId),
        );
    }
...
registry.category("public.interactions").add("website_event.slot_details", SlotDetails);
