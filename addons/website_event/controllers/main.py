...
class WebsiteEventController(http.Controller):
    ...
    @http.route(
        ...
        list_as_website_content=_lt("Events"),
    )
    def events(self, page=1, slug_tags=None, **searches):
        if (
            (slug_tags or searches.get("tags", "[]").count(",") > 0)
        ...
        keep = QueryURL(
            ...
            **{
                ...
                if (
                    ...
                    and (
                        key == "search"
                        or (value != "scheduled" if key == "date" else value != "all")
                    )
                )
            },
        )
    ...
    def event_page(self, event, page, **post):
        ...
        view = (
            request.env["website.event.menu"]
            .sudo()
            .search(
                [
                    ("event_id", "=", event.id),
    ...
    def _process_tickets_form(self, event, form_details):
        ...
        ticket_dict = {
            ticket.id: ticket
            for ticket in request.env["event.event.ticket"]
            .sudo()
            .search(
                [
        ...
        slot = (
            request.env["event.slot"].browse(int(slot))
            if (slot := form_details.get("event_slot_id", False))
            else slot
        )
    ...
    @http.route(
        ['/event/<model("event.event"):event>/registration/slot/<int:slot_id>/tickets'],
        type="jsonrpc",
        auth="public",
        methods=["POST"],
        website=True,
    ...
    def registration_tickets(self, event, slot_id):
        ...
        return request.env["ir.ui.view"]._render_template(
            "website_event.modal_ticket_registration",
            {
                "event": event,
                "event_slot": slot,
                "seats_available_slot_tickets": {
                    ticket.id: availability
                    for (_, ticket), availability in zip(
                        slot_tickets,
                        event._get_seats_availability(slot_tickets),
                        strict=True,
                    )
                },
            },
    ...
    def _process_attendees_form(self, event, form_details):
        ...
        for key, value in form_details.items():
            ...
            if len(key_values) == 2:
                ...
                if field_name not in registration_fields:
                    continue
                registrations.setdefault(registration_index, {})[field_name] = (
                    int(value) or False
                )
                continue
        ...
        if general_answer_ids:
            for registration in registrations.values():
                registration.setdefault("registration_answer_ids", []).extend(
                    general_answer_ids
                )
    ...
    def _create_attendees_from_registration_post(self, event, registration_data):
        ...
        for registration_values in registration_data:
            ...
            if not registration_values.get("partner_id") and visitor_sudo.partner_id:
                ...
                registration_values["partner_id"] = (
                    False
        ...
        try:
            event._verify_seats_availability(
                list(
                    {
                        (
                            slots_per_id.get(slot_id, False),
                            tickets_per_id.get(ticket_id, False),
                            count,
                        )
                        for (
                            slot_id,
                            ticket_id,
                        ), count in counter_per_combination.items()
                    }
                )
            )
    ...
    def _slugify_tags(self, tag_ids, toggle_tag_id=None):
        ...
        return (
            ",".join(
                request.env["ir.http"]._slug(tag_id)
                for tag_id in request.env["event.tag"].browse(tag_ids)
            )
            or ""
        )
        ...
        try:
            tag_ids = list(
                filter(
                    None,
                    [
                        request.env["ir.http"]._unslug(tag)[1]
                        for tag in (search_tags or "").split(",")
                    ],
                )
            )
        except Exception:
