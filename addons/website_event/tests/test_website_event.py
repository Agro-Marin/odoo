...
class TestEventRegisterUTM(HttpCase, TestEventOnlineCommon):
    def test_event_registration_utm_values(self):
        ...
        self.assertEqual(
            new_registration.utm_source_id, self.env.ref("utm.utm_source_newsletter")
        )
        self.assertEqual(
...
class TestUi(HttpCaseWithUserDemo, HttpCaseWithUserPortal):
    ...
    def test_website_event_pages_seo(self):
        ...
        self.start_tour(
            self.env["website"].get_client_action_url(url),
            "website_event_pages_seo",
            login="admin",
        )
    ...
    def test_website_event_questions(self):
        ...
        self.design_fair_event = self.env["event.event"].create(
            {
                ...
                "question_ids": [
                    ...
                    (0, 0, {"title": "Allergies", "question_type": "text_box"}),
                    (
                        0,
                        0,
                        {
        ...
        first_registration = registrations.filtered(
            lambda reg: reg.email == "attendee-a@gmail.com"
        )
        second_registration = registrations.filtered(
        ...
        self.assertEqual(
            first_registration_answers.filtered(
                lambda answer: answer.question_id.title == "Meal Type"
            ).value_answer_id.name,
            "Vegetarian",
        )
        ...
        self.assertEqual(
            first_registration_answers.filtered(
                lambda answer: (
                    answer.question_id.title == "How did you learn about this event?"
                )
            ).value_answer_id.name,
            "A friend",
        )
        ...
        self.assertEqual(
            second_registration_answers.filtered(
                lambda answer: answer.question_id.title == "Meal Type"
            ).value_answer_id.name,
            "Pastafarian",
        )
...
class TestWebsiteAccess(HttpCaseWithUserDemo, OnlineEventCase):
    ...
    def test_website_access_event_manager(self):
        ...
        self.assertEqual(
            resp.status_code, 200, "Managers must have access to published event."
        )

        unpublished_events = self.events.filtered(
            lambda event: not event.website_published
        )
        resp = self.url_open("/event/%i" % unpublished_events[0].id)
        self.assertEqual(
            resp.status_code, 200, "Managers must have access to unpublished event."
        )

        resp = self.url_open("/event")
        self.assertTrue(
            published_events[0].name in resp.text,
            "Managers must see the unpublished events.",
        )
        self.assertTrue(
            unpublished_events[0].name in resp.text,
            "Managers must see the published events.",
        )
    ...
    def test_website_access_event_uer(self):
        ...
        resp = self.url_open("/event/%i" % published_events[0].id)
        self.assertEqual(
            resp.status_code, 200, "Event user must have access to published event."
        )
        ...
        resp = self.url_open("/event/%i" % unpublished_events[0].id)
        self.assertEqual(
            resp.status_code, 200, "Event user must have access to unpublished event."
        )

        resp = self.url_open("/event")
        self.assertTrue(
            published_events[0].name in resp.text,
            "Event user must see the unpublished events.",
        )
        self.assertTrue(
            unpublished_events[0].name in resp.text,
            "Event user must see the published events.",
        )
    ...
    def test_website_access_portal(self):
        ...
        resp = self.url_open("/event/%i" % published_events[0].id)
        self.assertEqual(
            resp.status_code, 200, "Portal user must have access to published event."
        )
        ...
        self.assertEqual(
            resp.status_code,
            403,
            "Portal user must not have access to unpublished event.",
        )

        resp = self.url_open("/event")
        self.assertTrue(
            published_events[0].name in resp.text,
            "Portal must see the published events.",
        )
        self.assertFalse(
            unpublished_events[0].name in resp.text,
            "Portal should not see the unpublished events.",
        )
    ...
    def test_website_access_public(self):
        ...
        resp = self.url_open("/event/%i" % published_events[0].id)
        self.assertEqual(
            resp.status_code, 200, "Public must have access to published event"
        )
        ...
        resp = self.url_open("/event/%i" % unpublished_events[0].id)
        self.assertEqual(
            resp.status_code, 404, "Public must not have access to unpublished event"
        )

        resp = self.url_open("/event")
        self.assertTrue(
            published_events[0].name in resp.text,
            "Public must see the published events.",
        )
        self.assertFalse(
            unpublished_events[0].name in resp.text,
            "Public should not see the unpublished events.",
        )

    def test_sitemap(self):
        resp = self.url_open("/sitemap.xml")
        self.assertTrue(
            "/event/event-0" in resp.text,
            "Published events must be present in the sitemap",
        )
        self.assertTrue(
            "/event/event-1" in resp.text,
            "Published events must be present in the sitemap",
        )
        self.assertFalse(
            "/event/event-2" in resp.text,
            "Unpublished events must not be present in the sitemap",
        )
    ...
    def test_check_search_in_address(self):
        ret = self.env["event.event"]._search_get_detail(
            self.website,
            order=None,
            options={"displayDescription": "", "displayDetail": ""},
        )
