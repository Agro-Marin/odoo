...
class OnlineEventCase(EventCase):
    ...
    def setUpClass(cls):
        ...
        cls.event_customer.write(
            {
                "website_description": "<p>I am your best customer, %s</p>"
                % cls.event_customer.name,
            }
        )
        cls.event_customer2.write(
            {
                "website_description": "<p>I am your best customer, %s</p>"
                % cls.event_customer2.name,
            }
        )
    ...
    def _assert_website_menus(self, event, menus_in=None, menus_out=None):
        ...
        if menus_out:
            self.assertTrue(
                all(menu_name not in menus.mapped("name") for menu_name in menus_out)
            )
...
class TestEventOnlineCommon(OnlineEventCase):
    ...
    def setUpClass(cls):
        ...
        cls.event_0 = cls.env["event.event"].create(
            {
                "name": "TestEvent",
                "date_begin": datetime.combine(cls.reference_now, time(7, 0))
                - timedelta(days=1),
                "date_end": datetime.combine(cls.reference_now, time(13, 0))
                + timedelta(days=1),
