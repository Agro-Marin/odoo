...
MAP_REPAIR_LINE_TYPE_TO_MOVE_LOCATIONS_FROM_REPAIR = {
    ...
    "remove": {
        ...
        "location_dest_id": "parts_location_id",
    },
    "recycle": {
        "location_id": "location_dest_id",
...
class StockMove(models.Model):
    ...
    def _compute_forecast_information(self):
        moves_to_compute = self.filtered(
            lambda move: not move.repair_line_type or move.repair_line_type == "add"
        )
    ...
    def _compute_reference(self):
        ...
        super(
            StockMove, self - self.env["stock.move"].browse(moves_with_reference)
        )._compute_reference()
    ...
    def create(self, vals_list):
        ...
        for move in moves:
            if not move.repair_id:
                continue
            move.reference_ids = [
                Command.link(r.id) for r in move.repair_id.reference_ids
            ]
        ...
        draft_repair_moves = repair_moves.filtered(
            lambda m: (
                m.state == "draft"
                and m.repair_id.state in ("confirmed", "under_repair")
            )
        )
    ...
    def write(self, vals):
        ...
        for move in self:
            ...
            if move.sale_line_id and (
                "repair_line_type" in vals or "product_uom_qty" in vals
            ):
                repair_moves |= move
    ...
    def _create_repair_sale_order_line(self):
        ...
        for move in self:
            if (
                move.sale_line_id
                or move.repair_line_type != "add"
                or not move.repair_id.sale_order_id
            ):
                continue
            product_qty = (
                move.product_uom_qty
                if move.repair_id.state != "done"
                else move.quantity
            )
            so_line_vals.append(
                {
    ...
    def _get_repair_locations(self, repair_line_type, repair_id=False):
        ...
        if location_map:
            ...
            location_id, location_dest_id = [
                repair_id[field] for field in location_map.values()
            ]
        else:
    ...
    def action_show_details(self):
        ...
        if self.repair_line_type == "recycle":
            action["context"].update(
                {"show_quant": False, "show_destination_location": True}
            )
        return action
