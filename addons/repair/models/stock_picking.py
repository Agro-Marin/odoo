...
class StockPickingType(models.Model):
    ...
    code = fields.Selection(
        selection_add=[("repair_operation", "Repair")],
    ...
    def _compute_default_location_src_id(self):
        ...
        super(
            StockPickingType, remaining_picking_type
    ...
    def _compute_default_location_dest_id(self):
        ...
        prod_locations = self.env["stock.location"]._read_group(
            [
                ("usage", "=", "production"),
                ("company_id", "in", repair_picking_type.company_id.ids),
            ],
            ["company_id"],
            ["id:min"],
        )
        ...
        for picking_type in repair_picking_type:
            picking_type.default_location_dest_id = prod_locations.get(
                picking_type.company_id.id
            )
        super(
            StockPickingType, (self - repair_picking_type)
    ...
    def _compute_default_remove_location_dest_id(self):
        ...
        for picking_type in repair_picking_type:
            picking_type.default_remove_location_dest_id = scrap_locations.get(
                picking_type.company_id.id
            )
