...
class TestGlobalRouteRulesInstallation(common.TransactionCase):
    def test_rule_installation(self):
        ...
        self.assertEqual(
            rule.procure_method, "make_to_order", "Procure method is incorrect"
        )
        ...
        self.assertEqual(
            rule.route_id.name, "Replenish on Order (MTO)", "Route name is incorrect"
        )
        self.assertEqual(
            rule.location_dest_id.name, "Production", "Location dest ID is incorrect"
        )
        self.assertEqual(
            rule.location_src_id.name, "Stock", "Location src ID is incorrect"
        )
        self.assertEqual(
            rule.picking_type_id.name, "Repairs", "Picking type ID is incorrect"
        )
