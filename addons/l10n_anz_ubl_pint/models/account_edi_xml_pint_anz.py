...
class AccountEdiXmlPint_Anz(models.AbstractModel):
    ...
    * PINT ANZ Official documentation: https://docs.peppol.eu/poac/aunz/pint-aunz/
    ...
    def _ubl_default_tax_category_grouping_key(
        ...
        if grouping_key["tax_category_code"] == "O" and not grouping_key["percent"]:
    ...
    def _get_party_node(self, vals):
        ...
        if commercial_partner.country_code in ("AU", "NZ"):
            party_node["cac:PartyLegalEntity"]["cbc:CompanyID"]["schemeID"] = (
                commercial_partner.peppol_eas
            )
    ...
    def _export_invoice_constraints(self, invoice, vals):
        ...
        for tax_subtotal in tax_total_node["cac:TaxSubtotal"]:
            if any(
                tax_category_node["cbc:ID"]["_text"] != "O"
                for tax_category_node in tax_subtotal["cac:TaxCategory"]
            ):
                continue
            count_outside_of_scope_breakdown += 1

            if float_is_zero(
                tax_subtotal["cbc:TaxAmount"]["_text"], precision_digits=2
            ):
                continue
            ...
            if vals["supplier"].ref_company_ids[:1].l10n_au_is_gst_registered:
                ...
                constraints["anz_non_gst_supplier_tax_scope"] = self.env._(
                    "Suppliers not registered for GST cannot use taxes that are"
