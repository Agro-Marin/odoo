...
test("Setting section optional should reset some fields", async () => {
    ...
    await contains(
        ".o_data_row:contains(Sec3-sub2) .o_list_section_options button",
    ).click();
    await contains(".o-dropdown-item:contains(Hide Composition)").click();

    await contains(
        ".o_data_row:contains(Sec3-sub1) .o_list_section_options button",
    ).click();
...
test("Unsetting optional section should reset some fields", async () => {
    ...
    expect(".o_data_row:contains(Sec3-sub1-r1)").toHaveClass("text-primary", {
        message:
            "Line under subsection(which is under optional section) should be text-primary",
    });
...
test("drag and drop regular line inside optional section resets some fields", async () => {
    ...
    await contains(".o_data_row:contains(Sec4-r1):first .o_row_handle").dragAndDrop(
        ".o_data_row:contains(Sec3-sub2):first",
    );
    await contains(
        ".o_data_row:contains(Sec3-sub2-r1):first .o_row_handle",
    ).dragAndDrop(".o_data_row:contains(Sec4-sub1):first");
    await contains(
        ".o_data_row:contains(Sec3-sub1-r1):first .o_row_handle",
    ).dragAndDrop(".o_data_row:contains(Sec4-sub1):first");
...
test("Moving Optional Sections to include some lines should set quantity to 0", async () => {
    ...
    await contains(".o_data_row:contains(Sec4):first .o_row_handle").dragAndDrop(
        ".o_data_row:contains(Sec3-r2):first",
    );
    await clickSave();
    await expect.verifySteps(["web_save"]);
});
...
test("Moving Optional Sections to exclude some lines should set quantity to 1", async () => {
    ...
    await contains(".o_data_row:contains(Sec3):first .o_row_handle").dragAndDrop(
        ".o_data_row:contains(Sec4):first",
    );
    await clickSave();
    await expect.verifySteps(["web_save"]);
});
