...
class TestSelfOrderController(SelfOrderCommonTest):
    ...
    def test_get_orders_by_access_token(self):
        ...
        params["order_access_tokens"] = [
            {
                "access_token": order1.access_token,
                "state": order1.state,
                "write_date": (order1.write_date - timedelta(minutes=1)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            },
            {
                ...
                "state": order2.state,
                "write_date": (order2.write_date - timedelta(minutes=1)).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            },
        ]
        ...
        params["order_access_tokens"] = [
            {"access_token": order2.access_token, "write_date": "1970-01-01 00:00:00"}
        ]
        ...
        params["order_access_tokens"] = [
            {"access_token": order2.access_token, "state": "paid"}
        ]
    ...
    def test_access_right_with_message_follower(self):
        ...
        data = self.make_request_to_controller(
            "/pos-self-order/process-order/mobile", params
        )
