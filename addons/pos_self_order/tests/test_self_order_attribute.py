...
class TestSelfOrderAttribute(SelfOrderCommonTest):
    ...
    def test_self_order_always_attribute(self):
        ...
        self.pos_config.write({"iface_available_categ_ids": [(4, pos_categ_chairs.id)]})
        ...
        color_attribute = self.env["product.attribute"].create(
            {
                ...
                "value_ids": [
                    (0, 0, {"name": "White"}),
                    (0, 0, {"name": "Red", "default_extra_price": 5}),
                ],
            }
        )
        ...
        order = self.pos_config.current_session_id.order_ids[0]
        self.assertEqual(
            order.lines[0].product_id.id, chair_product_tmpl.product_variant_ids[0].id
        )
        self.assertEqual(
            order.lines[0].attribute_value_ids.ids,
            chair_product_tmpl.product_variant_ids[
                0
            ].product_template_attribute_value_ids.ids,
        )
        self.assertEqual(order.lines[0].price_unit, 10.0)
        self.assertEqual(
            order.lines[1].product_id.id, chair_product_tmpl.product_variant_ids[1].id
        )
        self.assertEqual(
            order.lines[1].attribute_value_ids.ids,
            chair_product_tmpl.product_variant_ids[
                1
            ].product_template_attribute_value_ids.ids,
        )
            ...
            [
                {
                    ...
                    "value_ids": [
                        ...
                        Command.create(
                            {
                                "name": "Blue",
                                "default_extra_price": 2,
                                "image": b"iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+ip1sAAAAASUVORK5CYII=",
                            }
                        ),
                    ],
                },
    ...
    def test_self_order_multi_check_attribute_with_extra_price(self):
        ...
        attributes = self.env["product.attribute"].create(
            [
                ...
                },
                {
                    ...
                    "value_ids": [
                        Command.create(
                            {"name": "Pen Holder", "default_extra_price": 1.0}
                        ),
                        Command.create(
                            {"name": "Mini Drawer", "default_extra_price": 2.0}
                        ),
                    ],
                },
            ]
        ...
        self_route = self.pos_config._get_self_order_route()
        self.start_tour(
            self_route, "test_self_order_multi_check_attribute_with_extra_price"
        )

        order = self.pos_config.current_session_id.order_ids[0]
        self.assertEqual(
            order.amount_total, 11.62
        )  # 5.10 (price) + 2.0 + 1.0 + 2.0 + 1.52 (tax)
