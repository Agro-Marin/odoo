...
class TestSelfOrderKiosk(SelfOrderCommonTest):
    def test_self_order_kiosk(self):
        ...
        self.start_tour(
            ...
            "self_kiosk_each_counter_takeaway_in",
        )
        ...
        self.pos_config.write(
            {
                "self_ordering_available_language_ids": [
                    Command.link(lang.id) for lang in self.env["res.lang"].search([])
                ],
                "self_ordering_mode": "kiosk",
                "self_ordering_pay_after": "each",
            }
    ...
    def test_self_order_kiosk_combo_qty_max_free(self):
        ...
        self.desks_combo.write(
            {
                "combo_item_ids": [
                    Command.unlink(item.id)
                    for item in self.desks_combo.combo_item_ids[1:]
                ],
                "qty_max": 3,
            }
        )
        self.office_combo.write(
            {
                "combo_ids": [
                    Command.unlink(item.id)
                    for item in self.office_combo.combo_ids
                    if item.id != self.desks_combo.id
                ]
            }
        )
        self.pos_config.write(
        ...
        self.start_tour(
            self.pos_config._get_self_order_route(),
            "test_self_order_kiosk_combo_qty_max_free",
        )
    ...
    def test_self_order_kiosk_to_cashier_payment(self):
        ...
        self.start_tour(
            f"/pos/ui/{cashier_pos.id}",
            "test_pay_unpaid_order_from_kiosk",
            login="admin",
        )
    ...
    def test_self_order_kiosk_ordering_images_public(self):
        def assert_all_image_public():
            self.assertTrue(
                all(img.public for img in self.pos_config.self_ordering_image_home_ids)
            )
            self.assertTrue(
                all(
                    img.public
                    for img in self.pos_config.self_ordering_image_background_ids
                )
            )
    ...
    def test_self_order_parent_category(self):
        ...
        test_child_category1 = self.env["pos.category"].create(
            {"name": "Test Child Category 1", "parent_id": test_parent_category.id}
        )
        test_child_category2 = self.env["pos.category"].create(
            {"name": "Test Child Category 2", "parent_id": test_parent_category.id}
        )
        ...
        self.pos_config.write(
            {
                ...
                "iface_available_categ_ids": [
                    Command.set(
                        [
                            test_parent_category.id,
                            test_child_category1.id,
                            test_child_category2.id,
                        ]
                    )
                ],
