...
class TestSelfOrderMobile(SelfOrderCommonTest):
    def test_self_order_mobile(self):
        ...
        self_route_table = self.pos_config._get_self_order_route(
            table_id=floor.table_ids[0].id
        )
        self.start_tour(
            self_route_table, "self_mobile_auto_table_selection_takeaway_in"
        )
    ...
    def test_self_order_category_with_only_special_products(self):
        ...
        with patch(
            "odoo.addons.point_of_sale.models.pos_config.PosConfig._get_special_products",
    ...
    def test_order_sequence_in_self(self):
        ...
        references = (
            self.env["pos.order"]
            .search([], limit=4, order="id desc")
            .mapped("pos_reference")
        )
        self.assertEqual(
            references,
            [f"{current_year}0-{self.pos_config.id}-00000{4 - i}" for i in range(4)],
        )
        self.assertEqual(self.pos_config.order_backend_seq_id.number_next, 5)

    def test_sub_categories_products_displayed(self):
        miscellaneous = self.env["pos.category"].search(
            [("name", "=", "Miscellaneous")], limit=1
        )
    ...
    def test_mobile_self_order_preparation_changes(self):
        ...
        self.start_tour(
            ...
            "test_mobile_self_order_preparation_changes",
        )
        ...
        self.start_tour(
            ...
            "test_pos_self_order_preparation_changes",
            login="pos_user",
        )
    ...
    def test_self_order_table_sharing(self):
        ...
        self.assertEqual(
            last_order.floating_order_name, f"Self-Order T {table.table_number}"
        )
    ...
    def test_delete_mobile_order_from_backend(self):
        ...
        @http.route(
            "/pos-self-order/test-delete-order-from-backend/",
            auth="public",
            type="jsonrpc",
            website=True,
        )
