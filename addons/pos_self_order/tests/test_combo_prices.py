...
class TestSelfOrderCombo(SelfOrderCommonTest):
    def setUp(self):
        ...
        self.combo_category = self.env["pos.category"].create(
            {"name": "Combo Category"}
        )
        self.combo1 = self.combo_generator(
            "Green", [0.0, 5.0, 10.0], [50.0, 70.0, 90.0], 2, 1
        )
        self.combo2 = self.combo_generator(
            "Red", [0.0, 0.0, 0.0], [40.0, 60.0, 80.0], 5, 0
        )
        self.combo3 = self.combo_generator(
            "Purple", [10.0, 20.0, 30.0], [0, 0, 0], 10, 0
        )
