...
class ProductTemplate(models.Model):
        ...
        combo_products_choice = self.search_read(
            [
                (
                    "id",
                    "in",
                    combo_products.combo_ids.combo_item_ids.product_id.product_tmpl_id.ids,
                ),
    ...
    def _compute_self_order_visible(self):
        active_self_order_configs = (
            self.env["pos.config"]
            .sudo()
            .search_count([("self_ordering_mode", "!=", "nothing")])
        )
        for product in self:
    ...
    def _can_return_content(self, field_name=None, access_token=None):
        if (
            field_name in ["image_512", "image_128"]
            and self.sudo().self_order_available
        ):
            return True
...
class ProductProduct(models.Model):
    ...
    def _send_availability_status(self):
        config_self = (
            self.env["pos.config"]
            .sudo()
            .search([("self_ordering_mode", "!=", "nothing")])
        )
