...
class PosSelfOrderController(http.Controller):
    ...
    def process_order(self, order, access_token, table_identifier, device_type):
        ...
        preset_id = (
            pos_config.env["pos.preset"].browse(preset_id) if preset_id else False
        )
        ...
        if device_type == "kiosk":
            order["floating_order_name"] = (
                f"Table tracker {order['table_stand_number']}"
                if order.get("table_stand_number")
        ...
        order["floating_order_name"] = (
            order.get("floating_order_name") or floating_order_name
        )
        ...
        order["fiscal_position_id"] = (
            preset_id.fiscal_position_id.id
            if preset_id
            else pos_config.default_fiscal_position_id.id
        )
        order["pricelist_id"] = (
            preset_id.pricelist_id.id if preset_id else pos_config.pricelist_id.id
        )
    ...
    def _generate_return_values(self, order, config):
        return {
            ...
            "res.partner": self.env["res.partner"]._load_pos_self_data_read(
                order.partner_id, config
            ),
            "pos.order.line": self.env["pos.order.line"]._load_pos_self_data_read(
                order.lines, config
            ),
            "pos.payment": self.env["pos.payment"]._load_pos_self_data_read(
                order.payment_ids, config
            ),
    ...
    def _verify_line_price(self, lines, pos_config, preset_id):
        ...
        for line in lines:
            ...
            lst_price = (
                pricelist._get_product_price(product, quantity=line.qty)
                if pricelist
            ...
            fiscal_pos = (
                preset_id.fiscal_position_id or pos_config.default_fiscal_position_id
                if preset_id
    ...
    def _compute_combo_price(self, parent_line, pricelist, fiscal_position):
        ...
        parent_lst_price = pricelist._get_product_price(
            parent_line.product_id, parent_line.qty
        )
        child_line_free = []
        child_line_extra = []
        ...
        original_total = sum(
            ...
            for line in child_line_free
            if line.combo_item_id.combo_id.qty_free > 0
        )
        ...
        for index, child in enumerate(child_line_free):
            ...
            price_unit = currency.round(
                combo.base_price * parent_lst_price / unit_devision_factor
            )
    ...
    def validate_partner(
        ...
        existing_partner = (
            pos_config.env["res.partner"].search([("id", "=", int(partner_id))])
            if partner_id
            else False
        )
        ...
        state_id = (
            pos_config.env["res.country.state"].browse(int(state_id))
            if state_id
            else False
        )
            ...
            .create(
                {
                    "name": name,
                    "email": email,
                    "phone_ids": [Command.create({"number": phone, "type": "mobile"})]
                    if phone
                    else [],
                    "street": street,
                    "zip": zip,
    ...
    def _partner_self_data(self, partner_sudo, pos_config):
        return {
            "res.partner": self.env["res.partner"]._load_pos_self_data_read(
                partner_sudo, pos_config
            ),
            "phone.number": self.env["phone.number"]._load_pos_self_data_read(
                partner_sudo.phone_ids, pos_config
            ),
        }
    ...
    def remove_order(self, access_token, order_id, order_access_token):
        ...
        if not pos_order.exists() or not consteq(
            pos_order.access_token, order_access_token
        ):
            raise MissingError(
                self.env._("Your order does not exist or has been removed")
            )

        if pos_order.state != "draft":
            raise Unauthorized(
                self.env._("You are not authorized to remove this order")
            )

        pos_order.remove_from_ui([pos_order.id])

    @http.route(
        "/pos-self-order/get-user-data", auth="public", type="jsonrpc", website=True
    )
    def get_orders_by_access_token(
        ...
        if not table_identifier or pos_config.self_ordering_pay_after == "each":
            ...
            domain = [
                ...
                (
                    "access_token",
                    "not in",
                    [data.get("access_token") for data in order_access_tokens],
                ),
            ]
        ...
        existing_order_tokens = (
            pos_config.env["pos.order"]
            .search([("access_token", "in", access_tokens)])
            .mapped("access_token")
        )
        if deleted_order_tokens := list(access_tokens - set(existing_order_tokens)):
            ...
            pos_config._notify(
                "REMOVE_ORDERS", {"deleted_order_tokens": deleted_order_tokens}
            )
        return self._generate_return_values(orders, pos_config) if orders else {}

    @http.route(
        "/kiosk/payment/<int:pos_config_id>/<device_type>",
        auth="public",
        type="jsonrpc",
        website=True,
    )
    def pos_self_order_kiosk_payment(
        ...
        payment_method_sudo = pos_config.env["pos.payment.method"].browse(
            payment_method_id
        )
        if (
            not order_sudo
            or not payment_method_sudo
            or payment_method_sudo not in order_sudo.config_id.payment_method_ids
        ):
        ...
        return {
            "order": self.env["pos.order"]._load_pos_self_data_read(
                order_sudo, pos_config
            ),
            "payment_status": status,
        }

    @http.route(
        "/pos_self_order/kiosk/increment_nb_print/",
        auth="public",
        type="jsonrpc",
        website=True,
    )
    ...
    @http.route(
        "/pos-self-order/change-printer-status",
        auth="public",
        type="jsonrpc",
        website=True,
    )
    ...
    def _verify_authorization(self, access_token, table_identifier, order):
        ...
        table_sudo = (
            ...
            .sudo()
            .search([("identifier", "=", table_identifier)], limit=1)
        )
        preset = request.env["pos.preset"].sudo().browse(order.get("preset_id"))
        is_takeaway = (
            order and pos_config.use_presets and preset and preset.service_at != "table"
        )
        if (
            not table_sudo
            and not pos_config.self_ordering_mode == "kiosk"
