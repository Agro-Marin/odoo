...
class PosSelfKiosk(http.Controller):
    @http.route(
        ["/pos-self/<config_id>", "/pos-self/<config_id>/<path:subpath>"],
        auth="public",
        website=True,
        sitemap=True,
    )
    def start_self_ordering(
        ...
        use_lna = bool(
            ...
            .get_param("point_of_sale.use_lna")
        )
    ...
    @http.route(
        "/pos-self/data/<config_id>", type="jsonrpc", auth="public", website=True
    )
    ...
    def _get_entry_config_and_table(
        ...
        if access_token:
            ...
            pos_config_sudo = (
                ...
                .sudo()
                .search(
                    [("id", "=", config_id), ("access_token", "=", access_token)],
                    limit=1,
                )
            )
        else:
        ...
        if (
            ...
            and pos_config.self_ordering_mode == "mobile"
        ):
            ...
            table_sudo = table_identifier and (
                ...
                .sudo()
                .search(
                    [("identifier", "=", table_identifier), ("active", "=", True)],
                    limit=1,
                )
            )
        ...
        table = (
            table_sudo.sudo(False).with_company(company).with_user(user)
            if table_sudo
            else False
        )
