...
class WebsiteForum(WebsiteProfile):
    ...
    def _prepare_user_values(self, **kwargs):
        ...
        values["forum_welcome_message"] = request.cookies.get(
        ...
        if kwargs.get("forum"):
            ...
            values["forum"] = request.env["forum.forum"].browse(
                int(kwargs.pop("forum_id"))
            )
    ...
    @http.route(
        ...
        sitemap=True,
        readonly=True,
        list_as_website_content=_lt("Forum"),
    )
    ...
    def questions(
        ...
        **post,
        ...
        question_ids = question_ids[
            (page - 1) * self._post_per_page : page * self._post_per_page
        ]
        ...
        url_args.update(
            {
                ...
                for name, value in zip(
                    ["filters", "search", "my"], [filters, search, my], strict=True
                )
                if value
            }
        )
    ...
    def forum_faq(self, forum, **post):
        values = self._prepare_user_values(
            forum=forum, searches={}, header={"is_guidelines": True}, **post
        )
        return request.render("website_forum.faq", values)

    @http.route(
        ['/forum/<model("forum.forum"):forum>/faq/karma'],
        type="http",
        auth="public",
        website=True,
        sitemap=False,
    ...
    def forum_faq_karma(self, forum, **post):
        values = self._prepare_user_values(
            forum=forum, header={"is_guidelines": True, "is_karma": True}, **post
        )
    ...
    def tags(self, forum, tag_char="", filters="all", search="", **post):
        ...
        domain = [
            ("forum_id", "=", forum.id),
            ("posts_count", "=" if filters == "unused" else ">", 0),
        ]
        ...
        if search:
            ...
            __, details, __ = request.website._search_with_fuzzy(
                ...
                limit=None,
                order=order,
                options={"forum": forum, "domain": search_domain},
            )
            tags = details[0].get("results", tags)

        if filters in ("unused", "most_used"):
            filter_tags = (
                forum.tag_most_used_ids
                if filters == "most_used"
                else forum.tag_unused_ids
            )
    ...
    @http.route(
        ["""/forum/<model("forum.forum"):forum>/<model("forum.post"):question>"""],
        type="http",
        auth="public",
        website=True,
        sitemap=sitemap_forum_post,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/question/<model("forum.post"):question>/edit_answer',
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/question/<model("forum.post"):question>/close',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/question/<model("forum.post"):question>/reopen',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/question/<model("forum.post"):question>/delete',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/question/<model("forum.post"):question>/undelete',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
    ...
    def post_create(self, forum, post_parent=None, **post):
        ...
        new_question = request.env["forum.post"].create(
            {
                ...
                or (post_parent and "Re: %s" % (post_parent.name or ""))
                or "",
        ...
        slug = request.env["ir.http"]._slug
        return request.redirect(
            f"/forum/{slug(forum)}/{slug(post_parent) if post_parent else new_question.id}"
        )

    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/comment',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/toggle_correct',
        type="jsonrpc",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/delete',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/edit',
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/save',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/downvote',
        type="jsonrpc",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/flagged_queue',
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/offensive_posts',
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/closed_posts',
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/validate',
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/refuse',
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/flag',
        type="jsonrpc",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        '/forum/<model("forum.post"):post>/ask_for_mark_as_offensive',
        type="jsonrpc",
        auth="user",
        website=True,
    )
    def post_json_ask_for_mark_as_offensive(self, post, **kwargs):
        ...
        return request.env["ir.ui.view"]._render_template(
            "website_forum.mark_as_offensive", values
        )

    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/ask_for_mark_as_offensive',
        type="http",
        auth="user",
        methods=["GET"],
        website=True,
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/mark_as_offensive',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
        ...
        if not post.get("no_forum"):
            ...
            values.update(
                self._prepare_user_values(
                    forum=forums[0] if len(forums) == 1 else True, **post
                )
            )
            if forums:
    ...
    def _prepare_open_forum_user(self, user, forums, **kwargs):
        ...
        activities = Activity.search(
            [
                (
                    "res_id",
                    "in",
                    Post._search(Domain.OR([question_base_domain, answer_base_domain])),
                ),
                ("model", "=", "forum.post"),
                ("subtype_id", "!=", comment),
            ],
        ...
        posts = {
            x.id: (x.parent_id or x, (x.parent_id and x) or False) for x in posts_ids
        }
        ...
        if search_question:
            ...
            activities_active_tab = (
                "activity" if request.env.user == user else "question"
            )
        values = {
    ...
    def convert_comment_to_answer(self, forum, post, comment, **kwarg):
        ...
        question = post.parent_id or post
        return request.redirect(
            "/forum/%s/%s" % (slug(forum), request.env["ir.http"]._slug(question))
        )

    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/convert_to_comment',
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
    ...
    @http.route(
        '/forum/<model("forum.forum"):forum>/post/<model("forum.post"):post>/comment/<model("mail.message"):comment>/delete',
        type="jsonrpc",
        auth="user",
        website=True,
    )
