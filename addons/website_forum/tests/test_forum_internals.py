...
class TestForumInternals(TestForumCommon):
    ...
    def test_website_forum_count(self):
        ...
        self.assertEqual(
            base_website.forum_count, 2, "Should count default global forums"
        )
        self.assertEqual(website_2.forum_count, 2, "Should count default global forums")
        ...
        self.assertEqual(base_website.forum_count, 4, "3 globals, 1 specific")
        self.assertEqual(website_2.forum_count, 5, "3 globals, 2 specific")

        new_forums.write({"website_id": False})
        self.assertEqual(base_website.forum_count, 6, "6 global forums")
    ...
    def test_website_forum_last_post_id(self):
        ...
        with self.assertQueryCount(1):
            self.assertEqual(
                test_forums.last_post_id.ids, (another_post | new_posts[1]).ids
            )
...
class TestPostInternals(TestForumCommon):
    def test_post_fields(self):
        ...
        forum_questions = Forum.create(
            {"name": "Questions Forum", "mode": "questions", "active": True}
        )
...
class TestTags(TestForumCommon):
    def test_tag_creation_multi_forum(self):
        Post = self.env["forum.post"]
        forum_1 = self.forum
        forum_2 = forum_1.copy({"name": "Questions Forum"})
        ...
        self.assertEqual(
            len(food_tags), 2, "One Food tag should have been created in each forum."
        )
        self.assertIn(
            forum_1,
            food_tags.forum_id,
            "One Food tag should have been created for forum 1.",
        )
        self.assertIn(
            forum_2,
            food_tags.forum_id,
            "One Food tag should have been created for forum 2.",
        )
    ...
    def test_tags_usage_most_used(self):
        ...
        posts_per_tag = [
            (tag_id, post_count)
            for tag_id, post_count in zip(
                self.base_forum.tag_ids, range(2, 9), strict=True
            )
        ]
        vals_list = [
            {
        ...
        self.assertListEqual(
            ...
            [
                tag_id.id
                for tag_id, _ in reversed(posts_per_tag[-MOST_USED_TAGS_COUNT:])
            ],
        )
    ...
    def test_tags_usage_unused(self):
        ...
        self.assertEqual(
            self.base_forum.tag_most_used_ids,
            self.base_forum.tag_ids[:MOST_USED_TAGS_COUNT],
        )
        self.assertEqual(self.base_forum.tag_unused_ids, self.env["forum.tag"])
