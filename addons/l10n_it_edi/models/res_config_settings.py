...
class ResConfigSettings(models.TransientModel):
    ...
    def _create_proxy_user(self, company_id, edi_mode):
        return self.env["account_edi_proxy_client.user"]._register_proxy_user(
            company_id, "l10n_it_edi", edi_mode
        )
    ...
    def _compute_l10n_it_edi_register(self):
        for config in self:
            config.l10n_it_edi_register = (
                config.company_id._l10n_it_get_edi_company().l10n_it_edi_register
            )
        ...
        for config in self:
                ...
                .search(
                    [
                        ...
                        (
                            ...
                            "!=",
                            "demo",
                        ),  # make sure it's a "real" proxy_user (edi_mode is 'test' or 'prod')
                    ],
                    limit=1,
                )
            )
            ...
            if proxy_user and proxy_user.active != config.l10n_it_edi_register:
                ...
                edi_mode = (
                    ...
                    .sudo()
                    .get_param("l10n_it_edi.proxy_user_edi_mode")
                    or "prod"
                )
    ...
    @api.depends(
        "company_id.account_edi_proxy_client_ids",
        "company_id.account_edi_proxy_client_ids.active",
    )
    def _compute_use_root_proxy_user(self):
        for record in self:
            ...
            record.use_root_proxy_user = (
                edi_company == main_company and self.company_id != main_company
            )
