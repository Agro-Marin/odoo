...
class ResPartner(models.Model):
    ...
    def _l10n_it_edi_get_values(self):
        ...
        in_eu = (
            not europe or not self.country_id or self.country_id in europe.country_ids
        )
    ...
    def _l10n_it_onchange_vat(self):
        if self.vat and (
            ...
            self.l10n_it_codice_fiscale = self._l10n_it_edi_normalized_codice_fiscale(
                self.vat
            )
        else:
    ...
    def check_codice_fiscale(self):
        for record in self:
            if record.l10n_it_codice_fiscale and (
                not codicefiscale.is_valid(record.l10n_it_codice_fiscale)
                and not iva.is_valid(record.l10n_it_codice_fiscale)
            ):
                raise UserError(
                    self.env._(
                        "Invalid Codice Fiscale '%s': should be like 'MRTMTT91D08F205J' for physical person and '12345670546' for businesses.",
    ...
    def _l10n_it_edi_export_check(self, checks=None):
        ...
        fields_to_check = {
            ...
            "partner_country_missing": {
                "fields": [("country_id",)],
                "message": self.env._(
                    "Partner(s) should have a Country when used for simplified invoices."
                ),
            },
            "partner_address_missing": {
        ...
        list_view = self.env.ref(
            "l10n_it_edi.res_partner_tree_l10n_it", raise_if_not_found=False
        )
                ...
                if invalid_records := self._filtered_l10n_it_missing_all(fields_tuple):
                    ...
                    errors[f"l10n_it_edi_{key}"] = {
                        ...
                        "action": invalid_records._get_records_action(
                            name=self.env._("Check Partner(s)"), views=views
                        ),
                    }
        return errors
