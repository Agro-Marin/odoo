...
class ResCompany(models.Model):
    ...
    def _check_eco_admin_index(self):
        for record in self:
            if record.l10n_it_has_eco_index and (
                not record.l10n_it_eco_index_office
    ...
    def _check_eco_incorporated(self):
        ...
        for record in self:
            if record.l10n_it_has_eco_index and (
                bool(record.l10n_it_eco_index_share_capital)
                ^ (
                    record.l10n_it_eco_index_sole_shareholder
                    and record.l10n_it_eco_index_sole_shareholder != "NO"
                )
            ):
                raise ValidationError(
                    _(
    ...
    @api.constrains(
        "l10n_it_has_tax_representative", "l10n_it_tax_representative_partner_id"
    )
    ...
    def _compute_l10n_it_edi_proxy_user_id(self):
        for company in self:
            ...
            company.l10n_it_edi_proxy_user_id = (
                edi_company.account_edi_proxy_client_ids.filtered(
            ...
            if not company.l10n_it_edi_proxy_user_id and company.l10n_it_codice_fiscale:
                company.l10n_it_edi_proxy_user_id = self.env[
