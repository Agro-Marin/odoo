...
class AccountMove(models.Model):
    ...
    def _auto_init(self):
        ...
        if not column_exists(self.env.cr, "account_move", "l10n_it_payment_method"):
            create_column(
                self.env.cr, "account_move", "l10n_it_payment_method", "varchar"
            )
        if not column_exists(self.env.cr, "account_move", "l10n_it_document_type"):
            create_column(
                self.env.cr, "account_move", "l10n_it_document_type", "integer"
            )
    ...
    def _compute_l10n_it_payment_method(self):
        if self.env.company.tax_config_id.account_fiscal_country_id.code != "IT":
            return
            ...
            .search(
                [
                    (
                        "matching_number",
                        "in",
                        self.line_ids.filtered("matching_number").mapped(
                            "matching_number"
                        ),
                    ),
        ...
        for move in self:
            matching_numbers = move.line_ids.filtered("matching_number").mapped(
                "matching_number"
            )
            if matching_numbers:
    ...
    def _compute_l10n_it_document_type(self):
        ...
        for move in self:
            if (
                ...
                or move.l10n_it_document_type
                or move.state != "posted"
            ):
                continue

            move.l10n_it_document_type = document_type.get(
                move._l10n_it_edi_get_document_type()
            )
    ...
    def _compute_l10n_it_edi_button_label(self):
        for move in self:
            if move.country_code == "IT" and move.l10n_it_edi_proxy_mode in (
                False,
                "demo",
            ):
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        for move in self:
            move.show_reset_to_draft_button = (
                not move.l10n_it_edi_transaction and move.show_reset_to_draft_button
            )
        ...
        if res is None and self._is_l10n_it_edi_import_file(file_data):
            ...
            xml_tree = (
                ...
                # The file may have a Cades signature, so we try removing it.
                    ...
                    parser, file_data["name"], remove_signature(file_data["raw"])
                )
            )
            if xml_tree is None:
                _logger.info(
                    "Italian EDI invoice file %s cannot be decoded.", file_data["name"]
                )
            return xml_tree
    ...
    def _unwrap_attachment(self, file_data, recurse=True):
        ...
        embedded = self._split_xml_into_new_attachments(
            file_data, tag="FatturaElettronicaBody"
        )
    ...
    def _get_edi_decoder(self, file_data, new=False):
        ...
        if file_data["import_file_type"] == "l10n_it.fatturapa":
            ...
            def decoder(invoice, file_data, new=False):
                ...
                )._l10n_it_edi_import_invoice(invoice, file_data, new)
    ...
    def action_l10n_it_edi_send(self):
            ...
            for error_key, error_data in errors.items():
                ...
                if len(split) > 3 and (
                    ...
                    if action := error_data.get("action"):
                        ...
                        message = (
                            f"{message} - {', '.join(records.mapped('display_name'))}"
                        )
        ...
        self.invalidate_recordset(
            fnames=["l10n_it_edi_attachment_name", "l10n_it_edi_attachment_file"]
        )
        self.message_post(
            attachments=[(self.l10n_it_edi_attachment_name, attachment_vals["raw"])]
        )
    ...
    def action_check_l10n_it_edi(self):
        ...
        if (
            ...
            raise UserError(
                self.env._("This move is not waiting for updates from the SdI.")
            )
    ...
    def _l10n_it_edi_ready_for_xml_export(self):
        ...
        return (
            self.state == "posted"
            and self.company_id.tax_config_id.account_fiscal_country_id.code == "IT"
    ...
    def _l10n_it_edi_add_base_lines_xml_values(
        ...
        for index, (base_line, aggregated_values) in enumerate(
            base_lines_aggregated_values, start=1
        ):
            line = base_line["record"]
            tax_details = base_line["tax_details"]
            discount = base_line["discount"]
            quantity = base_line["quantity"]
            price_subtotal = base_line["price_subtotal"] = tax_details[
            ...
            if not is_downpayment and price_subtotal < 0:
                ...
                if downpayment_moves:
                    downpayment_moves_description = ", ".join(
                        downpayment_moves.mapped("name")
                    )
            ...
            if quantity:
                it_values["prezzo_unitario"] = (
                    base_line["gross_price_subtotal"] / quantity
                )
            else:
            ...
            for values in aggregated_values.values():
                ...
                rates.append(
                    grouping_key["tax_amount_field"]
                    if grouping_key["tax_amount_type_field"] == "percent"
                    else 0.0
                )
                    ...
                    [
                        {
                            ...
                            "riferimento_numero": tax_details[
                                "raw_total_excluded_currency"
                            ],
    ...
    def _l10n_it_edi_is_neg_split_payment(self, tax_data):
        ...
        return (
            ...
            and any(
                ...
                for child_tax in tax_data["group"].children_tax_ids
            )
        )
            ...
            base_line.update(
                {
                    ...
                    "tax_ids": self._l10n_it_edi_search_tax_for_import(
                        self.company_id, 0.0, l10n_it_exempt_reason="N2.2"
                    ),
                }
            )
    ...
    def _l10n_it_edi_get_values(self, pdf_values=None):
        def grouping_function_withholding(base_line, tax_data):
            ...
            tax = tax_data["tax"]
            return {
                "tax_amount_field": -23.0
                if tax.amount in (-11.5, -4.6)
                else tax.amount,
        ...
        def grouping_function_pension_funds(base_line, tax_data):
            ...
            vat_tax = flatten_taxes.filtered(
                lambda t: t._l10n_it_filter_kind("vat") and t.amount >= 0
            )[:1]
            withholding_tax = flatten_taxes.filtered(
                lambda t: (
                    t._l10n_it_filter_kind("withholding") and t.sequence > tax.sequence
                )
            )[:1]
            return {
                "tax_amount_field": -23.0
                ...
                "vat_tax_amount_field": -23.0
                if vat_tax.amount in (-11.5, -4.6)
                else vat_tax.amount,
                ...
                "skip": not tax._l10n_it_filter_kind("pension_fund")
        ...
        for base_line in base_lines:
            ...
            base_line["discount_amount_before_dispatching"] = (
                gross_price_subtotal_before_discount - price_subtotal
            )
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, self._l10n_it_edi_grouping_function_base_lines
        )
        self._l10n_it_edi_add_base_lines_xml_values(
            base_lines_aggregated_values, is_downpayment
        )
        base_lines = sorted(
            base_lines, key=lambda base_line: base_line["it_values"]["numero_linea"]
        )

        # Tax lines.
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, self._l10n_it_edi_grouping_function_tax_lines
        )
        values_per_grouping_key = AccountTax._aggregate_base_lines_aggregated_values(
            base_lines_aggregated_values
        )
        tax_lines = self._l10n_it_edi_get_tax_lines_xml_values(
            base_lines_aggregated_values, values_per_grouping_key
        )

        # Total of the document.
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, self._l10n_it_edi_grouping_function_total
        )
        values_per_grouping_key = AccountTax._aggregate_base_lines_aggregated_values(
            base_lines_aggregated_values
        )
        ...
        buyer_info_values = (
            partner if not is_self_invoice else company.partner_id
        ...
        conversion_rate = (
            float_repr(
                abs(self.amount_total / self.amount_total_signed),
                precision_digits=5,
            )
        ...
        linked_moves = (
            self._get_reconciled_invoices() | self.reversed_entry_id
        ).filtered(lambda move: move.date <= self.date)
        ...
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, grouping_function_withholding
        )
        values_per_grouping_key = AccountTax._aggregate_base_lines_aggregated_values(
            base_lines_aggregated_values
        )
        ...
        # Pension fund.
        base_lines_aggregated_values = AccountTax._aggregate_base_lines_tax_details(
            base_lines, grouping_function_pension_funds
        )
        values_per_grouping_key = AccountTax._aggregate_base_lines_aggregated_values(
            base_lines_aggregated_values
        )
            ...
            pension_fund_values.append(
                {
                    ...
                    "natura": grouping_key["l10n_it_exempt_reason"],
                    "riferimento_amministrazione": html2plaintext(
                        grouping_key["description"]
                    ),
                }
            )
        ...
        for base_line in base_lines:
            ...
            # Withholding
            if any(
                x for x in taxes_data if x["tax"]._l10n_it_filter_kind("withholding")
            ):
                it_values["ritenuta"] = "SI"

            # Enasarco
            enasarco_taxes_data = [
                x for x in taxes_data if x["tax"].l10n_it_pension_fund_type == "TC07"
            ]
            ...
            if not enasarco_taxes_data:
                pension_fund_taxes_data = [
                    x
                    for x in taxes_data
                    if x["tax"]._l10n_it_filter_kind("pension_fund")
                ]
                for pension_fund_tax_data in pension_fund_taxes_data:
                    pension_type = pension_fund_tax_data[
                        "tax"
    ...
    def _l10n_it_edi_is_simplified(self):
        ...
        return bool(
            ...
            and list(buyer._l10n_it_edi_export_check(checks).keys())
            == ["l10n_it_edi_partner_address_missing"]
            and (not buyer.country_id or buyer.country_id.code == "IT")
            and (
                buyer.l10n_it_codice_fiscale
                or (
                    buyer.vat
                    and (buyer.vat[:2].upper() == "IT" or buyer.vat[:2].isdecimal())
                )
            )
    ...
    def _l10n_it_edi_features_for_document_type_selection(self):
        ...
        return {
            ...
            "tax_tags": {
                tag
                for tag in self.line_ids.tax_tag_ids.mapped(
                    lambda x: (x.name or "").upper()
                )
                if tag
            },
            "downpayment": self._is_downpayment(),
            "services_or_goods": services_or_goods,
            "goods_in_italy": services_or_goods == "consu"
            and self._l10n_it_edi_goods_in_italy(),
    ...
    def _l10n_it_edi_get_document_type(self):
        ...
        ) in self._l10n_it_edi_document_type_mapping().items():
            # By using a generator instead of a list, we can avoid some comparisons
            if all(
                compare(invoice_values, document_type_features[k])
                for k, invoice_values in invoice_features.items()
                if k in document_type_features
            ):
    ...
    def cron_l10n_it_edi_download_and_update(self):
        ...
        if retrigger:
            ...
            self.env.ref(
                "l10n_it_edi.ir_cron_l10n_it_edi_download_and_update"
            )._trigger()
    ...
    def _l10n_it_edi_process_downloads(self, invoices_data, proxy_user):
        ...
        for id_transaction, invoice_data in invoices_data.items():
            ...
            if (
                filename_and_decrypted_content
                := self._l10n_it_edi_check_and_decrypt_content(
        ...
        if attachment_vals:
            attachments = (
                ...
                .with_company(proxy_user.company_id)
            ...
            moves = self.with_company(proxy_user.company_id).create(
                [{}] * len(files_data)
            )
    ...
    def _l10n_it_edi_check_and_decrypt_content(
        self, filename, content, key, proxy_user
    ):
        ...
        Attachment = (
            self.env["ir.attachment"].sudo().with_company(proxy_user.company_id)
        )
    ...
    def _l10n_it_edi_search_partner(
        self, company, vat, codice_fiscale, email, destination_code=None
    ):
        ...
        for domain in [
            ...
            and [
                ("vat", "ilike", vat),
                ("l10n_it_pa_index", "ilike", destination_code),
            ],
            ...
            and [
                (
                    ...
                    "in",
                    ("IT" + codice_fiscale, codice_fiscale),
                )
            ],
    ...
    def _l10n_it_edi_search_tax_for_import(
        ...
        reversed_tax_tag = self._l10n_it_edi_exempt_reason_tag_mapping().get(
        ...
        if not reversed_tax_tag:
            ...
            domain += [
                (
                    ...
                    "=",
                    reversed_tax_tag.lower(),
                )
            ]
            taxes = (
                self.env["account.tax"]
                .search(domain, order="amount desc")
                .filtered(
                    lambda tax: any(
                        rep_line.factor_percent < 0
                        for rep_line in tax.invoice_repartition_line_ids
    ...
    def _l10n_it_edi_get_extra_info(
        ...
        extra_info = {
            "simplified": self.env[
                "account.move"
            ]._l10n_it_edi_is_simplified_document_type(document_type),
            "type_tax_use_domain": [
                ("type_tax_use", "=", "purchase" if incoming else "sale")
            ],
        }
        message_to_log = []
        ...
        for withholding in withholding_elements or []:
            ...
            withholding_type = (
                tipo_ritenuta.text if tipo_ritenuta is not None else "RT02"
            )
            ...
            if float_compare(withholding_percentage, -23.0, precision_digits=2) == 0:
                ...
                withholding_percentage = -float_round(
                    (importo_ritenuta / prezzo_totale) * 100, 1
                )
        ...
        for pension_fund in pension_fund_elements or []:
            ...
            pension_fund_type = (
                pension_fund_type.text if pension_fund_type is not None else ""
            )
        ...
        with self._get_edi_creation() as move:
            ...
            for incoming in incoming_possibilities:
                company_role, partner_role = (
                    ("buyer", "seller") if incoming else ("seller", "buyer")
                )
                ...
                if (
                    codice_fiscale
                    and codice_fiscale.casefold()
                    in (company.l10n_it_codice_fiscale or "").casefold()
                ):
                    break
            else:
            ...
            email = (
                get_text(tree, "//DatiTrasmissione//Email")
                if partner_info["role"] == "seller"
                else ""
            )
            destination_code = (
                get_text(tree, "//CodiceDestinatario")
            ...
            if partner := move._l10n_it_edi_search_partner(
                ...
                message = Markup("<br/>").join(
                    (
                        self.env._(
                            "Partner not found, useful informations from XML file:"
                        ),
            ...
            if document_date := get_date(tree, ".//DatiGeneraliDocumento/Data"):
                ...
                message_to_log.append(
                    self.env._("Document date invalid in XML file: %s", document_date)
                )
            ...
            if due_date := get_date(
                ...
                message_to_log.append(
                    self.env._(
                        "Payment due date invalid in XML file: %s", str(due_date)
                    )
                )
            ...
            if amount_total := sum(
                float(x) for x in get_text(tree, ".//ImportoPagamento", many=True) if x
            ):
                message_to_log.append(
                    self.env._("Total amount from the XML File: %s", amount_total)
                )

            # l10n_it_payment_method
            if payment_method := get_text(
                data["xml_tree"], "//DatiPagamento/DettaglioPagamento/ModalitaPagamento"
            ):
                if (
                    payment_method
                    in move.env[
            ...
            for element in tree.xpath(".//Allegati"):
                ...
                move.sudo().message_post(
                    body=(self.env._("Attachment from XML")),
                    attachments=[
                        (
            ...
            for additional_data_element in tree.xpath("//AltriDatiGestionali"):
                ...
                if data_kind == "cassa-prev":
                    data_text = additional_data_element.xpath("./RiferimentoTesto")[
                        0
                    ].text.lower()
    ...
    def _l10n_it_edi_import_line(self, element, move_line, extra_info=None):
        ...
        if elements_code := element.xpath(".//CodiceArticolo"):
            for element_code in elements_code:
                ...
                if partner:
                    ...
                    if (
                        product_supplier
                        and len(product_supplier) == 1
                        and product_supplier.product_id
                    ):
        ...
        if not extra_info["simplified"]:
            ...
            if float_compare(percentage, -23.0, precision_digits=2) == 0 and (
                ...
                for riepilogo in body_tree.xpath(".//DatiRiepilogo"):
                    if get_float(riepilogo, ".//AliquotaIVA") == percentage and (
                        imponibile := get_float(riepilogo, ".//ImponibileImporto")
                    ):
                        percentage = -float_round(
                            23.0 * (imponibile / prezzo_totale), 1
                        )
                        break
        ...
        if percentage is not None:
            ...
            if move_line.product_id:
                ...
                tax_scope = (
                    "service" if move_line.product_id.type == "service" else "consu"
                )
        ...
        if (
            ...
            if (
                ...
                message = Markup("<br/>").join(
                    (
                        self.env._(
                            "The amount_total %(current_total)s is different than PrezzoTotale %(expected_total)s for '%(move_name)s'",
        ...
        for other_data_element in element.xpath(".//AltriDatiGestionali"):
            ...
            if not data_kind_element or not text_element:
                continue
            data_kind, data_text = (
                data_kind_element[0].text.lower(),
                text_element[0].text.lower(),
            )
            if data_kind == "cassa-prev" and (
                "enasarco" in data_text or "tc07" in data_text
            ):
                ...
                enasarco_percentage = -self.env.company.currency_id.round(
                    enasarco_amount / price_subtotal * 100
                )
    ...
    def _l10n_it_edi_format_errors(self, header, errors):
        return Markup('{}<ul class="mb-0">{}</ul>').format(
            ...
            Markup().join(
                nl2br_enclose(" ".join(error.split()), "li") for error in errors
            ),
        )
    ...
    def _prepare_info_message(self, tree, tags):
        ...
        for tag in tags if isinstance(tags, list) else [tags]:
            for el in tree.xpath(tag):
                result += self._l10n_it_edi_format_errors(
                    "", [f"{subel.tag}: {subel.text}" for subel in el.iter()]
                )
        return result
    ...
    def _l10n_it_edi_export_data_check(self):
        ...
        full = moves_full.mapped("commercial_partner_id").filtered(
            lambda p: p not in companies_partners
        )
        simplified = moves_simplified.mapped("commercial_partner_id").filtered(
            lambda p: p not in companies_partners | full
        )
        representatives = companies.mapped(
            "l10n_it_tax_representative_partner_id"
        ).filtered(lambda p: p not in companies_partners | simplified | full)
        ...
        return {
            ...
            **(simplified | full)._l10n_it_edi_export_check(
    ...
    def _l10n_it_edi_base_export_check(self):
        ...
        if pa_moves := self.filtered(
            lambda move: (
                move.commercial_partner_id._l10n_it_edi_is_public_administration()
            )
        ):
            if moves := pa_moves.filtered(
                lambda move: not move.l10n_it_origin_document_type
            ):
                message = self.env._(
            ...
            if moves := pa_moves.filtered(
                lambda move: (
                    ...
                    and move.l10n_it_origin_document_date > fields.Date.today()
                )
            ):
                message = self.env._(
        ...
        if pa_moves := self.filtered(
            lambda move: len(move.commercial_partner_id.l10n_it_pa_index or "") == 7
        ):
            if moves := pa_moves.filtered(
                lambda move: (
                    not move.l10n_it_origin_document_type
                    and (move.l10n_it_cig or move.l10n_it_cup)
                )
            ):
                message = self.env._(
    ...
    def _l10n_it_edi_export_taxes_check(self):
        ...
        for kind_code, kind_desc, min_len in (
            ("vat", self.env._("VAT"), 1),
            ("withholding_no_enasarco", self.env._("Withholding"), 0),
            ("pension_fund", self.env._("Pension Fund"), 0),
        ):
            errors.update(
                self._l10n_it_edi_check_lines_for_tax_kind(
                    kind_code, kind_desc, min_len
                )
            )
        return errors
    ...
    def _l10n_it_edi_check_lines_for_tax_kind(self, kind_code, kind_desc, min_len=1):
        ...
        if self.invoice_line_ids.filtered(
            lambda line: (
                ...
                and not (
                    min_len <= len(line.tax_ids._l10n_it_filter_kind(kind_code)) <= 1
                )
            ),
        ):
            return {
                f"l10n_it_edi_move_{kind_code}_tax_per_line": {
                    "message": self.env._(
                        ...
                        number=self.env._("exactly")
                        if min_len == 1
                        else self.env._("at most"),
                        kind=kind_desc,
                    ),
                    **(
                        {
                            ...
                            "action": self._get_records_action(
                                name=self.env._("Check taxes on invoice lines")
                            ),
                        }
                        if len(self) > 1
                        else {}
    ...
    def _l10n_it_edi_generate_filename(self):
        ...
        n = (
            ...
            .next_by_code("l10n_it_edi.fattura_filename")
        )
        if not n:
        ...
        for move in self:
            ...
            try:
                ...
                if "error" in response:
                    ...
                    error_message = self._l10n_it_edi_upload_error_message(
                        error_code, error_description
                    )
                    ...
                    if response.get("id_transaction") == "demo":
                        message = self.env._(
                            "We are simulating the sending of the e-invoice file %s, as we are in demo mode.",
                            filename,
                        )
                    elif response.get("signed"):
                        message = self.env._(
                            "The e-invoice file %s was signed and sent to the SdI for processing.",
                            filename,
                        )
                    else:
                        message = self.env._(
                            "The e-invoice file %s was sent to the SdI for processing.",
                            filename,
                        )
    ...
    def _l10n_it_edi_upload(self, files):
        ...
        return {
            file["filename"]: self._l10n_it_edi_upload_single(file)
    ...
    def _l10n_it_edi_update_send_state(self):
        ...
        if proxy_user.edi_mode == "demo":
            for move in self:
                ...
                self._l10n_it_edi_write_send_state(
                    ...
                    message=self.env._(
                        "The e-invoice file %s has been sent in Demo EDI mode.",
                        filename,
                    ),
                )
            return
        ...
        try:
            ...
            raise UserError(
                self.env._(
                    "An error occurred while downloading updates from the Proxy Server: (%(code)s) %(message)s",
                    code=pe.code,
                    message=pe.message,
                )
            ) from pe
        ...
        for notification in notifications.values():
            ...
            if encrypted_update_content and encryption_key:
                notification["xml_content"] = proxy_user._decrypt_data(
                    encrypted_update_content, encryption_key
                )
        ...
        for move in self:
            ...
            transformed_notification = move._l10n_it_edi_transform_notification(
                parsed_notification
            )
            ...
            if (
                ...
                and (
                    id_transaction_to_ack := transformed_notification.get(
    ...
    def _l10n_it_edi_parse_notification(self, notification):
        ...
        date_arrival = (
            get_datetime(decrypted_update_content, ".//DataOraRicezione")
        ...
        parsed_notification.update(
            {
                ...
                "l10n_it_edi_transaction": False
                if new_state in (False, "rejected")
                else self.l10n_it_edi_transaction,
    ...
    def _l10n_it_edi_get_message(self, transformed_notification):
        ...
        if new_state == "rejected":
            ...
            for error_code, error_description in transformed_notification["errors"]:
                ...
                if error_code == DUPLICATE_MOVE:
                    error_description_copy = self.env._(
                        ...
                        file=filename,
                        message=error_description_copy,
                    )
            ...
            return self._l10n_it_edi_format_errors(
                self.env._("The e-invoice has been refused by the SdI."),
                error_descriptions,
            )
            ...
            pa_specific_map = {
                ...
                "accepted_by_pa_partner_after_expiry": nl2br(
                    self.env._(
        ...
        new_state_messages_map = {
            False: self.env._(
                "The e-invoice file %s has not been found on the EDI Proxy server.",
                filename,
            ),
            "processing": nl2br(
                self.env._(
    ...
    def _get_pension_fund_tax_for_line(self, element, extra_info):
        ...
        if match := re.match(
            r"(?P<kind>TC\d{2}) \((?P<tax_rate>\d+)%\)", reference_text
        ):
            rate = float(match.group("tax_rate"))
            match_kind = (
                match.group("kind") == pension_fund_tax.l10n_it_pension_fund_type
            )
            match_rate = (
                float_compare(rate, pension_fund_tax.amount, precision_digits=2) == 0
            )
