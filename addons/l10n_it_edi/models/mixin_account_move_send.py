...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _is_it_edi_applicable(self, move):
        return all(
            [
                move.company_id.tax_config_id.account_fiscal_country_id.code == "IT"
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        if it_moves := moves.filtered(
            lambda m: (
                "it_edi_send" in moves_data[m]["extra_edis"]
                or moves_data[m]["invoice_edi_format"] == "it_edi_xml"
            )
        ):
    ...
    def _get_invoice_extra_attachments(self, invoice):
        # EXTENDS 'account'
        return super()._get_invoice_extra_attachments(invoice) + self.env[
            "ir.attachment"
        ].search(
            [
    ...
    def _hook_invoice_document_before_pdf_report_render(self, invoice, invoice_data):
        ...
        if (
            "it_edi_send" in invoice_data["extra_edis"]
            and not invoice.l10n_it_edi_attachment_file
        ) or (
            invoice_data["invoice_edi_format"] == "it_edi_xml"
    ...
    def _hook_invoice_document_after_pdf_report_render(self, invoice, invoice_data):
        ...
        if invoice_data.get("pdf_attachment_values") and (
            (
                "it_edi_send" in invoice_data["extra_edis"]
                and not invoice.l10n_it_edi_attachment_file
            )
            ...
            invoice_data["l10n_it_edi_values"] = (
                invoice._l10n_it_edi_get_attachment_values(
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for move, move_data in moves_data.items():
            if attachment := move.l10n_it_edi_attachment_file:
                attachments_vals[move] = {
                    "name": move.l10n_it_edi_attachment_name,
        ...
        for move, move_data in moves_data.items():
            ...
            if attachment_data.get("signed") and (
                signed_data := attachment_data.get("signed_data")
            ):
                attachment["raw"] = signed_data.encode()
                # Show that those moves couldn't be sent
            if "error_message" in attachment_data:
                moves_data[move]["error"] = {
                    "error_title": attachment_data["error_message"]
                }
