...
WITHHOLDING_REASON_SELECTION = [
    ...
    (
        "C",
        "[C] Income from work as part of association groups or other cooperation determined by contracts",
    ),
    ...
    (
        "N",
        "[N] Compensation for travel, expenses, prizes, or other compensations for amateur sport activities",
    ),
    (
        "O",
    ...
    (
        "V2",
        "[V2] Income from unusual work activities from door-to-door sales representatives",
    ),
...
class AccountTax(models.Model):
    ...
    def _onchange_l10n_it_withholding_type(self):
        ...
        taxes_to_be_cleared = self.filtered(
            lambda tax: (
                tax.l10n_it_withholding_reason and not tax.l10n_it_withholding_type
            )
        )
    ...
    def _check_withholding(self):
        for tax in self:
            ...
            if (tax.l10n_it_withholding_type == "RT04") ^ (
            ...
            if (
                tax.l10n_it_withholding_type == "RT04"
