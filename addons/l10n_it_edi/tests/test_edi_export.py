...
class TestItEdiExport(TestItEdi):
    ...
    def test_invoice_more_400_simplified(self):
        ...
        self.assertEqual(
            ...
            list(invoice._l10n_it_edi_export_data_check().keys()),
        )
    ...
    def test_send_and_print_invoice_with_fallback_pdf(self):
        self.italian_partner_a.zip = (
            False  # invalid configuration for partner -> proforma pdf
        )
        invoice = (
            self.env["account.move"]
            .with_company(self.company)
    ...
    def test_export_foreign_currency(self):
        ...
        self._assert_export_invoice(
            invoice, "export_foreign_currency_simple_discount.xml"
        )
        ...
        invoice.action_post()
        self._assert_export_invoice(
            invoice, "export_foreign_currency_global_simple_discount.xml"
        )
        ...
        invoice.action_post()
        self._assert_export_invoice(
            invoice, "export_foreign_currency_global_discount.xml"
        )
    ...
    def test_export_invoice_with_two_downpayments(self):
        ...
        invoice.action_post()
        self._assert_export_invoice(
            invoice, "test_export_invoice_with_two_downpayments.xml"
        )
    ...
    def test_send_prezzo_unitario_converted_to_company_currency(self):
        ...
        self._assert_export_invoice(
            invoice, "prezzio_unitario_converted_company_currency.xml"
        )
    ...
    def test_export_XML_product_with_multiline_description_field(self):
        ...
        invoice.action_post()
        self._assert_export_invoice(
            invoice, "invoice_with_multiple_product_description_fields.xml"
        )
    ...
    def test_export_invoice_with_rounding_lines_value(self):
        ...
        self.env["res.config.settings"].create(
            {"company_id": self.company.id, "group_cash_rounding": True}
        )

        cash_rounding_add_invoice_line = (
            ...
            .create(
                {
                    ...
                    "profit_account_id": self.company_data_2[
                        "default_account_revenue"
                    ].id,
                    "loss_account_id": self.company_data_2[
    ...
    def test_export_invoice_exclude_postdated_moves(self):
        ...
        (invoice_b.line_ids + credit_note.line_ids).filtered(
            lambda line: line.account_type in ("asset_receivable")
        ).reconcile()
