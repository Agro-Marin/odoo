...
class TestItAccountMoveSend(TestItEdi, TestAccountMoveSendCommon):
            ...
            .search(
                [
                    ...
                    (
                        "res_field",
                        "in",
                        ("invoice_pdf_report_file", "l10n_it_edi_attachment_file"),
                    ),
                ]
            )
        )
    ...
    def generate_l10n_it_edi_send_attachments(self, invoices, from_cron=False):
        moves_data = {
            invoice: self.env["mixin.account.move.send"]._get_default_sending_settings(
    ...
    def test_invoice_multi_without_l10n_it_edi_xml_export(self):
        # Prepare
        invoice1, invoice2 = self.init_invoice(
            self.italian_partner_a + self.italian_partner_a
        )
        (self.italian_partner_a + self.italian_partner_b).with_company(
            invoice1.company_id
    ...
    def test_invoice_multi_with_l10n_it_edi_xml_export(self):
        ...
        with patch(
            ...
            self.env["mixin.account.move.send"]._generate_and_send_invoices(
                invoice1 + invoice2, sending_methods=["email"]
            )
    ...
    def test_invoice_send_with_multiple_company(self):
        ...
        second_company.write(
            {
                "vat": "IT12345670017",
                "phone_ids": [
                    Command.create({"number": "0266766700", "type": "landline"})
                ],
                "email": "test@test.it",
        ...
        with patch(
            ...
            self.env["mixin.account.move.send"].with_context(
                allowed_company_ids=[second_company.id, self.company.id]
            )._generate_and_send_invoices(
                invoice2 + invoice1, sending_methods=["email"]
            )
            self.assertEqual(mock_check.call_count, 2)
            res_call_invoice1, res_call_invoice2 = mock_check.call_args_list
            res_invoice1, res_invoice2 = (
                res_call_invoice2[0][0],
                res_call_invoice1[0][0],
            )
    ...
    def test_l10n_it_edi_send_success(self):
        ...
        with patch(
            ...
            attachments_vals = {
                invoice: {
                    "name": invoice.l10n_it_edi_attachment_name,
                    "raw": invoice.l10n_it_edi_attachment_file,
                }
            }
    ...
    def test_l10n_it_edi_send_proxy_error(self):
        ...
        with patch(
            ...
            results = invoice._l10n_it_edi_send(attachments_vals)
            proxy_error["error_message"] = invoice._l10n_it_edi_upload_error_message(
                proxy_error["error"], proxy_error["error_description"]
            )

            self.assertEqual(mock_check.call_count, 1)
            self.assertEqual(
                results, {invoice.l10n_it_edi_attachment_name: proxy_error}
            )
    ...
    def test_l10n_it_edi_send_proxy_exception(self):
        ...
        with patch(
            "odoo.addons.l10n_it_edi.models.account_move.AccountMove._l10n_it_edi_upload_single",
            side_effect=AccountEdiProxyError("error_code", message="error_description"),
        ) as mock_check:
            attachments_vals = {
                invoice: {
                    "name": invoice.l10n_it_edi_attachment_name,
    ...
    def test_l10n_it_edi_send_from_cron(self):
        invoices = self.init_invoice(self.italian_partner_a) | self.init_invoice(
            self.italian_partner_a
        )
        ...
        with patch(
            "odoo.addons.l10n_it_edi.models.account_move.AccountMove._l10n_it_edi_upload_single",
            side_effect=_l10n_it_edi_upload_single,
            autospec=True,
        ) as mock_check:
            ...
            self.assertEqual(
                invoices[0].l10n_it_edi_transaction, success["id_transaction"]
            )
    ...
    def test_enasarco_no_warnings(self):
        ...
        ref = (
            ...
            .with_company(self.proxy_user.company_id)
            .ref
        )
        ...
        invoice = self.init_invoice(
            partners=self.partner_a, taxes=ref("22v") | ref("23vwo") | ref("enasarcov")
        )
        wizard = self.create_send_and_print(invoice, sending_methods=["l10n_it_edi"])
        non_info_alerts = {
            k: v for k, v in wizard.alerts.items() if v.get("level") != "info"
        }
