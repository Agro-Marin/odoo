...
class TestWithholdingAndPensionFundTaxes(TestItEdi):
    ...
    def setUpClass(cls):
            ...
            "tax_ids": [
                (
                    ...
                    [
                        ...
                        cls.company.tax_config_id.account_sale_tax_id.id,
                    ],
                )
            ],
        }
        ...
        cls.withholding_sale_tax_23_base50_invoice = (
            cls.env["account.move"]
            .with_company(cls.company)
            .create(
                {
        ...
        cls.withholding_sale_tax_23_base20_invoice = (
            cls.env["account.move"]
            .with_company(cls.company)
            .create(
                {
    ...
    def test_withholding_tax_constraints(self):
        ...
        with self.assertRaises(ValidationError):
            self.company.tax_config_id.account_sale_tax_id.l10n_it_withholding_type = "RT02"
    ...
    def test_withholding_taxes_export(self):
        ...
        self._assert_export_invoice(
            self.withholding_tax_invoice, "withholding_tax_invoice.xml"
        )

    def test_withholding_taxes_import(self):
        invoice = self._assert_import_invoice(
            "IT00470550013_withh.xml",
            [
                {
        ...
        for line in invoice.line_ids.filtered(
            lambda x: x.name in [data[0] for data in self.invoice_lines]
        ):
            withholding_taxes = line.tax_ids.filtered(
                lambda x: x.l10n_it_withholding_type
            )
            pension_fund_taxes = line.tax_ids.filtered(
                lambda x: x.l10n_it_pension_fund_type
            )
            vat_taxes = line.tax_ids - withholding_taxes - pension_fund_taxes
            self.assertEqual(
                [1, 1, 0],
                [len(x) for x in (vat_taxes, withholding_taxes, pension_fund_taxes)],
            )
    ...
    def test_pension_fund_taxes_export(self):
        ...
        """
        self._assert_export_invoice(
            self.pension_fund_tax_invoice, "pension_fund_tax_invoice.xml"
        )

    def test_pension_fund_taxes_import_assosoftware_tag(self):
        invoice = self._assert_import_invoice(
            "IT00470550013_pfund.xml",
            [
                {
                    "invoice_date": fields.Date.from_string("2022-03-24"),
                    "amount_untaxed": 750.0,
        ...
        for line in invoice.line_ids.filtered(lambda x: x.display_type == "product"):
            self.assertEqual(
                ...
                (
                    ...
                    | self.company.tax_config_id.account_purchase_tax_id
                ),
            )

    def test_pension_fund_taxes_import(self):
        invoice = self._assert_import_invoice(
            "IT00470550013_pfun2.xml",
            [
                {
                    "invoice_date": datetime.date(2022, 3, 24),
                    "invoice_date_due": datetime.date(2022, 3, 24),
    ...
    def test_enasarco_tax_export(self):
        ...
        """
        self._assert_export_invoice(
            self.enasarco_tax_invoice, "enasarco_tax_invoice.xml"
        )

    def test_enasarco_tax_import(self):
        invoice = self._assert_import_invoice(
            "IT00470550013_enasa.xml",
            [
                {
                    "invoice_date": fields.Date.from_string("2022-03-24"),
                    "amount_untaxed": 750.0,
        ...
        for line in invoice.line_ids.filtered(
            ...
            enasarco_imported_tax = line.tax_ids.filtered(
                lambda x: x.l10n_it_pension_fund_type == "TC07"
            )
            self.assertEqual(self.enasarco_purchase_tax, enasarco_imported_tax)
            self.assertEqual(-8.5, enasarco_imported_tax.amount)
            self.assertEqual(
                self.withholding_purchase_tax_23_base50 | enasarco_imported_tax,
    ...
    def test_withholding_tax_23_base50_export(self):
        ...
        """
        self._assert_export_invoice(
            self.withholding_sale_tax_23_base50_invoice, "withholding_23_ret50.xml"
        )

    def test_withholding_tax_23_base50_import(self):
        invoice = self._assert_import_invoice(
            "IT00470550013_ret50.xml",
            [
                {
                    "invoice_date": fields.Date.from_string("2022-03-24"),
                    "amount_untaxed": 750.0,
        ...
        for line in invoice.line_ids.filtered(
            ...
            withholding_tax = line.tax_ids.filtered(
                lambda x: x.l10n_it_withholding_type
            )
    ...
    def test_withholding_tax_23_base20_export(self):
        ...
        """
        self._assert_export_invoice(
            self.withholding_sale_tax_23_base20_invoice, "withholding_23_ret20.xml"
        )

    def test_withholding_tax_23_base20_import(self):
        invoice = self._assert_import_invoice(
            "IT00470550013_ret20.xml",
            [
                {
                    "invoice_date": fields.Date.from_string("2022-03-24"),
                    "amount_untaxed": 750.0,
