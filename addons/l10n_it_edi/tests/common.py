...
class TestItEdi(AccountTestInvoicingCommon):
    ...
    def repartition_lines(cls, *lines):
        ...
        return [(5, 0, 0)] + [
            (0, 0, {**line.__dict__, "tag_ids": cls.get_tag_ids(line.tag_ids)})
            for line in lines
        ]
        ...
        cls.private_key_id = cls.env["certificate.key"].create(
            {
                ...
                "content": base64.b64encode(
                    file_open("l10n_it_edi/data/pkey.key", "rb").read()
                ),
            }
        )
    ...
    def _assert_import_invoice(
        self, filename, expected_values_list, xml_to_apply=None, move_type="in_invoice"
    ):
        ...
        if xml_to_apply:
            tree = self.with_applied_xpath(
                etree.fromstring(import_content), xml_to_apply
            )
        ...
        purchase_journal = self.company_data_2[journal_type].with_context(
        ...
        for expected_values in expected_values_list:
            ...
            if "invoice_line_ids" in invoice_values:
                expected_invoice_line_ids_values_list += invoice_values.pop(
                    "invoice_line_ids"
                )
        ...
        try:
            ...
            if expected_invoice_line_ids_values_list:
                self.assertRecordValues(
                    invoices.invoice_line_ids, expected_invoice_line_ids_values_list
                )
        except AssertionError as ae:
            ae.args = (ae.args[0] + f"\nFile used for comparison: {filename}",)
            raise
