...
class IrActionsReport(models.Model):
    ...
    def _get_value_from_path(self, form_field, order, order_line=None):
        ...
        :param recordset order_line: sale.order.line from where the values need to be taken
                                     (optional, only for document.document)
        ...
        path = path.split(".")  # ['order_id', 'line_ids', 'product_id', 'name']
    ...
    @api.model
    def _get_custom_value_from_order(
        self, document, form_field_name, order, order_line
    ):
        ...
        custom_form_fields = base_values.get(str(document.id), {}).get(
            "custom_form_fields", {}
        )
