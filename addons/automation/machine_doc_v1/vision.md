...
* **Load it lazily.** A top-level import in `web.assets_backend` resolves on every
  ...
  a single dynamic `import()`. Only `automation.rule` form views need a canvas, so
...
| Visual layer | **none — `web_flow` was deleted as unused, 2026-04-21** | Integrated with `automation.rule` form |
...
| `subflow` | Invoke another `automation.rule` rule, wait for its runtime to complete. |
