...
class L10nTWITestEdiPosOrderUI(L10nTWITestEdiPosOrder, TestPointOfSaleHttpCommon):
    ...
    def test_01_ecpay_b2c_check_mobile_barcode_tour(self):
        ...
        with (
            patch(
                "odoo.addons.l10n_tw_edi_ecpay_pos.models.pos_session.PosSession.l10n_tw_edi_check_mobile_barcode",
                new=mock_check_mobile_barcode,
            ),
            patch(CALL_API_METHOD, new=self.mock_ecpay_api),
        ):
    ...
    def test_02_ecpay_check_love_code_tour(self):
        ...
        with (
            patch(
                "odoo.addons.l10n_tw_edi_ecpay_pos.models.pos_session.PosSession.l10n_tw_edi_check_love_code",
                new=mock_check_love_code,
            ),
            patch(CALL_API_METHOD, new=self.mock_ecpay_api),
        ):
            self.main_pos_config.open_ui()
    ...
    def test_03_ecpay_check_print_invoice_tour(self):
        ...
        with (
            patch(CALL_API_POS, new=self.mock_ecpay_api),
            patch(CALL_API_METHOD, new=self.mock_ecpay_api),
        ):
            self.main_pos_config.open_ui()
            self.start_tour(
                "/pos/ui?config_id=%d" % self.main_pos_config.id,
                "l10n_tw_edi_ecpay_pos.ecpay_check_print_invoice_tour",
                login="accountman",
            )
    ...
    def test_04_ecpay_toggle_invoice_tour(self):
        ...
        with (
            ...
            self.start_tour(
                "/pos/ui?config_id=%d" % self.main_pos_config.id,
                "l10n_tw_edi_ecpay_pos.ecpay_toggle_invoice_tour",
                login="accountman",
            )
