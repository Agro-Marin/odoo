...
class PoSOrder(models.Model):
    ...
    def _prepare_invoice_vals(self):
        ...
        if len(companies) > 1:
            raise UserError(
                self.env._(
                    "POS orders from different companies cannot be consolidated into one invoice."
                )
            )

        source_order = self[0]
        if source_order.company_id.tax_config_id.account_fiscal_country_id.code == "TW":
            if len(self) > 1 and any(
                order.config_id.is_ecpay_enabled for order in self
            ):
                raise UserError(
                    self.env._(
                        "With EcPay enabled, POS orders cannot be consolidated into one invoice."
                    )
                )
            vals.update(
                {
    ...
    def _l10n_tw_edi_set_invoice_month(self, create_date):
        ...
        """
        invoice_create_date = datetime.datetime.strptime(
    ...
    def l10n_tw_edi_get_uniform_invoice(self):
        ...
        if not invoice:
            return {
                "ecpay_error": self.env._("No invoice found linked to this record.")
            }
        ...
        response_data = call_ecpay_api(
            "/GetIssue", json_data, self.company_id, invoice.l10n_tw_edi_is_b2b
        )
        json_response = {}
        ...
        json_response.update(
            {
                "invoice_month": self._l10n_tw_edi_set_invoice_month(parsed_create_date)
                if parsed_create_date
                else False,
                "iis_number": response_data.get("IIS_Number"),
                "iis_create_date": transfer_time(parsed_create_date)
                if parsed_create_date
                else False,
