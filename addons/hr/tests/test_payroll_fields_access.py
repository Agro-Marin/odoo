...
class TestPayrollFieldsAccess(TransactionCase):
    def test_related_fields_on_version(self):
        ...
        version_fields = {
            ...
            if field.groups
            and field.groups not in ["hr.group_hr_user", "base.group_user"]
            and not (field.related and field.related.startswith("employee_id"))
        }
        employee_fields = {
            f_name: field
        ...
        self.assertFalse(
            fields_without_group,
            "Inconsistency between some employee fields and version ones (those employees fields should have the same groups than related one in version",
        )
        self.assertFalse(
            fields_without_related,
            "Some employee fields have the same name than the version ones but they are not related",
        )
        self.assertFalse(
            fields_readonly,
            "(Readonly) Inconsistency between some employee fields and version ones, the both fields (in version and employee) have to be readonly or editable",
        )

    def _test_payroll_fields_are_hidden_to_non_payroll_users(
        self, model_name, view_id, payroll_page_name
    ):
        ...
        self.assertTrue(
            node.xpath(f"//page[@name='{payroll_page_name}']"),
            f"[{model_name}] Payroll page should be found in the form view.",
        )
        payroll_field_node_list = node.xpath(
            f"//page[@name='{payroll_page_name}']//field[not(ancestor::field)]"
        )
        self.assertTrue(
            payroll_field_node_list,
            f"[{model_name}] At least one field should be found inside Payroll information page.",
        )
        payroll_field_names = [
        ...
        current_payroll_field_names = {
            ...
            if field.groups
            and (
                "hr.group_hr_manager" in field.groups
                or "hr_payroll.group_hr_payroll_user" in field.groups
            )
        }
        ...
        missing_group_field_names = [
            ...
            if f_name not in current_payroll_field_names
            and f_name not in whitelist_field_names
        ]
        self.assertFalse(
            missing_group_field_names,
    ...
    def test_payroll_fields_are_hidden_to_non_payroll_users_in_employee_form_view(self):
        self._test_payroll_fields_are_hidden_to_non_payroll_users(
            "hr.employee", "hr.view_employee_form", "payroll_information"
        )

    def test_payroll_fields_are_hidden_to_non_payroll_users_in_version_form_view(self):
        self._test_payroll_fields_are_hidden_to_non_payroll_users(
            "hr.version", "hr.hr_contract_template_form_view", "information"
        )
