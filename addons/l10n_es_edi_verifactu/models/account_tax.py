...
class AccountTax(models.Model):
    ...
    def _l10n_es_edi_verifactu_get_applicability_name_map(self):
        ...
        applicability_string = dict(
            self.env["account.tax"]
            ._fields["l10n_es_applicability"]
    ...
    def _l10n_es_edi_verifactu_get_suggested_clave_regimen(
        self, special_regime, forced_tax_applicability=None
    ):
        """
        ...
        if forced_tax_applicability:
            ...
            taxes = taxes.filtered(
                lambda tax: (
                    tax.l10n_es_type not in main_tax_types
                    or tax._l10n_es_edi_verifactu_get_applicability()
                    == forced_tax_applicability
                )
            )

        tax_applicability = (
            forced_tax_applicability or taxes._l10n_es_edi_verifactu_get_applicability()
        )
        ...
        if VAT and oss_tag and oss_tag.id in taxes.repartition_line_ids.tag_ids.ids:
            ...
            lambda tax: (
                tax.l10n_es_type == "exento" and tax.l10n_es_exempt_reason == "E2"
            )
        ):
            # export
            regimen_key = "02"
    ...
    def _l10n_es_edi_verifactu_get_tax_details_functions(self, company):
        ...
        def tax_details_grouping_function(base_line, tax_data):
            ...
            l10n_es_exempt_reason = (
                tax.l10n_es_exempt_reason if tax.l10n_es_type == "exento" else False
            )
            ...
            if (
                tax.l10n_es_type
                in self.env["account.tax"]._l10n_es_get_sujeto_tax_types()
            ):
                recargo_taxes = base_line["tax_ids"].filtered(
