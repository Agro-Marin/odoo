...
class AccountMove(models.Model):
    ...
    def _l10n_es_edi_verifactu_clave_regimen_selection(self):
        return [
            ...
            (
                "17_iva",
                _(
                    "Operation under one of the regimes provided for in Chapter XI of Title IX (OSS and IOSS)."
                ),
            ),
    ...
    def _l10n_es_edi_verifactu_get_available_clave_regimens_map(self):
        ...
        return {
            "01": {
                ot
                for ot, _desc in clave_regimen_selection
                if len(ot.removesuffix("_iva")) == 2
            },
            "03": {
                ot
                for ot, _desc in clave_regimen_selection
                if len(ot.removesuffix("_igic")) == 2
            },
        }
    ...
    def _compute_l10n_es_edi_verifactu_available_clave_regimens(self):
        ...
        for move in self:
            ...
            move.l10n_es_edi_verifactu_available_clave_regimens = (
                available_clave_regimens.get(tax_applicability, False)
            )
    ...
    def _compute_l10n_es_edi_verifactu_clave_regimen(self):
        ...
        available_clave_regimens = (
            self._l10n_es_edi_verifactu_get_available_clave_regimens_map()
        )
        for move in self:
            clave_regimen = move.l10n_es_edi_verifactu_clave_regimen
            tax_applicability = move._l10n_es_edi_verifactu_get_tax_applicability()
            if clave_regimen not in available_clave_regimens.get(
                tax_applicability, set()
            ):
                clave_regimen = (
                    move._l10n_es_edi_verifactu_get_suggested_clave_regimen()
                )
            move.l10n_es_edi_verifactu_clave_regimen = clave_regimen

    @api.depends(
        "l10n_es_edi_verifactu_document_ids", "l10n_es_edi_verifactu_document_ids.state"
    )
    ...
    @api.depends(
        ...
        "l10n_es_edi_verifactu_document_ids.json_attachment_id",
    )
    def _compute_l10n_es_edi_verifactu_qr_code(self):
        for move in self:
            last_submission = move.l10n_es_edi_verifactu_document_ids._get_last(
                "submission"
            )
    ...
    @api.depends(
        ...
        "l10n_es_edi_verifactu_document_ids",
        "l10n_es_edi_verifactu_document_ids.state",
        "l10n_es_edi_verifactu_document_ids.errors",
    )
    ...
    def _compute_l10n_es_edi_verifactu_show_cancel_button(self):
        for move in self:
            move.l10n_es_edi_verifactu_show_cancel_button = (
                move.l10n_es_edi_verifactu_state
                in ("registered_with_errors", "accepted")
            )
    ...
    def _l10n_es_edi_verifactu_check(self, cancellation=False):
        ...
        if (
            ...
            and selected_clave_regimen
            not in available_clave_regimens_map.get(tax_applicability, set())
        ):
            errors.append(
                _(
    ...
    def _l10n_es_edi_verifactu_get_record_values(self, cancellation=False):
        ...
        selected_clave_regimen = self.l10n_es_edi_verifactu_clave_regimen
        clave_regimen = (
            selected_clave_regimen and selected_clave_regimen.split("_", 1)[0]
        )
        ...
        vals.update(
            {
                ...
                "description": self.invoice_origin[:500]
                ...
                "refunded_document": reversed_move.l10n_es_edi_verifactu_document_ids._get_last(
                    "submission"
                ),
                "substituted_document": substituted_move.l10n_es_edi_verifactu_document_ids._get_last(
                    "submission"
                ),
                "substituted_document_reversal_document": substituted_move.reversal_move_ids.l10n_es_edi_verifactu_document_ids._get_last(
                    "submission"
                ),
                "documents": documents,
                "record_identifier": documents._get_last(
                    "submission"
                )._get_record_identifier(),
        ...
        base_lines += [
            self._prepare_epd_base_line_for_taxes_computation(line) for line in epd_amls
        ]
        ...
        vals["tax_details"] = self.env[
            "l10n_es_edi_verifactu.document"
        ]._get_tax_details(base_lines, company, tax_lines=tax_lines)
    ...
    def _l10n_es_edi_verifactu_create_documents(self, cancellation=False):
        ...
        return {
            record_values["record"]: self.env[
                "l10n_es_edi_verifactu.document"
            ]._create_for_record(record_values)
            for record_values in record_values_list
        }

    def _l10n_es_edi_verifactu_mark_for_next_batch(self, cancellation=False):
        document_map = self._l10n_es_edi_verifactu_create_documents(
            cancellation=cancellation
        )
