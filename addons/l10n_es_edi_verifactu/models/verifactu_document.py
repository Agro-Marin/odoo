...
class L10nEsEdiVerifactuDocument(models.Model):
    ...
    def _compute_json_attachment_filename(self):
        for document in self:
            document_type = (
                "anulacion" if document.document_type == "cancellation" else "alta"
            )
            ...
            identifiers.update(
                {
                    ...
                    "FechaExpedicionFactura": id_factura[
            ...
            identifiers.update(
                {
                    ...
                    **{
                        key: record_type_vals[key]
                        for key in ["TipoFactura", "CuotaTotal", "ImporteTotal"]
                    },
                    "FechaOperacion": record_type_vals.get(
                        "FechaOperacion"
                    ),  # optional
                }
            )
    ...
    def _get_tax_details(self, base_lines, company, tax_lines=None):
        ...
        tax_details_functions = (
            AccountTax._l10n_es_edi_verifactu_get_tax_details_functions(company)
        )
        ...
        tax_details_grouping_function = tax_details_functions[
            "tax_details_grouping_function"
        ]

        base_lines = [
            base_line for base_line in base_lines if base_line_filter(base_line)
        ]
        ...
        # Totals
        base_lines_aggregated_values_for_totals = (
            AccountTax._aggregate_base_lines_tax_details(
                base_lines, total_grouping_function
            )
        )
        totals = AccountTax._aggregate_base_lines_aggregated_values(
            base_lines_aggregated_values_for_totals
        )[True]

        # Tax details
        base_lines_aggregated_values_for_tax_details = (
            AccountTax._aggregate_base_lines_tax_details(
                base_lines, tax_details_grouping_function
            )
        )
        tax_details = AccountTax._aggregate_base_lines_aggregated_values(
            base_lines_aggregated_values_for_tax_details
        )
        ...
        return {
            ...
            "tax_details": {
                key: tax_detail for key, tax_detail in tax_details.items() if key
            },
            "tax_details_per_record": {
                frozendict(base_line): {
                    key: tax_detail for key, tax_detail in tax_details.items() if key
                }
    ...
    def _get_last(self, document_type):
        return self.filtered(
            lambda doc: doc.document_type == document_type and doc.json_attachment_id
        ).sorted()[:1]
    ...
    def _get_state(self):
        ...
        last_registered_document = self.filtered(
    ...
    def _check_record_values(self, vals):
        ...
        company_NIF = (
            vals["company"].partner_id._l10n_es_edi_verifactu_get_values().get("NIF")
        )
        ...
        if vals["documents"] and vals["documents"]._filtered_waiting():
            errors.append(
                self.env._(
                    "We are waiting to send a Veri*Factu record to the AEAT already."
                )
            )

        verifactu_registered = vals["verifactu_state"] in (
        ...
        if vals["cancellation"] and not verifactu_registered:
            errors.append(
                self.env._(
                    "The cancelled record is not Veri*Factu registered (inside Odoo)."
                )
            )
        ...
        if not certificate:
            errors.append(
                self.env._(
                    "There is no certificate configured for Veri*Factu on the company."
                )
            )
        ...
        if (
            vals["verifactu_move_type"] == "correction_substitution"
            and not vals["substituted_document"]
        ):
            errors.append(
                self.env._(
                    "There is no Veri*Factu document for the substituted record."
                )
            )

        if (
            vals["verifactu_move_type"] == "correction_substitution"
            and not vals["substituted_document_reversal_document"]
        ):
            errors.append(
                self.env._(
        ...
        if (
            vals["verifactu_move_type"]
            in ("correction_incremental", "reversal_for_substitution")
            and not vals["refunded_document"]
        ):
            errors.append(
                self.env._("There is no Veri*Factu document for the refunded record.")
            )

        need_refund_reason = vals["verifactu_move_type"] in (
        ...
        if (
            need_refund_reason
            and vals["refund_reason"] != "R5"
            and vals["is_simplified"]
        ):
            errors.append(
                self.env._(
                    "A refund with Refund Reason %(refund_reason)s is not simplified (it needs a partner).",
                    refund_reason=vals["refund_reason"],
                )
            )

        if (
            vals["verifactu_move_type"] == "invoice"
            and not partner_specified
            and not vals["is_simplified"]
        ):
            errors.append(self.env._("A non-simplified invoice needs a partner."))

        if not vals["l10n_es_applicability"]:
            errors.append(
                self.env._("Missing Veri*Factu Tax Applicability (Impuesto).")
            )
        ...
        supported_tax_types = (
            sujeto_tax_types
            + ignored_tax_types
            + ["no_sujeto", "no_sujeto_loc", "recargo", "exento"]
        )
        tax_type_description = (
            self.env["account.tax"]._fields["l10n_es_type"].get_description(self.env)
        )
        ...
        for key, tax_detail in vals["tax_details"]["tax_details"].items():
            ...
            if tax_type not in supported_tax_types:
                ...
                errors.append(
                    self.env._(
                        "A tax with value '%(tax_type)s' as %(field)s is not supported.",
                ...
                if float_round(tax_percentage, precision_digits=2) or float_round(
                    tax_amount, precision_digits=2
                ):
                    errors.append(self.env._("No Sujeto VAT taxes must have 0 amount."))
            if len(key["recargo_taxes"]) > 1:
                errors.append(
                    self.env._('Only a single recargo tax may be used per "main" tax.')
                )
        ...
        if len(tax_applicabilities) > 1:
            name_map = self.env[
                "account.tax"
            ]._l10n_es_edi_verifactu_get_applicability_name_map()
            errors.append(
                self.env._(
                    "We only allow a single Veri*Factu Tax Applicability (Impuesto) per document: %(applicabilities)s.",
                    applicabilities=", ".join(
                        [name_map[t] for t in tax_applicabilities]
                    ),
                )
            )
        ...
        for record_detail in vals["tax_details"]["tax_details_per_record"].values():
            main_tax_details = [
                tax_detail
                for key, tax_detail in record_detail.items()
    ...
    def _create_for_record(self, record_values):
        ...
        if record_values["errors"]:
            document_vals["errors"] = self._format_errors(
                error_title, record_values["errors"]
            )
        else:
            previous_document = record_values[
                "company"
            ]._l10n_es_edi_verifactu_get_last_document()
            render_vals = self._render_vals(
                record_values,
                previous_record_identifier=previous_document._get_record_identifier(),
            )
            document_dict = {
                render_vals["record_type"]: render_vals[render_vals["record_type"]]
            }
            ...
            try:
                ...
                create_message, _zeep_info = _get_zeep_operation(
                    record_values["company"], "registration_xml"
                )
            except (
                ...
                _logger.error(
                    ...
                    error_title,
                    errors[0],
                    json.dumps(document_dict, indent=4),
                )

            if create_message:
                batch_dict = self.with_company(
                ...
                try:
                    ...
                    _logger.error(
                        ...
                        error_title,
                        errors[0],
                        json.dumps(batch_dict, indent=4),
                    )
            ...
            if not document_vals.get("errors"):
                chain_sequence = (
                    record_values["company"]
                    .sudo()
                    ._l10n_es_edi_verifactu_get_chain_sequence()
                )
                try:
        ...
        if document.chain_index:
            ...
            record_values["documents"].filtered(
                lambda rd: not rd.json_attachment_id
            ).sudo().unlink()
    ...
    def _render_vals(self, vals, previous_record_identifier=None):
        ...
        generation_time_string = (
            fields.Datetime.now(timezone("Europe/Madrid"))
            .astimezone(timezone("Europe/Madrid"))
            .isoformat()
        )
    ...
    def _render_vals_operation(self, vals):
        ...
        if vals["verifactu_move_type"] == "invoice":
            ...
            fecha_operacion = (
                delivery_date
                if delivery_date and delivery_date != invoice_date
                else None
            )
            ...
            fecha_operacion = (
                rectified["FechaOperacion"] or rectified["FechaExpedicionFactura"]
            )
        else:
        ...
        if vals["verifactu_move_type"] in (
            "correction_incremental",
            "correction_substitution",
        ):
    ...
    def _render_vals_monetary_amounts(self, vals):
        ...
        for record_tax_details in vals["tax_details"][
            "tax_details_per_record"
        ].values():
            main_key = None
            recargo_key = None
        ...
        for key, tax_detail in vals["tax_details"]["tax_details"].items():
            ...
            if tax_type in sujeto_tax_types:
                ...
                if key["recargo_taxes"]:
                    ...
                    recargo_tax_amount = math.copysign(
                        recargo_tax_detail["tax_amount"], base_amount
                    )
            ...
            if (
                calificacion_operacion in ("N1", "N2")
                and vals["l10n_es_applicability"] == "01"
            ):
                tax_percentage = None
                tax_amount = None
            ...
            detalle = {
                ...
                "BaseImponibleOimporteNoSujeto": self._round_format_number_2(
                    base_amount
                ),
                "CuotaRepercutida": self._round_format_number_2(tax_amount),
                "TipoRecargoEquivalencia": self._round_format_number_2(
                    recargo_percentage
                ),
        ...
        total_amount = sign * (
            vals["tax_details"]["base_amount"] + vals["tax_details"]["tax_amount"]
        )
    ...
    def _render_vals_SistemaInformatico(self, vals):
        ...
        return {
            "SistemaInformatico": {
                ...
                if spanish_companies_on_db_count > 1
                else "N",
            },
        }
    ...
    def _fingerprint(self, render_vals):
        ...
        record_type_vals = render_vals[render_vals["record_type"]]
        id_factura = record_type_vals["IDFactura"]
        registro_anterior = record_type_vals["Encadenamiento"].get(
            "RegistroAnterior"
        )  # does not exist for the first document
        record_type_vals_keys = (
            []
            if render_vals["cancellation"]
            else ["TipoFactura", "CuotaTotal", "ImporteTotal"]
        )
        fingerprint_values = [
        ...
        string = "&".join(
            [f"{field}={value.strip()}" for (field, value) in fingerprint_values]
        )
    ...
    def _send_batch(self, batch_dict):
        ...
        try:
            ...
            if certificate_error in error.message:
                errors.append(
                    self.env._(
                        "The document could not be sent; the access was denied due to a problem with the certificate."
                    )
                )
            else:
                errors.append(
            ...
            if certificate_error in error.message:
                errors.append(
                    self.env._(
                        "The response of the server had the wrong format (HTML instead of XML). It is most likely a problem with the certificate."
                    )
                )
            else:
                errors.append(
                    self.env._("Error while sending the batch document:\n%s", error)
                )
        except zeep.exceptions.Error as error:
            _logger.error("raw zeep response:\n%s", zeep_info.get("raw_response"))
            errors.append(
        ...
        for response_line in res["RespuestaLinea"]:
            ...
            duplicate = {}
            if duplicate_info:
                duplicate_state = duplicate_state_map[
                    duplicate_info["EstadoRegistroDuplicado"]
                ]
                duplicate = {
    ...
    def _send_as_batch(self):
        ...
        incident = any(
            document.create_date > fields.Datetime.now() + timedelta(seconds=240)
            for document in self
        )

        document_dict_list = [document._get_document_dict() for document in self]
        batch_dict = self.with_company(sender_company)._get_batch_dict(
            document_dict_list, incident=incident
        )
        ...
        for document, document_dict in zip(self, document_dict_list, strict=False):
            response_info = (
                ...
                or info["record_info"].get(
                    self._extract_record_key(document_dict), None
                )
                or {
                    "errors": [
                        self.env._(
            ...
            if (
                ...
                if document.document_type == "submission" and duplicate_info[
                    "state"
                ] in ("accepted", "registered_with_errors"):
                    ...
                    document.document_type == "cancellation"
                    and duplicate_info["state"] == "cancelled"
                ):
                    response_info.update(
                        {
                            "state": "accepted",
    ...
    def _extract_record_key(self, document_dict):
        ...
        return str(
            (record_identifier["IDEmisorFactura"], record_identifier["NumSerieFactura"])
        )
    ...
    def _cancel_after_sending(self, info):
        ...
        for document in self:
            ...
            if (
                invoice.l10n_es_edi_verifactu_state == "cancelled"
                ...
                try:
                    ...
                    _logger.error(
                        "Error while canceling journal entry %(name)s (id %(record_id)s) after Veri*Factu cancellation:\n%(error)s",
