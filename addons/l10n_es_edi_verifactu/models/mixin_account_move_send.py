...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _l10n_es_edi_verifactu_get_move_info(self, moves):
        ...
        verifactu_moves = moves.filtered(
        ...
        registered_moves = verifactu_moves.filtered(
            lambda m: (
                m.l10n_es_edi_verifactu_state in ("registered_with_errors", "accepted")
            )
        )
        return {
        ...
        res.update(
            {
                "es_verifactu": {
                    "label": _("Veri*Factu"),
                    "is_applicable": self._is_es_verifactu_applicable,
                }
            }
        )
        return res
    ...
    def _get_alerts(self, moves, moves_data):
        ...
        if verifactu_info["waiting_moves"]:
            alerts["l10n_es_edi_verifactu_warning_waiting_moves"] = {
                ...
                "action": verifactu_info["waiting_moves"]._get_records_action(
        ...
        if verifactu_info["registered_moves"]:
            alerts["l10n_es_edi_verifactu_warning_registered_moves"] = {
                ...
                "action_text": _("View Invoice(s)"),
                "action": verifactu_info["registered_moves"]._get_records_action(
                    name=_("Check Invoice(s)")
                ),
            }
        return alerts
    ...
    def _call_web_service_before_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            if (
                ...
                action = invoice._l10n_es_edi_verifactu_action_go_to_journal_entry(
                    substituted_move
                )
            elif (
                vals["verifactu_move_type"] == "correction_substitution"
                ...
                action = invoice._l10n_es_edi_verifactu_action_go_to_journal_entry(
                    substituted_move.reversal_move_ids
                )
            elif (
        ...
        checked_invoices = self.env["account.move"].browse(
            [
                ...
                and not invoice_data.get("error", {}).get("verifactu_redirect_action")
            ]
        )
        invoices_to_send = self._l10n_es_edi_verifactu_get_move_info(checked_invoices)[
            "moves_to_send"
        ]
        ...
        for invoice in invoices_to_send:
            if not created_document[invoice].chain_index:
                invoices_data[invoice]["error"] = {
                    "error_title": _(
                        "The Veri*Factu document could not be created for all invoices."
                    ),
            ...
            for move, move_data in moves_data.items():
                ...
                if isinstance(error, dict) and error.get("verifactu_redirect_action"):
                    raise RedirectWarning(
                        "\n".join(error["errors"]),
                        error["verifactu_redirect_action"],
                        error["error_title"],
                    )
