...
class TestL10nEsEdiVerifactuCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        patch_db_identifier = mock.patch(
            db_identifier_function, lambda _self: fake_db_identifier
        )
        ...
        cls.certificate = cls.env["certificate.certificate"].create(
            {
                "content": base64.encodebytes(
                    misc.file_open(certificate_path, "rb").read()
                ),
        ...
        cls.company.write(
            {
                ...
                "l10n_es_edi_verifactu_certificate_ids": [
                    Command.set(cls.certificate.ids)
                ],
    ...
    def _mock_response(
        self, status_code, response_file, content_type="text/xml;charset=UTF-8"
    ):
    ...
    def _mock_get_zeep_operation(
        self, registration_return_value=None, registration_xml_return_value=None
    ):
        ...
        def mocked_get_zeep_operation(company, operation):
            ...
            function = (
                ...
                else registration_xml_return_value
            )
            return (function, {})
    ...
    def _mock_zeep_registration_operation(self, response_file_json, name=""):
        ...
        if name:
            zeep_response_dict["RespuestaLinea"][0]["IDFactura"]["NumSerieFactura"] = (
                name
            )
        return self._mock_get_zeep_operation(
            registration_return_value=lambda *args, **kwargs: zeep_response_dict
        )
    ...
    def _mock_zeep_registration_operation_certificate_issue(self):
        ...
        return self._mock_get_zeep_operation(
            registration_return_value=_raise_certificate_error
        )
    ...
    def _mock_format_document_errors(self, errors, title):
        ...
        return html_sanitize(
            self.env["mixin.account.move.send"]._format_error_html(error)
        )
    ...
    def _mock_last_document(self, document):
        ...
        return mock.patch(
            function_path,
            return_value=(document or self.env["l10n_es_edi_verifactu.document"]),
        )
