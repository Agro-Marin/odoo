...
class TestL10nEsEdiVerifactuDocument(TestL10nEsEdiVerifactuCommon):
    ...
    def test_record_identifier(self):
        invoice = self._create_dummy_invoice(
            name="INV/2019/00006", invoice_date="2024-12-11"
        )
        ...
        self.assertDictEqual(
            record_identifier, expected_record_identifier | record_identifier
        )
    ...
    def test_generation_error(self):
        ...
        patched_render_xml_node = mock.patch(
            check_function_path, return_value=mock_errors
        )
        invoice = self._create_dummy_invoice()
    ...
    def test_refund_without_refunded_error(self):
        ...
        invoice = self._create_dummy_invoice(
            name="INV/2019/00026", invoice_date="2024-12-30"
        )
        ...
        with (
            self._mock_last_document(None),
            self.assertRaisesRegex(
                RedirectWarning,
                r".*There is no Veri\*Factu document for the refunded record\..*",
            ),
        ):
        ...
        errors = [
            "There is no Veri*Factu document for the refunded record.",
            "The refund reason is not specified.",
        ]
        expected_record_values = {
            "l10n_es_edi_verifactu_state": False,
            "l10n_es_edi_verifactu_warning": self._mock_format_document_generation_errors(
                errors
            ),
            "l10n_es_edi_verifactu_warning_level": "danger",
        }
    ...
    def test_substitution_without_documents_errors(self):
        invoice = self._create_dummy_invoice(
            name="INV/2019/00026", invoice_date="2019-01-30"
        )
        ...
        with self.assertRaisesRegex(
            RedirectWarning,
            r".*There is no Veri\*Factu document for the substituted record\..*",
        ):
            wizard.action_send_and_print()
        ...
        with (
            self._mock_zeep_registration_operation_certificate_issue(),
            self.assertRaisesRegex(
                RedirectWarning,
                r".*There is no Veri\*Factu document for the reversal of the substituted record\..*",
            ),
        ):
            wizard.action_send_and_print()
        ...
        self.assertTrue(
            substitution_move.l10n_es_edi_verifactu_document_ids.json_attachment_id
        )
    ...
    def test_certificate_issue(self):
        ...
        expected_document_values = {
            ...
            "response_csv": False,
            "state": False,
            "errors": self._mock_format_document_generic_errors(
                expected_response_info["errors"]
            ),
        }
        self.assertRecordValues(document, [expected_document_values])

        waiting_warning = (
            "A Veri*Factu document is waiting to be sent as soon as possible."
        )
        expected_record_values = {
            "l10n_es_edi_verifactu_state": False,
            "l10n_es_edi_verifactu_warning": expected_document_values["errors"]
    ...
    def test_soapfault(self):
        ...
        expected_document_values = {
            ...
            "response_csv": False,
            "state": "rejected",
            "errors": self._mock_format_document_aeat_errors(
                expected_response_info["errors"]
            ),
        }
        self.assertRecordValues(document, [expected_document_values])
    ...
    def test_batch_single_accepted_registration(self):
        ...
        with self._mock_last_document(None):
            document = invoice._l10n_es_edi_verifactu_create_documents()[invoice]
        with self._mock_zeep_registration_operation(
    ...
    def test_batch_single_accepted_cancellation(self):
        ...
        with self._mock_last_document(None):
            submission_document = invoice._l10n_es_edi_verifactu_create_documents()[
                invoice
            ]
        ...
        document = invoice._l10n_es_edi_verifactu_create_documents(cancellation=True)[
            invoice
        ]
        with self._mock_zeep_registration_operation(
            "l10n_es_edi_verifactu/tests/responses/batch_single_accepted_cancellation.json"
        ):
            _batch_xml, info = document._send_as_batch()
    ...
    def test_batch_single_rejected_registration(self):
        ...
        with self._mock_last_document(None):
            document = invoice._l10n_es_edi_verifactu_create_documents()[invoice]
        with self._mock_zeep_registration_operation(
            "l10n_es_edi_verifactu/tests/responses/batch_single_rejected_registration.json"
        ):
            _batch_xml, info = document._send_as_batch()
    ...
    def test_batch_single_registered_with_errors_registration(self):
        invoice = self._create_dummy_invoice(
            name="INV/2019/00007", invoice_date="2024-12-17"
        )
        with self._mock_last_document(None):
            document = invoice._l10n_es_edi_verifactu_create_documents()[invoice]
        with self._mock_zeep_registration_operation(
            "l10n_es_edi_verifactu/tests/responses/batch_single_registered_with_errors_registration.json"
        ):
            _batch_xml, info = document._send_as_batch()
    ...
    def test_batch_single_duplicate_original_registered_with_errors_registration_without_timeout(
        self,
    ):
        invoice = self._create_dummy_invoice(
            name="INV/2019/00006", invoice_date="2024-12-11"
        ...
        with self._mock_last_document(None):
            document = invoice._l10n_es_edi_verifactu_create_documents()[invoice]
        with self._mock_zeep_registration_operation(
            "l10n_es_edi_verifactu/tests/responses/batch_single_duplicate_original_registered_with_errors.json"
        ):
            _batch_xml, info = document._send_as_batch()
    ...
    def test_batch_single_duplicate_original_registered_with_errors_registration_with_timeout(
        self,
    ):
        invoice = self._create_dummy_invoice(
            name="INV/2019/00006", invoice_date="2024-12-11"
    ...
    def test_response_parsing_error_document_not_found(self):
        invoice = self._create_dummy_invoice(
            name="INV/2019/00500", invoice_date="2024-12-17"
        )
        with self._mock_last_document(None):
            document = invoice._l10n_es_edi_verifactu_create_documents()[invoice]
        with self._mock_zeep_registration_operation(
    ...
    def test_mark_for_next_batch(self):
        ...
        invoice = self._create_dummy_invoice(
            name="INV/2019/00027", invoice_date="2024-12-30"
        )
    ...
    def test_response_issue(self):
        ...
        with (
            self._mock_zeep_registration_operation_certificate_issue(),
            self._mock_cron_trigger(cron_trigger_result_dict),
        ):
            created_documents = invoice._l10n_es_edi_verifactu_mark_for_next_batch()
        document = created_documents[invoice]
