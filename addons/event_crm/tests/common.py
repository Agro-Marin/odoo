...
class EventCrmCase(TestCrmCommon, EventCase):
    ...
    def assertLeadConvertion(self, rule, registrations, partner=None, **expected):
        ...
        self.assertEqual(
            len(lead),
            1,
            "Invalid registrations -> lead creation, found %s leads where only 1 is expected."
            % len(lead),
        )
        self.assertEqual(
            lead.registration_ids,
            registrations,
        ...
        expected_reg_name = (
            partner.name
            or registrations._find_first_notnull("name")
            or registrations._find_first_notnull("email")
        )
        if partner:
        ...
        for registration in registrations:
            ...
            if registration.email:
                if (
                    tools.email_normalize(registration.email)
                    == registration.partner_id.email_normalized
                ):
