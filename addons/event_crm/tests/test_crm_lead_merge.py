...
class TestLeadCrmMerge(TestLeadMergeCommon, TestEventCrmCommon):
    ...
    def test_merge_different_events_and_update(self):
        ...
        self.assertTrue(
            registrations[0].lead_ids,
            "Order rule should have created leads for both registrations",
        )
        self.assertTrue(
            registrations[1].lead_ids,
            "Order rule should have created leads for both registrations",
        )
        self.assertFalse(
        ...
        final_lead = registrations.lead_ids._merge_opportunity(
            auto_unlink=False, max_length=None
        )
