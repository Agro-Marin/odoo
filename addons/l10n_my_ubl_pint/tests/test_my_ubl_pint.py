...
class TestMyUBLPint(AccountTestInvoicingCommon):
    ...
    def test_invoice_no_taxes(self):
        ...
        actual_xml, errors = self.env["account.edi.xml.pint_my"]._export_invoice(
            invoice
        )
        self.assertFalse(errors)

        with file_open(
            "l10n_my_ubl_pint/tests/expected_xmls/invoice_no_taxes.xml", "rb"
        ) as f:
            expected_xml = f.read()
    ...
    def test_invoice_with_sst(self):
        ...
        with file_open(
            "l10n_my_ubl_pint/tests/expected_xmls/invoice_with_sst.xml", "rb"
        ) as f:
            expected_xml = f.read()
