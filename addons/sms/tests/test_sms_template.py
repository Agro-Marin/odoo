...
class TestSmsTemplateAccessRights(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.basic_user = mail_new_test_user(
            cls.env, login="user_employee", groups="base.group_user"
        )
    ...
    def test_sms_template_rendering_restricted(self):
        ...
        self.assertEqual(
            sms_composer.body,
            self.partner.name,
            "Simple user should be able to render SMS template",
        )
        ...
        self.assertEqual(
            sms_composer.body,
            "{{ object.name }}",
            "In mass mode, we should not render the template",
        )
        ...
        with self.assertRaises(
            AccessError, msg="User should not be able to write new inline_template code"
        ):
    ...
    def test_sms_template_rendering_unrestricted(self):
        ...
        self.assertIn(
            self.partner.name,
            body,
            "Template Editor should be able to write new Jinja code",
        )
...
class TestSMSTemplateReset(TransactionCase):
    ...
    def test_sms_template_reset(self):
        ...
        sms_template = self.env.ref("sms.sms_template_test").with_context(
        ...
        sms_template_reset = (
            self.env["sms.template.reset"].with_context(context).create({})
        )
