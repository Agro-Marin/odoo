...
class MockSMS(common.HttpCase):
    ...
    def mockSMSGateway(
            ...
            if local_endpoint == "/iap/sms/2/send":
                ...
                for to_send in params["messages"]:
                    res = {
                        ...
                        "state": "delivered" if force_delivered else "success",
                        "credit": 1,
                    }
                    error = sim_error or (
                        nbr_t_error and nbr_t_error.get(to_send["number"])
                    )
                    if error and error == "credit":
                        res.update(credit=0, state="insufficient_credit")
                    elif error and error in {
                        "wrong_number_format",
                        "unregistered",
                        "server_error",
                    }:
                ...
                for message in params["messages"]:
                    for number in message["numbers"]:
                        error = sim_error or (
                            nbr_t_error and nbr_t_error.get(number["number"])
                        )
        ...
        def _sms_sms_send(
            records, unlink_failed=False, unlink_sent=True, raise_exception=False
        ):
            if sms_allow_unlink:
                return sms_send_origin(
                    records,
        ...
        with (
            patch.object(
                SmsApi, "_contact_iap", side_effect=_contact_iap
            ) as _sms_api_contact_iap_mock,
            patch.object(
                SmsSms,
                "create",
                autospec=True,
            ...
            patch.object(
                SmsSms, "_send", autospec=True, wraps=SmsSms, side_effect=_sms_sms_send
            ),
        ):
...
class SMSCase(MockSMS):
    ...
    def _find_sms_sent(self, partner, number):
        ...
        if not sent_sms:
            debug_info = "\n".join(f"To {sms['number']}" for sms in self._sms)
            raise AssertionError(
                f"sent sms not found for {partner} (number: {number})\n{debug_info}"
            )
        return sent_sms
    ...
    def assertSMSIapSent(self, numbers, content=None):
        ...
        for number in numbers:
            ...
            self.assertTrue(
                bool(sent_sms),
                "Number %s not found in %s"
                % (number, repr([s["number"] for s in self._sms])),
            )
    ...
    def assertSMSCanceled(
        self, partner, number, failure_type, content=None, fields_values=None
    ):
    ...
    def assertSMSOutgoing(self, partner, number, content=None, fields_values=None):
        ...
        partner can be an empty recordset."""
        self.assertSMS(
            partner, number, "outgoing", content=content, fields_values=fields_values
        )
    ...
    def assertSMSNotification(
        ...
        for recipient_info in recipients_info:
            ...
            self.assertTrue(
                ...
                % (partner, number, state, debug_info),
            )
            self.assertEqual(
                notif.author_id,
                notif.mail_message_id.author_id,
            ...
            if recipient_info.get("recipient_check_sms", check_sms):
                ...
                if state in {"process", "pending", "sent"}:
                    ...
                    self.assertSMS(
                        ...
                        number,
                        "error",
                        failure_type=recipient_info["failure_type"],
                        content=content,
                        fields_values=fields_values,
                    )
                elif state == "canceled":
        ...
        if messages is not None:
            ...
            with patch("odoo.libs.text.html.SANITIZE_TAGS", sanitize_tags):
                for message in messages:
                    self.assertEqual(
                        content,
                        tools.html2plaintext(tools.html_sanitize(message.body)).rstrip(
                            "\n"
                        ),
                    )
