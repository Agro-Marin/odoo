            ...
            with self.subTest(message_content=message_content):
                with self.with_user("admin"), self.mockSMSGateway():
                    message = self.env.user.partner_id.message_post(
                        ...
                        message_type="sms",
                        sms_numbers=["+3215228817386"],
                    )

                self.assertSMSNotification(
                    [{"number": "+3215228817386"}],
                    expected_sms_content,
                    message,
            ...
            with self.subTest(sms_content=sms_content):
                with self.with_user("admin"):
                    composer = (
                        ...
                        .with_context(
                            active_model="res.partner", active_id=self.partner_employee
                        )
                    ...
                    _message_sms_patch = patch.object(
                        ...
                        "_message_sms",
                        autospec=True,
                        side_effect=MixinMailThread._message_sms,
                    )
                    with (
                        self.mockSMSGateway(),
                        _message_sms_patch as _patched_message_sms,
                    ):
