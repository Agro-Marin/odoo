...
class SmsController(Controller):
    ...
    def update_sms_status(self, message_statuses):
        ...
        """
        all_uuids = []
        for uuids, iap_status in (
            (status["uuids"], status["sms_status"]) for status in message_statuses
        ):
            ...
            if (
                ...
                if state := request.env["sms.sms"].IAP_TO_SMS_STATE_SUCCESS.get(
                    iap_status
                ):
        ...
        request.env["sms.sms"].sudo().search(
            [("uuid", "in", all_uuids), ("to_delete", "=", False)]
        ).to_delete = True
        return "OK"
    ...
    def _check_status_values(uuids, iap_status, message_statuses):
        ...
        if (
            not uuids
            or not iap_status
            or not re.match(r"^\w+$", iap_status)
