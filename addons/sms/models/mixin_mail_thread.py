...
class MixinMailThread(models.AbstractModel):
    ...
    def _compute_message_has_sms_error(self):
        ...
        for record in self:
            record.message_has_sms_error = bool(res.get(record._origin.id))
    ...
    def _search_message_has_sms_error(self, operator, operand):
        ...
        return [
            "&",
            ("message_ids.has_sms_error", "=", True),
    ...
    def message_post(self, *args, body="", message_type="notification", **kwargs):
        ...
        return super().message_post(
            *args, body=body, message_type=message_type, **kwargs
        )
    ...
    def _message_sms_with_template(
        self,
        template=False,
        ...
        **kwargs,
        ...
        if partner_ids:
            for partner in self.env["res.partner"].sudo().browse(partner_ids):
                number = (
                    sms_pid_to_number.get(partner.id)
                    or partner._phone_get_number("mobile", "whatsapp").number
                )
        ...
        if sms_numbers:
            ...
            existing_partners_numbers = {
                vals_dict["number"] for vals_dict in sms_create_vals
            }
            sms_create_vals += [
                dict(
                    sms_base_vals,
