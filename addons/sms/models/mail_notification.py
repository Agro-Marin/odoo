...
class MailNotification(models.Model):
    ...
    notification_type = fields.Selection(
        selection_add=[("sms", "SMS")], ondelete={"sms": "cascade"}
    )
    ...
    sms_tracker_ids = fields.One2many(
        "sms.tracker", "mail_notification_id", string="SMS Trackers"
    )
    ...
    def _compute_sms_id(self):
        self.sms_id = False
        sms_notifications = self.filtered(
            lambda n: n.notification_type == "sms" and bool(n.sms_id_int)
        )
        ...
        existing_sms_ids = (
            ...
            .search(
                [
                    ("id", "in", sms_notifications.mapped("sms_id_int")),
                    ("to_delete", "!=", True),
                ]
            )
            .ids
        )
        for sms_notification in sms_notifications.filtered(
            lambda n: n.sms_id_int in set(existing_sms_ids)
        ):
