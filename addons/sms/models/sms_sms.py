...
class SmsSms(models.Model):
    ...
    def _compute_sms_tracker_id(self):
        ...
        existing_trackers = self.env["sms.tracker"].search(
            [("sms_uuid", "in", self.filtered("uuid").mapped("uuid"))]
        )
        tracker_ids_by_sms_uuid = {
            tracker.sms_uuid: tracker.id for tracker in existing_trackers
        }
    ...
    def resend_failed(self):
        sms_to_send = self.filtered(
            lambda sms: sms.state == "error" and not sms.to_delete
        )
    ...
    def _process_queue(self):
        ...
        records = self.search(
            domain, limit=batch_size, order="id"
        ).try_lock_for_update()
        if not records:
            return
    ...
    def _send_with_api(
        ...
        messages = [
            {
                "content": body,
                "numbers": [
                    {"number": sms.number, "uuid": sms.uuid} for sms in body_sms_records
                ],
            }
        ...
        try:
            results = sms_api._send_sms_batch(
            ...
            _logger.info(
                "Send batch %s SMS: %s: gave %s", len(self.ids), self.ids, results
            )
        ...
        for (iap_state, failure_reason), results_group in tools.groupby(
            ...
            sms_sudo = all_sms_sudo.filtered(
                lambda s, group_uuids=group_uuids: s.uuid in group_uuids
            )
            if success_state := self.IAP_TO_SMS_STATE_SUCCESS.get(iap_state):
                ...
                failure_type = sms_api.PROVIDER_TO_SMS_FAILURE_TYPE.get(
                    iap_state, "unknown"
                )
    ...
    def _update_sms_state_and_trackers(self, new_state, failure_type=None):
        ...
        self.sms_tracker_id.sudo()._action_update_from_sms_state(
            new_state, failure_type=failure_type
        )

    def _handle_call_result_hook(self, results):
        """Further process SMS sending API results."""
        if any(result.get("state") == "insufficient_credit" for result in results):
            # One notification per send, not per result: the paths that send
            # inside the user's own request (sms.composer's "numbers" mode, a
            # mass send with mass_force_send, resend_failed) hand us the whole
            # batch, so a 500-number send would otherwise push 500 bus messages.
