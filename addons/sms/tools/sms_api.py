...
ERROR_MESSAGES = {
    ...
    "banned_account": _lt(
        "This phone number/account has been banned from our service."
    ),
    # Errors that any of the three registration calls can return
    "country_not_supported": _lt(
        "Your country is not supported due to sender registration legislation."
    ),
    ...
    # Default error
    "unknown_error": _lt(
        "An unknown error occurred. Please contact Odoo support if this error persists."
    ),
}
...
class SmsApi(
    SmsApiBase
):  # TODO RIGR in master: rename SmsApi to SmsApiIAP, and  SmsApiBase to SmsApi
    ...
    def _contact_iap(self, local_endpoint, params, timeout=15):
        ...
        ):  # Don't reach IAP servers during module installation
    ...
    def _get_sms_api_error_messages(self):
        ...
        register_now = (
            '<a href="{}/1/account?account_token={}" target="_blank">{}</a>'.format(
                sms_endpoint, sms_account_token, _("Register now.")
            )
        )
    ...
    def _set_sender_name(self, sender_name):
        return self._contact_iap(
            "/api/sms/1/account/update_sender",
            {
