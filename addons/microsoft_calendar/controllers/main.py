...
class MicrosoftCalendarController(CalendarController):
    ...
    def microsoft_calendar_sync_data(self, model, **kw):
        ...
        if model == "calendar.event":
            ...
            client_id = request.env["microsoft.service"]._get_microsoft_client_id(
                "calendar"
            )
            ...
            if not client_id or client_id == "":
                ...
                if MicrosoftCal._can_authorize_microsoft(request.env.user):
                    action_id = request.env.ref("web.action_general_configuration").id
                return {
            ...
            if not MicrosoftCal.is_authorized(request.env.user):
                url = MicrosoftCal._microsoft_authentication_url(
            ...
            if not need_refresh and sync_status != "sync_active":
                return {"status": sync_status, "url": ""}
            return {
                "status": "need_refresh"
                if need_refresh
                else "no_new_event_from_microsoft",
                "url": "",
            }
