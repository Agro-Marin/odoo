...
class MicrosoftCalendarService:
    ...
    def _get_single_event(self, iCalUId, token, timeout=TIMEOUT):
        ...
        status, event, _dummy = self.microsoft_service._do_request(
            url, {}, headers, method="GET", timeout=timeout
        )
        return status not in RESOURCE_NOT_FOUND_STATUSES, event

    @requires_auth_token
    def _get_events_from_paginated_url(
        self, url, token=None, params=None, timeout=TIMEOUT
    ):
        """
        ...
        if not params:
            ...
            day_range = int(
                self.microsoft_service.env["ir.config_parameter"]
                .sudo()
                .get_param("microsoft_calendar.sync.range_days", default=365)
            )
            params = {
                "startDateTime": fields.Datetime.subtract(
                    fields.Datetime.now(), days=day_range
                ).strftime("%Y-%m-%dT00:00:00Z"),
                "endDateTime": fields.Datetime.add(
                    fields.Datetime.now(), days=day_range * 2
                ).strftime("%Y-%m-%dT00:00:00Z"),
            }
        ...
        next_sync_token = (
            parse_qs(urlsplit(token_url).query).get("$deltatoken", [False])[0]
            if token_url
            else None
        )
    ...
    def _check_full_sync_required(self, response):
        ...
        return any(
            error_code in response_code
            for error_code in ["fullSyncRequired", "SyncStateNotFound"]
        )
    ...
    def _get_occurrence_details(self, serieMasterId, token=None, timeout=TIMEOUT):
        ...
        events, _ = self._get_events_from_paginated_url(
            url, token=token, timeout=timeout
        )
    ...
    def get_events(self, sync_token=None, token=None, timeout=TIMEOUT):
        ...
        """
        events, next_sync_token = self._get_events_delta(
            sync_token=sync_token, token=token, timeout=timeout
        )

        # get occurences details for all serie masters
        for master in filter(lambda e: e.type == "seriesMaster", events):
            events |= self._get_occurrence_details(
                master.id, token=token, timeout=timeout
            )
    ...
    def insert(self, values, token=None, timeout=TIMEOUT):
        ...
        _dummy, data, _dummy = self.microsoft_service._do_request(
            url, json.dumps(values), headers, method="POST", timeout=timeout
        )
    ...
    def patch(self, event_id, values, token=None, timeout=TIMEOUT):
        ...
        try:
            status, _dummy, _dummy = self.microsoft_service._do_request(
    ...
    def _microsoft_authentication_url(self, from_url="http://www.odoo.com"):
        redirect_uri = (
            self.microsoft_service.get_base_url() + "/microsoft_account/authentication"
        )
