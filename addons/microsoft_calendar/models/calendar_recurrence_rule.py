...
class CalendarRecurrence(models.Model):
    ...
    def _apply_recurrence(
        ...
        for event in events._filtered_synced():
            if (
                ...
                and not event.recurrence_id.ms_universal_event_id
            ):
                vals += [
                    {
                        "name": event.name,
    ...
    def _has_base_event_time_fields_changed(self, new):
        ...
        old = (
            self.base_event_id
            and self.base_event_id.read(["start", "stop", "allday"])[0]
        )
        return old and (
    ...
    def _write_from_microsoft(self, microsoft_event, vals):
        ...
        if self._has_base_event_time_fields_changed(new_event_values) and (
            ...
            self.with_context(dont_notify=True)._write_events(
                dict(
                    {
                        field: value
    ...
    def _microsoft_values(self, fields_to_sync, initial_values=()):
        ...
        """
        return self.base_event_id._microsoft_values(
            fields_to_sync,
            initial_values={**dict(initial_values), "type": "seriesMaster"},
        )

    def _check_attendees_have_email(self):
        self.calendar_event_ids.filtered(
            lambda e: e.active
        )._check_attendees_have_email()
