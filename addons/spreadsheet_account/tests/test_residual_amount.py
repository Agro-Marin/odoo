...
class SpreadsheetAccountingFunctionsTest(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        (cls.move_22 + cls.payment_move_22.move_id).line_ids.filtered(
            lambda line: line.account_type == "asset_receivable"
        ).reconcile()
        (cls.move_23 + cls.payment_move_23.move_id).line_ids.filtered(
            lambda line: line.account_type == "asset_receivable"
        ).reconcile()

    def test_residual_empty_params(self):
        self.assertEqual(
            self.env["account.account"].spreadsheet_fetch_residual_amount([]), []
        )
