...
class AccountAccount(models.Model):
    ...
    def _prepare_spreadsheet_formula_domain(
        self, formula_params, default_accounts=False
    ):
        ...
        if "account_tag_ids" in formula_params:
            ...
            code_domain = Domain.OR(
                Domain("code", "=like", f"{code}%") for code in codes
            )
        ...
        posted_domain = (
            ...
            if formula_params.get("include_unposted")
            else [("move_id.state", "=", "posted")]
        )
        ...
        partner_ids = [
            int(partner_id)
            for partner_id in formula_params.get("partner_ids", [])
            if partner_id
        ]
        if partner_ids:
    ...
    def spreadsheet_fetch_residual_amount(self, args_list):
        ...
        for args in args_list:
            company_id = args["company_id"] or self.env.company.id
            domain = self._prepare_spreadsheet_formula_domain(
                args, default_accounts=True
            )
            MoveLines = self.env["account.move.line"].with_company(company_id)
    ...
    def spreadsheet_fetch_partner_balance(self, args_list):
        ...
        for args in args_list:
            partner_ids = [
                partner_id for partner_id in args.get("partner_ids", []) if partner_id
            ]
            if not partner_ids:
    ...
    def spreadsheet_fetch_balance_tag(self, args_list):
        ...
        for args in args_list:
            account_tag_ids = [
                tag_id for tag_id in args.get("account_tag_ids", []) if tag_id
            ]
