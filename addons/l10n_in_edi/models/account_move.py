...
try:
    ...
    _logger.warning(
        "The 'jwt' library is not installed. Decoding for duplicate IRN e-invoices will be skipped."
    )
    jwt = None
...
class AccountMove(models.Model):
    ...
    def _post_entries(self):
        ...
        self.filtered(
            lambda m: m._l10n_in_check_einvoice_eligible()
    ...
    def _l10n_in_edi_send_invoice(self):
        ...
        response = self._l10n_in_edi_connect_to_server(
            url_end_point="generate", json_payload=generate_json
        )
        if error := response.get("error", {}):
            ...
            if "2150" in error_codes:
                ...
                response = self._l10n_in_edi_connect_to_server(
                    ...
                    params={
                        ...
                        and self.invoice_date.strftime("%d/%m/%Y"),
                    },
                )
                mismatch_error = []
                decoded_response = {}
                    ...
                    try:
                        if data := response.get("data"):
                            ...
                            decoded_response = jwt.decode(
                                signinvoice, options={"verify_signature": False}
                            )
                ...
                if decoded_response:
                    ...
                    received_total_invoice_value = decoded_response["ValDtls"][
                        "TotInvVal"
                    ]
                    ...
                    if (
                        received_gstin != expected_gstin
                        or not expected_total_invoice_value - 1
                        < received_total_invoice_value
                        < expected_total_invoice_value + 1
                    ):
                        mismatch_error = [
                            {
                                "code": "2150",
            ...
            if (no_credit := "no-credit" in error_codes) or error:
                ...
                is_warning = any(
                    warning_code in error_codes for warning_code in ("404", "timeout")
                )
                ...
                return {"messages": [msg], "is_warning": is_warning}
    ...
    def _l10n_in_edi_cancel_invoice(self):
        ...
        if error := response.get("error"):
            ...
            if "no-credit" in error_codes:
                ...
                return None
            if error:
                self.l10n_in_edi_error = Markup("<br/>").join(
                    ["[%s] %s" % (e.get("code"), e.get("message")) for e in error]
                )
    ...
    def _get_l10n_in_edi_partner_details(
        ...
        if set_phone_and_email:
            ...
            partner_phone = partner._phone_get_number().number
            if partner_phone and re.match(
                r"^[0-9]{10,12}$", self._l10n_in_extract_digits(partner_phone)
            ):
    ...
    def _get_l10n_in_edi_line_details(self, index, line, line_tax_details):
        ...
        if line.discount == 100.00 or float_is_zero(quantity, 3):
            ...
            unit_price_in_inr = (
                (sign * line.balance) / (1 - (line.discount / 100))
            ) / quantity
        ...
        line_details = {
            "SlNo": str(index),
            "IsServc": (self._l10n_in_is_service_hsn(line.l10n_in_hsn_code) and "Y")
            or "N",
            ...
            "Discount": in_round(
                (unit_price_in_inr * quantity) * (line.discount / 100)
            ),
            ...
            "StateCesRt": in_round(
                tax_details_by_code.get("state_cess_rate_amount", 0.00), 3
            ),
            ...
            "TotItemVal": in_round(
                (sign * line.balance) + line_tax_details.get("tax_amount", 0.00)
            ),
        }
        if line.name:
    ...
    def _l10n_in_edi_generate_invoice_json_managing_negative_lines(self, json_payload):
        ...
        def put_discount_on(discount_line_vals, other_line_vals):
            ...
            if (
                ...
                other_line_vals["Discount"] = in_round(
                    other_line_vals["Discount"] + discount_to_allow
                )
                other_line_vals.update(dict.fromkeys(amount_keys, 0.00))
                return False
            other_line_vals["Discount"] = in_round(
                other_line_vals["Discount"] + discount
            )
        ...
        discount_lines = []
        for discount_line in json_payload[
            "ItemList"
        ].copy():  # to be sure to not skip in the loop:
        ...
        for line in sorted(
            json_payload["ItemList"], key=lambda i: i["AssAmt"], reverse=True
        ):
            lines_grouped_and_sorted[discount_group_key(line)].append(line)

        for discount_line in discount_lines:
            for apply_discount_on in lines_grouped_and_sorted[
                discount_group_key(discount_line)
            ]:
    ...
    def _l10n_in_edi_generate_invoice_json(self):
        ...
        grouping_lines = self.invoice_line_ids.grouped(
            lambda l: (
                ...
                and ((l._l10n_in_is_global_discount() and "global_discount") or "lines")
            )
        )
        ...
        global_discount_amount = (
            sum(line.balance for line in global_discount_line) * -sign
        )
        in_round = self._l10n_in_round_value
        json_payload = {
            "Version": "1.1",
            "TranDtls": {
                "TaxSch": "GST",
                "SupTyp": self._l10n_in_get_supply_type(
                    tax_details_by_code.get("igst_amount")
                ),
                ...
                "IgstOnIntra": (
                    ...
                    (
                        ...
                        not in ("special_economic_zone", "overseas")
                        and is_intra_state
            ...
            "DocDtls": {
                "Typ": (self.move_type == "out_refund" and "CRN")
                or (self.debit_origin_id and "DBN")
                or "INV",
                "No": self.name,
                "Dt": self.invoice_date and self.invoice_date.strftime("%d/%m/%Y"),
            },
            "SellerDtls": self._get_l10n_in_edi_partner_details(
                seller_buyer["seller_details"]
            ),
            ...
            "ItemList": [
                self._get_l10n_in_edi_line_details(
                    index, line, tax_details_per_record.get(line, {})
                )
            ...
            json_payload["ValDtls"].update(
                {
                    "TotInvValFc": in_round(
                        tax_details.get("base_amount_currency")
                        + tax_details.get("tax_amount_currency")
                    )
                }
            )
        ...
        if seller_buyer["buyer_details"] != seller_buyer["ship_to_details"]:
            json_payload["ShipDtls"] = self._get_l10n_in_edi_partner_details(
                seller_buyer["ship_to_details"], is_overseas=is_overseas
            )
        if is_overseas:
            ...
            if shipping_bill_date := self.l10n_in_shipping_bill_date:
                json_payload["ExpDtls"]["ShipBDt"] = shipping_bill_date.strftime(
                    "%d/%m/%Y"
                )
            ...
            base_and_tax_amount = tax_details.get("base_amount") + tax_details.get(
                "tax_amount"
            )
            # For Export If with payment of Tax then we need to include Tax in Total Invoice Value
            if (
                json_payload["TranDtls"]["SupTyp"] == "EXPWP"
                and json_valdtls["AssVal"] == base_and_tax_amount
            ):
                json_payload["ValDtls"]["TotInvVal"] = self._l10n_in_round_value(
                    sum(
                        [
                ...
                for line in json_payload["ItemList"]:
                    line["TotItemVal"] = self._l10n_in_round_value(
                        sum(
                            [
                                line["TotItemVal"],
                                line["IgstAmt"],
        ...
        return self._l10n_in_edi_generate_invoice_json_managing_negative_lines(
            json_payload
        )

    def _l10n_in_get_supply_type(self, is_igst_amount):
        if (
            self.l10n_in_gst_treatment in ("overseas", "special_economic_zone")
            and is_igst_amount
        ):
            return {
                "overseas": "EXPWP",
    ...
    def _l10n_in_check_einvoice_validation(self):
        alerts = {
            ...
            **(
            ...
            )._l10n_in_check_einvoice_validation(),
    ...
    # ================================ API methods ===========================
    def _l10n_in_edi_connect_to_server(
        self, url_end_point, json_payload=False, params=False
    ):
        """
        ...
        if (error := response.get("error")) and "1005" in [
            e.get("code") for e in error
        ]:
