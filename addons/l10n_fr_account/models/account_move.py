...
class AccountMove(models.Model):
    ...
    l10n_fr_is_company_french = fields.Boolean(
    ...
    def _get_view(self, view_id=None, view_type="form", **options):
        ...
        if (
            view_type == "form"
            and company.country_code in company._get_france_country_codes()
        ):
    ...
    def _compute_l10n_fr_is_company_french(self):
        for record in self:
            record.l10n_fr_is_company_french = (
                record.country_code in record.company_id._get_france_country_codes()
            )
