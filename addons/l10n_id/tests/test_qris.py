...
class TestQris(AccountTestInvoicingCommon):
    ...
    def test_qris_qr_code_generation(self):
        ...
        with patch(
            "odoo.addons.l10n_id.models.res_bank._l10n_id_make_qris_request",
            return_value=self.success_qris_get,
        ) as patched:
            ...
            result = self.qris_qr_invoice.with_context(
                {"is_online_qr": True}
            )._generate_qr_code()
        ...
        with patch(
            ...
            self.qris_qr_invoice.with_context(
                {"is_online_qr": True}
            )._generate_qr_code()
        ...
        with patch(
            "odoo.addons.l10n_id.models.res_bank._l10n_id_make_qris_request",
            return_value=self.success_qris_get_second,
        ):
            self.qris_qr_invoice.l10n_id_qris_transaction_ids[-1][
            ...
            self.assertEqual(
                qr_details[-1].qris_invoice_id, qris_data["qris_invoiceid"]
            )
    ...
    def test_fetch_payment_status_fail(self):
        ...
        with patch(
            "odoo.addons.l10n_id.models.res_bank._l10n_id_make_qris_request",
            return_value=self.qris_status_fail,
        ):
    ...
    def test_fetch_payment_status_success(self):
        ...
        with patch(
            "odoo.addons.l10n_id.models.res_bank._l10n_id_make_qris_request",
            return_value=self.qris_status_success,
        ):
            self.qris_qr_invoice.action_l10n_id_update_payment_status()
            self.assertEqual(
                self.qris_qr_invoice.payment_state,
            ...
            self.assertEqual(
                ...
                Markup(
                    "<p>This invoice was paid by Zainal Arief using QRIS with the payment method Sakuku.</p>"
                ),
            )
            # One of the QRIS transactions linked to the invoice should be paid already
            self.assertTrue(
                any(self.qris_qr_invoice.l10n_id_qris_transaction_ids.mapped("paid"))
            )
    ...
    def test_gc_no_remove_transactions_unpaid_within_30(self):
        ...
        with patch(
            ...
            ]._gc_remove_pointless_qris_transactions()
            self.assertEqual(self.env["l10n_id.qris.transaction"].search_count([]), 1)
