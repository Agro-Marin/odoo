...
class TestQrisTransaction(AccountTestInvoicingCommon):
    ...
    def _create_sample_transaction(
        self, model, model_id, qris_id, amount, create_at, content
    ):
    ...
    def test_latest_transaction(self):
        ...
        self._create_sample_transaction(
            ...
            11000,
            "2024-08-01 03:00:15",
            "qris_content_sample_latest",
        )

        trx = self.env["l10n_id.qris.transaction"]._get_latest_transaction(
            "account.move", "1"
        )
        self.assertTrue(
            trx["qris_amount"] == 11000
            and trx["qris_content"] == "qris_content_sample_latest"
        )
    ...
    def test_l10n_id_get_qris_qr_statuses(self):
        ...
        with patch(
            ...
            self.assertTrue(
                res["paid"]
                and success_response["qris_payment_customername"] == "Zainal Arief"
                and trx[0].paid
            )
