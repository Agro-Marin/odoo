...
def call_pine_labs(payment_method: object, endpoint: str, payload: dict) -> dict:
    ...
    if endpoint not in ALLOWED_ENDPOINTS:
        raise ValueError(
            f"Invalid endpoint: '{endpoint}'. Allowed endpoints are: {ALLOWED_ENDPOINTS}"
        )
    payload |= pine_labs_request_body(
        endpoint == "UploadBilledTransaction", payment_method=payment_method
    )
...
def pine_labs_request_body(payment_mode: bool, payment_method: object) -> dict:
    ...
    if payment_mode:
        ...
        pine_labs_auto_cancel_duration = (
            payment_method.env["ir.config_parameter"]
            .sudo()
            .get_param(
                "pos_pine_labs.payment_auto_cancel_duration",
                PINE_LABS_AUTO_CANCEL_DURATION_MIN,
            )
        )
        request_parameters.update(
            {
                "AllowedPaymentMode": ALLOWED_PAYMENT_MODES_MAPPING.get(
                    payment_method.pine_labs_allowed_payment_mode
                ),
...
def _get_pine_labs_url(payment_method) -> str:
    if (
        payment_method.env["ir.config_parameter"]
        .sudo()
        .get_param("pos_pine_labs.pine_labs_proxy_endpoint", "")
    ):
        return (
            payment_method.env["ir.config_parameter"]
            .sudo()
