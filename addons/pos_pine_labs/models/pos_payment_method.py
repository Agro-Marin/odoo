...
class PosPaymentMethod(models.Model):
    ...
    def pine_labs_make_payment_request(self, data):
        ...
        response = call_pine_labs(
            payment_method=self, endpoint="UploadBilledTransaction", payload=body
        )
        if (
            response.get("ResponseCode") == 0
            and response.get("ResponseMessage") == "APPROVED"
        ):
            return {
                "responseCode": response["ResponseCode"],
                "status": response["ResponseMessage"],
                "plutusTransactionReferenceID": response[
    ...
    def pine_labs_fetch_payment_status(self, data):
        ...
        response = call_pine_labs(
            payment_method=self, endpoint="GetCloudBasedTxnStatus", payload=body
        )
        if response.get("ResponseCode") in [0, 1001]:
            formatted_transaction_data = (
                {d["Tag"]: d["Value"] for d in response["TransactionData"]}
    ...
    def pine_labs_cancel_payment_request(self, data):
        ...
        response = call_pine_labs(
            payment_method=self, endpoint="CancelTransactionForced", payload=body
        )
        if (
            response.get("ResponseCode") == 0
            and response.get("ResponseMessage") == "APPROVED"
    ...
    def _check_pine_labs_terminal(self):
        if any(
            record.use_payment_terminal == "pine_labs"
