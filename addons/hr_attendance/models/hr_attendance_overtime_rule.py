...
from odoo.libs.intervals import Intervals, boundaries, invert_intervals
...
def _record_overlap_intervals(intervals):
    edges = sorted(boundaries(intervals, "start", "stop"))
    counts = {}
    interval_vals = []
    ids = set()
    start = None
    for time, flag, records in edges:
...
class HrAttendanceOvertimeRule(models.Model):
    ...
    def _compute_information_display(self):
        timing_types = dict(
            self._fields["timing_type"]._description_selection(self.env)
        )
        for rule in self:
