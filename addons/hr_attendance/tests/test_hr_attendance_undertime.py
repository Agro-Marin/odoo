...
class TestHrAttendanceUndertime(HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.ruleset = (
            cls.env["hr.attendance.overtime.ruleset"]
    ...
    def test_undertime_lunch(self):
        ...
        self.assertEqual(
            ...
            -4,
            "There should be only -4 since the employee did not work through the lunch period.",
        )
    ...
    def test_undertime_hours_with_multiple_attendance(self):
        ...
        self.assertEqual(
            len(overtime_2),
            1,
            "Only one overtime record should be created for that day.",
        )
    ...
    def test_undertime_hours_flexible_resource(self):
        ...
        self.assertAlmostEqual(
            ...
            -2,
            2,
            "There should be -2 hours of overtime for the flexible resource.",
        )
    ...
    def test_overtime_employee_tolerance(self):
        ...
        self.assertFalse(
            overtime, "No overtime should be counted because of the tolerance."
        )
        ...
        self.assertTrue(
            overtime,
            "Overtime entry should exist since the tolerance has been lowered.",
        )
        self.assertAlmostEqual(
            overtime.duration,
            -(10 / 60),
    ...
    def test_overtime_on_multiple_days(self):
        attendance = self.env["hr.attendance"].create(
            {
                ...
                "check_in": datetime(
                    2021, 1, 8, 8, 0
                ),  # Friday 8 AM - 17 PM work, 17 - 24 overtime (7 hours)
                "check_out": datetime(
                    2021, 1, 9, 3, 0
                ),  # Saturday 0-3 AM overtime (3 hours)
            }
        )

        overtime = attendance._linked_overtimes()
        self.assertEqual(
            len(overtime), 2, "There should be 2 overtime records for that attendance."
        )
        self.assertEqual(
            sum(overtime.mapped("duration")),
            10,
            "There should be a total of 10 hours of overtime for that attendance.",
        )
        ...
        self.assertEqual(
            len(overtime),
            1,
            "There should have only 1 overtime for that attendance after modification.",
        )
        self.assertEqual(
            sum(overtime.mapped("duration")),
            3,
            "There should be a total of 3 hours of overtime for that attendance after modification.",
        )

        all_overtimes = self.env["hr.attendance.overtime.line"].search(
            [("employee_id", "=", self.employee.id)]
        )
        self.assertEqual(
            len(all_overtimes),
            1,
            "There should be only 1 overtime record in total for that employee.",
        )
