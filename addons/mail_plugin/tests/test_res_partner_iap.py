...
class TestResPartnerIap(MailCommon):
    ...
    def test_res_partner_iap_constraint(self):
        ...
        with self.assertRaises(
            psycopg.IntegrityError, msg="Can create only one partner IAP per partner"
        ):
            self.env["res.partner.iap"].create(
                {"partner_id": partner.id, "iap_enrich_info": "test info"}
            )
    ...
    def test_res_partner_iap_compute_iap_enrich_info(self):
        ...
        partner_iap = self.env["res.partner.iap"].create(
            {"partner_id": partner.id, "iap_enrich_info": "test info"}
        )
        ...
        partner_iap = self.env["res.partner.iap"].search(
        ...
        new_partner_iap = self.env["res.partner.iap"].search(
            [("partner_id", "=", partner.id)]
        )
        self.assertEqual(
            new_partner_iap, partner_iap, "Should have write on the existing one"
        )
    ...
    def test_res_partner_iap_creation(self):
        ...
        self.assertEqual(
            len(partner_iap), 1, "Should create only one <res.partner.iap>"
        )
