...
class EventTrackController(http.Controller):
    ...
    def _event_tracks_get_values(self, event, tag=None, **searches):
        ...
        now_tz = fields.Datetime.now().replace(microsecond=0).replace(tzinfo=UTC).astimezone(timezone(event.date_tz))
    ...
    def _get_dt_in_event_tz(self, datetimes, event):
        ...
        return [
            dt.replace(tzinfo=UTC).astimezone(timezone(tz_name))
            for dt in datetimes
        ]
