...
class Website(models.Model):
    ...
    def _compute_app_icon(self):
        ...
        for website in self:
            image = (
                ImageProcess(base64.b64decode(website.favicon))
                if website.favicon
                else None
            )
            ...
            website.app_icon = base64.b64encode(
                image.image_quality(output_format="PNG")
            )
    ...
    def _search_get_details(self, search_type, order, options):
        ...
        if search_type in ["track", "all"] and options.get("event"):
            result.append(
                self.env["event.track"]._search_get_detail(self, order, options)
            )
        return result
