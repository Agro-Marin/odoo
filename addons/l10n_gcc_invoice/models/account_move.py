    ...
    def _load_narration_translation(self):
        ...
        for move in self.filtered(
            lambda m: (
                ...
                and m.company_id.country_id
                and "GCC" in m.company_id.country_id.country_group_codes
            )
        ):
...
class AccountMoveLine(models.Model):
    ...
    l10n_gcc_invoice_tax_amount = fields.Float(
        string="Tax Amount", compute="_compute_tax_amount", digits="Product Price"
    )
    ...
    def _compute_tax_amount(self):
        for record in self:
            record.l10n_gcc_invoice_tax_amount = (
    ...
    def _compute_l10n_gcc_line_name(self):
        ...
        for line in self:
            if line.product_id and line.name in [
                lang_product_name(line, lang) for lang in ("ar_001", "en_US")
            ]:
                line.l10n_gcc_line_name = lang_product_name(
                    line, line.move_id.partner_id.lang
                )
            else:
    ...
    def _get_child_lines(self):
        ...
        for line in res:
            line["l10n_gcc_invoice_tax_amount"] = (
