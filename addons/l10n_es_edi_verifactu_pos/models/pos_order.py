...
class PosOrder(models.Model):
    ...
    def _compute_l10n_es_edi_verifactu_qr_code(self):
        for order in self:
            ...
            if invoice:
                ...
                last_submission = order.l10n_es_edi_verifactu_document_ids._get_last(
                    "submission"
                )
                url = (
                    last_submission._get_qr_code_img_url() if last_submission else False
                )
    ...
    def _l10n_es_edi_verifactu_get_record_values(self, cancellation=False):
        ...
        if refunded_order.account_move:
            ...
            refunded_document = refunded_order.account_move.l10n_es_edi_verifactu_document_ids._get_last(
                "submission"
            )
        else:
            refunded_document = (
        ...
        vals.update(
            {
                ...
                "is_simplified": not refunded_order
        ...
        ]._get_tax_details(base_lines, company)
    ...
    def _order_fields(self, ui_order):
        ...
        vals["l10n_es_edi_verifactu_refund_reason"] = ui_order.get(
            "l10n_es_edi_verifactu_refund_reason", False
        )
        return vals
    ...
    def _process_saved_order(self, draft):
        ...
        if self.l10n_es_edi_verifactu_required:
            ...
            if refunded_order:
                ...
                partner_specified = (
                    self.partner_id and self.partner_id != simplified_partner
                )
                if (
                    not partner_specified
    ...
    def _generate_pos_order_invoice(self):
        ...
        for order in self:
            ...
            # Cancel the order
            if order.l10n_es_edi_verifactu_state in (
                "accepted",
            ...
            invoice = order.account_move
            if (
                invoice.l10n_es_edi_verifactu_required
                and invoice
                and not invoice.l10n_es_edi_verifactu_document_ids
            ):
    ...
    def _prepare_invoice_vals(self):
        ...
        res["l10n_es_edi_verifactu_refund_reason"] = (
            self.l10n_es_edi_verifactu_refund_reason
        )
    ...
    def _update_sequence_number(self, session, values):
        ...
        if not session.config_id.l10n_es_edi_verifactu_required or not values.get(
            "to_invoice"
        ):
