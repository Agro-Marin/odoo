...
class TestL10nEsEdiVerifactuPosOrder(TestL10nEsEdiVerifactuPosCommon):
    ...
    def _create_order(self, data):
        ...
        if name:
            name_function_path = (
                "odoo.addons.point_of_sale.models.pos_order.PosOrder._get_order_name"
            )
        ...
        if account_move:
            ...
            prepare_invoice_vals_patch = mock.patch(
                prepare_invoice_vals_function_path, _patched_prepare_invoice_vals
            )
    ...
    def test_record_identifier(self):
        with self.with_pos_session():
            ...
            record_identifier = (
                order.l10n_es_edi_verifactu_document_ids._get_record_identifier()
            )
        ...
        with self.with_pos_session():
            with (
                self.assertRaisesRegex(
                    UserError,
                    "The order needs to be invoiced since its total amount is above 400.0€.",
                ),
    ...
    def test_order_not_invoiced(self):
        ...
        self.assertRecordValues(
            (order_document + refund_document),
            [
                {
        ...
        json_contents = self._json_file_to_dict(
            "l10n_es_edi_verifactu_pos/tests/files/test_order_not_invoiced_order.json"
        )
        ...
        json_contents = self._json_file_to_dict(
            "l10n_es_edi_verifactu_pos/tests/files/test_order_not_invoiced_refund.json"
        )
        json_contents["RegistroAlta"]["IDFactura"]["NumSerieFactura"] = (
            str(self.basic_config.id) + "/000002"
        )
        json_contents["RegistroAlta"]["FacturasRectificadas"][0][
    ...
    def test_order_invoiced_simplified(self):
        ...
        self.assertRecordValues(
            invoice.l10n_es_edi_verifactu_document_ids,
            [
                {
                    "pos_order_id": False,
                    "move_id": invoice.id,
