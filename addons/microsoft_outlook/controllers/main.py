...
class MicrosoftOutlookController(http.Controller):
    ...
    def microsoft_outlook_callback(
        ...
        if error_description:
            _logger.warning(
                "Microsoft Outlook: an error occurred %s", error_description
            )
        ...
        try:
            refresh_token, access_token, expiration = (
                record_sudo._get_outlook_refresh_token(code)
            )
        ...
        return self._check_email_and_redirect_to_outlook_record(
            access_token, expiration, refresh_token, record_sudo
        )

    @http.route("/microsoft_outlook/iap_confirm", type="http", auth="user")
    def microsoft_outlook_iap_callback(
        self, model, rec_id, csrf_token, access_token, refresh_token, expiration
    ):
        ...
        return self._check_email_and_redirect_to_outlook_record(
            access_token, expiration, refresh_token, record
        )
    ...
    def _check_email_and_redirect_to_outlook_record(
        self, access_token, expiration, refresh_token, record
    ):
        if record._name == "ir.mail_server" and (
            ...
            refresh_token, access_token, id_token, expiration = (
                record._get_outlook_access_token(refresh_token)
            )
            id_token_data = id_token.split(".")[1]
            id_token_data += "=" * (
                -len(id_token_data) % 4
            )  # `=` padding can be missing
            email = json.loads(base64.b64decode(id_token_data)).get("email")
            if not email or email_normalize(email) != email_normalize(
                record[record._email_field]
            ):
                _logger.error(
                    "Microsoft Outlook: Invalid email address: %r != %s.",
                    email,
                    record[record._email_field],
                )
                return request.render(
