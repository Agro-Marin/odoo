...
def import_ctypes_library(lib_subfolder, lib_name):
    ...
    try:
        lib_path = file_path(
            f"iot_drivers/iot_handlers/lib/{lib_subfolder}/{lib_name}",
            supported_lib_extensions,
        )
        ctypes_lib = import_library_method(lib_path)
        _logger.info(
            'Successfully imported ctypes library "%s" from %s', lib_name, lib_path
        )
        return ctypes_lib
    except OSError, ValueError, FileNotFoundError:
        _logger.exception(
            'Failed to import ctypes library "%s" from iot_drivers/iot_handlers/lib/%s/',
            lib_name,
            lib_subfolder,
        )
...
class CtypesTerminalDriver(Driver):
    ...
    def _action_default(self, data):
        ...
        _logger.debug(
            "%s: _action_default %s %s", self.device_name, data_message_type, data
        )
        if data_message_type in ["Transaction", "Balance"]:
            if self.terminal_busy:
                self.send_status(
                    error=f"{self.device_name} is currently busy. Try again later.",
                    request_data=data,
                )
            else:
    ...
    def run(self):
        while True:
            ...
            _logger.debug(
                "%s: Starting next action in queue: %s", self.device_name, action_type
            )
    ...
    def _check_transaction_delay(self):
        ...
        if delay_diff > 0:
            ...
            _logger.info(
                "%s: Previous transaction is too recent, will sleep for %.2f seconds",
                self.device_name,
                delay_diff,
            )
            sleep(delay_diff)
    ...
    def send_status(
        ...
        _logger.debug(
            "%s: send_status data: %s", self.device_name, self.data, stack_info=True
        )
