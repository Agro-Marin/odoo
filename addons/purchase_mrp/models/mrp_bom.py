    ...
    def _check_bom_lines(self):
        ...
        for bom in self:
            ...
            for product in bom.product_tmpl_id.product_variant_ids:
                total_variant_cost_share = sum(bom.bom_line_ids.filtered(lambda bl, product=product: not bl._skip_bom_line(product) and not bl.product_uom_id.is_zero(bl.product_qty)).mapped('cost_share'))
