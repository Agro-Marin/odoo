...
class ReportMrpReport_Mo_Overview(models.AbstractModel):
        ...
        for po_line in po_lines:
            ...
            for move in dest_moves:
                if not move.raw_material_production_id:
                    continue
                prod_qty = min(line_qty, move.product_uom_id._compute_quantity_report(move.product_uom_qty, po_line.product_uom_id))
        ...
        if any(rule for rule in rules if rule.action == 'buy' and product.seller_ids):
            ...
            if supplier:
                return {
                    ...
                    'cost': supplier.price * uom_id._compute_quantity_report(quantity, supplier.product_uom_id),
    ...
    def _is_doc_in_done(self, doc_in):
        if doc_in._name == 'purchase.order':
            return doc_in.state == "done" and all(move.state in ('done', 'cancel') for move in doc_in.line_ids.move_ids)
    ...
    def _get_replenishment_mo_cost(self, product, quantity, uom_id, currency, move_in=False):
        if move_in and move_in.purchase_line_id:
            ...
            price = po_line.tax_ids.compute_all(
                po_line.price_unit,
                currency=po.currency_id,
                quantity=uom_id._compute_quantity_report(quantity, move_in.purchase_line_id.product_uom_id),
                product=po_line.product_id,
                partner=po.partner_id,
                rounding_method='round_globally',
            )['total_void']
