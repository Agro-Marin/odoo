...
class ReportMrpReport_Mo_Overview(models.AbstractModel):
    ...
    def _get_extra_replenishments(self, product):
        ...
        for po_line in po_lines:
            ...
            for move in dest_moves:
                ...
                res.append(
                    self._format_extra_replenishment(
                        po_line, prod_qty, move.raw_material_production_id.id
                    )
                )
                line_qty -= prod_qty
            if line_qty:
    ...
    def _get_replenishment_receipt(self, doc_in, components):
        ...
        if doc_in._name == "purchase.order":
            ...
            in_pickings = doc_in.picking_ids.filtered(
                lambda p: p.state not in ("done", "cancel")
            )
            planned_date = (
                max(in_pickings.mapped("date_planned"))
                if in_pickings
                else doc_in.date_commitment
            )
    ...
    def _is_doc_in_done(self, doc_in):
        if doc_in._name == "purchase.order":
            return doc_in.state == "done" and all(
                move.state in ("done", "cancel") for move in doc_in.line_ids.move_ids
            )
    ...
    def _get_replenishment_mo_cost(
        ...
        if move_in and move_in.purchase_line_id:
            ...
            price = po_line.currency_id._convert(
                price,
                currency,
                (move_in.company_id or self.env.company),
                fields.Date.today(),
            )
            return currency.round(price)
        return super()._get_replenishment_mo_cost(
            product, quantity, uom_id, currency, move_in
        )
