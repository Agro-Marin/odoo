...
class TestAccountDebitNote(AccountTestInvoicingCommon):
    def test_00_debit_note_out_invoice(self):
        ...
        self.assertEqual(
            len(debit_note.invoice_line_ids), 2, "Should have copied the invoice lines"
        )
        self.assertEqual(
            debit_note.move_type,
            "out_invoice",
            "Type of debit note should be the same as the original invoice",
        )
        self.assertEqual(
    ...
    def test_10_debit_note_in_refund(self):
        ...
        self.assertFalse(
            ...
            "We should not copy lines by default on debit notes",
        )
        self.assertEqual(
            debit_note.move_type,
            "in_invoice",
            "Type of debit note should not be refund anymore",
        )
        self.assertEqual(
            debit_note.state, "draft", "We should create debit notes in draft state"
        )
