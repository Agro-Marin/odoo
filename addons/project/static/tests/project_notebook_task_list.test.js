...
test("test Project Task Calendar Popover with task_step_with_state_selection widget", async () => {
    ...
    expect(
        "div[name='predecessor_ids'] .o_field_x2many_list_row_add a.o_toggle_closed_task_button",
    ).toHaveText("Hide closed tasks");
    ...
    expect('div[name="predecessor_ids"] .o_data_row').toHaveCount(2, {
        message:
            "The depend on tasks list should still display all blocking tasks, in this case 2",
    });
    ...
    expect('div[name="predecessor_ids"] .o_data_row').toHaveCount(1, {
        message:
            "The depend on tasks list should only display open tasks, in this case 1",
    });
});
