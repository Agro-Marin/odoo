...
registry.category("web_tour.tours").add("project_task_history_tour", {
    ...
    steps: () => [
        ...
        {
            content:
                "Go back to kanban view of tasks. this step is added because it takes some time to save the changes, so it's a sort of timeout to wait a bit for the save",
        ...
        },
        {
            ...
            run: function () {
                ...
                if (items.length !== 5) {
                    throw new Error(
                        "Expect 5 Revisions in the history dialog, got " + items.length,
                    );
                }
            },
        },
        ...
        {
            content:
                "Verify that the active revision is the one clicked in the previous step",
        ...
        },
        {
            ...
            run: function () {
                ...
                if (!comparaisonHtml.includes(correctHtml)) {
                    throw new Error(
                        `Expect comparison to be ${correctHtml}, got ${comparaisonHtml}`,
                    );
                }
            },
        },
        ...
        {
            content:
                "Go back to projects view. this step is added because Tour can't be finished with an open form view in edition mode.",
            trigger: 'a[data-menu-xmlid="project.menu_projects"]',
            run: "click",
        },
        {
    ...
    steps: () => [
        ...
        },
        {
            ...
            run: function () {
                ...
                if (items.length !== 2) {
                    throw new Error(
                        "Expect 2 Revisions in the history dialog, got " + items.length,
                    );
                }
            },
        },
