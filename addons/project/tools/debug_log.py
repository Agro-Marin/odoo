"""Temporary debug instrumentation for the project quality-improvement campaign.

Four loggers under ``odoo.addons.project.debug``; enable one or all with
``--log-handler odoo.addons.project.debug:DEBUG`` (or ``.logic``, ``.performance``,
``.pipeline``, ``.lifecycle``). Every call site imports this module as ``dbg``,
so ``grep -rn 'dbg\\.' addons/project`` lists the whole instrumentation for removal.
...
- ``pipeline``: hand-offs between stages of a flow (task create -> stage ->
  dependency -> recurrence; portal request -> access check -> render), tagged
  ``[<kind>:<ref>]`` for correlation.
...
_ROOT = "odoo.addons.project.debug"
DEBUG = logging.DEBUG
