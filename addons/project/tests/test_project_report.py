...
class TestProjectReport(TestProjectCommon):
    def test_avg_rating_measure(self) -> None:
        rating_vals = {
        ...
        for task in tasks:
            ...
            task_report = self.env["report.project.task.user"].search_read(
                [
                    ("project_id", "=", self.project_pigs.id),
                    ("task_id", "=", task.id),
                ],
                ["rating_avg", "rating_last_value"],
            )[0]
            self.assertDictEqual(
                task_report,
                rating_values,
                "The rating average and the last rating value for the task 1 should be the same in the report and on the task.",
            )
