...
class TestTaskTemplates(TestProjectCommon):
    ...
    def test_create_from_template(self) -> None:
        ...
        self.assertFalse(
            task.is_template,
            "The created task should be a normal task and not a template.",
        )
        ...
        self.assertFalse(
            child_task.is_template,
            "The child task should still not be a template.",
        )
        self.assertFalse(
            child_task.partner_id,
        ...
        self.assertEqual(
            task.partner_id,
            self.partner_3,
            "The created task should have the same partner as the project.",
        )
        child_task = task.child_ids
        self.assertEqual(
            child_task.partner_id,
            self.partner_3,
            "The child of the created task should have the same partner as the project.",
        )
    ...
    def test_copy_template(self) -> None:
        ...
        self.assertEqual(
            len(copied_template.child_ids),
            1,
            "The child of the template should be copied too.",
        )
        copied_template_child_task = copied_template.child_ids
        self.assertFalse(
            copied_template_child_task.is_template,
            "The child of the copy should still not be a template.",
        )
    ...
    def test_copy_project_with_templates(self) -> None:
        ...
        self.assertTrue(
            task, "The copied project should contain a copy of the template."
        )
        self.assertTrue(
            task.is_template, "The copied template should still be a template."
        )

    def test_has_template_ancestor(self) -> None:
        self.assertTrue(
            self.template_task.has_template_ancestor,
            "The template is a template.",
        )
        self.assertTrue(
            self.child_task.has_template_ancestor,
            "The child of the template has a template ancestor.",
        )
        ...
        self.assertFalse(
            task.has_template_ancestor,
            "The task does not have ancestors and is not a template.",
        )
        ...
        self.assertFalse(
            child.has_template_ancestor,
            "The task has ancestors, but none of them are templates.",
        )
