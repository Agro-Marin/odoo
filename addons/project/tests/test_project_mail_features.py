...
class TestProjectMailFeatures(TestProjectCommon, MailCommon):
            ...
            self.env["res.partner"].search(
                [
                    (
                        ...
                        "in",
                        {
                            "new.cc@test.agrolait.com",
                            "new.customer@test.agrolait.com",
                            "new.author@test.agrolait.com",
                        },
                    )
                ]
            )
    ...
    def test_project_notify_get_recipients_groups(self) -> None:
            ...
            [
                {
                    ...
                    "notif": [
                        {
                            "partner": self.user_projectmanager.partner_id,
                            "type": "inbox",
                        },
                        {
                            "partner": self.user_projectuser.partner_id,
                            "type": "email",
                        },
                    ],
                },
    ...
    def test_task_creation_notifies_author(self) -> None:
        ...
        for test_user in (self.user_employee, self.user_portal, False):
            with self.subTest(
                user_name=test_user.name if test_user else new_partner_email
            ):
                email_from = (
                    test_user.email_formatted if test_user else new_partner_email
                )
                with self.mock_mail_gateway():
                    task = self.format_and_process(
                        MAIL_TEMPLATE,
                ...
                if test_user:
                    ...
                    self.assertTrue(
                        author,
                        "Project automatically creates a partner for incoming email",
                    )
                    self.assertEqual(
                        author.email,
                        "new.author@test.agrolait.com",
                ...
                self.assertEqual(
                    task.email_cc,
                    f'"New Cc" <new.cc@test.agrolait.com>, {self.partner_2.email_formatted}, {self.partner_1.email_formatted}, "New Customer" <new.customer@test.agrolait.com>',
                )
                ...
                self.assertEqual(
                    new_partner_customer.email, "new.customer@test.agrolait.com"
                )
                ...
                with self.mock_mail_gateway():
                    ...
                    if not test_user:
                        recipients += author
                    responsible_answer = task.with_user(
                        self.user_projectuser
                    ).message_post(
                ...
                self.assertEqual(
                    len(task.message_ids),
                    4,
                    "Incoming email + acknowledgement + chatter reply + customer reply",
                )
                self.assertEqual(
                    task.message_partner_ids,
                    internal_followers
    ...
    def test_mail_alais_assignees_from_recipient_list(self) -> None:
        ...
        for incoming_to in [incoming_to_emails_with_name, incoming_to_emails]:
            ...
            self.assertEqual(
                task.email_cc,
                new_user.email,
                "The internal user in CC is not added into email_cc field",
            )
...
"""
        ...
        self.assertIn(
            ...
            gmail_task.description,
        )
        self.assertNotIn(
            "--",
            gmail_task.description,
            "The Gmail signature separator should have been removed.",
        )
        self.assertNotIn(
            "John Doe",
            gmail_task.description,
