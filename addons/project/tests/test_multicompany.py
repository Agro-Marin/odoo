    ...
    def switch_company(self, company) -> None:
        ...
        try:
            ...
            if company.id not in new_companies:
                ...
                new_companies.insert(
                    0, new_companies.pop(new_companies.index(company.id))
                )
...
class TestMultiCompanyProject(TestMultiCompanyCommon):
    ...
    def test_generate_analytic_account(self) -> None:
        ...
        with self.sudo("manager-b"):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                self.assertEqual(
                    ...
                    self.project_company_a.account_id.company_id,
        ...
        self.assertNotEqual(
            project_no_company_2.account_id,
            project_no_company.account_id,
            "The analytic account created should be different from the account created for the 1st project.",
        )
        self.assertEqual(
            project_no_company_2.account_id.plan_id,
            project_no_company.account_id.plan_id,
    ...
    def test_analytic_account_company_consistency(self) -> None:
        ...
        self.assertFalse(
            account_no_company.company_id,
            "The company of the account should not have been updated.",
        )
    ...
    def test_update_company_id(self) -> None:
        ...
        self.assertFalse(
            task.company_id,
            "Creating a task in a project without company set its company_id to False.",
        )
        ...
        self.assertEqual(
            task.company_id,
            self.company_a,
            "The company of the task should have been updated.",
        )
        ...
        self.assertEqual(
            project.company_id,
            self.company_b,
            "the company of the project should not have been updated.",
        )
        ...
        for task in project.task_ids:
            self.assertFalse(
                task.company_id, "The tasks should not have a company_id set."
            )
        ...
        self.assertEqual(
            task_1.company_id,
            self.company_a,
            "The company of task_1 should have been set to company A.",
        )
        self.assertEqual(
            task_2.company_id,
            self.company_b,
            "The company of task_2 should have been set to company B.",
        )
        self.assertFalse(
            task_3.company_id,
            "The company of the task_3 should not have been updated.",
        )
        self.assertFalse(
            project.company_id,
            "The company of the project should not have been updated.",
        ...
        with self.sudo("employee-a"):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                self.assertEqual(
                    task.company_id,
                    self.company_a,
                    "Moving a task should change its company.",
                )
    ...
    def test_create_subtask(self) -> None:
        ...
        self.assertEqual(
            subtask.company_id,
            self.project_company_b.company_id,
            "The company of the subtask should be the one from its project.",
        )
        ...
        with self.debug_mode(), Form(subtask) as subtask_form:
            subtask_form.parent_id = self.task_2
            self.assertEqual(
                subtask_form.project_id,
                self.task_2.project_id,
                "The task's project should already be set on the subtask.",
            )
            self.assertFalse(
                subtask.display_in_project,
        ...
        self.assertTrue(
            subtask.display_in_project,
            "The subtask's field 'display in project' should be checked.",
        )
        self.assertEqual(
            subtask.company_id,
            self.project_company_a.company_id,
        ...
        with self.sudo("employee-a"):
            with self.assertRaises(AccessError):
                with Form(task) as task_form:
                    task_form.name = (
                        "Testing changing name in a company I can not read/write"
                    )

    @freeze_time("2019-5-28 08:00:00")
    def test_date_to_assign_project(self) -> None:
        ...
        project = (
            ...
            .create({"name": "Project for company 1"})
        )
        task = (
