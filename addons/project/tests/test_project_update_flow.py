...
class TestProjectUpdate(TestProjectCommon):
    ...
    def test_project_update_form(self) -> None:
        with Form(
        ...
        self.assertEqual(
            update.user_id,
            self.env.user,
            "The author is the user who created the update.",
        )
        self.assertNotEqual(
            len(update.description), 0, "The description should not be empty."
        )
        self.assertTrue(
            "Activities" in update.description,
            "The description should contain 'Activities'.",
        )
        self.assertEqual(
            update.status, "on_track", "The status should be the default one."
        )
        ...
        self.assertEqual(
            update.progress,
            65,
            "The default progress is the one from the previous update by default",
        )
    ...
    def test_project_update_description(self) -> None:
        ...
        self.assertTrue(
            template_values["milestones"]["show_section"],
            "The milestone section should be visible since the feature is enabled",
        )
        self.assertEqual(
            len(template_values["milestones"]["list"]),
            2,
            "Milestone list length should be equal to 2",
        )
        self.assertEqual(
            len(template_values["milestones"]["created"]),
            3,
            "Milestone created length tasks should be equal to 3",
        )
        ...
        self.assertFalse(
            template_values["milestones"]["show_section"],
            "The milestone section should not be visible since the feature is disabled",
        )
        self.assertEqual(
            len(template_values["milestones"]["list"]),
            0,
            "Milestone list length should be equal to 0 because the Milestones feature is disabled.",
        )
        self.assertEqual(
            len(template_values["milestones"]["created"]),
            0,
            "Milestone created length tasks should be equal to 0 because the Milestones feature is disabled.",
        )

    def test_project_update_panel(self) -> None:
        with Form(
            self.env["project.milestone"].with_context(
                {"default_project_id": self.project_pigs.id}
            )
        ...
        self.assertEqual(
            len(panel_data["milestones"]["data"]),
            3,
            "Panel data should contain 'milestone' entry",
        )
        self.assertFalse(
            panel_data["milestones"]["data"][0]["is_deadline_exceeded"],
            "Milestone is achieved",
        )
        self.assertTrue(
            panel_data["milestones"]["data"][1]["is_deadline_exceeded"],
            "Milestone is exceeded",
        )
        self.assertTrue(
            panel_data["milestones"]["data"][0]["is_reached"],
            "Milestone is done",
        )
        self.assertFalse(
        ...
        self.assertEqual(
            panel_data["milestones"]["data"][1]["name"],
            "Test 1",
            "Sorting isn't correct",
        )
        self.assertEqual(
            panel_data["milestones"]["data"][2]["name"],
            "Test 3",
            "Sorting isn't correct",
        )
        ...
        self.assertNotIn(
            "milestones",
            panel_data,
            'Since the "Milestones" feature is globally disabled, the "Milestones" section is not loaded.',
        )
    ...
    def test_project_update_reflects_task_changes(self) -> None:
        ...
        def create_project_update_view() -> None:
            ...
            return [
                project_update.task_count,
                project_update.closed_task_count,
                project_update.closed_task_percentage,
            ]
