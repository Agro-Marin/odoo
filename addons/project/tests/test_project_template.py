...
class TestProjectTemplates(TestProjectCommon):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.task_inside_template, cls.task_template_inside_template = cls.env[
            "project.task"
        ].create(
            [
                {
    ...
    def test_convert_project_to_template(self) -> None:
        ...
        created_template = self.env["project.project"].browse(
            client_action["params"]["project_id"]
        )
        task_a, task_b = created_template.task_ids.sorted("name")
        self.assertEqual(
            task_a.name,
            self.task_inside_template.name,
            "The copied task should have the same name as the original.",
        )
        self.assertFalse(
            task_a.is_template,
            "The copied tasks should retain their template status.",
        )
        self.assertEqual(
            task_b.name,
            self.task_template_inside_template.name,
            "The copied task should have the same name as the original.",
        )
        self.assertTrue(
            task_b.is_template,
    ...
    def test_create_from_template(self) -> None:
        ...
        self.assertEqual(
            project.name,
            "Project Template",
            "The project name should be `Project Template`.",
        )
        self.assertFalse(
            project.is_template,
            "The created Project should be a normal project and not a template.",
        )
        self.assertFalse(
            project.partner_id,
            "The created Project should not have a customer.",
        )

        self.assertEqual(
            len(project.task_ids),
            2,
            "The tasks of the template should be copied too.",
        )
    ...
    def test_copy_template(self) -> None:
        ...
        self.assertEqual(
            ...
            "The project name should be `Project Template` (copy).",
        )
        self.assertTrue(
            copied_template.is_template,
            "The copy of the template should also be a template.",
        )
        self.assertEqual(
            len(copied_template.task_ids),
            2,
            "The child of the template should be copied too.",
        )
    ...
    def test_revert_template(self) -> None:
        ...
        self.assertFalse(
            ...
            "The reverted template should become a normal template.",
        )
    ...
    def test_create_from_template_no_dates(self) -> None:
        ...
        self.assertEqual(
            new_project.date_start, today, "Start date should default to today"
        )
        self.assertEqual(
            new_project.date,
            today + expected_delta,
            "End date should be start + template delta",
        )

    def test_create_from_template_start_only(self) -> None:
        ...
        expected_delta = self.project_template.date - self.project_template.date_start
        self.assertEqual(
            new_project.date_start,
            new_start,
            "Start date should be the one provided",
        )
        self.assertEqual(
            new_project.date,
            new_start + expected_delta,
            "End date should be start + template delta",
        )

    def test_create_from_template_with_dates(self) -> None:
        new_start = date(2025, 7, 1)
        ...
        self.assertEqual(
            new_project.date, new_end, "End date should be the one provided"
        )
