...
class TestProjectSharingPortalAccess(TestProjectSharingCommon):
    ...
    def test_mention_suggestions(self) -> None:
        data = self.task_portal.with_user(self.user_portal).get_mention_suggestions(
            search=""
        )
        ...
        self.assertEqual(
            suggestion_ids,
            {
                self.user_projectuser.partner_id.id,
    ...
    def test_readonly_fields(self) -> None:
        ...
        with self.get_project_sharing_form_view(
            self.task_portal, self.user_portal
        ) as form:
    ...
    def test_read_task_with_portal_user(self) -> None:
    ...
    def test_write_task_with_portal_user(self) -> None:
        ...
        def dummy_value(field_name) -> None:
        ...
        for field_name in self.write_protected_fields_task:
            ...
            if field.comodel_name == "project.task":
                other_task = self.env["project.task"].create(
                    {"name": "Parent task", "project_id": task.project_id.id}
                )
                ...
                self.assertEqual(
                    actual_value,
                    expected_value,
                    f"Field {field} should be editable.",
                )

        for field in self.readonly_protected_fields_task:
            with self.assertRaises(
                AccessError, msg=f"Field {field} should be readonly"
            ):
    ...
    def test_wizard_confirm(self) -> None:
        ...
        project_share_wizard_no_user_action = (
            project_share_wizard_no_user.action_share_record()
        )
        self.assertEqual(
            project_share_wizard_no_user_action["type"],
        ...
        mail_partner = self.env["mail.message"].search(
            [("partner_ids", "=", partner_portal_no_user.id)], limit=1
        )
        self.assertTrue(
...
class TestProjectSharingChatterAccess(TestProjectSharingCommon, HttpCase):
    ...
    def test_post_chatter_as_portal_user(self) -> None:
