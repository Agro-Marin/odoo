...
class TestProjectEmbeddedActionSettings(TestProjectCommon):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.embedded_action_1, cls.embedded_action_2 = cls.env[
            "ir.embedded.actions"
        ].create(
            [
                {
        ...
        cls.embedded_action_settings = cls.env[
    ...
    def test_copy_embedded_action_settings(self) -> None:
        ...
        self.assertEqual(
            len(copied_settings),
            1,
            "There should be one embedded action setting for the copied project.",
        )
        self.assertEqual(
            len(self.user_settings.embedded_actions_config_ids),
            2,
            "There should be two embedded action settings after copying the project.",
        )
        self.assertEqual(
            copied_settings.action_id,
            self.embedded_action_settings.action_id,
            "The action_id should match the original.",
        )
        self.assertEqual(
            copied_settings.res_id,
            copied_project.id,
            "The res_id should match the copied project id.",
        )
        self.assertEqual(
            copied_settings.res_model,
            self.embedded_action_settings.res_model,
            "The res_model should match the original.",
        )
        self.assertEqual(
            copied_settings.embedded_actions_order,
            self.embedded_action_settings.embedded_actions_order,
            "The embedded actions order should match the original.",
        )
        self.assertEqual(
            copied_settings.embedded_actions_visibility,
            self.embedded_action_settings.embedded_actions_visibility,
            "The embedded actions visibility should match the original.",
        )
        self.assertEqual(
            copied_settings.embedded_visibility,
            self.embedded_action_settings.embedded_visibility,
            "The embedded visibility should match the original.",
        )
    ...
    def test_copy_custom_embedded_action_settings(self) -> None:
        ...
        self.assertEqual(
            len(self.user_settings.embedded_actions_config_ids),
            3,
            "There should be three embedded action settings after copying the project.",
        )
        self.assertEqual(
            copied_settings.action_id,
            self.embedded_action_settings.action_id,
        ...
        self.assertEqual(
            copied_settings.embedded_actions_order,
            str(copied_shared_embedded_action.id),
            "Only the shared embedded action should be copied in the actions order.",
        )
        self.assertEqual(
            copied_settings.embedded_actions_visibility,
            str(copied_shared_embedded_action.id),
            "Only the shared embedded action should be copied in the visible actions.",
        )
        self.assertEqual(
            copied_settings.embedded_visibility,
            self.embedded_action_settings.embedded_visibility,
