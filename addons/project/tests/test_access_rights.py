...
class TestAccessRights(TestProjectCommon):
    def setUp(self) -> None:
        ...
        self.user = mail_new_test_user(
            self.env, "Internal user", groups="base.group_user"
        )
        self.portal = mail_new_test_user(
            self.env, "Portal user", groups="base.group_portal"
        )

    def create_task(self, name, *, with_user=None, **kwargs) -> None:
        values = dict(name=name, project_id=self.project_pigs.id, **kwargs)
        return (
            self.env["project.task"]
            .with_user(with_user or self.env.user)
            .create(values)
        )
...
class TestCRUDVisibilityFollowers(TestAccessRights):
    ...
    def test_project_no_write(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to write on the project" % self.env.user.name,
        ):
    ...
    @users("Internal user", "Portal user")
    def test_project_no_unlink(self) -> None:
        self.project_pigs.task_ids.unlink()
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to unlink the project" % self.env.user.name,
        ):
    ...
    @users("Internal user", "Portal user")
    def test_project_no_read(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to read the project" % self.env.user.name,
        ):
            _ = self.project_pigs.with_user(self.env.user).name

    @users("Portal user")
    def test_project_allowed_portal_no_read(self) -> None:
    ...
    @users("Internal user", "Portal user")
    def test_task_no_read(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to read the task" % self.env.user.name,
        ):
            _ = self.task.with_user(self.env.user).name

    @users("Portal user")
    def test_task_allowed_portal_no_read(self) -> None:
        self.project_pigs.privacy_visibility = "portal"
        self.project_pigs.message_subscribe(partner_ids=[self.env.user.partner_id.id])
        self.project_pigs.privacy_visibility = "followers"
        with self.assertRaises(
    ...
    @users("Internal user", "Portal user")
    def test_task_no_write(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to write on the task" % self.env.user.name,
        ):
    ...
    @users("Internal user", "Portal user")
    def test_task_no_create(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to create a task" % self.env.user.name,
        ):
    ...
    @users("Internal user", "Portal user")
    def test_task_no_unlink(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to unlink the task" % self.env.user.name,
        ):
...
class TestCRUDVisibilityPortal(TestAccessRights):
    ...
    @users("Portal user")
    def test_task_portal_no_read(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="%s should not be able to read the task" % self.env.user.name,
        ):
    ...
    @users("Portal user")
    def test_task_allowed_portal_read(self) -> None:
        self.project_pigs.message_subscribe(partner_ids=[self.env.user.partner_id.id])
        self.task.flush_model()
        self.task.invalidate_model()
        with self.assertRaises(
            AccessError,
            msg=f"{self.env.user.name} should not be able to read the task",
        ):
            _ = self.task.with_user(self.env.user).name

    @users("Internal user")
    def test_task_internal_read(self) -> None:
        self.task.flush_model()
        self.task.invalidate_model()
        _ = self.task.with_user(self.env.user).name
...
class TestAllowedUsers(TestAccessRights):
    ...
    def test_project_specific_permission(self) -> None:
        ...
        self.assertNotIn(
            ...
            "John should not be allowed to read the task",
        )
        task = self.create_task("New task")
        self.assertIn(
            john.partner_id,
            task.message_partner_ids,
            "John should allowed to read the task",
        )

    def test_project_specific_remove_mutliple_tasks(self) -> None:
        self.project_pigs.message_subscribe(partner_ids=[self.user.partner_id.id])
        john = mail_new_test_user(self.env, "John")
    ...
    def test_visibility_changed(self) -> None:
        self.project_pigs.privacy_visibility = "portal"
    ...
    def test_no_write_project(self) -> None:
        self.user.group_ids |= self.env.ref("project.group_project_user")
        self.assertNotIn(self.user.partner_id, self.project_pigs.message_partner_ids)
        with self.assertRaises(
            AccessError, msg="User should not be able to edit project"
        ):
...
class TestPortalProject(TestProjectPortalCommon):
    ...
    def test_employee_project_access_rights(self) -> None:
...
class TestAccessRightsPrivateTask(TestAccessRights):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.private_task = cls.env["project.task"].create(
            {"name": "OdooBot Private Task"}
        )

    def setUp(self) -> None:
        super().setUp()
        self.project_user = mail_new_test_user(
            self.env, "Project user", groups="project.group_project_user"
        )

    def create_private_task(self, name, with_user=None, **kwargs) -> None:
    ...
    @users("Project user")
    def test_project_user_crud_own_private_task(self) -> None:
    ...
    @users("Project user")
    def test_project_user_can_create_private_task_for_another_user(
        self,
    ) -> None:
    ...
    @users("Project user")
    def test_project_current_user_is_added_in_private_task_assignees(
        self,
    ) -> None:
        task_values = {"name": "Private task"}
        my_private_task = self.env["project.task"].create(task_values)
        self.assertEqual(
            my_private_task.user_ids,
            self.env.user,
        ...
        self.assertTrue(
            self.env.user in user_projectuser_private_task.user_ids,
            "When creating a private task for another user, the current user should be added to the assignees.",
        )

    @users("Project user")
    def test_project_current_user_is_added_in_task_assignees_when_project_id_is_set(
        self,
    ) -> None:
        task_values = {
            "name": "Private task",
        ...
        self.assertFalse(
            self.env.user in user_projectuser_task.user_ids,
            "When creating a task that has a project for another user, the current user should not be added to the assignees.",
        )

    @users("Project user")
    def test_project_current_user_is_set_as_assignee_in_task_when_project_id_is_set_with_no_assignees(
        self,
    ) -> None:
        task = self.env["project.task"].create(
            {"name": "Private task", "project_id": self.project_pigs.id}
        )
        self.assertEqual(
            task.user_ids,
            self.env.user,
    ...
    def test_project_current_user_is_not_added_in_private_task_assignees_when_default_project_id_is_in_the_context(
        ...
        ProjectTask_with_default_project_id = self.env["project.task"].with_context(
            context
        )
    ...
    @users("Project user")
    def test_project_user_cannot_write_private_task_of_another_user(
        self,
    ) -> None:
        with self.assertRaises(AccessError):
            self.private_task.with_user(self.env.user).write({"name": "Test write"})
    ...
    @users("Project user")
    def test_project_user_cannot_read_private_task_of_another_user(
        self,
    ) -> None:
        with self.assertRaises(AccessError):
            self.private_task.with_user(self.env.user).read(["name"])
    ...
    @users("Project user")
    def test_project_user_cannot_unlink_private_task_of_another_user(
        self,
    ) -> None:
        with self.assertRaises(AccessError):
            self.private_task.with_user(self.env.user).unlink()
...
class TestAccessRightsInvitedUsers(TestAccessRights):
    ...
    @users("admin")
    def test_admin_access_invited_task(self) -> None:
        self.assertEqual(
    ...
    @users("Project user", "Internal user", "Portal user")
    def test_other_users_access_invited_task(self) -> None:
        with self.assertRaises(
            AccessError,
            msg="The user is not a follower of the project, he's not supposed to have access to the project.",
        ):
