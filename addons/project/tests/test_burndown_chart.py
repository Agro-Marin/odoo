...
class TestBurndownChartCommon(TestProjectCommon):
    @classmethod
    def set_create_date(cls, table, res_id, create_date) -> None:
        cls.env.cr.execute(
            f"UPDATE {table} SET create_date=%s WHERE id=%s",
            (create_date, res_id),
        )
    ...
    def setUpClass(cls) -> None:
        ...
        cls.task_b = cls.task_a.copy(
            {
                "name": "Task B",
                "user_ids": [
                    Command.set([cls.user_projectuser.id, cls.user_projectmanager.id])
                ],
            }
        )
        ...
        cls.set_create_date(
            "project_task",
            cls.task_f.id,
            datetime(cls.current_year - 1, 12, 20),
        )
        ...
        cls.deleted_domain = Domain("project_id", "!=", False) & Domain(
            "project_id", "=", cls.project_1.id
        )
...
class TestBurndownChart(TestBurndownChartCommon):
    ...
    def map_read_group_is_closed_result(self, read_group_result) -> None:
        return {
            (res["date:month"][1], res["is_closed"]): int(res["__count"])
            for res in read_group_result
        }
    ...
    def check_read_group_is_closed_results(self, domain, expected_results_dict) -> None:
        read_group_result = self.env[
            "project.task.burndown.chart.report"
        ].formatted_read_group(domain, ["date:month", "is_closed"], ["__count"])
        read_group_result_dict = self.map_read_group_is_closed_result(read_group_result)
        self.assertDictEqual(read_group_result_dict, expected_results_dict)

    def test_burndown_chart(self) -> None:
        ...
        project_expected_dict = {
            ...
            (
                "February %s" % (self.current_year - 1),
                self.in_progress_stage.id,
            ): 3,
            (
                "March %s" % (self.current_year - 1),
                self.in_progress_stage.id,
            ): 5,
            (
                "April %s" % (self.current_year - 1),
                self.in_progress_stage.id,
            ): 3,
            ...
            (
                "September %s" % (self.current_year - 1),
                self.testing_stage.id,
            ): 3,
        ...
        self.check_read_group_is_closed_results(
            Domain.AND([burndown_chart_domain, project_domain]),
            project_expected_is_closed_dict,
        )
        ...
        project_expected_dict = {
            key: val if key[1] != self.todo_stage.id else val + 2
            for key, val in project_expected_dict.items()
        }
        project_expected_is_closed_dict = {
            key: val if key[1] == "closed" else val + 2
            for key, val in project_expected_is_closed_dict.items()
        }
        for i in range(2, 11):
            month_key = f"{months[i]} {self.current_year - 1}"
            project_expected_dict[(month_key, self.todo_stage.id)] = 2
        project_expected_is_closed_dict[
            (f"{months[11]} {self.current_year - 1}", "open")
        ] = 2
        for i in range(current_month):
            project_expected_is_closed_dict[
                (f"{months[i]} {self.current_year}", "open")
            ] = 2
        self.check_read_group_results(
            Domain.AND([burndown_chart_domain, all_projects_domain_with_ilike]),
            project_expected_dict,
        )
        self.check_read_group_is_closed_results(
            Domain.AND([burndown_chart_domain, all_projects_domain_with_ilike]),
        ...
        date_from, date_to = (
            "%s-01-01" % (self.current_year - 1),
            "%s-03-01" % (self.current_year - 1),
        )
        date_from_is_closed, date_to_is_closed = (
            "%s-10-01" % (self.current_year - 1),
            "%s-12-01" % (self.current_year - 1),
        )

        date_and_user_domain = [
            ("date", ">=", date_from),
            ("date", "<", date_to),
        ...
        complex_domain = Domain.AND(
            [
                burndown_chart_domain,
                all_projects_domain_with_ilike,
                date_and_user_domain,
            ]
        )
        ...
        date_and_user_domain = [
            ("date", ">=", date_from_is_closed),
            ("date", "<", date_to_is_closed),
            ("user_ids", "ilike", "ProjectUser"),
        ]
        complex_domain = Domain.AND(
            [
        ...
        self.check_read_group_is_closed_results(
            complex_domain, complex_domain_expected_dict
        )
    ...
    def test_burndown_chart_stage_deleted_3(self) -> None:
        with freeze_time("%s-08-10" % (self.current_year - 1)):
            self.stage_3.unlink()
            self.env.cr.flush()
        expected_dict = self.get_expected_dict()
    ...
    def get_expected_dict(self) -> None:
        expected_dict = {
