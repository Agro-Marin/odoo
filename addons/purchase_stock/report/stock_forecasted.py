...
class StockForecasted_Product_Product(models.AbstractModel):
    ...
    def _get_report_header(self, product_template_ids, product_ids, wh_location_ids):
        ...
        self._add_product_quantities(
            res, product_template_ids, product_ids, "draft_purchase_qty", in_qty
        )
        for product in self._get_products(product_template_ids, product_ids):
            if product not in po_lines:
                continue
            res["product"][product.id]["draft_purchase_orders"] = (
                po_lines[product]
                .mapped("order_id")
                .sorted(key=lambda po: po.name)
                .read(fields=["id", "name"])
            )
            res["product"][product.id]["draft_purchase_orders_matched"] = (
                self.env.context.get("purchase_line_to_match_id")
                in po_lines[product].ids
            )
        return res
