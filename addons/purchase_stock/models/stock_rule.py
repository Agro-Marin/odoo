...
class StockRule(models.Model):
    ...
    def _run_buy(self, procurements):
        ...
        for domain, procurements_rules in procurements_by_po_domain.items():
            ...
            grouped_po_lines = groupby(
                so.line_ids.filtered(lambda l: not l.display_type),
            ...
            for procurement in procurements:
                ...
                if po_line:
                    ...
                    order_date_planned = procurement.values[
                        "date_planned"
                    ] - relativedelta(days=procurement.values["supplier"].delay)
                    if (
                        fields.Date.to_date(order_date_planned)
                        < fields.Date.to_date(po.date_order)
                        and partner.group_rfq != "week"
                    ):
    ...
    def _post_vendor_notification(self, records_to_notify, users_to_notify, product):
        ...
        records_to_notify.message_post(
            body=notification_msg, partner_ids=users_to_notify.ids
        )
    ...
    def _get_lead_days(self, product, **values):
        ...
        seller = (
            "supplierinfo" in values
            and values["supplierinfo"]
    ...
    def _update_purchase_order_line(
        ...
        procurement_uom_po_qty = product_uom._compute_quantity(
            product_qty, line.product_uom_id, rounding_method="HALF-UP"
        )
        ...
        if seller.product_uom_id != line.product_uom_id and not values.get("force_uom"):
            res["product_qty"] = line.product_uom_id._compute_quantity(
                res["product_qty"], seller.product_uom_id, rounding_method="HALF-UP"
            )
    ...
    def _get_partner_id(self, values, rule):
        return values.get("supplierinfo_name") or (
            values.get("force_uom") and values.get("partner")
        )
