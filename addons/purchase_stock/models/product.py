...
class ProductTemplate(models.Model):
    ...
    def _check_buy_route(self):
        non_purchasable_products = self.filtered(
            lambda product: not product.purchase_ok
        )
        ...
        if buy_routes and any(
            buy_routes & product.route_ids for product in non_purchasable_products
        ):
            raise UserError(
                self.env._(
                    "The 'Buy' route cannot be assigned to a product that is not purchasable. Enable 'Can be Purchased' boolean to use this route."
                )
            )
...
class ProductProduct(models.Model):
    ...
    @api.depends_context(
        "suggest_based_on", "suggest_days", "suggest_percent", "warehouse_id"
    )
    def _compute_suggested_quantity(self):
        ...
        for product in self:
            ...
            product.suggested_qty = max(
                float_round(qty, precision_digits=0, rounding_method="UP"), 0
            )
    ...
    def _compute_suggest_estimated_price(self):
        ...
        for product in self:
            ...
            seller = product._select_seller(
                quantity=product.suggested_qty, **seller_args
            ) or product._select_seller(
                quantity=None, ordered_by="min_qty", **seller_args
            )
    ...
    def _compute_monthly_demand(self):
        ...
        if warehouse_id:
            move_domain = Domain.AND(
                [move_domain, [("location_id.warehouse_id", "=", warehouse_id)]]
            )
        move_qty_by_products = self.env["stock.move"]._read_group(
            move_domain, ["product_id"], ["product_qty:sum"]
        )
        ...
        if self.env.context.get("use_simplified_supplier_name"):
            ...
            for supplier in self:
                price_str = formatLang(
                    self.env, supplier.price, currency_obj=supplier.currency_id
                )
