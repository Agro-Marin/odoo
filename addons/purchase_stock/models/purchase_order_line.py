...
class PurchaseOrderLine(models.Model):
    ...
    def _ondelete_stock_moves(self):
        modified_fields = ["qty_transfered_manual", "qty_transfered_method"]
        self.flush_recordset(fnames=["qty_transfered", *modified_fields])
        self.invalidate_recordset(fnames=modified_fields, flush=False)
        query = f"""
            UPDATE {self._table}
            SET qty_transfered_manual = qty_transfered, qty_transfered_method = 'manual'
            WHERE id IN %(ids)s
        """
    ...
    def _compute_qty_transfered_method(self):
        super(PurchaseOrderLine, self)._compute_qty_transfered_method()
        for line in self.filtered(lambda l: not l.display_type):
            if line.product_id.type == "consu":
                line.qty_transfered_method = "stock_moves"
    ...
    def _compute_qty_transfered(self):
        from_stock_lines = self.filtered(
            lambda order_line: order_line.qty_transfered_method == "stock_moves"
        )
        super(PurchaseOrderLine, self - from_stock_lines)._compute_qty_transfered()
        for line in self:
            if line.qty_transfered_method == "stock_moves":
                total = 0.0
    ...
    def _create_or_update_picking(self):
        for line in self:
            if line.product_id and line.product_id.type == "consu":
                ...
                if (
                    float_compare(
                        line.product_qty, line.qty_invoiced, precision_rounding=rounding
                    )
                    < 0
    ...
    def _prepare_stock_move_vals(
        ...
        location_final = (
            self.location_final_id or self.order_id._get_final_location_record()
        )
    ...
    def _find_candidate(
        ...
        return (
            lines
            and lines.sorted(lambda l: l.orderpoint_id)[0]
