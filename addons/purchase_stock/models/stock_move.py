...
class StockMove(models.Model):
    ...
    def _get_price_unit(self):
        ...
        if (
            line.product_id.bill_policy == "purchase"
            and float_compare(
                line.qty_invoiced,
                received_qty,
            ...
            invoiced_layer = line.sudo().invoice_line_ids.stock_valuation_layer_ids
            ...
            remaining_qty = invoiced_qty - line.product_uom_id._compute_quantity(
                received_qty, line.product_id.uom_id, rounding_method="HALF-UP"
            )
            has_remaining = not order.currency_id.is_zero(
                remaining_value
    ...
    def _get_qty_received_without_self(self):
        ...
        if self.state == "done":
            qty_received -= self.product_uom._compute_quantity(
                self.quantity,
                self.purchase_line_id.product_uom_id,
                rounding_method="HALF-UP",
            )
        return qty_received
    ...
    def _get_currency_convert_date(self):
        ...
        convert_date = (
            fields.Date.context_today(self) if self.state != "done" else self.date
        )
        line = self.purchase_line_id
        if not line:
            return convert_date
        ...
        if (
            ...
            posted_bills = line.sudo().invoice_line_ids.move_id.filtered(
                lambda m: m.state == "posted"
            )
            convert_date = max(
                posted_bills.mapped("invoice_date"), default=convert_date
            )
        return convert_date
    ...
    def _get_valuation_price_and_qty(self, related_aml, to_curr):
        ...
        for val_stock_move in self:
            ...
            svl = (
                val_stock_move.with_context(active_test=False)
        ...
        if float_is_zero(
            valuation_total_qty,
            precision_rounding=related_aml.product_uom_id.rounding
            or related_aml.product_id.uom_id.rounding,
        ):
            raise UserError(
                _(
    ...
    def _account_entry_move(self, qty, description, svl_id, cost):
        ...
        am_vals_list = super(
            StockMove, self.with_context(move_directions=move_directions)
        ...
        if move_directions:
            self_is_out_move = move_directions.get(
                self.id
            ) and "out" in move_directions.get(self.id)
        else:
        ...
        if returned_move and self_is_out_move and self._is_returned(valued_type="out"):
            ...
            unit_diff = (
                returned_layer._get_layer_price_unit()
                - self.purchase_line_id._get_price_unit_gross()
            )
        else:
            return am_vals_list
    ...
    def _prepare_merge_negative_moves_excluded_distinct_fields(self):
        ...
        if (
            ...
            .sudo()
            .get_param("purchase_stock.merge_different_procurement")
        ):
    ...
    def _generate_valuation_lines_data(
        ...
        """Overridden from stock_account to support amount_currency on valuation lines
        generated from po"""
        self.ensure_one()
