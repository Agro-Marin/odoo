...
class TestPurchaseOldRules(PurchaseTestCommon):
    ...
    def setUpClass(cls):
        ...
        delivery_route_3.rule_ids[0].location_dest_id = delivery_route_3.rule_ids[
            1
        ].location_src_id.id
        ...
        delivery_route_2.rule_ids[0].location_dest_id = delivery_route_2.rule_ids[
            1
        ].location_src_id.id
        ...
        cls.route_buy.rule_ids.filtered(
            lambda r: r.picking_type_id == cls.warehouse_2_steps.in_type_id
    ...
    def test_03_cancel_draft_purchase_order_two_steps_pull(self):
        ...
        storage = picking_ids.filtered(
            lambda r: (
                r.picking_type_id == self.warehouse_2_steps.store_type_id
                and r.reference_ids.id == self.reference.id
            )
        )
        pick = picking_ids.filtered(
            lambda r: (
                r.picking_type_id == self.warehouse_2_steps.pick_type_id
                and r.reference_ids.id == self.reference.id
            )
        )
    ...
    def test_04_cancel_confirm_purchase_order_two_steps_pull(self):
        ...
        internal = picking_ids.filtered(
            lambda r: r.picking_type_id == self.warehouse_2_steps.int_type_id
        )
        pick = picking_ids.filtered(
    ...
    def test_05_cancel_draft_purchase_order_three_steps_pull(self):
        ...
        internal = picking_ids.filtered(
            lambda r: r.picking_type_id == self.warehouse_3_steps.int_type_id
        )
        pick = picking_ids.filtered(
            lambda r: r.picking_type_id == self.warehouse_3_steps.pick_type_id
        )
        pack = picking_ids.filtered(
    ...
    def test_02_product_route_level_delays(self):
        ...
        self.assertEqual(
            incoming_shipment1.date_planned,
        ...
        incoming_shipment2_date = schedule_date - timedelta(
            days=incoming_shipment2.move_ids[0].rule_id.delay
        )
        self.assertEqual(
            incoming_shipment2.date_planned,
        ...
        self.assertEqual(
            incoming_shipment1.date_deadline,
