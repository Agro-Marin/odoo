...
class TestDeleteOrder(PurchaseTestCommon):
    ...
    def test_00_delete_order(self):
        ...
        # Create PO with the purchase user to ensure proper access rights
        purchase_order_2 = self.env['purchase.order'].with_user(self.res_users_purchase_user).create({
            'partner_id': partner.id,
            'state': 'done',
        })
        ...
        # Create PO with the purchase user to ensure proper access rights
        purchase_order_3 = self.env['purchase.order'].with_user(self.res_users_purchase_user).create({
            'partner_id': partner.id,
            'state': 'draft',
        })
