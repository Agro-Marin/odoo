...
class TestPurchaseOrderSuggest(PurchaseTestCommon, HttpCase):
    ...
    def test_purchase_order_suggest_pricelist_selection(self):
        ...
        self.assertEstimatedPrice(
            po, 17, based_on="one_week", days=7
        )  # suggested qty 1 --> should use lowest qty pricelist
        self.assertEstimatedPrice(
            po, 34, based_on="one_week", days=14
        )  # suggested qty 2 --> should matching pricelist
        self.assertEstimatedPrice(
            po, 52, based_on="one_week", days=28
        )  # suggested qty 4 --> should matching pricelist

        partner_2 = self.env["res.partner"].create({"name": "No pricelist"})
        po_2 = self.env["purchase.order"].create({"partner_id": partner_2.id})
        self.assertEstimatedPrice(
            po_2, 20, based_on="one_week", days=7
        )  # No pricelist --> should use standard price
