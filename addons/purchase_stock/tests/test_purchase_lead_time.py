...
class TestPurchaseLeadTime(PurchaseTestCommon):
    ...
    def test_01_product_level_delay(self):
        ...
        # Check order date of purchase order
        order_line_pro_1 = purchase2.line_ids.filtered(lambda r: r.product_id == self.product_1)
    ...
    def test_02_product_level_delay(self):
        ...
        # Check order date of purchase order
        order_line_pro_1 = purchase1.line_ids.filtered(lambda r: r.product_id == self.product_1)
        order_line_pro_2 = purchase2.line_ids.filtered(lambda r: r.product_id == self.product_2)
    ...
    def test_03_group_week(self):
        ...
        # Check date deadline and date planned of purchase order.
        # The purchase order is planned on Tuesday 16th.
        # Note: date_order calculation with week grouping applies delta_days shift to both
        # date_planned and date_order, resulting in Sep 17 (shifted from procurement date).
        date_p = fields.Datetime.from_string('2025-09-16 10:00:00')
        date_d = fields.Datetime.from_string('2025-09-17 10:00:00')
        self.assertRecordValues(purchase1, [{
            'date_planned': date_p, 'date_order': date_d,
        }])
        # Lines have different date_planned based on their procurement dates and grouping
        # Line for product_1 (date_planned Sep 12 + delta) and product_2 (date_planned Sep 13 + delta)
        self.assertRecordValues(purchase1.line_ids, [
            {'date_planned': date_d},  # Sep 17
            {'date_planned': date_p},  # Sep 16
        ])
