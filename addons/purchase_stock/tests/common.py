...
class PurchaseTestCommon(TestStockValuationCommon):
    def _create_bill(
        self, product=None, quantity=None, price_unit=None, post=True, **kwargs
    ):
        if "purchase_order" not in kwargs:
            return super()._create_bill(
                product=product, quantity=quantity, price_unit=price_unit, **kwargs
            )
    ...
    def _create_purchase(
        self, product, quantity=1.0, price_unit=100.0, confirm=True, **kwargs
    ):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.purchase_user = cls._create_new_internal_user(
            name="Purchase User",
            login="purchase_user",
