...
class TestOnchangeProductId(TransactionCase):
    ...
    def test_onchange_product_id(self):
        ...
        partner_id = self.res_partner_model.create({"name": "George"})
        ...
        po_line.write(
            {"product_qty": 3, "product_uom_id": self.ref("uom.product_uom_dozen")}
        )
        self.assertEqual(
            1200, po_line.price_unit, "Unit price should be 1200 for one Dozen"
        )
        ...
        self.assertEqual(
            1000,
            po_line2.price_unit,
            "Price for 10 Ipad UoM should be 10x the standard_price",
        )
