...
class TestReplenishWizard(PurchaseTestCommon):
    def test_replenish_buy_1(self):
        ...
        order_line = last_po_id.line_ids.search([("product_id", "=", self.product.id)])
        self.assertTrue(order_line, "The product is not in the Purchase Order")
        self.assertEqual(
            order_line.product_qty, self.product_uom_qty, "Quantities does not match"
        )
    ...
    def test_supplier_delay(self):
        ...
        with freeze_time("2023-01-01"):
            ...
            self.assertEqual(
                fields.Datetime.from_string("2023-01-04 00:00:00"), wizard.date_planned
            )
    ...
    def test_purchase_supplier_route_delay(self):
        ...
        with freeze_time("2023-01-01"):
            ...
            self.assertEqual(
                fields.Datetime.from_string("2023-01-08 00:00:00"), wizard.date_planned
            )
    ...
    def test_inter_wh_replenish(self):
        ...
        main_warehouse.write({"resupply_wh_ids": [Command.set(second_warehouse.ids)]})
        interwh_route = self.env["stock.route"].search(
            [
                ("supplied_wh_id", "=", main_warehouse.id),
                ("supplier_wh_id", "=", second_warehouse.id),
            ]
        )
    ...
    def test_purchase_order_uom(self):
        ...
        self.assertEqual(
            po.line_ids.product_qty,
            10,
            "Generated PO line must respect the requested quantity from the wizard",
        )
        self.assertEqual(
            po.line_ids.product_uom_id,
            replenish_wizard.product_uom_id,
            "Generated PO line must respect the requested UOM from the wizard",
        )
        self.assertEqual(
            po.line_ids.price_unit,
            1,
            'Generated PO line must respect the supplier price of UoM "Unit"',
        )
        po.action_cancel()
        ...
        self.assertEqual(
            po.line_ids.price_unit,
            6,
            'Generated PO line must respect the supplier price of UoM "Unit" because the quantity doesn\'t match the "Pack of 6" pricelist',
        )
        po.action_cancel()
        ...
        self.assertEqual(
            po.line_ids.price_unit,
            5,
            'Generated PO line must respect the supplier price of UoM "Pack of 6" because the quantity matches the "Pack of 6" pricelist',
        )
        po.action_cancel()
