...
class TestPurchaseOrder(ValuationReconciliationTestCommon):
    ...
    def setUpClass(cls):
        ...
        cls.product_id_1 = cls.env["product.product"].create(
            {"name": "Large Desk", "bill_policy": "ordered", "is_storable": True}
        )
        cls.product_id_2 = cls.env["product.product"].create(
            {"name": "Conference Chair", "bill_policy": "ordered", "is_storable": True}
        )
    ...
    def test_00_purchase_order_flow(self):
        if self.partner_a in self.product_id_2.seller_ids.partner_id:
            id_to_remove = (
                self.product_id_2.seller_ids.filtered(
                    lambda r: r.partner_id == self.partner_a
                ).ids[0]
                if self.product_id_2.seller_ids.filtered(
                    lambda r: r.partner_id == self.partner_a
                )
                else False
            )
        ...
        self.assertEqual(
            self.po.invoice_state,
            "no",
            'Purchase: PO invoice_state should be "Not purchased"',
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.po.line_ids.mapped("qty_invoiced"),
            [0.0, 0.0],
        ...
        self.assertEqual(
            self.po.invoice_state,
            "to do",
            'Purchase: PO invoice_state should be "Waiting Invoices"',
        )
        ...
        if (
            price_unit
            and seller
            and self.po.currency_id
            and seller.currency_id != self.po.currency_id
        ):
            price_unit = seller.currency_id._convert(
                price_unit, self.po.currency_id, self.po.company_id, self.po.date_order
            )
        self.assertEqual(
            price_unit,
            250.0,
    ...
    def test_02_po_return(self):
        ...
        return_wiz.product_return_moves.write({"quantity": 2.0, "to_refund": True})
        ...
        self.assertEqual(
            self.po.line_ids[0].qty_transferred,
            3.0,
    ...
    def test_03_po_return_and_modify(self):
        ...
        po1.create_invoice()
        po1.invoice_ids.invoice_date = fields.Date.today()
        po1.invoice_ids.action_post()
    ...
    def test_04_update_date_commitment(self):
        ...
        self.assertEqual(
            ...
            % (today.date(), tomorrow.date(), po.picking_ids.name),
            activity.note,
        )
    ...
    def test_04_multi_uom(self):
        ...
        picking.button_validate()
        self.assertEqual(
            po.line_ids.mapped("qty_transferred"),
            [4.0],
            'Purchase: no conversion error on receipt in different uom"',
        )

    def test_05_po_update_qty_stock_move_merge(self):
        unit_price_precision = self.env["decimal.precision"].search(
            [("name", "=", "Product Price")]
        )
        ...
        self.assertEqual(
            purchase_order.picking_ids.move_line_ids.quantity_product_uom, 7
        )
        ...
        self.assertEqual(len(purchase_order.picking_ids.move_line_ids), 1)
        self.assertEqual(
            purchase_order.picking_ids.move_line_ids.quantity_product_uom, 4
        )
        ...
        self.assertEqual(len(purchase_order.picking_ids.move_line_ids), 1)
        self.assertEqual(
            purchase_order.picking_ids.move_line_ids.quantity_product_uom, 14
        )
    ...
    def test_message_qty_already_received(self):
        ...
        second_picking = self.env["stock.picking"].search(
            [("backorder_id", "=", first_picking.id)], limit=1
        )
        ...
        third_picking = self.env["stock.picking"].search(
            [("backorder_id", "=", second_picking.id)], limit=1
        )
        ...
        self.assertEqual(
            _purchase_order.line_ids.qty_transferred,
            15.0,
            "Total received quantity should be 15",
        )
    ...
    def test_po_edit_after_receive(self):
        ...
        self.assertEqual(
            self.po.picking_ids.move_ids.mapped("product_uom_qty"), [5.0, 5.0]
        )
        self.po.with_context(import_file=True).line_ids[0].product_qty = 10
        self.assertEqual(
            self.po.picking_ids.move_ids.mapped("product_uom_qty"), [5.0, 5.0, 5.0]
        )
    ...
    def test_receive_negative_quantity(self):
        ...
        po.picking_ids.move_ids.quantity = 5.0
        po.picking_ids.move_ids.picked = True
        po.picking_ids.button_validate()
    ...
    def test_receive_qty_invoiced_but_no_posted(self):
        ...
        move = receipt01.move_ids.filtered(
            lambda m: m.product_id == self.product_id_1,
        )
    ...
    def test_bill_on_ordered_qty_correct_converted_amount_on_bill(self):
        ...
        bill1_line_balance = purchase_order.invoice_ids.invoice_line_ids.filtered(
            "balance"
        ).balance
    ...
    def test_foreign_bill_tax_included(self):
        ...
        self.assertRecordValues(
            po.invoice_ids.line_ids.sorted("tax_line_id"),
            [
                {
    ...
    def test_merge_purchase_order_incoterm(self):
        ...
        self.assertEqual(
            po_1.incoterm_id,
            incoterm_1,
            "Incoterm should be preserved from the first PO after merge",
        )
