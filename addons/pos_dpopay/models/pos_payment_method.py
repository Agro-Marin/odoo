...
class PosPaymentMethod(models.Model):
    ...
    dpopay_client_id = fields.Char(
        ...
        help="The Client ID provided by DPO Pay for authenticating requests.",
    )
    ...
    dpopay_mid = fields.Char(
        string="DPO Pay Merchant ID",
        help="Enter the Merchant ID assigned by DPO Pay (e.g., 123456789012).",
    )
    dpopay_tid = fields.Char(
        string="DPO Pay Terminal ID",
        help="Enter the unique Terminal ID (TID) of your DPO Pay POS terminal (e.g., XXXXXXXX).",
    )
    ...
    dpopay_chain_id = fields.Char(
        ...
        help="Enter the Chain-ID header value(e.g., DPO-DTM-Testing)",
    )
    dpopay_test_mode = fields.Boolean(
    ...
    def _get_dpopay_base_url(self, is_token=False):
        self.check_singleton()
        host = (
            self.dpopay_test_mode and "api-dev.network.global"
        ) or "api.network.global"
    ...
    def _dpopay_headers(self, token_expired=False):
        self.check_singleton()
        token = (
            self._generate_dpopay_token() if token_expired else self.dpopay_bearer_token
        )
        return {
    ...
    def _generate_dpopay_token(self):
        ...
        auth = requests.auth.HTTPBasicAuth(
            self.dpopay_client_id, self.dpopay_client_secret
        )
    ...
    def _execute_dpopay_api_request(self, payload, endpoint):
        ...
        try:
            ...
            def _send_request(token_expired=False):
                ...
                _logger.info(
                    "Sending request to %s | Mode: %s | Headers: %s",
                    url,
                    mode,
                    list(headers.keys()),
                )
            ...
            errorCode = response_json.get("error_code") or response_json.get(
                "resultCode"
            )
            ...
            error_code = str(
                ...
                or error_json.get("resultCode")
            )
            error_message = (
                error_json.get("errorMessage")
                or error_json.get("error_description")
                or error_json.get("resultDescription")
                or str(error_json)
            )
