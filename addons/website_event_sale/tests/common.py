...
class TestWebsiteEventSaleCommon(WebsiteSaleCommon):
    ...
    def setUpClass(cls):
        ...
        cls.event, cls.event_2 = cls.env["event.event"].create(
            [
                {
                    "date_begin": (Datetime.today() + timedelta(days=5)).strftime(
                        "%Y-%m-%d 07:00:00"
                    ),
                    "date_end": (Datetime.today() + timedelta(days=5)).strftime(
                        "%Y-%m-%d 16:30:00"
                    ),
                    "name": "Pycon",
        ...
        cls.pricelist_without_discount = create_pricelist(
            currency=cls.env.company.currency_id, name="EUR Without Discount Included"
        )
        cls.ex_pricelist_without_discount = create_pricelist(
            currency=cls.currency_test, name="EX Without Discount Included"
        )
