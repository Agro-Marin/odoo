...
class PosMercadoPagoWebhook(http.Controller):
    ...
    def notification(self):
        ...
        if not external_reference or not (
            match := re.fullmatch(mercado_pago_pattern, external_reference)
        ):
            _logger.warning(
                'POST message received with no or malformed "external_reference" key: %s',
                external_reference,
            )
            return http.Response(status=400)
        ...
        if (
            not payment_method_sudo
            or payment_method_sudo.use_payment_terminal != "mercado_pago"
        ):
