...
def _prepare_request(company, endpoint_type, params=False):
    ...
    endpoint = (
        f"/apis/v2/notifyimport/{params.pop('eid')}"
        if endpoint_type == "notify_import"
        else endpoints.get(endpoint_type)
    )
    if not endpoint:
    ...
    if response.status_code != 200:
        try:
            ...
            error_message = False
        raise UserError(
            company.env._("Error handling request: %s", error_message)
            if error_message
            else company.env._("HTTP %s: Connection error.", response.status_code)
        )
...
def _mer_api_receive_document(company, electronic_id):
    ...
    if response_bstr[:5] != b"<?xml":
        raise MojEracunServiceError(
            "service_error",
            company.env._(
                "Failed to retrieve document XML for ElectronicId: %s", electronic_id
            ),
        )
    return response_bstr


def _mer_api_update_document_process_status(
    company, electronic_id, status_id, rejection_reason=None
):
    """
...
def _mer_api_mark_paid(
    company, electronic_id, payment_date, payment_amount, payment_method
):
    """
...
def _mer_api_reject_with_id(
    company, electronic_id, rejection_date, rejection_type, rejection_desc
):
    """
