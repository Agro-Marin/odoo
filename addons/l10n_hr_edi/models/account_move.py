...
class AccountMove(models.Model):
    ...
    l10n_hr_process_type = fields.Selection(
        [
            ...
            (
                "P2",
                "P2: Periodic invoicing for deliveries of goods and services based on contracts",
            ),
            (
                "P3",
            ...
            (
                "P8",
                "P8: Issuing invoices with references to the shipping and receipt notes",
            ),
            (
                "P9",
                "P9: Credits or invoices with negative amounts, issued for various reasons, including empty returns packaging",
            ),
            (
                "P10",
                "P10: Issuing a corrective invoice (reversal/correction of invoice)",
            ),
    ...
    l10n_hr_invoice_sending_time = fields.Datetime(
    ...
    l10n_hr_business_status_reason = fields.Char(
        related="l10n_hr_edi_addendum_id.business_status_reason"
    )
    l10n_hr_fiscalization_number = fields.Char(
    ...
    l10n_hr_fiscalization_request = fields.Char(
    ...
    def _auto_init(self):
        if not column_exists(self.env.cr, "account_move", "l10n_hr_process_type"):
            create_column(
                self.env.cr, "account_move", "l10n_hr_process_type", "varchar"
            )
        return super()._auto_init()

    @api.depends(
        "l10n_hr_edi_addendum_id.payment_reported_amount",
        "amount_residual",
        "amount_total",
    )
    def _compute_l10n_hr_payment_unreported(self):
        for move in self:
            move.l10n_hr_payment_unreported = move.l10n_hr_payment_reported_amount != (
    ...
    def _search_l10n_hr_payment_unreported(self, operator, value):
        ...
        if operator == "!=":
            ...
            query.add_where(
                SQL(
                    """ROUND(%(payment_reported_amount)s - %(amount_total)s + %(amount_residual)s, 8) != 0""",
                    payment_reported_amount=self.env[
                        "l10n_hr_edi.addendum"
                    ]._field_to_sql(
                        "account_move__addendum", "payment_reported_amount", query
                    ),
                    amount_total=self._field_to_sql(
                        "account_move", "amount_total", query
                    ),
                    amount_residual=self._field_to_sql(
                        "account_move", "amount_residual", query
                    ),
                )
            )
            return [("id", "in", query)]
    ...
    def _check_l10n_hr_process_type(self):
        for record in self:
            if record.country_code == "HR" and (
                record.l10n_hr_process_type == "P9"
    ...
    def _get_invoice_reference_odoo_invoice(self):
        ...
        if (
            ...
            and self.partner_id.country_code == "HR"
        ):
            fisc_num_hr_format = re.sub(
                r"\D", "", self._get_l10n_hr_fiscalization_number(self.name)
            )
    ...
    def l10n_hr_edi_mer_action_fetch_status(self):
        ...
        if self.is_sale_document():
            ...
            response_fisc = _mer_api_check_fiscalization_status_outbox(
                self.company_id, electronic_id=self.l10n_hr_mer_document_eid
            )[0]
        elif self.is_purchase_document():
            response_mer = _mer_api_query_document_process_status_inbox(
                self.company_id, electronic_id=self.l10n_hr_mer_document_eid
            )[0]
            response_fisc = _mer_api_check_fiscalization_status_inbox(
                self.company_id, electronic_id=self.l10n_hr_mer_document_eid
            )[0]
        else:
            return
        self.l10n_hr_edi_addendum_id.write(
            {
                ...
                "business_document_status": str(
                    response_mer.get("DocumentProcessStatusId")
                ),
                "fiscalization_status": str(
                    response_fisc["messages"][-1].get("status")
                ),
                "fiscalization_error": str(
                    response_fisc["messages"][-1].get("errorCode")
                    + " - "
                    + response_fisc["messages"][-1].get("errorCodeDescription")
                ),
                "fiscalization_request": str(
                    response_fisc["messages"][-1].get("fiscalizationRequestId")
                ),
                "business_status_reason": str(
                    response_fisc["messages"][-1].get("businessStatusReason")
                ),
    ...
    def l10n_hr_edi_mer_action_report_paid(self):
        for move in self:
            ...
            amount_to_report = (
                move.amount_total - move.amount_residual
            ) - move.l10n_hr_payment_reported_amount
            if amount_to_report and state_to_set:
                try:
                    ...
                    _logger.error(
                        "Failed to report payments document: %s",
                        move.l10n_hr_mer_document_eid,
                    )
                    continue
                move.l10n_hr_edi_addendum_id.payment_reported_amount += amount_to_report
                move.l10n_hr_edi_addendum_id.fiscalization_request = response.get(
