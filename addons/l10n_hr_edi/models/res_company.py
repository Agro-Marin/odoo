...
class ResCompany(models.Model):
    ...
    def _compute_l10n_hr_edi_config_id(self):
        self._compute_config_link("l10n_hr_edi_config_id")
    ...
    def _l10n_hr_mer_import_invoice(self, attachment, document):
        ...
        original_document_id = self.env[
            "account.edi.xml.ubl_hr"
        ]._get_rejection_reference(attachment)
        if original_document_id == "not_found":
            _logger.error(
                "Failed to find origin document for rejection note %s",
                document["mer_document_eid"],
            )
            return False
            ...
            if original_addendum:
                original_addendum.move_id.message_post(
                    body=self.env._(
                        'eRacun rejected by document with Electronic ID: %(eid)s\n"%(reason)s"',
                        eid=document["mer_document_eid"],
                        reason=original_document_id[1],
                    )
                )
        ...
        _logger.info(
            "Successfully imported MER document %s", document["mer_document_eid"]
        )
        return move

    def _l10n_hr_mer_get_new_documents(
        self, undelivered_only=True, slc=False, from_cron=False
    ):
        """
        ...
        for company in self.filtered(
            ...
            try:
                response = _mer_api_query_inbox(
                    company, "Undelivered" if undelivered_only else None
                )
            ...
            if any(
                not (item.get("ElectronicId") and item.get("StatusId"))
                for item in response
            ):
                _logger.error(
                    "MojEracun service error: incorrect response format while querying inbox for company: %s",
                    company.name,
                )
                continue
            documents = [
                {
                    "mer_document_eid": str(document["ElectronicId"]),
                    "mer_document_status": str(document["StatusId"]),
                }
            ...
            existing_documents = self.env["l10n_hr_edi.addendum"].search(  # noqa: E8507 - one query per company of the cron, over the fetched documents
                [
                    (
                        "mer_document_eid",
                        "in",
                        [i["mer_document_eid"] for i in documents],
                    ),
            ...
            documents_to_import = [
                i
                for i in documents
                if i["mer_document_eid"]
                not in existing_documents.mapped("mer_document_eid")
            ]
            proxy_acks = []
            ...
            for document in documents_to_import:
                try:
                    fisc_data = _mer_api_check_fiscalization_status_inbox(
                        company, electronic_id=document["mer_document_eid"]
                    )
                    if fisc_data == []:
                        _logger.error(
                            "Fiscalization data for document eID %s is not available on MojEracun server.",
                            document["mer_document_eid"],
                        )
                        continue
                    ...
                    _logger.error(
                        "Failed to retreive fisc data for document eID %s",
                        document["mer_document_eid"],
                    )
                    if from_cron:
                        continue
                ...
                document.update(
                    {
                        "fiscalization_status": str(
                            fisc_data["messages"][-1].get("status")
                        ),
                        "fiscalization_error": str(
                            fisc_data["messages"][-1].get("errorCode")
                        )
                        + " - "
                        + str(fisc_data["messages"][-1].get("errorCodeDescription")),
                        "fiscalization_request": str(
                            fisc_data["messages"][-1].get("fiscalizationRequestId")
                        ),
                        "business_status_reason": str(
                            fisc_data["messages"][-1].get("businessStatusReason")
                        ),
                ...
                if document["fiscalization_status"] != "0":
                    _logger.warning(
                        "Document eID %s is not successfully fiscalized by MojEracun.",
                        document["mer_document_eid"],
                    )
                try:
                    business_data = _mer_api_query_document_process_status_inbox(
                        company, electronic_id=document["mer_document_eid"]
                    )[0]
                except MojEracunServiceError, UserError:
                    _logger.error(
                        "Failed to retreive business data for document: %s",
                        document["mer_document_eid"],
                    )
                    if from_cron:
                        continue
                ...
                document.update(
                    {
                        "business_document_status": str(
                            business_data.get("DocumentProcessStatusId")
                        ),
                    }
                )
                try:
                    document_xml = _mer_api_receive_document(
                        company, document["mer_document_eid"]
                    )
                except MojEracunServiceError as e:
            ...
            for query_function, check_function in [
                ...
                documents = {
                    str(item["ElectronicId"]): {
                        ...
                        "business_document_status": str(
                            item.get("DocumentProcessStatusId")
                        ),
                    }
                ...
                for item in response_fisc:
                    ...
                    if item.get("ElectronicId") in documents:
                        documents[item["ElectronicId"]].update(
                            {
                                "fiscalization_status": str(
                                    item["messages"][-1].get("status")
                                ),
                                "fiscalization_error": str(
                                    item["messages"][-1].get("errorCode")
                                )
                                + " - "
                                + str(item["messages"][-1].get("errorCodeDescription")),
                                "fiscalization_request": str(
                                    item["messages"][-1].get("fiscalizationRequestId")
                                ),
                                "business_status_reason": str(
                                    item["messages"][-1].get("fiscalizationRequestId")
                                ),
                                "fiscalization_channel_type": str(
    ...
    def _l10n_hr_mer_archive_signed_xmls(self, from_cron=False):
        ...
        for move in moves:
            try:
                signed_xml = _mer_api_receive_document(
                    self, move.l10n_hr_mer_document_eid
                )
            except MojEracunServiceError as e:
                _logger.error(
