...
class MixinAccountMoveSend(models.AbstractModel):
    ...
    def _check_move_constrains(self, moves):
        ...
        if any(
            ...
            raise UserError(
                self.env._(
                    "Operator label is required for sending invoices in Croatia."
                )
            )
        # HR-BR-9: Invoice must contain HR-BT-5: Operator OIB in accordance with the Fiscalization Act.
        if any(
            (move.country_code == "HR" and not move.l10n_hr_operator_oib)
            for move in moves
        ):
            raise UserError(
                self.env._("Operator OIB is required for sending invoices in Croatia.")
            )
        # HR-BR-25: ensure KPD is provided for every business line except for advance (P4)
        if any(
            (
                move.country_code == "HR"
                and move.l10n_hr_process_type != "P4"
                and any(
                    line.display_type == "product" and not line.l10n_hr_kpd_category_id
            ...
            raise UserError(
                self.env._(
                    "KPD categories must be defined on every invoice line for any Business Process Type other than P4."
                )
            )
        if any(
            (
                move.country_code == "HR"
                and move.l10n_hr_process_type == "P99"
                and not move.l10n_hr_customer_defined_process_name
            )
            for move in moves
        ):
            raise UserError(
            ...
            and any(
                any(
                    (
                        ...
                        and not tax.invoice_legal_notes
                    )
    ...
    def _hook_if_errors(self, moves_data, allow_raising=True):
        ...
        for move, move_data in moves_data.items():
            if "mojeracun" in move_data["sending_methods"] and move_data.get(
                "blocking_error"
            ):
        ...
        for move in moves:
            if move.country_code == "HR" and move.is_sale_document():
                move.l10n_hr_edi_addendum_id = self.env["l10n_hr_edi.addendum"].create(
                    {
                        "move_id": move.id,
                        "fiscalization_number": move._get_l10n_hr_fiscalization_number(
                            move.name
                        ),
                        "invoice_sending_time": fields.Datetime.now(
    ...
    def _call_web_service_after_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            if not self._is_applicable_to_move("mojeracun", invoice, **invoice_data):
                raise UserError(
                    self.env._(
                        "Failed to send invoice via MojEracun: check configuration."
                    )
                )

            if invoice_data.get("ubl_cii_xml_attachment_values"):
                xml_file = invoice_data["ubl_cii_xml_attachment_values"]["raw"]
            elif invoice.ubl_cii_xml_id and invoice.l10n_hr_mer_document_status not in {
                "20",
                "30",
                "40",
            }:
            ...
            try:
                ...
                if not response.get("ElectronicId"):
                    ...
                    log_message = self.env._(
                        "The document has been sent to MojEracun service provider for processing"
                    )
