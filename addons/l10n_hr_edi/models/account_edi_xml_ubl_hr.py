...
class AccountEdiXmlUBLHR(models.AbstractModel):
    ...
    def _export_invoice_constraints(self, invoice, vals):
        # CIUS HR, not Peppol: ubl_20's rules directly, bypassing bis3's own.
        constraints = self.env["account.edi.xml.ubl_20"]._export_invoice_constraints(
            invoice, vals
        )
        constraints.update(self._invoice_constraints_cen_en16931_ubl(invoice, vals))
        constraints.update(self._invoice_constraints_eracun_new(invoice, vals))
        return constraints
    ...
    def _invoice_constraints_eracun_new(self, invoice, vals):
        ...
        if vals["document_type"] in ["invoice", "credit_note"]:
            if any(
                c.isspace()
                for c in vals["document_node"]["cac:PaymentMeans"][
                    "cac:PayeeFinancialAccount"
                ]["cbc:ID"].get("_text")
            ):
                constraints.update(
                    {
                        "ubl_hr_br_1": self.env._(
            ...
            constraints.update(
                {
                    ...
                    if not vals["document_node"]["cac:AccountingSupplierParty"][
                        "cac:Party"
                    ]["cac:Contact"]["cbc:ElectronicMail"].get("_text")
                    else None,
                    "ubl_hr_br_10_buyer_email_required": (
                        self.env._("The Buyer's e-mail must be provided.")
                    )
                    if not vals["document_node"]["cac:AccountingCustomerParty"][
                        "cac:Party"
                    ]["cac:Contact"]["cbc:ElectronicMail"].get("_text")
                    else None,
                    ...
                    if any(
                        not item["cbc:CompanyID"].get("_text")
                        for item in vals["document_node"][
                    ...
                    "ubl_hr_br_37_operator_label_required": (
                        self.env._(
                            "The invoice must contain the Operator Label (HR-BT-4)."
                        )
                    )
                    if not vals["document_node"]["cac:AccountingSupplierParty"][
                        "cac:SellerContact"
                    ...
                    "ubl_hr_br_9_operator_oib_required": (
                        self.env._(
                            "The invoice must contain the Operator OIB (HR-BT-5)."
                        )
                    )
                    if not vals["document_node"]["cac:AccountingSupplierParty"][
                        "cac:SellerContact"
    ...
    def _add_invoice_header_nodes(self, document_node, vals):
        ...
        document_node.update(
            {
                ...
                "cbc:ID": {"_text": invoice.l10n_hr_fiscalization_number},
                ...
                "cbc:IssueDate": {
                    "_text": fields.Datetime.to_string(
                        invoice.l10n_hr_invoice_sending_time
                    ).split()[0]
                },
                "cbc:IssueTime": {
                    "_text": fields.Datetime.to_string(
                ...
                "cbc:ProfileID": {
                    "_text": f"P99:{invoice.l10n_hr_customer_defined_process_name}"
                    if invoice.l10n_hr_process_type == "P99"
                    else invoice.l10n_hr_process_type
                },
        ...
        if (
            ...
            and not document_node["cbc:DueDate"]
            and vals["document_type"] == "invoice"
        ):
            if invoice.invoice_date_due:
                document_node.update(
                    {"cbc:DueDate": {"_text": invoice.invoice_date_due}}
                )
        ...
        if invoice.l10n_hr_process_type in ("P4", "P6"):
            if invoice.move_type == "out_invoice":
                document_node["cbc:InvoiceTypeCode"].update({"_text": "386"})
            elif invoice.move_type == "out_refund":
                document_node["cbc:CreditNoteTypeCode"].update({"_text": "386"})
        elif invoice.l10n_hr_process_type == "P9":
            document_node["cbc:CreditNoteTypeCode"].update({"_text": "381"})
    ...
    def _add_hr_extension_node(self, document_node):
        ...
        for i in range(len(tax_subtotals)):
            ...
            cash_basis_flag = tax_subtotals[i]["cac:TaxCategory"][0].pop(
                "hrextac:HRObracunPDVPoNaplati"
            )  # Ensure pop() always runs
            ...
            new_item = {
                ...
                "hrextac:HRTaxCategory": {
                    ...
                    "cbc:Percent": tax_subtotals[i]["cac:TaxCategory"][0][
                        "cbc:Percent"
                    ],
                    "cbc:TaxExemptionReasonCode": tax_subtotals[i]["cac:TaxCategory"][
                        0
                    ]["cbc:TaxExemptionReasonCode"],
                    "cbc:TaxExemptionReason": tax_subtotals[i]["cac:TaxCategory"][0][
                        "cbc:TaxExemptionReason"
                    ],
                    "hrextac:HRTaxScheme": tax_subtotals[i]["cac:TaxCategory"][0][
                        "cac:TaxScheme"
                    ]
                    if hr_tax_name["_text"] != "HR:POVNAK"
                    else {"_text": "OTH"},
                },
            }
            hr_tax_subtotals.append(new_item)
        out_of_scope_node = {
            "currencyID": document_node["cac:LegalMonetaryTotal"][
                "cbc:TaxExclusiveAmount"
            ].get("currencyID"),
                    ...
                    "ext:UBLExtension": {
                        "ext:ExtensionContent": {
                            "hrextac:HRFISK20Data": {
                                ...
                                "hrextac:HRTaxTotal": {
                                    "cbc:TaxAmount": document_node["cac:TaxTotal"][0][
                                        "cbc:TaxAmount"
                                    ],
                                    "hrextac:HRTaxSubtotal": hr_tax_subtotals,
                                },
                                "hrextac:HRLegalMonetaryTotal": {
                                    "cbc:TaxExclusiveAmount": document_node[
        ...
        if commercial_partner.l10n_hr_personal_oib:
            if commercial_partner.l10n_hr_business_unit_code:
                party_node["cac:PartyIdentification"]["cbc:ID"].update(
                    {
                        "_text": "9934:"
                        + commercial_partner.l10n_hr_personal_oib
                        + "::HR99:"
                        + commercial_partner.l10n_hr_business_unit_code
                    }
                )
            else:
                party_node["cac:PartyIdentification"]["cbc:ID"].update(
    ...
    def _add_invoice_accounting_supplier_party_nodes(self, document_node, vals):
        ...
        document_node["cac:AccountingSupplierParty"].update(
            {
                "cac:SellerContact": {
                    "cbc:ID": {"_text": invoice.l10n_hr_operator_oib},
                    "cbc:Name": {"_text": invoice.l10n_hr_operator_name},
                }
            }
        )
    ...
    def _get_tax_exemption_reason(self, customer, supplier, tax):
        ...
        if hr_category:
            ...
            if tax.amount == 0:
                tax_extension = (
                    "ubl_cii_tax_exemption_reason_code" in tax._fields
    ...
    def _ubl_default_tax_category_grouping_key(
        ...
        if tax_data and (
            ...
            return {
                "tax_category_code": self._get_tax_category_code(
                    customer.commercial_partner_id, supplier, self.env["account.tax"]
                ),
                **self._get_tax_exemption_reason(
                    customer.commercial_partner_id, supplier, self.env["account.tax"]
                ),
    ...
    def _ubl_get_tax_category_node(self, vals, tax_category):
        ...
        return {
            ...
            "cbc:TaxExemptionReasonCode": {
                "_text": tax_category.get("tax_exemption_reason_code")
            },  # Behavior inconsistency between 18.3 and 18.4
            "cbc:TaxExemptionReason": {
                "_text": tax_category.get("tax_exemption_reason")
            },
            "cac:TaxScheme": {
                "cbc:ID": {"_text": tax_category["scheme_id"]},
            },
            "hrextac:HRObracunPDVPoNaplati": {
                "_text": html2plaintext(
                    tax_category["invoice_legal_notes"]
                    or "Obračun po naplaćenoj naknadi"
                )
            }
            if tax_category["tax_exigibility"] == "on_payment"
            else None,
        }
    ...
    def _import_fill_invoice(self, invoice, tree, qty_factor):
        ...
        invoice_values = {
            ...
            "l10n_hr_customer_defined_process_name": profile_id[4:]
            if profile_id[:3] == "P99"
            else False,
        }
        invoice.write(invoice_values)
        invoice.l10n_hr_edi_addendum_id.write(
            {"fiscalization_number": tree.findtext("./{*}ID")}
        )
        return logs
    ...
    def _import_invoice_lines(self, invoice, tree, xpath, qty_factor):
        ...
        tax_exigibility = (
            "on_payment"
            if tree.find(".//{*}HRObracunPDVPoNaplati") is not None
            else "on_invoice"
        )
        logs = []
        lines_values = []
        for line_tree in tree.iterfind(xpath):
            line_values = self.with_company(
                invoice.company_id
            )._prepare_invoice_line_vals(line_tree, invoice.move_type, qty_factor)
            ...
            lines_values += self._prepare_line_charge_vals(
                invoice, line_values, line_values["tax_ids"]
            )
        return lines_values, logs

    def _prepare_import_line_vals(self, tree, document_type=False, qty_factor=1):
        line_values = super()._prepare_import_line_vals(tree, document_type, qty_factor)
        kpd_category_code = tree.findtext(
            "./{*}Item/{*}CommodityClassification/{*}ItemClassificationCode"
        )
        if kpd_category_code:
            line_kpd_category = self.env["l10n_hr.kpd.category"].search(
                [("name", "=", kpd_category_code)], limit=1
            )
    ...
    def _get_rejection_reference(self, attachment):
        ...
        if string_to_find in attachment["raw"]:
            ...
            if "Electronic ID:" in reason_node:
                original_document_id = reason_node[
                    reason_node.find("Electronic ID:") + 15 : reason_node.find(
                        "Electronic ID:"
                    )
                    + 22
                ]
