"""Overrides to Odoo's mixin.sequence for Croatian fiscal invoice format.
...
HR_FORMAT_STR = (
    "{code}-{year:0{year_length}d}-{seq:0{seq_length}d}/{premises_label}/{device_label}"
)
    ...
    def _get_sequence_format_param(self, previous):
        ...
        if not premises_label:
            ...
            raise ValidationError(
                self.env._("Business premises label is not set on the journal.")
            )
        if not device_label:
            _logger.error("Issuing device label is not set on the journal.")
            raise ValidationError(
                self.env._("Issuing device label is not set on the journal.")
            )
        ...
        if not match:
            ...
            ):  # First invoice in the sequence = no need to raise warnings
                _logger.warning(
                    "The previous sequence '%s' does not match the expected format. Resetting to initial sequence.",
                    previous,
                )
            _logger.debug(
                "Reset sequence format: %s, Format values: %s",
                HR_FORMAT_STR,
                format_values,
            )
        ...
        try:
            ...
            _logger.error(
                "Invalid sequence number in '%s': seq='%s'",
                previous,
        ...
        format_values["year_end"] = (
            int(format_values.get("year_end", "0"))
            if "year_end" in format_values
            else 0
        )
        ...
        format_values["year_end_length"] = len(
            str(format_values["year_end"])
        )  # Ensure 'year_end_length' is always set
        # Add 'month' key with a default value
        format_values["month"] = 0  # Setting to 0 indicates that month is not used

        _logger.debug(
            "Sequence format: %s, Format values: %s", HR_FORMAT_STR, format_values
        )
        return HR_FORMAT_STR, format_values
