...
class TestHrEdiFlowsMocked(
    TestL10nHrEdiCommon, TestAccountMoveSendCommon, PatchRequestsMixin
):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def assertRequestsEqual(self, actual_request, expected_request):
        ...
        if (actual_file := actual_request.get("json", {}).get("File")) and (
            expected_file := expected_request.get("json", {}).get("File")
        ):

            @contextmanager
            def restore_files():
                try:
                    actual_request["json"]["File"] = expected_request["json"][
                        "File"
    ...
    def test_send_invoice_error(self):
        ...
        with self.assertRaisesRegex(
            UserError,
            "Korisničko ime i lozinka nisu ispravni.. Trace ID: 4f701362-96cc-49c6-a297-854e740ad719.",
        ):
            with self.assertRequests(
                [
                    (
    ...
    def test_20_receive_invoice(self):
        ...
        with self.assertRequests(
            [
                ...
                (
                    ...
                    self._build_response(
                        status_code=200, response_content=mocked_in_invoice_bytes
                    ),
                ),
            ]
        ):
            ...
            ].l10n_hr_mer_get_new_documents_all()

        fetched_invoice = self.env["account.move"].search(
        ...
        self.assertRecordValues(
            fetched_invoice.invoice_line_ids,
            [
                {
