...
class TestL10nHrEdiMerApi(TestL10nHrEdiCommon, TestAccountMoveSendCommon):
    ...
    def _test_mer_api_send(self):
        ...
        moves_data = {
            move.sudo(): {
                **self.env["mixin.account.move.send"]._get_default_sending_settings(
                    move
                )
            }
            for move in [invoice]
    ...
    def test_mer_api_receive(self):
        ...
        response = _mer_api_receive_document(
            self.env.company, electronic_id=electronic_id
        )
        self.assertEqual(
            response[:100],
            b'<?xml version="1.0" encoding="Utf-8" standalone="no"?><Invoice xmlns="urn:oasis:names:specification:',
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                self.env["account.chart.template"].ref(
                    "VAT_P_IN_ROC_25"
                ),  # The tax is flipped to purchase
                "31.00.12",
            ],
        )
    ...
    def test_mer_flow_get_new_documents(self):
        ...
        self.env.company._l10n_hr_mer_get_new_documents(
            ...
            slc=(
                10,
                20,
            ),  # Documents 3107515, 3110637, 3110651, 3111902, 3111911, 3114870, 3120196, 3120260, 3120276, 3120289
        )[self.env.company.id]
        moves = self.env["account.move"].search(
            [("l10n_hr_mer_document_eid", "!=", False)]
        )
        for move in moves:
            ...
            self.assertEqual(
                any(line.l10n_hr_kpd_category_id for line in move.line_ids), True
            )
