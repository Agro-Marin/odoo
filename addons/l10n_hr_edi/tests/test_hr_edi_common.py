...
class TestL10nHrEdiCommon(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.env.company.tax_config_id.tax_calculation_rounding_method = "round_globally"
        cls.partner_a.invoice_edi_format = "ubl_hr"
        cls.product_a.product_tmpl_id.l10n_hr_kpd_category_id = cls.env[
            "l10n_hr.kpd.category"
        ].search([("name", "=", "01.11.11")])
        ...
        partner.write(
            {
                ...
                "bank_account_ids": [
                    Command.create(
                        {"acc_number": "HR10000000000000", "allow_out_payment": True}
                    )
                ],
        ...
        partner.write(
            {
                ...
                "country_id": self.env.ref("base.hr").id,
                "bank_account_ids": [
                    Command.create({"acc_number": "HR20000000000000"})
                ],
        ...
        partner.write(
            {
                ...
                "country_id": self.env.ref("base.hr").id,
                "bank_account_ids": [
                    Command.create({"acc_number": "HR30000000000000"})
                ],
