...
class TestL10nHrEdiXml(TestL10nHrEdiCommon, AccountTestInvoicingCommon):
        ...
        invoice.l10n_hr_edi_addendum_id = self.env["l10n_hr_edi.addendum"].create(
            {
                ...
                "fiscalization_number": self.env[
                    "account.move"
                ]._get_l10n_hr_fiscalization_number(invoice.name),
            }
        )
        actual_content, _dummy = (
            self.env["account.edi.xml.ubl_hr"]
            .with_context(lang="en_US")
            ._export_invoice(invoice)
        )
        with misc.file_open(
            f"addons/{self.test_module}/tests/test_files/test_invoice.xml", "rb"
        ) as file:
    ...
    def test_export_invoice_with_cash_basis(self):
        ...
        self.setup_partner_as_hr_alt(self.partner_a)
        tax = self.env["account.chart.template"].ref("VAT_S_IN_ROC_25")
        tax.cash_basis_transition_account_id = self.safe_copy(
            self.company_data["default_account_tax_sale"]
        )
        ...
        with misc.file_open(
            f"addons/{self.test_module}/tests/test_files/test_invoice_cash_basis.xml",
            "rb",
        ) as file:
            expected_content = file.read()
        self.assertXmlTreeEqual(
    ...
    def test_export_invoice_with_exemption_k(self):
        ...
        with misc.file_open(
            f"addons/{self.test_module}/tests/test_files/test_invoice_exemption_k.xml",
            "rb",
        ) as file:
            expected_content = file.read()
        self.assertXmlTreeEqual(
    ...
    def test_export_invoice_with_exemption_e(self):
        ...
        with misc.file_open(
            f"addons/{self.test_module}/tests/test_files/test_invoice_exemption_e.xml",
            "rb",
        ) as file:
            expected_content = file.read()
        self.assertXmlTreeEqual(
    ...
    def test_export_invoice_with_refund(self):
        ...
        with misc.file_open(
            f"addons/{self.test_module}/tests/test_files/test_invoice_refund.xml", "rb"
        ) as file:
            expected_content = file.read()
        self.assertXmlTreeEqual(
            self.get_xml_tree_from_string(actual_content),
