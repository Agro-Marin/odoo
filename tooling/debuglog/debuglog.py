...
def _header_mentions_debug(stmt: ast.stmt) -> bool:
    return any(_mentions_debug(node) for node in _iter_header_nodes(stmt))
...
class _Scanner(ast.NodeVisitor):
    ...
    def _site(self, node: ast.stmt, kind: str, channel: str | None = None) -> None:
        end = node.end_lineno or node.lineno
        self.report.sites.append(Site(self.path, node.lineno, end, kind, channel))
    ...
    def _visit_stmt(self, stmt: ast.stmt, body: list[ast.stmt]) -> None:
        ...
        if isinstance(stmt, ast.ImportFrom) and _is_debug_import(stmt):
            ...
            return
        if _header_mentions_debug(stmt):
