...
# Members an addon owns on its OWN objects, whose names merely collide with
# something `web` also declares. This gate does no type inference -- it cannot,
# and the alternative rules all cost more than they buy: treating "the addon
# writes it somewhere" as ownership would excuse `record._update = ...` in
# `documents`, which is the worst entry here. So the collisions are pinned
# individually with a reason, as `js_forced_render` pins its known sites, rather
# than left in a floor that could never reach zero.
...
class CrossTreeAccess:
    """An addon reaching into a private member `web` declares.

    Kept apart from `Access` on purpose. Within `web` the question is which of
    two collaborating modules owns a name, and the answer is often "several" --
    so owners are reported, not chosen. Across the tree boundary the question is
    simpler and the answer is never ambiguous: the addon declares this name
    nowhere, `web` declares it, therefore the addon is reaching.
    """
...
def addon_private_names(src: Path) -> set[str]:
    """Every private member declared anywhere under one addon's `static/src`."""
    names: set[str] = set()
...
def measure_cross_tree(
    ...
    """Accesses from a bundled addon into a private member `web` owns.

    The within-`web` scan cannot see these, and not for want of scope: it
    resolves a member's owner from the declarations in the tree it is pointed at,
    so aimed at `addons/account` it finds no owner for `_values` and reports
    nothing while `currency_form/form_controller.js` reads it. Both trees have to
    be indexed at once, which is what this does.

    A member declared anywhere in the consuming addon is left alone -- the
    addon plausibly means its own object, and deciding otherwise would need type
    inference this deliberately does not attempt.
    """
...
def iter_source_files(web_src: Path = WEB_SRC) -> list[Path]:
    if not web_src.is_dir():
        return []
    # Vendored and retired trees are somebody else's code: their underscores say
    # nothing about this one's conventions, and counting them would let a
    # bundled library move the budget.
    return sorted(
        path
