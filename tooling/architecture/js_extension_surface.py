...
MEASURED (regenerated by --update-doc; do not hand-edit)
    points=379 sites=1431 single_use=233 owners=103 subclasses=777 patch_sites=30 triage=250 promote=24 generalize=0 inline=0 keep=227
"""
...
# `const components = formView.Renderer.components;` -- a base class reached
# through a local alias for a member chain. _DESCRIPTOR only sees an object
# literal, so without this the alias resolves to nothing and every class
# extending `components.X` silently contributes no override point.
_MEMBER_ALIAS = re.compile(
    r"^(?:export\s+)?const\s+([A-Za-z_$][\w$]*)\s*=\s*"
    r"([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)+)\s*;",
    re.MULTILINE,
)
_STATIC_OBJECT = re.compile(r"^\s{4}static\s+([A-Za-z_$][\w$]*)\s*=\s*\{", re.MULTILINE)
# A shorthand entry (`Notebook,`) names its own value; `_DESC_PROP` only reads
# the `key: Value` form.
_DESC_SHORTHAND = re.compile(r"^\s*([A-Za-z_$][\w$]*)\s*,\s*$", re.MULTILINE)
...
def object_map(body: str) -> dict[str, str]:
    """`{ a: B, C }` as `{"a": "B", "C": "C"}` -- both spellings of a member."""
    mapping = dict(_DESC_PROP.findall(body))
    for name in _DESC_SHORTHAND.findall(body):
...
def scan_file(source: str) -> dict:
    ...
    for match in _DESCRIPTOR.finditer(src):
        record["descriptors"][match.group(1)] = object_map(match.group(2))
    for match in _MEMBER_ALIAS.finditer(src):
        record["aliases"][match.group(1)] = match.group(2).split(".")
    for match in _CLASS.finditer(src):
        ...
        body = _class_body(src, match.end())
        record["classes"][match.group(1)] = {
            "base": base,
            "prop": prop,
            "form": form,
            "methods": frozenset(_METHOD.findall(body)),
            "statics": {
                static.group(1): object_map(_class_body(body, static.end() - 1))
                for static in _STATIC_OBJECT.finditer(body)
            },
        }
...
class Index:
    ...
    def _lookup_value(self, path: Path, name: str, depth: int = 0):
        """`name` as seen from `path`: a class, an object map, or nothing.

        Returns ("class", home, class name) or ("map", home, properties).
        """
        if depth > 12:
            return None
        record = self.files.get(path)
        ...
        if name in record["aliases"]:
            return self.resolve_member_path(path, record["aliases"][name], depth + 1)
        if name in record["descriptors"]:
            return ("map", path, record["descriptors"][name])
        if name in record["classes"]:
        ...
        if spec:
            target = self.resolve_spec(spec, path)
            if target:
                return self._lookup_value(target, original, depth + 1)
        for star in record["stars"]:
            target = self.resolve_spec(star, path)
            if target and (found := self._lookup_value(target, name, depth + 1)):
                return found
        return None

    def resolve_member_path(self, path: Path, parts: list[str], depth: int = 0):
        """Walk `a.b.c` from `path`, through descriptors and static literals.

        `class X extends components.Notebook` where
        `const components = formView.Renderer.components` is four hops, and
        only the last two are object properties. Following none of them was
        why every such subclass contributed no override point at all -- and
        this list is SHRINK-ONLY, so a base that stops resolving deletes the
        record that a consumer depends on it.
        """
        if depth > 12 or not parts:
            return None
        current = self._lookup_value(path, parts[0], depth + 1)
        for part in parts[1:]:
            if current is None:
                return None
    ...
    def parent_of(self, path: Path, name: str):
        ...
        if info["form"] == "prop":
            ...
            if descriptor:
                ...
                if target:
                    return self.find_class(home, target)
            resolved = self.resolve_member_path(path, [info["base"], info["prop"]])
