...
subclass may override and expect the base to call — is pinned by nothing.

Measured, that contract is large: classes outside ``web`` inherit from classes
inside it and override hundreds of distinct ``Owner.method`` points. Those
points are web's real API. They are declared nowhere.

MEASURED (regenerated by --update-doc; do not hand-edit)
    points=448 sites=1896 single_use=242 owners=112 subclasses=1175
...
* **``patch()`` is out of scope.** It is a different mechanism with a different
  failure mode (load-order dependence) and belongs to a separate gate;
  ``js_patch_blind_facade.py`` already owns part of that ground.
...
def write_pinned(measured_provenance: dict[str, frozenset[str]]) -> None:
    header = (
        "# The `Owner.method` points that classes outside `web` override: web's\n"
        "# inheritance surface, as it is rather than as anyone designed it. Each\n"
        "# entry names the consumer checkout(s) overriding it — its provenance —\n"
        ...
        "# overridden by exactly one subclass — the worklist for narrowing the\n"
