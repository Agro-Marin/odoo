...
MEASURED (regenerated by --update-doc; do not hand-edit)
    points=493 sites=1978 single_use=273 owners=129 subclasses=1175 patch_sites=70
...
def scan_file(source: str) -> dict:
    ...
    for names, spec in _IMPORT.findall(src):
        for raw in names.split(","):
            name = raw.strip()
            if name:
                record["imports"][name.split(" as ")[-1].strip()] = spec
...
class Index:
    ...
    def find_class(self, path: Path, name: str, depth: int = 0):
        ...
        if name in record["classes"]:
            ...
            target = self.resolve_spec(record["imports"][name], path)
            if target:
                found = self.find_class(target, name, depth + 1)
        else:
    ...
    def find_descriptor(self, path: Path, name: str, depth: int = 0):
        ...
        if name in record["reexports"]:
            spec = record["reexports"][name][0]
        elif name in record["imports"]:
            spec = record["imports"][name]
        if spec:
            target = self.resolve_spec(spec, path)
            if target:
                return self.find_descriptor(target, name, depth + 1)
