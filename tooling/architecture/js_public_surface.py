...
the surface cannot grow or silently shrink. A per-specifier ``internal:N``
tier existed here for a while, populated by nobody — no entry ever used it —
and was removed; if per-case judgements ever want a tier column, it should be
...
def load_pinned() -> set[str]:
    """The pinned specifiers; membership-only."""
    pinned: set[str] = set()
...
def write_pinned(measured: dict[str, int]) -> None:
    """Rewrite the pin from the measured surface, sorted, membership-only."""
    header = (
        ...
        "#\n"
        "# Every entry is deliberately unclassified — see the module docstring\n"
        "# on why most of this surface has no defensible tier yet.\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.update:
        ...
        print(f"wrote {PINNED.name}: {len(measured)} specifier(s)")
        return 0

    pinned = load_pinned()
    new = sorted(set(measured) - pinned)
    gone = sorted(pinned - set(measured))
