...
def write_pinned(
    ...
    # Wrapped rather than hand-broken: the text interpolates `addon`, so any
    # fixed set of line breaks is only correct for one addon name. It was
    # hand-broken, and the reflow left an orphan "# Each" line in the pin.
    paragraphs = (
        (
    ...
    header = (
        "\n#\n".join(
            textwrap.fill(
                ...
                initial_indent="# ",
                subsequent_indent="# ",
                break_on_hyphens=False,  # "repo-alone" is one word
