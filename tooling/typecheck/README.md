...
Per-file type-check gates, one lock per module. Every `.js`/`.ts` under a gated
module's `static/src/` and `static/tests/` must be error-free under a given
tsconfig unless it is named in `exceptions/<gate>/<module>.txt`.
