...
§2.4.1   Names wearing a hook prefix with no binding                   152
§2.4.1   … definitions under those names                               217
...
§2.4.2   ``@api.constrains`` hooks                                     662
§2.4.2   … spelled ``_check_*``                                        610
...
§2.4.3   Non-test methods declared on a model class                 25,746
...
deriving and computing. The split that matters is against ``_prepare_``: 673
definitions are payload builders -- they end in ``_vals``, ``_values``, ``_data``,
``_dict``, ``_context``, ``_defaults``, ``_list``, ``_args`` or ``_params`` -- yet
are spelled ``get_*``, against 838 already spelled ``_prepare_*``.
...
tail -- the tree spells that tail ``_uniq`` **91** times against ``_unique``'s
