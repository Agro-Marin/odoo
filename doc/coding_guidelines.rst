...
§2.4.1   Names wearing a hook prefix with no binding                   154
§2.4.1   … definitions under those names                               219
...
§2.4.2   ``@api.constrains`` hooks                                     671
§2.4.2   … spelled ``_check_*``                                        619
§2.4.2   … with a first token carrying no rule                          47
§2.4.2   … binding exactly one field                                   308
§2.4.2   … of those, spelled ``_check_<field>``                        136
§2.4.2   Multi-field constraints named for one trigger                  60
§2.4.3   Non-test methods declared on a model class                 25,796
...
§2.4.11  ``_find_*`` methods                                            28
§2.4.11  … performing an ORM read                                        4
§2.4.11  … doing something else entirely                                23
...
§2.4.13  Methods on plain classes in model files                       409
§2.4.13  … such classes                                                165
§2.4.13  Functions nested inside model methods                         579
...
deriving and computing. The split that matters is against ``_prepare_``: 674
definitions are payload builders -- they end in ``_vals``, ``_values``, ``_data``,
``_dict``, ``_context``, ``_defaults``, ``_list``, ``_args`` or ``_params`` -- yet
are spelled ``get_*``, against 835 already spelled ``_prepare_*``.
...
call sites ever see. **94** parameters annotated ``field_name`` are ``str`` and
