...
js_unreached_assertions jsunreached
js_class_length    jsclasslen
...
| `mail_hook_keyword_check.py` | the keywords `mail` passes to its own `_notify_*` / `_message_*` / `_track_*` / `_mail_*` hooks, against every override of them — `mail` is a framework whose extension points are implemented in dozens of addons across four repos, so its own **658**-test suite is structurally unable to see a signature it just broke, and `28ed9db3341` broke six overrides and merged green |
...
| `js_view_descriptor.py` | three halves of the view-type contribution contract, resolved through the class chain across every repository: every base view type's `ArchParser` extends `ViewArchParser` (zero of nine extension parsers did until 2026-09-09, because the base sat on no public-surface row), and a controller asking for sample data has a model that overrides `hasData()` -- `Model.hasData()` answers true unconditionally, so without the override `sample="1"` is inert and the hook warns once per mount instead of once per tree; and every base view type has a `studio_editors` entry somewhere in the tree, contributed by a `<module>_studio` bridge or by web_studio's own editors for the types web ships, or is pinned with a reason -- since the Studio inversion, editability is a fact about that registry and not about web_studio's manifest |
...
| `js_eager_mock_fixture.py` | a mock fixture mutating another addon's model at module scope, or a service mocked at module scope (`mockService(...)` outside any hook, which is the last imported file's mock for every suite in the bundle -- five of them cost sign fifteen reds) — hoot imports every test file during collection and model definitions are job-scoped per test, so such a statement runs for every suite in the bundle EXCEPT the one that wrote it. Fifty of these cost 37 scoped failures across twelve POS addons; a hard zero with no baseline file |
...
| `js_field_record_surface.py` | what field widgets reach through `props.record` — `standardFieldProps` hands all **85** members of a live `RelationalRecord` to **117** widgets in this checkout, and a prop read is neither an import nor a class member, so no other gate sees it. Both figures are this repository's, not the workspace's: the gate's block was pinned once at 155 widgets from an assembled workspace, and failed in the build and nowhere else |
...
| `field_hook_purity.py` | whether the method a field attribute names is a hook at all — **13** are also called from production code, down from the 342 the gate opened with, which makes a compute's dependency graph something its callers compensate for |
...
| `module_category_vocabulary.py` | the roots a manifest's `category` may name. `get_or_create_category_id` builds an `ir.module.category` row per path prefix, so a manifest CREATES whatever it names and a typo is indistinguishable from a decision — the sweep that emptied this gate found 47 root categories where 18 were meant, among them `Fleet` beside `Human Resources/Fleet` and one whose name was a summary sentence. Only the ROOT is closed: a module has always been free to name its own leaf, and the root is the part `ir.ui.menu._get_app_categories` hands the app launcher as a group heading. Its second rule reads the data files rather than the manifests — a declared category whose xml id path names a parent that also exists must declare it, or it becomes a heading of its own. Cross-repo by construction, since agromarin declares `Agriculture` and community cannot know that |
| `credential_storage.py` | a third-party secret resting in a stored `Char`/`Text` field instead of the vault |
| `computed_default.py` | an editable stored compute that also carries a class `default=` (a falsy one too: `default_get` returns it and `create` puts it in the values; only `default=None` declares none), merged across every extension of the model and every sibling repo. `create` fills the default before computing, and `_create_partition_values` protects any editable compute present in the values, so a caller that passes the compute's dependencies still gets the default: `fleet.vehicle.range_unit` stored km for a miles model, `calendar.event.stop` a three-year all-day event, and a copied inbox user lost its inbox group, until `333673259700`. Each declaration is either in `REVIEWED`, with the reason the default cannot disagree with the compute, or in `PENDING`, a candidate not yet exercised on a real create; an unlisted one fails `--check`, and so does a listed one that no longer has the shape. `PENDING` only shrinks. Prefer `precompute=True` with the fallback inside the compute, and read the precompute chain first: `resolve_depends` refuses a precompute over a stored compute that does not precompute |
| `settings_default_fields.py` | a `res.config.settings` field named `default_<name>` whose `default_model` declares no `<name>`, merged across every declaration of the model in the checkout and the sibling repos, following `_inherit` parents and `_inherits` delegations and counting the magic fields. `set_values` stores the value through `ir.default.set`, which raises `Invalid field`, so the settings page renders and only a save fails: every save in every database with the module, and every install beside a module whose data calls `execute`. `l10n_be_hr_payroll` kept `default_mobile` after `hr.version.mobile` became `mobile_subscription`. Hard zero, no list |
...
**Drift-zero count ratchet** (`tooling/ratchet/`) — turns eighty-four tool
counts into one-way contracts: **mypy, ruff, c901, c901_addons, tsc, tsc_serviceworker, jsfunclen, jsfunclen_mail, jsfunclen_account, jsfunclen_survey, pyfunclen, pyfunclen_addons, pyclasslen, pyclasslen_addons, werkzeug_in_addons, config_in_addons, pyfunclen_mail, pyfunclen_crm, pyfunclen_survey, pyfunclen_tooling, py_x2many_count, py_x2many_count_addons, py_x2many_count_account, py_x2many_count_enterprise, py_x2many_count_agromarin, py_shadowed_member_addons, bridge_budget, jsprivate, jsprivate_crosstree, jsserviceshape, jsserviceshape_mail, jsforcedrender, jsvacuous, jsunreached, jsclasslen, jstscheck, jsduplication, prettier_scss, prettier_dts, naming, naming_enterprise, naming_agromarin, fieldhooks, hookpurity, computectx, translations, mypy_tools, service_types_untyped, orderlineqty, orderlineqty_enterprise, unresolved_calls, unresolved_calls_enterprise, unresolved_calls_agromarin, bundle_double_eval, lint_docstring, lint_gettext_developer_error, lint_gettext_placeholders, lint_gettext_repr, lint_gettext_variable, lint_missing_gettext, lint_n_plus_one_query, lint_noqa_rationale, lint_null_exempt_composite_unique, lint_raise_unlink_override, lint_sql_injection, lint_gettext_developer_error_enterprise, lint_missing_gettext_enterprise, lint_n_plus_one_query_enterprise, lint_noqa_rationale_enterprise, lint_null_exempt_composite_unique_enterprise, lint_raise_unlink_override_enterprise, lint_sql_injection_enterprise, lint_gettext_developer_error_agromarin, lint_gettext_placeholders_agromarin, lint_gettext_repr_agromarin, lint_gettext_variable_agromarin, lint_missing_gettext_agromarin, lint_n_plus_one_query_agromarin, lint_noqa_rationale_agromarin, lint_null_exempt_composite_unique_agromarin, lint_sql_injection_agromarin, lint_noqa_rationale_design-themes, lint_record_reference and orphandepends**
...
| `c901_addons` | the same gate over `addons/`, where the 668 bundled modules and most business logic live and complexity was unbounded. The two trees move by different hands |
...
`test_orm` — **1,230 test methods** under its `tests/` directory — is the addon
