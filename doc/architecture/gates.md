...
| `field_hook_purity.py` | whether the method a field attribute names is a hook at all — **16** are also called from production code, down from the 342 the gate opened with, which makes a compute's dependency graph something its callers compensate for |
...
| `c901_addons` | the same gate over `addons/`, where the 642 bundled modules and most business logic live and complexity was unbounded. The two trees move by different hands |
