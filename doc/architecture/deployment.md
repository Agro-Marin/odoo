...
The framework does not ship one deployment shape. It ships three, chosen at
startup by two config values, and they differ in ways that reach the
architecture rather than the ops runbook — most of all in whether memory is
shared.
...
`PreforkServer` maintains three distinct worker populations, not one:
...
So `--workers 4` does **not** mean four processes: it means four HTTP workers
*plus* the cron and job populations, each sized independently. The listen
backlog is `8 * population`, and the population itself can be adjusted at
runtime (`self.population += 1`).
...
Failure is handled in `odoo/db/`, and each piece has one job:
...
| **`odoo_rust`** | imported unconditionally at startup — no pure-Python fallback |
