...
| R4 | "Enforced" means structural only — 66 gates cannot see behaviour | High | 2026-08-08 | — |
...
gate — all 66 are structural and DB-free — and not caught by either DB-free test
tier.
...
disk: **255** scripts in `migrations/` and **5** in `upgrades/`, all correctly
prefixed, **0** dropped.
...
**What.** The 66 boundary checkers read import graphs, call graphs,
reached-member sets and documents. None executes the framework. A change can
satisfy all 66 and both DB-free tiers and still be wrong.
