...
from odoo.service.db.dump import _zip_filestore_into


def _pack(filestore: Path) -> dict[str, bytes]:
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w") as zipf:
        _zip_filestore_into(zipf, str(filestore))
...
class TestNothingOutsideTheFilestoreLeavesInABackup:
    """`_zip_filestore_into` decides what a downloadable backup contains.
