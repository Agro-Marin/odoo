...
def sanitize_excel_sheet_name(name: str, taken: Iterable[str] = ()) -> str:
    ...
    Edge whitespace goes too, including whatever the cut exposes. Excel
    tolerates a trailing space but a formula referring to such a sheet needs
    quoting, and truncation manufactures them from any name with a space at
    character 31.
    ...
    name = name[:_MAX_SHEET_NAME_LENGTH].rstrip()
    if not name:
        return name
