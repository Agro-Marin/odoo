...
class TestSubqueries(TransactionCase):
    ...
    def test_or_bypass_access_many2one_with_subfield(self):
        self.patch(
            self.env["test_orm.multi"]._fields["partner"],
...
class TestSearchRelated(TransactionCase):
        ...
        with self.assertQueries(["""
            ...
            WHERE "test_orm_related"."foo_id" IN (
                ...
                WHERE EXISTS(SELECT FROM (
                    ...
                    FROM "test_orm_related"
                    WHERE "test_orm_related"."id" = ANY(%s)
    ...
    def test_related_through_one2many(self):
        model = self.env["test_orm.related_foo"].with_user(
    ...
    def test_related_inherited(self):
        model = self.env["test_orm.related_inherits"].with_user(
            self.env.ref("base.user_admin")
        )
    ...
    def test_many2one_any(self):
        ...
        model.search(
            Domain(
                "foo_id",
                "any",
                Domain("bar_id", "any", Domain("name", "=", "a")),
            )
        )
        model.search(
            Domain(
    ...
    def test_one2many_any(self):
        ...
        model.search(
            Domain(
                "foo_ids",
                "any",
                Domain("foo_id", "any", Domain("name", "=", "a")),
            )
        )
...
class TestFlushSearch(TransactionCase):
    ...
    def test_flush_bypass_access_field_in_domain(self):
        self.patch(
            self.env.registry["test_orm.city"].country_id,
            "bypass_search_access",
            True,
        )
    ...
    def test_do_not_flush_fields_to_fetch(self):
        ...
        """,
            ...
            self.model.search_fetch(
                [("id", "=", self.brussels.id)], ["name"], order="name"
            )
    ...
    def test_depends_with_table_query_model(self):
        ...
        child.quantity = 25
        self.assertEqual(
            self.env["test_orm.custom.table_query"].search([]).sum_quantity, 25
        )
...
class TestDatePartNumber(TransactionExpressionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.person = cls.env["test_orm.person"].create(
            {"name": "that person", "birthday": "1990-02-09"}
        )
        cls.lesson = cls.env["test_orm.lesson"].create(
    ...
    def test_datetime_filtered(self):
        Person = self.env["test_orm.person"].with_context(active_test=False)
        self.assertEqual(
            self._search(Person, [("birthday.month_number", "=", 2)]),
            self.person,
        )

    def test_many2one(self):
        result = self._search(
            self.env["test_orm.lesson"],
            [("teacher_id.birthday.month_number", "=", 2)],
        )
    ...
    def test_many2many(self):
        result = self._search(
            self.env["test_orm.lesson"],
            [("attendee_ids.birthday.month_number", "=", 2)],
        )
        self.assertEqual(result, self.lesson)
    ...
    def test_related_field(self):
        result = self._search(
            self.env["test_orm.lesson"],
            [("teacher_birthdate.month_number", "=", 2)],
        )
        self.assertEqual(result, self.lesson)

    def test_inherit(self):
        account = self.env["test_orm.person.account"].create(
            {"person_id": self.person.id, "activation_date": "2020-03-09"}
        )

        result = self._search(
            self.env["test_orm.person.account"],
            [("activation_date.quarter_number", "=", 1)],
        )
        self.assertEqual(result, account)

        result = self._search(
            self.env["test_orm.person.account"],
            [("person_id.birthday.month_number", "=", 2)],
        )
        self.assertEqual(result, account)
...
class TestNonIntId(TransactionCase):
    ...
    def test_query_non_int_read_group(self):
        result = self.env["test_orm.view.str.id"].formatted_read_group(
            [], ["name"], ["__count"]
        )
        self.assertEqual(
            result,
