...
class TestReflection(common.TransactionCase):
    ...
    def test_models_fields(self):
        model_names = [
            ...
            "domain.bool",
            "mixin.test_orm",
            "mixin.test_orm.count",
            "mixin.test_orm.state",
            "test_orm.any.child",
            "test_orm.any.parent",
            "test_orm.any.tag",
            "test_orm.attachment",
            ...
            "test_orm.inverse.with.unlink",
            "test_orm.inverse.with.unlink.child",
            ...
            "test_orm.model_selection_company_dependent",
        ...
        for ir_model in ir_models:
            with self.subTest(model=ir_model.model):
                ...
                self.assertTrue(
                    model_field_names <= ir_field_names,
                    f"Fields in _fields but not in ir.model.fields: "
                ...
                for ir_field in ir_model.field_id:
                    ...
                    with self.subTest(field=ir_field.name):
                        ...
                        self.assertEqual(
                            ir_field.state, "manual" if field.manual else "base"
                        )
                        ...
                        self.assertEqual(
                            FIELD_TRANSLATE.get(ir_field.translate or None, True),
                            field.translate,
                        )
                        if field.relational:
                        ...
                        if field.type == "one2many" and field.store:
                            self.assertEqual(
                                ir_field.relation_field, field.inverse_name
                            )
...
class TestSchema(common.TransactionCase):
    ...
    def test_00_table(self):
        ...
        self.assertTrue(
            expected_columns <= set(columns_data),
    ...
    def test_10_text(self):
        ...
        if body_field.translate:
            ...
            expected_type, expected_udt = "text", "text"
        self.assertEqual(
            columns_data["body"],
            {
                "character_maximum_length": None,
    ...
    def test_20_unique_indexes(self):
        ...
        self.env.cr.execute(
            ...
            SELECT tablename
            FROM pg_indexes
            WHERE tablename = ANY(%s) AND indexdef LIKE %s
        """,
