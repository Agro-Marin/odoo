...
class TestUnityRead(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.course_no_author = cls.env["test_orm.course"].create(
            {"name": "some other course without author"}
        )
    ...
    def test_read_add_id(self):
        ...
        self.assertEqual(
            read,
            [{"id": self.course.id, "display_name": "introduction to OWL"}],
        )
    ...
    def test_read_many2one_can_read_extra_fields(self):
        read = self.course.web_read(
            {"display_name": {}, "author_id": {"fields": {"write_date": {}}}}
        )
        self.assertEqual(
            read,
            [
                {
                    ...
                    "author_id": {
                        "id": self.author.id,
                        "write_date": self.author.write_date,
                    },
                },
            ],
        )
            ...
            [
                {
                    ...
                    "lesson_ids": [
                        ...
                        {
                            "id": self.lesson_day2.id,
                            "display_name": "special second day",
                        },
                    ],
                }
            ],
    ...
    def test_one2many_with_order_respects_field_context(self):
        archived = self.env["test_orm.person"].create(
        ...
        employer = self.env["test_orm.employer"].create(
            {
                "name": "JS Corp",
                "all_employee_ids": [
                    Command.set([archived.id, self.teacher.id, self.author.id])
                ],
            }
        )
                ...
                [
                    {
                        ...
                        "lesson_ids": [
                            {
                                "id": self.lesson_day1.id,
                                "attendee_ids": [*self.lesson_day1.attendee_ids._ids],
                            },
                    ...
                    "lesson_ids": [
                        {
                            ...
                            "attendee_ids": [
                                ...
                                {
                                    "id": self.lesson_day1.attendee_ids._ids[1],
                                    "display_name": "456",
                                },
                                {
    ...
    def test_many2one_reference_naked(self):
        read = self.course.web_read({"m2o_reference_id": {}, "m2o_reference_model": {}})
        self.assertEqual(
            read,
            [
                {
    ...
    def test_read_with_pending_stored_computed(self):
        ...
        disc = Discussion.create(
            {
                "name": "OrigName",
                "moderator": user.id,
                "participants": [Command.link(user.id)],
            }
        )
        msg = Message.create(
            {
        ...
        self.assertEqual(
            result[0]["name"],
            f"[NewName] {author_name}",
            "read() must process pending recomputations before returning cached values",
        )
    ...
    def test_read_format_pending_vs_null(self):
        ...
        self.assertTrue(
            result[0]["name"],
            "Stored computed field should have a real value after create+read",
        )
        ...
        self.assertNotEqual(
            ...
            "Sentinel.PENDING",
            "PENDING sentinel must never leak into read() results",
        )
