...
class TestDefaultView(common.TransactionCase):
    def test_default_form_view(self):
        self.assertEqual(
            ...
            b'<form><sheet string="Test ORM Message"><group><group><field name="discussion"/></group></group><group><field name="body"/></group><group><group><field name="author"/><field name="size"/><field name="discussion_name"/><field name="important"/></group><group><field name="name"/><field name="double_size"/><field name="author_partner"/><field name="label"/></group></group><group><field name="plain_text"/></group><group><group><field name="priority"/><field name="has_important_sibling"/></group><group><field name="active"/><field name="attributes"/></group></group><group><separator/></group></sheet></form>',
        )
        self.assertEqual(
            etree.tostring(
                self.env["test_orm.creativework.edition"]._get_default_form_view()
            ),
...
class TestViewGroups(ViewCase):
    def test_attrs_groups(self):
        if "notification_type" not in self.env["res.users"]._fields:
            self.env.cr.execute(
                ...
                "WHERE table_name = 'res_users' AND column_name = 'notification_type'"
            )
            if self.env.cr.fetchone():
                self.env.cr.execute(
                    "UPDATE res_users SET notification_type = 'email' "
                    "WHERE id = %s AND notification_type != 'email'",
                    [self.env.user.id],
                )
        self.env.user.group_ids = [(6, 0, [self.env.ref("base.group_system").id])]
        f = Form(
            self.env["test_orm.model.some_access"],
            view="test_orm.view_model_some_access",
        )
        f.a = 1
        f.b = 2
        with self.assertRaisesRegex(
            AssertionError, "can't write on readonly field 'c'"
        ):
            f.c = 3
        with self.assertRaisesRegex(
            AssertionError, "can't write on readonly field 'e'"
        ):
            f.e = 3
        with self.assertRaisesRegex(
            AssertionError, "can't write on readonly field 'f'"
        ):
            f.f = 3
    ...
    def test_model_access(self):
        ...
        arch = self.env["test_orm.model.all_access"]._get_view_cache(view_id=view.id)[
            "arch"
        ]
    ...
    def test_tree(self):
        ...
        arch = self.env["test_orm.model.some_access"].get_views([(view.id, "list")])[
            "views"
        ]["list"]["arch"]
        tree = etree.fromstring(arch)

        nodes = tree.xpath(
            "//list/field[@name='a'][@column_invisible='True'][@readonly='True']"
        )
        self.assertTrue(nodes, "list should contains the missing field 'a'")

        nodes = tree.xpath(
            "//groupby/field[@name='ab'][@invisible='True'][@readonly='True']"
        )
    ...
    def test_add_properties_definition_field(self):
        ...
        self.assertEqual(
            discussion_node.get("data-used-by"),
            "fieldname='attributes' (field,attributes)",
        )
    ...
    def test_auto_add_filename_with_binary_requesting(self):
        ...
        self.assertEqual(
            dict(filename.attrib),
            {
                "name": "binary_x_filename",
                "invisible": "True",
                "readonly": "context.get('dummy')",
                "data-used-by": "filename='binary_x_filename' (field,binary)",
            },
        )

        filename2 = form.xpath("//field[@name='binary_x_filename2']")[0]
        self.assertEqual(
            dict(filename2.attrib),
            {
                "name": "binary_x_filename2",
                "invisible": "True",
                "readonly": "False",
                "data-used-by": "filename='binary_x_filename2' (field,binary)",
            },
        )
