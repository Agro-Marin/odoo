...
class TestOrmCategory(models.Model):
    ...
    discussions = fields.Many2many(
        "test_orm.discussion",
        "test_orm_discussion_category",
        "category",
        "discussion",
    )
...
class TestOrmDiscussion(models.Model):
    ...
    important_messages = fields.One2many(
        "test_orm.message", "discussion", domain=[("important", "=", True)]
    )
...
class TestOrmMessage(models.Model):
    ...
    discussion_name = fields.Char(
        related="discussion.name", string="Discussion Name", readonly=False
    )
    ...
    def _compute_has_important_sibling(self):
        for record in self:
            siblings = (
                record.discussion.with_context(active_test=False).messages - record
            )
    ...
    def _check_author(self):
        for message in self.with_context(active_test=False):
            if (
                message.discussion
                and message.author not in message.discussion.sudo().participants
            ):
                raise ValidationError(
                    self.env._("Author must be among the discussion participants.")
                )

    @api.depends("author.name", "discussion.name")
    def _compute_name(self):
        for message in self:
            message.name = self.env.context.get(
                "compute_name",
    ...
    def _search_size(self, operator, value):
        ...
        if field.translate:
            ...
            self.env.cr.execute(query, (lang, value))
        else:
...
class TestOrmCreativeworkBook(models.Model):
    ...
    editions = fields.One2many(
        ...
        "res_id",
        domain=[("res_model", "=", "test_orm.creativework.book")],
    )
...
class TestOrmCreativeworkMovie(models.Model):
    ...
    editions = fields.One2many(
        "test_orm.creativework.edition",
        "res_id",
        domain=[("res_model", "=", "test_orm.creativework.movie")],
    )
...
class TestOrmMixed(models.Model):
    ...
    reference = fields.Reference(
        string="Related Document", selection="_reference_models"
    )
...
class TestOrmBar(models.Model):
    ...
    def _compute_foo(self):
        for bar in self:
            bar.foo = self.env["test_orm.foo"].search(
...
class TestOrmRelated(models.Model):
    ...
    related_related_name = fields.Char(
    ...
    foo_name_sudo = fields.Char(
        "foo_name_sudo", related="foo_id.name", related_sudo=True
    )

    foo_bar_name = fields.Char(
        "foo_bar_name", related="foo_id.bar_id.name", related_sudo=False
    )
    foo_bar_name_sudo = fields.Char(
        "foo_bar_name_sudo", related="foo_id.bar_id.name", related_sudo=True
    )

    foo_id_bar_name = fields.Char(
        "foo_id_bar_name", related="foo_id.bar_name", related_sudo=False
    )

    foo_bar_id = fields.Many2one(
        related="foo_id.bar_id", related_sudo=False, string="Bar"
    )
    foo_bar_id_name = fields.Char(
        related="foo_bar_id.name", related_sudo=False, string="Bar Name"
    )

    foo_bar_sudo_id = fields.Many2one(
        related="foo_id.bar_id", related_sudo=True, string="Bar Sudo"
    )
    foo_bar_sudo_id_name = fields.Char(
        related="foo_bar_sudo_id.name",
        related_sudo=False,
        string="Bar Sudo Name",
    )

    foo_bar_ids = fields.Many2many(related="foo_id.bar_ids", related_sudo=False)
    foo_bar_sudo_ids = fields.Many2many(
        related="foo_id.bar_ids", related_sudo=True, string="Bars Sudo"
    )

    foo_foo_ids = fields.One2many(
        related="foo_id.foo_ids", related_sudo=False, string="Foo Foos"
    )
    foo_foo_sudo_ids = fields.One2many(
        related="foo_id.foo_ids", related_sudo=True, string="Foo Foos Sudo"
    )

    foo_binary_att = fields.Binary(related="foo_id.binary_att", related_sudo=False)
    foo_binary_att_sudo = fields.Binary(
        related="foo_id.binary_att", related_sudo=True, string="Binary Att Sudo"
    )

    foo_binary_bin = fields.Binary(related="foo_id.binary_bin", related_sudo=False)
    foo_binary_bin_sudo = fields.Binary(
        related="foo_id.binary_bin", related_sudo=True, string="Binary Bin Sudo"
    )
...
class TestOrmRelated_Foo(models.Model):
    ...
    foo_names = fields.Char(
        related="foo_ids.name", related_sudo=False, string="Foo Names"
    )
    foo_names_sudo = fields.Char(
        related="foo_ids.name", related_sudo=True, string="Foo Names Sudo"
    )

    bar_names = fields.Char(
        related="bar_ids.name", related_sudo=False, string="Bar Names"
    )
    bar_names_sudo = fields.Char(
        related="bar_ids.name", related_sudo=True, string="Bar Names Sudo"
    )
...
class TestOrmMove(models.Model):
    ...
    line_ids = fields.One2many(
        "test_orm.move_line", "move_id", domain=[("visible", "=", True)]
    )
...
class TestOrmRecursive(models.Model):
    ...
    context_dependent_name = fields.Char(
    ...
    def _compute_context_dependent_name(self):
        for rec in self:
            if rec.parent:
                rec.context_dependent_name = (
                    rec.parent.context_dependent_name + " / " + rec.name
                )
            else:
...
class TestOrmComputeOnchange(models.Model):
    ...
    def copy_data(self, default=None):
        ...
        return [
            dict(vals, foo=self.env._("%s (copy)", record.foo))
    ...
    def _compute_full_name(self):
        ...
        for record in self:
            record.full_name = ", ".join(
                filter(None, (record[fname] for fname in fnames))
            )
...
class TestOrmComputedInverseOne2many(models.Model):
    ...
    all_line_ids = fields.One2many(
        "test_orm.computed_inverse_one2many_line", "parent_id"
    )
    low_priority_line_ids = fields.One2many(
        "test_orm.computed_inverse_one2many_line",
        compute="_compute_priority_line_ids",
        inverse="_inverse_line_ids",
    )
    high_priority_line_ids = fields.One2many(
        "test_orm.computed_inverse_one2many_line",
        compute="_compute_priority_line_ids",
        inverse="_inverse_line_ids",
    )
    ...
    def _inverse_line_ids(self):
        for record in self:
            record.all_line_ids = (
                record.low_priority_line_ids | record.high_priority_line_ids
            )
...
class TestOrmModel_Binary(models.Model):
    ...
    binary_related_store = fields.Binary(
        "Binary Related Store", related="binary", store=True, readonly=False
    )
    binary_related_no_store = fields.Binary(
        "Binary Related No Store", related="binary", store=False, readonly=False
    )
...
class TestOrmMonetary_Related(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency", related="monetary_id.base_currency_id"
    )
...
class TestOrmMonetary_Inherits(models.Model):
    ...
    monetary_id = fields.Many2one(
        "test_orm.monetary_base", required=True, ondelete="cascade"
    )
...
class TestOrmMonetary_Order_Line(models.Model):
    ...
    order_id = fields.Many2one(
        "test_orm.monetary_order", required=True, ondelete="cascade"
    )
...
class TestOrmModel_A(models.Model):
    ...
    name = fields.Char()
    a_restricted_b_ids = fields.Many2many(
        "test_orm.model_b", relation="rel_model_a_model_b_1"
    )
    b_restricted_b_ids = fields.Many2many(
        "test_orm.model_b",
        relation="rel_model_a_model_b_2",
        ondelete="restrict",
    )
...
class TestOrmModel_B(models.Model):
    ...
    name = fields.Char()
    a_restricted_a_ids = fields.Many2many(
        "test_orm.model_a",
        relation="rel_model_a_model_b_1",
        ondelete="restrict",
    )
    b_restricted_a_ids = fields.Many2many(
        "test_orm.model_a", relation="rel_model_a_model_b_2"
    )
...
class TestOrmModel_Active_Field(models.Model):
    ...
    all_children_ids = fields.One2many(
        "test_orm.model_active_field",
        "parent_id",
        context={"active_test": False},
    )
    active_children_ids = fields.One2many(
        "test_orm.model_active_field",
        "parent_id",
...
class TestOrmModel_Selection_Required(models.Model):  # noqa: F811
    ...
    my_selection = fields.Selection(
        ...
        ondelete={
            "pikachu": "set default",
            "eevee": lambda r: r.write({"my_selection": "bar"}),
        },
    )
...
class TestOrmModel_Selection_Required(models.Model):  # noqa: F811
    ...
    my_selection = fields.Selection(
        ...
        ondelete={
            "knickers": lambda recs: recs.write(
                {"active": False, "my_selection": "foo"}
            ),
        },
    )
...
class TestOrmModel_Selection_Required_For_Write_Override(models.Model):
    ...
    _description = (
        "Model with required selection field for an extension with write override"
    )
...
class TestOrmModel_Shared_Cache_Compute_Parent(models.Model):
    ...
    total_amount = fields.Integer(
        compute="_compute_total_amount", store=True, compute_sudo=True
    )
...
class TestOrmComputeMember(models.Model):
    ...
    container_id = fields.Many2one(
        "test_orm.compute.container", compute="_compute_container", store=True
    )
    container_super_id = fields.Many2one(
        "test_orm.compute.container",
        string="Container For SUPERUSER",
    )
    ...
    container_context_name = fields.Char(
        related="container_context_id.name",
        string="Container Context Name",
    )
    container_context_name_translated = fields.Char(
        related="container_context_id.name_translated",
        string="Container Context Name Translated",
    )
    ...
    def _compute_container(self):
        ...
        for member in self:
            member.container_id = container.search(
                [("name", "=", member.name)], limit=1
            )
    ...
    def _compute_container_context_id(self):
        field_name = (
            "container_super_id" if self.env.user._is_superuser() else "container_id"
        )
        for member in self:
...
class TestOrmComputeCreator(models.Model):
    ...
    name = fields.Char()
    created_id = fields.Many2one(
        "test_orm.compute.created", compute="_compute_created", store=True
    )
    ...
    def _compute_created(self):
        ...
        for record in self:
            record.created_id = model.search(
...
class TestOrmModelSome_Access(models.Model):
    ...
    g = fields.Integer(
        default=1,
        groups="base.group_erp_manager,base.group_multi_company,!base.group_portal",
    )
...
class TestOrmCompute_EditableLine(models.Model):
    ...
    def _compute_one_compute(self):
        for rec in self:
            rec.one_compute = float_round(
                99.9999999, precision_rounding=rec.parent_id.precision_rounding
            )
...
class TestOrmShip(models.Model):
    ...
    prisoner_ids = fields.Many2many(
        "test_orm.prisoner", "test_orm_crew", "ship_id", "prisoner_id"
    )
...
class TestOrmPrecomputeCombo(models.Model):
    ...
    editer = fields.Char(
        compute="_compute_editer", precompute=True, store=True, readonly=False
    )
    setter = fields.Char(
        compute="_compute_setter",
        precompute=True,
        inverse="_inverse_setter",
        store=True,
    )
...
class TestOrmPrecomputeEditable(models.Model):
    ...
    foo = fields.Char()
    bar = fields.Char(
        compute="_compute_bar", precompute=True, store=True, readonly=False
    )
    baz = fields.Char(
        compute="_compute_baz", precompute=True, store=True, readonly=False
    )
...
class TestOrmPrecomputeReadonly(models.Model):
    ...
    bar = fields.Char(
        compute="_compute_bar", precompute=True, store=True, readonly=True
    )
    baz = fields.Char(
        compute="_compute_baz", precompute=True, store=True, readonly=False
    )
...
class TestOrmPrecomputeRequired(models.Model):
    ...
    name = fields.Char(
        related="partner_id.name", precompute=True, store=True, required=True
    )
...
class TestOrmPrecomputeMonetary(models.Model):
    ...
    amount = fields.Monetary(compute="_compute_amount", store=True, precompute=True)
    currency_id = fields.Many2one(
        "res.currency",
...
class TestOrmRelated_Translation_2(models.Model):
    ...
    related_id = fields.Many2one(
        "test_orm.related_translation_1", string="Parent Model"
    )
...
class TestOrmRelated_Translation_3(models.Model):
    ...
    related_id = fields.Many2one(
        "test_orm.related_translation_2", string="Parent Model"
    )
    name = fields.Char("Name Related", related="related_id.name", readonly=False)
    html = fields.Html("HTML Related", related="related_id.html", readonly=False)
...
class TestOrmAnyChild(models.Model):
    ...
    parent_id = fields.Many2one(
        "test_orm.any.parent", required=True, ondelete="cascade"
    )
...
class TestOrmCreatePerformance(models.Model):
    ...
    tag_ids = fields.Many2many(
        "test_orm.create.performance.line", "test_create_perf_tag_ids"
    )
