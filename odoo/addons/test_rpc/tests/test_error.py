...
class TestError(common.HttpCase):
    ...
    def test_01_private(self):
        with (
            self.assertRaisesRegex(Exception, r"Private method"),
    ...
    def test_02_delete(self):
        ...
        self.rpc(
            "test_rpc.model_a",
            "create",
            {"name": "A1", "field_b1": b1, "field_b2": b2},
        )
        ...
        self.assertIn(
            "The troublemaker is: 'Model A' (test_rpc.model_a)", e.faultString
        )
        self.assertIn(
            "Thanks to the following constraint: 'required field'",
            e.faultString,
        )
        ...
        self.assertIn(
            "Thanks to the following constraint: 'restricted field'",
            e.faultString,
        )
    ...
    def test_03_sql_constraint(self):
        with (
            mute_logger("odoo.db"),
            self.assertLogs("odoo.http", level="WARNING") as capture,
        ):
            with self.assertRaisesRegex(
                Fault,
                r"The operation cannot be completed: The value must be positive",
            ):
