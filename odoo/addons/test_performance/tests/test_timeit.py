...
class TestPerformanceTimeit(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.Model = cls.env["test_performance.simple.minded"].with_context(
            active_test=False
        )
    ...
    def test_perf_field_set_flush(self):
        ...
        self.launch_perf(
            "records.write({'name': 'ok'}); records.flush_recordset()",
            self.parent_0_child,
        )
    ...
    def test_perf_access_one2many_active_test(self):
        record_list = [p.with_context(active_test=True) for p in self.get_parents()]
        self.launch_perf_set("records.child_ids", record_list=record_list)

    def test_perf_access_iter(self):
        self.launch_perf_set("list(records)", number=100)
