...
from odoo.tools.misc import file_open, file_path, mute_logger
...
class TestImport(common.TransactionCase):
    ...
    def test_import_model_translation(self):
        ...
        record = self.env.ref(
            "test_translation_import.test_translation_import_model1_record2"
        )
        self.assertEqual(record.with_context(lang="fr_FR").name, "Meuble")
        record = self.env.ref(
            "test_translation_import.test_translation_import_model1_record3"
        )
        self.assertEqual(
        ...
        record = self.env.ref(
            "test_translation_import.test_translation_import_model1_record4"
        )
        self.assertEqual(
            record.with_context(lang="fr_FR").name,
    ...
    def test_noupdate(self):
        ...
        self.assertEqual(
            menu.with_context(lang="fr_FR").name,
    ...
    def test_lang_with_base(self):
        ...
        self.assertEqual(record.with_context(lang="fr_BE").name, "Vaisselle, Belgium")
        self.assertEqual(
        ...
        self.assertEqual(record.with_context(lang="fr_CA").name, "Vaisselle")
        self.assertEqual(
    ...
    def test_lazy_translation(self):
        ...
        self.assertEqual(
            "Do you speak " + TRANSLATED_TERM,
            "Do you speak Code Lazy, Klingon",
            "str + _lt concatenation failed",
        )
        self.assertEqual(
            TRANSLATED_TERM + ", I speak it",
            "Code Lazy, Klingon, I speak it",
            "_lt + str concatenation failed",
        )
        self.assertEqual(
            TRANSLATED_TERM + TRANSLATED_TERM,
            "Code Lazy, KlingonCode Lazy, Klingon",
...
class TestTranslationFlow(common.TransactionCase):
    ...
    def test_export_import(self):
        ...
        with io.BytesIO(base64.b64decode(po_file_data)) as po_file:
            ...
            def filter_func_for_python(row):
                return (
                    row.get("value") and PYTHON_TRANSLATION_COMMENT in row["comments"]
                )

            new_code_translations.python_translations[
                ("test_translation_import", "fr_FR")
            ] = CodeTranslations._read_code_translations_file(
                po_file, filter_func_for_python
            )
            ...
            new_code_translations.web_translations[
                ("test_translation_import", "fr_FR")
            ] = {
                "messages": tuple(
                    {"id": src, "string": value}
                    for src, value in CodeTranslations._read_code_translations_file(
                        po_file, filter_func_for_javascript
                    ).items()
                )
            }

        old_python = code_translations.get_python_translations(
            "test_translation_import", "fr_FR"
        )
        new_python = new_code_translations.get_python_translations(
            "test_translation_import", "fr_FR"
        )
        self.assertEqual(
            old_python,
            new_python,
            "python code translations are not exported/imported correctly",
        )

        old_web = code_translations.get_web_translations(
            "test_translation_import", "fr_FR"
        )
        new_web = new_code_translations.get_web_translations(
            "test_translation_import", "fr_FR"
        )
        self.assertEqual(
            old_web,
            new_web,
            "web client code translations are not exported/imported correctly",
        )
        ...
        self.assertFalse(
            any(tran["id"] == "Code Lazy, English" for tran in new_web["messages"]),
        ...
        self.assertEqual(record.with_context(lang="fr_FR").name, "Tableware")
        self.assertEqual(
            record.with_context(lang="fr_FR").xml,
    ...
    def test_export_spreadsheet(self):
        ...
        for line in po_reader:
            ...
            if (
                name
                == "addons/test_translation_import/data/files/test_spreadsheet_dashboard.json"
            ):
                terms.append(source)
        self.assertEqual(
            set(terms),
    ...
    def test_export_spreadsheet_new_dataset(self):
        ...
        for line in po_reader:
            ...
            if (
                name
                == "addons/test_translation_import/data/files/test_spreadsheet_v16_dashboard.json"
            ):
                terms.append(source)
        self.assertEqual(
            set(terms),
    ...
    def test_export_records(self):
        ...
        po_reader = TranslationRecordReader(
            self.env.cr,
        ...
        model2_ids = self.env.ref(
            "test_translation_import.test_translation_import_model2_record1"
        ).ids
        po_reader = TranslationRecordReader(
            self.env.cr,
