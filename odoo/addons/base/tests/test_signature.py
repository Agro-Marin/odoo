...
class TestSignature(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.private_key = rsa.generate_private_key(public_exponent=65537, key_size=4096)

        cert_subject = x509.Name(
            [
                x509.NameAttribute(NameOID.COUNTRY_NAME, "BE"),
                x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, "Brabant Wallon"),
    ...
    def test_odoo_pdf_signer(self):
        ...
        with file_open(self.pdf_path, "rb") as stream:
            ...
            byte_range = (
                pdf_data[start_bracket_index + 1 : end_bracket_index]
                .strip()
                .split(b" ")
            )
            ...
            for i in range(0, len(byte_range), 2):
                hash.update(
                    pdf_data[
                        int(byte_range[i]) : int(byte_range[i]) + int(byte_range[i + 1])
                    ]
                )
                    ...
                    cms.CMSAttribute(
                        {
                            "type": "signing_time",
                            "values": [
                                cms.Time({"utc_time": core.UTCTime(fixed_time)})
                            ],
                        }
                    ),
                    cms.CMSAttribute(
