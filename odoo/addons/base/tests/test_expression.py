    ...
    def _search(self, model, domain, init_domain=Domain.TRUE, test_complement=True):
        ...
        if test_complement and domain:
            ...
            if not init_domain.is_true():
                ...
                uni = model.with_context(active_test=False).search(
                    Domain.TRUE, order="id"
                )
                self.assertEqual(
                    sorted(sql._ids + cpl._ids),
                    uni.ids,
            ...
            self.assertEqual(
                sorted(sql._ids + cpl._ids),
                uni.ids,
                f"{domain} and {complement_domain} don't cover all records",
            )
        return sql
...
class TestExpression(SavepointCaseWithUserDemo, TransactionExpressionCase):
    ...
    def test_00_in_not_in_m2m(self):
        ...
        self.assertLessEqual(
            a + ab + c,
            with_categ_a_none,
            "search for all having cat_a or no categories (1)",
        )
        self.assertNotIn(
            b,
            with_categ_a_none,
            "search for all having cat_a or no categories (2)",
        )
    ...
    def test_05_not_str_m2m(self):
        ...
        def test(op, value, expected):
            ...
            self.assertItemsEqual(
                found_ids,
                expected_ids,
                "%s %r should return %r" % (op, value, expected),
            )
    ...
    def test_09_hierarchy_filtered_domain(self):
        ...
        p3 = self._search(
            Partner, [("parent_id", "parent_of", p.id)], [("id", "=", p.id)]
        )
    ...
    def test_10_equivalent_id(self):
        ...
        res_1 = self._search(
            Currency, [("name", "not like", "probably_unexisting_name")]
        )
    ...
    def test_15_m2o(self):
        ...
        res_partners = self._search(
            Partner,
            ["|", ("company_id", "not in", []), ("company_id", "=", False)],
        )
    ...
    def test_15_equivalent_one2many_1(self):
        ...
        self.assertEqual(
            set(all_ids) - {p1},
            set(res.ids),
            "o2m NOT IN matches none on the right side",
        )
        res = self._search(Partner, [("user_ids", "!=", "Dédé Boitaclou")])
        self.assertEqual(
            set(all_ids) - {p1},
    ...
    def test_15_equivalent_one2many_2(self):
        ...
        currency_rate2 = self._search(
            CurrencyRate, [("id", "not in", [non_currency_id])]
        )
    ...
    def test_15_hierarchy_ilike(self):
        ...
        self.assertEqual(
            companies,
            self._search(Company, base_domain & Domain("id", "child_of", "Parent")),
        )
        self.assertEqual(
            companies,
            self._search(Company, base_domain & Domain("id", "parent_of", "Chi")),
        )
    ...
    def test_20_expression_parse(self):
        ...
        users = self._search(
            Users,
            [("name", "like", "test"), ("parent_id", "=?", b1.partner_id.id)],
        )
    ...
    def test_30_instantiate_domain(self):
        ...
        self.assertIsInstance(
            simple, Domain, "Domain constructor must be instance of Domain"
        )
        simple_list = [("foo", "=", "bar")]
        ...
        self.assertEqual(
            list(neg_domain),
            ["!", ("foo.x", ">", "bar")],
            "Internal test that we are inversing a domain",
        )
        ...
        for dom, is_bool in [
            ...
            self.assertEqual(
                bool(dom),
                dom != Domain.TRUE,
                "Only TRUE is falsy because it's equivalent to empty domain",
            )
    ...
    def test_32_iter_conditions(self):
        ...
        self.assertEqual(
            list((simple & Domain("x", "=", "y")).iter_conditions()),
            [simple, Domain("x", "=", "y")],
        )
    ...
    def test_33_map_conditions(self):
        ...
        self.assertEqual(
            ...
            condition_a1,
            "simple replacement, no match",
        )
        self.assertEqual(
            Domain.TRUE.map_conditions(replace(Domain.TRUE, Domain.FALSE)),
            Domain.TRUE,
            "Constant predicates are not conditions, they are not replaced",
        )
        ...
        self.assertEqual(
            (and_domain | condition_c3).map_conditions(
                replace(condition_b2, condition_c3)
            ),
            (condition_a1 & condition_c3) | condition_c3,
            "replace inside different nary conditions",
        )

        self.assertEqual(
            Domain("foo", "any", condition_a1).map_conditions(
                replace(condition_a1, condition_b2)
            ),
            Domain("foo", "any", condition_a1),
            "We don't follow the 'any' operator",
        )
    ...
    def test_35_negating_thruty_leafs(self):
        ...
        self.assertEqual(
            Domain(["!", "!", _TRUE_LEAF]),
            Domain.TRUE,
            "distribute_not applied wrongly",
        )
        self.assertEqual(
            Domain(["!", "!", _FALSE_LEAF]),
            Domain.FALSE,
    ...
    def test_40_negating_long_expression(self):
        ...
        self.assertEqual(
            source, expect, "distribute_not on long expression applied wrongly"
        )
    ...
    def test_like_wildcards(self):
        ...
        self.assertTrue(
            all(partner.name == "Inner Works" for partner in partners),
            "Must match only 'Inner Works'",
        )
        ...
        self.assertTrue(
            len(countries) == 2,
            "Must match only countries with names starting with Z (currently 2)",
        )
    ...
    def test_invalid(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"^Invalid operator.*\('create_date', '>>', 'foo'\)$"
        ):
    ...
    def test_active(self):
        ...
        self.assertTrue(
            partner, "Record not Found with country Belgium and active False."
        )
    ...
    def test_lp1071710(self):
        ...
        self.assertNotEqual(
            be.name,
            "Belgique",
            "Setting a translation should not impact other languages",
        )
    ...
    def test_negate_conditions(self):
        ...
        self.assertEqual(
            next(iter(cond3_neg)),
            "!",
            "The negative condition should not be distributed",
        )
    ...
    def test_filtered_domain_any_operator(self):
        ...
        children_partner_1 = self._search(
            Partner, [("parent_id", "any", [("name", "=", partner.name)])]
        )
        self.assertEqual(children_partner_1, partner.child_ids)

        children_other_partners = self._search(
            Partner, [("parent_id", "not any", [("name", "=", partner.name)])]
        )
        self.assertEqual(children_other_partners, all_partner - partner.child_ids)

        one_child_partner = partner.child_ids[0]
        parent_partner = self._search(
            Partner,
            [("child_ids", "any", [("name", "=", one_child_partner.name)])],
        )
        self.assertEqual(parent_partner, partner)

        other_partners = self._search(
            Partner,
            [("child_ids", "not any", [("name", "=", one_child_partner.name)])],
        )
        ...
        with patch.object(
            ...
            "filtered_domain",
            autospec=True,
            side_effect=PartnerClass.filtered_domain,
        ) as patched:
            all_partner.filtered_domain(
                [("child_ids", "any", [("name", "=", partner.name)])]
            )
            self.assertEqual(
                patched.call_count,
                2,
                "should be called here and only once for the any operator",
            )
    ...
    def test_filtered_domain_any_bypass_access(self):
        ...
        self.assertEqual(
            partner.filtered_domain(domain),
            partner,
            "We should be able to check inaccessible child name",
        )
        Partner.invalidate_model()
        self.assertFalse(
            partner.filtered_domain(Domain("child_ids", "any", domain.value)),
            "We should not find the record anymore (child is inaccessible)",
        )
        ...
        domain = Domain(
            ...
            "any!",
            Domain("child_ids", "any", domain.value),
        )
        self.assertFalse(
            partner.filtered_domain(domain),
            "Filtering the child and getting the parent without access should return no records",
        )
        Partner.invalidate_model()
        domain = Domain(
            "child_ids",
            "any!",
            Domain("parent_id", "any", Domain("child_ids", "any!", domain.value)),
        )
        self.assertFalse(
            partner.filtered_domain(domain),
            "We should not find the record anymore (parent_id is inaccessible)",
        )
...
class TestExpression2(TransactionExpressionCase):
    def test_long_table_alias(self):
        ...
        self.patch(
            ...
            "_order",
            "commercial_partner_id,company_id,name",
        )
...
class TestBypassAccess(TransactionExpressionCase):
    def test_bypass_search_access(self):
        ...
        self.assertIn(
            p_a, partners, "bypass_search_access: disjunction incorrect result"
        )
        self.assertIn(
        ...
        patch_domain(
            partner_obj,
            "child_ids",
            lambda self: [("name", "=", "__%s" % self._name)],
        )
        partners = self._search(
            partner_obj,
            [
    ...
    def test_access_rules_active_test(self):
        ...
        accessible_records = PartnerCateg.create(
            [
                ...
                {
                    "name": "a3",
                    "parent_id": pub_active.id,
                    "child_ids": [Command.create({"name": "not PRI"})],
                },
            ]
        )
        inaccessible_records = PartnerCateg.create(
            [
                ...
                {"name": "ua2", "parent_id": pri_active.id},
                {
                    "name": "ua3",
                    "parent_id": pub_active.id,
                    "child_ids": [Command.link(pri_active.id)],
                },
                {
                    "name": "ua4",
                    "parent_id": pub_active.id,
                    "child_ids": [Command.link(pri_inactive.id)],
                },
            ]
        )
...
class TestOne2many(TransactionCase):
    ...
    def testbypass_search_access_mixed(self):
        ...
        self.patch(
            self.Partner.state_id._fields["country_id"],
...
class TestAnyfy(TransactionCase):
    ...
    def test_true_leaf_as_list(self):
        self._test_combine_anies([[1, "=", 1]], [(1, "=", 1)])

    def test_single_field(self):
        self._test_combine_anies([("name", "=", "Jack")], [("name", "=", "Jack")])
