...
class TestDateUtils(TransactionCase):
    ...
    def test_fiscal_year(self):
        self.assertEqual(
            get_fiscal_year(date(2024, 12, 31)),
            (date(2024, 1, 1), date(2024, 12, 31)),
        )
        self.assertEqual(
            get_fiscal_year(date(2024, 12, 31), 30, 11),
            (date(2024, 12, 1), date(2025, 11, 30)),
        )
        self.assertEqual(
            get_fiscal_year(date(2024, 10, 31), 30, 11),
            (date(2023, 12, 1), date(2024, 11, 30)),
        )
        self.assertEqual(
            get_fiscal_year(date(2024, 10, 31), 30, 12),
            (date(2023, 12, 31), date(2024, 12, 30)),
        )

        self.assertEqual(
            get_fiscal_year(date(2024, 10, 31), month=11),
            (date(2023, 12, 1), date(2024, 11, 30)),
        )
        self.assertEqual(
            get_fiscal_year(date(2024, 2, 29)),
            (date(2024, 1, 1), date(2024, 12, 31)),
        )

        self.assertEqual(
            get_fiscal_year(date(2024, 12, 31), 29, 2),
            (date(2024, 3, 1), date(2025, 2, 28)),
        )
        self.assertEqual(
            get_fiscal_year(date(2024, 12, 31), 28, 2),
            (date(2024, 3, 1), date(2025, 2, 28)),
        )
        self.assertEqual(
            get_fiscal_year(date(2023, 12, 31), 28, 2),
            (date(2023, 3, 1), date(2024, 2, 29)),
        )
        self.assertEqual(
            get_fiscal_year(date(2023, 12, 31), 29, 2),
            (date(2023, 3, 1), date(2024, 2, 29)),
        )

        self.assertEqual(
            get_fiscal_year(date(2024, 2, 29), 28, 2),
            (date(2023, 3, 1), date(2024, 2, 29)),
        )
        self.assertEqual(
            get_fiscal_year(date(2023, 2, 28), 28, 2),
            (date(2022, 3, 1), date(2023, 2, 28)),
        )
        self.assertEqual(
            get_fiscal_year(date(2023, 2, 28), 29, 2),
            (date(2022, 3, 1), date(2023, 2, 28)),
        )
    ...
    def test_parse_iso_date(self):
        ...
        self.assertEqual(
            parse_iso_date("2024-01-05 00:30:00"), datetime(2024, 1, 5, 0, 30)
        )
    ...
    def test_parse_date_relative_utc(self):
        self.env["res.lang"]._get_lang_cached(self.env.user.lang).week_start = "1"
        ...
        self.env["res.lang"]._get_lang_cached(self.env.user.lang).week_start = "6"
        ...
        self.env["res.lang"]._get_lang_cached(self.env.user.lang).week_start = "5"
...
class TestDateRangeFunction(BaseCase):
    ...
    def test_date_range_with_timezone_aware_datetimes_other_than_utc(self):
        ...
        end = datetime(1986, 1, 1, tzinfo=tz)
        ...
        expected = [e.replace(tzinfo=tz) for e in expected]
    ...
    def test_date_range_with_mismatching_zones(self):
        ...
        start = datetime(1985, 1, 1, tzinfo=start_tz)
        end = datetime(1986, 1, 1, tzinfo=end_tz)
