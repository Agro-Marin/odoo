...
class TestSafeEval(BaseCase):
    ...
    def test_02_literal_eval(self):
        ...
        self.assertEqual(
            actual,
            expected,
            "Simple python expressions are not working with literal_eval",
        )
...
class TestParentStore(TransactionCase):
    ...
    def test_duplicate_parent(self):
        ...
        self.assertEqual(
            len(new_struct),
            4,
            "After duplication, the new object must have the childs records",
        )
        ...
        self.assertFalse(
            ...
            "After duplication, nodes should not be mixed",
        )
    ...
    def test_duplicate_children_02(self):
        ...
        new_cat2 = self.cat2.copy()
        new_cat0 = self.cat0.copy(
            {"child_ids": [Command.set((new_cat1 + new_cat2).ids)]}
        )
        new_struct = new_cat0.search([("parent_id", "child_of", new_cat0.id)])
        self.assertEqual(
            len(new_struct),
    ...
    def test_duplicate_children_03(self):
        ...
        new_cat0.write(
            {
                "child_ids": [
                    Command.link(new_cat1.id),
                    Command.link(new_cat2.id),
                ]
            }
        )
        new_struct = new_cat0.search([("parent_id", "child_of", new_cat0.id)])
...
class TestGroups(TransactionCase):
    def test_res_groups_fullname_search(self):
        ...
        groups = all_groups.search([("get_full_name", "like", "Sale")])
        self.assertItemsEqual(
            groups.ids,
            [g.id for g in all_groups if "Sale" in g.full_name],
            "did not match search for 'Sale'",
        )

        groups = all_groups.search([("get_full_name", "like", "Master Data")])
        self.assertItemsEqual(
            groups.ids,
            [g.id for g in all_groups if "Master Data" in g.full_name],
            "did not match search for 'Master Data'",
        )

        groups = all_groups.search([("get_full_name", "like", "Monkey/Banana")])
        self.assertItemsEqual(
            groups.mapped("get_full_name"),
            ["Monkey / Banana"],
            "did not match search for 'Monkey/Banana'",
        )

        groups = all_groups.search([("get_full_name", "like", "Monkey /")])
        self.assertItemsEqual(
            groups.ids,
            [g.id for g in all_groups if "Monkey /" in g.full_name],
            "did not match search for 'Monkey /'",
        )

        groups = all_groups.search([("get_full_name", "like", "Monk /")])
        self.assertItemsEqual(
            groups.ids,
            [g.id for g in all_groups if "Monkey /" in g.full_name],
        ...
        self.assertItemsEqual(
            groups.ids,
            [g.id for g in all_groups if "Monk" in g.full_name],
            "did not match search for 'Monk'",
        )

        groups = all_groups.search([("get_full_name", "in", ["Creation"])])
        self.assertItemsEqual(groups.mapped("get_full_name"), ["Contact / Creation"])

        groups = all_groups.search(
            [("get_full_name", "in", ["Role / Administrator", "Creation"])]
        )
        self.assertItemsEqual(
            groups.mapped("get_full_name"),
            ["Contact / Creation", "Role / Administrator"],
        )

        groups = all_groups.search([("get_full_name", "like", "Admin")])
        self.assertItemsEqual(
            groups.mapped("get_full_name"),
            [g.full_name for g in all_groups if "Admin" in g.full_name],
        )

        groups = all_groups.search([("get_full_name", "not like", "Role /")])
        self.assertItemsEqual(
            groups.mapped("get_full_name"),
            [g.full_name for g in all_groups if "Role /" not in g.full_name],
        )

        groups = all_groups.search([("get_full_name", "=", False)])
        self.assertFalse(groups)

        groups = all_groups.search([("get_full_name", "!=", False)])
        self.assertEqual(groups, all_groups)

        groups = all_groups.search([("get_full_name", "like", "/")])
    ...
    def test_remove_groups(self):
        ...
        p = self.env["res.users"].create(
            {"login": "p", "name": "P", "group_ids": [Command.set([portal.id])]}
        )

        a = self.env["res.groups"].create(
            {"name": "A", "user_ids": [Command.set(u1.ids)]}
        )
        b = self.env["res.groups"].create(
            {"name": "B", "user_ids": [Command.set(u1.ids)]}
        )
        c = self.env["res.groups"].create(
            {
                "name": "C",
                "implied_ids": [Command.set(a.ids)],
                "user_ids": [Command.set([p.id, u2.id])],
            }
        )
        d = self.env["res.groups"].create(
            {
        ...
        def assertUsersEqual(users, group):
            self.assertEqual(
                ...
                sorted(
                    group.with_context(active_test=False).mapped("all_user_ids.login")
                ),
            )
