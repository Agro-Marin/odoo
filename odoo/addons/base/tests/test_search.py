...
class test_search(TransactionCase):
    ...
    def test_00_search_order(self):
        ...
        self.assertEqual(
            [a, ab, b, c],
            list(name_asc),
            "Search with 'NAME ASC' order failed.",
        )
        name_desc = Partner.search(
            [("name", "like", "test_search_order%")], order="name desc"
        )
        self.assertEqual(
            [c, b, ab, a],
            list(name_desc),
            "Search with 'NAME DESC' order failed.",
        )
        id_asc = Partner.search(
            [("name", "like", "test_search_order%")], order="id asc"
        )
        self.assertEqual(
            [c, a, b, ab], list(id_asc), "Search with 'ID ASC' order failed."
        )
        id_desc = Partner.search(
        ...
        self.assertEqual(
            [d, e, c, a, b, ab],
            list(active_asc_id_asc),
            "Search with 'ACTIVE ASC, ID ASC' order failed.",
        )
        active_desc_id_asc = Partner.search(
            [
                ("name", "like", "test_search_order%"),
                "|",
                ("active", "=", True),
        ...
        self.assertEqual(
            [c, a, b, ab, d, e],
            list(active_desc_id_asc),
            "Search with 'ACTIVE DESC, ID ASC' order failed.",
        )
        active_asc_id_desc = Partner.search(
            [
                ("name", "like", "test_search_order%"),
                "|",
                ("active", "=", True),
        ...
        self.assertEqual(
            [e, d, ab, b, a, c],
            list(active_asc_id_desc),
            "Search with 'ACTIVE ASC, ID DESC' order failed.",
        )
        active_desc_id_desc = Partner.search(
            [
                ("name", "like", "test_search_order%"),
                "|",
                ("active", "=", True),
        ...
        self.assertEqual(
            [ab, b, a, c, e, d],
            list(active_desc_id_desc),
            "Search with 'ACTIVE DESC, ID DESC' order failed.",
        )
        id_asc_active_asc = Partner.search(
            [
                ("name", "like", "test_search_order%"),
                "|",
                ("active", "=", True),
            ...
            order="id asc, active asc",
        )
        self.assertEqual(
            [c, d, a, b, ab, e],
            list(id_asc_active_asc),
            "Search with 'ID ASC, ACTIVE ASC' order failed.",
        )
        id_asc_active_desc = Partner.search(
            [
                ("name", "like", "test_search_order%"),
                "|",
                ("active", "=", True),
            ...
            order="id asc, active desc",
        )
        self.assertEqual(
            [c, d, a, b, ab, e],
            list(id_asc_active_desc),
            "Search with 'ID ASC, ACTIVE DESC' order failed.",
        )
        id_desc_active_asc = Partner.search(
            [
                ("name", "like", "test_search_order%"),
                "|",
                ("active", "=", True),
            ...
            order="id desc, active asc",
        )
        self.assertEqual(
            [e, ab, b, a, d, c],
            list(id_desc_active_asc),
            "Search with 'ID DESC, ACTIVE ASC' order failed.",
        )
        id_desc_active_desc = Partner.search(
            [
                ("name", "like", "test_search_order%"),
                "|",
                ("active", "=", True),
            ...
            order="id desc, active desc",
        )
        self.assertEqual(
            [e, ab, b, a, d, c],
            list(id_desc_active_desc),
            "Search with 'ID DESC, ACTIVE DESC' order failed.",
        )
        ...
        a.country_id, c.country_id = self.env["res.country"].create(
            [
                ...
                {"name": "Country 2", "code": "C2", "phone_code": "02"},
            ]
        )
    ...
    def test_10_inherits_m2order(self):
        ...
        u = Users.create(
            {
                "name": "__search",
                "login": "__search",
                "group_ids": [Command.set([group_employee.id])],
            }
        )
        a = Users.create(
            {
        ...
        user_ids = Users.search(
            [("id", "in", expected_ids)],
            order="state_id asc, country_id desc, name asc, login desc",
        ).ids
        self.assertEqual(
            user_ids,
            expected_ids,
        ...
        user_ids = Users.search(
            [("id", "in", expected_ids)],
            order="country_id desc, state_id desc, name asc, login desc",
        ).ids
        self.assertEqual(
            user_ids,
            expected_ids,
    ...
    def test_20_x_active(self):
        ...
        country_ussr = model_country.create(
            {"name": "USSR", "x_active": False, "code": "ZV"}
        )
        ussr_search = model_country.search([("name", "=", "USSR")])
        self.assertFalse(ussr_search)
        ussr_search = model_country.with_context(active_test=False).search(
            [("name", "=", "USSR")]
        )
        self.assertIn(
            country_ussr,
        ...
        ussr_search = model_country.search(
            [("name", "=", "USSR"), ("x_active", "=", False)]
        )
        self.assertIn(
            country_ussr,
            ussr_search,
        ...
        bank_credit_communal = model_bank.create(
            {"name": "Crédit Communal", "x_active": False, "active": True}
        )
        cc_search = model_bank.search([("name", "=", "Crédit Communal")])
        self.assertIn(
            bank_credit_communal,
            cc_search,
            "Search for active record with x_active set to False has failed",
        )
        ...
        self.assertNotIn(
            bank_credit_communal,
            cc_search,
            "Search for inactive record with x_active set to True has failed",
        )
