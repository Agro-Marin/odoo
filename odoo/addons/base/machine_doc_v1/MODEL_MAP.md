...
JSON args) executed asynchronously by the job workers (`WorkerJob` in a
prefork server, a `JOB_LISTENER` thread in a threaded one, LISTEN/NOTIFY on
channel `job_queue`), each in its own
