...
In the graphic below you can see a summary of the call of the methods performed
in the IrQweb class.
...
the expression is evaluated as truthy.
...
# an ``xml`` fragment can itself contain the old ``' , '`` separator (e.g.
# ``t-att-title="a , b"``), which silently corrupted the parsed path/html.
# Emitter and parser MUST stay in sync — keep them both here.
...
# The generated ``code`` stored in ``__qweb_loaded_codes`` is executed wrapped in
# a one-line ``def generate_functions():`` preamble (see
# ``_generate_code_uncached``), so a traceback reports line numbers offset by
# that preamble relative to the stored code. The error helpers index the stored
# code, so ``_error_line_number`` subtracts this offset to realign them. If the
# wrapper preamble ever grows, update this constant (the error-surrounding tests
# pin the alignment end-to-end).
...
class IrQweb(models.AbstractModel):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
        ...
        try:
            while stack:
                ...
                for item in frame.iterator:
                    ...
                    # Fetch the compiled function and template options. Both were
                    # previously obtained by re-entering `_compile` on every
                    # frame — for `t-call` frames (params.method is None) to get
                    # the function, and for already-loaded content frames just to
                    # re-read `options`. `_compile` is ormcached for DB templates,
                    # but re-entry still costs a cache-key computation per frame
                    # (and t-call defeated the `loaded_functions` fast-path
                    # entirely). Memoize the `(functions, def_name, options)`
                    # triple for this render, keyed by the template ref AND the
                    # compile cache-key signature — a `t-call` + `t-lang`
                    # legitimately recompiles the same ref under a different
    ...
    def _wrap_render_error(
        ...
        """Convert an exception raised while rendering into a ``QWebError`` (or
        annotate the existing one), then re-raise. This method never returns
        normally — it always propagates an exception.
        """
    ...
    def _error_surrounding(
        ...
        """Build the dev-mode code snippet that frames the error line with its
        preceding and following context.

        ``line_nb`` is already realigned to the stored code by
        ``_error_line_number``, so ``code_lines[line_nb - 1]`` is the true
        culprit for every directive — no per-directive nudging needed (a
        ``t-if``-only ``line_nb -= 1`` band-aid previously papered over the
        wrapped/unwrapped off-by-one for that one case)."""
    ...
    def _generate_code(
        ...
        #
        # It starts as a copy of ``self.env.context`` and is threaded (by
        # reference) through every ``_compile_*`` method. The QWeb-specific keys
        # written below and their owners:
    ...
    def _prepare_globals(self) -> dict[str, Any]:
        ...
        Single leading underscore (not the historical name-mangled ``__``): the
        method is a normal protected hook. Name mangling gave no real protection
        (the eval sandbox is enforced by ``__builtins__`` and opcode validation,
        not by hiding this method) and only made it awkward to override or
        reference (tests had to reach for ``_IrQweb__prepare_globals``).
        """
        return {
            "__name__": __name__,
            "Sized": Sized,
    ...
    def _compile_directive_foreach(
        ...
        * ``t-foreach`` compiled value can be an iterable, a dictionary or a
        ...
        # Wrap the fallback in a list, consistent with the ``if``/``groups``
        # directives: ``indent_code`` returns a str, so a bare
        # ``code.extend(str)`` would splice the string character-by-character
        # into ``code`` and emit broken Python. Harmless today only because
        # ``_compile_directive_att`` always emits at least one line (so
        # ``content_foreach`` is never empty), but the invariant lives in
        # another method — don't rely on it here.
