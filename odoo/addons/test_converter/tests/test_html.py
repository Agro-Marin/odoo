...
class TestFloatExport(TestBasicExport):
    ...
    def test_float(self):
        ...
        self.assertEqual(value, "-\N{ZERO WIDTH NO-BREAK SPACE}42.0")
...
class TestCurrencyExport(TestExport):
    ...
    def test_currency_post(self):
        ...
        self.assertEqual(
            converted,
            '<span class="oe_currency_value">-\N{ZERO WIDTH NO-BREAK SPACE}0.12</span>'
            f"\N{NO-BREAK SPACE}{currency.symbol}",
        )
    ...
    def test_currency_pre(self):
        ...
        self.assertEqual(
            converted,
            f"{currency.symbol}\N{NO-BREAK SPACE}"
...
class TestBinaryExport(TestBasicExport):
    def test_image(self):
        ...
        with file_open(str(Path(directory, "test_vectors", "image")), "rb") as f:
            content = f.read()
        ...
        self.assertEqual(
            value,
            '<img src="data:image/jpeg;base64,%s">' % encoded_content.decode("ascii"),
        )

        with file_open(str(Path(directory, "test_vectors", "pdf")), "rb") as f:
            content = f.read()

        with self.assertRaises(ValueError):
            converter.value_to_html(base64.b64encode(content), {})

        with file_open(str(Path(directory, "test_vectors", "pptx")), "rb") as f:
            content = f.read()
...
class TestDurationExport(TestBasicExport):
    ...
    def test_negative_digital(self):
        ...
        result = converter(
            -90,
            {"unit": "minute", "round": "minute", "digital": True},
            {"lang": "fr_FR"},
        )
...
class TestRelativeDatetime(TestBasicExport):
    ...
    def test_basic(self):
        ...
        t = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=1)
