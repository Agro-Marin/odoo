...
`test_owl3_api.py` counts, in every addon's `static/src` outside the vendored trees and with JS comments stripped, the bare calls of the five OWL 2 APIs that OWL 3 removes: `owl_on_rendered` (`onRendered(`), `owl_on_will_render` (`onWillRender(`), `owl_this_render` (`this.render(`), `owl_use_component` (`useComponent(`) and `owl_env_services` (`this.env.services`, which a component takes in setup through `useService` instead). A method that only shares the name, such as `props.comp.onRendered`, is not counted. Each is an exact ratchet whose floor is in `floors.json`, lowered as the OWL 3 migration removes the sites. The same file ratchets the component reads of `this.env` that OWL 3 drops with the env itself, JS and static XML templates together (XML comments stripped): `owl_env_is_small` (`this.env.isSmall`, read as `this.ui.isSmall`), `owl_env_model`, `owl_env_search_model`, `owl_env_config`, `owl_env_bus`, `owl_env_debug`, `owl_env_utils` (each read through its owner's accessor -- `useViewModel`, `useSearchModel`, `useViewConfig`, `useEventBus`, `useDebugMode`, the POS utils service), the hard zeros `owl_env_mail_context`, `owl_env_builder_context`, `owl_env_dialog_context` and `owl_env_addon_contexts` (the grouped context accessors), and `owl_env_reads`, every `this.env` left in `static/src` -- services, stores and models that keep an env of their own, and the spreadsheet addons' o_spreadsheet reads.
