...
`mixin.band`, and `lint_link_token_compare`, the bearer tokens checked outside
`access.link` -- the AST rules with a floor. `access_check_override` counts
...
| `_checker_link_token.py` | `link-token-compare` |
...
owners: base defines `user`, `none`, `public`, `bearer` and `link` (a record opened
by an `access.link` token), integration defines
...
`link-token-compare` (E8537) counts a bearer token compared (`consteq`, `compare_digest`) or searched for (`('access_token', '=', t)`) anywhere but `access.link`'s resolver (`odoo/addons/base/models/access_link.py`), outside tests. A token checked in its own controller is a capability nobody can list, expire or revoke; P7 moves each family onto `access.link` and the floor, `lint_link_token_compare` (56 when the rule landed, 54 once documents moved), comes down with it, to zero. A vendor's or a device's credential (`api_`, `oauth_`, `push_`, `csrf_` tokens) is a receiver's, not a link's, and is not counted.
...
`test_owl3_api.py` counts, in every addon's `static/src` outside the vendored trees and with JS comments stripped, the bare calls of the six OWL 2 APIs that OWL 3 removes or redefines: `owl_on_rendered` (`onRendered(`), `owl_on_will_render` (`onWillRender(`), `owl_this_render` (`this.render(`), `owl_use_component` (`useComponent(`), `owl_use_effect` (`useEffect(`, whose OWL 3 form drops the dependency array; a component calls `useLayoutEffect` from `@web/core/utils/layout_effect`, OWL 2's implementation over `onMounted` / `onPatched`) and `owl_env_services` (`this.env.services`, which a component takes in setup through `useService` instead). A method that only shares the name, such as `props.comp.onRendered`, is not counted. Beside them it counts the `this.env` keys a component will no longer have, in the JS and, with XML comments stripped, in the static templates: `owl_env_bus`, `owl_env_config`, `owl_env_debug`, `owl_env_is_small`, `owl_env_model`, `owl_env_search_model`, `owl_env_utils`, and the context families `owl_env_dialog_context`, `owl_env_mail_context`, `owl_env_builder_context` and `owl_env_addon_contexts`; each gate's failure message names the hook that replaces the read. `owl_env_reads` counts every `this.env` left in `static/src` -- services, stores and models that keep an env of their own, and the spreadsheet addons' o_spreadsheet reads. `owl_sub_env_raw` counts the `useSubEnv(` / `useChildSubEnv(` calls outside a top-level `provide*` function, so a scope hands a value down only through the `provide*` hook paired with the accessor its readers call. `owl_use_env_raw` counts the `useEnv()` calls outside a top-level `use*` / `provide*` accessor whose body only returns it -- the hooks still reading an env of their own, and those handing the whole env to a model, a dialog or a service. `owl_patched_props` counts a component's `props` or `defaultProps` rewritten after its class is defined -- `X.props = ...`, `Object.assign(X.props, ...)` and a `props` / `defaultProps` key of `patch(X, {...})` -- because OWL 3 reads props from the schema a component passes to `useProps`; the owner exports that object and a patch extends it in place. Every one is an exact ratchet per repository, as the elevation gates are, keyed `<gate>_<repository>` in `floors.json` with the repository from `lint_case.repo_of`: `owl_on_rendered_odoo`, `owl_on_will_render_odoo`, `owl_on_will_render_enterprise`, `owl_this_render_odoo`, `owl_this_render_enterprise`, `owl_use_component_odoo`, `owl_env_services_odoo`, `owl_env_services_enterprise`, `owl_env_bus_odoo`, `owl_env_config_odoo`, `owl_env_config_enterprise`, `owl_env_debug_odoo`, `owl_env_debug_enterprise`, `owl_env_is_small_odoo`, `owl_env_is_small_enterprise`, `owl_env_model_odoo`, `owl_env_model_enterprise`, `owl_env_reads_odoo`, `owl_env_reads_enterprise`, `owl_env_search_model_odoo`, `owl_env_search_model_enterprise`, `owl_env_utils_odoo`, `owl_env_utils_enterprise`, `owl_use_env_raw_odoo` and `owl_use_env_raw_enterprise`. A repository with no key is a hard zero, which is every context family, `owl_sub_env_raw`, `owl_patched_props` and `owl_use_effect` everywhere and every gate in agromarin and design-themes. A narrow addons path reads only the odoo keys, so the enterprise ones are checked where test_lint runs with enterprise on the path. The floors go down as the OWL 3 migration removes the sites.

The same file counts the other ambient reads a component makes of `this.env`, each an exact ratchet lowered as components take the value through a hook instead: `owl_env_bus` (`this.env.bus`), `owl_env_config` (`this.env.config`), `owl_env_debug` (`this.env.debug`), `owl_env_is_small` (`this.env.isSmall`), `owl_env_model` (`this.env.model`), `owl_env_search_model` (`this.env.searchModel`) and `owl_env_utils` (`this.env.utils`); `owl_env_dialog_context`, `owl_env_mail_context`, `owl_env_builder_context` and `owl_env_addon_contexts` hold at zero the reads their provider hooks replaced.
