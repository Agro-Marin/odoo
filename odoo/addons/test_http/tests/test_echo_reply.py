...
from odoo.http import SESSION_ROTATION_INTERVAL, Request, root
...
class TestHttpEchoReplyHttpNoDB(TestHttpBase):
    ...
    def test_echohttp3_post_qs_form_nodb(self):
        res = self.nodb_url_open(
            "/test_http/echo-http-post?race=Asgard", data={"commander": "Thor"}
        )
        self.assertEqual(res.status_code, 200)
    ...
    def test_echohttp4_post_json_nodb(self):
        ...
        res = self.nodb_url_open(
            "/test_http/echo-http-post", data=payload, headers=CT_JSON
        )
        self.assertEqual(res.status_code, 200)
    ...
    def test_echohttp5_post_csrf(self):
        res = self.nodb_url_open(
            "/test_http/echo-http-csrf?race=Asgard", data={"commander": "Thor"}
        )
...
class TestHttpEchoReplyJsonNoDB(TestHttpBase):
    ...
    def test_echojson2_http_post_nodb(self):
        ...
        self.assertEqual(
            res.headers.get("Accept"), "application/json, application/json-rpc"
        )
