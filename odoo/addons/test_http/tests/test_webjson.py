...
class TestHttpWebJson_1(TestHttpBase):
    ...
    def test_webjson_bad_stuff(self):
        ...
        with self.subTest(bad="action"):
            ...
            self.assertIn("expected action at word 1 but found “idontexist”", res.text)
        ...
        with self.subTest(bad="view_type"):
            ...
            res = self.url_open_json(
                "/res.users?view_type=idontexist", expected_code=400
            )
            self.assertEqual(res.headers["Content-Type"], CT_HTML)
    ...
    def test_webjson_form_subtree(self):
        ...
        self.assertEqual(
            ...
            self.milky_way.with_env(env).web_read(
                {
                    "name": {},
                    "stargate_ids": {"fields": {"name": {}, "sgc_designation": {}}},
                }
            )[0],
        )
    ...
    def test_webjson_list_limit_offset(self):
        ...
        self.assertEqual(
            res_limit.json(),
            {"length": len(stargates), "records": stargates[:1]},
        )
        ...
        self.assertEqual(
            res_limit_offset.json(),
            {"length": len(stargates), "records": stargates[1:2]},
        )
    ...
    def test_webjson_list_domain_default_filter(self):
        ...
        self.assertNotIn(
            action_domain[0][0],
            res.url,
            "URL should not contain a domain with action domain",
        )
    ...
    def test_webjson_list_args(self):
        ...
        self.assertEqual(
            parse_qs(urlsplit(res.url).query),
            {
                "domain": ["[('name', 'ilike', 'earth')]"],
                "offset": ["0"],
                "limit": ["80"],
    ...
    def test_webjson_activity(self):
        env = self.authenticate_demo()
        env["test_http.stargate"].search([], limit=1).activity_schedule(
        ...
        self.assertEqual(
            sorted(
                field_name
                for field_name in res.json()["records"][0]
    ...
    def test_webjson_calendar(self):
        ...
        self.assertEqual(
            res.json()["length"],
            env["test_http.stargate"].search_count(
                [("last_use_date", ">=", last_date)]
            ),
        )
