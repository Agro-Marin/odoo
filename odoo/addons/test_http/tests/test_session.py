...
class TestHttpSession(TestHttpBase):
    ...
    def test_session09_logout(self):
        ...
        self.assertFalse(
            odoo.http.root.session_store.get(sid), "session should not exist"
        )
    ...
    def test_session10_explicit_session(self):
        ...
        with self.assertLogs("odoo.http.routing") as capture:
...
class TestSessionStore(HttpCaseWithUserDemo):
    def setUp(self):
        ...
        if os.getenv("ODOO_FAKETIME_TEST_MODE"):
            self.skipTest(
                "Those tests are not working in with faketime (filesystem times are used)"
            )
        ...
        self.enterContext(config.patch(data_dir=self.tmpdir.name))
