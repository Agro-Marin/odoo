...
class TestPrivateReadGroupingSets(common.TransactionCase):
    ...
    def test_many2many_read_grouping_sets(self):
        ...
        expected_result = [
            tasks._read_group(domain, groupby, aggregates) for groupby in grouping_sets
        ]
        ...
        expected_result = [
            ...
            for groupby, order in zip(grouping_sets, orders, strict=False)
        ]
        ...
        with self.assertQueries(
            ...
            self.assertEqual(
                tasks._read_grouping_sets(
                    domain, grouping_sets, aggregates, order=complete_order
                ),
                expected_result,
            )
        ...
        for i, case in enumerate(cases):
            ...
            with (
                ...
                self.assertQueryCount(nb_queries),
            ):
                result = tasks._read_grouping_sets(
                    domain, grouping_sets, aggregates, order=complete_order
                )
...
class TestFormattedReadGroupingSets(common.TransactionCase):
    ...
    def test_many2many_formatted_read_grouping_sets(self):
        ...
        for i, case in enumerate(cases):
            ...
            with (
                ...
                result = tasks.formatted_read_grouping_sets(
                    domain, grouping_sets, aggregates, order=complete_order
                )
                self.assertEqual(result, expected_result)
    ...
    def test_related(self):
        ...
        self.assertEqual(
            RelatedBase.formatted_read_grouping_sets(
                [], grouping_sets, ["__count", "name:array_agg"]
            ),
            [
                RelatedBase.formatted_read_group(
                    [], groupby, ["__count", "name:array_agg"]
                )
                for groupby in grouping_sets
            ],
        )
    ...
    def test_chain_inherited_fname(self):
        ...
        grouping_sets = [
            ["inherited_id.value"],
            ["inherited_id.base_id.name"],
            ["inherited_id.foo_id.name"],
            [],
        ]
        expected_result = [
