...
def test_a_freshly_minted_token_validates():
    req = _FakeRequest()
    assert req.is_valid_csrf(req.csrf_token()) is True
...
def test_token_survives_a_soft_rotation():
    ...
    assert req.is_valid_csrf(token) is True, (
...
def test_token_is_not_transferable_between_sessions():
    ...
    assert other.is_valid_csrf(token) is False


def test_token_from_a_different_secret_is_rejected():
    token = _FakeRequest(secret="secret-one").csrf_token()
    assert _FakeRequest(secret="secret-two").is_valid_csrf(token) is False
...
def test_a_positive_time_limit_is_still_honoured():
    req = _FakeRequest()
    assert req.is_valid_csrf(req.csrf_token(time_limit=60)) is True
...
def test_malformed_tokens_are_rejected_without_raising(bad):
    assert _FakeRequest().is_valid_csrf(bad) is False


def test_empty_hmac_with_a_future_timestamp_is_rejected():
    assert _FakeRequest().is_valid_csrf(str(int(time.time()) + 10_000)) is False
...
def test_missing_database_secret_raises_rather_than_allowing():
    ...
    with pytest.raises(ValueError):
        req.csrf_token()
    with pytest.raises(ValueError):
        req.is_valid_csrf("anything-non-empty")
