...
class RegistryError(RuntimeError):
    ...
    # Both attributes qualify the failure for the recovery path
    # (``Application._recover_from_registry_error``).
    # What the catalog said: ``True``, the database is confirmed gone;
    # ``False``, it exists; ``None``, the catalog itself could not be consulted
    # (PostgreSQL unreachable).
    db_absent: bool | None = None
    # Whether the underlying failure is a passing condition (connection loss,
    # pool starvation, a registry observed mid-build) rather than a durable one
    # (broken schema). A durable session logout is warranted only for a
    # confirmed-dropped database or a durably broken registry; logging every
    # user out over a passing blip forces a site-wide re-login. The class
    # defaults (``None``/``False``) give a foreign raiser upstream's
