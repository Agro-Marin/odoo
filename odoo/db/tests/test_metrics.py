...
from odoo.db.metrics import classify_query


class TestCategorizeQuery(unittest.TestCase):
    def test_select_from(self):
        qtype, table = classify_query("SELECT * FROM res_users")
        self.assertEqual(qtype, "from")
        self.assertEqual(table, "res_users")

    def test_insert_into(self):
        qtype, table = classify_query("INSERT INTO res_users (name) VALUES ('x')")
        self.assertEqual(qtype, "into")
        self.assertEqual(table, "res_users")

    def test_insert_select_prioritizes_into(self):
        qtype, table = classify_query("INSERT INTO t1 SELECT * FROM t2")
        self.assertEqual(qtype, "into")
        self.assertEqual(table, "t1")

    def test_update_is_a_write(self):
        qtype, table = classify_query("UPDATE res_users SET name='x'")
        self.assertEqual(qtype, "into")
        self.assertEqual(table, "res_users")

    def test_update_schema_qualified(self):
        qtype, table = classify_query('UPDATE "public"."res_users" SET name=1')
        self.assertEqual(qtype, "into")
        self.assertEqual(table, "res_users")

    def test_update_with_from_subquery(self):
        qtype, table = classify_query(
    ...
    def test_delete_is_a_write(self):
        qtype, table = classify_query("DELETE FROM res_users WHERE id = 1")
        self.assertEqual(qtype, "into")
        self.assertEqual(table, "res_users")
    ...
    def test_other(self):
        qtype, table = classify_query("COMMIT")
        self.assertEqual(qtype, "other")
        self.assertIsNone(table)

    def test_quoted_table_name(self):
        qtype, table = classify_query('SELECT * FROM "my_table" WHERE id = 1')
        self.assertEqual(qtype, "from")
    ...
    def test_multiline_query(self):
        qtype, table = classify_query("SELECT id\n  FROM res_partner\n WHERE active")
        self.assertEqual(qtype, "from")
...
class TestFromInsideAFunctionCall(unittest.TestCase):
    ...
    def test_a_from_clause_subquery_still_names_the_inner_table(self):
        self.assertEqual(
            classify_query("SELECT a FROM (SELECT b FROM inner_t) s"),
            ("from", "inner_t"),
    ...
    def test_delete_with_a_subquery_names_the_deleted_table(self):
        self.assertEqual(
            classify_query("DELETE FROM res_partner WHERE id IN (SELECT id FROM o)"),
            ("into", "res_partner"),
        )
...
class TestDataModifyingCte(unittest.TestCase):
    ...
    def test_a_read_only_cte_still_categorizes_by_the_main_statement(self):
        self.assertEqual(
            classify_query("WITH a AS (SELECT id FROM t1) SELECT * FROM a"),
            ("from", "a"),
        )
