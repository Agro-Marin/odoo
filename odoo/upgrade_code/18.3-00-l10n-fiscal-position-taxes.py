...
def upgrade(file_manager: FileManager) -> None:
        ...
        for row_nb, row in enumerate(csv_data, start=1):
            ...
            if row_nb == 1:
                ...
                if (
                    ...
                    and (
                        not has_country
                        or row.get("country_id") != f"base.{module_country}"
                    )
                ):
            ...
            if src_tax and dest_tax:
                existing_src_tax_fp = tax_positions[module_name][coa_name][src_tax][
                    "fp"
                ]
                ...
                if module_name not in MODS_WITH_DYNAMIC_DOMESTIC_FP:
                    tax_positions[module_name][coa_name][src_tax]["fp"].add(
                        domestic_fp_id
                    )
                ...
                existing_dest_tax_fp = tax_positions[module_name][coa_name][dest_tax][
                    "fp"
                ]
                ...
                tax_positions[module_name][coa_name][dest_tax]["fp"].add(
                    fiscal_position
                )
                ...
                if src_tax:
                    log.warning(
                        "%s tax in module %s is mapped to nothing",
                        src_tax,
                        module_name,
                    )
                writer.writerow(row)
    ...
    for i, file in enumerate(tax_data_files, start=1):
        ...
        if (
            ...
            log.warning(
                "Missing fiscal position file %s while processing %s",
                fpfile,
                module_name,
            )
        ...
        for tax_row in csv_data:
            ...
            if tax_id and is_primary_tax:
                tax_info = (
                    tax_positions[module_name][coa_name][tax_id] if tax_id else {}
                )
