...
class UpgradeDomainTransformer(ast.NodeTransformer):
    ...
    def visit_Call(self, node: ast.Call) -> ast.expr | str | datetime.time:
        ...
        match node.func, node.args, node.keywords:
            ...
            case (
                ast.Attribute(
                    value=ast.Attribute(value=ast.Name(id="datetime"), attr="datetime"),
                    attr="now",
                ),
                [],
                [],
            ...
            case (
                ast.Attribute(value=ast.Name(id="datetime"), attr="timedelta"),
                [],
                kws,
            ):
                value = self.parse_offset_keywords(kws)
            ...
            case (
                ast.Attribute(value=ast.Name(id="datetime"), attr="time"),
                args,
                [],
            ):
...
def upgrade(file_manager: FileManager) -> None:
    ...
    for file in file_manager:
        if not (
            file.path.parent.name in ("data", "report", "views")
        ...
        all_domains.extend(
            el.text for el in tree.findall(".//field[@name='domain_force']")
        )
        ...
        def replacement_attr(match: re.Match[str], replacements: dict = replacements) -> str:
            ...
            if not domain:
                return match[0]
            domain = (
                domain.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
            )
