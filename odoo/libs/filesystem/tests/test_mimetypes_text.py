...
class TestMultibyteAtTheCut(unittest.TestCase):
    def test_char_straddling_the_boundary(self):
        ...
        self.assertEqual(_guess_mimetype_by_signature(data), "text/plain")
...
class TestStillRejectsBinary(unittest.TestCase):
    def test_invalid_utf8_in_the_body(self):
        self.assertEqual(
            _guess_mimetype_by_signature(b"hello" + b"\xff\xfe" + b"world"),
            UNKNOWN_MIMETYPE,
        )

    def test_latin1_bytes(self):
        self.assertEqual(
            _guess_mimetype_by_signature("caf\xe9".encode("latin-1") * 300),
            UNKNOWN_MIMETYPE,
        )

    def test_nul_byte(self):
        self.assertEqual(
            _guess_mimetype_by_signature(b"text\x00text"), UNKNOWN_MIMETYPE
        )

    def test_control_characters(self):
        self.assertEqual(
            _guess_mimetype_by_signature(b"text\x07bell"), UNKNOWN_MIMETYPE
        )

    def test_empty(self):
        self.assertEqual(_guess_mimetype_by_signature(b""), UNKNOWN_MIMETYPE)

    def test_custom_default_is_honoured(self):
        self.assertEqual(
            _guess_mimetype_by_signature(b"\x00\x01", default="x/y"), "x/y"
        )


class TestPlainText(unittest.TestCase):
    def test_ascii(self):
        self.assertEqual(_guess_mimetype_by_signature(b"plain text"), "text/plain")

    def test_tabs_and_newlines_allowed(self):
        self.assertEqual(_guess_mimetype_by_signature(b"a\tb\r\nc\n"), "text/plain")

    def test_utf8_astral_plane(self):
        self.assertEqual(
            _guess_mimetype_by_signature("hi \N{PARTY POPPER} there".encode()),
            "text/plain",
        )

    def test_signature_detection_still_wins(self):
        self.assertEqual(
            _guess_mimetype_by_signature(b"%PDF-1.4 rest"), "application/pdf"
        )
...
class TestNoStateLeakBetweenCalls(unittest.TestCase):
    def test_truncated_then_binary(self):
        ...
        self.assertEqual(_guess_mimetype_by_signature(truncated), "text/plain")
        self.assertEqual(
            _guess_mimetype_by_signature(b"\xff\xfe\x00"), UNKNOWN_MIMETYPE
        )

    def test_repeated_truncated_calls_are_stable(self):
        data = ("a" * 1022 + "\N{EURO SIGN}" + "b" * 100).encode()
        self.assertEqual(
            [_guess_mimetype_by_signature(data) for _ in range(3)], ["text/plain"] * 3
        )
...
class TestBytearrayMatchesBytes(unittest.TestCase):
    ...
    def test_ooxml_survives_the_bytearray_branch(self):
        ...
        self.assertEqual(_guess_mimetype_by_signature(raw), expected)
...
class TestOlecfStreamNames(unittest.TestCase):
    ...
    def test_word_stream_away_from_the_first_sector(self):
        ...
        self.assertEqual(_guess_mimetype_by_signature(data), "application/msword")
