...
from .letters import (
    ...
    connects_with_letter_before,
    connects_with_letter_after,
)
...
GROUP_INDEX_TO_LIGATURE_FORMs = [
    ...
    (
        "\N{ARABIC LIGATURE LAM WITH ALEF ISOLATED FORM}",
        "",
        "",
        "\N{ARABIC LIGATURE LAM WITH ALEF FINAL FORM}",
    ),
    (
...
def reshape(text: str) -> str:
    ...
    for letter in text:
        if HARAKAT_RE.match(letter):
            ...
            if (
                previous_letter[FORM] == NOT_SUPPORTED
                or not connects_with_letter_before(letter, LETTERS_ARABIC)
                or not connects_with_letter_after(
                    previous_letter[LETTER], LETTERS_ARABIC
                )
                or (
                    previous_letter[FORM] == FINAL
                    and not connects_with_letters_before_and_after(
                        previous_letter[LETTER], LETTERS_ARABIC
                    )
                )
            ):
