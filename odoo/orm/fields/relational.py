...
from odoo.tools.misc import PENDING, SENTINEL, Sentinel, unquote

from ..domain import Domain
from ..primitives import COLLECTION_TYPES, SQL_OPERATORS, Command, NewId
from ..validation import check_pg_name
from .base import IR_MODELS, Field, _logger
...
class _Relational(Field["BaseModel"]):
    ...
    @override
    def __get__(
        self, records: BaseModel, owner: type[BaseModel] | None = None
    ) -> BaseModel:
...
class Many2one(_Relational):
    ...
    @override
    def __get__(
        self, record: BaseModel, owner: type[BaseModel] | None = None
    ) -> BaseModel:
        if record is None:
            return self
        env = record.env
    ...
    def convert_to_cache(
        ...
        if type(value) is int or type(value) is NewId:
            id_ = value
        elif hasattr(value, "_name") and hasattr(value, "_ids"):
    ...
    def convert_to_write(
        ...
        if (
            hasattr(value, "_name")
            and hasattr(value, "_ids")
            and value._name == self.comodel_name
        ):
            return value.id
...
class _RelationalMulti(_Relational):
    ...
    def convert_to_cache(
        ...
        if hasattr(value, "_name") and hasattr(value, "_ids"):
    ...
    def convert_to_record(
        ...
        if Comodel._active_name and self.context.get(
            "active_test", record.env.context.get("active_test", True)
        ):
    ...
    def convert_to_record_multi(
        ...
        corecords = object.__new__(Comodel)
        corecords.env = records.env
        corecords._ids = ids
        corecords._prefetch_ids = prefetch_ids
        if Comodel._active_name and self.context.get(
            "active_test", records.env.context.get("active_test", True)
        ):
            corecords = corecords.filtered(Comodel._active_name).with_prefetch(
                prefetch_ids
            )
                    ...
                    match command[0]:
                        case Command.CREATE:
                            for record in recs:
                                link(
                                    record,
                                    comodel.new(command[2], ref=command[1]),
                                )
                        case Command.UPDATE:
...
class Many2many(_RelationalMulti):
    ...
    def read(self, records: BaseModel) -> None:
        ...
        filter_access = (
            self.bypass_search_access
            and type(comodel)._search is not _BaseModel._search
        )
